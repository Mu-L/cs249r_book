...
class GPT2TrainingMem:
    ...
    accel_class_gb_str = fmt(80, precision=0, commas=False)
    grad_ckpt_overhead_pct_str = fmt(33, precision=0, commas=False)
```
...
- **Total model state: `{python} GPT2TrainingMem.model_state_gb_str` GB**—fits on a `{python} GPT2TrainingMem.accel_class_gb_str` GB-class A100/H100 (`{python} GPT2TrainingMem.accel_gb_str` GB in decimal units), but leaves only `{python} GPT2TrainingMem.remaining_gb_str` GB for activations.
