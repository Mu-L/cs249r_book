...
# │          BYTES_ADAM_STATE,
# │          Hardware.Cloud.A100.compute.peak_flops, Hardware.Cloud.A100.memory.bandwidth, ureg, byte, GB, MB,
...
class AlgorithmFoundations:
    ...
    small_intensity_str = fmt(int(small_intensity), precision=1, allow_zero=True, commas=False)
    small_efficiency_pct_str = fmt(small_efficiency_pct, precision=1, allow_zero=True, commas=False)
    a100_ridge_str = fmt(a100_ridge, precision=1, allow_zero=True, commas=False)
    sparsity_pct_str = fmt(sparsity_pct, precision=1, allow_zero=True, commas=False)
    reduction_factor_str = fmt(reduction_factor, precision=1, allow_zero=True, commas=False)
    vocab_size_str = fmt(vocab_size, precision=1, allow_zero=True)
    embed_dim_str = fmt(embed_dim, precision=1, allow_zero=True)
    total_elements_str = fmt(total_elements/BILLION, precision=1, allow_zero=True, commas=False)
    dense_gb_str = fmt(dense_gb, precision=1, allow_zero=True, commas=False)
    nonzeros_str = fmt(nonzeros/MILLION, precision=1, allow_zero=True, commas=False)
    sparse_mb_str = fmt(sparse_mb, precision=1, allow_zero=True, commas=False)

    bytes_fp16_str = fmt(int(BYTES_FP16.m_as(ureg.byte)), precision=1, allow_zero=True, commas=False)
    bytes_fp32_str = fmt(int(BYTES_FP32.m_as(ureg.byte)), precision=1, allow_zero=True, commas=False)
    bytes_int32_str = fmt(int(BYTES_INT32.m_as(ureg.byte)), precision=1, allow_zero=True, commas=False)
    csr_break_even_density_pct_str = fmt(csr_break_even_density_pct, precision=1, allow_zero=True, commas=False)
    coo_break_even_density_pct_str = fmt(coo_break_even_density_pct, precision=1, allow_zero=True, commas=False)
    ...
    gemm_intensity_eq = fmt_math(rf"\text{{Intensity}} = \frac{{O}}{{D_{{\text{{vol}}}}}} = \frac{{2n^3}}{{3n^2 \times {bytes_fp16_str}}} = \frac{{n}}{{3}} \text{{ FLOP/byte}}")
```
...
class GPT2TrainingMem:
    ...
    d_str = fmt(d, precision=1, allow_zero=True, commas=False)
    bf16_str = fmt(bf16, precision=1, allow_zero=True, commas=False)
    weights_gb_str = fmt(weights_gb, precision=1, commas=False)
    grads_gb_str = fmt(grads_gb, precision=1, commas=False)
    optim_bytes_str = fmt(optimizer_bytes_per_param, precision=1, allow_zero=True, commas=False)
    optim_gb_str = fmt(optim_gb, precision=1, commas=False)
    model_state_gb_str = fmt(model_state_gb, precision=1, allow_zero=True, commas=False)
    accel_gb_str = fmt(accel_gb, precision=1, commas=False)
    remaining_gb_str = fmt(remaining_gb, precision=1, allow_zero=True, commas=False)
    batch_small_str = fmt(batch_small, precision=1, allow_zero=True, commas=False)
    seq_len_str = fmt(seq_len, precision=1, allow_zero=True, commas=False)
    act_small_gb_str = fmt(act_small_gb, precision=1, allow_zero=True, commas=False)
    total_small_gb_str = fmt(total_small_gb, precision=1, allow_zero=True, commas=False, prefix="~")
    batch_large_str = fmt(batch_large, precision=1, allow_zero=True, commas=False)
    act_large_gb_str = fmt(act_large_gb, precision=1, allow_zero=True, commas=False, prefix="~")
```
