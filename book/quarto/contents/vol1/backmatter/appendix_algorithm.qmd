...
- **When GEMMs are slow**: Use @sec-algorithm-foundations-general-matrix-multiply-gemm-b55d and compare intensity to the hardware’s ridge point.
...
- **When tensor code “should work” but does not**: Use @sec-algorithm-foundations-shapes-strides-1aea and @sec-algorithm-foundations-broadcasting-ec16.
...
# │
# │ Goal: Compute sparse vs. dense storage trade-off and GEMM intensity
# │       examples for a vocabulary embedding matrix and small GEMM.
# │ Show: ~4 GB dense vs. ~80 MB sparse at 1% density; ridge point ~153;
# │       small 64×64 GEMM at ~14% efficiency.
# │ How: Scalar arithmetic using byte counts from pint constants (.m_as).
# │
# │ Imports: mlsysim.core.constants (BYTES_FP32, BYTES_FP16, BYTES_INT32,
...
# │          optimizer_overhead_str, csr_break_even_density_pct_str,
# │          coo_break_even_density_pct_str, gemm_intensity_eq
# └─────────────────────────────────────────────────────────────────────────────
...
class AlgorithmFoundations:
    ...
    small_intensity_str = fmt(int(small_intensity), precision=0, commas=False)
    small_efficiency_pct_str = fmt(small_efficiency_pct, precision=0, commas=False)
    a100_ridge_str = fmt(a100_ridge, precision=0, commas=False)
    sparsity_pct_str = fmt(sparsity_pct, precision=0, commas=False)
    ...
    bytes_fp16_str = fmt(int(BYTES_FP16.m_as(ureg.byte)), precision=0, commas=False)
    bytes_fp32_str = fmt(int(BYTES_FP32.m_as(ureg.byte)), precision=0, commas=False)
    bytes_int32_str = fmt(int(BYTES_INT32.m_as(ureg.byte)), precision=0, commas=False)
    csr_break_even_density_pct_str = fmt(csr_break_even_density_pct, precision=0, commas=False)
    coo_break_even_density_pct_str = fmt(coo_break_even_density_pct, precision=0, commas=False)
    optimizer_overhead_str = MarkdownStr("8–12")

    gemm_intensity_eq = fmt_math(rf"\text{{Intensity}} = \frac{{\text{{Ops}}}}{{\text{{Bytes}}}} = \frac{{2n^3}}{{3n^2 \times {bytes_fp16_str}}} = \frac{{n}}{{3}} \text{{ FLOP/byte}}")
```
...
Deep learning systems are, at their core, engines for transforming massive matrices. Frameworks like PyTorch abstract away the raw math, but performance engineering still depends on the underlying linear algebra. Building on how numbers are stored in @sec-machine-foundations, this section focuses on how they are manipulated.
...
- **Small matrices are inefficient**: A square GEMM with $n =$ `{python} AlgorithmFoundations.n_small_str` has intensity `{python} AlgorithmFoundations.n_small_str`/3 ≈ `{python} AlgorithmFoundations.small_intensity_str` FLOP/byte, well below the ridge point (`{python} AlgorithmFoundations.a100_ridge_str`), achieving only ~`{python} AlgorithmFoundations.small_efficiency_pct_str` percent of peak throughput.
...
CSR is essential for recommendation systems (sparse embedding tables) and pruned models. For a matrix with $N$ elements, $R$ rows, and $K$ nonzeros, CSR uses $O(K + R)$ storage instead of $O(N)$; when $K$ is large relative to $R$, this is often summarized as $O(K)$.

To see the trade-off concretely, consider a vocabulary embedding matrix with `{python} AlgorithmFoundations.vocab_size_str` rows and `{python} AlgorithmFoundations.embed_dim_str` columns (`{python} AlgorithmFoundations.total_elements_str` billion parameters):
...
| **LayerNorm**               | $(B, S, d_{\text{model}})$    |                                                          $2 \times d_{\text{model}}$ |                                                $O(B \times S \times d_{\text{model}})$ |
...
The dimension with size one is "stretched" to match the other, as illustrated in @fig-broadcasting-rules. This stretching is **virtual**: the data is not copied in memory. Instead, the stride for that dimension is set to 0, allowing the hardware to read the same value repeatedly with $O(1)$ memory overhead.
...
Modern frameworks use **reverse-mode automatic differentiation**, which computes gradients for all $N$ parameters in a single backward pass. The key insight is that starting from the output and working backward (reverse mode) requires one pass regardless of the number of parameters, whereas starting from each input and working forward (forward mode) would require $N$ passes—one per parameter. This is why a training step is a small constant multiple of inference, commonly about 2--3$\times$ a forward pass for dense networks, rather than $N$ passes.
...
*   **Activations**: The hidden giant. $O(\text{Batch} \times \text{Sequence} \times \text{Layers} \times \text{Width})$.
...
class GPT2TrainingMem:
    ...
    accel_gb = A100_MEM_CAPACITY.m_as(GB)
    batch_small = 8
    batch_large = 64
    seq_len = 1024
    ...
    grads_gb_str = fmt(grads_gb, precision=1, commas=False)
    optim_bytes_str = fmt(optimizer_bytes_per_param, precision=0, commas=False)
    optim_gb_str = fmt(optim_gb, precision=1, commas=False)
    model_state_gb_str = fmt(model_state_gb, precision=0, commas=False)
    accel_gb_str = fmt(accel_gb, precision=1, commas=False)
    remaining_gb_str = fmt(remaining_gb, precision=0, commas=False)
    batch_small_str = fmt(batch_small, precision=0, commas=False)
    seq_len_str = fmt(seq_len, precision=0, commas=False)
    act_small_gb_str = fmt(act_small_gb, precision=0, commas=False)
    total_small_gb_str = fmt(total_small_gb, precision=0, commas=False, prefix="~")
    batch_large_str = fmt(batch_large, precision=0, commas=False)
    act_large_gb_str = fmt(act_large_gb, precision=0, commas=False, prefix="~")
```
...
- **Total model state: `{python} GPT2TrainingMem.model_state_gb_str` GB**—fits on an 80 GB-class A100/H100 (`{python} GPT2TrainingMem.accel_gb_str` GB in decimal units), but leaves only `{python} GPT2TrainingMem.remaining_gb_str` GB for activations.
...
[^fn-grad-checkpoint]: **Gradient Checkpointing**: Trades compute for memory. Instead of storing all activations, only a subset (checkpoints) is kept, and the missing ones are recomputed during the backward pass. This reduces memory usage from $O(N)$ to $O(\sqrt{N})$ at the cost of ~33 percent more compute.
...
Sparse formats like CSR and COO add metadata overhead in the form of index arrays. With FP32 values and INT32 indices, CSR storage breaks even at roughly `{python} AlgorithmFoundations.csr_break_even_density_pct_str` percent density before row-pointer overhead, while COO breaks even around `{python} AlgorithmFoundations.coo_break_even_density_pct_str` percent density because it stores both row and column indices. A separate performance rule of thumb is stricter: sparsity usually needs to exceed 90–95 percent before generic sparse kernels outrun dense kernels, though specialized hardware patterns (like NVIDIA's 2:4 structured sparsity) change this calculus by encoding sparsity in fixed-ratio bitmasks rather than explicit indices.

**Fallacy**: *Training memory equals model size.*

As shown in @sec-algorithm-foundations-true-cost-training-memory-e54e, training memory includes gradients, optimizer states, and activations in addition to weights. For a model with $P$ parameters trained with Adam in mixed precision, the nonactivation overhead alone is roughly `{python} AlgorithmFoundations.bytes_fp16_str` + `{python} AlgorithmFoundations.bytes_fp16_str` + `{python} AlgorithmFoundations.optimizer_overhead_str` = 12–16 bytes per parameter—far more than the `{python} AlgorithmFoundations.bytes_fp16_str`-byte weight footprint that naive estimates assume.

**Pitfall**: *Ignoring tensor layout when optimizing performance.*

A matrix multiplication can run at 80 percent of peak throughput or 5 percent of peak depending entirely on whether the input tensors are contiguous in memory and aligned to the hardware's tile dimensions. Calling `.contiguous()` or transposing before a hot loop seems trivial, but neglecting it is one of the most common causes of unexplained slowdowns in custom model code.
...
- Sparse storage formats reduce memory once density falls below the metadata break-even point (roughly `{python} AlgorithmFoundations.csr_break_even_density_pct_str` percent density for CSR with FP32 values and INT32 indices), but sparse-kernel performance usually needs much higher sparsity, often 90–95 percent, because irregular index traversal costs throughput.
- Tensor shapes, strides, and broadcasting are the source of many subtle bugs: a noncontiguous tensor can silently degrade kernel performance by orders of magnitude, and an incorrect broadcast can produce a result of the wrong shape without raising an error.
...
- Backpropagation's efficiency comes from computing all parameter gradients in a single backward pass, but it requires caching forward-pass activations, creating a fundamental memory–compute trade-off that gradient checkpointing partially resolves.
