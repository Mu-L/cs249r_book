...
*   **Activations**: The hidden giant. $\mathcal{O}(\text{Batch} \times \text{Sequence} \times \text{Layers} \times \text{Width})$.
...
[^fn-grad-checkpoint]: **Gradient Checkpointing**: Trades compute for memory. Instead of storing all activations, only a subset (checkpoints) is kept, and the missing ones are recomputed during the backward pass. This reduces memory usage from $\mathcal{O}(N)$ to $\mathcal{O}(\sqrt{N})$ at the cost of ~33 percent more compute.
