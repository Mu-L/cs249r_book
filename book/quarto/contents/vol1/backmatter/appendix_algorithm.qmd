...
class AlgorithmFoundations:
    ...
    small_efficiency_pct_str = fmt(int(small_efficiency_pct), precision=0, commas=False)
...
The diagram lays out a simple two-layer network with both passes annotated: a roadmap for tracing through each step below.
...
# │                           BYTES_FP16, BYTES_FP32, GB, GiB, byte, ureg, param)
...
class GPT2TrainingMem:
    ...
    accel_gb = A100_MEM_CAPACITY.m_as(GiB)
    batch_small = 8
    batch_large = 64
    seq_len = 1024
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    P_str = fmt(sci_latex(P, precision=1), precision=0, commas=False, prefix="$", suffix="$")  # LaTeX-safe: 1.5 \times 10^{9} (avoids 1.5e+09)
...
- Sparse storage formats only reduce memory when sparsity exceeds roughly 90–95 percent, because index metadata consumes space proportional to the number of nonzeros.
