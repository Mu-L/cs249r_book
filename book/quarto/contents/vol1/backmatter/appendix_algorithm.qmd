...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-algorithm-foundations-general-matrix-multiply-gemm-b55d
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-algorithm-foundations-sparse-matrix-formats-0bd0 embedding example
...
*   **Dense (FP32)**: `{python} SparseEmbeddingExample.total_elements_str` $\times$ $10^{9}$ $\times$ `{python} SparseEmbeddingExample.bytes_fp32_str` bytes = **`{python} SparseEmbeddingExample.dense_gb_str` GB**.
*   **Sparse (`{python} SparseEmbeddingExample.sparsity_pct_str` percent density)**: Storing only nonzeros requires roughly `{python} SparseEmbeddingExample.nonzeros_str` $\times$ $10^{6}$ $\times$ (`{python} SparseEmbeddingExample.bytes_fp32_str` bytes value + `{python} SparseEmbeddingExample.bytes_int32_str` bytes index) ≈ **`{python} SparseEmbeddingExample.sparse_mb_str` MB**.
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-algorithm-foundations-true-cost-training-memory-e54e
...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Fallacies and Summary — sparse format break-even densities
...
```{python}
#| echo: false
#| label: training-memory-recap
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Fallacies — training memory ≠ model size
# │ Exports: TrainingMemoryRecap.*_str
from mlsysim.fmt import fmt_int, fmt, MarkdownStr

class TrainingMemoryRecap:
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bytes_fp16_str = fmt_int(BYTES_FP16.m_as(ureg.byte), commas=False)
    optimizer_overhead_min = int(BYTES_ADAM_STATE.m_as(ureg.byte))
    optimizer_overhead_max = optimizer_overhead_min + int(BYTES_FP32.m_as(ureg.byte))
...
Sparse formats like CSR and COO add metadata overhead in the form of index arrays. With FP32 values and INT32 indices, CSR storage breaks even at roughly `{python} SparseBreakEvenRecap.csr_break_even_density_pct_str` percent density before row-pointer overhead, while COO breaks even around `{python} SparseBreakEvenRecap.coo_break_even_density_pct_str` percent density because it stores both row and column indices. A separate performance rule of thumb is stricter: sparsity usually needs to exceed 90–95 percent before generic sparse kernels outrun dense kernels, though specialized hardware patterns (like NVIDIA's 2:4 structured sparsity) change this calculus by encoding sparsity in fixed-ratio bitmasks rather than explicit indices.

**Fallacy**: *Training memory equals model size.*

As shown in @sec-algorithm-foundations-true-cost-training-memory-e54e, training memory includes gradients, optimizer states, and activations in addition to weights. For a model with $P$ parameters trained with Adam in mixed precision, the nonactivation overhead alone is roughly `{python} TrainingMemoryRecap.bytes_fp16_str` + `{python} TrainingMemoryRecap.bytes_fp16_str` + `{python} TrainingMemoryRecap.optimizer_overhead_str` = 12–16 bytes per parameter—far more than the `{python} TrainingMemoryRecap.bytes_fp16_str`-byte weight footprint that naive estimates assume.
