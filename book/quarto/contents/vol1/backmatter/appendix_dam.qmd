...
class DAMTaxonomy:
    ...
    ex1_gpu_util_str = fmt(ex1_gpu_util_pct, precision=1, commas=False)
    ex1_disk_sat_str = fmt(ex1_disk_sat_pct, precision=1, commas=False)

    ex2_params_str = fmt(ex2_params.m_as(param) / BILLION, precision=1, commas=False, suffix="B")
    ex2_flops_per_pass_str = fmt(ex2_flops_per_pass, precision=3, commas=False)
    ex2_latency_ms_str = fmt(int(ex2_latency_s * 1000), precision=1, commas=False)
    ...
    h100_fp16_tflops_str = fmt(h100_fp16_tflops_peak, precision=1, commas=False)
    ex2_model_size_gb_str = fmt(ex2_model_size_gb, precision=1, commas=False)
    ...
    ex3_params_start_str = fmt(ex3_params_start / MILLION, precision=1, commas=False, suffix="M")
    ex3_params_end_str = fmt(ex3_params_end / BILLION, precision=1, commas=False, suffix="B")
    ex3_scale_factor_str = fmt(ex3_scale_factor, precision=1, commas=False)
    ...
    ex3_scaling_baseline_pred_str = fmt(ex3_scaling_baseline_pred_pct, precision=1, commas=False)

    # Mixed prose + math symbol → MarkdownStr (keeps _str semantics; bypasses
    # Quarto's auto-escape on the embedded $\times$).
    ex4_gpu_old_str = fmt(ex4_gpu_old_n, precision=1, commas=False, suffix="$\\times$ A100")
    ex4_gpu_new_str = fmt(ex4_gpu_new_n, precision=1, commas=False, suffix="$\\times$ H100")
    # Custom currency prefix + K suffix → MarkdownStr escape-hatch
    ex4_cost_str = fmt(ex4_cost_k, precision=1, commas=False, prefix="$", suffix="K")
```
