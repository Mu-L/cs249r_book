...
#| label: appendix-dam-exercises
from mlsysim.physics import calc_transformer_decode_flops
from mlsysim.fmt import fmt_int, fmt, check, MarkdownStr

# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-dam-taxonomy-exercises-135b — D·A·M diagnostic exercises
# │ Goal: GPU starvation, iron-law decode, scaling-law, hardware-upgrade scenarios
# │ Exports: ex1_*, ex2_*, ex3_*, ex4_* formatted strings and equations
class DAMTaxonomy:
    """Namespace for D·A·M Taxonomy diagnostic exercises."""
    ...
    ex2_latency_s = 0.050
    h100_fp16_tflops_peak = h.peak_flops.m_as(TFLOPs/second)

    ex3_params_start = 125 * MILLION
    ex3_params_end = 1 * BILLION
    ex3_loss_start = 0.45
    ex3_loss_end = 0.42
    ex3_scaling_baseline_pred_pct = 15
    ...
    ex3_scale_factor = ex3_params_end / ex3_params_start
    ...
    check(ex3_scale_factor == 8, f"Exercise 3 scale factor ({ex3_scale_factor}) should be 8x.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ex1_gpu_util_str = fmt(ex1_gpu_util_pct, precision=0, commas=False, suffix=' percent')
    ex1_disk_sat_str = fmt(ex1_disk_sat_pct, precision=0, commas=False, suffix=' percent')

    ex2_params_str = fmt(ex2_params.m_as(param) / BILLION, precision=0, commas=False, suffix="B")
    ex2_flops_per_pass_str = fmt(ex2_flops_per_pass, precision=3, commas=False)
    ex2_latency_ms_str = fmt_int(ex2_latency_s * THOUSAND, commas=False, suffix=' ms')
    ex2_latency_s_str = fmt(ex2_latency_s, precision=3, commas=False)
    ex2_achieved_str = fmt(ex2_achieved_tflops, precision=2, commas=False, suffix=' TFLOP/s')
    ex2_util_str = fmt(ex2_util, precision=2, commas=False, suffix=' percent')
    h100_fp16_tflops_str = fmt(h100_fp16_tflops_peak, precision=0, commas=False, suffix=' TFLOP/s')
    ex2_model_size_gb_str = fmt(ex2_model_size_gb, precision=0, commas=False, suffix=' GB')

    ex2_achieved_eq = MarkdownStr(
        f"$$\\text{{Achieved FLOP/s}} = \\frac{{{ex2_flops_per_pass_str} \\text{{ TFLOPs}}}}"
        f"{{{ex2_latency_s_str} \\text{{ s}}}} = {ex2_achieved_str} \\text{{ TFLOP/s}}$$"
    )
    ex2_util_eq = MarkdownStr(
        f"$$\\eta_{{\\text{{hw}}}} = \\frac{{{ex2_achieved_str}}}{{{h100_fp16_tflops_str}}} \\approx {ex2_util_str}\\%$$"
    )

    ex3_params_start_str = fmt(ex3_params_start / MILLION, precision=0, commas=False, suffix="M")
    ex3_params_end_str = fmt(ex3_params_end / BILLION, precision=0, commas=False, suffix="B")
    ex3_scale_factor_str = fmt(ex3_scale_factor, precision=0, commas=False)
    ex3_loss_start_str = fmt(ex3_loss_start, precision=2, commas=False)
    ex3_loss_end_str = fmt(ex3_loss_end, precision=2, commas=False)
    ex3_imp_str = fmt(ex3_imp_pct, precision=1, commas=False, suffix=' percent')
    ex3_scaling_baseline_pred_str = fmt(ex3_scaling_baseline_pred_pct, precision=0, commas=False, suffix=' percent')

    ex4_gpu_old_str = fmt(ex4_gpu_old_n, precision=0, commas=False, suffix="$\\times$ A100")
    ex4_gpu_new_str = fmt(ex4_gpu_new_n, precision=0, commas=False, suffix="$\\times$ H100")
    ex4_cost_str = fmt(ex4_cost_k, precision=0, commas=False, prefix="$", suffix="K")
```

### Exercise one: Component identification {.unnumbered}

A production image classification service runs on an A100 GPU. The `nvidia-smi` output shows `{python} DAMTaxonomy.ex1_gpu_util_str` GPU utilization, while `iotop` reveals the disk is saturated at `{python} DAMTaxonomy.ex1_disk_sat_str`. Which D·A·M axis is the bottleneck? What are two specific optimizations you would recommend? <!-- lego-ok: GPU is part of "GPU utilization", not a unit -->
...
Consider a decoder-only transformer model with `{python} DAMTaxonomy.ex2_params_str` parameters generating one token in the autoregressive decode phase at batch size 1. Using the 2 FLOPs-per-parameter rule of thumb, the decode step requires `{python} DAMTaxonomy.ex2_flops_per_pass_str` TFLOP. On an H100 GPU (`{python} DAMTaxonomy.h100_fp16_tflops_str` peak FP16 Tensor Core throughput), the measured latency is `{python} DAMTaxonomy.ex2_latency_ms_str`. Calculate the achieved utilization ($\eta_{\text{hw}}$). Is this system Data-bound, Algorithm-bound, or Machine-bound? Justify your answer.
...
Computed values: Achieved = `{python} DAMTaxonomy.ex2_achieved_str`, Utilization = `{python} DAMTaxonomy.ex2_util_str`.

These numbers prove **low Machine utilization**, but they do not by themselves prove a saturated HBM path. At batch size 1, each layer performs a matrix-vector multiply (GEMV) rather than a general matrix multiply (GEMM). The model's `{python} DAMTaxonomy.ex2_params_str` parameters (~`{python} DAMTaxonomy.ex2_model_size_gb_str` in FP16) must be loaded for every forward pass, so the workload has low arithmetic intensity and is a strong **Data-bottleneck candidate**. To classify it as memory-bound rather than overhead/latency-bound, confirm with profiling evidence such as near-peak HBM bandwidth or most time attributed to weight reads.
...
A team has been training a sentiment analysis model. After scaling from `{python} DAMTaxonomy.ex3_params_start_str` to `{python} DAMTaxonomy.ex3_params_end_str` parameters (`{python} DAMTaxonomy.ex3_scale_factor_str`$\times$ increase), validation loss improved from `{python} DAMTaxonomy.ex3_loss_start_str` to `{python} DAMTaxonomy.ex3_loss_end_str` (`{python} DAMTaxonomy.ex3_imp_str` improvement). Assume a scaling-law baseline predicts a ~`{python} DAMTaxonomy.ex3_scaling_baseline_pred_str` improvement for this scaling intervention. What does this discrepancy suggest? Which D·A·M axis should be investigated first, and why?

**Answer**: The discrepancy points to the **Information Roofline**---the ceiling imposed by data quality rather than model capacity. Scaling laws predict the *slope* of improvement assuming sufficient information in the data. When actual improvement (`{python} DAMTaxonomy.ex3_imp_str`) falls well short of the assumed baseline (`{python} DAMTaxonomy.ex3_scaling_baseline_pred_str`), the model has likely extracted most learnable signal from the training distribution.
...
A colleague proposes upgrading from `{python} DAMTaxonomy.ex4_gpu_old_str` to `{python} DAMTaxonomy.ex4_gpu_new_str` because training is "too slow." Before approving the `{python} DAMTaxonomy.ex4_cost_str` hardware purchase, what three diagnostic questions would you ask? Map each question to the D·A·M axis it investigates.
