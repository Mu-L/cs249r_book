...
# │          NETWORK_10G_BW, NETWORK_100G_BW, time constants)
...
class DataGravity:
    ...
    bw_10g = NETWORK_10G_BW.m_as('bit/second')
    bw_100g = NETWORK_100G_BW.m_as('bit/second')
    truck_load_hours = 8
...
class SerializationCost:
    ...
    csv_speed_str = fmt(csv_speed_mb, precision=1, allow_zero=True, commas=False, prefix="~")
    parquet_speed_str = fmt(parquet_speed_mb, precision=1, allow_zero=True, prefix="> ")
    csv_cycles_str = MarkdownStr("High")
    parquet_cycles_str = MarkdownStr("Low")
    proto_speed_str = fmt(proto_speed_mb, precision=1, allow_zero=True, commas=False, prefix="~")
...
class DataAlgebra:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    row_size_kb_str = fmt(row_size_kb, precision=1, allow_zero=True, commas=False)
    int_size_bytes_str = fmt(int_size_bytes, precision=1, allow_zero=True, commas=False)
    waste_pct_str = fmt(waste_pct, precision=1, commas=False)
    join_table_tb_str = fmt(join_table_tb, precision=1, allow_zero=True, commas=False)
    # Custom "~" prefix → MarkdownStr per recipe Rule 1 escape-hatch
    join_network_tb_str = fmt(join_network_tb, precision=1, allow_zero=True, commas=False, prefix="~")
```
...
class LongTailScale:
    ...
    n_unhappy_str = fmt(n_unhappy, precision=1, allow_zero=True)
```
...
Computed: $\Pr(\text{Slow})$ = `{python} TailLatency.p_slow_pct_str` percent.
...
class DriftThresholds:
    ...
    logit_val_str = fmt(logit_val, precision=1, allow_zero=True, commas=False)
```
...
class LogSumExp:
    ...
    a_str = fmt(a, precision=1, allow_zero=True, commas=False)
