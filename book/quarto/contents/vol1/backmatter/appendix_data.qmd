...
class SerializationCost:
    ...
    csv_cycles_str = "High"
    parquet_cycles_str = "Low"
    proto_speed_str = f"~{proto_speed_mb}"
    proto_cycles_str = "Medium"
```
...
class DriftThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    psi_threshold_str = f"{psi_threshold:.1f}"
...
class KLDrift:
    ...
    kl_qp_calc = md_math(
        ...
        f"+ {Q_str[1]} \\times ({-lr[1]:.3f}) "
        f"+ {Q_str[2]} \\times ({-lr[2]:.3f}) "
...
class LogSumExp:
    ...
    z_str = md_math(f"\\lbrack {z[0]}, {z[1]}, {z[2]} \\rbrack")
    a_str = f"{a}"
    ...
    naive0_str = md_math(_sci(naive_exp[0]))
    naive1_str = md_math(_sci(naive_exp[1]))
    naive2_str = md_math(_sci(naive_exp[2]))

    # Shifted logits (use \lbrack/\rbrack for LaTeX math); md_math wraps in $...$
    shifted_str = md_math(f"\\lbrack {shifted[0]}, {shifted[1]}, {shifted[2]} \\rbrack")
    ...
    naive_exp0_expr = md_math(f"\\exp({z[0]}) \\approx {_sci(naive_exp[0])}")
    naive_exp1_expr = md_math(f"\\exp({z[1]}) \\approx {_sci(naive_exp[1])}")
    naive_exp2_expr = md_math(f"\\exp({z[2]}) \\approx {_sci(naive_exp[2])}")

    stable_exp0_expr = md_math(f"\\exp({shifted[0]}) \\approx {stable_exp[0]:.3f}")
    stable_exp1_expr = md_math(f"\\exp({shifted[1]}) \\approx {stable_exp[1]:.3f}")
    stable_exp2_expr = md_math(f"\\exp({shifted[2]}) = {stable_exp[2]:.1f}")

    log_sum_expr = md_math(f"+ \\log({stable_sum:.3f}) = {lse:.3f}")
...
**The setup**: A three-class classifier outputs logits $z =$ `{python} LogSumExp.z_str`.

**Without the trick (naive softmax)**:

`{python} LogSumExp.naive_exp0_expr`, `{python} LogSumExp.naive_exp1_expr`, `{python} LogSumExp.naive_exp2_expr`
...
`{python} LogSumExp.stable_exp0_expr`, `{python} LogSumExp.stable_exp1_expr`, `{python} LogSumExp.stable_exp2_expr`

Sum = `{python} LogSumExp.sum_str`. LogSumExp = `{python} LogSumExp.a_str` `{python} LogSumExp.log_sum_expr`.

Softmax: `{python} LogSumExp.sm_expr`
...
These pitfalls share a common root: treating data as a solved problem rather than a continuous engineering discipline. The summary distills the core principles that guard against each one.
