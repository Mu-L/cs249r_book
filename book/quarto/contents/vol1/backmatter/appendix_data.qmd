...
- **When data will not move**: Start with @tbl-data-gravity and the transfer-time equation in @sec-data-foundations-data-engineering-foundations-0c20.
...
# │
# │ Imports: mlsysim.core.constants (TB, PB, Gbps, byte, BITS_PER_BYTE, time constants)
...
class DataGravity:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    # Custom unit suffix ("Minutes"/"Hours"/"Days"/"Months") → MarkdownStr per recipe Rule 1 escape-hatch
...
: **The Cost of Inertia**: Why we "ship compute to data" rather than "ship data to compute." At 1 PB, the network is often not a viable option. {#tbl-data-gravity tbl-colwidths="[23,20,20,20,17]"}
...
class SerializationCost:
    ...
    csv_speed_str = fmt(csv_speed_mb, precision=0, commas=False, prefix="~")
    parquet_speed_str = fmt(parquet_speed_mb, precision=0, prefix="> ")
    csv_cycles_str = MarkdownStr("High")
    parquet_cycles_str = MarkdownStr("Low")
    proto_speed_str = fmt(proto_speed_mb, precision=0, commas=False, prefix="~")
...
|:------------------|----------------------------------------------------:|------------------------------------------------:|:--------------------|
| **CSV/JSON**      |     `{python} SerializationCost.csv_speed_str` MB/s |     `{python} SerializationCost.csv_cycles_str` | Debugging only      |
| **Protobuf**      |   `{python} SerializationCost.proto_speed_str` MB/s |   `{python} SerializationCost.proto_cycles_str` | RPC/Messages        |
| **Parquet/Arrow** | `{python} SerializationCost.parquet_speed_str` MB/s | `{python} SerializationCost.parquet_cycles_str` | High-Scale Training |

: **Serialization Overhead**: Zero-copy formats like Arrow are 10--100$\times$ faster than row-based text formats because they align directly with internal memory structures. {#tbl-serialization-cost tbl-colwidths="[25,25,25,25]"}
...
class DataAlgebra:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    row_size_kb_str = fmt(row_size_kb, precision=0, commas=False)
    int_size_bytes_str = fmt(int_size_bytes, precision=0, commas=False)
    waste_pct_str = fmt(waste_pct, precision=1, commas=False)
    join_table_tb_str = fmt(join_table_tb, precision=0, commas=False)
    # Custom "~" prefix → MarkdownStr per recipe Rule 1 escape-hatch
    join_network_tb_str = fmt(join_network_tb, precision=0, commas=False, prefix="~")
```
...
2.  **Projection ($\pi$)**: Selecting columns (for example, `SELECT age`).
    *   *Cost*: Free in columnar formats (only read relevant blocks). In row formats, the entire `{python} DataAlgebra.row_size_kb_str` KB row must be read just to extract the `{python} DataAlgebra.int_size_bytes_str`-byte integer, wasting `{python} DataAlgebra.waste_pct_str` percent of I/O bandwidth.
...
# │
# │ Imports: mlsysim.book (fmt_percent, fmt, md_math)
...
Computed: P(Slow) = `{python} TailLatency.p_slow_pct_str` percent.
...
Detecting drift between serving data ($Q$) and training data ($P$) requires measuring the "distance" between their distributions.

KL divergence[^fn-kl] measures how much information is lost if we approximate $P$ with $Q$ [@kullback1951]:
...
# │
# │ Imports: math (log), mlsysim.book (fmt, check, md_math)
...
class KLDrift:
    ...
    P_fmt = [f"{p:.2f}" for p in P]
    Q_fmt = [f"{q:.2f}" for q in Q]
    lr_fmt = [f"{l:.3f}" for l in lr]

    kl_pq_t_fmt = [f"{t:.3f}" for t in kl_pq_t]
    kl_qp_t_fmt = [f"{t:.3f}" for t in kl_qp_t]
    diff_fmt = [f"{d:.2f}" for d in diffs]
    psi_t_fmt = [f"{t:.3f}" for t in psi_t]

    # Scalar prose exports (canonical fmt for the .3f case)
    kl_pq_str = fmt(kl_pq, precision=3, commas=False)
    kl_qp_str = fmt(kl_qp, precision=3, commas=False)
    psi_str = fmt(psi, precision=3, commas=False)
    ...
    scenario_str = MarkdownStr(
        f"{P_pct[0]} percent of reviews were positive, {P_pct[1]} percent negative, "
        f"and {P_pct[2]} percent neutral. After deployment, the serving distribution "
        f"shifts to {Q_pct[0]} percent positive, {Q_pct[1]} percent negative, "
        f"and {Q_pct[2]} percent neutral"
    )
    # Custom bracketed-vector format → MarkdownStr per recipe Rule 1 escape-hatch
    P_vec_str = MarkdownStr(f"[{P_fmt[0]}, {P_fmt[1]}, {P_fmt[2]}]")
    Q_vec_str = MarkdownStr(f"[{Q_fmt[0]}, {Q_fmt[1]}, {Q_fmt[2]}]")

    # Renamed from kl_pq_formula → kl_pq_eq (canonical _eq suffix for md_math expression)
    kl_pq_eq = md_math(
        f"D_{{KL}}(P || Q) = {P_fmt[0]} \\log\\frac{{{P_fmt[0]}}}{{{Q_fmt[0]}}} "
        f"+ {P_fmt[1]} \\log\\frac{{{P_fmt[1]}}}{{{Q_fmt[1]}}} "
        f"+ {P_fmt[2]} \\log\\frac{{{P_fmt[2]}}}{{{Q_fmt[2]}}}"
    )
    # Renamed from kl_pq_calc → kl_pq_calc_eq
    kl_pq_calc_eq = md_math(
        f"= {P_fmt[0]} \\times {lr_fmt[0]} "
        f"+ {P_fmt[1]} \\times ({lr_fmt[1]}) "
        f"+ {P_fmt[2]} \\times ({lr_fmt[2]}) "
        f"= {kl_pq_t_fmt[0]} + ({kl_pq_t_fmt[1]}) + ({kl_pq_t_fmt[2]}) "
        f"= {kl_pq_str}"
    )
    # Renamed from kl_qp_formula → kl_qp_eq
    kl_qp_eq = md_math(
        f"D_{{KL}}(Q || P) = {Q_fmt[0]} \\log\\frac{{{Q_fmt[0]}}}{{{P_fmt[0]}}} "
        f"+ {Q_fmt[1]} \\log\\frac{{{Q_fmt[1]}}}{{{P_fmt[1]}}} "
        f"+ {Q_fmt[2]} \\log\\frac{{{Q_fmt[2]}}}{{{P_fmt[2]}}}"
    )
    # Renamed from kl_qp_calc → kl_qp_calc_eq
    kl_qp_calc_eq = md_math(
        f"= {Q_fmt[0]} \\times ({-lr[0]:.3f}) "
        f"+ {Q_fmt[1]} \\times ({-lr[1]:.3f}) "
        f"+ {Q_fmt[2]} \\times ({-lr[2]:.3f}) "
...
Notice the asymmetry: $D_{\text{KL}}(P||Q) \neq D_{\text{KL}}(Q||P)$. The Population Stability Index (PSI) symmetrizes this:
...
Because KL divergence is asymmetric ($D_{\text{KL}}(P||Q) \neq D_{\text{KL}}(Q||P)$), practitioners often use PSI, a symmetric metric derived from KL divergence that is easier to threshold. A PSI > `{python} DriftThresholds.psi_threshold_str` typically triggers an alert for retraining.

[^fn-kl]: **KL Divergence**: Named after Solomon Kullback and Richard Leibler, who introduced it in 1951. Also called *relative entropy*, it quantifies the expected extra information needed to encode samples from $P$ using a code optimized for $Q$. With natural logarithms, as in the example above, the unit is nats; with base-2 logarithms, the unit is bits. In production ML, KL divergence is the theoretical backbone of many drift-detection metrics.
...
**Entropy ($H$)**[^fn-entropy] is the average information content (uncertainty) in a distribution, defined as $H(X) = -\sum p(x) \log p(x)$. The log base sets the unit: natural logs give nats, while $\log_2$ gives bits. A uniform distribution has maximum entropy (maximum uncertainty). This connects directly to KL divergence above: $D_{\text{KL}}$ measures excess information needed when using the wrong distribution.
...
The problem is that if $z_i$ is large (for example, `{python} DriftThresholds.logit_val_str`), the exponential $e^{z_i}$ overflows common training and inference formats such as FP32, BF16, and FP16. FP64 can represent this specific value, but production ML kernels rarely use FP64 for softmax. The solution is to compute in log-space: the "Log-Sum-Exp" trick allows us to compute $\log(\sum e^{z_j})$ without ever calculating the massive exponentials directly, preserving numerical precision.[^fn-logsumexp]
...
The following example shows why this trick matters in practice, even for large but realistic logit values:
...
# │
# │ Imports: math (exp, log, log10, floor), mlsysim.book (fmt, check, md_math)
...
class LogSumExp:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    # Use \lbrack/\rbrack for LaTeX math; md_math wraps in $...$ so prose needs no extra delimiters
    # Renamed from z_str → z_math (canonical _math suffix for md_math result)
    z_math = md_math(f"\\lbrack {z[0]}, {z[1]}, {z[2]} \\rbrack")
    ...
    _naive0 = md_math(_sci(naive_exp[0]))
    _naive1 = md_math(_sci(naive_exp[1]))
    _naive2 = md_math(_sci(naive_exp[2]))

    # Shifted logits (use \lbrack/\rbrack for LaTeX math); md_math wraps in $...$
    # Renamed from shifted_str → shifted_math (canonical _math suffix for md_math result)
    shifted_math = md_math(f"\\lbrack {shifted[0]}, {shifted[1]}, {shifted[2]} \\rbrack")
    ...
    # Renamed *_expr → *_eq (canonical _eq suffix for md_math equation results)
    naive_exp0_eq = md_math(f"\\exp({z[0]}) \\approx {_sci(naive_exp[0])}")
    naive_exp1_eq = md_math(f"\\exp({z[1]}) \\approx {_sci(naive_exp[1])}")
    naive_exp2_eq = md_math(f"\\exp({z[2]}) \\approx {_sci(naive_exp[2])}")

    stable_exp0_eq = md_math(f"\\exp({shifted[0]}) \\approx {stable_exp[0]:.3f}")
    stable_exp1_eq = md_math(f"\\exp({shifted[1]}) \\approx {stable_exp[1]:.3f}")
    stable_exp2_eq = md_math(f"\\exp({shifted[2]}) = {stable_exp[2]:.1f}")

    log_sum_eq = md_math(f"+ \\log({stable_sum:.3f}) = {lse:.3f}")
...
These pitfalls share a common root: treating data as a solved problem rather than a continuous engineering discipline. The following summary distills the core principles that guard against each one.
...
- Drift detection requires comparing full distributions, not summary statistics. KL divergence and the symmetric PSI/Jeffreys divergence provide principled measures of distribution shift that catch problems before accuracy metrics react.
