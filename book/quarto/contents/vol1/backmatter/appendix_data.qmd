...
Computed: $P(\text{Slow})$ = `{python} TailLatency.p_slow_pct_str` percent.
...
$$ \mathcal{D}_{\text{KL}}(P \lVert Q) = \sum_x P(x) \log \frac{P(x)}{Q(x)} $$
...
class KLDrift:
    ...
    kl_pq_eq = fmt_math(
        f"\\mathcal{{D}}_{{\\text{{KL}}}}(P \\lVert Q) = {P_fmt[0]} \\log\\frac{{{P_fmt[0]}}}{{{Q_fmt[0]}}} "
    ...
    kl_qp_eq = fmt_math(
        f"\\mathcal{{D}}_{{\\text{{KL}}}}(Q \\lVert P) = {Q_fmt[0]} \\log\\frac{{{Q_fmt[0]}}}{{{P_fmt[0]}}} "
...
Notice the asymmetry: $\mathcal{D}_{\text{KL}}(P \lVert Q) \neq \mathcal{D}_{\text{KL}}(Q \lVert P)$. The Population Stability Index (PSI) symmetrizes this:
...
Because KL divergence is asymmetric ($\mathcal{D}_{\text{KL}}(P \lVert Q) \neq \mathcal{D}_{\text{KL}}(Q \lVert P)$), practitioners often use PSI, a symmetric metric derived from KL divergence that is easier to threshold. A PSI > `{python} DriftThresholds.psi_threshold_str` typically triggers an alert for retraining.
