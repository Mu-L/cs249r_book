...
The transfer time for moving a dataset is simply $T = D_{\text{vol}} / \text{BW}$ (for the large volumes here, latency is negligible; the full equation appears in @sec-machine-foundations-bandwidth-vs-latency-5320). @tbl-data-gravity illustrates the sobering reality:
...
$$ D_{\text{KL}}(P \lVert Q) = \sum P(x) \log \frac{P(x)}{Q(x)} $$
...
# │
# │ Goal:    Compute KL(P lVert Q), KL(Q lVert P), and PSI for a worked drift example.
...
class KLDrift:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(kl_pq > 0, "KL(P lVert Q) must be positive")
    check(kl_qp > 0, "KL(Q lVert P) must be positive")
    ...
    kl_pq_eq = fmt_math(
        f"D_{{\\text{{KL}}}}(P \\lVert Q) = {P_fmt[0]} \\log\\frac{{{P_fmt[0]}}}{{{Q_fmt[0]}}} "
    ...
    kl_qp_eq = fmt_math(
        f"D_{{\\text{{KL}}}}(Q \\lVert P) = {Q_fmt[0]} \\log\\frac{{{Q_fmt[0]}}}{{{P_fmt[0]}}} "
...
Notice the asymmetry: $D_{\text{KL}}(P \lVert Q) \neq D_{\text{KL}}(Q \lVert P)$. The Population Stability Index (PSI) symmetrizes this:
...
Because KL divergence is asymmetric ($D_{\text{KL}}(P \lVert Q) \neq D_{\text{KL}}(Q \lVert P)$), practitioners often use PSI, a symmetric metric derived from KL divergence that is easier to threshold. A PSI > `{python} DriftThresholds.psi_threshold_str` typically triggers an alert for retraining.
