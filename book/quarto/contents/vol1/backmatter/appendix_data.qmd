...
class SerializationCost:
    ...
    csv_speed_str = fmt(csv_speed_mb, precision=1, commas=False, prefix="~")
    parquet_speed_str = fmt(parquet_speed_mb, precision=1, prefix="> ")
    csv_cycles_str = MarkdownStr("High")
    parquet_cycles_str = MarkdownStr("Low")
    proto_speed_str = fmt(proto_speed_mb, precision=1, commas=False, prefix="~")
...
class DataAlgebra:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    row_size_kb_str = fmt(row_size_kb, precision=1, commas=False)
    int_size_bytes_str = fmt(int_size_bytes, precision=0, commas=False)
    waste_pct_str = fmt(waste_pct, precision=1, commas=False)
    join_table_tb_str = fmt(join_table_tb, precision=1, commas=False)
    # Custom "~" prefix → MarkdownStr per recipe Rule 1 escape-hatch
    join_network_tb_str = fmt(join_network_tb, precision=1, commas=False, prefix="~")
```
...
class LongTailScale:
    ...
    n_unhappy_str = fmt(n_unhappy, precision=1)
```
...
class DriftThresholds:
    ...
    logit_val_str = fmt(logit_val, precision=1, commas=False)
```
...
class LogSumExp:
    ...
    a_str = fmt(a, precision=1, commas=False)
