...
class NumbersToKnow:
    ...
    dram_vs_compute_str = fmt(dram_vs_compute, precision=1, commas=False)
    fp32_vs_int8_str = fmt(fp32_vs_int8, precision=1, commas=False)
    fp32_vs_fp16_str = fmt(fp32_vs_fp16, precision=1, commas=False)
    l1_vs_reg_str = fmt(l1_vs_reg, precision=1, commas=False)

    hbm_vs_register_str = fmt(hbm_vs_register, precision=1, commas=False)
    ssd_vs_register_str = fmt(ssd_vs_register, precision=1, commas=True)
    network_vs_local_str = fmt(network_vs_local, precision=1, commas=False)
    gpu_bw_vs_pcie_str = fmt(gpu_bw_vs_pcie, precision=1, commas=False)

    lat_l1_ns_str = fmt(lat_l1_ns, precision=1, commas=False)
    lat_l2_ns_str = fmt(lat_l2_ns, precision=1, commas=False)
    lat_hbm_ns_str = fmt(lat_hbm_ns, precision=1, commas=False)
    lat_pcie_ns_str = fmt(lat_pcie_ns, precision=1, commas=False)
    lat_ib_ns_str = fmt(lat_ib_ns, precision=1, commas=False)
    lat_ssd_ns_str = fmt(lat_ssd_ns, precision=1, commas=False)

    bw_hbm_h100_str = fmt(h_h100.memory_bw.m_as(TB/second), precision=1, commas=False)
    bw_pcie5_str = fmt(bw_pcie5, precision=1, commas=False)
    bw_dram_str = fmt(bw_dram, precision=1, commas=False)
    bw_ib_str = fmt(bw_ib, precision=1, commas=False)
    bw_nvme_str = fmt(NVME_SEQUENTIAL_BW.m_as(GB/second), precision=1, commas=False)

    flops_h100_fp16_str = fmt(flops_h100_fp16, precision=1, commas=False)
    flops_h100_fp8_str = fmt(flops_h100_fp8, precision=1, commas=False)
    flops_a100_fp16_str = fmt(flops_a100_fp16, precision=1, commas=False)
    mobile_tops_int8_str = fmt(mobile_tops_int8, precision=1, commas=False)
    dc_mobile_ratio_str = fmt(dc_mobile_ratio, precision=1, commas=False)

    ridge_a100_str = fmt(ridge_a100, precision=1, commas=False)
    ridge_h100_str = fmt(ridge_h100, precision=1, commas=False)
```
...
class AppendixMachineSetup:
    ...
    def duration_label(seconds):
        if seconds < SECONDS_PER_MINUTE:
            return f"{fmt(seconds, precision=1, commas=False)} seconds"
        ...
        return f"{fmt(seconds / SEC_PER_DAY, precision=1, commas=False)} days"

    a100_fp16_str = fmt(a100_fp16_raw_value, precision=1)
    a100_bw_tb_str = fmt(a100_bw_raw_value, precision=2, commas=False)

    n_gemm_str = fmt(n_gemm_value, precision=1, commas=False)
    ridge_point_str = fmt(ridge_point_value, precision=1, commas=False)
    gemm_intensity_str = fmt(gemm_intensity_value, precision=1, commas=False)
    ...
    dram_pj_str = fmt(dram_pj_value, precision=1, commas=False)
    flop_pj_str = fmt(flop_pj_value, precision=1, commas=False)
    energy_ratio_str = fmt(energy_ratio_value, precision=1, commas=False)

    l1_ns_str = fmt(l1_ns_value, precision=1, commas=False)
    l2_ns_str = fmt(l2_ns_value, precision=1, commas=False)
    hbm_ns_str = fmt(hbm_ns_value, precision=1, commas=False)
    nvlink_ns_str = fmt(nvlink_ns_value, precision=1, commas=False)
    pcie_ns_str = fmt(pcie_ns_value, precision=1, commas=False)
    ib_ns_str = fmt(ib_ns_value, precision=1, commas=False)
    ssd_ns_str = fmt(ssd_ns_value, precision=1, commas=False)
    ...
    h100_flops_str = fmt(h100_flops_value, precision=1, commas=False)
    h100_bw_str = fmt(h100_bw_value, precision=2, commas=False)
    h100_cap_str = fmt(h100_cap_value, precision=1, commas=False)
    h100_nvlink_str = fmt(h100_nvlink_value, precision=1, commas=False)
    h100_l2_mb_str = fmt(h100_l2_mb_value, precision=1, commas=False)

    tpuv5_flops_str = fmt(tpuv5_flops_value, precision=1, commas=False)
    tpuv5_bw_str = fmt(tpuv5_bw_value, precision=2, commas=False)
    tpuv5_cap_str = fmt(tpuv5_cap_value, precision=1, commas=False)
    tpuv5_ici_str = fmt(tpuv5_ici_value, precision=1, commas=False)
    tpuv5_l2_mb_str = fmt(tpuv5_l2_mb_value, precision=1, commas=False)

    #—Unified Hierarchy Stats (Reference Table) ---
    bw_hbm_str = fmt(Hardware.Cloud.H100.memory.bandwidth.m_as(GB/second), precision=1)
    bw_nvlink_str = fmt(NVLINK_H100_BW.m_as(GB/second), precision=1)
    bw_pcie_str = fmt(PCIE_GEN5_BW.m_as(GB/second), precision=1)
    bw_dram_str = fmt(SYSTEM_MEMORY_BW.m_as(GB/second), precision=1)
    bw_ssd_str = fmt(NVME_SEQUENTIAL_BW.m_as(GB/second), precision=1, commas=False)
    bw_net_str = fmt(Hardware.Networks.InfiniBand_NDR.bandwidth.m_as(GB/second), precision=1, commas=False)
...
class AmdahlSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_pct_str = fmt(s_pct_value, precision=1, commas=False)
    p_pct_str = fmt(p_pct_value, precision=1, commas=False)
    n_8_str = fmt(n_8_value, precision=1, commas=False)
...
class AmdahlExample:
    ...
    amdahl_inf_str = fmt(amdahl_inf_value, precision=1, commas=False)
```
...
class GustafsonSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_g_pct_str = fmt(s_g_pct_value, precision=1, commas=False)
...
class GustafsonExample:
    ...
    gustafson_1000_str = fmt(gustafson_1000_value, precision=1, commas=False)
    n_8_g_str = fmt(n_8_g_value, precision=1, commas=False)
    n_8_g_minus_1_str = fmt(n_8_g_value - 1, precision=1, commas=False)
    n_1000_str = fmt(n_1000_value, precision=1, commas=False)
    n_1000_minus_1_str = fmt(n_1000_value - 1, precision=1, commas=False)
```
...
class TrainingTimeRef:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    T_seconds_str = fmt(t_seconds_value, precision=1, commas=True)
    T_minutes_str = fmt(t_minutes_value, precision=1, commas=False)
    ...
    p_params_str = fmt(p_params/BILLION, precision=1, commas=False, suffix="B")
    d_tokens_str = fmt(d_tokens/BILLION, precision=1, commas=False, suffix="B")
    n_gpus_str = fmt(int(n_gpus), precision=1, commas=False)
    u_mfu_pct_str = fmt(int(u_mfu * 100), precision=1, commas=False)
...
class LittlesLawExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    L_concurrent_str = fmt(l_concurrent_value, precision=1, commas=False)
    lambda_qps_str = fmt(lambda_qps_value, precision=1)
    lambda_qps_raw_str = fmt(lambda_qps_value, precision=1, commas=False)
    w_latency_ms_str = fmt(int(w_latency_s_value * 1000), precision=1, commas=False)
    w_latency_s_str = fmt(w_latency_s_value, precision=2, commas=False)
    mem_per_req_gb_str = fmt(mem_per_req_gb_value, precision=1, commas=False)
    gpu_mem_gb_str = fmt(gpu_mem_gb_value, precision=1, commas=False)
    max_concurrent_str = fmt(max_concurrent_value, precision=1, commas=False)
...
class BandwidthLatencySetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bw_gbps_str = fmt(bw_gbps_value, precision=1, commas=False)
    ping_ms_str = fmt(ping_ms_value, precision=1, commas=False)
    data_kb_str = fmt(data_kb_value, precision=1, commas=False)
```
...
class BandwidthLatencyExample:
    ...
    large_data_gb_str = fmt(large_data_gb_value, precision=1, commas=False)
