...
class AppendixMachineSetup:
    ...
    h100_cap_value = int(H100_MEM_CAPACITY.m_as(GB))
    ...
    tpuv5_cap_value = int(TPUV5P_MEM_CAPACITY.m_as(GB))
...
For capacity planning in inference systems, **Little's Law** [@little1961] relates concurrency ($N_{\text{req}}$), arrival rate ($\lambda$), and latency ($L_{\text{lat}}$):[^fn-little]
$$ N_{\text{req}} = \lambda \times L_{\text{lat}} $$
...
* **Little's Law**: The relationship $N_{\text{req}} = \lambda L_{\text{lat}}$ directly sizes inference infrastructure: concurrency, memory, and maximum throughput are all linked by this simple identity.
