...
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-machine-foundations-concrete-example-a100-analysis-5b30
...
**ReLU (Element-wise)**: For a square tensor of size `{python} A100RooflineExample.n_gemm_str` by `{python} A100RooflineExample.n_gemm_str`, intensity is approximately `{python} A100RooflineExample.relu_intensity_str` FLOP/byte. Since `{python} A100RooflineExample.relu_intensity_str` ≪ `{python} A100RooflineExample.ridge_point_str`, this operation is severely memory bound, achieving only about `{python} A100RooflineExample.relu_util_str` percent of peak TFLOP/s. The hardware is mostly waiting for data.
...
class TrainingTimeRef:
    ...
    a100_fp16_str = fmt(x_flops, precision=0, commas=False)
...
**Example**: Training a **`{python} TrainingTimeRef.p_params_str` parameter** model on **`{python} TrainingTimeRef.d_tokens_str` tokens** using **`{python} TrainingTimeRef.n_gpus_str` A100** (`{python} TrainingTimeRef.a100_fp16_str` TFLOP/s) at **`{python} TrainingTimeRef.u_mfu_pct_str` percent utilization**.
...
from mlsysim.core.constants import (
    H100_L2_CACHE, TPUV5P_L2_SRAM, NVLINK_H100_BW, TPUV5P_ICI_BW,
    LATENCY_NVLINK, SECONDS_PER_MINUTE, SEC_PER_HOUR, SEC_PER_DAY,
)
...
|:---------------------|:------------------------------------------------------------:|:--------------------------------------------------------------------------------:|:--------------------------------------|
| **FP16/BF16 Peak**   |    `{python} ArchReferenceTables.h100_flops_str` TFLOP/s     |              `{python} ArchReferenceTables.tpuv5_flops_str` TFLOP/s              | The "Speed Limit" ($R_{\text{peak}}$) |
| **Memory Bandwidth** |       `{python} ArchReferenceTables.h100_bw_str` TB/s        |                 `{python} ArchReferenceTables.tpuv5_bw_str` TB/s                 | The "Width of the Pipe" ($\text{BW}$) |
| **HBM Capacity**     |        `{python} ArchReferenceTables.h100_cap_str` GB        |                 `{python} ArchReferenceTables.tpuv5_cap_str` GB                  | Max Model Size ($P$)/Batch Size ($B$) |
| **L2/SRAM Cache**    |       `{python} ArchReferenceTables.h100_l2_mb_str` MB       |                ~`{python} ArchReferenceTables.tpuv5_l2_mb_str` MB                | Critical for Operator Fusion          |
| **Interconnect**     | `{python} ArchReferenceTables.h100_nvlink_str` GB/s (NVLink) | `{python} ArchReferenceTables.tpuv5_ici_str` GB/s (Inter-Chip Interconnect, ICI) | Determines Model Parallelism Scaling  |
...
| **Memory (Local)**   |      HBM3      |  ~`{python} ArchReferenceTables.hbm_ns_str` ns   |   `{python} ArchReferenceTables.bw_hbm_str` GB/s  |  `{python} ArchReferenceTables.e_dram_str` pJ |
| **Interconnect**     |   NVLink 4.0   | ~`{python} ArchReferenceTables.nvlink_ns_str` ns | `{python} ArchReferenceTables.bw_nvlink_str` GB/s | ~`{python} ArchReferenceTables.e_dram_str` pJ |
| **Host Link**        |   PCIe Gen5    |  ~`{python} ArchReferenceTables.pcie_ns_str` ns  |  `{python} ArchReferenceTables.bw_pcie_str` GB/s  | ~`{python} ArchReferenceTables.e_dram_str` pJ |
| **System RAM**       |      DDR5      |                     ~100 ns                      |  `{python} ArchReferenceTables.bw_dram_str` GB/s  | ~`{python} ArchReferenceTables.e_dram_str` pJ |
| **Network (Fabric)** | InfiniBand NDR |   ~`{python} ArchReferenceTables.ib_ns_str` ns   |   `{python} ArchReferenceTables.bw_net_str` GB/s  |  `{python} ArchReferenceTables.e_net_str` pJ  |
| **Storage (Local)**  |    NVMe SSD    |  ~`{python} ArchReferenceTables.ssd_ns_str` ns   |   `{python} ArchReferenceTables.bw_ssd_str` GB/s  |  `{python} ArchReferenceTables.e_ssd_str` pJ  |
...
#| echo: false
from mlsysim import *
#| label: fp16-energy-recap
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: Fallacies — FP32 vs FP16 energy recap
# │ Exports: Fp16EnergyRecap.fp32_vs_fp16_str
from mlsysim.core.constants import ENERGY_FLOP_FP32_PJ, ENERGY_FLOP_FP16_PJ
from mlsysim.fmt import fmt

class Fp16EnergyRecap:
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    fp32_vs_fp16 = round(
        ENERGY_FLOP_FP32_PJ.m_as(ureg.picojoule) / ENERGY_FLOP_FP16_PJ.m_as(ureg.picojoule), 1
    )
    fp32_vs_fp16_str = fmt(fp32_vs_fp16, precision=1, commas=False)
```

For deep learning, FP32 often hurts performance without improving convergence. It consumes 2$\times$ memory bandwidth compared to BF16 and about `{python} Fp16EnergyRecap.fp32_vs_fp16_str`$\times$ the arithmetic energy of FP16-class compute in the reference table above. Since neural networks are resilient to noise, the extra mantissa bits in FP32 are often modeling random variance rather than signal.
