...
|:-----------------------|:---------------------------------------------------:|:-----------------------------------------------------------:|:-----:|
| Data center GPU (H100) | `{python} NumbersToKnow.flops_h100_fp16_str` TFLOPS | `{python} NumbersToKnow.flops_h100_fp8_str` TOPS (FP8 peak) | 700 W |
| Data center GPU (A100) | `{python} NumbersToKnow.flops_a100_fp16_str` TFLOPS |                           624 TOPS                          | 400 W |
...
**Example**: Training a **`{python} TrainingTimeRef.p_params_str` parameter** model on **`{python} TrainingTimeRef.d_tokens_str` tokens** using **`{python} TrainingTimeRef.n_gpus_str` A100** (`{python} AppendixMachineSetup.a100_fp16_str` TFLOPS) at **`{python} TrainingTimeRef.u_mfu_pct_str` percent utilization**.
...
|:---------------------|:-------------------------------------------------------------:|:---------------------------------------------------------------------------------:|:--------------------------------------|
| **FP16/BF16 Peak**   |     `{python} AppendixMachineSetup.h100_flops_str` TFLOPS     |               `{python} AppendixMachineSetup.tpuv5_flops_str` TFLOPS              | The "Speed Limit" ($R_{\text{peak}}$) |
