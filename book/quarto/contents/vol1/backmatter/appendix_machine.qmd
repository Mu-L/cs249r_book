...
from mlsysim.fmt import fmt, check, md_math, sci_latex, MarkdownStr
...
class TrainingTimeRef:
    ...
    total_flops_eq = md_math(f"\\text{{Total FLOPs}} = 6 \\times {sci_latex(p_params, 0)} \\times {sci_latex(d_tokens, 0)} = {sci_latex(total_flops, 1)} \\text{{ FLOPs}}")
    throughput_eq = md_math(f"\\text{{Throughput}} = {int(n_gpus)} \\times ({sci_latex(x_flops*TRILLION, 0)}) \\times {u_mfu:.2f} \\approx {sci_latex(throughput, 2)} \\text{{ FLOP/s}}")
    time_eq = md_math(f"T = \\frac{{{sci_latex(total_flops, 1)}}}{{{sci_latex(throughput, 2)}}} \\approx {fmt(t_seconds_value, precision=0, commas=True)} \\text{{ seconds}} \\approx {fmt(t_minutes_value, precision=0, commas=True)} \\text{{ minutes}}")
...
class LittlesLawExample:
    ...
    max_throughput_eq = md_math(f"L/W = {max_concurrent_value} / {w_latency_s_value} = {int(max_throughput_value)} \\text{{ QPS}}")
```
...
class BandwidthLatencyExample:
    ...
    large_tx_eq = md_math(f"{large_data_gb_value}\\text{{ GB}} / {int(bw_gbps_value/BILLION)}\\text{{ Gbps}} \\approx {int(large_tx_time_s_value*1000)}\\text{{ ms}}")
    large_total_eq = md_math(f"\\approx {int(ping_ms_value)}\\text{{ ms}} + {int(large_tx_time_s_value*1000)}\\text{{ ms}} = {int(total_large_time_ms_value)}\\text{{ ms}}")
```
