...
# │          flops_a100_fp16_str, flops_mobile_int8_str, dc_mobile_ratio_str,
...
class NumbersToKnow:
    ...
    dc_mobile_ratio = flops_h100_fp16 / flops_mobile_int8
    ...
    bw_hbm_h100_str = fmt(H100_MEM_BW.m_as(TB/second), precision=1, commas=False)
    ...
    flops_mobile_int8_str = fmt(flops_mobile_int8, precision=0, commas=False)
...
* **16 bytes/parameter for training**: Model weights (2B FP16) + gradients (2B FP16) + master weights (4B FP32) + optimizer states for Adaptive Moment Estimation (Adam) at 8B. That totals 16 bytes per parameter, so a 7B model needs 112 GB just to start training.
...
|:-----------------------|:----------------------------------------------------:|:-----------------------------------------------------------:|:-----:|
| Data center GPU (H100) | `{python} NumbersToKnow.flops_h100_fp16_str` TFLOP/s | `{python} NumbersToKnow.flops_h100_fp8_str` TOPS (FP8 peak) | 700 W |
| Data center GPU (A100) | `{python} NumbersToKnow.flops_a100_fp16_str` TFLOP/s |                           624 TOPS                          | 400 W |
| Mobile NPU             |                          —                           |     `{python} NumbersToKnow.flops_mobile_int8_str` TOPS     | 3–5 W |
...
|:------------|:------------------------------------------------:|:-----------------------------|
| A100 (FP16) | `{python} NumbersToKnow.ridge_a100_str` ops/byte | Below → memory-bound         |
| H100 (FP16) | `{python} NumbersToKnow.ridge_h100_str` ops/byte | Higher bar for compute-bound |
...
class AppendixMachineSetup:
    ...
    def duration_label(seconds):
        if seconds < 60:
            return f"{fmt(seconds, precision=0, commas=False)} seconds"
        if seconds < 3600:
            return f"{fmt(seconds / 60, precision=1, commas=False)} minutes"
        if seconds < 86400:
            return f"{fmt(seconds / 3600, precision=1, commas=False)} hours"
        return f"{fmt(seconds / 86400, precision=0, commas=False)} days"

    a100_fp16_str = fmt(a100_fp16_raw_value, precision=0)
    ...
    flop_pj_str = fmt(flop_pj_value, precision=0, commas=False)
    ...
    bw_hbm_str = fmt(H100_MEM_BW.m_as(GB/second), precision=0)
    bw_nvlink_str = fmt(NVLINK_H100_BW.m_as(GB/second), precision=0)
    bw_pcie_str = fmt(PCIE_GEN5_BW.m_as(GB/second), precision=0)
    bw_dram_str = fmt(SYSTEM_MEMORY_BW.m_as(GB/second), precision=0)
    bw_ssd_str = fmt(NVME_SEQUENTIAL_BW.m_as(GB/second), precision=1, commas=False)
    bw_net_str = fmt(INFINIBAND_NDR_BW_GBS, precision=0, commas=False)
    ...
    e_reg_str = MarkdownStr("0.01")
    e_l1_str = MarkdownStr("0.5")
    e_l2_str = MarkdownStr("2.0")
    e_dram_str = MarkdownStr("640")
    e_ssd_str = MarkdownStr("~5,000")   # ~1.2 uJ per 1KB read -> ~5nJ per 32b
    e_net_str = MarkdownStr("~10,000")  # ~1uJ per 1KB packet (header overhead)
```
...
*   **Compute Term**: $\frac{\text{FLOPs}}{\text{FLOP/s}} = \text{FLOPs} \times \frac{\text{s}}{\text{FLOPs}} = \mathbf{s}$
...
class AmdahlSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_pct_str = fmt(s_pct_value, precision=0, commas=False)
    p_pct_str = fmt(p_pct_value, precision=0, commas=False)
...
class GustafsonSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_g_pct_str = fmt(s_g_pct_value, precision=0, commas=False)
...
class TrainingTimeRef:
    ...
    n_gpus_str = fmt(int(n_gpus), precision=0, commas=False)
    u_mfu_pct_str = fmt(int(u_mfu * 100), precision=0, commas=False)
    total_flops_eq = fmt_math(f"\\text{{Total FLOPs}} = 6 \\times {sci_latex(p_params, 0)} \\times {sci_latex(d_tokens, 0)} = {sci_latex(total_flops, 1)} \\text{{ FLOPs}}")
    ...
    time_eq = fmt_math(f"T = \\frac{{{sci_latex(total_flops, 1)}}}{{{sci_latex(throughput, 2)}}} \\approx {fmt(t_seconds_value, precision=0, commas=True)} \\text{{ seconds}} \\approx {fmt(t_minutes_value, precision=0, commas=True)} \\text{{ minutes}}")
...
$$ T \approx \frac{6 \cdot P \cdot D}{N \cdot R_{\text{peak}} \cdot \eta_{\text{hw}}} $$
Where:
...
class LittlesLawExample:
    ...
    w_latency_ms_str = fmt(int(w_latency_s_value * 1000), precision=0, commas=False)
    w_latency_s_str = fmt(w_latency_s_value, precision=2, commas=False)
    mem_per_req_gb_str = fmt(mem_per_req_gb_value, precision=0, commas=False)
    gpu_mem_gb_str = fmt(gpu_mem_gb_value, precision=0, commas=False)
    max_concurrent_str = fmt(max_concurrent_value, precision=0, commas=False)
    max_throughput_eq = fmt_math(f"L/W = {max_concurrent_value} / {w_latency_s_value} = {int(max_throughput_value)} \\text{{ QPS}}")
```
...
class BandwidthLatencySetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bw_gbps_str = fmt(bw_gbps_value, precision=0, commas=False)
    ping_ms_str = fmt(ping_ms_value, precision=0, commas=False)
    data_kb_str = fmt(data_kb_value, precision=0, commas=False)
```
...
class BandwidthLatencyExample:
    ...
    large_tx_eq = fmt_math(f"{large_data_gb_value}\\text{{ GB}} / {int(bw_gbps_value/BILLION)}\\text{{ Gbps}} \\approx {int(large_tx_time_s_value*1000)}\\text{{ ms}}")
...
* **Little's Law**: The relationship $N_{\text{req}} = \lambda T_{\text{lat}}$ directly sizes inference infrastructure: concurrency, memory, and maximum throughput are all linked by this simple identity.
