...
# │          l1_vs_reg_str, hbm_vs_l1_str, ssd_vs_l1_str,
...
class NumbersToKnow:
    ...
    hbm_vs_l1_str = fmt(hbm_vs_l1, precision=0, commas=False)
    ssd_vs_l1_str = fmt(ssd_vs_l1, precision=0, commas=False)
...
|:-----------------------------------------------|:-------------------------------------------------------------:|:----------------------------------|
| Accelerator memory (HBM) vs. register          |     ~`{python} NumbersToKnow.hbm_vs_l1_str`$\times$ slower    | On-chip vs. off-chip              |
| SSD vs. register                               |     ~`{python} NumbersToKnow.ssd_vs_l1_str`$\times$ slower    | Electrical vs. mechanical/flash   |
...
class TrainingTimeRef:
    ...
    p_params_str = fmt(fmt(p_params/BILLION, precision=0, commas=False), precision=0, commas=False, suffix="B")
    d_tokens_str = fmt(fmt(d_tokens/BILLION, precision=0, commas=False), precision=0, commas=False, suffix="B")
    ...
    throughput_eq = fmt_math(f"\\text{{Throughput}} = {int(n_gpus)} \\times ({sci_latex(x_flops*TRILLION, 0)}) \\times {u_mfu:.2f} \\approx {sci_latex(throughput, 2)} \\text{{ FLOP/s}}")
...
| **L1 Cache**             |   ~`{python} AppendixMachineSetup.l1_ns_str` ns   |      3–4 cycles     |         1 minute        |
| **L2 Cache**             |   ~`{python} AppendixMachineSetup.l2_ns_str` ns   |      12 cycles      |        4 minutes        |
| **HBM3 (GPU Memory)**    |   ~`{python} AppendixMachineSetup.hbm_ns_str` ns  |     1,000 cycles    |         5 hours         |
| **NVLink (GPU-GPU)**     | ~`{python} AppendixMachineSetup.nvlink_ns_str` ns |     1,500 cycles    |         8 hours         |
| **PCIe (CPU-GPU)**       |  ~`{python} AppendixMachineSetup.pcie_ns_str` ns  |     3,000 cycles    |          1 day          |
| **InfiniBand (Network)** |   ~`{python} AppendixMachineSetup.ib_ns_str` ns   |    15,000 cycles    |          1 week         |
| **SSD (NVMe)**           |   ~`{python} AppendixMachineSetup.ssd_ns_str` ns  |    300,000 cycles   |         3 months        |
