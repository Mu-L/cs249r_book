...
from mlsysim.core.constants import (
    SPEED_OF_LIGHT_FIBER_KM_S, ENERGY_DRAM_ACCESS_PJ, ENERGY_FLOP_FP16_PJ,
    ENERGY_FLOP_FP32_PJ, ENERGY_OP_INT8_PJ, ENERGY_SRAM_L1_PJ, ENERGY_SRAM_L2_PJ, ENERGY_REG_PJ,
    LATENCY_REGISTER_REF, LATENCY_HBM3, LATENCY_NVME_SSD, LATENCY_INFINIBAND,
    PCIE_GEN5_BW, LATENCY_L1_REGISTER, LATENCY_L2_CACHE, SYSTEM_MEMORY_BW,
    NVME_SEQUENTIAL_BW, LATENCY_PCIE_GEN5, TOPS,
...
class NumbersToKnow:
    ...
    gpu_bw_vs_pcie = int(h_h100.memory_bw.m_as(GB/second) / PCIE_GEN5_BW.m_as(GB/second))
    ...
    flops_h100_fp16 = int(h_h100.peak_flops.m_as(TFLOPs/second))
    flops_h100_fp8 = int(h_h100.compute.precision_flops["fp8"].m_as(TFLOPs/second))
    flops_a100_fp16 = int(h_a100.peak_flops.m_as(TFLOPs/second))
    ...
    ridge_a100 = int(h_a100.ridge_point("fp16").m_as("flop/byte"))
    ridge_h100 = int(h_h100.ridge_point("fp16").m_as("flop/byte"))

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Canonical _str forms for prose; raw ints/floats above kept for arithmetic.
    from mlsysim.fmt import fmt
    dram_vs_compute_str = fmt(dram_vs_compute, precision=1, allow_zero=True, commas=False)
    fp32_vs_int8_str = fmt(fp32_vs_int8, precision=1, allow_zero=True, commas=False)
    fp32_vs_fp16_str = fmt(fp32_vs_fp16, precision=1, commas=False)
    l1_vs_reg_str = fmt(l1_vs_reg, precision=1, allow_zero=True, commas=False)

    hbm_vs_register_str = fmt(hbm_vs_register, precision=1, allow_zero=True, commas=False)
    ssd_vs_register_str = fmt(ssd_vs_register, precision=1, allow_zero=True, commas=True)
    network_vs_local_str = fmt(network_vs_local, precision=1, allow_zero=True, commas=False)
    gpu_bw_vs_pcie_str = fmt(gpu_bw_vs_pcie, precision=1, allow_zero=True, commas=False)

    lat_l1_ns_str = fmt(lat_l1_ns, precision=1, allow_zero=True, commas=False)
    lat_l2_ns_str = fmt(lat_l2_ns, precision=1, allow_zero=True, commas=False)
    lat_hbm_ns_str = fmt(lat_hbm_ns, precision=1, allow_zero=True, commas=False)
    lat_pcie_ns_str = fmt(lat_pcie_ns, precision=1, allow_zero=True, commas=False)
    lat_ib_ns_str = fmt(lat_ib_ns, precision=1, allow_zero=True, commas=False)
    lat_ssd_ns_str = fmt(lat_ssd_ns, precision=1, allow_zero=True, commas=False)

    bw_hbm_h100_str = fmt(h_h100.memory_bw.m_as(TB/second), precision=1, commas=False)
    bw_pcie5_str = fmt(bw_pcie5, precision=1, allow_zero=True, commas=False)
    bw_dram_str = fmt(bw_dram, precision=1, allow_zero=True, commas=False)
    bw_ib_str = fmt(bw_ib, precision=1, allow_zero=True, commas=False)
    bw_nvme_str = fmt(NVME_SEQUENTIAL_BW.m_as(GB/second), precision=1, commas=False)

    flops_h100_fp16_str = fmt(flops_h100_fp16, precision=1, allow_zero=True, commas=False)
    flops_h100_fp8_str = fmt(flops_h100_fp8, precision=1, allow_zero=True, commas=False)
    flops_a100_fp16_str = fmt(flops_a100_fp16, precision=1, allow_zero=True, commas=False)
    mobile_tops_int8_str = fmt(mobile_tops_int8, precision=1, allow_zero=True, commas=False)
    dc_mobile_ratio_str = fmt(dc_mobile_ratio, precision=1, commas=False)

    ridge_a100_str = fmt(ridge_a100, precision=1, allow_zero=True, commas=False)
    ridge_h100_str = fmt(ridge_h100, precision=1, allow_zero=True, commas=False)
```
...
| GPU HBM3             | ~`{python} NumbersToKnow.lat_hbm_ns_str` ns  | `{python} NumbersToKnow.bw_hbm_h100_str` TB/s |
| PCIe Gen5 (CPU↔GPU)  | ~`{python} NumbersToKnow.lat_pcie_ns_str` ns |   `{python} NumbersToKnow.bw_pcie5_str` GB/s  |
| CPU DRAM             |                   ~100 ns                    |   `{python} NumbersToKnow.bw_dram_str` GB/s   |
| InfiniBand (network) |  ~`{python} NumbersToKnow.lat_ib_ns_str` ns  |    `{python} NumbersToKnow.bw_ib_str` GB/s    |
| NVMe SSD             | ~`{python} NumbersToKnow.lat_ssd_ns_str` ns  |   `{python} NumbersToKnow.bw_nvme_str` GB/s   |
...
|:-----------------------|:----------------------------------------------------:|:--------------------------------------------------------------:|:-----:|
| Data center GPU (H100) | `{python} NumbersToKnow.flops_h100_fp16_str` TFLOP/s | `{python} NumbersToKnow.flops_h100_fp8_str` TFLOP/s (FP8 peak) | 700 W |
| Data center GPU (A100) | `{python} NumbersToKnow.flops_a100_fp16_str` TFLOP/s |                            624 TOPS                            | 400 W |
...
# │ Show: A100 ridge ~153 FLOP/byte; n=4096 GEMM is compute-bound (~1365);
# │       ReLU at 0.25 FLOP/byte is deeply memory-bound (~0.16% utilization).
...
# │
# │ Imports: mlsysim.core.constants (*), mlsysim.book (fmt)
# │ Exports: a100_fp16_str, a100_bw_tb_str, n_gemm_str, ridge_point_str,
# │          gemm_intensity_str, relu_intensity_str, relu_util_str,
# │          dram_pj_str, flop_pj_str, energy_ratio_str,
# │          l1_ns_str, l2_ns_str, hbm_ns_str, nvlink_ns_str, pcie_ns_str,
# │          ib_ns_str, ssd_ns_str,
# │          register_distance_str, l1_distance_str, l2_distance_str,
# │          hbm_distance_str, nvlink_distance_str, pcie_distance_str,
# │          ib_distance_str, ssd_distance_str,
# │          h100_flops_str, h100_bw_str, h100_cap_str, h100_nvlink_str,
# │          h100_l2_mb_str,
# │          tpuv5_flops_str, tpuv5_bw_str, tpuv5_cap_str, tpuv5_ici_str,
# │          tpuv5_l2_mb_str,
# │          bw_hbm_str, bw_nvlink_str, bw_pcie_str, bw_dram_str, bw_ssd_str,
# │          bw_net_str,
# │          e_reg_str, e_l1_str, e_l2_str, e_dram_str, e_ssd_str, e_net_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AppendixMachineSetup:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    from mlsysim.core.constants import H100_L2_CACHE, TPUV5P_L2_SRAM, NVLINK_H100_BW, TPUV5P_ICI_BW, LATENCY_NVLINK, SECONDS_PER_MINUTE, SEC_PER_HOUR, SEC_PER_DAY
    n_gemm_value = 4096
    relu_intensity_value = 0.25
    h100_l2_mb_value = H100_L2_CACHE.m_as(MB)
    tpuv5_l2_mb_value = TPUV5P_L2_SRAM.m_as(MB)
    ...
    def duration_label(seconds):
        if seconds < SECONDS_PER_MINUTE:
            return f"{fmt(seconds, precision=1, allow_zero=True, commas=False)} seconds"
        if seconds < SEC_PER_HOUR:
            return f"{fmt(seconds / SECONDS_PER_MINUTE, precision=1, commas=False)} minutes"
        if seconds < SEC_PER_DAY:
            return f"{fmt(seconds / SEC_PER_HOUR, precision=1, commas=False)} hours"
        return f"{fmt(seconds / SEC_PER_DAY, precision=1, allow_zero=True, commas=False)} days"

    a100_fp16_str = fmt(a100_fp16_raw_value, precision=1, allow_zero=True)
    a100_bw_tb_str = fmt(a100_bw_raw_value, precision=2, commas=False)

    n_gemm_str = fmt(n_gemm_value, precision=1, allow_zero=True, commas=False)
    ridge_point_str = fmt(ridge_point_value, precision=1, allow_zero=True, commas=False)
    gemm_intensity_str = fmt(gemm_intensity_value, precision=1, allow_zero=True, commas=False)
    ...
    dram_pj_str = fmt(dram_pj_value, precision=1, allow_zero=True, commas=False)
    flop_pj_str = fmt(flop_pj_value, precision=1, allow_zero=True, commas=False)
    energy_ratio_str = fmt(energy_ratio_value, precision=1, allow_zero=True, commas=False)

    l1_ns_str = fmt(l1_ns_value, precision=1, allow_zero=True, commas=False)
    l2_ns_str = fmt(l2_ns_value, precision=1, allow_zero=True, commas=False)
    hbm_ns_str = fmt(hbm_ns_value, precision=1, allow_zero=True, commas=False)
    nvlink_ns_str = fmt(nvlink_ns_value, precision=1, allow_zero=True, commas=False)
    pcie_ns_str = fmt(pcie_ns_value, precision=1, allow_zero=True, commas=False)
    ib_ns_str = fmt(ib_ns_value, precision=1, allow_zero=True, commas=False)
    ssd_ns_str = fmt(ssd_ns_value, precision=1, allow_zero=True, commas=False)
    ...
    h100_flops_str = fmt(h100_flops_value, precision=1, allow_zero=True, commas=False)
    h100_bw_str = fmt(h100_bw_value, precision=2, commas=False)
    h100_cap_str = fmt(h100_cap_value, precision=1, allow_zero=True, commas=False)
    h100_nvlink_str = fmt(h100_nvlink_value, precision=1, allow_zero=True, commas=False)
    h100_l2_mb_str = fmt(h100_l2_mb_value, precision=1, allow_zero=True, commas=False)

    tpuv5_flops_str = fmt(tpuv5_flops_value, precision=1, allow_zero=True, commas=False)
    tpuv5_bw_str = fmt(tpuv5_bw_value, precision=2, commas=False)
    tpuv5_cap_str = fmt(tpuv5_cap_value, precision=1, allow_zero=True, commas=False)
    tpuv5_ici_str = fmt(tpuv5_ici_value, precision=1, allow_zero=True, commas=False)
    tpuv5_l2_mb_str = fmt(tpuv5_l2_mb_value, precision=1, allow_zero=True, commas=False)

    #—Unified Hierarchy Stats (Reference Table) ---
    bw_hbm_str = fmt(Hardware.Cloud.H100.memory.bandwidth.m_as(GB/second), precision=1, allow_zero=True)
    bw_nvlink_str = fmt(NVLINK_H100_BW.m_as(GB/second), precision=1, allow_zero=True)
    bw_pcie_str = fmt(PCIE_GEN5_BW.m_as(GB/second), precision=1, allow_zero=True)
    bw_dram_str = fmt(SYSTEM_MEMORY_BW.m_as(GB/second), precision=1, allow_zero=True)
    bw_ssd_str = fmt(NVME_SEQUENTIAL_BW.m_as(GB/second), precision=1, commas=False)
    bw_net_str = fmt(Hardware.Networks.InfiniBand_NDR.bandwidth.m_as(GB/second), precision=1, allow_zero=True, commas=False)

    # Energy-per-32b column values: short literal labels rendered verbatim in
    # the reference table. Use MarkdownStr (recipe Rule 1 escape hatch) so the
    # leading "~" and embedded comma in "~5,000" / "~10,000" pass through
    ...
    e_ssd_str = MarkdownStr("~5,000")   # ~1.2 μJ per 1 KB read -> ~5 nJ per 32b
    e_net_str = MarkdownStr("~10,000")  # ~1 μJ per 1 KB packet (header overhead)
```

#### A concrete example: The A100 analysis {#sec-machine-foundations-concrete-example-a100-analysis-5b30}

Consider an NVIDIA A100 GPU with FP16 Tensor Core performance of `{python} AppendixMachineSetup.a100_fp16_str` TFLOP/s and HBM2e bandwidth of `{python} AppendixMachineSetup.a100_bw_tb_str` TB/s. The ridge point is `{python} AppendixMachineSetup.a100_fp16_str`/`{python} AppendixMachineSetup.a100_bw_tb_str` = `{python} AppendixMachineSetup.ridge_point_str` FLOP/byte (the Tera prefixes cancel, yielding FLOP/byte).

Consider two common operations:

**General matrix multiply (GEMM)**: For two square matrices of size `{python} AppendixMachineSetup.n_gemm_str` by `{python} AppendixMachineSetup.n_gemm_str`, arithmetic intensity is approximately `{python} AppendixMachineSetup.gemm_intensity_str` FLOP/byte. Since `{python} AppendixMachineSetup.gemm_intensity_str` > `{python} AppendixMachineSetup.ridge_point_str`, this operation is compute bound. You are using the hardware efficiently.

**ReLU (Element-wise)**: For a square tensor of size `{python} AppendixMachineSetup.n_gemm_str` by `{python} AppendixMachineSetup.n_gemm_str`, intensity is approximately `{python} AppendixMachineSetup.relu_intensity_str` FLOP/byte. Since `{python} AppendixMachineSetup.relu_intensity_str` ≪ `{python} AppendixMachineSetup.ridge_point_str`, this operation is severely memory bound, achieving only about `{python} AppendixMachineSetup.relu_util_str` percent of peak TFLOP/s. The hardware is mostly waiting for data.
...
class AmdahlSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_pct_str = fmt(s_pct_value, precision=1, allow_zero=True, commas=False)
    p_pct_str = fmt(p_pct_value, precision=1, allow_zero=True, commas=False)
    n_8_str = fmt(n_8_value, precision=1, allow_zero=True, commas=False)
...
class AmdahlExample:
    ...
    amdahl_inf_str = fmt(amdahl_inf_value, precision=1, allow_zero=True, commas=False)
```
...
class GustafsonSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    s_g_pct_str = fmt(s_g_pct_value, precision=1, allow_zero=True, commas=False)
...
class GustafsonExample:
    ...
    gustafson_1000_str = fmt(gustafson_1000_value, precision=1, allow_zero=True, commas=False)
    n_8_g_str = fmt(n_8_g_value, precision=1, allow_zero=True, commas=False)
    n_8_g_minus_1_str = fmt(n_8_g_value - 1, precision=1, allow_zero=True, commas=False)
    n_1000_str = fmt(n_1000_value, precision=1, allow_zero=True, commas=False)
    n_1000_minus_1_str = fmt(n_1000_value - 1, precision=1, allow_zero=True, commas=False)
```
...
from mlsysim.fmt import fmt, check, fmt_math, sci_latex, MarkdownStr
...
class TrainingTimeRef:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    T_seconds_str = fmt(t_seconds_value, precision=1, allow_zero=True, commas=True)
    T_minutes_str = fmt(t_minutes_value, precision=1, allow_zero=True, commas=False)
    T_days_str = fmt(t_days_value, precision=1, allow_zero=True, commas=False)
    ...
    p_params_str = fmt(p_params/BILLION, precision=1, allow_zero=True, commas=False, suffix="B")
    d_tokens_str = fmt(d_tokens/BILLION, precision=1, allow_zero=True, commas=False, suffix="B")
    n_gpus_str = fmt(int(n_gpus), precision=1, allow_zero=True, commas=False)
    u_mfu_pct_str = fmt(int(u_mfu * 100), precision=1, allow_zero=True, commas=False)
    total_flops_eq = fmt_math(f"\\text{{Total FLOPs}} = {TRANSFORMER_TRAINING_FLOPS_PER_PARAM_TOKEN} \\times {sci_latex(p_params, 0)} \\times {sci_latex(d_tokens, 0)} = {sci_latex(total_flops, 1)} \\text{{ FLOPs}}")
    ...
    time_eq = fmt_math(f"T = \\frac{{{sci_latex(total_flops, 1)}}}{{{sci_latex(throughput, 2)}}} \\approx {fmt(t_seconds_value, precision=1, allow_zero=True, commas=True)} \\text{{ seconds}} \\approx {fmt(t_minutes_value, precision=1, allow_zero=True, commas=True)} \\text{{ minutes}}")
...
**Example**: Training a **`{python} TrainingTimeRef.p_params_str` parameter** model on **`{python} TrainingTimeRef.d_tokens_str` tokens** using **`{python} TrainingTimeRef.n_gpus_str` A100** (`{python} AppendixMachineSetup.a100_fp16_str` TFLOP/s) at **`{python} TrainingTimeRef.u_mfu_pct_str` percent utilization**.
...
class LittlesLawExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    L_concurrent_str = fmt(l_concurrent_value, precision=1, allow_zero=True, commas=False)
    lambda_qps_str = fmt(lambda_qps_value, precision=1, allow_zero=True)
    lambda_qps_raw_str = fmt(lambda_qps_value, precision=1, allow_zero=True, commas=False)
    w_latency_ms_str = fmt(int(w_latency_s_value * 1000), precision=1, allow_zero=True, commas=False)
    w_latency_s_str = fmt(w_latency_s_value, precision=2, commas=False)
    mem_per_req_gb_str = fmt(mem_per_req_gb_value, precision=1, allow_zero=True, commas=False)
    gpu_mem_gb_str = fmt(gpu_mem_gb_value, precision=1, allow_zero=True, commas=False)
    max_concurrent_str = fmt(max_concurrent_value, precision=1, allow_zero=True, commas=False)
...
This directly determines how to size inference worker pools. If serving one request requires `{python} LittlesLawExample.mem_per_req_gb_str` GB of temporary memory (KV cache, activations), handling `{python} LittlesLawExample.L_concurrent_str` concurrent requests requires `{python} LittlesLawExample.L_concurrent_str` GB of memory. If the accelerator only has `{python} LittlesLawExample.gpu_mem_gb_str` GB, the system is physically limited to `{python} LittlesLawExample.max_concurrent_str` concurrent requests. Maximum throughput is capped at `{python} LittlesLawExample.max_throughput_eq`, regardless of how many requests arrive.
...
|:-------------------------|:-------------------------------------------------:|:-------------------:|:-----------------------------------------------------:|
| **Register**             |                      ~0.3 ns                      |       1 cycle       | `{python} AppendixMachineSetup.register_distance_str` |
| **L1 Cache**             |   ~`{python} AppendixMachineSetup.l1_ns_str` ns   |      3–4 cycles     |    `{python} AppendixMachineSetup.l1_distance_str`    |
| **L2 Cache**             |   ~`{python} AppendixMachineSetup.l2_ns_str` ns   |      12 cycles      |    `{python} AppendixMachineSetup.l2_distance_str`    |
| **HBM3 (GPU Memory)**    |   ~`{python} AppendixMachineSetup.hbm_ns_str` ns  |     1,000 cycles    |    `{python} AppendixMachineSetup.hbm_distance_str`   |
| **NVLink (GPU-GPU)**     | ~`{python} AppendixMachineSetup.nvlink_ns_str` ns |     1,500 cycles    |  `{python} AppendixMachineSetup.nvlink_distance_str`  |
| **PCIe (CPU-GPU)**       |  ~`{python} AppendixMachineSetup.pcie_ns_str` ns  |     3,000 cycles    |   `{python} AppendixMachineSetup.pcie_distance_str`   |
| **InfiniBand (Network)** |   ~`{python} AppendixMachineSetup.ib_ns_str` ns   |    15,000 cycles    |    `{python} AppendixMachineSetup.ib_distance_str`    |
| **SSD (NVMe)**           |   ~`{python} AppendixMachineSetup.ssd_ns_str` ns  |    300,000 cycles   |    `{python} AppendixMachineSetup.ssd_distance_str`   |
...
|:---------------------|:-------------------------------------------------------------:|:---------------------------------------------------------------------------------:|:--------------------------------------|
| **FP16/BF16 Peak**   |     `{python} AppendixMachineSetup.h100_flops_str` TFLOP/s    |              `{python} AppendixMachineSetup.tpuv5_flops_str` TFLOP/s              | The "Speed Limit" ($R_{\text{peak}}$) |
| **Memory Bandwidth** |        `{python} AppendixMachineSetup.h100_bw_str` TB/s       |                 `{python} AppendixMachineSetup.tpuv5_bw_str` TB/s                 | The "Width of the Pipe" ($\text{BW}$) |
| **HBM Capacity**     |        `{python} AppendixMachineSetup.h100_cap_str` GB        |                  `{python} AppendixMachineSetup.tpuv5_cap_str` GB                 | Max Model Size ($P$)/Batch Size ($B$) |
| **L2/SRAM Cache**    |       `{python} AppendixMachineSetup.h100_l2_mb_str` MB       |                ~`{python} AppendixMachineSetup.tpuv5_l2_mb_str` MB                | Critical for Operator Fusion          |
| **Interconnect**     | `{python} AppendixMachineSetup.h100_nvlink_str` GB/s (NVLink) | `{python} AppendixMachineSetup.tpuv5_ici_str` GB/s (Inter-Chip Interconnect, ICI) | Determines Model Parallelism Scaling  |
...
|:---------------------|:--------------:|:-------------------------------------------------:|:--------------------------------------------------:|:----------------------------------------------:|
| **Registers**        |   Flip-Flops   |                      ~0.3 ns                      |                         —                          |  `{python} AppendixMachineSetup.e_reg_str` pJ  |
| **L1 Cache**         |      SRAM      |   ~`{python} AppendixMachineSetup.l1_ns_str` ns   |                         —                          |  `{python} AppendixMachineSetup.e_l1_str` pJ   |
| **L2 Cache**         |      SRAM      |   ~`{python} AppendixMachineSetup.l2_ns_str` ns   |                         —                          |  `{python} AppendixMachineSetup.e_l2_str` pJ   |
| **Memory (Local)**   |      HBM3      |   ~`{python} AppendixMachineSetup.hbm_ns_str` ns  |  `{python} AppendixMachineSetup.bw_hbm_str` GB/s   | `{python} AppendixMachineSetup.e_dram_str` pJ  |
| **Interconnect**     |   NVLink 4.0   | ~`{python} AppendixMachineSetup.nvlink_ns_str` ns | `{python} AppendixMachineSetup.bw_nvlink_str` GB/s | ~`{python} AppendixMachineSetup.e_dram_str` pJ |
| **Host Link**        |   PCIe Gen5    |  ~`{python} AppendixMachineSetup.pcie_ns_str` ns  |  `{python} AppendixMachineSetup.bw_pcie_str` GB/s  | ~`{python} AppendixMachineSetup.e_dram_str` pJ |
| **System RAM**       |      DDR5      |                      ~100 ns                      |  `{python} AppendixMachineSetup.bw_dram_str` GB/s  | ~`{python} AppendixMachineSetup.e_dram_str` pJ |
| **Network (Fabric)** | InfiniBand NDR |   ~`{python} AppendixMachineSetup.ib_ns_str` ns   |  `{python} AppendixMachineSetup.bw_net_str` GB/s   |  `{python} AppendixMachineSetup.e_net_str` pJ  |
| **Storage (Local)**  |    NVMe SSD    |   ~`{python} AppendixMachineSetup.ssd_ns_str` ns  |  `{python} AppendixMachineSetup.bw_ssd_str` GB/s   |  `{python} AppendixMachineSetup.e_ssd_str` pJ  |
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
from mlsysim.core.constants import NETWORK_10G_BW
...
class BandwidthLatencySetup:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    bw_gbps_value = NETWORK_10G_BW.m_as(Gbps)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bw_gbps_str = fmt(bw_gbps_value, precision=1, allow_zero=True, commas=False)
    ping_ms_str = fmt(ping_ms_value, precision=1, allow_zero=True, commas=False)
    data_kb_str = fmt(data_kb_value, precision=1, allow_zero=True, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_math
from mlsysim.core.constants import NETWORK_10G_BW, BITS_PER_BYTE
...
class BandwidthLatencyExample:
    ...
    bw_gbps_value = NETWORK_10G_BW.m_as(Gbps) * BILLION
    ...
    large_data_gb_str = fmt(large_data_gb_value, precision=1, allow_zero=True, commas=False)
    large_tx_eq = fmt_math(f"{large_data_gb_value}\\text{{ GB}} \\times 8 / {int(bw_gbps_value/BILLION)}\\text{{ Gbps}} \\approx {int(large_tx_time_s_value*1000)}\\text{{ ms}}")
    large_total_eq = fmt_math(f"\\approx {int(ping_ms_value)}\\text{{ ms}} + {int(large_tx_time_s_value*1000)}\\text{{ ms}} = {int(total_large_time_ms_value)}\\text{{ ms}}")
```

*   **Latency-Bound (`{python} BandwidthLatencySetup.data_kb_str` KB Packet)**:
    *   Transmission: `{python} BandwidthLatencySetup.data_kb_str` KB $\times$ 8 / `{python} BandwidthLatencySetup.bw_gbps_str` Gbps ≈ `{python} BandwidthLatencyExample.tx_time_us_str` μs.
...
Even experienced engineers fall into traps when reasoning about hardware performance, especially when scaling and precision trade-offs look deceptively simple.

::: {.callout-warning title="Fallacy: Doubling accelerators halves training time"}

This assumes perfect strong scaling (Amdahl's Law). In practice, communication overhead (all-reduce) grows with $N_{\text{GPU}}$, and batch size constraints may limit parallelism. At large scale, you often hit diminishing returns unless you also scale the problem size (weak scaling).
...
For deep learning, FP32 often hurts performance without improving convergence. It consumes 2$\times$ memory bandwidth compared to BF16 and about `{python} NumbersToKnow.fp32_vs_fp16_str`$\times$ the arithmetic energy of FP16-class compute in the reference table above. Since neural networks are resilient to noise, the extra mantissa bits in FP32 are often modeling random variance rather than signal.
