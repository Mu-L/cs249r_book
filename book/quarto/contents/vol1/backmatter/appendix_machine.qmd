...
class LittlesLawExample:
    ...
    total_mem_gb_str = fmt(total_mem_gb_value, precision=0, commas=False)
...
This directly determines how to size inference worker pools. If serving one request requires `{python} LittlesLawExample.mem_per_req_gb_str` GB of temporary memory (KV cache, activations), handling `{python} LittlesLawExample.L_concurrent_str` concurrent requests requires `{python} LittlesLawExample.total_mem_gb_str` GB of memory. If the accelerator only has `{python} LittlesLawExample.gpu_mem_gb_str` GB, the system is physically limited to `{python} LittlesLawExample.max_concurrent_str` concurrent requests. Maximum throughput is capped at `{python} LittlesLawExample.max_throughput_eq`, regardless of how many requests arrive.
...
class BandwidthLatencySetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bw_gbps_str = fmt(bw_gbps_value, precision=0, commas=False, suffix=" Gbps")
    ping_ms_str = fmt(ping_ms_value, precision=0, commas=False)
    data_kb_str = fmt(data_kb_value, precision=0, commas=False)
```

Consider sending data over a `{python} BandwidthLatencySetup.bw_gbps_str` link with `{python} BandwidthLatencySetup.ping_ms_str` ms ping (latency). The dominant bottleneck depends entirely on the transfer size:
...
*   **Latency-Bound (`{python} BandwidthLatencySetup.data_kb_str` KB Packet)**:
    *   Transmission: `{python} BandwidthLatencySetup.data_kb_str` KB $\times$ 8 / `{python} BandwidthLatencySetup.bw_gbps_str` ≈ `{python} BandwidthLatencyExample.tx_time_us_str` μs.
