...
: A memory-efficient storage format for sparse matrices that uses three arrays (values, column indices, row pointers) to store only nonzero elements, reducing memory from $O(N^2)$ (for an $N{\times}N$ matrix) to $O(K + N)$ where $K$ is the number of nonzeros.
