...
**backward pass**
: The training phase that propagates gradients from the loss back through the network to compute parameter updates, pairing with the forward pass.
...
**carbon intensity**
: The emissions produced per unit of energy, usually measured as kg CO2e per kWh, used to convert ML energy use into carbon impact.

**cerebras wafer-scale engine**
: A single-wafer processor containing 2.6 trillion transistors and 850,000 cores, designed to eliminate inter-device communication bottlenecks in large-scale machine learning training.
...
: A memory-efficient storage format for sparse matrices that uses three arrays (values, column indices, row pointers) to store only nonzero elements, reducing memory from $O(N^2)$ (for an $N{\times}N$ matrix) to $O(K)$ where $K$ is the number of nonzeros.
...
: The integration of multiple ML paradigms such as cloud, edge, mobile, and tiny ML to form unified distributed systems that combine complementary strengths.
...
: A data transformation that produces the same output when repeated on the same input, supporting safe retries and reprocessing.
...
**int8**
: 8-bit integer numerical representation used in quantized neural networks, where model weights and activations are represented using 8-bit integers instead of 32-bit floating point, reducing memory usage by roughly 4$\times$ and accelerating inference on specialized hardware while attempting to maintain model accuracy.
...
: A quantitative framework decomposing ML system performance into three terms: Data (limited by bandwidth), Compute (limited by FLOPS), and Latency (limited by overhead). Formulated as $T = D_{\text{vol}}/\text{BW} + O/(R_{\text{peak}} \cdot \eta_{\text{hw}}) + L_{\text{lat}}$.
...
: Formal contract specifying minimum performance standards and uptime guarantees for production services, with penalties for noncompliance.
...
: Dedicated hardware for operations such as exponentials, roots, activations, and reductions that do not map cleanly to matrix multiply units.
...
: A mismatch between how features or data are computed during model training vs. serving in production, causing model performance to degrade despite unchanged code. Common causes include different preprocessing pipelines, feature computation timing, or data sources between training and inference
...
**transformer**
: A neural network architecture based entirely on attention mechanisms, eliminating recurrence and convolution while delivering substantial accuracy gains over RNN/CNN baselines across language, vision, and multimodal tasks.
...
: A theoretical result proving that neural networks with sufficient width and nonlinear activation functions can approximate any continuous function on a compact domain.
