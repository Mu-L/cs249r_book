...
class MobileNetJourneyMath:
    ...
    int8_fp32_reduction_str = fmt(int8_vs_fp32.m_as(""), precision=1, commas=False)
    int8_fp16_reduction_str = fmt(int8_vs_fp16.m_as(""), precision=1, commas=False)
```
...
class EnergyMovementMath:
    ...
    # OUTPUT
    dram_vs_fp32_str = fmt(dram_vs_fp32.m_as(""), precision=1, commas=False)
    dram_vs_fp16_str = fmt(dram_vs_fp16.m_as(""), precision=1, commas=False)
```
...
class ConclusionRoofline:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    llama_params_b = fmt(model.parameters.to("count").magnitude / BILLION, precision=1, commas=False)
    ...
    llama_dvol_gb_str = fmt(d_vol.m_as('GB'), precision=1, commas=False)
    llama_compute_gflops_str = fmt(compute_req.m_as('GFLOPs'), precision=1, commas=False)

    h100_bw_tb_str = fmt(gpu.memory_bw.m_as('TB/s'), precision=2, commas=False)
    h100_peak_tflops_str = fmt(gpu.peak_flops.m_as('TFLOP/s'), precision=1, commas=False)

    t_mem_ms_str = fmt(t_mem.m_as('ms'), precision=1, commas=False)
    t_comp_ms_str = fmt(t_comp.m_as('ms'), precision=2, commas=False)
    ratio_str = fmt(ratio.m_as(''), precision=1, commas=False)
...
class TailLatencyRatio:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    conclusion_tail_ratio_str = fmt(ratio, precision=1, commas=False)
```
...
class EdgeReferenceRatios:
    ...
    # OUTPUT
    int8_peak_ratio_str = fmt(int8_peak_ratio.m_as(""), precision=1, commas=False)
    power_ratio_str = fmt(power_ratio.m_as(""), precision=1, commas=False)
    memory_ratio_str = fmt(memory_ratio.m_as(""), precision=1, commas=False)
```
...
class FleetFailureMath:
    ...
    # OUTPUT
    gpu_count_str = fmt(gpu_count, precision=1)
    gpu_mtbf_years_str = fmt(gpu_mtbf_years, precision=1, commas=False)
    cluster_mtbf_hours_str = fmt(cluster_mtbf.m_as("hour"), precision=1, commas=False)
```
...
class AmdahlPitfallMath:
    ...
    # OUTPUT
    local_speedup_str = fmt(local_speedup, precision=1, commas=False)
    serial_percent_str = fmt(serial_fraction * 100, precision=1, commas=False)
