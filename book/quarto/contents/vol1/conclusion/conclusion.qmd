...
# │
# │ Imports: mlsysim.core.constants (H100_MEM_BW, H100_FLOPS_FP16_TENSOR, BYTES_FP16,
# │          flop, GFLOPs), mlsysim.book (fmt_frac, fmt_sci, fmt_math, md)
...
from mlsysim.fmt import fmt_math, check, fmt, MarkdownStr
...
| **Architecture (@sec-network-architectures)** | Algorithmic Efficiency         | **Depthwise Separable Convolutions**: about 8--9$\times$ fewer FLOPs than comparable standard convolution layers and roughly 14$\times$ fewer operations than ResNet-50 at ImageNet scale |
...
| **Compression (@sec-model-compression)**      | Navigating the Pareto Frontier | **INT8 Quantization**\index{INT8 Quantization!bandwidth trade-off}: 4$\times$ memory reduction with minimal accuracy loss (<1 percent)                                                    |
...
| \ref{pri-energy-movement}       | Energy-Movement Invariant   | III: Optimize  | $E_{\text{move}} \gg E_{\text{compute}}$ (100--1,000$\times$)                                              | Data locality, not raw FLOPS, drives efficiency                                |
