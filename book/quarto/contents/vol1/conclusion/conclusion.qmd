...
class ConclusionRoofline:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    llama_params_count_str = MarkdownStr("70 billion")
    llama_params_prose_str = MarkdownStr("70-billion-parameter")
    ...
    h100_peak_tflops_str = fmt(gpu.peak_flops.m_as('TFLOPs/second'), precision=0, commas=False)
    ...
    h100_peak_tflops_val = gpu.peak_flops.m_as('TFLOPs/second')
...
We can apply the iron law (Principle \ref{pri-iron-law}) and Arithmetic Intensity (Principle \ref{pri-arithmetic-intensity}) to a real-world problem: serving one token from a `{python} ConclusionRoofline.llama_params_prose_str` model (like a 70-billion-parameter Llama 2 model\index{Llama 2!70-billion-parameter memory-bound workload}) on an NVIDIA H100.

**Physics**:

- **Model-weight byte volume moved** $(D_{\text{vol}})$: `{python} ConclusionRoofline.llama_params_count_str` parameters $\times$ 2 bytes (FP16) = `{python} ConclusionRoofline.llama_dvol_gb_str` GB.
