...
::: {.callout-checkpoint title="Systems thinking"}
...
The insight that system boundaries define model capabilities has guided the exploration throughout this book. The journey from Part I's foundations through Part IV's production reality traced a deliberate arc, and each stage built on what came before.
...
from mlsysim.core.constants import BYTES_FP32, BYTES_FP16, BYTES_INT8
...
class MobileNetJourneyMath:
    ...
    # OUTPUT
    depthwise_reduction_str = fmt(standard_to_depthwise, precision=1, commas=False)
    resnet_ops_reduction_str = fmt(resnet_to_mobilenet.m_as(""), precision=1, commas=False)
    int8_fp32_reduction_str = fmt(int8_vs_fp32.m_as(""), precision=0, commas=False)
    int8_fp16_reduction_str = fmt(int8_vs_fp16.m_as(""), precision=0, commas=False)
```
...
| **Journey Phase**                             | **System Lens**                | **MobileNetV2 Implementation**\index{MobileNetV2!efficiency under constraint}                                                                                                                                                                                                    |
|:----------------------------------------------|:-------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| **Compression (@sec-model-compression)**      | Navigating the Pareto Frontier | **INT8 Quantization**\index{INT8 Quantization!bandwidth trade-off}: `{python} MobileNetJourneyMath.int8_fp32_reduction_str`× memory reduction versus FP32 (`{python} MobileNetJourneyMath.int8_fp16_reduction_str`× versus FP16), with accuracy revalidated per deployment       |
...
# │          ENERGY_FLOP_FP32_PJ, ENERGY_FLOP_FP16_PJ), mlsysim.fmt (fmt, check)
# │ Exports: EnergyMovementMath.dram_vs_fp32_str,
# │          EnergyMovementMath.dram_vs_fp16_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import ENERGY_DRAM_ACCESS_PJ, ENERGY_FLOP_FP32_PJ, ENERGY_FLOP_FP16_PJ
from mlsysim.fmt import fmt, check
...
class EnergyMovementMath:
    ...
    # OUTPUT
    dram_vs_fp32_str = fmt(dram_vs_fp32.m_as(""), precision=0, commas=False)
    dram_vs_fp16_str = fmt(dram_vs_fp16.m_as(""), precision=0, commas=False)
```
...
| \ref{pri-data-gravity}          | Data Gravity Invariant      | I: Foundations | $C_{\text{move}}(D) \gg C_{\text{move}}(\text{Compute})$                                                                                                                       | Move compute to data, not data to compute                                      |
...
| \ref{pri-arithmetic-intensity}  | Arithmetic Intensity Law    | III: Optimize  | $R = \min(R_{\text{peak}},\; I \times \text{BW})$                                                                                                                              | Adding compute to a memory-bound model yields zero gain                        |
| \ref{pri-energy-movement}       | Energy-Movement Invariant   | III: Optimize  | $E_{\text{move}} \gg E_{\text{compute}}$ (`{python} EnergyMovementMath.dram_vs_fp32_str`--`{python} EnergyMovementMath.dram_vs_fp16_str`× for DRAM access vs. FP32/FP16 FLOPs) | Data locality, not raw FLOPS, drives efficiency                                |
| \ref{pri-amdahl}                | Amdahl's Law                | III: Optimize  | $\text{Speedup} = \frac{1}{(1-p) + \frac{p}{s}}$                                                                                                                               | The serial fraction caps all parallelism gains                                 |
| \ref{pri-verification-gap}      | Verification Gap            | IV: Deploy     | $\Pr(f(X) \approx Y) > 1 - \epsilon$                                                                                                                                           | ML testing is statistical; it bounds error, not proves correctness             |
...
| \ref{pri-bias-feedback}         | Bias Feedback Invariant     | IV: Deploy     | $\Delta_g(k) \approx \Delta_g(0) \cdot \alpha^k$, $\alpha > 1$                                                                                                                 | Errors against subgroups compound across cycles when outputs reshape inputs    |
...
\index{Energy!movement invariant}
The four optimization invariants form a tightly coupled diagnostic chain. The Pareto frontier\index{Pareto Frontier!multi-objective optimization} (Principle \ref{pri-pareto-frontier}) establishes that no free improvements exist: quantization trades precision for bandwidth, pruning trades capacity for speed, and distillation trades training compute for inference efficiency. The arithmetic intensity law (Principle \ref{pri-arithmetic-intensity}) diagnoses which resource is the bottleneck, revealing whether optimization should target compute or memory. The energy-movement invariant (Principle \ref{pri-energy-movement}) explains why data locality dominates efficiency: in the book's reference constants, a DRAM access costs about `{python} EnergyMovementMath.dram_vs_fp32_str`--`{python} EnergyMovementMath.dram_vs_fp16_str`$\times$ as much energy as an FP32/FP16 arithmetic operation. Amdahl's Law (Principle \ref{pri-amdahl}) sets the ceiling on any parallelism gain, explaining why data loading and preprocessing become the ultimate bottlenecks in highly optimized systems.

MobileNetV2 (our Lighthouse from @sec-network-architectures) navigates all four simultaneously: depthwise separable convolutions reshape the Pareto frontier, quantization to INT8 exploits the arithmetic intensity law by fitting more operations per byte of bandwidth, and the resulting energy savings respect the energy-movement invariant while Amdahl's Law explains why the nonaccelerable preprocessing stage limits end-to-end speedup. The KWS Lighthouse pushes these trade-offs to their extreme, where sub-megabyte models on microcontrollers leave zero margin for waste on any axis.
...
::: {.callout-checkpoint title="Applying the invariants"}
...
# │ Show: That modern LLMs (Llama-2-70B) are about 295× memory-bound on H100 hardware.
...
from mlsysim.core.constants import BYTES_FP16, flop
...
class ConclusionRoofline:
    ...
    compute_req = model.parameters.to("count").magnitude * 2 * flop  # 2 FLOPs per param per token
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    llama_params_str = MarkdownStr("70B")
    llama_dvol_gb_str = fmt(d_vol.m_as('GB'), precision=0, commas=False)
    llama_compute_gflops_str = fmt(compute_req.m_as('GFLOPs'), precision=0, commas=False)

    h100_bw_tb_str = fmt(gpu.memory_bw.m_as('TB/s'), precision=2, commas=False)
    h100_peak_tflops_str = fmt(gpu.peak_flops.m_as('TFLOPs/s'), precision=0, commas=False)

    t_mem_ms_str = fmt(t_mem.m_as('ms'), precision=1, commas=False)
    t_comp_ms_str = fmt(t_comp.m_as('ms'), precision=2, commas=False)
    ratio_str = fmt(ratio.m_as(''), precision=0, commas=False)

    # LaTeX equations
    h100_bw_gb_val = gpu.memory_bw.m_as('GB/s')
    t_mem_eq = fmt_math(f"T_{{\\text{{mem}}}} = \\frac{{{llama_dvol_gb_str} \\text{{ GB}}}}{{{h100_bw_gb_val:.0f} \\text{{ GB/s}}}} \\approx {t_mem_ms_str} \\text{{ ms}}")

    h100_peak_tflops_val = gpu.peak_flops.m_as('TFLOPs/s')
    t_comp_eq = fmt_math(f"T_{{\\text{{comp}}}} = \\frac{{{llama_compute_gflops_str} \\times 10^9}}{{{h100_peak_tflops_val:.0f} \\times 10^{{12}}}} = {t_comp_ms_str} \\text{{ ms}}")
```

::: {.callout-perspective title="The cost of a token"}

We can apply the iron law (Principle \ref{pri-iron-law}) and Arithmetic Intensity (Principle \ref{pri-arithmetic-intensity}) to a real-world problem: serving one token from a `{python} ConclusionRoofline.llama_params_str` parameter model (like Llama-2-70B\index{Llama-2-70B!memory-bound workload}) on an NVIDIA H100.
...
- **Compute** $(O)$: $\approx 2 \times P$ per token = `{python} ConclusionRoofline.llama_compute_gflops_str` GFLOPs.
- **Hardware**: H100 with $\text{BW}$ = `{python} ConclusionRoofline.h100_bw_tb_str` TB/s, $R_{\text{peak}} \approx$ `{python} ConclusionRoofline.h100_peak_tflops_str` TFLOPs/s FP16.
...
The memory time $T_{\text{mem}}$ is `{python} ConclusionRoofline.ratio_str`$\times$ larger than compute time $T_{\text{comp}}$. The system is heavily memory-bound (arithmetic intensity $\approx$ 1). To honor the silicon contract, we must either increase arithmetic intensity (via batching users to reuse $D_{\text{vol}}$) or reduce data volume (via quantization to INT4). A systems engineer who optimizes compute kernels $(T_{\text{comp}})$ without addressing memory $(T_{\text{mem}})$ can improve only the `{python} ConclusionRoofline.t_comp_ms_str` ms compute term while leaving the `{python} ConclusionRoofline.t_mem_ms_str` ms memory term untouched.
...
# │ How:     Divide H100 INT8 FLOPS, TDP, and memory capacity by the
# │          corresponding mobile constants (TOPS, mobile TDP, smartphone RAM).
# │
# │ Imports: mlsysim (Hardware), mlsysim.core.constants (H100_FLOPS_INT8,
# │          MOBILE_NPU_TOPS_INT8, MOBILE_TDP_W, SMARTPHONE_RAM_GB),
...
from mlsysim.core.constants import H100_FLOPS_INT8, MOBILE_NPU_TOPS_INT8, MOBILE_TDP_W, SMARTPHONE_RAM_GB
from mlsysim.fmt import fmt, check

class EdgeReferenceRatios:
    # LOAD
    h100 = Hardware.H100
    mobile_int8_peak = MOBILE_NPU_TOPS_INT8
    mobile_power = MOBILE_TDP_W
    phone_ram = SMARTPHONE_RAM_GB

    # EXECUTE
    int8_peak_ratio = H100_FLOPS_INT8 / mobile_int8_peak
    power_ratio = h100.tdp / mobile_power
    memory_ratio = h100.memory_capacity / phone_ram
    ...
    # OUTPUT
    int8_peak_ratio_str = fmt(int8_peak_ratio.m_as(""), precision=0, commas=False)
    power_ratio_str = fmt(power_ratio.m_as(""), precision=0, commas=False)
    memory_ratio_str = fmt(memory_ratio.m_as(""), precision=0, commas=False)
```

\index{AI Democratization!edge deployment}
In contrast, mobile and edge systems face stringent power, memory, and latency constraints that demand sophisticated hardware-software co-design\index{Hardware-Software Co-design!edge deployment}. Efficient architectures introduced in @sec-network-architectures (such as depthwise separable convolutions and neural architecture search) combined with compression techniques from @sec-model-compression (such as quantization and pruning) enable deployment on devices where the book's reference mobile NPU has about `{python} EdgeReferenceRatios.int8_peak_ratio_str`$\times$ lower INT8 peak throughput, about `{python} EdgeReferenceRatios.memory_ratio_str`$\times$ less memory headroom, and about `{python} EdgeReferenceRatios.power_ratio_str`$\times$ smaller power envelope than an H100-class accelerator. Systems that cannot run on billions of edge devices cannot achieve global impact, making edge deployment essential for AI democratization[^fn-ai-democratization].
...
The golden age demands concrete engineering advances. Achieving exascale sustained throughput $(\geq 10^{18} \text{ FLOP/s})$ and beyond requires entirely new approaches to power delivery, cooling, interconnects, and software coordination, not merely faster chips. The analytical tools developed in this volume, applied to those challenges, are what equip engineers to navigate the regime ahead.
...
# │ How:     Convert GPU_MTTF_HOURS to years via HOURS_PER_DAY * DAYS_PER_YEAR;
...
# │          mlsysim.core.defaults (GPU_MTTF_HOURS),
# │          mlsysim.core.formulas (calc_mtbf_cluster),
# │          mlsysim.fmt (fmt, check)
# │ Exports: FleetFailureMath.gpu_count_str,
# │          FleetFailureMath.gpu_mtbf_years_str,
# │          FleetFailureMath.cluster_mtbf_hours_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import DAYS_PER_YEAR, HOURS_PER_DAY, ureg
...
class FleetFailureMath:
    # LOAD
    gpu_count = 1_000
    gpu_mtbf = float(GPU_MTTF_HOURS) * ureg.hour
...
The following errors arise from a common source: treating ML systems as decomposable into independent parts. Each fallacy assumes that optimizing one dimension, one metric, or one stage suffices; each pitfall shows the consequence when that assumption meets production reality.
...
# │
# │ Imports: mlsysim.core.formulas (calc_amdahls_speedup),
# │          mlsysim.fmt (fmt, check)
...
class AmdahlPitfallMath:
    ...
    # OUTPUT
    local_speedup_str = fmt(local_speedup, precision=0, commas=False)
    serial_percent_str = fmt(serial_fraction * 100, precision=0, commas=False)
    system_speedup_str = fmt(system_speedup, precision=1, commas=False)
```

Amdahl's Law (Principle \ref{pri-amdahl}) applies directly to end-to-end ML pipelines. Optimizing inference latency by `{python} AmdahlPitfallMath.local_speedup_str`$\times$ yields only `{python} AmdahlPitfallMath.system_speedup_str`$\times$ system speedup if data loading accounts for `{python} AmdahlPitfallMath.serial_percent_str` percent of end-to-end latency. The iron law of ML systems (Principle \ref{pri-iron-law}) decomposes execution time into data movement, computation, and latency terms precisely so that engineers can identify the dominant term before investing optimization effort. @sec-benchmarking formalized this diagnostic process through profiling methodologies that measure where time actually goes. Engineers who optimize without profiling are guessing, and Amdahl's Law is unforgiving of guesses that target the wrong term.
