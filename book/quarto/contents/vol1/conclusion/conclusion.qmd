...
| \ref{pri-arithmetic-intensity}  | Arithmetic Intensity Law    | III: Optimize  | $R_{\text{attainable}} = \min(R_{\text{peak}},\; \text{AI} \times \text{BW})$                                                                                                  | Adding compute to a memory-bound model yields zero gain                        |
...
| \ref{pri-verification-gap}      | Verification Gap            | IV: Deploy     | $\Pr(f(x) \approx y) > 1 - \epsilon_{\text{test}}$                                                                                                                             | ML testing is statistical; it bounds error, not proves correctness             |
