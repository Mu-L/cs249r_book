...
# │ Exports: DataPrepEffortIntro.data_prep_effort_low_str,
# │          DataPrepEffortIntro.data_prep_effort_high_str
# └─────────────────────────────────────────────────────────────────────────────
...
class DataPrepEffortIntro:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    data_prep_effort_low_str = fmt(data_prep_effort_low, precision=0, commas=False)
    data_prep_effort_high_str = fmt(data_prep_effort_high, precision=0, commas=False)
```

## Dataset Compilation {#sec-data-engineering-data-engineering-dataset-compilation-0496}

The workflow stages from @sec-ml-workflow establish the *when* and *why* of data preparation, showing that data work consumes `{python} DataPrepEffortIntro.data_prep_effort_low_str` to `{python} DataPrepEffortIntro.data_prep_effort_high_str` percent of ML project effort (as @fig-ds-time quantified) and represents the foundational "D" in the D·A·M taxonomy summarized in @tbl-dam-taxonomy. Executing those stages at scale, however, requires dedicated infrastructure. If the workflow is the plan, data engineering is the factory floor. This chapter uses Keyword Spotting (KWS) to demonstrate data engineering under extreme resource constraints, where every byte and operation matters.
...
# │
# │ Imports: mlsysim.core.constants (NETWORK_100G_BW, Infrastructure.Pricing.Cloud.EgressPerGB.rate,
# │              Infrastructure.Pricing.Cloud.TpuV4PerHour.rate, Gbps, GB, USD, hour, second, MILLION,
...
class DataGravity:
    ...
    egress_cost_gb = Infrastructure.Pricing.Cloud.EgressPerGB.rate.m_as(USD/GB)
    tpu_hourly_cost = Infrastructure.Pricing.Cloud.TpuV4PerHour.rate.m_as(USD/hour)
    ...
    transfer_hours_str = fmt_int(transfer_hours, commas=False)
    transfer_days_10g_str = fmt(transfer_days_10g, precision=1, commas=False)
    ...
    dataset_gb_str = fmt(dataset_gb, precision=0)
    network_gbps_str = fmt(network_bw.m_as(Gbps), precision=0, commas=False)
    egress_cost_per_gb_str = fmt(egress_cost_gb, precision=2, commas=False)
    tpu_cost_per_hour_str = fmt(tpu_hourly_cost, precision=0, commas=False)
```
...
class FeedingProblem:
    ...
    feeding_tax_pct_str = fmt(feeding_tax_pct, precision=1, commas=False)
    img_per_sec_str = fmt_int(img_per_sec, commas=True)
```
...
# │
# │ Imports: mlsysim.core.constants (ENERGY_*), mlsysim.fmt (fmt, fmt_int, check)
# │ Exports: EnergyAccessCosts.energy_flop_fp32_str, EnergyAccessCosts.energy_dram_str,
# │          EnergyAccessCosts.dram_multiplier_str, EnergyAccessCosts.local_ssd_multiplier_str,
# │          EnergyAccessCosts.network_multiplier_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check
...
class EnergyAccessCosts:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    prune_pct_str = fmt(prune_pct, precision=0, commas=False)
    ...
    dram_multiplier_str = fmt_int(energy_dram_pj / energy_flop_fp32_pj)
    local_ssd_multiplier_str = fmt(energy_local_ssd_pj/energy_flop_fp32_pj_per_bit, precision=1, commas=True)
    network_multiplier_str = fmt(energy_network_pj/energy_flop_fp32_pj_per_bit, precision=1, commas=True)
```
...
|:-----------------------------------|:----------------------------------------------------:|:-------------------------------------------------------------:|
| **32-bit Floating Point MAC**      | `{python} EnergyAccessCosts.energy_flop_fp32_str` pJ |                           1$\times$                           |
| **DRAM Memory Access (32-bit)**    | **`{python} EnergyAccessCosts.energy_dram_str` pJ**  |  **`{python} EnergyAccessCosts.dram_multiplier_str`$\times$** |
| **Local SSD Access (per bit)**     |                      ~10,000 pJ                      | `{python} EnergyAccessCosts.local_ssd_multiplier_str`$\times$ |
| **Network Transfer (Data Center)** |                      ~50,000 pJ                      |  `{python} EnergyAccessCosts.network_multiplier_str`$\times$  |
...
# │
# │ Imports: mlsysim.fmt (fmt, check)
# │ Exports: KWSProblemTargets.kws_accuracy_target_str,
# │          KWSProblemTargets.kws_latency_limit_ms_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class KWSProblemTargets:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    kws_accuracy_target_str = fmt(kws_accuracy_target, precision=0, commas=False)
    kws_latency_limit_ms_str = fmt(kws_latency_limit_ms, precision=0, commas=False)
```

The core problem is deceptively simple: detect specific keywords amidst ambient sounds and other spoken words, with high accuracy, low latency, and minimal false activations, on devices with severely limited computational resources. A well-specified problem definition identifies the desired keywords, the envisioned application, and the deployment scenario. The objectives that follow must balance competing requirements: performance targets of `{python} KWSProblemTargets.kws_accuracy_target_str` percent accuracy in keyword detection with latency under `{python} KWSProblemTargets.kws_latency_limit_ms_str` ms, alongside resource constraints demanding minimal power consumption and model sizes optimized for available device memory.
...
class FalsePositiveTarget:
    ...
    windows_per_month_str = fmt(windows_per_month, precision=0)
...
- **Duty Cycle**: Always-on (`{python} FalsePositiveTarget.hr_str` hours/day).
...
class BudgetAllocation:
    ...
    labeling_k_str = fmt(labeling_budget/THOUSAND, precision=0, commas=False)
    ...
    labeled_k_str = fmt(labeled_examples/THOUSAND, precision=0, commas=False)
    labeling_pct_str = fmt(labeling_pct * 100, precision=0, commas=False)
    storage_pct_str = fmt(storage_pct * 100, precision=0, commas=False)
    governance_pct_str = fmt(governance_pct * 100, precision=0, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (THOUSAND), mlsysim.fmt (fmt, check)
# │ Exports: KWSDesignConstraints.kws_accuracy_target_str,
# │          KWSDesignConstraints.kws_false_wake_month_str,
# │          KWSDesignConstraints.kws_budget_str,
# │          KWSDesignConstraints.kws_memory_limit_design_kb_str,
# │          KWSDesignConstraints.kws_timeline_months_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class KWSDesignConstraints:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    kws_accuracy_target_str = fmt(kws_accuracy_target, precision=0, commas=False)
    kws_false_wake_month_str = fmt(kws_false_wake_month, precision=0, commas=False)
    kws_budget_str = fmt(kws_budget/THOUSAND, precision=0, commas=False) + "K"
    kws_memory_limit_design_kb_str = fmt(kws_memory_limit_design_kb, precision=0, commas=False)
    kws_timeline_months_str = fmt(kws_timeline_months, precision=0, commas=False)
    kws_base_model_acc_str = fmt(kws_base_model_acc, precision=0, commas=False)
```
...
- **Target**: `{python} KWSDesignConstraints.kws_accuracy_target_str` percent accuracy, < `{python} KWSDesignConstraints.kws_false_wake_month_str` false wake per month
- **Budget**: \$`{python} KWSDesignConstraints.kws_budget_str` total data engineering budget
- **Memory**: `{python} KWSDesignConstraints.kws_memory_limit_design_kb_str` KB model size limit (always-on island)
- **Timeline**: `{python} KWSDesignConstraints.kws_timeline_months_str` months to production
...
- **Labeling** (~`{python} BudgetAllocation.labeling_pct_str` percent): `{python} BudgetAllocation.labeling_k_str` K available
- **Storage/Processing** (~`{python} BudgetAllocation.storage_pct_str` percent): `{python} BudgetAllocation.storage_k_str` K
- **Governance/Other** (~`{python} BudgetAllocation.governance_pct_str` percent): `{python} BudgetAllocation.governance_k_str` K
...
- Base model: ~`{python} KWSDesignConstraints.kws_base_model_acc_str` percent (minimal data)

```{=latex}
\tcbbreak
```
- Partial data-volume gain from 750K real examples
- Need: higher sampling rate plus synthetic/noisy augmentation to reach the `{python} KWSDesignConstraints.kws_accuracy_target_str` percent target
...
: **KWS Final Configuration**: Budget-optimized data engineering choices that satisfy the four-pillar constraints derived in @tbl-kws-design-space, trading higher sampling and synthetic augmentation against fixed memory and labeling budgets to reach an estimated 97--99 percent accuracy range around the `{python} KWSDesignConstraints.kws_accuracy_target_str` percent target. {#tbl-data-engineering-kws-configuration tbl-colwidths="[26,26,48]"}

**Projected Outcome**: 97–99 percent estimated accuracy, straddling the `{python} KWSDesignConstraints.kws_accuracy_target_str` percent target, within the \$`{python} KWSDesignConstraints.kws_budget_str` budget, and a model footprint under the `{python} KWSDesignConstraints.kws_memory_limit_design_kb_str` KB limit.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: data-engineering-cost-constants-calc
# ┌─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.constants (LABELING_COST_*, Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate, USD),
# │          mlsysim.fmt (fmt, check)
# │ Exports: DataEngineeringCostConstants.labeling_cost_*_str,
# │          DataEngineeringCostConstants.storage_cost_s3_str,
# │          DataEngineeringCostConstants.retrieval_cost_glacier_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class DataEngineeringCostConstants:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    labeling_cost_crowd_low = Infrastructure.Pricing.Labeling.CrowdLow.rate.m_as(USD)
    labeling_cost_crowd_high = Infrastructure.Pricing.Labeling.CrowdHigh.rate.m_as(USD)
    labeling_cost_box_low = Infrastructure.Pricing.Labeling.BoxLow.rate.m_as(USD)
    labeling_cost_box_high = Infrastructure.Pricing.Labeling.BoxHigh.rate.m_as(USD)
    labeling_cost_medical_low = Infrastructure.Pricing.Labeling.MedicalLow.rate.m_as(USD)
    labeling_cost_medical_high = Infrastructure.Pricing.Labeling.MedicalHigh.rate.m_as(USD)
    storage_cost_s3 = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate.m_as(USD/TB/ureg.month)
    retrieval_cost_glacier = Infrastructure.Pricing.Storage.GlacierRetrievalPerGB.rate.m_as(USD/GB)
...
The pattern that emerges in @tbl-data-engineering-cost-constants is that labeling consistently dominates storage and compute costs, so teams should reason from cost ratios rather than isolated prices:

| **Operation**                |                                                                    **Cost**                                                                    | **Notes**              |
|:-----------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------|
| **Crowdsourced image label** |   \$`{python} DataEngineeringCostConstants.labeling_cost_crowd_low_str`–`{python} DataEngineeringCostConstants.labeling_cost_crowd_high_str`   | Simple classification  |
| **Bounding box annotation**  |     \$`{python} DataEngineeringCostConstants.labeling_cost_box_low_str`–`{python} DataEngineeringCostConstants.labeling_cost_box_high_str`     | Per box, simple scenes |
| **Expert medical label**     | \$`{python} DataEngineeringCostConstants.labeling_cost_medical_low_str`–`{python} DataEngineeringCostConstants.labeling_cost_medical_high_str` | Per study, radiologist |
| **S3 storage (Standard)**    |                                     \$`{python} DataEngineeringCostConstants.storage_cost_s3_str`/TB/month                                     | Hot storage            |
| **S3 retrieval (Glacier)**   |                                    \$`{python} DataEngineeringCostConstants.retrieval_cost_glacier_str`/GB                                     | Standard: 3-5 hours    |
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate, THOUSAND, USD,
...
class LabelingComputeRatio:
    ...
    train_hours_low = 4
    train_hours_high = 6
    gpu_hour_cost_low = Infrastructure.Pricing.Fleet.GpuHourRef.rate.m_as(USD/hour)
    gpu_hour_cost_high = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    labeling_budget_str = fmt(labeling_budget/THOUSAND, precision=0, commas=False) + "K"
    train_cost_low_str = fmt(train_cost_low, precision=0, commas=False)
    train_cost_high_str = fmt(train_cost_high, precision=0, commas=False)
    ratio_low_str = fmt(ratio_low, precision=1, commas=True)
    ratio_high_str = fmt(ratio_high, precision=1, commas=True)
    effort_majority_pct = 80
    effort_minority_pct = 20
    effort_majority_pct_str = fmt(effort_majority_pct, precision=0, commas=False)
    effort_minority_pct_str = fmt(effort_minority_pct, precision=0, commas=False)
```
...
**The 80/20 Split**: `{python} LabelingComputeRatio.effort_majority_pct_str` percent of data engineering effort goes to `{python} LabelingComputeRatio.effort_minority_pct_str` percent of features: the "long tail" of edge cases, rare categories, and quality exceptions.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: glacier-retrieval-pricing-calc
# ┌─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Storage.GlacierRetrievalPerGB.rate, USD), mlsysim.fmt (fmt, check)
# │ Exports: GlacierRetrievalPricing.retrieval_cost_glacier_str,
# │          GlacierRetrievalPricing.retrieval_cost_glacier_expedited_str,
# │          GlacierRetrievalPricing.retrieval_ratio_expedited_standard_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class KWSAcquisitionScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt_int(round(dataset_size_m))
    languages_str = fmt(languages, precision=0, commas=False)
```
...
# │
# │ Imports: math (sqrt), mlsysim.constants (Monitoring.KsTestCoefficient),
# │          mlsysim.book (fmt)
...
class KSTest:
    ...
    n_train = 1000
    n_serving = 1000
    coeff = Monitoring.KsTestCoefficient # 1.36 for alpha=0.05
...
class CohensKappaChance:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    chance_agreement_pct_str = fmt(chance_agreement * 100, precision=0, commas=False)
```
...
class RealtimeCost:
    ...
    hourly_tb_str = fmt_int(hourly_data_tb, commas=False)
    ...
    events_per_sec_str = fmt(events_per_sec/MILLION, precision=0, commas=False, suffix="M")
    ...
    batch_window_min_str = fmt(batch_window_min, precision=0, commas=False)
    batch_core_hours_str = fmt(batch_core_hours, precision=0, commas=False)
    batch_window_hr_str = fmt(1, precision=0, commas=False)
...
3.  **Batch requirements**: Process `{python} RealtimeCost.hourly_tb_str` TB (`{python} RealtimeCost.batch_window_hr_str` hour data) in `{python} RealtimeCost.batch_window_min_str` mins. High throughput (sequential I/O) is efficient. The batch design needs `{python} RealtimeCost.batch_cores_str` cores for `{python} RealtimeCost.batch_window_min_str` mins/hour $\times$ `{python} RealtimeCost.hours_per_day_str` hours = `{python} RealtimeCost.batch_core_hours_str` core-hours/day. Cost: `{python} RealtimeCost.batch_core_hours_str` $\times$ USD `{python} RealtimeCost.stream_cost_per_hr_str` = **USD `{python} RealtimeCost.batch_cost_str`/day**.
...
class EtlEltCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    etl_spark_str = fmt(etl_spark_daily, precision=0, commas=False)
    etl_storage_str = fmt(etl_storage_mo, precision=0, commas=False)
    elt_storage_tb_str = fmt(elt_storage_tb, precision=0, commas=False)
    elt_storage_str = fmt(elt_storage_mo, precision=0)
    elt_query_str = fmt(elt_query_mo, precision=0, commas=False)
    savings_str = fmt(storage_savings_mo, precision=0)
    etl_spark_monthly_str = fmt(etl_spark_monthly, precision=0)
    etl_total_str = fmt(etl_total_mo, precision=0)
    elt_total_str = fmt(elt_total_mo, precision=0)
    total_savings_str = fmt(total_savings_mo, precision=0)
    ...
    etl_total_storage_tb_str = fmt(etl_storage_tb, precision=0, commas=False)
    ...
    schema_change_ratio_str = fmt(schema_change_ratio, precision=0, commas=False)
    schema_change_etl_hours_str = fmt(schema_change_etl_hours, precision=0, commas=False)
    schema_change_elt_minutes_str = fmt(schema_change_elt_minutes, precision=0, commas=False)
```
...
1.  **ETL approach**: Transform before loading. Compute all three aggregation windows in a Spark cluster before loading into the warehouse.
    ...
    *   Schema change cost: Re-run full pipeline (~`{python} EtlEltCost.schema_change_etl_hours_str` hours) per change
...
2.  **ELT approach**: Load raw data first, transform in warehouse.
    ...
    *   Schema change cost: Rewrite SQL query (~`{python} EtlEltCost.schema_change_elt_minutes_str` minutes) per change
...
class KWSQualityThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    snr_threshold_db_str = fmt(snr_threshold_db, precision=0, commas=False)
    sample_duration_ms_low_str = fmt(sample_duration_ms_low, precision=0, commas=False)
    sample_duration_ms_high_str = fmt(sample_duration_ms_high, precision=0, commas=False)
```
...
class KWSScalabilityTargets:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt_int(round(dataset_size_m))
    memory_limit_edge_kb_str = fmt(memory_limit_edge_kb, precision=0, commas=False)
    model_size_limit_kb_str = fmt(model_size_limit_kb, precision=0, commas=False)
```
...
class ActiveLearningBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_images_str = fmt(dataset_images / MILLION, precision=0, commas=False, suffix="M")
    label_budget_str = fmt(label_budget / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    ...
    random_labels_str = fmt(random_labels_needed / MILLION, precision=0, commas=False, suffix="M")
    random_label_cost_str = fmt(random_label_cost / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    active_labels_low_str = fmt(active_labels_low / THOUSAND, precision=0, commas=False, suffix="K")
    active_labels_high_str = fmt(active_labels_high / THOUSAND, precision=0, commas=False, suffix="K")
    active_label_cost_low_str = fmt(active_label_cost_low / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    active_label_cost_high_str = fmt(active_label_cost_high / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    full_pool_inference_cost_str = fmt(full_pool_inference_cost / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    active_total_low_str = fmt(active_total_low / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    active_total_high_str = fmt(active_total_high / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    compute_budget_pct_str = fmt(compute_budget_fraction * 100, precision=0, commas=False)
    compute_budget_str = fmt(compute_budget / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    candidate_pool_str = fmt(candidate_pool_images / THOUSAND, precision=0, commas=False, suffix="K")
    affordable_labels_str = fmt(affordable_labels_after_compute / THOUSAND, precision=0, commas=False, suffix="K")
    baseline_acc_str = fmt(baseline_acc, precision=0, commas=False)
    target_acc_str = fmt(target_acc, precision=0, commas=False)
```
...
**Problem**: A `{python} ActiveLearningBudget.dataset_images_str` image dataset has a `{python} ActiveLearningBudget.label_budget_str` labeling budget. Random sampling achieves `{python} ActiveLearningBudget.baseline_acc_str` percent accuracy with `{python} ActiveLearningBudget.active_labels_low_str` images, while the target is `{python} ActiveLearningBudget.target_acc_str` percent accuracy.
...
3.  **The multiplier**:
    *   **Random**: Reaching `{python} ActiveLearningBudget.target_acc_str` percent may require `{python} ActiveLearningBudget.random_labels_str` labels (`{python} ActiveLearningBudget.random_label_cost_str`). **Budget Exceeded.**
...
class KWSAutomatedLabelingScale:
    ...
    dataset_size_m_round_str = fmt_int(round(dataset_size_m))
    label_hours_str = fmt(label_hours, precision=0, commas=True)
    label_years_str = fmt(label_years, precision=1, commas=False)
    accuracy_target_str = fmt(accuracy_target, precision=0, commas=False)
    languages_str = fmt(languages, precision=0, commas=False)
    sample_duration_ms_low_str = fmt(sample_duration_ms_low, precision=0, commas=False)
    sample_duration_ms_high_str = fmt(sample_duration_ms_high, precision=0, commas=False)
```
...
# │
# │ Imports: Infrastructure.Pricing.Storage.*, mlsysim.core.constants (GB, TB, USD, ureg, byte, MILLION),
# │          mlsysim.book (fmt, check)
...
class KWSStorageEconomics:
    ...
    bytes_per_sample = 2
    storage_cost_s3 = Infrastructure.Pricing.Storage.S3StandardPerTbMonth.rate.m_as(USD/TB/ureg.month)
    storage_cost_nvme_low = Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as(USD/TB/ureg.month)
    storage_cost_nvme_high = Infrastructure.Pricing.Storage.NvmeHighPerTbMonth.rate.m_as(USD/TB/ureg.month)
    nvme_bw_gbs = 5
    obj_bw_gbs = 0.1
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_gb_str = fmt(dataset_size_gb, precision=0, commas=False)
    storage_s3_cost_str = fmt(storage_s3_cost, precision=1, commas=False)
    storage_nvme_low_cost_str = fmt(storage_nvme_low_cost, precision=1, commas=False)
    storage_nvme_high_cost_str = fmt(storage_nvme_high_cost, precision=1, commas=False)
...
# │          StorageLoading.load_speedup_str, StorageLoading.nvme_bw_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class StorageLoading:
    ...
    sample_count = rounded_sample_count_m * MILLION
    dataset_size_bytes = sample_count * sample_duration_s * sample_rate_hz * bytes_per_sample
    kws_dataset_gb = (dataset_size_bytes * byte).m_as(GB)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvme_load_str = fmt(nvme_load_s, precision=1, commas=False)
...
class ModelStorageScale:
    ...
    alexnet_params_str = fmt(alexnet_params_m, precision=0, commas=False)
    model_growth_str = fmt(model_growth_factor, precision=0)
```
...
class StorageBandwidth:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    max_img_per_sec_str = fmt(max_img_per_sec, precision=0)
    required_bw_gbs_str = fmt(required_bw_gbs, precision=1, commas=False)
    a100_flops_t_str = fmt_int(h_a100.peak_flops.m_as(TFLOPs/second), commas=False)
    resnet_flops_g_str = fmt_int((resnet_flops_per_img * flop).m_as(GFLOPs), commas=False)
    image_size_kb_str = fmt(image_size_kb, precision=0, commas=False)
    sata_bw_mbs_str = fmt(sata_bw_mbs, precision=0, commas=False)
    sata_max_img_sec_str = fmt(sata_max_img_sec, precision=0)
    sata_utilization_pct_str = fmt(sata_utilization_pct, precision=0, commas=False)
    s3_bw_mbs_str = fmt(s3_bw_mbs, precision=0, commas=False)
    s3_workers_str = fmt(s3_workers, precision=0, commas=False)
    nvme_bw_low_gbs_str = fmt(nvme_bw_low_gbs, precision=0, commas=False)
    nvme_bw_high_gbs_str = fmt(nvme_bw_high_gbs, precision=0, commas=False)
```
...
*   **NVMe SSD**\index{NVMe SSD!data pipeline}: approximately `{python} StorageBandwidth.nvme_bw_low_gbs_str`–`{python} StorageBandwidth.nvme_bw_high_gbs_str` GB/s. **Required.**
...
class CompressionTradeoff:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    decompress_speedup_str = fmt(decompress_speedup, precision=1, commas=False)
    gzip_min_str = fmt(gzip_decompress_min, precision=1, commas=False)
    snappy_min_str = fmt(snappy_decompress_min, precision=1, commas=False)
    time_diff_str = fmt(time_diff_min, precision=1, commas=False)
    total_diff_hours_str = fmt_int(total_diff_hours, commas=False)
    snappy_decompress_mbs_str = fmt(snappy_decompress_mbs, precision=0, commas=False)
    gzip_decompress_mbs_str = fmt(gzip_decompress_mbs, precision=0, commas=False)
    compress_dataset_gb_str = fmt(dataset_gb, precision=0, commas=False)
...
class LifecycleStorageRequirements:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    train_img_per_sec_str = fmt(train_img_per_sec, precision=0, commas=True)
    image_size_kb_str = fmt(image_size_kb, precision=0, commas=False)
    training_bw_gbs_str = fmt(training_bw_gbs, precision=0, commas=False)
    storage_reduction_str = fmt(storage_reduction, precision=0, commas=False)
    feature_vector_kb_str = fmt(feature_vector_kb, precision=0, commas=False)
    serving_rps_str = fmt(serving_rps, precision=0, commas=True)
    feature_reads_per_request_str = fmt(feature_reads_per_request, precision=0, commas=False)
    random_reads_per_sec_str = fmt(random_reads_per_sec, precision=0, commas=True)
```
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate, MILLION,
...
class DataEngineeringSummaryRecap:
    ...
    sample_count = rounded_sample_count_m * MILLION
    dataset_size_bytes = sample_count * sample_duration_s * sample_rate_hz * bytes_per_sample
    kws_dataset_size_gb = (dataset_size_bytes * byte).m_as(GB)
    train_cost_low = a100_count * train_hours_low * gpu_hour_cost_low
    train_cost_high = a100_count * train_hours_high * gpu_hour_cost_high
    ratio_low = labeling_budget/train_cost_high
    ratio_high = labeling_budget/train_cost_low
    ...
    ratio_low_str = fmt(ratio_low, precision=1, commas=True)
    ratio_high_str = fmt(ratio_high, precision=1, commas=True)
    kws_dataset_size_m_round_str = fmt_int(round(kws_dataset_size_m))
    kws_dataset_size_gb_str = fmt(kws_dataset_size_gb, precision=0, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (MILLION, GB, byte), mlsysim.fmt (fmt, check)
# │ Exports: StorageLoadingRecap.load_speedup_str, StorageLoadingRecap.nvme_bw_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
* **Storage hierarchy determines iteration speed**: The `{python} StorageLoadingRecap.load_speedup_str`$\times$ throughput gap between local NVMe (`{python} StorageLoadingRecap.nvme_bw_gbs_str` GB/s) and cloud object storage (100 MB/s) determines whether iterations occur daily or weekly.
