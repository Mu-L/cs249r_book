...
# │ Exports: DataEngineeringSetup.energy_*,
# │          DataEngineeringSetup.kws_* objective values,
# │          DataEngineeringSetup.labeling_cost_*,
# │          DataEngineeringSetup.storage_cost_*, DataEngineeringSetup.retrieval_cost_*,
# │          DataEngineeringSetup.data_prep_*,
...
class DataEngineeringSetup:
    ...
    dram_multiplier_str = f"{int(energy_dram_pj / energy_flop_fp32_pj):,}"
...
::: {#def-data-engineering-data-engineering .callout-definition title="Data engineering"}
...
# │          DataGravity.transfer_time_10g_md
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
...
::: {#exm-data-engineering-pipeline-jungle .callout-example title="The pipeline jungle"}
...
# │          BudgetAllocation.labeling_k, BudgetAllocation.storage_k,
# │          BudgetAllocation.governance_k, BudgetAllocation.effective_cost_str,
# │          BudgetAllocation.labeled_k
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class BudgetAllocation:
    ...
    labeling_k = f"{labeling_budget/THOUSAND:.0f}"
    storage_k = f"{storage_budget/THOUSAND:.1f}"
    governance_k = f"{governance_budget/THOUSAND:.1f}"
    effective_cost_str = fmt(effective_cost, precision=2, commas=False)
    labeled_k = f"{labeled_examples/THOUSAND:.0f}"
```

::: {#exm-data-engineering-optimizing-kws-design-space .callout-example title="Optimizing the KWS design space"}
...
- **Labeling** (~60 percent): `{python} BudgetAllocation.labeling_k` K available
- **Storage/Processing** (~25 percent): `{python} BudgetAllocation.storage_k` K
- **Governance/Other** (~15 percent): `{python} BudgetAllocation.governance_k` K

At USD `{python} BudgetAllocation.cost_per_label_str`/label with `{python} BudgetAllocation.review_overhead_pct_str` percent review overhead: `{python} BudgetAllocation.labeling_k` K ÷ `{python} BudgetAllocation.effective_cost_str` = **`{python} BudgetAllocation.labeled_k` K labeled examples**
...
class KWSAcquisitionScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt(dataset_size_m, precision=0)
    languages_str = f"{languages}"
```
...
::: {#exm-data-engineering-detecting-drift-with-k-s-test .callout-example title="Detecting drift with K-S test"}
...
# │
# │ Imports: math (ceil), mlsysim.book (check)
...
# │          EtlEltCost.etl_storage_tb_str2, EtlEltCost.s3_per_tb_mo_str,
...
1.  **ETL approach**: Transform before loading. Compute all three aggregation windows in a Spark cluster before loading into the warehouse.
    ...
    *   Storage: `{python} EtlEltCost.etl_datasets_str` transformed datasets, ~`{python} EtlEltCost.etl_tb_each_str` TB each = `{python} EtlEltCost.etl_storage_tb_str2` TB at USD `{python} EtlEltCost.s3_per_tb_mo_str`/TB per month = USD `{python} EtlEltCost.etl_storage_str`/month
...
::: {#def-data-engineering-consistency-imperative .callout-definition title="The consistency imperative"}
...
class KWSAutomatedLabelingScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_str = f"{dataset_size_m}"
    dataset_size_m_round_str = fmt(dataset_size_m, precision=0)
...
class StorageBandwidth:
    ...
    resnet_flops_g_str = str(int((resnet_flops_per_img * flop).m_as(GFLOPs)))
...
::: {#def-data-engineering-feature-store .callout-definition title="Feature store"}
...
::: {#def-data-engineering-data-debt .callout-definition title="Data debt"}
