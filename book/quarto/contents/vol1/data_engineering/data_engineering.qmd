...
- Explain how data cascades propagate errors through the four pillars: quality, reliability, scalability, and governance
...
- Build labeling systems that balance accuracy, throughput, cost, and quality control
...
- Identify data debt categories and debug production pipeline failures
...
# │
# │ Goal: Centralize early chapter constants used by the running context.
# │ Show: A single source of truth for data-prep effort, energy ratios,
# │       KWS objectives, and early reference tables.
# │ How: Retrieve constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (*), mlsysim.fmt (fmt, fmt_sci, fmt_math, check),
# │          mlsysim (Hardware, Models), mlsysim.core.formulas (model_memory)
# │ Exports: DataEngineeringSetup.energy_*, DataEngineeringSetup.a100_*,
# │          DataEngineeringSetup.gpt3_*, DataEngineeringSetup.alexnet_*,
# │          DataEngineeringSetup.kws_*, DataEngineeringSetup.labeling_cost_*,
# │          DataEngineeringSetup.storage_cost_*, DataEngineeringSetup.retrieval_cost_*,
# │          DataEngineeringSetup.retrieval_ratio_*,
# │          DataEngineeringSetup.data_prep_*, DataEngineeringSetup.model_growth_*,
# │          DataEngineeringSetup.dram_*, DataEngineeringSetup.network_*,
# │          DataEngineeringSetup.transfer_days_10g_str,
# │          DataEngineeringSetup.transfer_time_10g_math
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Models
from mlsysim.fmt import fmt, fmt_sci, fmt_math, check
...
class DataEngineeringSetup:
    ...
    Scenario: Constants for early tables, KWS objectives, and physical invariants.
    """
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Energy
    energy_dram_str = fmt(energy_dram_pj, precision=1, allow_zero=True, commas=False)
    energy_flop_fp32_str = fmt(energy_flop_fp32_pj, precision=1, commas=False)
    dram_multiplier_str = fmt(int(ENERGY_DRAM_ACCESS_PJ.m_as(ureg.picojoule) / ENERGY_FLOP_FP32_PJ.m_as(ureg.picojoule/ureg.count)), precision=1, allow_zero=True)
    local_ssd_multiplier_str = fmt(energy_local_ssd_pj/energy_flop_fp32_pj_per_bit, precision=1, allow_zero=True, commas=True)
    network_multiplier_str = fmt(energy_network_pj/energy_flop_fp32_pj_per_bit, precision=1, allow_zero=True, commas=True)

    # Data Prep
    data_prep_effort_low_str = fmt(data_prep_effort_low, precision=1, allow_zero=True, commas=False)
    data_prep_effort_high_str = fmt(data_prep_effort_high, precision=1, allow_zero=True, commas=False)

    # KWS
    kws_accuracy_target_str = fmt(kws_accuracy_target, precision=1, allow_zero=True, commas=False)
    kws_latency_limit_ms_str = fmt(kws_latency_limit_ms, precision=1, allow_zero=True, commas=False)
    kws_memory_limit_design_kb_str = fmt(kws_memory_limit_design_kb, precision=1, allow_zero=True, commas=False)
    kws_false_wake_month_str = fmt(kws_false_wake_month, precision=1, allow_zero=True, commas=False)
    kws_budget_str = fmt(kws_budget/THOUSAND, precision=1, allow_zero=True, commas=False) + "K"
    kws_timeline_months_str = fmt(kws_timeline_months, precision=1, allow_zero=True, commas=False)
    ...
    labeling_cost_medical_low_str = fmt(labeling_cost_medical_low, precision=1, allow_zero=True, commas=False)
    labeling_cost_medical_high_str = fmt(labeling_cost_medical_high, precision=1, allow_zero=True, commas=False)

    # Storage
    storage_cost_s3_str = fmt(storage_cost_s3, precision=1, allow_zero=True, commas=False)
    retrieval_cost_glacier_str = fmt(retrieval_cost_glacier, precision=2, commas=False)
    retrieval_cost_glacier_expedited_str = fmt(retrieval_cost_glacier_expedited, precision=2, commas=False)
    retrieval_ratio_expedited_standard_str = fmt(retrieval_ratio_expedited_standard, precision=1, allow_zero=True, commas=False)
```

## Dataset Compilation {#sec-data-engineering-data-engineering-dataset-compilation-0496}

The workflow stages from @sec-ml-workflow establish the *when* and *why* of data preparation, showing that data work consumes `{python} DataEngineeringSetup.data_prep_effort_low_str` to `{python} DataEngineeringSetup.data_prep_effort_high_str` percent of ML project effort (as @fig-ds-time quantified) and represents the foundational "D" in the D·A·M taxonomy summarized in @tbl-dam-taxonomy. Executing those stages at scale, however, requires dedicated infrastructure. If the workflow is the plan, data engineering is the factory floor. This chapter uses Keyword Spotting (KWS) to demonstrate data engineering under extreme resource constraints, where every byte and operation matters.
...
2.  **Distinction (durable)**: Unlike **Data Science**\index{Data Science!vs. data engineering}, which focuses on **Inference and Insight**, Data Engineering addresses the **Scalability and Reliability** of the data pipeline.
...
# │
# │ Imports: mlsysim.core.constants (NETWORK_100G_BW, CLOUD_EGRESS_PER_GB,
# │              TPU_V4_PER_HOUR, Gbps, GB, USD, hour, second, MILLION,
# │              SEC_PER_HOUR, SEC_PER_DAY, BITS_PER_BYTE),
# │          mlsysim.fmt (fmt, fmt_math, check)
# │ Exports: DataGravity.transfer_seconds_str, DataGravity.transfer_hours_str,
# │          DataGravity.transfer_days_10g_str, DataGravity.transfer_cost_str,
# │          DataGravity.tpu_hours_str, DataGravity.network_gbs_str,
# │          DataGravity.dataset_gb_str, DataGravity.network_gbps_str,
# │          DataGravity.egress_cost_per_gb_str, DataGravity.tpu_cost_per_hour_str,
# │          DataGravity.transfer_time_10g_math
# └─────────────────────────────────────────────────────────────────────────────
...
class DataGravity:
    ...
    t_100g = (dataset_pb * ureg.petabyte) / network_bw
    transfer_seconds = t_100g.m_as(second)
    transfer_hours = t_100g.m_as(hour)

    # Step 2: 10 Gbps = 1.25 GB/s
    t_10g = (dataset_pb * ureg.petabyte) / (10 * Gbps)
    transfer_days_10g = t_10g.m_as(ureg.day)
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    transfer_seconds_str = fmt(transfer_seconds, precision=1, allow_zero=True, commas=True)
    transfer_hours_str = fmt(transfer_hours, precision=1, allow_zero=True, commas=False)
    transfer_days_10g_str = fmt(transfer_days_10g, precision=1, allow_zero=True, commas=False)
    transfer_time_10g_math = fmt_math(f"T = D_{{\\text{{vol}}}}/\\text{{BW}} \\approx {transfer_days_10g_str} \\text{{ days}}")
    transfer_cost_str = fmt(transfer_cost, precision=1, allow_zero=True, commas=True)
    tpu_hours_str = fmt(equiv_tpu_hours, precision=1, allow_zero=True, commas=True)
    network_gbs_str = fmt(network_bw.m_as(GB/second), precision=1, commas=False)
    dataset_gb_str = fmt(dataset_gb, precision=1, allow_zero=True)
    network_gbps_str = fmt(network_bw.m_as(Gbps), precision=1, allow_zero=True, commas=False)
...
# │ Exports: FeedingProblem.req_bw_gbs_str, FeedingProblem.disk_bw_mbs_str,
# │          FeedingProblem.feeding_tax_pct_str, FeedingProblem.img_per_sec_str
# └─────────────────────────────────────────────────────────────────────────────
...
class FeedingProblem:
    ...
    disk_bw_mbs_str = fmt(disk_bw_mbs, precision=1, allow_zero=True, commas=False)
    feeding_tax_pct_str = fmt(feeding_tax_pct, precision=1, allow_zero=True, commas=False)
    img_per_sec_str = fmt(img_per_sec, precision=1, allow_zero=True, commas=True)
```
...
The iron law in @sec-introduction-iron-law-ml-systems-c32a and the memory-wall analysis in @sec-hardware-acceleration-understanding-ai-memory-wall-3ea9 imply a data-engineering invariant: **moving a bit costs anywhere from a hundred to nearly a million times more energy than computing on it**, with the multiplier rising as the data leaves the chip. While @sec-model-compression examines the energy cost inside the processor, we must also consider the cost of the information flow from the external world. The table quantifies just how dramatic these differences are:

::: {tbl-colwidths="[34,33,33]"}

| **Operation**                      |                     **Energy (pJ)**                     |                        **Relative Cost**                         |
|:-----------------------------------|:-------------------------------------------------------:|:----------------------------------------------------------------:|
| **32-bit Floating Point MAC**      | `{python} DataEngineeringSetup.energy_flop_fp32_str` pJ |                            1$\times$                             |
| **DRAM Memory Access (32-bit)**    |  **`{python} DataEngineeringSetup.energy_dram_str` pJ** | **`{python} DataEngineeringSetup.dram_multiplier_str`$\times$**  |
| **Local SSD Access (per bit)**     |                        ~10,000 pJ                       | `{python} DataEngineeringSetup.local_ssd_multiplier_str`$\times$ |
| **Network Transfer (Data Center)** |                        ~50,000 pJ                       |  `{python} DataEngineeringSetup.network_multiplier_str`$\times$  |

: **Energy Cost of Moving vs. Computing on a Bit**: Approximate energy per operation at each stage of the data-movement hierarchy, with multipliers normalized to a single 32-bit MAC on a per-bit basis. The first two rows list energy per 32-bit access; the last two list energy per bit transferred. Moving a bit costs hundreds of times more energy than computing on it for DRAM-resident data, rising to hundreds of thousands of times more when the data must traverse a data-center network. {#tbl-energy-movement-cost}
...
- [ ] Can you explain the energy-movement invariant\index{Energy-Movement Invariant}: why moving a byte of data costs orders of magnitude more energy than processing it?
...
Machine learning systems face a unique failure pattern called **data cascades**\index{Data Cascade!definition}\index{Pipeline!cascading failures}[^fn-data-cascades-silent], where poor data quality in early stages amplifies throughout the entire pipeline, causing downstream model failures, project termination, and potential user harm [@sambasivan2021]. Traditional software produces immediate errors when encountering bad inputs. ML systems degrade silently until quality issues become severe enough to necessitate complete system rebuilds.
...
Data cascades occur when teams skip establishing clear quality criteria, reliability requirements, and governance principles before beginning data collection and processing. Preventing them requires a systematic framework that guides technical choices from data acquisition through production deployment. The cascade pattern becomes concrete when examining how a single upstream change propagates through a pipeline jungle without validation.
...
Data quality provides the foundation, as the cascade pattern in @sec-data-engineering-data-cascades-systematic-foundations-matter-2efe demonstrates: when upstream data is wrong, everything downstream fails. Quality encompasses three dimensions: correctness (are values accurate?), coverage (does the data represent the deployment environment?), and freshness (does the distribution still match production?). The quality pillar motivates validation, monitoring, and drift detection infrastructure examined throughout this chapter.
...
: **DLRM Scalability Profile**: Recommendation models like DLRM stress data engineering along memory capacity and sparse-access bandwidth rather than compute, distinguishing them from compute-bound vision models or bandwidth-bound language models and motivating embedding-table sharding as a first-class system concern. {#tbl-data-engineering-dlrm-scalability}
...
# │
# │ Imports: mlsysim.core.constants (SECONDS_PER_MINUTE, MINUTES_PER_HOUR,
# │              HOURS_PER_DAY, DAYS_PER_MONTH),
# │          mlsysim.book (fmt)
# │ Exports: FalsePositiveTarget.sec_str, FalsePositiveTarget.min_str,
# │          FalsePositiveTarget.hr_str, FalsePositiveTarget.day_str,
# │          FalsePositiveTarget.windows_per_month_str,
# │          FalsePositiveTarget.fpr_str, FalsePositiveTarget.rejection_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class FalsePositiveTarget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    sec_str = MarkdownStr("60")
    min_str = MarkdownStr("60")
    hr_str = MarkdownStr("24")
    day_str = fmt(days_month, precision=1, allow_zero=True, commas=False)
    windows_per_month_str = fmt(windows_per_month, precision=1, allow_zero=True)
...
- **Windows per Month**: `{python} FalsePositiveTarget.sec_str` $\times$ `{python} FalsePositiveTarget.min_str` $\times$ `{python} FalsePositiveTarget.hr_str` $\times$ `{python} FalsePositiveTarget.day_str` = `{python} FalsePositiveTarget.windows_per_month_str` windows.

**Required Accuracy**

- **False Positive Rate (FPR)**\index{False Positive Rate!always-on systems}: 1/windows ≈ `{python} FalsePositiveTarget.fpr_str`
- **Precision Requirement**: `{python} FalsePositiveTarget.rejection_pct_str` percent rejection of nonkeywords.
...
|:------------------------------------|:----------------------------|:-------------------------:|:-------------------------:|:------------------------:|
| **16 kHz vs. 8 kHz sampling**       | +2–4 percent accuracy       |     2$\times$ storage     |    2$\times$ processing   |  2$\times$ feature size  |
| **13 vs. 40 MFCC coefficients**     | +3–5 percent accuracy       | 3$\times$ feature compute |          Minimal          | 3$\times$ feature memory |
| **1M vs. 10M training examples**    | +5–8 percent accuracy       |  10$\times$ training time |  10$\times$ labeling cost |    10$\times$ storage    |
| **Clean vs. noisy training data**   | +10–15 percent real-world   |          Minimal          | 3$\times$ collection cost |         Minimal          |
...
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.core.constants (THOUSAND)
# │ Exports: BudgetAllocation.cost_per_label_str,
# │          BudgetAllocation.review_overhead_pct_str,
# │          BudgetAllocation.labeling_k_str, BudgetAllocation.storage_k_str,
# │          BudgetAllocation.governance_k_str, BudgetAllocation.effective_cost_str,
# │          BudgetAllocation.labeled_k_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class BudgetAllocation:
    ...
    review_overhead_pct_str = fmt(review_overhead_pct, precision=1, allow_zero=True, commas=False)
    labeling_k_str = fmt(labeling_budget/THOUSAND, precision=1, allow_zero=True, commas=False)
    storage_k_str = fmt(storage_budget/THOUSAND, precision=1, commas=False)
    governance_k_str = fmt(governance_budget/THOUSAND, precision=1, commas=False)
    effective_cost_str = fmt(effective_cost, precision=2, commas=False)
    labeled_k_str = fmt(labeled_examples/THOUSAND, precision=1, allow_zero=True, commas=False)
```
...
- **Target**: `{python} DataEngineeringSetup.kws_accuracy_target_str` percent accuracy, < `{python} DataEngineeringSetup.kws_false_wake_month_str` false wake per month
- **Budget**: \$`{python} DataEngineeringSetup.kws_budget_str` total data engineering budget
...
Allocating our \$150K budget across three cost categories (using the data engineering constants summarized later in this section):

- **Labeling** (~60 percent): `{python} BudgetAllocation.labeling_k_str` K available
- **Storage/Processing** (~25 percent): `{python} BudgetAllocation.storage_k_str` K
- **Governance/Other** (~15 percent): `{python} BudgetAllocation.governance_k_str` K

At USD `{python} BudgetAllocation.cost_per_label_str`/label with `{python} BudgetAllocation.review_overhead_pct_str` percent review overhead: `{python} BudgetAllocation.labeling_k_str` K ÷ `{python} BudgetAllocation.effective_cost_str` = **`{python} BudgetAllocation.labeled_k_str` K labeled examples**
...
@tbl-data-engineering-kws-configuration captures the budget-optimized choices that satisfy the four-pillar constraints:

::: {tbl-colwidths="[26,26,48]"}

| **Choice**            | **Selection**            | **Rationale**                                             |
|:----------------------|:-------------------------|:----------------------------------------------------------|
| **Sampling rate**     | 16 kHz                   | +3 percent accuracy worth 2$\times$ storage within budget |
...
| **Inference**         | Local, INT8 quantized    | Memory-constrained                                        |
| **Augmentation**      | Heavy synthetic          | 10$\times$ cost efficiency                                |

: **KWS Final Configuration**: Budget-optimized data engineering choices that satisfy the four-pillar constraints derived in @tbl-kws-design-space, trading higher sampling and synthetic augmentation against fixed memory and labeling budgets to reach an estimated 97--99 percent accuracy range around the `{python} DataEngineeringSetup.kws_accuracy_target_str` percent target. {#tbl-data-engineering-kws-configuration}

:::

**Projected Outcome**: 97–99 percent estimated accuracy, straddling the `{python} DataEngineeringSetup.kws_accuracy_target_str` percent target, within the \$`{python} DataEngineeringSetup.kws_budget_str` budget, and a model footprint under the `{python} DataEngineeringSetup.kws_memory_limit_design_kb_str` KB limit.
...
With optimal parameters selected from our design space, implementation requires combining multiple data collection approaches. Our KWS system demonstrates how these approaches work together across the project lifecycle. Preexisting datasets like Google's Speech Commands\index{Speech Commands} [@warden2018speech] provide a foundation for initial development, offering carefully curated voice samples for common wake words. For multilingual coverage, the Multilingual Spoken Words Corpus\index{Multilingual Spoken Words Corpus} (MSWC) [@mazumder2021multilingual] extends this foundation to 50 languages with over 23 million examples. However, even these large-scale datasets often lack diversity in accents, environments, and recording conditions, necessitating additional strategies.

To address coverage gaps, web scraping supplements baseline datasets by gathering diverse voice samples from video platforms and speech databases, capturing natural speech patterns and wake word variations. Crowdsourcing platforms like Amazon Mechanical Turk[^fn-mturk-labeling-scale] enable targeted collection of wake word samples across different demographics and environments [@sheng2019]. This approach is particularly valuable for underrepresented languages or specific acoustic conditions.

[^fn-mturk-labeling-scale]: **Amazon Mechanical Turk (MTurk)**: A crowdsourcing platform that routes tasks to workers matching specific demographic or environmental profiles, enabling targeted acquisition of wake word recordings from specific accents, age groups, or noisy environments that synthetic data cannot authentically reproduce. The trade-off is cost vs. validation overhead: raw MTurk collection runs 10--50$\times$ cheaper per sample than professional field recording, but each submission requires acoustic quality verification (SNR, duration, speaker proximity) to prevent low-quality recordings from degrading the KWS model's accuracy on precisely the underrepresented conditions the collection was designed to cover [@snow2008]. \index{Mechanical Turk!labeling scale}

Finally, synthetic data generation fills remaining gaps through speech synthesis [@werchniak2021] and audio augmentation, creating unlimited wake word variations across acoustic environments, speaker characteristics, and background conditions. The combination enables KWS systems that perform well across diverse real-world conditions.
...
::: {#psp-data-engineering-key-data-engineering-numbers .callout-perspective title="Key data engineering numbers"}

Just as systems engineers memorize latency numbers, ML engineers should internalize these data engineering constants:

@tbl-data-engineering-cost-constants lists the per-unit costs (2024 estimates) ML engineers should internalize:

::: {tbl-colwidths="[43,30,37]"}

| **Operation**                |                                                            **Cost**                                                            | **Notes**              |
|:-----------------------------|:------------------------------------------------------------------------------------------------------------------------------:|:-----------------------|
| **Crowdsourced image label** |   \$`{python} DataEngineeringSetup.labeling_cost_crowd_low_str`–`{python} DataEngineeringSetup.labeling_cost_crowd_high_str`   | Simple classification  |
| **Bounding box annotation**  |     \$`{python} DataEngineeringSetup.labeling_cost_box_low_str`–`{python} DataEngineeringSetup.labeling_cost_box_high_str`     | Per box, simple scenes |
| **Expert medical label**     | \$`{python} DataEngineeringSetup.labeling_cost_medical_low_str`–`{python} DataEngineeringSetup.labeling_cost_medical_high_str` | Per study, radiologist |
| **S3 storage (Standard)**    |                                 \$`{python} DataEngineeringSetup.storage_cost_s3_str`/TB/month                                 | Hot storage            |
| **S3 retrieval (Glacier)**   |                                \$`{python} DataEngineeringSetup.retrieval_cost_glacier_str`/GB                                 | Standard: 3-5 hours    |
| **GPU training hour (A100)** |                                                             \$2–4                                                              | Cloud spot pricing     |
| **Human review hour**        |                                                            \$15–50                                                             | Depending on expertise |

: **Data Engineering Cost Constants (2024)**: Per-unit costs that ML engineers should memorize in the same way systems engineers memorize latency numbers. Labeling commonly dominates storage and compute, so teams should reason from ratios rather than isolated prices. {#tbl-data-engineering-cost-constants}
...
# │
# │ Imports: math.log10, mlsysim.book (fmt, check)
# │ Exports: DataEngineeringTimeConstants.span_orders_str
# └─────────────────────────────────────────────────────────────────────────────
...
class DataEngineeringTimeConstants:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    span_orders_str = fmt(span_orders, precision=1, commas=False)
```

::: {tbl-colwidths="[40,34,36]"}
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: labeling-compute-ratio-calc
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.constants (CLOUD_GPU_TRAINING_PER_HOUR, THOUSAND, USD,
# │          hour), mlsysim.book (fmt, check)
# │ Exports: LabelingComputeRatio.labeling_budget_str,
# │          LabelingComputeRatio.train_cost_low_str,
# │          LabelingComputeRatio.train_cost_high_str,
# │          LabelingComputeRatio.ratio_low_str,
# │          LabelingComputeRatio.ratio_high_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class LabelingComputeRatio:
    ...
    train_hours_low = 4
    train_hours_high = 6
    gpu_hour_cost_low = FLEET_GPU_HOUR_COST_REF.m_as(USD/hour)
    gpu_hour_cost_high = CLOUD_GPU_TRAINING_PER_HOUR.m_as(USD/hour)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    labeling_budget_str = fmt(labeling_budget/THOUSAND, precision=1, allow_zero=True, commas=False) + "K"
    train_cost_low_str = fmt(train_cost_low, precision=1, allow_zero=True, commas=False)
    train_cost_high_str = fmt(train_cost_high, precision=1, allow_zero=True, commas=False)
    ratio_low_str = fmt(ratio_low, precision=1, allow_zero=True, commas=True)
    ratio_high_str = fmt(ratio_high, precision=1, allow_zero=True, commas=True)
```

The contrast matters: **weeks** for human labeling, **hours** for GPU training, **milliseconds** for serving. Labeling is the bottleneck.

**The Labeling/Compute Rule**: Labeling often costs hundreds to more than a thousand times more than a single optimized training run. In the reference above, a \$`{python} LabelingComputeRatio.labeling_budget_str` labeling budget compares with USD `{python} LabelingComputeRatio.train_cost_low_str`--`{python} LabelingComputeRatio.train_cost_high_str` for one 8$\times$ A100 ResNet-50 run, a `{python} LabelingComputeRatio.ratio_low_str`--`{python} LabelingComputeRatio.ratio_high_str`$\times$ ratio.
...
- [ ] Given one pipeline change (for example, stricter validation, stronger privacy constraints, or more data sources), can you predict which pillar improves and which pillar(s) you stress?
...
\index{Data Acquisition!strategies}ImageNet's 14.2 million labeled images took 49,000 crowdsourced workers to assemble. GPT-3's training corpus used tens of terabytes of raw Common Crawl text filtered to hundreds of gigabytes, combined with curated web, books, and Wikipedia data [@brown2020language]. Our KWS system needs 23 million audio samples spanning fifty languages, a volume that no single collection method can economically provide. These examples illustrate a recurring reality: data acquisition is a strategic decision that determines a system's capabilities and limitations. Sourcing strategies (existing datasets, web scraping, crowdsourcing, synthetic generation) trade off quality, cost, scale, and ethics differently. No single approach satisfies all requirements; successful ML systems combine multiple strategies, balancing complementary strengths against competing constraints.
...
The choice\index{Data Acquisition!source evaluation} between curated datasets, expert crowdsourcing, and controlled web scraping depends on accuracy targets, domain expertise needed, and benchmark requirements. Achieving quality requires understanding whether data accurately represents the deployment environment and provides sufficient coverage of edge cases that might cause failures.

\index{Fei-Fei Li!ImageNet creation}
Preexisting datasets from repositories such as Kaggle, UCI [@uci_repo], and ImageNet\index{ImageNet!benchmark dataset}[^fn-imagenet-data-scale] [@imagenet_website] offer cost-efficient starting points with established benchmarks for consistent performance comparison.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: KWS multi-source acquisition prose
# │
...
# │ Exports: KWSAcquisitionScale.dataset_size_m_round_str,
# │          KWSAcquisitionScale.languages_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class KWSAcquisitionScale:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    total_examples = dataset_size_m * MILLION

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(total_examples > 20 * MILLION, f"KWS corpus unexpectedly small: {total_examples:.0f}.")
    check(languages >= 50, f"KWS language coverage unexpectedly narrow: {languages}.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt(int(round(dataset_size_m)), precision=1, allow_zero=True)
    languages_str = fmt(languages, precision=1, allow_zero=True, commas=False)
```

For our KWS system, `{python} KWSAcquisitionScale.dataset_size_m_round_str` million training examples across `{python} KWSAcquisitionScale.languages_str` languages demand a volume that manual collection cannot economically provide. The multi-source strategy introduced in @sec-data-engineering-framework-application-keyword-spotting-case-study-c8e9, combining curated datasets with web scraping, crowdsourcing, and synthetic generation, addresses this scale requirement while maintaining coverage across acoustic environments and speaker demographics.

### Coverage and diversity requirements {#sec-data-engineering-coverage-diversity-requirements-ce87}

\index{Data!coverage, requirements}\index{Bias!geographic, training data}Scale alone does not guarantee reliable models. Coverage gaps in even large datasets (geographic bias, demographic underrepresentation, temporal drift, missing edge cases) cause systematic failures that aggregate metrics obscure [@wang2019; @oakden-rayner2020]. As @fig-misalignment makes clear, multiple systems training on identical datasets inherit identical blind spots; diverse sourcing strategies are the defense against correlated failure modes.
...
\index{Data Validation!monitoring}\index{Schema Validation!ML pipelines}A self-driving car company discovered that 15 percent of their LiDAR point-cloud labels were misaligned by 10–20 cm—enough to place pedestrian bounding boxes on empty sidewalk. The mislabeling had persisted for three months, silently degrading the perception model's recall on pedestrians at crosswalks. No schema check flagged the error because every record was structurally valid; only statistical monitoring of label-to-sensor alignment distributions caught the drift.
...
**Failure**: As Corporate Vice President Peter Lee later wrote, Microsoft had implemented filtering, conducted user studies, and stress-tested Tay before launch---yet "in the first 24 hours of coming online, a coordinated attack by a subset of people exploited a vulnerability in Tay." Public inputs became a data poisoning surface: adversaries deliberately crafted inputs that Tay's running system kept ingesting and learning from.

**Consequence**: Within the first 24 hours, Tay produced racist and otherwise offensive outputs. Microsoft took the service offline and publicly apologized.
...
# │
# │ Imports: math (sqrt), mlsysim.constants (KS_TEST_COEFFICIENT),
# │          mlsysim.book (fmt)
# │ Exports: KSTest.ks_dcrit_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt, check
...
- Sample sizes $n_0 = n_t = 1000$.
- Critical value $\mathcal{D}_{\text{crit}} \approx 1.36\sqrt{(1000 + 1000)/(1000 \cdot 1000)}$ ≈ `{python} KSTest.ks_dcrit_str`.
- If observed max difference $\mathcal{D}_{\text{KS}} = 0.08$, then 0.08 > `{python} KSTest.ks_dcrit_str` → **Reject Null Hypothesis**. Significant drift detected. Trigger retraining or investigation.
...
These mechanical expectations become executable assertions that can run automatically.
...
\index{Data Drift!detection}\index{Distribution Shift!monitoring}ML models rest on the assumption that production data resembles training data. When this assumption breaks, not through *immediate schema violations* but through *gradual statistical shifts*, model performance degrades silently without obvious errors or system failures. Unlike the validation and monitoring techniques we have examined that catch immediate data quality issues, drift detection identifies gradual statistical changes in data distributions that compound over time to undermine model effectiveness. Production experience reveals that drift detection and response consume 30–40 percent of ongoing ML operations effort, making this a core data engineering responsibility rather than an optional advanced topic. @sec-ml-operations builds on this foundation to address operational response orchestration and automated retraining pipelines at scale.

The degradation equation in @eq-degradation, introduced in @sec-introduction-ml-vs-traditional-software-e19a, now becomes actionable through the divergence metrics formalized in @sec-data-foundations-measuring-drift-divergence-a6dd: the divergence term $\mathcal{D}(P_t \lVert P_0)$ is exactly what PSI and KL divergence measure. Two key metrics operationalize this measurement: the **Population Stability Index (PSI)**\index{Population Stability Index!definition}, which quantifies how much a categorical or binned feature distribution has shifted (values above 0.2 indicate significant drift), and **Kullback-Leibler (KL) Divergence**\index{KL Divergence!definition}, which measures the information-theoretic distance between continuous distributions. When PSI exceeds 0.2 or KL divergence crosses a threshold, the system signals that $\mathcal{D}(P_t \lVert P_0)$ has grown large enough to materially impact $\text{Accuracy}(t)$.
...
**Covariate shift**\index{Covariate Shift!definition} occurs when input feature distributions change while the relationship between features and labels remains constant: $p(x)$ changes but $p(y \mid x)$ stays the same. A medical imaging system trained on one camera model might see production data from a different camera manufacturer. The disease-image relationship remains unchanged (same pathologies produce same visual indicators), but pixel value distributions shift due to different sensor characteristics, color calibration, or image processing pipelines. Detection focuses on monitoring feature distributions using statistical metrics like PSI or KL divergence applied to input features.

#### Label shift {#sec-data-engineering-label-shift-dd6c}

**Label shift**\index{Label Shift!definition}\index{Distribution Shift!label shift} occurs when the output label distribution changes while the relationship between labels and features remains constant: $p(y)$ changes but $p(x \mid y)$ stays the same. Disease prevalence might change seasonally (flu cases spike in winter) while symptoms remain consistent predictors of each disease. A recommendation system might see label shift when new product categories launch, changing the distribution of user preferences without altering what makes products appealing within each category. Detection monitors prediction distributions for shifts in relative frequencies of predicted classes, which can be done without ground truth labels by tracking model output distributions.

#### Concept drift {#sec-data-engineering-concept-drift-2b83}

**Concept drift**\index{Concept Drift!definition} represents the most challenging case: the relationship between features and labels changes, meaning $p(y \mid x)$ evolves over time [@gama2014]. Medical treatment protocols change, altering disease outcomes for given symptoms. User preferences shift as social trends evolve, changing what product features drive purchases. Fraud patterns evolve as attackers adapt to detection systems. Concept drift requires ground truth labels for detection since we must monitor whether the feature-to-label relationship has changed, making it inherently more difficult and delayed than detecting covariate or label shift.
...
# │ Exports: CohensKappaChance.chance_agreement_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class CohensKappaChance:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    chance_agreement_pct_str = fmt(chance_agreement * 100, precision=1, allow_zero=True, commas=False)
```
...
[^fn-cohens-kappa-label-quality]: **Cohen's Kappa ($\kappa$)**: Introduced by @cohen1960 to measure inter-rater agreement while correcting for chance agreement, which raw percentage agreement ignores. The correction matters: two annotators labeling 90 percent of images as "not spam" in a binary task will agree `{python} CohensKappaChance.chance_agreement_pct_str` percent of the time by pure chance, making raw agreement a dangerously misleading quality metric. For ML label quality monitoring, $\kappa$ below 0.4 signals unreliable training data whose noise the model will inherit. \index{Cohen's Kappa!label quality}
...
# │
# │ Imports: math (ceil), mlsysim.fmt (fmt, check)
# │ Exports: DataloaderStats.resnet_worker_count_str and plot constants
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt, check
...
class DataloaderStats:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    resnet_worker_count_str = fmt(resnet_worker_count, precision=1, allow_zero=True, commas=False)
```
...
cpu_throughput = np.minimum(
    DataloaderStats.worker_img_sec * workers,
    DataloaderStats.cpu_throughput_cap_img_sec
)
...
# │
# │ Imports: mlsysim.core.constants (KB, GB, TB, MILLION, SEC_PER_HOUR),
# │          mlsysim.book (fmt, check)
# │ Exports: RealtimeCost.throughput_str, RealtimeCost.hourly_tb_str,
# │          RealtimeCost.stream_cost_str, RealtimeCost.batch_cost_str,
# │          RealtimeCost.cost_ratio_str, RealtimeCost.events_per_sec_str,
# │          RealtimeCost.event_size_kb_str, RealtimeCost.stream_cores_str,
# │          RealtimeCost.stream_hours_str, RealtimeCost.stream_cost_per_hr_str,
# │          RealtimeCost.batch_core_hours_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class RealtimeCost:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(throughput_gbs - 1.0) < 1e-9, f"1 million events/s at 1 KB/event should be 1 GB/s, got {throughput_gbs:.2f}.")
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    throughput_str = fmt(throughput_gbs, precision=1, allow_zero=True, commas=False)
    hourly_tb_str = fmt(hourly_data_tb, precision=1, commas=False)
    stream_cost_str = fmt(stream_cost_day, precision=1, allow_zero=True, commas=False)
    batch_cost_str = fmt(batch_cost_day, precision=1, allow_zero=True, commas=False)
    cost_ratio_str = fmt(cost_ratio, precision=1, allow_zero=True, commas=False)
    events_per_sec_str = fmt(events_per_sec/MILLION, precision=1, allow_zero=True, commas=False, suffix="M")
    event_size_kb_str = fmt(event_size_kb, precision=1, allow_zero=True, commas=False)
    stream_cores_str = fmt(stream_cores, precision=1, allow_zero=True, commas=False)
    stream_hours_str = fmt(stream_hours, precision=1, allow_zero=True, commas=False)
    stream_cost_per_hr_str = fmt(stream_cost_per_hr, precision=2, commas=False)
    batch_core_hours_str = fmt(batch_core_hours, precision=1, allow_zero=True, commas=False)
```

::: {#nbk-data-engineering-cost-real-time .callout-notebook title="The cost of real-time"}

**Problem**: A pipeline ingests `{python} RealtimeCost.events_per_sec_str` events/second. Which is cheaper, Batch (hourly) or Stream (sub-second) ingestion?

**Physics**:

1.  **Throughput**: `{python} RealtimeCost.events_per_sec_str` events/sec $\times$ `{python} RealtimeCost.event_size_kb_str` KB/event = **`{python} RealtimeCost.throughput_str` GB/s**.
2.  **Stream requirements**: To sustain `{python} RealtimeCost.throughput_str` GB/s with < 100 ms latency, the system needs ~50 primary cores plus ~50 redundant cores, or `{python} RealtimeCost.stream_cores_str` always-on cores total. Cost: `{python} RealtimeCost.stream_cores_str` cores $\times$ `{python} RealtimeCost.stream_hours_str` hrs $\times$ USD `{python} RealtimeCost.stream_cost_per_hr_str`/hr = **USD `{python} RealtimeCost.stream_cost_str`/day**.
3.  **Batch requirements**: Process `{python} RealtimeCost.hourly_tb_str` TB (1 hour data) in ten mins. High throughput (sequential I/O) is efficient. The batch design needs 200 cores for ten mins/hour $\times$ 24 hours = `{python} RealtimeCost.batch_core_hours_str` core-hours/day. Cost: `{python} RealtimeCost.batch_core_hours_str` $\times$ USD `{python} RealtimeCost.stream_cost_per_hr_str` = **USD `{python} RealtimeCost.batch_cost_str`/day**.

**Systems insight**: Real-time is **~`{python} RealtimeCost.cost_ratio_str`$\times$ more expensive** for the same data volume. This tax is justified only when the value of < 1 s latency exceeds the added cost.

:::

#### ETL and ELT comparison {#sec-data-engineering-etl-elt-comparison-2e2b}

\index{ETL!vs. ELT}\index{ELT!vs. ETL}Beyond choosing ingestion patterns based on timing requirements, designing effective data ingestion pipelines requires understanding the differences between Extract, Transform, Load (ETL)[^fn-etl-quality-first] and Extract, Load, Transform (ELT)[^fn-elt-flexibility-first] approaches. The core distinction is ordering: ETL cleanses and structures data *before* it enters storage, ensuring only validated data reaches the warehouse; ELT loads raw data first and applies transformations *within* the target system, preserving flexibility at the cost of storing uncleaned data. Compare the two flows side by side in @fig-etl-vs-elt—note where the "Transform" step falls relative to "Load" in each paradigm, as this ordering significantly impacts pipeline flexibility and efficiency. The choice between ETL and ELT affects where computational resources are consumed, how quickly data becomes available for analysis, and how easily transformation logic can evolve as requirements change.
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: EtlEltCost.etl_spark_str, EtlEltCost.etl_storage_str,
# │          EtlEltCost.elt_storage_tb_str, EtlEltCost.elt_storage_str,
# │          EtlEltCost.elt_query_str, EtlEltCost.savings_str,
# │          EtlEltCost.daily_raw_tb_str, EtlEltCost.spark_per_tb_str,
# │          EtlEltCost.etl_datasets_str, EtlEltCost.etl_tb_each_str,
# │          EtlEltCost.etl_total_storage_tb_str, EtlEltCost.s3_per_tb_mo_str,
# │          EtlEltCost.retention_days_str, EtlEltCost.query_cost_per_str,
# │          EtlEltCost.n_models_str, EtlEltCost.schema_change_ratio_str,
# │          EtlEltCost.etl_spark_monthly_str, EtlEltCost.etl_total_str,
# │          EtlEltCost.elt_total_str, EtlEltCost.total_savings_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class EtlEltCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    etl_spark_str = fmt(etl_spark_daily, precision=1, allow_zero=True, commas=False)
    etl_storage_str = fmt(etl_storage_mo, precision=1, allow_zero=True, commas=False)
    elt_storage_tb_str = fmt(elt_storage_tb, precision=1, allow_zero=True, commas=False)
    elt_storage_str = fmt(elt_storage_mo, precision=1, allow_zero=True)
    elt_query_str = fmt(elt_query_mo, precision=1, allow_zero=True, commas=False)
    savings_str = fmt(storage_savings_mo, precision=1, allow_zero=True)
    etl_spark_monthly_str = fmt(etl_spark_monthly, precision=1, allow_zero=True)
    etl_total_str = fmt(etl_total_mo, precision=1, allow_zero=True)
    elt_total_str = fmt(elt_total_mo, precision=1, allow_zero=True)
    total_savings_str = fmt(total_savings_mo, precision=1, allow_zero=True)
    daily_raw_tb_str = fmt(daily_raw_tb, precision=1, allow_zero=True, commas=False)
    spark_per_tb_str = fmt(spark_per_tb, precision=1, allow_zero=True, commas=False)
    etl_datasets_str = fmt(etl_datasets, precision=1, allow_zero=True, commas=False)
    etl_tb_each_str = fmt(etl_tb_each, precision=1, allow_zero=True, commas=False)
    etl_total_storage_tb_str = fmt(etl_storage_tb, precision=1, allow_zero=True, commas=False)
    s3_per_tb_mo_str = fmt(s3_per_tb_mo, precision=1, allow_zero=True, commas=False)
    retention_days_str = fmt(retention_days, precision=1, allow_zero=True, commas=False)
    query_cost_per_str = fmt(query_cost_per, precision=1, allow_zero=True, commas=False)
    n_models_str = fmt(n_models, precision=1, allow_zero=True, commas=False)
    schema_change_ratio_str = fmt(schema_change_ratio, precision=1, allow_zero=True, commas=False)
```

::: {#nbk-data-engineering-cost-transformation-placement .callout-notebook title="The cost of transformation placement"}

**Problem**: A team processes `{python} EtlEltCost.daily_raw_tb_str` TB of raw clickstream data daily and must compute user session features for `{python} EtlEltCost.n_models_str` ML models, each requiring different aggregation windows (1-hour, 24-hour, 7-day). Which is cheaper, ETL or ELT?
...
1.  **ETL approach**: Transform before loading. Compute all three aggregation windows in a Spark cluster before loading into the warehouse.
    *   Spark compute: `{python} EtlEltCost.daily_raw_tb_str` TB at USD `{python} EtlEltCost.spark_per_tb_str`/TB = USD `{python} EtlEltCost.etl_spark_str`/day
    *   Storage: `{python} EtlEltCost.etl_datasets_str` transformed datasets, ~`{python} EtlEltCost.etl_tb_each_str` TB each = `{python} EtlEltCost.etl_total_storage_tb_str` TB at USD `{python} EtlEltCost.s3_per_tb_mo_str`/TB per month = USD `{python} EtlEltCost.etl_storage_str`/month
    *   Schema change cost: Re-run full pipeline (~4 hours) per change

2.  **ELT approach**: Load raw data first, transform in warehouse.
    *   Storage: `{python} EtlEltCost.daily_raw_tb_str` TB raw/day, `{python} EtlEltCost.retention_days_str`-day retention = `{python} EtlEltCost.elt_storage_tb_str` TB at USD `{python} EtlEltCost.s3_per_tb_mo_str`/TB per month = USD `{python} EtlEltCost.elt_storage_str`/month
    *   Query compute: `{python} EtlEltCost.n_models_str` models$\times$ USD `{python} EtlEltCost.query_cost_per_str`/query $\times$ `{python} EtlEltCost.retention_days_str` days = USD `{python} EtlEltCost.elt_query_str`/month
    *   Schema change cost: Rewrite SQL query (~30 minutes) per change

**Systems insight**: ETL saves USD `{python} EtlEltCost.savings_str`/month in storage. After normalizing Spark compute to a monthly cost, ETL totals USD `{python} EtlEltCost.etl_total_str`/month (USD `{python} EtlEltCost.etl_spark_monthly_str` compute + USD `{python} EtlEltCost.etl_storage_str` storage), while ELT totals USD `{python} EtlEltCost.elt_total_str`/month (USD `{python} EtlEltCost.elt_storage_str` storage + USD `{python} EtlEltCost.elt_query_str` query compute), for a cloud-cost advantage of USD `{python} EtlEltCost.total_savings_str`/month before engineering labor. ETL still costs `{python} EtlEltCost.schema_change_ratio_str`$\times$ more engineering time per schema change. The exact break-even point depends on engineering labor cost and schema-change frequency: stable schemas favor ETL's lower cloud cost, while frequent feature-definition changes favor ELT's faster iteration.
...
For ML pipelines, an additional decision extends beyond ETL vs. ELT: where to compute features\index{Feature Computation!placement}. This choice significantly impacts training speed, storage costs, and reproducibility.
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: KWSQualityThresholds.snr_threshold_db_str,
# │          KWSQualityThresholds.sample_duration_ms_low_str,
# │          KWSQualityThresholds.sample_duration_ms_high_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
# │
# │ Imports: mlsysim.core.constants (MILLION), mlsysim.book (fmt, check)
# │ Exports: KWSScalabilityTargets.dataset_size_m_round_str,
# │          KWSScalabilityTargets.memory_limit_edge_kb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class KWSScalabilityTargets:
    ...
    model_size_limit_kb = DataEngineeringSetup.kws_memory_limit_design_kb
    ...
    check(memory_limit_edge_kb < model_size_limit_kb,
          "Edge preprocessing-state budget should be stricter than the model-size design limit.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt(int(round(dataset_size_m)), precision=1, allow_zero=True)
    memory_limit_edge_kb_str = fmt(memory_limit_edge_kb, precision=1, allow_zero=True, commas=False)
    model_size_limit_kb_str = fmt(model_size_limit_kb, precision=1, allow_zero=True, commas=False)
```

Scalability in the KWS pipeline manifests at multiple stages. Development uses single-machine processing on sample datasets to iterate rapidly. Training at scale requires distributed processing when dataset size (`{python} KWSScalabilityTargets.dataset_size_m_round_str` million examples) exceeds single-machine capacity or when multiple experiments run concurrently. The processing pipeline parallelizes naturally: audio files are independent, so transforming them requires no coordination between workers. Each worker reads its assigned audio files from distributed storage, computes features, and writes results back—a trivially parallel pattern achieving near-linear scalability. Production deployment adds a stricter `{python} KWSScalabilityTargets.memory_limit_edge_kb_str` KB preprocessing-state budget alongside the `{python} KWSScalabilityTargets.model_size_limit_kb_str` KB model-size limit, necessitating careful optimization and quantization to fit processing within device capabilities.
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: SegmentationLabelScale.pixel_labels_m_str,
# │          SegmentationLabelScale.box_count_str,
# │          SegmentationLabelScale.box_coords_str,
# │          SegmentationLabelScale.label_entry_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class SegmentationLabelScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pixel_labels_m_str = fmt(pixel_labels/1_000_000, precision=1, commas=False)
    box_count_str = fmt(box_count, precision=1, allow_zero=True, commas=False)
    box_coords_str = fmt(box_coords, precision=1, allow_zero=True, commas=False)
    label_entry_ratio_str = fmt(label_entry_ratio, precision=1, allow_zero=True, commas=True)
```

Segmentation maps\index{Semantic Segmentation!pixel-level labels} provide the most comprehensive information by classifying objects at the pixel level, highlighting each object in a distinct color. For our traffic monitoring system, this might mean precisely outlining each vehicle, pedestrian, and road sign. These detailed annotations significantly increase our storage and processing requirements. A segmentation mask for a $1920{\times}1080$ image requires about `{python} SegmentationLabelScale.pixel_labels_m_str` million labels (one per pixel), compared to perhaps `{python} SegmentationLabelScale.box_count_str` bounding boxes or a single classification label. If each box stores `{python} SegmentationLabelScale.box_coords_str` coordinates, that is roughly `{python} SegmentationLabelScale.label_entry_ratio_str`$\times$ more scalar label entries than ten boxes before accounting for per-value encoding, and the hours required per image for manual segmentation make this approach suitable only when pixel-level precision is essential.
...
# │
# │ Imports: mlsysim.core.constants (THOUSAND, MILLION), mlsysim.fmt (fmt, check)
# │ Exports: ActiveLearningBudget.*_str values for callout prose.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class ActiveLearningBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_images_str = fmt(dataset_images / MILLION, precision=1, allow_zero=True, commas=False, suffix="M")
    label_budget_str = fmt(label_budget / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    label_cost_str = fmt(label_cost, precision=2, commas=False, prefix="$")
    inference_cost_str = fmt(inference_cost_per_image, precision=2, commas=False, prefix="$")
    random_labels_str = fmt(random_labels_needed / MILLION, precision=1, allow_zero=True, commas=False, suffix="M")
    random_label_cost_str = fmt(random_label_cost / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    active_labels_low_str = fmt(active_labels_low / THOUSAND, precision=1, allow_zero=True, commas=False, suffix="K")
    active_labels_high_str = fmt(active_labels_high / THOUSAND, precision=1, allow_zero=True, commas=False, suffix="K")
    active_label_cost_low_str = fmt(active_label_cost_low / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    active_label_cost_high_str = fmt(active_label_cost_high / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    full_pool_inference_cost_str = fmt(full_pool_inference_cost / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    active_total_low_str = fmt(active_total_low / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    active_total_high_str = fmt(active_total_high / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    compute_budget_pct_str = fmt(compute_budget_fraction * 100, precision=1, allow_zero=True, commas=False)
    compute_budget_str = fmt(compute_budget / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    candidate_pool_str = fmt(candidate_pool_images / THOUSAND, precision=1, allow_zero=True, commas=False, suffix="K")
    affordable_labels_str = fmt(affordable_labels_after_compute / THOUSAND, precision=1, allow_zero=True, commas=False, suffix="K")
```

[^fn-active-learning-budget]: **Active Learning**: Inverts the traditional labeling paradigm: instead of randomly selecting examples to label, the model queries for the examples it needs most, typically those where prediction uncertainty is highest. This converts labeling from a data-proportional cost into a model-proportional cost, achieving target accuracy with 50--90 percent fewer labels than random sampling. The infrastructure trade-off is compute for labels: at `{python} ActiveLearningBudget.inference_cost_str`/image, scoring a full `{python} ActiveLearningBudget.dataset_images_str`-image pool costs `{python} ActiveLearningBudget.full_pool_inference_cost_str` before any human labels. Active learning becomes budget leverage only when the candidate pool is pre-filtered, inference is much cheaper, or the compute budget is separate from the labeling budget. \index{Active Learning!budget multiplier}
...
**Problem**: A `{python} ActiveLearningBudget.dataset_images_str` image dataset has a `{python} ActiveLearningBudget.label_budget_str` labeling budget. Random sampling achieves 85 percent accuracy with `{python} ActiveLearningBudget.active_labels_low_str` images, while the target is 95 percent accuracy.
...
2.  **Cost per point**: Random sampling = `{python} ActiveLearningBudget.label_cost_str`/label. Active learning adds compute cost (~`{python} ActiveLearningBudget.inference_cost_str`/image for inference) to find hard examples.
3.  **The multiplier**:
    *   **Random**: Reaching 95 percent may require `{python} ActiveLearningBudget.random_labels_str` labels (`{python} ActiveLearningBudget.random_label_cost_str`). **Budget Exceeded.**
...
# │
# │ Imports: mlsysim.core.constants (MILLION, SEC_PER_HOUR), mlsysim.book (fmt, check)
# │ Exports: KWSAutomatedLabelingScale.dataset_size_m_str,
# │          KWSAutomatedLabelingScale.dataset_size_m_round_str,
# │          KWSAutomatedLabelingScale.label_hours_str,
# │          KWSAutomatedLabelingScale.label_years_str,
# │          KWSAutomatedLabelingScale.accuracy_target_str,
# │          KWSAutomatedLabelingScale.languages_str,
# │          KWSAutomatedLabelingScale.sample_duration_ms_low_str,
# │          KWSAutomatedLabelingScale.sample_duration_ms_high_str
# └─────────────────────────────────────────────────────────────────────────────
...
class KWSAutomatedLabelingScale:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    total_examples = dataset_size_m * MILLION
    label_hours = total_examples * label_seconds_per_example / SEC_PER_HOUR
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_str = MarkdownStr(f"{dataset_size_m}")
    dataset_size_m_round_str = fmt(int(round(dataset_size_m)), precision=1, allow_zero=True)
    label_hours_str = fmt(label_hours, precision=1, allow_zero=True, commas=True)
    label_years_str = fmt(label_years, precision=1, commas=False)
    accuracy_target_str = fmt(accuracy_target, precision=1, allow_zero=True, commas=False)
    languages_str = fmt(languages, precision=1, allow_zero=True, commas=False)
    sample_duration_ms_low_str = fmt(sample_duration_ms_low, precision=1, allow_zero=True, commas=False)
    sample_duration_ms_high_str = fmt(sample_duration_ms_high, precision=1, allow_zero=True, commas=False)
```

\index{Keyword Spotting!automated labeling}Our KWS case study has now progressed through problem definition (@sec-data-engineering-framework-application-keyword-spotting-case-study-c8e9), data collection, ingestion, and processing. At the labeling stage, we confront a challenge unique to speech systems at scale. Generating millions of labeled wake word samples without proportional human annotation cost requires moving beyond the manual and crowdsourced approaches we examined earlier. The Multilingual Spoken Words Corpus (MSWC) [@mazumder2021multilingual] demonstrates how automated labeling addresses this challenge through its innovative approach to generating labeled wake word data; the corpus contains over `{python} KWSAutomatedLabelingScale.dataset_size_m_str` million one-second spoken examples across 340,000 keywords in `{python} KWSAutomatedLabelingScale.languages_str` different languages.

This scale makes manual annotation infeasible: `{python} KWSAutomatedLabelingScale.dataset_size_m_str` million examples at even 10 seconds per label would require approximately `{python} KWSAutomatedLabelingScale.label_hours_str` person-hours, roughly `{python} KWSAutomatedLabelingScale.label_years_str` person-years of full-time effort. Achieving `{python} KWSAutomatedLabelingScale.accuracy_target_str` percent accuracy across diverse environments requires millions of training examples covering acoustic variations (background noises, speaking styles, recording environments), and transparent sourcing across `{python} KWSAutomatedLabelingScale.languages_str` languages ensures the technology serves diverse speaker populations.

Walk through the pipeline in @fig-mswc to see how this automated system works. It begins with paired sentence audio recordings and corresponding transcriptions from projects like Common Voice [@mozilla_common_voice] or multilingual captioned content platforms, processing these inputs through forced alignment\index{Forced Alignment!speech labeling}\index{Speech Recognition!forced alignment}[^fn-forced-alignment-kws] to identify precise word boundaries within continuous speech.
...
\index{IOPS!storage performance}\index{Throughput!storage bandwidth}Storage system selection extends beyond capacity planning. The goal is minimizing the **Data Term** $(\frac{D_{\text{vol}}}{\text{BW}})$ in the iron law of ML systems. Every storage medium imposes physical constraints on bandwidth that determine the maximum speed of the training and serving pipelines.

Optimizing this term requires understanding two core storage performance metrics:

1.  **IOPS (Input/Output Operations Per Second)**: The number of distinct read/write requests a device can handle per second. This limits performance for *random access* workloads (for example, fetching small batches of images or individual user profiles).
2.  **Throughput (Bandwidth)**: The volume of data transferred per second, typically $\text{IOPS} \times \text{Block Size}$. This limits performance for *sequential access* workloads (for example, scanning a Parquet file for training).
...
Each storage architecture exhibits distinct strengths when applied to specific ML tasks. For online feature serving, the high-IOPS characteristics of databases enable millisecond lookups of individual records. The DLRM recommendation lighthouse discussed in @sec-network-architectures-sparse-architectures-recommendation-systems-5d19 exemplifies this challenge at its most extreme: its terabyte-scale embedding tables must serve billions of sparse lookups per second, requiring storage architectures that optimize IOPS over sequential throughput. More generally, a recommendation system looking up a user's profile during real-time inference requires random access optimized for per-request latency.
...
For exploratory analysis and training on unstructured data (images, audio, text), data lakes provide the flexibility and low-cost storage needed for massive volumes. A computer vision system storing terabytes of raw images alongside metadata, annotations, and intermediate processing results requires the schema flexibility and cost efficiency that only data lakes provide, where the sheer scale of the $D_{\text{vol}}$ numerator demands the highest aggregate bandwidth.

Databases\index{Database!OLTP for ML}\index{OLTP} excel at operational and transactional purposes, maintaining product catalogs, user profiles, or transaction histories with strong consistency guarantees and low-latency point lookups. For ML workflows, databases serve specific roles well: storing feature metadata that changes frequently, managing experiment tracking where transactional consistency matters, or maintaining model registries that require atomic updates. A PostgreSQL database handling structured user attributes (user_id, age, country, preferences) provides millisecond lookups for serving systems that need individual user features in real-time. However, databases struggle when ML training requires scanning millions of records repeatedly across multiple epochs. The row-oriented storage that optimizes transactional lookups becomes inefficient when training needs only 20 of 100 columns from each record but must read entire rows to extract those columns.

Data warehouses\index{Data Warehouse!analytical queries}\index{Data Warehouse!OLAP}\index{OLAP}\index{Storage!columnar, ML training} fill this analytical gap, optimized for complex queries across integrated datasets transformed into standardized schemas. Modern warehouses like Google BigQuery, Amazon Redshift, and Snowflake use columnar storage formats [@stonebraker2018] that enable reading specific features without loading entire records, essential when tables contain hundreds of columns but training needs only a subset. This columnar organization delivers five to ten times I/O reduction compared to row-based formats for typical ML workloads. Consider a fraud detection dataset with 100 columns where models typically use 20 features—columnar storage reads only needed columns, achieving 80 percent I/O reduction before even considering compression. Many successful ML systems draw training data from warehouses because the structured environment simplifies exploratory analysis and iterative development. Data analysts can quickly compute aggregate statistics, identify correlations between features, and validate data quality using familiar SQL interfaces.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: kws-storage-economics-calc
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.constants (STORAGE_COST_*, GB, TB, USD, ureg, byte, MILLION),
# │          mlsysim.book (fmt, check)
# │ Exports: KWSStorageEconomics.dataset_size_gb_str,
# │          KWSStorageEconomics.storage_s3_cost_str,
# │          KWSStorageEconomics.storage_nvme_low_cost_str,
# │          KWSStorageEconomics.storage_nvme_high_cost_str,
# │          KWSStorageEconomics.load_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class KWSRawAudioStorage:
    """Raw-audio storage size for the rounded KWS corpus scale used in storage examples."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    rounded_sample_count_m = 23
    sample_duration_s = 1
    sample_rate_hz = 16_000
    bytes_per_sample = 2

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    sample_count = rounded_sample_count_m * MILLION
    dataset_size_bytes = sample_count * sample_duration_s * sample_rate_hz * bytes_per_sample
    dataset_size_gb = (dataset_size_bytes * byte).m_as(GB)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(dataset_size_gb - 736) < 1e-9, f"Rounded KWS storage example should be 736 GB, got {dataset_size_gb:.1f} GB.")

class KWSStorageEconomics:
    """Storage-cost recap for the KWS training dataset."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    dataset_size_gb = KWSRawAudioStorage.dataset_size_gb
    storage_cost_s3 = STORAGE_COST_S3_STD.m_as(USD/TB/ureg.month)
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    dataset_size_tb = (dataset_size_gb * GB).m_as(TB)
    storage_s3_cost = dataset_size_tb * storage_cost_s3
    storage_nvme_low_cost = dataset_size_tb * storage_cost_nvme_low
    storage_nvme_high_cost = dataset_size_tb * storage_cost_nvme_high
    load_speedup = nvme_bw_gbs/obj_bw_gbs
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_gb_str = fmt(dataset_size_gb, precision=1, allow_zero=True, commas=False)
    storage_s3_cost_str = fmt(storage_s3_cost, precision=1, allow_zero=True, commas=False)
    storage_nvme_low_cost_str = fmt(storage_nvme_low_cost, precision=1, allow_zero=True, commas=False)
    storage_nvme_high_cost_str = fmt(storage_nvme_high_cost, precision=1, allow_zero=True, commas=False)
    load_speedup_str = fmt(load_speedup, precision=1, allow_zero=True, commas=False)
```

@tbl-storage-performance reveals why ML systems employ tiered storage architectures. Consider the economics of storing our KWS training dataset (`{python} KWSStorageEconomics.dataset_size_gb_str` GB): object storage costs \$`{python} KWSStorageEconomics.storage_s3_cost_str`/month, enabling affordable long-term retention of raw audio, while maintaining working datasets on NVMe[^fn-nvme-gpu-utilization] for active training costs \$`{python} KWSStorageEconomics.storage_nvme_low_cost_str`–`{python} KWSStorageEconomics.storage_nvme_high_cost_str`/month but provides `{python} KWSStorageEconomics.load_speedup_str`$\times$ faster data loading.
...
: **Storage Cost-Performance Trade-Offs**: Different storage tiers provide distinct cost-performance characteristics that determine their suitability for specific ML workloads. Training data loading requires high-throughput sequential access, an I/O pattern that must align with the memory hierarchy in @sec-machine-foundations-memory-hierarchy-2278. Online serving needs low-latency random reads, while archival storage prioritizes cost over access speed for compliance and historical data. {#tbl-storage-performance tbl-colwidths="[26,15,19,19,21]"}
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: StorageLoading.nvme_load_str, StorageLoading.obj_load_str,
# │          StorageLoading.load_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class StorageLoading:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    kws_dataset_gb = KWSStorageEconomics.dataset_size_gb
    ...
    check(obj_load_s > 3600, f"Object storage load should take hours at 100 MB/s, got {obj_load_s:.0f}s.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvme_load_str = fmt(nvme_load_s, precision=1, allow_zero=True, commas=False)
    obj_load_str = fmt(obj_load_s, precision=1, allow_zero=True, commas=True)
    load_speedup_str = fmt(load_speedup, precision=1, allow_zero=True, commas=False)
    nvme_bw_gbs_str = fmt(nvme_bw_gbs, precision=1, allow_zero=True, commas=False)
```

The performance difference directly impacts iteration velocity. Training that loads data at `{python} StorageLoading.nvme_bw_gbs_str` GB/s completes dataset loading in `{python} StorageLoading.nvme_load_str` seconds, compared to `{python} StorageLoading.obj_load_str` seconds at typical object storage speeds. This `{python} StorageLoading.load_speedup_str`$\times$ difference determines whether teams can iterate multiple times daily or must wait hours between experiments.
...
# │
# │ Imports: mlsysim digital twins, mlsysim.core.constants (Bparam, Mparam,
# │          GB, BYTES_FP32, BYTES_FP16, BILLION, MILLION),
# │          mlsysim.book (fmt, check)
# │ Exports: ModelStorageScale.gpt3_fp32_str,
# │          ModelStorageScale.gpt3_params_str,
# │          ModelStorageScale.gpt3_fp16_str,
# │          ModelStorageScale.alexnet_params_str,
# │          ModelStorageScale.model_growth_str
# └─────────────────────────────────────────────────────────────────────────────
...
class ModelStorageScale:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(gpt3_fp32_gb > gpt3_fp16_gb, "FP32 model storage should exceed FP16 storage.")
    check(model_growth_factor > 1000, f"AlexNet-to-GPT-3 growth factor unexpectedly low: {model_growth_factor}.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    gpt3_params_str = fmt(gpt3_params_b, precision=1, allow_zero=True, commas=False)
    ...
    alexnet_params_str = MarkdownStr(f"{alexnet_params_m}")
    model_growth_str = fmt(model_growth_factor, precision=1, allow_zero=True)
```

Modern ML models contain millions to trillions of parameters\index{Model Parameters!storage requirements} requiring storage and retrieval patterns fundamentally different from traditional data. GPT-3 [@brown2020language] requires approximately `{python} ModelStorageScale.gpt3_fp32_str` gigabytes for model weights when stored in FP32 format (`{python} ModelStorageScale.gpt3_params_str` billion parameters times 4 bytes), though practical deployments typically use FP16 (`{python} ModelStorageScale.gpt3_fp16_str` GB) or quantized formats for reduced storage and faster inference (see @sec-model-compression for quantization techniques). Even at FP16 precision, this exceeds many organizations' entire operational databases. The trajectory reveals accelerating scale: from AlexNet's `{python} ModelStorageScale.alexnet_params_str` million parameters [@krizhevsky2012imagenet] to GPT-3's `{python} ModelStorageScale.gpt3_params_str` billion parameters [@brown2020language], model size grew approximately `{python} ModelStorageScale.model_growth_str`-fold in eight years (`{python} ModelStorageScale.alexnet_params_str` million to `{python} ModelStorageScale.gpt3_params_str` billion parameters). Storage systems must handle these dense numerical arrays efficiently for both capacity and access speed. Unlike typical files where sequential organization matters for readability, model weights benefit from block-aligned storage enabling parallel reads across parameter groups. When multiple GPUs need to read model data from shared storage, whether during training initialization or checkpoint loading, storage systems must deliver aggregate bandwidth approaching network interface limits, often 25 Gbps or higher, without introducing bottlenecks that would idle expensive compute resources.
...
# │
# │ Imports: mlsysim.core.constants (KB, GB, flop, GFLOPs, TFLOPs),
# │          mlsysim.book (fmt, check), mlsysim digital twins
# │ Exports: StorageBandwidth.max_img_per_sec_str,
# │          StorageBandwidth.required_bw_gbs_str,
# │          StorageBandwidth.a100_flops_t_str,
# │          StorageBandwidth.resnet_flops_g_str,
# │          StorageBandwidth.image_size_kb_str,
# │          StorageBandwidth.sata_bw_mbs_str,
# │          StorageBandwidth.sata_max_img_sec_str,
# │          StorageBandwidth.sata_utilization_pct_str,
# │          StorageBandwidth.s3_bw_mbs_str,
# │          StorageBandwidth.s3_workers_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Models
from mlsysim.fmt import fmt, check
...
class StorageBandwidth:
    ...
    sata_max_img_sec = int((sata_bw_mbs * MB).m_as(byte) / (image_size_kb * KB).m_as(byte))
    sata_utilization_pct = int(round(sata_max_img_sec/max_img_per_sec * 100))
    s3_workers = int(round((required_bw_gbs * GB).m_as(MB) / s3_bw_mbs))

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(required_bw_gbs > 3.0, f"A100 storage budget should be multiple GB/s, got {required_bw_gbs:.1f}.")
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    max_img_per_sec_str = fmt(max_img_per_sec, precision=1, allow_zero=True)
    required_bw_gbs_str = fmt(required_bw_gbs, precision=1, commas=False)
    a100_flops_t_str = fmt(int(h_a100.peak_flops.m_as(TFLOPs/second)), precision=1, allow_zero=True, commas=False)
    resnet_flops_g_str = fmt(int((resnet_flops_per_img * flop).m_as(GFLOPs)), precision=1, allow_zero=True, commas=False)
    image_size_kb_str = fmt(image_size_kb, precision=1, allow_zero=True, commas=False)
    sata_bw_mbs_str = fmt(sata_bw_mbs, precision=1, allow_zero=True, commas=False)
    sata_max_img_sec_str = fmt(sata_max_img_sec, precision=1, allow_zero=True)
    sata_utilization_pct_str = fmt(sata_utilization_pct, precision=1, allow_zero=True, commas=False)
    s3_bw_mbs_str = fmt(s3_bw_mbs, precision=1, allow_zero=True, commas=False)
    s3_workers_str = fmt(s3_workers, precision=1, allow_zero=True, commas=False)
```

Designing for high-throughput training starts by matching storage throughput to accelerator demand.
...
*   **Hardware peak**: NVIDIA A100 = `{python} StorageBandwidth.a100_flops_t_str` TFLOP/s (FP16 Tensor Core, dense operations).
*   **Model cost**: ResNet-50 requires approximately four GFLOPs for the forward pass per image; including the backward pass, total is approximately `{python} StorageBandwidth.resnet_flops_g_str` GFLOPs per training step.
*   **Maximum theoretical throughput**:
    `{python} StorageBandwidth.a100_flops_t_str` $\times 10^{12}$ FLOP/s / `{python} StorageBandwidth.resnet_flops_g_str` $\times 10^{9}$ FLOPs/img = **`{python} StorageBandwidth.max_img_per_sec_str` images/sec**

**The data term (bandwidth requirement)**

*   **Image size**: `{python} StorageBandwidth.image_size_kb_str` KB (JPEG compressed).
*   **Required bandwidth**:
    `{python} StorageBandwidth.max_img_per_sec_str` img/sec $\times$ `{python} StorageBandwidth.image_size_kb_str` KB/img ≈ **`{python} StorageBandwidth.required_bw_gbs_str` GB/s**

**Storage options**:
To saturate a single A100, storage must deliver **`{python} StorageBandwidth.required_bw_gbs_str` GB/s**.

*   **S3 Standard**: ~`{python} StorageBandwidth.s3_bw_mbs_str` MB/s per thread. The pipeline needs **`{python} StorageBandwidth.s3_workers_str` concurrent worker threads**.
*   **SATA SSD**: ~`{python} StorageBandwidth.sata_bw_mbs_str` MB/s sequential. Totally insufficient (bottleneck).
*   **NVMe SSD**\index{NVMe SSD!data pipeline}: approximately 3–7 GB/s. **Required.**

**Systems insight**: With SATA SSDs, maximum throughput is capped at `{python} StorageBandwidth.sata_bw_mbs_str` MB/s / `{python} StorageBandwidth.image_size_kb_str` KB ≈ `{python} StorageBandwidth.sata_max_img_sec_str` img/s. The USD 15,000 GPU will run at **`{python} StorageBandwidth.sata_utilization_pct_str` percent utilization** (`{python} StorageBandwidth.sata_max_img_sec_str`/`{python} StorageBandwidth.max_img_per_sec_str`). Storage physics dictates training speed.
...
This calculation illustrates the general principle governing data pipelines: training throughput is bounded by the lower of compute capacity and the data supply rate. The bottleneck relation in @eq-training-throughput captures this limit, with the data supply rate defined in @eq-data-supply.

$$\text{Training Throughput} = \min(\text{Compute Capacity}, \text{Data Supply Rate})$$ {#eq-training-throughput}
\index{Throughput!training, compute vs. data}
...
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: FormatEfficiency.eta_csv_str, FormatEfficiency.eta_parquet_str,
# │          FormatEfficiency.waste_pct_str,
# │          FormatEfficiency.throughput_ratio_str,
# │          FormatEfficiency.needed_cols_str, FormatEfficiency.total_cols_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class FormatEfficiency:
    ...
    waste_pct_str = fmt(waste_pct, precision=1, allow_zero=True, commas=False)
    throughput_ratio_str = fmt(throughput_ratio, precision=1, allow_zero=True, commas=False)
    needed_cols_str = fmt(needed_cols, precision=1, allow_zero=True, commas=False)
    total_cols_str = fmt(total_cols, precision=1, allow_zero=True, commas=False)
```

::: {#nbk-data-engineering-format-efficiency .callout-notebook title="Format efficiency"}

Storage formats determine how much "waste" data must move to retrieve the signal needed for training, as captured in @eq-effective-bandwidth:
...
- **Row-Oriented (CSV)**: Must read all `{python} FormatEfficiency.total_cols_str` columns to get the `{python} FormatEfficiency.needed_cols_str` needed.
    ...
    - **CSV outcome**: The scan wastes `{python} FormatEfficiency.waste_pct_str` percent of disk bandwidth.
- **Column-Oriented (Parquet)**: Reads only the `{python} FormatEfficiency.needed_cols_str` columns needed.
    ...
    - **Parquet outcome**: The pipeline gets **`{python} FormatEfficiency.throughput_ratio_str`$\times$ higher effective throughput**.

**Systems insight**: Switching from CSV to Parquet is not just a file change; it is mathematically equivalent to buying a **`{python} FormatEfficiency.throughput_ratio_str`$\times$ faster hard drive**. @tbl-serialization-cost quantifies the serialization overhead of different formats. For a deeper treatment of row vs. columnar storage layouts and the algebra of data operations (selection, projection, join), see @sec-data-foundations-row-vs-columnar-formats-bca2.
...
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.core.constants (GB, MB)
# │ Exports: CompressionTradeoff.decompress_speedup_str,
# │          CompressionTradeoff.gzip_min_str,
# │          CompressionTradeoff.snappy_min_str,
# │          CompressionTradeoff.time_diff_str,
# │          CompressionTradeoff.total_diff_hours_str,
# │          CompressionTradeoff.snappy_decompress_mbs_str,
# │          CompressionTradeoff.gzip_decompress_mbs_str,
# │          CompressionTradeoff.dataset_gb_str,
# │          CompressionTradeoff.n_epochs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class CompressionTradeoff:
    ...
    dataset_mb = (dataset_gb * GB).m_as(MB)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    decompress_speedup_str = fmt(decompress_speedup, precision=1, allow_zero=True, commas=False)
    gzip_min_str = fmt(gzip_decompress_min, precision=1, allow_zero=True, commas=False)
    snappy_min_str = fmt(snappy_decompress_min, precision=1, allow_zero=True, commas=False)
    time_diff_str = fmt(time_diff_min, precision=1, allow_zero=True, commas=False)
    total_diff_hours_str = fmt(total_diff_hours, precision=1, allow_zero=True, commas=False)
    snappy_decompress_mbs_str = fmt(snappy_decompress_mbs, precision=1, allow_zero=True, commas=False)
    gzip_decompress_mbs_str = fmt(gzip_decompress_mbs, precision=1, allow_zero=True, commas=False)
    compress_dataset_gb_str = fmt(dataset_gb, precision=1, allow_zero=True, commas=False)
    n_epochs_str = fmt(n_epochs, precision=1, allow_zero=True, commas=False)
```

Compression algorithm\index{Compression!algorithm selection} selection involves trade-offs between compression ratio and decompression speed. While gzip achieves higher compression ratios of six to eight times, Snappy\index{Snappy!decompression speed} achieves only two to three times compression but decompresses at `{python} CompressionTradeoff.snappy_decompress_mbs_str` megabytes per second, roughly `{python} CompressionTradeoff.decompress_speedup_str` times faster than gzip's `{python} CompressionTradeoff.gzip_decompress_mbs_str` megabytes per second. For ML training where throughput matters more than storage costs, Snappy's speed advantage often outweighs gzip's space savings. Training on a `{python} CompressionTradeoff.compress_dataset_gb_str` gigabyte dataset compressed with gzip requires `{python} CompressionTradeoff.gzip_min_str` minutes of decompression time, while Snappy requires only `{python} CompressionTradeoff.snappy_min_str` minutes. When training iterates over data for `{python} CompressionTradeoff.n_epochs_str` epochs, this `{python} CompressionTradeoff.time_diff_str`-minute difference per epoch compounds to `{python} CompressionTradeoff.total_diff_hours_str` hours total, potentially the difference between running experiments overnight vs. waiting multiple days for results. The choice cascades through the system: faster decompression enables higher input throughput, reduced buffering requirements (less decompressed data needs staging), and better GPU utilization (less time idle waiting for data).
...
# │
# │ Imports: mlsysim.core.constants (KB, GB), mlsysim.book (fmt, check)
# │ Exports: LifecycleStorageRequirements.train_img_per_sec_str,
# │          LifecycleStorageRequirements.image_size_kb_str,
# │          LifecycleStorageRequirements.training_bw_gbs_str,
# │          LifecycleStorageRequirements.storage_reduction_str,
# │          LifecycleStorageRequirements.feature_vector_kb_str,
# │          LifecycleStorageRequirements.serving_rps_str,
# │          LifecycleStorageRequirements.feature_reads_per_request_str,
# │          LifecycleStorageRequirements.random_reads_per_sec_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class LifecycleStorageRequirements:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    training_bw_gbs = (train_img_per_sec * image_size_kb * KB).m_as(GB)
    storage_reduction = image_size_kb / feature_vector_kb
    random_reads_per_sec = serving_rps * feature_reads_per_request

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(round(training_bw_gbs) == 6, f"40K img/s at 150 KB should be about 6 GB/s, got {training_bw_gbs:.1f}.")
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    train_img_per_sec_str = fmt(train_img_per_sec, precision=1, allow_zero=True, commas=True)
    image_size_kb_str = fmt(image_size_kb, precision=1, allow_zero=True, commas=False)
    training_bw_gbs_str = fmt(training_bw_gbs, precision=1, allow_zero=True, commas=False)
    storage_reduction_str = fmt(storage_reduction, precision=1, allow_zero=True, commas=False)
    feature_vector_kb_str = fmt(feature_vector_kb, precision=1, allow_zero=True, commas=False)
    serving_rps_str = fmt(serving_rps, precision=1, allow_zero=True, commas=True)
    feature_reads_per_request_str = fmt(feature_reads_per_request, precision=1, allow_zero=True, commas=False)
    random_reads_per_sec_str = fmt(random_reads_per_sec, precision=1, allow_zero=True, commas=True)
```

Training phase requirements shift dramatically toward throughput. Modern deep learning processes massive datasets repeatedly across dozens or hundreds of epochs, making I/O efficiency critical. Training ResNet-50 on ImageNet across eight GPUs at `{python} LifecycleStorageRequirements.train_img_per_sec_str` images per second would require roughly `{python} LifecycleStorageRequirements.training_bw_gbs_str` GB/s for `{python} LifecycleStorageRequirements.image_size_kb_str` KB compressed images, and substantially more if decoded FP32 tensors are staged. Storage unable to sustain this throughput idles GPUs, directly increasing infrastructure costs. The feature computation placement trade-off (@sec-data-engineering-feature-computation-placement-a998) is especially acute here: precomputing features achieves `{python} LifecycleStorageRequirements.storage_reduction_str`-fold storage reduction (`{python} LifecycleStorageRequirements.image_size_kb_str` KB images to `{python} LifecycleStorageRequirements.feature_vector_kb_str` KB vectors) but introduces staleness risk when extraction logic changes.

Deployment and serving requirements prioritize low-latency random access.\index{Inference!real-time, storage requirements} A recommendation system serving `{python} LifecycleStorageRequirements.serving_rps_str` requests per second with 10 ms latency budgets and `{python} LifecycleStorageRequirements.feature_reads_per_request_str` feature reads per request requires `{python} LifecycleStorageRequirements.random_reads_per_sec_str` random reads per second, achievable only through in-memory databases like Redis\index{In-memory Database!feature serving}\index{In-memory Database!ML serving} or aggressive caching. Edge deployment\index{Edge Deployment!storage} adds further constraints: limited device storage, intermittent connectivity, and the need for model updates without disrupting inference, typically addressed through tiered storage\index{Tiered Storage!edge deployment} where models cache locally while reference data pulls from the cloud. Model versioning\index{Model Versioning!deployment patterns} must support smooth transitions between versions, rapid rollback, and serving multiple versions simultaneously for A/B testing, operational patterns examined in @sec-ml-operations.
...
\index{Documentation Debt!data provenance}Documentation debt accumulates when data provenance, meaning, and quality characteristics go unrecorded. Datasets without data cards [@gebru2021], unlabeled columns, and undocumented transformations create debt that compounds when original authors leave organizations. A survey of production ML systems found that 40 percent of data quality incidents traced to misunderstanding data semantics due to missing documentation [@sambasivan2021]. The symptoms are unmistakable: columns whose meanings must be reverse-engineered from usage patterns, missing provenance records that block compliance audits, undocumented assumptions that cause silent failures when those assumptions change, and absent quality metrics that prevent informed dataset selection.
...
\index{Quality Debt!uncorrected errors}Quality debt consists of known data errors that remain uncorrected due to time or resource constraints. A dataset with 3 percent known label errors represents quality debt: each training run on this data produces models carrying those errors. Quality debt compounds through a particularly dangerous feedback loop: models trained on erroneous data make predictions that become training data for downstream systems, amplifying the original errors across the ecosystem. Concrete indicators include known label error rates not yet corrected, documented biases not yet mitigated, identified duplicates not yet deduplicated, and detected drift not yet addressed through retraining.
...
Most production ML failures trace to data, not models.[^fn-failure-data-dominance] Industry experience suggests a consistent pattern: training-serving skew accounts for 30–40 percent of failures, data drift for another 20–30 percent, and label quality issues for 15–25 percent. Model architecture, the component engineers most naturally investigate first, accounts for only 10–15 percent. *Debugging the model before verifying data consistency wastes engineering cycles on the wrong tenth of the problem space.*

[^fn-failure-data-dominance]: **ML Failure Distribution**: The 65--95 percent data-dominance ratio is synthesized from industry reports at Uber, Google, and Meta and holds across domains despite variation in absolute proportions. The invariant has a structural explanation: model architecture is tested before deployment, but data distributions shift continuously after deployment. This asymmetry means model bugs are caught by validation; data bugs are caught by users -- making data infrastructure the primary reliability investment, not model sophistication. \index{ML Failures!data dominance}
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: data-engineering-summary-recap-calc
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.constants (CLOUD_GPU_TRAINING_PER_HOUR, MILLION,
# │          USD, hour), mlsysim.book (fmt, check)
# │ Exports: DataEngineeringSummaryRecap.ratio_low_str,
# │          DataEngineeringSummaryRecap.ratio_high_str,
# │          DataEngineeringSummaryRecap.kws_dataset_size_m_round_str,
# │          DataEngineeringSummaryRecap.kws_dataset_size_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class DataEngineeringSummaryRecap:
    ...
    train_hours_low = 4
    train_hours_high = 6
    gpu_hour_cost_low = 2.0
    gpu_hour_cost_high = CLOUD_GPU_TRAINING_PER_HOUR.m_as(USD/hour)
    kws_dataset_size_m = 23.4
    kws_dataset_size_gb = KWSStorageEconomics.dataset_size_gb
    ...
    ratio_low = labeling_budget/train_cost_high
    ratio_high = labeling_budget/train_cost_low
    kws_total_examples = kws_dataset_size_m * MILLION

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(ratio_low > 500, f"Labeling/compute ratio unexpectedly low: {ratio_low:.0f}x.")
    check(ratio_high > ratio_low, "Labeling/compute ratio range must be ordered.")
    check(kws_total_examples > 20 * MILLION, f"KWS corpus unexpectedly small: {kws_total_examples:.0f}.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    ratio_low_str = fmt(ratio_low, precision=1, allow_zero=True, commas=True)
    ratio_high_str = fmt(ratio_high, precision=1, allow_zero=True, commas=True)
    kws_dataset_size_m_round_str = fmt(int(round(kws_dataset_size_m)), precision=1, allow_zero=True)
    kws_dataset_size_gb_str = fmt(kws_dataset_size_gb, precision=1, allow_zero=True, commas=False)
```
...
* **Labeling costs dominate and require substantial resource allocation**: Labeling can cost hundreds to more than a thousand times one optimized training run; in the reference calculation above, the ratio is `{python} DataEngineeringSummaryRecap.ratio_low_str`--`{python} DataEngineeringSummaryRecap.ratio_high_str`$\times$. Labeling is the serial bottleneck that parallelization cannot solve.
* **Storage hierarchy determines iteration speed**: The `{python} StorageLoading.load_speedup_str`$\times$ throughput gap between local NVMe (`{python} StorageLoading.nvme_bw_gbs_str` GB/s) and cloud object storage (100 MB/s) determines whether iterations occur daily or weekly.
