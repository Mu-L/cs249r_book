...
- **Duty Cycle**: Always-on (`{python} FalsePositiveTarget.hr_str`/day).
...
- **Labeling** (~`{python} BudgetAllocation.labeling_pct_str`): `{python} BudgetAllocation.labeling_k_str` K available
- **Storage/Processing** (~`{python} BudgetAllocation.storage_pct_str`): `{python} BudgetAllocation.storage_k_str` K
- **Governance/Other** (~`{python} BudgetAllocation.governance_pct_str`): `{python} BudgetAllocation.governance_k_str` K

At USD `{python} BudgetAllocation.cost_per_label_str`/label with `{python} BudgetAllocation.review_overhead_pct_str` review overhead: `{python} BudgetAllocation.labeling_k_str` K ÷ `{python} BudgetAllocation.effective_cost_str` = **`{python} BudgetAllocation.labeled_k_str` K labeled examples**
...
1.  **Throughput**: `{python} RealtimeCost.events_per_sec_str` events/sec $\times$ `{python} RealtimeCost.event_size_kb_str`/event = **`{python} RealtimeCost.throughput_str`/s**.
2.  **Stream requirements**: To sustain `{python} RealtimeCost.throughput_str`/s with < 100 ms latency, the system needs ~50 primary cores plus ~50 redundant cores, or `{python} RealtimeCost.stream_cores_str` always-on cores total. Cost: `{python} RealtimeCost.stream_cores_str` cores $\times$ `{python} RealtimeCost.stream_hours_str` $\times$ USD `{python} RealtimeCost.stream_cost_per_hr_str`/hr = **USD `{python} RealtimeCost.stream_cost_str`/day**.
3.  **Batch requirements**: Process `{python} RealtimeCost.hourly_tb_str` (`{python} RealtimeCost.batch_window_hr_str` data) in `{python} RealtimeCost.batch_window_min_str`. High throughput (sequential I/O) is efficient. The batch design needs `{python} RealtimeCost.batch_cores_str` cores for `{python} RealtimeCost.batch_window_min_str`/hour $\times$ `{python} RealtimeCost.hours_per_day_str` = `{python} RealtimeCost.batch_core_hours_str` core-hours/day. Cost: `{python} RealtimeCost.batch_core_hours_str` $\times$ USD `{python} RealtimeCost.stream_cost_per_hr_str` = **USD `{python} RealtimeCost.batch_cost_str`/day**.
...
1.  **ETL approach**: Transform before loading. Compute all three aggregation windows in a Spark cluster before loading into the warehouse.
    *   Spark compute: `{python} EtlEltCost.daily_raw_tb_str` at USD `{python} EtlEltCost.spark_per_tb_str`/TB = USD `{python} EtlEltCost.etl_spark_str`/day
    *   Storage: `{python} EtlEltCost.etl_datasets_str` transformed datasets, ~`{python} EtlEltCost.etl_tb_each_str` each = `{python} EtlEltCost.etl_total_storage_tb_str` at USD `{python} EtlEltCost.s3_per_tb_mo_str`/TB per month = USD `{python} EtlEltCost.etl_storage_str`/month
...
2.  **ELT approach**: Load raw data first, transform in warehouse.
    *   Storage: `{python} EtlEltCost.daily_raw_tb_str` raw/day, `{python} EtlEltCost.retention_days_str`-day retention = `{python} EtlEltCost.elt_storage_tb_str` at USD `{python} EtlEltCost.s3_per_tb_mo_str`/TB per month = USD `{python} EtlEltCost.elt_storage_str`/month
...
The performance difference directly impacts iteration velocity. Training that loads data at `{python} StorageLoading.nvme_bw_gbs_str`/s completes dataset loading in `{python} StorageLoading.nvme_load_str`, compared to `{python} StorageLoading.obj_load_str` at typical object storage speeds. This `{python} StorageLoading.load_speedup_str`$\times$ difference determines whether teams can iterate multiple times daily or must wait hours between experiments.
...
*   **Maximum theoretical throughput**:
    `{python} StorageBandwidth.a100_flops_t_str` $\times 10^{12}$ FLOP/s / `{python} StorageBandwidth.resnet_flops_g_str` $\times 10^{9}$ FLOPs/img = **`{python} StorageBandwidth.max_img_per_sec_str`/sec**

**The data term (bandwidth requirement)**

*   **Image size**: `{python} StorageBandwidth.image_size_kb_str` (JPEG compressed).
*   **Required bandwidth**:
    `{python} StorageBandwidth.max_img_per_sec_str` img/sec $\times$ `{python} StorageBandwidth.image_size_kb_str`/img ≈ **`{python} StorageBandwidth.required_bw_gbs_str`/s**

**Storage options**:
To saturate a single A100, storage must deliver **`{python} StorageBandwidth.required_bw_gbs_str`/s**.

*   **S3 Standard**: ~`{python} StorageBandwidth.s3_bw_mbs_str`/s per thread. The pipeline needs **`{python} StorageBandwidth.s3_workers_str` concurrent worker threads**.
*   **SATA SSD**: ~`{python} StorageBandwidth.sata_bw_mbs_str`/s sequential. Totally insufficient (bottleneck).
*   **NVMe SSD**\index{NVMe SSD!data pipeline}: approximately `{python} StorageBandwidth.nvme_bw_low_gbs_str`–`{python} StorageBandwidth.nvme_bw_high_gbs_str`/s. **Required.**

**Systems insight**: With SATA SSDs, maximum throughput is capped at `{python} StorageBandwidth.sata_bw_mbs_str`/s / `{python} StorageBandwidth.image_size_kb_str` ≈ `{python} StorageBandwidth.sata_max_img_sec_str` img/s. The USD 15,000 GPU will run at **`{python} StorageBandwidth.sata_utilization_pct_str` utilization** (`{python} StorageBandwidth.sata_max_img_sec_str`/`{python} StorageBandwidth.max_img_per_sec_str`). Storage physics dictates training speed.

:::

The `{python} StorageBandwidth.sata_bw_mbs_str`/s figure represents SATA III sequential read throughput (the interface maximum is 600 MB/s). Real-world random read performance with small files can be significantly lower.
...
Training phase requirements shift dramatically toward throughput. Modern deep learning processes massive datasets repeatedly across dozens or hundreds of epochs, making I/O efficiency critical. Training ResNet-50 on ImageNet across eight GPUs at `{python} LifecycleStorageRequirements.train_img_per_sec_str` per second would require roughly `{python} LifecycleStorageRequirements.training_bw_gbs_str`/s for `{python} LifecycleStorageRequirements.image_size_kb_str` compressed images, and substantially more if decoded FP32 tensors are staged. Storage unable to sustain this throughput idles GPUs, directly increasing infrastructure costs. The feature computation placement trade-off (@sec-data-engineering-feature-computation-placement-a998) is especially acute here: precomputing features achieves `{python} LifecycleStorageRequirements.storage_reduction_str`-fold storage reduction (`{python} LifecycleStorageRequirements.image_size_kb_str` to `{python} LifecycleStorageRequirements.feature_vector_kb_str` vectors) but introduces staleness risk when extraction logic changes.
...
* **Storage hierarchy determines iteration speed**: The `{python} StorageLoadingRecap.load_speedup_str`$\times$ throughput gap between local NVMe (`{python} StorageLoadingRecap.nvme_bw_gbs_str`/s) and cloud object storage (100 MB/s) determines whether iterations occur daily or weekly.
