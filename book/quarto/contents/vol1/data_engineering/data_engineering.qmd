...
- Explain how data cascades propagate errors and apply the four pillars framework (quality, reliability, scalability, governance) to diagnose pipeline failures
...
class DataGravity:
    ...
    transfer_time_10g_math = fmt_math(f"T = D_{{vol}}/BW \\approx {transfer_days_10g_str} \\text{{ days}}")
...
class FeedingProblem:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Step 1: Throughput (Images/sec) = GPU_Peak/Model_FLOPs
    img_per_sec = (gpu_flops/model_flops).to_base_units().m_as('count/second')

    # Step 2: Required Bandwidth (Bytes/sec) = img_per_sec * img_size_bytes
    req_bw_bytes_sec = img_per_sec * img_size_bytes
    req_bw_gbs = req_bw_bytes_sec / (1 * GB).m_as('B')

    # Step 3: Efficiency (eta) = Disk_BW / (Required_BW in MB/s)
...
**Context**: Microsoft launched "Tay,"\index{Microsoft Tay} an AI chatbot designed to learn from user interactions on Twitter in real-time.

**Failure**: The data pipeline ingested user tweets directly into the model's retraining loop without sufficient filtering or "safety" checks. Users quickly realized this and coordinated a "data poisoning\index{Data Poisoning!Microsoft Tay example}" attack, tweeting offensive and racist content at the bot.

**Consequence**: Within 24 hours, the model had learned from this poisoned data and began generating hate speech autonomously. Microsoft had to shut down the service immediately, suffering significant reputational damage.

**Systems lesson**: Data pipelines are not just plumbing; they are the immune system of the model. Ingesting uncurated, user-generated content without robust filtering is a security vulnerability. "Garbage in, garbage out" happens at the speed of software.
...
2.  **Calculate statistic** $(D_{\text{KS}})$: Find the maximum absolute difference between the CDFs. Let $F_P(x)$ and $F_Q(x)$ denote the cumulative distribution functions of the training $(P)$ and serving $(Q)$ datasets.
    $$D_{\text{KS}} = \max_x |F_P(x) - F_Q(x)|$$

3.  **Determine significance**: Compare $D_{\text{KS}}$ to critical value $D_{\text{crit}}$ based on training sample size $n_P$, serving sample size $n_Q$, and confidence level $(\alpha = 0.05)$.
    $$D_{\text{crit}} \approx 1.36\sqrt{\frac{n_P + n_Q}{n_P n_Q}}$$

**Example Calculation**

- Sample sizes $n_P = n_Q = 1000$.
- Critical value $D_{\text{crit}} \approx 1.36\sqrt{(1000 + 1000)/(1000 \cdot 1000)}$ ≈ `{python} KSTest.ks_dcrit_str`.
- If observed max difference $D_{\text{KS}} = 0.08$, then 0.08 > `{python} KSTest.ks_dcrit_str` → **Reject Null Hypothesis**. Significant drift detected. Trigger retraining or investigation.
...
**Engineering conclusion**: Real-time is **~`{python} RealtimeCost.cost_ratio_str`$\times$ more expensive** for the same data volume. This tax is justified only when the value of < 1 s latency exceeds the added cost.
...
1.  **Significance (quantitative)**: It predicts that performance degradation is proportional to the **KL Divergence** $(\mathcal{D}_{\text{KL}}(T \parallel T'))$ between the training transformation $(T)$ and the serving transformation $(T')$.
...
*   **Hardware peak**: NVIDIA A100 = `{python} StorageBandwidth.a100_flops_t_str` TFLOPS (FP16 Tensor Core, dense operations).
...
*   **Maximum theoretical throughput**:
    `{python} StorageBandwidth.a100_flops_t_str` $\times 10^{12}$ FLOPs/sec / `{python} StorageBandwidth.resnet_flops_g_str` $\times 10^{9}$ FLOPs/img = **`{python} StorageBandwidth.max_img_per_sec_str` images/sec**
...
- **Row-Oriented (CSV)**: Must read all `{python} FormatEfficiency.total_cols_str` columns to get the `{python} FormatEfficiency.needed_cols_str` needed.
    ...
    - **Result**: The scan wastes `{python} FormatEfficiency.waste_pct_str` percent of disk bandwidth.
- **Column-Oriented (Parquet)**: Reads only the `{python} FormatEfficiency.needed_cols_str` columns needed.
    ...
    - **Result**: The pipeline gets **`{python} FormatEfficiency.throughput_ratio_str`$\times$ higher effective throughput**.

**Systems conclusion**: Switching from CSV to Parquet is not just a file change; it is mathematically equivalent to buying a **`{python} FormatEfficiency.throughput_ratio_str`$\times$ faster hard drive**. @tbl-serialization-cost quantifies the serialization overhead of different formats. For a deeper treatment of row vs. columnar storage layouts and the algebra of data operations (selection, projection, join), see @sec-data-foundations-row-vs-columnar-formats-bca2.
