...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    CLOUD_EGRESS_PER_GB, TPU_V4_PER_HOUR, NETWORK_100G_BW,
...
class DataGravity:
    ...
    network_gbs = network_bw.m_as(GB/second)
    network_gbps = network_bw.m_as(Gbps)
    ...
    network_gbps_str = fmt(network_gbps, precision=0, commas=False)
...
class FeedingProblem:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    gpu_peak_rate = A100_FLOPS_FP16_TENSOR
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Step 1: Throughput (images/s) = R_peak / model FLOPs
    img_per_sec = (gpu_peak_rate/model_flops).to_base_units().m_as('count/second')
    ...
    eta_feed = min(disk_bw_mbs / (req_bw_bytes_sec / (1 * MB).m_as('B')), 1.0)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import CLOUD_GPU_TRAINING_PER_HOUR, THOUSAND, USD, hour
from mlsysim.fmt import fmt, check
...
class LabelingComputeRatio:
    ...
    ratio_low_str = fmt(ratio_low, precision=0, commas=True)
    ratio_high_str = fmt(ratio_high, precision=0, commas=True)
```
...
**The context**: Microsoft launched "Tay,"\index{Microsoft Tay} an AI chatbot designed to learn from user interactions on Twitter in real-time.

**The failure**: The data pipeline ingested user tweets directly into the model's retraining loop without sufficient filtering or "safety" checks. Users quickly realized this and coordinated a "data poisoning\index{Data Poisoning!Microsoft Tay example}" attack, tweeting offensive and racist content at the bot.
...
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.core.constants import KS_TEST_COEFFICIENT
from mlsysim.fmt import fmt, check
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import KB, GB, TB, MILLION, SEC_PER_HOUR
from mlsysim.fmt import fmt, check
...
class RealtimeCost:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(throughput_gbs - 1.0) < 1e-9, f"1M 1KB events/sec should be 1 GB/s, got {throughput_gbs:.2f}.")
...
\index{Feature Engineering!computation placement}For ML pipelines, an additional decision extends beyond ETL vs. ELT: where to compute features. This choice significantly impacts training speed, storage costs, and reproducibility.
...
1.  **Significance (quantitative)**: It predicts that performance degradation is proportional to the **KL Divergence** $(\mathcal{D}_{\text{KL}}(p_{g_{\text{train}}} \lVert p_{g_{\text{serve}}}))$ between feature distributions induced by the training transformation $g_{\text{train}}$ and serving transformation $g_{\text{serve}}$.
...
\node[Box,below=of B4](B5){\textbf{Transfer Learning\index{Transfer Learning!labeling alternative}:}\\ Use models
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MILLION, SEC_PER_HOUR
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    STORAGE_COST_S3_STD, STORAGE_COST_NVME_LOW, STORAGE_COST_NVME_HIGH,
    GB, TB, USD, ureg, byte, MILLION
)
from mlsysim.fmt import fmt, check
...
class KWSStorageEconomics:
    ...
    storage_s3_cost_str = fmt(storage_s3_cost, precision=0, commas=False)
    storage_nvme_low_cost_str = fmt(storage_nvme_low_cost, precision=0, commas=False)
    storage_nvme_high_cost_str = fmt(storage_nvme_high_cost, precision=0, commas=False)
...
from mlsysim.core.constants import Bparam, Mparam, GB, BYTES_FP32, BYTES_FP16, BILLION, MILLION
from mlsysim.fmt import fmt, check, MarkdownStr
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware, Models
from mlsysim.core.constants import KB, MB, GB, byte, flop, GFLOPs, TFLOPs, second
from mlsysim.fmt import fmt, check
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import GB, MB
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import KB, GB
from mlsysim.fmt import fmt, check
...
\index{Data Debt!hidden liability}\index{Technical Debt!data systems}Imagine a KWS system that launched with 98 percent accuracy and excellent training-serving consistency. Six months later, accuracy has slipped to 94 percent, not because anyone changed the model, but because new smartphone microphone hardware shifted the audio distribution, labeling guidelines were updated without retraining, and a schema change in the user metadata pipeline went undocumented. Each is a form of *data debt*: accumulated compromises in data quality, documentation, and infrastructure that compound silently.\index{Technical Debt!data engineering} Unlike code debt, which manifests as slower development velocity, data debt directly degrades model performance and can remain invisible until failures become catastrophic. This section addresses the categories of data debt, how to measure them, and the debugging techniques for diagnosing pipeline failures when debt comes due.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import CLOUD_GPU_TRAINING_PER_HOUR, MILLION, USD, hour
from mlsysim.fmt import fmt, check
