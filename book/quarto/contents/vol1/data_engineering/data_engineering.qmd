...
# │
# │ Imports: mlsysim.core.constants (NETWORK_100G_BW, defaults.CLOUD_EGRESS_PER_GB,
# │              defaults.TPU_V4_PER_HOUR, Gbps, GB, USD, hour, second, MILLION,
...
class DataGravity:
    ...
    egress_cost_gb = defaults.CLOUD_EGRESS_PER_GB.m_as(USD/GB)
    tpu_hourly_cost = defaults.TPU_V4_PER_HOUR.m_as(USD/hour)
...
# │
# │ Imports: mlsysim.core.constants (LABELING_COST_*, defaults.STORAGE_COST_S3_STD, USD),
...
class DataEngineeringCostConstants:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    labeling_cost_crowd_low = defaults.LABELING_COST_CROWD_LOW.m_as(USD)
    labeling_cost_crowd_high = defaults.LABELING_COST_CROWD_HIGH.m_as(USD)
    labeling_cost_box_low = defaults.LABELING_COST_BOX_LOW.m_as(USD)
    labeling_cost_box_high = defaults.LABELING_COST_BOX_HIGH.m_as(USD)
    labeling_cost_medical_low = defaults.LABELING_COST_MEDICAL_LOW.m_as(USD)
    labeling_cost_medical_high = defaults.LABELING_COST_MEDICAL_HIGH.m_as(USD)
    storage_cost_s3 = defaults.STORAGE_COST_S3_STD.m_as(USD/TB/ureg.month)
    retrieval_cost_glacier = defaults.RETRIEVAL_COST_GLACIER.m_as(USD/GB)
...
# │
# │ Imports: mlsysim.core.constants (defaults.CLOUD_GPU_TRAINING_PER_HOUR, THOUSAND, USD,
...
class LabelingComputeRatio:
    ...
    train_hours_low = 4
    train_hours_high = 6
    gpu_hour_cost_low = defaults.FLEET_GPU_HOUR_COST_REF.m_as(USD/hour)
    gpu_hour_cost_high = defaults.CLOUD_GPU_TRAINING_PER_HOUR.m_as(USD/hour)
...
# │
# │ Imports: mlsysim.core.constants (defaults.RETRIEVAL_COST_GLACIER, USD), mlsysim.fmt (fmt, check)
...
# │
# │ Imports: math (sqrt), mlsysim.constants (defaults.KS_TEST_COEFFICIENT),
# │          mlsysim.book (fmt)
...
class KSTest:
    ...
    n_train = 1000
    n_serving = 1000
    coeff = defaults.KS_TEST_COEFFICIENT # 1.36 for alpha=0.05
...
class RealtimeCost:
    ...
    batch_core_hours = batch_cores * (batch_window_min / 60) * 24
...
3.  **Batch requirements**: Process `{python} RealtimeCost.hourly_tb_str` TB (1 hour data) in `{python} RealtimeCost.batch_window_min_str` mins. High throughput (sequential I/O) is efficient. The batch design needs `{python} RealtimeCost.batch_cores_str` cores for `{python} RealtimeCost.batch_window_min_str` mins/hour $\times$ 24 hours = `{python} RealtimeCost.batch_core_hours_str` core-hours/day. Cost: `{python} RealtimeCost.batch_core_hours_str` $\times$ USD `{python} RealtimeCost.stream_cost_per_hr_str` = **USD `{python} RealtimeCost.batch_cost_str`/day**.
...
class KWSStorageEconomics:
    ...
    bytes_per_sample = 2
    storage_cost_s3 = defaults.STORAGE_COST_S3_STD.m_as(USD/TB/ureg.month)
    storage_cost_nvme_low = defaults.STORAGE_COST_NVME_LOW.m_as(USD/TB/ureg.month)
    storage_cost_nvme_high = defaults.STORAGE_COST_NVME_HIGH.m_as(USD/TB/ureg.month)
    nvme_bw_gbs = 5
    obj_bw_gbs = 0.1
...
# │
# │ Imports: mlsysim.core.constants (defaults.CLOUD_GPU_TRAINING_PER_HOUR, MILLION,
