...
The Labeling/Compute Rule. Labeling often costs hundreds to more than a thousand times more than a single optimized training run. In the reference above, a `{python} LabelingComputeRatio.labeling_budget_str` labeling budget compares with `{python} LabelingComputeRatio.train_cost_low_str`--`{python} LabelingComputeRatio.train_cost_high_str` for one 8× A100 ResNet-50 run, a `{python} LabelingComputeRatio.ratio_low_str`--`{python} LabelingComputeRatio.ratio_high_str` ratio.

The 80/20 Split. `{python} LabelingComputeRatio.effort_majority_pct_str` of data engineering effort goes to `{python} LabelingComputeRatio.effort_minority_pct_str` of features: the "long tail" of edge cases, rare categories, and quality exceptions.
...
class StorageBandwidth:
    ...
    a100_flops_t_str = fmt_int(h_a100.peak_flops.m_as(TFLOPs/second), commas=False, suffix=" TFLOP/s")
