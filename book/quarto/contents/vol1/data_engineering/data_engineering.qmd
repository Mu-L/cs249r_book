...
# │ How: Retrieve constants from mlsysim.core.constants and Digital Twins.
# │
# │ Imports: mlsysim.core.constants (*), mlsysim.book (fmt, sci, fmt_math),
...
from mlsysim.fmt import fmt, fmt_sci, fmt_math
...
class DataEngineeringSetup:
    ...
    alexnet_params_str = fmt(alexnet_params_m, precision=1, commas=False)
    ...
    kws_memory_limit_edge_kb_str = fmt(kws_memory_limit_edge_kb, precision=0, commas=False)
    kws_dataset_size_m_str = fmt(kws_dataset_size_m, precision=1, commas=False)
    ...
    kws_languages_str = fmt(kws_languages, precision=0, commas=False)
    kws_dataset_size_gb_str = fmt(kws_dataset_size_gb, precision=0, commas=False)
    kws_sample_duration_ms_low_str = fmt(kws_sample_duration_ms_low, precision=0, commas=False)
    kws_sample_duration_ms_high_str = fmt(kws_sample_duration_ms_high, precision=0, commas=False)
    kws_snr_threshold_db_str = fmt(kws_snr_threshold_db, precision=0, commas=False)
...
# │
# │ Imports: mlsysim.book (fmt), mlsysim.core.constants (THOUSAND)
...
**Projected Outcome**: 97–99 percent estimated accuracy (meeting target), \$145K spend (under budget), 48 KB model (under limit).
...
: **Data Engineering Cost Constants (2024)**: Per-unit costs that ML engineers should memorize in the same way systems engineers memorize latency numbers. Labeling dominates the budget by two to three orders of magnitude over storage and compute, which is why the 1000$\times$ rule below holds. {#tbl-data-engineering-cost-constants}
...
For our KWS system, `{python} DataEngineeringSetup.kws_dataset_size_m_round_str` million training examples across `{python} DataEngineeringSetup.kws_languages_str` languages demand a volume that manual collection cannot economically provide. The multi-source strategy introduced in @sec-data-engineering-framework-application-keyword-spotting-case-study-c8e9, combining curated datasets with web scraping, crowdsourcing, and synthetic generation, addresses this scale requirement while maintaining coverage across acoustic environments and speaker demographics.
...
# │
# │ Imports: mlsysim.core.constants (KB, GB, TB), mlsysim.book (fmt)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import KB, GB, TB
...
**Engineering conclusion**: ETL saves USD `{python} EtlEltCost.savings_str`/month in storage but costs 8$\times$ more engineering time per schema change. The exact break-even point depends on engineering labor cost and schema-change frequency: stable schemas favor ETL's lower storage cost, while frequent feature-definition changes favor ELT's faster iteration.
...
Scalability in the KWS pipeline manifests at multiple stages. Development uses single-machine processing on sample datasets to iterate rapidly. Training at scale requires distributed processing when dataset size (`{python} DataEngineeringSetup.kws_dataset_size_m_round_str` million examples) exceeds single-machine capacity or when multiple experiments run concurrently. The processing pipeline parallelizes naturally: audio files are independent, so transforming them requires no coordination between workers. Each worker reads its assigned audio files from distributed storage, computes features, and writes results back—a trivially parallel pattern achieving near-linear scalability. Production deployment requires real-time processing on edge devices with severe resource constraints (our `{python} DataEngineeringSetup.kws_memory_limit_edge_kb_str` kilobyte memory limit), necessitating careful optimization and quantization to fit processing within device capabilities.
...
Segmentation maps\index{Semantic Segmentation!pixel-level labels} provide the most comprehensive information by classifying objects at the pixel level, highlighting each object in a distinct color. For our traffic monitoring system, this might mean precisely outlining each vehicle, pedestrian, and road sign. These detailed annotations significantly increase our storage and processing requirements. A segmentation mask for a $1920{\times}1080$ image requires about two million labels (one per pixel), compared to perhaps ten bounding boxes or a single classification label. If each box stores four coordinates, that is roughly 50,000$\times$ more scalar label entries than ten boxes before accounting for per-value encoding, and the hours required per image for manual segmentation make this approach suitable only when pixel-level precision is essential.
...
\index{Keyword Spotting!automated labeling}Our KWS case study has now progressed through problem definition (@sec-data-engineering-framework-application-keyword-spotting-case-study-c8e9), data collection, ingestion, and processing. At the labeling stage, we confront a challenge unique to speech systems at scale. Generating millions of labeled wake word samples without proportional human annotation cost requires moving beyond the manual and crowdsourced approaches we examined earlier. The Multilingual Spoken Words Corpus (MSWC) [@mazumder2021multilingual] demonstrates how automated labeling addresses this challenge through its innovative approach to generating labeled wake word data; the corpus contains over `{python} DataEngineeringSetup.kws_dataset_size_m_str` million one-second spoken examples across 340,000 keywords in `{python} DataEngineeringSetup.kws_languages_str` different languages.

This scale makes manual annotation infeasible: `{python} DataEngineeringSetup.kws_dataset_size_m_str` million examples at even 10 seconds per label would require approximately `{python} DataEngineeringSetup.kws_label_hours_str` person-hours, roughly `{python} DataEngineeringSetup.kws_label_years_str` person-years of full-time effort. Achieving `{python} DataEngineeringSetup.kws_accuracy_target_str` percent accuracy across diverse environments requires millions of training examples covering acoustic variations (background noises, speaking styles, recording environments), and transparent sourcing across `{python} DataEngineeringSetup.kws_languages_str` languages ensures the technology serves diverse speaker populations.
...
@tbl-storage-performance reveals why ML systems employ tiered storage architectures. Consider the economics of storing our KWS training dataset (`{python} DataEngineeringSetup.kws_dataset_size_gb_str` GB): object storage costs \$`{python} DataEngineeringSetup.kws_storage_s3_cost_str`/month, enabling affordable long-term retention of raw audio, while maintaining working datasets on NVMe[^fn-nvme-gpu-utilization] for active training costs \$`{python} DataEngineeringSetup.kws_storage_nvme_low_cost_str`–`{python} DataEngineeringSetup.kws_storage_nvme_high_cost_str`/month but provides 50$\times$ faster data loading.
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: StorageLoadSpeedup.nvme_load_str, StorageLoadSpeedup.obj_load_str,
# │          StorageLoadSpeedup.load_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
Modern ML models contain millions to trillions of parameters\index{Model Parameters!storage requirements} requiring storage and retrieval patterns fundamentally different from traditional data. GPT-3 [@brown2020language] requires approximately `{python} DataEngineeringSetup.gpt3_fp32_str` gigabytes for model weights when stored in FP32 format (`{python} DataEngineeringSetup.gpt3_params_str` billion parameters times 4 bytes), though practical deployments typically use FP16 (`{python} DataEngineeringSetup.gpt3_fp16_str` GB) or quantized formats for reduced storage and faster inference (see @sec-model-compression for quantization techniques). Even at FP16 precision, this exceeds many organizations' entire operational databases. The trajectory reveals accelerating scale: from AlexNet's `{python} DataEngineeringSetup.alexnet_params_str` million parameters [@krizhevsky2012imagenet] to GPT-3's `{python} DataEngineeringSetup.gpt3_params_str` billion parameters [@brown2020language], model size grew approximately `{python} DataEngineeringSetup.model_growth_str`-fold in eight years (`{python} DataEngineeringSetup.alexnet_params_str` M to `{python} DataEngineeringSetup.gpt3_params_str` B parameters). Storage systems must handle these dense numerical arrays efficiently for both capacity and access speed. Unlike typical files where sequential organization matters for readability, model weights benefit from block-aligned storage enabling parallel reads across parameter groups. When multiple GPUs need to read model data from shared storage, whether during training initialization or checkpoint loading, storage systems must deliver aggregate bandwidth approaching network interface limits, often 25 Gbps or higher, without introducing bottlenecks that would idle expensive compute resources.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import KB, MB, GB, byte, flop, GFLOPs, TFLOPs
from mlsysim.fmt import fmt
...
*   **Hardware peak**: NVIDIA A100 = `{python} DataEngineeringSetup.a100_tflops_fp16_str` TFLOPS (FP16 Tensor Core, dense operations).
...
# │
# │ Imports: mlsysim.book (fmt), mlsysim.core.constants (GB, MB)
...
SELECT * FROM training_data TIMESTAMP AS OF 47
```
