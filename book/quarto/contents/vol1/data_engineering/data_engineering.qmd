...
class DataEngineeringSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Energy
    energy_dram_str = fmt(energy_dram_pj, precision=1, commas=False)
    energy_flop_fp32_str = fmt(energy_flop_fp32_pj, precision=1, commas=False)
    dram_multiplier_str = fmt(int(ENERGY_DRAM_ACCESS_PJ.m_as(ureg.picojoule) / ENERGY_FLOP_FP32_PJ.m_as(ureg.picojoule/ureg.count)), precision=0)
    ...
    # Data Prep
    data_prep_effort_low_str = fmt(data_prep_effort_low, precision=1, commas=False)
    data_prep_effort_high_str = fmt(data_prep_effort_high, precision=1, commas=False)

    # KWS
    kws_accuracy_target_str = fmt(kws_accuracy_target, precision=1, commas=False)
    kws_latency_limit_ms_str = fmt(kws_latency_limit_ms, precision=1, commas=False)
    kws_memory_limit_design_kb_str = fmt(kws_memory_limit_design_kb, precision=1, commas=False)
    kws_false_wake_month_str = fmt(kws_false_wake_month, precision=0, commas=False)
    kws_budget_str = fmt(kws_budget/THOUSAND, precision=1, commas=False) + "K"
    ...
    labeling_cost_medical_low_str = fmt(labeling_cost_medical_low, precision=1, commas=False)
    labeling_cost_medical_high_str = fmt(labeling_cost_medical_high, precision=1, commas=False)

    # Storage
    storage_cost_s3_str = fmt(storage_cost_s3, precision=1, commas=False)
    ...
    retrieval_ratio_expedited_standard_str = fmt(retrieval_ratio_expedited_standard, precision=1, commas=False)
```
...
class DataGravity:
    ...
    transfer_cost_str = fmt(transfer_cost, precision=1, commas=True)
    ...
    dataset_gb_str = fmt(dataset_gb, precision=1)
    network_gbps_str = fmt(network_bw.m_as(Gbps), precision=1, commas=False)
...
class FeedingProblem:
    ...
    disk_bw_mbs_str = fmt(disk_bw_mbs, precision=1, commas=False)
...
class BudgetAllocation:
    ...
    review_overhead_pct_str = fmt(review_overhead_pct, precision=1, commas=False)
    labeling_k_str = fmt(labeling_budget/THOUSAND, precision=1, commas=False)
...
class LabelingComputeRatio:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    labeling_budget_str = fmt(labeling_budget/THOUSAND, precision=1, commas=False) + "K"
    train_cost_low_str = fmt(train_cost_low, precision=1, commas=False)
    train_cost_high_str = fmt(train_cost_high, precision=1, commas=False)
...
class KWSAcquisitionScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt(int(round(dataset_size_m)), precision=0)
...
class CohensKappaChance:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    chance_agreement_pct_str = fmt(chance_agreement * 100, precision=1, commas=False)
```
...
class RealtimeCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    throughput_str = fmt(throughput_gbs, precision=1, commas=False)
    hourly_tb_str = fmt(hourly_data_tb, precision=0, commas=False)
    stream_cost_str = fmt(stream_cost_day, precision=1, commas=False)
    batch_cost_str = fmt(batch_cost_day, precision=1, commas=False)
    ...
    event_size_kb_str = fmt(event_size_kb, precision=1, commas=False)
...
class EtlEltCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    etl_spark_str = fmt(etl_spark_daily, precision=1, commas=False)
    etl_storage_str = fmt(etl_storage_mo, precision=1, commas=False)
    elt_storage_tb_str = fmt(elt_storage_tb, precision=1, commas=False)
    elt_storage_str = fmt(elt_storage_mo, precision=1)
    elt_query_str = fmt(elt_query_mo, precision=0, commas=False)
    savings_str = fmt(storage_savings_mo, precision=1)
    etl_spark_monthly_str = fmt(etl_spark_monthly, precision=1)
    etl_total_str = fmt(etl_total_mo, precision=1)
    elt_total_str = fmt(elt_total_mo, precision=1)
    total_savings_str = fmt(total_savings_mo, precision=1)
    daily_raw_tb_str = fmt(daily_raw_tb, precision=1, commas=False)
    spark_per_tb_str = fmt(spark_per_tb, precision=1, commas=False)
    etl_datasets_str = fmt(etl_datasets, precision=0, commas=False)
    etl_tb_each_str = fmt(etl_tb_each, precision=1, commas=False)
    etl_total_storage_tb_str = fmt(etl_storage_tb, precision=1, commas=False)
    s3_per_tb_mo_str = fmt(s3_per_tb_mo, precision=1, commas=False)
    ...
    schema_change_ratio_str = fmt(schema_change_ratio, precision=1, commas=False)
```
...
class KWSQualityThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    snr_threshold_db_str = fmt(snr_threshold_db, precision=1, commas=False)
    sample_duration_ms_low_str = fmt(sample_duration_ms_low, precision=1, commas=False)
    sample_duration_ms_high_str = fmt(sample_duration_ms_high, precision=1, commas=False)
```
...
class KWSScalabilityTargets:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_m_round_str = fmt(int(round(dataset_size_m)), precision=0)
    memory_limit_edge_kb_str = fmt(memory_limit_edge_kb, precision=1, commas=False)
    model_size_limit_kb_str = fmt(model_size_limit_kb, precision=1, commas=False)
```
...
class SegmentationLabelScale:
    ...
    label_entry_ratio_str = fmt(label_entry_ratio, precision=1, commas=True)
```
...
class ActiveLearningBudget:
    ...
    compute_budget_pct_str = fmt(compute_budget_fraction * 100, precision=1, commas=False)
...
class KWSAutomatedLabelingScale:
    ...
    accuracy_target_str = fmt(accuracy_target, precision=1, commas=False)
    languages_str = fmt(languages, precision=0, commas=False)
    sample_duration_ms_low_str = fmt(sample_duration_ms_low, precision=1, commas=False)
    sample_duration_ms_high_str = fmt(sample_duration_ms_high, precision=1, commas=False)
```
...
class KWSStorageEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_gb_str = fmt(dataset_size_gb, precision=1, commas=False)
    ...
    load_speedup_str = fmt(load_speedup, precision=1, commas=False)
```
...
class StorageLoading:
    ...
    obj_load_str = fmt(obj_load_s, precision=1, commas=True)
    load_speedup_str = fmt(load_speedup, precision=1, commas=False)
    nvme_bw_gbs_str = fmt(nvme_bw_gbs, precision=1, commas=False)
```
...
class ModelStorageScale:
    ...
    model_growth_str = fmt(model_growth_factor, precision=1)
```
...
class StorageBandwidth:
    ...
    a100_flops_t_str = fmt(int(h_a100.peak_flops.m_as(TFLOPs/second)), precision=0, commas=False)
    resnet_flops_g_str = fmt(int((resnet_flops_per_img * flop).m_as(GFLOPs)), precision=0, commas=False)
    image_size_kb_str = fmt(image_size_kb, precision=1, commas=False)
    sata_bw_mbs_str = fmt(sata_bw_mbs, precision=1, commas=False)
    sata_max_img_sec_str = fmt(sata_max_img_sec, precision=0)
    sata_utilization_pct_str = fmt(sata_utilization_pct, precision=1, commas=False)
    s3_bw_mbs_str = fmt(s3_bw_mbs, precision=1, commas=False)
...
class FormatEfficiency:
    ...
    waste_pct_str = fmt(waste_pct, precision=1, commas=False)
    throughput_ratio_str = fmt(throughput_ratio, precision=1, commas=False)
...
class CompressionTradeoff:
    ...
    snappy_decompress_mbs_str = fmt(snappy_decompress_mbs, precision=1, commas=False)
    gzip_decompress_mbs_str = fmt(gzip_decompress_mbs, precision=1, commas=False)
    compress_dataset_gb_str = fmt(dataset_gb, precision=1, commas=False)
...
class LifecycleStorageRequirements:
    ...
    image_size_kb_str = fmt(image_size_kb, precision=1, commas=False)
    training_bw_gbs_str = fmt(training_bw_gbs, precision=1, commas=False)
    storage_reduction_str = fmt(storage_reduction, precision=1, commas=False)
    feature_vector_kb_str = fmt(feature_vector_kb, precision=1, commas=False)
...
class DataEngineeringSummaryRecap:
    ...
    ratio_low_str = fmt(ratio_low, precision=1, commas=True)
    ratio_high_str = fmt(ratio_high, precision=1, commas=True)
    kws_dataset_size_m_round_str = fmt(int(round(kws_dataset_size_m)), precision=0)
    kws_dataset_size_gb_str = fmt(kws_dataset_size_gb, precision=1, commas=False)
```
