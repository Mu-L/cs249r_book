...
: **Storage Cost-Performance Trade-Offs**: Different storage tiers provide distinct cost-performance characteristics that determine their suitability for specific ML workloads. Training data loading requires high-throughput sequential access, an I/O pattern that must align with the memory hierarchy in @sec-appdx-machine-foundations-memory-hierarchy-2278. Online serving needs low-latency random reads, while archival storage prioritizes cost over access speed for compliance and historical data. {#tbl-storage-performance tbl-colwidths="[26,15,19,19,21]"}
...
**Systems insight**: Switching from CSV to Parquet is not just a file change; it is mathematically equivalent to buying a `{python} FormatEfficiency.throughput_ratio_str` faster hard drive. @Tbl-serialization-cost quantifies the serialization overhead of different formats. For a deeper treatment of row vs. columnar storage layouts and the algebra of data operations (selection, projection, join), see @sec-appdx-data-foundations-row-vs-columnar-formats-bca2.
