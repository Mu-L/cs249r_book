...
class DataPrepEffortIntro:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    data_prep_effort_low_str = fmt(data_prep_effort_low, precision=0, commas=False, suffix="%")
    data_prep_effort_high_str = fmt(data_prep_effort_high, precision=0, commas=False, suffix="%")
```
...
class DataGravity:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    transfer_seconds_str = fmt(transfer_seconds, precision=0, commas=True, suffix=" seconds")
    transfer_hours_str = fmt_int(round(transfer_hours), commas=False, suffix=" h")
    transfer_days_10g_str = fmt(transfer_days_10g, precision=1, commas=False, suffix=" days")
    transfer_time_10g_math = fmt_math(
        f"T = D_{{\\text{{vol}}}}/\\text{{BW}} \\approx {transfer_days_10g:.1f} \\text{{ days}}"
    )
    transfer_cost_str = fmt(transfer_cost, precision=0, commas=True, prefix="$")
    tpu_hours_str = fmt(equiv_tpu_hours, precision=0, commas=True, suffix=" TPUv4-hours")
    network_gbs_str = fmt(network_bw.m_as(GB/second), precision=1, commas=False, suffix=" GB/s")
    dataset_gb_str = fmt(dataset_gb, precision=0, suffix=" GB")
    network_gbps_str = fmt(network_bw.m_as(Gbps), precision=0, commas=False, suffix=" Gb/s")
    egress_cost_per_gb_str = fmt(egress_cost_gb, precision=2, commas=False, prefix="$", suffix="/GB")
    tpu_cost_per_hour_str = fmt(tpu_hourly_cost, precision=0, commas=False, prefix="$", suffix="/hour")
```
...
class FeedingProblem:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    from mlsysim import Models, Hardware, Engine, Datasets
    ...
    req_bw_gbs = req_bw_bytes_sec / BILLION

    # Step 3: Feeding efficiency = disk BW / required BW
    eta_feed = min(disk_bw_mbs / (req_bw_bytes_sec / MILLION), 1.0)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    req_bw_gbs_str = fmt(req_bw_gbs, precision=1, commas=False, suffix=" GB/s")
    disk_bw_mbs_str = fmt(disk_bw_mbs, precision=0, commas=False, suffix=" MB/s")
    feeding_tax_pct_str = fmt(feeding_tax_pct, precision=1, commas=False, suffix="%")
    img_per_sec_str = fmt_int(img_per_sec, commas=True, suffix=" img/s")
```
...
According to the iron law, the system is only as fast as its slowest term. If a modern accelerator running ResNet-50 can process `{python} FeedingProblem.img_per_sec_str`, but the storage pipeline delivers only `{python} FeedingProblem.disk_bw_mbs_str`, the expensive silicon sits idle. We quantify this as **The Feeding Tax**\index{Feeding Tax!I/O wait cost}: the wall-clock time lost to I/O wait, which directly reduces the system efficiency $(\eta_{\text{hw}})$\index{System Efficiency!feeding tax} term. For a standard cloud volume, the feeding tax can exceed `{python} FeedingProblem.feeding_tax_pct_str`, meaning the GPU spends the majority of its time waiting for bits. This tax transforms the Data Pipeline from a simple storage concern into the primary regulator of the system's Duty Cycle. To saturate a 300 TFLOP/s processor, the pipeline must often sustain `{python} FeedingProblem.req_bw_gbs_str` transfer rates—a requirement that forces the shift from traditional file systems to the specialized storage architectures we examine in @sec-data-engineering-ml-storage-systems-architecture-options-67fa.
...
# │
# │ Imports: mlsysim Hardware.Tech.Memory.HBM3.energy_per_access, Hardware.Tech.Op.FlopFp32.energy;
# │          mlsysim.fmt (fmt, fmt_int, check)
...
class EnergyAccessCosts:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    prune_pct_str = fmt(prune_pct, precision=0, commas=False, suffix="%")
    energy_dram_str = fmt(energy_dram_pj, precision=0, commas=False, suffix=" pJ")
    energy_flop_fp32_str = fmt(energy_flop_fp32_pj, precision=1, commas=False, suffix=" pJ")
    local_ssd_energy_str = fmt(energy_local_ssd_pj * 32, precision=0, commas=True, suffix=" pJ")
    network_energy_str = fmt(energy_network_pj * 32, precision=0, commas=True, suffix=" pJ")
    dram_multiplier_str = fmt_int(energy_dram_pj / energy_flop_fp32_pj, suffix="×")
    local_ssd_multiplier_str = fmt(energy_local_ssd_pj/energy_flop_fp32_pj_per_bit, precision=1, commas=True, suffix="×")
    network_multiplier_str = fmt(energy_network_pj/energy_flop_fp32_pj_per_bit, precision=1, commas=True, suffix="×")
```
...
| **DRAM Memory Access (32-bit)** |    `{python} EnergyAccessCosts.energy_dram_str`   |    `{python} EnergyAccessCosts.dram_multiplier_str`   |
| **Local SSD Access (32-bit)**   | `{python} EnergyAccessCosts.local_ssd_energy_str` | `{python} EnergyAccessCosts.local_ssd_multiplier_str` |
| **Network Transfer (32-bit)**   |  `{python} EnergyAccessCosts.network_energy_str`  |  `{python} EnergyAccessCosts.network_multiplier_str`  |
...
These principles operate at the level of individual files and batches. At datacenter scale, the cost of moving data compounds into a constraint that makes large datasets so expensive to transfer that compute must relocate to the data rather than the reverse. Transferring a petabyte dataset between data centers puts a dollar figure on the constraint.
...
1.  **Network bandwidth**: A dedicated `{python} DataGravity.network_gbps_str` line = `{python} DataGravity.network_gbs_str`.
2.  **Transfer time**: `{python} DataGravity.dataset_gb_str` / `{python} DataGravity.network_gbs_str` = `{python} DataGravity.transfer_seconds_str` ≈ `{python} DataGravity.transfer_hours_str`.
3.  **Cost**: At `{python} DataGravity.egress_cost_per_gb_str` egress, moving 1 PB costs `{python} DataGravity.transfer_cost_str`. (Baseline: AWS data transfer out pricing, 2024.)
...
class KWSProblemTargets:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    kws_accuracy_target_str = fmt(kws_accuracy_target, precision=0, commas=False, suffix="%")
    kws_latency_limit_ms_str = fmt(kws_latency_limit_ms, precision=0, commas=False, suffix=" ms")
```
...
class FalsePositiveTarget:
    ...
    hr_duty_str = MarkdownStr("24 hours/day")
    ...
    rejection_pct_str = fmt(rejection_pct, precision=5, commas=False, suffix="%")
```
...
class BudgetAllocation:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_per_label_str = fmt(cost_per_label, precision=2, commas=False, prefix="$", suffix="/label")
    review_overhead_pct_str = fmt(review_overhead_pct, precision=0, commas=False, suffix="%")
    labeling_k_str = fmt(labeling_budget/THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    storage_k_str = fmt(storage_budget/THOUSAND, precision=1, commas=False, prefix="$", suffix="K")
    governance_k_str = fmt(governance_budget/THOUSAND, precision=1, commas=False, prefix="$", suffix="K")
    effective_cost_str = fmt(effective_cost, precision=2, commas=False, prefix="$", suffix="/label")
    labeled_k_str = fmt(labeled_examples/THOUSAND, precision=0, commas=False, suffix="K")
    labeling_pct_str = fmt(labeling_pct * 100, precision=0, commas=False, suffix="%")
    storage_pct_str = fmt(storage_pct * 100, precision=0, commas=False, suffix="%")
    governance_pct_str = fmt(governance_pct * 100, precision=0, commas=False, suffix="%")
```
...
class KWSDesignConstraints:
    ...
    kws_budget_str = fmt(kws_budget/THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    kws_memory_limit_design_kb_str = fmt(kws_memory_limit_design_kb, precision=0, commas=False, suffix=" KB")
    kws_timeline_months_str = fmt(kws_timeline_months, precision=0, commas=False, suffix=" months")
    kws_base_model_acc_str = fmt(kws_base_model_acc, precision=0, commas=False, suffix="%")
```
...
- **Target**: `{python} KWSDesignConstraints.kws_accuracy_target_str` accuracy, < `{python} KWSDesignConstraints.kws_false_wake_month_str` false wake per month
...
- **Labeling** (~`{python} BudgetAllocation.labeling_pct_str`): `{python} BudgetAllocation.labeling_k_str` available
- **Storage/Processing** (~`{python} BudgetAllocation.storage_pct_str`): `{python} BudgetAllocation.storage_k_str`
- **Governance/Other** (~`{python} BudgetAllocation.governance_pct_str`): `{python} BudgetAllocation.governance_k_str`

At `{python} BudgetAllocation.cost_per_label_str` with `{python} BudgetAllocation.review_overhead_pct_str` review overhead: `{python} BudgetAllocation.labeling_k_str` ÷ `{python} BudgetAllocation.effective_cost_str` = `{python} BudgetAllocation.labeled_k_str` labeled examples
...
**Projected Outcome**: 97–99 percent estimated accuracy, straddling the `{python} KWSDesignConstraints.kws_accuracy_target_str` target, within the `{python} KWSDesignConstraints.kws_budget_str` budget, and a model footprint under the `{python} KWSDesignConstraints.kws_memory_limit_design_kb_str` limit.
...
class LabelingComputeRatio:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    labeling_budget_str = fmt(labeling_budget/THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    train_cost_low_str = fmt(train_cost_low, precision=0, commas=False, prefix="$")
    train_cost_high_str = fmt(train_cost_high, precision=0, commas=False, prefix="$")
    ratio_low_str = fmt(ratio_low, precision=1, commas=True, suffix="×")
    ratio_high_str = fmt(ratio_high, precision=1, commas=True, suffix="×")
    effort_majority_pct = 80
    effort_minority_pct = 20
    effort_majority_pct_str = fmt(effort_majority_pct, precision=0, commas=False, suffix="%")
    effort_minority_pct_str = fmt(effort_minority_pct, precision=0, commas=False, suffix="%")
```

The contrast matters: weeks for human labeling, hours for GPU training, milliseconds for serving. Labeling is the bottleneck. It often costs hundreds to more than a thousand times more than a single optimized training run: a `{python} LabelingComputeRatio.labeling_budget_str` labeling budget compares with `{python} LabelingComputeRatio.train_cost_low_str`--`{python} LabelingComputeRatio.train_cost_high_str` for one 8× A100 ResNet-50 run, a `{python} LabelingComputeRatio.ratio_low_str`--`{python} LabelingComputeRatio.ratio_high_str` ratio. And within that labeling spend, the effort distribution itself is skewed: `{python} LabelingComputeRatio.effort_majority_pct_str` of the work goes to `{python} LabelingComputeRatio.effort_minority_pct_str` of features—the long tail of edge cases, rare categories, and quality exceptions.
...
The choice\index{Data Acquisition!source evaluation} among curated datasets, expert crowdsourcing, and controlled web scraping turns on a single question: which source best covers the deployment distribution at acceptable cost and domain expertise?
...
class CohensKappaChance:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    chance_agreement_pct_str = fmt(chance_agreement * 100, precision=0, commas=False, suffix="%")
```
...
class RealtimeCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    throughput_str = fmt(throughput_gbs, precision=0, commas=False, suffix=" GB/s")
    hourly_tb_str = fmt_int(hourly_data_tb, commas=False, suffix=" TB")
    stream_cost_str = fmt(stream_cost_day, precision=0, commas=False, prefix="$", suffix="/day")
    batch_cost_str = fmt(batch_cost_day, precision=0, commas=False, prefix="$", suffix="/day")
    cost_ratio_str = fmt(cost_ratio, precision=0, commas=False, suffix="×")
    events_per_sec_str = fmt(events_per_sec/MILLION, precision=0, commas=False, suffix="M")
    event_size_kb_str = fmt(event_size_kb, precision=0, commas=False, suffix=" KB")
    stream_cores_str = fmt(stream_cores, precision=0, commas=False)
    stream_hours_str = fmt(stream_hours, precision=0, commas=False, suffix=" hours/day")
    stream_cost_per_hr_str = fmt(stream_cost_per_hr, precision=2, commas=False, prefix="$", suffix="/hr")
    batch_cores_str = fmt(batch_cores, precision=0, commas=False)
    batch_window_min_str = fmt(batch_window_min, precision=0, commas=False, suffix=" minutes")
    batch_core_hours_str = fmt(batch_core_hours, precision=0, commas=False)
    batch_window_hr_str = fmt(1, precision=0, commas=False, suffix="-hour")
...
**Systems insight**: Real-time is ~`{python} RealtimeCost.cost_ratio_str` more expensive for the same data volume. This tax is justified only when the value of < 1 s latency exceeds the added cost.
...
class EtlEltCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    etl_spark_str = fmt(etl_spark_daily, precision=0, commas=False, prefix="$", suffix="/day")
    etl_storage_str = fmt(etl_storage_mo, precision=0, commas=False, prefix="$", suffix="/month")
    elt_storage_tb_str = fmt(elt_storage_tb, precision=0, commas=False, suffix=" TB")
    elt_storage_str = fmt(elt_storage_mo, precision=0, prefix="$", suffix="/month")
    elt_query_str = fmt(elt_query_mo, precision=0, commas=False, prefix="$", suffix="/month")
    savings_str = fmt(storage_savings_mo, precision=0, prefix="$", suffix="/month")
    etl_spark_monthly_str = fmt(etl_spark_monthly, precision=0, prefix="$", suffix="/month")
    etl_total_str = fmt(etl_total_mo, precision=0, prefix="$", suffix="/month")
    elt_total_str = fmt(elt_total_mo, precision=0, prefix="$", suffix="/month")
    total_savings_str = fmt(total_savings_mo, precision=0, prefix="$", suffix="/month")
    daily_raw_tb_str = fmt(daily_raw_tb, precision=0, commas=False, suffix=" TB")
    spark_per_tb_str = fmt(spark_per_tb, precision=0, commas=False, prefix="$", suffix="/TB")
    etl_datasets_str = fmt(etl_datasets, precision=0, commas=False)
    etl_tb_each_str = fmt(etl_tb_each, precision=0, commas=False, suffix=" TB")
    etl_total_storage_tb_str = fmt(etl_storage_tb, precision=0, commas=False, suffix=" TB")
    s3_per_tb_mo_str = fmt(s3_per_tb_mo, precision=0, commas=False, prefix="$", suffix="/TB/month")
    retention_days_str = fmt(retention_days, precision=0, commas=False, suffix=" days")
    query_cost_per_str = fmt(query_cost_per, precision=0, commas=False, prefix="$", suffix="/query")
    n_models_str = fmt(n_models, precision=0, commas=False)
    schema_change_ratio_str = fmt(schema_change_ratio, precision=0, commas=False, suffix="×")
    schema_change_etl_hours_str = fmt(schema_change_etl_hours, precision=0, commas=False, suffix=" hours")
    schema_change_elt_minutes_str = fmt(schema_change_elt_minutes, precision=0, commas=False, suffix=" minutes")
```
...
1.  **ETL approach**: Transform before loading. Compute all three aggregation windows in a Spark cluster before loading into the warehouse.
    *   Spark compute: `{python} EtlEltCost.daily_raw_tb_str` at `{python} EtlEltCost.spark_per_tb_str` = `{python} EtlEltCost.etl_spark_str`
    *   Storage: `{python} EtlEltCost.etl_datasets_str` transformed datasets, ~`{python} EtlEltCost.etl_tb_each_str` each = `{python} EtlEltCost.etl_total_storage_tb_str` at `{python} EtlEltCost.s3_per_tb_mo_str` = `{python} EtlEltCost.etl_storage_str`
...
2.  **ELT approach**: Load raw data first, transform in warehouse.
    *   Storage: `{python} EtlEltCost.daily_raw_tb_str` raw/day, `{python} EtlEltCost.retention_days_str` retention = `{python} EtlEltCost.elt_storage_tb_str` at `{python} EtlEltCost.s3_per_tb_mo_str` = `{python} EtlEltCost.elt_storage_str`
    *   Query compute: `{python} EtlEltCost.n_models_str` models $\times$ `{python} EtlEltCost.query_cost_per_str` $\times$ `{python} EtlEltCost.retention_days_str` = `{python} EtlEltCost.elt_query_str`
    *   Schema change cost: Rewrite SQL query (~`{python} EtlEltCost.schema_change_elt_minutes_str`) per change

**Systems insight**: ETL saves `{python} EtlEltCost.savings_str` in storage. After normalizing Spark compute to a monthly cost, ETL totals `{python} EtlEltCost.etl_total_str` (`{python} EtlEltCost.etl_spark_monthly_str` compute + `{python} EtlEltCost.etl_storage_str` storage), while ELT totals `{python} EtlEltCost.elt_total_str` (`{python} EtlEltCost.elt_storage_str` storage + `{python} EtlEltCost.elt_query_str` query compute), for a cloud-cost advantage of `{python} EtlEltCost.total_savings_str` before engineering labor. ETL still costs `{python} EtlEltCost.schema_change_ratio_str` more engineering time per schema change. The exact break-even point depends on engineering labor cost and schema-change frequency: stable schemas favor ETL's lower cloud cost, while frequent feature-definition changes favor ELT's faster iteration.
...
class KWSQualityThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    snr_threshold_db_str = fmt(snr_threshold_db, precision=0, commas=False, suffix=" dB")
    sample_duration_ms_low_str = fmt(sample_duration_ms_low, precision=0, commas=False, suffix=" ms")
    sample_duration_ms_high_str = fmt(sample_duration_ms_high, precision=0, commas=False, suffix=" ms")
```
...
class SegmentationLabelScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pixel_labels_m_str = fmt(pixel_labels/ MILLION, precision=1, commas=False)
...
class ActiveLearningBudget:
    ...
    label_cost_str = fmt(label_cost, precision=2, commas=False, prefix="$", suffix="/label")
    inference_cost_str = fmt(inference_cost_per_image, precision=2, commas=False, prefix="$", suffix="/image")
    ...
    compute_budget_pct_str = fmt(compute_budget_fraction * 100, precision=0, commas=False, suffix="%")
    ...
    baseline_acc_str = fmt(baseline_acc, precision=0, commas=False, suffix="%")
    target_acc_str = fmt(target_acc, precision=0, commas=False, suffix="%")
```
...
class KWSAutomatedLabelingScale:
    ...
    dataset_size_m_round_str = fmt_int(round(dataset_size_m))
    label_hours_str = fmt(label_hours, precision=0, commas=True, suffix=" hours")
    label_years_str = fmt(label_years, precision=1, commas=False, suffix=" person-years")
    accuracy_target_str = fmt(accuracy_target, precision=0, commas=False, suffix="%")
    languages_str = fmt(languages, precision=0, commas=False)
    sample_duration_ms_low_str = fmt(sample_duration_ms_low, precision=0, commas=False, suffix=" ms")
    sample_duration_ms_high_str = fmt(sample_duration_ms_high, precision=0, commas=False, suffix=" ms")
```
...
This scale makes manual annotation infeasible: `{python} KWSAutomatedLabelingScale.dataset_size_m_str` million examples at even 10 seconds per label would require approximately `{python} KWSAutomatedLabelingScale.label_hours_str`, roughly `{python} KWSAutomatedLabelingScale.label_years_str` of full-time effort. Achieving `{python} KWSAutomatedLabelingScale.accuracy_target_str` accuracy across diverse environments requires millions of training examples covering acoustic variations (background noises, speaking styles, recording environments), and transparent sourcing across `{python} KWSAutomatedLabelingScale.languages_str` languages ensures the technology serves diverse speaker populations.
...
class KWSStorageEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    dataset_size_gb_str = fmt(dataset_size_gb, precision=0, commas=False, suffix=" GB")
    storage_s3_cost_str = fmt(storage_s3_cost, precision=1, commas=False, prefix="$", suffix="/month")
    storage_nvme_low_cost_str = fmt(storage_nvme_low_cost, precision=1, commas=False, prefix="$", suffix="/month")
    storage_nvme_high_cost_str = fmt(storage_nvme_high_cost, precision=1, commas=False, prefix="$", suffix="/month")
    load_speedup_str = fmt(load_speedup, precision=0, commas=False, suffix="×")
```

@Tbl-storage-performance reveals why ML systems employ tiered storage architectures. Consider the economics of storing our KWS training dataset (`{python} KWSStorageEconomics.dataset_size_gb_str`): object storage costs `{python} KWSStorageEconomics.storage_s3_cost_str`, enabling affordable long-term retention of raw audio, while maintaining working datasets on NVMe[^fn-nvme-gpu-utilization] for active training costs `{python} KWSStorageEconomics.storage_nvme_low_cost_str`–`{python} KWSStorageEconomics.storage_nvme_high_cost_str` but provides `{python} KWSStorageEconomics.load_speedup_str` faster data loading.
...
class StorageLoading:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvme_load_str = fmt(nvme_load_s, precision=1, commas=False, suffix=" s")
    obj_load_str = fmt(obj_load_s, precision=0, commas=True, suffix=" s")
    load_speedup_str = fmt(load_speedup, precision=0, commas=False, suffix="×")
    nvme_bw_gbs_str = fmt(nvme_bw_gbs, precision=0, commas=False, suffix=" GB/s")
```

The performance difference directly impacts iteration velocity. Training that loads data at `{python} StorageLoading.nvme_bw_gbs_str` completes dataset loading in `{python} StorageLoading.nvme_load_str`, compared to `{python} StorageLoading.obj_load_str` at typical object storage speeds. This `{python} StorageLoading.load_speedup_str` difference determines whether teams can iterate multiple times daily or must wait hours between experiments.
...
Modern ML models contain millions to trillions of parameters\index{Model Parameters!storage requirements} requiring storage and retrieval patterns fundamentally different from traditional data. GPT-3 [@brown2020language] requires approximately `{python} ModelStorageScale.gpt3_fp32_str` for model weights when stored in FP32 format (`{python} ModelStorageScale.gpt3_params_str` billion parameters times 4 bytes), though practical deployments typically use FP16 (`{python} ModelStorageScale.gpt3_fp16_str`) or quantized formats for reduced storage and faster inference (see @sec-model-compression for quantization techniques). Even at FP16 precision, this exceeds many organizations' entire operational databases. The trajectory reveals accelerating scale: from AlexNet's `{python} ModelStorageScale.alexnet_params_str` million parameters [@krizhevsky2012imagenet] to GPT-3's `{python} ModelStorageScale.gpt3_params_str` billion parameters [@brown2020language], model size grew approximately `{python} ModelStorageScale.model_growth_str`-fold in eight years. Storage systems must handle these dense numerical arrays efficiently for both capacity and access speed. Unlike typical files where sequential organization matters for readability, model weights benefit from block-aligned storage enabling parallel reads across parameter groups. When multiple GPUs need to read model data from shared storage, whether during training initialization or checkpoint loading, storage systems must deliver aggregate bandwidth approaching network interface limits, often 25 Gbps or higher, without introducing bottlenecks that would idle expensive compute resources.
...
class StorageBandwidth:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    a100_flops_val = h_a100.compute.peak_flops.m_as(flop/second)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    max_img_per_sec_str = fmt(max_img_per_sec, precision=0, suffix=" img/s")
    required_bw_gbs_str = fmt(required_bw_gbs, precision=1, commas=False, suffix=" GB/s")
    a100_flops_t_str = fmt_int(h_a100.compute.peak_flops.m_as(TFLOPs/second), commas=False, suffix=" TFLOP/s")
    resnet_flops_g_str = fmt((resnet_flops_per_img * flop).m_as(GFLOPs), precision=1, commas=False, suffix=" GFLOP")
    image_size_kb_str = fmt(image_size_kb, precision=0, commas=False, suffix=" KB")
    sata_bw_mbs_str = fmt(sata_bw_mbs, precision=0, commas=False, suffix=" MB/s")
    sata_max_img_sec_str = fmt(sata_max_img_sec, precision=0, suffix=" img/s")
    sata_utilization_pct_str = fmt(sata_utilization_pct, precision=0, commas=False, suffix="%")
    s3_bw_mbs_str = fmt(s3_bw_mbs, precision=0, commas=False, suffix=" MB/s")
    s3_workers_str = fmt(s3_workers, precision=0, commas=False, suffix=" concurrent worker threads")
    nvme_bw_low_gbs_str = fmt(nvme_bw_low_gbs, precision=0, commas=False, suffix=" GB/s")
    nvme_bw_high_gbs_str = fmt(nvme_bw_high_gbs, precision=0, commas=False, suffix=" GB/s")
```
...
*   **Hardware peak**: NVIDIA A100 = `{python} StorageBandwidth.a100_flops_t_str` (FP16 Tensor Core, dense operations).
*   **Model cost**: ResNet-50 requires approximately four GFLOPs for the forward pass per image; including the backward pass, total is approximately `{python} StorageBandwidth.resnet_flops_g_str` per training step.
*   **Maximum theoretical throughput**: `{python} StorageBandwidth.max_img_per_sec_str` (A100 peak FP16 throughput divided by ResNet-50 forward-plus-backward FLOPs per image).

*The data term (bandwidth requirement)*

*   **Image size**: `{python} StorageBandwidth.image_size_kb_str` (JPEG compressed).
*   **Required bandwidth**:
    `{python} StorageBandwidth.max_img_per_sec_str` $\times$ `{python} StorageBandwidth.image_size_kb_str` per image ≈ `{python} StorageBandwidth.required_bw_gbs_str`

*Storage options:*
To saturate a single A100, storage must deliver `{python} StorageBandwidth.required_bw_gbs_str`.

*   **S3 Standard**: ~`{python} StorageBandwidth.s3_bw_mbs_str` per thread. The pipeline needs `{python} StorageBandwidth.s3_workers_str`.
*   **SATA SSD**: ~`{python} StorageBandwidth.sata_bw_mbs_str` sequential. Totally insufficient (bottleneck).
*   **NVMe SSD**\index{NVMe SSD!data pipeline}: approximately `{python} StorageBandwidth.nvme_bw_low_gbs_str`–`{python} StorageBandwidth.nvme_bw_high_gbs_str`. Required.

**Systems insight**: With SATA SSDs, maximum throughput is capped at `{python} StorageBandwidth.sata_bw_mbs_str` / `{python} StorageBandwidth.image_size_kb_str` per image ≈ `{python} StorageBandwidth.sata_max_img_sec_str`. The USD 15,000 GPU will run at `{python} StorageBandwidth.sata_utilization_pct_str` utilization (`{python} StorageBandwidth.sata_max_img_sec_str` / `{python} StorageBandwidth.max_img_per_sec_str`). Storage physics dictates training speed.

:::

The `{python} StorageBandwidth.sata_bw_mbs_str` figure represents SATA III sequential read throughput (the interface maximum is 600 MB/s). Real-world random read performance with small files can be significantly lower.
...
class FormatEfficiency:
    ...
    waste_pct_str = fmt(waste_pct, precision=0, commas=False, suffix="%")
    throughput_ratio_str = fmt(throughput_ratio, precision=0, commas=False, suffix="×")
...
- **Column-Oriented (Parquet)**: Reads only the `{python} FormatEfficiency.needed_cols_str` columns needed.
    ...
    - **Parquet outcome**: The pipeline gets `{python} FormatEfficiency.throughput_ratio_str` higher effective throughput.

**Systems insight**: Switching from CSV to Parquet is not just a file change; it is mathematically equivalent to buying a `{python} FormatEfficiency.throughput_ratio_str` faster hard drive. @Tbl-serialization-cost quantifies the serialization overhead of different formats. For a deeper treatment of row vs. columnar storage layouts and the algebra of data operations (selection, projection, join), see @sec-data-foundations-row-vs-columnar-formats-bca2.
...
class CompressionTradeoff:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    decompress_speedup_str = fmt(decompress_speedup, precision=1, commas=False, suffix="×")
    gzip_min_str = fmt(gzip_decompress_min, precision=1, commas=False, suffix=" minutes")
    snappy_min_str = fmt(snappy_decompress_min, precision=1, commas=False, suffix=" minutes")
    time_diff_str = fmt(time_diff_min, precision=1, commas=False, suffix=" minutes")
    total_diff_hours_str = fmt_int(total_diff_hours, commas=False, suffix=" hours")
    snappy_decompress_mbs_str = fmt(snappy_decompress_mbs, precision=0, commas=False, suffix=" MB/s")
    gzip_decompress_mbs_str = fmt(gzip_decompress_mbs, precision=0, commas=False, suffix=" MB/s")
    compress_dataset_gb_str = fmt(dataset_gb, precision=0, commas=False, suffix=" GB")
    n_epochs_str = fmt(n_epochs, precision=0, commas=False, suffix=" epochs")
```

Compression algorithm\index{Compression!algorithm selection} selection involves trade-offs between compression ratio and decompression speed. While gzip achieves higher compression ratios of six to eight times, Snappy\index{Snappy!decompression speed} achieves only two to three times compression but decompresses at `{python} CompressionTradeoff.snappy_decompress_mbs_str`, roughly `{python} CompressionTradeoff.decompress_speedup_str` faster than gzip's `{python} CompressionTradeoff.gzip_decompress_mbs_str`. For ML training where throughput matters more than storage costs, Snappy's speed advantage often outweighs gzip's space savings. Training on a `{python} CompressionTradeoff.compress_dataset_gb_str` dataset compressed with gzip requires `{python} CompressionTradeoff.gzip_min_str` of decompression time, while Snappy requires only `{python} CompressionTradeoff.snappy_min_str`. When training iterates over data for `{python} CompressionTradeoff.n_epochs_str`, this `{python} CompressionTradeoff.time_diff_str` difference per epoch compounds to `{python} CompressionTradeoff.total_diff_hours_str` total, potentially the difference between running experiments overnight vs. waiting multiple days for results. The choice cascades through the system: faster decompression enables higher input throughput, reduced buffering requirements (less decompressed data needs staging), and better GPU utilization (less time idle waiting for data).
...
class LifecycleStorageRequirements:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    train_img_per_sec_str = fmt(train_img_per_sec, precision=0, commas=True, suffix=" img/s")
    image_size_kb_str = fmt(image_size_kb, precision=0, commas=False, suffix=" KB")
    training_bw_gbs_str = fmt(training_bw_gbs, precision=0, commas=False, suffix=" GB/s")
    storage_reduction_str = fmt(storage_reduction, precision=0, commas=False, suffix="×")
    feature_vector_kb_str = fmt(feature_vector_kb, precision=0, commas=False, suffix=" KB")
...
Training phase requirements shift dramatically toward throughput. Modern deep learning processes massive datasets repeatedly across dozens or hundreds of epochs, making I/O efficiency critical. Training ResNet-50 on ImageNet across eight GPUs at `{python} LifecycleStorageRequirements.train_img_per_sec_str` would require roughly `{python} LifecycleStorageRequirements.training_bw_gbs_str` for `{python} LifecycleStorageRequirements.image_size_kb_str` compressed images, and substantially more if decoded FP32 tensors are staged. Storage unable to sustain this throughput idles GPUs, directly increasing infrastructure costs. The feature computation placement trade-off (@sec-data-engineering-feature-computation-placement-a998) is especially acute here: precomputing features achieves `{python} LifecycleStorageRequirements.storage_reduction_str` storage reduction (`{python} LifecycleStorageRequirements.image_size_kb_str` to `{python} LifecycleStorageRequirements.feature_vector_kb_str` vectors) but introduces staleness risk when extraction logic changes.
...
Teams meticulously optimize accelerator kernels while leaving data loading as JSON or CSV. Text-based formats can be at least 10$\times$ slower to decode than columnar binary formats (Parquet, Arrow), and the CPU---not the accelerator---becomes the bottleneck. The consequence is the worst-case profile of the data-throughput hierarchy: an H100 that should sustain hundreds of TFLOP/s instead idles at single-digit utilization waiting for the next batch. The fix is structural, not algorithmic: convert the training corpus to Parquet once, version it, and let every downstream job read the columnar form. A one-time conversion pays back within hours of training time.
...
class DataEngineeringSummaryRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    ratio_low_str = fmt_int(round(ratio_low), commas=True, suffix="×")
    ratio_high_str = fmt_int(round(ratio_high), commas=True, suffix="×")
    kws_dataset_size_m_round_str = fmt_int(round(kws_dataset_size_m))
    kws_dataset_size_gb_str = fmt(kws_dataset_size_gb, precision=0, commas=False, suffix=" GB")
```
...
* **Labeling costs dominate and require substantial resource allocation**: Labeling can cost hundreds to more than a thousand times one optimized training run; in the reference calculation above, the ratio is `{python} DataEngineeringSummaryRecap.ratio_low_str`--`{python} DataEngineeringSummaryRecap.ratio_high_str`. Labeling is the serial bottleneck that parallelization cannot solve.
* **Storage hierarchy determines iteration speed**: The `{python} StorageLoadingRecap.load_speedup_str` throughput gap between local NVMe (`{python} StorageLoadingRecap.nvme_bw_gbs_str`) and cloud object storage (100 MB/s) determines whether iterations occur daily or weekly.
