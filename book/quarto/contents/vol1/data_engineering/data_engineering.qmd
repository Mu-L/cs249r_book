...
Underlying these physical properties is a hardware fact that determines every downstream design choice: moving a bit costs orders of magnitude more energy than computing on it, and the multiplier widens with every step the data takes away from the chip.
...
Allocating our \$150K budget across three cost categories (refer to the Key data engineering numbers callout below):
...
**Engineering conclusion**: ETL saves USD `{python} EtlEltCost.savings_str`/month in storage but costs `{python} EtlEltCost.schema_change_ratio_str`$\times$ more engineering time per schema change. The exact break-even point depends on engineering labor cost and schema-change frequency: stable schemas favor ETL's lower storage cost, while frequent feature-definition changes favor ELT's faster iteration.
...
3.  **The multiplier**:
    ...
    *   **Active**: The system may need ~100K–200K *hard* examples (\$50K–\$100K). **Feasible.**

**Engineering conclusion**: Algorithm choice is a 10$\times$ lever on data budget. Spending 10 percent of the budget on compute to select data can save 90 percent of the budget on human labeling.
...
The preceding KWS scenario illustrates each of the four categories of data debt. The microphone hardware shift is *freshness debt*; the updated labeling guidelines are *quality debt*; the undocumented schema change is both *schema debt* and *documentation debt*. Each category requires different detection and remediation strategies, but all share the property captured in the following definition.
