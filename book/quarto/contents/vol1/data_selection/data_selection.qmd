...
$$
\text{samples}_t = \texttt{sort\_by\_difficulty}[:D \cdot \min(1, t/T_{\text{warmup}})]
$$
where $t$ is the current epoch, $D$ is the full dataset size, and $T_{\text{warmup}}$ is the epoch at which the full dataset becomes available. Early epochs train on the easiest $D \cdot (t/T_{\text{warmup}})$ fraction; after warmup, training proceeds on the full dataset.
