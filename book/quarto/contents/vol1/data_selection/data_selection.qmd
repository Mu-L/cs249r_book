...
class ScalingAsymmetry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpu_growth_str = fmt(gpu_growth_factor, precision=1, allow_zero=True, commas=False) + "×"
    # Custom "years" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    gpu_period_str = fmt(int(gpu_period_years), precision=1, allow_zero=True, commas=False, suffix=" years")

    web_data_growth_str = fmt(web_growth_factor, precision=1, allow_zero=True, commas=False) + "×"
    web_data_period_str = fmt(int(web_period_years), precision=1, allow_zero=True, commas=False, suffix=" years")

    label_data_growth_str = fmt(label_growth_factor, precision=1, commas=False) + "×"
    label_data_period_str = fmt(int(label_period_years), precision=1, allow_zero=True, commas=False, suffix=" years")
```
...
from mlsysim.core.defaults import REFERENCE_MFU_SUSTAINED
...
class ComputeDataGap:
    ...
    sustained_mfu = REFERENCE_MFU_SUSTAINED
    ...
    cluster_cost = (
        ...
        * c.DAYS_PER_MONTH
        * c.HOURS_PER_DAY
        * c.CLOUD_GPU_TRAINING_PER_HOUR.m_as(c.USD / c.hour)
    )
    ...
    # OUTPUT
    h100_count_str = fmt(h100_count, precision=1, allow_zero=True, commas=True)
    tokens_capacity_str = fmt(tokens_capacity/c.TRILLION, precision=1, allow_zero=True, commas=False) + "T"
    filtered_corpus_tokens_str = fmt(filtered_corpus_tokens/c.TRILLION, precision=1, allow_zero=True, commas=False) + "T"
    public_text_stock_str = fmt(public_text_stock/c.TRILLION, precision=1, allow_zero=True, commas=False) + "T"
    corpus_reuse_ratio_str = fmt(corpus_reuse_ratio, precision=1, allow_zero=True, commas=False)
    training_months_str = fmt(months, precision=1, allow_zero=True, commas=False)
    sustained_mfu_pct_str = fmt_percent(sustained_mfu, precision=1, commas=False)
    cluster_cost_m_str = fmt(cluster_cost/c.MILLION, precision=1, allow_zero=True, commas=False)
```

The compute budget (`{python} ComputeDataGap.h100_count_str` H100 GPUs for `{python} ComputeDataGap.training_months_str` months) represents roughly USD `{python} ComputeDataGap.cluster_cost_m_str` M at the chapter's cloud-training price anchor and can process about `{python} ComputeDataGap.tokens_capacity_str` tokens at `{python} ComputeDataGap.sustained_mfu_pct_str` percent sustained MFU. Estimates of quality- and repetition-adjusted public human-generated text are on the order of `{python} ComputeDataGap.public_text_stock_str` tokens, far larger than a single curated web corpus. The practical bottleneck is narrower: how much of that stock is accessible, legally usable, high quality, deduplicated, and useful for the target distribution. If a team has only a `{python} ComputeDataGap.filtered_corpus_tokens_str` filtered corpus ready for use, the compute budget can already process it roughly `{python} ComputeDataGap.corpus_reuse_ratio_str`$\times$ over. The team faces three options: train on the same data for multiple epochs (diminishing returns after epochs 2--3), lower quality thresholds to include more data (which can degrade model quality), or invest in data selection through better filtering, curriculum design, and synthetic augmentation to extract more learning from each token. The third option is increasingly the dominant approach.
...
class IronLawSavings:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_cost_m_str = fmt(budget_m, precision=1, allow_zero=True, commas=False)
    dataset_reduction_pct_str = fmt(data_pruning_pct, precision=1, allow_zero=True, commas=False)
    compute_savings_m_str = fmt(compute_savings_m, precision=1, allow_zero=True, commas=False)
    combined_factor_str = fmt(total_speedup, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (Models.Vision.ResNet50.inference_flops, GFLOPs, IMAGENET_IMAGES)
...
class IcrCoresetComparison:
    ...
    full_epoch_flops_str = fmt_sci(full_epoch_flops_value, precision=2)
    ...
    coreset_size_str = fmt(coreset_size_value / THOUSAND, precision=1, allow_zero=True, commas=False, suffix="K")
    coreset_flops_str = fmt_sci(coreset_flops_value, precision=1)
    ...
    coreset_pct_str = fmt_percent(coreset_fraction_value, precision=1, commas=False)
```
...
class QualityMultiplier:
    ...
    epsilon_pct_str = fmt_percent(epsilon, precision=1, commas=False)
    n_clean_str = fmt(n_clean, precision=1, allow_zero=True, commas=False)
    n_noisy_str = fmt(n_noisy, precision=1, allow_zero=True, commas=True)
    ratio_str = fmt(ratio, precision=1, allow_zero=True, commas=False)
```
...
class CoresetPractice:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_train_images_str = fmt(n_train_images_value/MILLION, precision=1, allow_zero=True) + " million"
    coreset_fraction_pct_str = fmt_percent(coreset_fraction_value, precision=1, commas=False)
    n_coreset_str = fmt(n_coreset_value, precision=1, allow_zero=True, commas=True)
    n_epochs_proxy_str = fmt(n_epochs_proxy_value, precision=1, allow_zero=True, commas=False)
```
...
class CurriculumBenchmarks:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cifar10_speedup_str = fmt(cifar10_speedup_pct, precision=1, allow_zero=True, commas=False)
    cifar100_speedup_str = fmt(cifar100_speedup_pct, precision=1, allow_zero=True, commas=False)
    imagenet_speedup_str = fmt(imagenet_speedup_pct, precision=1, allow_zero=True, commas=False)
    mentornet_speedup_str = fmt(mentornet_speedup_pct, precision=1, allow_zero=True, commas=False)
    # Surface suffix-bearing aliases for prose; raw int attributes kept above
    # for the arithmetic at lines 1514–1517.
    cifar10_baseline_epochs_str = fmt(cifar10_baseline_epochs, precision=1, allow_zero=True, commas=False)
    cifar10_curriculum_epochs_str = fmt(cifar10_curriculum_epochs, precision=1, allow_zero=True, commas=False)
    cifar100_baseline_epochs_str = fmt(cifar100_baseline_epochs, precision=1, allow_zero=True, commas=False)
    cifar100_curriculum_epochs_str = fmt(cifar100_curriculum_epochs, precision=1, allow_zero=True, commas=False)
    imagenet_baseline_epochs_str = fmt(imagenet_baseline_epochs, precision=1, allow_zero=True, commas=False)
    imagenet_curriculum_epochs_str = fmt(imagenet_curriculum_epochs, precision=1, allow_zero=True, commas=False)
    mentornet_baseline_epochs_str = fmt(mentornet_baseline_epochs, precision=1, allow_zero=True, commas=False)
    mentornet_curriculum_epochs_str = fmt(mentornet_curriculum_epochs, precision=1, allow_zero=True, commas=False)
```
...
class ActiveLearningRoi:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_unlabeled_str = fmt(n_unlabeled_value/MILLION, precision=1, allow_zero=True) + " Million"
    cost_per_label_str = fmt(cost_per_label_value, precision=2, commas=False)
    budget_str = fmt(budget_value, precision=1, allow_zero=True, commas=True)
    cost_all_str = fmt(cost_all_value, precision=1, allow_zero=True, commas=True)
    n_random_str = fmt(n_random_value, precision=1, allow_zero=True, commas=True)
    n_random_pct_str = fmt(n_random_pct_value, precision=1, allow_zero=True, commas=False)
    n_active_str = fmt(n_active_value, precision=1, allow_zero=True, commas=True)
    cost_active_str = fmt(cost_active_value, precision=1, allow_zero=True, commas=True)
    cost_active_pct_str = fmt(cost_active_pct_value, precision=1, allow_zero=True, commas=False)
    speedup_str = fmt(speedup_value, precision=1, allow_zero=True, commas=False)
    cost_saving_str = fmt(cost_saving_value/MILLION, precision=2) + " Million"
    deadline_months_str = fmt(deadline_months_value, precision=1, allow_zero=True, commas=False)
```
...
3.  **Naive labeling outcome**: The model misses rare pathologies because they were not in the random `{python} ActiveLearningRoi.n_random_pct_str` percent.
...
class ClinicalLabelingEconomics:
    ...
    # OUTPUT
    labeled_scans_str = fmt(labeled_scans, precision=1, allow_zero=True, commas=True)
    full_pool_scans_str = fmt(full_pool_scans, precision=1, allow_zero=True, commas=True)
    ...
    labeled_hours_range_str = f"{fmt(labeled_hours_low_display, precision=1, allow_zero=True, commas=False)}–{fmt(labeled_hours_high_display, precision=1, allow_zero=True, commas=False)}"
    labeled_cost_range_str = f"{fmt(labeled_cost_low_display, precision=1, allow_zero=True, commas=True)}–{fmt(labeled_cost_high_display, precision=1, allow_zero=True, commas=True)}"
    full_hours_range_str = f"{fmt(full_hours_low, precision=1, allow_zero=True, commas=True)}–{fmt(full_hours_high, precision=1, allow_zero=True, commas=True)}"
    full_weeks_range_str = f"{fmt(full_weeks_low, precision=1, allow_zero=True, commas=False)}–{fmt(full_weeks_high, precision=1, allow_zero=True, commas=False)}"
    full_cost_range_str = f"{fmt(full_cost_low_display, precision=1, allow_zero=True, commas=True)}–{fmt(full_cost_high_display, precision=1, allow_zero=True, commas=True)}"
```
...
class FixmatchLabelEfficiency:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    supervised_label_cost_str = fmt(supervised_label_cost, precision=1, allow_zero=True, commas=True)
    supervised_total_str = fmt(supervised_total, precision=1, allow_zero=True, commas=True)
    fixmatch_label_cost_str = fmt(fixmatch_label_cost, precision=1, allow_zero=True, commas=True)
    fixmatch_total_str = fmt(fixmatch_total, precision=1, allow_zero=True, commas=True)
    cost_reduction_str = fmt(cost_reduction, precision=1, allow_zero=True, commas=False)
    acc_loss_str = fmt(acc_loss, precision=1, commas=False)
    fixmatch_compute_multiplier_str = fmt(fixmatch_compute_multiplier_value, precision=1, allow_zero=True, commas=False)
    cifar10_full_labels_str = fmt(cifar10_full_labels, precision=1, allow_zero=True, commas=True)
    cifar10_fixmatch_4k_labels_str = fmt(cifar10_fixmatch_4k_labels, precision=1, allow_zero=True, commas=True)
    cifar10_fixmatch_250_labels_str = fmt(cifar10_fixmatch_250_labels, precision=1, allow_zero=True, commas=True)
    cifar10_fixmatch_40_labels_str = fmt(cifar10_fixmatch_40_labels, precision=1, allow_zero=True, commas=True)
    ...
    cifar10_full_acc_str = fmt(cifar10_full_acc, precision=1, commas=False)
    cifar10_fixmatch_4k_acc_str = fmt(cifar10_fixmatch_4k_acc, precision=1, commas=False)
    cifar10_fixmatch_250_acc_str = fmt(cifar10_fixmatch_250_acc, precision=1, commas=False)
    cifar10_fixmatch_40_acc_str = fmt(cifar10_fixmatch_40_acc, precision=1, commas=False)
    cifar10_fixmatch_4k_eff_str = fmt(cifar10_fixmatch_4k_eff, precision=1, commas=False)
    cifar10_fixmatch_250_eff_str = fmt(cifar10_fixmatch_250_eff, precision=1, commas=False)
    cifar10_fixmatch_40_eff_str = fmt(cifar10_fixmatch_40_eff, precision=1, commas=False)
    supervised_compute_cost_str = fmt(supervised_compute_cost, precision=1, allow_zero=True, commas=False)
    fixmatch_compute_cost_str = fmt(fixmatch_compute_cost, precision=1, allow_zero=True, commas=False)
```
...
class FoundationCostAmortization:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_a_hrs_str = fmt(cost_scratch_total_value, precision=1, allow_zero=True)
    total_b_hrs_str = fmt(cost_foundation_total_value, precision=1, allow_zero=True)
    labels_per_task_scratch_str = fmt(labels_per_task_scratch, precision=1, allow_zero=True, commas=True)
    # Currency prefix + M/K suffix → MarkdownStr per recipe Rule 1 escape-hatch
    label_cost_scratch_total_str = fmt(label_cost_scratch_total / MILLION, precision=1, allow_zero=True, commas=False, prefix="$", suffix="M")
    cost_scratch_per_task_str = fmt(cost_scratch_per_task_value, precision=1, allow_zero=True, commas=True)
    cost_scratch_total_str = fmt(cost_scratch_total_value, precision=1, allow_zero=True, commas=True)
    labels_per_task_finetune_str = fmt(labels_per_task_finetune, precision=1, allow_zero=True, commas=True)
    label_cost_finetune_total_str = fmt(label_cost_finetune_total / THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    cost_finetune_value_str = fmt(cost_finetune_value, precision=1, allow_zero=True, commas=True)
    cost_pretrain_value_str = fmt(cost_pretrain_value, precision=1, allow_zero=True, commas=True)
    marginal_compute_reduction_str = fmt(marginal_compute_reduction, precision=1, allow_zero=True, commas=False)
    crossover_tasks_str = fmt(crossover_tasks_value, precision=1, allow_zero=True, commas=False)
    label_cost_drop_str = fmt(label_cost_reduction, precision=1, allow_zero=True, commas=True)
```
...
class KwsDataSelectionCalc:
    ...
    # OUTPUT
    speakers_str = fmt(speakers, precision=1, allow_zero=True, commas=False)
    utterances_per_speaker_str = fmt(utterances_per_speaker, precision=1, allow_zero=True, commas=False)
    seed_samples_str = fmt(seed_samples, precision=1, allow_zero=True, commas=True)
    augmented_samples_str = fmt(augmented_samples, precision=1, allow_zero=True, commas=True)
    target_samples_str = fmt(target_samples, precision=1, allow_zero=True, commas=True)
    target_samples_plus_str = MarkdownStr(f"{target_samples_str}+")
    full_cost_range_str = f"{fmt(full_cost_low / THOUSAND, precision=1, allow_zero=True, commas=False)}–{fmt(full_cost_high / THOUSAND, precision=1, allow_zero=True, commas=False)}K"
    cost_per_sample_range_str = MarkdownStr(f"{cost_per_sample_low}–{cost_per_sample_high}")
    augmentation_multiplier_str = fmt(augmentation_multiplier, precision=1, allow_zero=True, commas=False)
    seed_cost_pct_str = fmt(seed_cost_pct, precision=1, allow_zero=True, commas=False)
```
...
**Systems lesson**: When the target model is tiny, the data selection challenge shifts from "reduce terabytes to gigabytes" to "create a useful dataset from almost nothing." Augmentation and simulation become essential rather than optional. `{python} KwsDataSelectionCalc.seed_samples_str` real recordings become `{python} KwsDataSelectionCalc.target_samples_str`+ training samples at `{python} KwsDataSelectionCalc.seed_cost_pct_str` percent of the recording cost. The noise injection serves as domain randomization, improving deployment robustness.
...
class SelectionBottleneckAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dataset_size_str = fmt(dataset_size_m, precision=1, allow_zero=True, commas=False, suffix="M")
    scoring_time_str = fmt(scoring_time_hrs, precision=1, commas=False, suffix=" hours")
    coreset_pct_str = fmt(coreset_pct, precision=1, allow_zero=True, commas=False, suffix="%")
```

Dynamic data selection introduces a new bottleneck: selection latency\index{Latency!selection, dynamic selection bottleneck}. In standard training, the data loader reads the next batch sequentially. In active learning or curriculum learning, the system must evaluate a selection function $f(x)$ over a large candidate pool to determine the next batch. Concretely, scoring a `{python} SelectionBottleneckAnchor.dataset_size_str` image dataset with a large model can take `{python} SelectionBottleneckAnchor.scoring_time_str`, potentially negating the savings from a `{python} SelectionBottleneckAnchor.coreset_pct_str` coreset if not performed with a smaller proxy model.
...
class SelectionInequalityCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    score_a_str = fmt(score_a_sec_value, precision=1, allow_zero=True, commas=True)
    score_a_hrs_str = fmt(score_a_sec_value/SEC_PER_HOUR, precision=1, commas=False)
    train_a_str = fmt(train_a_sec_value, precision=1, allow_zero=True, commas=True)
    ...
    score_b_str = fmt(score_b_sec_value, precision=1, allow_zero=True, commas=True)
    ...
    baseline_str = fmt(baseline_sec_value, precision=1, allow_zero=True, commas=True)
    baseline_hrs_str = fmt(baseline_hrs_value, precision=1, allow_zero=True, commas=False)
    savings_a_str = fmt(savings_a_hrs_value, precision=1, allow_zero=True, commas=False)
    savings_a_pct_str = fmt(savings_a_pct_value, precision=1, allow_zero=True, commas=False)
    savings_b_str = fmt(savings_b_hrs_value, precision=1, allow_zero=True, commas=False)
    savings_b_pct_str = fmt(savings_b_pct_value, precision=1, allow_zero=True, commas=False)
    ...
    trap_pct_str = fmt(trap_overhead_pct_value, precision=1, allow_zero=True, commas=False)
    n_images_str = fmt(n_images_value/MILLION, precision=1, allow_zero=True, commas=False) + "M"
    n_coreset_str = fmt(n_coreset_value/THOUSAND, precision=1, allow_zero=True, commas=False) + "k"
    n_epochs_str = fmt(n_epochs_value, precision=1, allow_zero=True, commas=False)
    coreset_pct_str = fmt_percent(coreset_fraction_value, precision=1, commas=False)
    resnet50_time_per_image_str = fmt(resnet50_time_per_image_value, precision=2, commas=False)
    resnet18_time_per_image_str = fmt(resnet18_time_per_image_value, precision=3, commas=False)
    trap_sel_hrs_str = fmt(trap_sel_hrs_value, precision=1, allow_zero=True, commas=False)
```

::: {#exmp-data-selection-selection-inequality-practice .callout-example title="Selection inequality in practice"}

**Scenario**: Given `{python} SelectionInequalityCalc.n_images_str` training images, the goal is to select a `{python} SelectionInequalityCalc.n_coreset_str` coreset (`{python} SelectionInequalityCalc.coreset_pct_str` percent) using EL2N scoring.

**Option A: Full Model Selection**

- Score all `{python} SelectionInequalityCalc.n_images_str` images with the target ResNet-50: `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.resnet50_time_per_image_str` s = **`{python} SelectionInequalityCalc.score_a_str` seconds** (`{python} SelectionInequalityCalc.score_a_hrs_str` hours)
- Train on `{python} SelectionInequalityCalc.n_coreset_str` coreset for `{python} SelectionInequalityCalc.n_epochs_str` epochs: `{python} SelectionInequalityCalc.n_coreset_str` $\times$ `{python} SelectionInequalityCalc.n_epochs_str` $\times$ `{python} SelectionInequalityCalc.resnet50_time_per_image_str` s = **`{python} SelectionInequalityCalc.train_a_str` seconds** (`{python} SelectionInequalityCalc.train_a_hrs_str` hours)
- **Total: `{python} SelectionInequalityCalc.total_a_hrs_str` hours**

**Option B: Proxy Model Selection**

- Score all `{python} SelectionInequalityCalc.n_images_str` images with a small proxy (ResNet-18): `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.resnet18_time_per_image_str` s = **`{python} SelectionInequalityCalc.score_b_str` seconds** (`{python} SelectionInequalityCalc.score_b_hrs_str` hours)
- Train on `{python} SelectionInequalityCalc.n_coreset_str` coreset for `{python} SelectionInequalityCalc.n_epochs_str` epochs: **`{python} SelectionInequalityCalc.train_a_str` seconds** (`{python} SelectionInequalityCalc.train_a_hrs_str` hours)
- **Total: `{python} SelectionInequalityCalc.total_b_hrs_str` hours**

**Baseline: No Selection**

- Train on the full `{python} SelectionInequalityCalc.n_images_str` dataset for `{python} SelectionInequalityCalc.n_epochs_str` epochs: `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.n_epochs_str` $\times$ `{python} SelectionInequalityCalc.resnet50_time_per_image_str` s = **`{python} SelectionInequalityCalc.baseline_str` seconds** (`{python} SelectionInequalityCalc.baseline_hrs_str` hours)
...
**Trap**: If selection required `{python} SelectionInequalityCalc.trap_sel_hrs_str` hours (for example, running a 7-billion-parameter model), the total would reach `{python} SelectionInequalityCalc.trap_total_str` hours, still better than baseline, but the selection overhead equals `{python} SelectionInequalityCalc.trap_pct_str` percent of the remaining net savings.
...
class SelectionInequalityMath:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_epochs_full_str = fmt(n_epochs_full, precision=1, allow_zero=True, commas=False)
    subset_fraction_pct_str = fmt_percent(subset_fraction, precision=1, commas=False)
    cost_selection_full_str = fmt(cost_selection_full, precision=1, allow_zero=True, commas=False)
    n_epochs_subset_str = fmt(n_epochs_subset, precision=1, allow_zero=True, commas=False)
    cost_total_efficient_str = fmt(cost_total_efficient, precision=1, allow_zero=True, commas=False)
    speedup_efficient_str = fmt(speedup_efficient, precision=1, allow_zero=True, commas=False)
    cost_selection_iterative_str = fmt(cost_selection_iterative, precision=1, allow_zero=True, commas=False)
    cost_total_iterative_str = fmt(cost_total_iterative, precision=1, allow_zero=True, commas=False)
    proxy_factor_inv_str = fmt(1 / proxy_factor, precision=1, allow_zero=True, commas=False)
...
::: {#nbk-data-selection-selection-inequality .callout-notebook title="The selection inequality"}
...
class RandomAccessPenalty:
    ...
    # OUTPUT
    hdd_seq_mb_s_str = fmt(hdd_seq_mb_s, precision=1, allow_zero=True, commas=False)
    hdd_iops_str = fmt(hdd_iops, precision=1, allow_zero=True, commas=False)
    hdd_random_mb_s_str = fmt(hdd_random_mb_s, precision=1, commas=False)
    hdd_penalty_str = fmt(round(hdd_penalty, -2), precision=1, allow_zero=True, commas=False)

    sata_seq_mb_s_str = fmt(sata_seq_mb_s, precision=1, allow_zero=True, commas=False)
    sata_iops_str = fmt(sata_iops / THOUSAND, precision=1, allow_zero=True, commas=False) + "k"
    sata_random_mb_s_str = fmt(sata_random_mb_s, precision=1, allow_zero=True, commas=False)
    sata_penalty_str = fmt(sata_penalty, precision=1, allow_zero=True, commas=False)

    nvme_seq_mb_s_str = fmt(nvme_seq_mb_s, precision=1, allow_zero=True, commas=True)
    nvme_iops_str = fmt(nvme_iops / THOUSAND, precision=1, allow_zero=True, commas=False) + "k"
    nvme_random_mb_s_str = fmt(nvme_random_mb_s, precision=1, allow_zero=True, commas=True)
...
|:-----------------|:------------------------------------------------------:|:---------------------------------------------:|:---------------------------------------------------------:|:-----------------------------------------------------------:|
| **HDD (7.2k)**   | ~`{python} RandomAccessPenalty.hdd_seq_mb_s_str` MB/s  |  ~`{python} RandomAccessPenalty.hdd_iops_str` |  ~`{python} RandomAccessPenalty.hdd_random_mb_s_str` MB/s |  **`{python} RandomAccessPenalty.hdd_penalty_str`$\times$** |
| **SATA SSD**     | ~`{python} RandomAccessPenalty.sata_seq_mb_s_str` MB/s | ~`{python} RandomAccessPenalty.sata_iops_str` | ~`{python} RandomAccessPenalty.sata_random_mb_s_str` MB/s | **`{python} RandomAccessPenalty.sata_penalty_str`$\times$** |
| **NVMe SSD**     | ~`{python} RandomAccessPenalty.nvme_seq_mb_s_str` MB/s | ~`{python} RandomAccessPenalty.nvme_iops_str` | ~`{python} RandomAccessPenalty.nvme_random_mb_s_str` MB/s | **`{python} RandomAccessPenalty.nvme_penalty_str`$\times$** |
...
class DataEchoingRoi:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pipeline_throughput_str = fmt(pipeline_throughput_value, precision=1, allow_zero=True, commas=False)
    gpu_throughput_str = fmt(gpu_throughput_value, precision=1, allow_zero=True, commas=False)
    pipeline_ratio_str = fmt(ratio_r_value, precision=2, commas=False)
    idle_pct_str = fmt(gpu_idle_pct_value, precision=1, allow_zero=True, commas=False)
    no_echo_sec_str = fmt(no_echo_sec_value, precision=1, allow_zero=True, commas=True)
    no_echo_hrs_str = fmt(no_echo_hrs_value, precision=1, allow_zero=True, commas=False)
    gpu_util_str = fmt(gpu_util_no_echo_value, precision=1, allow_zero=True, commas=False)
    echo_sec_str = fmt(echo_sec_value, precision=1, allow_zero=True, commas=True)
    echo_hrs_str = fmt(echo_hrs_value, precision=1, allow_zero=True, commas=False)
    echo_factor_str = fmt(echo_factor_value, precision=1, allow_zero=True, commas=False)
    effective_throughput_str = fmt(echo_throughput_value, precision=1, allow_zero=True, commas=False)
```
...
class CostBreakdown:
    ...
    c_raw_str = fmt(c_raw_value, precision=1, allow_zero=True, prefix="$")
    c_label_str = fmt(c_label_value, precision=1, allow_zero=True, prefix="$")
    c_store_str = fmt(c_store_value, precision=1, allow_zero=True, commas=False, prefix="$")
    c_train_str = fmt(c_train_value, precision=1, allow_zero=True, prefix="$")
    c_total_str = fmt(c_total_value, precision=1, allow_zero=True, prefix="$")
    p_data_str = fmt(p_data_value, precision=1, allow_zero=True, commas=False, suffix="%")
    p_compute_str = fmt(p_compute_value, precision=1, allow_zero=True, commas=False, suffix="%")
...
class BreakevenCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_label_str = fmt(cost_label, precision=1, allow_zero=True, commas=False)
    n_initial_str = fmt(n_initial, precision=1, allow_zero=True, commas=True)
    cost_initial_str = fmt(n_initial * cost_label, precision=1, allow_zero=True, commas=True)
    n_queries_str = fmt(n_queries_per_round, precision=1, allow_zero=True, commas=False)
    cost_inference_str = fmt(cost_inference, precision=1, allow_zero=True, commas=False)
    be_n_random_str = fmt(n_random, precision=1, allow_zero=True, commas=True)
    be_n_active_str = fmt(n_active, precision=1, allow_zero=True, commas=True)
    n_rounds_str = fmt(n_rounds, precision=1, allow_zero=True, commas=False)
    cost_random_total_str = fmt(cost_random_total, precision=1, allow_zero=True, commas=True)
    cost_active_label_str = fmt(cost_active_label, precision=1, allow_zero=True, commas=True)
    cost_active_inference_str = fmt(cost_active_inference, precision=1, allow_zero=True, commas=True)
    cost_active_total_str = fmt(cost_active_total, precision=1, allow_zero=True, commas=True)
    roi_pct_str = fmt(roi_pct, precision=1, allow_zero=True, commas=False)
```
...
class DeduplicationAmortization:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_build_str = fmt(cost_build, precision=1, allow_zero=True, commas=True)
    cost_compute_once_str = fmt(cost_compute_once, precision=1, allow_zero=True, commas=True)
    savings_per_run_str = fmt(savings_per_run, precision=1, allow_zero=True, commas=True)
    roi_1_str = fmt(rois[0], precision=1, allow_zero=True, commas=False)
    roi_5_str = fmt(rois[1], precision=1, allow_zero=True, commas=False)
    roi_10_str = fmt(rois[2], precision=1, allow_zero=True, commas=False)
    roi_50_str = fmt(rois[3], precision=1, allow_zero=True, commas=False)
```
...
With that caution in mind, the investment decision reduces to a practical split between high-reuse, data-limited workflows and one-off, already-curated ones.
...
class DistributedOverheadCalc:
    ...
    t_embed_str = fmt(t_embed_value, precision=1, allow_zero=True, commas=False, suffix=" minutes")
    t_dedup_str = fmt(t_dedup_value, precision=1, allow_zero=True, commas=False, suffix=" minutes")
    t_score_str = fmt(t_score_value, precision=1, allow_zero=True, commas=False, suffix=" minutes")
    t_select_str = fmt(t_select_value, precision=1, allow_zero=True, commas=False, suffix=" minutes")
    t_total_overhead_str = fmt(t_total_overhead_value, precision=1, allow_zero=True, commas=False, suffix=" minutes")
```
...
# │
# │ Imports: math; mlsysim.core.constants (GPT3_TRAINING_TOKENS, Models.Language.GPT3.parameters,
...
class ChinchillaDiagnostic:
    ...
    # EXECUTE
    gpt3_dn_ratio = c.GPT3_TRAINING_TOKENS.m_as(count) / m_gpt3.parameters.m_as(param)
    ...
    # OUTPUT
    tokens_per_param_opt_str = fmt(tokens_per_param_opt, precision=1, allow_zero=True, commas=False)
    starvation_ratio_str = fmt(starvation_ratio, precision=1, allow_zero=True, commas=False)
    diminishing_ratio_str = fmt(diminishing_ratio, precision=1, allow_zero=True, commas=False)
    gpt3_dn_ratio_str = fmt(gpt3_dn_ratio, precision=1, commas=False)
    llama2_dn_ratio_str = fmt(llama2_dn_ratio, precision=1, allow_zero=True, commas=False)
    doubling_token_growth_pct_str = fmt(doubling_token_growth_pct, precision=1, allow_zero=True, commas=False)
```

In the compute-optimal regime described by the Chinchilla scaling laws[^fn-chinchilla-ratio], training tokens and model parameters should scale together in a fixed ratio (roughly `{python} ChinchillaDiagnostic.tokens_per_param_opt_str` tokens per parameter). As a simplified consequence, when the model size is held proportional to its compute-optimal allocation, the number of training tokens grows roughly as $D_{\text{opt}} \propto \sqrt{C}$; doubling the compute budget then implies about $\sqrt{2} - 1$, or `{python} ChinchillaDiagnostic.doubling_token_growth_pct_str` percent, more tokens, not 100 percent. This explains why the data wall is so constraining: as compute grows exponentially, the demand for quality data grows roughly as its square root, but even that slower growth outpaces the supply of high-quality human-generated content.
...
class FpScalingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Fallacy 1
    data_1m_str = fmt(data_1m_value/MILLION, precision=1, allow_zero=True) + "M"
    data_10m_str = fmt(data_10m_value/MILLION, precision=1, allow_zero=True) + "M"
    acc_gain_10x_str = fmt(acc_gain_10x_value, precision=1, allow_zero=True, commas=False)
    curated_size_str = fmt(curated_size_value / THOUSAND, precision=1, allow_zero=True) + "K"
    raw_size_str = fmt(raw_size_value/MILLION, precision=1, allow_zero=True) + "M"
    curated_accuracy_str = fmt(curated_accuracy_value, precision=1, allow_zero=True, commas=False)
    raw_accuracy_str = fmt(raw_accuracy_value, precision=1, allow_zero=True, commas=False)
    curated_cost_ratio_str = fmt(curated_cost_ratio_value, precision=1, allow_zero=True, commas=False)

    # Fallacy 2
    synthetic_gen1_acc_str = fmt(synthetic_gen1_acc_value, precision=1, allow_zero=True, commas=False)
    synthetic_gen5_acc_str = fmt(synthetic_gen5_acc_value, precision=1, allow_zero=True, commas=False)
    synthetic_acc_drop_str = fmt(synthetic_acc_drop_value, precision=1, allow_zero=True, commas=False)
    # Range with en-dash separator → MarkdownStr per recipe Rule 1 escape-hatch
    optimal_synthetic_mix_str = MarkdownStr(f"{optimal_synthetic_mix_min_value}–{optimal_synthetic_mix_max_value}")

    # Fallacy 4
    training_run_cost_str = fmt(training_run_cost_value/MILLION, precision=1, allow_zero=True) + "M"
    efficiency_gain_pct_str = fmt(efficiency_gain_pct_value, precision=1, allow_zero=True, commas=False)
    savings_str = fmt(savings_value/MILLION, precision=1, allow_zero=True) + "M"

    # Pitfall 1
    selection_time_bad_str = fmt(selection_time_bad_value, precision=1, allow_zero=True, commas=False)
    training_time_str = fmt(training_time_value, precision=1, allow_zero=True, commas=False)
    full_training_time_str = fmt(full_training_time_value, precision=1, allow_zero=True, commas=False)
    selection_overhead_ratio_str = fmt(selection_overhead_ratio_value, precision=1, allow_zero=True, commas=False)
    selection_time_good_str = fmt(selection_time_good_value * 60, precision=1, allow_zero=True, commas=False)
    selection_overhead_good_pct_str = fmt(selection_overhead_good_pct_value, precision=1, allow_zero=True, commas=False)

    # Pitfall 2
    total_samples_str = fmt(total_samples_value/MILLION, precision=1, allow_zero=True) + "M"
    rare_class_pct_str = fmt(rare_class_pct_value, precision=1, commas=False)
    rare_class_count_str = fmt(rare_class_count_value, precision=1, allow_zero=True, commas=True)
    fp_coreset_pct_str = fmt(coreset_pct_value, precision=1, allow_zero=True, commas=False)
    expected_rare_in_coreset_str = fmt(expected_rare_in_coreset_value, precision=1, allow_zero=True, commas=False)
    min_samples_threshold_str = fmt(min_samples_threshold_value, precision=1, allow_zero=True, commas=False)

    # Pitfall 3
    test_contamination_pct_str = fmt(test_contamination_pct_value, precision=1, allow_zero=True, commas=False)
    inflated_acc_str = fmt(inflated_acc_value, precision=1, allow_zero=True, commas=False)
    true_acc_str = fmt(true_acc_value, precision=1, allow_zero=True, commas=False)
    inflation_gap_str = fmt(inflation_gap_value, precision=1, allow_zero=True, commas=False)

    # Pitfall 4
    al_latency_days_str = fmt(al_latency_days_value, precision=1, allow_zero=True, commas=False)
    model_drift_epochs_str = fmt(model_drift_epochs_value, precision=1, allow_zero=True, commas=False)
    batch_size_small_str = fmt(batch_size_small_value, precision=1, allow_zero=True, commas=False)
    batch_size_large_str = fmt(batch_size_large_value, precision=1, allow_zero=True, commas=True)

    # Fallacy 5
    cifar10_coreset_pct_str = fmt(cifar10_coreset_pct_value, precision=1, allow_zero=True, commas=False)
    cifar10_acc_retained_str = fmt(cifar10_acc_retained_value, precision=1, allow_zero=True, commas=False)
    imagenet_acc_retained_str = fmt(imagenet_acc_retained_value, precision=1, allow_zero=True, commas=False)
    medical_acc_retained_str = fmt(medical_acc_retained_value, precision=1, allow_zero=True, commas=False)

    # Pitfall 5
    coreset_size_pct_str = fmt(coreset_size_pct_value, precision=1, allow_zero=True, commas=False)
    rare_class_acc_str = fmt(rare_class_acc_value, precision=1, allow_zero=True, commas=False)
    majority_class_acc_str = fmt(majority_class_acc_value, precision=1, allow_zero=True, commas=False)
```
...
Active learning theory assumes instant oracle responses. In production annotation workflows, expert labels can require days or weeks in practice. With `{python} FpScalingCalc.al_latency_days_str`-day annotation latency, a model trained for `{python} FpScalingCalc.model_drift_epochs_str` epochs between query rounds may have drifted significantly: samples selected as uncertain become irrelevant as the decision boundary shifts. Select larger batches (`{python} FpScalingCalc.batch_size_large_str` vs. `{python} FpScalingCalc.batch_size_small_str` samples) to amortize latency and use diversity sampling to hedge against model drift. Active learning ROI therefore depends not only on label cost, as analyzed in @sec-data-selection-breakeven-analysis-9a38, but also on matching batch size to annotation turnaround time.
