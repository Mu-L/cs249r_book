...
Training each classifier from scratch would require substantial investment in both labeling and compute. With `{python} FoundationCostAmortization.n_tasks_str` tasks each needing `{python} FoundationCostAmortization.labels_per_task_scratch_str` labels at `{python} FoundationCostAmortization.cost_per_label_str` per label, the total labeling cost reaches **`{python} FoundationCostAmortization.label_cost_scratch_total_str`**. The compute burden amounts to `{python} FoundationCostAmortization.cost_scratch_total_str`-hours across all tasks, with each requiring its own data collection effort. From start to finish, each task takes six to twelve months to complete.

The fine-tuning approach restructures these costs. Pretraining requires a one-time investment of `{python} FoundationCostAmortization.cost_pretrain_value_str`-hours on unlabeled data, but this cost is paid only once. Fine-tuning each task then requires just `{python} FoundationCostAmortization.labels_per_task_finetune_str` labels (`{python} FoundationCostAmortization.label_cost_finetune_total_str` total across all `{python} FoundationCostAmortization.n_tasks_str` tasks) and only `{python} FoundationCostAmortization.cost_finetune_value_str`-hours of compute. Each task reaches deployment in 1–2 weeks after pretraining completes.
...
::: {#fig-amortization-comparison fig-env="figure" fig-pos="htb" fig-cap="**Cost Amortization in Foundation Models**: Training from scratch (left) requires `{python} FoundationCostAmortization.cost_scratch_per_task_str`-hours per task (`{python} FoundationCostAmortization.cost_scratch_total_str` total for `{python} FoundationCostAmortization.n_tasks_str` tasks). The foundation model approach (right) pays `{python} FoundationCostAmortization.cost_pretrain_value_str`-hours upfront for pretraining but reduces each subsequent task to just `{python} FoundationCostAmortization.cost_finetune_value_str`-hours. At `{python} FoundationCostAmortization.n_tasks_str` tasks the totals are comparable (`{python} FoundationCostAmortization.cost_scratch_total_str` vs. `{python} FoundationCostAmortization.cost_foundation_total_str`), but the per-task marginal cost drops by `{python} FoundationCostAmortization.marginal_compute_reduction_str`, and the crossover favoring the foundation model occurs around `{python} FoundationCostAmortization.crossover_tasks_str` tasks." fig-alt="Two bar charts side by side. Left (train from scratch) shows equal per-task GPU-hour bars totaling the scratch-training cost. Right (foundation model) shows one tall pretraining bar followed by short fine-tuning bars per task."}
