...
class ScalingAsymmetry:
    ...
    web_data_period_str = fmt(int(web_period_years), precision=0, commas=False, suffix=" years")

    label_data_growth_str = fmt(label_growth_factor, precision=1, commas=False) + "×"
    label_data_period_str = fmt(int(label_period_years), precision=0, commas=False, suffix=" years")
```
...
class ComputeDataGap:
    ...
    training_flops_per_token = 6 * c.LLAMA2_70B_PARAMS.m_as(c.param)
    seconds = months * c.DAYS_PER_MONTH * c.HOURS_PER_DAY * c.SEC_PER_HOUR

    # EXECUTE
    total_flops = (
        h100_count
        * c.H100_FLOPS_FP16_TENSOR.m_as(c.flop / c.second)
        * seconds
        * sustained_mfu
    )
    ...
    cluster_cost_m_str = fmt(cluster_cost/c.MILLION, precision=0, commas=False)
```
...
from mlsysim.core.constants import BILLION, RESNET50_FLOPs, GFLOPs, IMAGENET_IMAGES, MILLION, THOUSAND
...
class IcrCoresetComparison:
    ...
    coreset_size_str = fmt(coreset_size_value / THOUSAND, precision=0, commas=False, suffix="K")
...
|:--------------|:---------:|:--------------------|:-------------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------:|
| **CIFAR-10**  | ResNet-18 | Linear warmup       | `{python} CurriculumBenchmarks.cifar10_curriculum_epochs_str` vs. `{python} CurriculumBenchmarks.cifar10_baseline_epochs_str` baseline     |  **`{python} CurriculumBenchmarks.cifar10_speedup_str`%** faster  |
| **CIFAR-100** | ResNet-32 | Self-paced          | `{python} CurriculumBenchmarks.cifar100_curriculum_epochs_str` vs. `{python} CurriculumBenchmarks.cifar100_baseline_epochs_str` baseline   |  **`{python} CurriculumBenchmarks.cifar100_speedup_str`%** faster |
| **ImageNet**  | ResNet-50 | Loss-based          | `{python} CurriculumBenchmarks.imagenet_curriculum_epochs_str` vs. `{python} CurriculumBenchmarks.imagenet_baseline_epochs_str` baseline   |  **`{python} CurriculumBenchmarks.imagenet_speedup_str`%** faster |
| **ImageNet**  | ResNet-50 | MentorNet (noisy)   | `{python} CurriculumBenchmarks.mentornet_curriculum_epochs_str` vs. `{python} CurriculumBenchmarks.mentornet_baseline_epochs_str` baseline | **`{python} CurriculumBenchmarks.mentornet_speedup_str`%** faster |
...
class ClinicalLabelingEconomics:
    ...
    full_weeks_range_str = f"{fmt(full_weeks_low, precision=0, commas=False)}–{fmt(full_weeks_high, precision=0, commas=False)}"
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MILLION, SEC_PER_HOUR, THOUSAND
from mlsysim.fmt import fmt_percent, fmt, check
...
class RandomAccessPenalty:
    ...
    hdd_penalty_str = fmt(round(hdd_penalty, -2), precision=0, commas=False)
    ...
    sata_penalty_str = fmt(sata_penalty, precision=0, commas=False)
...
If $R > 1$ (data pipeline is the bottleneck), set echo factor $e \leq R$ to fully use GPU capacity. If $R < 1$ (GPU is the bottleneck), data echoing provides no benefit. A realistic scenario makes these trade-offs concrete.
...
class CostBreakdown:
    ...
    p_data_str = fmt(p_data_value, precision=0, commas=False, suffix="%")
    p_compute_str = fmt(p_compute_value, precision=0, commas=False, suffix="%")
...
::: {#ws-data-selection-test-set-leak .callout-war-story title="The test set leak"}
...
::: {#ws-data-selection-99-percent-sparsity-trap .callout-war-story title="The 99 percent sparsity trap"}
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ CHINCHILLA D/N DIAGNOSTIC ANCHORS
# ├─────────────────────────────────────────────────────────────────────────────
...
# │          diminishing) and the GPT-3 / LLaMA-2 70B reference D/N ratios.
# │ Show:    Threshold values, reference D/N ratios for both models, and the
...
# │
# │ Imports: math; mlsysim.core.constants (GPT3_TRAINING_TOKENS, GPT3_PARAMS,
# │          LLAMA2_70B_PARAMS, TRILLION, param); mlsysim.fmt (check, fmt)
...
class ChinchillaDiagnostic:
    ...
    # EXECUTE
    gpt3_dn_ratio = c.GPT3_TRAINING_TOKENS.m_as("count") / c.GPT3_PARAMS.m_as(c.param)
    llama2_dn_ratio = llama2_70b_training_tokens / c.LLAMA2_70B_PARAMS.m_as(c.param)
    ...
    llama2_dn_ratio_str = fmt(llama2_dn_ratio, precision=0, commas=False)
    doubling_token_growth_pct_str = fmt(doubling_token_growth_pct, precision=0, commas=False)
```
