...
**The context**: Unstructured pruning can remove a large fraction of weights from a dense network, sharply reducing theoretical operation counts.

**The failure**: On dense accelerators such as GPUs and Tensor Processing Units (TPUs), those FLOP reductions may not translate into proportional latency gains and can even lose to dense kernels when sparse overhead dominates.
...
**The systems lesson**: FLOPs are *not* latency. Large operation-count reductions can yield little or no wall-clock speedup when the remaining operations map poorly to dense hardware kernels or memory access patterns. Optimization must target the hardware's actual bottleneck, not just an abstract metric [@hooker2020hardware].
