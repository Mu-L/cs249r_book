...
The 10 percent heuristic is tied to a single model size, dataset, and epoch count. Generalizing it requires a formal framework that normalizes selection cost against training cost in epoch-equivalents, independent of hardware or model architecture.
...
class SelectionInequalityMath:
    ...
    subset_fraction_pct_str = fmt(subset_fraction * 100, precision=0, commas=False, suffix=" percent")
...
1.  **Full training**: `{python} SelectionInequalityMath.n_epochs_full_str` epochs, where $C_{\text{epoch}}$ is the compute cost of one full training epoch. Total cost = `{python} SelectionInequalityMath.n_epochs_full_str` $\times$ $C_{\text{epoch}}$.
