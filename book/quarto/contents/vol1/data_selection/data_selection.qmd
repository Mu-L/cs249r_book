...
class SelectionEconomicsAnchor:
    ...
    # Custom suffix ("hours", "%") → MarkdownStr per recipe Rule 1 escape-hatch
    scoring_time_str = MarkdownStr(f"{scoring_time_hrs:.1f} hours")
    coreset_pct_str = MarkdownStr(f"{coreset_pct}%")
...
class ScalingAsymmetry:
    ...
    gpu_period_str = MarkdownStr(f"{int(gpu_period_years)} years")

    web_data_growth_str = fmt(web_growth_factor, precision=0, commas=False) + "×"
    web_data_period_str = MarkdownStr(f"{int(web_period_years)} years")

    label_data_growth_str = fmt(label_growth_factor, precision=1, commas=False) + "×"
    label_data_period_str = MarkdownStr(f"{int(label_period_years)} years")
```
...
from mlsysim.fmt import fmt_percent, fmt, check, sci, MarkdownStr
...
class IcrCoresetComparison:
    ...
    full_epoch_flops_str = sci(full_epoch_flops_value, precision=2)
    icr_random_str = sci(icr_random_value, precision=1)
    ...
    coreset_size_str = MarkdownStr(f"{coreset_size_value / THOUSAND:.0f}K")
    coreset_flops_str = sci(coreset_flops_value, precision=1)
    icr_coreset_str = sci(icr_coreset_value, precision=1)
...
class FoundationCostAmortization:
    ...
    label_cost_scratch_total_str = MarkdownStr(f"${label_cost_scratch_total / MILLION:.0f}M")
    ...
    label_cost_finetune_total_str = MarkdownStr(f"${label_cost_finetune_total / THOUSAND:.0f}K")
...
class CostBreakdown:
    ...
    c_raw_str = MarkdownStr(f"${c_raw_value:,}")
    c_label_str = MarkdownStr(f"${c_label_value:,.0f}")
    c_store_str = MarkdownStr(f"${c_store_value}")
    c_train_str = MarkdownStr(f"${c_train_value:,}")
    c_total_str = MarkdownStr(f"${c_total_value:,.0f}")
    p_data_str = MarkdownStr(f"{p_data_value:.0f}%")
    p_compute_str = MarkdownStr(f"{p_compute_value:.0f}%")
...
class DistributedOverheadCalc:
    ...
    t_embed_str = MarkdownStr(f"{t_embed_value} minutes")
    t_dedup_str = MarkdownStr(f"{t_dedup_value} minutes")
    t_score_str = MarkdownStr(f"{t_score_value} minutes")
    t_select_str = MarkdownStr(f"{t_select_value} minutes")
    t_total_overhead_str = MarkdownStr(f"{t_total_overhead_value} minutes")
```
