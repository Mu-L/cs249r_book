...
- Accuracy gain: `{python} IcrCoresetComparison.acc_gain_coreset_str` percentage points (`{python} IcrCoresetComparison.acc_perf_pct_str` of full data performance)
...
- Score all `{python} SelectionInequalityCalc.n_images_str` with the target ResNet-50: `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.resnet50_time_per_image_str` s = **`{python} SelectionInequalityCalc.score_a_str`** (`{python} SelectionInequalityCalc.score_a_hrs_str`)
...
- Score all `{python} SelectionInequalityCalc.n_images_str` with a small proxy (ResNet-18): `{python} SelectionInequalityCalc.n_images_str` $\times$ `{python} SelectionInequalityCalc.resnet18_time_per_image_str` s = **`{python} SelectionInequalityCalc.score_b_str`** (`{python} SelectionInequalityCalc.score_b_hrs_str`)
...
**Problem**: An active learning system selects the best `{python} SelectionInequalityMath.subset_fraction_pct_str` of samples for training, and the selection algorithm requires running the full model on the unlabeled pool. Is this active-learning loop more efficient than training on the full dataset?
...
3.  **Subset training**: `{python} SelectionInequalityMath.n_epochs_full_str` epochs on `{python} SelectionInequalityMath.subset_fraction_pct_str` data = `{python} SelectionInequalityMath.n_epochs_full_str` $\times$ `{python} SelectionInequalityMath.subset_fraction_str` $\times$ $C_{\text{epoch}}$ = `{python} SelectionInequalityMath.n_epochs_subset_str` $\times$ $C_{\text{epoch}}$.
