...
class IcrCoresetComparison:
    ...
    acc_gain_random_str = fmt(acc_gain_random_value, precision=0, commas=False)
    ...
    acc_perf_pct_str = fmt_percent(acc_gain_coreset_value / acc_gain_random_value, precision=0, commas=False)
```

::: {#nbk-data-selection-computing-icr-coresets .callout-notebook title="Computing ICR: Coresets"}

**Problem**: Training the **ResNet-50 Lighthouse model** from @sec-network-architectures-lighthouse-roster-model-biographies-a763 on ImageNet for one epoch costs `{python} IcrCoresetComparison.full_epoch_flops_str` FLOPs. If an EL2N-based coreset (EL2N, or Error L2-Norm, scores each sample by how uncertain the model's prediction is; defined formally in @sec-data-selection-coreset-selection-algorithms-2c74) retains only `{python} IcrCoresetComparison.coreset_pct_str` percent of the data, how much does the information-compute ratio improve over random selection? ResNet-50's compute-bound nature (high **arithmetic intensity**; see @sec-machine-foundations-roofline-model-2529) makes it an ideal candidate: reducing dataset size directly reduces training FLOPs with minimal I/O impact.
...
- Accuracy gain: `{python} IcrCoresetComparison.acc_gain_coreset_str` percentage points (`{python} IcrCoresetComparison.acc_perf_pct_str` percent of full data performance)
...
class FixmatchLabelEfficiency:
    ...
    supervised_compute_cost_str = fmt(supervised_compute_cost, precision=0, commas=False)
    fixmatch_compute_cost_str = fmt(fixmatch_compute_cost, precision=0, commas=False)
```
...
class FoundationCostAmortization:
    ...
    cost_foundation_total_str = fmt(cost_foundation_total_value, precision=0, commas=True)
...
The fine-tuning approach restructures these costs. Pretraining requires a one-time investment of `{python} FoundationCostAmortization.cost_pretrain_value_str` GPU-hours on unlabeled data, but this cost is paid only once. Fine-tuning each task then requires just `{python} FoundationCostAmortization.labels_per_task_finetune_str` labels (`{python} FoundationCostAmortization.label_cost_finetune_total_str` total across all `{python} FoundationCostAmortization.n_tasks_str` tasks) and only `{python} FoundationCostAmortization.cost_finetune_value_str` GPU-hours of compute. Each task reaches deployment in 1–2 weeks after pretraining completes.
...
::: {#fig-amortization-comparison fig-env="figure" fig-pos="htb" fig-cap="**Cost Amortization in Foundation Models**: Training from scratch (left) requires `{python} FoundationCostAmortization.cost_scratch_per_task_str` GPU-hours per task (`{python} FoundationCostAmortization.cost_scratch_total_str` total for `{python} FoundationCostAmortization.n_tasks_str` tasks). The foundation model approach (right) pays `{python} FoundationCostAmortization.cost_pretrain_value_str` GPU-hours upfront for pretraining but reduces each subsequent task to just `{python} FoundationCostAmortization.cost_finetune_value_str` GPU-hours. At `{python} FoundationCostAmortization.n_tasks_str` tasks the totals are comparable (`{python} FoundationCostAmortization.cost_scratch_total_str` vs. `{python} FoundationCostAmortization.cost_foundation_total_str`), but the per-task marginal cost drops by `{python} FoundationCostAmortization.marginal_compute_reduction_str`×, and the crossover favoring the foundation model occurs around `{python} FoundationCostAmortization.crossover_tasks_str` tasks." fig-alt="Two bar charts side by side. Left (train from scratch) shows equal per-task GPU-hour bars totaling the scratch-training cost. Right (foundation model) shows one tall pretraining bar followed by short fine-tuning bars per task."}
...
class KwsDataSelectionCalc:
    ...
    seed_cost_pct_str = fmt(seed_cost_pct, precision=0, commas=False)
```
...
class SelectionInequalityCalc:
    ...
    score_a_hrs_str = fmt_int(score_a_sec_value/SEC_PER_HOUR, commas=False)
    train_a_str = fmt(train_a_sec_value, precision=0, commas=True)
    train_a_hrs_str = fmt_int(train_a_sec_value/SEC_PER_HOUR, commas=False)
    total_a_hrs_str = fmt_int(total_a_hrs_value, commas=False)
    score_b_str = fmt(score_b_sec_value, precision=0, commas=True)
    score_b_hrs_str = fmt_int(score_b_sec_value/SEC_PER_HOUR, commas=False)
    total_b_hrs_str = fmt_int(total_b_hrs_value, commas=False)
    baseline_str = fmt(baseline_sec_value, precision=0, commas=True)
    baseline_hrs_str = fmt_int(baseline_hrs_value, commas=False)
    savings_a_str = fmt_int(savings_a_hrs_value, commas=False)
    savings_a_pct_str = fmt(savings_a_pct_value, precision=0, commas=False)
    savings_b_str = fmt_int(savings_b_hrs_value, commas=False)
    savings_b_pct_str = fmt(savings_b_pct_value, precision=1, commas=False)
    b_beats_a_str = fmt_int(b_beats_a_hrs_value, commas=False)
    trap_total_str = fmt_int(trap_total_hrs_value, commas=False)
    trap_pct_str = fmt(trap_overhead_pct_value, precision=0, commas=False)
    n_images_str = fmt(n_images_value/MILLION, precision=0, commas=False) + "M"
    n_coreset_str = fmt(n_coreset_value/THOUSAND, precision=0, commas=False) + "k"
...
class RandomAccessPenalty:
    ...
    # OUTPUT
    hdd_seq_mb_s_str = fmt(hdd_seq_mb_s, precision=0, commas=False)
    ...
    sata_seq_mb_s_str = fmt(sata_seq_mb_s, precision=0, commas=False)
    sata_iops_str = fmt(sata_iops / THOUSAND, precision=0, commas=False) + "k"
    sata_random_mb_s_str = fmt(sata_random_mb_s, precision=0, commas=False)
    sata_penalty_str = fmt(sata_penalty, precision=1, commas=False)

    nvme_seq_mb_s_str = fmt(nvme_seq_mb_s, precision=0, commas=True)
    nvme_iops_str = fmt(nvme_iops / THOUSAND, precision=0, commas=False) + "k"
    nvme_random_mb_s_str = fmt(nvme_random_mb_s, precision=0, commas=True)
...
class DataEchoingRoi:
    ...
    gpu_util_str = fmt_int(gpu_util_no_echo_value, commas=False)
...
Engineers assume linear returns from data scaling: 10$\times$ more data should yield proportional accuracy gains. In reality, the ICR framework (@sec-data-selection-informationcompute-ratio-8c0b) reveals severe diminishing returns. Scaling from `{python} FpFallacyCalc.data_1m_str` to `{python} FpFallacyCalc.data_10m_str` samples typically yields only `{python} FpFallacyCalc.acc_gain_10x_str` percentage points of accuracy gain while incurring 10$\times$ the compute cost. @tbl-scaling-asymmetry quantifies the asymmetry: GPU compute grows 10$\times$ every three years while high-quality data grows only 2$\times$ every five years. A curated `{python} FpFallacyCalc.curated_size_str` dataset achieving `{python} FpFallacyCalc.curated_accuracy_str` percent accuracy often outperforms a raw `{python} FpFallacyCalc.raw_size_str` dataset at `{python} FpFallacyCalc.raw_accuracy_str` percent, despite `{python} FpFallacyCalc.curated_cost_ratio_str`$\times$ fewer samples. Teams that blindly scale data budgets waste compute on redundant samples that contribute near-zero gradient signal.

**Fallacy**: *Synthetic data can completely replace real data.*

Engineers assume generative models produce unlimited training data at marginal cost. Synthetic-only training can fail through two different mechanisms. First, @sec-data-selection-bridging-domain-gap-100b and @tbl-synthetic-mix show the domain-gap problem: generated data can diverge from the real deployment distribution, causing the learned decision boundary in @fig-domain-gap to misclassify real-world inputs. Second, recursive training on model-generated data can cause model collapse: accuracy degrades from `{python} FpFallacyCalc.synthetic_gen1_acc_str` percent to `{python} FpFallacyCalc.synthetic_gen5_acc_str` percent after five generations of training on model-generated data, a `{python} FpFallacyCalc.synthetic_acc_drop_str`-point drop. Optimal mixes are typically `{python} FpFallacyCalc.optimal_synthetic_mix_str` percent synthetic, with the remainder real data; pure synthetic training fails catastrophically on deployment distributions.
