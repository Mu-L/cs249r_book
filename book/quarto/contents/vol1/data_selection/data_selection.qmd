...
class ScalingAsymmetry:
    ...
    gpu_period_str = fmt(int(gpu_period_years), precision=0, commas=False, suffix=" years")

    web_data_growth_str = fmt(web_growth_factor, precision=1, commas=False) + "×"
    web_data_period_str = fmt(int(web_period_years), precision=1, commas=False, suffix=" years")

    label_data_growth_str = fmt(label_growth_factor, precision=1, commas=False) + "×"
    label_data_period_str = fmt(int(label_period_years), precision=1, commas=False, suffix=" years")
```
...
class ComputeDataGap:
    ...
    tokens_capacity_str = fmt(tokens_capacity/c.TRILLION, precision=0, commas=False) + "T"
    ...
    training_months_str = fmt(months, precision=1, commas=False)
...
class IronLawSavings:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_cost_m_str = fmt(budget_m, precision=1, commas=False)
    dataset_reduction_pct_str = fmt(data_pruning_pct, precision=1, commas=False)
    compute_savings_m_str = fmt(compute_savings_m, precision=1, commas=False)
    combined_factor_str = fmt(total_speedup, precision=1, commas=False)
```
...
class QualityMultiplier:
    ...
    n_clean_str = fmt(n_clean, precision=1, commas=False)
    n_noisy_str = fmt(n_noisy, precision=1, commas=True)
...
class CoresetPractice:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_train_images_str = fmt(n_train_images_value/MILLION, precision=1) + " million"
    coreset_fraction_pct_str = fmt_percent(coreset_fraction_value, precision=1, commas=False)
    n_coreset_str = fmt(n_coreset_value, precision=1, commas=True)
    n_epochs_proxy_str = fmt(n_epochs_proxy_value, precision=1, commas=False)
```
...
class CurriculumBenchmarks:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cifar10_speedup_str = fmt(cifar10_speedup_pct, precision=1, commas=False)
    cifar100_speedup_str = fmt(cifar100_speedup_pct, precision=1, commas=False)
    imagenet_speedup_str = fmt(imagenet_speedup_pct, precision=1, commas=False)
    mentornet_speedup_str = fmt(mentornet_speedup_pct, precision=1, commas=False)
    # Surface suffix-bearing aliases for prose; raw int attributes kept above
    # for the arithmetic at lines 1514–1517.
    cifar10_baseline_epochs_str = fmt(cifar10_baseline_epochs, precision=1, commas=False)
    cifar10_curriculum_epochs_str = fmt(cifar10_curriculum_epochs, precision=1, commas=False)
    cifar100_baseline_epochs_str = fmt(cifar100_baseline_epochs, precision=1, commas=False)
    cifar100_curriculum_epochs_str = fmt(cifar100_curriculum_epochs, precision=1, commas=False)
    imagenet_baseline_epochs_str = fmt(imagenet_baseline_epochs, precision=1, commas=False)
    imagenet_curriculum_epochs_str = fmt(imagenet_curriculum_epochs, precision=1, commas=False)
    mentornet_baseline_epochs_str = fmt(mentornet_baseline_epochs, precision=1, commas=False)
    mentornet_curriculum_epochs_str = fmt(mentornet_curriculum_epochs, precision=1, commas=False)
```
...
class ActiveLearningRoi:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_unlabeled_str = fmt(n_unlabeled_value/MILLION, precision=1) + " Million"
    cost_per_label_str = fmt(cost_per_label_value, precision=2, commas=False)
    budget_str = fmt(budget_value, precision=1, commas=True)
    cost_all_str = fmt(cost_all_value, precision=1, commas=True)
    n_random_str = fmt(n_random_value, precision=1, commas=True)
    n_random_pct_str = fmt(n_random_pct_value, precision=1, commas=False)
    n_active_str = fmt(n_active_value, precision=1, commas=True)
    cost_active_str = fmt(cost_active_value, precision=1, commas=True)
    cost_active_pct_str = fmt(cost_active_pct_value, precision=1, commas=False)
    speedup_str = fmt(speedup_value, precision=1, commas=False)
    cost_saving_str = fmt(cost_saving_value/MILLION, precision=2) + " Million"
    deadline_months_str = fmt(deadline_months_value, precision=1, commas=False)
```
...
class ClinicalLabelingEconomics:
    ...
    # OUTPUT
    labeled_scans_str = fmt(labeled_scans, precision=1, commas=True)
    full_pool_scans_str = fmt(full_pool_scans, precision=1, commas=True)
    ...
    labeled_hours_range_str = f"{fmt(labeled_hours_low_display, precision=0, commas=False)}–{fmt(labeled_hours_high_display, precision=0, commas=False)}"
    labeled_cost_range_str = f"{fmt(labeled_cost_low_display, precision=1, commas=True)}–{fmt(labeled_cost_high_display, precision=1, commas=True)}"
    full_hours_range_str = f"{fmt(full_hours_low, precision=0, commas=True)}–{fmt(full_hours_high, precision=0, commas=True)}"
    full_weeks_range_str = f"{fmt(full_weeks_low, precision=1, commas=False)}–{fmt(full_weeks_high, precision=1, commas=False)}"
    full_cost_range_str = f"{fmt(full_cost_low_display, precision=1, commas=True)}–{fmt(full_cost_high_display, precision=1, commas=True)}"
```
...
class FixmatchLabelEfficiency:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    supervised_label_cost_str = fmt(supervised_label_cost, precision=1, commas=True)
    supervised_total_str = fmt(supervised_total, precision=1, commas=True)
    fixmatch_label_cost_str = fmt(fixmatch_label_cost, precision=1, commas=True)
    fixmatch_total_str = fmt(fixmatch_total, precision=1, commas=True)
    cost_reduction_str = fmt(cost_reduction, precision=1, commas=False)
    acc_loss_str = fmt(acc_loss, precision=1, commas=False)
    fixmatch_compute_multiplier_str = fmt(fixmatch_compute_multiplier_value, precision=1, commas=False)
    cifar10_full_labels_str = fmt(cifar10_full_labels, precision=1, commas=True)
    cifar10_fixmatch_4k_labels_str = fmt(cifar10_fixmatch_4k_labels, precision=1, commas=True)
    cifar10_fixmatch_250_labels_str = fmt(cifar10_fixmatch_250_labels, precision=1, commas=True)
    cifar10_fixmatch_40_labels_str = fmt(cifar10_fixmatch_40_labels, precision=1, commas=True)
    ...
    supervised_compute_cost_str = fmt(supervised_compute_cost, precision=1, commas=False)
    fixmatch_compute_cost_str = fmt(fixmatch_compute_cost, precision=1, commas=False)
```
...
class FoundationCostAmortization:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_a_hrs_str = fmt(cost_scratch_total_value, precision=0)
    total_b_hrs_str = fmt(cost_foundation_total_value, precision=0)
    ...
    label_cost_scratch_total_str = fmt(label_cost_scratch_total / MILLION, precision=1, commas=False, prefix="$", suffix="M")
    ...
    label_cost_finetune_total_str = fmt(label_cost_finetune_total / THOUSAND, precision=1, commas=False, prefix="$", suffix="K")
    ...
    marginal_compute_reduction_str = fmt(marginal_compute_reduction, precision=1, commas=False)
    crossover_tasks_str = fmt(crossover_tasks_value, precision=1, commas=False)
    label_cost_drop_str = fmt(label_cost_reduction, precision=1, commas=True)
```
...
class KwsDataSelectionCalc:
    ...
    # OUTPUT
    speakers_str = fmt(speakers, precision=1, commas=False)
    utterances_per_speaker_str = fmt(utterances_per_speaker, precision=1, commas=False)
    seed_samples_str = fmt(seed_samples, precision=1, commas=True)
    augmented_samples_str = fmt(augmented_samples, precision=1, commas=True)
    target_samples_str = fmt(target_samples, precision=1, commas=True)
    target_samples_plus_str = MarkdownStr(f"{target_samples_str}+")
    full_cost_range_str = f"{fmt(full_cost_low / THOUSAND, precision=1, commas=False)}–{fmt(full_cost_high / THOUSAND, precision=1, commas=False)}K"
    cost_per_sample_range_str = MarkdownStr(f"{cost_per_sample_low}–{cost_per_sample_high}")
    augmentation_multiplier_str = fmt(augmentation_multiplier, precision=1, commas=False)
    seed_cost_pct_str = fmt(seed_cost_pct, precision=1, commas=False)
```
...
class SelectionBottleneckAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dataset_size_str = fmt(dataset_size_m, precision=1, commas=False, suffix="M")
    scoring_time_str = fmt(scoring_time_hrs, precision=1, commas=False, suffix=" hours")
    coreset_pct_str = fmt(coreset_pct, precision=1, commas=False, suffix="%")
```
...
class SelectionInequalityCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    score_a_str = fmt(score_a_sec_value, precision=1, commas=True)
    score_a_hrs_str = fmt(score_a_sec_value/SEC_PER_HOUR, precision=0, commas=False)
    train_a_str = fmt(train_a_sec_value, precision=1, commas=True)
    train_a_hrs_str = fmt(train_a_sec_value/SEC_PER_HOUR, precision=0, commas=False)
    total_a_hrs_str = fmt(total_a_hrs_value, precision=0, commas=False)
    score_b_str = fmt(score_b_sec_value, precision=1, commas=True)
    score_b_hrs_str = fmt(score_b_sec_value/SEC_PER_HOUR, precision=0, commas=False)
    total_b_hrs_str = fmt(total_b_hrs_value, precision=0, commas=False)
    baseline_str = fmt(baseline_sec_value, precision=1, commas=True)
    ...
    savings_a_pct_str = fmt(savings_a_pct_value, precision=1, commas=False)
    savings_b_str = fmt(savings_b_hrs_value, precision=0, commas=False)
    savings_b_pct_str = fmt(savings_b_pct_value, precision=1, commas=False)
    b_beats_a_str = fmt(b_beats_a_hrs_value, precision=0, commas=False)
    trap_total_str = fmt(trap_total_hrs_value, precision=0, commas=False)
    trap_pct_str = fmt(trap_overhead_pct_value, precision=1, commas=False)
    n_images_str = fmt(n_images_value/MILLION, precision=1, commas=False) + "M"
    n_coreset_str = fmt(n_coreset_value/THOUSAND, precision=1, commas=False) + "k"
    n_epochs_str = fmt(n_epochs_value, precision=1, commas=False)
    coreset_pct_str = fmt_percent(coreset_fraction_value, precision=1, commas=False)
    ...
    trap_sel_hrs_str = fmt(trap_sel_hrs_value, precision=0, commas=False)
```
...
class SelectionInequalityMath:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_epochs_full_str = fmt(n_epochs_full, precision=1, commas=False)
    subset_fraction_pct_str = fmt_percent(subset_fraction, precision=1, commas=False)
    cost_selection_full_str = fmt(cost_selection_full, precision=1, commas=False)
    n_epochs_subset_str = fmt(n_epochs_subset, precision=1, commas=False)
    cost_total_efficient_str = fmt(cost_total_efficient, precision=1, commas=False)
    speedup_efficient_str = fmt(speedup_efficient, precision=1, commas=False)
    cost_selection_iterative_str = fmt(cost_selection_iterative, precision=1, commas=False)
    cost_total_iterative_str = fmt(cost_total_iterative, precision=1, commas=False)
    proxy_factor_inv_str = fmt(1 / proxy_factor, precision=1, commas=False)
...
class RandomAccessPenalty:
    ...
    # OUTPUT
    hdd_seq_mb_s_str = fmt(hdd_seq_mb_s, precision=1, commas=False)
    hdd_iops_str = fmt(hdd_iops, precision=1, commas=False)
    hdd_random_mb_s_str = fmt(hdd_random_mb_s, precision=1, commas=False)
    hdd_penalty_str = fmt(round(hdd_penalty, -2), precision=1, commas=False)

    sata_seq_mb_s_str = fmt(sata_seq_mb_s, precision=1, commas=False)
    sata_iops_str = fmt(sata_iops / THOUSAND, precision=1, commas=False) + "k"
    sata_random_mb_s_str = fmt(sata_random_mb_s, precision=1, commas=False)
    sata_penalty_str = fmt(sata_penalty, precision=1, commas=False)

    nvme_seq_mb_s_str = fmt(nvme_seq_mb_s, precision=1, commas=True)
    nvme_iops_str = fmt(nvme_iops / THOUSAND, precision=1, commas=False) + "k"
    nvme_random_mb_s_str = fmt(nvme_random_mb_s, precision=1, commas=True)
...
class DataEchoingRoi:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pipeline_throughput_str = fmt(pipeline_throughput_value, precision=1, commas=False)
    ...
    idle_pct_str = fmt(gpu_idle_pct_value, precision=1, commas=False)
    no_echo_sec_str = fmt(no_echo_sec_value, precision=1, commas=True)
    no_echo_hrs_str = fmt(no_echo_hrs_value, precision=1, commas=False)
    gpu_util_str = fmt(gpu_util_no_echo_value, precision=0, commas=False)
    echo_sec_str = fmt(echo_sec_value, precision=1, commas=True)
    echo_hrs_str = fmt(echo_hrs_value, precision=1, commas=False)
    echo_factor_str = fmt(echo_factor_value, precision=1, commas=False)
    effective_throughput_str = fmt(echo_throughput_value, precision=1, commas=False)
```
...
class CostBreakdown:
    ...
    c_raw_str = fmt(c_raw_value, precision=1, prefix="$")
    c_label_str = fmt(c_label_value, precision=1, prefix="$")
    c_store_str = fmt(c_store_value, precision=1, commas=False, prefix="$")
    c_train_str = fmt(c_train_value, precision=1, prefix="$")
    c_total_str = fmt(c_total_value, precision=1, prefix="$")
...
class BreakevenCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_label_str = fmt(cost_label, precision=1, commas=False)
    n_initial_str = fmt(n_initial, precision=1, commas=True)
    cost_initial_str = fmt(n_initial * cost_label, precision=1, commas=True)
    n_queries_str = fmt(n_queries_per_round, precision=0, commas=False)
    cost_inference_str = fmt(cost_inference, precision=1, commas=False)
    be_n_random_str = fmt(n_random, precision=1, commas=True)
    be_n_active_str = fmt(n_active, precision=1, commas=True)
    n_rounds_str = fmt(n_rounds, precision=1, commas=False)
    cost_random_total_str = fmt(cost_random_total, precision=1, commas=True)
    cost_active_label_str = fmt(cost_active_label, precision=1, commas=True)
    cost_active_inference_str = fmt(cost_active_inference, precision=1, commas=True)
    cost_active_total_str = fmt(cost_active_total, precision=1, commas=True)
...
class DeduplicationAmortization:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_build_str = fmt(cost_build, precision=1, commas=True)
    cost_compute_once_str = fmt(cost_compute_once, precision=1, commas=True)
    savings_per_run_str = fmt(savings_per_run, precision=1, commas=True)
    roi_1_str = fmt(rois[0], precision=1, commas=False)
    roi_5_str = fmt(rois[1], precision=1, commas=False)
    roi_10_str = fmt(rois[2], precision=1, commas=False)
    roi_50_str = fmt(rois[3], precision=1, commas=False)
```
...
class DistributedOverheadCalc:
    ...
    t_embed_str = fmt(t_embed_value, precision=1, commas=False, suffix=" minutes")
    t_dedup_str = fmt(t_dedup_value, precision=1, commas=False, suffix=" minutes")
    t_score_str = fmt(t_score_value, precision=1, commas=False, suffix=" minutes")
    t_select_str = fmt(t_select_value, precision=1, commas=False, suffix=" minutes")
    t_total_overhead_str = fmt(t_total_overhead_value, precision=1, commas=False, suffix=" minutes")
```
...
class ChinchillaDiagnostic:
    ...
    # OUTPUT
    tokens_per_param_opt_str = fmt(tokens_per_param_opt, precision=1, commas=False)
    starvation_ratio_str = fmt(starvation_ratio, precision=1, commas=False)
    diminishing_ratio_str = fmt(diminishing_ratio, precision=1, commas=False)
    ...
    doubling_token_growth_pct_str = fmt(doubling_token_growth_pct, precision=1, commas=False)
```
...
class FpScalingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Fallacy 1
    data_1m_str = fmt(data_1m_value/MILLION, precision=1) + "M"
    data_10m_str = fmt(data_10m_value/MILLION, precision=1) + "M"
    acc_gain_10x_str = fmt(acc_gain_10x_value, precision=1, commas=False)
    curated_size_str = fmt(curated_size_value / THOUSAND, precision=1) + "K"
    raw_size_str = fmt(raw_size_value/MILLION, precision=1) + "M"
    curated_accuracy_str = fmt(curated_accuracy_value, precision=1, commas=False)
    raw_accuracy_str = fmt(raw_accuracy_value, precision=1, commas=False)
    curated_cost_ratio_str = fmt(curated_cost_ratio_value, precision=1, commas=False)

    # Fallacy 2
    synthetic_gen1_acc_str = fmt(synthetic_gen1_acc_value, precision=1, commas=False)
    synthetic_gen5_acc_str = fmt(synthetic_gen5_acc_value, precision=1, commas=False)
    synthetic_acc_drop_str = fmt(synthetic_acc_drop_value, precision=1, commas=False)
    ...
    # Fallacy 4
    training_run_cost_str = fmt(training_run_cost_value/MILLION, precision=1) + "M"
    efficiency_gain_pct_str = fmt(efficiency_gain_pct_value, precision=1, commas=False)
    savings_str = fmt(savings_value/MILLION, precision=1) + "M"
    ...
    selection_overhead_ratio_str = fmt(selection_overhead_ratio_value, precision=1, commas=False)
    selection_time_good_str = fmt(selection_time_good_value * 60, precision=1, commas=False)
    selection_overhead_good_pct_str = fmt(selection_overhead_good_pct_value, precision=1, commas=False)

    # Pitfall 2
    total_samples_str = fmt(total_samples_value/MILLION, precision=1) + "M"
    ...
    fp_coreset_pct_str = fmt(coreset_pct_value, precision=1, commas=False)
    expected_rare_in_coreset_str = fmt(expected_rare_in_coreset_value, precision=1, commas=False)
    min_samples_threshold_str = fmt(min_samples_threshold_value, precision=1, commas=False)

    # Pitfall 3
    test_contamination_pct_str = fmt(test_contamination_pct_value, precision=1, commas=False)
    inflated_acc_str = fmt(inflated_acc_value, precision=1, commas=False)
    true_acc_str = fmt(true_acc_value, precision=1, commas=False)
    inflation_gap_str = fmt(inflation_gap_value, precision=1, commas=False)

    # Pitfall 4
    al_latency_days_str = fmt(al_latency_days_value, precision=1, commas=False)
    model_drift_epochs_str = fmt(model_drift_epochs_value, precision=1, commas=False)
    batch_size_small_str = fmt(batch_size_small_value, precision=1, commas=False)
    batch_size_large_str = fmt(batch_size_large_value, precision=1, commas=True)

    # Fallacy 5
    cifar10_coreset_pct_str = fmt(cifar10_coreset_pct_value, precision=1, commas=False)
    cifar10_acc_retained_str = fmt(cifar10_acc_retained_value, precision=1, commas=False)
    imagenet_acc_retained_str = fmt(imagenet_acc_retained_value, precision=1, commas=False)
    medical_acc_retained_str = fmt(medical_acc_retained_value, precision=1, commas=False)

    # Pitfall 5
    coreset_size_pct_str = fmt(coreset_size_pct_value, precision=1, commas=False)
    rare_class_acc_str = fmt(rare_class_acc_value, precision=1, commas=False)
    majority_class_acc_str = fmt(majority_class_acc_value, precision=1, commas=False)
```
