...
class DispatchTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    python_overhead_str = fmt(int(python_overhead_us), precision=0, commas=False)
    small_kernel_str = fmt(int(small_kernel_us), precision=0, commas=False)
    large_kernel_str = fmt(int(large_kernel_us), precision=0, commas=False)
    small_tax_pct_str = fmt(small_tax_pct, precision=0, commas=False)
    large_tax_pct_str = fmt(large_tax_pct, precision=0, commas=False)
```
...
class FusionSpeedup:
    ...
    overhead_speedup_str = fmt(int(speedup), precision=0, commas=False)
    bw_efficiency_str = fmt(int(bw_efficiency), precision=0, commas=False)
...
class CompilationBenchmark:
    ...
    resnet_compile_speedup_pct_str = fmt(resnet_compile_speedup_pct, precision=0, commas=False)
...
class ResNetMemory:
    ...
    resnet_training_ratio_min_str = fmt(training_ratio_min, precision=0, commas=False)
    resnet_training_ratio_max_str = fmt(training_ratio_max, precision=0, commas=False)
...
from mlsysim.core.constants import (PCIE_GEN4_BW, NVLINK_A100_BW, A100_MEM_BW, A100_MEM_CAPACITY,
                             A100_FLOPS_FP16_TENSOR, GB, TB, TFLOPs, flop, byte, second,
...
class DeviceBandwidthHierarchy:
    ...
    pcie_to_hbm_gap_str = fmt(pcie_to_hbm_gap, precision=0, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (A100_FLOPS_FP16_TENSOR, TFLOPs, second)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.core.constants import BYTES_FP16, GB, Mparam, ureg
from mlsysim.core.formulas import model_memory
from mlsysim.fmt import fmt, check
...
# │
# │ Imports: mlsysim.core.constants (A100_MEM_CAPACITY, GB)
# │ Exports: HbmCapacityFootnote.a100_mem_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import A100_MEM_CAPACITY, GB
from mlsysim.fmt import fmt, check
...
class TrainingStepCalc:
    ...
    train_relu_flops_str = fmt((relu_flops * flop).m_as(KFLOPs), precision=0, commas=False) + " KFLOPs"
    ...
    train_mm2_flops_str = fmt((mm2_flops * flop).m_as(KFLOPs), precision=0, commas=False) + " KFLOPs"
    ...
    train_ce_flops_str = fmt((ce_flops * flop).m_as(KFLOPs), precision=0, commas=False) + " KFLOPs"
    ...
    train_bwd_flops_str = fmt((bwd_flops * flop).m_as(MFLOPs), precision=0, commas=False) + " MFLOPs"
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import A100_FLOPS_FP16_TENSOR, A100_MEM_BW, TRILLION, MILLION, flop, byte, second
from mlsysim.fmt import fmt, check, MarkdownStr
...
class MnistTrainingStepCalc:
    ...
    peak_flops = A100_FLOPS_FP16_TENSOR.m_as(flop/second)                      # 312e12 FLOP/s
    ...
    mnist_total_flops_str = fmt(total_flops/MILLION, precision=0, commas=False, suffix=" MFLOPs")
...
# │
# │ Imports: mlsysim.core.constants (A100_MEM_CAPACITY, BYTES_FP16, GB),
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import A100_MEM_CAPACITY, BYTES_FP16, GB
from mlsysim.core.formulas import model_memory
from mlsysim.fmt import fmt, check
...
class Model7BMemory:
    ...
    a100_mem = A100_MEM_CAPACITY.m_as(GB)                                      # decimal GB
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    ...
    a100_remaining_7b_gb_str = fmt(remaining_gb, precision=0, commas=False)     # e.g. "66"
...
class CompilationOverheadCalc:
    ...
    compiled_slowdown_str = fmt(compiled_slowdown, precision=0, commas=False)  # e.g. "44"
```
