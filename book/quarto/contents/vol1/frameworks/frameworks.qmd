...
# │
# │ Goal: Quantify A100 peak FP16/BF16 Tensor Core TFLOPS (dense ~312 and sparse ~624) to ground
# │       the BLAS-hardware connection and Memory Wall discussion.
# │ Show: "up to 312 TFLOPS (FP16/BF16) … or 624 TFLOPS with structured
...
# │
# │ Goal: Quantify the compute-vs-bandwidth gap on the A100 (312 TFLOPS vs 2.04 TB/s
# │       = 153 ops/byte ridge point) to show why element-wise ops hit <1% utilization.
# │ Show: "312 TFLOPS … 2.04 TB/s"—inline in Memory Wall opening paragraph;
...
|:------------------------------|:--------------------------------------------------------------------------------|:---------------------------------------------------------|:------------------------------------------------------------------------------------|
| **ResNet-50 (Compute Beast)** | $\frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}$ (Compute)                    | **TensorRT** (inference)<br>**torch.compile** (training) | Kernel fusion maximizes MFU; compute-bound workloads benefit most from optimization |
...
| **DLRM (Sparse Scatter)**     | $\frac{D_{\text{vol}}}{\text{BW}}(\text{Random Access})+L_{\text{lat,network}}$ | **Eager** with specialized kernels  (FBGEMM)             | Embedding lookups are inherently irregular and dynamic; compilation gains are small |
...
1.  **Significance (quantitative)**: Tensor memory footprint is fully deterministic from its metadata: a contiguous FP32 tensor of shape $[1024, 1024]$ occupies $1024{\times}1024{\times}4 = 4{,}194{,}304$ bytes (about `{python} TensorFootprint.tensor_1024_fp32_mb_str` MB). Noncontiguous layouts (for example, from a transpose operation) require explicit `.contiguous()` calls before certain CUDA kernels can execute, adding a memory-copy overhead that can dominate the $L_{\text{lat}}$ term for tensors under 1 MB.
...
# │ Show: A100 dense FP16 Tensor Core throughput in TFLOPS.
...
# │
# │ Goal: Keep 7B FP16 and ResNet-50 parameter counts local to module prose.
# │ Show: A 7B FP16 checkpoint is 14 GB; ResNet-50 has about 25.6M params.
...
class ModuleMemoryAnchor:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(round(model_7b_fp16_gb) == 14, "7B FP16 weights should be about 14 GB.")
...
# │ Show: A100 has an 80 GB HBM capacity anchor in this chapter.
...
- $T_{\text{compute}} \approx$ `{python} MnistTrainingStepCalc.mnist_total_flops_str`/`{python} MnistTrainingStepCalc.a100_peak_tflops_str` TFLOPS ≈ `{python} MnistTrainingStepCalc.mnist_t_compute_us_str`µs
...
# │
# │ Imports: mlsysim.core.constants (A100_MEM_CAPACITY, BYTES_FP16, GB, GiB),
...
class Model7BMemory:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(remaining_gb > 60, f"7B FP16 weights leave less A100 memory than expected: {remaining_gb:.1f} GB.")
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
...
Engineers assume that if memory is available, larger batches always improve throughput. The mini-batch processing discussion in @sec-model-training-minibatch-processing-4eb0 explains the throughput target, but activation memory can reverse the benefit. A 7B parameter model in FP16 consumes `{python} Model7BMemory.model_7b_fp16_gb_str` GB, leaving `{python} Model7BMemory.a100_remaining_7b_gb_str` GB on an A100-`{python} Model7BMemory.a100_mem_str` GB. Increasing batch size from 8 to 32 quadruples the batch-dependent activation footprint; transformer attention adds a large $O(S^2)$ term in sequence length that makes each sample expensive. The resulting memory pressure can trigger recomputation strategies that reduce throughput by 20--30 percent despite the larger batch. Teams that blindly maximize batch size often achieve lower throughput than smaller batches that avoid these memory management pathways.
