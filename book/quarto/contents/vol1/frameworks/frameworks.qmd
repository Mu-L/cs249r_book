...
**Key insight**: Compilation benefits scale with how much of the workload is *optimizable*. Compute Beasts (@tbl-training-benchmark: ResNet-50 sees `{python} CompilationBenchmark.resnet_tensorrt_speedup_str`$\times$ speedup from TensorRT) benefit most. Sparse Scatter workloads gain little because their bottleneck (embedding lookups) is inherently irregular.
...
*   **Conclusion**: The system spends `{python} DispatchTaxSmallLarge.dispatch_overhead_pct_str` percent of time in host-side dispatch and kernel-launch overhead. Compilation yields **`{python} DispatchTaxSmallLarge.dispatch_compilation_speedup_str`$\times$ speedup**.
