...
class DispatchTax:
    ...
    small_tax_pct_str = f"{small_tax_pct:.0f}"
    large_tax_pct_str = f"{large_tax_pct:.0f}"
```
...
*   **Compute**: `{python} DispatchTaxSmallLarge.dispatch_n_ops_value` small matrix/element-wise operations.
*   **Hardware time**: $T_{\text{hw}} \approx$ `{python} DispatchTaxSmallLarge.dispatch_hw_time_us_value` μs (mostly memory latency).
*   **Software overhead**: $T_{\text{sw}} \approx$ `{python} DispatchTaxSmallLarge.dispatch_n_ops_value` ops $\times$ `{python} DispatchTaxSmallLarge.dispatch_us_per_op_value` $\mu$s/op = `{python} DispatchTaxSmallLarge.dispatch_sw_time_str` $\mu$s.
*   **Ratio**: `{python} DispatchTaxSmallLarge.dispatch_sw_time_str`/`{python} DispatchTaxSmallLarge.dispatch_hw_time_us_value` ≈ **`{python} DispatchTaxSmallLarge.dispatch_ratio_small_str`**.
...
*   **Hardware time**: $T_{\text{hw}} \approx 100$ ms = `{python} DispatchTaxSmallLarge.gpt3_hw_time_us_value` μs.
*   **Software overhead**: `{python} DispatchTaxSmallLarge.t_sw_md`.
*   **Ratio**: `{python} DispatchTaxSmallLarge.gpt3_sw_time_us_value`/`{python} DispatchTaxSmallLarge.gpt3_hw_time_us_value` ≈ **`{python} DispatchTaxSmallLarge.dispatch_ratio_large_str`**.
...
4.  **Activations**: For a batch size of `{python} AdminTax.admin_batch_str` and a `{python} AdminTax.admin_layers_str`-layer network, the framework must store every intermediate layer output for the backward pass.
    ...
    For a `{python} AdminTax.admin_width_str`-width model: `{python} AdminTax.admin_act_calc_md`. (Each layer's activation is a $\text{Width}{\times}\text{Width}$ matrix per sample---appropriate for transformer-style models where intermediate projections scale with hidden dimension squared.)
...
class MnistTrainingStepCalc:
    ...
    mnist_mem_traffic_str = f"{mem_traffic_bytes/MILLION:.1f} MB"
