...
Forward mode's strength in single-input analysis becomes its fatal weakness for training. A neural network has one scalar loss but millions of parameters, and forward mode would require a separate pass for each one---an intractable $\mathcal{O}(N)$ cost that explains why no production framework uses forward mode for training. Forward mode remains useful for targeted analyses such as sensitivity analysis (measuring how a single pixel change affects the prediction) and feature importance (ranking which input dimensions most influence the output), where the number of inputs of interest is small.

Given forward mode's $\mathcal{O}(N)$ scaling with parameter count, we need an entirely different approach for training. Reverse mode provides exactly this: by propagating gradients backward from output to input, it computes all $N$ parameter gradients in a single pass.
...
During the forward pass, the autograd system constructs a reverse-linked graph of `Function` nodes. Each node records the operation performed and stores references to the tensors it needs for gradient computation. This graph is the data structure that makes reverse-mode automatic differentiation possible: regardless of how many parameters a model has, a single backward pass through this graph computes all gradients. For a model with $N$ parameters, reverse-mode AD requires $\mathcal{O}(1)$ backward passes (compared to $\mathcal{O}(N)$ for forward-mode), which is why every major framework implements this approach.
...
Use `.detach()` to break gradient flow, and clone the result before in-place mutation. @lst-detach-vs-data illustrates how the legacy `.data` attribute can silently corrupt gradient computation through in-place operations:
...
At the foundation of every framework's data representation lies a single abstraction: the **tensor**\index{Tensor!definition}.
...
\index{CUDA!runtime, OS layer}\index{DMA!pcie, data movement interface} While the high-level API focuses on math, the framework's backend functions as the operating system of the Single-Machine Stack. It manages the two critical resources of a single node: compute scheduling and data movement.
...
When an engineer writes `y = torch.matmul(x, w)`, the gap between Python and the GPU is larger than it appears. The gap between a single line of Python and thousands of parallel GPU threads is bridged by three groups of operations working in coordination. @fig-mlfm-core-ops is arranged left to right: system-level operations coordinate resources and execution, basic numerical operations implement mathematical computations, and hardware abstraction operations manage computing platform complexity.
...
%
\node[Box,fill= BlueL,draw= BlueLine,right=1.0 of $(B1.east)!0.5!(B7.east)$](B9){\textbf{SavedModel\index{SavedModel!TensorFlow export format}}};
%
\def\di{4.35}
\node[Box,text width=50mm,fill= RedL,right=\di of B1,
            draw= RedLine](L1){\textbf{TensorFlow Serving\index{TensorFlow Serving!production deployment}}\\ Cloud, on-prem};
...
class MnistTrainingStepCalc:
    ...
    mnist_total_flops_str = fmt(total_flops/MILLION, precision=0, commas=False, suffix="M")
