...
*   **Systems insight**: The system spends `{python} DispatchTaxSmallLarge.dispatch_overhead_pct_str` percent of time in host-side dispatch and kernel-launch overhead. Compilation yields **`{python} DispatchTaxSmallLarge.dispatch_compilation_speedup_str`$\times$ speedup**.
...
*   **Systems insight**: Python overhead is negligible. Compilation helps only via kernel fusion (memory bandwidth), not dispatch elimination.
...
class ResNetMemory:
    ...
    training_ratio = (training_min_gb * 1000) / fp32_mb
