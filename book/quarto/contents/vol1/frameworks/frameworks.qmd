...
class DispatchTaxSmallLarge:
    ...
    t_sw_math = md_math(f"T_{{sw}} \\approx {large_dispatch_us} \\, \\mu s")
...
class AdminTax:
    ...
    admin_act_calc_math = md_math(f"{admin_batch_str} \\times {admin_layers_str} \\times {admin_width_str}^{{2}} \\times 2 \\approx \\mathbf{{{admin_act_gb_str} \\text{{ GB}}}}")
```
