...
class A100BLAS:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dense_tflops_str = fmt(dense_flops, precision=1, commas=False)
    sparse_tflops_str = fmt(sparse_flops, precision=1, commas=False)
...
class MemoryWallSpecs:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_tflops_fp16_str = fmt(flops_fp16, precision=1, commas=False)
...
class DispatchTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    python_overhead_str = fmt(int(python_overhead_us), precision=1, commas=False)
    small_kernel_str = fmt(int(small_kernel_us), precision=1, commas=False)
    large_kernel_str = fmt(int(large_kernel_us), precision=1, commas=False)
    small_tax_pct_str = fmt(small_tax_pct, precision=1, commas=False)
    large_tax_pct_str = fmt(large_tax_pct, precision=1, commas=False)
```
...
class FusionSpeedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    python_dispatch_us_str = fmt(python_dispatch_us, precision=1, commas=False)
    kernel_launch_only_us_str = fmt(kernel_launch_us, precision=1, commas=False)
    memory_access_us_str = fmt(memory_access_us, precision=1, commas=False)
    kernel_launch_us_str = fmt(launch_overhead, precision=1, commas=False)

    eager_overhead_str = fmt(eager_total_overhead, precision=1, commas=False)
    compiled_overhead_str = fmt(fused_total_overhead, precision=1, commas=False)

    overhead_speedup_str = fmt(int(speedup), precision=1, commas=False)
    bw_efficiency_str = fmt(int(bw_efficiency), precision=1, commas=False)
...
class CompilationBenchmark:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    resnet_eager_str = fmt(resnet["eager"], precision=0, commas=True)
    resnet_compile_str = fmt(resnet["compile"], precision=0, commas=True)
    ...
    resnet_compile_speedup_pct_str = fmt(resnet_compile_speedup_pct, precision=1, commas=False)
    resnet_tensorrt_speedup_str = fmt(resnet_tensorrt_speedup, precision=1, commas=False)
    compile_time_ref_s_str = fmt(compile_time_ref_s, precision=0, commas=False)
    resnet_breakeven_images_str = fmt(resnet_breakeven_images_rounded, precision=0, commas=True)
```
...
class DispatchTaxSmallLarge:
    ...
    dispatch_sw_time_str = fmt(small_sw_total, precision=1, commas=False)
    dispatch_ratio_small_str = fmt(small_overhead_ratio, precision=1, commas=False)
    dispatch_overhead_pct_str = fmt(small_overhead_pct, precision=1, commas=False)
    dispatch_compilation_speedup_str = fmt(small_speedup_limit, precision=1, commas=False)
...
class GPT3Context:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_params_b_str = fmt(params_b, precision=1, commas=False, drop_zero=True)
...
class GPT3MemoryFootprint:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt3_params_b_str = fmt(params_b, precision=1, commas=False, drop_zero=True)               # e.g. "175"
    gpt3_fp16_gb_str = fmt(fp16_gb, precision=1, commas=False)                 # e.g. "350"
```
...
class ResNetMemory:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    resnet_params_m_str = fmt(params_m, precision=1, commas=False, drop_zero=True)
    resnet_fp32_mb_str = fmt(fp32_mb, precision=1, commas=False)
    resnet_adam_mb_str = fmt(adam_mb, precision=1, commas=False)

    resnet_training_min_gb_str = fmt(training_min_gb, precision=1, commas=False)
    resnet_training_max_gb_str = fmt(training_max_gb, precision=1, commas=False)
...
class Model7B:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    model_7b_fp16_gb_str = fmt(fp16_gb, precision=1, commas=False)
    model_7b_adam_gb_str = fmt(adam_gb, precision=1, commas=False)
    model_7b_total_gb_str = fmt(total_gb, precision=1, commas=False)
...
class AdminTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    admin_weights_gb_str = fmt(weights_gb, precision=1, commas=False)
    admin_grads_gb_str = fmt(grads_gb, precision=1, commas=False)
    admin_opt_gb_str = fmt(opt_gb, precision=1, commas=False)
    admin_act_gb_str = fmt(act_gb, precision=1, commas=False)
    admin_tax_gb_str = fmt(tax_gb, precision=1, commas=False)
...
class DeviceBandwidthHierarchy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    pcie4_gbs_str = fmt(pcie4_gbs, precision=1, commas=False)
    pcie4_bidir_gbs_str = fmt(pcie4_gbs * 2, precision=1, commas=False)
    nvlink_a100_gbs_str = fmt(nvlink_a100_gbs, precision=1, commas=False)
    nvlink_a100_per_direction_gbs_str = fmt(nvlink_a100_per_direction_gbs, precision=1, commas=False)
    a100_bw_gbs_str = fmt(a100_bw_gbs, precision=1, commas=False)
    ...
    a100_mem_str = fmt(a100_mem, precision=1, commas=False)
    ...
    pcie4_1gb_ms_str = fmt(pcie4_1gb_ms, precision=1, commas=False)
    pcie4_1gb_equiv_ops_str = fmt((pcie4_1gb_equiv_ops * flop).m_as(TFLOPs), precision=1, commas=False)
    pcie_to_hbm_gap_str = fmt(pcie_to_hbm_gap, precision=1, commas=False)
```
...
class DataloaderThroughput:
    ...
    dataloader_mbs_str = fmt(dataloader_mbs, precision=1, commas=False)
    batch_mb_str = fmt(batch_mb, precision=1, commas=False)
...
class ModuleMemoryAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_7b_fp16_gb_str = fmt(model_7b_fp16_gb, precision=1, commas=False)
    resnet_params_m_str = fmt(resnet_params_m, precision=1, commas=False, drop_zero=True)
```
...
class TrainingStepDims:
    ...
    train_batch_str = fmt(train_batch, precision=0, commas=False)
    train_input_str = fmt(train_input, precision=0, commas=False)
    train_hidden_str = fmt(train_hidden, precision=0, commas=False)
```
...
class HbmCapacityFootnote:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_str = fmt(a100_mem_gb, precision=1, commas=False)
```
...
class MnistTrainingStepCalc:
    ...
    mnist_t_overhead_us_str = fmt(t_overhead_us, precision=1, commas=False)
    ...
    mnist_us_per_op_str = fmt(us_per_op, precision=1, commas=False)
    a100_peak_tflops_str = fmt(peak_flops/TRILLION, precision=1, commas=False)
    a100_mem_bw_tbs_str = fmt(mem_bw_bytes/TRILLION, precision=1, commas=False)
```
...
class Model7BMemory:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_7b_fp16_gb_str = fmt(fp16_gb, precision=1, commas=False)             # e.g. "14"
    a100_remaining_7b_gb_str = fmt(remaining_gb, precision=1, commas=False)     # e.g. "66"
    a100_mem_str = fmt(a100_mem, precision=1, commas=False)                    # e.g. "80"
```
...
class CompilationOverheadCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    resnet_compile_speedup_pct_str = fmt(resnet_compile_speedup_pct, precision=1, commas=False)
    compile_min_s_str = fmt(compile_min_s, precision=1, commas=False)
    compile_max_s_str = fmt(compile_max_s, precision=1, commas=False)
    ...
    compiled_slowdown_str = fmt(compiled_slowdown, precision=1, commas=False)  # e.g. "44"
```
