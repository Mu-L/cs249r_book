...
# │
# │ Goal: Quantify A100 peak FP16/BF16 Tensor Core TFLOP/s (dense ~312 and sparse ~624) to ground
# │       the BLAS-hardware connection and Memory Wall discussion.
# │ Show: "up to 312 TFLOP/s (FP16/BF16) … or 624 TFLOP/s with structured
...
# │       (Kernel Manager subsection).
# │
# │ Imports: mlsysim.core.constants (TFLOPs, second), mlsysim.book (fmt, check)
# │ Exports: A100BLAS.dense_tflops_str, A100BLAS.sparse_tflops_str
# └─────────────────────────────────────────────────────────────────────────────
...
class A100BLAS:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dense_tflops_str = fmt(dense_flops, precision=1, allow_zero=True, commas=False)
    sparse_tflops_str = fmt(sparse_flops, precision=1, allow_zero=True, commas=False)
...
1.  **Solving Performance (1979–1992)**: The **Basic Linear Algebra Subprograms (BLAS)**\index{BLAS!historical foundation}[^fn-blas-performance] [@lawson1979] and **LAPACK**[^fn-lapack-algebra] [@bai2006] solved the problem of *Hardware Primitives*. They provided standardized, highly optimized implementations of matrix operations such as general matrix multiply (GEMM)[^fn-gemm-utilization]. This layer ensures that `C = A @ W` runs at near-peak silicon speed, regardless of the language calling it.

[^fn-gemm-utilization]: **GEMM**: The single operation that the "near-peak silicon speed" claim rests on. Hardware vendors hand-tune GEMM for their specific chips because every layer in a neural network reduces to matrix multiplication, making this one routine the performance floor for all frameworks above it on the ladder. The catch: GEMM achieves peak throughput only when matrix dimensions satisfy strict alignment constraints (multiples of eight for NVIDIA Tensor Cores), and violating these rules drops a framework from over 90 percent to roughly 30 percent of $R_{\text{peak}}$. \index{GEMM!hardware utilization}
...
[^fn-blas-performance]: **BLAS (Basic Linear Algebra Subprograms)**: The 1979 API specification that forms the bottom rung of the ladder described here. By decoupling `C = A @ W` from its hardware implementation, BLAS forced vendors to compete on optimized libraries (NVIDIA cuBLAS, Intel MKL) for a fixed set of primitives. Every framework above it on the ladder inherits this bargain: a single BLAS call from any language can saturate an A100, achieving over `{python} A100BLAS.dense_tflops_str` TFLOP/s for GEMM alone, without the framework knowing anything about the silicon. \index{BLAS!performance foundation}
...
As @fig-mlfm-timeline illustrates, this progression reveals a critical insight: frameworks exist to bridge the gap between mathematical intent and silicon reality [@virtanen2020; @kluyver_thomas2016]. As we move up the ladder, we gain *productivity* but lose *transparency*: a trade-off explored in @sec-ml-frameworks-execution-problem-e1e1.
...
**Systems lesson**: Frameworks are graph construction engines, and in-place operations must respect the values saved for automatic differentiation. Writing `x += 1` does not merely add a number: it can invalidate the graph's history, which is why production frameworks invest in mutation tracking and runtime checks [@paszke2019pytorch].
...
class DispatchTax:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    from mlsysim.core.defaults import KERNEL_LAUNCH_LATENCY_US
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    python_overhead_str = fmt(int(python_overhead_us), precision=1, allow_zero=True, commas=False)
    small_kernel_str = fmt(int(small_kernel_us), precision=1, allow_zero=True, commas=False)
    large_kernel_str = fmt(int(large_kernel_us), precision=1, allow_zero=True, commas=False)
    small_tax_pct_str = fmt(small_tax_pct, precision=1, allow_zero=True, commas=False)
    large_tax_pct_str = fmt(large_tax_pct, precision=1, allow_zero=True, commas=False)
```
...
*   **For small operations** (for example, a ReLU on a small vector), the kernel might execute in only **`{python} DispatchTax.small_kernel_str` $\mu$s**. The dispatch tax is **`{python} DispatchTax.small_tax_pct_str` percent**, meaning the GPU spends the vast majority of its time waiting for the next command.
*   **For large operations** (for example, a large matrix multiply), the kernel executes for **`{python} DispatchTax.large_kernel_str` $\mu$s**. The dispatch tax drops to **`{python} DispatchTax.large_tax_pct_str` percent**, and the system becomes compute bound.
...
class FusionSpeedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    python_dispatch_us_str = fmt(python_dispatch_us, precision=1, allow_zero=True, commas=False)
    kernel_launch_only_us_str = fmt(kernel_launch_us, precision=1, allow_zero=True, commas=False)
    memory_access_us_str = fmt(memory_access_us, precision=1, allow_zero=True, commas=False)
    kernel_launch_us_str = fmt(launch_overhead, precision=1, allow_zero=True, commas=False)

    eager_overhead_str = fmt(eager_total_overhead, precision=1, allow_zero=True, commas=False)
    compiled_overhead_str = fmt(fused_total_overhead, precision=1, allow_zero=True, commas=False)

    overhead_speedup_str = fmt(int(speedup), precision=1, allow_zero=True, commas=False)
    bw_efficiency_str = fmt(int(bw_efficiency), precision=1, allow_zero=True, commas=False)
...
class CompilationBenchmark:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    resnet_eager_str = fmt(resnet["eager"], precision=1, allow_zero=True, commas=True)
    resnet_compile_str = fmt(resnet["compile"], precision=1, allow_zero=True, commas=True)
    ...
    resnet_compile_speedup_pct_str = fmt(resnet_compile_speedup_pct, precision=1, allow_zero=True, commas=False)
    resnet_tensorrt_speedup_str = fmt(resnet_tensorrt_speedup, precision=1, commas=False)
    compile_time_ref_s_str = fmt(compile_time_ref_s, precision=1, allow_zero=True, commas=False)
    resnet_breakeven_images_str = fmt(resnet_breakeven_images_rounded, precision=1, allow_zero=True, commas=True)
```
...
: **Framework Execution Strategy by Workload**: Recommended execution strategy for each workload archetype, aligned to the dominant iron law term. Compute-bound workloads benefit most from compilation, while irregular access patterns favor eager execution. {#tbl-framework-archetype-strategy tbl-colwidths="[17,31,22,30]"}

**Systems insight**: Compilation benefits scale with how much of the workload is optimizable. Compute Beasts (@tbl-training-benchmark: ResNet-50 sees `{python} CompilationBenchmark.resnet_tensorrt_speedup_str`$\times$ speedup from TensorRT) benefit most. Sparse Scatter workloads gain little because their bottleneck (embedding lookups) is inherently irregular.
...
For *training runs* ($N_{\text{prod}} \gg N_{\text{dev}}$), compilation pays off. A typical training run executes millions of forward/backward passes, so even 60 seconds of compilation amortizes to microseconds per step. From @tbl-training-benchmark, torch.compile provides `{python} CompilationBenchmark.resnet_compile_speedup_pct_str` percent higher throughput on ResNet-50 (`{python} CompilationBenchmark.resnet_compile_str` vs. `{python} CompilationBenchmark.resnet_eager_str` img/sec). Using a `{python} CompilationBenchmark.compile_time_ref_s_str` s compile cost, this pays off after the breakeven point in @eq-compile-breakeven:

$$
N_{\text{breakeven}} = \frac{T_{\text{compile}}}{T_{\text{eager}} - T_{\text{compiled}}}
$$ {#eq-compile-breakeven}

Evaluating @eq-compile-breakeven with the ResNet-50 table values gives approximately `{python} CompilationBenchmark.resnet_breakeven_images_str` images. For ImageNet (1.28M training images), compilation pays off within the first epoch.
...
class DispatchTaxSmallLarge:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dispatch_n_ops_str = fmt(small_ops_count, precision=1, allow_zero=True, commas=False)
    dispatch_us_per_op_str = fmt(small_dispatch_us, precision=1, commas=False)
    dispatch_hw_time_us_str = fmt(small_hw_us, precision=1, commas=False)

    dispatch_sw_time_str = fmt(small_sw_total, precision=1, allow_zero=True, commas=False)
    dispatch_ratio_small_str = fmt(small_overhead_ratio, precision=1, commas=False)
    dispatch_overhead_pct_str = fmt(small_overhead_pct, precision=1, allow_zero=True, commas=False)
    dispatch_compilation_speedup_str = fmt(small_speedup_limit, precision=1, allow_zero=True, commas=False)

    gpt3_hw_time_us_str = fmt(large_hw_us, precision=1, commas=False)
    gpt3_sw_time_us_str = fmt(large_dispatch_us, precision=1, commas=False)
...
# Note: Use CompilationOverheadCalc.dispatch_sw_time_str directly.
```
...
*   **Hardware time**: $T_{\text{hw}} \approx$ `{python} DispatchTaxSmallLarge.dispatch_hw_time_us_str` μs (mostly memory latency).
*   **Software overhead**: $T_{\text{sw}} \approx$ `{python} DispatchTaxSmallLarge.dispatch_n_ops_str` ops $\times$ `{python} DispatchTaxSmallLarge.dispatch_us_per_op_str` $\mu$s/op = `{python} DispatchTaxSmallLarge.dispatch_sw_time_str` $\mu$s.
...
*   **Small-model outcome**: The system spends `{python} DispatchTaxSmallLarge.dispatch_overhead_pct_str` percent of time in host-side dispatch and kernel-launch overhead. Compilation yields **`{python} DispatchTaxSmallLarge.dispatch_compilation_speedup_str`$\times$ speedup**.

**Scenario two: GPT-3 Layer (Compute Bound)**

*   **Compute**: Huge matrix multiplications.
*   **Hardware time**: $T_{\text{hw}} \approx 100$ ms = `{python} DispatchTaxSmallLarge.gpt3_hw_time_us_str` μs.
...
Micro-frameworks like **TensorFlow Lite Micro (TFLM)**\index{TensorFlow Lite Micro!extreme AOT} [@david2021tensorflow] and **PyTorch ExecuTorch**\index{PyTorch ExecuTorch!microcontroller deployment} solve this through **Extreme AOT Compilation**:
...
2.  **Kernel specialization**: Only the specific kernels used by the model (for example, Conv2D, DepthwiseConv) are compiled into the binary. Unused code is stripped away.
...
The critical observation is that this single backward pass computed $dc/dx$ regardless of how many paths connect $x$ to $c$. In a neural network, each weight can affect the loss through thousands of paths across layers, and reverse mode handles them all in one traversal. This is why training a `{python} GPT3Context.gpt3_params_b_str`-billion-parameter model like GPT-3 is feasible at all: reverse mode's $\mathcal{O}(1)$ backward passes (relative to parameter count) keeps gradient computation tractable.
...
class Model7B:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    model_7b_fp16_gb_str = fmt(fp16_gb, precision=1, allow_zero=True, commas=False)
    model_7b_adam_gb_str = fmt(adam_gb, precision=1, allow_zero=True, commas=False)
    model_7b_total_gb_str = fmt(total_gb, precision=1, allow_zero=True, commas=False)

# Note: Use Model7B.model_7b_total_gb_str directly.
```

Resuming training after interruption requires restoring model weights and optimizer state together: momentum buffers, adaptive learning rates, and gradient statistics. For Adam, optimizer state adds about 4$\times$ the FP16 weight memory (two FP32 states per parameter), so weights plus optimizer state require about 5$\times$ the FP16 weight footprint. A 7-billion-parameter model therefore requires approximately `{python} Model7B.model_7b_total_gb_str` GB total (`{python} Model7B.model_7b_fp16_gb_str` GB weights + `{python} Model7B.model_7b_adam_gb_str` GB optimizer state). Checkpoint size therefore bounds recovery speed after failure, connecting fault tolerance directly to the iron law's $D_{\text{vol}}$ term.
...
A ResNet-50 forward pass touches `{python} ResNetAbstraction.resnet_params_m_str` million parameters, produces intermediate activations at every layer, and must coordinate memory across CPU and GPU address spaces. Frameworks organize all of this data so that a single Python call like `model(input)` executes millions of operations without the programmer managing a single pointer, by solving four problems in sequence: defining a universal data container (tensors), placing it on the right device (memory management), feeding data fast enough (data pipelines), and dispatching the right hardware kernel (core operations). We trace this path from data representation to hardware execution.
...
class TensorFootprint:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tensor_1024_fp32_mb_str = fmt(footprint_mb, precision=1, commas=False)
```
...
1.  **Significance (quantitative)**: Tensor memory footprint is fully deterministic from its metadata: a contiguous FP32 tensor of shape $[1024, 1024]$ occupies $1024{\times}1024{\times}4 = 4{,}194{,}304$ bytes (about `{python} TensorFootprint.tensor_1024_fp32_mb_str` MB). Noncontiguous layouts (for example, from a transpose operation) require explicit `.contiguous()` calls before certain CUDA kernels can execute, adding a memory-copy cost that can dominate the data movement term $(D_{\text{vol}}/\text{BW})$ for tensors under 1 MB.
...
class AdminTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    admin_weights_gb_str = fmt(weights_gb, precision=1, allow_zero=True, commas=False)
    admin_grads_gb_str = fmt(grads_gb, precision=1, allow_zero=True, commas=False)
    admin_opt_gb_str = fmt(opt_gb, precision=1, allow_zero=True, commas=False)
    admin_act_gb_str = fmt(act_gb, precision=1, commas=False)
    admin_tax_gb_str = fmt(tax_gb, precision=1, allow_zero=True, commas=False)
    admin_batch_str = fmt(batch_size, precision=1, allow_zero=True, commas=False)
    admin_layers_str = fmt(layers, precision=1, allow_zero=True, commas=False)
    admin_width_str = fmt(width, precision=1, allow_zero=True, commas=False)

    admin_act_calc_math = fmt_math(f"{admin_batch_str} \\times {admin_layers_str} \\times {admin_width_str}^{{2}} \\times 2 \\approx \\mathbf{{{admin_act_gb_str} \\text{{ GB}}}}")
```

::: {#nbk-frameworks-administrative-tax .callout-notebook title="The administrative tax"}

The memory breakdown for ResNet-50 in @sec-ml-frameworks-principle-2-memorycompute-tradeoff-19f2 showed a concrete ~`{python} ResNetMemory.resnet_training_ratio_min_str`--`{python} ResNetMemory.resnet_training_ratio_max_str`$\times$ ratio between training and inference memory. Here we generalize that analysis to reveal the full administrative overhead at billion-parameter scale.
...
3.  **Optimizer states (Adam)**: `{python} AdminTax.admin_opt_gb_str` GB ($4 \times$ FP16 weight memory for momentum and velocity stored in FP32).
...
**Systems insight**: A `{python} AdminTax.admin_weights_gb_str` GB model carries persistent training state before the first batch is processed (`{python} AdminTax.admin_grads_gb_str` GB gradients + `{python} AdminTax.admin_opt_gb_str` GB optimizer state). During a training step, batch-dependent activations add another `{python} AdminTax.admin_act_gb_str` GB, raising the peak **"Administrative Tax"** to ~`{python} AdminTax.admin_tax_gb_str` GB beyond the weights. During training, **Data Movement** includes saving and retrieving these activations, which is why training is often 3--4$\times$ slower than pure inference.
...
# │
# │ Imports: mlsysim.core.constants (PCIE_GEN4_BW, NVLINK_A100_BW, Hardware.Cloud.A100.memory.bandwidth, Hardware.Cloud.A100.memory.capacity,
...
class DeviceBandwidthHierarchy:
    ...
    a100_mem = Hardware.Cloud.A100.memory.capacity.m_as(GB)
    ...
    nvlink_4mb_ms = (tensor_4mb/(NVLINK_A100_BW.m_as(byte/second) / 2)) * THOUSAND
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    pcie4_gbs_str = fmt(pcie4_gbs, precision=1, allow_zero=True, commas=False)
    pcie4_bidir_gbs_str = fmt(pcie4_gbs * 2, precision=1, allow_zero=True, commas=False)
    nvlink_a100_gbs_str = fmt(nvlink_a100_gbs, precision=1, allow_zero=True, commas=False)
    nvlink_a100_per_direction_gbs_str = fmt(nvlink_a100_per_direction_gbs, precision=1, allow_zero=True, commas=False)
    a100_bw_gbs_str = fmt(a100_bw_gbs, precision=1, allow_zero=True, commas=False)
    ...
    a100_mem_str = fmt(a100_mem, precision=1, allow_zero=True, commas=False)
    ...
    pcie4_1gb_ms_str = fmt(pcie4_1gb_ms, precision=1, allow_zero=True, commas=False)
    pcie4_1gb_equiv_ops_str = fmt((pcie4_1gb_equiv_ops * flop).m_as(TFLOPs), precision=1, commas=False)
    pcie_to_hbm_gap_str = fmt(pcie_to_hbm_gap, precision=1, allow_zero=True, commas=False)
```
...
| **NVLink 3.0**   | `{python} DeviceBandwidthHierarchy.nvlink_a100_per_direction_gbs_str` GB/s per direction (`{python} DeviceBandwidthHierarchy.nvlink_a100_gbs_str` GB/s bidirectional) | `{python} DeviceBandwidthHierarchy.nvlink_4mb_ms_str` ms | Comparable to GPU compute          |
...
These numbers connect directly to the iron law of performance. Every cross-device transfer inflates the data movement term $(D_{\text{vol}}/\text{BW})$ at a fraction of the available on-device bandwidth. A PCIe 4.0 transfer at `{python} DeviceBandwidthHierarchy.pcie4_gbs_str` GB/s means moving a 1 GB activation tensor adds approximately `{python} DeviceBandwidthHierarchy.pcie4_1gb_ms_str` ms to the data movement cost, equivalent to roughly `{python} DeviceBandwidthHierarchy.pcie4_1gb_equiv_ops_str` trillion operations on a GPU delivering `{python} DeviceBandwidthHierarchy.a100_tflops_fp16_str` TFLOP/s. For a model forward pass taking 0.5 ms on GPU, transferring inputs and outputs over PCIe 3.0 doubles the total latency. When batches are small or models are lightweight, transfer overhead can exceed computation time entirely.
...
# │ Exports: img_res_str, dataloader_mbs_str, batch_mb_str, batch_transfer_ms_str, DeviceBandwidthHierarchy.pcie4_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
A GPT-3 scale model stores `{python} GPT3ParameterStructures.gpt3_params_b_str` billion parameters, occupying `{python} GPT3ParameterStructures.gpt3_fp16_gb_str` GB in FP16. Managing these parameters across devices, keeping gradients synchronized, and maintaining optimizer state (Adam state alone can add about 4$\times$ the FP16 weight memory, as the Administrative Tax notebook showed) is a core framework responsibility.
...
# │
# │ Imports: mlsysim (Models), mlsysim.core.constants, mlsysim.core.formulas
# │ Exports: ModuleMemoryAnchor.model_7b_fp16_gb_str,
# │          ModuleMemoryAnchor.resnet_params_m_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.core.formulas import model_memory
from mlsysim.fmt import fmt, check
...
# ┌── LEGO ───────────────────────────────────────────────
class ModuleMemoryAnchor:
    """Local memory anchors for module-level framework abstractions."""
...
### Automatic parameter discovery { .unnumbered}
...
::: {#psp-frameworks-cross-framework-parameter-discovery .callout-perspective title="Cross-framework parameter discovery"}

@tbl-frameworks-parameter-discovery-apis shows how the same principle manifests differently across frameworks:
...
: **Parameter Discovery APIs Across Frameworks**: Each framework exposes a different surface for distinguishing trainable parameters from nontrainable buffers, but all four solve the same underlying problem of letting optimizers find weights to update while preserving state (running statistics, frozen tensors) across forward passes. {#tbl-frameworks-parameter-discovery-apis}

Despite syntactic differences, all frameworks solve the same problem: enabling optimizers to discover and update trainable parameters while preserving nontrainable state across forward passes.
...
### Hierarchical composition and serialization { .unnumbered}
...
PyTorch's eager execution and TensorFlow's graph compilation represent two points on a spectrum, yet both share an imperative programming\index{Imperative Programming!framework heritage} heritage where computation proceeds as a sequence of stateful operations. JAX represents a radically different approach, one built on functional programming principles and composable program transformations rather than computational graphs [@bradbury2018jax]. Developed by Google Research, JAX has gained significant traction in research settings, particularly for work requiring custom differentiation, advanced optimization research, and large-scale distributed training.
...
| **TF Lite Micro/microTVM**      | Microcontroller inference      | KB-scale RAM budgets, often <256 KB for small TinyML deployments  | Static allocation, INT8 kernels               | Small operator set and no dynamic runtime  |
...
![](images/jpeg/onnx_new.jpg){width=60%}
...
# │
# │ Imports: mlsysim.fmt (fmt, check)
# │ Exports: train_batch (int), train_input (int), train_hidden (int), train_output (int),
# │          train_batch_str, train_input_str, train_hidden_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class TrainingStepDims:
    ...
    train_input = input_features                                                # MNIST input (28 × 28)
    train_hidden = 256                                                          # hidden layer
    train_output = 10                                                           # MNIST classes

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Suffix aliases for inline {python} use; raw ints kept for arithmetic at
    # the downstream consumer cell (see ForwardCost class below).
    train_batch_str = fmt(train_batch, precision=1, allow_zero=True, commas=False)
    train_input_str = fmt(train_input, precision=1, allow_zero=True, commas=False)
    train_hidden_str = fmt(train_hidden, precision=1, allow_zero=True, commas=False)
```
...
class TrainingStepCalc:
    ...
    train_relu_flops_str = fmt((relu_flops * flop).m_as(KFLOPs), precision=1, allow_zero=True, commas=False) + " KFLOPs"
    train_relu_mem_str = fmt((relu_mem_bytes * byte).m_as(KB), precision=1, allow_zero=True, commas=False) + " KB"
    train_relu_ai_str = fmt(relu_flops/relu_mem_bytes, precision=3, commas=False)

    train_mm2_flops_str = fmt((mm2_flops * flop).m_as(KFLOPs), precision=1, allow_zero=True, commas=False) + " KFLOPs"
    train_mm2_mem_str = fmt((mm2_mem_bytes * byte).m_as(KB), precision=1, allow_zero=True, commas=False) + " KB"
    train_mm2_ai_str = fmt(mm2_flops/mm2_mem_bytes, precision=1, commas=False)

    train_ce_flops_str = fmt((ce_flops * flop).m_as(KFLOPs), precision=1, allow_zero=True, commas=False) + " KFLOPs"
    train_ce_mem_str = fmt((ce_mem_bytes * byte).m_as(KB), precision=1, allow_zero=True, commas=False) + " KB"
    train_ce_ai_str = fmt(ce_flops/ce_mem_bytes, precision=1, commas=False)

    train_bwd_flops_str = fmt((bwd_flops * flop).m_as(MFLOPs), precision=1, allow_zero=True, commas=False) + " MFLOPs"
...
class MnistTrainingStepCalc:
    ...
    mem_traffic_bytes = TrainingStepCalc.forward_mem_bytes + TrainingStepCalc.bwd_mem_bytes
    ...
    mnist_t_overhead_us_str = fmt(t_overhead_us, precision=1, allow_zero=True, commas=False)
    mnist_total_flops_str = fmt(total_flops/MILLION, precision=1, allow_zero=True, commas=False, suffix=" MFLOPs")
    mnist_mem_traffic_str = fmt(mem_traffic_bytes/MILLION, precision=1, commas=False, suffix=" MB")
    mnist_n_ops_str = fmt(n_ops, precision=1, allow_zero=True, commas=False)
    mnist_us_per_op_str = fmt(us_per_op, precision=1, allow_zero=True, commas=False)
    a100_peak_tflops_str = fmt(peak_flops/TRILLION, precision=1, allow_zero=True, commas=False)
    a100_mem_bw_tbs_str = fmt(mem_bw_bytes/TRILLION, precision=1, allow_zero=True, commas=False)
```
...
- $T_{\text{compute}} \approx$ `{python} MnistTrainingStepCalc.mnist_total_flops_str`/`{python} MnistTrainingStepCalc.a100_peak_tflops_str` TFLOP/s ≈ `{python} MnistTrainingStepCalc.mnist_t_compute_us_str` μs
- $T_{\text{memory}} \approx$ `{python} MnistTrainingStepCalc.mnist_mem_traffic_str`/`{python} MnistTrainingStepCalc.a100_mem_bw_tbs_str` TB/s ≈ `{python} MnistTrainingStepCalc.mnist_t_memory_us_str` μs
- $T_{\text{overhead}} \approx$ `{python} MnistTrainingStepCalc.mnist_n_ops_str` ops $\times$ `{python} MnistTrainingStepCalc.mnist_us_per_op_str` μs ≈ `{python} MnistTrainingStepCalc.mnist_t_overhead_us_str` μs
...
Engineers assume high-level APIs handle all optimization automatically. The Roofline Model\index{Roofline Model!compute vs. memory bound} (@sec-machine-foundations-roofline-model-2529) proves otherwise: element-wise operations like ReLU achieve arithmetic intensity of 0.125 FLOP/byte, using under 0.1 percent of an A100's peak compute regardless of framework sophistication. @sec-ml-frameworks-execution-strategy-matters-memory-wall-1ce8 explains why: memory bandwidth, not compute, is the bottleneck for most operations. Engineers who lack this understanding leave 80--90 percent of hardware capacity unused, directly translating to 5--10$\times$ higher inference costs at production scale.
...
class Model7BMemory:
    ...
    a100_mem = Hardware.Cloud.A100.memory.capacity.m_as(GB)                                      # decimal GB
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
...
class CompilationOverheadCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    benchmark = CompilationBenchmark.resnet
    n_images = 10_000                                                           # small experiment
    eager_throughput = benchmark["eager"]                                       # images/sec (eager)
    compiled_throughput = benchmark["compile"]                                  # images/sec (compiled)
    compile_min_s = benchmark["compile_min_s"]
    compile_max_s = benchmark["compile_max_s"]
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    resnet_compile_speedup_pct_str = fmt(resnet_compile_speedup_pct, precision=1, allow_zero=True, commas=False)
    compile_min_s_str = fmt(compile_min_s, precision=1, allow_zero=True, commas=False)
    compile_max_s_str = fmt(compile_max_s, precision=1, allow_zero=True, commas=False)
    n_images_str = fmt(n_images, precision=1, allow_zero=True)                                   # e.g. "10,000"
    n_recompilations_str = fmt(n_recompilations, precision=1, allow_zero=True, commas=False)    # e.g. "10"
    eager_throughput_str = fmt(eager_throughput, precision=1, allow_zero=True)                   # e.g. "1,450"
    compiled_throughput_str = fmt(compiled_throughput, precision=1, allow_zero=True)             # e.g. "2,150"
    compilation_time_s_str = fmt(compilation_time_s, precision=1, allow_zero=True, commas=False)  # e.g. "30"
    ...
    compiled_slowdown_str = fmt(compiled_slowdown, precision=1, allow_zero=True, commas=False)  # e.g. "44"
```

Engineers assume compilation overhead is a one-time cost that pays off quickly. @tbl-training-benchmark shows torch.compile achieves `{python} CompilationOverheadCalc.resnet_compile_speedup_pct_str` percent higher ResNet-50 throughput, but the same table also assigns ResNet-50 a compile window of `{python} CompilationOverheadCalc.compile_min_s_str` to `{python} CompilationOverheadCalc.compile_max_s_str` seconds per graph change. For a `{python} CompilationOverheadCalc.n_images_str`-image experiment with `{python} CompilationOverheadCalc.n_recompilations_str` code changes: Eager completes in `{python} CompilationOverheadCalc.eager_total_str` seconds while Compiled requires `{python} CompilationOverheadCalc.compiled_total_str` seconds (including `{python} CompilationOverheadCalc.n_recompilations_str` $\times$ `{python} CompilationOverheadCalc.compilation_time_s_str` s recompilation overhead), making the compiled workflow about `{python} CompilationOverheadCalc.compiled_slowdown_str`$\times$ slower in this rapid-prototyping scenario. Teams that enable compilation during rapid prototyping can waste minutes waiting for recompilations that negate any throughput gains.
