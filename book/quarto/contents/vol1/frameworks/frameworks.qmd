...
| **PCIe 4.0 x16** |                                                         `{python} DeviceBandwidthHierarchy.pcie4_gbs_str` GB/s                                                        |  `{python} DeviceBandwidthHierarchy.pcie4_4mb_ms_str` | 5$\times$ slower than GPU compute  |
| **NVLink 3.0**   | `{python} DeviceBandwidthHierarchy.nvlink_a100_per_direction_gbs_str` GB/s per direction (`{python} DeviceBandwidthHierarchy.nvlink_a100_gbs_str` GB/s bidirectional) | `{python} DeviceBandwidthHierarchy.nvlink_4mb_ms_str` | Comparable to GPU compute          |
| **GPU Memory**   |                                                        `{python} DeviceBandwidthHierarchy.a100_bw_gbs_str` GB/s                                                       |   `{python} DeviceBandwidthHierarchy.hbm_4mb_ms_str`  | Optimal                            |
...
Engineers assume compilation overhead is a one-time cost that pays off quickly. @tbl-training-benchmark shows torch.compile achieves `{python} CompilationOverheadCalc.resnet_compile_speedup_pct_str` higher ResNet-50 throughput, but the same table also assigns ResNet-50 a compile window of `{python} CompilationOverheadCalc.compile_min_s_str` to `{python} CompilationOverheadCalc.compile_max_s_str` per graph change. For a `{python} CompilationOverheadCalc.n_images_str`-image experiment with `{python} CompilationOverheadCalc.n_recompilations_str` code changes: Eager completes in `{python} CompilationOverheadCalc.eager_total_str` while Compiled requires `{python} CompilationOverheadCalc.compiled_total_str` (including `{python} CompilationOverheadCalc.n_recompilations_str` $\times$ `{python} CompilationOverheadCalc.compilation_time_s_str` s recompilation overhead), making the compiled workflow about `{python} CompilationOverheadCalc.compiled_slowdown_str`$\times$ slower in this rapid-prototyping scenario. Teams that enable compilation during rapid prototyping can waste minutes waiting for recompilations that negate any throughput gains.
