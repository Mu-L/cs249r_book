...
# │          resnet_tensorrt_str, torch_compile_speedup_min_str,
...
class CompilationBenchmark:
    ...
    resnet_tensorrt_str = fmt(resnet["tensorrt"], precision=0, commas=True)
...
# │       decimal units.
# │ Show: 1024 × 1024 × 4 bytes = 4 MiB, about 4.2 MB.
# │ How: Multiply shape by BYTES_FP32 and format in MiB and MB.
# │
# │ Imports: mlsysim.core.constants (BYTES_FP32, byte, MB, MiB), mlsysim.book (fmt, check)
# │ Exports: TensorFootprint.tensor_1024_fp32_mib_str, tensor_1024_fp32_mb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import BYTES_FP32, byte, MB, MiB
from mlsysim.fmt import fmt, check
...
class TensorFootprint:
    ...
    footprint_mib = footprint.m_as(MiB)
    footprint_mb = footprint.m_as(MB)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(footprint_mib == 4, f"Expected exactly 4 MiB, got {footprint_mib}.")
    check(4.1 < footprint_mb < 4.2, f"Expected about 4.2 MB, got {footprint_mb}.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tensor_1024_fp32_mib_str = fmt(footprint_mib, precision=0, commas=False)
...
1.  **Significance (quantitative)**: Tensor memory footprint is fully deterministic from its metadata: a contiguous FP32 tensor of shape $[1024, 1024]$ occupies exactly $1024{\times}1024{\times}4$ bytes = `{python} TensorFootprint.tensor_1024_fp32_mib_str` MiB (about `{python} TensorFootprint.tensor_1024_fp32_mb_str` MB). Noncontiguous layouts (for example, from a transpose operation) require explicit `.contiguous()` calls before certain CUDA kernels can execute, adding a memory-copy overhead that can dominate the $L_{\text{lat}}$ term for tensors under 1 MB.
