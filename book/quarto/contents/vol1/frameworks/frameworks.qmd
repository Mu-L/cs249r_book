...
For *training runs* ($N_{\text{prod}} \gg N_{\text{dev}}$), compilation pays off. A typical training run executes millions of forward/backward passes, so even 60 seconds of compilation amortizes to microseconds per step. From @tbl-training-benchmark, torch.compile provides `{python} CompilationBenchmark.resnet_compile_speedup_pct_str` higher throughput on ResNet-50 (`{python} CompilationBenchmark.resnet_compile_str` vs. `{python} CompilationBenchmark.resnet_eager_str` img/sec). Using a `{python} CompilationBenchmark.compile_time_ref_s_str` s compile cost, this pays off after the breakeven point in @eq-compile-breakeven:
...
*   **Small-model outcome**: The system spends `{python} DispatchTaxSmallLarge.dispatch_overhead_pct_str` of time in host-side dispatch and kernel-launch overhead. Compilation yields **`{python} DispatchTaxSmallLarge.dispatch_compilation_speedup_str`$\times$ speedup**.
...
Every tensor resides on a specific device, and cross-device operations incur transfer costs that can dominate execution time. PCIe 4.0 delivers `{python} DeviceBandwidthHierarchy.pcie4_gbs_str`/s between CPU and GPU, while HBM2e provides `{python} DeviceBandwidthHierarchy.a100_bw_tbs_str`/s within the GPU. This `{python} DeviceBandwidthHierarchy.pcie_to_hbm_gap_str`$\times$ bandwidth gap means a single misplaced tensor transfer can erase the entire speedup from GPU acceleration.
...
[^fn-nvlink-bandwidth-hierarchy]: **NVLink**: NVIDIA's high-bandwidth GPU-to-GPU interconnect (see @sec-hardware-acceleration), providing `{python} DeviceBandwidthHierarchy.nvlink_a100_gbs_str`/s bidirectional bandwidth (NVLink 3.0 on A100) compared to `{python} DeviceBandwidthHierarchy.pcie4_bidir_gbs_str`/s for PCIe 4.0 x16. This ~10$\times$ bandwidth advantage determines whether tensor parallelism is practical for a given model size: splitting a model across GPUs connected by PCIe can make the $D_{\text{vol}}/\text{BW}$ communication term dominate total training time, erasing the benefit of additional compute. \index{NVLink!bandwidth hierarchy}

| **Interconnect** |                                                                          **Bandwidth**                                                                          |                   **Transfer Time**                   | **Relative to Compute**            |
|:-----------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------:|:-----------------------------------------------------:|:-----------------------------------|
| **PCIe 3.0 x16** |                                                                             16 GB/s                                                                             |                        0.25 ms                        | 10$\times$ slower than GPU compute |
| **PCIe 4.0 x16** |                                                       `{python} DeviceBandwidthHierarchy.pcie4_gbs_str`/s                                                       |  `{python} DeviceBandwidthHierarchy.pcie4_4mb_ms_str` | 5$\times$ slower than GPU compute  |
| **NVLink 3.0**   | `{python} DeviceBandwidthHierarchy.nvlink_a100_per_direction_gbs_str`/s per direction (`{python} DeviceBandwidthHierarchy.nvlink_a100_gbs_str`/s bidirectional) | `{python} DeviceBandwidthHierarchy.nvlink_4mb_ms_str` | Comparable to GPU compute          |
| **GPU Memory**   |                                                      `{python} DeviceBandwidthHierarchy.a100_bw_gbs_str`/s                                                      |   `{python} DeviceBandwidthHierarchy.hbm_4mb_ms_str`  | Optimal                            |
...
These numbers connect directly to the iron law of performance. Every cross-device transfer inflates the data movement term $(D_{\text{vol}}/\text{BW})$ at a fraction of the available on-device bandwidth. A PCIe 4.0 transfer at `{python} DeviceBandwidthHierarchy.pcie4_gbs_str`/s means moving a 1 GB activation tensor adds approximately `{python} DeviceBandwidthHierarchy.pcie4_1gb_ms_str` to the data movement cost, equivalent to roughly `{python} DeviceBandwidthHierarchy.pcie4_1gb_equiv_ops_str` trillion operations on a GPU delivering `{python} DeviceBandwidthHierarchy.a100_tflops_fp16_str`. For a model forward pass taking 0.5 ms on GPU, transferring inputs and outputs over PCIe 3.0 doubles the total latency. When batches are small or models are lightweight, transfer overhead can exceed computation time entirely.
...
- $T_{\text{compute}} \approx$ `{python} MnistTrainingStepCalc.mnist_total_flops_str`/`{python} MnistTrainingStepCalc.a100_peak_tflops_str` ≈ `{python} MnistTrainingStepCalc.mnist_t_compute_us_str` μs
- $T_{\text{memory}} \approx$ `{python} MnistTrainingStepCalc.mnist_mem_traffic_str`/`{python} MnistTrainingStepCalc.a100_mem_bw_tbs_str`/s ≈ `{python} MnistTrainingStepCalc.mnist_t_memory_us_str` μs
...
Engineers assume compilation overhead is a one-time cost that pays off quickly. @tbl-training-benchmark shows torch.compile achieves `{python} CompilationOverheadCalc.resnet_compile_speedup_pct_str` higher ResNet-50 throughput, but the same table also assigns ResNet-50 a compile window of `{python} CompilationOverheadCalc.compile_min_s_str` to `{python} CompilationOverheadCalc.compile_max_s_str` per graph change. For a `{python} CompilationOverheadCalc.n_images_str`-image experiment with `{python} CompilationOverheadCalc.n_recompilations_str` code changes: Eager completes in `{python} CompilationOverheadCalc.eager_total_str` s, whereas Compiled requires `{python} CompilationOverheadCalc.compiled_total_str` s (including `{python} CompilationOverheadCalc.n_recompilations_str` $\times$ `{python} CompilationOverheadCalc.compilation_time_s_str` s recompilation overhead), making the compiled workflow about `{python} CompilationOverheadCalc.compiled_slowdown_str`$\times$ slower in this rapid-prototyping scenario. Teams that enable compilation during rapid prototyping can waste minutes waiting for recompilations that negate any throughput gains.
