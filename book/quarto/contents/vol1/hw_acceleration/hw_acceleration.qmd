...
class AcceleratorEconomics:
    ...
    v100_pp_str    = fmt(v100_ratio, precision=0, commas=False, prefix="$", suffix="/(TFLOP/s)")
    ...
    a100_pp_str      = fmt(a100_ratio, precision=0, commas=False, prefix="$", suffix="/(TFLOP/s)")
    ...
    h100_pp_str      = fmt(h100_ratio, precision=0, commas=False, prefix="~$", suffix="/(TFLOP/s)")
    ...
    tpu_pp_str    = fmt(tpu_ratio, precision=0, commas=False, prefix="~$", suffix="/(TFLOP/s)")
    ...
    gaudi_pp_str    = fmt(gaudi_ratio, precision=0, commas=False, prefix="$", suffix="/(TFLOP/s)")
```
...
class FeasibilityAssessment:
    ...
    Check 1: Memory—7B model (FP16) on 16 GB GPU.
    Check 2: Bandwidth—70B model on 1 TB/s GPU.
    Check 3: Compute—30 FPS video processing.
    """

    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    # Memory check: Llama-7B on 16 GB GPU
    params_7b = 7e9
    gpu_mem_gb = 16

    # Bandwidth check: 70B model on 1 TB/s GPU
    params_70b = 70e9
...
*   **Formula**: $T_{\text{token}} = \frac{D_{\text{vol},\text{model}}}{\text{BW}}$
