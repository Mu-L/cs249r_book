...
class SystemsGapRates:
    ...
    gpu_growth_per_year_str = fmt_int(gpu_growth_per_year, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_percent, fmt, MarkdownStr, check
...
class SystolicEnergy:
    ...
    systolic_energy_str = fmt(e_systolic, precision=0, commas=False)
...
class TilingPrinciple:
    ...
    tile_count_str = fmt_int(total_tiles,)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  fmt_percent, fmt
...
class SystolicOpsCalc:
    ...
    tpuv4_bw_str     = fmt_int(tpuv4_bw_gbs)
```
...
class InterconnectBandwidth:
    ...
    nvlink_bw_str = fmt_int(nvlink_bw, commas=False, suffix=" GB/s")
    pcie_bw_str = fmt_int(pcie_bw, commas=False, suffix=" GB/s")
    net_bw_str = fmt_int(net_bw, commas=False, suffix=" GB/s")
    gap_ratio_str = fmt_int(hbm_bw/pcie_bw, commas=False, suffix="x")
...
class AcceleratorEconomics:
    ...
    # H100 specs
    h100_tflops_tf32_str = fmt(h_h100.compute.precision_flops["tf32"].m_as(TFLOPs/second), precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt_percent, fmt, MarkdownStr
...
class TensorLifecycleCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    kws_tensor_str         = fmt(kws_tensor_kb_value, precision=0, commas=False)
...
class RooflineGap:
    ...
    v100_ridge_str = fmt_int(round(v100_ridge), commas=False)
    a100_ridge_str = fmt_int(round(a100_ridge), commas=False)
    h100_ridge_str = fmt_int(round(h100_ridge), commas=False)
    a100_ridge_fp32_str = fmt_int(round(a100_ridge_fp32), commas=False)
...
class LayernormAnalysisCalc:
    ...
    ln_params_kb_str    = fmt(ln_params_kb, precision=1, commas=False)
    ...
    ln_ridge_gap_str    = fmt_int(round(ln_ridge_gap), commas=False)
...
class Gpt2ThroughputCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt2_weight_gb_str    = fmt(gpt2_weight_gb, precision=0, commas=False)
    gpt2_decode_gflops_str = fmt(gpt2_decode_gflops, precision=0, commas=False)
...
class FpSmallBatchCalc:
    ...
    fp_t4_ridge_str = fmt_int(round(t4_ridge), commas=False)
...
class CarbonRoiCalc:
    ...
    npu_eff_str        = fmt(npu_eff, precision=0)
