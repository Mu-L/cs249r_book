...
The serial bottleneck becomes concrete on real hardware, where the same accelerator that nears its parallel ceiling on one workload can stall on another.
...
Every computing paradigm follows a distinct lifecycle of three phases: ferment (initial slow progress), take-off (exponential growth), and saturation (diminishing returns at physical limits).
...
class InterconnectBandwidth:
    ...
    hbm_bw_str = fmt(h_h100.memory.bandwidth.m_as(TB/second), precision=1, commas=False, suffix=" TB/s")
...
class AcceleratorEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # V100 specs
    v100_tflops_str = fmt(h_v100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    v100_bw_str     = fmt(h_v100.memory.bandwidth.m_as(GB/second), precision=0, commas=False, suffix=" GB/s")
    ...
    a100_tflops_fp16_str = fmt(h_a100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    a100_bw_str          = fmt(h_a100.memory.bandwidth.m_as(GB/second), precision=0, suffix=" GB/s")
    a100_bw_tbs_str      = fmt(h_a100.memory.bandwidth.m_as(TB/second), precision=2, commas=False, suffix=" TB/s")
    ...
    h100_bw_str          = fmt(h_h100.memory.bandwidth.m_as(GB/second), precision=0, suffix=" GB/s")
    h100_bw_tbs_str      = fmt(h_h100.memory.bandwidth.m_as(TB/second), precision=2, commas=False, suffix=" TB/s")
    ...
    # TPUv4 specs
    tpuv4_tflops_str = fmt(h_tpu.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    tpuv4_bw_str     = fmt(h_tpu.memory.bandwidth.m_as(GB/second), precision=0, suffix=" GB/s")
