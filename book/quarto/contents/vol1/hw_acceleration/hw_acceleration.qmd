...
class AcceleratorEfficiencyAnchor:
    ...
    energy_dividend_str = MarkdownStr(f"{energy_dividend}x")
...
class InterconnectBandwidth:
    ...
    nvlink_bw_str = MarkdownStr(f"{int(nvlink_bw)} GB/s")
    pcie_bw_str = MarkdownStr(f"{int(pcie_bw)} GB/s")
    net_bw_str = MarkdownStr(f"{int(net_bw)} GB/s")
    gap_ratio_str = MarkdownStr(f"{int(hbm_bw/pcie_bw)}x")
...
class AcceleratorEconomics:
    ...
    v100_price_str = MarkdownStr(f"~${price_v100:,.0f}")
    v100_pp_str    = MarkdownStr(f"${v100_ratio:.0f}/TFLOP")
    ...
    a100_price_str   = MarkdownStr(f"~${price_a100:,.0f}")
    a100_pp_str      = MarkdownStr(f"${a100_ratio:.0f}/TFLOP")
    ...
    h100_price_str   = MarkdownStr(f"~${price_h100:,}-30,000")
    h100_pp_str      = MarkdownStr(f"~${h100_ratio:.0f}/TFLOP")
    ...
    tpu_price_str = MarkdownStr(f"~${price_tpu:,}*")
    tpu_pp_str    = MarkdownStr(f"~${tpu_ratio:.0f}/TFLOP")

    # Gaudi 2 specs
    gaudi_tflops_str = fmt(gaudi_tf, precision=0, commas=False)
    gaudi_bw_str     = fmt(gaudi_bw_value, precision=0)
    gaudi_price_str = MarkdownStr(f"~${price_gaudi:,}")
    gaudi_pp_str    = MarkdownStr(f"${gaudi_ratio:.0f}/TFLOP")
```
...
class FusionBenefitsCalc:
    ...
    naive_mb_str = MarkdownStr(f"{naive_mb:.1f} MB")
    fused_mb_str = MarkdownStr(f"{fused_mb:.1f} MB")
```
