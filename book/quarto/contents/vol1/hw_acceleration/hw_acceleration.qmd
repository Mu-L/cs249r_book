...
**Context**: Google initially treated special-purpose neural-network hardware as unnecessary because conventional data centers had enough spare capacity for the early workload [@jouppi2017].

**Failure**: In 2013, projections showed that if users performed voice search for only a few minutes per day with speech-recognition DNNs, Google's compute demand would roughly double under a conventional CPU deployment.

**Consequence**: Google started a high-priority custom-ASIC effort and deployed the first TPU for inference, optimizing for latency, cost, and performance per watt rather than general-purpose flexibility.

**Systems lesson**: Hardware acceleration becomes mandatory when a workload crosses a fleet-level economic threshold. The decision is not "GPU versus TPU" in the abstract; it is whether the workload's shape makes general-purpose capacity unaffordable.
