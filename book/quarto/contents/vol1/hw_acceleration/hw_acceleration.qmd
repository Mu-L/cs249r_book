...
**Systems conclusion**: A systolic array is **`{python} SystolicEnergy.energy_ratio_str`$\times$ more energy-efficient** than a naive vector unit for large matrix multiplications.
...
**Systems conclusion**: The `{python} InterconnectBandwidth.gap_ratio_str` gap between HBM and PCIe means that any data transfer from the CPU is a catastrophic performance event. **NVLink** is the only way to scale computation across multiple GPUs without hitting the "PCIe Wall."
...
**Systems conclusion**: Data *cannot* be fetched from DRAM in a single cycle. It is physically impossible. Local registers and SRAM (L1) are required to feed compute units at 2 GHz. The "memory wall" is partially a **distance wall**.
...
**Context**: Engineers at Tencent deployed a massive transformer model on NVIDIA A100 GPUs, expecting a 10$\times$ speedup over their old V100s due to the new Tensor Cores.

**Failure**: The model ran only 1.2$\times$ faster. Profiling revealed the Tensor Cores were active 0 percent of the time. The team had implemented their custom accumulation kernel in FP32 (32-bit float) to maintain precision.
