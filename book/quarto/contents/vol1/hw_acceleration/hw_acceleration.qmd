...
fig, ax, COLORS, plt = viz.setup_plot(figsize=(10, 4.2))
...
fig, ax, COLORS, plt = viz.setup_plot(figsize=(10, 4.1))
...
class WeightMatrixCalc:
    ...
    wm_batch_macs_m_str = fmt(wm_batch_macs_value / MILLION, precision=1, commas=False)
...
class AttentionHeadTensorCoreCalc:
    ...
    macs_m_str = fmt(macs / MILLION, precision=1, commas=False)
    flops_m_str = fmt(flops / MILLION, precision=1, commas=False)
...
# │          InterconnectHierarchy.ib_ndr_str, InterconnectHierarchy.taper_eq_math
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, MarkdownStr
...
class InterconnectHierarchy:
    ...
    taper_eq_math = MarkdownStr(
        rf"$$\begin{{aligned}} \text{{HBM ({taper_hbm_gbs:.0f} GB/s)}} &\gg \text{{NVLink ({taper_nvlink_gbs:.0f} GB/s)}} \\ &\gg \text{{PCIe ({taper_pcie_gbs:.0f} GB/s)}} \gg \text{{Network ({taper_net_gbs:.0f} GB/s)}} \end{{aligned}}$$")
```
...
class FpMultigpuScalingCalc:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    sync_time_ms     = gradient_size_gb/nvlink_bw_gbs * THOUSAND
...
class FeasibilityAssessment:
    ...
    token_latency_ms = (model_70b_gb/mem_bw_gb_s) * THOUSAND
    ...
    mem_bw_tb_s_str = fmt(mem_bw_gb_s / THOUSAND, precision=0, commas=False, suffix=" TB/s")
