...
# │ Exports: InterconnectBandwidth.hbm_bw_str,
# │          InterconnectBandwidth.nvlink_bw_str,
# │          InterconnectBandwidth.pcie_bw_str,
# │          InterconnectBandwidth.net_bw_str,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, fmt_int
...
class TransformerLayerCalc:
    ...
    a100_ridge_str      = fmt_int(round(a100_ridge_value), commas=False)
```
...
- AI = `{python} TransformerLayerCalc.qkv_flops_b_str` billion FLOPs / `{python} TransformerLayerCalc.qkv_mb_str` MB = `{python} TransformerLayerCalc.qkv_ai_str` FLOP/byte, which is **compute bound on A100** (above `{python} TransformerLayerCalc.a100_ridge_str` threshold)
...
# │          Conv2dAnalysisCalc.a100_tflops_fp16_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, fmt_int
...
This is **well above** A100's ridge point of `{python} Conv2dAnalysisCalc.a100_ridge_str` FLOP/byte, making this operation compute-bound. The layer will achieve near-peak performance of ~`{python} Conv2dAnalysisCalc.a100_tflops_fp16_str` TFLOP/s (FP16 with Tensor Cores).
...
This is **below** A100's ridge point of `{python} DenseLayerAnalysisCalc.a100_ridge_str` FLOP/byte, making this operation memory-bound. Attainable performance:
...
# │          Gpt2ThroughputCalc.a100_tflops_fp32_str,
...
class Gpt2ThroughputCalc:
    ...
    a100_ridge_value     = Hardware.Cloud.A100.ridge_point().m_as('flop/byte')
    a100_ridge_fp32_value = (Hardware.Cloud.A100.peak_flops_fp32 / Hardware.Cloud.A100.memory_bw).m_as('flop/byte')
    ...
    a100_tflops_fp32_str  = fmt(a100_tflops_fp32_val, precision=1, commas=False)
    a100_ridge_str        = fmt_int(round(a100_ridge_value), commas=False)
    a100_ridge_fp32_str   = fmt_int(round(a100_ridge_fp32_value), commas=False)
```
...
*   **Ridge point $(R_{\text{peak}}/\text{BW})$**: `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` / `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` = **`{python} Gpt2ThroughputCalc.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
    *   **Meaning**: Saturating this chip at FP16 precision requires `{python} Gpt2ThroughputCalc.a100_ridge_str` operations for every byte loaded. The ridge point varies by precision: FP32 operations (`{python} Gpt2ThroughputCalc.a100_tflops_fp32_str` TFLOP/s peak) have a ridge point of only ~`{python} Gpt2ThroughputCalc.a100_ridge_fp32_str` FLOP/byte.
