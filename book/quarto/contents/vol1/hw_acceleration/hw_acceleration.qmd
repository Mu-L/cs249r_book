...
*   **V100 (2017)**: `{python} AcceleratorEconomics.v100_tflops` TF/0.9 TB/s ≈ **`{python} RooflineGap.v100_ridge_str` FLOP/byte**.
*   **A100 (2020)**: `{python} AcceleratorEconomics.a100_tflops_fp16` TF / `{python} AcceleratorEconomics.a100_bw_tbs` TB/s ≈ **`{python} RooflineGap.a100_ridge_str` FLOP/byte**.
*   **H100 (2023)**: `{python} AcceleratorEconomics.h100_tflops_fp16` TF / `{python} AcceleratorEconomics.h100_bw_tbs` TB/s ≈ **`{python} RooflineGap.h100_ridge_str` FLOP/byte**.
...
*   **Peak compute**: `{python} TensorLifecycleCalc.a100_tflops_fp16` TFLOPS (FP16 Tensor Core).
*   **Peak bandwidth**: `{python} AcceleratorEconomics.a100_bw_tbs` TB/s (HBM2e).
*   **Ridge point $(R_{\text{peak}}/\text{BW})$**: `{python} TensorLifecycleCalc.a100_tflops_fp16` / `{python} AcceleratorEconomics.a100_bw_tbs` = **`{python} RooflineGap.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
...
*   **Maximum throughput**: `{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte $\times$ `{python} AcceleratorEconomics.a100_bw_tbs` TB/s = **`{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOPS**.
*   **Utilization ceiling**:
    `{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOPS (Actual) / `{python} TensorLifecycleCalc.a100_tflops_fp16` TFLOPS (Peak) ≈ **`{python} Gpt2ThroughputCalc.gpt2_utilization_str` percent**
