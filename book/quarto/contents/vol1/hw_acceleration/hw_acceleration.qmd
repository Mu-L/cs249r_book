...
$$ \text{Speedup} = \frac{1}{(1 - p) + \frac{p}{S}} $$ {#eq-amdahl}

*   **$p$ (parallel fraction)**: The matrix multiplications (typically 90–99 percent of an ML workload).
*   **$S$ (speedup)**: The raw speed advantage of the GPU or Tensor Processing Unit (TPU) over the CPU (typically 100--1,000$\times$).
...
**Pitfall**: If data loading takes 10 percent of the time ($p=0.9$), even an **infinite speed** accelerator ($S=\infty$) can only achieve a **10$\times$** total speedup. The serial component dominates the parallel accelerator component once the latter is sufficiently fast.
...
::: {#fig-iron-law-heatmap fig-env="figure" fig-pos="htb" fig-cap="**The Iron Law Heatmap**: Total system speedup as a function of accelerator speed ($S$) and parallel fraction ($p$). High speedup appears only near the top-right corner, where both accelerator speed and parallel fraction are high. The acceleration wall is the low-$p$ region: if a workload is even slightly serial ($p < 0.9$), increasing hardware speed yields little benefit. Contours span roughly 1×–500× speedup." fig-alt="Heatmap of speedup vs. accelerator speed and parallel fraction. High speedup in green and yellow appears near the top-right corner where parallel fraction is near 1.0 and accelerator speed is high. Lower-parallel-fraction regions remain blue, showing the serial bottleneck."}
...
**The context**: Google initially treated special-purpose neural-network hardware as unnecessary because conventional data centers had enough spare capacity for the early workload.

**The failure**: In 2013, projections showed that if users performed voice search for only a few minutes per day with speech-recognition DNNs, Google's compute demand would roughly double under a conventional CPU deployment.

**The consequence**: Google started a high-priority custom-ASIC effort and deployed the first TPU for inference, optimizing for latency, cost, and performance per watt rather than general-purpose flexibility.

**The systems lesson**: Hardware acceleration becomes mandatory when a workload crosses a fleet-level economic threshold. The decision is not "GPU versus TPU" in the abstract; it is whether the workload's shape makes general-purpose capacity unaffordable [@jouppi2017].
...
- H100 delivers $S$ = `{python} AmdahlH100.hw_speedup_str`$\times$ speedup over CPU for matrix multiply (`{python} AmdahlH100.h100_tflops_int8_str` TOPS INT8 vs. ~8 TOPS on baseline CPU without AMX extensions)
...
|:-----------------------------------|:------------------------------------------------|-------------------------------------------------------------------:|--------------------------------------------------:|:---------------------------------------------|
| **NVIDIA V100**\index{NVIDIA!V100} | `{python} AcceleratorEconomics.v100_price_str`  |             `{python} AcceleratorEconomics.v100_tflops_str` TFLOPS |  `{python} AcceleratorEconomics.v100_bw_str` GB/s | `{python} AcceleratorEconomics.v100_pp_str`  |
| **NVIDIA A100**\index{NVIDIA!A100} | `{python} AcceleratorEconomics.a100_price_str`  |        `{python} AcceleratorEconomics.a100_tflops_fp16_str` TFLOPS |  `{python} AcceleratorEconomics.a100_bw_str` GB/s | `{python} AcceleratorEconomics.a100_pp_str`  |
| **NVIDIA H100**                    | `{python} AcceleratorEconomics.h100_price_str`  | `{python} AcceleratorEconomics.h100_tflops_tf32_str` TFLOPS (TF32) |  `{python} AcceleratorEconomics.h100_bw_str` GB/s | `{python} AcceleratorEconomics.h100_pp_str`  |
| **Google TPUv4**                   | `{python} AcceleratorEconomics.tpu_price_str`   |     `{python} AcceleratorEconomics.tpuv4_tflops_str` TFLOPS (BF16) | `{python} AcceleratorEconomics.tpuv4_bw_str` GB/s | `{python} AcceleratorEconomics.tpu_pp_str`   |
| **Intel Gaudi 2**                  | `{python} AcceleratorEconomics.gaudi_price_str` |      `{python} AcceleratorEconomics.gaudi_tflops_str` TFLOPS (FP8) | `{python} AcceleratorEconomics.gaudi_bw_str` GB/s | `{python} AcceleratorEconomics.gaudi_pp_str` |
...
### Hardware balance ($B_{\text{hw}}$): The paradigm partition {#sec-hardware-acceleration-hardware-balance}

Different paradigms inhabit different regions of this "memory wall." We quantify this using the **hardware balance $(B_{\text{hw}})$\index{Hardware Balance}**, defined as the number of operations required to hide the cost of fetching one byte of data:
$$ B_{\text{hw}} = \frac{R_{\text{peak}}}{\text{BW}} $$
...
4.  **Registers**: The Tensor Core operates here.
    ...
    *   **Throughput**: `{python} TensorLifecycleCalc.a100_tflops_fp16_str` TFLOPS.
...
**The systems lesson**: Hardware features are brittle contracts. If the workload does not present supported data types and tile shapes, the accelerator falls back to generic execution. Hardware cannot be exploited without conforming to its contracts [@NVIDIA2020].
...
**Metric**: The Ridge Point $R$, defined as $R = \text{Peak FLOPS} / \text{Peak Bandwidth}$ (FLOP/byte). This number specifies how many math operations the hardware must perform for every byte of data loaded to keep the compute units busy.
...
This is **well above** A100's ridge point of `{python} RooflineGap.a100_ridge_str` FLOP/byte, making this operation compute-bound. The layer will achieve near-peak performance of ~`{python} TensorLifecycleCalc.a100_tflops_fp16_str` TFLOPS (FP16 with Tensor Cores).
...
$T_{\text{attainable}}$ = `{python} DenseLayerAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_str` TFLOPS
...
$T_{\text{attainable}}$ = `{python} LayernormAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte = `{python} LayernormAnalysisCalc.ln_attainable_str` TFLOPS
...
*   **Ridge point $(R_{\text{peak}}/\text{BW})$**: `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` / `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` = **`{python} RooflineGap.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
    *   **Meaning**: Saturating this chip at FP16 precision requires `{python} RooflineGap.a100_ridge_str` operations for every byte loaded. The ridge point varies by precision: FP32 operations (`{python} Gpt2ThroughputCalc.a100_tflops_fp32_str` TFLOPS peak) have a ridge point of only ~`{python} RooflineGap.a100_ridge_fp32_str` FLOP/byte.
...
*   **Maximum throughput**: `{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte $\times$ `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` TB/s = **`{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOPS**.
*   **Utilization ceiling**:
    `{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOPS (Actual) / `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` TFLOPS (Peak) ≈ **`{python} Gpt2ThroughputCalc.gpt2_utilization_str` percent**
...
(d!)^l
$$
where $l$ is the number of memory hierarchy levels. This rapid expansion shows why execution order optimization matters: poor loop ordering can lead to excessive memory traffic, while an optimized order improves cache utilization [@sze2017].
...
Consider a convolution applying 16 filters of size $3{\times}3$ to an $8{\times}8$ single-channel input. The computation can be expressed as five nested loops iterating over output rows ($H_{\text{out}}$), output columns ($W_{\text{out}}$), filter count ($K$), filter height ($R$), and filter width ($S$). The $5! = 120$ possible orderings of these loops all produce the same numerical result, but they generate dramatically different memory traffic.

**Ordering A (weight-stationary)**: Place the filter loops ($K$, $R$, $S$) outermost and the spatial loops ($H_{\text{out}}$, $W_{\text{out}}$) innermost. Each $3{\times}3$ filter is loaded into registers once and then applied across all 36 output positions before the next filter is loaded. Total weight loads: $16 \times 9 = 144$ values, each loaded exactly once.
...
class FpMemoryEnergyCalc:
    ...
    layernorm_tflops = layernorm_ai * peak_bw_tbs   # AI [FLOP/byte] × BW [TB/s] = TFLOPS
...
class FpSmallBatchCalc:
    ...
    a100_bw_tbs  = A100_MEM_BW.m_as(TB/second)
    t4_ridge     = t4_flops / t4_bw_tbs                  # TFLOPS / (TB/s) = FLOP/byte
    a100_roof_b1 = a100_bw_tbs * ai_b1                   # (TB/s) × (FLOP/byte) = TFLOPS
...
Teams deploy high-throughput training accelerators (A100, H100) for latency-sensitive inference with batch size 1–4. As the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff) predicts, small batches severely reduce arithmetic intensity: a dense layer with M=N=2048 achieves AI = `{python} FpSmallBatchCalc.fp_ai_b1_str` FLOP/byte at batch=1 vs. AI = `{python} FpSmallBatchCalc.fp_ai_b256_str` FLOP/byte at batch=256. At batch=1, the memory-bound roofline ceiling is only about `{python} FpSmallBatchCalc.fp_a100_roof_b1_str` TFLOPS on A100 and `{python} FpSmallBatchCalc.fp_t4_roof_b1_str` TFLOPS on T4. The T4's peak is `{python} FpSmallBatchCalc.fp_t4_flops_str` TFLOPS (FP16 Tensor Core) with a ridge point of `{python} FpSmallBatchCalc.fp_t4_ridge_str` FLOP/byte (`{python} FpSmallBatchCalc.fp_t4_flops_str` TFLOPS / `{python} FpSmallBatchCalc.fp_t4_bw_str` GB/s). Small-batch inference remains memory bound on both accelerators, so the T4's lower cost can make it more economical despite its much lower peak compute. Training-class accelerator instances often rent for several times more than inference-class instances for little latency gain in this regime. Inference deployments should match batch size to accelerator characteristics, using high-compute accelerators only for batched serving where arithmetic intensity exceeds ridge points.
...
*   **Formula**: $T_{\text{token}} = \frac{\text{Model Size}}{\text{Memory Bandwidth}}$
*   **Constraint**: $T_{\text{token}} < \text{Latency Target}$
*   **Example**: Serving a 70B model (140 GB) on a GPU with 1 TB/s bandwidth yields ≈ `{python} FeasibilityAssessment.token_latency_ms_str` ms per token. If a workload requires 50 ms latency, this hardware fails regardless of compute power.

**Compute check**

*   **Formula**: $T_{\text{process}} = \frac{\text{Ops}}{\text{Peak FLOPS} \times \text{Utilization}}$
...
*   **CPU inference**: `{python} CarbonRoiCalc.cpu_power_str` Watts for `{python} CarbonRoiCalc.cpu_tflops_str` TFLOP (Efficiency = `{python} CarbonRoiCalc.cpu_eff_str` TFLOPS/W).
*   **NPU inference**: `{python} CarbonRoiCalc.npu_power_str` Watts for `{python} CarbonRoiCalc.npu_tflops_str` TFLOPS (Efficiency = `{python} CarbonRoiCalc.npu_eff_str` TFLOPS/W).
