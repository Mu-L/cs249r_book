...
::: {#def-hw-acceleration-hardware-acceleration .callout-definition title="Hardware acceleration"}
...
::: {#def-hw-acceleration-hardware-software-co-design .callout-definition title="Hardware-software co-design"}
...
::: {#thm-hw-acceleration-fundamental-limit-acceleration-amdahls-law .callout-theorem title="The fundamental limit of acceleration (Amdahl's Law)"}
...
# │          AmdahlsH100.hw_speedup_str, AmdahlsH100.h100_tflops_int8
# └─────────────────────────────────────────────────────────────────────────────
...
- H100 delivers S = `{python} AmdahlH100.hw_speedup_str`$\times$ speedup over CPU for matrix multiply (`{python} AmdahlH100.h100_tflops_int8` TOPS INT8 vs. ~8 TOPS on baseline CPU without AMX extensions)
...
::: {#exm-hw-acceleration-tpuv1-vs-k80-efficiency-shock .callout-example title="The TPUv1 vs. K80 efficiency shock"}
...
::: {#def-hw-acceleration-ml-accelerator .callout-definition title="ML accelerator"}
...
class SystolicOpsCalc:
    ...
    tpuv4_bw_str     = f"{int(tpuv4_bw_gbs):,}"
```
...
class InterconnectBandwidth:
    ...
    hbm_bw_str = f"{h_h100.memory_bw.m_as(TB/second):.1f} TB/s"
    nvlink_bw_str = f"{int(nvlink_bw)} GB/s"
    pcie_bw_str = f"{int(pcie_bw)} GB/s"
    net_bw_str = f"{int(net_bw)} GB/s"
    gap_ratio_str = f"{int(hbm_bw/pcie_bw)}x"
...
# │ Exports: AcceleratorEconomics.v100_tflops, AcceleratorEconomics.v100_bw,
# │          AcceleratorEconomics.v100_price_str, AcceleratorEconomics.v100_pp_str,
# │          AcceleratorEconomics.a100_tflops_fp16, AcceleratorEconomics.a100_bw,
# │          AcceleratorEconomics.a100_bw_tbs, AcceleratorEconomics.a100_price_str,
# │          AcceleratorEconomics.a100_pp_str, AcceleratorEconomics.h100_tflops_tf32,
# │          AcceleratorEconomics.h100_tflops_fp16, AcceleratorEconomics.h100_bw,
# │          AcceleratorEconomics.h100_bw_tbs, AcceleratorEconomics.h100_price_str,
# │          AcceleratorEconomics.h100_pp_str, AcceleratorEconomics.h100_tflops_fp8,
# │          AcceleratorEconomics.tpuv4_tflops,
# │          AcceleratorEconomics.tpuv4_bw, AcceleratorEconomics.tpu_price_str,
# │          AcceleratorEconomics.tpu_pp_str, AcceleratorEconomics.gaudi_tflops,
# │          AcceleratorEconomics.gaudi_bw, AcceleratorEconomics.gaudi_price_str,
...
class AcceleratorEconomics:
    ...
    v100_price_str = f"~${price_v100:,.0f}"
    ...
    a100_bw_tbs      = f"{h_a100.memory_bw.m_as(TB/second):.2f}"
    a100_price_str   = f"~${price_a100:,.0f}"
    ...
    h100_tflops_fp8  = f"{H100_FLOPS_FP8_TENSOR.m_as(TFLOPs/second):,.0f}"
    ...
    # Gaudi 2 specs
    gaudi_tflops    = f"{gaudi_tf:.0f}"
    gaudi_bw        = f"{gaudi_bw_value:,.0f}"
...
|:-----------------------------------|:------------------------------------------------|---------------------------------------------------------------:|----------------------------------------------:|:---------------------------------------------|
| **NVIDIA V100**\index{NVIDIA!V100} | `{python} AcceleratorEconomics.v100_price_str`  |             `{python} AcceleratorEconomics.v100_tflops` TFLOPS |  `{python} AcceleratorEconomics.v100_bw` GB/s | `{python} AcceleratorEconomics.v100_pp_str`  |
| **NVIDIA A100**\index{NVIDIA!A100} | `{python} AcceleratorEconomics.a100_price_str`  |        `{python} AcceleratorEconomics.a100_tflops_fp16` TFLOPS |  `{python} AcceleratorEconomics.a100_bw` GB/s | `{python} AcceleratorEconomics.a100_pp_str`  |
| **NVIDIA H100**                    | `{python} AcceleratorEconomics.h100_price_str`  | `{python} AcceleratorEconomics.h100_tflops_tf32` TFLOPS (TF32) |  `{python} AcceleratorEconomics.h100_bw` GB/s | `{python} AcceleratorEconomics.h100_pp_str`  |
| **Google TPUv4**                   | `{python} AcceleratorEconomics.tpu_price_str`   |     `{python} AcceleratorEconomics.tpuv4_tflops` TFLOPS (BF16) | `{python} AcceleratorEconomics.tpuv4_bw` GB/s | `{python} AcceleratorEconomics.tpu_pp_str`   |
| **Intel Gaudi 2**                  | `{python} AcceleratorEconomics.gaudi_price_str` |      `{python} AcceleratorEconomics.gaudi_tflops` TFLOPS (FP8) | `{python} AcceleratorEconomics.gaudi_bw` GB/s | `{python} AcceleratorEconomics.gaudi_pp_str` |
...
::: {#def-hw-acceleration-ai-memory-wall .callout-definition title="AI memory wall"}
...
# │          AcceleratorEconomics.a100_tflops_fp16,
...
4.  **Registers**: The Tensor Core operates here.
    ...
    *   **Throughput**: `{python} TensorLifecycleCalc.a100_tflops_fp16` TFLOPS.
...
::: {#def-hw-acceleration-arithmetic-intensity .callout-definition title="Arithmetic intensity"}
...
This is **well above** A100's ridge point of `{python} RooflineGap.a100_ridge_str` FLOP/byte, making this operation compute-bound. The layer will achieve near-peak performance of ~`{python} TensorLifecycleCalc.a100_tflops_fp16` TFLOPS (FP16 with Tensor Cores).
...
$T_{\text{attainable}}$ = `{python} DenseLayerAnalysisCalc.a100_bw` GB/s $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_str` TFLOPS
...
*   **Ridge point $(R_{\text{peak}}/\text{BW})$**: `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` / `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` = **`{python} RooflineGap.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
    *   *Meaning:* Saturating this chip at FP16 precision requires `{python} RooflineGap.a100_ridge_str` operations for every byte loaded. The ridge point varies by precision: FP32 operations (`{python} Gpt2ThroughputCalc.a100_tflops_fp32` TFLOPS peak) have a ridge point of only ~`{python} RooflineGap.a100_ridge_fp32_str` FLOP/byte.
...
::: {#def-hw-acceleration-mapping-ai-acceleration .callout-definition title="Mapping in AI acceleration"}
...
::: {#exm-hw-acceleration-loop-ordering-small-convolution .callout-example title="Loop ordering in a small convolution"}
...
class FusionBenefitsCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    naive_mb_str = f"{naive_mb:.1f} MB"
    fused_mb_str = f"{fused_mb:.1f} MB"
```
...
# │ Exports: RuntimeProductionTdp.a100_tdp
# └─────────────────────────────────────────────────────────────────────────────
...
Sustained workloads may trigger thermal throttling that was not observed in short benchmarks. The A100 SXM operates at `{python} RuntimeProductionTdp.a100_tdp` W TDP while the A100 PCIe operates at 300 W TDP; these represent different form factors with different cooling requirements, not boost vs. sustained states. Production performance depends on which variant is deployed and whether thermal limits are approached.
