...
class HwAccelSetup:
    ...
    cpu_gap_min_str      = fmt(cpu_gap_min, precision=1, commas=False)
    cpu_gap_max_str      = fmt(cpu_gap_max, precision=1, commas=False)
```
...
class AmdahlH100:
    ...
    h100_tops_int8_str = fmt(h100_tops_int8, precision=1)
    hw_speedup_str = fmt(hw_speedup_factor, precision=1, commas=False)
    ...
    serial_resnet_pct_str = fmt_percent((1-p_resnet), precision=1, commas=False)
    ...
    amdahl_resnet_round_str = fmt(speedup_resnet, precision=1, commas=False)

    # GPT-2
    p_gpt2_str = fmt(p_gpt2, precision=2, commas=False)
    serial_gpt2_str = fmt(1-p_gpt2, precision=2, commas=False)
    serial_gpt2_pct_str = fmt_percent((1-p_gpt2), precision=1, commas=False)
    ...
    amdahl_gpt2_ceil_str = fmt(ceiling_gpt2, precision=1, commas=False)
```
...
class SystemsGapRates:
    ...
    gpu_growth_per_year_str = fmt(gpu_growth_per_year, precision=0, commas=False)
...
class CpuMlInefficiency:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cpu_utilization_min_str = fmt(cpu_utilization_min_value, precision=1, commas=False)
    cpu_utilization_max_str = fmt(cpu_utilization_max_value, precision=1, commas=False)
    cpu_gflops_str = fmt(cpu_gflops_value, precision=1, commas=False)
...
class WeightMatrixCalc:
    ...
    # OUTPUT
    wm_params_str = fmt(wm_params_value, precision=1, commas=True, drop_zero=True)
    wm_out_str = fmt(wm_out_value, precision=1, commas=False)
    wm_in_str = fmt(wm_in_value, precision=1, commas=False)
    wm_batch_str = fmt(wm_batch_value, precision=1, commas=False)
    wm_batch_macs_m_str = fmt(wm_batch_macs_value / 1e6, precision=1, commas=False, drop_zero=True)
    wm_vector_chunks_str = fmt(wm_vector_chunks_value, precision=1, commas=True)
```
...
class AttentionHeadTensorCoreCalc:
    ...
    macs_m_str = fmt(macs / 1e6, precision=1, commas=False)
    flops_m_str = fmt(flops / 1e6, precision=1, commas=False)
    scalar_ms_str = fmt(scalar_ms, precision=1, commas=False)
    tensor_speedup_str = fmt(tensor_speedup, precision=1, commas=False)
```
...
class AcceleratorEfficiencyAnchor:
    ...
    energy_dividend_str = fmt(energy_dividend, precision=1, commas=False, suffix="x")
...
class SystolicEnergy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dram_access_str = fmt(dram_pj, precision=1, commas=False)
    systolic_size_str = fmt(array_dim, precision=1, commas=False)
    vector_accesses_str = fmt(vector_dram_accesses, precision=1, commas=False)
    compute_energy_str = fmt(mac_pj, precision=1, commas=False)
    vector_energy_str = fmt(e_vector, precision=1)
    ...
    energy_ratio_str = fmt(efficiency_ratio, precision=1, commas=False)
```
...
class TilingPrinciple:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    layer_dim_str = fmt(layer_dim, precision=1)
    array_dim_str = fmt(array_dim, precision=1, commas=False)
    tile_count_str = fmt(int(total_tiles), precision=0)
    reuse_factor_str = fmt(reuse_factor, precision=1, commas=False)
```
...
class SystolicOpsCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    systolic_dim_str = fmt(systolic_dim_value, precision=1, commas=False)
    systolic_ops_str = fmt(systolic_ops_value, precision=0)
    tpuv4_bw_str     = fmt(int(tpuv4_bw_gbs), precision=1)
```
...
class InterconnectBandwidth:
    ...
    nvlink_bw_str = fmt(int(nvlink_bw), precision=1, commas=False, suffix=" GB/s")
    pcie_bw_str = fmt(int(pcie_bw), precision=1, commas=False, suffix=" GB/s")
    net_bw_str = fmt(int(net_bw), precision=1, commas=False, suffix=" GB/s")
    gap_ratio_str = fmt(int(hbm_bw/pcie_bw), precision=1, commas=False, suffix="x")
...
class AcceleratorEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # V100 specs
    v100_tflops_str = fmt(h_v100.peak_flops.m_as(TFLOPs/second), precision=1, commas=False)
    v100_bw_str     = fmt(h_v100.memory_bw.m_as(GB/second), precision=1, commas=False)
    # Currency prefix → MarkdownStr per recipe Rule 1 escape-hatch
    v100_price_str = fmt(price_v100, precision=1, prefix="~$")
    v100_pp_str    = fmt(v100_ratio, precision=1, commas=False, prefix="$", suffix="/(TFLOP/s)")

    # A100 specs (also used later in roofline sections)
    a100_tflops_fp16_str = fmt(h_a100.peak_flops.m_as(TFLOPs/second), precision=1, commas=False)
    a100_bw_str          = fmt(h_a100.memory_bw.m_as(GB/second), precision=1)
    a100_bw_tbs_str      = fmt(h_a100.memory_bw.m_as(TB/second), precision=2, commas=False)
    a100_price_str   = fmt(price_a100, precision=1, prefix="~$")
    a100_pp_str      = fmt(a100_ratio, precision=1, commas=False, prefix="$", suffix="/(TFLOP/s)")

    # H100 specs
    h100_tflops_tf32_str = fmt(h_h100.compute.precision_flops["tf32"].m_as(TFLOPs/second), precision=1, commas=False)
    h100_tflops_fp16_str = fmt(h_h100.peak_flops.m_as(TFLOPs/second), precision=1, commas=False)
    h100_tflops_fp8_str  = fmt(Hardware.Cloud.H100.compute.precision_flops["fp8"].m_as(TFLOPs/second), precision=1)
    h100_bw_str          = fmt(h_h100.memory_bw.m_as(GB/second), precision=1)
    h100_bw_tbs_str      = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False)
    h100_price_str   = fmt(price_h100, precision=1, prefix="~$", suffix="-30,000")
    h100_pp_str      = fmt(h100_ratio, precision=1, commas=False, prefix="~$", suffix="/(TFLOP/s)")

    # TPUv4 specs
    tpuv4_tflops_str = fmt(h_tpu.peak_flops.m_as(TFLOPs/second), precision=1, commas=False)
    tpuv4_bw_str     = fmt(h_tpu.memory_bw.m_as(GB/second), precision=1)
    tpu_price_str = fmt(price_tpu, precision=1, prefix="~$", suffix="*")
    tpu_pp_str    = fmt(tpu_ratio, precision=1, commas=False, prefix="~$", suffix="/(TFLOP/s)")

    # Gaudi 2 specs
    gaudi_tflops_str = fmt(gaudi_tf, precision=1, commas=False)
    gaudi_bw_str     = fmt(gaudi_bw_value, precision=1)
    gaudi_price_str = fmt(price_gaudi, precision=1, prefix="~$")
    gaudi_pp_str    = fmt(gaudi_ratio, precision=1, commas=False, prefix="$", suffix="/(TFLOP/s)")
```
...
class B200FP4Specs:
    ...
    dense_pflops_str  = fmt(dense_pflops, precision=1, commas=False)
    sparse_pflops_str = fmt(sparse_pflops, precision=1, commas=False)
```
...
class TensorLifecycleCalc:
    ...
    dram_energy_pj_bit_str = fmt(dram_energy_pj_bit_value, precision=1, commas=False)
    latency_hbm_str        = fmt(LATENCY_HBM3.m_as('ns'), precision=1, commas=False)
    latency_l2_str         = fmt(LATENCY_L2_CACHE.m_as('ns'), precision=1, commas=False)
    latency_l1_str         = fmt(LATENCY_L1_REGISTER.m_as('ns'), precision=1, commas=False)
    a100_tflops_fp16_str   = fmt(a100_fp16_tflops_value, precision=1, commas=False)
...
class InterconnectHierarchy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_str = fmt(nvlink_a100, precision=1, commas=False)
    nvlink_h100_str = fmt(nvlink_h100, precision=1, commas=False)
    ib_hdr_str = fmt(ib_hdr_gbps, precision=1, commas=False)
    ib_ndr_str = fmt(ib_ndr_gbps, precision=1, commas=False)
```
...
class RooflineGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_tflops_str = fmt(v100_tflops_value, precision=1, commas=False)
    v100_bw_tbs_str = fmt(v100_bw_tbs_value, precision=1, commas=False)
    a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=1, commas=False)
    a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=2, commas=False)
    h100_tflops_fp16_str = fmt(h100_tflops_fp16_value, precision=1, commas=False)
    h100_bw_tbs_str = fmt(h100_bw_tbs_value, precision=2, commas=False)

    v100_ridge_str = fmt(v100_ridge, precision=1, commas=False)
    a100_ridge_str = fmt(a100_ridge, precision=1, commas=False)
    h100_ridge_str = fmt(h100_ridge, precision=1, commas=False)
    a100_ridge_fp32_str = fmt(a100_ridge_fp32, precision=1, commas=False)

    legacy_ai_str = fmt(legacy_ai, precision=1, commas=False)
    relu_ai_str = fmt(relu_ai, precision=3, commas=False)
    dense_matmul_dim_str = fmt(dense_matmul_dim, precision=1, commas=False)
    ...
    relu_below_roofline_str = fmt(relu_gap, precision=1, commas=True)
```
...
class TransformerLayerCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    t_hidden_str        = fmt(t_hidden_value, precision=1, commas=False)
    t_batch_str         = fmt(t_batch_value, precision=1, commas=False)
    t_seq_str           = fmt(t_seq_value, precision=1, commas=False)
    t_heads_str         = fmt(t_heads_value, precision=0, commas=False)
    qkv_flops_b_str     = fmt(qkv_flops_b_value, precision=1, commas=False)
    qkv_mb_str          = fmt(qkv_mb_value, precision=1, commas=False)
    qkv_ai_str          = fmt(qkv_ai_value, precision=1, commas=False)
    softmax_flops_m_str = fmt(softmax_flops_m_value, precision=1, commas=False)
    softmax_mb_str      = fmt(softmax_mb_value, precision=1, commas=False)
...
class Conv2dAnalysisCalc:
    ...
    conv_flops_per_out_str = fmt(conv_flops_per_out, precision=1)
...
class DenseLayerAnalysisCalc:
    ...
    dense_input_kb_str     = fmt(dense_input_kb, precision=1, commas=False)
    dense_weights_mb_str   = fmt(dense_weights_mb, precision=1, commas=False)
    dense_output_kb_str    = fmt(dense_output_kb, precision=1, commas=False)
    dense_total_mb_str     = fmt(dense_total_mb, precision=1, commas=False)
    dense_ai_str           = fmt(dense_ai, precision=1, commas=False)
    dense_attainable_str   = fmt(dense_attainable_tflops, precision=1, commas=False)
    dense_util_pct_str     = fmt(dense_util_pct, precision=1, commas=False)
    a100_bw_str            = fmt(a100_bw_gbs_value, precision=1)
```
...
class LayernormAnalysisCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    ln_elements_m_str   = fmt(ln_elements_m, precision=1, commas=False, drop_zero=True)
    ln_total_mflops_str = fmt(ln_total_mflops, precision=1, commas=False)
    ln_input_mb_str     = fmt(ln_input_mb, precision=1, commas=False)
    ln_params_kb_str    = fmt(ln_params_kb, precision=1, commas=False, drop_zero=True)
    ...
    ln_ridge_gap_str    = fmt(ln_ridge_gap, precision=1, commas=False)
    ln_attainable_str   = fmt(ln_attainable_tflops, precision=1, commas=False)
    a100_bw_str         = fmt(a100_bw_gbs_value, precision=1, commas=False)
```
...
class Gpt2ThroughputCalc:
    ...
    a100_tflops_fp16_str  = fmt(a100_tflops_fp16_val, precision=1, commas=False)
...
class MemoryFootprintCalc:
    ...
    footprint_ratio_str = fmt(footprint_ratio, precision=1, commas=False)
```
...
# │ How: Re-format memory constants with precision=1.
# │
...
class CompilerSpeedupCalc:
    ...
    naive_inference_ms_str     = fmt(naive_inference_ms, precision=1, commas=False)
    optimized_inference_ms_str = fmt(optimized_inference_ms, precision=1, commas=False)
...
class RuntimeProductionTdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tdp_str = fmt(Hardware.Cloud.A100.tdp.m_as(watt), precision=1, commas=False)
```
...
class FpWorkloadFitRecap:
    ...
    # OUTPUT
    a100_ridge_str = fmt(a100_ridge, precision=1, commas=False)
    a100_tflops_fp16_str = fmt(a100_peak_tflops, precision=1, commas=False)
    softmax_ai_low_str = fmt(softmax_ai_low, precision=1, commas=False)
    softmax_ai_high_str = fmt(softmax_ai_high, precision=1, commas=False)
    softmax_tflops_low_str = fmt(softmax_tflops_low, precision=1, commas=False)
    softmax_tflops_high_str = fmt(softmax_tflops_high, precision=1, commas=False)
    softmax_util_high_str = fmt(softmax_util_high, precision=1, commas=False)
    cpu_ridge_low_str = fmt(cpu_ridge_low, precision=1, commas=False)
    cpu_ridge_high_str = fmt(cpu_ridge_high, precision=1, commas=False)
    cpu_softmax_util_low_str = fmt(cpu_softmax_util_low, precision=1, commas=False)
    cpu_softmax_util_high_str = fmt(cpu_softmax_util_high, precision=1, commas=False)
```
...
class FpMemoryEnergyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_ridge_example_str    = fmt(fp_ridge_example, precision=1, commas=False)
    fp_layernorm_tflops_str = fmt(layernorm_tflops, precision=1, commas=False)
    fp_layernorm_util_str   = fmt(layernorm_util, precision=1, commas=False)
    fp_dram_pj_str          = fmt(dram_pj, precision=1, commas=False)
...
class FpMultigpuScalingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_nvlink_bw_str     = fmt(nvlink_bw_gbs, precision=1, commas=False)
    ...
    fp_no_overlap_efficiency_str = fmt(no_overlap_efficiency_pct, precision=1, commas=False)
```
...
class FpPeakVsSustainedRecap:
    ...
    # OUTPUT
    a100_tflops_fp16_str = fmt(a100_peak_tflops, precision=1, commas=False)
    transformer_tflops_low_str = fmt(transformer_tflops_low, precision=1, commas=False)
    transformer_tflops_high_str = fmt(transformer_tflops_high, precision=1, commas=False)
    transformer_util_low_str = fmt(transformer_util_low_rounded, precision=1, commas=False)
    transformer_util_high_str = fmt(transformer_util_high_rounded, precision=1, commas=False)
    recsys_tflops_low_str = fmt(recsys_tflops_low, precision=1, commas=False)
    recsys_tflops_high_str = fmt(recsys_tflops_high, precision=1, commas=False)
    recsys_util_low_str = fmt(recsys_util_low, precision=1, commas=False)
    recsys_util_high_str = fmt(recsys_util_high, precision=1, commas=False)
```
...
class FpSmallBatchCalc:
    ...
    fp_t4_ridge_str = fmt(t4_ridge, precision=1, commas=False)
    fp_t4_flops_str = fmt(t4_flops, precision=1, commas=False)
    fp_t4_bw_str    = fmt(t4_bw, precision=1, commas=False)
...
class CarbonRoiCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cpu_power_str      = fmt(cpu_power_w, precision=1)
    npu_power_str      = fmt(npu_power_w, precision=1)
    cpu_servers_str    = fmt(cpu_servers, precision=1)
    npu_chips_str      = fmt(npu_chips, precision=1)
    cpu_tflops_str     = fmt(cpu_tflops, precision=1)
    npu_tflops_str     = fmt(npu_tflops, precision=1)
    cpu_eff_str        = fmt(cpu_eff, precision=2)
    npu_eff_str        = fmt(npu_eff, precision=1)
    eff_gap_str        = fmt(eff_gap, precision=1)
    workload_str       = fmt(inferences_per_day / BILLION, precision=0)
    cpu_energy_day_str = fmt(cpu_energy_kwh_day, precision=1)
    npu_energy_day_str = fmt(npu_energy_kwh_day, precision=1)
    carbon_intensity_str = fmt(carbon_kg_per_kwh, precision=1)
    co2_saved_str      = fmt(co2_saved_metric_tons, precision=1)
```
