...
class AmdahlH100:
    ...
    p_resnet_pct_str = fmt(p_resnet * 100, precision=0, commas=False, suffix="%")
    serial_resnet_str = fmt(1-p_resnet, precision=2, commas=False)
    serial_resnet_pct_str = fmt(((1-p_resnet) * 100), precision=0, commas=False, suffix="%")
    ...
    serial_gpt2_pct_str = fmt(((1-p_gpt2) * 100), precision=0, commas=False, suffix="%")
...
Despite a `{python} AmdahlH100.hw_speedup_str`$\times$ hardware advantage, total system speedup is only **`{python} AmdahlH100.amdahl_resnet_round_str`$\times$**. The `{python} AmdahlH100.serial_resnet_pct_str` serial fraction caps practical gains.
...
class CpuMlInefficiency:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cpu_utilization_min_str = fmt(cpu_utilization_min_value, precision=0, commas=False, suffix="%")
    cpu_utilization_max_str = fmt(cpu_utilization_max_value, precision=0, commas=False, suffix="%")
...
class AcceleratorEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # V100 specs
    v100_tflops_str = fmt(h_v100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    v100_bw_str     = fmt(h_v100.memory_bw.m_as(GB/second), precision=0, commas=False, suffix=" GB/s")
    ...
    a100_tflops_fp16_str = fmt(h_a100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    a100_bw_str          = fmt(h_a100.memory_bw.m_as(GB/second), precision=0, suffix=" GB/s")
    a100_bw_tbs_str      = fmt(h_a100.memory_bw.m_as(TB/second), precision=2, commas=False, suffix=" TB/s")
    ...
    # H100 specs
    h100_tflops_tf32_str = fmt(h_h100.compute.precision_flops["tf32"].m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    ...
    h100_bw_str          = fmt(h_h100.memory_bw.m_as(GB/second), precision=0, suffix=" GB/s")
    h100_bw_tbs_str      = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False, suffix=" TB/s")
    ...
    # TPUv4 specs
    tpuv4_tflops_str = fmt(h_tpu.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    tpuv4_bw_str     = fmt(h_tpu.memory_bw.m_as(GB/second), precision=0, suffix=" GB/s")
    ...
    # Gaudi 2 specs
    gaudi_tflops_str = fmt(gaudi_tf, precision=0, commas=False, suffix=" TFLOP/s")
    gaudi_bw_str     = fmt(gaudi_bw_value, precision=0, suffix=" GB/s")
...
|:-----------------------------------|:------------------------------------------------|------------------------------------------------------------:|---------------------------------------------:|:---------------------------------------------|
| **NVIDIA V100**\index{NVIDIA!V100} | `{python} AcceleratorEconomics.v100_price_str`  |             `{python} AcceleratorEconomics.v100_tflops_str` |  `{python} AcceleratorEconomics.v100_bw_str` | `{python} AcceleratorEconomics.v100_pp_str`  |
| **NVIDIA A100**\index{NVIDIA!A100} | `{python} AcceleratorEconomics.a100_price_str`  |        `{python} AcceleratorEconomics.a100_tflops_fp16_str` |  `{python} AcceleratorEconomics.a100_bw_str` | `{python} AcceleratorEconomics.a100_pp_str`  |
| **NVIDIA H100**                    | `{python} AcceleratorEconomics.h100_price_str`  | `{python} AcceleratorEconomics.h100_tflops_tf32_str` (TF32) |  `{python} AcceleratorEconomics.h100_bw_str` | `{python} AcceleratorEconomics.h100_pp_str`  |
| **Google TPUv4**                   | `{python} AcceleratorEconomics.tpu_price_str`   |     `{python} AcceleratorEconomics.tpuv4_tflops_str` (BF16) | `{python} AcceleratorEconomics.tpuv4_bw_str` | `{python} AcceleratorEconomics.tpu_pp_str`   |
| **Intel Gaudi 2**                  | `{python} AcceleratorEconomics.gaudi_price_str` |      `{python} AcceleratorEconomics.gaudi_tflops_str` (FP8) | `{python} AcceleratorEconomics.gaudi_bw_str` | `{python} AcceleratorEconomics.gaudi_pp_str` |
...
class TensorLifecycleCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    kws_tensor_str         = fmt(kws_tensor_kb_value, precision=0, commas=False, suffix=" KB")
    ...
    a100_tflops_fp16_str   = fmt(a100_fp16_tflops_value, precision=0, commas=False, suffix=" TFLOP/s")
...
1.  **DRAM (HBM)**: The tensor starts here.
    *   **Size**: `{python} TensorLifecycleCalc.kws_samples_str` samples $\times$ `{python} TensorLifecycleCalc.kws_bytes_str` bytes (FP16) = **`{python} TensorLifecycleCalc.kws_tensor_str`**.
...
4.  **Registers**: The Tensor Core operates here.
    ...
    *   **Throughput**: `{python} TensorLifecycleCalc.a100_tflops_fp16_str`.
...
class InterconnectHierarchy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_str = fmt(nvlink_a100, precision=0, commas=False, suffix=" GB/s")
    nvlink_h100_str = fmt(nvlink_h100, precision=0, commas=False, suffix=" GB/s")
    ib_hdr_str = fmt(ib_hdr_gbps, precision=0, commas=False, suffix=" Gb/s")
    ib_ndr_str = fmt(ib_ndr_gbps, precision=0, commas=False, suffix=" Gb/s")
```

1. **Device-Device Interconnect (NVLink/Infinity Fabric)**[^fn-nvlink-bandwidth]\index{NVLink!GPU interconnect}\index{GPU Interconnect!switching fabric}\index{Infinity Fabric!AMD interconnect}: Modern multi-GPU nodes use specialized high-speed bridges like NVLink to connect accelerators directly, bypassing the host CPU. Bandwidth ranges from `{python} InterconnectHierarchy.nvlink_a100_str` to `{python} InterconnectHierarchy.nvlink_h100_str` per GPU. The primary use case is gradient synchronization (AllReduce)\index{AllReduce!gradient synchronization}[^fn-allreduce-gradient-sync] during distributed training. This bandwidth is critical for scaling; without it, multi-GPU training often scales poorly.
...
class RooflineGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_tflops_str = fmt(v100_tflops_value, precision=0, commas=False, suffix=" TFLOP/s")
    v100_bw_tbs_str = fmt(v100_bw_tbs_value, precision=1, commas=False, suffix=" TB/s")
    a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=0, commas=False, suffix=" TFLOP/s")
    a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=2, commas=False, suffix=" TB/s")
    h100_tflops_fp16_str = fmt(h100_tflops_fp16_value, precision=0, commas=False, suffix=" TFLOP/s")
    ...
    full_util_pct_str = MarkdownStr("100%")
```
...
*   **V100 (2017)**: `{python} RooflineGap.v100_tflops_str` / `{python} RooflineGap.v100_bw_tbs_str` ≈ **`{python} RooflineGap.v100_ridge_str` FLOP/byte**.
*   **A100 (2020)**: `{python} RooflineGap.a100_tflops_fp16_str` / `{python} RooflineGap.a100_bw_tbs_str` ≈ **`{python} RooflineGap.a100_ridge_str` FLOP/byte**.
*   **H100 (2023)**: `{python} RooflineGap.h100_tflops_fp16_str` / `{python} RooflineGap.h100_bw_tbs_str` ≈ **`{python} RooflineGap.h100_ridge_str` FLOP/byte**.
...
class TransformerLayerCalc:
    ...
    qkv_mb_str          = fmt(qkv_mb_value, precision=1, commas=False, suffix=" MB")
    ...
    softmax_mb_str      = fmt(softmax_mb_value, precision=1, commas=False, suffix=" MB")
...
- AI = `{python} TransformerLayerCalc.qkv_flops_b_str` billion FLOPs / `{python} TransformerLayerCalc.qkv_mb_str` = `{python} TransformerLayerCalc.qkv_ai_str` FLOP/byte, which is **compute bound on A100** (above `{python} TransformerLayerCalc.a100_ridge_str` threshold)
...
- AI = `{python} TransformerLayerCalc.softmax_flops_m_str` M / `{python} TransformerLayerCalc.softmax_mb_str` = `{python} TransformerLayerCalc.softmax_ai_str` FLOP/byte, which is memory-bound
...
class Conv2dAnalysisCalc:
    ...
    conv_input_mb_str      = fmt(conv_input_mb, precision=1, commas=False, suffix=" MB")
    conv_weights_mb_str    = fmt(conv_weights_mb, precision=1, commas=False, suffix=" MB")
    conv_output_mb_str     = fmt(conv_output_mb, precision=1, commas=False, suffix=" MB")
    conv_total_mb_str      = fmt(conv_total_mb, precision=1, commas=False, suffix=" MB")
...
$\text{AI}$ = `{python} Conv2dAnalysisCalc.conv_total_gflops_str` GFLOPs / `{python} Conv2dAnalysisCalc.conv_total_mb_str` = `{python} Conv2dAnalysisCalc.conv_ai_str` FLOP/byte

This is **well above** A100's ridge point of `{python} Conv2dAnalysisCalc.a100_ridge_str` FLOP/byte, making this operation compute-bound. The layer will achieve near-peak performance of ~`{python} Conv2dAnalysisCalc.a100_tflops_fp16_str` (FP16 with Tensor Cores).
...
class DenseLayerAnalysisCalc:
    ...
    dense_input_kb_str     = fmt(dense_input_kb, precision=1, commas=False, suffix=" KB")
    dense_weights_mb_str   = fmt(dense_weights_mb, precision=1, commas=False, suffix=" MB")
    dense_output_kb_str    = fmt(dense_output_kb, precision=1, commas=False, suffix=" KB")
    dense_total_mb_str     = fmt(dense_total_mb, precision=1, commas=False, suffix=" MB")
    dense_ai_str           = fmt(dense_ai, precision=1, commas=False)
    dense_attainable_str   = fmt(dense_attainable_tflops, precision=1, commas=False, suffix=" TFLOP/s")
    dense_util_pct_str     = fmt(dense_util_pct, precision=1, commas=False, suffix="%")
    a100_bw_str            = fmt(a100_bw_gbs_value, precision=0, suffix=" GB/s")
    a100_ridge_str         = fmt_int(round(a100_ridge_value), commas=False)
```
...
**Arithmetic Intensity**:
$\text{AI}$ = `{python} DenseLayerAnalysisCalc.dense_total_mflops_str` MFLOPs / `{python} DenseLayerAnalysisCalc.dense_total_mb_str` = `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte
...
$R_{\text{attainable}}$ = `{python} DenseLayerAnalysisCalc.a100_bw_str` $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_str`
...
class LayernormAnalysisCalc:
    ...
    ln_input_mb_str     = fmt(ln_input_mb, precision=1, commas=False, suffix=" MB")
    ln_params_kb_str    = fmt(ln_params_kb, precision=1, commas=False, suffix=" KB")
    ln_output_mb_str    = fmt(ln_output_mb, precision=1, commas=False, suffix=" MB")
    ln_total_mb_str     = fmt(ln_total_mb, precision=1, commas=False, suffix=" MB")
    ...
    ln_flops_per_str    = fmt(ln_flops_per, precision=0, commas=False)
```
...
- Operations per element: mean (1 ADD), variance (1 ADD, 1 MUL), normalize (1 ADD, 1 MUL, 1 DIV) ≈ `{python} LayernormAnalysisCalc.ln_flops_per_str`
...
- Input: `{python} LayernormAnalysisCalc.ln_elements_m_str` M $\times$ 2 = `{python} LayernormAnalysisCalc.ln_input_mb_str`
- Parameters (scale, bias): $768 \times 2 \times 2$ = `{python} LayernormAnalysisCalc.ln_params_kb_str` (negligible)
- Output: `{python} LayernormAnalysisCalc.ln_elements_m_str` M $\times$ 2 = `{python} LayernormAnalysisCalc.ln_output_mb_str`
- Total: `{python} LayernormAnalysisCalc.ln_total_mb_str`

**Arithmetic Intensity**:
$\text{AI}$ = `{python} LayernormAnalysisCalc.ln_total_mflops_str` MFLOPs / `{python} LayernormAnalysisCalc.ln_total_mb_str` = `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte
...
$R_{\text{attainable}}$ = `{python} LayernormAnalysisCalc.a100_bw_str` $\times$ `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte = `{python} LayernormAnalysisCalc.ln_attainable_str`
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, fmt_int, MarkdownStr
...
class Gpt2ThroughputCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt2_weight_gb_str    = fmt(gpt2_weight_gb, precision=0, commas=False, suffix=" GB")
    ...
    gpt2_max_tflops_str   = fmt(gpt2_max_tflops, precision=1, commas=False, suffix=" TFLOP/s")
    gpt2_utilization_str  = fmt(gpt2_utilization, precision=1, commas=False)
    a100_tflops_fp16_str  = fmt(a100_tflops_fp16_val, precision=0, commas=False, suffix=" TFLOP/s")
    a100_bw_tbs_str       = fmt(a100_bw_tbs_val, precision=2, commas=False, suffix=" TB/s")
    ...
    a100_cost_usd_str     = fmt(15_000, precision=0)
    gpt2_util_ceiling_str = MarkdownStr("<1%")
```
...
*   **Peak compute**: `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` (FP16 Tensor Core).
*   **Peak bandwidth**: `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` (HBM2e).
*   **Ridge point $(R_{\text{peak}}/\text{BW})$**: `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` / `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` = **`{python} Gpt2ThroughputCalc.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
    *   **Meaning**: Saturating this chip at FP16 precision requires `{python} Gpt2ThroughputCalc.a100_ridge_str` operations for every byte loaded. The ridge point varies by precision: FP32 operations (`{python} Gpt2ThroughputCalc.a100_tflops_fp32_str` peak) have a ridge point of only ~`{python} Gpt2ThroughputCalc.a100_ridge_fp32_str` FLOP/byte.
...
*   **Arithmetic intensity**:
    `{python} Gpt2ThroughputCalc.gpt2_decode_gflops_str` GFLOPs / `{python} Gpt2ThroughputCalc.gpt2_weight_gb_str` = **`{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte**
...
*   **Maximum throughput**: `{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte $\times$ `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` = **`{python} Gpt2ThroughputCalc.gpt2_max_tflops_str`**.
*   **Utilization ceiling**:
    `{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` (Actual) / `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` (Peak) ≈ **`{python} Gpt2ThroughputCalc.gpt2_utilization_str`**
...
class MemoryFootprintCalc:
    ...
    total_mb_str        = fmt(total_mb, precision=1, commas=False, suffix=" MB")
```
...
Cloud accelerators are often shared, leading to multi-tenant interference. A model might run <!-- lego-ok: illustrative neighbor slowdown, not computed -->20 percent slower when a neighbor workload competes for memory bandwidth. Production systems need monitoring to detect and respond to this interference.
...
class FpMemoryEnergyCalc:
    ...
    fp_layernorm_tflops_str = fmt(layernorm_tflops, precision=0, commas=False, suffix=" TFLOP/s")
    fp_layernorm_util_str   = fmt(layernorm_util, precision=0, commas=False, suffix="%")
    fp_dram_pj_str          = fmt(dram_pj, precision=0, commas=False)
    fp_sram_pj_str          = fmt(sram_pj, precision=1, commas=False)
    fp_peak_tflops_str      = fmt(peak_tflops, precision=0, commas=False, suffix=" TFLOP/s")
    fp_peak_bw_tbs_str      = fmt(peak_bw_tbs, precision=0, commas=False, suffix=" TB/s")
...
class FpMultigpuScalingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_nvlink_bw_str     = fmt(nvlink_bw_gbs, precision=0, commas=False, suffix=" GB/s")
    fp_sync_time_str     = fmt(sync_time_ms, precision=2, commas=False, suffix=" ms")
    fp_sync_overhead_str = fmt(sync_overhead_pct, precision=1, commas=False, suffix="%")
    fp_no_overlap_speedup_str = fmt(no_overlap_speedup, precision=1, commas=False)
    fp_no_overlap_efficiency_str = fmt(no_overlap_efficiency_pct, precision=1, commas=False, suffix="%")
```

**Fallacy**: *Hardware acceleration benefits scale linearly with additional accelerators.*

Teams expect eight GPUs to train 8$\times$ faster than one GPU. Multi-accelerator scaling introduces communication overhead that violates linear scaling assumptions. As noted in @sec-hardware-acceleration-multichip-scaling-c649, AllReduce operations for gradient synchronization can require exchanging hundreds of gigabytes per training step for large models. With NVLink at `{python} FpMultigpuScalingCalc.fp_nvlink_bw_str` bidirectional, synchronizing 1 GB of gradients requires `{python} FpMultigpuScalingCalc.fp_sync_time_str`; for a 50 ms training step, this represents `{python} FpMultigpuScalingCalc.fp_sync_overhead_str` of step time. Without compute-communication overlap, eight-GPU setups achieve about `{python} FpMultigpuScalingCalc.fp_no_overlap_speedup_str`$\times$ speedup (`{python} FpMultigpuScalingCalc.fp_no_overlap_efficiency_str` efficiency) at best, and typical workloads see 6--7$\times$ (75--87 percent efficiency) due to load imbalance and synchronization barriers. Small models with insufficient parallel work achieve even worse scaling, sometimes seeing 3--4$\times$ speedup on eight GPUs (37--50 percent efficiency).
...
Vendors advertise peak FLOP/s as the definitive measure of accelerator capability, but real-world performance equals Peak FLOP/s $\times$ Utilization, where utilization is dictated by the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff). An A100 advertises `{python} FpPeakVsSustainedRecap.a100_tflops_fp16_str` at FP16, yet transformer training typically achieves only `{python} FpPeakVsSustainedRecap.transformer_tflops_low_str`--`{python} FpPeakVsSustainedRecap.transformer_tflops_high_str` (`{python} FpPeakVsSustainedRecap.transformer_util_low_str`--`{python} FpPeakVsSustainedRecap.transformer_util_high_str` utilization) because memory-bound operations such as attention and LayerNorm drag down the average throughput. Recommendation models fare even worse, often reaching only `{python} FpPeakVsSustainedRecap.recsys_tflops_low_str`--`{python} FpPeakVsSustainedRecap.recsys_tflops_high_str` (`{python} FpPeakVsSustainedRecap.recsys_util_low_str`--`{python} FpPeakVsSustainedRecap.recsys_util_high_str` utilization) due to sparse, irregular memory access patterns that leave compute units idle. Engineers should budget projects based on sustained throughput, measured or estimated via the Roofline Model, rather than peak marketing specifications.
...
class FpSmallBatchCalc:
    ...
    fp_t4_flops_str = fmt(t4_flops, precision=0, commas=False, suffix=" TFLOP/s")
    fp_t4_bw_str    = fmt(t4_bw, precision=0, commas=False, suffix=" GB/s")
    fp_a100_roof_b1_str = fmt(a100_roof_b1, precision=1, commas=False, suffix=" TFLOP/s")
    fp_t4_roof_b1_str = fmt(t4_roof_b1, precision=1, commas=False, suffix=" TFLOP/s")
```

**Pitfall**: *Deploying small-batch inference workloads on high-compute accelerators.*

Teams deploy high-throughput training accelerators (A100, H100) for latency-sensitive inference with batch size 1–4. As the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff) predicts, small batches severely reduce arithmetic intensity: a dense layer with M=N=2048 achieves AI = `{python} FpSmallBatchCalc.fp_ai_b1_str` FLOP/byte at batch=1 vs. AI = `{python} FpSmallBatchCalc.fp_ai_b256_str` FLOP/byte at batch=256. At batch=1, the memory-bound roofline ceiling is only about `{python} FpSmallBatchCalc.fp_a100_roof_b1_str` on A100 and `{python} FpSmallBatchCalc.fp_t4_roof_b1_str` on T4. The T4's peak is `{python} FpSmallBatchCalc.fp_t4_flops_str` (FP16 Tensor Core) with a ridge point of `{python} FpSmallBatchCalc.fp_t4_ridge_str` FLOP/byte (`{python} FpSmallBatchCalc.fp_t4_flops_str` / `{python} FpSmallBatchCalc.fp_t4_bw_str`). Small-batch inference remains memory bound on both accelerators, so the T4's lower cost can make it more economical despite its much lower peak compute. Training-class accelerator instances often rent for several times more than inference-class instances for little latency gain in this regime. Inference deployments should match batch size to accelerator characteristics, using high-compute accelerators only for batched serving where arithmetic intensity exceeds ridge points.
...
class FeasibilityAssessment:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    headroom_str = fmt(headroom_gb, precision=0, commas=False, suffix=" GB")
    model_7b_gb_str = fmt(model_7b_gb, precision=0, commas=False, suffix=" GB")
    model_70b_gb_str = fmt(model_70b_gb, precision=0, commas=False, suffix=" GB")
    mem_bw_tb_s_str = fmt(mem_bw_gb_s / 1000, precision=0, commas=False, suffix=" TB/s")
    latency_target_ms_str = fmt(latency_target_ms, precision=0, commas=False, suffix=" ms")
    token_latency_ms_str = fmt(token_latency_ms, precision=0, commas=False, suffix=" ms")
    frame_budget_str = fmt(frame_budget_ms, precision=1, commas=False, suffix=" ms")
    params_7b_billion_str = fmt(params_7b / BILLION, precision=0, commas=False)
    model_7b_gb_str = fmt(model_7b_gb, precision=0, commas=False, suffix=" GB")
    gpu_mem_gb_str = fmt(gpu_mem_gb, precision=0, commas=False, suffix=" GB")
```
...
*   **Example**: Running a `{python} FeasibilityAssessment.params_7b_billion_str`-billion-parameter Llama model (`{python} FeasibilityAssessment.model_7b_gb_str` weights at FP16) on a GPU with `{python} FeasibilityAssessment.gpu_mem_gb_str` leaves only `{python} FeasibilityAssessment.headroom_str` for context. Long prompts will OOM (Out of Memory).
...
*   **Example**: Serving a 70-billion-parameter model (`{python} FeasibilityAssessment.model_70b_gb_str`) on a GPU with `{python} FeasibilityAssessment.mem_bw_tb_s_str` bandwidth yields ≈ `{python} FeasibilityAssessment.token_latency_ms_str` per token. If a workload requires `{python} FeasibilityAssessment.latency_target_ms_str` latency, this hardware fails regardless of compute power.

**Compute check**

*   **Formula**: $T_{\text{process}} = \frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}$
*   **Constraint**: $T_{\text{process}} < \text{Throughput Target}$
*   **Example**: Processing video at 30 FPS requires completing all inference within `{python} FeasibilityAssessment.frame_budget_str`.
...
class CarbonRoiCalc:
    ...
    carbon_kg_per_kwh  = Infrastructure.Grids.US_Avg.carbon_intensity_kg_kwh
    cpu_power_total    = cpu_servers * cpu_power_w * watt
    npu_power_total    = npu_chips * npu_power_w * watt
    cpu_energy_kwh_day = (cpu_power_total * HOURS_PER_DAY * hour).to(kilowatt * hour).magnitude
    npu_energy_kwh_day = (npu_power_total * HOURS_PER_DAY * hour).to(kilowatt * hour).magnitude
    ...
    cpu_co2_metric_tons_year = cpu_energy_kwh_day * DAYS_PER_YEAR * carbon_kg_per_kwh / THOUSAND
    ...
    check(co2_saved_metric_tons < cpu_co2_metric_tons_year,
          "Annual savings cannot exceed total CPU fleet emissions.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cpu_power_str      = fmt(cpu_power_w, precision=0, suffix=" W")
    npu_power_str      = fmt(npu_power_w, precision=0, suffix=" W")
    cpu_servers_str    = fmt(cpu_servers, precision=0)
    npu_chips_str      = fmt(npu_chips, precision=0)
    cpu_tflops_str     = fmt(cpu_tflops, precision=0, suffix=" TFLOP/s")
    npu_tflops_str     = fmt(npu_tflops, precision=0, suffix=" TFLOP/s")
    cpu_eff_str        = fmt(cpu_eff, precision=2, suffix=" TFLOP/s per W")
    npu_eff_str        = fmt(npu_eff, precision=0, suffix=" TFLOP/s per W")
    eff_gap_str        = fmt(eff_gap, precision=0)
    workload_str       = fmt(inferences_per_day / BILLION, precision=0)
    cpu_energy_day_str = fmt(cpu_energy_kwh_day, precision=0, commas=True, suffix=" kWh")
    npu_energy_day_str = fmt(npu_energy_kwh_day, precision=0, commas=True, suffix=" kWh")
    carbon_intensity_str = fmt(carbon_kg_per_kwh, precision=3, suffix=" kg/kWh")
...
*   **CPU inference**: `{python} CarbonRoiCalc.cpu_power_str` for `{python} CarbonRoiCalc.cpu_tflops_str` (efficiency = `{python} CarbonRoiCalc.cpu_eff_str`).
*   **NPU inference**: `{python} CarbonRoiCalc.npu_power_str` for `{python} CarbonRoiCalc.npu_tflops_str` (efficiency = `{python} CarbonRoiCalc.npu_eff_str`).
...
2.  **CPU fleet energy**: `{python} CarbonRoiCalc.cpu_servers_str` CPU servers $\times$ `{python} CarbonRoiCalc.cpu_power_str` $\times$ 24 h $\approx$ `{python} CarbonRoiCalc.cpu_energy_day_str`/day.
3.  **NPU fleet energy**: `{python} CarbonRoiCalc.npu_chips_str` NPU chips $\times$ `{python} CarbonRoiCalc.npu_power_str` $\times$ 24 h $\approx$ `{python} CarbonRoiCalc.npu_energy_day_str`/day.
4.  **Carbon savings**: At `{python} CarbonRoiCalc.carbon_intensity_str`, switching to NPUs saves **~`{python} CarbonRoiCalc.co2_saved_str` metric tons of CO~2~ per year**.
