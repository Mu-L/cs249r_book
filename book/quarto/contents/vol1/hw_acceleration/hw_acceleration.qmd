...
::: {#lst-loop_level_dense lst-cap="**Processor-Level Execution**: Nested loops reveal the $\mathcal{O}(\text{batch} \times \text{input} \times \text{output})$ multiply-accumulate operations that accelerators must execute, with `{python} WeightMatrixCalc.wm_batch_macs_m_str` million MACs for batch=`{python} WeightMatrixCalc.wm_batch_str`, input=`{python} WeightMatrixCalc.wm_in_str`, output=`{python} WeightMatrixCalc.wm_out_str` configurations."}
