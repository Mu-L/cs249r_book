...
- Operations per element: mean (1 ADD), variance (1 ADD, 1 MUL), normalize (1 ADD, 1 MUL, 1 DIV) ≈ `{python} LayernormAnalysisCalc.ln_flops_per_str` FLOPs
...
**Systems insight**:
Without batching or caching, a \$`{python} Gpt2ThroughputCalc.a100_cost_usd_str` GPU runs at **`{python} Gpt2ThroughputCalc.gpt2_util_ceiling_str` efficiency** on LLM inference. This "utilization gap" drives the need for key-value caching and quantization.
...
class FpMemoryEnergyCalc:
    ...
    fp_peak_tflops_str      = fmt(peak_tflops, precision=0, commas=False)
    fp_peak_bw_tbs_str      = fmt(peak_bw_tbs, precision=0, commas=False)
...
class FeasibilityAssessment:
    ...
    model_7b_gb_str = fmt(model_7b_gb, precision=0, commas=False)
    model_70b_gb_str = fmt(model_70b_gb, precision=0, commas=False)
    mem_bw_tb_s_str = fmt(mem_bw_gb_s / 1000, precision=0, commas=False)
    latency_target_ms_str = fmt(latency_target_ms, precision=0, commas=False)
    ...
    model_7b_gb_str = fmt(model_7b_gb, precision=0, commas=False)
    gpu_mem_gb_str = fmt(gpu_mem_gb, precision=0, commas=False)
```
...
*   **Example**: Running a `{python} FeasibilityAssessment.params_7b_billion_str`-billion-parameter Llama model (`{python} FeasibilityAssessment.model_7b_gb_str` GB weights at FP16) on a `{python} FeasibilityAssessment.gpu_mem_gb_str` GB GPU leaves only `{python} FeasibilityAssessment.headroom_str` GB for context. Long prompts will OOM (Out of Memory).
...
*   **Example**: Serving a 70-billion-parameter model (`{python} FeasibilityAssessment.model_70b_gb_str` GB) on a GPU with `{python} FeasibilityAssessment.mem_bw_tb_s_str` TB/s bandwidth yields ≈ `{python} FeasibilityAssessment.token_latency_ms_str` ms per token. If a workload requires `{python} FeasibilityAssessment.latency_target_ms_str` ms latency, this hardware fails regardless of compute power.
