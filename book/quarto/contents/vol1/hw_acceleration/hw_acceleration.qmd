...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ HARDWARE ACCELERATION OPENING SPECS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: §Acceleration Fundamentals callout-definition (§Hardware
# │          Acceleration, §ML Accelerator). Provides A100 peak BF16 TFLOPS
# │          and HBM bandwidth used in the chapter's opening callouts before
# │          the AcceleratorEconomics cell is defined (~line 2347).
# │
# │ Goal: Centralize the A100 peak FLOPS and memory bandwidth so the
# │       opening definition callouts pull from the canonical constants.
# │ Show: "312 TFLOPS BF16" and "2.04 TB/s" inline.
# │ How: Read A100_FLOPS_FP16_TENSOR and A100_MEM_BW from constants.
# │
# │ Imports: mlsysim.core.constants (A100_FLOPS_FP16_TENSOR, A100_MEM_BW,
# │          TFLOPs, TB, second)
# │ Exports: HwAccelSetup.a100_tflops_fp16_str, HwAccelSetup.a100_bw_tbs_str
# └─────────────────────────────────────────────────────────────────────────────
...
class HwAccelSetup:
    """A100 peak BF16 throughput and HBM bandwidth for the opening callouts."""
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
...
1.  **Significance (quantitative)**: The efficiency differential over CPUs is quantifiable. An A100 GPU delivers `{python} HwAccelSetup.a100_tflops_fp16_str` TFLOPS BF16 with `{python} HwAccelSetup.a100_bw_tbs_str` TB/s memory bandwidth, while a high-end server CPU delivers roughly 1–2 TFLOPS FP32 with 200 GB/s bandwidth, a 150--300$\times$ compute throughput gap and a 10$\times$ bandwidth gap for the same matrix-multiply workloads that dominate neural network training and inference.
...
dense = Dense(512)(
    input_tensor
)  # $256{\times}512$ = 131K MACs per sample
```
...
::: {#lst-loop_level_dense lst-cap="**Processor-Level Execution**: Nested loops reveal the $O(\text{batch} \times \text{input} \times \text{output})$ multiply-accumulate operations that accelerators must execute, with 4 million MACs for typical batch=32, input=256, output=512 configurations."}
...
# Example: 32×512×256 = 4.2M multiply-accumulate
# operations
```
...
# │ Exports: WeightMatrixCalc.wm_in_str, WeightMatrixCalc.wm_out_str,
# │          WeightMatrixCalc.wm_params_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
\begin{axis}[
  ...
  precision=0
    ...
    xlabel={Year},
    ylabel={Peak performance (TOPS/TFLOPS)},
    title style={align=center},
    title={NVIDIA GPU Inference Performance\\
    {\color{black!60}\footnotesize\usefont{T1}{phv}{m}{it} Peak Throughput (TOPS/TFLOPS), logarithmic scale}},
    axis lines*=left,
...
\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2012,4.4e0) {\textbf{K20X} \\ {\color{black!60}3.9 TFLOPS}};
...
\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2021.0,0.95e4) {\textbf{H100} \\ {\color{black!60}4,000 TOPS}};
%
...
# │
# │ Imports: mlsysim.core.constants (V100/A100/H100 peak_flops, memory_bw, flop, byte, TB, second)
# │          mlsysim.Hardware (Cloud.V100, Cloud.A100, Cloud.H100)
# │ Exports: RidgePointTrends.v100_ridge, RidgePointTrends.a100_ridge,
# │          RidgePointTrends.h100_ridge, RidgePointTrends.a100_ridge_fp32,
# │          RidgePointTrends.v100_ridge_str, RidgePointTrends.a100_ridge_str,
# │          RidgePointTrends.h100_ridge_str, RidgePointTrends.a100_ridge_fp32_str,
# │          RidgePointTrends.relu_below_roofline_str, RidgePointTrends.legacy_ai_str,
# │          RidgePointTrends.bw_growth_str, RidgePointTrends.flops_growth_str
# │
# │ Note: PERSISTENT—RidgePointTrends.a100_ridge, RidgePointTrends.h100_ridge used throughout §Layer-by-Layer
...
1.  **Significance (quantitative)**: The intensity threshold separating memory-bound from compute-bound regimes is the roofline ridge point: $R_{\text{peak}} / \text{BW}$. For an A100 (`{python} AcceleratorEconomics.a100_tflops_fp16_str` TFLOPS BF16, `{python} AcceleratorEconomics.a100_bw_tbs_str` TB/s), the ridge point is roughly `{python} RooflineGap.a100_ridge_str` FLOP/byte. A large matrix-multiply achieves about 100--200 FLOP/byte (compute bound); a pointwise ReLU achieves about 0.5 FLOP/byte (memory bound), placing these two operations in completely different optimization regimes on the same hardware.
...
*   **V100 (2017)**: `{python} AcceleratorEconomics.v100_tflops_str` TF/0.9 TB/s ≈ **`{python} RooflineGap.v100_ridge_str` FLOP/byte**.
*   **A100 (2020)**: `{python} AcceleratorEconomics.a100_tflops_fp16_str` TF / `{python} AcceleratorEconomics.a100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.a100_ridge_str` FLOP/byte**.
*   **H100 (2023)**: `{python} AcceleratorEconomics.h100_tflops_fp16_str` TF / `{python} AcceleratorEconomics.h100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.h100_ridge_str` FLOP/byte**.
...
# │ Exports: Gpt2ThroughputCalc.gpt2_flops_b_str,
# │          Gpt2ThroughputCalc.gpt2_mem_gb_str,
# │          Gpt2ThroughputCalc.gpt2_ai_str,
# │          Gpt2ThroughputCalc.gpt2_attainable_str,
# │          Gpt2ThroughputCalc.gpt2_util_pct_str,
...
*   **Peak compute**: `{python} TensorLifecycleCalc.a100_tflops_fp16_str` TFLOPS (FP16 Tensor Core).
*   **Peak bandwidth**: `{python} AcceleratorEconomics.a100_bw_tbs_str` TB/s (HBM2e).
*   **Ridge point $(R_{\text{peak}}/\text{BW})$**: `{python} TensorLifecycleCalc.a100_tflops_fp16_str` / `{python} AcceleratorEconomics.a100_bw_tbs_str` = **`{python} RooflineGap.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
...
*   **Maximum throughput**: `{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte $\times$ `{python} AcceleratorEconomics.a100_bw_tbs_str` TB/s = **`{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOPS**.
*   **Utilization ceiling**:
    `{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOPS (Actual) / `{python} TensorLifecycleCalc.a100_tflops_fp16_str` TFLOPS (Peak) ≈ **`{python} Gpt2ThroughputCalc.gpt2_utilization_str` percent**
...
Engineers assume specialized accelerators automatically outperform general-purpose processors for all AI workloads. In reality, specialized hardware achieves peak performance only when workloads match architectural assumptions. As demonstrated in @sec-hardware-acceleration-roofline-model-42ff, operations must exceed the accelerator's ridge point to be compute bound; an A100 GPU has a ridge point of `{python} RooflineGap.a100_ridge_str` FLOP/byte, meaning operations with arithmetic intensity below this threshold are memory bound regardless of the accelerator's `{python} TensorLifecycleCalc.a100_tflops_fp16_str` TFLOPS peak compute. A transformer attention softmax with AI = 2--5 FLOP/byte achieves only 4--10 TFLOPS (3 percent utilization) on an A100, while achieving 80--90 percent of a CPU's lower peak because CPUs have ridge points of 10--20 FLOP/byte. Models with irregular memory access, small batch sizes, or dynamic computation graphs may perform better on flexible processors. Effective hardware selection requires matching workload arithmetic intensity to architectural ridge points, not assuming specialization always wins.
