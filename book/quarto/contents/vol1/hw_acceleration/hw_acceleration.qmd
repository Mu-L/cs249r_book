...
#| label: hw-acceleration-setup
#| echo: false
from mlsysim.fmt import fmt, check

class HwAccelSetup:
    """A100 peak FP16/BF16 throughput and HBM bandwidth for the opening callouts."""
    # LOAD
    ...
    cpu_tflops_low   = CPU_FLOPS_FP32.m_as(TFLOPs/second)
    cpu_tflops_high  = 2
    ...
    cpu_gap_min_str      = fmt(cpu_gap_min, precision=1, allow_zero=True, commas=False)
    cpu_gap_max_str      = fmt(cpu_gap_max, precision=1, allow_zero=True, commas=False)
```
...
Data was optimized in @sec-data-selection and the Algorithm (Model) was compressed in @sec-model-compression. The final Machine axis of the D·A·M taxonomy in @sec-dam-taxonomy is the subject of hardware acceleration. Hardware acceleration exists because of a striking asymmetry in modern computing: arithmetic is *cheap*, but moving data is *expensive*. In the time a modern accelerator computes a thousand floating-point operations, a single value travels from main memory. This inversion, where computation is the abundant resource and bandwidth is the scarce one, is the reason specialized hardware matters for machine learning.

::: {#dfn-hw-acceleration-hardware-acceleration .callout-definition title="Hardware acceleration"}

***Hardware Acceleration***\index{Hardware Acceleration!definition} is the practice of replacing general-purpose processor logic with domain-specific silicon optimized for a narrow class of operations, trading programmability for the compute density $(R_{\text{peak}})$ and performance-per-watt gains that regular, data-parallel workloads like matrix multiplication can exploit.

1.  **Significance (quantitative)**: The throughput gain is orders of magnitude. An A100 GPU delivers `{python} HwAccelSetup.a100_tflops_fp16_str` TFLOP/s for FP16/BF16 matrix multiplication, while a server-class CPU delivers roughly 1–2 TFLOP/s for the same operation, a `{python} HwAccelSetup.cpu_gap_min_str`--`{python} HwAccelSetup.cpu_gap_max_str`$\times$ gap achieved by dedicating 80+ billion transistors to parallel arithmetic units rather than to branch predictors, out-of-order schedulers, and large caches.
...
3.  **Common pitfall**: A frequent misconception is that hardware acceleration uniformly speeds up ML systems. Acceleration is a multiplier only on the parallelizable fraction of the workload (Amdahl's Law): if data loading, Python overhead, and kernel launches constitute 10 percent of total time, an infinitely fast accelerator can at most achieve a 10$\times$ end-to-end speedup.
...
***Hardware-Software Co-design***\index{Hardware-Software Co-design!definition} is a development methodology that intentionally violates traditional hardware-software abstraction layers, allowing algorithm constraints to inform silicon design and hardware capabilities to directly shape algorithm formulation.
...
Hardware acceleration targets specific terms in the iron law of ML systems\index{Iron Law!ML systems performance} (@sec-introduction-iron-law-ml-systems-c32a), which decomposes end-to-end time into data volume $(D_{\text{vol}}/\text{BW})$, computation $(O/(R_{\text{peak}} \cdot \eta_{\text{hw}}))$, and fixed latency $(L_{\text{lat}})$. While data selection reduced the total data and model compression reduced $O$ per sample, hardware acceleration increases the rate at which those operations execute by improving $R_{\text{peak}}$, $\eta_{\text{hw}}$, and $\text{BW}$. Yet acceleration has a hard ceiling, established by Amdahl's Law[^fn-amdahls-law-acceleration].

[^fn-amdahls-law-acceleration]: **Amdahl's Law**: Dictates that accelerating one component of a system (computation) yields diminishing returns as the un-accelerated components (data movement, latency) come to dominate the total time. Even if hardware makes the computation term $(O/(R_{\text{peak}} \cdot \eta_{\text{hw}}))$ instantaneous, the system is still bottlenecked by the serial data loading $(D_{\text{vol}}/\text{BW})$ and fixed latency $(L_{\text{lat}})$ terms from the iron law. This is why a 100$\times$ improvement in raw accelerator throughput often produces only a 5--20$\times$ improvement in end-to-end task time. \index{Amdahl's Law!acceleration ceiling}
...
*   **$p$ (parallel fraction)**: The matrix multiplications (typically 90–99 percent of an ML workload).
*   **$G_{\text{accel}}$ (accelerator gain)**: The raw speed advantage of the GPU or Tensor Processing Unit (TPU) over the CPU (typically 100--1,000$\times$).
...
**Pitfall**: If data loading takes 10 percent of the time ($p=0.9$), even an **infinite speed** accelerator ($G_{\text{accel}}=\infty$) can only achieve a **10$\times$** total speedup. The serial component dominates the parallel accelerator component once the latter is sufficiently fast.
...
# │
# │ Imports: sys, os, numpy (np), matplotlib.colors (mcolors), mlsysim.core.viz (viz)
...
fig, ax, COLORS, plt = viz.setup_plot(figsize=(10, 4.75))
...
overall_speedup = 1 / ((1 - p_grid) + (p_grid/g_accel_grid))
...
- [ ] **Workload Variation**: Why does ResNet (compute bound) scale better than MobileNet (latency-bound)? (ResNet spends more time in parallelizable matrix math).\index{ResNet-50!compute-bound workload}
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.compute.precision_flops["int8"]), mlsysim.book (fmt)
# │ Exports: AmdahlsH100.amdahl_resnet_str, AmdahlsH100.amdahl_gpt2_str,
# │          AmdahlsH100.hw_speedup_str, AmdahlsH100.h100_tops_int8_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AmdahlH100:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    hw_speedup_factor = 247.0  # H100 INT8 matmul vs ~8 TOPS baseline CPU
    ...
    def calc_speedup(p, g_accel):
        serial = 1 - p
        parallel_component = p/g_accel
    ...
    h100_tops_int8_str = fmt(h100_tops_int8, precision=1, allow_zero=True)
    hw_speedup_str = fmt(hw_speedup_factor, precision=1, allow_zero=True, commas=False)

    # ResNet
    p_resnet_str = fmt(p_resnet, precision=2, commas=False)
    p_resnet_pct_str = fmt_percent(p_resnet, precision=1, commas=False)
    serial_resnet_str = fmt(1-p_resnet, precision=2, commas=False)
    serial_resnet_pct_str = fmt_percent((1-p_resnet), precision=1, allow_zero=True, commas=False)
    ...
    amdahl_resnet_round_str = fmt(speedup_resnet, precision=1, allow_zero=True, commas=False)

    # GPT-2
    p_gpt2_str = fmt(p_gpt2, precision=2, commas=False)
    serial_gpt2_str = fmt(1-p_gpt2, precision=2, commas=False)
    serial_gpt2_pct_str = fmt_percent((1-p_gpt2), precision=1, allow_zero=True, commas=False)
    ...
    amdahl_gpt2_ceil_str = fmt(ceiling_gpt2, precision=1, allow_zero=True, commas=False)
```
...
- H100 delivers $G_{\text{accel}}$ = `{python} AmdahlH100.hw_speedup_str`$\times$ speedup over CPU for matrix multiply (`{python} AmdahlH100.h100_tops_int8_str` TOPS INT8 vs. ~8 TOPS on baseline CPU without AMX extensions)
- Typical inference has $p$ = `{python} AmdahlH100.p_resnet_str` (`{python} AmdahlH100.p_resnet_pct_str` percent parallelizable, `{python} AmdahlH100.serial_resnet_pct_str` percent serial: data loading, preprocessing, postprocessing)

\(\text{Speedup} = 1 / ((1-`{python} AmdahlH100.p_resnet_str`) + `{python} AmdahlH100.p_resnet_str` / `{python} AmdahlH100.hw_speedup_str`) = 1 / (`{python} AmdahlH100.serial_resnet_str` + `{python} AmdahlH100.p_resnet_per_s_str`) \approx `{python} AmdahlH100.amdahl_resnet_str`\times\)

Despite a `{python} AmdahlH100.hw_speedup_str`$\times$ hardware advantage, total system speedup is only **`{python} AmdahlH100.amdahl_resnet_round_str`$\times$**. The `{python} AmdahlH100.serial_resnet_pct_str` percent serial fraction caps practical gains.

**Contrast with GPT-2 (autoregressive)**:

- Same H100, but GPT-2 token generation has $p$ = `{python} AmdahlH100.p_gpt2_str` (`{python} AmdahlH100.serial_gpt2_pct_str` percent serial: KV-cache updates, sampling, Python overhead)

\(\text{Speedup} = 1 / ((1-`{python} AmdahlH100.p_gpt2_str`) + `{python} AmdahlH100.p_gpt2_str` / `{python} AmdahlH100.hw_speedup_str`) = 1 / (`{python} AmdahlH100.serial_gpt2_str` + `{python} AmdahlH100.p_gpt2_per_s_str`) \approx `{python} AmdahlH100.amdahl_gpt2_str`\times\)

The *Bandwidth Hog* archetype suffers more from serial bottlenecks. Even infinite accelerator speed yields only $1/(1-p)$ = `{python} AmdahlH100.amdahl_gpt2_ceil_str`$\times$ maximum speedup. This is *why* large language model (LLM) inference optimization focuses on reducing the serial fraction (batching, speculative decoding) rather than raw hardware speed.

:::

These examples reveal that the critical question for any hardware optimization is not "how fast is the chip?" but rather: *is this workload limited by how fast we can compute, or how fast we can move data?* The answer determines which accelerator to choose, which optimizations matter, and whether a 10$\times$ more powerful chip will help. The *roofline model* (introduced formally in @sec-machine-foundations-roofline-model-2529 and applied to AI workloads in @sec-hardware-acceleration-roofline-model-42ff) provides the analytical framework for answering this question. It plots an operation's *arithmetic intensity*[^fn-arithmetic-intensity-roofline], defined as the ratio of floating-point operations to bytes of memory traffic (FLOP/byte), against hardware capabilities, revealing whether performance is capped by compute or bandwidth. A dense matrix multiplication with high arithmetic intensity benefits from more TFLOP/s; a LayerNorm with low arithmetic intensity benefits from more memory bandwidth. High-reuse ResNet-50 convolutions can cross into the compute-bound regime while GPT-2's attention layers are memory bound, and this distinction is precisely *why* these architectures require different optimization strategies.

[^fn-arithmetic-intensity-roofline]: **Arithmetic Intensity**: The ratio of compute operations performed for each byte of data moved from memory (FLOP/byte). This metric provides the direct, quantitative answer to the text's central question: workloads with high arithmetic intensity, such as well-tiled convolutions and GEMMs above the hardware ridge point, are compute bound and accelerate with more TFLOP/s. Workloads with low intensity, like GPT-2's attention layers (<10 FLOP/byte), are memory bound, making faster chips irrelevant without more bandwidth. \index{Arithmetic Intensity!roofline diagnostic}
...
The history of specialized hardware reveals recurring design patterns that explain *why* today's accelerators take their current form.
...
**The shock**: The TPUv1 was not just slightly faster; it was **15--30$\times$ faster** on inference workloads and achieved **30--80$\times$ better performance-per-watt**.
...
[^fn-tpu-origin]: **TPU (Tensor Processing Unit)**: Google developed the TPU when projections showed that serving voice search on general-purpose CPUs would require doubling its data center footprint. The design made a radical trade-off, stripping away nonessential logic like caches and branch predictors to dedicate silicon to a massive $256{\times}256$ systolic array for matrix multiplication. This specialization delivered an immediate 15--30$\times$ improvement in throughput-per-watt over contemporary GPUs, validating the domain-specific approach. \index{TPU!origin}
...
The scale of this challenge becomes stark in @fig-systems-gap, which plots the *systems gap*: the divergence between what models demand and what hardware naturally provides. In the plotted normalization, GPU supply, often framed as Huang's Law,[^fn-huangs-law-gpu] rises about 1.7$\times$ per year while model demand rises about 6$\times$ per year, so the systems gap widens by roughly 3--4$\times$ each year.
...
# │
# │ Imports: numpy (np), mlsysim.core.viz (viz), mlsysim.fmt (fmt)
# │ Exports: SystemsGapRates.*
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
...
class SystemsGapRates:
    """Annualized growth rates implied by the systems-gap figure assumptions."""
    ...
    gpu_growth_per_year_str = fmt(gpu_growth_per_year, precision=1, commas=False)
    demand_growth_per_year_str = fmt(demand_growth_per_year, precision=1, commas=False)
    demand_supply_gap_per_year_str = fmt(demand_supply_gap_per_year, precision=1, commas=False)
...
ax.text(gap_x, gap_y, "THE SYSTEMS GAP\n(Closed by Parallelism,\nArchitecture & Co-design)",
        ha='center', va='center', fontweight='bold', color=COLORS['VioletLine'],linespacing=1.4, fontsize=8,
...
[^fn-moores-law-scaling]: **Moore's Law**: The consequence for ML is not just slower hardware improvement but a structurally widening gap: in the illustrative assumptions used by @fig-systems-gap, model compute demand grows roughly `{python} SystemsGapRates.demand_growth_per_year_str`$\times$ per year while accelerator peak supply improves roughly `{python} SystemsGapRates.gpu_growth_per_year_str`$\times$ per year, widening the demand/supply gap by about `{python} SystemsGapRates.demand_supply_gap_per_year_str`$\times$ per year. This divergence makes algorithmic efficiency techniques—model compression, quantization, sparsity—structurally necessary rather than optional optimizations. \index{Moore's Law!ML systems consequence}
...
The two overlapping curves in @fig-tech-s-curve illustrate how general-purpose computing has entered its saturation phase, and the industry is now riding the steep take-off of a new S-curve driven by domain-specific architectures.
...
fig, ax, COLORS, plt = viz.setup_plot(figsize=(10, 3.80))
...
ax.annotate("The Paradigm Shift\n(Hardware-Software Co-design)",
            ...
            fontsize=8,ha='center', fontweight='bold', color=COLORS['RedLine'],
...
ax.text(2028.5, 900, "The Systems\nGap (~100x)", ha='right', va='center', fontsize=8, fontweight='bold', color=COLORS['primary'])
...
The trend toward specialization continues to accelerate, with new architectures emerging for an expanding range of domains. Genomics processing benefits from custom accelerators that optimize sequence alignment and variant calling, reducing the time required for DNA analysis [@Shang2018GenomicsAccel]. Similarly, blockchain computation has produced application-specific integrated circuits (ASICs)[^fn-asic-flexibility] optimized for cryptographic hashing, substantially increasing the efficiency of mining operations [@bedford_taylor2017].
...
The engineering lesson is clear from this trajectory: the era of "free" performance gains from general-purpose scaling is over. For decades, software engineers could rely on Moore's Law to accelerate existing code without architectural changes. The breakdown of Dennard scaling forced a decisive change: we can no longer wait for faster CPUs to solve computational bottlenecks but must instead design the hardware to fit the algorithm. This necessity of hardware-software co-design is why modern AI engineering requires deep understanding of the underlying silicon. Performance is now determined by how well the algorithm's memory access patterns and parallelism map to the specialized physical structures of domain-specific architectures.
...
# │
# │ Imports: mlsysim.core.constants (A100_FLOPS*, Hardware.Mobile.iPhone15Pro.compute.peak_flops)
# │ Exports: CpuMlInefficiency.cpu_utilization_min_str,
# │          CpuMlInefficiency.cpu_utilization_max_str,
# │          CpuMlInefficiency.cpu_gflops_str,
# │          CpuMlInefficiency.a100_tflops_fp16,
# │          CpuMlInefficiency.a100_tflops_tf32,
# │          CpuMlInefficiency.mobile_tops
# └─────────────────────────────────────────────────────────────────────────────
...
class CpuMlInefficiency:
    ...
    cpu_gflops_value = 100                           # Typical CPU GFLOP/s for ML

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # (values are already given; no derivation needed)

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cpu_utilization_min_str = fmt(cpu_utilization_min_value, precision=1, allow_zero=True, commas=False)
    cpu_utilization_max_str = fmt(cpu_utilization_max_value, precision=1, allow_zero=True, commas=False)
    cpu_gflops_str = fmt(cpu_gflops_value, precision=1, allow_zero=True, commas=False)

    # A100/Mobile specs for footnote comparison
    a100_tflops_fp16 = f"{Hardware.Cloud.A100.compute.peak_flops.m_as(TFLOPs/second):.0f}"
    a100_tflops_tf32 = f"{Hardware.Cloud.A100.compute.precision_flops["tf32"].m_as(TFLOPs/second):.0f}"
...
class WeightMatrixCalc:
    ...
    # OUTPUT
    wm_params_str = fmt(wm_params_value, precision=1, allow_zero=True, commas=True)
    wm_out_str = fmt(wm_out_value, precision=1, allow_zero=True, commas=False)
    wm_in_str = fmt(wm_in_value, precision=1, allow_zero=True, commas=False)
    wm_batch_str = fmt(wm_batch_value, precision=1, allow_zero=True, commas=False)
    wm_batch_macs_m_str = fmt(wm_batch_macs_value / 1e6, precision=1, commas=False)
    wm_vector_chunks_str = fmt(wm_vector_chunks_value, precision=1, allow_zero=True, commas=True)
```
...
::: {#lst-dense_layer_def lst-cap="**Dense Layer Abstraction**: High-level framework APIs encapsulate `{python} WeightMatrixCalc.wm_params_str` multiply-accumulate operations (`{python} WeightMatrixCalc.wm_in_str` inputs times `{python} WeightMatrixCalc.wm_out_str` outputs) in a single function call, hiding the computational complexity from developers while enabling automatic hardware optimization."}
...
::: {#lst-loop_level_dense lst-cap="**Processor-Level Execution**: Nested loops reveal the $\mathcal{O}(B \times d_{\text{in}} \times d_{\text{out}})$ multiply-accumulate operations that accelerators must execute, with `{python} WeightMatrixCalc.wm_batch_macs_m_str` million MACs for $B$=`{python} WeightMatrixCalc.wm_batch_str`, $d_{\text{in}}$=`{python} WeightMatrixCalc.wm_in_str`, $d_{\text{out}}$=`{python} WeightMatrixCalc.wm_out_str` configurations."}
...
\index{Vector Operations!hardware acceleration}\index{SIMD!data-level parallelism}Vector operations provide the first level of hardware acceleration by processing multiple data elements simultaneously. Recall the nested-loop structure exposed in @lst-loop_level_dense: a batch of `{python} WeightMatrixCalc.wm_batch_str` samples through a `{python} WeightMatrixCalc.wm_in_str`-to-`{python} WeightMatrixCalc.wm_out_str` dense layer requires `{python} WeightMatrixCalc.wm_batch_macs_m_str` million multiply-accumulate operations. A traditional scalar processor executes these one at a time, loading an input value and a weight value, multiplying them, and accumulating the result. This sequential approach is hopelessly inefficient for neural networks that repeat this pattern across millions of parameters.

Vector processing units solve this by operating on multiple data elements simultaneously. @lst-riscv_vector_mac reveals these capabilities through RISC-V\index{RISC-V!vector extensions}[^fn-riscv-ai-customization], the fifth generation of the reduced instruction set computer (RISC) architecture, in assembly code, where a single instruction processes eight data elements in parallel.
...
::: {#lst-riscv_vector_mac lst-cap="**Vectorized Multiply-Accumulate Loop**: This loop showcases how RISC-V vector instructions enable efficient batch processing by performing 8 multiply-add operations simultaneously, reducing computational latency in neural network training. [@waterman2013]"}
...
The key insight from this assembly sequence is that the fused multiply-accumulate instruction (`vfmacc.vv`) performs the same operation that would require separate multiply and add instructions on a scalar processor, while the vector load instructions (`vle32.v`) amortize memory access overhead across multiple data elements. This vector implementation processes eight data elements in parallel, reducing both computation time and energy consumption. Vector load instructions transfer eight values simultaneously, maximizing memory bandwidth utilization. The vector multiply-accumulate instruction processes eight pairs of values in parallel, dramatically reducing the total instruction count from `{python} WeightMatrixCalc.wm_batch_macs_m_str` million scalar operations to roughly `{python} WeightMatrixCalc.wm_vector_chunks_str` vector chunks.
...
This pervasive pattern of matrix multiplication has direct implications for hardware design: accelerators need specialized units that can handle these computations at scale. @lst-matrix_unit demonstrates how modern processors implement dedicated matrix units that process entire $16{\times}16$ blocks simultaneously, achieving 32$\times$ higher throughput than vector processing alone.
...
Special Function Units (SFUs)\index{Special Function Unit!SFU} provide dedicated hardware for these nonlinear computations, completing the trio of core processing primitives. The need for such units is not new—floating-point co-processors in the 1970s-1980s and SSE/NEON instruction set extensions in the 1990s addressed similar demands for scientific computing [@broesch1997; @palmer1980intel8087]. Neural networks have intensified this demand because activation functions, normalization layers, and softmax transformations appear after every linear layer, making them a throughput bottleneck rather than an occasional convenience.
...
Even a simple rectified linear unit (ReLU) activation introduces branching logic that can disrupt instruction pipelining. @lst-traditional_overhead demonstrates these inefficiencies.
...
Processor architectures continue to expand SIMD capabilities to accommodate increasing computational demands. Intel's Advanced Matrix Extensions (AMX) [@intel2021amx] and ARM's SVE2 architecture [@stephens2017] provide flexible SIMD execution, enabling software to scale across different hardware implementations.
...
class AttentionHeadTensorCoreCalc:
    ...
    flops = 2 * macs
    scalar_ms = flops / scalar_clock_hz * 1000
    ...
    macs_m_str = fmt(macs / 1e6, precision=1, allow_zero=True, commas=False)
    flops_m_str = fmt(flops / 1e6, precision=1, allow_zero=True, commas=False)
    scalar_ms_str = fmt(scalar_ms, precision=1, allow_zero=True, commas=False)
    tensor_speedup_str = fmt(tensor_speedup, precision=1, allow_zero=True, commas=False)
```

Consider a single transformer attention head computing the $Q \times K^T$ product for a `{python} AttentionHeadTensorCoreCalc.seq_len_str`-token sequence with `{python} AttentionHeadTensorCoreCalc.head_dim_str`-dimensional embeddings. This operation requires multiplying a $2048{\times}64$ matrix by a $64{\times}2048$ matrix: roughly `{python} AttentionHeadTensorCoreCalc.macs_m_str` million multiply-accumulate operations, or about `{python} AttentionHeadTensorCoreCalc.flops_m_str` million FLOPs when the multiply and add are counted separately. On a scalar processor executing one FLOP per cycle at 2 GHz, this single attention head would take about `{python} AttentionHeadTensorCoreCalc.scalar_ms_str` milliseconds. A GPU's SIMT execution reduces this to roughly 34 milliseconds through thread-level parallelism [@blake2010], but a tensor core, processing entire $16{\times}16$ matrix tiles per instruction, completes the same operation in under 0.5 milliseconds: a roughly `{python} AttentionHeadTensorCoreCalc.tensor_speedup_str`$\times$ improvement over scalar execution. This dramatic speedup arises not from faster clock speeds but from a fundamentally different approach to organizing computation around matrix blocks rather than individual elements.
...
Tensor processing unit architectures differ based on design priorities. Some GPU families incorporate tensor cores\index{Tensor Core!precision support} optimized for general-purpose deep learning acceleration. TPU-style designs use large-scale matrix units arranged in systolic arrays to maximize sustained training throughput on dense tensor kernels. Mobile NPUs[^fn-npu-on-device-strategy] integrate smaller matrix processors optimized for low-power inference workloads, while some server CPUs introduce matrix instruction extensions (AMX-class tiles) designed for data center inference and mixed workloads.
...
\begin{axis}[
  ...
  precision=1, allow_zero=True
    ...
    ymode=log,
    log basis y=10,
    xtick={2012,2014,2016,2017,2020,2022},
...
\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2014,6.5e0) {\textbf{M40} \\ {\color{black!60}6.8 TFLOP/s FP32}};

\node[anchor=south, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2015.35,0.5e2) {\textbf{P40} \\ {\color{black!60}47 TOPS INT8}};

\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2017.3,1.2e2) {\textbf{V100} \\ {\color{black!60}125 TFLOP/s FP16}};

\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2020.4,1.0e3) {\textbf{A100} \\ {\color{black!60}1,248 TOPS INT8}};

\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2021.0,0.95e4) {\textbf{H100} \\ {\color{black!60}4,000 TFLOP/s FP8}};
%
...
# │
# │ Imports: mlsysim.core.constants (ENERGY_DRAM_ACCESS_PJ, SYSTOLIC_ARRAY_DIM)
# │ Exports: SystolicEnergy.energy_ratio_str, SystolicEnergy.vector_energy_str,
# │          SystolicEnergy.systolic_energy_str,
# │          AcceleratorEfficiencyAnchor.systolic_macs_cycle_str,
# │          AcceleratorEfficiencyAnchor.energy_dividend_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, MarkdownStr, check
...
class AcceleratorEfficiencyAnchor:
    ...
    systolic_macs_cycle_str = fmt(systolic_macs_cycle, precision=1, allow_zero=True)
    # Custom "x" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    energy_dividend_str = fmt(energy_dividend, precision=1, allow_zero=True, commas=False, suffix="x")
...
class SystolicEnergy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    dram_access_str = fmt(dram_pj, precision=1, allow_zero=True, commas=False)
    systolic_size_str = fmt(array_dim, precision=1, allow_zero=True, commas=False)
    vector_accesses_str = fmt(vector_dram_accesses, precision=1, allow_zero=True, commas=False)
    compute_energy_str = fmt(mac_pj, precision=1, allow_zero=True, commas=False)
    vector_energy_str = fmt(e_vector, precision=1, allow_zero=True)
    ...
    energy_ratio_str = fmt(efficiency_ratio, precision=1, allow_zero=True, commas=False)
```
...
1.  **Vector unit**: Loads $A$, loads $B$, computes $A \times B + C$, writes $C$.
    ...
    - **Energy**: ≈ `{python} SystolicEnergy.vector_accesses_str` $\times$ `{python} SystolicEnergy.dram_access_str` pJ + `{python} SystolicEnergy.compute_energy_str` pJ (compute) = **`{python} SystolicEnergy.vector_energy_str` pJ/operation**.
...
2.  **Systolic Array (`{python} SystolicEnergy.systolic_size_str` $\times$ `{python} SystolicEnergy.systolic_size_str` size)**: Loads A and B once at the edges. Data "pulses" through `{python} SystolicEnergy.systolic_size_str` processing elements.
    ...
    - **Energy**: ≈ `{python} SystolicEnergy.systolic_access_str` $\times$ `{python} SystolicEnergy.dram_access_str` pJ + `{python} SystolicEnergy.compute_energy_str` pJ (compute) ≈ **`{python} SystolicEnergy.systolic_energy_str` pJ/operation**.

**Systems insight**: A systolic array is **`{python} SystolicEnergy.energy_ratio_str`$\times$ more energy-efficient** than a naive vector unit for large matrix multiplications.

- Concretely, a $128{\times}128$ array can achieve over `{python} AcceleratorEfficiencyAnchor.systolic_macs_cycle_str` MACs per cycle with `{python} AcceleratorEfficiencyAnchor.energy_dividend_str` better energy efficiency than a standard vector unit by pulsing data through processing elements instead of repeatedly loading it from DRAM.
...
class TilingPrinciple:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    layer_dim_str = fmt(layer_dim, precision=1, allow_zero=True)
    array_dim_str = fmt(array_dim, precision=1, allow_zero=True, commas=False)
    tile_count_str = fmt(int(total_tiles), precision=1, allow_zero=True)
    reuse_factor_str = fmt(reuse_factor, precision=1, allow_zero=True, commas=False)
```
...
This tiling pattern is the central mechanism behind high-performance ML systems. It allows the hardware to maintain high system efficiency $(\eta_{\text{hw}})$ by ensuring that for every byte loaded from main memory, the data is reused `{python} TilingPrinciple.reuse_factor_str`$\times$ within the systolic grid. An engineer who understands tiling understands the "silicon contract": if a layer's dimensions are not multiples of the tile size (for example, a width of 129 on a 128 array), the system pays a *fringe tax* in underutilized silicon, where 127 units sit idle while one unit finishes the "remainder" tile.
...
: **Systolic-Array Dataflow Strategies**: Three stationary-operand choices for systolic arrays, the reuse pattern each maximizes, and the workload class that benefits, showing how a fixed dataflow choice hard-codes an accelerator's affinity for specific models. {#tbl-hw-acceleration-systolic-dataflow}
...
# │
# │ Imports: mlsysim.core.constants (SYSTOLIC_ARRAY_DIM, Hardware.Cloud.TPUv4.memory.bandwidth),
# │          mlsysim.book (fmt)
# │ Exports: SystolicOpsCalc.systolic_dim_str, SystolicOpsCalc.systolic_ops_str,
# │          SystolicOpsCalc.tpuv4_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
class SystolicOpsCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    systolic_dim_str = fmt(systolic_dim_value, precision=1, allow_zero=True, commas=False)
    systolic_ops_str = fmt(systolic_ops_value, precision=1, allow_zero=True)
    tpuv4_bw_str     = fmt(int(tpuv4_bw_gbs), precision=1, allow_zero=True)
```

A \(`{python} SystolicOpsCalc.systolic_dim_str`{\times}`{python} SystolicOpsCalc.systolic_dim_str`\) systolic array capable of `{python} SystolicOpsCalc.systolic_ops_str` operations per cycle requires continuous data feed to maintain utilization. Each cycle demands fresh input activations and weight parameters that must traverse from off-chip memory through on-chip buffers to the array edges. The TPU v4's `{python} SystolicOpsCalc.tpuv4_bw_str` GB/s HBM2 bandwidth enables high utilization, but even this substantial bandwidth becomes limiting when processing large transformer models where memory requirements exceed on-chip capacity.
...
Newer architectures incorporate a growing diversity of numerical formats, reflecting the need for greater flexibility across different AI workloads. Future AI accelerators will likely continue expanding support for adaptive precision, balancing computational efficiency against model accuracy.
...
\index{NVLink!intra-node scaling}\index{PCIe!host-to-device transfers}Mastery of the single-machine stack requires understanding how bits move between GPUs and the CPU. In the 1–8 GPU regime, scaling is achieved through high-speed intra-node interconnects that mitigate the memory wall. We define this hierarchy through the **bandwidth taper**\index{Bandwidth!taper, interconnect hierarchy}: the principle that data movement speed must decrease as we move further from the compute units.

1.  **HBM (2–3 TB/s)**: The fastest layer, feeding weights from local device memory to the PE grid.
2.  **NVLink (900 GB/s)**: The GPU-to-GPU interconnect. In a single-machine system, NVLink allows eight GPUs to share data at speeds approaching on-device bandwidth, enabling single-node model parallelism [@NVIDIA2020nvlink].
3.  **PCIe Gen5 (64 GB/s)**: The host-to-device interface. This is the "OS" layer where **DMA (Direct Memory Access)** moves data from CPU RAM to GPU VRAM. It is 15--30$\times$ slower than NVLink, making it the primary bottleneck for data loading.

```{python}
from mlsysim import *
from mlsysim.core.constants import *
#| label: interconnect-hierarchy-calc
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ INTERCONNECT HIERARCHY: THE BANDWIDTH TAPER
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: §Intra-Node Interconnects—bandwidth taper discussion
# │
# │ Goal: Demonstrate the 10-100× bandwidth gaps in the interconnect hierarchy.
# │ Show: HBM (3.3 TB/s) > NVLink (900 GB/s) > PCIe (64 GB/s) > Network (50 GB/s).
# │ How: Compare H100 specs via Hardware Digital Twin.
# │
# │ Imports: Hardware.Cloud.H100, mlsysim.constants (GB, second, TB,
# │          PCIE_GEN5_BW, INFINIBAND_NDR_BW)
# │ Exports: InterconnectHierarchy.hbm_bw_str,
# │          InterconnectHierarchy.nvlink_bw_str,
# │          InterconnectHierarchy.pcie_bw_str,
# │          InterconnectHierarchy.net_bw_str,
# │          InterconnectBandwidth.gap_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
...
class InterconnectBandwidth:
    ...
    hbm_bw = h_h100.memory_bw.m_as(GB/second)
    nvlink_bw = NVLINK_H100_BW.m_as(GB/second)
    pcie_bw = PCIE_GEN5_BW.m_as(GB/second)
    net_bw = INFINIBAND_NDR_BW.m_as(GB/second)

    # Invariant: HBM > NVLink > PCIe > Network
    check(hbm_bw > nvlink_bw > pcie_bw > net_bw, "Bandwidth taper violated!")

    # Custom unit suffixes (TB/s, GB/s, x) → MarkdownStr per recipe Rule 1 escape-hatch
    hbm_bw_str = fmt(h_h100.memory_bw.m_as(TB/second), precision=1, commas=False, suffix=" TB/s")
    nvlink_bw_str = fmt(int(nvlink_bw), precision=1, allow_zero=True, commas=False, suffix=" GB/s")
    pcie_bw_str = fmt(int(pcie_bw), precision=1, allow_zero=True, commas=False, suffix=" GB/s")
    net_bw_str = fmt(int(net_bw), precision=1, allow_zero=True, commas=False, suffix=" GB/s")
    gap_ratio_str = fmt(int(hbm_bw/pcie_bw), precision=1, allow_zero=True, commas=False, suffix="x")

# Note: Use InterconnectHierarchy.hbm_bw_str directly.
```

::: {#nbk-hw-acceleration-bandwidth-taper .callout-notebook title="The bandwidth taper"}

The **single-machine stack** is governed by an orders-of-magnitude bandwidth hierarchy. On an NVIDIA H100 node:

*   **HBM3**: `{python} InterconnectBandwidth.hbm_bw_str`
*   **NVLink 4.0**: `{python} InterconnectBandwidth.nvlink_bw_str`
*   **PCIe Gen5**\index{PCIe Gen5!bandwidth scaling}: `{python} InterconnectBandwidth.pcie_bw_str`
*   **Network (NDR)**: `{python} InterconnectBandwidth.net_bw_str`

**Systems insight**: The `{python} InterconnectBandwidth.gap_ratio_str` gap between HBM and PCIe means that any data transfer from the CPU is a catastrophic performance event. **NVLink** is the only way to scale computation across multiple GPUs without hitting the "PCIe Wall."
...
# │
# │ Imports: mlsysim.core.constants (V100/A100/H100/TPUV5P FLOP/s and bandwidth,
...
# │          TFLOPs, second, GB, TB, USD)
# │          mlsysim.Hardware (Cloud.V100, Cloud.A100, Cloud.H100, Cloud.TPUv5p)
# │ Exports: AcceleratorEconomics.v100_tflops_str, AcceleratorEconomics.v100_bw_str,
# │          AcceleratorEconomics.v100_price_str, AcceleratorEconomics.v100_pp_str,
# │          AcceleratorEconomics.a100_tflops_fp16_str, AcceleratorEconomics.a100_bw_str,
# │          AcceleratorEconomics.a100_bw_tbs_str, AcceleratorEconomics.a100_price_str,
# │          AcceleratorEconomics.a100_pp_str, AcceleratorEconomics.h100_tflops_tf32_str,
# │          AcceleratorEconomics.h100_tflops_fp16_str, AcceleratorEconomics.h100_bw_str,
# │          AcceleratorEconomics.h100_bw_tbs_str, AcceleratorEconomics.h100_price_str,
# │          AcceleratorEconomics.h100_pp_str, AcceleratorEconomics.h100_tflops_fp8_str,
# │          AcceleratorEconomics.tpuv4_tflops_str,
# │          AcceleratorEconomics.tpuv4_bw_str, AcceleratorEconomics.tpu_price_str,
# │          AcceleratorEconomics.tpu_pp_str, AcceleratorEconomics.gaudi_tflops_str,
# │          AcceleratorEconomics.gaudi_bw_str, AcceleratorEconomics.gaudi_price_str,
# │          AcceleratorEconomics.gaudi_pp_str
# │
...
class AcceleratorEconomics:
    ...
    price_v100  = Hardware.Cloud.V100.unit_cost.m_as(USD)   # older generation
    price_a100  = Hardware.Cloud.A100.unit_cost.m_as(USD)   # current workhorse
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # V100 specs
    v100_tflops_str = fmt(h_v100.peak_flops.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    v100_bw_str     = fmt(h_v100.memory_bw.m_as(GB/second), precision=1, allow_zero=True, commas=False)
    # Currency prefix → MarkdownStr per recipe Rule 1 escape-hatch
    v100_price_str = fmt(price_v100, precision=1, allow_zero=True, prefix="~$")
    v100_pp_str    = fmt(v100_ratio, precision=1, allow_zero=True, commas=False, prefix="$", suffix="/(TFLOP/s)")

    # A100 specs (also used later in roofline sections)
    a100_tflops_fp16_str = fmt(h_a100.peak_flops.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    a100_bw_str          = fmt(h_a100.memory_bw.m_as(GB/second), precision=1, allow_zero=True)
    a100_bw_tbs_str      = fmt(h_a100.memory_bw.m_as(TB/second), precision=2, commas=False)
    a100_price_str   = fmt(price_a100, precision=1, allow_zero=True, prefix="~$")
    a100_pp_str      = fmt(a100_ratio, precision=1, allow_zero=True, commas=False, prefix="$", suffix="/(TFLOP/s)")

    # H100 specs
    h100_tflops_tf32_str = fmt(h_h100.compute.precision_flops["tf32"].m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    h100_tflops_fp16_str = fmt(h_h100.peak_flops.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    h100_tflops_fp8_str  = fmt(Hardware.Cloud.H100.compute.precision_flops["fp8"].m_as(TFLOPs/second), precision=1, allow_zero=True)
    h100_bw_str          = fmt(h_h100.memory_bw.m_as(GB/second), precision=1, allow_zero=True)
    h100_bw_tbs_str      = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False)
    h100_price_str   = fmt(price_h100, precision=1, allow_zero=True, prefix="~$", suffix="-30,000")
    h100_pp_str      = fmt(h100_ratio, precision=1, allow_zero=True, commas=False, prefix="~$", suffix="/(TFLOP/s)")

    # TPUv4 specs
    tpuv4_tflops_str = fmt(h_tpu.peak_flops.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    tpuv4_bw_str     = fmt(h_tpu.memory_bw.m_as(GB/second), precision=1, allow_zero=True)
    tpu_price_str = fmt(price_tpu, precision=1, allow_zero=True, prefix="~$", suffix="*")
    tpu_pp_str    = fmt(tpu_ratio, precision=1, allow_zero=True, commas=False, prefix="~$", suffix="/(TFLOP/s)")

    # Gaudi 2 specs
    gaudi_tflops_str = fmt(gaudi_tf, precision=1, allow_zero=True, commas=False)
    gaudi_bw_str     = fmt(gaudi_bw_value, precision=1, allow_zero=True)
    gaudi_price_str = fmt(price_gaudi, precision=1, allow_zero=True, prefix="~$")
    gaudi_pp_str    = fmt(gaudi_ratio, precision=1, allow_zero=True, commas=False, prefix="$", suffix="/(TFLOP/s)")
```
...
|:-----------------------------------|:------------------------------------------------|--------------------------------------------------------------------:|--------------------------------------------------:|:---------------------------------------------|
| **NVIDIA V100**\index{NVIDIA!V100} | `{python} AcceleratorEconomics.v100_price_str`  |             `{python} AcceleratorEconomics.v100_tflops_str` TFLOP/s |  `{python} AcceleratorEconomics.v100_bw_str` GB/s | `{python} AcceleratorEconomics.v100_pp_str`  |
| **NVIDIA A100**\index{NVIDIA!A100} | `{python} AcceleratorEconomics.a100_price_str`  |        `{python} AcceleratorEconomics.a100_tflops_fp16_str` TFLOP/s |  `{python} AcceleratorEconomics.a100_bw_str` GB/s | `{python} AcceleratorEconomics.a100_pp_str`  |
| **NVIDIA H100**                    | `{python} AcceleratorEconomics.h100_price_str`  | `{python} AcceleratorEconomics.h100_tflops_tf32_str` TFLOP/s (TF32) |  `{python} AcceleratorEconomics.h100_bw_str` GB/s | `{python} AcceleratorEconomics.h100_pp_str`  |
| **Google TPUv4**                   | `{python} AcceleratorEconomics.tpu_price_str`   |     `{python} AcceleratorEconomics.tpuv4_tflops_str` TFLOP/s (BF16) | `{python} AcceleratorEconomics.tpuv4_bw_str` GB/s | `{python} AcceleratorEconomics.tpu_pp_str`   |
| **Intel Gaudi 2**                  | `{python} AcceleratorEconomics.gaudi_price_str` |      `{python} AcceleratorEconomics.gaudi_tflops_str` TFLOP/s (FP8) | `{python} AcceleratorEconomics.gaudi_bw_str` GB/s | `{python} AcceleratorEconomics.gaudi_pp_str` |
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.B200.compute.precision_flops["fp4"], PFLOPs, second)
# │ Exports: B200FP4Specs.dense_pflops_str, B200FP4Specs.sparse_pflops_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
The execution units examined in previous sections (SIMD units, tensor cores, and systolic arrays) provide impressive computational throughput: modern accelerators achieve 100 to 1000 TFLOP/s for neural network operations. Yet these theoretical capabilities remain unrealized in practice when memory subsystems cannot supply data at sufficient rates. This constraint, termed the AI memory wall, represents the dominant bottleneck in real-world accelerator performance.
...
***The AI Memory Wall***\index{AI Memory Wall!definition} is the performance constraint that arises when arithmetic throughput $(R_{\text{peak}})$ outpaces memory bandwidth $(\text{BW})$.
...
\index{Von Neumann, John!stored-program architecture}The underlying cause of this wall is physical: the Von Neumann[^fn-von-neumann-bottleneck] Bottleneck, which has constrained computing since 1945, means that moving data costs orders of magnitude more energy than processing it.

[^fn-von-neumann-bottleneck]: **Von Neumann Bottleneck**: The physical separation of the processor from its memory forces all instructions and data to traverse an energy-intensive bus. This distance is the direct cause of the high energy cost of data movement; every byte must be fetched, paying a physical tax. Accessing a value from external DRAM can cost over 20,000× more energy than performing an 8-bit integer operation on that value [@horowitz2014]. \index{Von Neumann Bottleneck!ML accelerator constraint}
...
ax.text(80, 3.1, "~128× Cost (The Memory Wall)", color=COLORS['RedLine'], ha='center', fontsize=9, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.5))
plt.show()
plt.close()
```
...
As @fig-energy-hierarchy illustrates, the severity of this constraint becomes apparent when examining scaling trends. Over the past two decades, peak computational capabilities have grown substantially faster than DRAM bandwidth [@gholami2024]. This divergence creates a widening gap where accelerators possess massive computational power but cannot access data quickly enough to use it. Representative high-end accelerators can deliver on the order of $10^3$ TFLOP/s of peak tensor throughput (e.g., NVIDIA H100 delivering `{python} AcceleratorEconomics.h100_tflops_fp16_str` TFLOP/s in FP16 or nearly 2,000 TFLOP/s in FP8) while providing approximately `{python} AcceleratorEconomics.h100_bw_tbs_str` TB/s of memory bandwidth. This implies a ridge point on the order of $10^2$ FLOP/byte to fully use compute, which can exceed the arithmetic intensity of many practical neural network workloads.
...
Different paradigms inhabit different regions of this "memory wall." We quantify this using the **hardware balance $(\text{AI}_{\text{ridge}})$\index{Hardware Balance}**, defined as the arithmetic intensity required to hide the cost of fetching one byte of data:
...
fig, ax, COLORS, plt = viz.setup_plot(figsize=(10, 4.0))
# =============================================================================
# DATA
# =============================================================================
...
ax.annotate('', xy=(2023, 1e13), xytext=(2023, 1e4),
            arrowprops=dict(arrowstyle='<->', color=COLORS['primary'], lw=1.5))
ax.text(2022.2, 3e6, 'Memory Wall', rotation=90, va='bottom', fontsize=9, color=COLORS['primary'], fontweight='bold', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.5))
...
The imbalance has a direct architectural consequence visible in @fig-rising-ridge: the hardware's "Ridge Point," the arithmetic intensity required to fully saturate the chip, has climbed sharply and remains high, pushing sparse and low-reuse operations further into the memory-bound regime on modern accelerators.
...
# │
# │ Imports: mlsysim.core.viz (viz), mlsysim.core.constants
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(2019, 50, "Memory-Rich Zone\n(Legacy Ops Safe)", color=COLORS['BlueLine'], ha='center', va='center', fontsize=8, fontweight='bold', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.5))
ax.axhspan(100, 600, color=COLORS['OrangeL'], alpha=0.1)
ax.text(2019, 350, "Compute-Dense Zone\n(Transformers Required)", color=COLORS['OrangeLine'], ha='center', va='center', fontsize=8, fontweight='bold', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.5))
...
# │
# │ Imports: mlsysim.core.constants (BYTES_FP16, ENERGY_DRAM_PJ_PER_BYTE,
# │          LATENCY_HBM3, LATENCY_L2_CACHE, LATENCY_L1_REGISTER,
# │          Hardware.Cloud.A100.compute.peak_flops, TFLOPs, second),
# │          mlsysim.book (fmt)
# │ Exports: TensorLifecycleCalc.kws_tensor_str,
# │          TensorLifecycleCalc.kws_samples_str,
# │          TensorLifecycleCalc.kws_bytes_str,
# │          TensorLifecycleCalc.dram_energy_pj_bit_str,
# │          TensorLifecycleCalc.latency_hbm_str,
# │          TensorLifecycleCalc.latency_l2_str,
# │          TensorLifecycleCalc.latency_l1_str,
# │          TensorLifecycleCalc.a100_tflops_fp16_str,
# │          TensorLifecycleCalc.reg_energy_pj_bit_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, MarkdownStr
...
class TensorLifecycleCalc:
    ...
    kws_samples_str        = fmt(kws_samples_value, precision=1, allow_zero=True, commas=True)
    kws_bytes_str          = fmt(kws_bytes_fp16_value, precision=1, allow_zero=True, commas=False)
    dram_energy_pj_bit_str = fmt(dram_energy_pj_bit_value, precision=1, allow_zero=True, commas=False)
    latency_hbm_str        = fmt(LATENCY_HBM3.m_as('ns'), precision=1, allow_zero=True, commas=False)
    latency_l2_str         = fmt(LATENCY_L2_CACHE.m_as('ns'), precision=1, allow_zero=True, commas=False)
    latency_l1_str         = fmt(LATENCY_L1_REGISTER.m_as('ns'), precision=1, allow_zero=True, commas=False)
    a100_tflops_fp16_str   = fmt(a100_fp16_tflops_value, precision=1, allow_zero=True, commas=False)
    # Plain-string literal → MarkdownStr per recipe Rule 1 (plain string literal _str)
    reg_energy_pj_bit_str  = MarkdownStr("0.1")
```

::: {#lhs-hw-acceleration-life-tensor-kws-journey .callout-lighthouse title="Life of a tensor: The KWS journey"}

Recall the Keyword Spotting lighthouse summarized in @tbl-ml-systems-lighthouse-archetypes. Here is a one-second audio clip's physical path through the hardware during inference:

1.  **DRAM (HBM)**: The tensor starts here.
    *   **Size**: `{python} TensorLifecycleCalc.kws_samples_str` samples $\times$ `{python} TensorLifecycleCalc.kws_bytes_str` bytes (FP16) = **`{python} TensorLifecycleCalc.kws_tensor_str` KB**.
    *   **Latency**: Fetching this from off-chip memory takes **~`{python} TensorLifecycleCalc.latency_hbm_str` ns** (plus queuing delay).
    *   **Energy**: Cost is **~`{python} TensorLifecycleCalc.dram_energy_pj_bit_str` pJ/bit**. High cost.

2.  **L2 Cache**: The GPU's DMA engine pulls it here.
    *   **Latency**: ~`{python} TensorLifecycleCalc.latency_l2_str` ns.
...
3.  **L1 Cache/Shared Memory**: A specific SM claims a tile of the audio.
    *   **Latency**: ~`{python} TensorLifecycleCalc.latency_l1_str` ns.
...
4.  **Registers**: The Tensor Core operates here.
    ...
    *   **Throughput**: `{python} TensorLifecycleCalc.a100_tflops_fp16_str` TFLOP/s.
...
When $T_{\text{mem}} > T_{\text{compute}}$, the system becomes memory bound. This imbalance means that the processing elements spend more time waiting for data than performing computations, demonstrating the need for memory-optimized architectures and efficient data movement strategies to sustain high performance.

@fig-memory-wall quantifies this disparity for specific public-count models and hardware generations, showing how model parameter counts have outpaced memory bandwidth improvements. The gap between these curves, from AlexNet's 60 million parameters to publicly disclosed hundred-billion-parameter frontier models, represents the engineering challenge that drives accelerator memory system design. Even the latest accelerators like NVIDIA's B200 (8 TB/s) and AMD's MI325X (6 TB/s) cannot close this gap: bandwidth has improved by roughly 16$\times$ since 2014, while model sizes have grown by over 10,000$\times$. Parameter counts for proprietary systems such as GPT-4 and Gemini are not officially disclosed, so they are not plotted as factual data points.

::: {#fig-memory-wall fig-env="figure" fig-pos="htb" fig-cap="**Model Size vs. Hardware Bandwidth**: Publicly disclosed model parameter counts and hardware memory bandwidth plotted from 2012 to 2025, showing how model growth from AlexNet to hundred-billion-parameter models has far outpaced bandwidth improvements across GPU and TPU generations." fig-alt="Scatter plot with trend lines comparing public AI model parameters (red) and hardware bandwidth (blue) from 2012 to 2025. Models grow from AlexNet to recent public-count frontier models. Shaded gap shows widening memory wall."}
...
proc_names = ["NVIDIA Tesla K80", "Google TPU v2", "NVIDIA Tesla V100", "NVIDIA A100",
              "Google TPU v4", "NVIDIA H100", "AMD MI300X", "Google TPU v6e",
              "AMD MI325X", "NVIDIA B200"]
...
model_names = ["AlexNet", "VGG-16", "ResNet-50", "BERT Large", "GPT-3", "PaLM",
               "Llama 3.1", "DeepSeek-V3", "Llama 4 Maverick"]
...
# -----------------------------------------------------------------------------
ax.scatter(
    proc_years, proc_log_bw,
    s=45,
    color=COLORS["BlueLine"],
    edgecolors="white",
    linewidths=0.8,
    zorder=4,
    label="Hardware bandwidth"
)

ax.scatter(
    model_years, model_log_params,
    s=45,
    color=COLORS["RedLine"],
    edgecolors="white",
    linewidths=0.8,
...
visible_proc_labels = {
    ...
    "NVIDIA Tesla V100":  {"xytext": (-15, 25), "ha": "center",
            "arrowprops": dict(
        arrowstyle="-",
        color=COLORS["BlueLine"],
        lw=0.8
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(2022.3, 4.2, 'AI Memory Wall', fontsize=9, fontweight='bold', ha='center', color=COLORS['primary'], bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.5))
...
Modern AI accelerators implement multilevel memory hierarchies that balance speed, capacity, and energy efficiency by exploiting these structured reuse patterns. While general-purpose computing contends with unpredictable memory access, ML workloads exhibit structured reuse that can be optimized through careful data organization across multiple memory levels.

At the highest level, large-capacity but slow storage devices provide long-term model storage. At the lowest level, high-speed registers\index{Registers!fastest memory} and caches ensure that compute units can access operands with minimal latency. Between these extremes, intermediate memory levels, such as scratchpad memory\index{Scratchpad Memory!software-managed}, high-bandwidth memory\index{HBM!intermediate tier}, and off-chip DRAM, offer trade-offs between performance and capacity.
...
**Systems insight**: Data *cannot* be fetched from DRAM in a single cycle. It is physically impossible. Local registers and SRAM (L1) are required to feed compute units at 2 GHz. The "memory wall" is partially a **distance wall**.
...
**On-chip memory**\index{On-Chip Memory!definition} is the fast local storage located on or near the accelerator die, including registers, caches, and software-managed scratchpads.
...
**Systems lesson**: Hardware features are brittle contracts. If the workload does not present supported data types and tile shapes, the accelerator falls back to generic execution. Hardware cannot be exploited without conforming to its contracts [@NVIDIA2020].
...
# │
# │ Imports: mlsysim.core.constants (NVLINK, INFINIBAND bandwidths)
# │ Exports: InterconnectHierarchy.nvlink_a100,
# │          InterconnectHierarchy.nvlink_h100, InterconnectHierarchy.ib_hdr,
# │          InterconnectHierarchy.ib_ndr, InterconnectHierarchy.ib_hdr_str,
# │          InterconnectHierarchy.ib_ndr_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class InterconnectHierarchy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_str = fmt(nvlink_a100, precision=1, allow_zero=True, commas=False)
    nvlink_h100_str = fmt(nvlink_h100, precision=1, allow_zero=True, commas=False)
    ib_hdr_str = fmt(ib_hdr_gbps, precision=1, allow_zero=True, commas=False)
    ib_ndr_str = fmt(ib_ndr_gbps, precision=1, allow_zero=True, commas=False)
```

1. **Device-Device Interconnect (NVLink/Infinity Fabric)**[^fn-nvlink-bandwidth]\index{NVLink!GPU interconnect}\index{GPU Interconnect!switching fabric}\index{Infinity Fabric!AMD interconnect}: Modern multi-GPU nodes use specialized high-speed bridges like NVLink to connect accelerators directly, bypassing the host CPU. Bandwidth ranges from `{python} InterconnectHierarchy.nvlink_a100_str` to `{python} InterconnectHierarchy.nvlink_h100_str` GB/s per GPU. The primary use case is gradient synchronization (AllReduce)\index{AllReduce!gradient synchronization}[^fn-allreduce-gradient-sync] during distributed training. This bandwidth is critical for scaling; without it, multi-GPU training often scales poorly.
...
Performance is bounded by two ceilings, as @eq-roofline formalizes. Here, attainable performance and peak compute are in FLOP/s (often reported as TFLOP/s), peak bandwidth is in bytes/s (often TB/s), and arithmetic intensity is in FLOP/byte:
...
# │          RooflineGap.v100_tflops_str, RooflineGap.v100_bw_tbs_str,
# │          RooflineGap.a100_tflops_fp16_str, RooflineGap.a100_bw_tbs_str,
# │          RooflineGap.h100_tflops_fp16_str, RooflineGap.h100_bw_tbs_str,
# │          RooflineGap.v100_ridge_str, RooflineGap.a100_ridge_str,
# │          RooflineGap.h100_ridge_str, RooflineGap.a100_ridge_fp32_str,
# │          RooflineGap.relu_below_roofline_str, RooflineGap.legacy_ai_str,
# │          RooflineGap.bandwidth_ratio_str, RooflineGap.flops_ratio_str,
# │          RooflineGap.dense_matmul_dim_str,
# │          RooflineGap.dense_matmul_ai_str
# │
# │ Note: PERSISTENT—RooflineGap.a100_ridge and formatting strings are reused
# │       through the nearby layer analyses and GPT-2 throughput derivations.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim import Hardware
...
class RooflineGap:
    ...
    v100_tflops_value      = h_v100.peak_flops.m_as(TFLOPs/second)
    v100_bw_tbs_value      = h_v100.memory_bw.m_as(TB/second)
    a100_tflops_fp16_value = h_a100.peak_flops.m_as(TFLOPs/second)
    a100_bw_tbs_value      = h_a100.memory_bw.m_as(TB/second)
    h100_tflops_fp16_value = h_h100.peak_flops.m_as(TFLOPs/second)
    h100_bw_tbs_value      = h_h100.memory_bw.m_as(TB/second)

    # Step 4: Comparisons
    bw_growth = h_h100.memory_bw.m_as(GB/second) / h_a100.memory_bw.m_as(GB/second)
    ...
    check(300 <= a100_tflops_fp16_value <= 320,
          f"A100 FP16 peak should be near 312 TFLOP/s, got {a100_tflops_fp16_value:.0f}.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_tflops_str = fmt(v100_tflops_value, precision=1, allow_zero=True, commas=False)
    v100_bw_tbs_str = fmt(v100_bw_tbs_value, precision=1, commas=False)
    a100_tflops_fp16_str = fmt(a100_tflops_fp16_value, precision=1, allow_zero=True, commas=False)
    a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=2, commas=False)
    h100_tflops_fp16_str = fmt(h100_tflops_fp16_value, precision=1, allow_zero=True, commas=False)
    h100_bw_tbs_str = fmt(h100_bw_tbs_value, precision=2, commas=False)

    v100_ridge_str = fmt(v100_ridge, precision=1, allow_zero=True, commas=False)
    ...
    a100_ridge_fp32_str = fmt(a100_ridge_fp32, precision=1, allow_zero=True, commas=False)

    legacy_ai_str = fmt(legacy_ai, precision=1, allow_zero=True, commas=False)
    relu_ai_str = fmt(relu_ai, precision=3, commas=False)
    dense_matmul_dim_str = fmt(dense_matmul_dim, precision=1, allow_zero=True, commas=False)
    dense_matmul_ai_str = fmt(dense_matmul_ai, precision=1, allow_zero=True, commas=False)
    bandwidth_ratio_str = fmt(bw_growth, precision=1, commas=False)
    flops_ratio_str = fmt(flops_growth, precision=1, commas=False)
    relu_below_roofline_str = fmt(relu_gap, precision=1, allow_zero=True, commas=True)
```
...
Arithmetic intensity (AI) measures floating-point operations per byte of memory traffic. FLOPs is a dimensionless count of floating-point operations and bytes transferred is measured in bytes, so AI has units of FLOP/byte, defined by @eq-arithmetic-intensity:
$$\text{AI} = \frac{\text{FLOPs}}{\text{bytes transferred}}$$ {#eq-arithmetic-intensity}

The roofline visualization shows performance (TFLOP/s) on the vertical axis and arithmetic intensity (FLOP/byte) on the horizontal axis. At low arithmetic intensity, performance increases linearly with intensity (memory-bound region). Above a threshold called the ridge point, performance saturates at peak compute (compute-bound region).

### Hardware ridge points {#sec-hardware-acceleration-hardware-ridge-points-b5b6}

The **ridge point**\index{Ridge Point!definition} determines the arithmetic intensity threshold where the transition from memory-bound to compute-bound occurs. @tbl-ridge-points quantifies how different accelerators exhibit distinct characteristics based on their compute-to-bandwidth ratios:
...
**Metric**: The ridge point $\text{AI}_{\text{ridge}}$, defined as $\text{AI}_{\text{ridge}} = R_{\text{peak}} / \text{BW}$ (FLOP/byte). This number specifies how many math operations the hardware must perform for every byte of data loaded to keep the compute units busy.

**Evolution**:

*   **V100 (2017)**: `{python} RooflineGap.v100_tflops_str` TFLOP/s / `{python} RooflineGap.v100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.v100_ridge_str` FLOP/byte**.
*   **A100 (2020)**: `{python} RooflineGap.a100_tflops_fp16_str` TFLOP/s / `{python} RooflineGap.a100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.a100_ridge_str` FLOP/byte**.
*   **H100 (2023)**: `{python} RooflineGap.h100_tflops_fp16_str` TFLOP/s / `{python} RooflineGap.h100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.h100_ridge_str` FLOP/byte**.

**Systems insight**: The "bar" for compute intensity has doubled. An algorithm with $\text{AI}$ = `{python} RooflineGap.legacy_ai_str` FLOP/byte was compute-bound (good) on A100 but is bandwidth-bound (bad) on H100. This explains why "legacy" code often sees only `{python} RooflineGap.bandwidth_ratio_str`$\times$ speedup on H100 (bandwidth ratio) instead of the advertised `{python} RooflineGap.flops_ratio_str`$\times$ (FLOPs ratio).

**Practical Examples**: A standard **ReLU** performs 1 operation for every 8 bytes (0.125 FLOP/byte), placing it `{python} RooflineGap.relu_below_roofline_str`$\times$ below the H100 roofline. A well-tiled `{python} RooflineGap.dense_matmul_dim_str` $\times$ `{python} RooflineGap.dense_matmul_dim_str` **Dense MatMul** reaches about `{python} RooflineGap.dense_matmul_ai_str` FLOP/byte, making it compute bound even on H100. Most operations fall short of the ridge point, which is why **Kernel Fusion** is the most important optimization, as explored in @sec-hardware-acceleration-kernel-fusion-7faf.
...
# │
# │ Imports: mlsysim.core.constants (TRANSFORMER_*, BYTES_FP16)
# │ Exports: TransformerLayerCalc.t_seq_str,
# │          TransformerLayerCalc.t_hidden_str,
# │          TransformerLayerCalc.qkv_flops_str,
# │          TransformerLayerCalc.qkv_params_str,
# │          TransformerLayerCalc.qkv_ai_str,
# │          TransformerLayerCalc.softmax_ops_str,
# │          TransformerLayerCalc.softmax_bytes_str,
...
class TransformerLayerCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    t_hidden_str        = fmt(t_hidden_value, precision=1, allow_zero=True, commas=False)
    t_batch_str         = fmt(t_batch_value, precision=1, allow_zero=True, commas=False)
    t_seq_str           = fmt(t_seq_value, precision=1, allow_zero=True, commas=False)
    t_heads_str         = fmt(t_heads_value, precision=1, allow_zero=True, commas=False)
    qkv_flops_b_str     = fmt(qkv_flops_b_value, precision=1, allow_zero=True, commas=False)
    qkv_mb_str          = fmt(qkv_mb_value, precision=1, allow_zero=True, commas=False)
    qkv_ai_str          = fmt(qkv_ai_value, precision=1, allow_zero=True, commas=False)
    softmax_flops_m_str = fmt(softmax_flops_m_value, precision=1, allow_zero=True, commas=False)
    softmax_mb_str      = fmt(softmax_mb_value, precision=1, allow_zero=True, commas=False)
...
- FLOPs: 2 $\times$ 3 $\times$ `{python} TransformerLayerCalc.t_batch_str` $\times$ `{python} TransformerLayerCalc.t_seq_str` $\times$ `{python} TransformerLayerCalc.t_hidden_str` $\times$ `{python} TransformerLayerCalc.t_hidden_str` = `{python} TransformerLayerCalc.qkv_flops_b_str` billion FLOPs
- Bytes: (input + weights + output) = (`{python} TransformerLayerCalc.t_batch_str` $\times$ `{python} TransformerLayerCalc.t_seq_str` $\times$ `{python} TransformerLayerCalc.t_hidden_str` + 3 $\times$ `{python} TransformerLayerCalc.t_hidden_str` $\times$ `{python} TransformerLayerCalc.t_hidden_str` + `{python} TransformerLayerCalc.t_batch_str` $\times$ `{python} TransformerLayerCalc.t_seq_str` $\times$ `{python} TransformerLayerCalc.t_hidden_str` $\times$ 3) $\times$ 2 ≈ `{python} TransformerLayerCalc.qkv_mb_str` MB
- AI = `{python} TransformerLayerCalc.qkv_flops_b_str` billion FLOPs / `{python} TransformerLayerCalc.qkv_mb_str` MB = `{python} TransformerLayerCalc.qkv_ai_str` FLOP/byte, which is **compute bound on A100** (above `{python} RooflineGap.a100_ridge_str` threshold)

**Softmax**:

- FLOPs: `{python} TransformerLayerCalc.t_batch_str` $\times$ `{python} TransformerLayerCalc.t_heads_str` $\times$ `{python} TransformerLayerCalc.t_seq_str` $\times$ `{python} TransformerLayerCalc.t_seq_str` $\times$ 3 ≈ `{python} TransformerLayerCalc.softmax_flops_m_str` M FLOPs (exp, sum, div)
- Bytes: `{python} TransformerLayerCalc.t_batch_str` $\times$ `{python} TransformerLayerCalc.t_heads_str` $\times$ `{python} TransformerLayerCalc.t_seq_str` $\times$ `{python} TransformerLayerCalc.t_seq_str` $\times 2 \times 2$ = `{python} TransformerLayerCalc.softmax_mb_str` MB
- AI = `{python} TransformerLayerCalc.softmax_flops_m_str` M / `{python} TransformerLayerCalc.softmax_mb_str` M = `{python} TransformerLayerCalc.softmax_ai_str` FLOP/byte, which is memory-bound
...
$$\text{Required Bandwidth} = \frac{R_{\text{ops}}}{\text{AI}} \text{ bytes/s}$$ {#eq-required-bandwidth}
...
$$R_{\text{attainable}} = \text{Peak Bandwidth} \times \text{AI}$$ {#eq-attainable-throughput}
...
# │
# │ Imports: mlsysim.core.constants (BYTES_FP16, byte, MB, flop, GFLOPs),
# │          mlsysim.book (fmt)
# │ Exports: Conv2dAnalysisCalc.conv_out_m_str,
# │          Conv2dAnalysisCalc.conv_flops_per_out_str,
# │          Conv2dAnalysisCalc.conv_total_gflops_str,
...
class Conv2dAnalysisCalc:
    ...
    conv_flops_per_out_str = fmt(conv_flops_per_out, precision=1, allow_zero=True)
    ...
    conv_ai_str            = fmt(conv_ai, precision=1, allow_zero=True, commas=False)
```
...
- Output size: $32 \times 256 \times 56 \times 56$ = `{python} Conv2dAnalysisCalc.conv_out_m_str` M elements
- FLOPs per output: $128 \times 3 \times 3 \times 2$ = `{python} Conv2dAnalysisCalc.conv_flops_per_out_str` (multiply-add)
- Total FLOPs: `{python} Conv2dAnalysisCalc.conv_out_m_str` M $\times$ `{python} Conv2dAnalysisCalc.conv_flops_per_out_str` = `{python} Conv2dAnalysisCalc.conv_total_gflops_str` billion FLOPs

**Memory Traffic Analysis**:

- Input: $32 \times 128 \times 56 \times 56 \times 2$ = `{python} Conv2dAnalysisCalc.conv_input_mb_str` MB (FP16)
- Weights: $256 \times 128 \times 3 \times 3 \times 2$ ≈ `{python} Conv2dAnalysisCalc.conv_weights_mb_str` MB (FP16)
- Output: $32 \times 256 \times 56 \times 56 \times 2$ = `{python} Conv2dAnalysisCalc.conv_output_mb_str` MB (FP16)
- Total: `{python} Conv2dAnalysisCalc.conv_total_mb_str` MB

**Arithmetic Intensity**:
$\text{AI}$ = `{python} Conv2dAnalysisCalc.conv_total_gflops_str` GFLOPs / `{python} Conv2dAnalysisCalc.conv_total_mb_str` MB = `{python} Conv2dAnalysisCalc.conv_ai_str` FLOP/byte

This is **well above** A100's ridge point of `{python} RooflineGap.a100_ridge_str` FLOP/byte, making this operation compute-bound. The layer will achieve near-peak performance of ~`{python} TensorLifecycleCalc.a100_tflops_fp16_str` TFLOP/s (FP16 with Tensor Cores).
...
# │
# │ Imports: mlsysim.core.constants (BYTES_FP16, byte, MB, flop, MFLOPs, GFLOPs,
# │          TFLOPs, Hardware.Cloud.A100.memory.bandwidth, Hardware.Cloud.A100.compute.peak_flops, GB, second),
# │          mlsysim.book (fmt)
# │ Exports: DenseLayerAnalysisCalc.dense_total_mflops_str,
...
# │          DenseLayerAnalysisCalc.dense_attainable_str,
# │          DenseLayerAnalysisCalc.dense_util_pct_str,
# │          DenseLayerAnalysisCalc.a100_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
class DenseLayerAnalysisCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dense_total_mflops_str = fmt(dense_total_mflops, precision=1, allow_zero=True, commas=False)
    dense_input_kb_str     = fmt(dense_input_kb, precision=1, allow_zero=True, commas=False)
    dense_weights_mb_str   = fmt(dense_weights_mb, precision=1, commas=False)
    dense_output_kb_str    = fmt(dense_output_kb, precision=1, allow_zero=True, commas=False)
    dense_total_mb_str     = fmt(dense_total_mb, precision=1, commas=False)
    dense_ai_str           = fmt(dense_ai, precision=1, commas=False)
    dense_attainable_str   = fmt(dense_attainable_tflops, precision=1, allow_zero=True, commas=False)
    dense_util_pct_str     = fmt(dense_util_pct, precision=1, allow_zero=True, commas=False)
    a100_bw_str            = fmt(a100_bw_gbs_value, precision=1, allow_zero=True)
```
...
- Total FLOPs: $2 \times 32 \times 2048 \times 2048$ = `{python} DenseLayerAnalysisCalc.dense_total_mflops_str` million FLOPs

**Memory Traffic Analysis**:

- Input: $32 \times 2048 \times 2$ = `{python} DenseLayerAnalysisCalc.dense_input_kb_str` KB (FP16)
- Weights: $2048 \times 2048 \times 2$ = `{python} DenseLayerAnalysisCalc.dense_weights_mb_str` MB (FP16)
- Output: $32 \times 2048 \times 2$ = `{python} DenseLayerAnalysisCalc.dense_output_kb_str` KB (FP16)
- Total: `{python} DenseLayerAnalysisCalc.dense_total_mb_str` MB

**Arithmetic Intensity**:
$\text{AI}$ = `{python} DenseLayerAnalysisCalc.dense_total_mflops_str` MFLOPs / `{python} DenseLayerAnalysisCalc.dense_total_mb_str` MB = `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte

This is **below** A100's ridge point of `{python} RooflineGap.a100_ridge_str` FLOP/byte, making this operation memory-bound. Attainable performance:
$R_{\text{attainable}}$ = `{python} DenseLayerAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_str` TFLOP/s

This is only `{python} DenseLayerAnalysisCalc.dense_util_pct_str` percent of peak compute capability, demonstrating the memory wall effect for small batch sizes.
...
# │
# │ Imports: mlsysim.core.constants (byte, MB, Hardware.Cloud.A100.memory.bandwidth, GB, GFLOPs, TFLOPs,
# │          second), mlsysim.book (fmt)
# │ Exports: LayernormAnalysisCalc.ln_elements_m_str,
# │          LayernormAnalysisCalc.ln_total_mflops_str,
...
# │          LayernormAnalysisCalc.ln_ridge_gap_str,
# │          LayernormAnalysisCalc.ln_attainable_str,
# │          LayernormAnalysisCalc.a100_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, check
...
class LayernormAnalysisCalc:
    ...
    ln_ridge_gap = RooflineGap.a100_ridge / ln_ai
    ...
    ln_params_kb_str    = fmt(ln_params_kb, precision=1, allow_zero=True, commas=False)
    ...
    ln_ridge_gap_str    = fmt(ln_ridge_gap, precision=1, allow_zero=True, commas=False)
    ln_attainable_str   = fmt(ln_attainable_tflops, precision=1, allow_zero=True, commas=False)
    a100_bw_str         = fmt(a100_bw_gbs_value, precision=1, allow_zero=True, commas=False)
```
...
- Elements: $32 \times 512 \times 768$ = `{python} LayernormAnalysisCalc.ln_elements_m_str` M
- Operations per element: mean (1 ADD), variance (1 ADD, 1 MUL), normalize (1 ADD, 1 MUL, 1 DIV) ≈ 6 FLOPs
- Total FLOPs: `{python} LayernormAnalysisCalc.ln_elements_m_str` M $\times$ 6 = `{python} LayernormAnalysisCalc.ln_total_mflops_str` M FLOPs

**Memory Traffic**:

- Input: `{python} LayernormAnalysisCalc.ln_elements_m_str` M $\times$ 2 = `{python} LayernormAnalysisCalc.ln_input_mb_str` MB
- Parameters (scale, bias): $768 \times 2 \times 2$ = `{python} LayernormAnalysisCalc.ln_params_kb_str` KB (negligible)
- Output: `{python} LayernormAnalysisCalc.ln_elements_m_str` M $\times$ 2 = `{python} LayernormAnalysisCalc.ln_output_mb_str` MB
- Total: `{python} LayernormAnalysisCalc.ln_total_mb_str` MB

**Arithmetic Intensity**:
$\text{AI}$ = `{python} LayernormAnalysisCalc.ln_total_mflops_str` MFLOPs / `{python} LayernormAnalysisCalc.ln_total_mb_str` MB = `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte

This is **severely memory-bound** (`{python} LayernormAnalysisCalc.ln_ridge_gap_str`$\times$ below the A100 ridge point). Performance is limited to:
$R_{\text{attainable}}$ = `{python} LayernormAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte = `{python} LayernormAnalysisCalc.ln_attainable_str` TFLOP/s
...
1. **High AI (>200 FLOP/byte)**: Compute-bound operations like large convolutions
   ...
   - Impact: Can approach 90--95 percent of peak TFLOP/s
...
4. **Very low AI (<2 FLOP/byte)**: Severely memory-bound operations like normalization, activation functions
   ...
   - Impact: Can achieve 10$\times$ speedup through fusion; for example, LayerNorm combined with the Gaussian Error Linear Unit (GELU) can become a single fused kernel\index{Operator Fusion!memory-bound optimization}\index{LayerNorm!kernel fusion target}

One of the most accessible levers for moving an operation up and right on the roofline is batching.
...
class BatchAiCalc:
    ...
    # OUTPUT
    batch1_ai_str = fmt(batch1_ai, precision=1, allow_zero=True, commas=False)
    batch32_ai_str = fmt(batch32_ai, precision=1, allow_zero=True, commas=False)
    batch256_ai_str = fmt(batch256_ai, precision=1, allow_zero=True, commas=False)
```
...
- Batch = 1: AI \approx `{python} BatchAiCalc.batch1_ai_str` FLOP/byte (memory bound)
- Batch = 32: AI \approx `{python} BatchAiCalc.batch32_ai_str` FLOP/byte (memory bound)
- Batch = 256: AI \approx `{python} BatchAiCalc.batch256_ai_str` FLOP/byte (compute bound on A100)
...
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT2.parameters, A100_*, BYTES_FP16)
# │ Exports: Gpt2ThroughputCalc.gpt2_weight_gb_str,
# │          Gpt2ThroughputCalc.gpt2_decode_gflops_str,
# │          Gpt2ThroughputCalc.gpt2_decode_ai_str,
# │          Gpt2ThroughputCalc.gpt2_max_tflops_str,
# │          Gpt2ThroughputCalc.gpt2_utilization_str,
# │          Gpt2ThroughputCalc.a100_tflops_fp16_str,
# │          Gpt2ThroughputCalc.a100_bw_tbs_str,
# │          Gpt2ThroughputCalc.a100_tflops_fp32
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
class Gpt2ThroughputCalc:
    ...
    a100_tflops_fp16_str  = fmt(a100_tflops_fp16_val, precision=1, allow_zero=True, commas=False)
    a100_bw_tbs_str       = fmt(a100_bw_tbs_val, precision=2, commas=False)
    a100_tflops_fp32_str  = fmt(Hardware.Cloud.A100.compute.precision_flops["fp32"].m_as(TFLOPs/second), precision=1, commas=False)
```
...
*   **Peak compute**: `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` TFLOP/s (FP16 Tensor Core).
*   **Peak bandwidth**: `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` TB/s (HBM2e).
*   **Ridge point $(R_{\text{peak}}/\text{BW})$**: `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` / `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` = **`{python} RooflineGap.a100_ridge_str` FLOP/byte** (for FP16 Tensor Core).
    *   **Meaning**: Saturating this chip at FP16 precision requires `{python} RooflineGap.a100_ridge_str` operations for every byte loaded. The ridge point varies by precision: FP32 operations (`{python} Gpt2ThroughputCalc.a100_tflops_fp32_str` TFLOP/s peak) have a ridge point of only ~`{python} RooflineGap.a100_ridge_fp32_str` FLOP/byte.
...
*   **Data movement**: Must load all weights (`{python} Gpt2ThroughputCalc.gpt2_weight_gb_str` GB @ FP16) for every token.
*   **Compute**: Vector-Matrix multiplication. 2$\times$ Params ≈ `{python} Gpt2ThroughputCalc.gpt2_decode_gflops_str` GFLOPs.
*   **Arithmetic intensity**:
    `{python} Gpt2ThroughputCalc.gpt2_decode_gflops_str` GFLOPs / `{python} Gpt2ThroughputCalc.gpt2_weight_gb_str` GB = **`{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte**

**The prediction (iron law)**

Since Actual Intensity (`{python} Gpt2ThroughputCalc.gpt2_decode_ai_str`) ≪ Ridge Point (`{python} RooflineGap.a100_ridge_str`), the system is **Bandwidth Bound**.

*   **Maximum throughput**: `{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte $\times$ `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` TB/s = **`{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOP/s**.
*   **Utilization ceiling**:
    `{python} Gpt2ThroughputCalc.gpt2_max_tflops_str` TFLOP/s (Actual) / `{python} Gpt2ThroughputCalc.a100_tflops_fp16_str` TFLOP/s (Peak) ≈ **`{python} Gpt2ThroughputCalc.gpt2_utilization_str` percent**

**Systems insight**:
Without batching or caching, a \$15,000 GPU runs at **less than 1 percent efficiency** on LLM inference. This "utilization gap" drives the need for key-value caching and quantization.

:::

As this derivation demonstrates, the Roofline model provides the diagnostic framework for identifying whether operations are compute bound or memory bound. Knowing that a workload is memory bound at `{python} Gpt2ThroughputCalc.gpt2_utilization_str` percent utilization is only the first step; the next challenge is translating this diagnosis into efficient execution plans that exploit accelerator architectures.
...
Effective mapping strategies minimize off-chip memory accesses, maximize compute utilization, and efficiently manage data movement across different levels of the memory hierarchy. In practice, compiler support is central to achieving these goals.

Those choices are too coupled to manage manually at scale, which is why the compiler becomes part of the accelerator system.
...
The efficient execution of machine learning models on AI accelerators requires careful consideration of placement, allocation, and scheduling. Placement assigns operations to hardware locations, allocation assigns data and resource capacity to memory tiers, and scheduling determines temporal order and synchronization. These decisions are interdependent, and each introduces trade-offs that impact performance, energy efficiency, and scalability. @tbl-combinatorial-complexity enumerates the key trade-offs between computation placement, resource allocation, and scheduling that shape overall performance. Placement decisions influence parallelism, memory access patterns, and communication overhead, while allocation and scheduling strategies determine how resources are used over time to balance execution efficiency. The interplay between these factors requires a careful balance to avoid bottlenecks such as excessive synchronization, memory congestion, or underutilized compute resources. These trade-offs must be optimized to ensure that AI accelerators operate at peak efficiency.
...
While developers can sometimes manually specify tensor layouts, the choice is often determined automatically by machine learning frameworks (e.g., TensorFlow, PyTorch, JAX), compilers, or AI accelerator runtimes. Low-level optimization tools such as cuDNN (for NVIDIA GPUs), XLA (for TPUs), and MLIR (for custom accelerators) may rearrange tensor layouts dynamically to optimize performance [@GoogleXLA]. In high-level frameworks, layout transformations are typically applied transparently, but developers working with custom kernels or low-level libraries (e.g., CUDA, Metal, or OpenCL) may have direct control over tensor format selection.
...
# │
# │ Imports: mlsysim.core.constants (BYTES_FP32, byte, MB), mlsysim.book (fmt)
# │ Exports: MemoryFootprintCalc.tensor_mb_str,
# │          MemoryFootprintCalc.total_mb_str,
# │          MemoryFootprintCalc.footprint_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
class MemoryFootprintCalc:
    ...
    footprint_ratio_str = fmt(footprint_ratio, precision=1, allow_zero=True, commas=False)
```
...
|:-----------------|:---------------------------------------------------|
| **X**            | `{python} MemoryFootprintCalc.tensor_mb_str` MB    |
| **X'**           | `{python} MemoryFootprintCalc.tensor_mb_str` MB    |
| **X''**          | `{python} MemoryFootprintCalc.tensor_mb_str` MB    |
| **Y**            | `{python} MemoryFootprintCalc.tensor_mb_str` MB    |
| **Total Memory** | **`{python} MemoryFootprintCalc.total_mb_str` MB** |

: **Intermediate Tensor Storage**: Naive execution models require substantial memory to store intermediate tensors generated by each operation. For a $1024{\times}1024$ tensor, storing intermediate results (even when only the final output is needed) quadruples the total memory footprint from `{python} MemoryFootprintCalc.tensor_mb_str` MB to `{python} MemoryFootprintCalc.total_mb_str` MB. Minimizing intermediate data storage is essential for improving memory efficiency. {#tbl-memory-footprint}
...
# │ How: Re-format memory constants with precision=1, allow_zero=True.
# │
# │ Imports: mlsysim.core.constants (BYTES_FP32), mlsysim.book (fmt)
# │ Exports: MemoryFootprintCalc.tensor_mb_str, MemoryFootprintCalc.total_mb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
class MemoryFootprintTableCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tensor_mb_str = fmt(tensor_mb, precision=1, allow_zero=True, commas=False)
    total_mb_str  = fmt(total_mb, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (BYTES_FP32)
...
class FusionBenefitsCalc:
    ...
    fused_mb = tensor_mb  # ~4.2 MB with only final result Y stored

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Custom " MB" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    naive_mb_str = fmt(naive_mb, precision=1, commas=False, suffix=" MB")
    fused_mb_str = fmt(fused_mb, precision=1, commas=False, suffix=" MB")
```
...
Different AI accelerators and compilers handle fusion in distinct ways. NVIDIA GPUs, for example, favor warp-level parallelism, where element-wise fusion is straightforward. TPUs, on the other hand, prioritize systolic array execution, which is optimized for matrix-matrix operations rather than element-wise fusion [@NVIDIA2020]. AI compilers such as XLA (TensorFlow), TorchScript (PyTorch), TensorRT (NVIDIA), and MLIR automatically detect fusion opportunities and apply heuristics to balance memory savings and execution efficiency [@GoogleXLA].
...
\index{Mapping!hybrid, layer-specific strategy}
...
# │          CompilerSpeedupCalc.compiler_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
Most practitioners never interact directly with ML compilers, yet compiler quality often determines whether a model achieves 20 percent or 80 percent of hardware peak performance. Calling `model.compile()` in Keras, `torch.compile()` in PyTorch, or deploying through TensorRT invokes multi-stage optimization pipelines that:
...
At each stage, the compiler applies the optimizations developed in @sec-hardware-acceleration-dataflow-optimization-strategies-ce52: kernel fusion, tiling, data movement strategies, and computation placement. These optimizations are systematically incorporated into the final execution plan.
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.A100.tdp)
# │ Exports: RuntimeProductionTdp.a100_tdp_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class RuntimeProductionTdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tdp_str = fmt(Hardware.Cloud.A100.tdp.m_as(watt), precision=1, allow_zero=True, commas=False)
```
...
Long-running inference servers gradually fragment GPU memory. Runtimes implement defragmentation strategies, but understanding when to restart services or preallocate memory pools requires awareness of these memory fragmentation dynamics.

Cloud accelerators are often shared, leading to multi-tenant interference. A model might run 20 percent slower when a neighbor workload competes for memory bandwidth. Production systems need monitoring to detect and respond to this interference.

Sustained workloads may trigger thermal throttling that was not observed in short benchmarks. The A100 SXM operates at `{python} RuntimeProductionTdp.a100_tdp_str` W TDP while the A100 PCIe operates at 300 W TDP; these represent different form factors with different cooling requirements, not boost vs. sustained states. Production performance depends on which variant is deployed and whether thermal limits are approached.
...
Overlapping computation with memory movement mitigates a common performance bottleneck: data movement between memory hierarchy levels limits computation speed. AI runtimes implement asynchronous execution and double buffering so that computations proceed without waiting for memory transfers to complete. In a large-scale model, for instance, image data can be prefetched while computations are performed on the previous batch, thus maintaining a steady flow of data and avoiding pipeline stalls.
...
#| echo: false
from mlsysim.core.formulas import dTime
from mlsysim.fmt import fmt, check
...
class Gpt3SingleH100Calc:
    ...
    # OUTPUT
    h100_pflops_fp8_str = fmt(Hardware.Cloud.H100.compute.precision_flops["fp8"].m_as(PFLOPs/second), precision=1, commas=False)
    peak_years_str = fmt(peak_years, precision=1, commas=False)
...
[^fn-amdahls-law-scaling]: **Amdahl's Law (Scaling Limit)**: The machine foundations appendix formalizes Amdahl's Law in @sec-machine-foundations-amdahls-law-gustafsons-law-b741; it bounds speedup by the serial fraction of a workload. In multi-accelerator training, gradient synchronization constitutes the dominant serial fraction: at just 5 percent synchronization overhead, maximum speedup is capped at 20$\times$ regardless of how many accelerators are added. This hard ceiling explains why scaling efficiency degrades sharply beyond 64--128 accelerators and drives algorithmic innovations such as gradient compression and communication-computation overlap that reduce the effective serial fraction. \index{Amdahl's Law!scaling limit}
...
::: {#lhs-hw-acceleration-case-heterogeneous-microcontrollers .callout-lighthouse title="The case for heterogeneous microcontrollers"}
...
To achieve real-time person detection (30 FPS) within this envelope, modern MCUs adopt the same heterogeneous strategy as their larger mobile cousins but at a micro-scale. A typical architecture pairs a general-purpose core (for example, Cortex-M) for system logic with a dedicated micro-NPU (for example, Ethos-U\index{Ethos-U!ARM micro-NPU}) for CNN acceleration. The NPU executes the Wake Vision MobileNet model at 50--100$\times$ better energy efficiency than the CPU could achieve alone. Without this specialized acceleration, the "always-on" promise of the Smart Doorbell would remain physically impossible.
...
\index{Unified Memory!mobile SoC}
...
The complexity of hardware acceleration, spanning data center architectures to heterogeneous mobile SoCs, leaves room for misconceptions and suboptimal design decisions.
...
class FpWorkloadFitRecap:
    ...
    softmax_tflops_low = softmax_ai_low * a100_bw_tbs
    softmax_tflops_high = softmax_ai_high * a100_bw_tbs
    softmax_util_high = softmax_tflops_high / a100_peak_tflops * 100
    cpu_softmax_util_low = softmax_ai_low / cpu_ridge_high * 100
    cpu_softmax_util_high = softmax_ai_high / cpu_ridge_low * 100

    # GUARD
    check(150 <= a100_ridge <= 160, f"Unexpected A100 ridge: {a100_ridge:.1f}")
    check(4 <= softmax_tflops_low <= 5, f"Unexpected softmax low TFLOP/s: {softmax_tflops_low:.1f}")
    check(10 <= softmax_tflops_high <= 11, f"Unexpected softmax high TFLOP/s: {softmax_tflops_high:.1f}")

    # OUTPUT
    a100_ridge_str = fmt(a100_ridge, precision=1, allow_zero=True, commas=False)
    a100_tflops_fp16_str = fmt(a100_peak_tflops, precision=1, allow_zero=True, commas=False)
    softmax_ai_low_str = fmt(softmax_ai_low, precision=1, allow_zero=True, commas=False)
    softmax_ai_high_str = fmt(softmax_ai_high, precision=1, allow_zero=True, commas=False)
    softmax_tflops_low_str = fmt(softmax_tflops_low, precision=1, allow_zero=True, commas=False)
    softmax_tflops_high_str = fmt(softmax_tflops_high, precision=1, allow_zero=True, commas=False)
    softmax_util_high_str = fmt(softmax_util_high, precision=1, allow_zero=True, commas=False)
    cpu_ridge_low_str = fmt(cpu_ridge_low, precision=1, allow_zero=True, commas=False)
    cpu_ridge_high_str = fmt(cpu_ridge_high, precision=1, allow_zero=True, commas=False)
    cpu_softmax_util_low_str = fmt(cpu_softmax_util_low, precision=1, allow_zero=True, commas=False)
    cpu_softmax_util_high_str = fmt(cpu_softmax_util_high, precision=1, allow_zero=True, commas=False)
```

\index{Hardware Selection!workload matching}
Engineers assume specialized accelerators automatically outperform general-purpose processors for all AI workloads. In reality, specialized hardware achieves peak performance only when workloads match architectural assumptions. As demonstrated in @sec-hardware-acceleration-roofline-model-42ff, operations must exceed the accelerator's ridge point to be compute bound; an A100 GPU has a ridge point of `{python} FpWorkloadFitRecap.a100_ridge_str` FLOP/byte, meaning operations with arithmetic intensity below this threshold are memory bound regardless of the accelerator's `{python} FpWorkloadFitRecap.a100_tflops_fp16_str` TFLOP/s peak compute. A transformer attention softmax with AI = `{python} FpWorkloadFitRecap.softmax_ai_low_str`--`{python} FpWorkloadFitRecap.softmax_ai_high_str` FLOP/byte achieves only `{python} FpWorkloadFitRecap.softmax_tflops_low_str`--`{python} FpWorkloadFitRecap.softmax_tflops_high_str` TFLOP/s (`{python} FpWorkloadFitRecap.softmax_util_high_str` percent utilization) on an A100. CPUs with ridge points around `{python} FpWorkloadFitRecap.cpu_ridge_low_str`--`{python} FpWorkloadFitRecap.cpu_ridge_high_str` FLOP/byte still treat this kernel as memory-bound, but the same AI range corresponds to about `{python} FpWorkloadFitRecap.cpu_softmax_util_low_str`--`{python} FpWorkloadFitRecap.cpu_softmax_util_high_str` percent of a CPU's lower peak. Models with irregular memory access, small batch sizes, or dynamic computation graphs may perform better on flexible processors. Effective hardware selection requires matching workload arithmetic intensity to architectural ridge points, not assuming specialization always wins.
...
# │
# │ Imports: mlsysim.core.constants (ENERGY_DRAM_ACCESS_PJ, ENERGY_SRAM_L1_PJ),
# │          mlsysim.book (fmt)
# │ Exports: FpMemoryEnergyCalc.fp_ridge_example_str,
# │          FpMemoryEnergyCalc.fp_layernorm_tflops_str,
...
class FpMemoryEnergyCalc:
    ...
    layernorm_tflops = layernorm_ai * peak_bw_tbs   # AI [FLOP/byte] × BW [TB/s] = TFLOP/s
    layernorm_util   = layernorm_tflops/peak_tflops * 100

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_ridge_example_str    = fmt(fp_ridge_example, precision=1, allow_zero=True, commas=False)
    fp_layernorm_tflops_str = fmt(layernorm_tflops, precision=1, allow_zero=True, commas=False)
    fp_layernorm_util_str   = fmt(layernorm_util, precision=1, allow_zero=True, commas=False)
    fp_dram_pj_str          = fmt(dram_pj, precision=1, allow_zero=True, commas=False)
    fp_sram_pj_str          = fmt(sram_pj, precision=1, commas=False)
```

**Pitfall**: *Ignoring memory bandwidth limitations when selecting acceleration strategies.*

Practitioners focus on peak TFLOP/s without analyzing whether their workloads can achieve compute-bound performance. As quantified in @sec-hardware-acceleration-understanding-ai-memory-wall-3ea9, the energy model used here assigns about `{python} FpMemoryEnergyCalc.fp_dram_pj_str` pJ to a DRAM access versus `{python} FpMemoryEnergyCalc.fp_sram_pj_str` pJ for an on-chip SRAM access, creating orders-of-magnitude energy penalties. An accelerator advertising 300 TFLOP/s with 2 TB/s bandwidth has a ridge point of `{python} FpMemoryEnergyCalc.fp_ridge_example_str` FLOP/byte; LayerNorm operations with AI = 1.5 FLOP/byte achieve only `{python} FpMemoryEnergyCalc.fp_layernorm_tflops_str` TFLOP/s (`{python} FpMemoryEnergyCalc.fp_layernorm_util_str` percent utilization). Organizations deploy expensive high-compute accelerators for memory-bound workloads, achieving 10–20 percent utilization when lower-cost, bandwidth-optimized alternatives would perform identically. Teams must calculate workload arithmetic intensity and compare against hardware ridge points before purchasing accelerators.
...
# │
# │ Imports: mlsysim.core.constants (NVLINK_A100_BW, GB, second),
# │          mlsysim.book (fmt)
# │ Exports: FpMultigpuScalingCalc.fp_nvlink_bw_str,
# │          FpMultigpuScalingCalc.fp_sync_time_str,
# │          FpMultigpuScalingCalc.fp_sync_overhead_str,
# │          FpMultigpuScalingCalc.fp_no_overlap_speedup_str,
...
class FpMultigpuScalingCalc:
    ...
    no_overlap_speedup = gpu_count * step_time_ms / (step_time_ms + sync_time_ms)
    no_overlap_efficiency_pct = no_overlap_speedup / gpu_count * 100

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_nvlink_bw_str     = fmt(nvlink_bw_gbs, precision=1, allow_zero=True, commas=False)
    ...
    fp_no_overlap_speedup_str = fmt(no_overlap_speedup, precision=1, commas=False)
    fp_no_overlap_efficiency_str = fmt(no_overlap_efficiency_pct, precision=1, allow_zero=True, commas=False)
```

**Fallacy**: *Hardware acceleration benefits scale linearly with additional accelerators.*

Teams expect eight GPUs to train 8$\times$ faster than one GPU. Multi-accelerator scaling introduces communication overhead that violates linear scaling assumptions. As noted in @sec-hardware-acceleration-multichip-scaling-c649, AllReduce operations for gradient synchronization can require exchanging hundreds of gigabytes per training step for large models. With NVLink at `{python} FpMultigpuScalingCalc.fp_nvlink_bw_str` GB/s bidirectional, synchronizing 1 GB of gradients requires `{python} FpMultigpuScalingCalc.fp_sync_time_str` ms; for a 50 ms training step, this represents `{python} FpMultigpuScalingCalc.fp_sync_overhead_str` percent overhead with perfect overlap. Without overlap, eight-GPU setups achieve about `{python} FpMultigpuScalingCalc.fp_no_overlap_speedup_str`$\times$ speedup (`{python} FpMultigpuScalingCalc.fp_no_overlap_efficiency_str` percent efficiency) at best, and typical workloads see 6--7$\times$ (75--87 percent efficiency) due to load imbalance and synchronization barriers. Small models with insufficient parallel work achieve even worse scaling, sometimes seeing 3--4$\times$ speedup on eight GPUs (37--50 percent efficiency).
...
class FpPeakVsSustainedRecap:
    ...
    recsys_tflops_high = 30

    # EXECUTE
    transformer_util_low = transformer_tflops_low / a100_peak_tflops * 100
    transformer_util_high = transformer_tflops_high / a100_peak_tflops * 100
    recsys_util_low = recsys_tflops_low / a100_peak_tflops * 100
    recsys_util_high = recsys_tflops_high / a100_peak_tflops * 100
    ...
    # GUARD
    check(300 <= a100_peak_tflops <= 320, f"Unexpected A100 peak: {a100_peak_tflops:.0f}")
    check(35 <= transformer_util_low <= 45, f"Unexpected transformer low utilization: {transformer_util_low:.1f}")
    check(55 <= transformer_util_high <= 65, f"Unexpected transformer high utilization: {transformer_util_high:.1f}")
    check(3 <= recsys_util_low <= 4, f"Unexpected recsys low utilization: {recsys_util_low:.1f}")
    check(9 <= recsys_util_high <= 10, f"Unexpected recsys high utilization: {recsys_util_high:.1f}")

    # OUTPUT
    a100_tflops_fp16_str = fmt(a100_peak_tflops, precision=1, allow_zero=True, commas=False)
    transformer_tflops_low_str = fmt(transformer_tflops_low, precision=1, allow_zero=True, commas=False)
    transformer_tflops_high_str = fmt(transformer_tflops_high, precision=1, allow_zero=True, commas=False)
    transformer_util_low_str = fmt(transformer_util_low_rounded, precision=1, allow_zero=True, commas=False)
    transformer_util_high_str = fmt(transformer_util_high_rounded, precision=1, allow_zero=True, commas=False)
    recsys_tflops_low_str = fmt(recsys_tflops_low, precision=1, allow_zero=True, commas=False)
    recsys_tflops_high_str = fmt(recsys_tflops_high, precision=1, allow_zero=True, commas=False)
    recsys_util_low_str = fmt(recsys_util_low, precision=1, allow_zero=True, commas=False)
    recsys_util_high_str = fmt(recsys_util_high, precision=1, allow_zero=True, commas=False)
```

**Fallacy**: *Peak FLOP/s specifications determine real-world accelerator performance.*

\index{Peak FLOP/s!misleading metric}
Vendors advertise peak FLOP/s as the definitive measure of accelerator capability, but real-world performance equals Peak FLOP/s $\times$ Utilization, where utilization is dictated by the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff). An A100 advertises `{python} FpPeakVsSustainedRecap.a100_tflops_fp16_str` TFLOP/s at FP16, yet transformer training typically achieves only `{python} FpPeakVsSustainedRecap.transformer_tflops_low_str`--`{python} FpPeakVsSustainedRecap.transformer_tflops_high_str` TFLOP/s (`{python} FpPeakVsSustainedRecap.transformer_util_low_str`--`{python} FpPeakVsSustainedRecap.transformer_util_high_str` percent utilization) because memory-bound operations such as attention and LayerNorm drag down the average throughput. Recommendation models fare even worse, often reaching only `{python} FpPeakVsSustainedRecap.recsys_tflops_low_str`--`{python} FpPeakVsSustainedRecap.recsys_tflops_high_str` TFLOP/s (`{python} FpPeakVsSustainedRecap.recsys_util_low_str`--`{python} FpPeakVsSustainedRecap.recsys_util_high_str` percent utilization) due to sparse, irregular memory access patterns that leave compute units idle. Engineers should budget projects based on sustained throughput, measured or estimated via the Roofline Model, rather than peak marketing specifications.
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.T4.compute.peak_flops, Hardware.Cloud.T4.memory.bandwidth, Hardware.Cloud.A100.memory.bandwidth, TFLOPs,
...
# │          FpSmallBatchCalc.fp_t4_flops_str,
# │          FpSmallBatchCalc.fp_t4_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
...
class FpSmallBatchCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fp_ai_b1_str   = fmt(ai_b1, precision=1, allow_zero=True, commas=False)
    fp_ai_b256_str = fmt(ai_b256, precision=1, allow_zero=True, commas=False)
    fp_t4_ridge_str = fmt(t4_ridge, precision=1, allow_zero=True, commas=False)
    fp_t4_flops_str = fmt(t4_flops, precision=1, allow_zero=True, commas=False)
    fp_t4_bw_str    = fmt(t4_bw, precision=1, allow_zero=True, commas=False)
    fp_a100_roof_b1_str = fmt(a100_roof_b1, precision=1, commas=False)
    fp_t4_roof_b1_str = fmt(t4_roof_b1, precision=1, commas=False)
```

**Pitfall**: *Deploying small-batch inference workloads on high-compute accelerators.*

Teams deploy high-throughput training accelerators (A100, H100) for latency-sensitive inference with batch size 1–4. As the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff) predicts, small batches severely reduce arithmetic intensity: a dense layer with M=N=2048 achieves AI = `{python} FpSmallBatchCalc.fp_ai_b1_str` FLOP/byte at batch=1 vs. AI = `{python} FpSmallBatchCalc.fp_ai_b256_str` FLOP/byte at batch=256. At batch=1, the memory-bound roofline ceiling is only about `{python} FpSmallBatchCalc.fp_a100_roof_b1_str` TFLOP/s on A100 and `{python} FpSmallBatchCalc.fp_t4_roof_b1_str` TFLOP/s on T4. The T4's peak is `{python} FpSmallBatchCalc.fp_t4_flops_str` TFLOP/s (FP16 Tensor Core) with a ridge point of `{python} FpSmallBatchCalc.fp_t4_ridge_str` FLOP/byte (`{python} FpSmallBatchCalc.fp_t4_flops_str` TFLOP/s / `{python} FpSmallBatchCalc.fp_t4_bw_str` GB/s). Small-batch inference remains memory bound on both accelerators, so the T4's lower cost can make it more economical despite its much lower peak compute. Training-class accelerator instances often rent for several times more than inference-class instances for little latency gain in this regime. Inference deployments should match batch size to accelerator characteristics, using high-compute accelerators only for batched serving where arithmetic intensity exceeds ridge points.
...
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.formulas (model_memory),
# │          mlsysim.constants (GB, BYTES_FP16)
# │ Exports: FeasibilityAssessment.headroom_str,
# │          FeasibilityAssessment.token_latency_ms_str,
# │          FeasibilityAssessment.frame_budget_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, check
from mlsysim.core.formulas import model_memory
...
class FeasibilityAssessment:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    headroom_str = fmt(headroom_gb, precision=1, allow_zero=True, commas=False)
    token_latency_ms_str = fmt(token_latency_ms, precision=1, allow_zero=True, commas=False)
    frame_budget_str = fmt(frame_budget_ms, precision=1, allow_zero=True, commas=False)
```
...
*   **Formula**: $M_{\text{req}} = \text{Weights} + \text{KV Cache} + \text{Activation Buffer}$
*   **Constraint**: $M_{\text{req}} < M_{\text{device}}$
*   **Example**: Running a 7-billion-parameter Llama model (14 GB weights at FP16) on a 16 GB GPU leaves only `{python} FeasibilityAssessment.headroom_str` GB for context. Long prompts will OOM (Out of Memory).

**Bandwidth check**

*   **Formula**: $T_{\text{token}} = \frac{D_{\text{vol}}}{\text{BW}}$, where $D_{\text{vol}}$ is the model bytes streamed for the token.
*   **Constraint**: $T_{\text{token}} < \text{Latency Target}$
*   **Example**: Serving a 70-billion-parameter model (140 GB) on a GPU with 1 TB/s bandwidth yields ≈ `{python} FeasibilityAssessment.token_latency_ms_str` ms per token. If a workload requires 50 ms latency, this hardware fails regardless of compute power.

**Compute check**

*   **Formula**: $T_{\text{process}} = \frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}$
*   **Constraint**: $T_{\text{process}} < \text{Throughput Target}$
*   **Example**: Processing video at 30 FPS requires completing all inference within `{python} FeasibilityAssessment.frame_budget_str` ms.
...
These fallacies and pitfalls reveal a recurring theme: optimizing for the wrong metric wastes resources. "Resources," however, extends beyond compute time and engineer-hours. As AI systems scale to planetary deployment, with millions of GPUs consuming megawatts of power, the environmental cost of suboptimal hardware choices accumulates at societal scale. The same architectural principles that maximize performance per watt also minimize carbon per inference, making efficiency optimization both an engineering imperative and an environmental one.
...
Beyond raw performance, we must evaluate hardware through the lens of silicon sustainability\index{Sustainability!silicon design}. In the era of planetary-scale AI, performance per watt\index{Performance per W!efficiency metric}\index{Energy Efficiency!TFLOP/s per W} is not merely a mobile constraint but a global environmental mandate. A quick fleet calculation makes the environmental stakes concrete.
...
# │
# │ Imports: mlsysim.core.constants (DAYS_PER_YEAR), mlsysim.book (fmt)
# │ Exports: CarbonRoiCalc.cpu_power_str, CarbonRoiCalc.npu_power_str,
# │          CarbonRoiCalc.cpu_tflops_str, CarbonRoiCalc.npu_tflops_str,
# │          CarbonRoiCalc.cpu_eff_str, CarbonRoiCalc.npu_eff_str,
# │          CarbonRoiCalc.eff_gap_str, CarbonRoiCalc.workload_str,
# │          CarbonRoiCalc.cpu_energy_day_str, CarbonRoiCalc.npu_energy_day_str,
...
class CarbonRoiCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    cpu_power_w        = 100    # W per CPU server doing inference
    cpu_tflops         = 1      # Peak TFLOP/s for CPU inference
    cpu_servers        = 1000   # Fleet size for 1B inferences/day example
    npu_power_w        = 5      # W per NPU chip
    npu_tflops         = 10     # Peak TFLOP/s for NPU inference
    npu_chips          = 100    # Same throughput class as 1000 CPUs
    inferences_per_day = 1 * BILLION    # 1 billion inferences/day
    carbon_kg_per_kwh  = 0.4    # Approximate global grid average (kg CO2e/kWh)
    cpu_energy_kwh_day = (cpu_servers * cpu_power_w * watt).m_as(kilowatt) * HOURS_PER_DAY
    npu_energy_kwh_day = (npu_chips * npu_power_w * watt).m_as(kilowatt) * HOURS_PER_DAY
    ...
    co2_saved_metric_tons  = co2_saved_kg_year / THOUSAND   # kg → metric ton (1 t = 1000 kg)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(eff_gap == 200, f"Efficiency gap should be 200×, got {eff_gap}×")
    check(co2_saved_metric_tons > 300, f"CO2e savings should exceed 300 tons, got {co2_saved_metric_tons}")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cpu_power_str      = fmt(cpu_power_w, precision=1, allow_zero=True)
    npu_power_str      = fmt(npu_power_w, precision=1, allow_zero=True)
    cpu_servers_str    = fmt(cpu_servers, precision=1, allow_zero=True)
    npu_chips_str      = fmt(npu_chips, precision=1, allow_zero=True)
    cpu_tflops_str     = fmt(cpu_tflops, precision=1, allow_zero=True)
    npu_tflops_str     = fmt(npu_tflops, precision=1, allow_zero=True)
    cpu_eff_str        = fmt(cpu_eff, precision=2)
    npu_eff_str        = fmt(npu_eff, precision=1)
    eff_gap_str        = fmt(eff_gap, precision=1, allow_zero=True)
    workload_str       = fmt(inferences_per_day / BILLION, precision=1, allow_zero=True)
    cpu_energy_day_str = fmt(cpu_energy_kwh_day, precision=1, allow_zero=True)
    npu_energy_day_str = fmt(npu_energy_kwh_day, precision=1, allow_zero=True)
    carbon_intensity_str = fmt(carbon_kg_per_kwh, precision=1)
    co2_saved_str      = fmt(co2_saved_metric_tons, precision=1, allow_zero=True)
```
...
*   **CPU inference**: `{python} CarbonRoiCalc.cpu_power_str` W for `{python} CarbonRoiCalc.cpu_tflops_str` TFLOP/s (efficiency = `{python} CarbonRoiCalc.cpu_eff_str` TFLOP/s per W).
*   **NPU inference**: `{python} CarbonRoiCalc.npu_power_str` W for `{python} CarbonRoiCalc.npu_tflops_str` TFLOP/s (efficiency = `{python} CarbonRoiCalc.npu_eff_str` TFLOP/s per W).
*   **The gap**: The NPU is **`{python} CarbonRoiCalc.eff_gap_str`$\times$ more energy-efficient** per operation.
...
class SummaryRooflineRecap:
    ...
    # GUARD
    check(150 <= a100_ridge <= 160, f"Unexpected A100 summary ridge: {a100_ridge:.1f}")
