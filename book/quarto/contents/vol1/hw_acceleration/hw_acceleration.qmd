...
# │
# │ Imports: mlsysim.core.constants (H100_TOPS_INT8), mlsysim.book (fmt)
...
from mlsysim.core.constants import (
    H100_TOPS_INT8, TOPS, TFLOPs, second,
...
from mlsysim.core.constants import (
    ...
    MOBILE_NPU_TOPS_INT8, TOPS, TFLOPs, second,
)
...
class CpuMlInefficiency:
    ...
    mobile_tops = f"{MOBILE_NPU_TOPS_INT8.m_as(TOPS):.0f}"
```
...
$$
\mathcal{M}_{\text{placement}} = n^{d_{\text{comp}} \times N_{\text{mem}}}
$$
where:
...
$$
\mathcal{S}_{\text{mapping}} = \left( n^{d_{\text{comp}}} \times n_{\text{loops}}! \times \frac{n_{\text{loops}}!}{(n_{\text{loops}}-k_{\text{parallel}})!} \right)^{N_{\text{mem}}}
$$
where:
