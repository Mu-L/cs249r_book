...
from mlsysim.core.constants import (
    A100_FLOPS_FP16_TENSOR, A100_MEM_BW, TFLOPs, TB, second,
)
...
class HwAccelSetup:
    ...
    cpu_gap_min_str      = fmt(cpu_gap_min, precision=0, commas=False)
    cpu_gap_max_str      = fmt(cpu_gap_max, precision=0, commas=False)
```
...
***Hardware Acceleration***\index{Hardware Acceleration!definition} is the practice of replacing general-purpose processor logic with domain-specific silicon optimized for a narrow class of operations, trading programmability for the compute density $(R_{\text{peak}})$ and energy efficiency $(\eta_{\text{hw}})$ gains that regular, data-parallel workloads like matrix multiplication can exploit.
...
Hardware acceleration targets specific terms in the iron law of ML systems (@sec-introduction-iron-law-ml-systems-c32a), which decomposes end-to-end time into data volume $(D_{\text{vol}}/\text{BW})$, computation $(O/(R_{\text{peak}} \cdot \eta_{\text{hw}}))$, and fixed latency $(L_{\text{lat}})$. While data selection reduced the total data and model compression reduced $O$ per sample, hardware acceleration increases the rate at which those operations execute by improving $R_{\text{peak}}$, $\eta_{\text{hw}}$, and $\text{BW}$. Yet acceleration has a hard ceiling, established by Amdahl's Law[^fn-amdahls-law-acceleration].
...
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Visualize speedup = 1/((1-p)+p/S) as function of S and p; show
import sys
import os
import numpy as np
...
# │
# │ Imports: mlsysim.core.constants (H100_FLOPS_INT8), mlsysim.book (fmt)
# │ Exports: AmdahlsH100.amdahl_resnet_str, AmdahlsH100.amdahl_gpt2_str,
# │          AmdahlsH100.hw_speedup_str, AmdahlsH100.h100_tflops_int8_str
# └─────────────────────────────────────────────────────────────────────────────
...
class AmdahlH100:
    ...
    # Step 1: Theoretical ceiling (if s -> infinity)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Hardware context
    h100_tflops_int8_str = fmt(H100_FLOPS_INT8.m_as(TFLOPs/second), precision=0)
...
::: {#lhs-amdahl-h100 .callout-lighthouse title="Amdahl's Law on H100"}

**ResNet-50 inference on NVIDIA H100**:

- H100 delivers $G_{\text{accel}}$ = `{python} AmdahlH100.hw_speedup_str`$\times$ speedup over CPU for matrix multiply (`{python} AmdahlH100.h100_tflops_int8_str` TOPS INT8 vs. ~8 TOPS on baseline CPU without AMX extensions)
...
# │
# │ Imports: mlsysim.core.constants (A100_FLOPS*, MOBILE_NPU_TOPS_INT8)
...
class WeightMatrixCalc:
    ...
    # OUTPUT
    wm_params_str = fmt(wm_params_value, precision=0, commas=True)
...
::: {#lst-loop_level_dense lst-cap="**Processor-Level Execution**: Nested loops reveal the $O(\text{batch} \times \text{input} \times \text{output})$ multiply-accumulate operations that accelerators must execute, with `{python} WeightMatrixCalc.wm_batch_macs_m_str` million MACs for batch=`{python} WeightMatrixCalc.wm_batch_str`, input=`{python} WeightMatrixCalc.wm_in_str`, output=`{python} WeightMatrixCalc.wm_out_str` configurations."}
...
class AttentionHeadTensorCoreCalc:
    ...
    macs_m_str = fmt(macs / 1e6, precision=0, commas=False)
    flops_m_str = fmt(flops / 1e6, precision=0, commas=False)
    scalar_ms_str = fmt(scalar_ms, precision=0, commas=False)
    tensor_speedup_str = fmt(tensor_speedup, precision=0, commas=False)
```
...
\begin{axis}[
  ...
  precision=0
    ...
    xlabel={Year},
    ylabel={Peak performance (TOPS / TFLOP/s)},
    title style={align=center},
    title={NVIDIA GPU Inference Performance\\
    {\color{black!60}\footnotesize\usefont{T1}{phv}{m}{it} Peak Throughput (TOPS / TFLOP/s), logarithmic scale}},
    axis lines*=left,
...
\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2012,4.4e0) {\textbf{K20X} \\ {\color{black!60}3.9 TFLOP/s}};

\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2014,6.5e0) {\textbf{M40} \\ {\color{black!60}6.8 TOPS}};

\node[anchor=south, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2015.35,0.5e2) {\textbf{P40} \\ {\color{black!60}47 TOPS}};

\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2017.3,1.2e2) {\textbf{V100} \\ {\color{black!60}125 TOPS}};

\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2020.4,1.0e3) {\textbf{A100} \\ {\color{black!60}1,248 TOPS}};

\node[anchor=north, align=center,text=black, font=\usefont{T1}{phv}{m}{n}\fontsize{7pt}{8}\selectfont]
    at (axis cs:2021.0,0.95e4) {\textbf{H100} \\ {\color{black!60}4,000 TFLOP/s}};
%
...
from mlsysim.core.constants import ENERGY_DRAM_ACCESS_PJ, SYSTOLIC_ARRAY_DIM
...
class AcceleratorEfficiencyAnchor:
    ...
    systolic_macs_cycle_str = fmt(systolic_macs_cycle, precision=0)
    # Custom "x" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    energy_dividend_str = fmt(energy_dividend, precision=0, commas=False, suffix="x")
...
class SystolicEnergy:
    ...
    vector_energy_str = fmt(e_vector, precision=0)
...
1.  **Vector unit**: Loads $A$, loads $B$, computes $A \times B + C$, writes $C$.
    ...
    - **Energy**: ≈ `{python} SystolicEnergy.vector_accesses_str` $\times$ `{python} SystolicEnergy.dram_access_str` pJ + `{python} SystolicEnergy.compute_energy_str` pJ (compute) = **`{python} SystolicEnergy.vector_energy_str` pJ/OP**.
...
2.  **Systolic Array (`{python} SystolicEnergy.systolic_size_str` $\times$ `{python} SystolicEnergy.systolic_size_str` size)**: Loads A and B once at the edges. Data "pulses" through `{python} SystolicEnergy.systolic_size_str` processing elements.
    ...
    - **Energy**: ≈ `{python} SystolicEnergy.systolic_access_str` $\times$ `{python} SystolicEnergy.dram_access_str` pJ + `{python} SystolicEnergy.compute_energy_str` pJ (compute) ≈ **`{python} SystolicEnergy.systolic_energy_str` pJ/OP**.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import SYSTOLIC_ARRAY_DIM
from mlsysim.fmt import fmt_percent, fmt, check
...
class TilingPrinciple:
    ...
    tile_count_str = fmt(int(total_tiles), precision=0)
    reuse_factor_str = fmt(reuse_factor, precision=0, commas=False)
```
...
The synchronized data flow ensures that matrix element $A[i,k]$ encounters corresponding $B[k,j]$ elements at precise temporal intervals, executing the multiply-accumulate operations required for matrix multiplication $C[i,j] = \Sigma A[i,k]\times B[k,j]$. This systematic reuse of operands across multiple processing elements substantially reduces memory bandwidth requirements by eliminating redundant data fetches from external memory subsystems.
...
# │
# │ Imports: mlsysim.core.constants (SYSTOLIC_ARRAY_DIM, TPUV4_MEM_BW),
# │          mlsysim.book (fmt)
# │ Exports: SystolicOpsCalc.systolic_dim_str, SystolicOpsCalc.systolic_ops_str,
# │          SystolicOpsCalc.tpuv4_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import SYSTOLIC_ARRAY_DIM, TPUV4_MEM_BW, GB, second
...
class SystolicOpsCalc:
    ...
    tpuv4_bw_gbs       = TPUV4_MEM_BW.m_as(GB/second)
    ...
    systolic_ops_str = fmt(systolic_ops_value, precision=0)
    tpuv4_bw_str     = fmt(int(tpuv4_bw_gbs), precision=0)
```
...
| **Ampere**\index{NVIDIA!Ampere architecture} |     2020 | FP64, TF32\index{TF32!tensor float 32}, bfloat16, FP16, INT8, INT4 | FP64, FP32, FP16, bfloat16, INT8   |
...
from mlsysim.core.constants import GB, second, TB, PCIE_GEN5_BW, INFINIBAND_NDR_BW, NVLINK_H100_BW
...
class InterconnectBandwidth:
    ...
    nvlink_bw_str = fmt(int(nvlink_bw), precision=0, commas=False, suffix=" GB/s")
    pcie_bw_str = fmt(int(pcie_bw), precision=0, commas=False, suffix=" GB/s")
    net_bw_str = fmt(int(net_bw), precision=0, commas=False, suffix=" GB/s")
    gap_ratio_str = fmt(int(hbm_bw/pcie_bw), precision=0, commas=False, suffix="x")
...
Consider an organization choosing between “more of an older accelerator” vs. “fewer of a newer accelerator.” Peak FLOPS can be misleading for transformer-style workloads with low arithmetic intensity, where training is often memory-bandwidth bound rather than compute bound. In such cases, bandwidth per dollar and achievable utilization can matter more than headline compute, so a newer accelerator with substantially higher bandwidth can deliver materially better sustained performance even if peak FLOPS improves by a smaller factor.
...
# │ Show: Cost per TFLOP across generations (V100→A100→H100, TF32 for H100).
# │ How: Divide list price by peak TFLOPs for each accelerator via Hardware Digital Twin.
# │
# │ Imports: mlsysim.core.constants (V100/A100/H100/TPUV5P FLOPS, \text{BW},
# │          V100_UNIT_COST, A100_UNIT_COST, GAUDI2_FLOPS_FP8, GAUDI2_MEM_BW,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    V100_FLOPS_FP16_TENSOR, V100_MEM_BW, V100_UNIT_COST,
    A100_FLOPS_FP16_TENSOR, A100_MEM_BW, A100_UNIT_COST,
    H100_FLOPS_TF32, H100_FLOPS_FP16_TENSOR, H100_FLOPS_FP8_TENSOR, H100_MEM_BW,
    TPUV4_FLOPS_BF16, TPUV4_MEM_BW,
    GAUDI2_FLOPS_FP8, GAUDI2_MEM_BW,
    TFLOPs, second, GB, TB, USD
)
from mlsysim import Hardware
...
class AcceleratorEconomics:
    ...
    price_v100  = V100_UNIT_COST.m_as(USD)   # older generation
    price_a100  = A100_UNIT_COST.m_as(USD)   # current workhorse
    ...
    gaudi_tf       = GAUDI2_FLOPS_FP8.m_as(TFLOPs/second)
    gaudi_bw_value = GAUDI2_MEM_BW.m_as(GB/second)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # V100 specs
    v100_tflops_str = fmt(h_v100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    v100_bw_str     = fmt(h_v100.memory_bw.m_as(GB/second), precision=0, commas=False)
    # Currency prefix → MarkdownStr per recipe Rule 1 escape-hatch
    v100_price_str = fmt(price_v100, precision=0, prefix="~$")
    v100_pp_str    = fmt(v100_ratio, precision=0, commas=False, prefix="$", suffix="/TFLOP")

    # A100 specs (also used later in roofline sections)
    a100_tflops_fp16_str = fmt(h_a100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    a100_bw_str          = fmt(h_a100.memory_bw.m_as(GB/second), precision=0)
    a100_bw_tbs_str      = fmt(h_a100.memory_bw.m_as(TB/second), precision=2, commas=False)
    a100_price_str   = fmt(price_a100, precision=0, prefix="~$")
    a100_pp_str      = fmt(a100_ratio, precision=0, commas=False, prefix="$", suffix="/TFLOP")

    # H100 specs
    h100_tflops_tf32_str = fmt(h_h100.tf32_flops.m_as(TFLOPs/second), precision=0, commas=False)
    h100_tflops_fp16_str = fmt(h_h100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    h100_tflops_fp8_str  = fmt(H100_FLOPS_FP8_TENSOR.m_as(TFLOPs/second), precision=0)
    h100_bw_str          = fmt(h_h100.memory_bw.m_as(GB/second), precision=0)
    h100_bw_tbs_str      = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False)
    h100_price_str   = fmt(price_h100, precision=0, prefix="~$", suffix="-30,000")
    h100_pp_str      = fmt(h100_ratio, precision=0, commas=False, prefix="~$", suffix="/TFLOP")

    # TPUv4 specs
    tpuv4_tflops_str = fmt(h_tpu.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    tpuv4_bw_str     = fmt(h_tpu.memory_bw.m_as(GB/second), precision=0)
    tpu_price_str = fmt(price_tpu, precision=0, prefix="~$", suffix="*")
    tpu_pp_str    = fmt(tpu_ratio, precision=0, commas=False, prefix="~$", suffix="/TFLOP")

    # Gaudi 2 specs
    gaudi_tflops_str = fmt(gaudi_tf, precision=0, commas=False)
    gaudi_bw_str     = fmt(gaudi_bw_value, precision=0)
    gaudi_price_str = fmt(price_gaudi, precision=0, prefix="~$")
    gaudi_pp_str    = fmt(gaudi_ratio, precision=0, commas=False, prefix="$", suffix="/TFLOP")
```

@tbl-accelerator-economics provides representative cost-performance data for common accelerators. These figures are approximate and vary by vendor, region, and purchase volume; the key insight is the trend rather than the absolute numbers. The cost per TFLOP has improved substantially from V100 to newer accelerators, even as the absolute power requirement (TDP)\index{TDP!Thermal Design Power} has climbed to nearly 1,000 Watts for flagship units, reflecting the industry's shift toward density over raw unit cost. The trend is not strictly monotonic at every generation under all precision modes: under the representative TF32 calculation shown here, H100's price-per-TFLOP runs slightly above A100's because list price scaled faster than TF32 throughput.
...
# │ Show: ~9 PFLOPS dense / ~18 PFLOPS sparse in FP4 per chip.
# │ How: Read B200_FLOPS_INT4 from constants (FP4 tensor throughput) and double
# │      for the sparse case per the constants-file comment.
# │
# │ Imports: mlsysim.core.constants (B200_FLOPS_INT4, PFLOPs, second)
# │ Exports: B200FP4Specs.dense_pflops_str, B200FP4Specs.sparse_pflops_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import B200_FLOPS_INT4, PFLOPs, second
from mlsysim.fmt import fmt

class B200FP4Specs:
    """B200 FP4 peak throughput (dense and sparse)."""
    dense_pflops      = B200_FLOPS_INT4.m_as(PFLOPs/second)
    sparse_pflops     = 2 * dense_pflops
    dense_pflops_str  = fmt(dense_pflops, precision=0, commas=False)
    sparse_pflops_str = fmt(sparse_pflops, precision=0, commas=False)
```

The preceding sections revealed impressive computational machinery: vector units achieving 8$\times$ parallelism through SIMD execution, matrix operations processing 256 elements simultaneously, and tensor cores executing $16{\times}16{\times}16$ fused multiply-accumulate blocks in single cycles. An NVIDIA A100's tensor cores can execute `{python} AcceleratorEconomics.a100_tflops_fp16_str` trillion operations per second, and an H100 pushes this further to nearly two petaFLOPS (dense) in FP8 precision [@blankevoort2022; @micikevicius2022fp8]. At these rates, the pure arithmetic for a ResNet-50 forward pass could complete in microseconds.

Looking ahead, the Blackwell (B200) architecture extends this trend by introducing native FP4 support, with NVIDIA reporting up to `{python} B200FP4Specs.dense_pflops_str` petaFLOPS (dense) or `{python} B200FP4Specs.sparse_pflops_str` petaFLOPS (sparse) peak throughput in FP4 per chip. This confirms the precision bottleneck trend: as models grow, hardware adapts by trading precision for massive parallelism, requiring systems engineers to master progressively lower-bit numerics (FP8, FP4) to unlock the silicon's full potential.
...
1.  **Significance (quantitative)**: It dictates that system performance is no longer bounded by FLOPs, but by the energy and latency cost of moving data. Within the iron law, it is the point where the $\frac{D_{\text{vol}}}{\text{BW}}$ term dominates the total execution time $(T)$.
...
ridges = [
    V100_FLOPS_FP16_TENSOR.m_as(TFLOPs/second) / V100_MEM_BW.m_as(TB/second),
    A100_FLOPS_FP16_TENSOR.m_as(TFLOPs/second) / A100_MEM_BW.m_as(TB/second),
    H100_FLOPS_FP16_TENSOR.m_as(TFLOPs/second) / H100_MEM_BW.m_as(TB/second),
    B200_FLOPS_FP16_TENSOR.m_as(TFLOPs/second) / B200_MEM_BW.m_as(TB/second),
]
...
from mlsysim.core.constants import (
    ...
    LATENCY_HBM3, LATENCY_L2_CACHE, LATENCY_L1_REGISTER,
    A100_FLOPS_FP16_TENSOR, TFLOPs, second, KB,
)
...
class TensorLifecycleCalc:
    ...
    a100_tflops_fp16_str   = fmt(a100_fp16_tflops_value, precision=0, commas=False)
...
::: {#ws-hw-acceleration-0-percent-tensor-core-mystery .callout-war-story title="The 0 percent Tensor Core mystery"}
...
**The failure**: The model ran only 1.2$\times$ faster. Profiling revealed the Tensor Cores were active 0 percent of the time. The team had implemented their custom accumulation kernel in FP32 (32-bit float) to maintain precision.

**The consequence**: Tensor Cores on A100s only trigger for specific precision formats (FP16, BF16, or TF32). By forcing FP32 accumulation in a way the hardware did not support for acceleration, the code fell back to the standard CUDA cores, which have $1/16$th the throughput.

**The systems lesson**: Hardware features are brittle contracts. If the workload does not present the exact data type (FP16/BF16) aligned to the exact dimensions (multiples of 8/16), the accelerator silently degrades to a generic processor. Hardware cannot be exploited without conforming to its contracts [@jia2018highly].
...
class InterconnectHierarchy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_str = fmt(nvlink_a100, precision=0, commas=False)
    nvlink_h100_str = fmt(nvlink_h100, precision=0, commas=False)
    ib_hdr_str = fmt(ib_hdr_gbps, precision=0, commas=False)
    ib_ndr_str = fmt(ib_ndr_gbps, precision=0, commas=False)
```
...
Performance is bounded by two ceilings, as @eq-roofline formalizes. Here, attainable performance and peak compute are in FLOPs/second (often reported as TFLOP/s), peak bandwidth is in bytes/second (often TB/s), and arithmetic intensity is in FLOP/byte:
...
class RooflineGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_tflops_str = fmt(v100_tflops_value, precision=0, commas=False)
    ...
    v100_ridge_str = fmt(v100_ridge, precision=0, commas=False)
    a100_ridge_str = fmt(a100_ridge, precision=0, commas=False)
    h100_ridge_str = fmt(h100_ridge, precision=0, commas=False)
    a100_ridge_fp32_str = fmt(a100_ridge_fp32, precision=0, commas=False)
    ...
    dense_matmul_ai_str = fmt(dense_matmul_ai, precision=0, commas=False)
...
- AI = `{python} TransformerLayerCalc.qkv_flops_b_str` B / `{python} TransformerLayerCalc.qkv_mb_str` M = `{python} TransformerLayerCalc.qkv_ai_str` FLOP/byte, which is **compute bound on A100** (above `{python} RooflineGap.a100_ridge_str` threshold)
...
The roofline model's memory-bound region is determined by the peak memory bandwidth. For an operation to achieve throughput $T_{\text{ops}}$ (FLOPs/second, often expressed in TFLOP/s) in the memory-bound regime, @eq-required-bandwidth gives the required bandwidth:
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import BYTES_FP16, byte, MB, flop, GFLOPs
...
class Conv2dAnalysisCalc:
    ...
    conv_flops_per_out_str = fmt(conv_flops_per_out, precision=0)
...
# │
# │ Imports: mlsysim.core.constants (BYTES_FP16, byte, MB, KiB, flop, MFLOPs, GFLOPs,
...
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import (
    BYTES_FP16, byte, KB, MB, flop, MFLOPs, GFLOPs, TFLOPs,
...
class DenseLayerAnalysisCalc:
    ...
    a100_bw_str            = fmt(a100_bw_gbs_value, precision=0)
```
...
$T_{\text{attainable}}$ = `{python} DenseLayerAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_str` TFLOP/s
...
# │
# │ Imports: mlsysim.core.constants (byte, MB, A100_MEM_BW, GB, GFLOPs, TFLOPs,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt, check
from mlsysim.core.constants import byte, KB, MB, A100_MEM_BW, GB, GFLOPs, TFLOPs, second
...
class LayernormAnalysisCalc:
    ...
    ln_ridge_gap_str    = fmt(ln_ridge_gap, precision=0, commas=False)
...
$T_{\text{attainable}}$ = `{python} LayernormAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte = `{python} LayernormAnalysisCalc.ln_attainable_str` TFLOP/s
...
class BatchAiCalc:
    ...
    # OUTPUT
    batch1_ai_str = fmt(batch1_ai, precision=0, commas=False)
    batch32_ai_str = fmt(batch32_ai, precision=0, commas=False)
    batch256_ai_str = fmt(batch256_ai, precision=0, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (GPT2_PARAMS, A100_*, BYTES_FP16)
...
class Gpt2ThroughputCalc:
    ...
    a100_tflops_fp16_str  = fmt(a100_tflops_fp16_val, precision=0, commas=False)
    a100_bw_tbs_str       = fmt(a100_bw_tbs_val, precision=2, commas=False)
    a100_tflops_fp32_str  = fmt(A100_FLOPS_FP32.m_as(TFLOPs/second), precision=1, commas=False)
```
...
$$
\mathcal{O}_{\text{order}} = n_{\text{loops}}!
$$
...
$$
(n_{\text{loops}}!)^{L_{\text{mem}}}
$$
where $L_{\text{mem}}$ is the number of memory hierarchy levels. This rapid expansion shows why execution order optimization matters: poor loop ordering can lead to excessive memory traffic, while an optimized order improves cache utilization [@sze2017].
...
$$
\mathcal{M}_{\text{placement}} = n^{d_{\text{comp}} \times L_{\text{mem}}}
$$
where:

- $n$ = number of placement choices per level,
- $d_{\text{comp}}$ = number of computational dimensions,
- $L_{\text{mem}}$ = number of memory hierarchy levels.

For a model with:

- $d_{\text{comp}} = 5$ computational dimensions,
- $L_{\text{mem}} = 3$ memory levels,
...
$$
\mathcal{S}_{\text{mapping}} = \left( n^{d_{\text{comp}}} \times n_{\text{loops}}! \times \frac{n_{\text{loops}}!}{(n_{\text{loops}}-k_{\text{parallel}})!} \right)^{L_{\text{mem}}}
$$
where:

- $n^{d_{\text{comp}}}$ represents memory placement choices,
...
- $L_{\text{mem}}$ is the number of memory hierarchy levels.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import BYTES_FP32, byte, MB
...
*Spatial tiling*\index{Tiling!spatial}\index{Tiling!spatial, data partitioning} partitions data structures into smaller blocks that fit within fast memory. The tiled matrix multiplication in @lst-tiled_matmul demonstrates cache-friendly loop blocking: the code does not issue explicit scratchpad loads, but the tile-shaped access pattern lets hardware caches reuse nearby values before they are evicted. This strategy is particularly beneficial for large tensors that exceed fast memory capacity—by breaking computations into smaller tiles, data movement between memory levels is minimized, keeping operations localized within cache hierarchies.
...
class CompilerSpeedupCalc:
    ...
    naive_inference_ms_str     = fmt(naive_inference_ms, precision=0, commas=False)
    optimized_inference_ms_str = fmt(optimized_inference_ms, precision=0, commas=False)
...
# │
# │ Imports: mlsysim.core.constants (A100_TDP)
...
class RuntimeProductionTdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tdp_str = fmt(A100_TDP.m_as(watt), precision=0, commas=False)
```
...
```{python}
#| label: gpt3-single-h100-time-calc
#| echo: false
from mlsysim.core.constants import H100_FLOPS_FP8_TENSOR, PFLOPs, second, SEC_PER_YEAR
from mlsysim.fmt import fmt, check
...
class Gpt3SingleH100Calc:
    ...
    # LOAD
    gpt3_training_flops = 3.14e23
    h100_pflops_fp8 = H100_FLOPS_FP8_TENSOR.m_as(PFLOPs/second)
    realistic_util_low = 0.40
    realistic_util_high = 0.60

    # EXECUTE
    peak_seconds = gpt3_training_flops / (h100_pflops_fp8 * 1e15)
    ...
    check(realistic_years_high > realistic_years_low > peak_years, "Utilization-adjusted years must exceed peak lower bound.")

    # OUTPUT
    h100_pflops_fp8_str = fmt(h100_pflops_fp8, precision=1, commas=False)
...
class FpWorkloadFitRecap:
    ...
    # LOAD
    a100_peak_tflops = A100_FLOPS_FP16_TENSOR.m_as(TFLOPs/second)
    a100_bw_tbs = A100_MEM_BW.m_as(TB/second)
    softmax_ai_low = 2
    softmax_ai_high = 5
    cpu_ridge_low = 10
    ...
    # OUTPUT
    a100_ridge_str = fmt(a100_ridge, precision=0, commas=False)
    a100_tflops_fp16_str = fmt(a100_peak_tflops, precision=0, commas=False)
    softmax_ai_low_str = fmt(softmax_ai_low, precision=0, commas=False)
    softmax_ai_high_str = fmt(softmax_ai_high, precision=0, commas=False)
    softmax_tflops_low_str = fmt(softmax_tflops_low, precision=0, commas=False)
    softmax_tflops_high_str = fmt(softmax_tflops_high, precision=0, commas=False)
    softmax_util_high_str = fmt(softmax_util_high, precision=0, commas=False)
    cpu_ridge_low_str = fmt(cpu_ridge_low, precision=0, commas=False)
    cpu_ridge_high_str = fmt(cpu_ridge_high, precision=0, commas=False)
    cpu_softmax_util_low_str = fmt(cpu_softmax_util_low, precision=0, commas=False)
    cpu_softmax_util_high_str = fmt(cpu_softmax_util_high, precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import ENERGY_DRAM_ACCESS_PJ, ENERGY_SRAM_L1_PJ
...
class FpMemoryEnergyCalc:
    ...
    fp_dram_pj_str          = fmt(dram_pj, precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import NVLINK_A100_BW, GB, second
...
class FpMultigpuScalingCalc:
    ...
    fp_no_overlap_efficiency_str = fmt(no_overlap_efficiency_pct, precision=0, commas=False)
```
...
#| echo: false
from mlsysim.core.constants import A100_FLOPS_FP16_TENSOR, TFLOPs, second
from mlsysim.fmt import fmt, check

class FpPeakVsSustainedRecap:
    """A100 peak and sustained ranges for the peak-FLOPS fallacy."""
...
# │
# │ Imports: mlsysim.core.constants (T4_FLOPS_FP16_TENSOR, T4_MEM_BW, A100_MEM_BW, TFLOPs,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_percent, fmt
from mlsysim.core.constants import T4_FLOPS_FP16_TENSOR, T4_MEM_BW, A100_MEM_BW, TFLOPs, second, GB, TB
...
class FpSmallBatchCalc:
    ...
    t4_bw        = T4_MEM_BW.m_as(GB/second)             # for "GB/s" prose display
    a100_bw      = A100_MEM_BW.m_as(GB/second)
    t4_bw_tbs    = T4_MEM_BW.m_as(TB/second)             # for dimensional roofline math
    a100_bw_tbs  = A100_MEM_BW.m_as(TB/second)
...
from mlsysim.fmt import fmt_percent, fmt, check
from mlsysim.core.formulas import model_memory
from mlsysim.core.constants import GB, BYTES_FP16
...
*   **Example**: Running Llama-7B (14 GB weights @ FP16) on a 16 GB GPU leaves only `{python} FeasibilityAssessment.headroom_str` GB for context. Long prompts will OOM (Out of Memory).

**Bandwidth check**

*   **Formula**: $T_{\text{token}} = \frac{D_{\text{vol,model}}}{\text{BW}}$
...
from mlsysim.fmt import fmt_percent, fmt, check
from mlsysim.core.constants import (
    DAYS_PER_YEAR, HOURS_PER_DAY, THOUSAND, BILLION,
    watt, kilowatt,
)
...
class CarbonRoiCalc:
    ...
    workload_str       = fmt(inferences_per_day / BILLION, precision=0)
...
*   **CPU inference**: `{python} CarbonRoiCalc.cpu_power_str` Watts for `{python} CarbonRoiCalc.cpu_tflops_str` TFLOP (Efficiency = `{python} CarbonRoiCalc.cpu_eff_str` TFLOP/s/W).
*   **NPU inference**: `{python} CarbonRoiCalc.npu_power_str` Watts for `{python} CarbonRoiCalc.npu_tflops_str` TFLOP/s (Efficiency = `{python} CarbonRoiCalc.npu_eff_str` TFLOP/s/W).
...
* **The Roofline model identifies performance bottlenecks**: Plotting arithmetic intensity against throughput reveals whether workloads are memory bound (attention, embeddings) requiring bandwidth optimization, or compute bound (high-reuse convolutions, GEMMs) requiring FLOPS optimization.
