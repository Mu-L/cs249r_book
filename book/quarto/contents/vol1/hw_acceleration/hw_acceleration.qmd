...
# │
# │ Imports: mlsysim.core.constants (V100/A100/H100/TPUV5P FLOP/s, \text{BW},
...
The preceding sections revealed impressive computational machinery: vector units achieving 8$\times$ parallelism through SIMD execution, matrix operations processing 256 elements simultaneously, and tensor cores executing $16{\times}16{\times}16$ fused multiply-accumulate blocks in single cycles. An NVIDIA A100's tensor cores can execute `{python} AcceleratorEconomics.a100_tflops_fp16_str` trillion operations per second, and an H100 pushes this further to nearly two petaFLOP/s (dense) in FP8 precision [@blankevoort2022; @micikevicius2022fp8]. At these rates, the pure arithmetic for a ResNet-50 forward pass could complete in microseconds.

Looking ahead, the Blackwell (B200) architecture extends this trend by introducing native FP4 support, with NVIDIA reporting up to `{python} B200FP4Specs.dense_pflops_str` petaFLOP/s (dense) or `{python} B200FP4Specs.sparse_pflops_str` petaFLOP/s (sparse) peak throughput in FP4 per chip. This confirms the precision bottleneck trend: as models grow, hardware adapts by trading precision for massive parallelism, requiring systems engineers to master progressively lower-bit numerics (FP8, FP4) to unlock the silicon's full potential.
...
The roofline model's memory-bound region is determined by the peak memory bandwidth. For an operation to achieve throughput $T_{\text{ops}}$ (FLOP/s, often expressed in TFLOP/s) in the memory-bound regime, @eq-required-bandwidth gives the required bandwidth:
$$\text{Required Bandwidth} = \frac{T_{\text{ops}}}{\text{AI}} \text{ bytes/s}$$ {#eq-required-bandwidth}

When required bandwidth exceeds peak bandwidth, performance is capped according to @eq-attainable-throughput. Here $T_{\text{ops}}$ and $T_{\text{attainable}}$ are in FLOP/s and AI is in FLOP/byte.
...
*   **CPU inference**: `{python} CarbonRoiCalc.cpu_power_str` Watts for `{python} CarbonRoiCalc.cpu_tflops_str` TFLOP/s (efficiency = `{python} CarbonRoiCalc.cpu_eff_str` TFLOP/s per watt).
*   **NPU inference**: `{python} CarbonRoiCalc.npu_power_str` Watts for `{python} CarbonRoiCalc.npu_tflops_str` TFLOP/s (efficiency = `{python} CarbonRoiCalc.npu_eff_str` TFLOP/s per watt).
