...
|:-----------------------------------|:------------------------------------------------|------------------------------------------------------------:|--------------------------------------------------:|:---------------------------------------------|
| **NVIDIA V100**\index{NVIDIA!V100} | `{python} AcceleratorEconomics.v100_price_str`  |             `{python} AcceleratorEconomics.v100_tflops_str` |  `{python} AcceleratorEconomics.v100_bw_str` GB/s | `{python} AcceleratorEconomics.v100_pp_str`  |
| **NVIDIA A100**\index{NVIDIA!A100} | `{python} AcceleratorEconomics.a100_price_str`  |        `{python} AcceleratorEconomics.a100_tflops_fp16_str` |  `{python} AcceleratorEconomics.a100_bw_str` GB/s | `{python} AcceleratorEconomics.a100_pp_str`  |
| **NVIDIA H100**                    | `{python} AcceleratorEconomics.h100_price_str`  | `{python} AcceleratorEconomics.h100_tflops_tf32_str` (TF32) |  `{python} AcceleratorEconomics.h100_bw_str` GB/s | `{python} AcceleratorEconomics.h100_pp_str`  |
| **Google TPUv4**                   | `{python} AcceleratorEconomics.tpu_price_str`   |     `{python} AcceleratorEconomics.tpuv4_tflops_str` (BF16) | `{python} AcceleratorEconomics.tpuv4_bw_str` GB/s | `{python} AcceleratorEconomics.tpu_pp_str`   |
| **Intel Gaudi 2**                  | `{python} AcceleratorEconomics.gaudi_price_str` |      `{python} AcceleratorEconomics.gaudi_tflops_str` (FP8) | `{python} AcceleratorEconomics.gaudi_bw_str` GB/s | `{python} AcceleratorEconomics.gaudi_pp_str` |
...
*   **V100 (2017)**: `{python} RooflineGap.v100_tflops_str` / `{python} RooflineGap.v100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.v100_ridge_str` FLOP/byte**.
*   **A100 (2020)**: `{python} RooflineGap.a100_tflops_fp16_str` / `{python} RooflineGap.a100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.a100_ridge_str` FLOP/byte**.
*   **H100 (2023)**: `{python} RooflineGap.h100_tflops_fp16_str` / `{python} RooflineGap.h100_bw_tbs_str` TB/s ≈ **`{python} RooflineGap.h100_ridge_str` FLOP/byte**.
...
$R_{\text{attainable}}$ = `{python} DenseLayerAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} DenseLayerAnalysisCalc.dense_ai_str` FLOP/byte = `{python} DenseLayerAnalysisCalc.dense_attainable_str`
...
$R_{\text{attainable}}$ = `{python} LayernormAnalysisCalc.a100_bw_str` GB/s $\times$ `{python} LayernormAnalysisCalc.ln_ai_str` FLOP/byte = `{python} LayernormAnalysisCalc.ln_attainable_str`
...
*   **Maximum throughput**: `{python} Gpt2ThroughputCalc.gpt2_decode_ai_str` FLOP/byte $\times$ `{python} Gpt2ThroughputCalc.a100_bw_tbs_str` TB/s = **`{python} Gpt2ThroughputCalc.gpt2_max_tflops_str`**.
*   **Utilization ceiling**:
...
Teams expect eight GPUs to train 8$\times$ faster than one GPU. Multi-accelerator scaling introduces communication overhead that violates linear scaling assumptions. As noted in @sec-hardware-acceleration-multichip-scaling-c649, AllReduce operations for gradient synchronization can require exchanging hundreds of gigabytes per training step for large models. With NVLink at `{python} FpMultigpuScalingCalc.fp_nvlink_bw_str` GB/s bidirectional, synchronizing 1 GB of gradients requires `{python} FpMultigpuScalingCalc.fp_sync_time_str`; for a 50 ms training step, this represents `{python} FpMultigpuScalingCalc.fp_sync_overhead_str` overhead with perfect overlap. Without overlap, eight-GPU setups achieve about `{python} FpMultigpuScalingCalc.fp_no_overlap_speedup_str`$\times$ speedup (`{python} FpMultigpuScalingCalc.fp_no_overlap_efficiency_str` efficiency) at best, and typical workloads see 6--7$\times$ (75--87 percent efficiency) due to load imbalance and synchronization barriers. Small models with insufficient parallel work achieve even worse scaling, sometimes seeing 3--4$\times$ speedup on eight GPUs (37--50 percent efficiency).
...
Teams deploy high-throughput training accelerators (A100, H100) for latency-sensitive inference with batch size 1–4. As the Roofline Model (@sec-hardware-acceleration-roofline-model-42ff) predicts, small batches severely reduce arithmetic intensity: a dense layer with M=N=2048 achieves AI = `{python} FpSmallBatchCalc.fp_ai_b1_str` FLOP/byte at batch=1 vs. AI = `{python} FpSmallBatchCalc.fp_ai_b256_str` FLOP/byte at batch=256. At batch=1, the memory-bound roofline ceiling is only about `{python} FpSmallBatchCalc.fp_a100_roof_b1_str` on A100 and `{python} FpSmallBatchCalc.fp_t4_roof_b1_str` on T4. The T4's peak is `{python} FpSmallBatchCalc.fp_t4_flops_str` (FP16 Tensor Core) with a ridge point of `{python} FpSmallBatchCalc.fp_t4_ridge_str` FLOP/byte (`{python} FpSmallBatchCalc.fp_t4_flops_str` / `{python} FpSmallBatchCalc.fp_t4_bw_str` GB/s). Small-batch inference remains memory bound on both accelerators, so the T4's lower cost can make it more economical despite its much lower peak compute. Training-class accelerator instances often rent for several times more than inference-class instances for little latency gain in this regime. Inference deployments should match batch size to accelerator characteristics, using high-compute accelerators only for batched serving where arithmetic intensity exceeds ridge points.
...
class CarbonRoiCalc:
    ...
    carbon_intensity_str = fmt(carbon_kg_per_kwh, precision=2, suffix=" kg/kWh")
...
*   **CPU inference**: `{python} CarbonRoiCalc.cpu_power_str` for `{python} CarbonRoiCalc.cpu_tflops_str` (efficiency = `{python} CarbonRoiCalc.cpu_eff_str` TFLOP/s per W).
*   **NPU inference**: `{python} CarbonRoiCalc.npu_power_str` for `{python} CarbonRoiCalc.npu_tflops_str` (efficiency = `{python} CarbonRoiCalc.npu_eff_str` TFLOP/s per W).
