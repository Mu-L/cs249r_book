...
- **FarmBeats**\index{Deployment!resource-constrained edge, precision agriculture}[^fn-farmbeats-edge] is Microsoft Research's precision agriculture platform, deploying ML models to farms with limited connectivity. FarmBeats represents the *resource-constrained edge* deployment pattern: models under 500 kilobytes run inference on low-power devices using TV white-space bandwidth measured in kilobits per second.
