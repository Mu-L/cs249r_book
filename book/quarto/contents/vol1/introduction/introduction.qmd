...
**Systems insight**: If we improve hardware utilization $(\eta_{\text{hw}})$ from `{python} GPT3Training.eta_base_pct_str` percent to `{python} GPT3Training.eta_opt_pct_str` percent through kernel fusion and better scheduling, training time drops to **`{python} GPT3Training.days_optimized_str` days**, saving nearly `{python} GPT3Training.days_saved_str` days of expensive compute time.
