...
class AIMomentStats:
    ...
    h100_fp16_tflops_str = fmt(gpu_tflops, precision=1, commas=False)
...
class GPT3WeightFootprint:
    ...
    fp16_weights_gb_str = fmt(fp16_weights_gb, precision=1, commas=False)
```
...
class VerificationGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    vg_digits_str = fmt(digits, precision=1, commas=True)
    imagenet_test_images_str = fmt(test_size, precision=1, commas=True)
```
...
class AlexNetBreakthrough:
    ...
    alexnet_relative_improvement_str = fmt(relative_improvement, precision=1)
    alexnet_params_m_str = fmt(params_m, precision=1)
```
...
class GPT3Scale:
    ...
    data_scale_gb_str = fmt(data_gb, precision=1, commas=False)
```
...
class GPT4ScaleEstimate:
    ...
    training_days_str = fmt(training_days, precision=1, commas=False)
    gpu_days_m_str = fmt(gpu_days_m, precision=0, commas=False)
```
...
class AlphaGoZeroCompute:
    ...
    tpus_str = fmt(tpus, precision=1, commas=False)
...
class GPT3TrainingEnergy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    energy_mwh_str = fmt(energy_mwh, precision=1, commas=True)
    households_str = fmt(households, precision=1, commas=False)
...
class EmailScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gmail_emails_t_str = fmt(gmail_emails_t_value, precision=1)
```
...
class ResNet50DamExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weight_mb_str = fmt(weight_mb, precision=1, commas=False)
    a100_hbm_tbs_str = fmt(a100_hbm_tbs, precision=1, commas=False)
    memory_time_us_str = fmt(memory_time_us, precision=1, commas=False)
    resnet_gflops_str = fmt(resnet_gflops, precision=1, commas=False)
    a100_peak_tflops_str = fmt(a100_peak_tflops, precision=1, commas=False)
    compute_time_us_str = fmt(compute_time_us, precision=1, commas=False)
    a100_ridge_str = fmt(a100_ridge, precision=1, commas=False)
    workload_ai_str = fmt(workload_ai, precision=1, commas=False)
```
...
class DeploymentSystems:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_mem_str = fmt(t_cloud.ram.m_as(GB), precision=1)
    cloud_compute_str = fmt(h_cloud.peak_flops.m_as(TFLOPs/second), precision=1)
    cloud_power_str = fmt(h_cloud.tdp.m_as(watt), precision=1)

    edge_mem_str = fmt(t_edge.ram.m_as(GB), precision=1)
    edge_compute_str = fmt(h_edge.peak_flops.m_as(TFLOPs/second), precision=1)
    edge_power_str = fmt(h_edge.tdp.m_as(watt), precision=1)

    mobile_mem_str = fmt(t_mobile.ram.m_as(GB), precision=1)
    mobile_compute_str = fmt(h_mobile.peak_flops.m_as(TOPS), precision=1, commas=False)
    mobile_power_str = fmt(h_mobile.tdp.m_as(watt), precision=1)

    tiny_mem_str = fmt(t_tiny.ram.m_as(KB), precision=1, commas=False)
    tiny_compute_str = fmt(h_tiny.peak_flops.m_as(TFLOPs/second), precision=4)
    tiny_power_str = fmt(h_tiny.tdp.m_as(milliwatt), precision=1)
...
class GPT3Training:
    ...
    days_initial_str = fmt(days_base, precision=1, commas=False)
    days_optimized_str = fmt(days_opt, precision=1, commas=False)
    days_saved_str = fmt(days_saved, precision=1, commas=False)

    # LaTeX math components
    _ops_mag = f"{ops:.2e}"
    _ops_coeff = _ops_mag.split("e+")[0]
    ops_exp_value = int(_ops_mag.split("e+")[1])
    peak_tflops_str = fmt(peak_tflops, precision=1, commas=False)
...
class EnergyMovementRatios:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fp16_ratio_str = fmt(fp16_ratio, precision=1, commas=False)
    int8_ratio_str = fmt(int8_ratio, precision=1, commas=False)
```
...
class TrainingComputeGrowth:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compute_growth_order_str = fmt(compute_growth_order, precision=1, commas=False)
    compute_growth_m_str = fmt(compute_growth_m, precision=1, commas=False)
```
...
class EfficiencyGains:
    ...
    algo_halving_months_str = fmt(algo_halving_months, precision=1, commas=False)
...
class ScenarioRegistry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_limit_str = MarkdownStr(f"SLA: {fmt(frontier_latency_ms, precision=1)} ms latency")
    drive_limit_str    = MarkdownStr(f"SLA: {fmt(drive_latency_ms, precision=1)} ms latency")
    doorbell_limit_str = MarkdownStr(
        f"Power: {fmt(doorbell_power_mw, precision=1)} mW"
        if doorbell_power_mw is not None
        else f"SLA: {fmt(doorbell_latency_ms, precision=1)} ms latency"
    )
```
...
class WaymoStats:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    low_str = fmt(val_low, precision=1, commas=False)
    high_str = fmt(val_high, precision=1, commas=False)
```
...
class AmdahlsPitfall:
    ...
    t_inference_str = fmt(t_inference, precision=1, commas=False)
    t_inf_new_str = fmt(t_inf_new, precision=1, commas=False)
    t_pre_str = fmt(t_pre, precision=1, commas=False)
    t_post_str = fmt(t_post, precision=1, commas=False)
    total_ms_str = fmt(t_total, precision=1, commas=False)
    new_total_ms_str = fmt(t_total_new, precision=1, commas=False)
    improv_pct_str = fmt(improvement_pct, precision=1, commas=False)
    naive_p_str = fmt(naive_pct, precision=1, commas=False)
```
...
class DriftFallacy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    acc_initial_str = fmt(acc_initial, precision=1, commas=False)
    ...
    months_str = fmt(months, precision=1, commas=False)
```
