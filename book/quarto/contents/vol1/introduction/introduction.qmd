...
# │ How: Compare daily search counts (SI units) and TFLOPS density (H100 vs. CPU).
# │
# │ Imports: mlsysim.core.constants (GOOGLE_SEARCHES_PER_DAY, H100/CPU FLOPS)
...
**Systems lesson**: "Machine Learning" is easy; **"Machine Learning Systems"** are hard. The friction in deployment rarely comes from the matrix multiplication alone; it comes from the interface between that math and the messy reality of the surrounding system. Optimizing only the model optimizes the visible center of a much larger engineering problem.

:::

The same lesson becomes sharper when the training signal itself is a behavioral proxy rather than a direct measurement.

::: {#ws-introduction-google-flu-trends .callout-war-story title="When search logs mistook attention for illness"}

**Context**: Google Flu Trends estimated influenza activity from search queries, making it a canonical example of a data-rich predictive system built on behavioral traces rather than direct measurement [@lazer2014].

**Failure**: The proxy drifted. Search behavior changed with media attention, product behavior, and user habits, so a model that had looked powerful against historical flu data began overstating real influenza activity.

**Consequence**: During the 2011--2013 period analyzed by researchers, the system repeatedly overpredicted flu prevalence and showed that large-scale behavioral data can amplify rather than remove measurement error.

**Systems lesson**: Data volume is not ground truth. ML systems need feedback loops to trusted measurements, drift monitoring, and skepticism about proxies whose meaning can change under the system itself.

:::

The critical implication is the data as code invariant (Principle \ref{pri-data-as-code}).\index{Software 2.0} In traditional software, a programmer writes explicit logic (`if x > 0 then y`). In machine learning, the programmer writes the *optimization meta-logic* (the training algorithm), but the *actual operational logic* is "compiled" from the training dataset through stochastic gradient descent[^fn-sgd-sampling] and related optimization methods. The dataset serves as source code, the training pipeline as compiler, and the model weights[^fn-model-weights-inference] as binary executable.
...
# │ Show: 175B parameters at 2 bytes/parameter occupy about 350 GB.
# │ How: Use mlsysim constants and the shared model_memory formula.
# │
# │ Imports: mlsysim.core.constants (GPT3_PARAMS, BYTES_FP16, Bparam, GB),
# │          mlsysim.core.formulas (model_memory), mlsysim.fmt (fmt, check)
# │ Exports: GPT3WeightFootprint.params_b_str, fp16_weights_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GPT3_PARAMS, BYTES_FP16, Bparam, GB
...
class GPT3WeightFootprint:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(params_b == 175, f"GPT-3 parameter count should be 175B, got {params_b}.")
...
::: {#psp-introduction-verification-gap .callout-perspective title="The verification gap"}

**The Verification Gap in ML Testing**\index{Verification Gap}: In Software 1.0, logic is discrete. We can write unit tests that cover edge cases because the input space is often enumerable or partitionable.
...
- [ ] Can you explain the **verification gap**: Why correctness for ML systems cannot be mathematically guaranteed in the same way we can for traditional logic?
...
AI's evolution reveals a progression of bottlenecks, each overcome by systems innovations that expanded what was computationally possible. The field traces its origin to Turing's[^fn-turing-outputs] paper "Computing Machinery and Intelligence" [@turing1950], which posed the foundational question: *Can machines think?* Early systems that attempted to answer this question, such as the Perceptron (1958) and ELIZA[^fn-eliza-brittleness] [@weizenbaum1966], ran into the limits of manual logic and mainframe-era hardware, resulting in brittleness. Subsequent eras hit the knowledge acquisition bottleneck: manual knowledge entry could not scale. Modern systems face a different constraint: computational throughput.
...
\textbf{Cornell psychologist Frank Rosenblatt invents the perceptron\index{Perceptron!early neural computation}}, laying the groundwork for
...
The first era of AI engineering (1950s–1970s) attempted to reduce intelligence to **Symbolic AI**\index{Symbolic AI} manipulation. Researchers at the 1956 **Dartmouth Conference**\index{Dartmouth Conference}[^fn-dartmouth-systems] [@mccarthy1955dartmouth] hypothesized that if they could formalize the rules of logic, machines could "think." Even then, some saw a different path: Arthur Samuel at IBM demonstrated in 1959 that a checkers program could improve through self-play, coining the very term "**machine learning**\index{Machine Learning}." But the dominant paradigm remained symbolic. Daniel Bobrow's STUDENT[^fn-bobrow-student] system exemplifies this approach [@bobrow1964student; @samuel1959].
...
In the 1980s, engineers pivoted from general logic to capturing deep domain expertise. MYCIN\index{MYCIN!expert system}, designed to diagnose blood infections, encoded approximately 600 rules derived from interviews with medical experts [@shortliffe1975].
...
P(spam|email) ~ P(spam) × product of P(word|spam)
```
...
# │
# │ Imports: mlsysim.core.constants (ALEXNET_PARAMS)
# │ Exports: AlexNetBreakthrough.alexnet_relative_improvement_str,
# │          AlexNetBreakthrough.alexnet_top5_accuracy_str,
...
# │
# │ Imports: mlsysim.core.constants (GPT3_PARAMS, GPT3_TRAINING_OPS)
...
from mlsysim.core.constants import Bparam, ZFLOPs, byte, GB, BILLION
...
# │
# │ Imports: mlsysim.fmt (fmt), mlsysim.core.constants (GPT4_TRAINING_GPU_DAYS, MILLION)
# │ Exports: GPT4ScaleEstimate.gpu_count_str, training_days_str, gpu_days_m_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GPT4_TRAINING_GPU_DAYS, MILLION
from mlsysim.fmt import fmt, check
...
class GPT4ScaleEstimate:
    ...
    gpu_count = 25_000
    training_days = 90
    reference_gpu_days = GPT4_TRAINING_GPU_DAYS
    ...
    gpu_days_m_str = fmt(gpu_days_m, precision=1, commas=False)
```
...
| **Deep Learning (2012)**         | End-to-end neural networks     | ImageNet top-5 accuracy       | `{python} AlexNetBreakthrough.alexnet_top5_accuracy_str` percent (AlexNet) | 6 days on 2 GPUs                                                                                                                                                                                                                  |
| **Modern Deep Learning (2020+)** | Large-scale transformers       | ImageNet top-5 accuracy       | 90.0 percent+ (ViT)                                                        | Hours on distributed systems                                                                                                                                                                                                      |
| **Modern Deep Learning (2023)**  | Foundation models              | MMLU benchmark                | 86.4 percent (GPT-4)                                                       | Not disclosed; public-estimate regime of about `{python} GPT4ScaleEstimate.gpu_days_m_str`M A100 GPU-days (same scale as `{python} GPT4ScaleEstimate.gpu_count_str` GPUs for `{python} GPT4ScaleEstimate.training_days_str` days) |
...
\index{Deep Blue}
The principle finds further validation across AI breakthroughs. In chess, IBM's Deep Blue defeated world champion Garry Kasparov[^fn-deepblue-compute] in 1997 [@campbell2002] not by encoding chess strategies, but through brute-force search evaluating millions of positions per second.

[^fn-deepblue-compute]: **Deep Blue**: IBM's chess system [@campbell2002] defeated World Champion Garry Kasparov in 1997 not through strategic understanding but through raw computational power: 200 million positions per second on 480 custom chess processors. Deep Blue was the first public demonstration that purpose-built silicon $(R_{\text{peak}})$ could substitute for human expertise, foreshadowing the domain-specific accelerator strategy that defines modern ML hardware.

\index{AlphaGo}
In Go, DeepMind's AlphaGo[^fn-alphago-scale] [@silver2016] achieved superhuman performance by learning from self-play rather than studying centuries of human Go wisdom.
...
# │
# │ Imports: mlsysim.core.constants (GPT3_TRAINING_ENERGY_MWH),
# │          mlsysim.fmt (fmt, check)
# │ Exports: GPT3TrainingEnergy.energy_mwh_str, households_str,
# │          household_mwh_year_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GPT3_TRAINING_ENERGY_MWH
from mlsysim.fmt import fmt, check
...
class GPT3TrainingEnergy:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    energy_mwh = GPT3_TRAINING_ENERGY_MWH
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    energy_mwh_str = fmt(energy_mwh, precision=0, commas=True)
    households_str = fmt(households, precision=0, commas=False)
    household_mwh_year_str = fmt(household_mwh_year, precision=1, commas=False)
```
...
[^fn-gpt3-training-energy]: **GPT-3 Training Energy**: @patterson2021carbon estimated GPT-3's single training run consumed approximately `{python} GPT3TrainingEnergy.energy_mwh_str` MWh and emitted 552 tonnes of CO~2~-equivalent, roughly the annual electricity of `{python} GPT3TrainingEnergy.households_str` average US households using a `{python} GPT3TrainingEnergy.household_mwh_year_str` MWh/household-year baseline. The energy cost is dominated not by arithmetic but by data movement through the memory hierarchy, making the $D_{\text{vol}}/\text{BW}$ term of the iron law, not the compute term, the primary driver of the power bill at this scale. \index{GPT-3!training energy}
...
# │
# │ Imports: mlsysim.core.constants (GMAIL_EMAILS_PER_DAY)
# │ Exports: EmailScale.gmail_emails_t_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GMAIL_EMAILS_PER_DAY, TRILLION
from mlsysim.fmt import fmt_percent, fmt, check
...
# │ How: Compare FP16 weight-stream time to peak tensor-core compute time.
# │
# │ Imports: mlsysim.core.constants (RESNET50_FLOPs, RESNET50_PARAMS,
# │          A100_FLOPS_FP16_TENSOR, A100_MEM_BW, units),
# │          mlsysim.fmt (fmt, check)
# │ Exports: ResNet50DamExample.weight_mb_str, a100_hbm_tbs_str,
...
from mlsysim.core.constants import (
    ...
    param, byte, MB, TB, GFLOPs, TFLOPs, second, US,
)
from mlsysim.fmt import fmt, check
...
class ResNet50DamExample:
    ...
    The estimate counts FP16 weight streaming only; activations and framework
    overhead make real latency higher and workload-dependent.
    """

    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    params = RESNET50_PARAMS.m_as(param)
    bytes_per_param_fp16 = 2
    ops = RESNET50_FLOPs
    a100_peak = A100_FLOPS_FP16_TENSOR
    a100_hbm = A100_MEM_BW

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    weight_bytes = params * bytes_per_param_fp16 * byte
    memory_time = weight_bytes / a100_hbm
    compute_time = ops / a100_peak
    workload_ai = ops.m_as(GFLOPs) * 1_000 / weight_bytes.m_as(MB)
    a100_ridge = a100_peak.m_as(TFLOPs/second) / a100_hbm.m_as(TB/second)
    memory_compute_ratio = memory_time.m_as(US) / compute_time.m_as(US)

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(45 <= weight_bytes.m_as(MB) <= 60, "ResNet-50 FP16 weights should be about 50 MB.")
    check(memory_time > compute_time, "Small-batch ResNet-50 example should be memory-side.")
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weight_mb_str = fmt(weight_bytes.m_as(MB), precision=0, commas=False)
    a100_hbm_tbs_str = fmt(a100_hbm.m_as(TB/second), precision=1, commas=False)
    memory_time_us_str = fmt(memory_time.m_as(US), precision=0, commas=False)
    resnet_gflops_str = fmt(ops.m_as(GFLOPs), precision=1, commas=False)
    a100_peak_tflops_str = fmt(a100_peak.m_as(TFLOPs/second), precision=0, commas=False)
    compute_time_us_str = fmt(compute_time.m_as(US), precision=0, commas=False)
    a100_ridge_str = fmt(a100_ridge, precision=0, commas=False)
    workload_ai_str = fmt(workload_ai, precision=0, commas=False)
```
...
1.  **Significance (quantitative)**: The diagnostic power is concrete. A ResNet-50 inference run at batch size one on an A100 is on the memory-bound side of the roofline before activation traffic and framework overhead are included: moving `{python} ResNet50DamExample.weight_mb_str` MB of FP16 weights across `{python} ResNet50DamExample.a100_hbm_tbs_str` TB/s of HBM takes about `{python} ResNet50DamExample.memory_time_us_str` microseconds, while `{python} ResNet50DamExample.resnet_gflops_str` GFLOPs at a `{python} ResNet50DamExample.a100_peak_tflops_str` TFLOP/s tensor-core peak takes about `{python} ResNet50DamExample.compute_time_us_str` microseconds. The workload's arithmetic intensity is about `{python} ResNet50DamExample.workload_ai_str` FLOPs/byte, below the A100 FP16 ridge point of about `{python} ResNet50DamExample.a100_ridge_str` FLOPs/byte. The lesson is not a precise latency prediction; it is that small-batch inference cannot be optimized by buying peak FLOPS alone. The binding axis is the Machine axis, especially data reuse, memory bandwidth, and utilization.
...
from mlsysim.core.constants import GB, MB, KB, watt, milliwatt, TFLOPs, second, flop
from mlsysim.fmt import fmt_percent, fmt, check, fmt_math
import math
...
class DeploymentSystems:
    ...
    t_cloud  = Systems.Tiers.Cloud
    t_edge   = Systems.Tiers.Edge
    t_mobile = Systems.Tiers.Mobile
    t_tiny   = Systems.Tiers.Tiny

    h_cloud  = Hardware.Cloud.H100
    h_edge   = Hardware.Edge.JetsonOrinNX
    h_mobile = Hardware.Mobile.iPhone15Pro
    h_tiny   = Hardware.Tiny.ESP32

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    mem_scaling = (t_cloud.ram/t_tiny.ram).to('count').magnitude
    compute_scaling = (h_cloud.peak_flops/h_tiny.peak_flops).to('count').magnitude
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_mem_str = fmt(t_cloud.ram.m_as(GB), precision=0)
    cloud_compute_str = fmt(h_cloud.peak_flops.m_as(TFLOPs/second), precision=0)
    cloud_power_str = fmt(h_cloud.tdp.m_as(watt), precision=0)

    edge_mem_str = fmt(t_edge.ram.m_as(GB), precision=0)
    edge_compute_str = fmt(h_edge.peak_flops.m_as(TFLOPs/second), precision=1)
    edge_power_str = fmt(h_edge.tdp.m_as(watt), precision=0)

    mobile_mem_str = fmt(t_mobile.ram.m_as(GB), precision=0)
    mobile_compute_str = fmt(h_mobile.peak_flops.m_as(TFLOPs/second), precision=1)
    mobile_power_str = fmt(h_mobile.tdp.m_as(watt), precision=0)

    tiny_mem_str = fmt(t_tiny.ram.m_as(KB), precision=0, commas=False)
    tiny_compute_str = fmt(h_tiny.peak_flops.m_as(TFLOPs/second), precision=4)
    tiny_power_str = fmt(h_tiny.tdp.m_as(milliwatt), precision=0)

    mem_span_math = fmt_math(f"10^{{{round(math.log10(mem_scaling))}}}")
    compute_span_math = fmt_math(f"10^{{{round(math.log10(compute_scaling))}}}")
```

The D·A·M taxonomy serves as a diagnostic lens throughout this text. Scale in ML systems is the relentless pursuit of the *moving bottleneck*. Alleviating a constraint along one axis often shifts the limitation to another. Upgrading to faster GPUs (Machine) might reveal that storage cannot feed data fast enough (Data). Collecting a massive dataset (Data) might reveal that the model lacks capacity to learn from it (Algorithm). Switching to a larger model (Algorithm) might exceed available memory (Machine). Understanding these dynamics is central to ML systems engineering. Part III formalizes this diagnostic approach in @tbl-dam-bottleneck, and the D·A·M Taxonomy appendix provides the complete diagnostic guide, intersection landscape, and troubleshooting matrix (@sec-dam-taxonomy).
...
::: {tbl-colwidths="[17,23,20,20,20]"}

| **Archetype** | **Example System** |                     **RAM/Memory**                    |                    **Peak Compute**                    |                 **Power Budget**                |
|:--------------|:-------------------|:-----------------------------------------------------:|:------------------------------------------------------:|:-----------------------------------------------:|
| **Cloud**     | H100 Cluster       |  `{python} DeploymentSystems.cloud_mem_str` gigabytes | `{python} DeploymentSystems.cloud_compute_str` TFLOPS  |  `{python} DeploymentSystems.cloud_power_str` W |
| **Edge**      | Jetson Robotics    |  `{python} DeploymentSystems.edge_mem_str` gigabytes  |  `{python} DeploymentSystems.edge_compute_str` TFLOPS  |  `{python} DeploymentSystems.edge_power_str` W  |
| **Mobile**    | Smartphone         | `{python} DeploymentSystems.mobile_mem_str` gigabytes | `{python} DeploymentSystems.mobile_compute_str` TFLOPS | `{python} DeploymentSystems.mobile_power_str` W |
| **TinyML**    | ESP32-S3           |  `{python} DeploymentSystems.tiny_mem_str` kilobytes  |  `{python} DeploymentSystems.tiny_compute_str` TFLOPS  |  `{python} DeploymentSystems.tiny_power_str` mW |

: **Four System Archetypes**: Representative memory, peak compute, and power budgets for cloud, edge, mobile, and TinyML systems, exposing the multi-order-of-magnitude span that prevents simple model reuse across tiers. {#tbl-introduction-system-archetypes}
...
These three components interact through a single economic constraint that systems engineers must optimize.
...
To reason about ML systems as engineers, we need more than qualitative descriptions. We need a quantitative framework that connects every layer of the stack. Just as classical mechanics is governed by Newton's laws and processor performance is governed by the Iron Law of Processor Performance, machine learning system performance is governed by the **Iron Law of ML Systems**\index{Iron Law of ML Systems!definition}\index{Iron Law of ML Systems!equation}, formalized in @eq-intro-iron-law:
$$T = \underbrace{ \frac{\text{Data} (D_{\text{vol}})}{\text{Bandwidth} (\text{BW})} }_{\text{The Data Term}} + \underbrace{ \frac{\text{Ops} (O)}{\text{Peak} (R_{\text{peak}}) \times \text{Efficiency} (\eta_{\text{hw}})} }_{\text{The Compute Term}} + \underbrace{ \text{Overhead} (L_{\text{lat}}) }_{\text{The Latency Term}}$$ {#eq-intro-iron-law}
...
# │
# │ Imports: mlsysim.core.constants (GPT3_TRAINING_OPS, A100_FLOPS_FP16_TENSOR,
# │          TFLOPs, second, day),
# │          mlsysim.formulas (dTime), mlsysim.book (fmt, fmt_math)
...
from mlsysim.core.constants import TFLOPs, second, day, flop
...
class GPT3Training:
    ...
    num_gpus = 1024
    peak_flops_per_device = h_a100.peak_flops
    ...
    time_formula_math = fmt_math(r"\text{Time} \approx \frac{O}{N \cdot R_{\text{peak}} \cdot \eta_{\text{hw}}}")
    time_value_math = fmt_math(
        rf"\approx \frac{{{_ops_coeff} \times 10^{{{ops_exp_value}}}}}{{{num_gpus} \times ({peak_tflops_str} \times 10^{{12}}) \times {eta_base}}}"
    )
    time_days_math = fmt_math(rf"\approx {days_initial_str}\ \text{{days}}")
    ops_math = fmt_math(rf"\approx {_ops_coeff} \times 10^{{{ops_exp_value}}}")
```
...
*   **Ops $(O)$**: `{python} GPT3Training.ops_math` FLOPs (from paper).
*   **Peak $(R_{\text{peak}})$**: `{python} GPT3Training.peak_tflops_str` TFLOPS (A100 FP16 tensor core peak).
*   **Efficiency $(\eta_{\text{hw}})$**: ≈ `{python} GPT3Training.eta_base_pct_str` percent (typical for large-scale distributed training).
*   **Scale $(N)$**: `{python} GPT3Training.num_gpus_str` GPUs.
...
**Systems insight**: If we improve software efficiency $(\eta_{\text{hw}})$ from `{python} GPT3Training.eta_base_pct_str` percent to `{python} GPT3Training.eta_opt_pct_str` percent through kernel fusion and better scheduling, training time drops to **`{python} GPT3Training.days_optimized_str` days**, saving nearly `{python} GPT3Training.days_saved_str` days of expensive compute time.
...
# │
# │ Imports: mlsysim.core.constants (ENERGY_* constants, byte, flop),
# │          mlsysim.fmt (fmt, check)
# │ Exports: EnergyMovementRatios.fp16_ratio_str, int8_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    ENERGY_DRAM_PJ_PER_BYTE, ENERGY_FLOP_FP16_PJ, ENERGY_FLOP_INT8_PJ,
    byte, flop,
)
from mlsysim.fmt import fmt, check
...
class EnergyMovementRatios:
    ...
    int8_op_energy = ENERGY_FLOP_INT8_PJ * flop
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fp16_ratio_str = fmt(fp16_ratio, precision=0, commas=False)
...
# │
# │ Imports: mlsysim.core.constants (IMAGENET_IMAGES)
# │ Exports: ImageNetStats.images_m_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import IMAGENET_IMAGES, MILLION
from mlsysim.fmt import fmt, check
...
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TRAINING COMPUTE GROWTH SUMMARY
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Efficiency Framework setup, @fig-ai-training-compute-growth caption,
# │          and summary takeaways
# │
# │ Goal: Keep the seven-orders-of-magnitude scale claim executable.
# │ Show: GPT-4-class public estimates are about 10^7× AlexNet training compute.
# │ How: Compare illustrative training-compute endpoints used in the figure.
# │
# │ Imports: math (standard library), mlsysim.fmt (fmt, check)
# │ Exports: TrainingComputeGrowth.compute_growth_order_str,
# │          compute_growth_m_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class TrainingComputeGrowth:
    """Scale comparison between AlexNet and GPT-4-class training estimates."""
    ...
    compute_growth_m = compute_growth_ratio / 1e6
    ...
    compute_growth_m_str = fmt(compute_growth_m, precision=0, commas=False)
```

This interdependence means that optimizing one component often shifts pressure to another. AlexNet's co-design success came at a cost affordable in 2012 (two consumer GPUs for a week), but modern models demand resources roughly `{python} TrainingComputeGrowth.compute_growth_order_str` orders of magnitude larger. If the iron law governs *how fast* a system runs, we still need a framework for reasoning about *how efficiently* it uses those resources.
...
# │ How: Calculate the algorithmic halving period, Moore's-law window speedup,
# │      and growth gap ratio between demand and hardware supply.
# │
# │ Imports: math (standard library), mlsysim.fmt (fmt, check)
# │ Exports: EfficiencyGains.algo_efficiency_max_str,
# │          EfficiencyGains.algo_halving_months_str,
# │          EfficiencyGains.moores_window_speedup_str,
# │          EfficiencyGains.growth_gap_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt, check
...
class EfficiencyGains:
    ...
    algo_halving_months_str = fmt(algo_halving_months, precision=0, commas=False)
    moores_window_speedup_str = fmt(moores_window_speedup, precision=1, commas=False)
    growth_gap_ratio_str = fmt(growth_gap_ratio, precision=1, commas=False)
```

The magnitude of efficiency improvements is measurable. Between 2012 and 2019, computational resources needed to train a neural network to achieve AlexNet-level performance on ImageNet classification decreased by approximately `{python} EfficiencyGains.algo_efficiency_max_str`$\times$ [@hernandez2020measuring]. This improvement, which halved about every `{python} EfficiencyGains.algo_halving_months_str` months, outpaced hardware efficiency gains predicted by Moore's Law[^fn-moores-law-baseline], demonstrating that algorithmic innovation drives efficiency as much as hardware advances.

[^fn-moores-law-baseline]: **Moore’s Law**: Gordon Moore’s 1965 observation that transistor density doubles approximately every two years. Over the same 2012–2019 window where algorithmic efficiency improved `{python} EfficiencyGains.algo_efficiency_max_str`$\times$, a two-year doubling cadence yields roughly `{python} EfficiencyGains.moores_window_speedup_str`$\times$ in transistor scaling, meaning algorithmic innovation closed more of the efficiency gap than hardware alone. Simultaneously, demand for training compute doubled every 3.4 months, about `{python} EfficiencyGains.growth_gap_ratio_str`$\times$ faster than the two-year hardware cadence, forcing the shift to domain-specific accelerators. \index{Moore's Law!vs. AI growth}
...
Taken together, these two figures reveal a seeming contradiction that defines the economics of modern AI development: @fig-algo-efficiency shows efficiency improving `{python} EfficiencyGains.algo_efficiency_max_str`$\times$ while @fig-ai-training-compute-growth shows compute demand growing by roughly `{python} TrainingComputeGrowth.compute_growth_order_str` orders of magnitude. The resolution lies in understanding how efficiency and scale co-evolve.
...
class ScenarioRegistry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_limit_str = MarkdownStr(f"SLA: {fmt(frontier_latency_ms, precision=0)} ms latency")
    drive_limit_str    = MarkdownStr(f"SLA: {fmt(drive_latency_ms, precision=0)} ms latency")
    doorbell_limit_str = MarkdownStr(
        f"Power: {fmt(doorbell_power_mw, precision=0)} mW"
        if doorbell_power_mw is not None
        else f"SLA: {fmt(doorbell_latency_ms, precision=0)} ms latency"
    )
```
...
: **Four Engineering Missions**: Pairs each system archetype with a scenario workload and its binding constraint, mapping the book's running application scenarios onto the cloud/edge/mobile/TinyML hierarchy. {#tbl-introduction-engineering-missions}
...
- **FarmBeats**\index{Resource-Constrained Edge Deployment!precision agriculture}[^fn-farmbeats-edge] is Microsoft Research's precision agriculture platform, deploying ML models to farms with limited connectivity. FarmBeats represents the *resource-constrained edge* deployment pattern: models under 500 kilobytes run inference on low-power devices using TV white-space bandwidth measured in kilobits per second.

[^fn-farmbeats-edge]: **FarmBeats**: The system demonstrates that the binding constraint for edge ML is often network $\text{BW}$ rather than compute or model quality. With TV white-space links measured in kilobits per second, even a 500-kilobyte model update takes minutes to deliver, making model freshness rather than model accuracy the dominant failure mode in connectivity-constrained deployments. \index{FarmBeats!connectivity constraint}

- **AlphaFold**\index{Deployment!compute-intensive cloud deployment, scientific computing}\index{AlphaFold!cloud deployment} [@jumper2021] is DeepMind's protein structure prediction system that solved a 50-year grand challenge in biology. AlphaFold represents the *compute-intensive cloud* deployment pattern: training required 128 TPUv3 cores for weeks, drawing on the Protein Data Bank's experimentally determined structures, and the trained model was later used to predict configurations for over 200 million proteins.
...
# │
# │ Imports: mlsysim.core.constants (WAYMO_DATA_PER_HOUR_LOW, WAYMO_DATA_PER_HOUR_HIGH,
# │          TB, hour), mlsysim.book (fmt)
# │ Exports: WaymoStats.low_str, WaymoStats.high_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import WAYMO_DATA_PER_HOUR_LOW, WAYMO_DATA_PER_HOUR_HIGH, TB, hour
from mlsysim.fmt import fmt_percent, fmt, check
...
class WaymoStats:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    # From constants (Waymo data-rate range)
...
For detailed guidance on reading paths, learning outcomes, prerequisites, and how to get the most from this textbook, @sec-about provides the front-of-book orientation.
...
Engineers assume algorithmic sophistication drives system performance, but this ignores the iron law (@sec-introduction-iron-law-ml-systems-c32a). A state-of-the-art vision transformer achieves 1–2 percent higher accuracy than ResNet-50 on ImageNet but requires 4$\times$ the FLOPs and 3$\times$ the memory bandwidth [@dosovitskiy2021image]. In production, a model that is 1 percent more accurate but violates latency requirements has effectively zero utility. Google's technical-debt schematic depicts ML code as a small central box, roughly 5 percent of the diagram, surrounded by data pipelines, serving infrastructure, monitoring, and other support systems [@sculley2015hidden]. A well-engineered system with a simpler model consistently outperforms a state-of-the-art architecture lacking robust infrastructure.

**Pitfall**: *Treating ML systems as traditional software that happens to include a model.*

Engineers apply traditional testing and deployment practices to ML systems, but these systems fail in qualitatively different ways (@sec-introduction-ml-vs-traditional-software-e19a). Traditional bugs produce stack traces within milliseconds; ML systems can silently degrade 10–15 percent over three to six months before anyone notices. A/B tests in conventional software show clear signals within 2–3 days; ML comparisons may require four to six weeks to detect 1–2 percent accuracy differences across subpopulations. Unit tests verify deterministic paths; ML systems require monitoring infrastructure to catch the 5–10 percent of predictions where models produce unreliable outputs. Teams deploying ML with only CI/CD pipelines risk silent failures affecting 20–30 percent of predictions before intervention.
...
# │
# │ Imports: mlsysim.core.formulas (calc_amdahls_speedup), mlsysim.book (fmt)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.formulas import calc_amdahls_speedup
from mlsysim.fmt import fmt_percent, fmt, check
...
class AmdahlsPitfall:
    ...
    total_ms_str = fmt(t_total, precision=0, commas=False)
    new_total_ms_str = fmt(t_total_new, precision=0, commas=False)
    improv_pct_str = fmt(improvement_pct, precision=0, commas=False)
    naive_p_str = fmt(naive_pct, precision=0, commas=False)
```

**Pitfall**: *Optimizing individual components without considering system interactions.*

Engineers optimize inference latency in isolation, but Amdahl's Law governs end-to-end performance. A team reduces model inference from `{python} AmdahlsPitfall.t_inference_str` ms to `{python} AmdahlsPitfall.t_inf_new_str` ms, expecting proportional improvement. Yet preprocessing consumes `{python} AmdahlsPitfall.t_pre_str` ms and postprocessing adds `{python} AmdahlsPitfall.t_post_str` ms, so total latency drops only from `{python} AmdahlsPitfall.total_ms_str` ms to `{python} AmdahlsPitfall.new_total_ms_str` ms: `{python} AmdahlsPitfall.improv_pct_str` percent improvement rather than the expected `{python} AmdahlsPitfall.naive_p_str` percent. The D·A·M taxonomy (@tbl-dam-taxonomy) shows that data, algorithms, and machines form interdependent systems where optimizing one component shifts bottlenecks rather than eliminating them. A model requiring 3$\times$ more preprocessing can increase total cost 40 percent while improving accuracy only 2 percent. Teams optimizing components independently often find 50–70 percent of their engineering effort fails to improve end-to-end metrics.
...
class DriftFallacy:
    ...
    acc_final_str = fmt(acc_final, precision=1, commas=False)
...
Engineers assume deployed systems maintain performance indefinitely, but the degradation equation in @eq-degradation quantifies why ML systems decay. A recommendation system deployed at `{python} DriftFallacy.acc_initial_str` percent accuracy drops to `{python} DriftFallacy.acc_final_str` percent within `{python} DriftFallacy.months_str` months as purchasing patterns shift, losing `{python} DriftFallacy.acc_drop_str` percentage points without any code changes. The ML development lifecycle (@sec-introduction-ml-development-lifecycle-4ea0) shows continuous monitoring and retraining as operational requirements. Fraud detection models degrade 5–10 percent per quarter as attackers adapt. NLP systems lose 2–3 percent accuracy annually from vocabulary drift. Without monitoring, systems appear healthy while 15–25 percent of predictions become unreliable. Organizations treating deployment as one-time typically discover failures through customer complaints three to six months after degradation begins.

**Pitfall**: *Assuming that ML expertise alone is sufficient for ML systems engineering.*

Organizations hire ML researchers expecting production-ready systems, but the five engineering disciplines (@sec-introduction-five-engineering-disciplines-fa08) require integrated expertise across algorithms, software, systems, and operations. Teams with strong ML skills but limited systems experience ship systems achieving only 10–20 percent of throughput targets because they lack API design and database optimization expertise. Conversely, software engineers without ML understanding build infrastructure that introduces preprocessing bugs causing 5–15 percent accuracy degradation undetected for months. Industry surveys report 60–85 percent of ML projects fail to reach production, primarily due to systems engineering gaps rather than algorithmic limitations [@paleyes2022]. Effective teams integrate ML researchers, software engineers, and operations specialists rather than expecting one role to master all skills.
...
* **The iron law decomposes performance into three terms**: Data movement, computation, and overhead each resolve to time; the slowest term dominates. Reducing inference from `{python} AmdahlsPitfall.t_inference_str` ms to `{python} AmdahlsPitfall.t_inf_new_str` ms yields only `{python} AmdahlsPitfall.improv_pct_str` percent end-to-end improvement when preprocessing (`{python} AmdahlsPitfall.t_pre_str` ms) and postprocessing (`{python} AmdahlsPitfall.t_post_str` ms) dominate total latency.
...
* **Efficiency and scale co-evolve across the deployment spectrum**: Algorithmic improvements delivered a `{python} EfficiencyScaleSummary.algo_efficiency_max_str`$\times$ compute reduction, yet total AI compute grew by roughly `{python} EfficiencyScaleSummary.compute_growth_order_str` orders of magnitude. Deployment context (from cloud to TinyML) determines which efficiency dimension (algorithmic, compute, or data) to prioritize.
...
# │
# │ Imports: math (standard library), mlsysim.core.Systems, mlsysim.Hardware,
# │          mlsysim.fmt (MarkdownStr, check)
...
class DeploymentSpectrumSummary:
    ...
    # ┌── 1. LOAD (Archetypes) ───────────────────────────────────────────────
    cloud_ram = Systems.Tiers.Cloud.ram
    tiny_ram = Systems.Tiers.Tiny.ram
    cloud_compute = Hardware.Cloud.H100.peak_flops
    tiny_compute = Hardware.Tiny.ESP32.peak_flops
