...
1.  **Significance (quantitative)**: The diagnostic power is concrete. A ResNet-50 inference run at batch size one on an A100 is on the memory-bound side of the roofline before activation traffic and framework overhead are included: moving `{python} ResNet50DamExample.weight_mb_str` of FP16 weights across `{python} ResNet50DamExample.a100_hbm_tbs_str` TB/s of HBM takes about `{python} ResNet50DamExample.memory_time_us_str` microseconds, while `{python} ResNet50DamExample.resnet_gflops_str` GFLOPs at a `{python} ResNet50DamExample.a100_peak_tflops_str` tensor-core peak takes about `{python} ResNet50DamExample.compute_time_us_str` microseconds. The workload's arithmetic intensity is about `{python} ResNet50DamExample.workload_ai_str` FLOP/byte, below the A100 FP16 ridge point of about `{python} ResNet50DamExample.a100_ridge_str` FLOP/byte. The lesson is not a precise latency prediction; it is that small-batch inference cannot be optimized by buying peak FLOP/s alone. The binding axis is the Machine axis, especially data reuse, memory bandwidth, and utilization.
...
|:--------------|:-------------------|:-----------------------------------------------------:|:----------------------------------------------------:|:-----------------------------------------------:|
| **Cloud**     | H100 Cluster       |  `{python} DeploymentSystems.cloud_mem_str` gigabytes |    `{python} DeploymentSystems.cloud_compute_str`    |  `{python} DeploymentSystems.cloud_power_str` W |
| **Edge**      | Jetson Robotics    |  `{python} DeploymentSystems.edge_mem_str` gigabytes  |    `{python} DeploymentSystems.edge_compute_str`     |  `{python} DeploymentSystems.edge_power_str` W  |
...
| **TinyML**    | ESP32-S3           |  `{python} DeploymentSystems.tiny_mem_str` kilobytes  |    `{python} DeploymentSystems.tiny_compute_str`     |   `{python} DeploymentSystems.tiny_power_str`   |
...
*   **Peak $(R_{\text{peak}})$**: `{python} GPT3Training.peak_tflops_str` (A100 FP16 tensor core peak).
...
**Systems insight**: If we improve hardware utilization $(\eta_{\text{hw}})$ from `{python} GPT3Training.eta_base_pct_str` to `{python} GPT3Training.eta_opt_pct_str` through kernel fusion and better scheduling, training time drops to **`{python} GPT3Training.days_optimized_str` days**, saving nearly `{python} GPT3Training.days_saved_str` days of expensive compute time.
