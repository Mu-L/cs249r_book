...
class GPT3Scale:
    ...
    gpt3_params_billion_str = fmt(gpt3_params_b_str, precision=0, commas=False, suffix=" billion")
...
class DeploymentSystems:
    ...
    mem_span_math = md_math(f"10^{{{round(math.log10(mem_scaling))}}}")
    compute_span_math = md_math(f"10^{{{round(math.log10(compute_scaling))}}}")
```
...
class GPT3Training:
    ...
    ops_math = md_math(rf"\approx {_ops_coeff} \times 10^{{{ops_exp_value}}}")
```
...
class ScenarioRegistry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_limit_str = MarkdownStr(f"SLA: {fmt(app_frontier.sla_latency.m_as('ms'), precision=0)} ms latency")
    drive_limit_str    = MarkdownStr(f"SLA: {fmt(app_drive.sla_latency.m_as('ms'), precision=0)} ms latency")
    doorbell_limit_str = MarkdownStr(f"Power: {fmt(app_doorbell.power_budget.m_as('mW'), precision=0)} mW") if app_doorbell.power_budget else MarkdownStr(f"SLA: {fmt(app_doorbell.sla_latency.m_as('ms'), precision=0)} ms latency")
```
