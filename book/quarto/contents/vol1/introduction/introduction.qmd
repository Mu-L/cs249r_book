...
3.  **Common pitfall**: A frequent misconception is that the three axes are independent. Changing the algorithm (for example, switching from a CNN to a transformer) typically mandates a different machine (more memory for $O(S^2)$ attention) and a different data distribution (more diverse training data to avoid overfitting the flexible hypothesis space).
...
$$T = \underbrace{ \frac{\text{Data} (D_{\text{vol}})}{\text{Bandwidth} (\text{BW})} }_{\text{The Data Term}} + \underbrace{ \frac{\text{Ops} (O)}{\text{Peak} (R_{\text{peak}}) \cdot \text{Efficiency} (\eta_{\text{hw}})} }_{\text{The Compute Term}} + \underbrace{ \text{Overhead} (L_{\text{lat}}) }_{\text{The Latency Term}}$$ {#eq-intro-iron-law}
...
2.  **The compute term $(O/(R_{\text{peak}} \cdot \eta_{\text{hw}}))$**: The cost of arithmetic. $O$ is the number of floating-point operations. $R_{\text{peak}}$ is the hardware's theoretical peak throughput (FLOP/s). $\eta_{\text{hw}}$ (eta) is the utilization factor $(0 \le \eta_{\text{hw}} \le 1)$, representing software efficiency. We address this in **Part II: Build** and **Part III: Optimize**.
