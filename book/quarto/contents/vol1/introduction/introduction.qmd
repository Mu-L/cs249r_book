...
class AIMomentStats:
    ...
    h100_fp16_tflops_str = fmt(gpu_tflops, precision=1, allow_zero=True, commas=False)
...
class GPT3WeightFootprint:
    ...
    fp16_weights_gb_str = fmt(fp16_weights_gb, precision=1, allow_zero=True, commas=False)
```
...
class VerificationGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    vg_digits_str = fmt(digits, precision=1, allow_zero=True, commas=True)
    imagenet_test_images_str = fmt(test_size, precision=1, allow_zero=True, commas=True)
```
...
class AlexNetBreakthrough:
    ...
    alexnet_relative_improvement_str = fmt(relative_improvement, precision=1, allow_zero=True)
    alexnet_params_m_str = fmt(params_m, precision=1, allow_zero=True)
```
...
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT3.parameters, Models.Language.GPT3.training_ops, GPT3_TRAINING_TOKENS)
...
class GPT3Scale:
    ...
    tokens_b = GPT3_TRAINING_TOKENS.m_as(Bparam)
    avg_token_bytes = 1.4
    ...
    gpt3_params_billion_str = fmt(params_b, precision=1, allow_zero=True, commas=False, suffix=" billion")
    gpt3_training_zflops_str = fmt(round(training_zflops), precision=1, allow_zero=True, commas=False)
    data_scale_gb_str = fmt(data_gb, precision=1, allow_zero=True, commas=False)
```
...
# │          (GPT4 public-estimate references, GPT4_TRAINING_GPU_DAYS, MILLION)
...
class GPT4ScaleEstimate:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    gpu_count = GPT4_CLASS_PUBLIC_ESTIMATE_GPU_COUNT_REF.m_as(count)
    training_days = GPT4_CLASS_PUBLIC_ESTIMATE_TRAINING_DAYS_REF.m_as(day)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpu_count_str = fmt(gpu_count, precision=1, allow_zero=True, commas=True)
    training_days_str = fmt(training_days, precision=1, allow_zero=True, commas=False)
...
class AlphaGoZeroCompute:
    ...
    tpus_str = fmt(tpus, precision=1, allow_zero=True, commas=False)
    tpu_hours_str = fmt(tpu_hours, precision=1, allow_zero=True, commas=False)
```
...
class GPT3TrainingEnergy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    energy_mwh_str = fmt(energy_mwh, precision=1, allow_zero=True, commas=True)
    households_str = fmt(households, precision=1, allow_zero=True, commas=False)
...
class EmailScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gmail_emails_t_str = fmt(gmail_emails_t_value, precision=1, allow_zero=True)
```
...
class ResNet50DamExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weight_mb_str = fmt(weight_mb, precision=1, allow_zero=True, commas=False)
    a100_hbm_tbs_str = fmt(a100_hbm_tbs, precision=1, commas=False)
    memory_time_us_str = fmt(memory_time_us, precision=1, allow_zero=True, commas=False)
    resnet_gflops_str = fmt(resnet_gflops, precision=1, commas=False)
    a100_peak_tflops_str = fmt(a100_peak_tflops, precision=1, allow_zero=True, commas=False)
    compute_time_us_str = fmt(compute_time_us, precision=1, allow_zero=True, commas=False)
    a100_ridge_str = fmt(a100_ridge, precision=1, allow_zero=True, commas=False)
    workload_ai_str = fmt(workload_ai, precision=1, allow_zero=True, commas=False)
```
...
class DeploymentSystems:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_mem_str = fmt(t_cloud.ram.m_as(GB), precision=1, allow_zero=True)
    cloud_compute_str = fmt(h_cloud.peak_flops.m_as(TFLOPs/second), precision=1, allow_zero=True)
    cloud_power_str = fmt(h_cloud.tdp.m_as(watt), precision=1, allow_zero=True)

    edge_mem_str = fmt(t_edge.ram.m_as(GB), precision=1, allow_zero=True)
    edge_compute_str = fmt(h_edge.peak_flops.m_as(TFLOPs/second), precision=1)
    edge_power_str = fmt(h_edge.tdp.m_as(watt), precision=1, allow_zero=True)

    mobile_mem_str = fmt(t_mobile.ram.m_as(GB), precision=1, allow_zero=True)
    mobile_compute_str = fmt(h_mobile.peak_flops.m_as(TOPS), precision=1, allow_zero=True, commas=False)
    mobile_power_str = fmt(h_mobile.tdp.m_as(watt), precision=1, allow_zero=True)

    tiny_mem_str = fmt(t_tiny.ram.m_as(KB), precision=1, allow_zero=True, commas=False)
    tiny_compute_str = fmt(h_tiny.peak_flops.m_as(TFLOPs/second), precision=4)
    tiny_power_str = fmt(h_tiny.tdp.m_as(milliwatt), precision=1, allow_zero=True)
...
| **Mobile**    | Smartphone         | `{python} DeploymentSystems.mobile_mem_str` gigabytes |  `{python} DeploymentSystems.mobile_compute_str` TOPS  | `{python} DeploymentSystems.mobile_power_str` W |
...
class GPT3Training:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Text strings
    num_gpus_str = fmt(num_gpus, precision=1, allow_zero=True, commas=False)
    eta_base_pct_str = fmt_percent(eta_base, precision=1, commas=False)
    eta_opt_pct_str = fmt_percent(eta_opt, precision=1, commas=False)
    days_initial_str = fmt(days_base, precision=1, allow_zero=True, commas=False)
    days_optimized_str = fmt(days_opt, precision=1, allow_zero=True, commas=False)
    days_saved_str = fmt(days_saved, precision=1, allow_zero=True, commas=False)

    # LaTeX math components
    _ops_mag = f"{ops:.2e}"
    _ops_coeff = _ops_mag.split("e+")[0]
    ops_exp_value = int(_ops_mag.split("e+")[1])
    peak_tflops_str = fmt(peak_tflops, precision=1, allow_zero=True, commas=False)
...
*   **Scale $(N_{\text{GPU}})$**: `{python} GPT3Training.num_gpus_str` GPUs.
...
# │
# │ Imports: mlsysim.core.constants (ENERGY_* constants, byte, flop, count),
# │          mlsysim.fmt (fmt, check)
...
class EnergyMovementRatios:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fp16_ratio_str = fmt(fp16_ratio, precision=1, allow_zero=True, commas=False)
    int8_ratio_str = fmt(int8_ratio, precision=1, allow_zero=True, commas=False)
```
...
class TrainingComputeGrowth:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compute_growth_order_str = fmt(compute_growth_order, precision=1, allow_zero=True, commas=False)
    compute_growth_m_str = fmt(compute_growth_m, precision=1, allow_zero=True, commas=False)
```
...
class EfficiencyGains:
    ...
    algo_halving_months_str = fmt(algo_halving_months, precision=1, allow_zero=True, commas=False)
...
class ScenarioRegistry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_limit_str = MarkdownStr(f"SLA: {fmt(frontier_latency_ms, precision=1, allow_zero=True)} ms latency")
    drive_limit_str    = MarkdownStr(f"SLA: {fmt(drive_latency_ms, precision=1, allow_zero=True)} ms latency")
    doorbell_limit_str = MarkdownStr(
        f"Power: {fmt(doorbell_power_mw, precision=1, allow_zero=True)} mW"
        if doorbell_power_mw is not None
        else f"SLA: {fmt(doorbell_latency_ms, precision=1, allow_zero=True)} ms latency"
    )
```
...
class WaymoStats:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    low_str = fmt(val_low, precision=1, allow_zero=True, commas=False)
    high_str = fmt(val_high, precision=1, allow_zero=True, commas=False)
```
...
class AmdahlsPitfall:
    ...
    t_inference_str = fmt(t_inference, precision=1, allow_zero=True, commas=False)
    t_inf_new_str = fmt(t_inf_new, precision=1, allow_zero=True, commas=False)
    t_pre_str = fmt(t_pre, precision=1, allow_zero=True, commas=False)
    t_post_str = fmt(t_post, precision=1, allow_zero=True, commas=False)
    total_ms_str = fmt(t_total, precision=1, allow_zero=True, commas=False)
    new_total_ms_str = fmt(t_total_new, precision=1, allow_zero=True, commas=False)
    improv_pct_str = fmt(improvement_pct, precision=1, allow_zero=True, commas=False)
    naive_p_str = fmt(naive_pct, precision=1, allow_zero=True, commas=False)
```
...
class DriftFallacy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    acc_initial_str = fmt(acc_initial, precision=1, allow_zero=True, commas=False)
    ...
    months_str = fmt(months, precision=1, allow_zero=True, commas=False)
```
