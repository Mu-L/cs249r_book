...
::: {#exm-introduction-hidden-technical-debt-ml-systems .callout-example title="The hidden technical debt of ML systems"}
...
::: {#exm-introduction-student-1964 .callout-example title="STUDENT (1964)"}
...
::: {#exm-introduction-mycin-1976 .callout-example title="MYCIN (1976)"}
...
::: {#exm-introduction-early-spam-detection-systems .callout-example title="Early spam detection systems"}
...
::: {#exm-introduction-traditional-computer-vision-pipeline .callout-example title="Traditional computer vision pipeline"}
...
::: {#def-introduction-machine-learning-systems .callout-definition title="Machine learning systems"}
...
::: {#def-introduction-d-m-taxonomy .callout-definition title="The D·A·M taxonomy"}
...
# │          DeploymentSystems.tiny_power_str, DeploymentSystems.mem_span_str,
# │          DeploymentSystems.compute_span_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core import Systems
from mlsysim import Hardware
from mlsysim.core.constants import GB, MB, KB, watt, milliwatt, TFLOPs, second, flop
from mlsysim.fmt import fmt_percent, fmt, check, md_math
import math
...
class DeploymentSystems:
    ...
    mem_span_str = md_math(f"10^{{{round(math.log10(mem_scaling))}}}")
    compute_span_str = md_math(f"10^{{{round(math.log10(compute_scaling))}}}")
```
...
Throughout this book, every optimization technique we study, from pruning to kernel fusion, is a method for manipulating one of these variables. The following notebook demonstrates this by *estimating GPT-3-class training time* as a worked example of the iron law in practice.
...
# │          TFLOPs, second, day),
# │          mlsysim.formulas (dTime), mlsysim.book (fmt, md_math)
...
class GPT3Training:
    ...
    time_formula_math = md_math(r"\text{Time} \approx \frac{O}{N \cdot R_{\text{peak}} \cdot \eta_{\text{hw}}}")
...
::: {#def-introduction-ai-engineering .callout-definition title="AI Engineering"}
...
# │
# │ Imports: mlsysim.Applications, mlsysim.fmt (fmt, check)
# │ Exports: ScenarioRegistry.frontier_limit, ScenarioRegistry.drive_limit,
# │          ScenarioRegistry.doorbell_limit
# └─────────────────────────────────────────────────────────────────────────────
...
class ScenarioRegistry:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_limit = f"SLA: {fmt(frontier_latency_ms, precision=0)} ms latency"
    drive_limit = f"SLA: {fmt(drive_latency_ms, precision=0)} ms latency"
    doorbell_limit = (
...
|:---------------------------------------------------------------------|:---------------------|:----------------------|:-------------------------------------------|
| **Frontier Training**\index{Frontier Training!cloud cluster}         | Cloud Cluster        | GPT-4                 | `{python} ScenarioRegistry.frontier_limit` |
| **Autonomous Perception**\index{Autonomous Perception!edge robotics} | Edge Robotics        | YOLOv8-nano           | `{python} ScenarioRegistry.drive_limit`    |
| **Mobile Assistant**\index{Mobile Assistant!smartphone constraints}  | Smartphone           | Quantized small LLM   | Thermal Throttling/RAM                     |
| **Smart Doorbell**                                                   | TinyML (MCU)         | Wake Vision           | `{python} ScenarioRegistry.doorbell_limit` |
...
For detailed guidance on reading paths, learning outcomes, prerequisites, and how to get the most from this textbook, refer to the front-of-book *About This Book* chapter.
...
# │          AmdahlsPitfall.total_ms, AmdahlsPitfall.new_total_ms,
# │          AmdahlsPitfall.improv_pct, AmdahlsPitfall.naive_p
# └─────────────────────────────────────────────────────────────────────────────
...
* **The iron law decomposes performance into three terms**: Data movement, computation, and overhead each resolve to time; the slowest term dominates. Reducing inference from `{python} AmdahlsPitfall.t_inference_str` ms to `{python} AmdahlsPitfall.t_inf_new_str` ms yields only `{python} AmdahlsPitfall.improv_pct` percent end-to-end improvement when preprocessing (`{python} AmdahlsPitfall.t_pre_str` ms) and postprocessing (`{python} AmdahlsPitfall.t_post_str` ms) dominate total latency.
