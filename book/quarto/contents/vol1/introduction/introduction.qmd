...
class DeploymentSystems:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_mem_str = fmt_int(round(t_cloud.ram.m_as(GB)), commas=False)
    ...
    edge_mem_str = fmt_int(round(t_edge.ram.m_as(GB)), commas=False)
    edge_compute_str = fmt(h_edge.peak_flops.m_as(TFLOPs/second), precision=0)
    edge_power_str = fmt(h_edge.tdp.m_as(watt), precision=0)

    mobile_mem_str = fmt_int(round(t_mobile.ram.m_as(GB)), commas=False)
