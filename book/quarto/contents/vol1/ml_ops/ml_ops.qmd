...
class RetrainingAnchor:
    ...
    """
    daily_decay_pct = 2
    daily_decay_str = fmt(daily_decay_pct, precision=1, commas=False, suffix="%")
...
class SkewEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    queries_daily_str = fmt(queries_daily, precision=0, commas=True)
    error_rate_pct_str = fmt(int(skew_error_rate * 100), precision=1, commas=False)
    error_cost_str = fmt(cost_per_error, precision=2, commas=False)
    skew_cost_str = fmt(annual_cost, precision=1, commas=True)
```
...
class AutomationROI:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mc_manual_hours_str = fmt(int(hrs_manual_week), precision=0, commas=False)
    mc_pipeline_hours_str = fmt(int(hrs_automation_once), precision=0, commas=False)
    mc_breakeven_str = fmt(int(breakeven_weeks), precision=1, commas=False)
    mc_time_horizon_str = fmt(int(time_horizon_years), precision=1, commas=False)
    mc_manual_final_str = fmt(100, precision=1, commas=False)
    mc_pipeline_final_str = fmt(0, precision=1, commas=False)
```
...
class SilentFailureCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    annual_revenue_str = fmt(int(annual_revenue // MILLION), precision=1, commas=False, suffix="M")
    quality_drop_pct_str = fmt(int(quality_drop * 100), precision=1, commas=False)
    quality_drop_str = fmt(quality_drop, precision=2, commas=False)
    manual_detect_days_str = fmt(days_manual, precision=1, commas=False)
    auto_detect_days_str = fmt(days_auto, precision=1, commas=False)
    incidents_per_year_str = fmt(incidents_per_year, precision=1, commas=False)

    loss_manual_str = fmt(loss_manual, precision=1, commas=True)
    loss_auto_str = fmt(loss_auto, precision=1, commas=True)
    loss_diff_str = fmt(savings_per_incident, precision=1, commas=True)
    annual_savings_str = fmt(annual_savings, precision=1, commas=True)
```
...
class RetrainingInterval:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    retrain_cost_str = fmt(retrain_cost, precision=1, commas=True)
    Q_str = fmt(Q, precision=1, commas=True)
    V_str = fmt(V, precision=2, commas=False)
    A0_str = fmt(A0, precision=2, commas=False)
    gamma_str = fmt(gamma, precision=2, commas=False)
    gamma_pct_str = fmt(gamma * 100, precision=1, commas=False)
...
class ABSampleSize:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    confidence_pct_str = fmt(confidence * 100, precision=1, commas=False)
    power_pct_str = fmt(power * 100, precision=1, commas=False)
    ...
    relative_lift_pct_str = fmt(relative_lift * 100, precision=1, commas=False)
...
class LatencyBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    slo_p99_str = fmt(slo_p99, precision=1, commas=False)
    network_str = fmt(network, precision=1, commas=False)
    feature_fetch_str = fmt(feature_fetch, precision=1, commas=False)
    request_parse_str = fmt(request_parse, precision=1, commas=False)
    inference_str = fmt(inference, precision=1, commas=False)
    post_proc_str = fmt(post_proc, precision=1, commas=False)
    serialization_str = fmt(serialization, precision=1, commas=False)
    non_inference_str = fmt(non_inference, precision=1, commas=False)
    inference_pct_str = fmt(inference_fraction * 100, precision=1, commas=False)
    model_speedup_str = fmt(model_speedup, precision=1, commas=False)
...
class KVCacheFootprint:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    layers_str = fmt(layers, precision=1, commas=False)
    kv_heads_str = fmt(kv_heads, precision=0, commas=False)
    seq_len_str = fmt(seq_len, precision=1, commas=True)
    head_dim_str = fmt(head_dim, precision=1, commas=False)
    bytes_per_element_str = fmt(bytes_per_element.m_as(byte), precision=1, commas=False)
...
class CostPerInference:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    hourly_gpu_cost_str = fmt(hourly_gpu_cost, precision=0, commas=False)
...
class DriftDetectionDelay:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    baseline_acc_pct_str = fmt(baseline_acc * 100, precision=1, commas=False)
    drop_pct_str = fmt(drop * 100, precision=1, commas=False)
    target_acc_pct_str = fmt(target_acc * 100, precision=1, commas=False)
    confidence_pct_str = fmt(confidence * 100, precision=1, commas=False)
    qps_high_str = fmt(qps_high, precision=1, commas=False)
    samples_needed_str = fmt(samples_needed, precision=1, commas=True)
    seconds_needed_high_str = fmt(seconds_needed_high, precision=1, commas=True)
    minutes_needed_high_str = fmt(minutes_needed_high, precision=1, commas=False)
    requests_per_day_low_str = fmt(100, precision=1, commas=False)
    days_needed_low_str = fmt(days_needed_low, precision=1, commas=False)
```
...
class ObservabilitySampling:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    request_rate_m_str = fmt(requests_per_second / MILLION, precision=1, commas=False)
    high_seconds_str = fmt(high_frequency_seconds, precision=1, commas=False)
    low_seconds_str = fmt(low_frequency_seconds, precision=1, commas=False)
    high_gb_s_str = fmt(high_bytes_per_second.m_as(GB), precision=1, commas=False)
    low_mb_s_str = fmt(low_bytes_per_second.m_as(MB), precision=1, commas=False)
    sampling_ratio_str = fmt(sampling_ratio, precision=1, commas=False)
```
...
class MonitoringBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_variants_str = fmt(n_variants, precision=1, commas=False)
    metrics_per_variant_str = fmt(metrics_per_variant, precision=1, commas=False)
    samples_per_min_str = fmt(samples_per_min, precision=1, commas=False)
    days_str = fmt(days, precision=1, commas=False)
    ingestion_cost_per_m_str = fmt(ingestion_cost_per_m, precision=2, commas=False)
    bytes_per_point_str = fmt(bytes_per_point, precision=1, commas=False)
    storage_cost_per_gb_str = fmt(storage_cost_per_gb, precision=2, commas=False)
    n_dashboards_str = fmt(n_dashboards, precision=1, commas=False)
    n_users_str = fmt(n_users, precision=0, commas=False)
    queries_per_hr_str = fmt(queries_per_hr, precision=0, commas=False)
    work_hours_str = fmt(work_hours, precision=0, commas=False)
    work_days_str = fmt(work_days, precision=1, commas=False)
    query_cost_per_str = fmt(query_cost_per, precision=2, commas=False)
    dp_m_str = fmt(datapoints_mo_m, precision=1, commas=False)
    ingest_str = fmt(ingestion_cost, precision=1, commas=False)
    ...
    total_str = fmt(total_cost, precision=1, commas=False)
```
...
class MonitoringROI:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    incidents_prevented_str = fmt(incidents_prevented, precision=1, commas=False)
    avg_incident_cost_str = fmt(avg_incident_cost, precision=1, commas=True)
    annual_monitoring_cost_str = fmt(annual_monitoring_cost, precision=1, commas=True)
    monitoring_roi_str = fmt(monitoring_roi, precision=1, commas=False)
```
...
class FraudDetectionImprovement:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    current_rate_pct_str = fmt(current_rate * 100, precision=1, commas=False)
    target_rate_pct_str = fmt(target_rate * 100, precision=1, commas=False)
    infra_cost_increase_pct_str = fmt(infra_cost_increase * 100, precision=1, commas=False)
    annual_loss_prevented_str = fmt(annual_loss_prevented // MILLION, precision=1, commas=False, suffix=" million")
    false_positives_reduced_str = fmt(false_positives_reduced, precision=1, commas=True)
```
...
class SingleModelROI:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    incidents_avoided_str = fmt(incidents_avoided, precision=1, commas=False)
    incident_cost_str = fmt(incident_cost // THOUSAND, precision=1, commas=False, suffix="K")
    hours_saved_monthly_str = fmt(hours_saved_monthly, precision=0, commas=False)
    hourly_cost_str = fmt(hourly_cost, precision=0, commas=False)
    incident_savings_k_str = fmt(incident_savings // THOUSAND, precision=1, commas=False)
    time_savings_k_str = fmt(time_savings // THOUSAND, precision=1, commas=False)
    mlops_investment_k_str = fmt(mlops_investment // THOUSAND, precision=1, commas=False)
...
class OuraValidationGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    accel_only_pct_str = fmt(accel_only_accuracy * 100, precision=1, commas=False)
    enhanced_pct_str = fmt(enhanced_accuracy * 100, precision=1, commas=False)
    scorer_low_pct_str = fmt(scorer_agreement_low * 100, precision=1, commas=False)
    scorer_high_pct_str = fmt(scorer_agreement_high * 100, precision=1, commas=False)
    gain_points_str = fmt(accuracy_gain * 100, precision=1, commas=False)
    gap_closed_range_str = MarkdownStr(
        f"{fmt(gap_closed_high_ceiling * 100, precision=1, commas=False)}-"
        f"{fmt(gap_closed_low_ceiling * 100, precision=1, commas=False)}"
    )
```
