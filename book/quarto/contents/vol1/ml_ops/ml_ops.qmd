...
class SkewEconomics:
    ...
    error_rate_pct_str = fmt(int(skew_error_rate * 100), precision=0, commas=False)
...
class AutomationROI:
    ...
    mc_breakeven_str = fmt(int(breakeven_weeks), precision=0, commas=False)
    mc_time_horizon_str = fmt(int(time_horizon_years), precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import DAYS_PER_YEAR, MILLION
from mlsysim.fmt import fmt, check, MarkdownStr
...
class SilentFailureCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    annual_revenue_str = fmt(int(annual_revenue // MILLION), precision=0, commas=False, suffix="M")
    quality_drop_pct_str = fmt(int(quality_drop * 100), precision=0, commas=False)
...
class ABSampleSize:
    ...
    users_per_variant = ceil(
        2 * (z_alpha_2 + z_beta) ** 2 * variance / absolute_lift ** 2
    )
...
class KVCacheFootprint:
    ...
    kv_cache_bytes = layers * 2 * kv_heads * seq_len * head_dim * bytes_per_element
    kv_cache_gb = kv_cache_bytes.m_as(GB)
    ...
    concurrent_8_gb_str = fmt(concurrent_8_gb, precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import SECONDS_PER_MINUTE
from mlsysim.fmt import fmt, check
...
class DriftDetectionDelay:
    ...
    minutes_needed_high_str = fmt(minutes_needed_high, precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MILLION, KB, MB, GB
from mlsysim.fmt import fmt, check
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import byte, GB, MILLION
from mlsysim.fmt import fmt, check
...
class OuraValidationGap:
    ...
    gap_closed_range_str = MarkdownStr(
        f"{fmt(gap_closed_high_ceiling * 100, precision=0, commas=False)}-"
