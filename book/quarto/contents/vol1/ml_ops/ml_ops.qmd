...
::: {#def-ml-ops-mlops .callout-definition title="MLOps"}
...
Monitoring priorities differ across workload archetypes; @tbl-monitoring-archetype-strategy maps each of the four volume archetypes to its dominant drift pattern, monitoring metric, and typical retraining trigger.
...
::: {#def-ml-ops-technical-debt-ml .callout-definition title="Technical debt in ML"}
...
class SilentFailureCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    annual_revenue_str = f"{int(annual_revenue // MILLION)}M"
...
class RetrainingAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    daily_decay_str = f"{daily_decay_pct:.0f}%"
```
...
# │ Exports: retrain_cost_str, Q_str, T_star_eq, V_str, A0_str, lam_str,
# │          lam_pct_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt, check
...
::: {#exm-ml-ops-fraud-detection-retraining .callout-example title="Fraud detection retraining"}
...
class LatencyBudget:
    ...
    request_parse_str = f"{request_parse}"
    inference_str = f"{inference}"
    post_proc_str = f"{post_proc}"
    serialization_str = f"{serialization}"
    non_inference_str = f"{non_inference}"
...
::: {#def-ml-ops-data-drift .callout-definition title="Data drift"}
...
class PSIAgeExample:
    ...
    difference_str = [f"{value:+.2f}" for value in differences]
    log_ratio_str = [f"{value:+.3f}" for value in log_ratios]
...
::: {#exm-ml-ops-single-model-monitoring-budget .callout-example title="Single-model monitoring budget"}
...
class FraudDetectionImprovement:
    ...
    annual_loss_prevented_str = f"{fmt(annual_loss_prevented / MILLION, precision=0, commas=False)} million"
...
class SingleModelROI:
    ...
    incident_cost_str = f"{incident_cost // THOUSAND:.0f}K"
    hours_saved_monthly_str = f"{hours_saved_monthly}"
    hourly_cost_str = f"{hourly_cost}"
    incident_savings_k = f"{incident_savings // THOUSAND:.0f}"
    time_savings_k = f"{time_savings // THOUSAND:.0f}"
    mlops_investment_k = f"{mlops_investment // THOUSAND:.0f}"
...
$$
\text{ROI = (USD `{python} SingleModelROI.incident_savings_k` K + USD `{python} SingleModelROI.time_savings_k` K) / USD `{python} SingleModelROI.mlops_investment_k` K = `{python} SingleModelROI.single_model_roi_str`$\times$}
$$
...
::: {#exm-ml-ops-principle-mapping-guide .callout-example title="Principle mapping guide"}
