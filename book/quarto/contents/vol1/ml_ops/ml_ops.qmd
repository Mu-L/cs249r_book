...
**Key insight**: Ground truth availability determines monitoring strategy. ResNet-50 (image classification) can use explicit labels; GPT-2 relies on proxy metrics (perplexity); Deep Learning Recommendation Model (DLRM) uses implicit feedback (clicks); DS-CNN, a depthwise-separable convolutional neural network (CNN), monitors operational metrics (energy, false positives). The retraining frequency spans four orders of magnitude: daily for recommendation systems to quarterly for embedded devices.
...
$$T^* \approx \sqrt{\frac{2 \times 5000}{1000000 \times 0.50 \times 0.95 \times 0.02}} \approx 1\text{ day}$$
