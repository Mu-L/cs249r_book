...
**Systems lesson**: Deployment is a systems problem that extends well beyond code. Configuration drift and partial rollouts are catastrophic failure modes in automated systems.
...
**The context**: Long-lived production models often accumulate features whose original owners, semantics, or intended use have faded.

**The failure mode**: A feature can be deprecated in application code while still flowing through a feature store, training dataset, or serialized model input contract. The model may learn to ignore it, split signal across a duplicate, or depend on a preprocessing artifact that nobody still owns.

**The consequence**: Removing such a feature is no longer a local cleanup: it becomes a compatibility change that can affect retraining, serving, monitoring, and downstream analysis.

**The systems lesson**: Features in ML systems do not disappear just because code owners stop thinking about them. Without explicit deprecation policies and feature-store governance, models accumulate "dead code" that consumes resources and complicates debugging [@sculley2015hidden].
