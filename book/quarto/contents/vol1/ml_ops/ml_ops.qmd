...
class SkewEconomics:
    ...
    error_rate_pct_str = fmt_int(skew_error_rate * 100, commas=False)
...
***Technical Debt in Machine Learning***\index{Technical Debt!definition} is the accumulating maintenance cost created by implicit data dependencies, entangled features, and undeclared consumers in ML systems, where the "interest" compounds as silent accuracy degradation rather than slower development velocity.

1.  **Significance (quantitative)**: Google's analysis of production ML systems found that model code constitutes roughly 5 percent of the total codebase; the remaining 95 percent is data pipelines, configuration, serving infrastructure, and monitoring [@sculley2015hidden]. ML-specific debt drivers compound this: changing one input feature can silently shift the learned representation of every other feature (entanglement), a model trained to correct another model's errors creates a fragile dependency chain (correction cascades), and downstream systems consuming model outputs without explicit contracts become undeclared consumers that break silently when the model is updated.
2.  **Distinction (durable)**: Unlike software technical debt (which manifests as slower development velocity and is visible in code review), ML technical debt manifests as silent accuracy degradation that is invisible to unit tests, integration tests, and system health monitors. The system continues to run and respond correctly by every infrastructure metric while predictions quietly worsen.
...
class AutomationROI:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mc_manual_hours_str = fmt_int(hrs_manual_week, commas=False)
    mc_pipeline_hours_str = fmt_int(hrs_automation_once, commas=False)
    mc_breakeven_str = fmt_int(breakeven_weeks, commas=False)
    mc_time_horizon_str = fmt_int(time_horizon_years, commas=False)
    mc_manual_final_str = fmt(100, precision=0, commas=False)
...
**Impact**: In the Google Play case study, TFX reduced custom code, shortened experiment cycles, and improved model analysis. More broadly, it illustrates how shared pipeline components can reduce glue code and technical debt in production ML systems.
...
class SilentFailureCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    annual_revenue_str = fmt_int(annual_revenue // MILLION, commas=False, suffix="M")
    quality_drop_pct_str = fmt_int(quality_drop * 100, commas=False)
    quality_drop_str = fmt(quality_drop, precision=2, commas=False)
    manual_detect_days_str = fmt(days_manual, precision=0, commas=False)
    manual_detect_weeks_str = fmt(days_manual / 7, precision=0, commas=False)
    auto_detect_days_str = fmt(days_auto, precision=0, commas=False)
    incidents_per_year_str = fmt(incidents_per_year, precision=0, commas=False)
    ...
    annual_savings_str = fmt_int(round(annual_savings), commas=True)
...
1.  **Manual ops (monthly review)**:
    *   Detection Time: ~`{python} SilentFailureCost.manual_detect_weeks_str` weeks (`{python} SilentFailureCost.manual_detect_days_str` days).
    *   Revenue Loss: USD `{python} SilentFailureCost.annual_revenue_str` $\times$ `{python} SilentFailureCost.quality_drop_str` $\times$ `{python} SilentFailureCost.manual_detect_days_str`/`{python} SilentFailureCost.days_per_year_str` ≈ **USD `{python} SilentFailureCost.loss_manual_str`**.

2.  **Automated MLOps (daily checks)**:
    *   Detection Time: `{python} SilentFailureCost.auto_detect_days_str` day.
    *   Revenue Loss: USD `{python} SilentFailureCost.annual_revenue_str` $\times$ `{python} SilentFailureCost.quality_drop_str` $\times$ `{python} SilentFailureCost.auto_detect_days_str`/`{python} SilentFailureCost.days_per_year_str` ≈ **USD `{python} SilentFailureCost.loss_auto_str`**.
...
class RetrainingAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    daily_decay_str = fmt(daily_decay_pct, precision=0, commas=False, suffix="%")
```
...
class RetrainingInterval:
    ...
    gamma_pct_str = fmt(gamma * 100, precision=0, commas=False)
...
class ABSampleSize:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    confidence_pct_str = fmt(confidence * 100, precision=0, commas=False)
    power_pct_str = fmt(power * 100, precision=0, commas=False)
    baseline_pct_str = fmt(baseline_rate * 100, precision=0, commas=False)
    target_pct_str = fmt(target_rate * 100, precision=1, commas=False)
    relative_lift_pct_str = fmt(relative_lift * 100, precision=0, commas=False)
...
class LatencyBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    slo_p99_str = fmt(slo_p99, precision=0, commas=False)
    network_str = fmt(network, precision=0, commas=False)
    feature_fetch_str = fmt(feature_fetch, precision=0, commas=False)
    request_parse_str = fmt(request_parse, precision=0, commas=False)
    inference_str = fmt(inference, precision=0, commas=False)
    post_proc_str = fmt(post_proc, precision=0, commas=False)
    serialization_str = fmt(serialization, precision=0, commas=False)
    non_inference_str = fmt(non_inference, precision=0, commas=False)
    inference_pct_str = fmt(inference_fraction * 100, precision=0, commas=False)
    model_speedup_str = fmt(model_speedup, precision=0, commas=False)
...
class DriftDetectionDelay:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    baseline_acc_pct_str = fmt(baseline_acc * 100, precision=0, commas=False)
    drop_pct_str = fmt(drop * 100, precision=0, commas=False)
    target_acc_pct_str = fmt(target_acc * 100, precision=0, commas=False)
    confidence_pct_str = fmt(confidence * 100, precision=0, commas=False)
    ...
    days_needed_low_str = fmt(days_needed_low, precision=0, commas=False)
```
...
class ObservabilitySampling:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    request_rate_m_str = fmt(requests_per_second / MILLION, precision=0, commas=False)
    high_seconds_str = fmt(high_frequency_seconds, precision=0, commas=False)
    low_seconds_str = fmt(low_frequency_seconds, precision=0, commas=False)
    high_gb_s_str = fmt(high_bytes_per_second.m_as(GB), precision=0, commas=False)
    low_mb_s_str = fmt(low_bytes_per_second.m_as(MB), precision=1, commas=False)
    sampling_ratio_str = fmt(sampling_ratio, precision=0, commas=False)
    telemetry_kb_str = fmt(telemetry_per_request.m_as(KB), precision=0, commas=False)
    success_sample_pct_str = fmt_percent(success_sample_fraction, precision=0, commas=False)
    error_sample_pct_str = fmt_percent(error_sample_fraction, precision=0, commas=False)
```
...
: **Sampling Frequency vs. Observability Cost**: Roughly `{python} ObservabilitySampling.sampling_ratio_str`$\times$ data-volume swing between `{python} ObservabilitySampling.high_seconds_str` s and `{python} ObservabilitySampling.low_seconds_str` s sampling at `{python} ObservabilitySampling.request_rate_m_str` M requests per second, assuming about `{python} ObservabilitySampling.telemetry_kb_str` KB of telemetry per request. {#tbl-observability-sampling-cost tbl-colwidths="[14,28,28,30]"}

**Engineering Decision**: Use **dynamic sampling**. Sample `{python} ObservabilitySampling.success_sample_pct_str` percent of successful requests but `{python} ObservabilitySampling.error_sample_pct_str` percent of errors. Use high-frequency (`{python} ObservabilitySampling.high_seconds_str` s) monitoring only for aggregate counters (like error rate), but low-frequency (`{python} ObservabilitySampling.low_seconds_str` s) for high-cardinality data (like user-level distribution sketches). @sec-ml-operations-monitoring-cost-model-7fe3 provides worked examples for budgeting monitoring infrastructure.
...
class PSIAgeExample:
    ...
    warning_threshold = Monitoring.PsiWarnThreshold
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    training_pct_str = [fmt(value * 100, precision=0, commas=False) for value in training]
    serving_pct_str = [fmt(value * 100, precision=0, commas=False) for value in serving]
...
class MonitoringBudget:
    ...
    sample_interval_s = SEC_PER_MIN / samples_per_min
    ...
    samples_per_min_str = fmt(samples_per_min, precision=0, commas=False)
    sample_interval_s_str = fmt(sample_interval_s, precision=0, commas=False)
    days_str = fmt(days, precision=0, commas=False)
    ingestion_cost_per_m_str = fmt(ingestion_cost_per_m, precision=2, commas=False)
    bytes_per_point_str = fmt(bytes_per_point, precision=0, commas=False)
    storage_cost_per_gb_str = fmt(storage_cost_per_gb, precision=0, commas=False)
    ...
    work_days_str = fmt(work_days, precision=0, commas=False)
    ...
    query_cost_str = fmt_int(query_cost, commas=False)
...
- Metrics sampled every `{python} MonitoringBudget.sample_interval_s_str` seconds
...
**Systems insight**: Features in ML systems do not disappear just because code owners stop thinking about them. Without explicit deprecation policies and feature-store governance, models accumulate "dead code" that consumes resources and complicates debugging [@sculley2015hidden].
...
class FraudDetectionImprovement:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    current_rate_pct_str = fmt(current_rate * 100, precision=0, commas=False)
    target_rate_pct_str = fmt(target_rate * 100, precision=0, commas=False)
    infra_cost_increase_pct_str = fmt(infra_cost_increase * 100, precision=0, commas=False)
    annual_loss_prevented_str = fmt(annual_loss_prevented // MILLION, precision=0, commas=False, suffix=" million")
...
class SingleModelROI:
    ...
    incident_cost_str = fmt(incident_cost // THOUSAND, precision=0, commas=False, suffix="K")
    ...
    incident_savings_k_str = fmt(incident_savings // THOUSAND, precision=0, commas=False)
    time_savings_k_str = fmt(time_savings // THOUSAND, precision=0, commas=False)
    mlops_investment_k_str = fmt(mlops_investment // THOUSAND, precision=0, commas=False)
...
class OuraValidationGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    accel_only_pct_str = fmt(accel_only_accuracy * 100, precision=0, commas=False)
    enhanced_pct_str = fmt(enhanced_accuracy * 100, precision=0, commas=False)
    scorer_low_pct_str = fmt(scorer_agreement_low * 100, precision=0, commas=False)
    scorer_high_pct_str = fmt(scorer_agreement_high * 100, precision=0, commas=False)
    gain_points_str = fmt(accuracy_gain * 100, precision=0, commas=False)
    gap_closed_range_str = MarkdownStr(
        f"{fmt(gap_closed_high_ceiling * 100, precision=1, commas=False)}-"
        f"{fmt(gap_closed_low_ceiling * 100, precision=0, commas=False)}"
    )
```
...
# │ How: LEGO class OuraModelAccuracy; scenario constants duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt)
# │ Exports: OuraModelAccuracy.accel_only_pct_str, enhanced_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
Through five-fold cross-validation against PSG annotations and iterative tuning, the enhanced models achieved `{python} OuraModelAccuracy.enhanced_pct_str` percent four-stage classification accuracy, a significant improvement from the `{python} OuraModelAccuracy.accel_only_pct_str` percent accelerometer-only baseline toward the clinical benchmark.
...
# │ How: LEGO class OuraOperationalAccuracy; scenario constants duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt)
# │ Exports: OuraOperationalAccuracy.accel_only_pct_str, enhanced_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
**Consistency**: PSG-aligned preprocessing ensures consistent feature computation across training and on-device inference. Five-fold cross-validation against the clinical gold standard verifies that model updates do not regress below the `{python} OuraOperationalAccuracy.enhanced_pct_str` percent four-stage accuracy target established by the enhanced feature set.
...
# │ How: LEGO class OuraCaseRecap; scenario constants duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt)
# │ Exports: OuraCaseRecap.accel_only_pct_str, enhanced_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
