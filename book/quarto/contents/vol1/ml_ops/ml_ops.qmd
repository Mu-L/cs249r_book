...
# │ Show:    About 2.5 GiB per sequence; 20 GiB across eight concurrent ones.
# │ How:     Multiply layers, KV heads, sequence length, head dimension, and
# │          bytes per element from the Llama-2-70B constants.
# │
# │ Imports: mlsysim.core.constants (BYTES_FP16, GiB, byte, LLAMA2_70B_*),
...
# │          KVCacheFootprint.kv_cache_gib_str,
# │          KVCacheFootprint.concurrent_8_gib_str
# └─────────────────────────────────────────────────────────────────────────────
...
class KVCacheFootprint:
    ...
    kv_cache_gib = kv_cache_bytes.m_as(GiB)
    concurrent_8_gib = 8 * kv_cache_gib

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(hidden_dim % attention_heads == 0, "Hidden dimension must divide evenly by attention heads.")
    check(2 <= kv_cache_gib <= 4,
          f"Expected 2-4 GiB per active sequence, got {kv_cache_gib:.2f} GiB.")
