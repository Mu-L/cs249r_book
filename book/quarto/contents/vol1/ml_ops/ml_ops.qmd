...
**Problem**: A model has **`{python} DriftDetectionDelay.baseline_acc_pct_str` baseline accuracy**. The goal is to detect a **`{python} DriftDetectionDelay.drop_pct_str` drop** (to `{python} DriftDetectionDelay.target_acc_pct_str`) with `{python} DriftDetectionDelay.confidence_pct_str` statistical confidence. The system handles **`{python} DriftDetectionDelay.qps_high_str` request per second (`{python} DriftDetectionDelay.qps_high_str`)**. How long will it take to "prove" the model has drifted?
...
|:-----------------------------------------------------:|:----------------|:------------------------------------------------------------------------------:|:----------------------------------|
| **`{python} ObservabilitySampling.high_seconds_str`** | Micro-bursts    |               ~`{python} ObservabilitySampling.high_gb_s_str`/s                | High (Requires dedicated cluster) |
|  **`{python} ObservabilitySampling.low_seconds_str`** | Trends          |                ~`{python} ObservabilitySampling.low_mb_s_str`/s                | Low (Standard sidecar)            |
