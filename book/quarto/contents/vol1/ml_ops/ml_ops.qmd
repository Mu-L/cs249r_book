...
*   **Result**: After `{python} AutomationROI.mc_time_horizon_str`, manual teams still spend `{python} AutomationROI.mc_manual_hours_str` per week on maintenance. Pipeline teams spend `0` recurring hours.
...
**Problem**: A model has **`{python} DriftDetectionDelay.baseline_acc_pct_str` baseline accuracy**. The goal is to detect a **`{python} DriftDetectionDelay.drop_pct_str` percentage-point drop** (to `{python} DriftDetectionDelay.target_acc_pct_str`) with `{python} DriftDetectionDelay.confidence_pct_str` statistical confidence. The system handles **`{python} DriftDetectionDelay.qps_high_str` request per second (`{python} DriftDetectionDelay.qps_high_str`)**. How long will it take to "prove" the model has drifted?
...
|:-----------------------------------------------------:|:----------------|:------------------------------------------------------------------------------:|:----------------------------------|
| **`{python} ObservabilitySampling.high_seconds_str`** | Micro-bursts    |              ~`{python} ObservabilitySampling.high_gb_s_str` GB/s              | High (Requires dedicated cluster) |
|  **`{python} ObservabilitySampling.low_seconds_str`** | Trends          |              ~`{python} ObservabilitySampling.low_mb_s_str` MB/s               | Low (Standard sidecar)            |
...
**Engineering Decision**: Use **dynamic sampling**. Sample `{python} ObservabilitySampling.success_sample_pct_str` of successful requests but `{python} ObservabilitySampling.error_sample_pct_str` of errors. Use high-frequency (`{python} ObservabilitySampling.high_seconds_str` s) monitoring only for aggregate counters (like error rate), but low-frequency (`{python} ObservabilitySampling.low_seconds_str` s) for high-cardinality data (like user-level distribution sketches). @sec-ml-operations-monitoring-cost-model-7fe3 provides worked examples for budgeting monitoring infrastructure.
...
The central objective was improving sleep stage classification accuracy to align more closely with polysomnography (PSG)[^fn-psg-ground-truth], the clinical gold standard. Initial evaluations showed `{python} OuraValidationGap.accel_only_pct_str` four-stage sleep classification accuracy for an accelerometer-only model, compared with `{python} OuraValidationGap.enhanced_pct_str` when autonomic nervous system and circadian features were included. Published human PSG inter-scorer reliability is about `{python} OuraValidationGap.scorer_low_pct_str` to `{python} OuraValidationGap.scorer_high_pct_str`, framing the remaining gap between wearable inference and expert clinical scoring. The `{python} OuraValidationGap.gain_points_str` percentage-point gain closes roughly `{python} OuraValidationGap.gap_closed_range_str` of the baseline-to-human-agreement gap, so the remaining improvement target is real but bounded by the noisiness of the clinical reference itself. This discrepancy prompted an effort to re-evaluate data collection, preprocessing, and model development workflows.
