...
where $\delta$ is the minimum detectable effect, $\sigma$ is outcome standard deviation, and $z$ values depend on desired confidence (typically 95 percent) and power (typically 80 percent). For a 2 percent *relative* lift on a 5 percent baseline conversion rate (5.0 percent to 5.1 percent) and 80 percent power, expect roughly 745,000 users per variant; ~25,000 users per variant would only detect a much larger lift, about 0.5–0.6 percentage points absolute.
...
**Problem**: A service has a 100 ms P99 SLO, and the model inference budget is 45 ms. Where must the remaining 55 ms go? @tbl-latency-budget-components allocates the budget across the request lifecycle:

::: {tbl-colwidths="[23,17,20,40]"}

| **Component**                                     | **Typical %** | **P99 Contribution** | **Optimization Lever**                       |
|:--------------------------------------------------|:-------------:|:--------------------:|:---------------------------------------------|
| **Network RTT**\index{Network RTT!latency budget} | 10–25 percent |       15–45 ms       | Edge deployment, connection pooling          |
...
: **Latency Budget Components**: Allocation of a 100 ms P99 SLO across the request lifecycle. Model inference accounts for 25–45 percent of total latency, leaving the majority to network, feature retrieval, and serialization. The optimization-lever column shows where each component can be reduced. {#tbl-latency-budget-components}
...
**Problem**: A model has **`{python} DriftDetectionDelay.baseline_acc_pct_str` percent baseline accuracy**. The goal is to detect a **`{python} DriftDetectionDelay.drop_pct_str` percent drop** (to `{python} DriftDetectionDelay.target_acc_pct_str` percent) with `{python} DriftDetectionDelay.confidence_pct_str` percent statistical confidence. The system handles **`{python} DriftDetectionDelay.qps_high_str` request per second (`{python} DriftDetectionDelay.qps_high_str` QPS)**. How long will it take to "prove" the model has drifted?
...
3.  **Low-traffic case**: If the model only processes **`{python} DriftDetectionDelay.requests_per_day_low_str` requests per day**, detecting the same `{python} DriftDetectionDelay.drop_pct_str` percent drift takes **`{python} DriftDetectionDelay.days_needed_low_str` days**.
...
@tbl-observability-sampling-cost contrasts the data-volume swing between high- and low-frequency sampling regimes at 1 M requests per second:
...
: **Sampling Frequency vs. Observability Cost**: Two-order-of-magnitude data-volume swing between 1 s and 60 s sampling at 1 M requests per second. High-frequency sampling captures micro-bursts but requires dedicated infrastructure; coarse sampling shows only trends. The choice frames the dynamic-sampling decision below. {#tbl-observability-sampling-cost tbl-colwidths="[14,28,28,30]"}
...
: **PSI Worked Example**: User age distribution drift from training to serving, decomposed across six age bins. The total PSI is 0.029, well below the 0.1 warning threshold, even though several bins shifted by 3 percentage points. The contribution column shows why aggregate PSI is more reliable than per-bin inspection: small individual shifts cancel out across the distribution. {#tbl-psi-age-example tbl-colwidths="[14,17,17,17,18,17]"}
...
Through five-fold cross-validation against PSG annotations and iterative tuning, the enhanced models achieved 79 percent four-stage classification accuracy, a significant improvement from the 57 percent accelerometer-only baseline toward the clinical benchmark.
...
**Consistency**: PSG-aligned preprocessing ensures consistent feature computation across training and on-device inference. Five-fold cross-validation against the clinical gold standard verifies that model updates do not regress below the 79 percent four-stage accuracy target established by the enhanced feature set.
