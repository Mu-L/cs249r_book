...
**Worked Example**: Given a P99 SLO of `{python} LatencyBudget.slo_p99_str`, allocate the budget:

- Network: `{python} LatencyBudget.network_str` (use regional deployment)
- Feature fetch: `{python} LatencyBudget.feature_fetch_str` (require feature store P99 < `{python} LatencyBudget.feature_fetch_str`)
- Request parsing: `{python} LatencyBudget.request_parse_str` (prefer compact binary protocols)
- Inference: `{python} LatencyBudget.inference_str` (sets model complexity ceiling)
- Postprocessing: `{python} LatencyBudget.post_proc_str` (allows light business logic)
- Response serialization: `{python} LatencyBudget.serialization_str` (avoid oversized payloads)
