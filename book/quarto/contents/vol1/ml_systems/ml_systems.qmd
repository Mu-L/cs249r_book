...
class MLSystemsSetup:
    ...
    nvlink_h100_bw_str = fmt_int(NVLINK_H100_BW.m_as(GB/second), commas=False, suffix=" GB/s")
```
...
class LightLatency:
    ...
    distance_str = fmt(distance_km.m_as('km'), precision=0)
```
...
class ThrottlingScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fps_start_str = fmt(fps_start, precision=0, commas=False)
    fps_throttled_str = fmt(fps_throttled, precision=0, commas=False)
    duration_min_str = fmt(duration_min, precision=0, commas=False)
```
...
class EnergyTransmission:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    data_mb_str = fmt(data_size.m_as(MB), precision=0, commas=False)
    tx_energy_str = fmt(tx_energy_per_mb.m_as(ureg.millijoule/MB), precision=0, commas=False)
    compute_energy_str = fmt(local_energy_op.m_as(ureg.millijoule), precision=1, commas=False)
    cloud_total_str = fmt(cloud_energy_total.m_as(ureg.millijoule), precision=0, commas=False)
...
class LighthouseModels:
    ...
    kws_params_str = fmt_int(kws_params, commas=False, suffix="K")
    kws_flops_m_str = fmt(kws_flops_m, precision=0)
...
class ResnetCloud:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tflops_str = fmt(a100_tflops_value, precision=0, commas=False)        # e.g. "312" TFLOP/s
    a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=2, commas=False)        # ~2.04 TB/s (HBM2e)
...
class ResnetMobile:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobile_tops_str = fmt(mobile_tops_value, precision=0, commas=False)        # e.g. "35" TOPS
    mobile_bw_gbs_str = fmt(mobile_bw_gbs_value, precision=0, commas=False)    # e.g. "100" GB/s
...
class MobileHardwareSpecs:
    ...
    mobile_bw_range_str = MarkdownStr(f"{fmt(mobile_bw_min, precision=0, commas=False)}-{fmt(mobile_bw_max, precision=0, commas=False)}")
    mobile_npu_range_str = MarkdownStr(f"{fmt(mobile_npu_min, precision=0, commas=False)}-{fmt(mobile_npu_max, precision=0, commas=False)}")
```
...
class HardwareSpectrumSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tpu_chips_str = fmt(TPU_POD_CHIPS, precision=0)          # e.g. "4,096" chips
    cloud_mem_tb_str = fmt(TPU_POD_MEM.m_as(TB), precision=0, commas=False)         # e.g. "131" TB
    cloud_pwr_mw_str = fmt(TPU_POD_POWER.m_as("megawatt"), precision=0, commas=False)  # e.g. "3" MW
    ...
    edge_price_min_str = fmt(DGX_PRICE_MIN.m_as(USD), precision=0)                  # e.g. "3,000"
    edge_price_max_str = fmt(DGX_PRICE_MAX.m_as(USD), precision=0)                  # e.g. "5,000"
    ...
    tiny_price_str = fmt(ESP32_PRICE.m_as(USD), precision=0, commas=False)           # e.g. "10" USD
...
class DistancePenalty:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sol_kms_str = fmt(SPEED_OF_LIGHT_FIBER_KM_S.m_as('km/s'), precision=0)   # e.g. "200,000" km/s
    ...
    distance_km_str = fmt(distance_km_value, precision=0)                    # e.g. "1,500" km
    safety_budget_str = fmt(safety_budget_ms_value, precision=0, commas=False)  # "10" ms
```
...
class CloudEdgeTCO:
    ...
    breakeven_requests_str = fmt_int(breakeven_requests_per_day/1_000, commas=False, suffix="K")
    ...
    server_life_str = fmt(server_life_years, precision=0, commas=False)
...
class DataLocalityInvariant:
    ...
    net_bw_str = fmt(net_bw_mbps, precision=0, commas=False)
```
...
class EdgeSizing:
    ...
    derate_pct_str = fmt_percent(derate, precision=0, commas=False)

    coral_power_w_str = fmt(coral.tdp.m_as(watt), precision=0, commas=False)
    coral_power_kw_str = fmt(coral.tdp.m_as(kilowatt), precision=3, commas=False)
    coral_tops_str = fmt(coral_peak_tops, precision=0, commas=False)
    coral_sustained_tops_str = fmt(coral_sustained_tops, precision=0, commas=False)
    coral_units_per_store_str = fmt(coral_units_per_store, precision=0, commas=False)
    coral_cluster_peak_tops_str = fmt(coral_cluster_peak_tops, precision=0, commas=False)
    coral_cluster_sustained_tops_str = fmt(coral_cluster_sustained_tops, precision=0, commas=False)
    coral_cluster_unit_cost_str = fmt(coral_cost * coral_units_per_store, precision=0, commas=False)
    coral_cluster_power_w_str = fmt(coral.tdp.m_as(watt) * coral_units_per_store, precision=0, commas=False)
    ...
    jetson_tops_str = fmt(jetson.compute.precision_flops["int8"].m_as(TOPS), precision=0, commas=False)

    nuc_cost_str = fmt(nuc_cost, precision=0, commas=False)
    nuc_power_w_str = fmt(nuc.tdp.m_as(watt), precision=0, commas=False)
    nuc_tops_str = fmt(nuc.compute.peak_flops.m_as(TOPS), precision=0, commas=False)
    ...
    years_str = fmt(years, precision=0, commas=False)
...
class ThermalQuantCalc:
    ...
    mobile_tdp_str = fmt(mobile_tdp_w_value, precision=0, commas=False)              # e.g. "3" W
    ...
    temp_rise_str = fmt(temp_rise_c_per_s_value, precision=0, commas=False)
    trip_time_str = fmt(trip_time_s_value, precision=0, commas=False)
    trip_temp_str = fmt(trip_temp_c_value, precision=0, commas=False)
    fps_before_str = fmt(fps_before_value, precision=0, commas=False)
    fps_after_str = fmt(fps_after_value, precision=0, commas=False)
```
...
class MobileResourceRecap:
    ...
    mobile_ram_range_str = mobile_ram_range
    mobile_storage_range_str = mobile_storage_range
    mobile_npu_range_str = f"{fmt(mobile_npu_min, precision=0, commas=False)}-{fmt(mobile_npu_max, precision=0, commas=False)}"
    mobile_bw_range_str = f"{fmt(mobile_bw_min, precision=0, commas=False)}-{fmt(mobile_bw_max, precision=0, commas=False)}"
    mobile_tdp_range_str = MarkdownStr(mobile_tdp_range.replace("-", " to "))
    mobile_latency_range_str = mobile_latency_range
```
...
class EnergyInference:
    ...
    q_gpt4_str = fmt_int(q_gpt4.m_as(''), commas=True)
    q_resnet_cloud_str = fmt_int(q_resnet_cloud.m_as(''), commas=True)
    q_resnet_edge_str = fmt_int(q_resnet_edge.m_as(''), commas=True)
    q_mobilenet_str = fmt_int(q_mobilenet.m_as(''), commas=True)
    ...
    batt_cap_mah_str = fmt(BATTERY_CAPACITY_MAH.m_as('mAh'), precision=0, commas=False)
...
class WildlifeMonitoring:
    ...
    summary_kb_str = fmt(summary_per_day.m_as(KB), precision=0, commas=False)
...
class BrakingDistance:
    ...
    latency_str = fmt(latency_ms, precision=0, commas=False)
```
...
class MobilePowerFallacyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    phone_battery_str = fmt(battery_wh_value, precision=0, commas=False)
    low_power_w_str = fmt(low_power_w_value, precision=0, commas=False)             # "1"
    high_power_w_str = fmt(high_power_w_value, precision=0, commas=False)           # "5"
