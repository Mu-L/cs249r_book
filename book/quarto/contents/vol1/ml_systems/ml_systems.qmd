...
class MLSystemsSetup:
    ...
    nvlink_h100_bw_str = fmt(int(NVLINK_H100_BW.m_as(GB/second)), precision=1, commas=False, suffix=" GB/s")
```
...
class LightLatency:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    min_latency_str = fmt(min_latency_ms, precision=1, commas=False)
    distance_str = fmt(distance_km.m_as('km'), precision=1)
```
...
class ThrottlingScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fps_start_str = fmt(fps_start, precision=1, commas=False)
    fps_throttled_str = fmt(fps_throttled, precision=1, commas=False)
    duration_min_str = fmt(duration_min, precision=1, commas=False)
```
...
class MemoryWallTrends:
    ...
    mem_bw_growth_pct_str = fmt(mem_bw_growth_pct, precision=1)
```
...
class EnergyTransmission:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    data_mb_str = fmt(data_size.m_as(MB), precision=1, commas=False)
    tx_energy_str = fmt(tx_energy_per_mb.m_as(ureg.millijoule/MB), precision=1, commas=False)
    compute_energy_str = fmt(local_energy_op.m_as(ureg.millijoule), precision=1, commas=False)
    cloud_total_str = fmt(cloud_energy_total.m_as(ureg.millijoule), precision=1, commas=False)
    local_total_str = fmt(local_energy_total.m_as(ureg.millijoule), precision=1, commas=False)
    ratio_str = fmt(ratio.m_as(""), precision=1, commas=True)
```
...
class LighthouseModels:
    ...
    resnet_params_m_str = fmt(resnet_params_m, precision=1, drop_zero=True)
    resnet_fp32_mb_str = fmt(resnet_fp32_mb, precision=1)

    gpt2_params_b_str = fmt(gpt2_params_b, precision=1, drop_zero=True)
    llama_range_str = MarkdownStr("7 to 70") # Llama family range

    dlrm_embedding_str = fmt(dlrm_embedding_gb, precision=1)

    mobilenet_flops_reduction_str = fmt(mobilenet_flops_reduction, precision=1)
    mobile_tdp_range_str = MarkdownStr("2 to 5") # Standard mobile envelope

    # Custom "K" suffix → canonical fmt() with suffix arg
    kws_params_str = fmt(int(kws_params), precision=1, commas=False, suffix="K")
    kws_flops_m_str = fmt(kws_flops_m, precision=1)
    kws_size_kb_str = fmt(kws_size_kb, precision=1)
```
...
class ResnetSetup:
    ...
    resnet_params_m_str = fmt(resnet_params_m_value, precision=1, commas=False, drop_zero=True)             # e.g. "25.6" M
    resnet_fp32_mb_str = fmt(resnet_fp32_bytes_value.m_as(MB), precision=1, commas=False)   # e.g. "102" MB
    resnet_fp16_mb_str = fmt(resnet_fp16_bytes_value.m_as(MB), precision=1, commas=False)   # e.g. "51" MB
    resnet_int8_mb_str = fmt(resnet_int8_bytes_value.m_as(MB), precision=1, commas=False)   # e.g. "26" MB
...
class ResnetCloud:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tflops_str = fmt(a100_tflops_value, precision=1, commas=False)        # e.g. "312" TFLOP/s
    a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=1, commas=False)        # e.g. "2" TB/s
    cloud_ratio_x_str = fmt(cloud_ratio_x_value, precision=1, commas=False)    # memory/compute ratio
...
class ResnetMobile:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobile_tops_str = fmt(mobile_tops_value, precision=1, commas=False)        # e.g. "35" TOPS
    mobile_bw_gbs_str = fmt(mobile_bw_gbs_value, precision=1, commas=False)    # e.g. "100" GB/s
    mobile_ratio_x_str = fmt(mobile_ratio_x_value, precision=1, commas=False)  # memory/compute ratio
    mobile_bottleneck_str = MarkdownStr(mobile_bottleneck_value)               # "Memory" or "Compute"
    bw_advantage_x_str = fmt(bw_advantage_x_value, precision=1, commas=False)  # A100 vs NPU bandwidth
    inference_speed_x_str = fmt(inference_speed_x_value, precision=1, commas=False)  # latency ratio
```
...
class MobileHardwareSpecs:
    ...
    mobile_bw_range_str = MarkdownStr(f"{fmt(mobile_bw_min, precision=1, commas=False)}-{fmt(mobile_bw_max, precision=1, commas=False)}")
    mobile_npu_range_str = MarkdownStr(f"{fmt(mobile_npu_min, precision=1, commas=False)}-{fmt(mobile_npu_max, precision=1, commas=False)}")
```
...
class HardwareSpectrumSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tpu_chips_str = fmt(TPU_POD_CHIPS, precision=1)          # e.g. "4,096" chips
    cloud_mem_tb_str = fmt(TPU_POD_MEM.m_as(TB), precision=1, commas=False)         # e.g. "131" TB
    cloud_pwr_mw_str = fmt(TPU_POD_POWER.m_as("megawatt"), precision=1, commas=False)  # e.g. "3" MW

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    edge_mem_gb_str = fmt(DGX_RAM.m_as(GB), precision=1, commas=False)              # e.g. "128" GB
    edge_stor_tb_str = fmt(DGX_STORAGE.m_as(TB), precision=1, commas=False)         # e.g. "4" TB
    edge_pwr_w_str = fmt(DGX_POWER.m_as(watt), precision=1, commas=False)           # e.g. "500" W
    edge_price_min_str = fmt(DGX_PRICE_MIN.m_as(USD), precision=1)                  # e.g. "3,000"
    edge_price_max_str = fmt(DGX_PRICE_MAX.m_as(USD), precision=1)                  # e.g. "5,000"

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tiny_flash_mb_str = fmt(ESP32_FLASH.m_as(MB), precision=1, commas=False)        # e.g. "4" MB
    ...
    tiny_price_str = fmt(ESP32_PRICE.m_as(USD), precision=1, commas=False)           # e.g. "10" USD
...
class GPT3TrainingScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_pflop_days_str = fmt(gpt3_pflop_days, precision=1, commas=True)
    gpt3_v100_count_str = fmt(gpt3_v100_count, precision=0, commas=True)
    gpt3_days_str = fmt(gpt3_days, precision=1)
...
class DistancePenalty:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sol_kms_str = fmt(SPEED_OF_LIGHT_FIBER_KM_S.m_as('km/s'), precision=1)   # e.g. "200,000" km/s
    rtt_formatted_str = fmt(round_trip_ms_value, precision=1, commas=False)  # e.g. "15" ms
    deficit_str = fmt(deficit_ms_value, precision=1, commas=False)           # e.g. "-5" ms
    distance_km_str = fmt(distance_km_value, precision=1)                    # e.g. "1,500" km
    safety_budget_str = fmt(safety_budget_ms_value, precision=1, commas=False)  # "10" ms
```
...
class CloudEdgeTCO:
    ...
    requests_str = fmt(requests_per_day/MILLION, precision=1, commas=False, suffix="M")
    inference_str = fmt(inference_ms, precision=1, commas=False, suffix=" ms")
    response_str = fmt(response_kb, precision=1, commas=False, suffix=" KB")
    ...
    egress_gb_str = fmt(egress_gb_per_day, precision=1, commas=False)
    egress_price_str = MarkdownStr(f"USD {egress_per_gb:.2f}/GB")
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=0)
    days_per_year_str = fmt(DAYS_PER_YEAR, precision=1, commas=False)

    c_gpu_str = fmt(c_gpu, precision=0, prefix="~$")
    c_egress_str = fmt(c_egress, precision=1, prefix="~$")
    c_lb_str = fmt(c_lb, precision=1, prefix="~$")
    c_logs_str = fmt(c_logs, precision=1, prefix="~$")
    c_total_str = fmt(c_total, precision=1, prefix="~$", suffix="/year")

    e_capex_str = fmt(e_capex, precision=1, prefix="~$")
    e_power_str = fmt(e_power, precision=1, prefix="~$")
    e_cool_str = fmt(e_cool, precision=1, prefix="~$")
    e_net_str = fmt(e_net, precision=1, prefix="~$")
    e_labor_str = fmt(e_labor, precision=1, prefix="~$")
    e_total_str = fmt(e_total, precision=1, prefix="~$", suffix="/year")
    ...
    breakeven_requests_str = fmt(breakeven_requests_per_day/1_000, precision=0, commas=False, suffix="K")
    breakeven_utilization_str = fmt(breakeven_fraction * 100, precision=2, commas=False, suffix="%")

    # Additional Outputs for Prose
    server_cost_str = fmt(server_cost, precision=1, prefix="$")
    server_life_str = fmt(server_life_years, precision=1, commas=False)
    power_str = fmt(power_watts, precision=1, commas=False, suffix=" W")
    ...
    devops_salary_str = fmt(devops_salary, precision=1, prefix="$")
```
...
class VoiceAssistantWall:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Economics Strings
    ww_devices_b_str = fmt(ww_devices_b, precision=1, commas=False, drop_zero=True)
    ww_cloud_cost_str = fmt(ww_cloud_cost_per_device, precision=2, commas=False)
    ww_total_cost_str = fmt(ww_total_cloud_cost, precision=1, commas=True)
    ...
    vi_audio_kb_str = fmt(vi_audio_kb_per_sec, precision=1, commas=False)
    vi_audio_tb_str = fmt(vi_total_audio_tb_per_sec, precision=1, commas=False)
```
...
class BandwidthBottleneck:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cam_rate_mbs_str = fmt(bytes_per_sec_single.m_as(MB/second), precision=1, commas=False)
    ...
    metadata_kb_per_event_str = fmt(metadata_kb_per_event.m_as(KB), precision=1, commas=False)
    metadata_reduction_x_str = fmt(metadata_reduction, precision=1, commas=True)

    num_cameras_str = fmt(num_cameras, precision=0, commas=False)
    bb_fps_str = fmt(int(fps.m_as('Hz')), precision=1, commas=False)
    egress_cost_str = fmt(CLOUD_EGRESS_PER_GB.m_as(USD/GB), precision=2, commas=False)
    video_width_str = fmt(width, precision=1, commas=False)
    video_height_str = fmt(height, precision=1, commas=False)
    bytes_per_pixel_str = fmt(bpp, precision=1, commas=False)
...
class DataLocalityInvariant:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frame_mb_str = fmt(frame_mb, precision=1)
    tx_time_ms_str = fmt(tx_time_ms, precision=1)
    remote_ms_str = fmt(remote_total_ms, precision=1)
    net_bw_str = fmt(net_bw_mbps, precision=1, commas=False)
```
...
class EdgeSizing:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    stores_str = fmt(stores, precision=1, commas=False)
    cameras_per_store_str = fmt(cameras_per_store, precision=1, commas=False)
    fps_str = fmt(fps, precision=1, commas=False)
    headroom_str = fmt(headroom, precision=1, commas=False)
    inf_per_sec_str = fmt(inf_per_sec, precision=1, commas=False)

    yolo_gflops_str = fmt(yolo_flops.m_as(GFLOPs), precision=1)
    sustained_gf_str = fmt(sustained_gflops, precision=1)
    ...
    coral_power_w_str = fmt(coral.tdp.m_as(watt), precision=1, commas=False)
    coral_power_kw_str = fmt(coral.tdp.m_as(kilowatt), precision=3, commas=False)
    coral_tops_str = fmt(coral_peak_tops, precision=1, commas=False)
    coral_sustained_tops_str = fmt(coral_sustained_tops, precision=1, commas=False)
    coral_units_per_store_str = fmt(coral_units_per_store, precision=1, commas=False)
    coral_cluster_peak_tops_str = fmt(coral_cluster_peak_tops, precision=1, commas=False)
    coral_cluster_sustained_tops_str = fmt(coral_cluster_sustained_tops, precision=1, commas=False)
    coral_cluster_unit_cost_str = fmt(coral_cost * coral_units_per_store, precision=1, commas=False)
    coral_cluster_power_w_str = fmt(coral.tdp.m_as(watt) * coral_units_per_store, precision=1, commas=False)
    coral_cluster_power_kw_str = fmt(coral.tdp.m_as(kilowatt) * coral_units_per_store, precision=3, commas=False)

    jetson_cost_str = fmt(jetson_cost, precision=1, commas=False)
    jetson_power_range_str = MarkdownStr("10-40")
    jetson_tops_str = fmt(jetson.compute.precision_flops["int8"].m_as(TOPS), precision=1, commas=False)

    nuc_cost_str = fmt(nuc_cost, precision=1, commas=False)
    nuc_power_w_str = fmt(nuc.tdp.m_as(watt), precision=1, commas=False)
    nuc_tops_str = fmt(nuc.compute.peak_flops.m_as(TOPS), precision=1, commas=False)

    coral_tco_str = fmt(coral_tco, precision=1, commas=True)
    jetson_tco_str = fmt(jetson_tco, precision=1, commas=True)
    nuc_tco_str = fmt(nuc_tco, precision=1, commas=True)

    # Additional Outputs for Prose
    jetson_fleet_str = fmt(jetson_cost * stores, precision=1, commas=True)
    nuc_fleet_str = fmt(nuc_cost * stores, precision=1, commas=True)
    coral_cluster_fleet_str = fmt(coral_cluster_capex, precision=1, commas=True)
    coral_cluster_tco_str = fmt(coral_cluster_tco, precision=1, commas=True)
    coral_cluster_power_opex_str = fmt(coral_cluster_power_opex, precision=1, commas=True)

    years_str = fmt(years, precision=1, commas=False)
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=0, commas=False)

    power_ratio_str = fmt(jetson.tdp.m_as(watt) / coral.tdp.m_as(watt), precision=1, commas=False)
    ...
    cloud_cost_str = fmt(cloud_tco_3yr, precision=1, commas=True)
...
class MobileEfficiencyRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobilenet_flops_reduction_str = fmt(mobilenet_flops_reduction, precision=1, commas=False)
...
class BatteryTax:
    ...
    pwr_w_str = fmt(power_draw.m_as(ureg.watt), precision=1, commas=False)
    batt_wh_str = fmt(battery_wh.m_as(ureg.Wh), precision=1, commas=False)
    budget_pct_str = fmt(daily_budget_pct.m_as(''), precision=1, commas=False)
...
class ThermalQuantCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    baseline_str = fmt(baseline_power_w_value, precision=1, commas=False)            # e.g. "12" W
    mobile_tdp_str = fmt(mobile_tdp_w_value, precision=1, commas=False)              # e.g. "3" W
    quant_power_str = fmt(quant_power_w_value, precision=1, commas=False)            # e.g. "3" W
    quant_red_str = fmt(quant_reduction_value, precision=1, commas=False)            # e.g. "4" ×
    temp_rise_str = fmt(temp_rise_c_per_s_value, precision=1, commas=False)
    trip_time_str = fmt(trip_time_s_value, precision=1, commas=False)
    trip_temp_str = fmt(trip_temp_c_value, precision=1, commas=False)
    fps_before_str = fmt(fps_before_value, precision=1, commas=False)
    fps_after_str = fmt(fps_after_value, precision=1, commas=False)
```
...
class MobileBatteryCapacity:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    phone_battery_str = fmt(phone_battery_wh, precision=1)
    added_ml_runtime_low_str = fmt(added_ml_runtime_low_h, precision=1, commas=False)
    added_ml_runtime_high_str = fmt(added_ml_runtime_high_h, precision=1, commas=False)
```
...
class MobileResourceRecap:
    ...
    mobile_ram_range_str = mobile_ram_range
    mobile_storage_range_str = mobile_storage_range
    mobile_npu_range_str = f"{fmt(mobile_npu_min, precision=1, commas=False)}-{fmt(mobile_npu_max, precision=1, commas=False)}"
    mobile_bw_range_str = f"{fmt(mobile_bw_min, precision=1, commas=False)}-{fmt(mobile_bw_max, precision=1, commas=False)}"
    mobile_tdp_range_str = MarkdownStr(mobile_tdp_range.replace("-", " to "))
    mobile_latency_range_str = mobile_latency_range
```
...
class MobileResnetPitfallRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    resnet_fp32_mb_str = fmt(resnet_fp32_mb_value, precision=1, commas=False)
...
class EnergyInference:
    ...
    e_resnet_edge_str = fmt(e_resnet_edge_j.m_as("mJ"), precision=1, suffix=" mJ")
    e_mobilenet_str = fmt(e_mobilenet_j.m_as("mJ"), precision=1, suffix=" mJ")
    e_kws_str = fmt(e_kws_j.m_as("mJ"), precision=1, suffix=" mJ")

    q_gpt4_str = fmt(q_gpt4.m_as(''), precision=1, commas=True)
    q_resnet_cloud_str = fmt(q_resnet_cloud.m_as(''), precision=1, commas=True)
    q_resnet_edge_str = fmt(q_resnet_edge.m_as(''), precision=1, commas=True)
    q_mobilenet_str = fmt(q_mobilenet.m_as(''), precision=1, commas=True)
    q_kws_str = fmt(q_kws.m_as(''), precision=1, commas=True)
    # Use BILLION constant
    q_kws_str = fmt(q_kws.m_as('') / MILLION, precision=1, commas=False) + " million"

    batt_cap_mah_str = fmt(BATTERY_CAPACITY_MAH.m_as('mAh'), precision=1, commas=False)
...
class WildlifeMonitoring:
    ...
    summary_kb_str = fmt(summary_per_day.m_as(KB), precision=1, commas=False)
    reduction_x_str = fmt(reduction, precision=1, commas=True)
```
...
class BrakingDistance:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    speed_str = fmt(speed_kmh, precision=1, commas=False)
    distance_str = fmt(distance_m, precision=1, commas=False)
    latency_str = fmt(latency_ms, precision=1, commas=False)
```
...
class MobilePowerFallacyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    phone_battery_str = fmt(battery_wh_value, precision=1, commas=False)
    low_power_w_str = fmt(low_power_w_value, precision=1, commas=False)             # "1"
    high_power_w_str = fmt(high_power_w_value, precision=1, commas=False)           # "5"
    ...
    mobile_tdp_str = fmt(mobile_tdp_w_value, precision=1, commas=False)
    quant_red_str = fmt(quant_reduction_value, precision=1, commas=False)
...
class TcoPitfallCalc:
    ...
    cloud_compute_str = fmt(cloud_compute_value, precision=1, commas=True)          # "2,000"
    edge_hw_str = fmt(edge_hardware_value, precision=1, commas=False)               # "500"
    edge_network_str = fmt(edge_network_value, precision=1, commas=True)            # "3,000"
    edge_maint_str = fmt(edge_maintenance_value, precision=1, commas=False)         # "500"
    edge_reliability_str = fmt(edge_reliability_value, precision=1, commas=True)    # "2,000"
    edge_total_str = fmt(edge_total_value, precision=1, commas=True)                # "6,000"
    tco_ratio_str = fmt(tco_ratio_value, precision=1, commas=False)                 # "3"
```
...
class AmdahlCameraCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cam_isp_str = fmt(cam_isp_ms_value, precision=1, commas=False)                 # "100"
    cam_ml_str = fmt(cam_ml_ms_value, precision=1, commas=False)                   # "60"
    cam_post_str = fmt(cam_post_ms_value, precision=1, commas=False)               # "40"
    cam_total_str = fmt(cam_total_ms_value, precision=1, commas=False)             # "200"
    ...
    cam_ml_opt_str = fmt(cam_ml_optimized_ms_value, precision=1, commas=False)     # "6"
    cam_total_opt_str = fmt(cam_total_optimized_ms_value, precision=1, commas=False)  # "146"
