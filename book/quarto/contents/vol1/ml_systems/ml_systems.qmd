...
class MLSystemsSetup:
    ...
    nvlink_h100_bw_str = fmt(int(NVLINK_H100_BW.m_as(GB/second)), precision=0, commas=False, suffix=" GB/s")
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import NETWORK_5G_ENERGY_PER_MB_MJ, ENERGY_MOBILENET_INF_MJ, MB, ureg
from mlsysim.fmt import fmt_percent, fmt, check
...
from mlsysim.core.constants import (
    RESNET50_FLOPs, GFLOPs, MFLOPs, Mparam, Bparam, Kparam, byte, MB, GB, KB,
    MOBILE_TDP_RANGE_W,
)
...
class LighthouseModels:
    ...
    mobilenet_flops_reduction = (RESNET50_FLOPs / m_mobilenet.inference_flops).m_as("")
    ...
    kws_params_str = fmt(int(kws_params), precision=0, commas=False, suffix="K")
...
class ResnetMobile:
    ...
    mobile_tops_value = h_phone.compute.peak_flops.m_as(TOPS)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.core.constants import MOBILE_RAM_RANGE_GB, MOBILE_STORAGE_RANGE, TFLOPs, GB, second
...
# │ How: Derive PFLOP-days from Models.GPT3.training_ops; format
...
class GPT3TrainingScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_pflop_days_str = fmt(gpt3_pflop_days, precision=0, commas=True)
...
class CloudEdgeTCO:
    ...
    e_power_str = fmt(e_power, precision=0, prefix="~$")
    e_cool_str = fmt(e_cool, precision=0, prefix="~$")
    e_net_str = fmt(e_net, precision=0, prefix="~$")
    e_labor_str = fmt(e_labor, precision=0, prefix="~$")
    e_total_str = fmt(e_total, precision=0, prefix="~$", suffix="/year")

    edge_savings_str = fmt(edge_savings_pct, precision=0, commas=False, suffix="%")
    labor_pct_str = fmt(labor_pct, precision=0, commas=False, suffix="%")
    breakeven_requests_str = fmt(breakeven_requests_per_day/1_000, precision=0, commas=False, suffix="K")
    breakeven_utilization_str = fmt(breakeven_fraction * 100, precision=0, commas=False, suffix="%")
...
class BandwidthBottleneck:
    ...
    bb_fps_str = fmt(int(fps.m_as('Hz')), precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MB, byte
from mlsysim.fmt import fmt, check
...
A realistic retail deployment shows how those constraints translate into a concrete hardware decision: given a throughput target and a three-year budget, which edge accelerator minimizes fleet cost?
...
from mlsysim import Hardware, Models
from mlsysim.core.constants import GFLOPs, CLOUD_ELECTRICITY_PER_KWH, HOURS_PER_YEAR, TOPS, USD, watt, kilowatt, second, ureg
...
class EdgeSizing:
    ...
    coral_cluster_tco_str = fmt(coral_cluster_tco, precision=0, commas=True)
    coral_cluster_power_opex_str = fmt(coral_cluster_power_opex, precision=0, commas=True)
...
**Problem**: A smart retail chain deploys person detection across `{python} EdgeSizing.stores_str` stores, each with `{python} EdgeSizing.cameras_per_store_str` cameras at `{python} EdgeSizing.fps_str` FPS. Which edge accelerator (USB-scale TPU, workstation-class embedded GPU, or general-purpose mini-PC) delivers the required throughput at the lowest three-year fleet cost?
...
: **Edge Inference Sizing Requirements**: Per-store throughput target for the smart-retail person-detection scenario. {#tbl-ml-systems-edge-sizing-requirements}
...
: **Edge Accelerator Options**: Throughput, power, and cost for three candidate edge accelerators at fleet scale. {#tbl-ml-systems-edge-hardware-options}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Models
from mlsysim.core.constants import RESNET50_FLOPs, MOBILE_TDP_RANGE_W
from mlsysim.fmt import fmt, check, MarkdownStr
...
class MobileEfficiencyRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobilenet_flops_reduction_str = fmt(mobilenet_flops_reduction, precision=0, commas=False)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
from mlsysim.core.constants import OBJECT_DETECTOR_POWER_W, HOURS_PER_DAY, ureg
...
from mlsysim.fmt import fmt_percent, fmt, check
from mlsysim.core.constants import MOBILE_TDP_W, watt
...
class MobileBatteryCapacity:
    ...
    added_ml_runtime_low_str = fmt(added_ml_runtime_low_h, precision=0, commas=False)
    added_ml_runtime_high_str = fmt(added_ml_runtime_high_h, precision=0, commas=False)
```
...
from mlsysim import Hardware
from mlsysim.core.constants import (
    MOBILE_RAM_RANGE_GB, MOBILE_STORAGE_RANGE, MOBILE_LATENCY_RANGE_MS,
    MOBILE_TDP_RANGE_W, TFLOPs, GB, second,
)
from mlsysim.fmt import fmt, check, MarkdownStr
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import RESNET50_FLOPs, RESNET50_PARAMS, GFLOPs, byte, MB
from mlsysim.fmt import fmt, check
...
# │ How: Calculate J per inference for TinyML, Mobile, and Cloud paradigms.
...
class EnergyInference:
    ...
    e_resnet_cloud_str = MarkdownStr("~10 J")
    e_resnet_edge_str = MarkdownStr("~500 mJ")
    e_mobilenet_str = MarkdownStr("~50 mJ")
    e_kws_str = MarkdownStr("~10 µJ")
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GB, KB
from mlsysim.fmt import fmt, check
