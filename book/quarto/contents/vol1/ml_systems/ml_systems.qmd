...
class MLSystemsSetup:
    ...
    nvlink_h100_bw_str = MarkdownStr(f"{int(NVLINK_H100_BW.m_as(GB/second))} GB/s")
```
...
class LighthouseModels:
    ...
    kws_params_str = MarkdownStr(f"{int(kws_params)}K")
...
from mlsysim.fmt import sci, fmt_percent, fmt, sci_latex, md_frac, MarkdownStr
...
from mlsysim.core.formulas import calc_bottleneck
from mlsysim.fmt import sci_latex, md_frac, fmt_percent, fmt, MarkdownStr
...
class CloudEdgeTCO:
    ...
    requests_str = MarkdownStr(f"{requests_per_day/MILLION:.0f}M")
    inference_str = MarkdownStr(f"{inference_ms}ms")
    response_str = MarkdownStr(f"{response_kb}KB")
    gpu_instances_str = fmt(gpu_instances, precision=0, commas=False)
    gpu_price_str = MarkdownStr(f"${gpu_price_per_hr:.2f}")
    egress_gb_str = fmt(egress_gb_per_day, precision=0, commas=False)

    c_gpu_str = MarkdownStr(f"~${c_gpu:,.0f}")
    c_egress_str = MarkdownStr(f"~${c_egress:,.0f}")
    c_lb_str = MarkdownStr(f"~${c_lb:,.0f}")
    c_logs_str = MarkdownStr(f"~${c_logs:,.0f}")
    c_total_str = MarkdownStr(f"~${c_total:,.0f}/year")

    e_capex_str = MarkdownStr(f"~${e_capex:,.0f}")
    e_power_str = MarkdownStr(f"~${e_power:,.0f}")
    e_cool_str = MarkdownStr(f"~${e_cool:,.0f}")
    e_net_str = MarkdownStr(f"~${e_net:,.0f}")
    e_labor_str = MarkdownStr(f"~${e_labor:,.0f}")
    e_total_str = MarkdownStr(f"~${e_total:,.0f}/year")

    edge_savings_str = MarkdownStr(f"{edge_savings_pct:.0f}%")
    labor_pct_str = MarkdownStr(f"{labor_pct:.0f}%")

    # Additional Outputs for Prose
    server_cost_str = MarkdownStr(f"${server_cost:,}")
    server_life_str = fmt(server_life_years, precision=0, commas=False)
    power_str = MarkdownStr(f"{power_watts}W")
    electricity_str = MarkdownStr(f"${electricity_per_kwh:.2f}")
    devops_fte_str = fmt(devops_fte, precision=0, commas=False)
    devops_salary_str = MarkdownStr(f"${devops_salary:,}")
```
