...
class MobileHardwareSpecs:
    ...
    mobile_npu_tops_values = [
        phone.compute.peak_flops.m_as(TFLOPs/second)
...
class EdgeSizing:
    ...
    coral_peak_tops = coral.compute.peak_flops.m_as(TFLOPs/second)
    coral_sustained_tops = coral_peak_tops * derate
    coral_units_per_store = ceil(required_tflops / coral_sustained_tops)
    ...
    check(coral_cluster_sustained_tops >= required_tflops, "Clustered Coral configuration should clear the requirement.")
    ...
    req_tflops_str = fmt(required_tflops, precision=2, commas=False)
    ...
    jetson_tops_str = fmt(jetson.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)

    nuc_cost_str = fmt(nuc_cost, precision=0, commas=False)
    nuc_power_w_str = fmt(nuc.tdp.m_as(watt), precision=0, commas=False)
    nuc_tops_str = fmt(nuc.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
...
| **Required throughput**  | `{python} EdgeSizing.sustained_gf_str` GFLOP/s$\times$ `{python} EdgeSizing.headroom_str` (headroom) |    ~`{python} EdgeSizing.req_tflops_str` TFLOP/s     |
...
**Decision**: At `{python} EdgeSizing.req_tflops_str` TFLOP/s required, one Coral Dev Board (`{python} EdgeSizing.coral_tops_str` TOPS peak, about `{python} EdgeSizing.coral_sustained_tops_str` TOPS after `{python} EdgeSizing.derate_pct_str` percent derating) is undersized. The low-cost edge choice is a sharded configuration with `{python} EdgeSizing.coral_units_per_store_str` Coral boards per store, providing `{python} EdgeSizing.coral_cluster_sustained_tops_str` derated TOPS and about `{python} EdgeSizing.jetson_vs_coral_cluster_capex_ratio_str`$\times$ lower per-store hardware capex than Jetson. Jetson remains the simpler single-device deployment when integration complexity matters more than hardware cost.
