...
class LighthouseModels:
    ...
    mobile_tdp_range_str = MOBILE_TDP_RANGE_W.replace("-", " to ")
...
class HardwareSpectrumSetup:
    ...
    edge_pwr_w_str = fmt(DGX_POWER.m_as(watt), precision=0, commas=False)           # e.g. "200" W
...
class CloudEdgeTCO:
    ...
    egress_price_str = f"USD {egress_per_gb:.2f}/GB"
    hours_per_year_str = f"{HOURS_PER_YEAR:,}"
    days_per_year_str = f"{DAYS_PER_YEAR}"
    ...
    breakeven_requests_str = f"{breakeven_requests_per_day/1_000:.0f}K"
    breakeven_utilization_str = f"{breakeven_fraction * 100:.0f}%"
...
class MobileEfficiencyRecap:
    ...
    mobile_tdp_range_str = mobile_tdp_range.replace("-", " to ")
```
...
from mlsysim.fmt import md_frac, fmt_percent, fmt, check
...
# │
# │ Imports: mlsysim.Hardware, mlsysim.core.constants, mlsysim.book (fmt, md_frac)
...
class MobilePowerFallacyCalc:
    ...
    low_power_frac = md_frac(f"{phone_battery_str} Wh", f"{low_power_w_value} W", f"**{low_power_hours_str} hours**")
    high_power_frac = md_frac(f"{phone_battery_str} Wh", f"{high_power_w_value} W", f"**{high_power_hours_str} hours**")
```
