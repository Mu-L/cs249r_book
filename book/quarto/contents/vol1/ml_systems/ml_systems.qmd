...
# │
# │ Imports: mlsysim.core.constants (CLOUD/EDGE/MOBILE/TINY latency, MOBILE_TDP)
...
class DeploymentOverviewTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_latency_range_str = CLOUD_LATENCY_RANGE_MS
    edge_latency_range_str = EDGE_LATENCY_RANGE_MS
    mobile_latency_range_str = MOBILE_LATENCY_RANGE_MS
    tiny_latency_range_str = TINY_LATENCY_RANGE_MS
    mobile_tdp_range_str = MOBILE_TDP_RANGE_W
```

| **Paradigm**  | **Where**        |                          **Latency**                           |                         **Power**                         | **Memory** | **Best For**                 |
|:--------------|:-----------------|:--------------------------------------------------------------:|:---------------------------------------------------------:|:----------:|:-----------------------------|
| **Cloud ML**  | Data centers     | `{python} DeploymentOverviewTable.cloud_latency_range_str` ms  |                             MW                            |     TB     | Training, complex inference  |
| **Edge ML**   | Local servers    |  `{python} DeploymentOverviewTable.edge_latency_range_str` ms  |                           100 W                           |     GB     | Real-time inference, privacy |
| **Mobile ML** | Smartphones      | `{python} DeploymentOverviewTable.mobile_latency_range_str` ms | `{python} DeploymentOverviewTable.mobile_tdp_range_str` W |     GB     | Personal AI, offline         |
| **TinyML**    | Microcontrollers |  `{python} DeploymentOverviewTable.tiny_latency_range_str` ms  |                             mW                            |     KB     | Always-on sensing            |
...
# │ How: Read MOBILE_TDP_RANGE_W.
# │
# │ Imports: mlsysim.core.constants (MOBILE_TDP_RANGE_W)
...
class LighthouseModels:
    ...
    llama_range_str = MarkdownStr("7 to 70")
    ...
    mobile_tdp_range_str = MarkdownStr("2 to 5")
...
class ResnetSetup:
    ...
    resnet_fp32_mb_str = fmt(resnet_fp32_bytes_value.m_as(MB), precision=1, commas=False)
    resnet_fp16_mb_str = fmt(resnet_fp16_bytes_value.m_as(MB), precision=1, commas=False)
    resnet_int8_mb_str = fmt(resnet_int8_bytes_value.m_as(MB), precision=1, commas=False)
```
...
class ResnetCloud:
    ...
    cloud_memory_frac = fmt_frac(resnet_fp16_bytes_latex, sci_latex(p.peak_bw_actual.to(byte/second)), f"{cloud_memory_ms_value:.3f}", "ms")
    cloud_ai_frac = fmt_frac(resnet_flops_latex, resnet_fp16_bytes_latex, f"{cloud_ai_value:.0f}", "FLOP/byte")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tflops_str = fmt(p.peak_flops_actual.m_as(TFLOPs/second), precision=0, commas=False)
    a100_bw_tbs_str = fmt(p.peak_bw_actual.m_as(TB/second), precision=2, commas=False)
...
class ResnetMobile:
    ...
    cloud_memory_ms_value = p_cloud.latency_memory.m_as("ms")
    ...
    inference_speed_x_value = mobile_memory_ms_value / cloud_memory_ms_value
    resnet_flops_latex = sci_latex(m.inference_flops.to(flop))
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobile_tops_str = fmt(p.peak_flops_actual.m_as(TOPS), precision=0, commas=False)
    mobile_bw_gbs_str = fmt(p.peak_bw_actual.m_as(GB/second), precision=0, commas=False)
...
# │
# │ Imports: mlsysim.core.constants (defaults.TPU_POD_MEM, defaults.TPU_POD_POWER, defaults.TPU_POD_CHIPS,
...
class HardwareSpectrumSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tpu_chips_str = fmt(defaults.TPU_POD_CHIPS, precision=0)          # e.g. "4,096" chips
    cloud_mem_tb_str = fmt(defaults.TPU_POD_MEM.m_as(TB), precision=0, commas=False)         # e.g. "131" TB
    cloud_pwr_mw_str = fmt(defaults.TPU_POD_POWER.m_as("megawatt"), precision=0, commas=False)  # e.g. "3" MW

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    edge_mem_gb_str = fmt(Hardware.Workstation.DGX_Spark.memory.capacity.m_as(GB), precision=0, commas=False)              # e.g. "128" GB
    edge_stor_tb_str = fmt(Hardware.Workstation.DGX_Spark.storage.capacity.m_as(TB), precision=0, commas=False)         # e.g. "4" TB
    edge_pwr_w_str = fmt(Hardware.Workstation.DGX_Spark.tdp.m_as(watt), precision=0, commas=False)           # e.g. "500" W
    edge_price_min_str = fmt(Hardware.Workstation.DGX_Spark.unit_cost.m_as(USD), precision=0)                  # e.g. "3,000"
    edge_price_max_str = fmt(Hardware.Workstation.DGX_Spark.unit_cost_max.m_as(USD), precision=0)                  # e.g. "5,000"

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tiny_flash_mb_str = fmt(Hardware.Tiny.ESP32_S3.memory.flash_capacity.m_as(MB), precision=0, commas=False)        # e.g. "4" MB
    tiny_pwr_min_str = fmt(Hardware.Tiny.ESP32_S3.tdp_min.m_as(watt), precision=2, commas=False)    # e.g. "0.05" W
    tiny_pwr_max_str = fmt(Hardware.Tiny.ESP32_S3.tdp_max.m_as(watt), precision=1, commas=False)    # e.g. "1.2" W
    tiny_price_str = fmt(10, precision=0, commas=False)           # e.g. "10" USD (ESP32-CAM BOM reference)
...
class RepresentativeSystemsMobileRow:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobile_ram_range_str = MOBILE_RAM_RANGE_GB
...
| **Mobile ML** | Flagship Smartphone |                       Mobile SoC (CPU + GPU + NPU)                      | `{python} RepresentativeSystemsMobileRow.mobile_ram_range_str` GB RAM | `{python} RepresentativeSystemsMobileRow.mobile_storage_range_str` |                             `{python} RepresentativeSystemsMobileRow.mobile_tdp_range_str` W                             |                                                  USD 999+                                                  |
...
class DeploymentThresholdsTable:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_thresh_tflops_str = MarkdownStr("1000")
    cloud_thresh_bw_str = MarkdownStr("100")
    cloud_latency_range_str = CLOUD_LATENCY_RANGE_MS
    edge_thresh_pflops_str = MarkdownStr("1")
    edge_thresh_bw_str = MarkdownStr("270")
    edge_latency_range_str = EDGE_LATENCY_RANGE_MS
    ...
    mobile_tdp_range_str = MOBILE_TDP_RANGE_W
    mobile_latency_range_str = MOBILE_LATENCY_RANGE_MS
    tiny_thresh_tops_str = MarkdownStr("1")
    tiny_thresh_mw_str = MarkdownStr("1")
    tiny_latency_range_str = TINY_LATENCY_RANGE_MS
```

| **Paradigm**  |                               **Compute**                               |                      **Memory bandwidth**                      |                                      **Power**                                       |                           **Latency**                            |
|:--------------|:-----------------------------------------------------------------------:|:--------------------------------------------------------------:|:------------------------------------------------------------------------------------:|:----------------------------------------------------------------:|
| **Cloud ML**  |  >`{python} DeploymentThresholdsTable.cloud_thresh_tflops_str` TFLOP/s  | >`{python} DeploymentThresholdsTable.cloud_thresh_bw_str` GB/s |                                MW class (PUE 1.1–1.3)                                | `{python} DeploymentThresholdsTable.cloud_latency_range_str` ms  |
| **Edge ML**   | ~`{python} DeploymentThresholdsTable.edge_thresh_pflops_str` PFLOP/s AI | >`{python} DeploymentThresholdsTable.edge_thresh_bw_str` GB/s  |                                     100 W class                                      |  `{python} DeploymentThresholdsTable.edge_latency_range_str` ms  |
| **Mobile ML** |      `{python} DeploymentThresholdsTable.mobile_npu_range_str` TOPS     | `{python} DeploymentThresholdsTable.mobile_bw_range_str` GB/s  |             `{python} DeploymentThresholdsTable.mobile_tdp_range_str` W              | `{python} DeploymentThresholdsTable.mobile_latency_range_str` ms |
| **TinyML**    |     <`{python} DeploymentThresholdsTable.tiny_thresh_tops_str` TOPS     |                               —                                | <`{python} DeploymentThresholdsTable.tiny_thresh_mw_str` mW always-on average target |  `{python} DeploymentThresholdsTable.tiny_latency_range_str` ms  |
...
# │      using HOURS_PER_YEAR, defaults.CLOUD_EGRESS_PER_GB, and defaults.CLOUD_ELECTRICITY_PER_KWH.
# │
# │ Imports: mlsysim.core.constants (DAYS_PER_YEAR, HOURS_PER_YEAR, defaults.CLOUD_EGRESS_PER_GB,
# │          defaults.CLOUD_ELECTRICITY_PER_KWH, USD, GB, KB, watt, ureg, MILLION)
...
class CloudEdgeTCO:
    ...
    gpu_instances = 4
    egress_per_gb = defaults.CLOUD_EGRESS_PER_GB.m_as(USD/GB)
    ...
    electricity_per_kwh = defaults.CLOUD_ELECTRICITY_PER_KWH.m_as(USD/ureg.kilowatt_hour)
...
# │          VIDEO_BYTES_PER_PIXEL_RGB, VIDEO_FPS_STANDARD, defaults.CLOUD_EGRESS_PER_GB,
...
class BandwidthBottleneck:
    ...
    monthly_cost = calc_monthly_egress_cost(total_bytes_per_sec, defaults.CLOUD_EGRESS_PER_GB)
    ...
    egress_cost_str = fmt(defaults.CLOUD_EGRESS_PER_GB.m_as(USD/GB), precision=2, commas=False)
...
**Scenario**: A smart retail chain deploying person detection across `{python} EdgeSizingThroughput.stores_str` stores, each with `{python} EdgeSizingThroughput.cameras_per_store_str` cameras at `{python} EdgeSizingThroughput.fps_str` FPS.
...
class EdgeSizingFleetTCO:
    ...
    coral_cluster_tco = calc_fleet_tco(coral_cost, coral.tdp, coral_cluster_quantity, years, defaults.CLOUD_ELECTRICITY_PER_KWH)
    ...
    elec_cost_str = fmt(defaults.CLOUD_ELECTRICITY_PER_KWH.m_as(USD/ureg.kilowatt_hour), precision=2, commas=False)
...
|:-----------------------------------------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------:|:---------------------------------------------------------:|:-------------------------------------------------------------:|:---------------------------------------------------------:|
| **NVIDIA Jetson Orin NX**                                                                |                                                 `{python} EdgeSizingFleetTCO.jetson_tops_str` TOPS                                                 |   `{python} EdgeSizingFleetTCO.jetson_power_range_str` W  |       USD `{python} EdgeSizingFleetTCO.jetson_cost_str`       |     USD `{python} EdgeSizingFleetTCO.jetson_fleet_str`    |
| **Intel NUC + Movidius**                                                                 |                                                  `{python} EdgeSizingFleetTCO.nuc_tops_str` TOPS                                                   |      `{python} EdgeSizingFleetTCO.nuc_power_w_str` W      |         USD `{python} EdgeSizingFleetTCO.nuc_cost_str`        |      USD `{python} EdgeSizingFleetTCO.nuc_fleet_str`      |
| **Google Coral Dev (`{python} EdgeSizingFleetTCO.coral_units_per_store_str` per store)** | `{python} EdgeSizingFleetTCO.coral_cluster_peak_tops_str` TOPS peak<br>`{python} EdgeSizingFleetTCO.coral_cluster_sustained_tops_str` TOPS derated | `{python} EdgeSizingFleetTCO.coral_cluster_power_w_str` W | USD `{python} EdgeSizingFleetTCO.coral_cluster_unit_cost_str` | USD `{python} EdgeSizingFleetTCO.coral_cluster_fleet_str` |
