...
class DeploymentOverviewTable:
    ...
    mobile_tdp_range_str = Platforms.Mobile.tdp_range_w
```
...
class HardwareSpectrumSetup:
    ...
    cloud_mem_tb_str = fmt(Systems.Pods.TPUv4_4096.memory.m_as(TB), precision=0, commas=False)         # e.g. "131" TB
    cloud_pwr_mw_str = fmt(Systems.Pods.TPUv4_4096.power.m_as("megawatt"), precision=0, commas=False)  # e.g. "3" MW
...
class RepresentativeSystemsMobileRow:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobile_ram_range_str = Platforms.Mobile.ram_range
...
3.  **Data center capacity**: A large data center (~`{python} VoiceAssistantWall.vi_gpus_dc_str` GPUs) provides `{python} VoiceAssistantWall.vi_gpu_hours_dc_str` GPU-hours/day.
...
class DataLocalityInvariant:
    ...
    frame_label_str = fmt(width // THOUSAND, precision=0, commas=False, suffix="K")
```
...
- **Data $(D_{\text{vol}})$**: `{python} DataLocalityInvariant.frame_label_str` frame ≈ `{python} DataLocalityInvariant.frame_mb_str` MB.
...
class EdgeSizingFleetTCO:
    ...
    coral_cluster_tco = calc_fleet_tco(coral_cost, coral.tdp, coral_cluster_quantity, years, Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate)
    ...
    elec_cost_str = fmt(Infrastructure.Pricing.Cloud.ElectricityPerKwh.rate.m_as(USD/ureg.kilowatt_hour), precision=2, commas=False)
...
class BatteryTax:
    ...
    phone_day_hours_str = fmt(HOURS_PER_DAY, precision=0, commas=False)
...
2.  **The reality**: A user expects their phone to last `{python} BatteryTax.phone_day_hours_str` hours. Running this single feature continuously for a day would require **`{python} BatteryTax.budget_pct_str` percent** of the phone's daily energy budget.
