...
class MobileHardwareSpecs:
    ...
    mobile_npu_range_str = MarkdownStr("1-10")
```
...
class CloudEdgeTCO:
    ...
    devops_fte_str = fmt(devops_fte, precision=0, commas=False)
...
from mlsysim.fmt import fmt_frac, fmt_percent, fmt
...
# │
# │ Imports: mlsysim.book (fmt, fmt_frac)
...
class MobilePowerFallacyCalc:
    ...
    low_power_frac = fmt_frac(f"{battery_wh_value} Wh", f"{low_power_w_value} W", f"**{low_power_hours_str} hours**")
    high_power_frac = fmt_frac(f"{battery_wh_value} Wh", f"{high_power_w_value} W", f"**{high_power_hours_str} hours**")
```
