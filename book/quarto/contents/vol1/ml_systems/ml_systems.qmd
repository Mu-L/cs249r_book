...
[^fn-iot-data-wall]: **IoT Data Wall**: McKinsey frames IoT's 2030 opportunity around pervasive sensing and actuation across physical settings [@mckinsey2021iot]. At that scale, continuous sensor streams create a data-locality pressure: the aggregate $D_{\text{vol}}$ can overwhelm the practical $\text{BW}$ available for centralized ingestion, making local edge processing an architectural necessity rather than only an optimization. \index{IoT!Data Wall}
