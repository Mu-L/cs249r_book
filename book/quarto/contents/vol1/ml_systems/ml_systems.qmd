...
***The Data Locality Invariant***\index{Data Locality Invariant!definition} states that a workload necessitates local processing whenever the transmission delay $(D_{\text{vol}}/\text{BW}_{\text{network}})$ dominates the remote response time, where $L_{\text{net}}$ is network round-trip latency:
$$\text{Data Locality} \iff \frac{D_{\text{vol}}}{\text{BW}_{\text{network}}} > L_{\text{net}} + \frac{O}{R_{\text{peak},\text{remote}}}$$
...
3.  **Common pitfall**: A frequent misconception is that 5G/6G "solves" locality. While these improve $\text{BW}_{\text{network}}$, they do not reduce $L_{\text{net}}$ below the light barrier, meaning latency-critical tasks remain inherently local.
...
- **Remote response $(L_{\text{net}} + T_{\text{remote}})$**: `{python} DataLocalityInvariant.remote_ms_str` ms (round-trip + remote compute).
