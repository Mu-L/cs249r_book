...
class MLSystemsSetup:
    ...
    # Custom unit suffix (GB/s) → MarkdownStr per recipe Rule 1 escape-hatch
    nvlink_h100_bw_str = fmt(int(NVLINK_H100_BW.m_as(GB/second)), precision=1, allow_zero=True, commas=False, suffix=" GB/s")
```
...
# │
# │ Imports: mlsysim.core.constants, mlsysim.core.formulas, mlsysim.book
...
class LightLatency:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    min_latency_str = fmt(min_latency_ms, precision=1, allow_zero=True, commas=False)
    distance_str = fmt(distance_km.m_as('km'), precision=1, allow_zero=True)
```
...
class ThrottlingScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    fps_start_str = fmt(fps_start, precision=1, allow_zero=True, commas=False)
    fps_throttled_str = fmt(fps_throttled, precision=1, allow_zero=True, commas=False)
    duration_min_str = fmt(duration_min, precision=1, allow_zero=True, commas=False)
```
...
# │ How: Compare historical growth rates for TFLOP/s and memory bandwidth.
# │
...
class MemoryWallTrends:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compute_doubling_months_str = fmt(compute_doubling_months, precision=1, allow_zero=True)
    mem_bw_growth_pct_str = fmt(mem_bw_growth_pct, precision=1, allow_zero=True)
```
...
class EnergyTransmission:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    data_mb_str = fmt(data_size.m_as(MB), precision=1, allow_zero=True, commas=False)
    tx_energy_str = fmt(tx_energy_per_mb.m_as(ureg.millijoule/MB), precision=1, allow_zero=True, commas=False)
    compute_energy_str = fmt(local_energy_op.m_as(ureg.millijoule), precision=1, commas=False)
    cloud_total_str = fmt(cloud_energy_total.m_as(ureg.millijoule), precision=1, allow_zero=True, commas=False)
    local_total_str = fmt(local_energy_total.m_as(ureg.millijoule), precision=1, commas=False)
    ratio_str = fmt(ratio.m_as(""), precision=1, allow_zero=True, commas=True)
```
...
# │
# │ Imports: mlsysim.core.constants (Models.Vision.ResNet50.inference_flops, GFLOPs, Mparam, Bparam, etc.)
...
class LighthouseModels:
    ...
    # MobileNet
    # Step 2: ResNet-50 vs MobileNetV2 FLOP reduction.
    ...
    resnet_fp32_mb_str = fmt(resnet_fp32_mb, precision=1, allow_zero=True)

    gpt2_params_b_str = fmt(gpt2_params_b, precision=1)
    llama_range_str = MarkdownStr("7 to 70") # Llama family range

    dlrm_embedding_str = fmt(dlrm_embedding_gb, precision=1, allow_zero=True)

    mobilenet_flops_reduction_str = fmt(mobilenet_flops_reduction, precision=1, allow_zero=True)
    mobile_tdp_range_str = MarkdownStr("2 to 5") # Standard mobile envelope

    # Custom "K" suffix → canonical fmt() with suffix arg
    kws_params_str = fmt(int(kws_params), precision=1, allow_zero=True, commas=False, suffix="K")
    kws_flops_m_str = fmt(kws_flops_m, precision=1, allow_zero=True)
    kws_size_kb_str = fmt(kws_size_kb, precision=1, allow_zero=True)
```
...
The dominant term varies by paradigm and workload, shifting the optimization strategy entirely; @tbl-ml-systems-paradigm-bottlenecks names which iron-law term limits each paradigm and the resulting optimization focus.
...
# │
# │ Imports: mlsysim (Models), mlsysim.core.constants (GFLOPs, Mparam, byte, MB)
...
class ResnetSetup:
    ...
    resnet_fp32_mb_str = fmt(resnet_fp32_bytes_value.m_as(MB), precision=1, allow_zero=True, commas=False)   # e.g. "102" MB
    resnet_fp16_mb_str = fmt(resnet_fp16_bytes_value.m_as(MB), precision=1, allow_zero=True, commas=False)   # e.g. "51" MB
    resnet_int8_mb_str = fmt(resnet_int8_bytes_value.m_as(MB), precision=1, allow_zero=True, commas=False)   # e.g. "26" MB
...
from mlsysim.fmt import fmt_sci, fmt_percent, fmt, sci_latex, fmt_frac, MarkdownStr
...
class ResnetCloud:
    ...
    a100_tflops_value = p.peak_flops_actual.m_as(TFLOPs/second)
    a100_bw_tbs_value = p.peak_bw_actual.m_as(TB/second)
    cloud_compute_ms_value = p.latency_compute.m_as("ms")
    cloud_memory_ms_value = p.latency_memory.m_as("ms")
    cloud_ratio_x_value = cloud_memory_ms_value / cloud_compute_ms_value
    cloud_ai_value = p.arithmetic_intensity.m_as("flop/byte")
    cloud_bottleneck_value = p.bottleneck

    #—LaTeX fraction components (for nice rendering) ---
    resnet_flops_latex = sci_latex(m.inference_flops.to(flop))
    a100_flops_latex = sci_latex(p.peak_flops_actual.to(flop/second))
    resnet_fp16_bytes_latex = sci_latex(p.memory_footprint.to(byte))
    a100_bw_latex = sci_latex(p.peak_bw_actual.to(byte/second))
    cloud_compute_frac = fmt_frac(resnet_flops_latex, a100_flops_latex, f"{cloud_compute_ms_value:.3f}", "ms")
    cloud_memory_frac = fmt_frac(resnet_fp16_bytes_latex, a100_bw_latex, f"{cloud_memory_ms_value:.3f}", "ms")
    cloud_ai_frac = fmt_frac(resnet_flops_latex, resnet_fp16_bytes_latex, f"{cloud_ai_value:.0f}", "FLOP/byte")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_tflops_str = fmt(a100_tflops_value, precision=1, allow_zero=True, commas=False)        # e.g. "312" TFLOP/s
    a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=1, allow_zero=True, commas=False)        # e.g. "2" TB/s
    cloud_ratio_x_str = fmt(cloud_ratio_x_value, precision=1, allow_zero=True, commas=False)    # memory/compute ratio
    cloud_bottleneck_str = MarkdownStr(cloud_bottleneck_value)                 # "Memory" or "Compute"
...
# │ How: Apply the mlsysim Engine to solve for Mobile NPU roofline bounds.
# │
# │ Imports: mlsysim (Models, Hardware, Engine)
# │ Imports: mlsysim.core.constants (Hardware.Cloud.A100.memory.bandwidth, TOPS, second, GB, byte)
...
from mlsysim import Models, Hardware, Engine
from mlsysim.fmt import sci_latex, fmt_frac, fmt_percent, fmt, MarkdownStr
...
class ResnetMobile:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # Solve for batch=1 inference at INT8 with 100% hardware efficiency
    p = Engine.solve(m, h, batch_size=1, precision="int8", efficiency=1.0)

    mobile_tops_value = p.peak_flops_actual.m_as(TOPS)
    mobile_bw_gbs_value = p.peak_bw_actual.m_as(GB/second)
    mobile_compute_ms_value = p.latency_compute.m_as("ms")
    mobile_memory_ms_value = p.latency_memory.m_as("ms")
    mobile_ratio_x_value = mobile_memory_ms_value / mobile_compute_ms_value
    mobile_bottleneck_value = p.bottleneck

    #—Cross-platform comparison ---
    bw_advantage_x_value = Hardware.Cloud.A100.memory.bandwidth / p.peak_bw_actual
    inference_speed_x_value = mobile_memory_ms_value / ResnetCloud.cloud_memory_ms_value

    #—LaTeX fraction components (for nice rendering) ---
    resnet_int8_ops_latex = f"{ResnetCloud.resnet_flops_latex} \\text{{ INT8 ops}}"
    mobile_npu_ops_latex = f"{sci_latex(mobile_tops_value * 1e12)} \\text{{ INT8 ops/s}}"
    ...
    mobile_compute_frac = fmt_frac(resnet_int8_ops_latex, mobile_npu_ops_latex, f"{mobile_compute_ms_value:.2f}", "ms")
    mobile_memory_frac = fmt_frac(resnet_int8_bytes_latex, mobile_npu_bw_latex, f"{mobile_memory_ms_value:.2f}", "ms")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobile_tops_str = fmt(mobile_tops_value, precision=1, allow_zero=True, commas=False)        # e.g. "35" TOPS
    mobile_bw_gbs_str = fmt(mobile_bw_gbs_value, precision=1, allow_zero=True, commas=False)    # e.g. "100" GB/s
    mobile_ratio_x_str = fmt(mobile_ratio_x_value, precision=1, allow_zero=True, commas=False)  # memory/compute ratio
    mobile_bottleneck_str = MarkdownStr(mobile_bottleneck_value)               # "Memory" or "Compute"
    bw_advantage_x_str = fmt(bw_advantage_x_value, precision=1, allow_zero=True, commas=False)  # A100 vs NPU bandwidth
    inference_speed_x_str = fmt(inference_speed_x_value, precision=1, allow_zero=True, commas=False)  # latency ratio
```
...
- Peak compute: `{python} ResnetCloud.a100_tflops_str` TFLOP/s (FP16)
...
# │ How: Read RAM/storage ranges from centralized constants; derive NPU and
# │      bandwidth ranges from representative phone hardware twins.
# │
...
class MobileHardwareSpecs:
    ...
    mobile_bw_range_str = MarkdownStr(f"{fmt(mobile_bw_min, precision=1, allow_zero=True, commas=False)}-{fmt(mobile_bw_max, precision=1, allow_zero=True, commas=False)}")
    mobile_npu_range_str = MarkdownStr(f"{fmt(mobile_npu_min, precision=1, allow_zero=True, commas=False)}-{fmt(mobile_npu_max, precision=1, allow_zero=True, commas=False)}")
```
...
class HardwareSpectrumSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tpu_chips_str = fmt(TPU_POD_CHIPS, precision=1, allow_zero=True)          # e.g. "4,096" chips
    cloud_mem_tb_str = fmt(TPU_POD_MEM.m_as(TB), precision=1, allow_zero=True, commas=False)         # e.g. "131" TB
    cloud_pwr_mw_str = fmt(TPU_POD_POWER.m_as("megawatt"), precision=1, allow_zero=True, commas=False)  # e.g. "3" MW

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    edge_mem_gb_str = fmt(DGX_RAM.m_as(GB), precision=1, allow_zero=True, commas=False)              # e.g. "128" GB
    edge_stor_tb_str = fmt(DGX_STORAGE.m_as(TB), precision=1, allow_zero=True, commas=False)         # e.g. "4" TB
    edge_pwr_w_str = fmt(DGX_POWER.m_as(watt), precision=1, allow_zero=True, commas=False)           # e.g. "500" W
    edge_price_min_str = fmt(DGX_PRICE_MIN.m_as(USD), precision=1, allow_zero=True)                  # e.g. "3,000"
    edge_price_max_str = fmt(DGX_PRICE_MAX.m_as(USD), precision=1, allow_zero=True)                  # e.g. "5,000"

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tiny_flash_mb_str = fmt(ESP32_FLASH.m_as(MB), precision=1, allow_zero=True, commas=False)        # e.g. "4" MB
    tiny_pwr_min_str = fmt(ESP32_POWER_MIN.m_as(watt), precision=2, commas=False)    # e.g. "0.05" W
    tiny_pwr_max_str = fmt(ESP32_POWER_MAX.m_as(watt), precision=1, commas=False)    # e.g. "1.2" W
    tiny_price_str = fmt(ESP32_PRICE.m_as(USD), precision=1, allow_zero=True, commas=False)           # e.g. "10" USD

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cloud_thresh_tflops_str = MarkdownStr("1000")            # TFLOP/s threshold for cloud
    cloud_thresh_bw_str = MarkdownStr("100")                 # GB/s memory bandwidth
    edge_thresh_pflops_str = MarkdownStr("1")                # PFLOP/s AI compute threshold
    edge_thresh_bw_str = MarkdownStr("270")                  # GB/s memory bandwidth
    tiny_thresh_tops_str = MarkdownStr("1")                  # TOPS compute threshold
    tiny_thresh_mw_str = MarkdownStr("1")                    # mW power threshold
```
...
|:--------------|:-------------------:|:-----------------------------------------------------------------------:|:-----------------------------------------------------------:|:-----------------------------------------------------------:|:------------------------------------------------------------------------------------------------------------------------:|:----------------------------------------------------------------------------------------------------------:|
| **Cloud ML**  |  Google TPU v4 Pod  | `{python} HardwareSpectrumSetup.tpu_chips_str` TPU v4 chips, >1 EFLOP/s |  `{python} HardwareSpectrumSetup.cloud_mem_tb_str` TB HBM2  |                       Cloud-scale (PB)                      |                                  ~`{python} HardwareSpectrumSetup.cloud_pwr_mw_str` MW                                   |                                           Cloud service (rental)                                           |
| **Edge ML**   |   NVIDIA DGX Spark  |                    GB10 Grace Blackwell, 1 PFLOP/s AI                   | `{python} HardwareSpectrumSetup.edge_mem_gb_str` GB LPDDR5x |  `{python} HardwareSpectrumSetup.edge_stor_tb_str` TB NVMe  |                                    ~`{python} HardwareSpectrumSetup.edge_pwr_w_str` W                                    | ~\$`{python} HardwareSpectrumSetup.edge_price_min_str`–`{python} HardwareSpectrumSetup.edge_price_max_str` |
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ DEPLOYMENT THRESHOLDS: LOCAL TABLE RECAP
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-deployment-thresholds immediately below.
# │
# │ Goal: Keep threshold table inline references local to their use site.
# │ Show: Compute, memory, power, and latency ranges for each deployment tier.
# │ How: Recap already-computed deployment and hardware spectrum values.
# │
# │ Imports: mlsysim.book (check)
# │ Exports: DeploymentThresholdsLocal.*_str values used by the table below
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check

# ┌── LEGO ───────────────────────────────────────────────
class DeploymentThresholdsLocal:
    """Local recap for the deployment decision thresholds table."""

    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    cloud_compute = HardwareSpectrumSetup.cloud_thresh_tflops_str
    cloud_bw = HardwareSpectrumSetup.cloud_thresh_bw_str
    cloud_latency = MLSystemsSetup.cloud_latency_range_str

    edge_compute = HardwareSpectrumSetup.edge_thresh_pflops_str
    edge_bw = HardwareSpectrumSetup.edge_thresh_bw_str
    edge_latency = MLSystemsSetup.edge_latency_range_str

    mobile_compute = MobileHardwareSpecs.mobile_npu_range_str
    mobile_bw = MobileHardwareSpecs.mobile_bw_range_str
    mobile_power = LighthouseModels.mobile_tdp_range_str
    mobile_latency = MLSystemsSetup.mobile_latency_range_str

    tiny_compute = HardwareSpectrumSetup.tiny_thresh_tops_str
    tiny_power = HardwareSpectrumSetup.tiny_thresh_mw_str
    tiny_latency = MLSystemsSetup.tiny_latency_range_str

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    table_values = [
        cloud_compute, cloud_bw, cloud_latency,
        edge_compute, edge_bw, edge_latency,
        mobile_compute, mobile_bw, mobile_power, mobile_latency,
        tiny_compute, tiny_power, tiny_latency,
    ]

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(all(table_values), "Deployment threshold recap values should be populated.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_thresh_tflops_str = cloud_compute
    cloud_thresh_bw_str = cloud_bw
    cloud_latency_range_str = cloud_latency

    edge_thresh_pflops_str = edge_compute
    edge_thresh_bw_str = edge_bw
    edge_latency_range_str = edge_latency

    mobile_npu_range_str = mobile_compute
    mobile_bw_range_str = mobile_bw
    mobile_tdp_range_str = mobile_power
    mobile_latency_range_str = mobile_latency

    tiny_thresh_tops_str = tiny_compute
    tiny_thresh_mw_str = tiny_power
    tiny_latency_range_str = tiny_latency
```

| **Paradigm**  |                               **Compute**                               |                      **Memory bandwidth**                      |                                      **Power**                                       |                           **Latency**                            |
|:--------------|:-----------------------------------------------------------------------:|:--------------------------------------------------------------:|:------------------------------------------------------------------------------------:|:----------------------------------------------------------------:|
| **Cloud ML**  |  >`{python} DeploymentThresholdsLocal.cloud_thresh_tflops_str` TFLOP/s  | >`{python} DeploymentThresholdsLocal.cloud_thresh_bw_str` GB/s |                                MW class (PUE 1.1–1.3)                                | `{python} DeploymentThresholdsLocal.cloud_latency_range_str` ms  |
| **Edge ML**   | ~`{python} DeploymentThresholdsLocal.edge_thresh_pflops_str` PFLOP/s AI | >`{python} DeploymentThresholdsLocal.edge_thresh_bw_str` GB/s  |                                     100 W class                                      |  `{python} DeploymentThresholdsLocal.edge_latency_range_str` ms  |
| **Mobile ML** |      `{python} DeploymentThresholdsLocal.mobile_npu_range_str` TOPS     | `{python} DeploymentThresholdsLocal.mobile_bw_range_str` GB/s  |             `{python} DeploymentThresholdsLocal.mobile_tdp_range_str` W              | `{python} DeploymentThresholdsLocal.mobile_latency_range_str` ms |
| **TinyML**    |     <`{python} DeploymentThresholdsLocal.tiny_thresh_tops_str` TOPS     |                               —                                | <`{python} DeploymentThresholdsLocal.tiny_thresh_mw_str` mW always-on average target |  `{python} DeploymentThresholdsLocal.tiny_latency_range_str` ms  |
...
class GPT3TrainingScale:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt3_pflop_days_str = fmt(gpt3_pflop_days, precision=1, allow_zero=True, commas=True)
    gpt3_v100_count_str = fmt(gpt3_v100_count, precision=1, allow_zero=True, commas=True)
    gpt3_days_str = fmt(gpt3_days, precision=1, allow_zero=True)
...
Consider what it took to train GPT-3: `{python} GPT3TrainingScale.gpt3_pflop_days_str` PFLOP-days of computation, `{python} GPT3TrainingScale.gpt3_v100_count_str` GPUs running for approximately `{python} GPT3TrainingScale.gpt3_days_str` days, consuming megawatts of power—at an estimated cost of ~\$`{python} GPT3TrainingScale.gpt3_cost_m_str`M[^fn-nlp-training-scale]. No smartphone, no edge server, no single machine on Earth could have performed this computation. Only a data center, with its virtually unlimited compute, memory, and storage, could aggregate enough resources to make this possible. This is the defining proposition of cloud ML: when latency can be tolerated, it offers computational scale that no other paradigm can match.
...
1.  **Significance (quantitative)**: It enables systems to scale peak compute rate $(R_{\text{peak}})$ proportional to workload variance, allowing for PFLOP/s bursts that would be economically unfeasible to maintain locally.
...
class DistancePenalty:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sol_kms_str = fmt(SPEED_OF_LIGHT_FIBER_KM_S.m_as('km/s'), precision=1, allow_zero=True)   # e.g. "200,000" km/s
    rtt_formatted_str = fmt(round_trip_ms_value, precision=1, allow_zero=True, commas=False)  # e.g. "15" ms
    deficit_str = fmt(deficit_ms_value, precision=1, allow_zero=True, commas=False)           # e.g. "-5" ms
    distance_km_str = fmt(distance_km_value, precision=1, allow_zero=True)                    # e.g. "1,500" km
    safety_budget_str = fmt(safety_budget_ms_value, precision=1, allow_zero=True, commas=False)  # "10" ms
```
...
class CloudEdgeTCO:
    ...
    requests_str = fmt(requests_per_day/MILLION, precision=1, allow_zero=True, commas=False, suffix="M")
    inference_str = fmt(inference_ms, precision=1, allow_zero=True, commas=False, suffix=" ms")
    response_str = fmt(response_kb, precision=1, allow_zero=True, commas=False, suffix=" KB")
    gpu_instances_str = fmt(gpu_instances, precision=1, allow_zero=True, commas=False)
    gpu_price_str = fmt(gpu_price_per_hr, precision=2, commas=False, prefix="$")
    egress_gb_str = fmt(egress_gb_per_day, precision=1, allow_zero=True, commas=False)
    egress_price_str = MarkdownStr(f"USD {egress_per_gb:.2f}/GB")
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=1, allow_zero=True)
    days_per_year_str = fmt(DAYS_PER_YEAR, precision=1, allow_zero=True, commas=False)

    c_gpu_str = fmt(c_gpu, precision=1, allow_zero=True, prefix="~$")
    c_egress_str = fmt(c_egress, precision=1, allow_zero=True, prefix="~$")
    c_lb_str = fmt(c_lb, precision=1, allow_zero=True, prefix="~$")
    c_logs_str = fmt(c_logs, precision=1, allow_zero=True, prefix="~$")
    c_total_str = fmt(c_total, precision=1, allow_zero=True, prefix="~$", suffix="/year")

    e_capex_str = fmt(e_capex, precision=1, allow_zero=True, prefix="~$")
    e_power_str = fmt(e_power, precision=1, allow_zero=True, prefix="~$")
    e_cool_str = fmt(e_cool, precision=1, allow_zero=True, prefix="~$")
    e_net_str = fmt(e_net, precision=1, allow_zero=True, prefix="~$")
    e_labor_str = fmt(e_labor, precision=1, allow_zero=True, prefix="~$")
    e_total_str = fmt(e_total, precision=1, allow_zero=True, prefix="~$", suffix="/year")

    edge_savings_str = fmt(edge_savings_pct, precision=1, allow_zero=True, commas=False, suffix="%")
    labor_pct_str = fmt(labor_pct, precision=1, allow_zero=True, commas=False, suffix="%")
    breakeven_requests_str = fmt(breakeven_requests_per_day/1_000, precision=1, allow_zero=True, commas=False, suffix="K")
    breakeven_utilization_str = fmt(breakeven_fraction * 100, precision=2, commas=False, suffix="%", allow_zero=True)

    # Additional Outputs for Prose
    server_cost_str = fmt(server_cost, precision=1, allow_zero=True, prefix="$")
    server_life_str = fmt(server_life_years, precision=1, allow_zero=True, commas=False)
    power_str = fmt(power_watts, precision=1, allow_zero=True, commas=False, suffix=" W")
    electricity_str = fmt(electricity_per_kwh, precision=2, commas=False, prefix="$")
    devops_fte_str = fmt(devops_fte, precision=1, commas=False)
    devops_salary_str = fmt(devops_salary, precision=1, allow_zero=True, prefix="$")
```
...
|:-------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------:|
| **GPU inference (A10G)** | `{python} CloudEdgeTCO.gpu_instances_str` instances $\times$ `{python} CloudEdgeTCO.hours_per_year_str` hrs $\times$ `{python} CloudEdgeTCO.gpu_price_str`/hr |       `{python} CloudEdgeTCO.c_gpu_str` |
| **Network egress**       | `{python} CloudEdgeTCO.egress_gb_str` GB/day $\times$ `{python} CloudEdgeTCO.days_per_year_str` $\times$ `{python} CloudEdgeTCO.egress_price_str`             |    `{python} CloudEdgeTCO.c_egress_str` |
...
| **Power (24/7)**     | `{python} CloudEdgeTCO.power_str` $\times$ `{python} CloudEdgeTCO.hours_per_year_str` hrs $\times$ `{python} CloudEdgeTCO.electricity_str`/kWh |     `{python} CloudEdgeTCO.e_power_str` |
...
class VoiceAssistantWall:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Economics Strings
    ww_devices_b_str = fmt(ww_devices_b, precision=1, allow_zero=True, commas=False)
    ww_cloud_cost_str = fmt(ww_cloud_cost_per_device, precision=2, commas=False)
    ww_total_cost_str = fmt(ww_total_cloud_cost, precision=1, allow_zero=True, commas=True)
    # Multi-value range → MarkdownStr per recipe Rule 1 escape-hatch
    ww_edge_power_range_str = MarkdownStr(f"{ww_edge_power_min_mw}--{ww_edge_power_max_mw}")
    ww_edge_cost_str = fmt(ww_edge_cost_per_year, precision=2, commas=False)

    # Infrastructure Strings
    vi_devices_str = fmt(vi_devices_b, precision=1, allow_zero=True, commas=False)
    vi_queries_str = fmt(vi_queries_per_day, precision=1, allow_zero=True, commas=False)
    vi_total_queries_str = fmt(vi_total_queries_day/BILLION, precision=1, allow_zero=True, commas=False)
    vi_gpu_ms_str = fmt(vi_gpu_ms_per_query, precision=1, allow_zero=True, commas=False)
    vi_gpu_hours_str = fmt(vi_gpu_hours_day, precision=1, allow_zero=True, commas=True)
    vi_gpus_dc_str = fmt(vi_gpus_per_datacenter, precision=1, allow_zero=True, commas=True)
    vi_dc_avg_str = fmt(vi_datacenters_avg, precision=1, allow_zero=True, commas=False)
    vi_dc_peak_str = fmt(vi_datacenters_peak, precision=1, allow_zero=True, commas=False)
    vi_peak_ratio_str = fmt(vi_peak_ratio, precision=1, commas=False)
    vi_audio_kb_str = fmt(vi_audio_kb_per_sec, precision=1, allow_zero=True, commas=False)
    vi_audio_tb_str = fmt(vi_total_audio_tb_per_sec, precision=1, allow_zero=True, commas=False)
```
...
# │          MB, GB, KB, second, MILLION, USD), mlsysim.formulas (calc_monthly_egress_cost)
...
class BandwidthBottleneck:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cam_rate_mbs_str = fmt(bytes_per_sec_single.m_as(MB/second), precision=1, allow_zero=True, commas=False)
    ...
    bw_short_x_str = fmt(shortfall_ratio, precision=1, allow_zero=True, commas=False)
    metadata_events_per_sec_str = fmt(metadata_events_per_sec, precision=1, allow_zero=True, commas=False)
    metadata_kb_per_event_str = fmt(metadata_kb_per_event.m_as(KB), precision=1, allow_zero=True, commas=False)
    metadata_reduction_x_str = fmt(metadata_reduction, precision=1, allow_zero=True, commas=True)

    num_cameras_str = fmt(num_cameras, precision=1, allow_zero=True, commas=False)
    bb_fps_str = fmt(int(fps.m_as('Hz')), precision=1, allow_zero=True, commas=False)
    egress_cost_str = fmt(CLOUD_EGRESS_PER_GB.m_as(USD/GB), precision=2, commas=False)
    video_width_str = fmt(width, precision=1, allow_zero=True, commas=False)
    video_height_str = fmt(height, precision=1, allow_zero=True, commas=False)
    bytes_per_pixel_str = fmt(bpp, precision=1, allow_zero=True, commas=False)
...
class DataLocalityInvariant:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(must_be_local, "4K video should require locality at 100 Mbps!")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frame_mb_str = fmt(frame_mb, precision=1, allow_zero=True)
    tx_time_ms_str = fmt(tx_time_ms, precision=1, allow_zero=True)
    remote_ms_str = fmt(remote_total_ms, precision=1, allow_zero=True)
    net_bw_str = fmt(net_bw_mbps, precision=1, allow_zero=True, commas=False)
```
...
***The Data Locality Invariant***\index{Data Locality Invariant!definition} states that a workload necessitates local processing whenever the transmission delay $(D_{\text{vol}}/\text{BW}_{\text{network}})$ dominates the remote response time, where $L_{\text{lat,network}}$ is network round-trip latency:
$$\text{Data Locality} \iff \frac{D_{\text{vol}}}{\text{BW}_{\text{network}}} > L_{\text{lat,network}} + \frac{O}{R_{\text{peak,remote}} \cdot \eta_{\text{hw,remote}}}$$

1.  **Significance (quantitative)**: It defines the **Locality Crossover**, the point where adding cloud compute (increasing $R_{\text{peak}}$) yields zero benefit because the "Pipe" $(\text{BW}_{\text{network}})$ is too narrow for the "Volume" $(D_{\text{vol}})$.
...
3.  **Common pitfall**: A frequent misconception is that 5G/6G "solves" locality. While these improve $\text{BW}_{\text{network}}$, they do not reduce $L_{\text{lat,network}}$ below the light barrier, meaning latency-critical tasks remain inherently local.
...
::: {#nbk-ml-systems-napkin-math-locality-crossover .callout-notebook title="Napkin math: The locality crossover"}
...
- **Bandwidth $(\text{BW}_{\text{network}})$**: `{python} DataLocalityInvariant.net_bw_str` Mbps home broadband (up).
- **Remote response $(L_{\text{lat,network}} + T_{\text{remote}})$**: `{python} DataLocalityInvariant.remote_ms_str` ms (round-trip + remote compute).
...
# │ How: Compute sustained GFLOP/s requirement from inference rate × model FLOPs;
# │      apply calc_fleet_tco() with hardware TDP and CLOUD_ELECTRICITY_PER_KWH.
# │
# │ Imports: mlsysim.core.constants (GFLOPs, TOPS, CLOUD_ELECTRICITY_PER_KWH,
...
# │          EdgeSizing.req_tops_equiv_str, EdgeSizing.coral_tops_str,
...
# │          EdgeSizing.years_str, EdgeSizing.coral_units_per_store_str,
# │          EdgeSizing.coral_cluster_*_str, EdgeSizing.derate_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class EdgeSizing:
    ...
    coral_cluster_tco = calc_fleet_tco(coral_cost, coral.tdp, coral_cluster_quantity, years, CLOUD_ELECTRICITY_PER_KWH)
    coral_cluster_power_opex = coral_cluster_tco - coral_cluster_capex
    ...
    check(coral_cluster_tco < jetson_tco, "Clustered Coral option should remain cheaper than Jetson in this reference scenario.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    stores_str = fmt(stores, precision=1, allow_zero=True, commas=False)
    cameras_per_store_str = fmt(cameras_per_store, precision=1, allow_zero=True, commas=False)
    fps_str = fmt(fps, precision=1, allow_zero=True, commas=False)
    headroom_str = fmt(headroom, precision=1, allow_zero=True, commas=False)
    inf_per_sec_str = fmt(inf_per_sec, precision=1, allow_zero=True, commas=False)

    yolo_gflops_str = fmt(yolo_flops.m_as(GFLOPs), precision=1)
    sustained_gf_str = fmt(sustained_gflops, precision=1, allow_zero=True)
    req_tops_equiv_str = fmt(required_tops_equiv, precision=2, commas=False)
    derate_pct_str = fmt_percent(derate, precision=1, commas=False)

    coral_power_w_str = fmt(coral.tdp.m_as(watt), precision=1, allow_zero=True, commas=False)
    coral_power_kw_str = fmt(coral.tdp.m_as(kilowatt), precision=3, commas=False)
    coral_tops_str = fmt(coral_peak_tops, precision=1, allow_zero=True, commas=False)
    coral_sustained_tops_str = fmt(coral_sustained_tops, precision=1, allow_zero=True, commas=False)
    coral_units_per_store_str = fmt(coral_units_per_store, precision=1, allow_zero=True, commas=False)
    coral_cluster_peak_tops_str = fmt(coral_cluster_peak_tops, precision=1, allow_zero=True, commas=False)
    coral_cluster_sustained_tops_str = fmt(coral_cluster_sustained_tops, precision=1, allow_zero=True, commas=False)
    coral_cluster_unit_cost_str = fmt(coral_cost * coral_units_per_store, precision=1, allow_zero=True, commas=False)
    coral_cluster_power_w_str = fmt(coral.tdp.m_as(watt) * coral_units_per_store, precision=1, allow_zero=True, commas=False)
    coral_cluster_power_kw_str = fmt(coral.tdp.m_as(kilowatt) * coral_units_per_store, precision=3, commas=False)

    jetson_cost_str = fmt(jetson_cost, precision=1, allow_zero=True, commas=False)
    jetson_power_range_str = MarkdownStr("10-40")
    jetson_tops_str = fmt(jetson.compute.precision_flops["int8"].m_as(TOPS), precision=1, allow_zero=True, commas=False)

    nuc_cost_str = fmt(nuc_cost, precision=1, allow_zero=True, commas=False)
    nuc_power_w_str = fmt(nuc.tdp.m_as(watt), precision=1, allow_zero=True, commas=False)
    nuc_tops_str = fmt(nuc.compute.peak_flops.m_as(TOPS), precision=1, allow_zero=True, commas=False)

    coral_tco_str = fmt(coral_tco, precision=1, allow_zero=True, commas=True)
    jetson_tco_str = fmt(jetson_tco, precision=1, allow_zero=True, commas=True)
    nuc_tco_str = fmt(nuc_tco, precision=1, allow_zero=True, commas=True)

    # Additional Outputs for Prose
    jetson_fleet_str = fmt(jetson_cost * stores, precision=1, allow_zero=True, commas=True)
    nuc_fleet_str = fmt(nuc_cost * stores, precision=1, allow_zero=True, commas=True)
    coral_cluster_fleet_str = fmt(coral_cluster_capex, precision=1, allow_zero=True, commas=True)
    coral_cluster_tco_str = fmt(coral_cluster_tco, precision=1, allow_zero=True, commas=True)
    coral_cluster_power_opex_str = fmt(coral_cluster_power_opex, precision=1, allow_zero=True, commas=True)

    years_str = fmt(years, precision=1, allow_zero=True, commas=False)
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=1, allow_zero=True, commas=False)

    power_ratio_str = fmt(jetson.tdp.m_as(watt) / coral.tdp.m_as(watt), precision=1, allow_zero=True, commas=False)
    jetson_vs_coral_cluster_capex_ratio_str = fmt(jetson_cost / (coral_cost * coral_units_per_store), precision=1, commas=False)
    elec_cost_str = fmt(CLOUD_ELECTRICITY_PER_KWH.m_as(USD/ureg.kilowatt_hour), precision=2, commas=False)
    ...
    cloud_cost_str = fmt(cloud_tco_3yr, precision=1, allow_zero=True, commas=True)
...
| **Model compute**        |                 YOLOv8-nano: `{python} EdgeSizing.yolo_gflops_str` GFLOPs/inference                  |       `{python} EdgeSizing.sustained_gf_str` GFLOP/s      |
| **Required throughput**  | `{python} EdgeSizing.sustained_gf_str` GFLOP/s$\times$ `{python} EdgeSizing.headroom_str` (headroom) | ~`{python} EdgeSizing.req_tops_equiv_str` TOPS equivalent |
...
| **Google Coral Dev (`{python} EdgeSizing.coral_units_per_store_str` per store)** | `{python} EdgeSizing.coral_cluster_peak_tops_str` TOPS peak<br>`{python} EdgeSizing.coral_cluster_sustained_tops_str` TOPS derated | `{python} EdgeSizing.coral_cluster_power_w_str` W | USD `{python} EdgeSizing.coral_cluster_unit_cost_str` | USD `{python} EdgeSizing.coral_cluster_fleet_str` |

: **Edge Accelerator Options**: Candidate edge platforms scored against the throughput target from the requirements table. A single Coral Dev Board is undersized after realistic derating, but a small sharded Coral configuration clears the target with lower hardware cost than the Jetson alternative; Jetson remains the simpler single-device option. {#tbl-ml-systems-edge-hardware-options}

:::

**Decision**: At `{python} EdgeSizing.req_tops_equiv_str` TOPS-equivalent required, one Coral Dev Board (`{python} EdgeSizing.coral_tops_str` TOPS peak, about `{python} EdgeSizing.coral_sustained_tops_str` TOPS after `{python} EdgeSizing.derate_pct_str` percent derating) is undersized. The low-cost edge choice is a sharded configuration with `{python} EdgeSizing.coral_units_per_store_str` Coral boards per store, providing `{python} EdgeSizing.coral_cluster_sustained_tops_str` derated TOPS and about `{python} EdgeSizing.jetson_vs_coral_cluster_capex_ratio_str`$\times$ lower per-store hardware capex than Jetson. Jetson remains the simpler single-device deployment when integration complexity matters more than hardware cost.

**TCO over `{python} EdgeSizing.years_str` years** (`{python} EdgeSizing.coral_units_per_store_str` Coral boards/store): Hardware USD `{python} EdgeSizing.coral_cluster_fleet_str` + Power (`{python} EdgeSizing.coral_cluster_power_kw_str` kW $\times$ `{python} EdgeSizing.stores_str` $\times$ `{python} EdgeSizing.hours_per_year_str` h$\times$ `{python} EdgeSizing.years_str` yr$\times$ USD `{python} EdgeSizing.elec_cost_str`/kWh = USD `{python} EdgeSizing.coral_cluster_power_opex_str`) = **USD `{python} EdgeSizing.coral_cluster_tco_str` total** vs. cloud inference at ~USD `{python} EdgeSizing.cloud_cost_str`.
...
class MobileEfficiencyRecap:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    m_mobilenet = Models.Vision.MobileNetV2
    mobile_tdp_range = MOBILE_TDP_RANGE_W
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobilenet_flops_reduction_str = fmt(mobilenet_flops_reduction, precision=1, allow_zero=True, commas=False)
    mobile_tdp_range_str = MarkdownStr(mobile_tdp_range.replace("-", " to "))
```

\index{Depthwise Separable Convolution!power reduction}
The mobile environment introduces a critical constraint absent from stationary deployments: energy per inference becomes a first-order design parameter. Under the iron law in @eq-iron-law-extended, cloud and edge systems optimize for minimizing $T$, total latency. Mobile systems face an additional constraint: $\text{Energy} = \text{Power} \times T$, and the power wall described by @eq-power-scaling caps sustained power at `{python} MobileEfficiencyRecap.mobile_tdp_range_str` W. In archetype terms, a compute beast workload like image classification must be transformed through architectural efficiency (for example, depthwise separable convolutions[^fn-depthwise-mobile-efficiency] in MobileNet) to become a compute beast (efficient), reducing FLOPs by `{python} MobileEfficiencyRecap.mobilenet_flops_reduction_str`$\times$ while preserving accuracy. This is not merely optimization; it represents a qualitative shift in the arithmetic intensity trade-off, accepting lower peak throughput in exchange for sustainable operation within a `{python} MobileEfficiencyRecap.mobile_tdp_range_str` W thermal envelope.

[^fn-computational-photo-ml]: **Computational Photography**: Uses ML algorithms (for example, multi-frame fusion, neural denoising) to overcome the physical limits of small mobile camera sensors. This exemplifies the mobile computing trade-off, as a pipeline of 10–15 models must execute within the user's perceived shutter delay (~200 ms) while adhering to a strict, shared `{python} MobileEfficiencyRecap.mobile_tdp_range_str` W thermal budget. \index{Computational Photography!pipeline constraint}

[^fn-depthwise-mobile-efficiency]: **Depthwise Separable Convolutions**: An architectural factorization that splits a standard convolution into a depthwise spatial filter and a pointwise channel mixer, reducing FLOPs by 8--9$\times$ for typical layer configurations. This reduction is not merely an efficiency improvement but a prerequisite for real-time vision on mobile devices, where the power wall caps sustained computation at `{python} MobileEfficiencyRecap.mobile_tdp_range_str` W. \index{Depthwise Separable Convolution!mobile power constraint}
...
class BatteryTax:
    ...
    pwr_w_str = fmt(power_draw.m_as(ureg.watt), precision=1, allow_zero=True, commas=False)
    batt_wh_str = fmt(battery_wh.m_as(ureg.Wh), precision=1, allow_zero=True, commas=False)
    budget_pct_str = fmt(daily_budget_pct.m_as(''), precision=1, allow_zero=True, commas=False)

    runtime_frac = fmt_frac(f"{batt_wh_str} \\,\\mathrm{{Wh}}", f"{pwr_w_str} \\,\\mathrm{{W}}", f"**{runtime_str} hours**")
```

::: {#nbk-ml-systems-battery-tax .callout-notebook title="The battery tax"}

**Problem**\index{Battery Tax!always-on constraints}\index{Battery Life!ML impact}\index{Mobile ML!energy budget}: Consider deploying a "real-time" background object detector on a smartphone. The model consumes **`{python} BatteryTax.pwr_w_str` W** of continuous power when active. The phone has a standard **`{python} BatteryTax.batt_wh_str` Wh** battery. Can the feature stay on all day?

**Physics**:

1.  **Ideal runtime**: `{python} BatteryTax.runtime_frac`
2.  **The reality**: A user expects their phone to last 24 hours. Running this single feature continuously for a day would require **`{python} BatteryTax.budget_pct_str` percent** of the phone's daily energy budget.
...
class ThermalQuantCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    baseline_str = fmt(baseline_power_w_value, precision=1, allow_zero=True, commas=False)            # e.g. "12" W
    mobile_tdp_str = fmt(mobile_tdp_w_value, precision=1, allow_zero=True, commas=False)              # e.g. "3" W
    quant_power_str = fmt(quant_power_w_value, precision=1, allow_zero=True, commas=False)            # e.g. "3" W
    quant_red_str = fmt(quant_reduction_value, precision=1, allow_zero=True, commas=False)            # e.g. "4" ×
    temp_rise_str = fmt(temp_rise_c_per_s_value, precision=1, allow_zero=True, commas=False)
    trip_time_str = fmt(trip_time_s_value, precision=1, allow_zero=True, commas=False)
    trip_temp_str = fmt(trip_temp_c_value, precision=1, allow_zero=True, commas=False)
    fps_before_str = fmt(fps_before_value, precision=1, allow_zero=True, commas=False)
    fps_after_str = fmt(fps_after_value, precision=1, allow_zero=True, commas=False)
```
...
1. **Thermal design power (TDP)**: A mobile system on chip (SoC) allows approximately **`{python} ThermalQuantCalc.mobile_tdp_str` W** for passive cooling.
2. **Temperature rise**: At `{python} ThermalQuantCalc.baseline_str` W, the device temperature rises at approximately `{python} ThermalQuantCalc.temp_rise_str` $^\circ\text{C}$ per second.
3. **Thermal trip**: Within `{python} ThermalQuantCalc.trip_time_str` seconds, the hardware reaches the **Thermal Trip Point**\index{Thermal Trip Point!hardware safety} (`{python} ThermalQuantCalc.trip_temp_str` $^\circ\text{C}$), triggering OS throttling.
4. **Result**: The `{python} ThermalQuantCalc.fps_before_str` FPS model suddenly drops to **`{python} ThermalQuantCalc.fps_after_str` FPS** to stay within the thermal envelope.
...
class MobileBatteryCapacity:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    phone_battery_str = fmt(phone_battery_wh, precision=1, allow_zero=True)
    added_ml_runtime_low_str = fmt(added_ml_runtime_low_h, precision=1, allow_zero=True, commas=False)
    added_ml_runtime_high_str = fmt(added_ml_runtime_high_h, precision=1, allow_zero=True, commas=False)
```
...
class MobileResourceRecap:
    ...
    mobile_ram_range_str = mobile_ram_range
    mobile_storage_range_str = mobile_storage_range
    mobile_npu_range_str = f"{fmt(mobile_npu_min, precision=1, allow_zero=True, commas=False)}-{fmt(mobile_npu_max, precision=1, allow_zero=True, commas=False)}"
    mobile_bw_range_str = f"{fmt(mobile_bw_min, precision=1, allow_zero=True, commas=False)}-{fmt(mobile_bw_max, precision=1, allow_zero=True, commas=False)}"
    mobile_tdp_range_str = MarkdownStr(mobile_tdp_range.replace("-", " to "))
    mobile_latency_range_str = mobile_latency_range
```
...
Mobile devices exemplify intermediate constraints: `{python} MobileResourceRecap.mobile_ram_range_str` GB RAM (varying from mid-range to flagship), `{python} MobileResourceRecap.mobile_storage_range_str` storage, `{python} MobileResourceRecap.mobile_npu_range_str` TOPS AI compute through Neural Processing Units[^fn-npu-energy-efficiency] consuming `{python} MobileResourceRecap.mobile_tdp_range_str` W power. System-on-Chip architectures[^fn-soc-integration] integrate computation and memory to minimize energy costs. Memory bandwidth of `{python} MobileResourceRecap.mobile_bw_range_str` GB/s limits models to 10–100 MB parameters, requiring the aggressive optimization techniques that @sec-model-compression details. Battery constraints (`{python} MobileBatteryCapacity.phone_battery_str`–22 Wh capacity) make energy optimization critical: adding 1 W of continuous ML processing to a phone that otherwise lasts 24 hours would reduce runtime to roughly `{python} MobileBatteryCapacity.added_ml_runtime_low_str`–`{python} MobileBatteryCapacity.added_ml_runtime_high_str` hours, depending on battery capacity. Specialized frameworks provide hardware-optimized inference enabling <`{python} MobileResourceRecap.mobile_latency_range_str` ms UI response times.
...
class EnergyInference:
    ...
    p_kws = Engine.solve(Models.Tiny.DS_CNN, Hardware.Tiny.ESP32, batch_size=1)
    ...
    e_resnet_cloud_str = fmt(e_resnet_cloud_j.m_as("J"), precision=1, suffix=" J")
    e_resnet_edge_str = fmt(e_resnet_edge_j.m_as("mJ"), precision=1, allow_zero=True, suffix=" mJ")
    e_mobilenet_str = fmt(e_mobilenet_j.m_as("mJ"), precision=1, suffix=" mJ")
    e_kws_str = fmt(e_kws_j.m_as("mJ"), precision=1, suffix=" mJ")

    q_gpt4_str = fmt(q_gpt4.m_as(''), precision=1, allow_zero=True, commas=True)
    q_resnet_cloud_str = fmt(q_resnet_cloud.m_as(''), precision=1, allow_zero=True, commas=True)
    q_resnet_edge_str = fmt(q_resnet_edge.m_as(''), precision=1, allow_zero=True, commas=True)
    q_mobilenet_str = fmt(q_mobilenet.m_as(''), precision=1, allow_zero=True, commas=True)
    q_kws_str = fmt(q_kws.m_as(''), precision=1, allow_zero=True, commas=True)
    # Use BILLION constant
    q_kws_str = fmt(q_kws.m_as('') / BILLION, precision=1, allow_zero=True, commas=False) + " billion"

    batt_cap_mah_str = fmt(BATTERY_CAPACITY_MAH.m_as('mAh'), precision=1, allow_zero=True, commas=False)
    batt_volt_str = fmt(BATTERY_VOLTAGE_V.m_as('V'), precision=1, commas=False)
```
...
class WildlifeMonitoring:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    raw_audio_gb_str = fmt(raw_audio_per_day.m_as(GB), precision=1, commas=False)
    summary_kb_str = fmt(summary_per_day.m_as(KB), precision=1, allow_zero=True, commas=False)
    reduction_x_str = fmt(reduction, precision=1, allow_zero=True, commas=True)
```

\index{Wildlife Conservation!TinyML monitoring}
Wildlife conservation uses TinyML for remote environmental monitoring. Researchers deploy solar-powered audio sensors consuming 100–500 mW that process continuous audio streams for species identification. By performing local analysis, these systems reduce satellite transmission requirements from `{python} WildlifeMonitoring.raw_audio_gb_str` GB per day to `{python} WildlifeMonitoring.summary_kb_str` KB of detection summaries, a `{python} WildlifeMonitoring.reduction_x_str`$\times$ reduction that makes large-scale deployments of 100–1,000 sensors economically feasible.
...
class ParadigmsTable:
    ...
    #—Cost structure ---
    cloud_cost_str = MarkdownStr("High ($1000s+/month)")                             # usage-based cloud
    edge_cost_str = MarkdownStr("Moderate ($100s–1000s)")                            # hardware capex
    mobile_cost_str = MarkdownStr("Low ($0–10s)")                                    # app distribution
    tiny_cost_str = MarkdownStr("Very Low ($1-10s)")                                 # MCU unit cost
```
...
class BrakingDistance:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    speed_str = fmt(speed_kmh, precision=1, allow_zero=True, commas=False)
    distance_str = fmt(distance_m, precision=1, commas=False)
    latency_str = fmt(latency_ms, precision=1, allow_zero=True, commas=False)
```
...
***Hybrid Machine Learning***\index{Hybrid Machine Learning|see{Hybrid ML}}\index{Hybrid ML!definition} is the architectural strategy of **Hierarchical Distribution** across cloud and edge resources.
...
::: {#psp-ml-systems-pattern-selection-guide .callout-perspective title="Pattern selection guide"}
...
class MobilePowerFallacyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    phone_battery_str = fmt(battery_wh_value, precision=1, allow_zero=True, commas=False)
    low_power_w_str = fmt(low_power_w_value, precision=1, allow_zero=True, commas=False)             # "1"
    high_power_w_str = fmt(high_power_w_value, precision=1, allow_zero=True, commas=False)           # "5"
    low_power_hours_str = fmt(low_power_hours_value, precision=1, allow_zero=True, commas=False)
    high_power_hours_str = fmt(high_power_hours_value, precision=1, allow_zero=True, commas=False)
    mobile_tdp_str = fmt(mobile_tdp_w_value, precision=1, allow_zero=True, commas=False)
    quant_red_str = fmt(quant_reduction_value, precision=1, allow_zero=True, commas=False)

    # Inline fractions showing the physics.
    low_power_frac = fmt_frac(f"{phone_battery_str} Wh", f"{low_power_w_value} W", f"**{low_power_hours_str} hours**")
    high_power_frac = fmt_frac(f"{phone_battery_str} Wh", f"{high_power_w_value} W", f"**{high_power_hours_str} hours**")
```

**Fallacy**: *Model optimization overcomes mobile device power and thermal limits.*

Compression techniques do not scale indefinitely against physics. Consider a smartphone with a `{python} MobilePowerFallacyCalc.phone_battery_str` Wh battery:

- **Light workload** (`{python} MobilePowerFallacyCalc.low_power_w_str` W inference): `{python} MobilePowerFallacyCalc.low_power_frac`
- **Heavy workload** (`{python} MobilePowerFallacyCalc.high_power_w_str` W, common for large on-device models): `{python} MobilePowerFallacyCalc.high_power_frac`

The `{python} MobilePowerFallacyCalc.high_power_w_str` W workload also triggers thermal throttling\index{Thermal Throttling!performance limits} that substantially reduces performance. As @sec-ml-systems-mobile-ml-benefits-resource-constraints-c568 establishes, sustained mobile inference cannot exceed approximately `{python} MobilePowerFallacyCalc.mobile_tdp_str` W without active cooling. Reducing numerical precision (using fewer bits to represent each weight; see @sec-model-compression) cuts power by approximately `{python} MobilePowerFallacyCalc.quant_red_str`$\times$, but aggressive precision reduction often causes accuracy loss. Applications requiring continuous inference beyond mobile thermal envelopes remain physically impossible regardless of algorithmic improvements.
...
class TcoPitfallCalc:
    ...
    cloud_compute_str = fmt(cloud_compute_value, precision=1, allow_zero=True, commas=True)          # "2,000"
    edge_hw_str = fmt(edge_hardware_value, precision=1, allow_zero=True, commas=False)               # "500"
    edge_network_str = fmt(edge_network_value, precision=1, allow_zero=True, commas=True)            # "3,000"
    edge_maint_str = fmt(edge_maintenance_value, precision=1, allow_zero=True, commas=False)         # "500"
    edge_reliability_str = fmt(edge_reliability_value, precision=1, allow_zero=True, commas=True)    # "2,000"
    edge_total_str = fmt(edge_total_value, precision=1, allow_zero=True, commas=True)                # "6,000"
    tco_ratio_str = fmt(tco_ratio_value, precision=1, allow_zero=True, commas=False)                 # "3"
```
...
class AmdahlCameraCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cam_isp_str = fmt(cam_isp_ms_value, precision=1, allow_zero=True, commas=False)                 # "100"
    cam_ml_str = fmt(cam_ml_ms_value, precision=1, allow_zero=True, commas=False)                   # "60"
    cam_post_str = fmt(cam_post_ms_value, precision=1, allow_zero=True, commas=False)               # "40"
    cam_total_str = fmt(cam_total_ms_value, precision=1, allow_zero=True, commas=False)             # "200"
    cam_ml_pct_str = fmt_percent(cam_ml_frac_value, precision=1, commas=False)       # "30"
    cam_non_ml_pct_str = fmt_percent(cam_non_ml_frac_value, precision=1, commas=False)  # "70"
    ...
    cam_ml_opt_str = fmt(cam_ml_optimized_ms_value, precision=1, allow_zero=True, commas=False)     # "6"
    cam_total_opt_str = fmt(cam_total_optimized_ms_value, precision=1, allow_zero=True, commas=False)  # "146"
    range_speedup_low_str = fmt(range_speedup_low_value, precision=1, commas=False)
    range_speedup_high_str = fmt(range_speedup_high_value, precision=1, commas=False)
```
