...
class IterationTax:
    ...
    acc_ceiling_pct_str = fmt(99, precision=0, commas=False)
```
...
class ConstraintPropagation:
    ...
    sensitivity_target_pct_str = fmt(sensitivity_target_pct, precision=0, commas=False)
    specificity_target_pct_str = fmt(specificity_target_pct, precision=0, commas=False)
    stage5_latency_ms_str = fmt(stage5_latency_ms, precision=0, commas=False)
    stage5_memory_mb_str = fmt(stage5_memory_mb, precision=0, commas=False)
    jetson_latency_ms_str = fmt(jetson_latency_ms, precision=0, commas=False)
    jetson_model_mb_str = fmt(jetson_model_mb, precision=0, commas=False)
    resnet_variant_mb_str = fmt(resnet_variant_mb, precision=0, commas=False)
```
...
1. **Measurable objectives**: ✓ "Achieve >`{python} ConstraintPropagation.sensitivity_target_pct_str` percent sensitivity and >`{python} ConstraintPropagation.specificity_target_pct_str` percent specificity for referable cases"
...
**Cost Analysis**: Proceeding without deployment paradigm selection risks discovering at stage 5 (Deployment) that the target is Edge deployment with <`{python} ConstraintPropagation.stage5_latency_ms_str` ms latency and <`{python} ConstraintPropagation.stage5_memory_mb_str` MB memory. By the constraint propagation principle, this would cost `{python} ConstraintPropagation.constraint_math` the effort of resolving it now at stage 1.

**Resolution**: Return to Problem Definition. Establish deployment target (for example, "Edge deployment on NVIDIA Jetson with <`{python} ConstraintPropagation.jetson_latency_ms_str` ms inference latency and <`{python} ConstraintPropagation.jetson_model_mb_str` MB model size"). This constraint will shape Data Collection (preprocessing must be device-compatible), Model Development (architecture must fit memory budget), and Evaluation (must include device-specific performance testing).
...
class BandwidthCompute:
    ...
    bw_upload_hours_str = fmt_int(upload_hours, commas=False)
...
2.  **Upload time**: The uplink is `{python} BandwidthCompute.bw_upload_mbps_str` Mbps, or `{python} BandwidthCompute.bw_upload_mbs_str` MB/s. Thus `{python} BandwidthCompute.bw_daily_mb_str` MB / `{python} BandwidthCompute.bw_upload_mbs_str` MB/s = `{python} BandwidthCompute.bw_upload_sec_str` seconds ≈ **`{python} BandwidthCompute.bw_upload_hours_str` hours**.
