...
**Context**: Amazon experimented with an internal machine-learning system for ranking technical job applicants, trained on years of historical resumes and hiring patterns [@dastin2018].

**Failure**: The data encoded the past. Because technical hiring histories were male-dominated, the model learned signals correlated with gender and reportedly downgraded resumes containing terms such as "women's" in organization names.

**Consequence**: Amazon abandoned the experimental tool rather than relying on it for hiring decisions, demonstrating that an apparently objective workflow can reproduce the constraints and biases embedded in its labels.

**Systems lesson**: Problem definition must specify fairness, auditability, and rejection criteria before data collection and model training. Otherwise the workflow optimizes for historical precedent rather than the operational goal.
