...
**Math**: In six months (~`{python} IterationTax.weeks_str`), the possible experiment count is:
...
**Cost Analysis**: Proceeding without deployment paradigm selection risks discovering at stage 5 (Deployment) that the target is Edge deployment with <`{python} ConstraintPropagation.stage5_latency_ms_str` latency and <`{python} ConstraintPropagation.stage5_memory_mb_str` memory. By the constraint propagation principle, this would cost `{python} ConstraintPropagation.constraint_math` the effort of resolving it now at stage 1.
...
2.  **Upload time**: The uplink is `{python} BandwidthCompute.bw_upload_mbps_str` Mbps, or `{python} BandwidthCompute.bw_upload_mbs_str` MB/s. Thus `{python} BandwidthCompute.bw_daily_mb_str` / `{python} BandwidthCompute.bw_upload_mbs_str` MB/s = `{python} BandwidthCompute.bw_upload_sec_str` ≈ **`{python} BandwidthCompute.bw_upload_hours_str`**.
...
- Plus: Network costs for uploading `{python} DeploymentEconomics.image_size_mb_str` images = ~USD `{python} DeploymentEconomics.cloud_network_str`/year
- **Total: ~USD `{python} DeploymentEconomics.cloud_total_str`/year** operational cost
- Risk: `{python} DeploymentEconomics.cloud_latency_risk_str` + latency breaks clinical workflow; connectivity outages halt screening
...
- Benefit: <`{python} DeploymentEconomics.edge_latency_benefit_str` latency; works offline; much lower per-inference cost
