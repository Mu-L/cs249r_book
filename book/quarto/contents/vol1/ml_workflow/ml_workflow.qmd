...
1.  **Significance (quantitative)**: It transforms the linear software "release" into a continuous loop of monitoring, retraining, and redeployment to maintain operational duty cycle and hardware efficiency $(\eta_{\text{hw}})$\index{Duty Cycle!ML lifecycle maintenance}.
...
class DataScientistTime:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    cleaning_str = fmt(cleaning_pct, precision=1, allow_zero=True, commas=False)
    collection_str = fmt(collection_pct, precision=1, allow_zero=True, commas=False)
    data_work_str = fmt(data_work_pct, precision=1, allow_zero=True, commas=False)
    model_focused_str = fmt(model_focused_pct, precision=1, allow_zero=True, commas=False)
```
...
class IterationTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weeks_str = fmt(weeks_total, precision=1, allow_zero=True, commas=False)
    hours_week_str = fmt(hours_per_week, precision=1, allow_zero=True, commas=False)

    large_time_str = MarkdownStr("1 week")
    large_acc_str = fmt(large_start_acc, precision=1, allow_zero=True, commas=False)
    ...
    small_acc_str = fmt(small_start_acc, precision=1, allow_zero=True, commas=False)
    small_gain_str = fmt(small_gain_per_iter, precision=1, commas=False)
    small_iters_str = fmt(small_effective_iters, precision=1, allow_zero=True, commas=False)
    small_uncapped_str = fmt(small_uncapped_acc, precision=1, allow_zero=True, commas=False)
    small_final_str = fmt(small_final_acc, precision=1, allow_zero=True, commas=False)

    # Logic check helper for text
    small_total_capacity_str = fmt(weeks_total * hours_per_week, precision=1, allow_zero=True, commas=True)
```
...
class MobilenetSpecs:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    mobilenet_size_str = fmt(mobilenet_size_mb, precision=1, allow_zero=True, commas=False)
    mobilenet_flops_str = fmt(mobilenet_flops_m, precision=1, allow_zero=True, commas=False)
```
...
from mlsysim.core.formulas import calc_constraint_propagation_factor
...
class ConstraintPropagation:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cost_factor_str = fmt(cost_factor, precision=1, allow_zero=True, commas=False)
...
class BandwidthCompute:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    bw_patients_str = fmt(patients_day, precision=1, allow_zero=True, commas=False)
    bw_photos_str = fmt(photos_per_patient, precision=1, allow_zero=True, commas=False)
    bw_mb_per_photo_str = fmt(mb_per_photo, precision=1, allow_zero=True, commas=False)
    bw_daily_mb_str = fmt(daily_mb, precision=1, allow_zero=True)
    bw_daily_gb_str = fmt(daily_gb, precision=1, commas=False)
    bw_upload_mbps_str = fmt(uplink_mbps, precision=1, allow_zero=True, commas=False)
    bw_upload_mbs_str = fmt(uplink_mbs, precision=2, commas=False)
    bw_upload_sec_str = fmt(upload_seconds, precision=1, allow_zero=True)
    bw_upload_hours_str = fmt(upload_hours, precision=1, commas=False)
    bw_clinic_hours_str = fmt(clinic_hours, precision=1, allow_zero=True, commas=False)
    bw_bandwidth_pct_str = fmt(saturation_pct, precision=1, allow_zero=True, commas=False)
    bw_summary_kb_str = fmt(summary_kb, precision=1, allow_zero=True, commas=False)
    bw_reduction_str = fmt(reduction_factor, precision=1, allow_zero=True)
```
...
2.  **Upload time**: The uplink is `{python} BandwidthCompute.bw_upload_mbps_str` Mbps, or `{python} BandwidthCompute.bw_upload_mbps_str`/8 MB/s. Thus `{python} BandwidthCompute.bw_daily_mb_str` MB / (`{python} BandwidthCompute.bw_upload_mbps_str`/8 MB/s) = `{python} BandwidthCompute.bw_upload_sec_str` seconds ≈ **`{python} BandwidthCompute.bw_upload_hours_str` hours**.
...
**Systems insight**: A Cloud-only architecture is too "expensive" in terms of bandwidth. Moving to the edge requires uploading only *detection summaries* (~`{python} BandwidthCompute.bw_summary_kb_str` KB/patient), reducing bandwidth usage by `{python} BandwidthCompute.bw_reduction_str`$\times$.
...
class HyperparameterGrid:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    n_hyperparams_str = fmt(n_hyperparams, precision=1, allow_zero=True, commas=False)
    values_per_param_str = fmt(values_per_param, precision=1, allow_zero=True, commas=False)
    experiment_count_str = fmt(experiment_count, precision=1, allow_zero=True, commas=True)
...
1.  **Significance (quantitative)**: It moves beyond "test set accuracy" to test for robustness against distribution shift $\mathcal{D}(P_t \lVert P_0)$ and efficiency against hardware limits $(R_{\text{peak}}, \text{BW})$.
...
class DeploymentEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_clinics_str = fmt(n_clinics, precision=1, allow_zero=True)
    patients_per_day_str = fmt(patients_day, precision=1, allow_zero=True, commas=False)
    billable_images_per_patient_str = fmt(billable_images_per_patient, precision=1, allow_zero=True, commas=False)
    days_per_year_str = fmt(days_year, precision=1, allow_zero=True, commas=False)
    monthly_images_str = fmt(round(total_images_year / 12 / THOUSAND) * THOUSAND, precision=1, allow_zero=True)
    cloud_cost_per_image_str = fmt(cloud_inf_cost, precision=2, commas=False)
    cloud_network_str = fmt(cloud_network_cost, precision=1, allow_zero=True)
    edge_cost_per_unit_str = fmt(edge_unit_cost, precision=1, allow_zero=True)
    image_size_mb_str = fmt(image_size_mb, precision=1, allow_zero=True, commas=False)
    edge_inference_cost_str = fmt(edge_inf_cost, precision=3, commas=False)
    cloud_latency_risk_str = fmt(cloud_latency_ms, precision=1, allow_zero=True, commas=False)
    edge_latency_benefit_str = fmt(edge_latency_ms, precision=1, allow_zero=True, commas=False)

    cloud_annual_str = fmt(cloud_annual_inf, precision=1, allow_zero=True, commas=True)
    cloud_total_str = fmt(cloud_total_year, precision=1, allow_zero=True, commas=True)
    edge_capex_str = fmt(edge_capex, precision=1, allow_zero=True, commas=True)
    edge_maintenance_str = fmt(edge_maint_cost, precision=1, allow_zero=True)
    edge_inference_annual_str = fmt(total_images_year * edge_inf_cost, precision=1, allow_zero=True, commas=True)
    edge_opex_year_str = fmt(edge_opex_year, precision=1, allow_zero=True, commas=True)
...
class MonitoringThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sens_target_str = fmt(sens_target, precision=1, allow_zero=True, commas=False)
    spec_target_str = fmt(spec_target, precision=1, allow_zero=True, commas=False)
    sens_alert_str = fmt(sens_alert, precision=1, allow_zero=True, commas=False)
    spec_alert_str = fmt(spec_alert, precision=1, allow_zero=True, commas=False)
    latency_p95_target_str = fmt(latency_p95_target, precision=1, allow_zero=True, commas=False)
    latency_alert_str = fmt(latency_alert, precision=1, allow_zero=True, commas=False)
```
...
***The Constraint Propagation Principle***\index{Constraint Propagation Principle!definition} states that constraints discovered late in the lifecycle stage index $(N_{\text{stage}})$ incur an exponential cost relative to catching them during Stage 1 problem definition ($\text{Correction Cost} \approx 2^{(N_{\text{stage}}-1)} \times \text{Base Effort}$).

1.  **Significance (quantitative)**: It dictates that system design must proceed end-to-end. Within the iron law, a constraint on $R_{\text{peak}}$ at deployment (Stage 5) propagates backward to redefine the Stage 1 requirements and the downstream dataset size $(D)$, data movement $(D_{\text{vol}})$, and algorithm complexity $(O)$ that those requirements permit.
