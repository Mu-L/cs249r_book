...
from mlsysim.fmt import fmt, check, MarkdownStr, md_math
...
class ConstraintPropagation:
    ...
    constraint_math = md_math(f"2^{{{stage_deployment - stage_definition}}} = {cost_factor}\\times")
```
...
class DeploymentEconomics:
    ...
    image_size_mb_str = MarkdownStr("5") # Constant from text context
    edge_inference_cost_str = fmt(edge_inf_cost, precision=0, commas=False)
...
class MonitoringThresholds:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sens_target_str      = fmt(90, precision=0, commas=False)    # target sensitivity %
    spec_target_str      = fmt(80, precision=0, commas=False)    # target specificity %
    sens_alert_str       = fmt(88, precision=0, commas=False)    # alert threshold (2% margin)
    spec_alert_str       = fmt(78, precision=0, commas=False)    # alert threshold (2% margin)
    latency_p95_target_str = fmt(50, precision=0, commas=False)  # P95 latency target (ms)
    latency_alert_str    = fmt(100, precision=0, commas=False)   # alert threshold (2× target)
```
