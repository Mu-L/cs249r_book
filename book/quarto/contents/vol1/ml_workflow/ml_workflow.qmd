...
The binding constraint differs dramatically across workload archetypes, causing each lifecycle stage to optimize different iron law terms. @Tbl-lighthouse-workflow-comparison shows *how* the same workflow stages manifest for three recurring workload archetypes the book returns to throughout (catalogued in @tbl-lighthouse-examples): a compute-bound vision model (ResNet-50), a memory-bound recommender (DLRM), and an energy-bound keyword spotter:
