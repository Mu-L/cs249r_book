...
class ModelDeviceComparison:
    ...
    gpt2_str = f"{gpt2_mem.m_as(GB):.0f} GB"
    resnet_str = f"{resnet_mem.m_as(MB):.0f} MB"
    mobilenet_str = f"{mobilenet_mem.m_as(MB):.0f} MB"
    mobilenet_int8_str = f"{mobilenet_int8_mem.m_as(MB):.1f} MB"
    dscnn_str = f"{dscnn_mem.m_as(KB):.0f} KB"
    ...
    mobilenet_int8_tiny_ratio_str = f"{mobilenet_int8_tiny_ratio:.0f}"
    dscnn_tiny_str = "ok"
```
...
class MobileNetCompressionAnchor:
    ...
    original_size_str = f"{original_size_mb}MB"
    pruned_size_str = f"{pruned_size_mb}MB"
```
