...
class CompressionSetup:
    ...
    llm_7b_mem_str = fmt(llm_7b_mem_fp16_gb, precision=1, commas=False)
    llm_175b_str = fmt(llm_175b_params, precision=0, commas=False)
    llm_175b_mem_str = fmt(llm_175b_mem_fp16_gb, precision=1, commas=False)
    llm_175b_mem_int8_str = fmt(llm_175b_mem_int8_gb, precision=1, commas=False)
    llm_175b_fp16_int8_reduction_str = fmt(llm_175b_fp16_int8_reduction, precision=1, commas=False)
    # smartphone_ram_gb and mcu_ram_kb are floats; canonical fmt would inject ".0" — escape-hatch
    smartphone_ram_str = fmt(smartphone_ram_gb, precision=1, commas=False)
    mcu_ram_str = fmt(mcu_ram_kb, precision=1, commas=False)
    energy_dram_str = fmt(energy_dram, precision=1, commas=False)
    energy_dram_read_bits_str = fmt(energy_dram_read_bits, precision=0, commas=False)
    energy_dram_per_byte_str = fmt(energy_dram_per_byte, precision=1, commas=False)
    ...
    energy_float_add_ratio_str = fmt(energy_float_add_ratio, precision=1, commas=False)
    ...
    int8_energy_reduction_str = fmt(int8_energy_reduction, precision=1, commas=False)
...
class NASCostCalc:
    ...
    weight_sharing_reduction_str = fmt(weight_sharing_reduction, precision=1)
```
...
class QuantizationSpeedup:
    ...
    device_ram_gb_str = fmt(device_ram_gb, precision=1, commas=False)
...
class ModelDeviceComparison:
    ...
    dlrm_str = fmt(dlrm_mem.m_as(GB), precision=1, commas=False, suffix=" GB")
    gpt2_str = fmt(gpt2_mem.m_as(GB), precision=1, commas=False, suffix=" GB")
    resnet_str = fmt(resnet_mem.m_as(MB), precision=1, commas=False, suffix=" MB")
    mobilenet_str = fmt(mobilenet_mem.m_as(MB), precision=1, commas=False, suffix=" MB")
    mobilenet_int8_str = fmt(mobilenet_int8_mem.m_as(MB), precision=1, commas=False, suffix=" MB")
    dscnn_str = fmt(dscnn_mem.m_as(KB), precision=1, commas=False, suffix=" KB")
    cloud_cap_str = fmt(cloud_mem.m_as(GB), precision=1, commas=False, prefix="~", suffix=" GB")
    mobile_cap_str = fmt(mobile_mem.m_as(GB), precision=1, commas=False, prefix="~", suffix=" GB")
...
class MobileNetCompressionAnchor:
    ...
    original_size_str = fmt(original_size_mb, precision=1, commas=False, suffix=" MB")
    pruned_size_str = fmt(pruned_size_mb, precision=1, commas=False, suffix=" MB")
```
...
class PruningPracticeAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pruning_pct_str = f"{fmt(pruning_pct, precision=1, commas=False)}%"
    original_size_str = fmt(original_size_mb, precision=1, commas=False, suffix=" MB")
    pruned_size_str = fmt(pruned_size_mb, precision=1, commas=False, suffix=" MB")
```
...
class LowRankFactorization:
    ...
    data_reduction_str = fmt(data_reduction, precision=1, commas=False)
```
...
class EnergyDividend:
    ...
    energy_dram_str = fmt(e_dram, precision=1, commas=False)
...
class QuantizationSavings:
    ...
    int4_size_gb_str = fmt(int(int4_size_gb), precision=1, commas=False)
...
class SIMDThroughput:
    ...
    simd_gain_str = fmt(gain, precision=1, commas=False)
...
class A100Int8Speedup:
    ...
    a100_tops_int8_str = fmt(a100_tops_int8, precision=1, commas=False)
    a100_int8_speedup_str = fmt(a100_int8_speedup, precision=1, commas=False)
    bandwidth_bound_speedup_str = fmt(bandwidth_bound_speedup, precision=1, commas=False)
```
...
class TheoryPracticeSpeedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    pruning_speedup_str = fmt(pruning_speedup, precision=1, commas=False)
    quant_speedup_str = fmt(quant_speedup, precision=1, commas=False)
    theoretical_speedup_str = fmt(theoretical_speedup, precision=1, commas=False)
    measured_speedup_str = fmt(measured_speedup, precision=1, commas=False)
    realized_fraction_pct_str = fmt(realized_fraction_pct, precision=1, commas=False)
```
...
class V100Specs:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    v100_bw_gbs_str = fmt(v100_bw_gbs, precision=1, commas=False)
...
class FusionCalc:
    ...
    feat_map_kb_str = fmt(feat_map_mb * THOUSAND, precision=1, commas=False)   # MB → KB
    weights_mb_str = fmt(weights_mb, precision=1, commas=False)
    bn_params_kb_str = fmt(bn_params_mb * THOUSAND, precision=1, commas=False)  # MB → KB (was wrongly using KIB_TO_BYTES = 1024)
    ...
    unfused_time_us_str = fmt(unfused_time_us, precision=1, commas=False)
...
class ConvFusionCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    transfer_reduction_str = fmt(transfer_reduction, precision=1, commas=False)
...
class AmdahlCompression:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    model_fraction_pct_str = fmt(model_fraction * 100, precision=1, commas=False)
    non_model_pct_str = fmt(non_model_fraction * 100, precision=1, commas=False)
...
class ResNet50Int8Metrics:
    ...
    fp32_size_mb_str = fmt(fp32_size_mb, precision=1, commas=False)
    int8_size_mb_str = fmt(int8_size_mb, precision=1, commas=False)
...
class FallaciesAnalysis:
    ...
    expected_speedup_str = fmt(combined_expected, precision=1, commas=False)
    actual_speedup_str = fmt(actual_combined_pct, precision=1, commas=False)
    dequant_overhead_str = fmt(overhead_pct, precision=1, commas=False)
