...
class QuantizationSavings:
    ...
    gpu_mem_fp16_gb_str = fmt(gpu_mem_fp16_gb, precision=0, commas=False)
    gpu_mem_int4_gb_str = fmt(gpu_mem_int4_gb, precision=0, commas=False)
```
...
- **Hardware Req**: Requires `{python} QuantizationSavings.gpu_mem_fp16_gb_str` GB GPU (for example, A10G, 3090, 4090).
...
- **Hardware Req**: Fits comfortably on `{python} QuantizationSavings.gpu_mem_int4_gb_str` GB GPU (for example, T4, consumer laptops).
