...
**Problem**: The unoptimized MobileNetV3 (FP32), a NAS-optimized successor to our MobileNetV2 lighthouse model, runs at 8 FPS on mid-tier Android phones. It is too slow to ship.
...
**Systems insight**: Compression did not just "optimize" the feature; it *enabled* it. Without INT8 quantization, the product simply could not exist for the target market.
...
**Problem**: Low-rank factorization illustrates a classic systems trade-off: trading computation for bandwidth reduction. Storing a `{python} LowRankFactorization.mat_dim_str` by `{python} LowRankFactorization.mat_dim_str` matrix requires `{python} LowRankFactorization.full_mb_str` (at FP32). Fetching this matrix for a single inference is a massive memory bandwidth hit, especially when limited by physical memory bandwidth constraints.
...
The DLRM case demonstrates quantization as a capacity optimization: fitting terabyte-scale embedding tables onto finite accelerator memory. A TinyML device faces the opposite constraint: it quantizes not to hold more data but to survive on a coin-cell battery.
...
**Problem**: Why is INT8 faster than FP32 on the same processor?

**Setup**: SIMD (Single Instruction, Multiple Data). A CPU or GPU core processes data in fixed-width vector registers (for example, AVX-512 is 512 bits wide).
...
**Setup**\index{Quantization!affine}: An affine map sends a floating-point range $[\alpha, \beta]$ to an integer range $[0, 2^b-1]$ (for example, UINT8) via the following construction.
...
class ResNet50Int8Metrics:
    ...
    per_class_max_drop_str = fmt(per_class_max_drop, precision=1, commas=False, suffix=' percent')
    ...
    top1_drop_str = fmt(top1_drop, precision=1, commas=False, suffix=' percent')
...
When quantizing ResNet-50 from FP32 to INT8, baseline metrics show Top-1 accuracy of `{python} ResNet50Int8Metrics.fp32_top1_str`, inference latency on V100 of `{python} ResNet50Int8Metrics.fp32_latency_ms_str`, model size of `{python} ResNet50Int8Metrics.fp32_size_mb_str`, and energy per inference of `{python} ResNet50Int8Metrics.fp32_energy_j_str`. Postquantization metrics reveal Top-1 accuracy of `{python} ResNet50Int8Metrics.int8_top1_str` (`{python} ResNet50Int8Metrics.top1_drop_str` age point degradation), inference latency of `{python} ResNet50Int8Metrics.int8_latency_ms_str` (`{python} ResNet50Int8Metrics.latency_speedup_str` speedup), model size of `{python} ResNet50Int8Metrics.int8_size_mb_str` (`{python} ResNet50Int8Metrics.size_reduction_str` reduction), and energy per inference of `{python} ResNet50Int8Metrics.int8_energy_j_str` (`{python} ResNet50Int8Metrics.energy_improvement_str` improvement). Additional analysis shows per-class accuracy degradation ranging from `{python} ResNet50Int8Metrics.per_class_min_drop_str` to `{python} ResNet50Int8Metrics.per_class_max_drop_str` age points with highest impact on fine-grained categories, calibration error increasing from `{python} ResNet50Int8Metrics.calibration_error_fp32_str` to `{python} ResNet50Int8Metrics.calibration_error_int8_str`, and INT8 quantization providing `{python} ResNet50Int8Metrics.latency_speedup_str` speedup on GPU but only `{python} ResNet50Int8Metrics.cpu_speedup_str` on CPU, demonstrating hardware-dependent gains.
