...
- Explain the three-part optimization framework: model representation (pruning, distillation, architecture search), numerical precision (quantization), and architectural/hardware efficiency (fusion, sparsity exploitation, scheduling)
...
```{python}
#| label: compression-model-scale
#| echo: false
from mlsysim.core.constants import (
    GPT3_PARAMS, SMARTPHONE_RAM_GB, MCU_RAM_KIB,
    BYTES_FP16, BYTES_INT8, BYTES_FP32,
    ENERGY_DRAM_ACCESS_PJ, ENERGY_DRAM_PJ_PER_BYTE,
    ENERGY_FLOP_FP32_PJ, ENERGY_FLOP_INT8_PJ,
    ENERGY_ADD_FP32_PJ, ENERGY_ADD_INT8_PJ,
    Bparam, GB, KiB, byte, ureg
)
...
class CompressionSetup:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    llm_7b_params = 7
    int8_energy_reduction = 20                       # empirical INT8-vs-FP32 ratio
    ...
    energy_int8_mac = energy_flop_int8 + energy_add_int8
    dram_read_vs_int8_mac = energy_dram / energy_int8_mac

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(llm_7b_mem_fp16_gb > smartphone_ram_gb,
          "7B model must exceed smartphone RAM to illustrate the deployment gap.")
    ...
    check(dram_read_vs_int8_mac > 1000,
          "DRAM reads should dominate INT8 MAC energy by orders of magnitude.")
    ...
    # smartphone_ram_gb and mcu_ram_kb are floats; canonical fmt would inject ".0" — escape-hatch
    smartphone_ram_str = fmt(smartphone_ram_gb, precision=0, commas=False)
    mcu_ram_str = fmt(mcu_ram_kb, precision=0, commas=False)
    ...
    energy_flop_int8_str = fmt(energy_flop_int8, precision=1, commas=False)
    energy_float_add_ratio_str = fmt(energy_float_add_ratio, precision=0, commas=False)
    energy_dram_read_ratio_str = fmt(energy_dram_read_ratio, precision=0)
    dram_read_vs_int8_mac_str = fmt(dram_read_vs_int8_mac, precision=0)
...
A `{python} CompressionSetup.llm_7b_str`-billion parameter language model requires `{python} CompressionSetup.llm_7b_mem_str` GB merely to store its weights in FP16. The deployment target is a smartphone with `{python} CompressionSetup.smartphone_ram_str` GB of RAM shared across the operating system, applications, and the model. *The math does not work.* No amount of clever engineering changes this arithmetic: `{python} CompressionSetup.llm_7b_mem_str` GB cannot fit in `{python} CompressionSetup.smartphone_ram_str` GB. Yet users expect the model to run: responsively, offline, without draining their battery in an hour. The gap between what training produces and what deployment permits, defined formally as the latency budget in @sec-model-serving-latency-budget-ef40, is not a minor inconvenience but a defining challenge of model compression.
...
The scale of this renegotiation makes model optimization an engineering discipline, not a collection of ad hoc tricks. A `{python} CompressionSetup.llm_175b_str` billion parameter model consumes over `{python} CompressionSetup.llm_175b_mem_str` GB in FP16 representation alone, yet a smartphone provides `{python} CompressionSetup.smartphone_ram_str` GB of RAM and a microcontroller offers `{python} CompressionSetup.mcu_ram_str` KB. Bridging six orders of magnitude requires systematic methods with predictable trade-offs, not trial and error. Every optimization technique removes something from the model (redundant parameters, numerical precision, or architectural complexity), and the engineer must understand exactly what is lost, what is preserved, and how these losses compose when techniques are combined.
...
Throughout this chapter, we ground each technique in concrete systems: ResNet-50 and MobileNetV2 (our Lighthouse Models from @sec-network-architectures) for vision workloads, transformer-based language models for sequence tasks, and the DS-CNN, a depthwise-separable convolutional neural network (CNN), as the keyword spotter for TinyML deployment. These recurring models let us compare techniques under consistent conditions, making the trade-offs between accuracy, latency, memory, and energy tangible rather than abstract.
...
1.  **Significance (quantitative)**: Compression directly reduces both iron law terms. INT8 quantization of a `{python} CompressionSetup.llm_175b_str`B-parameter large language model (LLM) cuts weight memory from `{python} CompressionSetup.llm_175b_mem_str` GB (FP16) to `{python} CompressionSetup.llm_175b_mem_int8_str` GB, a `{python} CompressionSetup.llm_175b_fp16_int8_reduction_str`$\times$ reduction in $D_{\text{vol}}$, while Tensor Core INT8 paths can increase compute throughput when kernels and layouts use the supported low-precision path. Unstructured pruning to 50 percent sparsity theoretically halves $O$, but hardware speedup only materializes when sparsity is structured (for example, 2:4 sparse) to match accelerator capabilities.
...
#| echo: false
from mlsysim.core.constants import HOURS_PER_DAY
...
class NASCostCalc:
    ...
    gpu_days_str = fmt(gpu_days, precision=0)
    gpu_hours_str = fmt(gpu_hours, precision=0)
...
[^fn-nas-search-cost]: **Neural Architecture Search (NAS)**: @zoph2017neural at Google Brain used reinforcement learning to *learn* the architecture itself at a cost of `{python} NASCostCalc.gpu_days_str` GPU-days (`{python} NASCostCalc.gpus_str` GPUs for `{python} NASCostCalc.days_str` days), equivalent to `{python} NASCostCalc.gpu_hours_str` GPU-hours. Weight-sharing approaches later reduced this by `{python} NASCostCalc.weight_sharing_reduction_str`$\times$, transforming NAS from a data-center-scale experiment into a practical tool whose one-time search budget is amortized across every deployment of the resulting architecture (EfficientNet, MobileNetV3). \index{NAS!search cost}
...
The energy-movement invariant $(E_{\text{move}} \gg E_{\text{compute}})$ means that in the physics of silicon, **bits represent energy** (see @sec-machine-foundations-numerical-representations-c889 for a detailed comparison of FP32 vs. INT8 energy costs).
...
1.  **Memory energy $(D_{\text{vol}})$**: Fetching a 32-bit float from DRAM costs ≈ **`{python} CompressionSetup.energy_dram_str` pJ**. Fetching an 8-bit integer costs ≈ **`{python} CompressionSetup.energy_dram_per_byte_str` pJ**.
...
| **Float Add**   |        32-bit |     `{python} CompressionSetup.energy_float_add_ratio_str`$\times$ |
| **DRAM Read**   |        64-bit | **`{python} CompressionSetup.energy_dram_read_ratio_str`$\times$** |
...
For inference workloads, moving from FP32 to INT8 saves 4$\times$ memory and can reduce the energy per inference by up to `{python} CompressionSetup.int8_energy_reduction_str`$\times$ on hardware with dedicated INT8 units, depending on the compute-to-memory ratio of the workload. The practical impact: a battery lasting one hour or 20 hours.

These same physics apply at data center scale: distributed training systems use reduced precision to cut gradient communication overhead, a topic covered in @sec-model-training. For a deeper treatment of how silicon architectures exploit these energy differences, see @sec-hardware-acceleration.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import BYTES_FP16, BYTES_INT4, byte, MS_PER_SEC
...
class QuantizationSpeedup:
    ...
    bytes_fp16_str = fmt(int(bytes_fp16), precision=0, commas=False)
    bytes_int4_str = fmt(bytes_int4, precision=1, commas=False)
    device_ram_gb_str = fmt(device_ram_gb, precision=0, commas=False)
    mem_bw_gbs_str = fmt(int(mem_bw_gbs), precision=0, commas=False)
    kv_cache_gb_str = fmt(int(kv_cache_gb), precision=0, commas=False)

    fp16_size_str = fmt(int(fp16_size_gb), precision=0, commas=False)
    fp16_total_str = fmt(int(fp16_total_gb), precision=0, commas=False)
    fp16_latency_str = fmt(int(fp16_latency_ms), precision=0, commas=False)
    fp16_toks_str = fmt(fp16_toks, precision=1, commas=False)

    int4_size_str = fmt(int4_size_gb, precision=1, commas=False)
    int4_latency_str = fmt(int(int4_latency_ms), precision=0, commas=False)
    int4_toks_str = fmt(int(int4_toks), precision=0, commas=False)
    speedup_str = fmt(int(speedup), precision=0, commas=False)
```

The result is a bandwidth-driven speedup: fewer bits moved per token means more tokens per second.

::: {.callout-notebook title="The quantization speedup"}

**Problem**: A deployment scenario calls for running a `{python} QuantizationSpeedup.params_b_str` B parameter LLM on a device with `{python} QuantizationSpeedup.device_ram_gb_str` GB RAM. The weights are FP16 (`{python} QuantizationSpeedup.bytes_fp16_str` bytes).

**Math**:

1.  **Model size**: `{python} QuantizationSpeedup.params_b_str` $\times 10^9 \times$ `{python} QuantizationSpeedup.bytes_fp16_str` bytes = `{python} QuantizationSpeedup.fp16_size_str` GB.
2.  **KV Cache**: Context window (4096 tokens) requires ≈ `{python} QuantizationSpeedup.kv_cache_gb_str` GB.
3.  **Total memory**: `{python} QuantizationSpeedup.fp16_size_str` + `{python} QuantizationSpeedup.kv_cache_gb_str` = `{python} QuantizationSpeedup.fp16_total_str` GB. This barely fits, leaving no room for OS or buffers.
4.  **Bandwidth cost**: Loading `{python} QuantizationSpeedup.fp16_size_str` GB at `{python} QuantizationSpeedup.mem_bw_gbs_str` GB/s takes **`{python} QuantizationSpeedup.fp16_latency_str` ms** per token. That is `{python} QuantizationSpeedup.fp16_toks_str` tokens/sec, too slow for chat.

**Fix (INT4)**:

1.  **Quantization**: Convert weights to four-bit integers (`{python} QuantizationSpeedup.bytes_int4_str` bytes).
2.  **New size**: `{python} QuantizationSpeedup.params_b_str` $\times 10^9 \times$ `{python} QuantizationSpeedup.bytes_int4_str` = `{python} QuantizationSpeedup.int4_size_str` GB.
3.  **New speed**: Loading `{python} QuantizationSpeedup.int4_size_str` GB takes **`{python} QuantizationSpeedup.int4_latency_str` ms**. Speed jumps to **`{python} QuantizationSpeedup.int4_toks_str` tokens/sec**.

**Conclusion**: Quantization is not just about "fitting" the model; it is a **`{python} QuantizationSpeedup.speedup_str`$\times$ Linear Speedup** because LLM generation is bandwidth bound.
...
::: {.callout-example title="The 4× MobileNet win"}
...
# │
# │ Imports: mlsysim.core.constants (GB, GiB, MiB, KiB, MB, KB, byte,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (GB, GiB, MiB, KiB, MB, KB, byte,
...
class ModelDeviceComparison:
    ...
    gpt2_mem = 6 * GB                # 1.5B params × 4 bytes = 6 GB
    resnet_mem = 100 * MB            # ~25M params × 4 bytes
    mobilenet_mem = 14 * MB
    mobilenet_int8_mem = 3.5 * MB
    dscnn_mem = 500 * KB
    ...
    cloud_cap_str = fmt(cloud_mem.m_as(GiB), precision=0, commas=False, prefix="~", suffix=" GB")
    mobile_cap_str = fmt(mobile_mem.m_as(GiB), precision=0, commas=False, prefix="~", suffix=" GB")
    tiny_cap_str = fmt(tiny_mem.m_as(KiB), precision=0, commas=False, prefix="~", suffix=" KB")
    ...
    mobilenet_int8_tiny_ratio_str = fmt(mobilenet_int8_tiny_ratio, precision=0, commas=False)
...
::: {.callout-perspective title="The compression-accuracy trade-off curve"}
...
*   **Region 2: The efficient trade**: Techniques like "INT8 Quantization" might drop accuracy by 0.5 percent but improve speed by 400 percent. This is usually a winning trade.
...
: **The Optimization Trade-Offs**: Region 1 = Free Lunch, Region 2 = Efficient Trade, Region 3 = Danger Zone. Batch size affects training dynamics rather than model quality directly. These ranges are empirical guidelines from published benchmarks [@jacob2018; @han2015deep; @hinton2015distilling]; actual results vary with architecture, task, and implementation quality. {#tbl-optimization-tradeoffs tbl-colwidths="[22,24,19,25,10]"}

The table reveals a pattern: techniques that preserve model structure (fusion, precision reduction) tend to be "free" or cheap, while techniques that alter structure (pruning, distillation) extract more savings but require careful tuning. Before examining each technique in depth, the following checkpoint tests intuition about these trade-offs.

::: {.callout-checkpoint title="The efficiency frontier"}
...
- [ ] **The "Free Lunch"**: Can you identify optimizations like **Operator Fusion**\index{Operator Fusion!attention tiling} that improve speed without hurting accuracy?
...
[^fn-pareto-frontier-compression]: **Pareto Frontier**: Named after Italian economist Vilfredo Pareto (1848--1923), who observed that 80 percent of Italy's land was owned by 20 percent of the population. In multi-objective optimization, the Pareto frontier is the set of solutions where improving one objective (for example, speed) necessarily sacrifices another (for example, accuracy). EfficientNet traces this frontier concretely: B0 (77.1 percent accuracy, 390M FLOPs) to B7 (84.4 percent, 37B FLOPs)—a 95$\times$ compute increase for 7.3 percentage points of accuracy, quantifying exactly how steep the trade-off becomes at the frontier's edge. \index{Pareto Frontier!compression trade-off}
...
# │ Show: About 86% pruning with significant size reduction (14MB to 2MB).
...
class MobileNetCompressionAnchor:
    ...
    pruning_pct_str = fmt(pruning_pct, precision=0, commas=False, suffix="%")
...
\hat{W} = M \odot W
$$
where $\odot$ represents the element-wise *Hadamard product*. The mask $M$ is constructed based on a pruning criterion, typically weight magnitude. A common approach is magnitude-based pruning, which removes a fraction $s$ of the lowest-magnitude weights by defining a threshold $\tau$ such that:
$$
M_{i,j} =
\begin{cases}
1, & \text{if } |W_{i,j}| > \tau \\
...
| **Aspect**                 | **Unstructured Pruning**\index{Unstructured Pruning!definition}                               | **Structured Pruning**                                                    | **Dynamic Pruning**\index{Dynamic Pruning!definition}  |
|:---------------------------|:----------------------------------------------------------------------------------------------|:--------------------------------------------------------------------------|:-------------------------------------------------------|
...
```{python}
#| label: pruning-practice-anchor
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ PRUNING PRACTICE ANCHOR
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Pruning in practice paragraph immediately below.
# │
# │ Goal: Keep MobileNet pruning numbers local to the deployment example.
# │ Show: 14 MB to 2 MB is about an 86% reduction.
# │ How: Compute the size reduction from before/after model sizes.
# │
# │ Imports: mlsysim.book (fmt, check)
# │ Exports: PruningPracticeAnchor.pruning_pct_str, .original_size_str,
# │          .pruned_size_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class PruningPracticeAnchor:
    """Local MobileNet pruning anchor for the practice paragraph."""
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pruning_pct_str = f"{fmt(pruning_pct, precision=0, commas=False)}%"
    original_size_str = fmt(original_size_mb, precision=0, commas=False, suffix="MB")
    pruned_size_str = fmt(pruned_size_mb, precision=0, commas=False, suffix="MB")
```

These trade-offs become concrete when examining real-world deployments. Several high-profile models have successfully integrated pruning to optimize performance. MobileNet, designed for mobile and embedded applications, has been pruned to reduce inference latency while preserving accuracy [@howard2017mobilenets]. Concretely, removing about `{python} PruningPracticeAnchor.pruning_pct_str` of near-zero weights in a MobileNet can reduce its size from `{python} PruningPracticeAnchor.original_size_str` to `{python} PruningPracticeAnchor.pruned_size_str` with less than 1 percent accuracy loss. BERT-style transformers[^fn-bert-pruning-scale] have been pruned by removing redundant attention heads or intermediate dimensions, while separate distillation methods such as DistilBERT and TinyBERT train smaller dense student models that retain much of BERT's performance [@sanh2019distilbert]. In computer vision, EfficientNet has been pruned to remove unnecessary filters, optimizing it for deployment in resource-constrained environments [@tan2019efficientnet].
...
[^fn-distillation-chemistry]: **Distillation**: Borrowed from chemistry, where distillation separates mixtures by selective evaporation, extracting the essence while leaving impurities behind. @hinton2015distilling introduced temperature-scaled softmax to control how much "dark knowledge" about class relationships the student absorbs—the temperature parameter $T$ mirrors literal temperature in chemical distillation. The metaphor captures the systems trade-off precisely: distillation moves complexity from inference compute to training compute, producing a model 2--10$\times$ smaller at the cost of a one-time training budget to generate the teacher's soft targets. \index{Knowledge Distillation!etymology}
...
Starting from the softmax normalization in @sec-neural-computation-softmax-ebe5, we use a **temperature parameter**\index{Knowledge Distillation!temperature parameter}[^fn-temperature-boltzmann] $T$ to soften the probability distribution. The softmax output for class $i$ becomes:

[^fn-temperature-boltzmann]: **Temperature (Softmax)**: Borrowed from statistical mechanics, where the Boltzmann distribution $p_i \propto \exp(-E_i/kT)$ describes particle states at temperature $T$—higher temperature means more uniform distribution across states. The analogy is load-bearing: at $T{=}1$ (standard softmax), the teacher's output is a near-one-hot vector carrying almost no inter-class information; at $T{=}3$--$5$, the distribution softens enough to reveal which wrong classes the teacher considers plausible. This temperature tuning directly controls the bandwidth of information transferred from teacher to student, making it the primary hyperparameter governing distillation quality. \index{Temperature!softmax distillation}
$$
...
A higher $T$ (typically three to 5) produces a smoother distribution, allowing the student to learn from the "uncertainty" the teacher assigns to incorrect classes. The total loss $\mathcal{L}_{\text{distill}}$ balances standard cross-entropy with the KL divergence:
$$
\mathcal{L}_{\text{distill}} = (1 - \alpha) \mathcal{L}_{\text{CE}}(y_s, y) + \alpha T^2 \text{KL}(p_{\text{teacher}}^T, p_{\text{student}}^T)
$$

The factor $T^2$ ensures that gradient scales remain consistent when $T$ is changed. This hybrid approach enables compact models (like DistilBERT) to achieve up to 97 percent of their teacher's performance with a fraction of the memory and compute.
...
Distillation also enables *multi-task deployment*: a single large teacher can guide multiple specialized students for different tasks (for example, language-specific NLP models, task-specific vision models), amortizing the teacher's training cost across many deployment targets. The resulting students can be further optimized with pruning and quantization for hardware-specific acceleration [@gordon2020].
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import byte, MB
...
[^fn-svd-rank-tradeoff]: **Singular Value Decomposition (SVD)**: The Eckart-Young theorem (1936) proves that retaining the top $k$ singular values yields the optimal rank-$k$ approximation, minimizing information loss. The key systems trade-off is whether the high, one-time compute cost of this factorization—$O(mn \cdot \min(m,n))$—is amortized by the memory and bandwidth savings from using the smaller model in repeated inference calls. \index{SVD!rank-bandwidth trade-off}
...
**Reducing the Memory Pressure**: Low-rank factorization illustrates a classic systems trade-off: **trading computation for bandwidth reduction**. Storing a `{python} LowRankFactorization.mat_dim_str` by `{python} LowRankFactorization.mat_dim_str` matrix requires `{python} LowRankFactorization.full_mb_str` MB (at FP32). Fetching this matrix for a single inference is a massive memory bandwidth hit, especially when limited by physical memory bandwidth constraints.

If we factorize it with rank $k$ = `{python} LowRankFactorization.rank_k_str`, we store two matrices (`{python} LowRankFactorization.mat_dim_str` by `{python} LowRankFactorization.rank_k_str` and `{python} LowRankFactorization.rank_k_str` by `{python} LowRankFactorization.mat_dim_str`), totaling only `{python} LowRankFactorization.factored_mb_str` MB, a **`{python} LowRankFactorization.data_reduction_str`$\times$ reduction in data movement**. When inference uses the two factors directly, matrix-vector compute also falls from $O(mn)$ to $O(k(m+n))$ for small $k$. The system speedup is often dramatic because the processor moves much less data and performs fewer operations than with the original dense matrix. Explicitly materializing $UV$ would add $O(mkn)$ work and defeat the purpose.
...
To see why this matters, study @fig-matrix-factorization: the matrix $M$ can be approximated by the product of matrices $L_k$ and $R_k^T$. For intuition, most fully connected layers in networks are stored as a projection matrix $M$, which requires $m{\times}n$ parameters to be loaded during computation. However, by decomposing and approximating it as the product of two lower-rank matrices, we only need to store $m \times k + k \times n$ parameters in terms of storage. Applying the factors directly costs $O(k(m+n))$ for a vector input, instead of $O(mn)$ for the original dense matrix; explicitly forming the dense product $UV$ would cost $O(mkn)$ and should be avoided in inference [@gu2023deep].
...
Pruning, knowledge distillation, and other techniques explored in previous sections rely on human expertise to determine optimal model configurations.\index{NAS!definition} Selecting optimal architectures requires extensive experimentation, and even experienced practitioners may overlook more efficient designs [@elsken2019]. Neural architecture search (NAS) automates this process by systematically exploring large spaces of possible architectures to identify those that best balance accuracy, computational cost, memory efficiency, and inference latency [@zoph2017neural].
...
# │
# │ Goal: Keep the 7B FP16 weight-footprint comparison local to quantization prose.
# │ Show: 7B parameters at FP16 require 14 GB, exceeding an 8 GB smartphone RAM budget.
...
class QuantizationMemoryAnchor:
    """Local 7B FP16 memory anchor for quantization motivation."""
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(llm_7b_mem_fp16_gb > smartphone_ram_gb,
          "7B FP16 weights should exceed the smartphone RAM anchor.")
...
A `{python} QuantizationMemoryAnchor.llm_7b_str` billion parameter language model stored in FP16 consumes `{python} QuantizationMemoryAnchor.llm_7b_mem_str` GB, yet users expect it to run on a smartphone with `{python} QuantizationMemoryAnchor.smartphone_ram_str` GB of shared RAM. Structural optimization alone cannot bridge this gap: even aggressive pruning rarely exceeds 50--70 percent parameter reduction, leaving a model far too large for the target device. The remaining gains come from a different dimension entirely: reducing the number of bits used to represent each parameter. *Quantization*, the process of reducing numerical precision, offers one of the most impactful optimizations for deployment, because it trades bits for speed and efficiency with minimal accuracy loss.
...
1.  **Significance (quantitative)**: It reduces the **Memory Bandwidth $(\text{BW})$** and energy consumption by 4$\times$ (FP32 to INT8) or more, exploiting the inherent robustness of neural networks to low-precision arithmetic.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    ENERGY_ADD_FP32_PJ, ENERGY_ADD_FP16_PJ,
    ENERGY_ADD_INT32_PJ, ENERGY_ADD_INT8_PJ, ureg
)
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ ENERGY DIVIDEND CALCULATION
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: "The Energy Dividend" section
# │
# │ Goal: Contrast bit-reduction vs energy-reduction in quantization.
# │ Show: That INT8 is ~30× more energy-efficient than FP32 for addition.
# │ How: Calculate the ratio of energy per OP using Horowitz (2014) numbers.
# │
# │ Imports: mlsysim.core.constants (ENERGY_ADD_FP32_PJ, ENERGY_ADD_INT8_PJ)
# │ Exports: EnergyDividend.energy_add_fp32_str, .energy_add_int8_str, .energy_dividend_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    ENERGY_ADD_FP32_PJ, ENERGY_ADD_INT8_PJ,
    ENERGY_DRAM_ACCESS_PJ, ENERGY_FLOP_INT8_PJ, ureg
)
from mlsysim.fmt import fmt, check
...
class EnergyDividend:
    ...
    e_int8_mac = ENERGY_FLOP_INT8_PJ.m_as(ureg.picojoule/ureg.count) + e_add_int8

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Step 1: Dividend = FP32_Energy/INT8_Energy
    ...
    energy_dividend_str = fmt(int(dividend), precision=0, commas=False)
...
In FP32, even the compact DS-CNN architecture consumes 4$\times$ more memory bandwidth and energy per inference than in INT8. For an always-on device running on a coin cell battery, this 4$\times$ energy difference translates directly to battery life: a device that lasts one month on FP32 might last four months on INT8. Here, quantization is the primary lever for the **Energy Term** $(O / (R_{\text{peak}} \cdot \eta_{\text{hw}}))$ of the iron law.

:::

Beyond direct compute savings, reducing numerical precision has a significant impact on memory energy consumption, which often dominates total system power. Lower-precision representations reduce data storage requirements and memory bandwidth usage, leading to fewer and more efficient memory accesses. Accessing memory, particularly off-chip DRAM, is far more energy-intensive than performing arithmetic operations: the representative 64-bit DRAM read used in this chapter costs `{python} EnergyDividend.energy_dram_str` pJ, compared with picojoule-scale cache and arithmetic operations. An instruction's total energy can therefore be dominated by memory access patterns rather than computation[^fn-int8-energy-deployment].

[^fn-int8-energy-deployment]: **INT8 Energy Impact**: The energy dominance of memory access is extreme: with the Horowitz constants used here, a single 64-bit DRAM read costs roughly `{python} EnergyDividend.dram_read_vs_int8_mac_str`$\times$ the energy of an INT8 multiply-accumulate [@horowitz2014]. Quantizing from FP32 to INT8 attacks this disparity on both fronts—4$\times$ fewer bytes moved *and* cheaper arithmetic per operation. Representative MobileNetV2 INT8 measurements show multi-fold energy-per-inference reductions, with most savings coming from reduced memory traffic rather than cheaper arithmetic. \index{Quantization!energy impact}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import BYTES_FP16, BYTES_INT4, byte, GB
...
class QuantizationSavings:
    ...
    fp16_bytes_str = fmt(int(bytes_fp16), precision=0, commas=False)
    int4_bytes_str = fmt(bytes_int4, precision=1, commas=False)
    fp16_size_gb_str = fmt(int(fp16_size_gb), precision=0, commas=False)
    int4_size_gb_str = fmt(int(int4_size_gb), precision=0, commas=False)
    compression_ratio_str = fmt(int(ratio), precision=0, commas=False)
```

::: {.callout-notebook title="Quantization savings"}

**Scenario**: Deploying Llama 3 `{python} QuantizationSavings.llm_params_b_str` B (`{python} QuantizationSavings.llm_params_b_str` billion parameters).

**FP16 (Half Precision)**

- **Size**: `{python} QuantizationSavings.llm_params_b_str` $\times 10^9 \times$ `{python} QuantizationSavings.fp16_bytes_str` bytes (16-bit) = `{python} QuantizationSavings.fp16_size_gb_str` GB
- **Hardware Req**: Requires 24 GB GPU (for example, A10G, 3090, 4090).

**INT4 (4-bit Quantization)**

- **Size**: `{python} QuantizationSavings.llm_params_b_str` $\times 10^9 \times$ `{python} QuantizationSavings.int4_bytes_str` bytes (4-bit) = `{python} QuantizationSavings.int4_size_gb_str` GB
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import SIMD_REGISTER_BITS, FP32_BITS, INT8_BITS
...
**Systems conclusion**: Quantization delivers up to **`{python} SIMDThroughput.simd_gain_str`$\times$ speedup** on compute-bound layers from vector packing alone, on hardware whose INT8 vector instructions have comparable throughput to FP32, even before considering memory bandwidth savings.
...
# │ Show:    FP16 vs INT8 TFLOPS and the resulting speedup factor.
# │ How:     Read A100 specs from mlsysim.core.constants; compute ratio.
# │
# │ Imports: mlsysim.core.constants (A100_FLOPS_FP16_TENSOR, A100_FLOPS_INT8,
# │          A100_MEM_BW, TFLOPs, TB, second)
# │ Exports: A100Int8Speedup.a100_tflops_fp16_str, .a100_tflops_int8_str,
# │          .a100_int8_speedup_str, .bandwidth_bound_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    A100_FLOPS_FP16_TENSOR, A100_FLOPS_INT8, A100_MEM_BW,
    TFLOPs, TB, second
)
from mlsysim.fmt import fmt, check
...
1. **Tensor Core throughput**: NVIDIA A100 delivers `{python} A100Int8Speedup.a100_tflops_fp16_str` TFLOPS for FP16 vs. `{python} A100Int8Speedup.a100_tflops_int8_str` TOPS for INT8, a `{python} A100Int8Speedup.a100_int8_speedup_str`$\times$ peak throughput increase.
...
: **Numerical Precision Formats**: Comparison of precision formats by bit width, memory reduction, computational efficiency, accuracy retention, and typical use cases across deployment contexts. {#tbl-numerics tbl-colwidths="[17,10,13,18,12,24]"}
...
INT8\index{INT8!efficiency gains}\index{Quantization!INT8 (8-bit integer)} precision offers more aggressive efficiency improvements for inference workloads. Many quantized models use INT8 for inference, reducing storage by 4$\times$ while accelerating computation by 4--8$\times$ on optimized hardware. INT8 is widely used in mobile and embedded AI, where energy constraints are significant. As the quantization savings calculation demonstrated earlier, this 4$\times$ reduction in model size enables a `{python} QuantizationSavings.llm_params_b_str` billion parameter model to fit on a single consumer GPU rather than requiring data center hardware.

Binary and ternary networks\index{Quantization!binary networks}\index{Quantization!ternary networks} represent the extreme end of quantization, where weights and activations are constrained to one-bit (binary) or two-bit (ternary) values. This results in massive storage and energy savings, but model accuracy often degrades significantly unless specialized architectures are used. Our keyword-spotting lighthouse lives precisely in this regime, where extreme compression is required just to fit the device budget.
...
# │
# │ Imports: mlsysim.core.constants (ENERGY_FLOP_FP32_PJ, ENERGY_FLOP_INT8_PJ,
# │          ENERGY_ADD_FP32_PJ, ENERGY_ADD_INT8_PJ)
# │ Exports: EnergyRatio.int8_fp32_energy_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    ENERGY_FLOP_FP32_PJ, ENERGY_FLOP_INT8_PJ,
    ENERGY_ADD_FP32_PJ, ENERGY_ADD_INT8_PJ, ureg
)
from mlsysim.fmt import fmt, check
...
class EnergyRatio:
    ...
    int8_mac_pj = energy_flop_int8 + energy_add_int8_pj     # ~0.23 pJ
...
@lst-quantization_example demonstrates uniform quantization from FP32 to INT8, achieving 4$\times$ memory reduction while measuring the resulting quantization error.\index{Uniform Quantization!definition}
...
class QuantizationMathCalc:
    ...
    steps_str = fmt(int_steps, precision=0, commas=False)       # "255"
    scale_str = fmt(scale, precision=4, commas=False)           # "0.0157"
    zero_point_str = fmt(int(zero_point), precision=0, commas=False)  # "64"
    x_val_str = fmt(x_val, precision=1, commas=False)           # "0.0"
    x_q_str = fmt(int(x_q), precision=0, commas=False)          # "64"
...
**Math**:
We need a linear mapping $x \approx s(x_q - z)$, where $s$ is the **scale** (step size) and $z$ is the **zero-point** (integer value corresponding to real zero). The affine quantization process consists of three steps:
...
*   **Zero-Point**: $z = \text{round}(-(\alpha) / s)$ = round(−`{python} QuantizationMathCalc.alpha_str`/`{python} QuantizationMathCalc.scale_str`) = round(63.75) = `{python} QuantizationMathCalc.zero_point_str`.

Thus, the real value $0.0$ is represented by the integer `{python} QuantizationMathCalc.zero_point_str`. This ensures that zero-padding (common in CNNs) is represented exactly, preventing "quantization drift" where padding introduces nonzero noise.
...
Weight Quantization\index{Quantization!weight} involves converting the continuous, high-precision weights of a model into lower-precision values, such as converting 32-bit floating-point (Float32) weights to 8-bit integer (INT8) weights. In @fig-weight-activations-quantization, focus on the violet quantization boxes that feed INT8 operands into the matrix multiplication; the red boxes show accumulator and output tensors after the multiply-accumulate path. This process significantly reduces the model size, decreasing both the memory required to store the model and the computational resources needed for inference. For example, a weight matrix in a neural network layer with Float32 weights like $[0.215, -1.432, 0.902,\ldots]$ might be mapped with a max-absolute INT8 scale to values such as $[19, -127, 80,\ldots]$, leading to a significant reduction in memory usage.
...
Recent advancements have explored **Activation-aware Weight Quantization (AWQ)**\index{Quantization!activation-aware weight (AWQ)}[^fn-awq-salient-weights] for the compression and acceleration of LLMs. This approach is particularly relevant for our GPT-2/Llama Lighthouse, which is memory-bandwidth bound. By protecting only a small fraction of the most salient weights (approximately 1 percent) based on activation magnitude, AWQ enables effective four-bit weight quantization. This reduces the memory traffic required to load the massive parameter set for every token generation, directly attacking the primary bottleneck of generative inference [@lin2025].

[^fn-awq-salient-weights]: **Activation-aware Weight Quantization (AWQ)**: Salience is determined by activation magnitude, not weight magnitude—a distinction that matters because a small weight multiplied by a large activation produces a large output contribution. Protecting these ~1 percent of weights in higher precision while quantizing the rest to four-bit reduces a 7B-parameter model from ~14 GB to ~3.5 GB, converting what would be a memory-bandwidth-bound bottleneck into a compute-bound operation on modern accelerators. \index{AWQ!bandwidth reduction}
...
| **Aspect**                    | **Post-Training Quantization** | **Quantization-Aware Training** | **Dynamic Quantization**\index{Dynamic Quantization!definition} |
|:------------------------------|:------------------------------:|:-------------------------------:|:---------------------------------------------------------------:|
| **Pros**                      |                                |                                 |                                                                 |
| **Simplicity**                |               ✓                |                ✗                |                                ✗                                |
...
This comparison highlights the diverse strategies available for precision reduction. Before proceeding to advanced techniques, the following checkpoint tests comprehension of these core quantization modes.

::: {.callout-checkpoint title="The quantization gate"}
...
\index{Straight-Through Estimator!etymology}
...
1. **Quantization**\index{Quantization!affine}\index{Quantization!zero-point}\index{Quantization!drift}: Map floating-point value to discrete quantization level
...
The cost is additional engineering complexity. Simulated quantization at every forward pass adds 20–50 percent to training time, and QAT introduces hyperparameters (quantization schemes, scale factor update schedules) that require careful tuning [@choukroun2019]. This overhead makes QAT less practical for models exceeding tens of billions of parameters, where training budgets are already constrained.
...
Yet practitioners often discover a frustrating gap between theory and practice: a model pruned to 50 percent parameters and quantized to INT8 has a back-of-envelope `{python} TheoryPracticeSpeedup.theoretical_speedup_str`$\times$ speedup target, but may measure only about `{python} TheoryPracticeSpeedup.measured_speedup_str`$\times$ on actual hardware. That is roughly `{python} TheoryPracticeSpeedup.realized_fraction_pct_str` percent of the paper speedup. This theory-practice gap reveals that theoretical compression does not translate into proportional speedup. Optimization must extend beyond the model itself to how computations execute on physical hardware.

The gap arises from several sources. Sparse matrices stored in dense format waste memory bandwidth loading zeros—the hardware cannot skip what it does not know is zero. Operations that could run in parallel execute sequentially due to data dependencies the compiler cannot resolve. Simple inputs receive the same computational budget as complex ones because the model has no mechanism to exit early. Closing the gap between "optimized on paper" and "optimized in practice" is the domain of our third optimization dimension: **architectural efficiency**\index{Architectural Efficiency!factorization techniques}. This dimension ensures that structural and precision optimizations translate into real-world speedups by aligning computation patterns with hardware capabilities.
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ V100 HARDWARE SPECS FOR FUSION ANALYSIS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: FusionCalc cell (uses v100_bw_gbs for timing) and prose at
# │          @sec-model-compression-architectural-efficiency-8dd3 ("On a V100
# │          GPU with ...").
# │
# │ Goal:    Provide V100 bandwidth and compute specs for fusion benchmarks.
# │ Show:    V100 HBM bandwidth (GB/s) and FP32 compute (TFLOPS).
# │ How:     Read V100 specs from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (V100_MEM_BW, V100_FLOPS_FP32, GB, TFLOPs, second)
# │ Exports: V100Specs.v100_bw_gbs, .v100_bw_gbs_str,
# │          .v100_tflops_fp32_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import V100_MEM_BW, V100_FLOPS_FP32, GB, TFLOPs, second
from mlsysim.fmt import fmt

class V100Specs:
    """V100 bandwidth and compute specs for operator fusion analysis."""
    ...
    # ┌── 3. GUARD—(no derived quantities to guard) ─────────────────────
...
class FusionCalc:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # Step 1: Feature map size (SI MB)
    feat_map_mb = conv_channels * conv_spatial * conv_spatial * bytes_per_element/MILLION

    # Step 2: Conv-BN-ReLU intermediate (2 feature maps written: conv→BN boundary)
    conv_bn_relu_intermediate_mb = (
        2 * conv_channels * conv_spatial * conv_spatial * bytes_per_element / MILLION
    )

    # Step 3: GEMM intermediate
    gemm_intermediate_mb = gemm_hidden * gemm_seq * bytes_per_element / MILLION

    # Step 4: Unfused bandwidth: Conv (feat*2 + weights) + BN (feat*2 + bn) + ReLU (feat*2)
    ...
    # Step 5: Fused bandwidth: read input + weights once, write output once
    total_fused_mb = feat_map_mb * 2 + weights_mb
    bandwidth_reduction_pct = (1 - total_fused_mb/total_unfused_mb) * 100

    # Step 6: Kernel launch savings
    saved_latency_us = (kernels_unfused - kernels_fused) * latency_per_kernel_us
    saved_latency_ms = saved_latency_us / THOUSAND   # μs → ms

    # Step 7: V100 timing (memory-bound): MB / (GB/s) * THOUSAND = µs
    unfused_time_us = total_unfused_mb/V100Specs.v100_bw_gbs * THOUSAND
    fused_time_us = total_fused_mb/V100Specs.v100_bw_gbs * THOUSAND
    ...
    feat_map_kb_str = fmt(feat_map_mb * THOUSAND, precision=0, commas=False)   # MB → KB
    weights_mb_str = fmt(weights_mb, precision=1, commas=False)
    bn_params_kb_str = fmt(bn_params_mb * THOUSAND, precision=0, commas=False)  # MB → KB (was wrongly using KIB_TO_BYTES = 1024)
...
The read-compute-write cycle creates memory bandwidth bottlenecks for operations with low arithmetic intensity (FLOPs/byte accessed). The memory traffic for $N$ unfused operations operating on tensors of size $M$ bytes is:
$$
...
class ConvFusionCalc:
    ...
    conv_bn_relu_mem_math = fmt_math(
        f"2 \\times 256 \\times 28 \\times 28 \\times 4 \\text{{ bytes}} \\approx \\text{{{FusionCalc.conv_bn_relu_intermediate_mb_str} MB}}"
    )
```
...
The same principle extends beyond CNNs. General matrix multiply (GEMM) bias-activation fusion eliminates intermediate writes in transformer linear layers by computing element-wise operations in registers immediately after each matrix multiplication output element. Attention tiling, as in FlashAttention[^fn-flashattention-fusion], reduces HBM traffic from $O(n^2)$ to $O(n)$ for long-context transformers by processing attention in SRAM-sized tiles rather than materializing the full $n{\times}n$ attention matrix, as detailed in @sec-model-training-flash-attention-ioaware-attention-optimization-3da0.

[^fn-flashattention-fusion]: **FlashAttention**\index{FlashAttention!operator fusion}: Demonstrates fusion's power for memory bound attention by tiling computation to SRAM, yielding 2--4$\times$ speedup and enabling 64K context windows by reducing memory traffic from $O(n^2)$ to $O(n)$. This exemplifies how operator fusion transforms memory-bound bottlenecks into compute-bound operations—the arithmetic is identical, but the memory access pattern changes from quadratic to linear, making long-context inference feasible on hardware that could not otherwise hold the full attention matrix. \index{FlashAttention!memory traffic reduction}

Memory bandwidth analysis quantifies these fusion benefits concretely. Consider a Conv-BN-ReLU sequence operating on a $28{\times}28{\times}256$ feature map (`{python} FusionCalc.feat_map_kb_str` KB). Without fusion, each operation performs its own memory round-trip: Conv reads input (`{python} FusionCalc.feat_map_kb_str` KB) plus weights (`{python} FusionCalc.weights_mb_str` MB) and writes output (`{python} FusionCalc.feat_map_kb_str` KB), totaling `{python} FusionCalc.unfused_conv_mb_str` MB. BN then reads that output, adds its parameters (`{python} FusionCalc.bn_params_kb_str` KB), and writes again, for `{python} FusionCalc.unfused_bn_mb_str` MB. ReLU repeats the pattern for another `{python} FusionCalc.unfused_relu_mb_str` MB. The total unfused memory traffic is `{python} FusionCalc.total_unfused_mb_str` MB. With fusion, the entire sequence reads input and weights once and writes the final output once, requiring only `{python} FusionCalc.total_fused_mb_str` MB—a `{python} FusionCalc.bandwidth_reduction_pct_str` percent bandwidth reduction.

On a V100 GPU with `{python} V100Specs.v100_bw_gbs_str` GB/s HBM bandwidth and `{python} V100Specs.v100_tflops_fp32_str` TFLOPS FP32 compute, the unfused sequence takes approximately `{python} FusionCalc.unfused_time_us_str` microseconds (memory bound), while the fused version takes approximately `{python} FusionCalc.fused_time_us_str` microseconds (`{python} FusionCalc.fusion_speedup_str`$\times$ speedup). The speedup comes entirely from reducing memory traffic, as the compute remains identical.
...
Dynamic Routing Networks, such as Capsule Networks (CapsNets), take a different approach: routing mechanisms dynamically choose the path that activations follow through the network. The router selects specific pathways based on input complexity, reducing the total operations required [@sabour2017dynamic]. Different routing strategies provide computational efficiency while preserving prediction quality.
...
[^fn-sparsity-hardware-gap]: **Sparsity**\index{Sparsity!L0 norm}\index{Sparsity!sparse matrix operations}\index{Sparsity!sparse gradient descent}\index{Sparsity!sparsity-aware training}\index{Sparsity!Monarch matrices}: From Latin *sparsus* (scattered), past participle of *spargere* (to scatter). The mathematical concept dates to the 1950s when researchers studying linear systems noticed most real-world matrices had predominantly zero entries. In ML, L1 regularization (the lasso [@tibshirani1996regression]) first exploited this by inducing exact zeros rather than merely small values. The persistent systems challenge: software can represent arbitrary sparsity patterns, but hardware acceleration requires structured patterns (for example, NVIDIA's 2:4 sparsity), creating a gap between theoretical compression and realized speedup. \index{Sparsity!etymology}
...
$$
where $\mathbf{1}_{\{T_{ij} = 0\}}$ is an indicator function that yields one if $T_{ij} = 0$ and 0 otherwise, and $\Vert \cdot \Vert_0$ represents the L0 norm, which counts the number of nonzero elements.
...
Sparsity-aware training complements these methods by helping models learn sparse representations during the training process itself. Sparse gradient descent, which updates only nonzero elements, reduces the number of active parameters throughout training rather than applying compression after the fact.
...
Reducing a model's FLOPs by 50 percent does not guarantee 50 percent latency reduction. Memory-bound operations (common in LLM inference and normalization layers) see minimal benefit from compute reduction because they are bottlenecked by data movement, not arithmetic. Critically, Amdahl's Law\index{Amdahl's Law!model compression} (@sec-machine-foundations-amdahls-law-gustafsons-law-b741) applies at the system level: if model inference accounts for only `{python} AmdahlCompression.model_fraction_pct_str` percent of end-to-end latency (with the remaining `{python} AmdahlCompression.non_model_pct_str` percent spent on data loading, preprocessing, and postprocessing), then even perfect model optimization yields at most `{python} AmdahlCompression.max_speedup_str`$\times$ overall speedup. Always profile on target hardware before estimating optimization benefits.
...
class ResNet50Int8Metrics:
    ...
    check(latency_speedup > cpu_speedup, "GPU speedup should exceed CPU speedup.")
    check(size_reduction > 3, "INT8 size reduction should be close to 4x.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fp32_top1_str = fmt(fp32_top1, precision=1, commas=False)
    int8_top1_str = fmt(int8_top1, precision=1, commas=False)
    fp32_latency_ms_str = fmt(fp32_latency_ms, precision=1, commas=False)
    int8_latency_ms_str = fmt(int8_latency_ms, precision=1, commas=False)
    fp32_size_mb_str = fmt(fp32_size_mb, precision=0, commas=False)
    int8_size_mb_str = fmt(int8_size_mb, precision=0, commas=False)
    fp32_energy_j_str = fmt(fp32_energy_j, precision=2, commas=False)
    int8_energy_j_str = fmt(int8_energy_j, precision=2, commas=False)
    ...
    calibration_error_fp32_str = fmt(calibration_error_fp32, precision=1, commas=False)
    calibration_error_int8_str = fmt(calibration_error_int8, precision=1, commas=False)
...
When quantizing ResNet-50 from FP32 to INT8, baseline metrics show Top-1 accuracy of `{python} ResNet50Int8Metrics.fp32_top1_str` percent, inference latency on V100 of `{python} ResNet50Int8Metrics.fp32_latency_ms_str` ms, model size of `{python} ResNet50Int8Metrics.fp32_size_mb_str` MB, and energy per inference of `{python} ResNet50Int8Metrics.fp32_energy_j_str` J. Postquantization metrics reveal Top-1 accuracy of `{python} ResNet50Int8Metrics.int8_top1_str` percent (`{python} ResNet50Int8Metrics.top1_drop_str` percent degradation), inference latency of `{python} ResNet50Int8Metrics.int8_latency_ms_str` ms (`{python} ResNet50Int8Metrics.latency_speedup_str`$\times$ speedup), model size of `{python} ResNet50Int8Metrics.int8_size_mb_str` MB (`{python} ResNet50Int8Metrics.size_reduction_str`$\times$ reduction), and energy per inference of `{python} ResNet50Int8Metrics.int8_energy_j_str` J (`{python} ResNet50Int8Metrics.energy_improvement_str`$\times$ improvement). Additional analysis shows per-class accuracy degradation ranging from `{python} ResNet50Int8Metrics.per_class_min_drop_str` percent to `{python} ResNet50Int8Metrics.per_class_max_drop_str` percent with highest impact on fine-grained categories, calibration error increasing from `{python} ResNet50Int8Metrics.calibration_error_fp32_str` percent to `{python} ResNet50Int8Metrics.calibration_error_int8_str` percent, and INT8 quantization providing `{python} ResNet50Int8Metrics.latency_speedup_str`$\times$ speedup on GPU but only `{python} ResNet50Int8Metrics.cpu_speedup_str`$\times$ on CPU, demonstrating hardware-dependent gains.
...
class FallaciesAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    int8_size_reduction_str = fmt(int(quant_speedup), precision=0, commas=False)
