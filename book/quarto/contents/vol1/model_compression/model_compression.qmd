...
**The context**: Early CNNs such as AlexNet and VGG delivered strong accuracy but carried storage and memory footprints that made mobile and embedded deployment difficult.

**The failure**: The uncompressed model treated every learned connection as equally expensive to store and move, even though many weights could be pruned, shared, or encoded more compactly without changing predictions.

**The consequence**: Deep Compression combined pruning, trained quantization, and Huffman coding to shrink AlexNet by 35 times and VGG-16 by 49 times without accuracy loss in the reported experiments.

**The systems lesson**: Compression is not cosmetic. It can move a model from off-chip DRAM traffic into cache-resident execution, changing the dominant term in the deployment cost equation [@han2015deep].
