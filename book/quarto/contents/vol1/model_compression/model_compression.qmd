...
class CompressionSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    llm_7b_str = fmt(llm_7b_params, precision=1, allow_zero=True, commas=False)
    llm_7b_mem_str = fmt(llm_7b_mem_fp16_gb, precision=1, allow_zero=True, commas=False)
    llm_175b_str = fmt(llm_175b_params, precision=1, allow_zero=True, commas=False)
    llm_175b_mem_str = fmt(llm_175b_mem_fp16_gb, precision=1, allow_zero=True, commas=False)
    llm_175b_mem_int8_str = fmt(llm_175b_mem_int8_gb, precision=1, allow_zero=True, commas=False)
    llm_175b_fp16_int8_reduction_str = fmt(llm_175b_fp16_int8_reduction, precision=1, allow_zero=True, commas=False)
    # smartphone_ram_gb and mcu_ram_kb are floats; canonical fmt would inject ".0" — escape-hatch
    smartphone_ram_str = fmt(smartphone_ram_gb, precision=1, allow_zero=True, commas=False)
    mcu_ram_str = fmt(mcu_ram_kb, precision=1, allow_zero=True, commas=False)
    energy_dram_str = fmt(energy_dram, precision=1, allow_zero=True, commas=False)
    energy_dram_read_bits_str = fmt(energy_dram_read_bits, precision=1, allow_zero=True, commas=False)
    energy_dram_per_byte_str = fmt(energy_dram_per_byte, precision=1, allow_zero=True, commas=False)
    ...
    energy_float_add_ratio_str = fmt(energy_float_add_ratio, precision=1, allow_zero=True, commas=False)
    energy_dram_read_ratio_str = fmt(energy_dram_read_ratio, precision=1, allow_zero=True)
    dram_read_vs_int8_mac_str = fmt(dram_read_vs_int8_mac, precision=1, allow_zero=True)
    int8_energy_reduction_str = fmt(int8_energy_reduction, precision=1, allow_zero=True, commas=False)
    mobilenet_int8_tiny_ratio_str = fmt(mobilenet_int8_tiny_ratio, precision=1, allow_zero=True, commas=False)
```
...
class NASCostCalc:
    ...
    gpu_days_str = fmt(gpu_days, precision=1, allow_zero=True)
    gpu_hours_str = fmt(gpu_hours, precision=1, allow_zero=True)
    weight_sharing_reduction_str = fmt(weight_sharing_reduction, precision=1, allow_zero=True)
```
...
class QuantizationSpeedup:
    ...
    bytes_fp16_str = fmt(int(bytes_fp16), precision=1, allow_zero=True, commas=False)
    bytes_int4_str = fmt(bytes_int4, precision=1, commas=False)
    device_ram_gb_str = fmt(device_ram_gb, precision=1, allow_zero=True, commas=False)
    mem_bw_gbs_str = fmt(int(mem_bw_gbs), precision=1, allow_zero=True, commas=False)
    kv_cache_gb_str = fmt(int(kv_cache_gb), precision=1, allow_zero=True, commas=False)

    fp16_size_str = fmt(int(fp16_size_gb), precision=1, allow_zero=True, commas=False)
    fp16_total_str = fmt(int(fp16_total_gb), precision=1, allow_zero=True, commas=False)
    fp16_latency_str = fmt(int(fp16_latency_ms), precision=1, allow_zero=True, commas=False)
    fp16_toks_str = fmt(fp16_toks, precision=1, commas=False)

    int4_size_str = fmt(int4_size_gb, precision=1, commas=False)
    int4_latency_str = fmt(int(int4_latency_ms), precision=1, allow_zero=True, commas=False)
    int4_toks_str = fmt(int(int4_toks), precision=1, allow_zero=True, commas=False)
    speedup_str = fmt(int(speedup), precision=1, allow_zero=True, commas=False)
```
...
# │          CLOUD_MEM_GIB, MOBILE_MEM_GIB, TINY_MEM_KIB, Models.Recommendation.DLRM.model_size)
...
class ModelDeviceComparison:
    ...
    dlrm_str = fmt(dlrm_mem.m_as(GB), precision=1, allow_zero=True, commas=False, suffix=" GB")
    gpt2_str = fmt(gpt2_mem.m_as(GB), precision=1, allow_zero=True, commas=False, suffix=" GB")
    resnet_str = fmt(resnet_mem.m_as(MB), precision=1, allow_zero=True, commas=False, suffix=" MB")
    mobilenet_str = fmt(mobilenet_mem.m_as(MB), precision=1, allow_zero=True, commas=False, suffix=" MB")
    mobilenet_int8_str = fmt(mobilenet_int8_mem.m_as(MB), precision=1, commas=False, suffix=" MB")
    dscnn_str = fmt(dscnn_mem.m_as(KB), precision=1, allow_zero=True, commas=False, suffix=" KB")
    cloud_cap_str = fmt(cloud_mem.m_as(GB), precision=1, allow_zero=True, commas=False, prefix="~", suffix=" GB")
    mobile_cap_str = fmt(mobile_mem.m_as(GB), precision=1, allow_zero=True, commas=False, prefix="~", suffix=" GB")
    tiny_cap_str = fmt(tiny_mem.m_as(KB), precision=1, allow_zero=True, commas=False, prefix="~", suffix=" KB")
...
class MobileNetCompressionAnchor:
    ...
    pruning_pct_str = fmt(pruning_pct, precision=1, allow_zero=True, commas=False, suffix="%")
    original_size_str = fmt(original_size_mb, precision=1, allow_zero=True, commas=False, suffix=" MB")
    pruned_size_str = fmt(pruned_size_mb, precision=1, allow_zero=True, commas=False, suffix=" MB")
```
...
class PruningPracticeAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pruning_pct_str = f"{fmt(pruning_pct, precision=1, allow_zero=True, commas=False)}%"
    original_size_str = fmt(original_size_mb, precision=1, allow_zero=True, commas=False, suffix=" MB")
    pruned_size_str = fmt(pruned_size_mb, precision=1, allow_zero=True, commas=False, suffix=" MB")
```
...
class LowRankFactorization:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mat_dim_str = fmt(mat_dim, precision=1, allow_zero=True, commas=False)
    rank_k_str = fmt(rank_k, precision=1, allow_zero=True, commas=False)
    full_mb_str = fmt(full_mb, precision=1, allow_zero=True, commas=False)
    factored_mb_str = fmt(factored_mb, precision=1, commas=False)
    data_reduction_str = fmt(data_reduction, precision=1, allow_zero=True, commas=False)
```
...
class QuantizationMemoryAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    llm_7b_str = fmt(llm_7b_params_b, precision=1, allow_zero=True, commas=False)
    llm_7b_mem_str = fmt(llm_7b_mem_fp16_gb, precision=1, allow_zero=True, commas=False)
    smartphone_ram_str = fmt(smartphone_ram_gb, precision=1, allow_zero=True, commas=False)
```
...
class EnergyDividend:
    ...
    energy_dividend_str = fmt(int(dividend), precision=1, allow_zero=True, commas=False)
    energy_dram_str = fmt(e_dram, precision=1, allow_zero=True, commas=False)
    dram_read_bits_str = fmt(dram_read_bits, precision=1, allow_zero=True, commas=False)
    dram_read_vs_int8_mac_str = fmt(dram_read_vs_int8_mac, precision=1, allow_zero=True)
```
...
class QuantizationSavings:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    llm_params_b_str = fmt(params_b, precision=1, allow_zero=True, commas=False)
    fp16_bytes_str = fmt(int(bytes_fp16), precision=1, allow_zero=True, commas=False)
    int4_bytes_str = fmt(bytes_int4, precision=1, commas=False)
    fp16_size_gb_str = fmt(int(fp16_size_gb), precision=1, allow_zero=True, commas=False)
    int4_size_gb_str = fmt(int(int4_size_gb), precision=1, allow_zero=True, commas=False)
    compression_ratio_str = fmt(int(ratio), precision=1, allow_zero=True, commas=False)
```
...
class SIMDThroughput:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    simd_fp32_str = fmt(ops_fp32, precision=1, allow_zero=True, commas=False)
    simd_int8_str = fmt(ops_int8, precision=1, allow_zero=True, commas=False)
    simd_gain_str = fmt(gain, precision=1, allow_zero=True, commas=False)
    simd_register_bits_str = fmt(register_bits, precision=1, allow_zero=True, commas=False)
    simd_fp32_bits_str = fmt(fp32_bits, precision=1, allow_zero=True, commas=False)
    simd_int8_bits_str = fmt(int8_bits, precision=1, allow_zero=True, commas=False)
```
...
class A100Int8Speedup:
    ...
    a100_tops_int8_str = fmt(a100_tops_int8, precision=1, allow_zero=True, commas=False)
    a100_int8_speedup_str = fmt(a100_int8_speedup, precision=1, allow_zero=True, commas=False)
    bandwidth_bound_speedup_str = fmt(bandwidth_bound_speedup, precision=1, allow_zero=True, commas=False)
```
...
class QuantizationMathCalc:
    ...
    steps_str = fmt(int_steps, precision=1, allow_zero=True, commas=False)       # "255"
    scale_str = fmt(scale, precision=4, commas=False)           # "0.0157"
    zero_point_str = fmt(int(zero_point), precision=1, allow_zero=True, commas=False)  # "64"
    x_val_str = fmt(x_val, precision=1, commas=False)           # "0.0"
    x_q_str = fmt(int(x_q), precision=1, allow_zero=True, commas=False)          # "64"
...
class TheoryPracticeSpeedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    pruning_speedup_str = fmt(pruning_speedup, precision=1, allow_zero=True, commas=False)
    quant_speedup_str = fmt(quant_speedup, precision=1, allow_zero=True, commas=False)
    theoretical_speedup_str = fmt(theoretical_speedup, precision=1, allow_zero=True, commas=False)
    measured_speedup_str = fmt(measured_speedup, precision=1, commas=False)
    realized_fraction_pct_str = fmt(realized_fraction_pct, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.V100.memory.bandwidth, Hardware.Cloud.V100.compute.precision_flops["fp32"], GB, TFLOPs, second)
...
class V100Specs:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    v100_bw_gbs_str = fmt(v100_bw_gbs, precision=1, allow_zero=True, commas=False)
...
class FusionCalc:
    ...
    feat_map_kb_str = fmt(feat_map_mb * THOUSAND, precision=1, allow_zero=True, commas=False)   # MB → KB
    weights_mb_str = fmt(weights_mb, precision=1, commas=False)
    bn_params_kb_str = fmt(bn_params_mb * THOUSAND, precision=1, allow_zero=True, commas=False)  # MB → KB (was wrongly using KIB_TO_BYTES = 1024)
    ...
    bandwidth_reduction_pct_str = fmt(bandwidth_reduction_pct, precision=1, allow_zero=True, commas=False)
    kernels_unfused_str = fmt(kernels_unfused, precision=1, allow_zero=True, commas=False)
    kernels_fused_str = fmt(kernels_fused, precision=1, allow_zero=True, commas=False)
    saved_latency_ms_str = fmt(saved_latency_ms, precision=1, allow_zero=True, commas=False)
    unfused_time_us_str = fmt(unfused_time_us, precision=1, allow_zero=True, commas=False)
...
class ConvFusionCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    transfer_reduction_str = fmt(transfer_reduction, precision=1, allow_zero=True, commas=False)
...
class AmdahlCompression:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    model_fraction_pct_str = fmt(model_fraction * 100, precision=1, allow_zero=True, commas=False)
    non_model_pct_str = fmt(non_model_fraction * 100, precision=1, allow_zero=True, commas=False)
...
class ResNet50Int8Metrics:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # Use Engine to solve for both precisions
    ...
    fp32_size_mb_str = fmt(fp32_size_mb, precision=1, allow_zero=True, commas=False)
    int8_size_mb_str = fmt(int8_size_mb, precision=1, allow_zero=True, commas=False)
...
class FallaciesAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    int8_size_reduction_str = fmt(int(quant_speedup), precision=1, allow_zero=True, commas=False)
    expected_speedup_str = fmt(combined_expected, precision=1, allow_zero=True, commas=False)
    actual_speedup_str = fmt(actual_combined_pct, precision=1, allow_zero=True, commas=False)
    dequant_overhead_str = fmt(overhead_pct, precision=1, allow_zero=True, commas=False)
