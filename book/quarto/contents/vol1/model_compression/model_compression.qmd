...
2.  **Compute work $(O \times E_{\text{compute}})$**: A 32-bit FLOP costs ≈ **`{python} CompressionSetup.energy_flop_fp32_str` pJ**. An INT8 operation costs ≈ **`{python} CompressionSetup.energy_flop_int8_str` pJ**.
...
4.  **Bandwidth cost**: Loading `{python} QuantizationSpeedup.fp16_size_str` GB at `{python} QuantizationSpeedup.mem_bw_gbs_str` GB/s takes **`{python} QuantizationSpeedup.fp16_latency_str` ms** per token. That is `{python} QuantizationSpeedup.fp16_toks_str` tokens/s, too slow for chat.
...
3.  **New speed**: Loading `{python} QuantizationSpeedup.int4_size_str` GB takes **`{python} QuantizationSpeedup.int4_latency_str` ms**. Speed jumps to **`{python} QuantizationSpeedup.int4_toks_str` tokens/s**.
...
# │ How: Calculate the ratio of energy per operation using Horowitz (2014) numbers.
# │
