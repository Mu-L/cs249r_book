...
from mlsysim.core.constants import (
    ...
    Bparam, GB, MB, KB, TINY_MEM_KIB, byte, ureg
)
...
class CompressionSetup:
    ...
    mcu_ram_kb = MCU_RAM_KIB.m_as(KB)
...
1.  **Memory movement $(D_{\text{vol}} \times E_{\text{move}})$**: Fetching a 32-bit float from DRAM costs ≈ **`{python} CompressionSetup.energy_dram_str` pJ**. Fetching an 8-bit integer costs ≈ **`{python} CompressionSetup.energy_dram_per_byte_str` pJ**.
2.  **Compute work $(O \times E_{\text{compute}})$**: A 32-bit FLOP costs ≈ **`{python} CompressionSetup.energy_flop_fp32_str` pJ**. An 8-bit integer OP costs ≈ **`{python} CompressionSetup.energy_flop_int8_str` pJ**.
...
**Problem**: A deployment scenario calls for running a `{python} QuantizationSpeedup.params_b_str`-billion-parameter LLM on a device with `{python} QuantizationSpeedup.device_ram_gb_str` GB RAM. The weights are FP16 (`{python} QuantizationSpeedup.bytes_fp16_str` bytes).
...
| **Mobile/Edge** | hundreds of MB to GB |   10–50 ms  | Watts     | Size, latency    |
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (GB, MB, KB, byte,
...
class ModelDeviceComparison:
    ...
    gpt2_mem = 6 * GB                # 1.5 billion params × 4 bytes = 6 GB
    ...
    cloud_cap_str = fmt(cloud_mem.m_as(GB), precision=0, commas=False, prefix="~", suffix=" GB")
    mobile_cap_str = fmt(mobile_mem.m_as(GB), precision=0, commas=False, prefix="~", suffix=" GB")
    tiny_cap_str = fmt(tiny_mem.m_as(KB), precision=0, commas=False, prefix="~", suffix=" KB")
...
[^fn-distillation-chemistry]: **Distillation**: Borrowed from chemistry, where distillation separates mixtures by selective evaporation, extracting the essence while leaving impurities behind. @hinton2015distilling introduced temperature-scaled softmax to control how much "dark knowledge" about class relationships the student absorbs—the temperature parameter $\tau_{\text{distill}}$ mirrors literal temperature in chemical distillation. The metaphor captures the systems trade-off precisely: distillation moves complexity from inference compute to training compute, producing a model 2--10$\times$ smaller at the cost of a one-time training budget to generate the teacher's soft targets. \index{Knowledge Distillation!etymology}
...
%%
\node[Box2, right= 1.3of GB3,fill=OliveL,draw=OliveLine](S1){Softmax ($\tau=t$)};
\node[Box2,above right=0 and 1.3 of B3,fill=OliveL,draw=OliveLine](S2){Softmax ($\tau=t$)};
\node[Box2,below right=0 and 1.3 of B3,fill=OliveL,draw=OliveLine](S3){Softmax ($\tau=1$)};
%
...
Starting from the softmax normalization in @sec-neural-computation-softmax-ebe5, we use a **temperature parameter**\index{Knowledge Distillation!temperature parameter}[^fn-temperature-boltzmann] $\tau_{\text{distill}}$ to soften the probability distribution. The softmax output for class $i$ becomes:

[^fn-temperature-boltzmann]: **Temperature (Softmax)**: Borrowed from statistical mechanics, where the Boltzmann distribution $p_i \propto \exp(-E_i/k\tau_{\text{distill}})$ describes particle states at temperature $\tau_{\text{distill}}$—higher temperature means more uniform distribution across states. The analogy is load-bearing: at $\tau_{\text{distill}}{=}1$ (standard softmax), the teacher's output is a near-one-hot vector carrying almost no inter-class information; at $\tau_{\text{distill}}{=}3$--$5$, the distribution softens enough to reveal which wrong classes the teacher considers plausible. This temperature tuning directly controls the bandwidth of information transferred from teacher to student, making it the primary hyperparameter governing distillation quality. \index{Temperature!softmax distillation}
$$
p_i^{(\tau_{\text{distill}})} = \frac{\exp(z_i/\tau_{\text{distill}})}{\sum_j \exp(z_j/\tau_{\text{distill}})}
$$

\index{Cross-Entropy Loss!student loss}
\index{Kullback-Leibler Divergence!etymology}
A higher $\tau_{\text{distill}}$ (typically three to 5) produces a smoother distribution, allowing the student to learn from the "uncertainty" the teacher assigns to incorrect classes. The total loss $\mathcal{L}_{\text{distill}}$ balances standard cross-entropy with the KL divergence:
$$
\mathcal{L}_{\text{distill}} = (1 - \gamma_{\text{KD}}) \mathcal{L}_{\text{CE}}(\mathbf{p}_{\text{student}}, y) + \gamma_{\text{KD}} \tau_{\text{distill}}^2 D_{\text{KL}}(\mathbf{p}_{\text{teacher}}^{(\tau_{\text{distill}})} \lVert \mathbf{p}_{\text{student}}^{(\tau_{\text{distill}})})
$$

Here $\mathbf{p}_{\text{teacher}}^{(\tau_{\text{distill}})}$ and $\mathbf{p}_{\text{student}}^{(\tau_{\text{distill}})}$ are the teacher and student probability distributions computed with $\tau_{\text{distill}}$, $y$ is the hard label, and $\gamma_{\text{KD}} \in [0,1]$ weights the hard-label and distillation terms. The factor $\tau_{\text{distill}}^2$ ensures that gradient scales remain consistent when $\tau_{\text{distill}}$ is changed. This hybrid approach enables compact models (like DistilBERT) to achieve up to 97 percent of their teacher's performance with a fraction of the memory and compute.
...
# │ Show:    FP16 vs INT8 TFLOP/s and the resulting speedup factor.
...
1. **Tensor Core throughput**: NVIDIA A100 delivers `{python} A100Int8Speedup.a100_tflops_fp16_str` TFLOP/s for FP16 vs. `{python} A100Int8Speedup.a100_tflops_int8_str` TOPS for INT8, a `{python} A100Int8Speedup.a100_int8_speedup_str`$\times$ peak throughput increase.
...
[^fn-awq-salient-weights]: **Activation-aware Weight Quantization (AWQ)**: Salience is determined by activation magnitude, not weight magnitude—a distinction that matters because a small weight multiplied by a large activation produces a large output contribution. Protecting these ~1 percent of weights in higher precision while quantizing the rest to four-bit reduces a 7-billion-parameter model from ~14 GB to ~3.5 GB, converting what would be a memory-bandwidth-bound bottleneck into a compute-bound operation on modern accelerators. \index{AWQ!bandwidth reduction}
...
# │ Show:    V100 HBM bandwidth (GB/s) and FP32 compute (TFLOP/s).
...
$$
where $\mathbf{1}_{\{(\mathbf{W}_{\text{sparse}})_{ij} = 0\}}$ is an indicator function that yields one if entry $(i,j)$ is zero and 0 otherwise, and $\Vert \cdot \Vert_0$ represents the L0 norm, which counts the number of nonzero elements.
