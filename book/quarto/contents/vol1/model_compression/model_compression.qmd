...
[^fn-nas-search-cost]: **Neural Architecture Search (NAS)**: @zoph2017neural at Google Brain used reinforcement learning to learn the architecture itself at a cost of `{python} NASCostCalc.gpu_days_str` GPU-days (`{python} NASCostCalc.gpus_str` for `{python} NASCostCalc.days_str` days), equivalent to `{python} NASCostCalc.gpu_hours_str` GPU-hours. Weight-sharing approaches later reduced this by `{python} NASCostCalc.weight_sharing_reduction_str`$\times$, transforming NAS from a data-center-scale experiment into a practical tool whose one-time search budget is amortized across every deployment of the resulting architecture (EfficientNet, MobileNetV3). \index{NAS!search cost}
...
4.  **Bandwidth cost**: Loading `{python} QuantizationSpeedup.fp16_size_str` at `{python} QuantizationSpeedup.mem_bw_gbs_str` GB/s takes **`{python} QuantizationSpeedup.fp16_latency_str`** per token. That is `{python} QuantizationSpeedup.fp16_toks_str` tokens/s, too slow for chat.
...
3.  **New speed**: Loading `{python} QuantizationSpeedup.int4_size_str` takes **`{python} QuantizationSpeedup.int4_latency_str`**. Speed jumps to **`{python} QuantizationSpeedup.int4_toks_str` tokens/s**.
...
Memory bandwidth analysis quantifies these fusion benefits concretely. Consider a Conv-BN-ReLU sequence operating on a $28{\times}28{\times}256$ feature map (`{python} FusionCalc.feat_map_kb_str`). Without fusion, each operation performs its own memory round-trip: Conv reads input (`{python} FusionCalc.feat_map_kb_str`) plus weights (`{python} FusionCalc.weights_mb_str`) and writes output (`{python} FusionCalc.feat_map_kb_str`), totaling `{python} FusionCalc.unfused_conv_mb_str`. BN then reads that output, adds its parameters (`{python} FusionCalc.bn_params_kb_str`), and writes again, for `{python} FusionCalc.unfused_bn_mb_str`. ReLU repeats the pattern for another `{python} FusionCalc.unfused_relu_mb_str`. The total unfused memory traffic is `{python} FusionCalc.total_unfused_mb_str`. With fusion, the entire sequence reads input and weights once and writes the final output once, requiring only `{python} FusionCalc.total_fused_mb_str` MB—a `{python} FusionCalc.bandwidth_reduction_pct_str` bandwidth reduction.
...
When quantizing ResNet-50 from FP32 to INT8, baseline metrics show Top-1 accuracy of `{python} ResNet50Int8Metrics.fp32_top1_str`, inference latency on V100 of `{python} ResNet50Int8Metrics.fp32_latency_ms_str`, model size of `{python} ResNet50Int8Metrics.fp32_size_mb_str`, and energy per inference of `{python} ResNet50Int8Metrics.fp32_energy_j_str` J. Postquantization metrics reveal Top-1 accuracy of `{python} ResNet50Int8Metrics.int8_top1_str` (`{python} ResNet50Int8Metrics.top1_drop_str` degradation), inference latency of `{python} ResNet50Int8Metrics.int8_latency_ms_str` (`{python} ResNet50Int8Metrics.latency_speedup_str`$\times$ speedup), model size of `{python} ResNet50Int8Metrics.int8_size_mb_str` (`{python} ResNet50Int8Metrics.size_reduction_str`$\times$ reduction), and energy per inference of `{python} ResNet50Int8Metrics.int8_energy_j_str` J (`{python} ResNet50Int8Metrics.energy_improvement_str`$\times$ improvement). Additional analysis shows per-class accuracy degradation ranging from `{python} ResNet50Int8Metrics.per_class_min_drop_str` to `{python} ResNet50Int8Metrics.per_class_max_drop_str` with highest impact on fine-grained categories, calibration error increasing from `{python} ResNet50Int8Metrics.calibration_error_fp32_str` to `{python} ResNet50Int8Metrics.calibration_error_int8_str`, and INT8 quantization providing `{python} ResNet50Int8Metrics.latency_speedup_str`$\times$ speedup on GPU but only `{python} ResNet50Int8Metrics.cpu_speedup_str`$\times$ on CPU, demonstrating hardware-dependent gains.
