...
from mlsysim.core.constants import (
    ...
    Bparam, GB, MB, KiB, TINY_MEM_KIB, byte, ureg
)
...
class CompressionSetup:
    ...
    mobilenet_int8_tiny_ratio = mobilenet_int8_mem.m_as(byte) / TINY_MEM_KIB.m_as(byte)
    ...
    check(6 <= mobilenet_int8_tiny_ratio <= 8,
          "MobileNetV2 INT8 should exceed TinyML memory by about 7x.")
...
| **DRAM Read**   | `{python} CompressionSetup.energy_dram_read_bits_str`-bit | **`{python} CompressionSetup.energy_dram_read_ratio_str`$\times$** |

: **The Energy Hierarchy**: Relative energy cost of representative arithmetic and memory operations, normalized to an 8-bit integer add. A 32-bit float add costs `{python} CompressionSetup.energy_float_add_ratio_str`$\times$ more than its 8-bit integer counterpart, but a single `{python} CompressionSetup.energy_dram_read_bits_str`-bit DRAM read costs `{python} CompressionSetup.energy_dram_read_ratio_str`$\times$ more—more than four orders of magnitude above on-chip compute. This gap is why bit-width reductions that shrink data movement dominate the energy budget of inference far more than the arithmetic savings alone would suggest. {#tbl-model-compression-energy-hierarchy}
...
::: {#ws-model-compression-deep-compression .callout-war-story title="The model that fit after compression"}

**Context**: Early CNNs such as AlexNet and VGG delivered strong accuracy but carried storage and memory footprints that made mobile and embedded deployment difficult [@han2016deepcompression].

**Failure**: The uncompressed model treated every learned connection as equally expensive to store and move, even though many weights could be pruned, shared, or encoded more compactly without changing predictions.

**Consequence**: Deep Compression combined pruning, trained quantization, and Huffman coding to shrink AlexNet by 35 times and VGG-16 by 49 times without accuracy loss in the reported experiments.

**Systems lesson**: Compression is not cosmetic. It can move a model from off-chip DRAM traffic into cache-resident execution, changing the dominant term in the deployment cost equation.

:::

The same compression logic appears in everyday product constraints when the model must run on commodity mobile hardware.
...
from mlsysim.core.constants import (
    ENERGY_ADD_FP32_PJ, ENERGY_ADD_INT8_PJ,
    ENERGY_DRAM_ACCESS_PJ, ENERGY_DRAM_PJ_PER_BYTE, ENERGY_FLOP_INT8_PJ, ureg
)
from mlsysim.fmt import fmt, check
...
::: {#lhs-model-compression-keyword-spotting-extreme-compression .callout-lighthouse title="Keyword spotting and extreme compression"}
...
# │
# │ Imports: mlsysim.book (fmt)
# │ Exports: gemm_intermediate_mb_str,
...
class FusionCalc:
    ...
    # Step 2: Conv-BN-ReLU intermediate traffic eliminated (2 intermediate maps × read+write)
    conv_bn_relu_eliminated_traffic_mb = (
...
class ConvFusionCalc:
    ...
    conv_bn_relu_mem_math = fmt_math(
        f"4 \\times 256 \\times 28 \\times 28 \\times 4 \\text{{ bytes}} \\approx \\text{{{FusionCalc.conv_bn_relu_eliminated_traffic_mb:.1f} MB}}"
    )
```
...
More formally, for a matrix $T \in \mathbb{R}^{m \times n}$, the sparsity $S$ can be expressed as:
$$
