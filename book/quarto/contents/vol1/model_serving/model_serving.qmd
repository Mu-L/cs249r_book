...
- Throughput: `{python} ResNetServingSpectrum.cloud_throughput_str`/second (batched)
...
- Throughput: ~`{python} ResNetServingSpectrum.mobile_throughput_str`/second (single-stream)
...
- Throughput: ~`{python} ResNetServingSpectrum.tiny_throughput_str`/second
...
Single V100 throughput at $B = 16$: `{python} CapacityPlanningCalc.cp_v100_throughput_str`/second
...
For a `{python} ModelSwapCalc.model_size_gb_str` model on PCIe Gen4 x16 (`{python} ModelSwapCalc.pcie_bw_gbs_str`/s theoretical bandwidth), loading takes at least:
...
# │
# │ Imports: mlsysim.core.constants (ms, second), mlsysim.book (fmt)
# │ Exports: batch1_ms_str, batch1_imgs_str, batch8_* strings,
# │          latency_increase_str, throughput_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
...
class BatchingSweetspotCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    batch1_ms = 5.0 * ms
    batch1_imgs = 200 * count / second
    batch8_wait_ms = 5.0 * ms
    batch8_inference_ms = 9.0 * ms

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    batch8_user_latency_ms = batch8_wait_ms + batch8_inference_ms
    batch8_throughput = (8 * count) / batch8_user_latency_ms.to(second)
    latency_increase_value = batch8_user_latency_ms / batch1_ms
    throughput_ratio_value = batch8_throughput / batch1_imgs
    batch1_util_pct_value = 15
    latency_budget_ms_value = 20

    check(batch8_user_latency_ms.m_as(ms) == 14, "Batch-8 user latency should be 14 ms")
    check(
        abs(throughput_ratio_value - 40 / 14) < 0.01,
        f"Batch-8 throughput gain should be about 2.9×, got {throughput_ratio_value:.2f}×",
    )

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    batch1_ms_str = fmt_qty(batch1_ms, ms, precision=0, commas=False)
    batch1_imgs_str = fmt_int(round(batch1_imgs.m_as(count / second)), commas=False, suffix=" img/s")
    batch8_wait_ms_str = fmt_qty(batch8_wait_ms, ms, precision=0, commas=False)
    batch8_inference_ms_str = fmt_qty(batch8_inference_ms, ms, precision=0, commas=False)
    batch8_user_latency_str = fmt_qty(batch8_user_latency_ms, ms, precision=0, commas=False)
    batch8_throughput_str = fmt_int(round(batch8_throughput.m_as(count / second)), commas=False, suffix=" img/s")
    latency_increase_str = fmt(latency_increase_value, precision=1, commas=False, suffix="×")
    ...
    latency_budget_ms_str = fmt(latency_budget_ms_value, precision=0, commas=False, suffix=" ms")
```
...
1.  **Baseline (Batch 1)**: Inference = **`{python} BatchingSweetspotCalc.batch1_ms_str`**. Throughput = **`{python} BatchingSweetspotCalc.batch1_imgs_str`**.
2.  **Optimized (Batch 8)**:
    ...
    - **Throughput**: eight img / `{python} BatchingSweetspotCalc.batch8_user_latency_str` ≈ **`{python} BatchingSweetspotCalc.batch8_throughput_str`**.

**Systems insight**: By accepting a **`{python} BatchingSweetspotCalc.latency_increase_str` increase in latency** (`{python} BatchingSweetspotCalc.batch1_ms_str` → `{python} BatchingSweetspotCalc.batch8_user_latency_str`), the system achieves nearly **`{python} BatchingSweetspotCalc.throughput_ratio_str` higher throughput** on the same hardware. As long as `{python} BatchingSweetspotCalc.batch8_user_latency_str` remains under the `{python} BatchingSweetspotCalc.latency_budget_ms_str` budget, this is "free" capacity. This trade-off is the primary lever of serving economics.
...
class CarbonCostCalc:
    ...
    cc_tokens_req_str = fmt(cc_tokens_per_sec_req_value, precision=0, commas=False, suffix="/s")
...
1.  **Throughput**: `{python} CarbonCostCalc.cc_concurrent_str` concurrent requests $\times$ `{python} CarbonCostCalc.cc_tokens_req_str`/req ≈ **`{python} CarbonCostCalc.cc_total_tokens_str`**.
...
[^fn-layer-fusion-serving]: **Layer Fusion**: Analogous to loop fusion in compiler optimization, where adjacent loops over the same array are combined to reduce memory traffic. Kernel fusion applies the identical principle to GPU operations: sequential kernels that write and re-read intermediate tensors from HBM are merged into a single kernel that keeps data in registers. The savings compound---a typical ResNet-50 has ~35 fusible operation pairs, and each eliminated HBM round-trip saves 1--3 $\mu$s at `{python} LayerFusionH100Bw.h100_bw_tbs_str`/s bandwidth, converting memory-bound chains into compute-bound fused kernels. \index{Layer Fusion!compiler analogy}
...
*   **Hardware**: 1$\times$ NVIDIA H100 SXM5 GPU (`{python} LlmCaseStudyHwSpecs.h100_mem_str` HBM3, `{python} LlmCaseStudyH100BwRecap.h100_bw_tbs_str`/s bandwidth).
...
Every token generation pays a massive "energy tax" to move the model's logic from HBM into compute registers. For comparison, on an A100 `{python} LlmCaseStudyHwSpecs.a100_mem_str` (`{python} LlmCaseStudyHwSpecs.a100_bw_tbs_str`/s HBM2e), an 8-billion-parameter Llama 3 model (`{python} LlmCaseStudyHwSpecs.case_study_model_weight_gb_str` INT4) generates tokens at $\approx$ `{python} LlmCaseStudyHwSpecs.a100_token_ms_str`/token. Adding more *compute cores* yields **zero** latency improvement; only *faster memory* (Physics) or *smaller models* (Algorithm) can speed up generation.
...
class LlmServingCalc:
    ...
    kv_per_token_mb_str = fmt_qty(kv_per_token, MB, precision=5, commas=False)
    kv_capacity_tokens_str = fmt_int(kv_capacity_tokens_value, commas=True)
...
*   $T_{\text{token}} \approx$ `{python} LlmServingCalc.model_weight_gb_str` / `{python} LlmCaseStudyH100BwRecap.h100_bw_tbs_str`/s ≈ `{python} LlmServingCalc.token_time_theoretical_ms_str` (theoretical limit).
...
*   Total cache capacity ≈ `{python} LlmServingCalc.kv_cache_gb_str` / `{python} LlmServingCalc.kv_per_token_mb_str` ≈ **`{python} LlmServingCalc.kv_capacity_tokens_str`** tokens.
