...
@Tbl-model-serving-resnet50-latency-budget breaks a typical ResNet-50 serving request down per phase. Preprocessing consumes `{python} LatencyBudgetCalc.preprocess_pct_str` of latency despite model inference being the computationally intensive phase. With TensorRT optimization reducing inference to `{python} LatencyBudgetCalc.tensorrt_inference_ms_str`, preprocessing would dominate at `{python} LatencyBudgetCalc.tensorrt_preprocess_pct_str`.
...
@Tbl-model-serving-resnet-coldstart traces the per-phase duration of a ResNet-50 cold start, from weight loading and CUDA context creation through TensorRT compilation and warmup.
...
@Tbl-model-serving-batching-throughput-tradeoff illustrates the throughput-latency trade-off for ResNet-50 on a V100 GPU across batch sizes one through thirty-two. Times shown are pure inference time, excluding queue wait; @sec-model-serving-traffic-patterns-batching-strategy-2e6b analyzes how user-perceived latency includes batching window wait.
