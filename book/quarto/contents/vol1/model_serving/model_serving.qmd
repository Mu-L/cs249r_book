...
class SerializationEfficiency:
    ...
    json_parse_str = fmt_int(json_parse_us, commas=False)
    protobuf_size_str = MarkdownStr("4")
    protobuf_parse_str = fmt_int(proto_parse_us, commas=False)
...
class CapacityPlanning:
    ...
    littles_w_ms_str = fmt_int(latency_slo_s * MS_PER_SEC, commas=False)
...
class BatchingTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    wait_time_b1_ms_str = fmt_int(w_form_b1, commas=False)
    wait_time_b32_ms_str = fmt_int(w_form_b32, commas=False)
    lat_b1_ms_str = fmt(lat_b1, precision=0, commas=False)
    lat_b32_ms_str = fmt(lat_b32, precision=0, commas=False)
    penalty_ratio_str = fmt_int(penalty_ratio, commas=False)
```
...
class CapacityPlanningCalc:
    ...
    cp_gpus_raw_str = fmt_int(cp_gpus_raw_value, commas=False)
...
class BatchingOptimization:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    s1_window_ms_str = fmt_int(s1_window, commas=False)
    ...
    s1_throughput_str = fmt_int(s1_tput)

    s2_window_ms_str = fmt_int(s2_window, commas=False)
    ...
    s2_throughput_str = fmt_int(s2_tput)
    ...
    latency_p99_increase_ms_str = fmt_int(s2_window - s1_window, commas=False)
```
...
class SloViolationCalc:
    ...
    mean_service_str = fmt(mean_service_value, precision=0, commas=False)
...
class MobileServingCalc:
    ...
    m_total_mj_str = fmt_int(round(m_total_mj_value), commas=False, suffix=" mJ")
```
...
- **Energy per inference**: `{python} MobileServingCalc.m_total_mj_str` enables ~12 million inferences per 10 Wh battery (typical smartphone)
- **Thermal budget**: At `{python} MobileServingCalc.m_total_mj_str`/45 ms = 67 mW sustained, indefinite operation without throttling
...
class CarbonCostCalc:
    ...
    cc_tokens_req_str = fmt_int(cc_tokens_per_sec_req_value, commas=False)
...
class LlmServingCalc:
    ...
    tokens_per_req_str = fmt(total_tokens, precision=0)
    ...
    decode_req_capacity_str = fmt_int(round(decode_req_capacity_value), commas=False)
    hourly_cost_str = fmt(hourly_cost_value, precision=0, commas=False)
...
class FallacySkewCalc:
    ...
    fs_resize_drop_str = MarkdownStr(f"{fs_resize_drop_min_value}-1")
```
...
class FallacyBatchingCalc:
    ...
    fb_throughput_gain_str = fmt_int(round(fb_throughput_gain_pct_value), commas=False)
