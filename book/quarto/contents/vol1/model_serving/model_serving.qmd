...
from mlsysim.fmt import fmt_int,  MarkdownStr
...
2.  **Power**: `{python} CarbonCostH100Tdp.h100_tdp_str` W (GPU) + `{python} CarbonCostCalc.cc_host_overhead_str` W (Host/Overhead) = **`{python} CarbonCostCalc.cc_total_power_str` W**.
3.  **Energy per Token**:
...
[^fn-layer-fusion-serving]: **Layer Fusion**: Analogous to loop fusion in compiler optimization, where adjacent loops over the same array are combined to reduce memory traffic. Kernel fusion applies the identical principle to GPU operations: sequential kernels that write and re-read intermediate tensors from HBM are merged into a single kernel that keeps data in registers. The savings compound---a typical ResNet-50 has ~35 fusible operation pairs, and each eliminated HBM round-trip saves 1--3 $\mu$s at `{python} LayerFusionH100Bw.h100_bw_tbs_str` TB/s bandwidth, converting memory-bound chains into compute-bound fused kernels. \index{Layer Fusion!compiler analogy}
...
The worked example in @sec-model-serving-queuing-theory-tail-latency-29a6 demonstrates the complete workflow: starting from a 50 ms p99 SLO and 5,000 QPS target, deriving the conservative M/M/1 safe utilization threshold of `{python} CapacityPlanningRecap.cp_rho_safe_pct_str` percent from @eq-p99-latency, and determining GPU count with headroom of `{python} CapacityPlanningRecap.cp_final_ceil_str` V100s. Production systems typically provision for peak load plus 30 percent headroom, using auto-scaling to reduce costs during low-traffic periods while meeting latency objectives during peaks. The key insight from capacity planning is that throughput numbers are meaningful only when coupled with latency guarantees: a system achieving 10,000 QPS but violating the p99 SLO on 5 percent of requests is actually serving 9,500 valid QPS and failing on the rest.
...
*   **Hardware**: 1$\times$ NVIDIA H100 SXM5 GPU (`{python} LlmCaseStudyHwSpecs.h100_mem_str` GB HBM3, `{python} LlmCaseStudyH100BwRecap.h100_bw_tbs_str` TB/s bandwidth).
...
*   $T_{\text{token}} \approx$ `{python} LlmServingCalc.model_weight_gb_str` GB / `{python} LlmCaseStudyH100BwRecap.h100_bw_tbs_str` TB/s ≈ `{python} LlmServingCalc.token_time_theoretical_ms_str` ms (theoretical limit).
