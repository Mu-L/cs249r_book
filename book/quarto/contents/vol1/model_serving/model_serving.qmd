...
class BlackFridayCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bf_latency_ms_str = fmt(bf_latency_ms_value, precision=1, allow_zero=True, commas=False)              # e.g. "50" ms
    bf_qps_normal_str = fmt(bf_qps_normal_value, precision=1, allow_zero=True)            # e.g. "1,000" QPS
    bf_qps_spike_str = fmt(bf_qps_spike_value, precision=1, allow_zero=True)              # e.g. "10,000" QPS
    bf_spike_factor_str = fmt(bf_spike_factor_value, precision=1, allow_zero=True, commas=False)          # e.g. "10" x
    bf_collapse_latency_s_str = fmt(bf_collapse_latency_s_value, precision=1, allow_zero=True, commas=False)  # e.g. "10" seconds
```
...
The queueing curve behind this collapse makes the headroom requirement visible.
...
The same serving-economics pressure appears in public API prices, where each order-of-magnitude drop changes which applications are feasible.
...
class StaticBatchCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_photos_str = fmt(n_photos_value, precision=1, allow_zero=True)                                   # e.g. "10,000" photos
    inference_ms_str = fmt(inference_ms_value, precision=1, allow_zero=True, commas=False)                             # e.g. "5" ms
    batch_total_s_str = fmt(batch_total_s_value, precision=1, allow_zero=True, commas=False)# e.g. "50" seconds
    dynamic_latency_budget_ms_str = fmt(dynamic_latency_budget_ms_value, precision=1, allow_zero=True, commas=False)   # e.g. "100" ms
```
...
class CostLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpu_cost_per_hour_str = fmt(gpu_cost_per_hour_value, precision=1, allow_zero=True, commas=False)
    latency_a_ms_str = fmt(latency_a_ms_value, precision=1, allow_zero=True, commas=False)
    throughput_a_rps_str = fmt(throughput_a_rps_value, precision=1, allow_zero=True, commas=False)
    latency_b_ms_str = fmt(latency_b_ms_value, precision=1, allow_zero=True, commas=False)
    throughput_b_rps_str = fmt(throughput_b_rps_value, precision=1, allow_zero=True, commas=False)
    ...
    cost_increase_str = fmt(cost_increase_pct_value, precision=1, allow_zero=True, commas=False)
    cost_ratio_str = fmt(cost_ratio_value, precision=1, allow_zero=True, commas=False)
```
...
class ResNetServingSpectrum:
    ...
    cloud_model_mb_str = fmt(cloud_size_mb, precision=1, allow_zero=True)
    cloud_inf_b1_ms_str = fmt(cloud_inf_b1_ms, precision=1, commas=False)
    cloud_inf_b16_ms_str = fmt(cloud_inf_b16_ms, precision=1, allow_zero=True, commas=False)
    cloud_throughput_str = fmt(cloud_throughput, precision=1, allow_zero=True)
    cloud_vram_gb_str = fmt(cloud_vram_gb, precision=1, allow_zero=True, commas=False)

    mobile_model_mb_str = fmt(mobile_size_mb, precision=1, allow_zero=True)
    mobile_inf_npu_ms_str = fmt(mobile_inf_npu_ms, precision=1, allow_zero=True, commas=False)
    mobile_inf_cpu_ms_str = fmt(mobile_inf_cpu_ms, precision=1, allow_zero=True, commas=False)
    mobile_throughput_str = fmt(mobile_throughput, precision=1, allow_zero=True, commas=False)
    mobile_energy_npu_mj_str = fmt(mobile_energy_npu_mj, precision=1, commas=False)
    mobile_energy_cpu_mj_str = fmt(mobile_energy_cpu_mj, precision=1, commas=False)
    mobile_mem_mb_str = MarkdownStr("150")

    tiny_model_mb_str = fmt(tiny_original_mb, precision=1, allow_zero=True)
    tiny_alt_mb_str = fmt(tiny_alt_mb, precision=1)
    tiny_inf_ms_str = fmt(tiny_inf_ms, precision=1, allow_zero=True, commas=False)
    tiny_throughput_str = MarkdownStr("8")
    tiny_arena_kb_str = MarkdownStr("320")
    tiny_sram_kb_str = fmt(s_tiny.ram.m_as('KB'), precision=1, allow_zero=True)
    tiny_energy_mj_str = fmt(tiny_energy_mj, precision=1, allow_zero=True, commas=False)
```
...
**`{python} ResNetServingSpectrum.cloud_name_str`**:
...
**`{python} ResNetServingSpectrum.mobile_name_str`**:
...
**`{python} ResNetServingSpectrum.tiny_name_str`**:
...
# │
# │ Imports: mlsysim.core.constants (IMAGE_DIM_RESNET), mlsysim.book (fmt)
...
class SerializationEfficiency:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    serial_floats_str = fmt(floats_count, precision=1, allow_zero=True)
    json_size_str = MarkdownStr("9")
    json_parse_str = fmt(int(json_parse_us), precision=1, allow_zero=True, commas=False)
    protobuf_size_str = MarkdownStr("4")
    protobuf_parse_str = fmt(int(proto_parse_us), precision=1, allow_zero=True, commas=False)
    requests_per_sec_str = MarkdownStr("10,000")
    efficiency_gain_str = fmt(efficiency_gain, precision=1, allow_zero=True, commas=False)
```
...
class TailLatencyRatioCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tail_ratio_str = fmt(tail_ratio_value, precision=1, allow_zero=True, commas=False)      # e.g. "40" times
```
...
class LatencyTableCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    l_jpeg_str = fmt(l_jpeg_value, precision=1, commas=False, suffix=" ms")
    l_resize_str = fmt(l_resize_value, precision=1, commas=False, suffix=" ms")
    ...
    l_inf_str = fmt(l_inf_value, precision=1, commas=False, suffix=" ms")
    ...
    p_jpeg_str = fmt(p_jpeg_value, precision=1, allow_zero=True, commas=False, suffix="%")
    p_resize_str = fmt(p_resize_value, precision=1, allow_zero=True, commas=False, suffix="%")
    p_norm_str = fmt(p_norm_value, precision=1, allow_zero=True, commas=False, suffix="%")
    p_transfer_str = fmt(p_transfer_value, precision=1, allow_zero=True, commas=False, suffix="%")
    p_inf_str = fmt(p_inf_value, precision=1, allow_zero=True, commas=False, suffix="%")
    p_post_str = fmt(p_post_value, precision=1, allow_zero=True, commas=False, prefix="~", suffix="%")
```
...
class LatencyBudgetCalc:
    ...
    resnet_inference_ms_str = fmt(resnet_inference_ms_value, precision=1, allow_zero=True, commas=False)
    tensorrt_inference_ms_str = fmt(tensorrt_inference_ms_value, precision=1, allow_zero=True, commas=False)
    ...
    preprocess_pct_str = fmt(preprocess_pct_value, precision=1, allow_zero=True, commas=False)
    tensorrt_preprocess_pct_str = fmt(tensorrt_preprocess_pct_value, precision=1, allow_zero=True, commas=False)
```
...
class DlrmLatencyCalc:
    ...
    dlrm_embed_str = fmt(dlrm_embed_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    dlrm_mlp_str = fmt(dlrm_mlp_ms_value, precision=1, commas=False, suffix=" ms")
    dlrm_post_str = fmt(dlrm_post_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    dlrm_total_str = fmt(dlrm_total_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
```
...
class AmdahlServingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    non_model_pct_str = fmt(non_model_pct_value, precision=1, allow_zero=True, commas=False)
...
class ResolutionScalingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    r1_str = fmt(r1_value, precision=1, allow_zero=True, commas=False)
    r2_str = fmt(r2_value, precision=1, allow_zero=True, commas=False)
    theoretical_str = fmt(theoretical_slowdown_value, precision=1, allow_zero=True, commas=False)
...
class ResolutionBottleneckCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    act_224_mb_str = fmt(act_224_mb_value, precision=1, commas=False)
    act_384_mb_str = fmt(act_384_mb_value, precision=1, commas=False)
    act_512_mb_str = fmt(act_512_mb_value, precision=1, commas=False)
    act_640_mb_str = fmt(act_640_mb_value, precision=1, commas=False)

    ai_224_str = fmt(ai_224_value, precision=1, allow_zero=True, commas=False)
    ai_384_str = fmt(ai_384_value, precision=1, allow_zero=True, commas=False)
    ai_512_str = fmt(ai_512_value, precision=1, allow_zero=True, commas=False)
    ai_640_str = fmt(ai_640_value, precision=1, allow_zero=True, commas=False)

    ridge_point_str = fmt(ridge_point_value, precision=1, allow_zero=True, commas=False)
```
...
|:---------------------|:-----------------------------------------------------:|:--------------------------------------------------------:|:--------------:|
| **$224{\times}224$** | `{python} ResolutionBottleneckCalc.act_224_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_224_str` FLOP/byte |    Compute     |
| **$384{\times}384$** | `{python} ResolutionBottleneckCalc.act_384_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_384_str` FLOP/byte |    Compute     |
| **$512{\times}512$** | `{python} ResolutionBottleneckCalc.act_512_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_512_str` FLOP/byte |    Compute     |
| **$640{\times}640$** | `{python} ResolutionBottleneckCalc.act_640_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_640_str` FLOP/byte |   Near ridge   |
...
class CapacityPlanningAnchor:
    ...
    qps_str = fmt(qps_target, precision=1, allow_zero=True, commas=False)
    slo_str = fmt(slo_ms, precision=1, allow_zero=True, commas=False, suffix=" ms")
    slots_str = fmt(concurrency_slots, precision=1, allow_zero=True, commas=False)
...
class CapacityPlanning:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    littles_lambda_str = fmt(lambda_qps, precision=1, allow_zero=True)
    littles_w_ms_str = fmt(int(latency_slo_s * MS_PER_SEC), precision=1, allow_zero=True, commas=False)
    littles_w_str = fmt(latency_slo_s, precision=2, commas=False)
    littles_l_str = fmt(concurrency, precision=1, allow_zero=True, commas=False)
```
...
**Problem**: How much concurrent request capacity does a system need to serve 1,000 queries per second?

**Math**: Little's Law gives $N_{\text{req}} = \lambda T_{\text{lat}}$, so concurrency equals throughput multiplied by latency (see @sec-machine-foundations-littles-law-21a3 for the derivation).
...
class BatchingTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    wait_time_b1_ms_str = fmt(int(w_form_b1), precision=1, allow_zero=True, commas=False)
    wait_time_b32_ms_str = fmt(int(w_form_b32), precision=1, allow_zero=True, commas=False)
    lat_b1_ms_str = fmt(lat_b1, precision=1, commas=False)
    lat_b32_ms_str = fmt(lat_b32, precision=1, commas=False)
    penalty_ratio_str = fmt(int(penalty_ratio), precision=1, allow_zero=True, commas=False)
```
...
class CapacityPlanningCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cp_mu_required_str = fmt(cp_mu_required_value, precision=1, allow_zero=True, commas=False)
    cp_gpus_raw_str = fmt(cp_gpus_raw_value, precision=1, commas=False)
    cp_gpus_ceil_str = fmt(cp_gpus_ceil_value, precision=1, allow_zero=True, commas=False)
    cp_final_raw_str = fmt(cp_final_raw_value, precision=1, commas=False)
    cp_final_ceil_str = fmt(cp_final_ceil_value, precision=1, allow_zero=True, commas=False)
    cp_gpus_after_fail_str = fmt(cp_gpus_after_fail_value, precision=1, allow_zero=True, commas=False)
    cp_util_after_fail_str = fmt(cp_util_after_fail_value, precision=1, commas=False)
    cp_n_plus_2_min_str = fmt(cp_n_plus_2_min_value, precision=1, allow_zero=True, commas=False)
    cp_n_plus_2_headroom_str = fmt(cp_n_plus_2_headroom_value, precision=1, allow_zero=True, commas=False)
    cp_peak_qps_str = fmt(cp_peak_qps_value, precision=1, allow_zero=True)
    cp_v100_throughput_str = fmt(cp_v100_throughput_value, precision=1, allow_zero=True)
    cp_service_ms_str = fmt(cp_service_ms_value, precision=1, allow_zero=True, commas=False)
    cp_p99_target_ms_str = fmt(cp_p99_target_ms_value, precision=1, allow_zero=True, commas=False)
    cp_rho_safe_str = fmt(cp_rho_safe_value, precision=2, commas=False)
    cp_rho_safe_pct_str = fmt(cp_rho_safe_value * 100, precision=1, allow_zero=True, commas=False)
...
From @eq-p99-latency, $T_{\text{lat},\text{p99}} \approx$ `{python} CapacityPlanningCalc.mm1_p99_factor_str` $\times$ service time / $(1 - \rho)$. Setting $T_{\text{lat},\text{p99}} \leq 50$ ms with 5 ms service time gives `{python} CapacityPlanningCalc.cp_rho_safe_math` (`{python} CapacityPlanningCalc.cp_rho_safe_pct_str` percent maximum utilization). This uses the conservative M/M/1 p99 bound from the displayed equation rather than applying an average-wait M/D/1 adjustment to a tail-latency SLO.
...
This remains well below the `{python} CapacityPlanningCalc.cp_rho_safe_pct_str` percent safe utilization threshold, confirming N+1 redundancy is satisfied. For stricter fault tolerance requirements, N+2 redundancy (tolerating two simultaneous failures) would require `{python} CapacityPlanningCalc.cp_n_plus_2_min_str` GPUs under the same safe-utilization threshold, or about `{python} CapacityPlanningCalc.cp_n_plus_2_headroom_str` GPUs if the 30 percent headroom must remain after two simultaneous failures.
...
class ColdStartCalc:
    ...
    cs_cloud_str = fmt(cs_cloud_total_value, precision=1, allow_zero=True, commas=False, prefix="~", suffix=" s")
...
class ModelSwapCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_size_gb_str = fmt(model_size_gb_value, precision=1, allow_zero=True, commas=False)
    pcie_bw_gbs_str = fmt(pcie_bw_gbs_value, precision=1, allow_zero=True, commas=False)
    model_swap_ms_str = fmt(model_swap_ms_value, precision=1, allow_zero=True, commas=False)
...
class BatchThroughputCalc:
    ...
    batch_window_ms_str = fmt(batch_window_ms_value, precision=1, allow_zero=True, commas=False)
    batch32_inference_ms_str = fmt(batch32_inference_ms_value, precision=1, allow_zero=True, commas=False)
    batch32_total_str = fmt(batch32_total_ms_value, precision=1, allow_zero=True, commas=False)
    batch1_inference_total_ms_str = fmt(batch1_inference_total_ms_value, precision=1, allow_zero=True, commas=False)
```
...
class BatchingSweetspotCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    batch1_ms_str = fmt(batch1_ms_value, precision=1, allow_zero=True, commas=False)
    batch1_imgs_str = fmt(batch1_imgs_value, precision=1, allow_zero=True, commas=False)
    batch8_wait_ms_str = fmt(batch8_wait_ms_value, precision=1, allow_zero=True, commas=False)
    batch8_inference_ms_str = fmt(batch8_inference_ms_value, precision=1, allow_zero=True, commas=False)
    batch8_user_latency_str = fmt(batch8_user_latency_ms_value, precision=1, allow_zero=True, commas=False)
    batch8_throughput_str = fmt(batch8_throughput_value, precision=1, allow_zero=True, commas=False)
    latency_increase_str = fmt(latency_increase_value, precision=1, allow_zero=True, commas=False)
```

::: {#nbk-model-serving-batching-sweet-spot .callout-notebook title="The batching sweet spot"}
...
class BatchingBudgetCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    avg_wait_str = fmt(avg_wait_ms_value, precision=1, allow_zero=True, commas=False)
    budget_pct_str = fmt(budget_pct_value, precision=1, allow_zero=True, commas=False)
```
...
class BatchingAnalysisCalc:
    ...
    b1_throughput_str = fmt(throughputs_value[1], precision=1, allow_zero=True, commas=False)
    b32_throughput_str = fmt(throughputs_value[32], precision=1, allow_zero=True, commas=False)
    ...
    il_overhead_str = fmt(il_overhead_ms_value, precision=1, allow_zero=True, commas=False)
    il_compute_b1_str = fmt(il_compute_b1_ms_value, precision=1, commas=False)
    il_compute_b32_str = fmt(il_compute_b32_ms_value, precision=1, commas=False)
    il_eff_b1_str = fmt(il_eff_b1_pct_value, precision=1, allow_zero=True, commas=False)
    il_eff_b32_str = fmt(il_eff_b32_pct_value, precision=1, allow_zero=True, commas=False)
    il_threshold_str = fmt(il_threshold_pct_value, precision=1, allow_zero=True, commas=False)
    T_window_str = fmt(T_window_value, precision=1, allow_zero=True, commas=False)
...
class BatchingOptimization:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    s1_window_ms_str = fmt(int(s1_window), precision=1, allow_zero=True, commas=False)
    s1_wait_ms_str = fmt(s1_wait, precision=1, commas=False)
    s1_budget_ms_str = fmt(s1_budget, precision=1, commas=False)
    s1_max_batch_str = MarkdownStr("70") # Theoretical ceiling
    s1_batch_str = fmt(s1_batch, precision=1, allow_zero=True, commas=False)
    s1_throughput_str = fmt(int(s1_tput), precision=1, allow_zero=True)

    s2_window_ms_str = fmt(int(s2_window), precision=1, allow_zero=True, commas=False)
    s2_wait_ms_str = fmt(s2_wait, precision=1, commas=False)
    s2_budget_ms_str = fmt(s2_budget, precision=1, commas=False)
    s2_batch_str = fmt(s2_batch, precision=1, allow_zero=True, commas=False)
    s2_throughput_str = fmt(int(s2_tput), precision=1, allow_zero=True)

    throughput_gain_pct_str = fmt(tput_gain, precision=1, allow_zero=True, commas=False)
    latency_avg_increase_ms_str = fmt(latency_increase, precision=1, allow_zero=True, commas=False)
    # Simplified P99 increase for prose consistency
    latency_p99_increase_ms_str = fmt(int(s2_window - s1_window), precision=1, allow_zero=True, commas=False)
```

**Scenario one: Conservative window ($T_{\text{window}}$ = `{python} BatchingOptimization.s1_window_ms_str` ms)**
...
**Scenario two: Aggressive window ($T_{\text{window}}$ = `{python} BatchingOptimization.s2_window_ms_str` ms)**
...
- Configured batch size: `{python} BatchingOptimization.s2_batch_str`
...
class SloViolationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    qps_str = fmt(qps_value, precision=1, allow_zero=True, commas=False)
    T_slo_str = fmt(T_slo_value, precision=1, allow_zero=True, commas=False)
    mean_wait_str = fmt(mean_wait_value, precision=1, allow_zero=True, commas=False)
    mean_batch_str = fmt(mean_batch_value, precision=1, allow_zero=True, commas=False)
    mean_service_str = fmt(mean_service_value, precision=1, commas=False)
    mean_latency_str = fmt(mean_latency_value, precision=1, allow_zero=True, commas=False)
    ...
    p99_batch_str = fmt(p99_batch_value, precision=1, allow_zero=True, commas=False)
```
...
where $B_{\text{p99}}$ is the 99th percentile batch size. For $\lambda$ = `{python} SloViolationCalc.qps_str` QPS and $T_{\text{window}}$ = `{python} SloViolationCalc.T_slo_str` ms:
...
class PracticalConfigCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pc_slo_ms_str = fmt(pc_slo_ms_value, precision=1, allow_zero=True, commas=False)
    pc_qps_str = fmt(pc_qps_value, precision=1, allow_zero=True, commas=False)
    pc_batch_budget_ms_str = fmt(pc_batch_budget_ms_value, precision=1, allow_zero=True, commas=False)
    pc_max_window_ms_str = fmt(pc_max_window_ms_value, precision=1, allow_zero=True, commas=False)
    pc_expected_batch_str = fmt(pc_expected_batch_value, precision=1, allow_zero=True, commas=False)
    pc_mem_limit_batch_str = fmt(pc_mem_limit_batch_value, precision=1, allow_zero=True, commas=False)
    pc_config_window_ms_str = fmt(pc_config_window_ms_value, precision=1, allow_zero=True, commas=False)
    pc_config_batch_str = fmt(pc_config_batch_value, precision=1, allow_zero=True, commas=False)
    pc_p99_batch_str = fmt(pc_p99_batch_value, precision=1, allow_zero=True, commas=False)
    pc_predicted_p99_ms_str = fmt(pc_predicted_p99_ms_value, precision=1, allow_zero=True, commas=False)
    pc_predicted_throughput_str = fmt(pc_predicted_throughput_value, precision=1, allow_zero=True)
```
...
- Configuration: $T_{\text{window}}$ = `{python} PracticalConfigCalc.pc_config_window_ms_str` ms, $B_{\text{max}}$ = `{python} PracticalConfigCalc.pc_config_batch_str`
...
class TrafficAdaptiveBatchingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    qps_100_str = fmt(rows[0][0], precision=1, allow_zero=True)
    window_100_ms_str = fmt((rows[0][1] / (1 * ms).m_as(second)), precision=1, allow_zero=True, commas=False)
    avg_batch_100_str = fmt(rows[0][2], precision=1, commas=False)
    latency_100_ms_str = fmt(rows[0][3], precision=1, allow_zero=True, commas=False)

    qps_500_str = fmt(rows[1][0], precision=1, allow_zero=True)
    window_500_ms_str = fmt((rows[1][1] / (1 * ms).m_as(second)), precision=1, allow_zero=True, commas=False)
    avg_batch_500_str = fmt(rows[1][2], precision=1, commas=False)
    latency_500_ms_str = fmt(rows[1][3], precision=1, allow_zero=True, commas=False)

    qps_1000_str = fmt(rows[2][0], precision=1, allow_zero=True)
    window_1000_ms_str = fmt((rows[2][1] / (1 * ms).m_as(second)), precision=1, allow_zero=True, commas=False)
    avg_batch_1000_str = fmt(rows[2][2], precision=1, commas=False)
    latency_1000_ms_str = fmt(rows[2][3], precision=1, allow_zero=True, commas=False)

    qps_5000_str = fmt(rows[3][0], precision=1, allow_zero=True)
    window_5000_ms_str = fmt((rows[3][1] / (1 * ms).m_as(second)), precision=1, allow_zero=True, commas=False)
    avg_batch_5000_str = fmt(rows[3][2], precision=1, commas=False)
    latency_5000_ms_str = fmt(rows[3][3], precision=1, allow_zero=True, commas=False)
```

| **Arrival Rate**                                            |                      **Optimal Window**                      |                     **Avg Batch Size**                    |                      **Approx. Latency**                      |
|:------------------------------------------------------------|:------------------------------------------------------------:|:---------------------------------------------------------:|:-------------------------------------------------------------:|
| **`{python} TrafficAdaptiveBatchingCalc.qps_100_str` QPS**  | `{python} TrafficAdaptiveBatchingCalc.window_100_ms_str` ms  |  `{python} TrafficAdaptiveBatchingCalc.avg_batch_100_str` |  `{python} TrafficAdaptiveBatchingCalc.latency_100_ms_str` ms |
| **`{python} TrafficAdaptiveBatchingCalc.qps_500_str` QPS**  | `{python} TrafficAdaptiveBatchingCalc.window_500_ms_str` ms  |  `{python} TrafficAdaptiveBatchingCalc.avg_batch_500_str` |  `{python} TrafficAdaptiveBatchingCalc.latency_500_ms_str` ms |
| **`{python} TrafficAdaptiveBatchingCalc.qps_1000_str` QPS** | `{python} TrafficAdaptiveBatchingCalc.window_1000_ms_str` ms | `{python} TrafficAdaptiveBatchingCalc.avg_batch_1000_str` | `{python} TrafficAdaptiveBatchingCalc.latency_1000_ms_str` ms |
| **`{python} TrafficAdaptiveBatchingCalc.qps_5000_str` QPS** | `{python} TrafficAdaptiveBatchingCalc.window_5000_ms_str` ms | `{python} TrafficAdaptiveBatchingCalc.avg_batch_5000_str` | `{python} TrafficAdaptiveBatchingCalc.latency_5000_ms_str` ms |
...
class MobileServingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    m_cam_ms_str = fmt(m_cam_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    m_jpeg_ms_str = fmt(m_jpeg_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    m_resize_ms_str = fmt(m_resize_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    m_npu_ms_str = fmt(m_npu_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    m_ui_ms_str = fmt(m_ui_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    m_total_ms_str = fmt(m_total_ms_value, precision=1, allow_zero=True, commas=False, suffix=" ms")
    m_cam_mj_str = fmt(m_cam_mj_value, precision=2, commas=False, suffix=" mJ")
    m_jpeg_mj_str = fmt(m_jpeg_mj_value, precision=1, commas=False, suffix=" mJ")
    m_resize_mj_str = fmt(m_resize_mj_value, precision=1, commas=False, suffix=" mJ")
    m_npu_mj_str = fmt(m_npu_mj_value, precision=1, commas=False, suffix=" mJ")
    m_ui_mj_str = fmt(m_ui_mj_value, precision=1, commas=False, suffix=" mJ")
    m_total_mj_str = fmt(m_total_mj_value, precision=1, commas=False, suffix=" mJ")
```
...
- **NPU vs. CPU trade-off**: CPU fallback replaces the 12 ms, 0.8 mJ NPU inference stage with a 45 ms, 4.2 mJ CPU stage; the full pipeline would rise from 45 ms and 3.0 mJ to about 78 ms and 6.4 mJ before additional system overhead.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: h_h100-tdp-bw-specs
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ H100 TDP AND BANDWIDTH SPECIFICATIONS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: "Carbon Cost of Chat" callout (~10 lines below: GpuSpecs.h100_tdp_str),
# │          Layer Fusion footnote (~120 lines below: GpuSpecs.h100_bw_tbs_str)
# │
...
# │ Exports: GpuSpecs.h100_tdp_str, GpuSpecs.h100_bw_tbs_str
# └─────────────────────────────────────────────────────────────────────────────
...
class GpuSpecs:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    h100_bw_tbs_str = fmt(h100_bw_tbs_value, precision=2, commas=False)    # e.g. "3.35" TB/s
    h100_tdp_str = fmt(h100_tdp_value, precision=1, allow_zero=True, commas=False)          # e.g. "700" W
```
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.tdp, energy comparisons)
...
class CarbonCostCalc:
    ...
    cc_tokens_per_sec_req_value = 7.5                 # tokens/s per request (decode phase)
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    _h100_tdp_w = Hardware.Cloud.H100.tdp.m_as(watt)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cc_concurrent_str = fmt(cc_concurrent_req_value, precision=1, allow_zero=True, commas=False)
    cc_tokens_req_str = fmt(cc_tokens_per_sec_req_value, precision=1, commas=False)
    cc_total_tokens_str = fmt(cc_total_tokens_sec_value, precision=1, allow_zero=True, commas=False)
    cc_host_overhead_str = fmt(cc_host_overhead_w_value, precision=1, allow_zero=True, commas=False)
    cc_total_power_str = fmt(cc_total_power_w_value, precision=1, allow_zero=True, commas=False)
    cc_joules_token_str = fmt(cc_joules_per_token_value, precision=2, commas=False)
    cc_response_tokens_str = fmt(cc_response_tokens_value, precision=1, allow_zero=True, commas=False)
    cc_response_joules_str = fmt(cc_response_joules_value, precision=1, allow_zero=True, commas=False)
    cc_smartphone_str = fmt(_smartphone_joules, precision=1, allow_zero=True)
    cc_boiling_str = fmt(_boiling_joules, precision=1, allow_zero=True)
    cc_low_util_str = fmt(cc_low_util_pct_value, precision=1, allow_zero=True, commas=False)
    cc_idle_power_str = fmt(cc_idle_power_w_value, precision=1, allow_zero=True, commas=False)
...
As LLMs scale, energy efficiency becomes a first-class operational metric alongside latency. For an H100 GPU (`{python} GpuSpecs.h100_tdp_str` W TDP), we can quantify the energy footprint of serving:

1.  **Throughput**: `{python} CarbonCostCalc.cc_concurrent_str` concurrent requests $\times$ `{python} CarbonCostCalc.cc_tokens_req_str` tokens/s/req ≈ **`{python} CarbonCostCalc.cc_total_tokens_str` tokens/s**.
2.  **Power**: `{python} GpuSpecs.h100_tdp_str` W (GPU) + `{python} CarbonCostCalc.cc_host_overhead_str` W (Host/Overhead) = **`{python} CarbonCostCalc.cc_total_power_str` W**.
3.  **Energy per Token**:

    `{python} CarbonCostCalc.cc_total_power_str` J/s / `{python} CarbonCostCalc.cc_total_tokens_str` tokens/s ≈ **`{python} CarbonCostCalc.cc_joules_token_str` J/token**

**Systems insight**: A typical `{python} CarbonCostCalc.cc_response_tokens_str`-token response consumes ≈ **`{python} CarbonCostCalc.cc_response_joules_str` J**.

- For comparison, charging a smartphone consumes ≈ `{python} CarbonCostCalc.cc_smartphone_str` J.
- Boiling a cup of water consumes ≈ `{python} CarbonCostCalc.cc_boiling_str` J.

**The engineering lever**: The primary way to reduce J/token is to **increase hardware utilization** and **eliminate redundant compute**. If the GPU sits at `{python} CarbonCostCalc.cc_low_util_str` percent utilization due to poor batching, the "Idle Power" is still ~`{python} CarbonCostCalc.cc_idle_power_str` W, causing the energy-per-token to skyrocket to **>`{python} CarbonCostCalc.cc_low_util_joules_str` J**. Furthermore, architectural optimizations like **Prefix Caching** skip the energy-intensive prefill phase for shared context, directly reducing the energy footprint of RAG and chat applications. MLOps is not just about speed; it is about sustainability through efficiency.
...
[^fn-layer-fusion-serving]: **Layer Fusion**: Analogous to loop fusion in compiler optimization, where adjacent loops over the same array are combined to reduce memory traffic. Kernel fusion applies the identical principle to GPU operations: sequential kernels that write and re-read intermediate tensors from HBM are merged into a single kernel that keeps data in registers. The savings compound---a typical ResNet-50 has ~35 fusible operation pairs, and each eliminated HBM round-trip saves 1--3 $\mu$s at `{python} GpuSpecs.h100_bw_tbs_str` TB/s bandwidth, converting memory-bound chains into compute-bound fused kernels. \index{Layer Fusion!compiler analogy}
...
class PrecisionTradeoffCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pt_fp32_ms_str = fmt(pt_fp32_ms_value, precision=1, commas=False)
    pt_fp32_mem_mb_str = fmt(pt_fp32_mem_mb_value, precision=1, allow_zero=True, commas=False)
    pt_fp32_acc_str = fmt(pt_fp32_acc_value, precision=2, commas=False)
    pt_fp16_ms_str = fmt(pt_fp16_ms_value, precision=1, commas=False)
    pt_fp16_mem_mb_str = fmt(pt_fp16_mem_mb_value, precision=1, allow_zero=True, commas=False)
    pt_fp16_acc_str = fmt(pt_fp16_acc_value, precision=2, commas=False)
    pt_fp16_util_str = fmt(pt_fp16_util_value, precision=1, allow_zero=True, commas=False)
    pt_int8_ms_str = fmt(pt_int8_ms_value, precision=1, commas=False)
    pt_int8_mem_mb_str = fmt(pt_int8_mem_mb_value, precision=1, allow_zero=True, commas=False)
    pt_int8_ptq_acc_str = fmt(pt_int8_ptq_acc_value, precision=2, commas=False)
    pt_int8_qat_acc_str = fmt(pt_int8_qat_acc_value, precision=2, commas=False)
    pt_int8_util_str = fmt(pt_int8_util_value, precision=1, allow_zero=True, commas=False)
    pt_int8_speedup_str = fmt(pt_int8_speedup_value, precision=1, commas=False)
    pt_fp16_speedup_str = fmt(pt_fp16_speedup_value, precision=1, allow_zero=True, commas=False)
    pt_int8_vs_fp16_speedup_str = fmt(pt_int8_vs_fp16_speedup_value, precision=1, commas=False)
    pt_int8_acc_loss_str = fmt(pt_int8_acc_loss_value, precision=2, commas=False)
```
...
class CostAnalysisCalc:
    ...
    ca_cpu_throughput_str = fmt(ca_cpu_throughput_value, precision=1, allow_zero=True, commas=False)
    ca_cpu_cpm_str = fmt(ca_cpu_cpm_value, precision=2, commas=False)
    ca_t4_cost_str = fmt(ca_t4_cost_value, precision=2, commas=False)
    ca_t4_throughput_str = fmt(ca_t4_throughput_value, precision=1, allow_zero=True, commas=False)
    ...
    ca_v100_throughput_str = fmt(ca_v100_throughput_value, precision=1, allow_zero=True)
    ca_v100_cpm_str = fmt(ca_v100_cpm_value, precision=2, commas=False)
    ca_v100_price_increase_str = fmt(ca_v100_price_increase_value, precision=1, allow_zero=True, commas=False)
```
...
: **ResNet-50 Cloud Inference Cost Comparison**: AWS hourly cost, sustained throughput, and resulting cost per one million images for CPU, T4, and V100 instances, showing the T4 as the cost-per-inference winner despite its higher hourly rate. {#tbl-model-serving-resnet-cloud-cost}
...
class LlmCaseStudyHwSpecs:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_bw_tbs_str = fmt(a100_bw_tbs_value, precision=2, commas=False)    # e.g. "2.04" TB/s
    h100_mem_str = fmt(h100_mem_value, precision=1, allow_zero=True, commas=False)          # e.g. "80" GB
```
...
*   **Hardware**: 1$\times$ NVIDIA H100 SXM5 GPU (`{python} LlmCaseStudyHwSpecs.h100_mem_str` GB HBM3, `{python} GpuSpecs.h100_bw_tbs_str` TB/s bandwidth).
...
Every token generation pays a massive "energy tax" to move the model's logic from HBM into compute registers. For comparison, on an A100 80 GB (`{python} LlmCaseStudyHwSpecs.a100_bw_tbs_str` TB/s HBM2e), an 8-billion-parameter Llama 3 model (3.5 GB INT4) generates tokens at $\approx 1.7$ ms/token. Adding more *compute cores* yields **zero** latency improvement; only *faster memory* (Physics) or *smaller models* (Algorithm) can speed up generation.
...
class LlmServingCalc:
    ...
    realized_tpot_ms_value = res.itl.m_as(ms)

    _h100_bw_tb = h.memory.bandwidth.m_as(TB/second)
    _h100_bw_gbs = h.memory.bandwidth.m_as(GB/second)

    # Theoretical floor (100% BW efficiency)
    token_time_theoretical_ms_value = (model_weight_gb_value / _h100_bw_gbs * 1000)

    total_decode_s_value = (decode_tokens_value * realized_tpot_ms_value / 1000)

    # KV Cache Sizing using library results
    kv_cache_gb_value = 72 # Reserved for KV
    kv_per_token_mb_value = res.kv_cache_size.m_as(MB) / total_tokens
    kv_capacity_tokens_value = int((kv_cache_gb_value * 1024) / kv_per_token_mb_value)

    concurrent_batch_value = int(kv_capacity_tokens_value / total_tokens)
    req_time_s_value = res.ttft.m_as(second) + total_decode_s_value
    ...
    tokens_per_hour_value = serving_req_per_s_value * 3600 * total_tokens
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_weight_gb_str = fmt(model_weight_gb_value, precision=1, commas=False)
    h100_bw_tb_str = fmt(_h100_bw_tb, precision=1, commas=False)
    token_time_theoretical_ms_str = fmt(token_time_theoretical_ms_value, precision=1, allow_zero=True, commas=False)
    realized_tpot_ms_str = fmt(realized_tpot_ms_value, precision=1, allow_zero=True, commas=False)
    decode_tokens_str = fmt(decode_tokens_value, precision=1, allow_zero=True, commas=False)
    total_decode_s_str = fmt(total_decode_s_value, precision=2, commas=False)
    kv_cache_gb_str = fmt(kv_cache_gb_value, precision=1, allow_zero=True, commas=False)
    kv_per_token_mb_str = fmt(kv_per_token_mb_value, precision=3, commas=False)
    kv_capacity_tokens_str = fmt(kv_capacity_tokens_value, precision=1, allow_zero=True)
    tokens_per_req_str = fmt(total_tokens, precision=1, allow_zero=True)
    concurrent_batch_str = fmt(concurrent_batch_value, precision=1, allow_zero=True, commas=False)
    req_time_s_str = fmt(req_time_s_value, precision=2, commas=False)
    prefill_tokens_per_s_str = fmt(prefill_tokens_per_s_value, precision=1, allow_zero=True)
    prefill_req_per_s_str = fmt(prefill_req_per_s_value, precision=1, allow_zero=True, commas=False)
    decode_req_capacity_str = fmt(decode_req_capacity_value, precision=1, allow_zero=True, commas=False)
    hourly_cost_str = fmt(hourly_cost_value, precision=2, commas=False)
    tokens_per_hour_m_str = fmt(tokens_per_hour_value/MILLION, precision=1, allow_zero=True, commas=False)
    cost_per_m_tokens_str = fmt(cost_per_m_tokens_value, precision=3, commas=False)
    remaining_vram_gb_str = fmt(remaining_vram_gb_value, precision=1, allow_zero=True, commas=False)
```

*   $T_{\text{token}} \approx$ `{python} LlmServingCalc.model_weight_gb_str` GB / `{python} GpuSpecs.h100_bw_tbs_str` TB/s ≈ `{python} LlmServingCalc.token_time_theoretical_ms_str` ms (theoretical limit).
...
*   Each token requires approximately `{python} LlmServingCalc.kv_per_token_mb_str` MB of KV cache after accounting for Llama 3-style Grouped Query Attention in the 8-billion-parameter configuration (standard multi-head attention would be about 32 layers $\times$ 4096 dim $\times$ 2 vectors $\times$ 2-byte precision $\approx 0.5$ MB/token, and GQA reduces the KV-head count by about 4$\times$).
...
For an H100 SXM5 instance at approximately USD `{python} LlmServingCalc.hourly_cost_str` per hour (specialized cloud providers; major hyperscaler on-demand rates were in the USD 2--13 per hour range in 2026):

*   Prefill-limited admissions: `{python} LlmServingCalc.prefill_tokens_per_s_str` prompt tokens/s divided by 1,000 prompt tokens/req = `{python} LlmServingCalc.prefill_req_per_s_str` requests/s. This is below the KV-residency limit of `{python} LlmServingCalc.concurrent_batch_str` batch / `{python} LlmServingCalc.req_time_s_str` s/req ≈ `{python} LlmServingCalc.decode_req_capacity_str` requests/s, so full-request throughput is `{python} LlmServingCalc.prefill_req_per_s_str` req/s $\times$ 3,600 s/hr $\times$ `{python} LlmServingCalc.tokens_per_req_str` tokens/req ≈ `{python} LlmServingCalc.tokens_per_hour_m_str` million tokens/hour.
...
class FallacyLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fl_utilization_high_pct_str = fmt(fl_utilization_high_value * 100, precision=1, allow_zero=True, commas=False)
    fl_service_slow_ms_str = fmt(fl_service_slow_ms_value, precision=1, allow_zero=True, commas=False)
    fl_wait_slow_ms_str = fmt(fl_wait_slow_ms_value, precision=1, allow_zero=True, commas=False)
    fl_service_fast_ms_str = fmt(fl_service_fast_ms_value, precision=1, allow_zero=True, commas=False)
    fl_utilization_new_pct_str = fmt(fl_utilization_new_value * 100, precision=1, allow_zero=True, commas=False)
    fl_wait_fast_ms_str = fmt(fl_wait_fast_ms_value, precision=1, commas=False)
    fl_inference_gain_ms_str = fmt(fl_inference_gain_ms_value, precision=1, allow_zero=True, commas=False)
    fl_queuing_improvement_str = fmt(fl_queuing_improvement_value, precision=1, allow_zero=True, commas=False)
    fl_total_slow_ms_str = fmt(fl_total_slow_ms_value, precision=1, allow_zero=True, commas=False)
    fl_total_fast_ms_str = fmt(fl_total_fast_ms_value, precision=1, commas=False)
    fl_system_speedup_str = fmt(fl_system_speedup_value, precision=1, commas=False)
    fl_model_speedup_str = fmt(fl_model_speedup_value, precision=1, commas=False)
```
...
class FallacyUtilizationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fu_util_high_pct_str = fmt(fu_util_high_value * 100, precision=1, allow_zero=True, commas=False)
    fu_util_mod_pct_str = fmt(fu_util_mod_value * 100, precision=1, allow_zero=True, commas=False)
    fu_wait_high_factor_str = fmt(fu_wait_high_factor_value, precision=1, allow_zero=True, commas=False)
    fu_total_high_factor_str = fmt(1/(1-fu_util_high_value), precision=1, allow_zero=True, commas=False)
    fu_cost_reduction_str = fmt(fu_cost_reduction_pct_value, precision=1, allow_zero=True, commas=False)
    fu_latency_increase_str = fmt(fu_latency_increase_factor_value, precision=1, allow_zero=True, commas=False)
    fu_service_ms_str = fmt(fu_service_ms_value, precision=1, allow_zero=True, commas=False)
    fu_p99_mod_ms_str = fmt(fu_p99_mod_value, precision=1, commas=False)
    fu_p99_high_ms_str = fmt(fu_p99_high_value, precision=1, commas=False)
```
...
class FallacySkewCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fs_val_acc_str = fmt(fs_val_acc_value, precision=1, allow_zero=True, commas=False)
    fs_prod_acc_str = fmt(fs_prod_acc_value, precision=1, allow_zero=True, commas=False)
    fs_acc_drop_str = fmt(fs_acc_drop_value, precision=1, allow_zero=True, commas=False)
    fs_resize_drop_str = MarkdownStr(f"{fs_resize_drop_min_value}-{fs_resize_drop_max_value}")
```

Engineers assume identical model weights preserve validation set performance\index{Training-Serving Skew!serving accuracy fallacy}. In production, preprocessing differences silently shift inputs outside the training distribution. @sec-model-serving-trainingserving-skew-7b99 shows *how* training-serving skew causes accuracy degradation despite identical weights: PIL vs. OpenCV resize interpolation alone can shift accuracy by `{python} FallacySkewCalc.fs_resize_drop_str` percent, FP64 vs. FP32 normalization produces different values, or feature computation timing changes. A model achieving `{python} FallacySkewCalc.fs_val_acc_str` percent validation accuracy drops to `{python} FallacySkewCalc.fs_prod_acc_str` percent in production from these preprocessing mismatches, a `{python} FallacySkewCalc.fs_acc_drop_str` percentage point loss invisible to latency monitoring. Standard monitoring checking exceptions and latency violations fails to detect this silent degradation. Production systems require either identical preprocessing code for training and serving, or statistical monitoring comparing input distributions to catch drift before accuracy degrades.
...
class TailLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tl_util_pct_str = fmt(tl_util_value * 100, precision=1, allow_zero=True, commas=False)
    tl_service_ms_str = fmt(tl_service_ms_value, precision=1, allow_zero=True, commas=False)
    tl_avg_ms_str = fmt(tl_avg_ms_value, precision=1, allow_zero=True, commas=False)
    tl_p99_ms_str = fmt(tl_p99_ms_value, precision=1, commas=False)
    tl_gap_str = fmt(tl_gap_value, precision=1, commas=False)
```
...
class FallacyBatchingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fb_batch_small_str = fmt(fb_batch_small_value, precision=1, allow_zero=True, commas=False)
    fb_batch_large_str = fmt(fb_batch_large_value, precision=1, allow_zero=True, commas=False)
    fb_throughput_gain_str = fmt(fb_throughput_gain_pct_value, precision=1, allow_zero=True, commas=False)
    fb_inf_small_ms_str = fmt(fb_inf_small_ms_value, precision=1, allow_zero=True, commas=False)
    fb_inf_large_ms_str = fmt(fb_inf_large_ms_value, precision=1, allow_zero=True, commas=False)
    fb_padding_waste_min_str = fmt(fb_padding_waste_min_pct_value, precision=1, allow_zero=True, commas=False)
    fb_padding_waste_max_str = fmt(fb_padding_waste_max_pct_value, precision=1, allow_zero=True, commas=False)
```
...
class FallacyCalibrationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fc_acc_loss_str = fmt(fc_acc_loss_pct_value, precision=1, commas=False)
    fc_imagenet_acc_str = fmt(fc_imagenet_acc_value, precision=1, commas=False)
    fc_ood_acc_str = fmt(fc_ood_acc_value, precision=1, commas=False)
```
...
class FallacyColdstartCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cs_new_instances_str = fmt(cs_new_instances_value, precision=1, allow_zero=True, commas=False)
    cs_compile_time_str = fmt(cs_compile_time_s_value, precision=1, allow_zero=True, commas=False)
    cs_aggregate_cold_str = fmt(cs_aggregate_cold_s_value, precision=1, allow_zero=True, commas=False)
    cs_parallel_warm_str = fmt(cs_parallel_warm_s_value, precision=1, allow_zero=True, commas=False)
    cs_steady_latency_str = fmt(cs_steady_latency_ms_value, precision=1, allow_zero=True, commas=False)
    cs_cold_latency_str = fmt(cs_cold_latency_ms_value, precision=1, allow_zero=True, commas=False)
    cs_cold_mult_str = fmt(cs_cold_multiplier_value, precision=1, allow_zero=True, commas=False)
```

Engineers optimize steady-state latency\index{Cold Start!bursty traffic impact} assuming most requests hit warm instances. In production, cold starts compound during the events that matter most: traffic spikes requiring scale-up, deployments rolling out new versions, and recovery from instance failures. @sec-model-serving-model-loading-initialization-cc5a details the anatomy of cold start: TensorRT compilation alone takes `{python} FallacyColdstartCalc.cs_compile_time_str` seconds per instance. During a traffic spike requiring `{python} FallacyColdstartCalc.cs_new_instances_str` new instances, aggregate cold-start work reaches `{python} FallacyColdstartCalc.cs_aggregate_cold_str` instance-seconds; if the instances warm in parallel, new capacity becomes useful after about `{python} FallacyColdstartCalc.cs_parallel_warm_str` seconds. Worse, requests hitting cold instances experience `{python} FallacyColdstartCalc.cs_cold_latency_str` ms latency vs. `{python} FallacyColdstartCalc.cs_steady_latency_str` ms steady-state, a `{python} FallacyColdstartCalc.cs_cold_mult_str`$\times$ degradation that violates SLOs precisely when traffic is highest. Systems ignoring cold start meet SLOs during steady state but fail during scale-up events and deployment windows when reliability matters most.
