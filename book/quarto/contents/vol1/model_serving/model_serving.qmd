...
**The context**: Facebook's production ML workload had grown beyond offline training into user-facing ranking, recommendation, and content systems served inside a latency-sensitive data center.

**The failure**: The expensive part of ML moved into production serving. The system had to execute many heterogeneous models under strict tail-latency targets, where batching opportunities and hardware utilization were constrained by live requests.

**The consequence**: Facebook treated inference as a first-class data-center infrastructure problem, co-designing models, platforms, accelerators, and memory systems around production serving demand rather than treating serving as an afterthought.

**The systems lesson**: Training creates the model, but serving pays the recurring bill. A model architecture that is cheap to train can still be too expensive or too variable to serve at fleet scale [@hazelwood2018].
