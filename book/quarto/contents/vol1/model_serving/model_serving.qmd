...
class CostLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpu_cost_per_hour_str = fmt(gpu_cost_per_hour_value, precision=0, commas=False, prefix="USD ")
    ...
    cost_a_str = fmt(cost_per_million_a_value, precision=2, commas=False, prefix="USD ")
    cost_b_str = fmt(cost_per_million_b_value, precision=4, commas=False, prefix="USD ")
...
- Throughput: `{python} ResNetServingSpectrum.cloud_throughput_str` (batched)
...
- Throughput: ~`{python} ResNetServingSpectrum.mobile_throughput_str` (single-stream)
- Memory: `{python} ResNetServingSpectrum.mobile_mem_mb_str` peak (shared with app)
- Energy: `{python} ResNetServingSpectrum.mobile_energy_npu_mj_str` per inference (NPU), `{python} ResNetServingSpectrum.mobile_energy_cpu_mj_str` (CPU)
...
- Throughput: ~`{python} ResNetServingSpectrum.tiny_throughput_str`
...
- Energy: `{python} ResNetServingSpectrum.tiny_energy_mj_str` per inference
...
class BatchingSweetspotCalc:
    ...
    throughput_ratio_str = fmt(throughput_ratio_value, precision=1, commas=False, suffix="×")
    batch1_util_pct_str = fmt(batch1_util_pct_value, precision=0, commas=False, suffix="%")
    batch8_min_arrival_str = fmt_int(round(batch8_throughput.m_as(count / second)), commas=False, suffix=" img/s")
...
2.  **Optimized (Batch 8)** (assumes offered load ≥ **`{python} BatchingSweetspotCalc.batch8_min_arrival_str`**, so the window can fill):
    - **Wait time**: Up to **`{python} BatchingSweetspotCalc.batch8_wait_ms_str`** batching window before inference starts.
    - **Inference time**: Batch 8 inference takes **`{python} BatchingSweetspotCalc.batch8_inference_ms_str`**.
    - **User latency**: `{python} BatchingSweetspotCalc.batch8_wait_ms_str` (wait) + `{python} BatchingSweetspotCalc.batch8_inference_ms_str` (compute) = **`{python} BatchingSweetspotCalc.batch8_user_latency_str`** (worst case for the first request in a full batch).
    - **Service throughput**: 8 images / `{python} BatchingSweetspotCalc.batch8_user_latency_str` ≈ **`{python} BatchingSweetspotCalc.batch8_throughput_str`**.

**Systems insight**: By accepting a **`{python} BatchingSweetspotCalc.latency_increase_str` increase in latency** (`{python} BatchingSweetspotCalc.batch1_ms_str` → `{python} BatchingSweetspotCalc.batch8_user_latency_str`), the system achieves nearly **`{python} BatchingSweetspotCalc.throughput_ratio_str` higher service throughput** on the same hardware when load sustains batch-8 formation. As long as `{python} BatchingSweetspotCalc.batch8_user_latency_str` remains under the `{python} BatchingSweetspotCalc.latency_budget_ms_str` budget, this is "free" capacity relative to batch-1 serving.
...
class LlmCaseStudyHwSpecs:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_str = fmt(h_a100.memory_capacity.m_as(GiB), precision=0, commas=False, suffix=" GB")
...
class LlmServingCalc:
    ...
    h100_bw_tb_str = fmt(_h100_bw_tb, precision=1, commas=False, suffix=" TB/s")
    ...
    kv_capacity_tokens_str = fmt_int(kv_capacity_tokens_value, commas=True, suffix=" tokens")
    ...
    hourly_cost_str = fmt(hourly_cost_value, precision=0, commas=False, prefix="USD ")
    tokens_per_hour_m_str = fmt(tokens_per_hour_value/MILLION, precision=1, commas=False, suffix=" million tokens/hour")
    cost_per_m_tokens_str = fmt(cost_per_m_tokens_value, precision=3, commas=False, prefix="USD ")
