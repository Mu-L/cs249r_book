...
**The scenario**: An e-commerce recommendation system runs comfortably at `{python} BlackFridayCalc.bf_latency_ms_str` latency with `{python} BlackFridayCalc.bf_qps_normal_str` queries per second (QPS).

**The event**: On Black Friday, traffic spikes\index{Traffic Spike!nonlinear failure mode} `{python} BlackFridayCalc.bf_spike_factor_str`$\times$ to `{python} BlackFridayCalc.bf_qps_spike_str`.

**Failure**: The system does not slow down `{python} BlackFridayCalc.bf_spike_factor_str`$\times$. It **collapses**. Latency hits `{python} BlackFridayCalc.bf_collapse_latency_s_str`, then requests start timing out. The servers are `{python} BlackFridayCalc.bf_full_util_pct_str` loaded, but useful throughput drops to near zero because most completed requests have already timed out from the client's perspective.
...
**Trade-off**: Reducing latency from `{python} CostLatencyCalc.latency_b_ms_str` to `{python} CostLatencyCalc.latency_a_ms_str` increases the hardware bill by **`{python} CostLatencyCalc.cost_increase_str`**. Engineers must quantify whether that `{python} CostLatencyCalc.latency_a_ms_str` speedup generates enough business value to justify the `{python} CostLatencyCalc.cost_ratio_str`$\times$ cost increase.
...
- Model format: TensorRT FP16 engine (`{python} ResNetServingSpectrum.cloud_model_mb_str`)
- Inference: `{python} ResNetServingSpectrum.cloud_inf_b1_ms_str` at batch-1, `{python} ResNetServingSpectrum.cloud_inf_b16_ms_str` at batch-16
- Throughput: `{python} ResNetServingSpectrum.cloud_throughput_str` images/second (batched)
- Memory: `{python} ResNetServingSpectrum.cloud_vram_gb_str` VRAM (model + activations for batch-32)

**`{python} ResNetServingSpectrum.mobile_name_str`**:

- Model format: TensorFlow Lite INT8 (`{python} ResNetServingSpectrum.mobile_model_mb_str`)
- Inference: `{python} ResNetServingSpectrum.mobile_inf_npu_ms_str` at batch-1 (NPU), `{python} ResNetServingSpectrum.mobile_inf_cpu_ms_str` (CPU fallback)
- Throughput: ~`{python} ResNetServingSpectrum.mobile_throughput_str` images/second (single-stream)
- Memory: `{python} ResNetServingSpectrum.mobile_mem_mb_str` peak (shared with app)
...
- Model format: Not feasible; ResNet-50 requires `{python} ResNetServingSpectrum.tiny_model_mb_str` weights
- Alternative: MobileNetV2-0.35 quantized to INT8 (`{python} ResNetServingSpectrum.tiny_alt_mb_str`)
- Inference: `{python} ResNetServingSpectrum.tiny_inf_ms_str` at batch-1
- Throughput: ~`{python} ResNetServingSpectrum.tiny_throughput_str` images/second
- Memory: `{python} ResNetServingSpectrum.tiny_arena_kb_str` tensor arena (fits in `{python} ResNetServingSpectrum.tiny_sram_kb_str` SRAM)
...
*   **JSON**\index{JSON!serialization overhead}: Uses ~`{python} SerializationEfficiency.json_size_str` on the wire. Requires ~`{python} SerializationEfficiency.json_parse_str` μs to parse.
*   **Protobuf**: Uses ~`{python} SerializationEfficiency.protobuf_size_str` on the wire. Requires ~`{python} SerializationEfficiency.protobuf_parse_str` μs to parse.
...
* **Latency target $(T_{\text{lat}})$**: `{python} CapacityPlanning.littles_w_ms_str` (0.05 s).
...
Production systems add `{python} CapacityPlanningCalc.cp_headroom_pct_str` headroom for traffic spikes and variance: final count = `{python} CapacityPlanningCalc.cp_gpus_ceil_str` $\times$ `{python} CapacityPlanningCalc.cp_headroom_multiplier_str` = `{python} CapacityPlanningCalc.cp_final_raw_str`, rounded up to **`{python} CapacityPlanningCalc.cp_final_ceil_str`**.
...
For a `{python} ModelSwapCalc.model_size_gb_str` model on PCIe Gen4 x16 (`{python} ModelSwapCalc.pcie_bw_gbs_str` GB/s theoretical bandwidth), loading takes at least:
...
**Systems insight**: Batch size thirty-two achieves `{python} BatchThroughputCalc.throughput_ratio_str`$\times$ higher throughput than batch size 1. However, user-perceived latency includes both queue wait and inference time. With a `{python} BatchThroughputCalc.batch_window_ms_str` batching window and `{python} BatchThroughputCalc.batch32_inference_ms_str` inference, total latency reaches `{python} BatchThroughputCalc.batch32_total_str` vs. `{python} BatchThroughputCalc.batch1_inference_total_ms_str` at batch size 1.
...
**Problem**: A ResNet-50 model is served at $B = 1$, leaving the GPU mostly idle (`{python} BatchingSweetspotCalc.batch1_util_pct_str` utilization). The goal is to increase throughput to reduce cost, subject to a **`{python} BatchingSweetspotCalc.latency_budget_ms_str`** latency budget.

**Math**:

1.  **Baseline (Batch 1)**: Inference = **`{python} BatchingSweetspotCalc.batch1_ms_str`**. Throughput = **`{python} BatchingSweetspotCalc.batch1_imgs_str` img/s**.
2.  **Optimized (Batch 8)**:
    - **Wait time**: A **`{python} BatchingSweetspotCalc.batch8_wait_ms_str`** batching window collects requests.
    - **Inference time**: Batch 8 inference takes **`{python} BatchingSweetspotCalc.batch8_inference_ms_str`**.
    - **User latency**: `{python} BatchingSweetspotCalc.batch8_wait_ms_str` (wait) + `{python} BatchingSweetspotCalc.batch8_inference_ms_str` (compute) = **`{python} BatchingSweetspotCalc.batch8_user_latency_str`**.
    - **Throughput**: eight img / `{python} BatchingSweetspotCalc.batch8_user_latency_str` ≈ **`{python} BatchingSweetspotCalc.batch8_throughput_str` img/s**.

**Systems insight**: By accepting a **`{python} BatchingSweetspotCalc.latency_increase_str`$\times$ increase in latency** (`{python} BatchingSweetspotCalc.batch1_ms_str` → `{python} BatchingSweetspotCalc.batch8_user_latency_str`), the system achieves nearly **`{python} BatchingSweetspotCalc.latency_increase_str`$\times$ higher throughput** on the same hardware. As long as `{python} BatchingSweetspotCalc.batch8_user_latency_str` remains under the 20 ms budget, this is "free" capacity. This trade-off is the primary lever of serving economics.
...
- Average wait: `{python} BatchingOptimization.s1_wait_ms_str`
- Latency budget for inference: `{python} BatchingOptimization.s1_budget_ms_str`
...
from mlsysim.core.constants import THOUSAND, MILLION, ureg
...
*   **Hardware**: 1$\times$ NVIDIA H100 SXM5 GPU (`{python} LlmCaseStudyHwSpecs.h100_mem_str` HBM3, `{python} LlmCaseStudyH100BwRecap.h100_bw_tbs_str` TB/s bandwidth).
...
Every token generation pays a massive "energy tax" to move the model's logic from HBM into compute registers. For comparison, on an A100 `{python} LlmCaseStudyHwSpecs.a100_mem_str` (`{python} LlmCaseStudyHwSpecs.a100_bw_tbs_str` TB/s HBM2e), an 8-billion-parameter Llama 3 model (`{python} LlmCaseStudyHwSpecs.case_study_model_weight_gb_str` INT4) generates tokens at $\approx$ `{python} LlmCaseStudyHwSpecs.a100_token_ms_str` ms/token. Adding more *compute cores* yields **zero** latency improvement; only *faster memory* (Physics) or *smaller models* (Algorithm) can speed up generation.
...
*   $T_{\text{token}} \approx$ `{python} LlmServingCalc.model_weight_gb_str` / `{python} LlmCaseStudyH100BwRecap.h100_bw_tbs_str` TB/s ≈ `{python} LlmServingCalc.token_time_theoretical_ms_str` (theoretical limit).
...
*   Total decode time: `{python} LlmServingCalc.decode_tokens_str` $\times$ `{python} LlmServingCalc.realized_tpot_ms_str` ms/token = `{python} LlmServingCalc.total_decode_s_str`.
...
With 4-bit weights occupying `{python} LlmServingCalc.model_weight_gb_str`, the remaining ~`{python} LlmServingCalc.remaining_vram_gb_str` of VRAM is available for the KV Cache. Using PagedAttention, we can allocate this memory with near-zero fragmentation.

*   Each token requires approximately `{python} LlmServingCalc.kv_per_token_mb_str` of KV cache after accounting for Llama 3-style Grouped Query Attention in the 8-billion-parameter configuration (standard multi-head attention would be about 32 layers $\times$ 4096 dim $\times$ 2 vectors $\times$ 2-byte precision $\approx 0.5$ MB/token, and GQA reduces the KV-head count by about 4$\times$).
*   Total cache capacity ≈ `{python} LlmServingCalc.kv_cache_gb_str` / `{python} LlmServingCalc.kv_per_token_mb_str` MB/token ≈ `{python} LlmServingCalc.kv_capacity_tokens_str`.
*   At `{python} LlmServingCalc.tokens_per_req_str` per request (input + output), the GPU can handle a concurrent batch size of ~`{python} LlmServingCalc.concurrent_batch_str` requests.
