...
class BlackFridayCalc:
    ...
    bf_spike_factor_value = bf_qps_spike_value / bf_qps_normal_value

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(bf_spike_factor_value == 10, "Black Friday example should model a 10x spike.")
    ...
    bf_spike_factor_str = f"{bf_spike_factor_value:.0f}"      # e.g. "10" x
...
**The event**: On Black Friday, traffic spikes\index{Traffic Spike!nonlinear failure mode} `{python} BlackFridayCalc.bf_spike_factor_str`$\times$ to `{python} BlackFridayCalc.bf_qps_spike_str` QPS.

**Failure**: The system does not slow down `{python} BlackFridayCalc.bf_spike_factor_str`$\times$. It **collapses**. Latency hits `{python} BlackFridayCalc.bf_collapse_latency_s_str` seconds, then requests start timing out. The servers are 100 percent loaded, but *useful* throughput drops to near zero because most completed requests have already timed out from the client's perspective.

**Physics**: This is Little's Law and queueing theory in action. As utilization approaches 100 percent, queue lengths diverge nonlinearly rather than linearly. The system spends more time managing the queue (context switching, thrashing) than doing useful work.
...
# │
# │ Goal: Visualize the ~4× per-18-month collapse in inference token cost that
...
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Visualize the ~4× per-18-month collapse in inference token cost that
import numpy as np
from mlsysim import viz
...
ax.text(2021, 0.05, "Trend: ~4× Cheaper\nEvery 18 Months", color=COLORS['grid'],
...
Static inference\index{Static Inference!precomputed predictions} (also called offline or batch inference) precomputes predictions for anticipated inputs and stores them for retrieval. Consider a recommendation system that generates predictions for all user-item pairs nightly. When a user requests recommendations, the system retrieves precomputed results from a lookup table rather than running inference. This approach eliminates inference latency entirely since results already exist, enables quality verification before deployment, and reduces serving costs. However, static inference cannot handle novel inputs that were not anticipated during the batch computation and introduces hours or days of latency when models update [@crankshaw2020].
...
class CostLatencyCalc:
    ...
    cost_b_str = fmt(cost_per_million_b_value, precision=4, commas=False)
...
**Trade-off**: Reducing latency from `{python} CostLatencyCalc.latency_b_ms_str` ms to `{python} CostLatencyCalc.latency_a_ms_str` ms increases the hardware bill by **`{python} CostLatencyCalc.cost_increase_str` percent**. Engineers must quantify whether that `{python} CostLatencyCalc.latency_a_ms_str` ms speedup generates enough business value to justify the `{python} CostLatencyCalc.cost_ratio_str`$\times$ cost increase.
...
from mlsysim.core import Systems
...
class ResNetServingSpectrum:
    ...
    s_cloud = Systems.Tiers.Cloud
    s_mobile = Systems.Tiers.Mobile
    s_tiny = Systems.Tiers.Tiny
...
[^fn-nchw-nhwc-serving]: **NCHW and NHWC (Tensor Memory Layouts)**: These acronyms encode the memory layout order of 4D image tensors: N (batch), C (channels), H (height), W (width). NCHW places all values for one channel contiguously, enabling vectorized convolution on GPUs; NHWC interleaves channels at each spatial position, aligning better with CPU single instruction, multiple data (SIMD) instructions. A format mismatch between client and server silently corrupts inference: the model interprets pixel rows as color channels, producing garbage outputs without raising errors [@aminabadi2022]. \index{NCHW!tensor layout}
...
class SerializationEfficiency:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    floats_count = 1000
    json_bytes_per_float = 9              # text decimal plus separators, approximate
    protobuf_bytes_per_float = 4          # binary float32 payload
    json_parse_us = 50.0
    proto_parse_us = 5.0
    requests_per_sec = 10_000

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    json_size_kb = floats_count * json_bytes_per_float / 1000
    protobuf_size_kb = floats_count * protobuf_bytes_per_float / 1000
    efficiency_gain = json_parse_us/proto_parse_us
    core_saved = (json_parse_us - proto_parse_us) * requests_per_sec / 1_000_000

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(efficiency_gain >= 5, f"Protobuf gain ({efficiency_gain:.1f}x) is too small to justify switching.")
    check(0.4 <= core_saved <= 0.5, f"Serialization CPU savings should be about half a core, got {core_saved:.2f}.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    serial_floats_str = f"{floats_count:,}"
    json_size_str = fmt(json_size_kb, precision=0, commas=False)
    json_parse_str = f"{int(json_parse_us)}"
    protobuf_size_str = fmt(protobuf_size_kb, precision=0, commas=False)
    ...
    core_saved_str = fmt(core_saved, precision=2, commas=False)
```
...
*   **JSON**\index{JSON!serialization overhead}: Uses ~`{python} SerializationEfficiency.json_size_str` KB on the wire. Requires ~`{python} SerializationEfficiency.json_parse_str` μs to parse.
...
For a system processing `{python} SerializationEfficiency.requests_per_sec_str` requests per second, switching to Protobuf saves about `{python} SerializationEfficiency.core_saved_str` CPU cores of serialization work. This `{python} SerializationEfficiency.efficiency_gain_str`$\times$ efficiency gain makes gRPC essential for high-throughput internal microservices.
...
Faster hardware does not automatically mean faster serving\index{Amdahl's Law!preprocessing bottleneck}. In practice, preprocessing and postprocessing often dominate total latency. Studies of production systems show preprocessing consuming 60 to 70 percent of total request time when inference runs on optimized accelerators [@nvidia_triton]. Optimizing exclusively the inference phase yields diminishing returns if the surrounding pipeline remains bottlenecked by CPU operations.
...
# │          dlrm_total_str, dlrm_embed_pct_str, dlrm_mlp_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class DlrmLatencyCalc:
    ...
    dlrm_embed_pct_value = dlrm_embed_ms_value/dlrm_total_ms_value * 100
    dlrm_mlp_pct_value = dlrm_mlp_ms_value/dlrm_total_ms_value * 100

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(dlrm_embed_pct_value > 50, "DLRM example should be dominated by embedding lookups.")
    check(dlrm_mlp_pct_value < 20, "DLRM MLP should not dominate the serving latency example.")
    ...
    dlrm_embed_pct_str = fmt(dlrm_embed_pct_value, precision=0, commas=False)
    dlrm_mlp_pct_str = fmt(dlrm_mlp_pct_value, precision=0, commas=False)
```
...
: **DLRM Serving Latency**: Per-phase breakdown of a recommendation request under a 10 ms p99 budget, contrasting DLRM's memory-bandwidth-bound embedding lookups against ResNet-50's compute-bound forward pass. Adding compute to the inference stage does not help once embedding-table bandwidth is the binding constraint. {#tbl-model-serving-dlrm-latency-budget}

:::

**Key Systems Insight**:
In DLRM, the "Inference" multilayer perceptron (MLP) stage is only `{python} DlrmLatencyCalc.dlrm_mlp_pct_str` percent of the latency. The majority of time (`{python} DlrmLatencyCalc.dlrm_embed_pct_str` percent) is spent in embedding lookups, retrieving massive 128-dim vectors from terabyte-scale tables. This is a memory-bandwidth and capacity-bound workload where adding more compute does not help unless the embedding tables can be served faster.

:::

The two lighthouse cases illustrate the same general failure mode: straightforward optimization efforts target where ML expertise applies (model quantization, pruning) while the binding constraint sits elsewhere (image decoding on CPU for ResNet-50, embedding-table memory bandwidth for DLRM). Adopting *the quantitative approach to serving* exposes these hidden bottlenecks before engineering effort is misallocated.
...
# │ Imports: mlsysim.book (fmt)
# │ Exports: preprocess_ms_str, cpu_gpu_ms_str, resnet_inference_ms_str,
# │          model_10x_ms_str, total_latency_str, non_model_pct_str,
# │          optimized_total_str, amdahl_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class AmdahlServingCalc:
    ...
    cpu_gpu_ms_str = fmt(cpu_gpu_ms_value, precision=1, commas=False)
    resnet_inference_ms_str = fmt(resnet_inference_ms_value, precision=0, commas=False)
    model_10x_ms_str = fmt(model_10x_ms_value, precision=1, commas=False)
    total_latency_str = fmt(total_latency_ms_value, precision=1, commas=False)
...
**Amdahl's Law at Work** (see @sec-machine-foundations-amdahls-law-gustafsons-law-b741 for the formal derivation): preprocessing (`{python} AmdahlServingCalc.preprocess_ms_str` ms) and data transfer (`{python} AmdahlServingCalc.cpu_gpu_ms_str` ms) consume `{python} AmdahlServingCalc.non_model_pct_str` percent of total latency. Optimizing the model 10$\times$ faster (`{python} AmdahlServingCalc.resnet_inference_ms_str` ms → `{python} AmdahlServingCalc.model_10x_ms_str` ms) yields only `{python} AmdahlServingCalc.amdahl_speedup_str`$\times$ end-to-end speedup (from `{python} AmdahlServingCalc.total_latency_str` ms to `{python} AmdahlServingCalc.optimized_total_str` ms). This is why focusing exclusively on model optimization (quantization, pruning) often disappoints: the bottleneck is elsewhere.
...
**Engineering Implication**: Profile before optimizing. If preprocessing dominates, GPU-accelerated pipelines (NVIDIA DALI) may outperform model quantization\index{Model Quantization|see{Quantization}}.

:::

Moving preprocessing to GPU\index{Preprocessing!gpu, accelerated pipelines} can reduce total latency by 6$\times$ in some pipelines by eliminating CPU-GPU data transfers between stages [@nvidia_triton].
...
|:---------------------|:-----------------------------------------------------:|:---------------------------------------------------------:|:--------------:|
| **$224{\times}224$** | `{python} ResolutionBottleneckCalc.act_224_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_224_str` FLOPs/byte |    Compute     |
| **$384{\times}384$** | `{python} ResolutionBottleneckCalc.act_384_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_384_str` FLOPs/byte |    Compute     |
| **$512{\times}512$** | `{python} ResolutionBottleneckCalc.act_512_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_512_str` FLOPs/byte |    Compute     |
| **$640{\times}640$** | `{python} ResolutionBottleneckCalc.act_640_mb_str` MB | `{python} ResolutionBottleneckCalc.ai_640_str` FLOPs/byte |   Near ridge   |

: **Resolution and Compute Bottleneck**: ResNet-50 arithmetic intensity decreases with resolution as activation sizes grow. For a V100 PCIe (14 TFLOPS FP32; the SXM2 variant runs 15.7 TFLOPS FP32) with 900 GB/s of HBM2 memory bandwidth, the ridge point is approximately 16 FLOPs/byte. At $224{\times}224$, compute dominates; by $640{\times}640$, the model is close enough to the ridge that further resolution increases, lower-bandwidth hardware, or cache misses can make memory bandwidth the limiting factor. {#tbl-resolution-bottleneck tbl-colwidths="[25,25,25,25]"}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MS_PER_SEC, ms, second
from mlsysim.fmt import fmt, check
...
class CapacityPlanning:
    ...
    example_latency_s = 0.010 # 10 ms
    example_concurrency = 50
    memory_limited_concurrency = 10

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Step 1: L = lambda * W
    concurrency = lambda_qps * latency_slo_s
    example_arrival_qps = example_concurrency / example_latency_s
    memory_limited_qps = memory_limited_concurrency / example_latency_s

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(concurrency == 50, f"Math broken: 1000 * 0.05 should be 50, got {concurrency}")
    check(example_arrival_qps == 5000, f"Math broken: 50 / 0.01 should be 5000, got {example_arrival_qps}")
    check(memory_limited_qps == 1000, f"Math broken: 10 / 0.01 should be 1000, got {memory_limited_qps}")
    ...
    example_latency_ms_str = f"{int(example_latency_s * MS_PER_SEC)}"
    example_concurrency_str = f"{example_concurrency}"
    example_arrival_qps_str = f"{example_arrival_qps:,.0f}"
    memory_limited_concurrency_str = f"{memory_limited_concurrency}"
    memory_limited_qps_str = f"{memory_limited_qps:,.0f}"
```
...
In queuing theory, $N_{\text{req}}$ traditionally denotes the *length* of the queue (number of items in the system), and $T_{\text{lat}}$ denotes the *response time* or *time in system per request*; queue-only waiting time is $W_q$. Elsewhere in this book, we use $L_{\text{lat}}$ for latency with descriptive subscripts ($L_{\text{lat,wait}}$, $L_{\text{lat,compute}}$) to denote latency components. To preserve standard queuing notation, we retain $N_{\text{req}}$ for queue length and $T_{\text{lat}}$ for time in system in this section. In the batching analysis that follows (@sec-model-serving-dynamic-batching-latencythroughput-tradeoffs-986d), $L_{\text{lat,wait}}$ corresponds to the queueing wait component $W_q$, and $L_{\text{lat,compute}}$ includes inference time.
...
**The law**: $N_{\text{req}} = \lambda T_{\text{lat}}$ (Concurrency = Throughput $\times$ Latency) (see @sec-machine-foundations-littles-law-21a3 for the derivation).

**Scenario**:

*   **Throughput target $(\lambda)$**: `{python} CapacityPlanning.littles_lambda_str` requests/sec.
*   **Latency target $(T_{\text{lat}})$**: `{python} CapacityPlanning.littles_w_ms_str` ms (0.05 s).
...
*   The only options are to reduce latency $(T_{\text{lat}})$ or add more memory $(N_{\text{req}})$.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import second, ms
from mlsysim.fmt import fmt, check
...
1.  **Batch Formation Delay $(W_{\text{form}})$**\index{Batching!batch formation delay, definition}: The time the first request in a batch waits for the last request to arrive.
2.  **Inference Inflation $(W_{\text{inf}})$**\index{Inference Inflation!definition}: The increase in execution time when the GPU processes $B$ samples instead of 1.
...
$$ W_{\text{total}} \approx \underbrace{ \frac{B-1}{2\lambda} }_{\text{Formation Delay}} + \underbrace{ T_{\text{inf}}(B) }_{\text{Inference Time}} $$ {#eq-batching-tax}
...
Little's Law has immediate practical implications. If an inference service averages `{python} CapacityPlanning.example_latency_ms_str` ms per request $(T_{\text{lat}} = 0.01 \text{ s})$ and the system shows `{python} CapacityPlanning.example_concurrency_str` concurrent requests on average $(N_{\text{req}} = 50)$, then the arrival rate must be `{python} CapacityPlanning.example_arrival_qps_str` requests per second. Conversely, if the system must limit concurrent requests to `{python} CapacityPlanning.memory_limited_concurrency_str` (perhaps due to GPU memory constraints) and the service time is `{python} CapacityPlanning.example_latency_ms_str` ms, it can sustain at most `{python} CapacityPlanning.memory_limited_qps_str` requests per second.
...
```{python}
#| label: mm1-utilization-table-calc
#| echo: false
from mlsysim.fmt import fmt, check

class Mm1UtilizationTable:
    """Computes the M/M/1 utilization-latency table for a 5 ms service time."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    service_ms_value = 5
    utilization_values = [0.50, 0.70, 0.80, 0.90, 0.95]

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    latency_multiples = []
    example_latencies_ms = []
    for rho in utilization_values:
        multiple = 1/(1-rho)
        latency_multiples.append(multiple)
        example_latencies_ms.append(service_ms_value * multiple)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(abs(latency_multiples[2] - 5) < 1e-9, "80 percent utilization should be 5x service time.")
    check(abs(latency_multiples[3] - 10) < 1e-9, "90 percent utilization should be 10x service time.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    u50_pct_str = fmt(utilization_values[0] * 100, precision=0, commas=False)
    u70_pct_str = fmt(utilization_values[1] * 100, precision=0, commas=False)
    u80_pct_str = fmt(utilization_values[2] * 100, precision=0, commas=False)
    u90_pct_str = fmt(utilization_values[3] * 100, precision=0, commas=False)
    u95_pct_str = fmt(utilization_values[4] * 100, precision=0, commas=False)
    u50_mult_str = fmt(latency_multiples[0], precision=1, commas=False)
    u70_mult_str = fmt(latency_multiples[1], precision=1, commas=False)
    u80_mult_str = fmt(latency_multiples[2], precision=1, commas=False)
    u90_mult_str = fmt(latency_multiples[3], precision=1, commas=False)
    u95_mult_str = fmt(latency_multiples[4], precision=1, commas=False)
    u50_example_ms_str = fmt(example_latencies_ms[0], precision=0, commas=False)
    u70_example_ms_str = fmt(example_latencies_ms[1], precision=0, commas=False)
    u80_example_ms_str = fmt(example_latencies_ms[2], precision=0, commas=False)
    u90_example_ms_str = fmt(example_latencies_ms[3], precision=0, commas=False)
    u95_example_ms_str = fmt(example_latencies_ms[4], precision=0, commas=False)
```

```{=latex}
\begingroup
\renewcommand{\arraystretch}{1.3} % default is 1,
```

| **Utilization $(\rho)$** | **Latency Multiple** | **Example (5 ms service)** |
|:------------------------:|:--------------------:|:--------------------------:|
| `{python} Mm1UtilizationTable.u50_pct_str` percent | `{python} Mm1UtilizationTable.u50_mult_str`$\times$  | `{python} Mm1UtilizationTable.u50_example_ms_str` ms |
| `{python} Mm1UtilizationTable.u70_pct_str` percent | `{python} Mm1UtilizationTable.u70_mult_str`$\times$  | `{python} Mm1UtilizationTable.u70_example_ms_str` ms |
| `{python} Mm1UtilizationTable.u80_pct_str` percent | `{python} Mm1UtilizationTable.u80_mult_str`$\times$  | `{python} Mm1UtilizationTable.u80_example_ms_str` ms |
| `{python} Mm1UtilizationTable.u90_pct_str` percent | `{python} Mm1UtilizationTable.u90_mult_str`$\times$ | `{python} Mm1UtilizationTable.u90_example_ms_str` ms |
| `{python} Mm1UtilizationTable.u95_pct_str` percent | `{python} Mm1UtilizationTable.u95_mult_str`$\times$ | `{python} Mm1UtilizationTable.u95_example_ms_str` ms |
...
$$W_{\text{p99}} \approx \frac{\text{service time}}{1 - \rho} \cdot \ln\left(\frac{1}{1 - 0.99}\right) \approx \frac{4.6 \cdot \text{service time}}{1 - \rho}$$ {#eq-p99-latency}
...
# │ Show: That 10 V100 GPUs are needed for 5000 QPS at 50 ms p99 with N+1 redundancy.
...
From @eq-p99-latency, $W_{\text{p99}} \approx$ `{python} CapacityPlanningCalc.mm1_p99_factor_str` $\times$ service time / $(1 - \rho)$. Setting $W_{\text{p99}} \leq 50$ ms with 5 ms service time gives $\rho \leq 1 - ($ `{python} CapacityPlanningCalc.mm1_p99_factor_str` $\times 5)/50 = 0.54$. However, the M/M/1 model is conservative for ML inference, which has near-deterministic service times (closer to M/D/1). For M/D/1 queues, average wait is roughly half of M/M/1 at the same utilization, allowing a higher safe operating point. Using the M/D/1-adjusted threshold yields $\rho$ ≤ `{python} CapacityPlanningCalc.cp_rho_safe_str` (`{python} CapacityPlanningCalc.cp_rho_safe_pct_str` percent maximum utilization).

**Step 2**: Calculate required service rate.
\(\mu_{\text{required}} = `{python} CapacityPlanningCalc.cp_peak_qps_str` / `{python} CapacityPlanningCalc.cp_rho_safe_str` = `{python} CapacityPlanningCalc.cp_mu_required_str` \text{ requests/second}\)

**Step 3**: Determine GPU count.
Single V100 throughput at $B = 16$: `{python} CapacityPlanningCalc.cp_v100_throughput_str` images/second
...
Production systems add 30 percent headroom for traffic spikes and variance: final count = `{python} CapacityPlanningCalc.cp_gpus_ceil_str` $\times$ 1.3 = `{python} CapacityPlanningCalc.cp_final_raw_str`, rounded up to **`{python} CapacityPlanningCalc.cp_final_ceil_str` GPUs**.

**Step 5**: Verify fault tolerance.
The 30 percent headroom addresses traffic variance, but production systems also need fault tolerance. With `{python} CapacityPlanningCalc.cp_final_ceil_str` GPUs, losing one leaves `{python} CapacityPlanningCalc.cp_gpus_after_fail_str` GPUs handling `{python} CapacityPlanningCalc.cp_peak_qps_str` QPS. The postfailure utilization is (`{python} CapacityPlanningCalc.cp_peak_qps_str` / `{python} CapacityPlanningCalc.cp_v100_throughput_str`) / `{python} CapacityPlanningCalc.cp_gpus_after_fail_str` = **`{python} CapacityPlanningCalc.cp_util_after_fail_str` percent**.

This remains well below the `{python} CapacityPlanningCalc.cp_rho_safe_pct_str` percent safe utilization threshold, confirming N+1 redundancy is satisfied. For stricter fault tolerance requirements, N+2 redundancy (tolerating two simultaneous failures) would require nine GPUs under the same safe-utilization threshold, or about ten GPUs if the 30 percent headroom must remain after two simultaneous failures.
...
3.  **Common pitfall**: A frequent misconception is that cold start is "just loading weights." In reality, it includes **graph compilation** and **memory allocation**, which can often take longer than the data transfer itself $(\text{BW})$.
...
# │ Show: That precompiling models reduces cold start from ~35s to ~1.5s.
...
class ColdStartCalc:
    """Decomposes cold start latency showing precompiled TensorRT reduces startup from ~35s to ~1.5s."""
    ...
    cs_compile_removed_value = cs_compile_value

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(cs_cloud_total_value > cs_local_total_value * 20, "Cold first deploy should be much slower than local optimized start.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cs_local_str = f"~{fmt(cs_local_total_value, precision=1, commas=False)} s"
    cs_cloud_str = f"~{fmt(cs_cloud_total_value, precision=0, commas=False)} s"
    cs_ssd_str = f"{fmt(cs_ssd_value, precision=1, commas=False)} s"
    cs_s3_str = "3-5 s"                                                       # range for S3
    cs_cuda_str = "0.3-0.5 s"                                                 # range for CUDA
    cs_compile_str = "15-30 s"                                                # range for TensorRT
    cs_warmup_str = f"{fmt(cs_warmup_value, precision=1, commas=False)} s"
    cs_runtime_str = f"{fmt(cs_runtime_overhead_value, precision=1, commas=False)} s"
    cs_compile_removed_str = fmt(cs_compile_removed_value, precision=0, commas=False)
```
...
| **Runtime setup**               |  `{python} ColdStartCalc.cs_runtime_str`  | Allocators, serving process, and framework state |
...
: **ResNet-50 Cold Start Timeline**: Per-phase durations for weight loading, CUDA context creation, TensorRT compilation, and warmup, with totals for the optimized local case and the first-deploy cloud case showing where the dominant cost lives. {#tbl-model-serving-resnet-coldstart}

:::

**Key insight**: Precompiling models and storing the optimized engine eliminates up to `{python} ColdStartCalc.cs_compile_removed_str` seconds of compilation on subsequent deployments.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import PCIE_GEN4_BW, GB, second, ms
...
class ModelSwapCalc:
    ...
    swap_calc_math = md_math(
        f"T_{{\\text{{load}}}} = {model_size_gb_str} \\text{{ GB}} "
        f"/ {pcie_bw_gbs_str} \\text{{ GB/s}} "
        f"\\approx {model_swap_ms_str} \\text{{ ms}}"
    )
```
...
# │          batch32_total_str, batch1_inference_total_ms_str, bt_* table strings
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class BatchThroughputCalc:
    ...
    bt_batch_values = [1, 4, 8, 16, 32]
    bt_inference_ms_values = [5.0, 7.2, 9.1, 14.0, 25.0]
    bt_gpu_util_pct_values = [15, 42, 65, 85, 95]
    ...
    bt_per_image_ms_values = []
    bt_throughput_values = []
    for batch, inf_ms in zip(bt_batch_values, bt_inference_ms_values):
        bt_per_image_ms_values.append(inf_ms / batch)
        bt_throughput_values.append(batch / (inf_ms / 1000))
    ...
    bt_b1_inf_str = fmt(bt_inference_ms_values[0], precision=1, commas=False)
    bt_b4_inf_str = fmt(bt_inference_ms_values[1], precision=1, commas=False)
    bt_b8_inf_str = fmt(bt_inference_ms_values[2], precision=1, commas=False)
    bt_b16_inf_str = fmt(bt_inference_ms_values[3], precision=1, commas=False)
    bt_b32_inf_str = fmt(bt_inference_ms_values[4], precision=1, commas=False)
    bt_b1_per_image_str = fmt(bt_per_image_ms_values[0], precision=1, commas=False)
    bt_b4_per_image_str = fmt(bt_per_image_ms_values[1], precision=1, commas=False)
    bt_b8_per_image_str = fmt(bt_per_image_ms_values[2], precision=1, commas=False)
    bt_b16_per_image_str = fmt(bt_per_image_ms_values[3], precision=1, commas=False)
    bt_b32_per_image_str = fmt(bt_per_image_ms_values[4], precision=1, commas=False)
    bt_b1_throughput_str = fmt(bt_throughput_values[0], precision=0, commas=True)
    bt_b4_throughput_str = fmt(bt_throughput_values[1], precision=0, commas=True)
    bt_b8_throughput_str = fmt(bt_throughput_values[2], precision=0, commas=True)
    bt_b16_throughput_str = fmt(bt_throughput_values[3], precision=0, commas=True)
    bt_b32_throughput_str = fmt(bt_throughput_values[4], precision=0, commas=True)
    bt_b1_util_str = f"{bt_gpu_util_pct_values[0]}"
    bt_b4_util_str = f"{bt_gpu_util_pct_values[1]}"
    bt_b8_util_str = f"{bt_gpu_util_pct_values[2]}"
    bt_b16_util_str = f"{bt_gpu_util_pct_values[3]}"
    bt_b32_util_str = f"{bt_gpu_util_pct_values[4]}"
```
...
|:--------------:|:-------------------:|:---------------------:|:--------------:|:-------------:|
|       1        | `{python} BatchThroughputCalc.bt_b1_inf_str` ms  | `{python} BatchThroughputCalc.bt_b1_per_image_str` ms  | `{python} BatchThroughputCalc.bt_b1_throughput_str` img/s  | `{python} BatchThroughputCalc.bt_b1_util_str` percent  |
|       4        | `{python} BatchThroughputCalc.bt_b4_inf_str` ms  | `{python} BatchThroughputCalc.bt_b4_per_image_str` ms  | `{python} BatchThroughputCalc.bt_b4_throughput_str` img/s  | `{python} BatchThroughputCalc.bt_b4_util_str` percent  |
|       8        | `{python} BatchThroughputCalc.bt_b8_inf_str` ms  | `{python} BatchThroughputCalc.bt_b8_per_image_str` ms  | `{python} BatchThroughputCalc.bt_b8_throughput_str` img/s  | `{python} BatchThroughputCalc.bt_b8_util_str` percent  |
|       16       | `{python} BatchThroughputCalc.bt_b16_inf_str` ms | `{python} BatchThroughputCalc.bt_b16_per_image_str` ms | `{python} BatchThroughputCalc.bt_b16_throughput_str` img/s | `{python} BatchThroughputCalc.bt_b16_util_str` percent |
|       32       | `{python} BatchThroughputCalc.bt_b32_inf_str` ms | `{python} BatchThroughputCalc.bt_b32_per_image_str` ms | `{python} BatchThroughputCalc.bt_b32_throughput_str` img/s | `{python} BatchThroughputCalc.bt_b32_util_str` percent |
...
**Key insight**: Batch size thirty-two achieves `{python} BatchThroughputCalc.throughput_ratio_str`$\times$ higher throughput than batch size 1. However, user-perceived latency includes both queue wait and inference time. With a `{python} BatchThroughputCalc.batch_window_ms_str` ms batching window and `{python} BatchThroughputCalc.batch32_inference_ms_str` ms inference, total latency reaches `{python} BatchThroughputCalc.batch32_total_str` ms vs. `{python} BatchThroughputCalc.batch1_inference_total_ms_str` ms at batch size 1.
...
class BatchingSweetspotCalc:
    """Demonstrates that batch-8 yields ~3× throughput within a 20 ms SLO budget."""
    ...
    batch8_throughput_value = 8 / (batch8_user_latency_ms_value * ms).m_as(second)
...
**Problem**: A ResNet-50 model is served at $B = 1$, leaving the GPU mostly idle (15 percent utilization). The goal is to increase throughput to reduce cost, subject to a **20 ms** latency budget.
...
2.  **Optimized (Batch 8)**:
    ...
    - **Throughput**: eight img / `{python} BatchingSweetspotCalc.batch8_user_latency_str` ms ≈ **`{python} BatchingSweetspotCalc.batch8_throughput_str` img/s**.

**Systems conclusion**: By accepting a **`{python} BatchingSweetspotCalc.latency_increase_str`$\times$ increase in latency** (`{python} BatchingSweetspotCalc.batch1_ms_str` ms → `{python} BatchingSweetspotCalc.batch8_user_latency_str` ms), the system achieves nearly **`{python} BatchingSweetspotCalc.latency_increase_str`$\times$ higher throughput** on the same hardware. As long as `{python} BatchingSweetspotCalc.batch8_user_latency_str` ms remains under the 20 ms budget, this is "free" capacity. This trade-off is the primary lever of serving economics.
...
**Recall** from @sec-model-serving-littles-law-9352: Little's Law $(N_{\text{req}} = \lambda T_{\text{lat}})$ governs all stable queues. When hardware is saturated (throughput $\lambda$ is maxed out), any increase in traffic increases queue depth $(N_{\text{req}})$. Since $\lambda$ cannot grow, **latency $(T_{\text{lat}})$ must grow linearly with queue depth**. This is why **admission control** (rejecting requests when $L_{\text{lat}}$ exceeds a threshold) is the only way to preserve latency during overload.
...
where $L_{\text{lat,wait}}$ is the time spent waiting in the batching queue (corresponding to $L_{\text{lat,queue}}$ in the overall latency budget) and $L_{\text{lat,compute}}(B)$ is the inference time for batch size $B$ (encompassing $L_{\text{lat,infer}}$ plus portions of $L_{\text{lat,pre}}$ and $L_{\text{lat,post}}$). The batching window $T$ bounds wait time ($L_{\text{lat,wait}} \leq T$), while batch size affects compute time through GPU utilization characteristics.
...
class BatchVariabilityCalc:
    """Computes Poisson batch variability for a 10 ms batching window."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    window_s_value = 0.010
    qps_values = [50, 200, 500, 1000]
    ...
    for qps in qps_values:
        mean = qps * window_s_value
        mean_values.append(mean)
        std_values.append(math.sqrt(mean))
        empty_pct_values.append(math.exp(-mean) * 100)

    tail_pct_values = []
    for mean in mean_values:
        threshold = math.ceil(2 * mean)
        cdf_before_threshold = 0
        for k in range(threshold):
            cdf_before_threshold += math.exp(-mean) * mean**k / math.factorial(k)
        tail_pct_values.append((1 - cdf_before_threshold) * 100)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(60 <= empty_pct_values[0] <= 62, "50 QPS empty-window probability should be about 61 percent.")
    check(0.25 <= tail_pct_values[-1] <= 0.40, "1000 QPS upper-tail probability should be about 0.3 percent.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bv_50_mean_str = fmt(mean_values[0], precision=1, commas=False)
    bv_200_mean_str = fmt(mean_values[1], precision=1, commas=False)
    bv_500_mean_str = fmt(mean_values[2], precision=1, commas=False)
    bv_1000_mean_str = fmt(mean_values[3], precision=1, commas=False)
    bv_50_std_str = fmt(std_values[0], precision=1, commas=False)
    bv_200_std_str = fmt(std_values[1], precision=1, commas=False)
    bv_500_std_str = fmt(std_values[2], precision=1, commas=False)
    bv_1000_std_str = fmt(std_values[3], precision=1, commas=False)
    bv_50_empty_str = fmt(empty_pct_values[0], precision=0, commas=False)
    bv_200_empty_str = fmt(empty_pct_values[1], precision=0, commas=False)
    bv_500_empty_str = fmt(empty_pct_values[2], precision=1, commas=False)
    bv_1000_empty_str = fmt(empty_pct_values[3], precision=3, commas=False)
    bv_50_tail_str = fmt(tail_pct_values[0], precision=0, commas=False)
    bv_200_tail_str = fmt(tail_pct_values[1], precision=0, commas=False)
    bv_500_tail_str = fmt(tail_pct_values[2], precision=0, commas=False)
    bv_1000_tail_str = fmt(tail_pct_values[3], precision=1, commas=False)
```

| **Arrival Rate** | **Mean Batch** | **Std Dev** | $P(\text{batch}=0)$ | $P(\text{batch} \ge 2 \times \text{mean})$ |
|:----------------:|:--------------:|:-----------:|:-------------------:|:------------------------------------------:|
|    **50 QPS**    | `{python} BatchVariabilityCalc.bv_50_mean_str` | `{python} BatchVariabilityCalc.bv_50_std_str` | `{python} BatchVariabilityCalc.bv_50_empty_str` percent | `{python} BatchVariabilityCalc.bv_50_tail_str` percent |
|   **200 QPS**    | `{python} BatchVariabilityCalc.bv_200_mean_str` | `{python} BatchVariabilityCalc.bv_200_std_str` | `{python} BatchVariabilityCalc.bv_200_empty_str` percent | `{python} BatchVariabilityCalc.bv_200_tail_str` percent |
|   **500 QPS**    | `{python} BatchVariabilityCalc.bv_500_mean_str` | `{python} BatchVariabilityCalc.bv_500_std_str` | `{python} BatchVariabilityCalc.bv_500_empty_str` percent | `{python} BatchVariabilityCalc.bv_500_tail_str` percent |
|   **1000 QPS**   | `{python} BatchVariabilityCalc.bv_1000_mean_str` | `{python} BatchVariabilityCalc.bv_1000_std_str` | `{python} BatchVariabilityCalc.bv_1000_empty_str` percent | `{python} BatchVariabilityCalc.bv_1000_tail_str` percent |

: **Batch Size Variability**: At low traffic, batching windows frequently contain zero requests (wasted GPU cycles). At moderate traffic, batch sizes fluctuate significantly around the mean. High traffic provides more stable batching, and the probability of batches exceeding twice the mean size decreases as traffic grows (from `{python} BatchVariabilityCalc.bv_50_tail_str` percent at 50 QPS to `{python} BatchVariabilityCalc.bv_1000_tail_str` percent at 1000 QPS), reflecting the law of large numbers. {#tbl-batch-variability}
...
For ResNet-50 on a V100 GPU, service time approximately scales as $S(B) = 5 \text{ ms} + 0.6 B$ (5 ms fixed overhead plus 0.6 ms per additional image in the batch). This linear approximation captures the dominant trend; actual service times may deviate slightly due to memory hierarchy effects. With $T = 10 \text{ ms}$ batching window:
...
# │
# │ Imports: mlsysim.core.constants (ms, second), mlsysim.book (fmt)
# │ Exports: throughput_*, b1_*, b32_*, ba_* table strings, il_*, T_window_str
...
class BatchingAnalysisCalc:
    ...
    ba_b1_service_str = fmt(service_time_value(1), precision=1, commas=False)
    ba_b4_service_str = fmt(service_time_value(4), precision=1, commas=False)
    ba_b8_service_str = fmt(service_time_value(8), precision=1, commas=False)
    ba_b16_service_str = fmt(service_time_value(16), precision=1, commas=False)
    ba_b32_service_str = fmt(service_time_value(32), precision=1, commas=False)
    ba_b1_latency_str = fmt(latencies_value[1], precision=1, commas=False)
    ba_b4_latency_str = fmt(latencies_value[4], precision=1, commas=False)
    ba_b8_latency_str = fmt(latencies_value[8], precision=1, commas=False)
    ba_b16_latency_str = fmt(latencies_value[16], precision=1, commas=False)
    ba_b32_latency_str = fmt(latencies_value[32], precision=1, commas=False)
    ba_b1_throughput_str = fmt(throughputs_value[1], precision=0, commas=False)
    ba_b4_throughput_str = fmt(throughputs_value[4], precision=0, commas=False)
    ba_b8_throughput_str = fmt(throughputs_value[8], precision=0, commas=False)
    ba_b16_throughput_str = fmt(throughputs_value[16], precision=0, commas=False)
    ba_b32_throughput_str = fmt(throughputs_value[32], precision=0, commas=False)
...
|:--------------:|:----------------:|:-----------------:|:--------------:|:--------------:|
|       1        | `{python} BatchingAnalysisCalc.ba_b1_service_str` ms  | `{python} BatchingAnalysisCalc.ba_b1_latency_str` ms  | `{python} BatchingAnalysisCalc.ba_b1_throughput_str` img/s  |      Low       |
|       4        | `{python} BatchingAnalysisCalc.ba_b4_service_str` ms  | `{python} BatchingAnalysisCalc.ba_b4_latency_str` ms  | `{python} BatchingAnalysisCalc.ba_b4_throughput_str` img/s  |    Moderate    |
|       8        | `{python} BatchingAnalysisCalc.ba_b8_service_str` ms  | `{python} BatchingAnalysisCalc.ba_b8_latency_str` ms  | `{python} BatchingAnalysisCalc.ba_b8_throughput_str` img/s  |      Good      |
|       16       | `{python} BatchingAnalysisCalc.ba_b16_service_str` ms | `{python} BatchingAnalysisCalc.ba_b16_latency_str` ms | `{python} BatchingAnalysisCalc.ba_b16_throughput_str` img/s |      High      |
|       32       | `{python} BatchingAnalysisCalc.ba_b32_service_str` ms | `{python} BatchingAnalysisCalc.ba_b32_latency_str` ms | `{python} BatchingAnalysisCalc.ba_b32_throughput_str` img/s |    Maximum     |

: **Batching Throughput Analysis**: ResNet-50 throughput on V100 with 10 ms batching window. Throughput increases 14.6$\times$ from batch size one to 32 (64 to 935 img/s), but total latency more than doubles (15.6 ms to 34.2 ms). The optimal configuration depends on whether the latency SLO or throughput target is the binding constraint. {#tbl-batching-throughput}
...
*   **Case 1 (batch 1)**: Overhead (`{python} BatchingAnalysisCalc.il_overhead_str` ms) ≫ Compute (`{python} BatchingAnalysisCalc.il_compute_b1_str` ms). Efficiency ≈ `{python} BatchingAnalysisCalc.il_eff_b1_str` percent. The GPU is mostly waiting.
*   **Case 2 (batch 32)**: Overhead (`{python} BatchingAnalysisCalc.il_overhead_str` ms) ≪ Compute (`{python} BatchingAnalysisCalc.il_compute_b32_str` ms). Efficiency ≈ `{python} BatchingAnalysisCalc.il_eff_b32_str` percent. The GPU is crunching numbers.

**Golden rule**: Increase batch size until the **Latency Term** becomes negligible (< `{python} BatchingAnalysisCalc.il_threshold_str` percent of total time). Beyond this point, additional batching yields minimal throughput but imposes a linear latency penalty.
...
$$B_{\text{max}} = \max\left\{B : \frac{T}{2} + S(B) \leq L_{\text{lat,target}}\right\}$$ {#eq-latency-constrained-batch}
...
class BatchingOptimization:
    ...
    s1_wait_ms_str = f"{s1_wait:.1f}"
    s1_budget_ms_str = f"{s1_budget:.1f}"
...
**Scenario one: Conservative window (T = `{python} BatchingOptimization.s1_window_ms_str` ms)**
...
**Scenario two: Aggressive window (T = `{python} BatchingOptimization.s2_window_ms_str` ms)**
...
# │
# │ Imports: math, mlsysim.core.constants (ms, second), mlsysim.book (fmt, check);
...
class SloViolationCalc:
    ...
    cumulative_prob_value = 0
    p99_batch_value = 0
    while cumulative_prob_value < 0.99:
        cumulative_prob_value += (
            math.exp(-mean_batch_value)
            * mean_batch_value**p99_batch_value
            / math.factorial(p99_batch_value)
        )
        p99_batch_value += 1
    p99_batch_value -= 1
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(p99_batch_value == 11, f"Poisson p99 batch should be 11, got {p99_batch_value}.")
...
$$L_{\text{lat,p99}} \approx T + S(b_{\text{p99}})$$ {#eq-p99-batch-latency}
where $b_{\text{p99}}$ is the 99th percentile batch size. For $\lambda$ = `{python} SloViolationCalc.qps_str` QPS and $T$ = `{python} SloViolationCalc.T_slo_str` ms:
...
def adaptive_batching_window(queue_depth, arrival_rate, slo_ms):
    ...
    if arrival_rate > 0:
        ...
        estimated_wait_ms = requests_needed / arrival_rate * 1000
...
```{python}
#| label: adaptive-batching-impact-calc
#| echo: false
from mlsysim.fmt import fmt, check

class AdaptiveBatchingImpactCalc:
    """Computes latency and throughput changes from adaptive batching."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    fixed_window_ms_value = 10
    fixed_latency_ms_value = 15
    fixed_throughput_value = 650
    adaptive_latency_ms_value = 11
    adaptive_throughput_value = 680

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    latency_reduction_pct_value = (1 - adaptive_latency_ms_value/fixed_latency_ms_value) * 100
    throughput_improvement_pct_value = (adaptive_throughput_value/fixed_throughput_value - 1) * 100

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(latency_reduction_pct_value > 25, "Adaptive batching should reduce average latency by about 27 percent.")
    check(throughput_improvement_pct_value < 10, "Adaptive batching throughput gain should be modest.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    ab_fixed_window_ms_str = fmt(fixed_window_ms_value, precision=0, commas=False)
    ab_fixed_latency_ms_str = fmt(fixed_latency_ms_value, precision=0, commas=False)
    ab_fixed_throughput_str = fmt(fixed_throughput_value, precision=0, commas=False)
    ab_adaptive_latency_ms_str = fmt(adaptive_latency_ms_value, precision=0, commas=False)
    ab_adaptive_throughput_str = fmt(adaptive_throughput_value, precision=0, commas=False)
    ab_latency_reduction_str = fmt(latency_reduction_pct_value, precision=0, commas=False)
    ab_throughput_improvement_str = fmt(throughput_improvement_pct_value, precision=0, commas=False)
```

- Fixed window (`{python} AdaptiveBatchingImpactCalc.ab_fixed_window_ms_str` ms): Average latency `{python} AdaptiveBatchingImpactCalc.ab_fixed_latency_ms_str` ms, throughput `{python} AdaptiveBatchingImpactCalc.ab_fixed_throughput_str` img/s
- Adaptive window: Average latency `{python} AdaptiveBatchingImpactCalc.ab_adaptive_latency_ms_str` ms (`{python} AdaptiveBatchingImpactCalc.ab_latency_reduction_str` percent reduction), throughput `{python} AdaptiveBatchingImpactCalc.ab_adaptive_throughput_str` img/s (`{python} AdaptiveBatchingImpactCalc.ab_throughput_improvement_str` percent improvement)
...
class ParetoBatchingCalc:
    """Summarizes the representative batching Pareto frontier."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    low_window_throughput_value = 890
    max_window_throughput_value = 1350
    low_window_p99_ms_value = 18
    max_window_p99_ms_value = 98

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    throughput_gain_pct_value = (max_window_throughput_value/low_window_throughput_value - 1) * 100
    p99_ratio_value = max_window_p99_ms_value/low_window_p99_ms_value

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(50 <= throughput_gain_pct_value <= 55, "Pareto table should show about 52 percent throughput gain.")
    check(5 <= p99_ratio_value <= 6, "Pareto table should show about 5.4x p99 latency increase.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pb_throughput_gain_str = fmt(throughput_gain_pct_value, precision=0, commas=False)
    pb_p99_ratio_str = fmt(p99_ratio_value, precision=1, commas=False)
```
...
class PracticalConfigCalc:
    ...
    pc_expected_batch_value = pc_qps_value * (pc_config_window_ms_value * ms).m_as(second)
...
- Configuration: $T$ = `{python} PracticalConfigCalc.pc_config_window_ms_str` ms, $b_{\text{max}}$ = `{python} PracticalConfigCalc.pc_config_batch_str`
- Predicted p99 latency: `{python} PracticalConfigCalc.pc_predicted_p99_ms_str` ms (within SLO)
- Predicted peak capacity: `{python} PracticalConfigCalc.pc_predicted_throughput_str` img/s
- Served throughput at stated traffic: `{python} PracticalConfigCalc.pc_qps_str` img/s (arrival-limited; batch-32 cap handles bursts)

### Continuous batching {#sec-model-serving-continuous-batching-8bb6}

Autoregressive models\index{Autoregressive Models!token generation} like language models generate outputs token by token—each new token depends on all previously generated tokens, so generation is inherently sequential. The dynamic batching examined in @sec-model-serving-throughput-optimization-18d1 assumes fixed-length outputs. LLMs violate this assumption: if one sequence in a batch of eight finishes after ten tokens while others need 100 tokens, 87.5 percent of the compute for that sequence slot is wasted\index{Sequence Length Variability!batch waste} [@yu2022orca].
...
class TrafficAdaptiveBatchingCalc:
    """Computes average batch sizes for traffic-adaptive batching choices."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    qps_values = [100, 500, 1000, 5000]
    window_ms_values = [20, 8, 5, 2]

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    avg_batch_values = []
    for qps, window_ms in zip(qps_values, window_ms_values):
        avg_batch_values.append(qps * (window_ms * ms).m_as(second))

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(avg_batch_values[0] < avg_batch_values[-1], "Average batch size should grow with traffic.")
    check(window_ms_values[0] > window_ms_values[-1], "Batching window should shrink as traffic rises.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tab_100_avg_batch_str = fmt(avg_batch_values[0], precision=1, commas=False)
    tab_500_avg_batch_str = fmt(avg_batch_values[1], precision=1, commas=False)
    tab_1000_avg_batch_str = fmt(avg_batch_values[2], precision=1, commas=False)
    tab_5000_avg_batch_str = fmt(avg_batch_values[3], precision=1, commas=False)
```

| **Arrival Rate** | **Optimal Window** | **Avg Batch Size** | **p99 Latency** |
|:-----------------|:------------------:|:------------------:|:---------------:|
| **100 QPS**      |       20 ms        | `{python} TrafficAdaptiveBatchingCalc.tab_100_avg_batch_str` |      45 ms      |
| **500 QPS**      |        8 ms        | `{python} TrafficAdaptiveBatchingCalc.tab_500_avg_batch_str` |      42 ms      |
| **1,000 QPS**    |        5 ms        | `{python} TrafficAdaptiveBatchingCalc.tab_1000_avg_batch_str` |      38 ms      |
| **5,000 QPS**    |        2 ms        | `{python} TrafficAdaptiveBatchingCalc.tab_5000_avg_batch_str` |      35 ms      |

: **Traffic-Adaptive Batching**\index{Adaptive Batching!queue depth}: Higher traffic enables shorter windows while still achieving larger batches. The optimal window decreases even as batch sizes grow because more requests arrive per unit time. {#tbl-traffic-adaptive}
...
: **Multi-Camera Frame Timeline**: Event-by-event timeline for one synchronized frame set across six cameras at 30 FPS, illustrating the synchronization budget (12 ms of the 33 ms hard deadline) consumed by sensor jitter before batch inference can begin. {#tbl-model-serving-multicamera-timeline}
...
# │          m_battery_million_str, m_power_mw_str, m_cpu_total_ms_str,
# │          m_cpu_total_mj_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class MobileServingCalc:
    ...
    m_npu_mj_value = 0.8
    m_ui_mj_value = 0.2
    m_phone_battery_wh_value = 10
    m_cpu_fallback_ms_value = 45
    m_cpu_fallback_mj_value = 4.2
    ...
    m_battery_j_value = m_phone_battery_wh_value * 3600
    m_battery_inferences_value = m_battery_j_value / (m_total_mj_value / 1000)
    m_power_mw_value = m_total_mj_value / m_total_ms_value * 1000
    m_cpu_total_ms_value = m_total_ms_value - m_npu_ms_value + m_cpu_fallback_ms_value
    m_cpu_total_mj_value = m_total_mj_value - m_npu_mj_value + m_cpu_fallback_mj_value

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(m_jpeg_mj_value > m_npu_mj_value, "JPEG decode should dominate NPU energy in this example.")
    check(11 <= m_battery_inferences_value/1_000_000 <= 13, "10 Wh battery should support about 12M modeled inferences.")
    ...
    m_battery_wh_str = fmt(m_phone_battery_wh_value, precision=0, commas=False)
    m_battery_million_str = fmt(m_battery_inferences_value/1_000_000, precision=0, commas=False)
    m_power_mw_str = fmt(m_power_mw_value, precision=0, commas=False)
    m_cpu_stage_ms_str = fmt(m_cpu_fallback_ms_value, precision=0, commas=False)
    m_cpu_stage_mj_str = fmt(m_cpu_fallback_mj_value, precision=1, commas=False)
    m_cpu_total_ms_str = fmt(m_cpu_total_ms_value, precision=0, commas=False)
    m_cpu_total_mj_str = fmt(m_cpu_total_mj_value, precision=1, commas=False)
```
...
- **Energy per inference**: `{python} MobileServingCalc.m_total_mj_str` enables ~`{python} MobileServingCalc.m_battery_million_str` million inferences per `{python} MobileServingCalc.m_battery_wh_str` Wh battery (typical smartphone)
- **Thermal budget**: At `{python} MobileServingCalc.m_total_mj_str`/`{python} MobileServingCalc.m_total_ms_str` = `{python} MobileServingCalc.m_power_mw_str` mW sustained, indefinite operation without throttling
- **NPU vs. CPU trade-off**: CPU fallback replaces the `{python} MobileServingCalc.m_npu_ms_str`, `{python} MobileServingCalc.m_npu_mj_str` NPU inference stage with a `{python} MobileServingCalc.m_cpu_stage_ms_str` ms, `{python} MobileServingCalc.m_cpu_stage_mj_str` mJ CPU stage; the full pipeline would rise from `{python} MobileServingCalc.m_total_ms_str` and `{python} MobileServingCalc.m_total_mj_str` to about `{python} MobileServingCalc.m_cpu_total_ms_str` ms and `{python} MobileServingCalc.m_cpu_total_mj_str` mJ before additional system overhead.
- **Memory footprint**: 150 MB peak (model + activations), competing with app memory

**Critical insight**: Even at batch size one, the mobile NPU achieves 82 percent utilization because its compute capacity matches single-image workloads. This differs from data center GPUs, which achieve only 15 percent utilization at batch size one because their massive parallelism requires larger batches to saturate.
...
1. **Energy Budget**\index{Energy!budget, mobile inference}: Each inference depletes battery. In the modeled pipeline, `{python} MobileServingCalc.m_total_mj_str` at 22 FPS is about `{python} MobileServingCalc.m_power_mw_str` mW for the inference path alone; camera, display, ISP, and OS overhead add to that total in a full photo app. The optimization target shifts from throughput to energy-per-inference.
...
[^fn-kv-cache-serving]: **KV Cache (Key-Value Cache)**: To avoid redundant work, the system caches the Key and Value vectors from previous tokens, which remain valid throughout generation. This design choice is the direct cause of the dynamic memory growth described; the cache's size grows linearly with every generated token, making memory management, not computation, the primary constraint. For the 70B-class grouped-query-attention example used below, the FP16 KV cache is about 0.31 MB per token per sequence, so a batch of 32 requests at an 8,000-token context requires roughly 80 GB just for KV cache. Without grouped-query or multi-query attention, the KV cache can be several times larger. \index{KV Cache!memory scaling}
...
class CarbonCostCalc:
    """Energy cost per LLM token: poor utilization causes about 3× higher Joules/token."""
...
**Systems conclusion**: A typical `{python} CarbonCostCalc.cc_response_tokens_str`-token response consumes ≈ **`{python} CarbonCostCalc.cc_response_joules_str` Joules**.
...
**Key insight**: The `{python} RuntimeComparisonCalc.rt_trt_int8_speedup_str`$\times$ speedup from TensorRT INT8 comes at the cost of: (1) quantization calibration data, (2) potential accuracy loss (<1 percent for ResNet-50), and (3) NVIDIA-specific deployment.
...
::: {.callout-notebook title="ResNet-50: Precision trade-offs on V100"}
...
: **Precision Trade-offs on V100**: Latency, memory footprint, accuracy, and Tensor Core utilization for ResNet-50 in FP32, FP16, and INT8 (PTQ and QAT). FP16 is a near-free 2$\times$ speedup, while INT8 buys an additional 3$\times$ at the cost of calibration data and a fraction of a percentage point in accuracy. {#tbl-model-serving-precision-tradeoffs}
...
- INT8 achieves `{python} PrecisionTradeoffCalc.pt_int8_speedup_str`$\times$ speedup but loses `{python} PrecisionTradeoffCalc.pt_int8_acc_loss_str` percentage points of accuracy with post-training quantization (PTQ)
...
Since the graph structure is known, the compiler can precalculate the exact memory offsets for every tensor\index{Memory Planning!tensor allocation}. This leads to the central architectural choice of *JIT vs. ahead-of-time (AOT) compilation*.
...
*   **Ahead-of-Time (AOT)**\index{AOT Compilation!predeployment}: Compiles the graph before deployment (for example, `torch.export`, TensorRT `trtexec`).
...
# │ How: Calculate unit costs using representative hourly rates and measured images-per-second.
# │
...
@tbl-model-serving-resnet-cloud-cost compares representative hourly cost, throughput, and per-million-image cost for serving ResNet-50 on common cloud instance classes:

::: {tbl-colwidths="[31,22,22,25]"}
...
: **ResNet-50 Cloud Inference Cost Comparison**: Representative hourly cost, sustained throughput, and resulting cost per one million images for CPU, T4, and V100 instance classes, showing the T4-class GPU as the cost-per-inference winner despite its higher hourly rate. {#tbl-model-serving-resnet-cloud-cost}
...
```{python}
#| label: capacity-planning-summary-calc
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ CAPACITY PLANNING SUMMARY
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Capacity planning section summary
# │
# │ Goal: Keep the later economics summary tied to the detailed ResNet-50
# │       capacity-planning calculation.
# │ Show: The p99 target, QPS target, safe utilization, and final GPU count.
# │ How: Re-export the earlier detailed workflow's outputs near this prose.
# │
# │ Imports: mlsysim.book (check)
# │ Exports: cp_p99_target_ms_str, cp_peak_qps_str, cp_rho_safe_pct_str,
# │          cp_final_ceil_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check

class CapacityPlanningSummary:
    """Local summary values for the economics capacity-planning paragraph."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    cp_p99_target_ms_value = CapacityPlanningCalc.cp_p99_target_ms_value
    cp_peak_qps_value = CapacityPlanningCalc.cp_peak_qps_value
    cp_rho_safe_value = CapacityPlanningCalc.cp_rho_safe_value
    cp_gpus_ceil_value = CapacityPlanningCalc.cp_gpus_ceil_value
    cp_final_ceil_value = CapacityPlanningCalc.cp_final_ceil_value

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    cp_rho_safe_pct_value = cp_rho_safe_value * 100

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(cp_p99_target_ms_value == 50, "Capacity summary should use the detailed 50 ms p99 target.")
    check(cp_final_ceil_value >= cp_gpus_ceil_value, "Headroom should not reduce the detailed GPU count.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cp_p99_target_ms_str = f"{cp_p99_target_ms_value:.0f}"
    cp_peak_qps_str = f"{cp_peak_qps_value:,}"
    cp_rho_safe_pct_str = f"{cp_rho_safe_pct_value:.0f}"
    cp_final_ceil_str = f"{cp_final_ceil_value}"
```

The worked example in @sec-model-serving-queuing-theory-tail-latency-29a6 demonstrates the complete workflow: starting from a `{python} CapacityPlanningSummary.cp_p99_target_ms_str` ms p99 SLO and `{python} CapacityPlanningSummary.cp_peak_qps_str` QPS target, deriving the safe utilization threshold of `{python} CapacityPlanningSummary.cp_rho_safe_pct_str` percent from @eq-p99-latency, and determining GPU count with headroom of `{python} CapacityPlanningSummary.cp_final_ceil_str` V100s. Production systems typically provision for peak load plus 30 percent headroom, using auto-scaling to reduce costs during low-traffic periods while meeting latency objectives during peaks. The key insight from capacity planning is that throughput numbers are meaningful only when coupled with latency guarantees: a system achieving 10,000 QPS but violating the p99 SLO on 5 percent of requests is actually serving 9,500 valid QPS and failing on the rest.

### Production case study: Serving Llama-3-8B {#sec-model-serving-production-case-study-serving-llama38b-0499}

To synthesize the principles of latency budgeting, memory management, and hardware efficiency, this case study analyzes a complete production profile for a modern LLM serving workload\index{LLM Serving!8B parameter case study}\index{LLM Serving!production case study}. The Llama-3-8B\index{Llama-3-8B!serving case study} numbers in the rest of this section are computed for the 8B architecture; @fig-kv-cache-growth below is a separate 70B-class contrast that motivates why KV-cache memory is the dominant cost at any LLM scale before the 8B-specific analysis begins.

The bottleneck that dominates LLM serving costs is KV cache memory. The memory curves in @fig-kv-cache-growth climb steeply for larger batch sizes, illustrating *why* long-context serving is memory bound even on H100s; the figure uses 70B-class assumptions to amplify the effect, and the 8B-specific numbers follow.

::: {#fig-kv-cache-growth fig-env="figure" fig-pos="htb" fig-cap="**The KV-Cache Explosion**: Memory usage vs. Context Length for a 70B-class model. Assumes 80 layers, d_model = 8192, FP16 KV cache, GQA (8$\times$). The linear growth of the Key-Value cache (storing attention history) quickly consumes available GPU memory (red dashed line). For batch size 32 (purple), the system hits the 'OOM Zone' at just 8k context length, forcing a trade-off between batch size (throughput) and context window (capability)." fig-alt="Line chart showing memory usage increasing linearly with context length. Multiple lines for different batch sizes. Red dashed horizontal line marks GPU memory limit. Purple line for batch 32 crosses into OOM zone at 8k context."}
...
# │          LlmCaseStudyHwSpecs.h100_bw_tbs),
# │          "Physics of Token Generation" callout (~50 lines below:
# │          LlmCaseStudyHwSpecs.a100_bw_tbs)
# │
# │ Goal: Provide H100 memory/bandwidth and A100 bandwidth for the Llama-3-8B
...
# │ Exports: LlmCaseStudyHwSpecs.h100_mem, LlmCaseStudyHwSpecs.h100_bw_tbs,
# │          LlmCaseStudyHwSpecs.a100_bw_tbs,
# │          LlmCaseStudyHwSpecs.a100_token_time_ms
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim import Hardware
...
class LlmCaseStudyHwSpecs:
    ...
    h_h100 = Hardware.Cloud.H100
    llama8b_int4_gb_value = 3.5

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    a100_bw_tbs_value = h_a100.memory_bw.m_as(TB/second)
    h100_bw_tbs_value = h_h100.memory_bw.m_as(TB/second)
    h100_mem_value = h_h100.memory_capacity.m_as(GiB)
    a100_token_time_ms_value = llama8b_int4_gb_value / (a100_bw_tbs_value * 1000) * 1000

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_bw_tbs = f"{a100_bw_tbs_value:.2f}"                               # e.g. "2.04" TB/s
    h100_bw_tbs = f"{h100_bw_tbs_value:.2f}"                               # e.g. "3.35" TB/s
    h100_mem = f"{h100_mem_value:.0f}"                                     # e.g. "80" GB
    a100_token_time_ms = f"{a100_token_time_ms_value:.1f}"                 # e.g. "1.7" ms
```

#### Workload profile {#sec-model-serving-workload-profile-a380}

*   **Model**: Llama-3-8B (quantized to 4-bit using activation-aware weight quantization (AWQ); see @sec-model-compression for quantization techniques).
*   **Hardware**: 1$\times$ NVIDIA H100 SXM5 GPU (`{python} LlmCaseStudyHwSpecs.h100_mem` GB HBM3, `{python} LlmCaseStudyHwSpecs.h100_bw_tbs` TB/s bandwidth).
...
Recall the energy-movement invariant\index{Energy-Movement Invariant!decode phase} from @sec-data-engineering: moving a bit is 100--1,000$\times$ more expensive than computing on it. In the **Decode Phase**, this law determines the physical "cost per word."
...
Every token generation pays a massive "energy tax" to move the model's logic from HBM into compute registers. For comparison, on an A100 80 GB (`{python} LlmCaseStudyHwSpecs.a100_bw_tbs` TB/s HBM2e), Llama-3-8B (3.5 GB int4) generates tokens at approximately `{python} LlmCaseStudyHwSpecs.a100_token_time_ms` ms/token. Adding more *compute cores* yields **zero** latency improvement; only faster memory (Physics) or smaller models (Algorithm) can speed up generation.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import H100_MEM_BW, TB, GB, MB, second, ms, SEC_PER_HOUR, MILLION
from mlsysim.fmt import fmt
...
class LlmServingCalc:
    ...
    realized_tpot_ms_value = 10      # conservative production target (theoretical min ~1-2 ms)
    decode_tokens_value = 256
    kv_cache_gb_value = 72
    kv_per_token_mb_value = 0.125
    ...
    _h100_bw_gbs = H100_MEM_BW.m_as(GB/second)
    token_time_theoretical_ms_value = (model_weight_gb_value / _h100_bw_gbs * second).m_as(ms)
    total_decode_s_value = (decode_tokens_value * realized_tpot_ms_value * ms).m_as(second)
    kv_capacity_tokens_value = int((kv_cache_gb_value * GB).m_as(MB) / kv_per_token_mb_value)
    concurrent_batch_value = int(kv_capacity_tokens_value/tokens_per_req_value)
    req_time_s_value = ttft_s_value + (decode_tokens_value * realized_tpot_ms_value * ms).m_as(second)
    ...
    h100_bw_tb_str = fmt(_h100_bw_tb, precision=2, commas=False)
    ...
    kv_per_token_mb_str = f"{kv_per_token_mb_value:.3f}"
...
*   $T_{\text{token}} \approx$ `{python} LlmServingCalc.model_weight_gb_str` GB / `{python} LlmServingCalc.h100_bw_tb_str` TB/s ≈ `{python} LlmServingCalc.token_time_theoretical_ms_str` ms (theoretical limit).
*   Accounting for kernel launch overhead, attention computation, and a conservative production safety margin, realized $T_{\text{token}}$ is approximately `{python} LlmServingCalc.realized_tpot_ms_str` ms.
...
*   Each token requires approximately `{python} LlmServingCalc.kv_per_token_mb_str` MB of KV cache after accounting for Llama-3-8B-style Grouped Query Attention (standard multi-head attention would be about 32 layers $\times$ 4096 dim $\times$ 2 vectors $\times$ 2-byte precision $\approx 0.5$ MB/token, and GQA reduces the KV-head count by about 4$\times$).
...
For a representative H100 SXM5 instance cost of USD `{python} LlmServingCalc.hourly_cost_str` per hour (actual rates vary substantially by provider, region, commitment, and availability):

*   Total tokens per hour: `{python} LlmServingCalc.concurrent_batch_str` batch $\times$ (3,600 s/hr / `{python} LlmServingCalc.req_time_s_str` s/req) $\times$ `{python} LlmServingCalc.tokens_per_req_str` tokens/req ≈ `{python} LlmServingCalc.tokens_per_hour_m_str` million tokens/hour.
...
Engineers assume identical model weights preserve validation set performance\index{Latency!inference vs. user-perceived}. In production, preprocessing differences silently shift inputs outside the training distribution. @sec-model-serving-trainingserving-skew-7b99 shows *how* training-serving skew causes accuracy degradation despite identical weights: PIL vs. OpenCV resize interpolation alone can shift accuracy by `{python} FallacySkewCalc.fs_resize_drop_str` percent, float64 vs. float32 normalization produces different values, or feature computation timing changes. A model achieving `{python} FallacySkewCalc.fs_val_acc_str` percent validation accuracy drops to `{python} FallacySkewCalc.fs_prod_acc_str` percent in production from these preprocessing mismatches, a `{python} FallacySkewCalc.fs_acc_drop_str` percentage point loss invisible to latency monitoring. Standard monitoring checking exceptions and latency violations fails to detect this silent degradation. Production systems require either identical preprocessing code for training and serving, or statistical monitoring comparing input distributions to catch drift before accuracy degrades.
