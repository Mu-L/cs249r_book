...
class BlackFridayCalc:
    ...
    bf_full_util_pct_str = fmt(bf_full_util_pct_value, precision=0, commas=False)
```
...
**Failure**: The system does not slow down `{python} BlackFridayCalc.bf_spike_factor_str`$\times$. It **collapses**. Latency hits `{python} BlackFridayCalc.bf_collapse_latency_s_str` seconds, then requests start timing out. The servers are `{python} BlackFridayCalc.bf_full_util_pct_str` percent loaded, but useful throughput drops to near zero because most completed requests have already timed out from the client's perspective.

**Physics**: This is Little's Law and queueing theory in action. As utilization approaches `{python} BlackFridayCalc.bf_full_util_pct_str` percent, queue lengths diverge nonlinearly rather than linearly. The system spends more time managing the queue (context switching, thrashing) than doing useful work.
...
class CapacityPlanningAnchor:
    ...
    qps_display_str = fmt(qps_target, precision=0, commas=True)
...
**Problem**: How much concurrent request capacity does a system need to serve `{python} CapacityPlanningAnchor.qps_display_str` queries per second?
...
class CapacityPlanningCalc:
    ...
    cp_headroom_pct_str = fmt(cp_headroom_pct_value, precision=0, commas=False)
    cp_headroom_multiplier_str = fmt(cp_headroom_value, precision=1, commas=False)
```
...
Production systems add `{python} CapacityPlanningCalc.cp_headroom_pct_str` percent headroom for traffic spikes and variance: final count = `{python} CapacityPlanningCalc.cp_gpus_ceil_str` $\times$ `{python} CapacityPlanningCalc.cp_headroom_multiplier_str` = `{python} CapacityPlanningCalc.cp_final_raw_str`, rounded up to **`{python} CapacityPlanningCalc.cp_final_ceil_str` GPUs**.

**Step 5**: Verify fault tolerance.
The `{python} CapacityPlanningCalc.cp_headroom_pct_str` percent headroom addresses traffic variance, but production systems also need fault tolerance. With `{python} CapacityPlanningCalc.cp_final_ceil_str` GPUs, losing one leaves `{python} CapacityPlanningCalc.cp_gpus_after_fail_str` GPUs handling `{python} CapacityPlanningCalc.cp_peak_qps_str` QPS. The postfailure utilization is (`{python} CapacityPlanningCalc.cp_peak_qps_str` / `{python} CapacityPlanningCalc.cp_v100_throughput_str`) / `{python} CapacityPlanningCalc.cp_gpus_after_fail_str` = **`{python} CapacityPlanningCalc.cp_util_after_fail_str` percent**.

This remains well below the `{python} CapacityPlanningCalc.cp_rho_safe_pct_str` percent safe utilization threshold, confirming N+1 redundancy is satisfied. For stricter fault tolerance requirements, N+2 redundancy (tolerating two simultaneous failures) would require `{python} CapacityPlanningCalc.cp_n_plus_2_min_str` GPUs under the same safe-utilization threshold, or about `{python} CapacityPlanningCalc.cp_n_plus_2_headroom_str` GPUs if the `{python} CapacityPlanningCalc.cp_headroom_pct_str` percent headroom must remain after two simultaneous failures.
...
class BatchingSweetspotCalc:
    ...
    batch1_util_pct_value = 15
    latency_budget_ms_value = 20
    ...
    batch1_util_pct_str = fmt(batch1_util_pct_value, precision=0, commas=False)
    latency_budget_ms_str = fmt(latency_budget_ms_value, precision=0, commas=False)
```

::: {#nbk-model-serving-batching-sweet-spot .callout-notebook title="The batching sweet spot"}

**Problem**: A ResNet-50 model is served at $B = 1$, leaving the GPU mostly idle (`{python} BatchingSweetspotCalc.batch1_util_pct_str` percent utilization). The goal is to increase throughput to reduce cost, subject to a **`{python} BatchingSweetspotCalc.latency_budget_ms_str` ms** latency budget.
...
class MobileServingCalc:
    ...
    battery_wh = 10
    sustained_mw = m_total_mj_value / m_total_ms_value
    inferences_per_battery_m = (battery_wh * 3600 * THOUSAND) / m_total_mj_value / MILLION
    ...
    battery_wh_str = fmt(battery_wh, precision=0, commas=False)
    sustained_mw_str = fmt(sustained_mw, precision=0, commas=False)
    inferences_per_battery_m_str = fmt(inferences_per_battery_m, precision=0, commas=False)
    peak_memory_mb_str = fmt(peak_memory_mb_value, precision=0, commas=False)
```
...
- **Energy per inference**: `{python} MobileServingCalc.m_total_mj_str` enables ~`{python} MobileServingCalc.inferences_per_battery_m_str` million inferences per `{python} MobileServingCalc.battery_wh_str` Wh battery (typical smartphone)
- **Thermal budget**: At `{python} MobileServingCalc.m_total_mj_str` / `{python} MobileServingCalc.m_total_ms_str` = **`{python} MobileServingCalc.sustained_mw_str` mW** sustained, indefinite operation without throttling
...
- **Memory footprint**: `{python} MobileServingCalc.peak_memory_mb_str` MB peak (model + activations), competing with app memory
...
class LlmCaseStudyHwSpecs:
    ...
    a100_bw_gbs_value = h_a100.memory_bw.m_as(GB/second)
    ...
    a100_token_ms_value = case_study_model_weight_gb / a100_bw_gbs_value * 1000

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_str = fmt(h_a100.memory_capacity.m_as(GiB), precision=0, commas=False)
    ...
    case_study_model_weight_gb_str = fmt(case_study_model_weight_gb, precision=1, commas=False)
    a100_token_ms_str = fmt(a100_token_ms_value, precision=1, commas=False)
```
...
Every token generation pays a massive "energy tax" to move the model's logic from HBM into compute registers. For comparison, on an A100 `{python} LlmCaseStudyHwSpecs.a100_mem_str` GB (`{python} LlmCaseStudyHwSpecs.a100_bw_tbs_str` TB/s HBM2e), an 8-billion-parameter Llama 3 model (`{python} LlmCaseStudyHwSpecs.case_study_model_weight_gb_str` GB INT4) generates tokens at $\approx$ `{python} LlmCaseStudyHwSpecs.a100_token_ms_str` ms/token. Adding more *compute cores* yields **zero** latency improvement; only *faster memory* (Physics) or *smaller models* (Algorithm) can speed up generation.
...
class LlmServingCalc:
    ...
    prompt_tokens_str = fmt(prompt_tokens_value, precision=0, commas=True)
    sec_per_hr_str = fmt_int(sec_per_hr, commas=True)
...
*   Prefill-limited admissions: `{python} LlmServingCalc.prefill_tokens_per_s_str` prompt tokens/s divided by `{python} LlmServingCalc.prompt_tokens_str` prompt tokens/req = `{python} LlmServingCalc.prefill_req_per_s_str` requests/s. This is below the KV-residency limit of `{python} LlmServingCalc.concurrent_batch_str` batch / `{python} LlmServingCalc.req_time_s_str` s/req ≈ `{python} LlmServingCalc.decode_req_capacity_str` requests/s, so full-request throughput is `{python} LlmServingCalc.prefill_req_per_s_str` req/s $\times$ `{python} LlmServingCalc.sec_per_hr_str` s/hr $\times$ `{python} LlmServingCalc.tokens_per_req_str` tokens/req ≈ `{python} LlmServingCalc.tokens_per_hour_m_str` million tokens/hour.
