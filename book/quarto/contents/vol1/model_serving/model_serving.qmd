...
A single traffic spike that exceeds this margin can cascade into system-wide failure.
...
class CostLatencyCalc:
    ...
    cost_b_str = fmt(cost_per_million_b_value, precision=4, commas=False, prefix="$")
...
The table reveals the throughput-latency trade-off in stark terms: larger batches dramatically improve hardware efficiency but increase per-request latency. The 6.4$\times$ throughput gain from batch one to batch thirty-two comes at the cost of 5$\times$ higher inference time, and user-perceived latency compounds further once the batching window wait is included. In practice, the optimal batch size depends on both the latency Service Level Objective (SLO) and the arrival rate of requests. A system serving interactive recommendations under a 20 ms SLO cannot use batch thirty-two, even though hardware utilization at that point approaches 95 percent. The question facing every serving engineer is therefore quantitative: determining the largest batch size that still meets a given latency SLO. A small sweep over batch sizes turns that trade-off into an operating point.
...
The model processes the 1,000-token prompt in parallel\index{Prefill Phase!compute-bound}\index{Prefill Phase!parallel processing}. On an H100, this compute-bound operation achieves approximately 10,000 tokens/s: $T_{\text{prefill}} = 1000$ tokens / 10{,}000 tokens/s = 100 ms. Accounting for 20 ms of system overhead (network ingress, tokenization), the TTFT is 120 ms, comfortably within the 200 ms SLO.
