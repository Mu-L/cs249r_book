...
class BlackFridayCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bf_latency_ms_str = fmt(bf_latency_ms_value, precision=0, commas=False)              # e.g. "50" ms
    bf_qps_normal_str = fmt(bf_qps_normal_value, precision=0)            # e.g. "1,000" QPS
    bf_qps_spike_str = fmt(bf_qps_spike_value, precision=0)              # e.g. "10,000" QPS
    bf_spike_factor_str = fmt(bf_spike_factor_value, precision=0, commas=False)          # e.g. "10" x
    bf_collapse_latency_s_str = fmt(bf_collapse_latency_s_value, precision=0, commas=False)  # e.g. "10" seconds
```
...
**Context**: Facebook's production ML workload had grown beyond offline training into user-facing ranking, recommendation, and content systems served inside a latency-sensitive data center [@hazelwood2018].

**Failure**: The expensive part of ML moved into production serving. The system had to execute many heterogeneous models under strict tail-latency targets, where batching opportunities and hardware utilization were constrained by live requests.

**Consequence**: Facebook treated inference as a first-class data-center infrastructure problem, co-designing models, platforms, accelerators, and memory systems around production serving demand rather than treating serving as an afterthought.

**Systems lesson**: Training creates the model, but serving pays the recurring bill. A model architecture that is cheap to train can still be too expensive or too variable to serve at fleet scale.
...
class StaticBatchCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_photos_str = fmt(n_photos_value, precision=0)                                   # e.g. "10,000" photos
    inference_ms_str = fmt(inference_ms_value, precision=0, commas=False)                             # e.g. "5" ms
    batch_total_s_str = fmt(batch_total_s_value, precision=0, commas=False)# e.g. "50" seconds
    dynamic_latency_budget_ms_str = fmt(dynamic_latency_budget_ms_value, precision=0, commas=False)   # e.g. "100" ms
```
...
class CostLatencyCalc:
    ...
    latency_a_ms_str = fmt(latency_a_ms_value, precision=0, commas=False)
    throughput_a_rps_str = fmt(throughput_a_rps_value, precision=0, commas=False)
    latency_b_ms_str = fmt(latency_b_ms_value, precision=0, commas=False)
    throughput_b_rps_str = fmt(throughput_b_rps_value, precision=0, commas=False)
...
class ResNetServingSpectrum:
    ...
    cloud_inf_b16_ms_str = fmt(cloud_inf_b16_ms, precision=0, commas=False)
    cloud_throughput_str = fmt(cloud_throughput, precision=0)
    cloud_vram_gb_str = fmt(cloud_vram_gb, precision=0, commas=False)

    mobile_model_mb_str = fmt(mobile_size_mb, precision=0)
    mobile_inf_npu_ms_str = fmt(mobile_inf_npu_ms, precision=0, commas=False)
    mobile_inf_cpu_ms_str = fmt(mobile_inf_cpu_ms, precision=0, commas=False)
    mobile_throughput_str = fmt(mobile_throughput, precision=0, commas=False)
    ...
    tiny_inf_ms_str = fmt(tiny_inf_ms, precision=0, commas=False)
    tiny_throughput_str = MarkdownStr("8")
    tiny_arena_kb_str = MarkdownStr("320")
    tiny_sram_kb_str = fmt(s_tiny.ram.m_as('KiB'), precision=0)
    tiny_energy_mj_str = fmt(tiny_energy_mj, precision=0, commas=False)
```
...
class SerializationEfficiency:
    ...
    json_parse_str = fmt(int(json_parse_us), precision=0, commas=False)
    protobuf_size_str = MarkdownStr("4")
    protobuf_parse_str = fmt(int(proto_parse_us), precision=0, commas=False)
...
class LatencyTableCalc:
    ...
    p_jpeg_str = fmt(p_jpeg_value, precision=0, commas=False, suffix="%")
    p_resize_str = fmt(p_resize_value, precision=0, commas=False, suffix="%")
    p_norm_str = fmt(p_norm_value, precision=0, commas=False, suffix="%")
    p_transfer_str = fmt(p_transfer_value, precision=0, commas=False, suffix="%")
    p_inf_str = fmt(p_inf_value, precision=0, commas=False, suffix="%")
    p_post_str = fmt(p_post_value, precision=0, commas=False, prefix="~", suffix="%")
```
...
class ServingTaxMicrosecondsCalc:
    ...
    micro_tax_max_pct_str = fmt(micro_tax_max_pct, precision=0, commas=False)
```
...
class CapacityPlanningAnchor:
    ...
    qps_str = fmt(qps_target, precision=0, commas=False)
...
class CapacityPlanning:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    littles_lambda_str = fmt(lambda_qps, precision=0)
    littles_w_ms_str = fmt(int(latency_slo_s * MS_PER_SEC), precision=0, commas=False)
...
class BatchingTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    wait_time_b1_ms_str = fmt(int(w_form_b1), precision=0, commas=False)
    wait_time_b32_ms_str = fmt(int(w_form_b32), precision=0, commas=False)
    lat_b1_ms_str = fmt(lat_b1, precision=1, commas=False)
    lat_b32_ms_str = fmt(lat_b32, precision=1, commas=False)
    penalty_ratio_str = fmt(int(penalty_ratio), precision=0, commas=False)
```
...
class CapacityPlanningCalc:
    ...
    cp_gpus_ceil_str = fmt(cp_gpus_ceil_value, precision=0, commas=False)
    ...
    cp_v100_throughput_str = fmt(cp_v100_throughput_value, precision=0)
    cp_rho_safe_str = fmt(cp_rho_safe_value, precision=2, commas=False)
    cp_rho_safe_pct_str = fmt(cp_rho_safe_value * 100, precision=0, commas=False)
...
From @eq-p99-latency, $W_{\text{p99}} \approx$ `{python} CapacityPlanningCalc.mm1_p99_factor_str` $\times$ service time / $(1 - \rho)$. Setting $W_{\text{p99}} \leq 50$ ms with 5 ms service time gives $\rho \leq 1 - ($ `{python} CapacityPlanningCalc.mm1_p99_factor_str` $\times 5)/50 = `{python} CapacityPlanningCalc.cp_rho_safe_str` (`{python} CapacityPlanningCalc.cp_rho_safe_pct_str` percent maximum utilization). This uses the conservative M/M/1 p99 bound from the displayed equation rather than applying an average-wait M/D/1 adjustment to a tail-latency SLO.
...
class ModelSwapCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_size_gb_str = fmt(model_size_gb_value, precision=0, commas=False)
...
class BatchingSweetspotCalc:
    ...
    batch1_imgs_str = fmt(batch1_imgs_value, precision=0, commas=False)
...
For ResNet-50 on a V100 GPU, service time approximately scales as $S(B) = 5 \text{ ms} + 0.6 B$ (5 ms fixed overhead plus 0.6 ms per image in the batch). This linear approximation captures the dominant trend; actual service times may deviate slightly due to memory hierarchy effects. With $T = 10 \text{ ms}$ batching window:
...
class BatchingAnalysisCalc:
    ...
    il_eff_b1_str = fmt(il_eff_b1_pct_value, precision=0, commas=False)
    il_eff_b32_str = fmt(il_eff_b32_pct_value, precision=0, commas=False)
...
class BatchingOptimization:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    s1_window_ms_str = fmt(int(s1_window), precision=0, commas=False)
    ...
    s1_batch_str = fmt(s1_batch, precision=0, commas=False)
    s1_throughput_str = fmt(int(s1_tput), precision=0)

    s2_window_ms_str = fmt(int(s2_window), precision=0, commas=False)
    ...
    s2_throughput_str = fmt(int(s2_tput), precision=0)

    throughput_gain_pct_str = fmt(tput_gain, precision=0, commas=False)
    latency_avg_increase_ms_str = fmt(latency_increase, precision=0, commas=False)
    # Simplified P99 increase for prose consistency
    latency_p99_increase_ms_str = fmt(int(s2_window - s1_window), precision=0, commas=False)
```
...
class SloViolationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    qps_str = fmt(qps_value, precision=0, commas=False)
...
class PracticalConfigCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pc_slo_ms_str = fmt(pc_slo_ms_value, precision=0, commas=False)
    pc_qps_str = fmt(pc_qps_value, precision=0, commas=False)
    pc_batch_budget_ms_str = fmt(pc_batch_budget_ms_value, precision=0, commas=False)
    pc_max_window_ms_str = fmt(pc_max_window_ms_value, precision=0, commas=False)
    ...
    pc_config_window_ms_str = fmt(pc_config_window_ms_value, precision=0, commas=False)
    ...
    pc_predicted_p99_ms_str = fmt(pc_predicted_p99_ms_value, precision=0, commas=False)
    pc_predicted_throughput_str = fmt(pc_predicted_throughput_value, precision=0)
```
...
class TrafficAdaptiveBatchingCalc:
    ...
    window_100_ms_str = fmt((rows[0][1] / (1 * ms).m_as(second)), precision=0, commas=False)
    avg_batch_100_str = fmt(rows[0][2], precision=1, commas=False)
    latency_100_ms_str = fmt(rows[0][3], precision=0, commas=False)

    qps_500_str = fmt(rows[1][0], precision=0)
    window_500_ms_str = fmt((rows[1][1] / (1 * ms).m_as(second)), precision=0, commas=False)
    avg_batch_500_str = fmt(rows[1][2], precision=1, commas=False)
    latency_500_ms_str = fmt(rows[1][3], precision=0, commas=False)
    ...
    window_5000_ms_str = fmt((rows[3][1] / (1 * ms).m_as(second)), precision=0, commas=False)
    avg_batch_5000_str = fmt(rows[3][2], precision=1, commas=False)
    latency_5000_ms_str = fmt(rows[3][3], precision=0, commas=False)
```
...
class MobileServingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    m_cam_ms_str = fmt(m_cam_ms_value, precision=0, commas=False, suffix=" ms")
    m_jpeg_ms_str = fmt(m_jpeg_ms_value, precision=0, commas=False, suffix=" ms")
    m_resize_ms_str = fmt(m_resize_ms_value, precision=0, commas=False, suffix=" ms")
    m_npu_ms_str = fmt(m_npu_ms_value, precision=0, commas=False, suffix=" ms")
    m_ui_ms_str = fmt(m_ui_ms_value, precision=0, commas=False, suffix=" ms")
    m_total_ms_str = fmt(m_total_ms_value, precision=0, commas=False, suffix=" ms")
...
class CarbonCostCalc:
    ...
    cc_smartphone_str = fmt(_smartphone_joules, precision=0)
    cc_boiling_str = fmt(_boiling_joules, precision=0)
...
**Systems insight**: A typical `{python} CarbonCostCalc.cc_response_tokens_str`-token response consumes ≈ **`{python} CarbonCostCalc.cc_response_joules_str` Joules**.
...
class PrecisionTradeoffCalc:
    ...
    pt_fp32_mem_mb_str = fmt(pt_fp32_mem_mb_value, precision=0, commas=False)
    ...
    pt_fp16_mem_mb_str = fmt(pt_fp16_mem_mb_value, precision=0, commas=False)
    pt_fp16_acc_str = fmt(pt_fp16_acc_value, precision=2, commas=False)
    pt_fp16_util_str = fmt(pt_fp16_util_value, precision=0, commas=False)
    pt_int8_ms_str = fmt(pt_int8_ms_value, precision=1, commas=False)
    pt_int8_mem_mb_str = fmt(pt_int8_mem_mb_value, precision=0, commas=False)
    ...
    pt_int8_util_str = fmt(pt_int8_util_value, precision=0, commas=False)
...
class LlmCaseStudyHwSpecs:
    ...
    h100_mem_str = fmt(h100_mem_value, precision=0, commas=False)          # e.g. "80" GB
```
...
Every token generation pays a massive "energy tax" to move the model's logic from HBM into compute registers. For comparison, on an A100 80 GB (`{python} LlmCaseStudyHwSpecs.a100_bw_tbs_str` TB/s HBM2e), Llama-3-8B (3.5 GB int4) generates tokens at $\approx 1.7$ ms/token. Adding more *compute cores* yields **zero** latency improvement; only *faster memory* (Physics) or *smaller models* (Algorithm) can speed up generation.
...
# │ How: Calculate TPOT, concurrent batch size, request throughput, and $/M tokens for Llama-3-8B on H100.
# │
...
class LlmServingCalc:
    """Llama-3-8B serving economics: capacity bounds residency, prefill/decode bound throughput."""
    ...
    prefill_tokens_per_s_value = 10_000
    kv_cache_gb_value = 72
    kv_per_token_mb_value = 0.125
    tokens_per_req_value = prompt_tokens_value + decode_tokens_value
    ttft_s_value = 0.12
    ...
    tokens_per_hour_value = serving_req_per_s_value * SEC_PER_HOUR * tokens_per_req_value
    ...
    check(prefill_req_per_s_value < decode_req_capacity_value, "Prefill should bound full-request throughput")
    ...
    realized_tpot_ms_str = fmt(realized_tpot_ms_value, precision=0, commas=False)
    decode_tokens_str = fmt(decode_tokens_value, precision=0, commas=False)
    total_decode_s_str = fmt(total_decode_s_value, precision=2, commas=False)
    kv_cache_gb_str = fmt(kv_cache_gb_value, precision=0, commas=False)
    kv_per_token_mb_str = fmt(kv_per_token_mb_value, precision=3, commas=False)
    kv_capacity_tokens_str = fmt(kv_capacity_tokens_value, precision=0)
    tokens_per_req_str = fmt(tokens_per_req_value, precision=0)
    concurrent_batch_str = fmt(concurrent_batch_value, precision=0, commas=False)
    req_time_s_str = fmt(req_time_s_value, precision=2, commas=False)
    prefill_tokens_per_s_str = fmt(prefill_tokens_per_s_value, precision=0)
    prefill_req_per_s_str = fmt(prefill_req_per_s_value, precision=0, commas=False)
    decode_req_capacity_str = fmt(decode_req_capacity_value, precision=0, commas=False)
    ...
    remaining_vram_gb_str = fmt(remaining_vram_gb_value, precision=0, commas=False)
```
...
class FallacyLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fl_utilization_high_pct_str = fmt(fl_utilization_high_value * 100, precision=0, commas=False)
    fl_service_slow_ms_str = fmt(fl_service_slow_ms_value, precision=0, commas=False)
    fl_wait_slow_ms_str = fmt(fl_wait_slow_ms_value, precision=0, commas=False)
    fl_service_fast_ms_str = fmt(fl_service_fast_ms_value, precision=0, commas=False)
    fl_utilization_new_pct_str = fmt(fl_utilization_new_value * 100, precision=0, commas=False)
    fl_wait_fast_ms_str = fmt(fl_wait_fast_ms_value, precision=1, commas=False)
    fl_inference_gain_ms_str = fmt(fl_inference_gain_ms_value, precision=0, commas=False)
    fl_queuing_improvement_str = fmt(fl_queuing_improvement_value, precision=0, commas=False)
    fl_total_slow_ms_str = fmt(fl_total_slow_ms_value, precision=0, commas=False)
...
class FallacyUtilizationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fu_util_high_pct_str = fmt(fu_util_high_value * 100, precision=0, commas=False)
    fu_util_mod_pct_str = fmt(fu_util_mod_value * 100, precision=0, commas=False)
    fu_wait_high_factor_str = fmt(fu_wait_high_factor_value, precision=0, commas=False)
    fu_total_high_factor_str = fmt(1/(1-fu_util_high_value), precision=0, commas=False)
    fu_cost_reduction_str = fmt(fu_cost_reduction_pct_value, precision=0, commas=False)
    fu_latency_increase_str = fmt(fu_latency_increase_factor_value, precision=0, commas=False)
    fu_service_ms_str = fmt(fu_service_ms_value, precision=0, commas=False)
...
class FallacySkewCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fs_val_acc_str = fmt(fs_val_acc_value, precision=0, commas=False)
    fs_prod_acc_str = fmt(fs_prod_acc_value, precision=0, commas=False)
    fs_acc_drop_str = fmt(fs_acc_drop_value, precision=0, commas=False)
...
class TailLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tl_util_pct_str = fmt(tl_util_value * 100, precision=0, commas=False)
    tl_service_ms_str = fmt(tl_service_ms_value, precision=0, commas=False)
    tl_avg_ms_str = fmt(tl_avg_ms_value, precision=0, commas=False)
...
class FallacyBatchingCalc:
    ...
    fb_throughput_gain_str = fmt(fb_throughput_gain_pct_value, precision=0, commas=False)
    fb_inf_small_ms_str = fmt(fb_inf_small_ms_value, precision=0, commas=False)
    fb_inf_large_ms_str = fmt(fb_inf_large_ms_value, precision=0, commas=False)
    fb_padding_waste_min_str = fmt(fb_padding_waste_min_pct_value, precision=0, commas=False)
    fb_padding_waste_max_str = fmt(fb_padding_waste_max_pct_value, precision=0, commas=False)
```
...
# │ Show: 10 new instances × 30s TensorRT compilation = 300 instance-seconds;
...
class FallacyColdstartCalc:
    """Cold starts compound: 10 new instances at 30s compile time = 300 instance-seconds."""
    ...
    cs_compile_time_str = fmt(cs_compile_time_s_value, precision=0, commas=False)
    cs_aggregate_cold_str = fmt(cs_aggregate_cold_s_value, precision=0, commas=False)
    cs_parallel_warm_str = fmt(cs_parallel_warm_s_value, precision=0, commas=False)
    cs_steady_latency_str = fmt(cs_steady_latency_ms_value, precision=0, commas=False)
    cs_cold_latency_str = fmt(cs_cold_latency_ms_value, precision=0, commas=False)
    cs_cold_mult_str = fmt(cs_cold_multiplier_value, precision=0, commas=False)
```
