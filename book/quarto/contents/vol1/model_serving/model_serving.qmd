...
Cloud APIs\index{Server Traffic!Poisson process} and web services typically receive requests following a Poisson process\index{Poisson Process!serving arrivals},[^fn-poisson-serving] where arrivals are independent and uniformly distributed over time. @eq-poisson-batch expresses the expected batch size for Poisson arrivals with rate $\lambda$ and batching window $T$:
