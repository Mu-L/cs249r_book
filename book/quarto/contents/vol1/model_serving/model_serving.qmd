...
class BlackFridayCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    bf_latency_ms_str = fmt(bf_latency_ms_value, precision=1, commas=False)              # e.g. "50" ms
    bf_qps_normal_str = fmt(bf_qps_normal_value, precision=1)            # e.g. "1,000" QPS
    bf_qps_spike_str = fmt(bf_qps_spike_value, precision=1)              # e.g. "10,000" QPS
    bf_spike_factor_str = fmt(bf_spike_factor_value, precision=1, commas=False)          # e.g. "10" x
    bf_collapse_latency_s_str = fmt(bf_collapse_latency_s_value, precision=1, commas=False)  # e.g. "10" seconds
```
...
class StaticBatchCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    n_photos_str = fmt(n_photos_value, precision=1)                                   # e.g. "10,000" photos
    inference_ms_str = fmt(inference_ms_value, precision=1, commas=False)                             # e.g. "5" ms
    batch_total_s_str = fmt(batch_total_s_value, precision=1, commas=False)# e.g. "50" seconds
    dynamic_latency_budget_ms_str = fmt(dynamic_latency_budget_ms_value, precision=1, commas=False)   # e.g. "100" ms
```
...
class CostLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpu_cost_per_hour_str = fmt(gpu_cost_per_hour_value, precision=2, commas=False)
    latency_a_ms_str = fmt(latency_a_ms_value, precision=1, commas=False)
    throughput_a_rps_str = fmt(throughput_a_rps_value, precision=1, commas=False)
    latency_b_ms_str = fmt(latency_b_ms_value, precision=1, commas=False)
    throughput_b_rps_str = fmt(throughput_b_rps_value, precision=1, commas=False)
    ...
    cost_increase_str = fmt(cost_increase_pct_value, precision=1, commas=False)
    cost_ratio_str = fmt(cost_ratio_value, precision=1, commas=False)
```
...
class ResNetServingSpectrum:
    ...
    cloud_model_mb_str = fmt(cloud_size_mb, precision=1)
    cloud_inf_b1_ms_str = fmt(cloud_inf_b1_ms, precision=1, commas=False)
    cloud_inf_b16_ms_str = fmt(cloud_inf_b16_ms, precision=1, commas=False)
    cloud_throughput_str = fmt(cloud_throughput, precision=1)
    cloud_vram_gb_str = fmt(cloud_vram_gb, precision=1, commas=False)

    mobile_model_mb_str = fmt(mobile_size_mb, precision=1)
    mobile_inf_npu_ms_str = fmt(mobile_inf_npu_ms, precision=1, commas=False)
    mobile_inf_cpu_ms_str = fmt(mobile_inf_cpu_ms, precision=1, commas=False)
    mobile_throughput_str = fmt(mobile_throughput, precision=1, commas=False)
    ...
    tiny_model_mb_str = fmt(tiny_original_mb, precision=1)
    tiny_alt_mb_str = fmt(tiny_alt_mb, precision=1)
    tiny_inf_ms_str = fmt(tiny_inf_ms, precision=1, commas=False)
    tiny_throughput_str = MarkdownStr("8")
    tiny_arena_kb_str = MarkdownStr("320")
    tiny_sram_kb_str = fmt(s_tiny.ram.m_as('KB'), precision=1)
    tiny_energy_mj_str = fmt(tiny_energy_mj, precision=1, commas=False)
```
...
class SerializationEfficiency:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    serial_floats_str = fmt(floats_count, precision=1)
    json_size_str = MarkdownStr("9")
    json_parse_str = fmt(int(json_parse_us), precision=1, commas=False)
    protobuf_size_str = MarkdownStr("4")
    protobuf_parse_str = fmt(int(proto_parse_us), precision=1, commas=False)
    requests_per_sec_str = MarkdownStr("10,000")
    efficiency_gain_str = fmt(efficiency_gain, precision=1, commas=False)
```
...
class TailLatencyRatioCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tail_ratio_str = fmt(tail_ratio_value, precision=1, commas=False)      # e.g. "40" times
```
...
class LatencyTableCalc:
    ...
    p_jpeg_str = fmt(p_jpeg_value, precision=1, commas=False, suffix="%")
    p_resize_str = fmt(p_resize_value, precision=1, commas=False, suffix="%")
    p_norm_str = fmt(p_norm_value, precision=1, commas=False, suffix="%")
    p_transfer_str = fmt(p_transfer_value, precision=1, commas=False, suffix="%")
    p_inf_str = fmt(p_inf_value, precision=1, commas=False, suffix="%")
    p_post_str = fmt(p_post_value, precision=1, commas=False, prefix="~", suffix="%")
```
...
class LatencyBudgetCalc:
    ...
    resnet_inference_ms_str = fmt(resnet_inference_ms_value, precision=1, commas=False)
    tensorrt_inference_ms_str = fmt(tensorrt_inference_ms_value, precision=1, commas=False)
    ...
    preprocess_pct_str = fmt(preprocess_pct_value, precision=1, commas=False)
    tensorrt_preprocess_pct_str = fmt(tensorrt_preprocess_pct_value, precision=1, commas=False)
```
...
class DlrmLatencyCalc:
    ...
    dlrm_embed_str = fmt(dlrm_embed_ms_value, precision=1, commas=False, suffix=" ms")
    dlrm_mlp_str = fmt(dlrm_mlp_ms_value, precision=1, commas=False, suffix=" ms")
    dlrm_post_str = fmt(dlrm_post_ms_value, precision=1, commas=False, suffix=" ms")
    dlrm_total_str = fmt(dlrm_total_ms_value, precision=1, commas=False, suffix=" ms")
```
...
class AmdahlServingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    non_model_pct_str = fmt(non_model_pct_value, precision=1, commas=False)
...
class ServingTaxMicrosecondsCalc:
    ...
    micro_tax_max_pct_str = fmt(micro_tax_max_pct, precision=1, commas=False)
```
...
class ResolutionScalingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    r1_str = fmt(r1_value, precision=1, commas=False)
    r2_str = fmt(r2_value, precision=1, commas=False)
    theoretical_str = fmt(theoretical_slowdown_value, precision=1, commas=False)
...
class ResolutionBottleneckCalc:
    ...
    ai_224_str = fmt(ai_224_value, precision=1, commas=False)
    ai_384_str = fmt(ai_384_value, precision=1, commas=False)
    ai_512_str = fmt(ai_512_value, precision=1, commas=False)
    ai_640_str = fmt(ai_640_value, precision=1, commas=False)

    ridge_point_str = fmt(ridge_point_value, precision=1, commas=False)
```
...
class CapacityPlanningAnchor:
    ...
    qps_str = fmt(qps_target, precision=1, commas=False)
    slo_str = fmt(slo_ms, precision=1, commas=False, suffix=" ms")
    slots_str = fmt(concurrency_slots, precision=1, commas=False)
...
class CapacityPlanning:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    littles_lambda_str = fmt(lambda_qps, precision=1)
    littles_w_ms_str = fmt(int(latency_slo_s * MS_PER_SEC), precision=1, commas=False)
    littles_w_str = fmt(latency_slo_s, precision=2, commas=False)
    littles_l_str = fmt(concurrency, precision=1, commas=False)
```
...
class BatchingTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    wait_time_b1_ms_str = fmt(int(w_form_b1), precision=1, commas=False)
    wait_time_b32_ms_str = fmt(int(w_form_b32), precision=1, commas=False)
    lat_b1_ms_str = fmt(lat_b1, precision=1, commas=False)
    lat_b32_ms_str = fmt(lat_b32, precision=1, commas=False)
    penalty_ratio_str = fmt(int(penalty_ratio), precision=1, commas=False)
```
...
class CapacityPlanningCalc:
    ...
    cp_gpus_raw_str = fmt(cp_gpus_raw_value, precision=0, commas=False)
    ...
    cp_final_ceil_str = fmt(cp_final_ceil_value, precision=1, commas=False)
    ...
    cp_n_plus_2_min_str = fmt(cp_n_plus_2_min_value, precision=1, commas=False)
    cp_n_plus_2_headroom_str = fmt(cp_n_plus_2_headroom_value, precision=1, commas=False)
    cp_peak_qps_str = fmt(cp_peak_qps_value, precision=1)
    cp_v100_throughput_str = fmt(cp_v100_throughput_value, precision=1)
    cp_service_ms_str = fmt(cp_service_ms_value, precision=1, commas=False)
    cp_p99_target_ms_str = fmt(cp_p99_target_ms_value, precision=1, commas=False)
    cp_rho_safe_str = fmt(cp_rho_safe_value, precision=2, commas=False)
    cp_rho_safe_pct_str = fmt(cp_rho_safe_value * 100, precision=1, commas=False)
...
class ColdStartCalc:
    ...
    cs_cloud_str = fmt(cs_cloud_total_value, precision=1, commas=False, prefix="~", suffix=" s")
...
class ModelSwapCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    model_size_gb_str = fmt(model_size_gb_value, precision=1, commas=False)
    pcie_bw_gbs_str = fmt(pcie_bw_gbs_value, precision=1, commas=False)
    model_swap_ms_str = fmt(model_swap_ms_value, precision=1, commas=False)
...
class BatchThroughputCalc:
    ...
    batch_window_ms_str = fmt(batch_window_ms_value, precision=1, commas=False)
    batch32_inference_ms_str = fmt(batch32_inference_ms_value, precision=1, commas=False)
    batch32_total_str = fmt(batch32_total_ms_value, precision=1, commas=False)
    batch1_inference_total_ms_str = fmt(batch1_inference_total_ms_value, precision=1, commas=False)
```
...
class BatchingSweetspotCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    batch1_ms_str = fmt(batch1_ms_value, precision=1, commas=False)
    batch1_imgs_str = fmt(batch1_imgs_value, precision=1, commas=False)
    batch8_wait_ms_str = fmt(batch8_wait_ms_value, precision=1, commas=False)
    batch8_inference_ms_str = fmt(batch8_inference_ms_value, precision=1, commas=False)
    batch8_user_latency_str = fmt(batch8_user_latency_ms_value, precision=1, commas=False)
    batch8_throughput_str = fmt(batch8_throughput_value, precision=1, commas=False)
    latency_increase_str = fmt(latency_increase_value, precision=1, commas=False)
```
...
class BatchingBudgetCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    avg_wait_str = fmt(avg_wait_ms_value, precision=1, commas=False)
    budget_pct_str = fmt(budget_pct_value, precision=1, commas=False)
```
...
class BatchingAnalysisCalc:
    ...
    b1_throughput_str = fmt(throughputs_value[1], precision=1, commas=False)
    b32_throughput_str = fmt(throughputs_value[32], precision=1, commas=False)
    ...
    il_overhead_str = fmt(il_overhead_ms_value, precision=1, commas=False)
    ...
    il_eff_b1_str = fmt(il_eff_b1_pct_value, precision=1, commas=False)
    il_eff_b32_str = fmt(il_eff_b32_pct_value, precision=1, commas=False)
    il_threshold_str = fmt(il_threshold_pct_value, precision=1, commas=False)
    T_window_str = fmt(T_window_value, precision=1, commas=False)
...
class BatchingOptimization:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    s1_window_ms_str = fmt(int(s1_window), precision=1, commas=False)
    ...
    s1_batch_str = fmt(s1_batch, precision=1, commas=False)
    s1_throughput_str = fmt(int(s1_tput), precision=1)

    s2_window_ms_str = fmt(int(s2_window), precision=1, commas=False)
    s2_wait_ms_str = fmt(s2_wait, precision=1, commas=False)
    s2_budget_ms_str = fmt(s2_budget, precision=1, commas=False)
    s2_batch_str = fmt(s2_batch, precision=1, commas=False)
    s2_throughput_str = fmt(int(s2_tput), precision=1)

    throughput_gain_pct_str = fmt(tput_gain, precision=1, commas=False)
    latency_avg_increase_ms_str = fmt(latency_increase, precision=1, commas=False)
    # Simplified P99 increase for prose consistency
    latency_p99_increase_ms_str = fmt(int(s2_window - s1_window), precision=1, commas=False)
```
...
class SloViolationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    qps_str = fmt(qps_value, precision=1, commas=False)
    T_slo_str = fmt(T_slo_value, precision=1, commas=False)
    mean_wait_str = fmt(mean_wait_value, precision=1, commas=False)
    mean_batch_str = fmt(mean_batch_value, precision=1, commas=False)
    mean_service_str = fmt(mean_service_value, precision=1, commas=False)
    mean_latency_str = fmt(mean_latency_value, precision=1, commas=False)
    ...
    p99_batch_str = fmt(p99_batch_value, precision=1, commas=False)
```
...
class PracticalConfigCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    pc_slo_ms_str = fmt(pc_slo_ms_value, precision=1, commas=False)
    pc_qps_str = fmt(pc_qps_value, precision=1, commas=False)
    pc_batch_budget_ms_str = fmt(pc_batch_budget_ms_value, precision=1, commas=False)
    pc_max_window_ms_str = fmt(pc_max_window_ms_value, precision=1, commas=False)
    pc_expected_batch_str = fmt(pc_expected_batch_value, precision=1, commas=False)
    pc_mem_limit_batch_str = fmt(pc_mem_limit_batch_value, precision=1, commas=False)
    pc_config_window_ms_str = fmt(pc_config_window_ms_value, precision=1, commas=False)
    pc_config_batch_str = fmt(pc_config_batch_value, precision=1, commas=False)
    pc_p99_batch_str = fmt(pc_p99_batch_value, precision=1, commas=False)
    pc_predicted_p99_ms_str = fmt(pc_predicted_p99_ms_value, precision=1, commas=False)
    pc_predicted_throughput_str = fmt(pc_predicted_throughput_value, precision=1)
```
...
class TrafficAdaptiveBatchingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    qps_100_str = fmt(rows[0][0], precision=1)
    ...
    qps_500_str = fmt(rows[1][0], precision=1)
    ...
    qps_1000_str = fmt(rows[2][0], precision=1)
    window_1000_ms_str = fmt((rows[2][1] / (1 * ms).m_as(second)), precision=1, commas=False)
    avg_batch_1000_str = fmt(rows[2][2], precision=1, commas=False)
    latency_1000_ms_str = fmt(rows[2][3], precision=1, commas=False)

    qps_5000_str = fmt(rows[3][0], precision=1)
...
class MobileServingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    m_cam_ms_str = fmt(m_cam_ms_value, precision=1, commas=False, suffix=" ms")
    m_jpeg_ms_str = fmt(m_jpeg_ms_value, precision=1, commas=False, suffix=" ms")
    m_resize_ms_str = fmt(m_resize_ms_value, precision=1, commas=False, suffix=" ms")
    m_npu_ms_str = fmt(m_npu_ms_value, precision=1, commas=False, suffix=" ms")
    m_ui_ms_str = fmt(m_ui_ms_value, precision=1, commas=False, suffix=" ms")
    m_total_ms_str = fmt(m_total_ms_value, precision=1, commas=False, suffix=" ms")
...
class GpuSpecs:
    ...
    h100_tdp_str = fmt(h100_tdp_value, precision=1, commas=False)          # e.g. "700" W
```
...
class CarbonCostCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    cc_concurrent_str = fmt(cc_concurrent_req_value, precision=1, commas=False)
    ...
    cc_host_overhead_str = fmt(cc_host_overhead_w_value, precision=1, commas=False)
    cc_total_power_str = fmt(cc_total_power_w_value, precision=1, commas=False)
    ...
    cc_response_joules_str = fmt(cc_response_joules_value, precision=1, commas=False)
    cc_smartphone_str = fmt(_smartphone_joules, precision=1)
    cc_boiling_str = fmt(_boiling_joules, precision=1)
    cc_low_util_str = fmt(cc_low_util_pct_value, precision=1, commas=False)
    cc_idle_power_str = fmt(cc_idle_power_w_value, precision=1, commas=False)
...
class PrecisionTradeoffCalc:
    ...
    pt_fp32_mem_mb_str = fmt(pt_fp32_mem_mb_value, precision=1, commas=False)
    ...
    pt_fp16_mem_mb_str = fmt(pt_fp16_mem_mb_value, precision=1, commas=False)
    pt_fp16_acc_str = fmt(pt_fp16_acc_value, precision=2, commas=False)
    pt_fp16_util_str = fmt(pt_fp16_util_value, precision=1, commas=False)
    pt_int8_ms_str = fmt(pt_int8_ms_value, precision=1, commas=False)
    pt_int8_mem_mb_str = fmt(pt_int8_mem_mb_value, precision=1, commas=False)
    ...
    pt_int8_util_str = fmt(pt_int8_util_value, precision=1, commas=False)
    pt_int8_speedup_str = fmt(pt_int8_speedup_value, precision=1, commas=False)
    pt_fp16_speedup_str = fmt(pt_fp16_speedup_value, precision=1, commas=False)
...
class CostAnalysisCalc:
    ...
    ca_cpu_throughput_str = fmt(ca_cpu_throughput_value, precision=1, commas=False)
    ca_cpu_cpm_str = fmt(ca_cpu_cpm_value, precision=2, commas=False)
    ca_t4_cost_str = fmt(ca_t4_cost_value, precision=2, commas=False)
    ca_t4_throughput_str = fmt(ca_t4_throughput_value, precision=1, commas=False)
    ...
    ca_v100_throughput_str = fmt(ca_v100_throughput_value, precision=1)
...
class LlmCaseStudyHwSpecs:
    ...
    h100_mem_str = fmt(h100_mem_value, precision=1, commas=False)          # e.g. "80" GB
```
...
class LlmServingCalc:
    ...
    token_time_theoretical_ms_str = fmt(token_time_theoretical_ms_value, precision=1, commas=False)
    realized_tpot_ms_str = fmt(realized_tpot_ms_value, precision=1, commas=False)
    ...
    kv_cache_gb_str = fmt(kv_cache_gb_value, precision=1, commas=False)
    ...
    tokens_per_req_str = fmt(total_tokens, precision=1)
    concurrent_batch_str = fmt(concurrent_batch_value, precision=1, commas=False)
    ...
    prefill_req_per_s_str = fmt(prefill_req_per_s_value, precision=1, commas=False)
    decode_req_capacity_str = fmt(decode_req_capacity_value, precision=1, commas=False)
    hourly_cost_str = fmt(hourly_cost_value, precision=2, commas=False)
    tokens_per_hour_m_str = fmt(tokens_per_hour_value/MILLION, precision=1, commas=False)
    cost_per_m_tokens_str = fmt(cost_per_m_tokens_value, precision=3, commas=False)
    remaining_vram_gb_str = fmt(remaining_vram_gb_value, precision=1, commas=False)
```
...
class FallacyLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fl_utilization_high_pct_str = fmt(fl_utilization_high_value * 100, precision=1, commas=False)
    fl_service_slow_ms_str = fmt(fl_service_slow_ms_value, precision=1, commas=False)
    fl_wait_slow_ms_str = fmt(fl_wait_slow_ms_value, precision=1, commas=False)
    fl_service_fast_ms_str = fmt(fl_service_fast_ms_value, precision=1, commas=False)
    fl_utilization_new_pct_str = fmt(fl_utilization_new_value * 100, precision=1, commas=False)
    fl_wait_fast_ms_str = fmt(fl_wait_fast_ms_value, precision=1, commas=False)
    fl_inference_gain_ms_str = fmt(fl_inference_gain_ms_value, precision=1, commas=False)
    fl_queuing_improvement_str = fmt(fl_queuing_improvement_value, precision=1, commas=False)
    fl_total_slow_ms_str = fmt(fl_total_slow_ms_value, precision=1, commas=False)
...
class FallacyUtilizationCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fu_util_high_pct_str = fmt(fu_util_high_value * 100, precision=1, commas=False)
    fu_util_mod_pct_str = fmt(fu_util_mod_value * 100, precision=1, commas=False)
    fu_wait_high_factor_str = fmt(fu_wait_high_factor_value, precision=1, commas=False)
    fu_total_high_factor_str = fmt(1/(1-fu_util_high_value), precision=1, commas=False)
    fu_cost_reduction_str = fmt(fu_cost_reduction_pct_value, precision=1, commas=False)
    fu_latency_increase_str = fmt(fu_latency_increase_factor_value, precision=1, commas=False)
    fu_service_ms_str = fmt(fu_service_ms_value, precision=1, commas=False)
...
class FallacySkewCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fs_val_acc_str = fmt(fs_val_acc_value, precision=1, commas=False)
    fs_prod_acc_str = fmt(fs_prod_acc_value, precision=1, commas=False)
    fs_acc_drop_str = fmt(fs_acc_drop_value, precision=1, commas=False)
...
class TailLatencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tl_util_pct_str = fmt(tl_util_value * 100, precision=1, commas=False)
    tl_service_ms_str = fmt(tl_service_ms_value, precision=1, commas=False)
    tl_avg_ms_str = fmt(tl_avg_ms_value, precision=1, commas=False)
...
class FallacyBatchingCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fb_batch_small_str = fmt(fb_batch_small_value, precision=1, commas=False)
    fb_batch_large_str = fmt(fb_batch_large_value, precision=1, commas=False)
    fb_throughput_gain_str = fmt(fb_throughput_gain_pct_value, precision=1, commas=False)
    fb_inf_small_ms_str = fmt(fb_inf_small_ms_value, precision=1, commas=False)
    fb_inf_large_ms_str = fmt(fb_inf_large_ms_value, precision=1, commas=False)
    fb_padding_waste_min_str = fmt(fb_padding_waste_min_pct_value, precision=1, commas=False)
    fb_padding_waste_max_str = fmt(fb_padding_waste_max_pct_value, precision=1, commas=False)
```
...
class FallacyColdstartCalc:
    ...
    cs_compile_time_str = fmt(cs_compile_time_s_value, precision=1, commas=False)
    cs_aggregate_cold_str = fmt(cs_aggregate_cold_s_value, precision=1, commas=False)
    cs_parallel_warm_str = fmt(cs_parallel_warm_s_value, precision=1, commas=False)
    cs_steady_latency_str = fmt(cs_steady_latency_ms_value, precision=1, commas=False)
    cs_cold_latency_str = fmt(cs_cold_latency_ms_value, precision=1, commas=False)
    cs_cold_mult_str = fmt(cs_cold_multiplier_value, precision=1, commas=False)
```
