...
::: {.callout-principle #pri-data-as-code title="The Data as Code Invariant"}
**Invariant**: Data *is* the source code of the ML system. A change in the training dataset ($\Delta D$) is functionally equivalent to a change in the executable logic ($\Delta P$).
...
**Invariant**: Data possesses mass. As dataset scale ($D$) increases, the cost (latency, bandwidth, energy) of moving data exceeds the cost of moving compute.
$$ C_{\text{move}}(D) \gg C_{\text{move}}(\text{Compute}) $$
