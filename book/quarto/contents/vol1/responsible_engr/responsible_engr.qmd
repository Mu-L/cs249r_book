...
*   **Conversion factor**: (`{python} CarbonFactor.gpu_power_kw_str` $\times$ `{python} CarbonFactor.scenario_hr_str`) $\times$ `{python} CarbonFactor.carbon_intensity_str` kg/kWh = **`{python} CarbonFactor.carbon_per_gpu_hr_str` kg CO~2~eq per GPU-hour**.
