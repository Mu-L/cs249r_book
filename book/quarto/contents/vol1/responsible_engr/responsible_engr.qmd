...
class RepresentationStats:
    ...
    repr_group_fraction_pct_str = fmt_int(minority_fraction * 100, commas=False, suffix=" percent")
...
class LoanFairness:
    ...
    a_fnr_str = fmt(a_fnr_pct, precision=0, commas=False, suffix=" percent")
    b_fnr_str = fmt(b_fnr_pct, precision=0, commas=False, suffix=" percent")
...
class GDPRReviewLoad:
    ...
    review_rate_str = fmt(review_rate_pct, precision=1, commas=False, suffix=" percent")
...
class InferenceCostCalc:
    ...
    annual_inf_str      = fmt_int(annual_inf_cost_value)
...
*   **Intensity**: `{python} CarbonFactor.carbon_intensity_str` CO~2~eq (US-average grid baseline).
...
class TCOTrainingCalc:
    ...
    p_train_str = fmt(p_train, precision=1, suffix=" percent")
...
class TCOInferenceCalc:
    ...
    i_latency_ms_str = fmt(i_latency.m_as(ms), precision=0, commas=False, suffix=" ms")
    ...
    p_inf_str = fmt(p_inf, precision=1, suffix=" percent")
```
...
class TCOOperationsCalc:
    ...
    p_ops_str = fmt(p_ops, precision=1, suffix=" percent")
```
...
class TCOSummaryTable:
    ...
    quant_reduction_pct_str = fmt_int(quant_reduction_pct * 100, commas=False, suffix=" percent")
```
...
class CarbonScaleCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_energy_mwh_str  = fmt(train_energy_mwh, precision=0, commas=True, suffix=" MWh")
    train_energy_kwh_str  = fmt(train_energy_kwh, precision=0, commas=True, suffix=" kWh")
...
**Problem**: A foundation model is being trained at the scale of GPT-3, consuming `{python} CarbonScaleCalc.train_energy_mwh_str` of electricity. What is the environmental impact?
