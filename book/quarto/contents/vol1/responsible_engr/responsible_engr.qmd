...
class TestingConstraintAnchor:
    ...
    minority_pct_str = MarkdownStr(f"{minority_pct}%")
    minority_samples_str = fmt(minority_samples, precision=0, commas=False)
    # Custom "×" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    multiplier_str = MarkdownStr(f"{data_multiplier}×")
...
class RepresentationStats:
    ...
    repr_group_fraction_str = MarkdownStr(f"{minority_fraction}")
...
class FairnessPrice:
    ...
    hire_value_k_str    = MarkdownStr(f"USD {hire_value:,.0f}")            # e.g. "USD 100,000"
    bad_hire_cost_k_str = MarkdownStr(f"USD {bad_hire_cost:,.0f}")         # e.g. "USD 50,000"
    fp_increase_str     = MarkdownStr(f"{fp_increase_pp} percent")         # e.g. "5%"
    utility_loss_str    = MarkdownStr(f"{utility_loss_pct:.1f} percent")   # computed from assumptions above
    minority_share_str  = MarkdownStr(f"{minority_group_share * 100:.0f} percent")
```
...
class TCOCalc:
    ...
    t_total_k_str = MarkdownStr(f"${train_tco_3yr/THOUSAND:.0f}K")
    ...
    t_data_prep_carbon_str = MarkdownStr(f"{t_data_prep_hrs * carbon_per_gpu_hr:.0f} kg")
    t_hparam_carbon_str = MarkdownStr(f"{(t_hparam_exps * t_hparam_cost_exp/train_gpu_rate) * carbon_per_gpu_hr:.0f} kg")
    t_final_carbon_str = MarkdownStr(f"{t_final_hrs * carbon_per_gpu_hr:.0f} kg")
    t_subtotal_carbon_str = MarkdownStr(f"{train_carbon_cycle:.0f} kg")
    t_total_carbon_str = MarkdownStr(f"{train_carbon_3yr:,.0f} kg")
    t_total_carbon_tons_str = MarkdownStr(f"{train_carbon_3yr/THOUSAND:.1f} tons")  # kg → metric ton; mlsysim mass pint units pending
    ...
    i_gpu_hr_day_str = MarkdownStr(f"{inf_gpu_hours_day:.0f} GPU-hr")
    i_daily_carbon_str = MarkdownStr(f"{inf_carbon_day:.0f} kg")
    i_annual_calc_str = MarkdownStr(f"{inf_gpu_hours_day:.0f} × 365 × ${inf_gpu_rate:.2f} = ${inf_cost_day * 365 / THOUSAND:.0f}K")
    i_annual_carbon_str = MarkdownStr(f"{inf_carbon_3yr/3:,.0f} kg")
    i_total_str = MarkdownStr(f"${inf_tco_3yr/MILLION:.2f}M")
    i_carbon_str = MarkdownStr(f"{inf_carbon_3yr:,.0f} kg")
    i_carbon_tons_str = MarkdownStr(f"{inf_carbon_3yr/THOUSAND:.1f} tons")  # kg → metric ton; mlsysim mass pint units pending

    # Operations table strings — currency prefix → MarkdownStr per recipe Rule 1 escape-hatch
    o_monitor_annual_str = MarkdownStr(f"${o_monitor_yr/THOUSAND:.0f}K")
    o_monitor_str = MarkdownStr(f"${o_monitor_yr*3/THOUSAND:.0f}K")
    o_oncall_annual_str = MarkdownStr(f"${o_oncall_yr/THOUSAND:.0f}K")
    o_oncall_str = MarkdownStr(f"${o_oncall_yr*3/THOUSAND:.0f}K")
    o_incident_annual_str = MarkdownStr(f"${o_incident_yr/THOUSAND:.0f}K")
    o_incident_str = MarkdownStr(f"${o_incident_yr*3/THOUSAND:.0f}K")
    o_total_str = MarkdownStr(f"${o_total_3yr/THOUSAND:.0f}K")

    # Summary strings — currency / suffix → MarkdownStr per recipe Rule 1 escape-hatch
    total_tco_str = MarkdownStr(f"${total_tco/MILLION:.2f}M")
    total_carbon_tons_str = MarkdownStr(f"~{total_carbon_kg/THOUSAND:.0f} tons")  # kg → metric ton; mlsysim mass pint units pending
