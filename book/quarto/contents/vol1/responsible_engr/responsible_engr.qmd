...
::: {#exmp-responsible-engr-compas-recidivism-algorithm-audit .callout-example title="The COMPAS recidivism algorithm audit"}

**Context**: COMPAS\index{COMPAS!recidivism algorithm audit} is a risk assessment tool used in US courtrooms to predict re-offending. Judges use these scores to inform bail and sentencing decisions.\index{Risk Assessment!criminal justice}

**Failure**: A ProPublica investigation [@angwin2016] revealed that while the system was "calibrated" (a score of seven meant the same probability of re-offending for any group), its error rates were skewed:

*   **False Positives**\index{False Positive Rate!demographic disparity}: Black defendants who *did not* re-offend were incorrectly flagged as high-risk at nearly twice the rate of White defendants (44.9 percent vs. 23.5 percent).
*   **False Negatives**\index{False Negative Rate!risk assessment bias}: White defendants who *did* re-offend were incorrectly labeled as low-risk far more often than Black defendants (47.7 percent vs. 28 percent).

**Systems insight**: The system optimized for *Calibration*\index{Calibration!fairness impossibility theorem} but violated *Equalized Odds*. Mathematically, it is impossible to satisfy both simultaneously when base rates differ between groups (the "Impossibility Theorem of Fairness").\index{Fairness Metrics!impossibility theorem}\index{Bias!algorithmic} Engineering responsibility requires explicitly choosing which fairness constraint matters for the domain; in criminal justice, false positives (wrongly jailing someone) are typically considered worse than false negatives.
...
This sepsis scenario illustrates a class of failure that traditional engineering is poorly equipped to handle. Traditional software fails *loudly*. A null pointer exception crashes the program, a network timeout returns an error code. These visible failures enable rapid detection and response. In contrast, ML systems fail *silently* because degraded predictions look like normal predictions.\index{Silent Failure!detection challenges} The primary mechanism behind this silent degradation is *distribution shift*.

::: {#dfn-responsible-engr-distribution-shift .callout-definition title="Distribution shift"}

***Distribution shift***\index{Distribution Shift!definition} is the violation of the stationarity assumption\index{Stationarity Assumption!distribution shift} $(P_0 \neq P_t)$ that underpins all supervised learning. It is the umbrella term for a family of drift types: data drift (see @sec-ml-operations) occurs when $p(x)$ shifts while $p(y \mid x)$ remains stable; concept drift occurs when $p(y \mid x)$ itself shifts.

1.  **Significance (quantitative)**: Accuracy degradation can be measured against divergence statistics such as Jensen-Shannon divergence $\mathcal{D}_{\text{JS}}(P_t \lVert P_0)$, but useful alert thresholds must be calibrated empirically for each task, representation, label process, and deployment environment. A $\mathcal{D}_{\text{JS}}$ value of 0.1 may be harmless for one feature space and severe for another. This degradation occurs regardless of code quality, because the model is correct given its training distribution; the environment changed, not the code.
2.  **Distinction (durable)**: Unlike model error\index{Model Error!definition} (which is a learning failure caused by the algorithm or data quality at training time), distribution shift is an environmental failure\index{Environmental Failure!distribution shift}: the model's learned mapping was correct at training time but is no longer representative of current reality.
...
Distribution shift\index{Distribution Shift!causes of model degradation} explains *why* models degrade over time (the operational detection and monitoring strategies for drift are covered in @sec-ml-operations). A second mechanism for silent failure can occur even when the data distribution is stable: misalignment between the metric the model optimizes and the outcome the organization actually values. The dynamics of that divergence\index{Alignment Gap!proxy metric divergence} are made precise by Goodhart's Law—once a proxy becomes the optimization target, it stops tracking the goal it was chosen to represent.

::: {#nbk-responsible-engr-alignment-gap .callout-notebook title="The alignment gap"}
...
**Physics**: Goodhart's Law\index{Goodhart's Law!metric optimization} states that optimizing a proxy eventually decouples it from the goal.
...
**Systems insight**: Engineers cannot optimize what they cannot measure. If the true goal is unobservable, **Counterfactual Evaluation**\index{Counterfactual Evaluation!proxy recalibration} (random holdouts) is required to periodically re-calibrate the proxy.
...
**Failure mode**: The change was a public example of a platform acknowledging that engagement proxies are incomplete. A system can optimize for relevance, clicks, or time spent while still producing passive consumption patterns that conflict with long-term user welfare.

**Consequence**: Facebook warned that pages could see reach, video watch time, and referral traffic decrease as the ranking objective changed.

**Systems insight**: Metrics are proxies for value, not value itself. Ranking systems need long-term health metrics and policy constraints, not only short-term engagement targets [@facebook2018newsfeed].
...
::: {#ws-responsible-engr-proxy-variable-trap .callout-war-story title="The proxy variable trap"}
**Context**: In 2019, Ziad Obermeyer and colleagues at UC Berkeley audited a commercial Optum algorithm used by health systems to enroll patients with complex needs into high-risk care management programs, examining roughly fifty thousand patients across one large academic hospital [@obermeyer2019].

**Failure**: The model predicted "future healthcare cost" as a proxy for "future health need." The proxy was reasonable in the abstract---sicker people generally cost more---but the US healthcare system spends less on Black patients than on White patients with the same level of illness. The algorithm learned this pattern and assigned Black patients lower risk scores than White patients with comparable disease burdens.

**Consequence**: At any given risk score, Black patients carried substantially more chronic conditions than White patients with the same score. When the team reformulated the algorithm to predict illness markers directly rather than cost, the share of Black patients identified for the high-risk program rose from 17.7 percent to 46.5 percent. Optum subsequently adopted reformulations grounded in illness rather than spending.
...
\index{Facial Recognition!demographic disparities}Following the Gender Shades findings, Microsoft invested in improving facial recognition performance across demographic groups.\index{Bias!mitigation strategies} The approach combined technical and organizational interventions: targeted data collection to address underrepresented populations, model architecture changes to improve feature extraction for diverse skin tones, and systematic disaggregated evaluation across all demographic intersections. Microsoft had reduced error rates for darker-skinned subjects by up to 20 times, bringing error rates below 2 percent for all demographic groups [@raji2019]. The company published these improvements transparently, enabling external verification. The business outcome: Microsoft's facial recognition API maintained enterprise customer trust while competitors faced regulatory scrutiny and contract cancellations.

\index{Feature Removal!principled design choice}Twitter's automatic image cropping system exhibited a different failure mode. In 2020, users discovered it showed racial bias in choosing which faces to display in preview thumbnails.\index{Image Cropping!racial bias} Twitter responded with a responsible engineering approach: systematic analysis to characterize the problem quantitatively, publication of results enabling independent verification, and ultimately removal of the automatic cropping feature entirely [@yee2021]. The company determined that no technical solution could guarantee equitable outcomes across all contexts. This decision prioritized user fairness over engagement optimization and demonstrated that responsible engineering sometimes means not shipping a feature.

**Differential privacy**\index{Differential Privacy!mathematical guarantees} represents responsible engineering because a privacy requirement becomes a mathematical guarantee rather than a policy aspiration [@dwork2008].[^fn-differential-privacy-tradeoff] Systems that use differential privacy must calibrate noise to balance utility against privacy, track privacy budget across repeated analyses, and document the chosen privacy parameters. The business value is the same pattern as the other examples: a responsibility requirement becomes an engineering constraint that can be specified, tested, and communicated.

[^fn-differential-privacy-tradeoff]: **Differential Privacy**: Introduced by @dwork2006calibrating, a mechanism satisfies $\epsilon$-differential privacy if any output's probability changes by at most $e^\epsilon$ when a single individual's data is added or removed. The systems trade-off is utility rather than mere implementation complexity: stronger privacy generally requires more noise, and a finite privacy budget $(\epsilon)$ constrains repeated queries---forcing engineers to choose between richer analytics and stronger privacy guarantees. \index{Differential Privacy!epsilon trade-off}
...
The trade-off between fairness and accuracy is not a sign that fairness is impractical; it is a fundamental property of constrained optimization that engineers must understand. A Pareto frontier represents the set of optimal configurations where improving one metric necessarily degrades another. @fig-fairness-frontier visualizes this fairness-accuracy Pareto frontier.\index{Pareto Frontier!fairness-accuracy trade-off}\index{Fairness!accuracy trade-off} The curve is not linear: while perfect fairness (zero disparity) often requires a significant drop in accuracy, a "Sweet Spot" typically exists where large fairness gains can be achieved with minimal accuracy loss. The shape of the frontier explains why responsible engineering is feasible: in many practical settings, substantial fairness gains can be achieved with modest accuracy loss.

::: {#fig-fairness-frontier fig-env="figure" fig-pos="htb" fig-cap="**The Fairness-Accuracy Pareto Frontier**: Model accuracy vs. demographic disparity. The x-axis is inverted so fairness increases left-to-right. Point A represents unconstrained optimization (maximum accuracy, high disparity) on the left. Point C represents strict equality constraints (zero disparity, significant accuracy drop) on the far right. Point B is the sweet spot where engineers can often achieve substantial fairness gains with modest accuracy loss. Responsible engineering is the practice of finding and implementing Point B." fig-alt="Curve showing trade-off between accuracy (y-axis) and disparity (x-axis, inverted so lower disparity is to the right). Point A is top-left (high accuracy, high disparity). Point C is bottom-right (zero disparity, lower accuracy). Point B is upper-right (high accuracy, low disparity), showing the optimal trade-off."}
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(0.18, 0.943, "A: Unconstrained\n(Max Accuracy)", ha='center', va='top', fontsize=8, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.5))

ax.plot(0.05, 0.913, 'o', color=COLORS['GreenLine'], markersize=8)
ax.text(0.053, 0.905, "B: Sweet Spot\n(91 percent Acc, 3.6 times Fairer)", ha='right', va='bottom', fontsize=8, bbox=dict(facecolor='white', alpha=0.8, edgecolor='none', pad=0.5))
...
class TestingConstraintAnchor:
    ...
    # Custom "%" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    minority_pct_str = fmt(minority_pct, precision=0, commas=False, suffix="%")
    minority_samples_str = fmt(minority_samples, precision=0, commas=False)
    # Custom "×" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    multiplier_str = fmt(data_multiplier, precision=0, commas=False, suffix="×")

class GenderShadesDisparity:
    """Gender Shades error disparity exports for @tbl-gender-shades-results."""
    ...
    error_dark_male_str = fmt(err_dark_male, precision=0, commas=False)
    ...
    disparity_dark_male_str = fmt(disparity_dark_male, precision=0, commas=False)
...
: **Gender Shades Facial Recognition Error Rates**: Disaggregated evaluation reveals that aggregate accuracy metrics conceal severe performance disparities. Systems that appear highly accurate overall show error rates varying by more than 43$\times$ across demographic groups. Worst-case results across systems studied; source: @buolamwini2018gender. {#tbl-gender-shades-results tbl-colwidths="[34,33,33]"}
...
Despite the inherent challenges, several concrete testing approaches can surface responsibility issues before deployment. Slice-based evaluation partitions test data into meaningful subgroups and reports metrics separately for each slice. A model may achieve 95 percent accuracy overall but only 78 percent accuracy on low-income applicants or users from rural areas, a disparity invisible in aggregate reporting. Invariance testing\index{Invariance Testing!fairness verification} checks whether predictions change when they should not: replacing "John" with "Jamal" in a loan application should not change approval likelihood if the feature is not legitimate for the decision. Behavioral testing frameworks such as CheckList apply this idea by organizing tests around model capabilities and invariance-style expectations rather than accuracy alone [@ribeiro2020beyond]. Boundary testing evaluates model behavior at the edges of input distributions (unusual ages, extreme values, rare categories) where training data may be sparse and predictions unreliable. Stress testing extends boundary testing to adversarial conditions: corrupted inputs, distribution shift, adversarial examples, and edge cases designed to probe failure modes systematically. Stakeholder red-teaming\index{Red Teaming!stakeholder engagement} engages domain experts and affected community members to identify scenarios that engineers may not anticipate but users will encounter, surfacing failure modes that no automated test can discover because they require lived experience to imagine.
...
\tikzset{%
  EL1/.style={align=flush center,
    ...
    minimum width=131mm, minimum height=76mm
  },
  Box2/.style={Box, draw=myred, fill=magenta!03 },
  Txt/.style={crimson,align=center,font=\usefont{T1}{phv}{b}{n}\fontsize{8pt}{8}\selectfont},
  Txt1/.style={black!60,align=center,font=\usefont{T1}{phv}{m}{n}\fontsize{6pt}{7}\selectfont},
...
{\color{black!70}\usefont{T1}{phv}{m}{it}\fontsize{6pt}{6}\selectfont Laws, executive orders, regulatory agencies}};

\node[EL1,minimum width=105mm, minimum height=61.5mm,fill=col2](E2)
at(0,-0.16){};
\node[below=2pt of E2.north,Txt]{Industry\\
{\color{black!70}\usefont{T1}{phv}{m}{it}\fontsize{6pt}{6}\selectfont
...
%line below
\coordinate(L)at ($(E1.south)+(-45mm,-3mm)$);
\coordinate(D)at ($(E1.south)+(45mm,-3mm)$);
...
Amazon's recruiting tool could have been caught before deployment by a structured predeployment review. COMPAS's error rate disparity would have surfaced through disaggregated testing. Both failures shared a common cause: responsibility was treated as a separate review stage rather than integrated into the development workflow.\index{Responsible Engineering!checklist methodology} A responsible engineering checklist embeds assessment at three points where engineering decisions have the greatest ethical impact: *predeployment assessment* evaluates potential harms before a system reaches users, *fairness evaluation* quantifies whether performance holds equitably across demographic groups, and *documentation standards* create the audit trails that make accountability possible. Each phase builds on the previous one: assessment identifies what to measure, fairness evaluation measures it, and documentation ensures the measurements persist beyond any single team member's tenure.
...
# │ Exports: repr_*_str, repr_equation_math
# └─────────────────────────────────────────────────────────────────────────────
...
class RepresentationStats:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    repr_target_images_str = fmt(target_imgs, precision=0)
    repr_group_fraction_pct_str = fmt_int(minority_fraction * 100, commas=False)
    repr_group_fraction_str = fmt(minority_fraction, precision=2, commas=False)
    repr_random_total_str = fmt_int(random_total)
    repr_multiplier_str = fmt_int(multiplier, commas=False)
    # Display math ($$..$$) original; preserve via MarkdownStr per recipe Rule 2 escape-hatch
    repr_equation_math = MarkdownStr(f"$$ D_{{\\text{{eval,target}}}} = {target_imgs:,} \\text{{ images}} $$")
    repr_moe_pp_str = fmt(moe_pp, precision=0, commas=False, suffix=" percentage points")
    repr_confidence_pct_str = fmt(confidence_pct, precision=0, commas=False)
```

::: {#nbk-responsible-engr-statistics-representation .callout-notebook title="The statistics of representation"}

**Problem**: An engineering team needs to verify that a Face ID model works for a minority group representing `{python} RepresentationStats.repr_group_fraction_pct_str` of the user base. A worst-case binomial margin of error near `{python} RepresentationStats.repr_moe_pp_str` at `{python} RepresentationStats.repr_confidence_pct_str` confidence requires roughly `{python} RepresentationStats.repr_target_images_str` for this group.
...
[^fn-model-card-transparency]: **Model Cards**: The primary failure mode model cards address is scope creep\index{Scope Creep!model cards}: an estimated 40--60 percent of deployments that exceed a model's documented scope do so not through deliberate decision but through gradual expansion—"it worked for case A, so we tried case B." In practice, cards are often written after deployment decisions are made, documenting observed behavior rather than constraining it. The companion "Datasheets for Datasets" [@gebru2021] applies the same principle to training data. Without both, the card becomes a historical record rather than a guard rail. \index{Model Card!documentation standard}
...
: **Example Model Card: MobileNetV2 for Edge Deployment**: Abstract model card categories translate to practical documentation that guides responsible deployment decisions. {#tbl-model-card-example tbl-colwidths="[25,75]"}
...
|:------------------------------|:------------------------------------------------------------------------------------------------|:---------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------|
| **ResNet-50 (Compute Beast)** | Training data bias (underrepresentation of minority groups in ImageNet)                         | Disaggregated accuracy by demographic group                    | Gender Shades: 99 percent accuracy on light-skinned males, 65 percent on dark-skinned females [@buolamwini2018gender] |
...
: **Fairness Risk by ML Archetype**: Fairness risks vary by archetype's data source and deployment context. {#tbl-fairness-archetype tbl-colwidths="[18,30,22,30]"}
...
Several open-source tools support responsible testing workflows. Fairlearn [@bird2020fairlearn] provides fairness metrics and mitigation algorithms that integrate with scikit-learn pipelines. AI Fairness 360 [@bellamy2019] offers over 70 fairness metrics and ten bias mitigation algorithms across the ML lifecycle.
...
A loan approval model reports 85 percent accuracy on the majority group and 82.5 percent overall accuracy across the evaluated applicants---numbers that may satisfy a coarse aggregate dashboard.\index{Fairness Metrics!loan approval case study} @tbl-confusion-group-a and @tbl-confusion-group-b reveal what the aggregate conceals: loan approval outcomes for the same model evaluated separately on two demographic groups.
...
class LoanFairness:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a_approval_str = fmt(a_app_pct, precision=0, commas=False, suffix="%")
    b_approval_str = fmt(b_app_pct, precision=0, commas=False, suffix="%")
    dp_disparity_str = fmt(dp_disparity, precision=0, commas=False, suffix=" percentage points")
    a_tpr_str = fmt(a_tpr_pct, precision=0, commas=False, suffix="%")
    b_tpr_str = fmt(b_tpr_pct, precision=0, commas=False, suffix="%")
    tpr_disparity_str = fmt(tpr_disparity, precision=0, commas=False, suffix=" percentage points")
    a_fpr_str = fmt(a_fpr_pct, precision=0, commas=False, suffix="%")
    b_fpr_str = fmt(b_fpr_pct, precision=0, commas=False, suffix="%")
...
: **Confusion Matrix for Group A (Majority)**: Loan approval outcomes for 10,000 applicants from the majority demographic group. The 90 percent true positive rate (4,500 approved of 5,000 qualified) and 20 percent false positive rate establish the baseline for fairness comparison. {#tbl-confusion-group-a}
...
: **Confusion Matrix for Group B (Minority)**: Loan approval outcomes for 2,000 applicants from the minority demographic group. The 60 percent true positive rate (600 approved of 1,000 qualified) reveals a 30 percentage point disparity compared to Group A, indicating the model applies stricter criteria to minority applicants. {#tbl-confusion-group-b}

Three standard fairness metrics\index{Fairness Metrics!confusion matrix analysis} computed from the confusion matrices in @tbl-confusion-group-a and @tbl-confusion-group-b reveal significant disparities.[^fn-fairness-metric-incompatibility]

[^fn-fairness-metric-incompatibility]: **Fairness Metric Incompatibility**: The measured disparities in this worked example show how one set of confusion matrices can violate demographic parity, equal opportunity, and equalized odds. A separate impossibility theorem proves that when group base rates differ, multiple fairness criteria *cannot* generally be satisfied simultaneously [@chouldechova2017]. In those settings, optimizing for one metric, such as equal opportunity, can degrade another, such as predictive parity. A system designer must therefore make the trade-off explicit rather than assuming all guarantees can be achieved at once. \index{Fairness Metrics!impossibility theorem}

Demographic parity\index{Fairness Metrics!demographic parity} requires equal approval rates across groups. Group A receives approval at a rate of (`{python} LoanFairness.a_tp_str` + `{python} LoanFairness.a_fp_str`) / `{python} LoanFairness.a_total_str` = `{python} LoanFairness.a_approval_str`, while Group B receives approval at (`{python} LoanFairness.b_tp_str` + `{python} LoanFairness.b_fp_str`) / `{python} LoanFairness.b_total_str` = `{python} LoanFairness.b_approval_str`. The `{python} LoanFairness.dp_disparity_str` disparity indicates unequal treatment in approval decisions.

Equal opportunity\index{Fairness Metrics!equal opportunity} requires equal true positive rates\index{True Positive Rate!fairness computation} among qualified applicants. Group A achieves a TPR of `{python} LoanFairness.a_tp_str` / (`{python} LoanFairness.a_tp_str` + `{python} LoanFairness.a_fn_str`) = `{python} LoanFairness.a_tpr_str`, meaning `{python} LoanFairness.a_tpr_str` of applicants who would repay receive approval. Group B achieves only `{python} LoanFairness.b_tp_str` / (`{python} LoanFairness.b_tp_str` + `{python} LoanFairness.b_fn_str`) = `{python} LoanFairness.b_tpr_str` TPR. This `{python} LoanFairness.tpr_disparity_str` disparity means qualified applicants from Group B face substantially higher rejection rates than equally qualified applicants from Group A.
...
Automated monitoring achieves what manual auditing cannot at scale: continuous tracking of fairness metrics with immediate alerting when disparities emerge. The `{python} LoanFairness.tpr_disparity_str` TPR disparity far exceeds common industry thresholds of 5 percentage points for high-stakes applications, indicating the model requires fairness intervention before deployment.
...
|:------------------------|:--------------------------------------:|:--------------------------------------:|:------------------------------------------|
| **Approval Rate**       | `{python} LoanFairness.a_approval_str` | `{python} LoanFairness.b_approval_str` | `{python} LoanFairness.dp_disparity_str`  |
| **True Positive Rate**  |   `{python} LoanFairness.a_tpr_str`    |   `{python} LoanFairness.b_tpr_str`    | `{python} LoanFairness.tpr_disparity_str` |
...
To understand *why* aggregate metrics hide these disparities, look closely at @fig-fairness-threshold. When a single threshold is applied to populations with different score distributions, the same decision boundary produces vastly different outcomes for each group [@barocas2016]. The figure exposes a fundamental tension: any fixed threshold is simultaneously "correct" for the combined population while being systematically wrong for each subpopulation.

::: {#fig-fairness-threshold fig-env="figure" fig-pos="htb" fig-cap="**Threshold Effects on Subgroup Outcomes**: A single classification threshold (vertical lines) applied to two subgroups with different score distributions produces disparate outcomes. Circles represent positive outcomes (loan repayment); plus markers represent negative outcomes (default). The 75 percent threshold approves most of Subgroup A but rejects most of Subgroup B, even when qualified individuals exist in both groups. The 81.25 percent threshold shows how threshold adjustment changes the fairness-accuracy trade-off. This visualization explains why aggregate accuracy can mask severe subgroup disparities." fig-alt="Diagram showing two subgroups A and B with different score distributions. Vertical threshold lines at 75 percent and 81.25 percent show how the same threshold produces different approval rates for each group."}
...
class FairnessPrice:
    ...
    positive_base_rate     = 0.50      # Fraction of applicants who would be successful hires
    tpr_gap_pp             = 20
    group_a_tpr_pct        = 90
    group_b_tpr_pct        = 70
    aggregate_accuracy_pct = 85

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    # The intervention raises False Positives by fp_increase_pp percentage
    # points for the disadvantaged group only. Extra cost per applicant from
    # that group is fp_increase_pp/100 * bad_hire_cost; the baseline gain per
    # applicant pair is positive_base_rate * hire_value. The aggregate
    # utility loss scales by minority_group_share since only that group sees
    # the threshold change:
    extra_cost_per_applicant = (fp_increase_pp / 100.0) * bad_hire_cost
    baseline_gain_per_pair   = positive_base_rate * hire_value
    loss_fraction_within_group = extra_cost_per_applicant / (baseline_gain_per_pair + extra_cost_per_applicant)
    utility_loss_pct         = loss_fraction_within_group * minority_group_share * 100.0

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(1.0 < utility_loss_pct < 2.0, f"Utility tax {utility_loss_pct:.1f}% no longer matches the stated assumptions.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Custom "USD " prefix / " percent" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    hire_value_k_str    = fmt(hire_value, precision=0, prefix="USD ")            # e.g. "USD 100,000"
    bad_hire_cost_k_str = fmt(bad_hire_cost, precision=0, prefix="USD ")         # e.g. "USD 50,000"
    fp_increase_str     = fmt(fp_increase_pp, precision=0, commas=False, suffix=" percent")         # e.g. "5%"
    utility_loss_str    = fmt(utility_loss_pct, precision=1, commas=False, suffix=" percent")   # computed from assumptions above
    minority_share_str  = fmt(minority_group_share * 100, precision=0, commas=False, suffix=" percent")
    positive_base_rate_str = fmt(positive_base_rate * 100, precision=0, commas=False, suffix=" percent")
    tpr_gap_str = fmt(tpr_gap_pp, precision=0, commas=False, suffix=" percent")
    group_a_tpr_str = fmt(group_a_tpr_pct, precision=0, commas=False, suffix=" percent")
    group_b_tpr_str = fmt(group_b_tpr_pct, precision=0, commas=False, suffix=" percent")
    aggregate_accuracy_str = fmt(aggregate_accuracy_pct, precision=0, commas=False, suffix=" percent")
```

::: {#nbk-responsible-engr-price-fairness .callout-notebook title="The price of fairness"}

**Problem**: Stakeholders demand elimination of a `{python} FairnessPrice.tpr_gap_str` True Positive Rate (TPR) disparity in a hiring model. What is the "Price of Fairness" in terms of hiring quality?

**Physics**: TPRs can be equalized by adjusting the classification threshold $(\gamma_{\text{cls}})$ for the disadvantaged group.

*   **Original state**: Group A (TPR = `{python} FairnessPrice.group_a_tpr_str`), Group B (TPR = `{python} FairnessPrice.group_b_tpr_str`). Aggregate Accuracy = `{python} FairnessPrice.aggregate_accuracy_str`.
*   **Intervention**: Lower $\gamma_{\text{cls},g=B}$ until Group B TPR = `{python} FairnessPrice.group_a_tpr_str`.
*   **The cost**: Lowering the threshold increases **False Positives** (hiring candidates who do not meet the bar).

**Math**:

1.  Closing the `{python} FairnessPrice.tpr_gap_str` TPR gap requires accepting a **`{python} FairnessPrice.fp_increase_str` increase** in False Positives for the disadvantaged group.
2.  If the value of a successful hire is `{python} FairnessPrice.hire_value_k_str` and the cost of a bad hire is `{python} FairnessPrice.bad_hire_cost_k_str`:
    ...
    - Under the assumption that the disadvantaged group is `{python} FairnessPrice.minority_share_str` of the applicant pool and that the base rate of qualified applicants is `{python} FairnessPrice.positive_base_rate_str`, the aggregate utility loss is **`{python} FairnessPrice.utility_loss_str`**.

**Systems insight**: The "Price of Fairness" in this system is a `{python} FairnessPrice.utility_loss_str` utility tax under the stated assumptions, a **System Constraint**\index{Price of Fairness!utility trade-off}, not a bug. The exact figure depends on the base rate of qualified applicants and the disadvantaged group's share of the pool; the engineer's job is to present the Pareto frontier to stakeholders so they can choose the Utility/Fairness trade-off that aligns with organizational values.
...
: **Explainability Requirements by Domain**: Different applications require different levels of decision transparency. Credit and medical applications face regulatory requirements for individual explanations. Fraud detection may intentionally limit explainability to prevent gaming. The engineering challenge is matching explainability mechanisms to domain requirements. {#tbl-explainability-requirements tbl-colwidths="[18,24,58]"}

Engineering teams should select explainability approaches based on these domain requirements. Post-hoc explanation methods\index{Explainability!post-hoc methods (SHAP, LIME)}\index{SHAP!feature attribution}\index{LIME!local explanations} (LIME, SHAP) generate feature importance scores\index{Feature Importance!prediction attribution} for individual predictions without requiring model architecture changes.[^fn-lime-shap-cost] Inherently interpretable models\index{Interpretable Models!transparency by design} (linear models, decision trees, attention mechanisms) provide explanations as part of their structure but may sacrifice predictive performance. Concept-based explanations\index{Concept-based Explanations!human-understandable features} map model behavior to human-understandable concepts rather than raw features. The choice involves trade-offs between explanation fidelity, computational cost, and model flexibility. @fig-interpretability-spectrum arranges these trade-offs along a single axis. On the left side, decision trees and linear regression offer direct auditability: an engineer can inspect every coefficient or branching rule that produced a prediction, at the cost of limited representational capacity. On the right side, deep neural networks and convolutional architectures achieve higher accuracy on complex tasks but resist human inspection, requiring post-hoc tools like LIME or SHAP to approximate explanations. The choice depends on the application's accountability requirements: high-stakes credit decisions subject to adverse action notice laws demand models near the interpretable end, while large-scale recommendation systems that face no per-decision regulatory scrutiny can tolerate opaque architectures. The spectrum does not imply "simple is always better," because a highly interpretable model that makes wrong predictions serves no one. The engineering challenge is selecting the most interpretable model that meets accuracy requirements for the application.

::: {#ws-responsible-engr-hospital-shortcut .callout-war-story title="The hospital shortcut"}

**Context**: A team at Mount Sinai's Icahn School of Medicine trained deep learning models for pneumonia detection on chest X-rays from their own hospital and tested them against scans from the National Institutes of Health and Indiana University [@zech2018variable; @lapuschkin2019].

**Failure**: Models that appeared strong on internal data performed worse on external hospitals. The problem was not simply architecture quality: hospital-specific prevalence, acquisition patterns, and dataset artifacts created shortcuts that did not transfer cleanly across sites.

**Consequence**: A model could appear clinically useful in one hospital's data while failing when moved to another institution. That is precisely the dangerous form of shortcut learning: the model has learned the data-generating process, not necessarily the disease process.

**Systems lesson**: Neural networks exploit the easiest statistical signal available. External validation and interpretability tools (saliency maps\index{Saliency Maps!interpretability tool}) are quality-assurance gates, not presentation polish.

:::

[^fn-lime-shap-cost]: **LIME and SHAP**: LIME [@ribeiro2016] fits a local interpretable model around each prediction---fast but potentially inconsistent across nearby inputs. SHAP [@lundberg2017unified] adapts Shapley values\index{Shapley Values!feature attribution} from cooperative game theory to compute feature contributions under a unified additive framework. Exact Shapley-value computation can be expensive, so practical SHAP implementations rely on approximations or model-specific algorithms. The systems trade-off is that explanation fidelity, latency, and implementation complexity must be budgeted explicitly rather than treated as free. \index{SHAP!inference latency}
...
\tikzset{%
  ...
  Box2/.style={Box, draw=myred, fill=magenta!03 },
  Txt/.style={,font=\usefont{T1}{phv}{m}{it}\footnotesize,black!50},
  LineA/.style={black!50,line width=1.25pt,
  {{Triangle[width=1.0*5pt,length=1.0*8pt]}-{Triangle[width=1.0*5pt,length=1.0*8pt]}},
  shorten <=0pt,shorten >=0pt},
}
...
Neural\\ Network,
CNN/\\Transformer}{
\node[align=flush center,font=\usefont{T1}{phv}{m}{n}\footnotesize,below=5pt of T\x]{\i};
}
...
($(POG)+(0,4mm)$)--($(SRG)+(-2mm,4mm)$)
node [text=black,midway,above=2mm] {Intrinsically Interpretable};
%
\draw[mybrown,line width=0.75pt,decoration={brace,amplitude=7pt},decorate]
($(SRG)+(2mm,4mm)$)--($(KRG)+(0mm,4mm)$)
node [text=black,midway,above=2mm] {Requires Post-Hoc Explanation};
%
\node[below=23mm of T2,Txt]{Inspect decision logic directly};
\node[below=23mm of T5,Txt]{Use LIME, SHAP, or attention maps};
\end{tikzpicture}
...
[^fn-eu-ai-act-compliance]: **EU AI Act (Regulation 2024/1689)**: The first comprehensive AI legal framework, defining four risk tiers with penalties that vary by infringement category. Prohibited AI-practice violations can reach EUR 35 million or 7 percent of global turnover; many other obligations, including many high-risk operator obligations, are capped at EUR 15 million or 3 percent. The Act has extraterritorial reach: non-EU organizations may need to comply when they place systems on the EU market or when system outputs are used in the EU. Systems engineering implications are concrete: high-risk AI requires logging infrastructure for audit trails, human oversight mechanisms built into the architecture, and CE marking---all capabilities that must be designed in from inception, not retrofitted after deployment. \index{EU AI Act!systems requirements}
...
\index{GDPR!Article 22 automated decisions} GDPR Article 22 grants EU data subjects the right not to be subject to decisions based solely on automated processing that produce legal or similarly significant effects, subject to specified exceptions and safeguards.[^fn-gdpr-automated-decisions] GDPR Article 15(1)(h) separately gives data subjects access to meaningful information about the logic involved in automated decision-making referred to in Article 22. While legal interpretation varies, engineering teams should assume that every high-stakes automated decision requires both a human review mechanism where required and an explainability capability.
...
# │ Show:    One million daily decisions at a 0.1 percent error rate yield 1,000
# │          review cases per day.
# │ How:     Multiply daily decision volume by the review-rate fraction.
# │
# │ Imports: mlsysim.core.constants (MILLION), mlsysim.fmt (fmt, check)
# │ Exports: GDPRReviewLoad.daily_decisions_str, GDPRReviewLoad.review_rate_str,
# │          GDPRReviewLoad.review_cases_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check
...
class GDPRReviewLoad:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    daily_decisions_str = fmt(daily_decisions / MILLION, precision=0, commas=False)
    review_rate_str = fmt(review_rate_pct, precision=1, commas=False)
    review_cases_str = fmt(review_cases, precision=0, commas=True)
```

[^fn-gdpr-automated-decisions]: **GDPR (General Data Protection Regulation) Articles 15 and 22**: Article 22 restricts certain solely automated decisions with legal or similarly significant effects and, for specified exceptions, requires safeguards including human intervention, the ability to express a point of view, and the ability to contest the decision. Article 15(1)(h) contains the access right to meaningful information about the logic involved in such automated decision-making. The European Data Protection Board's guidance emphasizes that required human oversight must be substantive and not merely a "rubber-stamping" exercise. A system making `{python} GDPRReviewLoad.daily_decisions_str` million daily decisions with a `{python} GDPRReviewLoad.review_rate_str` error rate requiring substantive review would generate `{python} GDPRReviewLoad.review_cases_str` cases per day, an operational load that is untenable without built-in summarization and audit tools. \index{GDPR!Article 22 requirements}

#### US sectoral regulations {#sec-responsible-engineering-us-sectoral-regulations-5377}

US sectoral regulations impose domain-specific requirements that, while less unified than the EU AI Act, collectively create significant compliance obligations for ML systems. Fair lending laws (ECOA, Fair Housing Act) require creditors to provide specific reasons for adverse credit decisions—the origin of the "adverse action notice" requirement that drives explainability needs in financial ML. Healthcare regulations, including the Health Insurance Portability and Accountability Act (HIPAA)[^fn-hipaa-ml-constraints] and FDA guidance, layer data protection and validation requirements onto medical AI systems, while employment law prohibits discriminatory hiring practices regardless of whether discrimination results from human or algorithmic decision-making. The cumulative effect is that any ML system operating across multiple domains faces an intersection of regulatory requirements, each mandating different technical capabilities.
...
Zillow reported a USD 304 million[^fn-zillow-dam] Q3 2021 Homes-segment inventory write-down after buying homes at prices above revised estimates of future selling prices. A systems diagnosis can interpret the failure as a combination of forecasting uncertainty, distribution shift, operational capacity limits, and insufficient circuit breakers. Planning for system failures before they occur is a core responsible engineering practice.\index{Incident Response!ML system failures}\index{Monitoring!responsible operations} Building on the incident severity classification and response framework from @sec-ml-operations-incident-response-ml-systems-c637, @tbl-incident-response extends the general framework with fairness-specific detection and response criteria, structuring preparation into five components with both requirements and predeployment verification criteria.

[^fn-zillow-dam]: **Zillow's D·A·M Failure**: Zillow's USD 304M write-down in 2021 is a useful systems case because the documented business failure combined forecast uncertainty with operational execution. A D·A·M diagnosis interprets the **D**ata axis as the mismatch between historical home-sale data and pandemic-era price volatility, the **A**lgorithm axis as the difficulty of pricing homes with reliable uncertainty estimates, and the **M**achine axis as an automated iBuying pipeline that needed stronger capacity limits and circuit breakers. This is an engineering interpretation of Zillow's public disclosure, not a claim that Zillow identified one root technical cause. \index{Zillow!distribution shift failure}
...
ML systems create unique maintenance challenges [@sculley2015hidden].\index{Technical Debt!ML systems} Models degrade silently, dependencies shift unexpectedly, and feedback loops amplify small problems into large ones.\index{Feedback Loop!problem amplification} Incident response planning must account for these ML-specific failure modes, and effective response depends on continuous monitoring infrastructure that detects problems in the first place.
...
The techniques from earlier chapters directly serve responsibility goals. Quantization\index{Quantization!environmental benefits} (@sec-model-compression) reduces compute by 2--4$\times$ with minimal accuracy impact. Pruning\index{Pruning!carbon reduction} removes 50–90 percent of parameters. Knowledge distillation\index{Knowledge Distillation!efficiency gains} typically achieves 5--20$\times$ compression while retaining 90–95 percent of the original accuracy. Hardware acceleration (@sec-hardware-acceleration) achieves 10--100$\times$ better energy efficiency than general-purpose processors.
...
class EdgeEfficiencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Custom unit suffixes (W, ms, mW, second, M, K) → MarkdownStr per recipe Rule 1 escape-hatch
    smart_power_str   = fmt(smart_power.m_as(watt), precision=0, commas=False, suffix=" W")
    smart_latency_str = fmt(smart_latency.m_as(ms), precision=0, commas=False, suffix=" ms")
    iot_power_str     = fmt(iot_power.m_as(milliwatt), precision=0, commas=False, suffix=" mW")
    iot_latency_str   = fmt(iot_latency.m_as(second), precision=0, commas=False, suffix=" second")
    cam_power_str     = fmt(cam_power.m_as(watt), precision=0, commas=False, suffix=" W")
    cam_latency_str   = fmt(cam_latency.m_as(ms), precision=0, commas=False, suffix=" ms")
    wear_power_str    = fmt(wear_power.m_as(milliwatt), precision=0, commas=False, suffix=" mW")
    wear_latency_str  = fmt(wear_latency.m_as(ms), precision=0, commas=False, suffix=" ms")

    mv2_params_str  = fmt(mv2_params.m_as(Mparam), precision=1, commas=False, suffix=" M")
    mv2_power_str   = fmt(mv2_power.m_as(watt), precision=1, commas=False, suffix=" W")
    mv2_latency_str = fmt(mv2_latency.m_as(ms), precision=0, commas=False, suffix=" ms")

    eff_params_str  = fmt(eff_params.m_as(Mparam), precision=1, commas=False, suffix=" M")
    eff_power_str   = fmt(eff_power.m_as(watt), precision=1, commas=False, suffix=" W")
    eff_latency_str = fmt(eff_latency.m_as(ms), precision=0, commas=False, suffix=" ms")

    rn50_params_str  = fmt(rn50_params.m_as(Mparam), precision=1, commas=False, suffix=" M")
    rn50_power_str   = fmt(rn50_power.m_as(watt), precision=1, commas=False, suffix=" W")
    rn50_latency_str = fmt(rn50_latency.m_as(ms), precision=0, commas=False, suffix=" ms")

    tiny_params_str  = fmt(tiny_params.m_as(Kparam), precision=0, commas=False, suffix=" K")
    tiny_power_str   = fmt(tiny_power.m_as(milliwatt), precision=0, commas=False, suffix=" mW")
    tiny_latency_str = fmt(tiny_latency.m_as(ms), precision=0, commas=False, suffix=" ms")
    tiny_wear_power_margin_str = fmt(tiny_wear_power_margin, precision=0, commas=False)
    mv2_wear_power_overage_str = fmt(mv2_wear_power_overage, precision=1, commas=False)
...
| **IoT Sensor**\index{IoT!power budget constraints} |  `{python} EdgeEfficiencyCalc.iot_power_str`  |     `{python} EdgeEfficiencyCalc.iot_latency_str`      | Anomaly detection, environmental monitoring |
...
: **Edge Deployment Constraints**: Power and latency requirements across four deployment contexts. Smartphones allow 5 W and 100 ms latency for photo enhancement and voice assistants. IoT sensors operate at 100 mW with one second tolerance for anomaly detection. Embedded cameras require 1 W at 33 ms (30 FPS) for real-time object detection. Wearables budget 500 mW with 500 ms latency for health monitoring. These concrete constraints transform abstract efficiency discussions into engineering requirements. {#tbl-edge-deployment-constraints tbl-colwidths="[17,16,24,43]"}
...
|:--------------------|:---------------------------------------------:|:--------------------------------------------:|:----------------------------------------------:|:-------------------------------------------------:|:-----------------------------------------------:|
| **MobileNetV2**     |  `{python} EdgeEfficiencyCalc.mv2_params_str` | `{python} EdgeEfficiencyCalc.mv2_power_str`  | `{python} EdgeEfficiencyCalc.mv2_latency_str`  |  `{python} EdgeEfficiencyCalc.mv2_fits_phone_str` |  `{python} EdgeEfficiencyCalc.mv2_fits_iot_str` |
| **EfficientNet-B0** |  `{python} EdgeEfficiencyCalc.eff_params_str` | `{python} EdgeEfficiencyCalc.eff_power_str`  | `{python} EdgeEfficiencyCalc.eff_latency_str`  |  `{python} EdgeEfficiencyCalc.eff_fits_phone_str` |  `{python} EdgeEfficiencyCalc.eff_fits_iot_str` |
| **ResNet-50**       | `{python} EdgeEfficiencyCalc.rn50_params_str` | `{python} EdgeEfficiencyCalc.rn50_power_str` | `{python} EdgeEfficiencyCalc.rn50_latency_str` | `{python} EdgeEfficiencyCalc.rn50_fits_phone_str` | `{python} EdgeEfficiencyCalc.rn50_fits_iot_str` |
| **TinyML Model**    | `{python} EdgeEfficiencyCalc.tiny_params_str` | `{python} EdgeEfficiencyCalc.tiny_power_str` | `{python} EdgeEfficiencyCalc.tiny_latency_str` | `{python} EdgeEfficiencyCalc.tiny_fits_phone_str` | `{python} EdgeEfficiencyCalc.tiny_fits_iot_str` |

: **Model Efficiency Comparison**: Model selection must account for deployment constraints. Larger models provide better accuracy but require more power and time. The smallest model that meets accuracy requirements minimizes both cost and environmental impact. {#tbl-model-efficiency-comparison tbl-colwidths="[20,15,20,10,25,10]"}

For the wearable budget in @tbl-edge-deployment-constraints, the TinyML model leaves a `{python} EdgeEfficiencyCalc.tiny_wear_power_margin_str`$\times$ power margin, while MobileNetV2 exceeds the same power budget by `{python} EdgeEfficiencyCalc.mv2_wear_power_overage_str`$\times$ before accounting for sustained thermals.

### Total cost of ownership {#sec-responsible-engineering-total-cost-ownership-35c1}

A team spends USD 3,200 training a recommendation model and celebrates the modest cost. Six months later, they discover they are spending USD 500,000 per year serving it. The surprise exposes a structural asymmetry in **total cost of ownership**\index{Total Cost of Ownership!definition}[^fn-tco-inference-dominance]: power budgets translate directly to financial costs (a model that consumes 2 W instead of 4 W cuts electricity expenses in half), and for successful production systems, inference costs typically exceed training costs by ten to 1,000 times\index{Total Cost of Ownership!inference dominance} depending on traffic volume. Inference cost dominance dictates where optimization efforts should focus.
...
# │
# │ Imports: mlsysim.core.constants (Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate, Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate)
...
class InferenceCostCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    gpu_rate_value       = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)
    ...
    gpu_inf_rate_value  = Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate.m_as(USD/hour)
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(total_train_cost_value == 3_200, f"Training cycle cost changed to ${total_train_cost_value:,.0f}.")
    check(inf_train_ratio_value >= 35, f"Inference ratio {inf_train_ratio_value:.1f}:1 should dominate training.")
    ...
    inferences_m_str    = fmt(inferences_daily_value/MILLION, precision=0, commas=False)
    gpus_str            = fmt(gpus_needed_value, precision=1, commas=False)
    annual_inf_str      = fmt(annual_inf_cost_value, precision=1, commas=True)
    lifecycle_inf_str   = fmt(lifecycle_inf_cost_value/MILLION, precision=1, commas=False)
    ratio_str           = fmt(inf_train_ratio_value, precision=1, commas=False)
    gpu_rate_input_str  = fmt(gpu_rate_value, precision=0, commas=False)
    data_prep_hrs_str   = fmt(data_prep_hrs_value, precision=0)
    hyperparam_hrs_str  = fmt(hyperparam_hrs_value, precision=0)
    train_hrs_str       = fmt(train_hrs_value, precision=0)
    users_daily_m_str   = fmt(users_daily_value/MILLION, precision=0, commas=False)
    recs_per_user_str   = fmt(recs_per_user_value, precision=0, commas=False)
    inference_ms_str    = fmt(inference_ms_value, precision=0, commas=False)
    gpu_inf_rate_str    = fmt(gpu_inf_rate_value, precision=2, commas=False)
```

Consider a concrete example of a recommendation system serving `{python} InferenceCostCalc.users_daily_m_str` million users daily. Training costs appear considerable: data preparation consumes `{python} InferenceCostCalc.data_prep_hrs_str`-hours at approximately USD `{python} InferenceCostCalc.gpu_rate_input_str` per hour (USD `{python} InferenceCostCalc.data_prep_str`), hyperparameter search across multiple configurations requires `{python} InferenceCostCalc.hyperparam_hrs_str`-hours (USD `{python} InferenceCostCalc.hyperparam_str`), and the final training run uses `{python} InferenceCostCalc.train_hrs_str`-hours (USD `{python} InferenceCostCalc.train_cost_str`). Total training cost reaches approximately USD `{python} InferenceCostCalc.total_train_str`.

Inference costs dominate. With `{python} InferenceCostCalc.users_daily_m_str` million users each receiving `{python} InferenceCostCalc.recs_per_user_str` recommendations per day, the system serves `{python} InferenceCostCalc.inferences_m_str` million inferences daily. Assuming `{python} InferenceCostCalc.inference_ms_str` milliseconds per inference on GPU hardware, the system requires approximately `{python} InferenceCostCalc.gpus_str` running continuously. At USD `{python} InferenceCostCalc.gpu_inf_rate_str` per GPU-hour, annual GPU costs reach USD `{python} InferenceCostCalc.annual_inf_str`.
...
Per-query optimization\index{Optimization!inference, per-query cost} becomes essential when serving billions of requests. Reducing inference latency by ten milliseconds per query translates to measurable reductions in required hardware across billions of queries despite appearing negligible for individual requests. Hardware selection between CPU, GPU, and Tensor Processing Unit (TPU) deployment changes costs and carbon footprint by factors of ten or more. Model compression through quantization and pruning delivers immediate return on investment for high-volume systems because inference cost reduction compounds across every subsequent query.

Total cost of ownership encompasses additional dimensions beyond computation.\index{Total Cost of Ownership!operational costs} Operational costs include monitoring, maintenance, retraining, and incident response, all of which scale with system complexity and the rate of distribution shift in the application domain. Opportunity costs reflect that resources consumed by ML systems cannot be used for other purposes. Wasteful resource consumption in one project constrains what other projects can attempt.

Engineers should evaluate whether the value an ML system delivers justifies its resource consumption.\index{ROI!ML systems} A recommendation system that increases engagement by 1 percent might not justify millions of dollars in computational costs, while a medical diagnosis system that saves lives does. Explicit trade-offs enable responsible resource allocation.[^fn-ml-roi-heuristic]

[^fn-ml-roi-heuristic]: **ML Return on Investment**: The 10:1 deployment-to-training cost ratio emerges from the composition of monitoring (continuous), retraining (periodic), infrastructure (ongoing), and incident response (unpredictable), each of which scales with deployment duration and data volume rather than with the initial development effort. A model deployed for three years accumulates roughly 10--15$\times$ its development cost in operational overhead. Responsible engineering practices that reduce incident frequency and severity therefore yield ROI proportional to deployment lifetime, explaining why a logistic regression at 1 percent of the cost often represents the correct engineering decision when the TCO difference compounds over years. \index{ML ROI!heuristic baseline}

Quantifying environmental impact requires converting compute hours into carbon emissions\index{Carbon Footprint!compute to emissions conversion}, making carbon a first-class engineering metric alongside dollar cost.
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.A100.tdp, Infrastructure.Pricing.Fleet.CarbonPerGpuHr.rate, watt, ureg)
# │          mlsysim.fmt (fmt, check)
# │ Exports: CarbonFactor.gpu_power_w_str, CarbonFactor.gpu_power_kw_str,
# │          CarbonFactor.carbon_intensity_str, CarbonFactor.carbon_per_gpu_hr_str
# └─────────────────────────────────────────────────────────────────────────────
...
class CarbonFactor:
    ...
    carbon_intensity_kg_kwh = baseline_grid.carbon_intensity_kg_kwh

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    gpu_power_kw = Hardware.Cloud.A100.tdp.to("kilowatt")
    gpu_power_w = Hardware.Cloud.A100.tdp.to(watt)
    scenario_hours = 1 * hour
    it_energy_kwh = (gpu_power_kw * scenario_hours).to(kilowatt * hour)
    carbon_per_gpu_hr_kg = it_energy_kwh.magnitude * carbon_intensity_kg_kwh

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(gpu_power_w.magnitude - 400) < 1e-9, "A100 baseline power should be 400 W.")
    check(abs(carbon_per_gpu_hr_kg - gpu_power_kw.magnitude * carbon_intensity_kg_kwh) < 0.005,
          "Per-GPU-hour carbon must equal kW × h × grid intensity.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpu_power_w_str = fmt(gpu_power_w.magnitude, precision=0, commas=False, suffix=" W")
    gpu_power_kw_str = fmt(gpu_power_kw.magnitude, precision=1, commas=False, suffix=" kW")
    carbon_intensity_str = fmt(carbon_intensity_kg_kwh, precision=2, suffix=" kg/kWh")
    carbon_per_gpu_hr_str = fmt(carbon_per_gpu_hr_kg, precision=2, suffix=" kg")
    scenario_hr_str = fmt(1, precision=0, commas=False, suffix=" h")
```

::: {#psp-responsible-engr-carbon-cost-compute .callout-perspective title="The carbon cost of compute"}

**Quantifying Environmental Impact**: To make carbon a first-class engineering metric, we must convert "compute hours" into "kg CO~2~eq". @eq-carbon-footprint captures this standard conversion:
...
*   **Power**: `{python} CarbonFactor.gpu_power_w_str` TDP per GPU (scenario baseline).
*   **Intensity**: `{python} CarbonFactor.carbon_intensity_str` CO~2~eq/kWh (US-average grid baseline).
*   **Conversion factor**: (`{python} CarbonFactor.gpu_power_kw_str` $\times$ `{python} CarbonFactor.scenario_hr_str`) $\times$ `{python} CarbonFactor.carbon_intensity_str` = **`{python} CarbonFactor.carbon_per_gpu_hr_str` CO~2~eq per GPU-hour**.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: tco-training-calc
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TCO TRAINING COSTS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-tco-training and training-costs subsection.
# │
# │ Goal: Training-cycle and three-year training TCO with carbon accounting.
# │ Show: Training is a small share of three-year TCO despite recurring cycles.
# │ How: LEGO class TCOTrainingCalc; scenario inputs duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt, check, MarkdownStr)
# │ Exports: TCOTrainingCalc.t_*_str, TCOTrainingCalc.p_train_str,
# │          TCOTrainingCalc.train_gpu_rate_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr

# ┌── LEGO ───────────────────────────────────────────────
class TCOTrainingCalc:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    train_gpu_rate = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)
    inf_gpu_rate = Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate.m_as(USD/hour)
    ...
    inf_tco_3yr = inf_gpu_hours_day * inf_gpu_rate * DAYS_PER_YEAR * 3
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(p_train < 5, f"Training share {p_train:.1f}% should be a small TCO fraction.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    t_data_prep_calc_str = MarkdownStr(f"{t_data_prep_hrs} GPU-hr × ${train_gpu_rate:.0f} = ${t_data_prep_hrs * train_gpu_rate:,.0f}")
    t_hparam_calc_str = MarkdownStr(f"{t_hparam_exps} × ${t_hparam_cost_exp:.0f} = ${t_hparam_exps * t_hparam_cost_exp:,.0f}")
    t_final_calc_str = MarkdownStr(f"{t_final_hrs} GPU-hr × ${train_gpu_rate:.0f} = ${t_final_hrs * train_gpu_rate:,.0f}")
    t_cycles_calc_str = MarkdownStr(f"{t_cycles_3yr // 3}/year × 3 years = {t_cycles_3yr}")
    ...
    t_data_prep_carbon_str = fmt(t_data_prep_hrs * carbon_per_gpu_hr, precision=0, commas=False, suffix=" kg")
    t_hparam_carbon_str = fmt((t_hparam_exps * t_hparam_cost_exp / train_gpu_rate) * carbon_per_gpu_hr, precision=0, commas=False, suffix=" kg")
    t_final_carbon_str = fmt(t_final_hrs * carbon_per_gpu_hr, precision=0, commas=False, suffix=" kg")
    t_subtotal_carbon_str = fmt(train_carbon_cycle, precision=0, commas=False, suffix=" kg")
    t_total_carbon_str = fmt(train_carbon_3yr, precision=0, suffix=" kg")
    p_train_str = fmt(p_train, precision=1)
    train_gpu_rate_str = fmt(train_gpu_rate, precision=0, commas=False)
```
...
|:--------------------------------|:------------------------------------:|:-----------------------------------------------:|:----------------------------------------------------------------:|
| **Initial data preparation**    |         hours $\times$ rate          | `{python} TCOTrainingCalc.t_data_prep_calc_str` |        `{python} TCOTrainingCalc.t_data_prep_carbon_str`         |
| **Hyperparameter search**       | experiments $\times$ cost/experiment |   `{python} TCOTrainingCalc.t_hparam_calc_str`  |          `{python} TCOTrainingCalc.t_hparam_carbon_str`          |
| **Final training**              |         hours $\times$ rate          |   `{python} TCOTrainingCalc.t_final_calc_str`   |          `{python} TCOTrainingCalc.t_final_carbon_str`           |
| **Subtotal per training cycle** |                                      |  **`{python} TCOTrainingCalc.t_subtotal_str`**  |       **`{python} TCOTrainingCalc.t_subtotal_carbon_str`**       |
| **Retraining frequency**        |      cycles/year $\times$ years      |   `{python} TCOTrainingCalc.t_cycles_calc_str`  | same multiplier (`{python} TCOTrainingCalc.t_cycles_str` cycles) |
| **Total training cost**         |       subtotal $\times$ cycles       |    **`{python} TCOTrainingCalc.t_total_str`**   |        **`{python} TCOTrainingCalc.t_total_carbon_str`**         |

: **Training Cost Calculation**: Training costs accumulate through initial development (USD `{python} TCOTrainingCalc.t_subtotal_str` per cycle) and quarterly retraining over a three-year operational period. Data preparation, hyperparameter search, and final training consume GPU hours at USD `{python} TCOTrainingCalc.train_gpu_rate_str`/hour, totaling USD `{python} TCOTrainingCalc.t_total_str` across `{python} TCOTrainingCalc.t_cycles_str` training cycles. Despite appearing substantial, training represents only `{python} TCOTrainingCalc.p_train_str` of total cost of ownership. {#tbl-tco-training tbl-colwidths="[25,32,23,20]"}
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: tco-inference-calc
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TCO INFERENCE COSTS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-tco-inference and inference-costs subsection.
# │
# │ Goal: Daily and three-year inference TCO with carbon accounting.
# │ Show: Inference dominates TCO for high-traffic recommendation systems.
# │ How: LEGO class TCOInferenceCalc; scenario inputs duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt, check, MarkdownStr)
# │ Exports: TCOInferenceCalc.i_*_str, TCOInferenceCalc.p_inf_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr

# ┌── LEGO ───────────────────────────────────────────────
class TCOInferenceCalc:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    train_gpu_rate = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)
    inf_gpu_rate = Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate.m_as(USD/hour)
    carbon_per_gpu_hr = Infrastructure.Pricing.Fleet.CarbonPerGpuHr.rate.m_as(ureg.kilogram)
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(p_inf > 50, f"Inference share {p_inf:.1f}% should dominate TCO.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    i_daily_q_calc_str = MarkdownStr(f"{i_users/MILLION:.0f}M × {i_recs_per_user} = {inf_daily_total/MILLION:.0f}M")
    i_gpu_sec_calc_str = MarkdownStr(f"{inf_daily_total/MILLION:.0f}M × {i_latency_s} s = {inf_daily_total * i_latency_s/MILLION:.1f}M sec")
    i_gpu_hr_day_str = fmt(inf_gpu_hours_day, precision=1, commas=False, suffix=" GPU-hr")
    i_daily_carbon_str = fmt(inf_carbon_day, precision=1, commas=False, suffix=" kg")
    i_annual_calc_str = MarkdownStr(f"{inf_gpu_hours_day:.0f} × {DAYS_PER_YEAR} × ${inf_gpu_rate:.2f} = ${inf_cost_day * DAYS_PER_YEAR / THOUSAND:.0f}K")
    i_annual_carbon_str = fmt(inf_carbon_3yr / 3, precision=1, suffix=" kg")
    i_total_str = fmt(inf_tco_3yr / MILLION, precision=2, commas=False, prefix="$", suffix="M")
    i_carbon_str = fmt(inf_carbon_3yr, precision=1, suffix=" kg")
    i_daily_queries_m_str = fmt(inf_daily_total / MILLION, precision=0, commas=False)
    i_latency_ms_str = fmt(i_latency.m_as(ms), precision=0, commas=False)
    i_annual_cost_k_str = fmt(inf_cost_day * DAYS_PER_YEAR / THOUSAND, precision=1, commas=False)
    i_total_m_str = fmt(inf_tco_3yr / MILLION, precision=2, commas=False)
    p_inf_str = fmt(p_inf, precision=1)
```
...
| **Cost Component**        | **Calculation**                  |               **Financial Cost**               |              **Carbon (kg CO~2~)**              |
|:--------------------------|:---------------------------------|:----------------------------------------------:|:-----------------------------------------------:|
| **Daily queries**         | users $\times$ queries/user      | `{python} TCOInferenceCalc.i_daily_q_calc_str` |                        -                        |
| **GPU-seconds/day**       | queries $\times$ latency         | `{python} TCOInferenceCalc.i_gpu_sec_calc_str` |                        -                        |
| **GPU-hours/day**         | seconds ÷ SEC_PER_HOUR           |  `{python} TCOInferenceCalc.i_gpu_hr_day_str`  |  `{python} TCOInferenceCalc.i_daily_carbon_str` |
| **Annual GPU cost**       | hours $\times$ 365 $\times$ rate | `{python} TCOInferenceCalc.i_annual_calc_str`  | `{python} TCOInferenceCalc.i_annual_carbon_str` |
| **3-year inference cost** | annual $\times$ 3                |  **`{python} TCOInferenceCalc.i_total_str`**   |   **`{python} TCOInferenceCalc.i_carbon_str`**  |

: **Inference Cost Calculation**: Inference costs scale with query volume: `{python} TCOInferenceCalc.i_daily_queries_m_str` million daily queries at `{python} TCOInferenceCalc.i_latency_ms_str` each require `{python} TCOInferenceCalc.i_gpu_hr_day_str` daily, totaling USD `{python} TCOInferenceCalc.i_annual_cost_k_str`K annually and USD `{python} TCOInferenceCalc.i_total_m_str` million over three years. At `{python} TCOInferenceCalc.p_inf_str` of total cost, inference dominates for high-traffic systems and justifies aggressive per-query optimization through quantization, pruning, and efficient serving. {#tbl-tco-inference tbl-colwidths="[25,27,27,21]"}
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: tco-operations-calc
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TCO OPERATIONAL COSTS
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-tco-operations and operational-costs subsection.
# │
# │ Goal: Three-year monitoring, on-call, and incident-response TCO.
# │ Show: Operational costs are a persistent share of three-year TCO.
# │ How: LEGO class TCOOperationsCalc; scenario inputs duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt, check)
# │ Exports: TCOOperationsCalc.o_*_str, TCOOperationsCalc.p_ops_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class TCOOperationsCalc:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    train_gpu_rate = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)
    inf_gpu_rate = Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate.m_as(USD/hour)
    t_data_prep_hrs = 100
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(o_total_3yr > train_tco_3yr, "Operational TCO should exceed training TCO.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    o_monitor_annual_str = fmt(o_monitor_yr / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    o_monitor_str = fmt(o_monitor_yr * 3 / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    o_oncall_annual_str = fmt(o_oncall_yr / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    o_oncall_str = fmt(o_oncall_yr * 3 / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    o_incident_annual_str = fmt(o_incident_yr / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    o_incident_str = fmt(o_incident_yr * 3 / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    o_total_str = fmt(o_total_3yr / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    o_monitor_annual_k_str = fmt(o_monitor_yr / THOUSAND, precision=0, commas=False)
    o_oncall_annual_k_str = fmt(o_oncall_yr / THOUSAND, precision=0, commas=False)
    o_incident_annual_k_str = fmt(o_incident_yr / THOUSAND, precision=0, commas=False)
    o_total_k_str = fmt(o_total_3yr / THOUSAND, precision=0, commas=False)
    p_ops_str = fmt(p_ops, precision=1)
```
...
|:----------------------------------|:--------------------------------------------------:|:--------------------------------------------:|
| **Monitoring infrastructure**     | `{python} TCOOperationsCalc.o_monitor_annual_str`  |  `{python} TCOOperationsCalc.o_monitor_str`  |
| **On-call engineering (0.5 FTE)** |  `{python} TCOOperationsCalc.o_oncall_annual_str`  |  `{python} TCOOperationsCalc.o_oncall_str`   |
| **Incident response (estimated)** | `{python} TCOOperationsCalc.o_incident_annual_str` | `{python} TCOOperationsCalc.o_incident_str`  |
| **Total operational**             |                                                    | **`{python} TCOOperationsCalc.o_total_str`** |

: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (USD `{python} TCOOperationsCalc.o_monitor_annual_k_str`K/year), on-call engineering at 0.5 FTE (USD `{python} TCOOperationsCalc.o_oncall_annual_k_str`K/year), and incident response reserves (USD `{python} TCOOperationsCalc.o_incident_annual_k_str`K/year). The USD `{python} TCOOperationsCalc.o_total_k_str`K three-year total represents `{python} TCOOperationsCalc.p_ops_str` of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2--5$\times$ more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations tbl-colwidths="[34,33,33]"}
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @tbl-tco-summary, checkpoint callout, and summary prose nearby.
# │
# │ Goal: Summarize 3-year lifecycle costs and quantization ROI.
# │ Show: Inference dominates TCO; 20% latency cuts yield large savings.
# │ How: LEGO class TCOSummaryTable; scenario inputs duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt_int, fmt, check)
# │ Exports: TCOSummaryTable.p_*_str, TCOSummaryTable.t_total_k_str,
# │          TCOSummaryTable.i_total_str, TCOSummaryTable.o_total_str,
# │          TCOSummaryTable.total_tco_str, TCOSummaryTable.*_carbon_*_str,
# │          TCOSummaryTable.inf_train_ratio_str, TCOSummaryTable.quant_*_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class TCOSummaryTable:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    train_gpu_rate = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)
    inf_gpu_rate = Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate.m_as(USD/hour)
    carbon_per_gpu_hr = Infrastructure.Pricing.Fleet.CarbonPerGpuHr.rate.m_as(ureg.kilogram)
    ...
    train_tco_3yr = train_cost_cycle * t_cycles_3yr
    train_gpu_hrs_cycle = t_data_prep_hrs + t_final_hrs + (t_hparam_exps * t_hparam_cost_exp / train_gpu_rate)
    train_carbon_3yr = train_gpu_hrs_cycle * carbon_per_gpu_hr * t_cycles_3yr

    inf_daily_total = i_users * i_recs_per_user
    inf_gpu_hours_day = (inf_daily_total * i_latency_s) / SEC_PER_HOUR
    inf_tco_3yr = inf_gpu_hours_day * inf_gpu_rate * DAYS_PER_YEAR * 3
    inf_carbon_3yr = inf_gpu_hours_day * carbon_per_gpu_hr * DAYS_PER_YEAR * 3
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(inf_train_ratio >= 35, f"Inference ratio {inf_train_ratio:.1f}:1 should dominate training.")
    check(savings_dollars >= 100_000, f"Savings (${savings_dollars:,.0f}) too small to justify optimization.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    p_train_str = fmt(p_train, precision=1, suffix="%")
    p_inf_str = fmt(p_inf, precision=1, suffix="%")
    p_ops_str = fmt(p_ops, precision=1, suffix="%")
    t_total_k_str = fmt(train_tco_3yr / THOUSAND, precision=1, commas=False, prefix="$", suffix="K")
    i_total_str = fmt(inf_tco_3yr / MILLION, precision=2, commas=False, prefix="$", suffix="M")
    o_total_str = fmt(o_total_3yr / THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    total_tco_str = fmt(total_tco / MILLION, precision=2, commas=False, prefix="$", suffix="M")
    t_total_carbon_tons_str = fmt(train_carbon_3yr / THOUSAND, precision=1, commas=False, suffix=" tons")
    i_carbon_tons_str = fmt(inf_carbon_3yr / THOUSAND, precision=1, commas=False, suffix=" tons")
    total_carbon_tons_str = fmt(total_carbon_kg / THOUSAND, precision=1, commas=False, prefix="~", suffix=" tons")
    inf_train_ratio_str = fmt(inf_train_ratio, precision=1, commas=False)
    quant_savings_str = fmt(savings_dollars / THOUSAND, precision=1, commas=False)
    quant_carbon_str = fmt(savings_carbon_kg / THOUSAND, precision=1, commas=False)
    quant_reduction_pct_str = fmt_int(quant_reduction_pct * 100, commas=False)
```

The stark breakdown in @tbl-tco-summary answers where the money goes: inference at `{python} TCOSummaryTable.p_inf_str`, operations at `{python} TCOSummaryTable.p_ops_str`, and training at only `{python} TCOSummaryTable.p_train_str`.

| **Category**   |               **3-Year Cost**                |             **Percentage**             |                  **Carbon Impact**                   |
|:---------------|:--------------------------------------------:|:--------------------------------------:|:----------------------------------------------------:|
| **Training**   |   `{python} TCOSummaryTable.t_total_k_str`   | `{python} TCOSummaryTable.p_train_str` |  `{python} TCOSummaryTable.t_total_carbon_tons_str`  |
| **Inference**  |    `{python} TCOSummaryTable.i_total_str`    |  `{python} TCOSummaryTable.p_inf_str`  |     `{python} TCOSummaryTable.i_carbon_tons_str`     |
| **Operations** |    `{python} TCOSummaryTable.o_total_str`    |  `{python} TCOSummaryTable.p_ops_str`  |                          -                           |
| **Total TCO**  | **`{python} TCOSummaryTable.total_tco_str`** |                  100%                  | **`{python} TCOSummaryTable.total_carbon_tons_str`** |

: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The `{python} TCOSummaryTable.inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummaryTable.quant_reduction_pct_str` reduction in inference latency through quantization saves about USD `{python} TCOSummaryTable.quant_savings_str`K and `{python} TCOSummaryTable.quant_carbon_str` metric tons of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
- [ ] **Inference dominance**: Can you explain why a `{python} TCOSummaryTable.quant_reduction_pct_str` inference latency reduction delivers more savings than <!-- lego-ok: pedagogical comparison baseline -->a 50 percent training time reduction for a production system serving millions of users?
...
class CarbonScaleCalc:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    train_energy_mwh = Models.Language.GPT3.training_energy_mwh
    carbon_intensity_g_kwh = Infrastructure.Grids.US_Avg.carbon_intensity_g_kwh
    car_annual_tons  = 4.6    # Passenger car annual emissions (tons CO2e)

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    carbon_intensity_kg_kwh = carbon_intensity_g_kwh / THOUSAND
    train_energy_kwh    = train_energy_mwh * 1000
    total_emissions_kg  = train_energy_kwh * carbon_intensity_kg_kwh
    total_emissions_tons = total_emissions_kg / THOUSAND  # kg → metric ton; mlsysim mass pint units pending
    cars_equivalent     = total_emissions_tons/car_annual_tons
    one_pct_cars_equivalent = cars_equivalent * 0.01

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(total_emissions_tons > 500, "GPT-3-scale training should exceed 500 metric tons CO2e.")
    check(cars_equivalent > 100, "Passenger-car equivalent should exceed 100 cars.")
    check(one_pct_cars_equivalent > 1, "One percent efficiency improvement should exceed one car-year.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_energy_mwh_str  = fmt(train_energy_mwh, precision=0, commas=True)
    train_energy_kwh_str  = fmt(train_energy_kwh, precision=0, commas=True)
    carbon_intensity_g_kwh_str = fmt(carbon_intensity_g_kwh, precision=0, commas=False)
    carbon_intensity_str  = fmt(carbon_intensity_kg_kwh, precision=3, commas=False)
    total_emissions_kg_str   = fmt(total_emissions_kg, precision=0, commas=True)
    total_emissions_tons_str = fmt(total_emissions_tons, precision=1, commas=False)
    cars_eq_str           = fmt_int(round(cars_equivalent), commas=False)
    one_pct_cars_equivalent_str = fmt(one_pct_cars_equivalent, precision=1, commas=False)
    car_annual_tons_str   = fmt(car_annual_tons, precision=1, commas=False)
```

::: {#nbk-responsible-engr-carbon-cost-scale .callout-notebook title="The carbon cost of scale"}

**Problem**: A foundation model is being trained at the scale of GPT-3, consuming `{python} CarbonScaleCalc.train_energy_mwh_str` Megawatt-hours (MWh) of electricity. What is the environmental impact?

**Math**:

1.  **Energy consumption**: `{python} CarbonScaleCalc.train_energy_mwh_str` = `{python} CarbonScaleCalc.train_energy_kwh_str`.
2.  **Carbon intensity**\index{Carbon Intensity!definition}: The average US grid emits $\approx$ `{python} CarbonScaleCalc.carbon_intensity_g_kwh_str` g CO~2~ per kWh (`{python} CarbonScaleCalc.carbon_intensity_str` kg/kWh).
3.  **Total emissions**: `{python} CarbonScaleCalc.train_energy_kwh_str` $\times$ `{python} CarbonScaleCalc.carbon_intensity_str` = `{python} CarbonScaleCalc.total_emissions_kg_str` kg CO~2~ (`{python} CarbonScaleCalc.total_emissions_tons_str` metric tons).
4.  **Comparison**: A typical passenger car emits ≈ `{python} CarbonScaleCalc.car_annual_tons_str` metric tons of CO~2~ per year.

**Systems insight**: Training a single state-of-the-art model is equivalent to the annual carbon footprint of **`{python} CarbonScaleCalc.cars_eq_str` cars**. At this scale, efficiency transforms from a technical preference into a moral requirement. Every 1 percent improvement in the **Efficiency $(\eta_{\text{hw}})$** of a training pipeline removes the equivalent of about `{python} CarbonScaleCalc.one_pct_cars_equivalent_str` cars' annual emissions from the atmosphere.
...
# │
# │ Imports: mlsysim.core.constants (MILLION), mlsysim.fmt (fmt, check)
# │ Exports: MetaGovernanceFine.facebook_fine_str,
# │          MetaGovernanceFine.instagram_fine_str,
# │          MetaGovernanceFine.total_fine_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check
...
::: {.callout-warning title="Compliance as an engineering need"}
...
[^fn-federated-learning-minimization]: **Federated Learning**: From Latin *foedus* (treaty, covenant)---the name describes independent entities collaborating while retaining autonomy. @mcmahan2017federated introduced Federated Averaging\index{Federated Averaging!data minimization} (FedAvg): each device trains locally and shares only gradient updates, never raw data. The etymology explains the design: federated learning provides "data minimization by architecture." However, gradient updates can leak training data through reconstruction attacks\index{Reconstruction Attack!federated learning vulnerability}, motivating the combination of federated learning with differential privacy---a defense-in-depth pattern where neither mechanism alone suffices. \index{Federated Learning!data minimization}
...
[^fn-data-lineage-compliance]: **Data Lineage Systems**: Apache Atlas and LinkedIn's DataHub capture metadata about data flows automatically from pipeline execution logs, creating directed graphs where nodes are datasets and edges are transformations. GDPR Article 30 requires detailed records of all processing activities, making automated lineage tracking essential: when a user invokes the right to erasure\index{Right to Erasure!data lineage requirement}, the lineage graph identifies every derived artifact---features, embeddings, trained models---that must be removed or retrained, a task that is infeasible manually at production scale. \index{Data Lineage!GDPR compliance}
...
# │ Exports: FairnessPitfallAnchor.tpr_disparity_str, .fnr_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check
...
Teams treat responsibility as external oversight rather than engineering practice. Engineering decisions made months before legal review constrain the solution space more than any compliance assessment. Architecture selection determines what fairness interventions are feasible (adding demographic tracking to a six-month-old pipeline costs 3--4$\times$ the initial implementation). Data pipeline design establishes whether disaggregated evaluation is even possible. As @sec-responsible-engineering-engineering-leadership-responsibility-e03c establishes, systems designed with responsibility as an engineering objective enable efficient validation; systems where responsibility is added at late-stage review face 6--12 months of redesign or deployment with documented risks.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: tco-inference-carbon-pitfall
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TCO INFERENCE-VS-TRAINING CARBON PITFALL
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Fallacies pitfall on training-vs-inference carbon focus.
# │
# │ Goal: Restate TCO and carbon ratios for the recommendation scenario.
# │ Show: Inference dominates both cost and carbon over three years.
# │ How: LEGO class TCOInferenceCarbonPitfall; scenario inputs duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt, check)
# │ Exports: TCOInferenceCarbonPitfall.p_train_str, p_inf_str,
# │          carbon_train_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class TCOInferenceCarbonPitfall:
    train_gpu_rate = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)
    inf_gpu_rate = Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate.m_as(USD/hour)
    carbon_per_gpu_hr = Infrastructure.Pricing.Fleet.CarbonPerGpuHr.rate.m_as(ureg.kilogram)
    t_data_prep_hrs = 100
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: tco-quantization-roi
# ┌─────────────────────────────────────────────────────────────────────────────
# │ TCO QUANTIZATION ROI RECAP
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: Chapter summary prose on efficiency and carbon accounting.
# │
# │ Goal: Restate quantization savings for the recommendation scenario.
# │ Show: A 20% inference latency cut yields large dollar and carbon savings.
# │ How: LEGO class TCOQuantizationROI; scenario inputs duplicated locally.
# │
# │ Imports: mlsysim.fmt (fmt_int, fmt, check)
# │ Exports: TCOQuantizationROI.quant_reduction_pct_str, quant_savings_str,
# │          quant_carbon_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int, fmt, check

# ┌── LEGO ───────────────────────────────────────────────
class TCOQuantizationROI:
    inf_gpu_rate = Infrastructure.Pricing.Cloud.GpuInferencePerHour.rate.m_as(USD/hour)
    carbon_per_gpu_hr = Infrastructure.Pricing.Fleet.CarbonPerGpuHr.rate.m_as(ureg.kilogram)
    i_users = 10_000_000
    i_recs_per_user = 20
...
The key insight unifying these tools is that translating responsibility concerns into measurable properties makes them tractable.\index{Measurable Properties!responsibility requirements} "Fairness gap <5 percent across groups" is actionable; "be fair" is not. This translation extends beyond fairness: efficiency becomes carbon accounting and TCO analysis, where a `{python} TCOQuantizationROI.quant_reduction_pct_str` latency reduction through quantization saves USD `{python} TCOQuantizationROI.quant_savings_str`K and eliminates `{python} TCOQuantizationROI.quant_carbon_str` metric tons of CO~2~. Documentation becomes model cards with explicit intended use and known limitations. Governance becomes access control, lineage tracking, and audit infrastructure that makes compliance demonstrable rather than aspirational. At every level, the same pattern holds: abstract ethical obligations become concrete engineering requirements that can be specified, tested, monitored, and enforced.
...
* **Proactive monitoring**: Biased systems\index{Silent Bias!proactive detection} continue operating without alerts because degraded predictions look identical to normal predictions. Monitoring must track outcome distributions across demographic groups, not just aggregate accuracy.
