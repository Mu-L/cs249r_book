...
***Distribution shift***\index{Distribution Shift!definition} is the violation of the **stationarity assumption**\index{Stationarity Assumption!distribution shift} $(P_{\text{train}} \neq P_{\text{deploy}})$ that underpins all supervised learning. It is the umbrella term for a family of drift types: data drift (see @sec-ml-operations) occurs when $P(X)$ shifts while $P(Y|X)$ remains stable; concept drift occurs when $P(Y|X)$ itself shifts.
...
class RepresentationStats:
    ...
    repr_equation_math = MarkdownStr(f"$$ N_{{\\text{{total}}}} = {target_imgs:,} \\text{{ images}} $$")
```
...
$N_{\text{total}}$ = `{python} RepresentationStats.repr_target_images_str` / `{python} RepresentationStats.repr_group_fraction_str` = `{python} RepresentationStats.repr_random_total_str` images
...
# │          kg CO2eq for downstream TCO calculations.
# │ Show:    A 400 W GPU emitting 0.16 kg CO2eq per hour implies a grid
...
class TCOSummary:
    ...
    quant_carbon_str = fmt(savings_carbon_kg / THOUSAND, precision=0, commas=False) # In Tons; kg → metric ton, mlsysim mass pint units pending
...
: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The `{python} TCOSummary.inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummary.quant_reduction_pct_str` percent reduction in inference latency through quantization saves about USD `{python} TCOSummary.quant_savings_str`K and `{python} TCOSummary.quant_carbon_str` tons of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
class CarbonScaleCalc:
    ...
    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(total_emissions_tons > 500, "GPT-3-scale training should exceed 500 metric tons CO2.")
