...
class TestingConstraintAnchor:
    ...
    multiplier_str = f"{data_multiplier}×"
...
class RepresentationStats:
    ...
    repr_equation_md = Markdown(f"$$ N_{{\\text{{total}}}} = {target_imgs:,} \\text{{ images}} $$")
```
...
class LoanFairness:
    ...
    b_fp_str = f"{b_fp:,}"
    b_tn_str = f"{b_tn:,}"
    a_total_str = f"{a_total:,}"
    b_total_str = f"{b_total:,}"
```
...
class FairnessPrice:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    hire_value_k_str    = f"USD {hire_value:,.0f}"            # e.g. "USD 100,000"
    bad_hire_cost_k_str = f"USD {bad_hire_cost:,.0f}"         # e.g. "USD 50,000"
    fp_increase_str     = f"{fp_increase_pp} percent"         # e.g. "5%"
    utility_loss_str    = f"{utility_loss_pct:.1f} percent"   # computed from assumptions above
    minority_share_str  = f"{minority_group_share * 100:.0f} percent"
    positive_base_rate_str = f"{positive_base_rate * 100:.0f} percent"
```
...
class EdgeEfficiencyCalc:
    ...
    mv2_fits_phone_str = "Yes" if mv2_fits_phone else "No"
    mv2_fits_iot_str = "Yes" if mv2_fits_iot else "No"
    eff_fits_phone_str = "Yes" if eff_fits_phone else "No"
    eff_fits_iot_str = "Yes" if eff_fits_iot else "No"
    rn50_fits_phone_str = "Yes" if rn50_fits_phone else "No"
    rn50_fits_iot_str = "Yes" if rn50_fits_iot else "No"
    tiny_fits_phone_str = "Yes" if tiny_fits_phone else "No"
    tiny_fits_iot_str = "Yes" if tiny_fits_iot else "No"
```
...
class TCOCalc:
    ...
    t_total_k_str = f"${train_tco_3yr/THOUSAND:.0f}K"
    ...
    t_total_carbon_tons_str = f"{train_carbon_3yr/THOUSAND:.1f} tons"  # kg → metric ton; mlsysim mass pint units pending
    ...
    i_annual_calc_str = f"{inf_gpu_hours_day:.0f} × {DAYS_PER_YEAR} × ${inf_gpu_rate:.2f} = ${inf_cost_day * DAYS_PER_YEAR / THOUSAND:.0f}K"
    i_annual_carbon_str = f"{inf_carbon_3yr/3:,.0f} kg"
    i_total_str = f"${inf_tco_3yr/MILLION:.2f}M"
    i_carbon_str = f"{inf_carbon_3yr:,.0f} kg"
    i_carbon_tons_str = f"{inf_carbon_3yr/THOUSAND:.1f} tons"  # kg → metric ton; mlsysim mass pint units pending
    ...
    o_monitor_annual_str = f"${o_monitor_yr/THOUSAND:.0f}K"
    o_monitor_str = f"${o_monitor_yr*3/THOUSAND:.0f}K"
    o_oncall_annual_str = f"${o_oncall_yr/THOUSAND:.0f}K"
    o_oncall_str = f"${o_oncall_yr*3/THOUSAND:.0f}K"
    o_incident_annual_str = f"${o_incident_yr/THOUSAND:.0f}K"
    o_incident_str = f"${o_incident_yr*3/THOUSAND:.0f}K"
    o_total_str = f"${o_total_3yr/THOUSAND:.0f}K"
    ...
    total_carbon_tons_str = f"~{total_carbon_kg/THOUSAND:.0f} tons"  # kg → metric ton; mlsysim mass pint units pending
...
class CarbonScaleCalc:
    ...
    carbon_intensity_str  = f"{carbon_intensity_kg_kwh:.3f}"
