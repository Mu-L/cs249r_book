...
*   **False Negatives**\index{False Negative Rate!risk assessment bias}: White defendants who *did* re-offend were incorrectly labeled as low-risk far more often than Black defendants (47.7 percent vs. 28.0 percent).
...
The alignment gap illustrates a failure that originates in the Algorithm axis: the optimization objective is misspecified, so even a model that generalizes flawlessly to new data can drive outcomes that conflict with organizational or societal goals. Distribution shift, by contrast, originates in the Data axis: the inputs changed, and the learned mapping no longer reflects reality. Both failures are silent, but they demand different remediations (better objectives vs. better monitoring), and conflating the two wastes engineering effort on the wrong fix. The D·A·M taxonomy introduced in @sec-introduction maps each failure to the axis it originates from\index{D·A·M Taxonomy!responsibility diagnosis} (Data · Algorithm · Machine, defined in @sec-appdx-dam-taxonomy).
...
The News Feed case demonstrates proxy drift at the platform level, where an aggregate engagement metric diverges from long-term user welfare. A distinct failure mode operates at the population level: proxy variables that appear neutral in the aggregate can encode systematic disparities across demographic groups. The distribution shift\index{Distribution Shift!population mismatch} defined earlier also manifests as population mismatch, where models trained on one population perform differently on another without obvious indicators. In healthcare, this population-level proxy failure produced one of the most widely studied cases of algorithmic harm.
...
class GenderShadesDisparity:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    error_light_male_str = fmt(err_light_male, precision=1, commas=False, suffix=" percent")
    error_light_female_str = fmt(err_light_female, precision=1, commas=False, suffix=" percent")
    error_dark_male_str = fmt_int(int(err_dark_male), commas=False, suffix=" percent")
    error_dark_female_str = fmt(err_dark_female, precision=1, commas=False, suffix=" percent")
    ...
    disparity_dark_male_str = fmt_int(int(disparity_dark_male), commas=False)
    acc_light_str = fmt(acc_light_male, precision=1, commas=False, suffix=" percent")
    acc_dark_str = fmt(acc_dark_female, precision=1, commas=False, suffix=" percent")
```

Responsible properties become testable when engineers work with stakeholders to define criteria appropriate for specific applications. The Gender Shades project\index{Gender Shades Study!algorithmic audit methodology}[^fn-gender-shades-audit] demonstrated how disaggregated evaluation\index{Disaggregated Evaluation!demographic stratification} across demographic categories reveals disparities invisible in aggregate metrics [@buolamwini2018gender], exposing the subgroup failures that responsibility monitoring must catch before deployment. The results captured dramatic error rate differences that commercial facial recognition systems showed across demographic groups. Concretely, a `{python} TestingConstraintAnchor.base_size_str`-sample test set that suffices for the majority group provides only `{python} TestingConstraintAnchor.minority_samples_str` samples for a `{python} TestingConstraintAnchor.minority_pct_str` ority subgroup—effectively requiring `{python} TestingConstraintAnchor.multiplier_str` more data than the majority group for high-confidence validation.
...
class RepresentationStats:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    repr_target_images_str = fmt(target_imgs, precision=0, suffix=" images")
    repr_group_fraction_pct_str = fmt_int(int(minority_fraction * 100), commas=False, suffix=" percent")
    repr_group_fraction_str = fmt(minority_fraction, precision=2, commas=False)
    repr_random_total_str = fmt_int(int(random_total), suffix=" images")
    repr_multiplier_str = fmt_int(int(multiplier), commas=False)
...
class LoanFairness:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a_approval_str = fmt(a_app_pct, precision=0, commas=False, suffix=" percent")
    b_approval_str = fmt(b_app_pct, precision=0, commas=False, suffix=" percent")
    dp_disparity_str = fmt(dp_disparity, precision=0, commas=False, suffix=" percentage-point")
    a_tpr_str = fmt(a_tpr_pct, precision=0, commas=False, suffix=" percent")
    b_tpr_str = fmt(b_tpr_pct, precision=0, commas=False, suffix=" percent")
    tpr_disparity_str = fmt(tpr_disparity, precision=0, commas=False, suffix=" percentage-point")
    a_fpr_str = fmt(a_fpr_pct, precision=0, commas=False, suffix=" percent")
    b_fpr_str = fmt(b_fpr_pct, precision=0, commas=False, suffix=" percent")
...
class FairnessPrice:
    ...
    hire_value_k_str    = fmt(hire_value, precision=0, prefix="$")            # e.g. "USD 100,000"
    bad_hire_cost_k_str = fmt(bad_hire_cost, precision=0, prefix="$")         # e.g. "USD 50,000"
    fp_increase_str     = fmt(fp_increase_pp, precision=0, commas=False, suffix=" percentage-point")  # absolute FPR change
...
1.  Closing the 20 percentage-point TPR gap requires accepting a `{python} FairnessPrice.fp_increase_str` increase in False Positives for the disadvantaged group.
...
# │
# │ Imports: mlsysim (Infrastructure.Pricing.Cloud), mlsysim.core.constants (MILLION, SEC_PER_HOUR, USD, hour, ms)
...
class InferenceCostCalc:
    ...
    gpus_str            = fmt(gpus_needed_value, precision=1, commas=False, suffix=" GPUs")
    annual_inf_str      = fmt_int(round(annual_inf_cost_value), commas=True)
    lifecycle_inf_str   = fmt(lifecycle_inf_cost_value/MILLION, precision=1, commas=False)
    ratio_str           = fmt_int(round(inf_train_ratio_value), commas=False)
    gpu_rate_input_str  = fmt(gpu_rate_value, precision=0, commas=False)
    data_prep_hrs_str   = fmt(data_prep_hrs_value, precision=0, suffix=" GPU-hours")
    hyperparam_hrs_str  = fmt(hyperparam_hrs_value, precision=0, suffix=" GPU-hours")
    train_hrs_str       = fmt(train_hrs_value, precision=0, suffix=" GPU-hours")
    ...
    inference_ms_str    = fmt(inference_ms_value, precision=0, commas=False, suffix=" milliseconds")
    gpu_inf_rate_str    = fmt(gpu_inf_rate_value, precision=2, commas=False)
```

Consider a concrete example of a recommendation system serving `{python} InferenceCostCalc.users_daily_m_str` million users daily. Training costs appear considerable: data preparation consumes `{python} InferenceCostCalc.data_prep_hrs_str` at approximately USD `{python} InferenceCostCalc.gpu_rate_input_str` per hour (USD `{python} InferenceCostCalc.data_prep_str`), hyperparameter search across multiple configurations requires `{python} InferenceCostCalc.hyperparam_hrs_str` (USD `{python} InferenceCostCalc.hyperparam_str`), and the final training run uses `{python} InferenceCostCalc.train_hrs_str` (USD `{python} InferenceCostCalc.train_cost_str`). Total training cost reaches approximately USD `{python} InferenceCostCalc.total_train_str`.

Inference costs dominate. With `{python} InferenceCostCalc.users_daily_m_str` million users each receiving `{python} InferenceCostCalc.recs_per_user_str` recommendations per day, the system serves `{python} InferenceCostCalc.inferences_m_str` million inferences daily. Assuming `{python} InferenceCostCalc.inference_ms_str` per inference on GPU hardware, the system requires approximately `{python} InferenceCostCalc.gpus_str` running continuously. At USD `{python} InferenceCostCalc.gpu_inf_rate_str` per GPU-hour, annual GPU costs reach USD `{python} InferenceCostCalc.annual_inf_str`.
...
# │
# │ Imports: mlsysim (Hardware.Cloud.A100.tdp), mlsysim.core.constants (Infrastructure.Pricing.Fleet.CarbonPerGpuHr.rate, watt, ureg)
...
class CarbonFactor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpu_power_w_str = fmt(gpu_power.m_as(watt), precision=0, commas=False, suffix=" W")
    gpu_power_kw_str = fmt(gpu_power_kw, precision=1, commas=False, suffix=" kW")
...
*   **Conversion factor**: (`{python} CarbonFactor.gpu_power_kw_str` $\times$ 1 hour) $\times$ `{python} CarbonFactor.carbon_intensity_str` kg/kWh = `{python} CarbonFactor.carbon_per_gpu_hr_str` kg CO~2~eq per GPU-hour.
...
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.constants (MILLION, ...)
...
# │          TCOCalc.i_daily_queries_m_str, TCOCalc.i_latency_ms_str,
# │          TCOCalc.i_annual_cost_k_str, TCOCalc.i_total_m_str,
# │          TCOCalc.o_monitor_annual_k_str, TCOCalc.o_oncall_annual_k_str,
# │          TCOCalc.o_incident_annual_k_str, TCOCalc.o_total_k_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_int, check, MarkdownStr
from mlsysim import Infrastructure
from mlsysim.core.constants import (
...
class TCOCalc:
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    train_gpu_rate = Infrastructure.Pricing.Cloud.GpuTrainingPerHour.rate.m_as(USD/hour)  # $4/hour
    ...
    check(abs(carbon_per_gpu_hr - CarbonFactor.carbon_per_gpu_hr_kg) < 1e-9, "TCO carbon factor diverges from carbon-factor-calc.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    # Training table strings — multi-value compound expressions → MarkdownStr per recipe Rule 1 escape-hatch
    ...
    t_total_k_str = fmt_int(round(train_tco_3yr/THOUSAND), commas=False, prefix="$", suffix="K")
    t_cycles_str = fmt(t_cycles_3yr, precision=0, commas=False)

    # Training carbon strings — custom " kg" / " tons" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    ...
    t_total_carbon_tons_str = fmt(train_carbon_3yr/THOUSAND, precision=1, commas=False, suffix=" tons")  # kg → metric ton; mlsysim mass pint units pending

    # Inference table strings — compound expressions → MarkdownStr per recipe Rule 1 escape-hatch
    ...
    i_gpu_hr_day_str = fmt_int(round(inf_gpu_hours_day), commas=False, suffix=" GPU-hr")
    ...
    i_carbon_tons_str = fmt(inf_carbon_3yr/THOUSAND, precision=1, commas=False, suffix=" tons")  # kg → metric ton; mlsysim mass pint units pending
    ...
    i_annual_cost_k_str = fmt_int(round(inf_cost_day * DAYS_PER_YEAR / THOUSAND), commas=False)
    i_total_m_str = fmt(inf_tco_3yr / MILLION, precision=2, commas=False)

    # Operations table strings — currency prefix → MarkdownStr per recipe Rule 1 escape-hatch
    ...
    # Summary strings — currency / suffix → MarkdownStr per recipe Rule 1 escape-hatch
    total_tco_str = fmt(total_tco/MILLION, precision=2, commas=False, prefix="$", suffix="M")
    total_carbon_tons_str = fmt_int(round(total_carbon_kg/THOUSAND), commas=False, prefix="~", suffix=" tons")  # kg → metric ton; mlsysim mass pint units pending
    p_train_str = fmt(p_train, precision=1, commas=False, suffix=" percent")
    p_inf_str = fmt(p_inf, precision=1, commas=False, suffix=" percent")
    p_ops_str = fmt(p_ops, precision=1, commas=False, suffix=" percent")
...
: **Training Cost Calculation**: Training costs accumulate through initial development (USD `{python} TCOCalc.t_subtotal_str` per cycle) and quarterly retraining over a three-year operational period. Data preparation, hyperparameter search, and final training consume GPU hours at USD `{python} TCOCalc.train_gpu_rate_str`/hour, totaling USD `{python} TCOCalc.t_total_str` across `{python} TCOCalc.t_cycles_str` training cycles. Despite appearing substantial, training represents only `{python} TCOCalc.p_train_str` of total cost of ownership. {#tbl-tco-training tbl-colwidths="[25,32,23,20]"}
...
: **Inference Cost Calculation**: Inference costs scale with query volume: `{python} TCOCalc.i_daily_queries_m_str` million daily queries at `{python} TCOCalc.i_latency_ms_str` each require `{python} TCOCalc.i_gpu_hr_day_str` daily, totaling USD `{python} TCOCalc.i_annual_cost_k_str`K annually and USD `{python} TCOCalc.i_total_m_str` million over three years. At `{python} TCOCalc.p_inf_str` of total cost, inference dominates for high-traffic systems and justifies aggressive per-query optimization through quantization, pruning, and efficient serving. {#tbl-tco-inference tbl-colwidths="[25,27,27,21]"}
...
: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (USD `{python} TCOCalc.o_monitor_annual_k_str`K/year), on-call engineering at 0.5 FTE (USD `{python} TCOCalc.o_oncall_annual_k_str`K/year), and incident response reserves (USD `{python} TCOCalc.o_incident_annual_k_str`K/year). The USD `{python} TCOCalc.o_total_k_str`K three-year total represents `{python} TCOCalc.p_ops_str` of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2--5$\times$ more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations tbl-colwidths="[34,33,33]"}
...
# │ Exports: inf_train_ratio_str, carbon_train_ratio_str, quant_savings_str,
# │          quant_carbon_str, quant_reduction_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TCOSummary:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    inf_train_ratio_str = fmt_int(round(inf_train_ratio), commas=False)
    carbon_train_ratio_str = fmt_int(round(carbon_train_ratio), commas=False)
    quant_savings_str = fmt_int(round(savings_dollars / THOUSAND), commas=False) # In K$
    quant_carbon_str = fmt_int(round(savings_carbon_kg / THOUSAND), commas=False) # In metric tons; kg -> metric ton, mlsysim mass pint units pending
    quant_reduction_pct_str = fmt_int(int(quant_reduction_pct * 100), commas=False, suffix=" percent")
```
...
: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The `{python} TCOSummary.inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummary.quant_reduction_pct_str` reduction in inference latency through quantization saves about USD `{python} TCOSummary.quant_savings_str`K and `{python} TCOSummary.quant_carbon_str` metric tons of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
- [ ] **Inference dominance**: Can you explain why a `{python} TCOSummary.quant_reduction_pct_str` inference latency reduction delivers more savings than a 50 percent training time reduction for a production system serving millions of users?
...
class CarbonScaleCalc:
    ...
    train_energy_kwh    = train_energy_mwh * THOUSAND
    ...
    total_emissions_tons_str = fmt_int(round(total_emissions_tons), commas=False)
...
**Problem**: A foundation model is being trained at the scale of GPT-3, consuming `{python} CarbonScaleCalc.train_energy_mwh_str` (Megawatt-hours) of electricity. What is the environmental impact?
...
The key insight unifying these tools is that translating responsibility concerns into measurable properties makes them tractable.\index{Measurable Properties!responsibility requirements} "Fairness gap <5 percent across groups" is actionable; "be fair" is not. This translation extends beyond fairness: efficiency becomes carbon accounting and TCO analysis, where a `{python} TCOSummary.quant_reduction_pct_str` latency reduction through quantization saves USD `{python} TCOSummary.quant_savings_str`K and eliminates `{python} TCOSummary.quant_carbon_str` metric tons of CO~2~. Documentation becomes model cards with explicit intended use and known limitations. Governance becomes access control, lineage tracking, and audit infrastructure that makes compliance demonstrable rather than aspirational. At every level, the same pattern holds: abstract ethical obligations become concrete engineering requirements that can be specified, tested, monitored, and enforced.
...
* **Tractable responsibility**: Vague mandates become engineering targets only when expressed as measurable thresholds, such as a bounded fairness gap across groups rather than an unquantified instruction to "be fair." The Pareto frontier makes fairness-accuracy trade-offs explicit and quantifiable for stakeholder decisions.
