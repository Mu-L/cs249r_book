...
class RepresentationStats:
    ...
    repr_equation_math = MarkdownStr(f"$$ N_{{\\text{{eval,target}}}} = {target_imgs:,} \\text{{ images}} $$")
```
...
$N_{\text{eval,total}}$ = `{python} RepresentationStats.repr_target_images_str` / `{python} RepresentationStats.repr_group_fraction_str` = `{python} RepresentationStats.repr_random_total_str` images
...
*   **Intervention**: Lower $\tau_{\text{cls},g=B}$ until $\text{TPR}_{g=B} = 90$ percent.
