...
class EdgeEfficiencyCalc:
    ...
    smart_power_str   = MarkdownStr(f"{smart_power.m_as(watt):.0f} W")
    smart_latency_str = MarkdownStr(f"{smart_latency.m_as(ms):.0f} ms")
    iot_power_str     = MarkdownStr(f"{iot_power.m_as(milliwatt):.0f} mW")
    iot_latency_str   = MarkdownStr(f"{iot_latency.m_as(second):.0f} second")
    cam_power_str     = MarkdownStr(f"{cam_power.m_as(watt):.0f} W")
    cam_latency_str   = MarkdownStr(f"{cam_latency.m_as(ms):.0f} ms")
    wear_power_str    = MarkdownStr(f"{wear_power.m_as(milliwatt):.0f} mW")
    wear_latency_str  = MarkdownStr(f"{wear_latency.m_as(ms):.0f} ms")

    mv2_params_str  = MarkdownStr(f"{mv2_params.m_as(Mparam):.1f} M")
    mv2_power_str   = MarkdownStr(f"{mv2_power.m_as(watt):.1f} W")
    mv2_latency_str = MarkdownStr(f"{mv2_latency.m_as(ms):.0f} ms")

    eff_params_str  = MarkdownStr(f"{eff_params.m_as(Mparam):.1f} M")
    eff_power_str   = MarkdownStr(f"{eff_power.m_as(watt):.1f} W")
    eff_latency_str = MarkdownStr(f"{eff_latency.m_as(ms):.0f} ms")

    rn50_params_str  = MarkdownStr(f"{rn50_params.m_as(Mparam):.1f} M")
    rn50_power_str   = MarkdownStr(f"{rn50_power.m_as(watt):.1f} W")
    rn50_latency_str = MarkdownStr(f"{rn50_latency.m_as(ms):.0f} ms")

    tiny_params_str  = MarkdownStr(f"{tiny_params.m_as(Kparam):.0f} K")
    tiny_power_str   = MarkdownStr(f"{tiny_power.m_as(milliwatt):.0f} mW")
    tiny_latency_str = MarkdownStr(f"{tiny_latency.m_as(ms):.0f} ms")
```
...
class TCOCalc:
    ...
    t_hparam_carbon_str = MarkdownStr(f"{(t_hparam_exps * t_hparam_cost_exp/train_gpu_rate) * carbon_per_gpu_hr:.0f} kg")
