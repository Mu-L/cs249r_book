...
class RepresentationStats:
    ...
    # Float 0.01 → canonical fmt would corrupt; use MarkdownStr escape-hatch per recipe
    repr_group_fraction_str = fmt(minority_fraction, precision=0, commas=False)
...
class LoanFairness:
    ...
    a_tp_str = fmt(a_tp, precision=0); a_fn_str = fmt(a_fn, precision=0); a_fp_str = fmt(a_fp, precision=0); a_tn_str = fmt(a_tn, precision=0)
    b_tp_str = fmt(b_tp, precision=0); b_fn_str = fmt(b_fn, precision=0); b_fp_str = fmt(b_fp, precision=0); b_tn_str = fmt(b_tn, precision=0)
    a_total_str = fmt(a_total, precision=0); b_total_str = fmt(b_total, precision=0)
```
...
2.  If the value of a successful hire is `{python} FairnessPrice.hire_value_k_str` and the cost of a bad hire is `{python} FairnessPrice.bad_hire_cost_k_str`:
    ...
    - Under the assumption that the disadvantaged group is `{python} FairnessPrice.minority_share_str` of the applicant pool and that the base rate of qualified applicants is 50 percent, the aggregate utility loss is **`{python} FairnessPrice.utility_loss_str`**.
...
# │ Show: The feasibility of different model scales (MobileNet vs. ResNet vs. TinyML) on edge hardware.
# │ How: Compare power budgets and parameter counts for mobile and IoT devices.
# │
# │ Imports: mlsysim.core.constants (watt, milliwatt, second, ms, Mparam, Kparam)
# │          mlsysim.Hardware, mlsysim.Models (Digital Twins)
...
*   **Conversion factor**: (0.4 kW $\times$ 1 hour) $\times$ 0.4 kg/kWh = **0.16 kg CO~2~eq per GPU-hour**.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check, MarkdownStr
from mlsysim.core.constants import THOUSAND, MILLION, MS_PER_SEC, HOURS_PER_DAY, SEC_PER_HOUR, CLOUD_GPU_TRAINING_PER_HOUR, USD, hour
...
class TCOCalc:
    ...
    i_annual_calc_str = MarkdownStr(f"{inf_gpu_hours_day:.0f} × 365 × ${inf_gpu_rate:.2f} = ${inf_cost_day * 365 / THOUSAND:.0f}K")
...
: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (USD 50K/year), on-call engineering at 0.5 FTE (USD 100K/year), and incident response reserves (USD 20K/year). The USD 510K three-year total represents 25 percent of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2--5$\times$ more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations tbl-colwidths="[34,33,33]"}
...
: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The ~40:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummary.quant_reduction_pct_str` percent reduction in inference latency through quantization can save hundreds of thousands of dollars and tons of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
class CarbonScaleCalc:
    ...
    carbon_intensity_str  = fmt(carbon_intensity, precision=1, commas=False)
...
2.  **Carbon intensity**\index{Carbon Intensity!definition}: The average US grid emits $\approx$ `{python} CarbonScaleCalc.carbon_intensity_str` kg CO~2~ per kWh.
3.  **Total emissions**: `{python} fmt(CarbonScaleCalc.train_energy_kwh, commas=True)` $\times$ `{python} CarbonScaleCalc.carbon_intensity_str` = `{python} CarbonScaleCalc.total_emissions_kg_str` kg CO~2~ (`{python} CarbonScaleCalc.total_emissions_tons_str` metric tons).
