...
class RepresentationStats:
    ...
    repr_moe_pp_str = fmt(moe_pp, precision=0, commas=False)
...
**Problem**: An engineering team needs to verify that a Face ID model works for a minority group representing `{python} RepresentationStats.repr_group_fraction_pct_str` percent of the user base. A worst-case binomial margin of error near `{python} RepresentationStats.repr_moe_pp_str` percentage point at `{python} RepresentationStats.repr_confidence_pct_str` percent confidence requires roughly `{python} RepresentationStats.repr_target_images_str` images for this group.

**Random Sampling**\index{Random Sampling!minority representation cost}: To get `{python} RepresentationStats.repr_target_images_str` images of a `{python} RepresentationStats.repr_group_fraction_pct_str` percent group via random sampling, the team must collect and label:
$D_{\text{eval,total}}$ = `{python} RepresentationStats.repr_target_images_str` / `{python} RepresentationStats.repr_group_fraction_str` = `{python} RepresentationStats.repr_random_total_str` images
...
Demographic parity\index{Fairness Metrics!demographic parity} requires equal approval rates across groups. Group A receives approval at a rate of (`{python} LoanFairness.a_tp_str` + `{python} LoanFairness.a_fp_str`) / `{python} LoanFairness.a_total_str` = `{python} LoanFairness.a_approval_str` percent, while Group B receives approval at (`{python} LoanFairness.b_tp_str` + `{python} LoanFairness.b_fp_str`) / `{python} LoanFairness.b_total_str` = `{python} LoanFairness.b_approval_str` percent. The `{python} LoanFairness.dp_disparity_str` percentage point disparity indicates unequal treatment in approval decisions.

Equal opportunity\index{Fairness Metrics!equal opportunity} requires equal true positive rates\index{True Positive Rate!fairness computation} among qualified applicants. Group A achieves a TPR of `{python} LoanFairness.a_tp_str` / (`{python} LoanFairness.a_tp_str` + `{python} LoanFairness.a_fn_str`) = `{python} LoanFairness.a_tpr_str` percent, meaning `{python} LoanFairness.a_tpr_str` percent of applicants who would repay receive approval. Group B achieves only `{python} LoanFairness.b_tp_str` / (`{python} LoanFairness.b_tp_str` + `{python} LoanFairness.b_fn_str`) = `{python} LoanFairness.b_tpr_str` percent TPR. This `{python} LoanFairness.tpr_disparity_str` percentage point disparity means qualified applicants from Group B face substantially higher rejection rates than equally qualified applicants from Group A.

Equalized odds[^fn-equalized-odds-postprocessing]\index{Fairness Metrics!equalized odds} requires both equal true positive rates and equal false positive rates. Group A shows an FPR of `{python} LoanFairness.a_fp_str` / (`{python} LoanFairness.a_fp_str` + `{python} LoanFairness.a_tn_str`) = `{python} LoanFairness.a_fpr_str` percent, and Group B shows `{python} LoanFairness.b_fp_str` / (`{python} LoanFairness.b_fp_str` + `{python} LoanFairness.b_tn_str`) = `{python} LoanFairness.b_fpr_str` percent. While false positive rates are equal, the true positive rate disparity means equalized odds is violated.
...
class CarbonFactor:
    ...
    # Create a minimal fleet for a single GPU-hour analysis

    class DummyFleet:
        node = SimpleNamespace(accelerator=Hardware.Cloud.A100)
        total_accelerators = 1
        effective_pue = 1.0 # analysis uses IT power directly
    ...
    res = SustainabilityModel().solve(
        ...
        duration_days=1/24.0,
        mfu=1.0 # peak analysis
    )

    gpu_power_kw = Hardware.Cloud.A100.tdp.m_as("kilowatt")
    carbon_per_gpu_hr_kg = res.it_energy_kwh.magnitude * baseline_grid.carbon_intensity_kg_kwh
    carbon_intensity_kg_kwh = baseline_grid.carbon_intensity_kg_kwh

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(abs(Hardware.Cloud.A100.tdp.m_as(watt) - 400) < 1e-9, "A100 baseline power should be 400 W.")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpu_power_w_str = fmt(Hardware.Cloud.A100.tdp.m_as(watt), precision=0, commas=False)
    gpu_power_kw_str = fmt(gpu_power_kw, precision=1, commas=False)
    carbon_intensity_str = fmt(carbon_intensity_kg_kwh, precision=1, commas=False)
    carbon_per_gpu_hr_str = fmt(carbon_per_gpu_hr_kg, precision=2, commas=False)
    scenario_hr_str = fmt(1, precision=0, commas=False)
```
...
*   **Power**: `{python} CarbonFactor.gpu_power_w_str` W per GPU-hour (scenario baseline).
*   **Intensity**: `{python} CarbonFactor.carbon_intensity_str` kg CO~2~eq/kWh (rounded grid baseline).
*   **Conversion factor**: (`{python} CarbonFactor.gpu_power_kw_str` kW $\times$ `{python} CarbonFactor.scenario_hr_str` hour) $\times$ `{python} CarbonFactor.carbon_intensity_str` kg/kWh = **`{python} CarbonFactor.carbon_per_gpu_hr_str` kg CO~2~eq per GPU-hour**.
...
: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (USD `{python} TCOOperationsCalc.o_monitor_annual_k_str`K/year), on-call engineering at 0.5 FTE (USD `{python} TCOOperationsCalc.o_oncall_annual_k_str`K/year), and incident response reserves (USD `{python} TCOOperationsCalc.o_incident_annual_k_str`K/year). The USD `{python} TCOOperationsCalc.o_total_k_str`K three-year total represents `{python} TCOOperationsCalc.p_ops_str` percent of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2--5$\times$ more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations tbl-colwidths="[34,33,33]"}
...
The stark breakdown in @tbl-tco-summary answers where the money goes: inference at `{python} TCOSummaryTable.p_inf_str` percent, operations at `{python} TCOSummaryTable.p_ops_str` percent, and training at only `{python} TCOSummaryTable.p_train_str` percent.
...
- [ ] **Inference dominance**: Can you explain why a `{python} TCOSummaryTable.quant_reduction_pct_str` percent inference latency reduction delivers more savings than <!-- lego-ok: pedagogical comparison baseline -->a 50 percent training time reduction for a production system serving millions of users?
...
1.  **Energy consumption**: `{python} CarbonScaleCalc.train_energy_mwh_str` MWh = `{python} CarbonScaleCalc.train_energy_kwh_str` kWh.
...
The key insight unifying these tools is that translating responsibility concerns into measurable properties makes them tractable.\index{Measurable Properties!responsibility requirements} "Fairness gap <5 percent across groups" is actionable; "be fair" is not. This translation extends beyond fairness: efficiency becomes carbon accounting and TCO analysis, where a `{python} TCOQuantizationROI.quant_reduction_pct_str` percent latency reduction through quantization saves USD `{python} TCOQuantizationROI.quant_savings_str`K and eliminates `{python} TCOQuantizationROI.quant_carbon_str` metric tons of CO~2~. Documentation becomes model cards with explicit intended use and known limitations. Governance becomes access control, lineage tracking, and audit infrastructure that makes compliance demonstrable rather than aspirational. At every level, the same pattern holds: abstract ethical obligations become concrete engineering requirements that can be specified, tested, monitored, and enforced.
