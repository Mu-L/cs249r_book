...
**The context**: In 2018, Facebook announced that News Feed ranking would shift toward posts that created more meaningful interactions among friends and family.

**The failure mode**: The change was a public example of a platform acknowledging that engagement proxies are incomplete. A system can optimize for relevance, clicks, or time spent while still producing passive consumption patterns that conflict with long-term user welfare.

**The consequence**: Facebook warned that pages could see reach, video watch time, and referral traffic decrease as the ranking objective changed.

**The systems lesson**: Metrics are proxies for value, not value itself. Ranking systems need long-term health metrics and policy constraints, not only short-term engagement targets [@facebook2018newsfeed].
...
**Context**: Optum, a healthcare services company, developed an algorithm to identify patients with complex health needs for enrollment in a high-risk care management program [@obermeyer2019].
...
**Systems lesson**: Proxies are dangerous. Optimizing for a proxy (cost) inherits the biases of the system that generated that proxy. The relationship between proxy and true objective (health) must be audited across all demographic subgroups.
...
**Context**: Researchers trained deep learning models for pneumonia detection across chest X-ray datasets from multiple hospitals [@zech2018variable; @lapuschkin2019].
