...
**The systems lesson**: Proxies are dangerous. Optimizing for a proxy (cost) inherits the biases of the system that generated that proxy. The relationship between proxy and true objective (health) must be audited across all demographic subgroups [@obermeyer2019].
...
**The systems lesson**: Adding a human backup to an unreliable system does not make it reliable; it creates a new system with complex failure modes. If the AI is 99 percent reliable, the human will eventually trust it 100 percent, making the "backup" useless precisely when it is needed most [@ntsb2019uber].
...
| **True Positive Rate**\index{True Positive Rate!fairness computation} |    `{python} LoanFairness.a_tpr_str`%   |    `{python} LoanFairness.b_tpr_str`%   | `{python} LoanFairness.tpr_disparity_str` percentage points |
...
- [ ] **Metric definitions**: Can you distinguish demographic parity, equal opportunity, equalized odds, and calibration\index{Calibration!fairness impossibility theorem} in terms of which rates must match across groups?
...
::: {#ws-responsible-engr-clever-hans-effect .callout-war-story title="The Clever Hans effect"}

**The context**: Researchers at Mount Sinai Hospital trained a neural network to detect pneumonia in chest X-rays [@zech2018variable]. The model achieved high internal test performance.

**The failure**: When tested on data from other hospitals, performance dropped and varied by site. Heatmap analysis revealed that the model sometimes relied on image regions outside the lungs. In one case, it had learned to detect a metal token that technicians at the training hospital placed on the patient's shoulder.

**The consequence**: The model risked becoming a "metal token detector," not a pneumonia detector. It had learned a spurious correlation in the training distribution that was irrelevant to the medical pathology.

**The systems lesson**: Neural networks are lazy optimizers. They will exploit the easiest statistical signal to minimize loss, even if that signal is medically irrelevant. Interpretability tools (saliency maps\index{Saliency Maps!interpretability tool}) are not optional; they are quality assurance gates [@lapuschkin2019].
...
- [ ] **Silent Failure**\index{Silent Failure!definition}: How will you know if the model is biased against a specific subgroup after deployment? (Aggregate metrics will not tell you).
