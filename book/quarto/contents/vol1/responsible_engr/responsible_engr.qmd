...
class TestingConstraintAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    base_size_str = fmt(base_test_size, precision=1, allow_zero=True)
    # Custom "%" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    minority_pct_str = fmt(minority_pct, precision=1, allow_zero=True, commas=False, suffix="%")
    minority_samples_str = fmt(minority_samples, precision=1, allow_zero=True, commas=False)
    # Custom "×" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    multiplier_str = fmt(data_multiplier, precision=1, allow_zero=True, commas=False, suffix="×")
...
class RepresentationStats:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    repr_target_images_str = fmt(target_imgs, precision=1, allow_zero=True)
    repr_group_fraction_pct_str = fmt(int(minority_fraction * 100), precision=1, allow_zero=True, commas=False)
    repr_group_fraction_str = fmt(minority_fraction, precision=2, commas=False)
    repr_random_total_str = fmt(int(random_total), precision=1, allow_zero=True)
    repr_multiplier_str = fmt(int(multiplier), precision=1, allow_zero=True, commas=False)
...
class LoanFairness:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a_approval_str = fmt(a_app_pct, precision=1, allow_zero=True, commas=False)
    b_approval_str = fmt(b_app_pct, precision=1, allow_zero=True, commas=False)
    dp_disparity_str = fmt(dp_disparity, precision=1, allow_zero=True, commas=False)
    a_tpr_str = fmt(a_tpr_pct, precision=1, allow_zero=True, commas=False)
    b_tpr_str = fmt(b_tpr_pct, precision=1, allow_zero=True, commas=False)
    tpr_disparity_str = fmt(tpr_disparity, precision=1, allow_zero=True, commas=False)
    a_fpr_str = fmt(a_fpr_pct, precision=1, allow_zero=True, commas=False)
    b_fpr_str = fmt(b_fpr_pct, precision=1, allow_zero=True, commas=False)
    a_fnr_str = fmt(a_fnr_pct, precision=1, allow_zero=True, commas=False)
    b_fnr_str = fmt(b_fnr_pct, precision=1, allow_zero=True, commas=False)

    a_tp_str = fmt(a_tp, precision=1, allow_zero=True)
    a_fn_str = fmt(a_fn, precision=1, allow_zero=True)
    a_fp_str = fmt(a_fp, precision=1, allow_zero=True)
    a_tn_str = fmt(a_tn, precision=1, allow_zero=True)
    b_tp_str = fmt(b_tp, precision=1, allow_zero=True)
    b_fn_str = fmt(b_fn, precision=1, allow_zero=True)
    b_fp_str = fmt(b_fp, precision=1, allow_zero=True)
    b_tn_str = fmt(b_tn, precision=1, allow_zero=True)
    a_total_str = fmt(a_total, precision=1, allow_zero=True)
    b_total_str = fmt(b_total, precision=1, allow_zero=True)
```
...
class FairnessPrice:
    ...
    hire_value_k_str    = fmt(hire_value, precision=1, allow_zero=True, prefix="USD ")            # e.g. "USD 100,000"
    bad_hire_cost_k_str = fmt(bad_hire_cost, precision=1, allow_zero=True, prefix="USD ")         # e.g. "USD 50,000"
    fp_increase_str     = fmt(fp_increase_pp, precision=1, allow_zero=True, commas=False, suffix=" percent")         # e.g. "5%"
    utility_loss_str    = fmt(utility_loss_pct, precision=1, commas=False, suffix=" percent")   # computed from assumptions above
    minority_share_str  = fmt(minority_group_share * 100, precision=1, allow_zero=True, commas=False, suffix=" percent")
    positive_base_rate_str = fmt(positive_base_rate * 100, precision=1, allow_zero=True, commas=False, suffix=" percent")
```
...
class GDPRReviewLoad:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    daily_decisions_str = fmt(daily_decisions / MILLION, precision=1, allow_zero=True, commas=False)
    review_rate_str = fmt(review_rate_pct, precision=1, commas=False)
    review_cases_str = fmt(review_cases, precision=1, allow_zero=True, commas=True)
```
...
class EdgeEfficiencyCalc:
    ...
    smart_power_str   = fmt(smart_power.m_as(watt), precision=1, allow_zero=True, commas=False, suffix=" W")
    smart_latency_str = fmt(smart_latency.m_as(ms), precision=1, allow_zero=True, commas=False, suffix=" ms")
    iot_power_str     = fmt(iot_power.m_as(milliwatt), precision=1, allow_zero=True, commas=False, suffix=" mW")
    iot_latency_str   = fmt(iot_latency.m_as(second), precision=1, allow_zero=True, commas=False, suffix=" second")
    cam_power_str     = fmt(cam_power.m_as(watt), precision=1, allow_zero=True, commas=False, suffix=" W")
    cam_latency_str   = fmt(cam_latency.m_as(ms), precision=1, allow_zero=True, commas=False, suffix=" ms")
    wear_power_str    = fmt(wear_power.m_as(milliwatt), precision=1, allow_zero=True, commas=False, suffix=" mW")
    wear_latency_str  = fmt(wear_latency.m_as(ms), precision=1, allow_zero=True, commas=False, suffix=" ms")
    ...
    mv2_latency_str = fmt(mv2_latency.m_as(ms), precision=1, allow_zero=True, commas=False, suffix=" ms")
    ...
    eff_latency_str = fmt(eff_latency.m_as(ms), precision=1, allow_zero=True, commas=False, suffix=" ms")
    ...
    rn50_latency_str = fmt(rn50_latency.m_as(ms), precision=1, allow_zero=True, commas=False, suffix=" ms")

    tiny_params_str  = fmt(tiny_params.m_as(Kparam), precision=1, allow_zero=True, commas=False, suffix=" K")
    tiny_power_str   = fmt(tiny_power.m_as(milliwatt), precision=1, allow_zero=True, commas=False, suffix=" mW")
    tiny_latency_str = fmt(tiny_latency.m_as(ms), precision=1, allow_zero=True, commas=False, suffix=" ms")
    tiny_wear_power_margin_str = fmt(tiny_wear_power_margin, precision=1, allow_zero=True, commas=False)
...
class InferenceCostCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    data_prep_str       = fmt(data_prep_cost_value, precision=1, allow_zero=True, commas=True)
    hyperparam_str      = fmt(hyperparam_cost_value, precision=1, allow_zero=True, commas=True)
    train_cost_str      = fmt(train_cost_value, precision=1, allow_zero=True, commas=True)
    total_train_str     = fmt(total_train_cost_value, precision=1, allow_zero=True, commas=True)
    lifecycle_train_str = fmt(lifecycle_train_cost_value, precision=1, allow_zero=True, commas=True)
    inferences_m_str    = fmt(inferences_daily_value/MILLION, precision=1, allow_zero=True, commas=False)
    gpus_str            = fmt(gpus_needed_value, precision=1, allow_zero=True, commas=False)
    annual_inf_str      = fmt(annual_inf_cost_value, precision=1, allow_zero=True, commas=True)
    lifecycle_inf_str   = fmt(lifecycle_inf_cost_value/MILLION, precision=1, commas=False)
    ratio_str           = fmt(inf_train_ratio_value, precision=1, allow_zero=True, commas=False)
    gpu_rate_input_str  = fmt(gpu_rate_value, precision=1, allow_zero=True, commas=False)
    data_prep_hrs_str   = fmt(data_prep_hrs_value, precision=1, allow_zero=True)
    hyperparam_hrs_str  = fmt(hyperparam_hrs_value, precision=1, allow_zero=True)
    train_hrs_str       = fmt(train_hrs_value, precision=1, allow_zero=True)
    users_daily_m_str   = fmt(users_daily_value/MILLION, precision=1, allow_zero=True, commas=False)
    recs_per_user_str   = fmt(recs_per_user_value, precision=1, allow_zero=True, commas=False)
    inference_ms_str    = fmt(inference_ms_value, precision=1, allow_zero=True, commas=False)
...
class CarbonFactor:
    ...
    class DummyFleet:
        node = type('obj', (object,), {'accelerator': Hardware.Cloud.A100})
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpu_power_w_str = fmt(Hardware.Cloud.A100.tdp.m_as(watt), precision=1, allow_zero=True, commas=False)
...
class TCOCalc:
    ...
    t_subtotal_str = fmt(train_cost_cycle, precision=1, allow_zero=True, commas=True)
    t_total_str = fmt(train_tco_3yr, precision=1, allow_zero=True, commas=True)
    t_total_k_str = fmt(train_tco_3yr/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    t_cycles_str = fmt(t_cycles_3yr, precision=1, allow_zero=True, commas=False)

    # Training carbon strings — custom " kg" / " tons" suffix → MarkdownStr per recipe Rule 1 escape-hatch
    t_data_prep_carbon_str = fmt(t_data_prep_hrs * carbon_per_gpu_hr, precision=1, allow_zero=True, commas=False, suffix=" kg")
    t_hparam_carbon_str = fmt((t_hparam_exps * t_hparam_cost_exp/train_gpu_rate) * carbon_per_gpu_hr, precision=1, allow_zero=True, commas=False, suffix=" kg")
    t_final_carbon_str = fmt(t_final_hrs * carbon_per_gpu_hr, precision=1, allow_zero=True, commas=False, suffix=" kg")
    t_subtotal_carbon_str = fmt(train_carbon_cycle, precision=1, allow_zero=True, commas=False, suffix=" kg")
    t_total_carbon_str = fmt(train_carbon_3yr, precision=1, allow_zero=True, suffix=" kg")
    ...
    i_gpu_hr_day_str = fmt(inf_gpu_hours_day, precision=1, allow_zero=True, commas=False, suffix=" GPU-hr")
    i_daily_carbon_str = fmt(inf_carbon_day, precision=1, allow_zero=True, commas=False, suffix=" kg")
    ...
    i_annual_carbon_str = fmt(inf_carbon_3yr/3, precision=1, allow_zero=True, suffix=" kg")
    i_total_str = fmt(inf_tco_3yr/MILLION, precision=2, commas=False, prefix="$", suffix="M")
    i_carbon_str = fmt(inf_carbon_3yr, precision=1, allow_zero=True, suffix=" kg")
    ...
    i_daily_queries_m_str = fmt(inf_daily_total / MILLION, precision=1, allow_zero=True, commas=False)
    i_latency_ms_str = fmt(i_latency.m_as(ms), precision=1, allow_zero=True, commas=False)
    i_annual_cost_k_str = fmt(inf_cost_day * DAYS_PER_YEAR / THOUSAND, precision=1, allow_zero=True, commas=False)
    ...
    o_monitor_annual_str = fmt(o_monitor_yr/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    o_monitor_str = fmt(o_monitor_yr*3/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    o_oncall_annual_str = fmt(o_oncall_yr/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    o_oncall_str = fmt(o_oncall_yr*3/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    o_incident_annual_str = fmt(o_incident_yr/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    o_incident_str = fmt(o_incident_yr*3/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    o_total_str = fmt(o_total_3yr/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    o_monitor_annual_k_str = fmt(o_monitor_yr / THOUSAND, precision=1, allow_zero=True, commas=False)
    o_oncall_annual_k_str = fmt(o_oncall_yr / THOUSAND, precision=1, allow_zero=True, commas=False)
    o_incident_annual_k_str = fmt(o_incident_yr / THOUSAND, precision=1, allow_zero=True, commas=False)
    o_total_k_str = fmt(o_total_3yr / THOUSAND, precision=1, allow_zero=True, commas=False)
    ...
    total_carbon_tons_str = fmt(total_carbon_kg/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="~", suffix=" tons")  # kg → metric ton; mlsysim mass pint units pending
    p_train_str = fmt(p_train, precision=1, allow_zero=True)
    p_inf_str = fmt(p_inf, precision=1, allow_zero=True)
    p_ops_str = fmt(p_ops, precision=1, allow_zero=True)
    train_gpu_rate_str = fmt(train_gpu_rate, precision=1, allow_zero=True, commas=False)
...
: **Training Cost Calculation**: Training costs accumulate through initial development (USD `{python} TCOCalc.t_subtotal_str` per cycle) and quarterly retraining over a three-year operational period. Data preparation, hyperparameter search, and final training consume GPU hours at USD `{python} TCOCalc.train_gpu_rate_str`/hour, totaling USD `{python} TCOCalc.t_total_str` across `{python} TCOCalc.t_cycles_str` training cycles. Despite appearing substantial, training represents only `{python} TCOCalc.p_train_str` percent of total cost of ownership. {#tbl-tco-training tbl-colwidths="[25,32,23,20]"}
...
: **Operational Cost Calculation**: Operational costs include monitoring infrastructure (USD `{python} TCOCalc.o_monitor_annual_k_str`K/year), on-call engineering at 0.5 FTE (USD `{python} TCOCalc.o_oncall_annual_k_str`K/year), and incident response reserves (USD `{python} TCOCalc.o_incident_annual_k_str`K/year). The USD `{python} TCOCalc.o_total_k_str`K three-year total represents `{python} TCOCalc.p_ops_str` percent of TCO and often surprises teams focused primarily on compute costs. These estimates represent minimum staffing; production systems at this scale typically require 2--5$\times$ more engineering support. These expenses persist regardless of model performance and grow with system complexity. {#tbl-tco-operations tbl-colwidths="[34,33,33]"}
...
class TCOSummary:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    inf_train_ratio_str = fmt(inf_train_ratio, precision=1, allow_zero=True, commas=False)
    carbon_train_ratio_str = fmt(carbon_train_ratio, precision=1, allow_zero=True, commas=False)
    quant_savings_str = fmt(savings_dollars / THOUSAND, precision=1, allow_zero=True, commas=False) # In K$
    quant_carbon_str = fmt(savings_carbon_kg / THOUSAND, precision=1, allow_zero=True, commas=False) # In metric tons; kg -> metric ton, mlsysim mass pint units pending
    quant_reduction_pct_str = fmt(int(quant_reduction_pct * 100), precision=1, allow_zero=True, commas=False)
```

The stark breakdown in @tbl-tco-summary answers where the money goes: inference at `{python} TCOCalc.p_inf_str` percent, operations at `{python} TCOCalc.p_ops_str` percent, and training at only `{python} TCOCalc.p_train_str` percent.
...
: **Total Cost of Ownership Summary**: Three-year TCO breakdown: training, inference, and operations costs. The `{python} TCOSummary.inf_train_ratio_str`:1 ratio between inference and training costs is typical for production systems serving millions of daily users. A `{python} TCOSummary.quant_reduction_pct_str` percent reduction in inference latency through quantization saves about USD `{python} TCOSummary.quant_savings_str`K and `{python} TCOSummary.quant_carbon_str` metric tons of CO~2~, easily justifying the optimization engineering investment. {#tbl-tco-summary tbl-colwidths="[25,25,25,25]"}
...
- [ ] **Inference dominance**: Can you explain why a `{python} TCOSummary.quant_reduction_pct_str` percent inference latency reduction delivers more savings than a 50 percent training time reduction for a production system serving millions of users?
...
class CarbonScaleCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    train_energy_mwh_str  = fmt(train_energy_mwh, precision=1, allow_zero=True, commas=True)
    train_energy_kwh_str  = fmt(train_energy_kwh, precision=1, allow_zero=True, commas=True)
    carbon_intensity_g_kwh_str = fmt(carbon_intensity_g_kwh, precision=1, allow_zero=True, commas=False)
    carbon_intensity_str  = fmt(carbon_intensity_kg_kwh, precision=3, commas=False)
    total_emissions_kg_str   = fmt(total_emissions_kg, precision=1, allow_zero=True, commas=True)
    total_emissions_tons_str = fmt(total_emissions_tons, precision=1, allow_zero=True, commas=False)
    cars_eq_str           = fmt(cars_equivalent, precision=1, allow_zero=True, commas=False)
...
class MetaGovernanceFine:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    facebook_fine_str = fmt(facebook_fine_eur / MILLION, precision=1, allow_zero=True, commas=False)
    instagram_fine_str = fmt(instagram_fine_eur / MILLION, precision=1, allow_zero=True, commas=False)
    total_fine_str = fmt(total_fine_eur / MILLION, precision=1, allow_zero=True, commas=False)
```
...
class FairnessPitfallAnchor:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tpr_disparity_str = fmt(tpr_disparity, precision=1, allow_zero=True, commas=False)
    fnr_ratio_str = fmt(fnr_ratio, precision=1, allow_zero=True, commas=False)
```
...
The key insight unifying these tools is that translating responsibility concerns into measurable properties makes them tractable.\index{Measurable Properties!responsibility requirements} "Fairness gap <5 percent across groups" is actionable; "be fair" is not. This translation extends beyond fairness: efficiency becomes carbon accounting and TCO analysis, where a `{python} TCOSummary.quant_reduction_pct_str` percent latency reduction through quantization saves USD `{python} TCOSummary.quant_savings_str`K and eliminates `{python} TCOSummary.quant_carbon_str` metric tons of CO~2~. Documentation becomes model cards with explicit intended use and known limitations. Governance becomes access control, lineage tracking, and audit infrastructure that makes compliance demonstrable rather than aspirational. At every level, the same pattern holds: abstract ethical obligations become concrete engineering requirements that can be specified, tested, monitored, and enforced.
