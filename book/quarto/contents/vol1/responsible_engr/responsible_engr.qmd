...
**The context**: Researchers trained deep learning models for pneumonia detection across chest X-ray datasets from multiple hospitals.

**The failure**: Models that appeared strong on internal data performed worse on external hospitals. The problem was not simply architecture quality: hospital-specific prevalence, acquisition patterns, and dataset artifacts created shortcuts that did not transfer cleanly across sites.

**The consequence**: A model could appear clinically useful in one hospital's data while failing when moved to another institution. That is precisely the dangerous form of shortcut learning: the model has learned the data-generating process, not necessarily the disease process.

**The systems lesson**: Neural networks exploit the easiest statistical signal available. External validation and interpretability tools (saliency maps\index{Saliency Maps!interpretability tool}) are quality-assurance gates, not presentation polish [@zech2018variable; @lapuschkin2019].
