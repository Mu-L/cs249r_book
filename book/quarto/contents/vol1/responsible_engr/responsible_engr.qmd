...
1.  Closing the 20 percent TPR gap requires accepting a **`{python} FairnessPrice.fp_increase_str` increase** in False Positives for the disadvantaged group.
...
class CarbonFactor:
    ...
    class DummyRegion:
        name = "Baseline Grid"
        carbon_intensity_g_kwh = 400.0 # 0.4 kg/kWh
        wue = 0.0
    ...
    res = SustainabilityModel().solve(
        fleet=DummyFleet(),
        datacenter=DummyRegion(),
        duration_days=1/24.0,
        mfu=1.0 # peak analysis
    )

    gpu_power_kw = Hardware.Cloud.A100.tdp.m_as("kilowatt")
    carbon_per_gpu_hr_kg = res.it_energy_kwh.magnitude * (DummyRegion.carbon_intensity_g_kwh / 1000.0)
    carbon_intensity_kg_kwh = DummyRegion.carbon_intensity_g_kwh / 1000.0
...
*   **Conversion factor**: (`{python} CarbonFactor.gpu_power_kw_str` kW $\times$ 1 hour) $\times$ `{python} CarbonFactor.carbon_intensity_str` kg/kWh = **`{python} CarbonFactor.carbon_per_gpu_hr_str` kg CO~2~eq per GPU-hour**.
...
- [ ] **Inference dominance**: Can you explain why a `{python} TCOSummaryTable.quant_reduction_pct_str` percent inference latency reduction delivers more savings than a 50 percent training time reduction for a production system serving millions of users?
