...
Scaling laws---Kaplan, Chinchilla, and their successors---predict model quality as a function of algorithmic training compute. They usually abstract away systems efficiency: wall-clock time, accelerator peak FLOPS, MFU, communication overhead, and scheduler losses enter later when teams provision hardware to deliver the target compute budget. In C$^3$ terms, scaling-law FLOPs must be converted into raw fleet capacity after MFU, communication, and goodput losses.
...
class EffectiveFLOPS:
    """Display math for the Effective FLOPS callout."""
    ...
    effective_eq_math = MarkdownStr(
        f"$$\\text{{Effective}} = {C3.peak_pflops_str} \\times {_mfu_fmt} \\times {_scaling_fmt} \\times {_goodput_fmt} \\approx {C3.effective_pflops_str} \\text{{ PFLOPS}}$$"
    )
```

The **Effective FLOPS** delivered by a fleet compounds three independent C$^3$ losses:
...
::: {.callout-perspective title="The C³ tax on a 100,000-GPU cluster"}

Consider a 100,000-GPU H100 cluster with `{python} C3.peak_pflops_str` PFLOPS of peak aggregate throughput. After the three C$^3$ losses:
...
- **Measure the C$^3$ Scorecard** (MFU $>$ `{python} C3.mfu_high_str` percent, Scaling Efficiency $>$ `{python} C3.eff_256_str` percent, Goodput Ratio $>$ 90 percent) before investing in optimizations.
- **The C$^3$ tax is multiplicative**: Peak FLOPS$\times$ MFU$\times$ Scaling Efficiency$\times$ Goodput Ratio = Effective FLOPS. At 100,000 GPUs, expect only ~`{python} C3.eff_fraction_pct_str` percent of peak.
...
class Exercise3:
    """Effective FLOPS calculation for Exercise 3 answer."""
    # ┌── 1. LOAD ─────────────────────────────────────────
