...
class C3TrafficLight:
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    mfu_low_str = fmt_percent(Literature.Training.MfuLow, precision=0, commas=False)
    mfu_high_str = fmt_percent(Literature.Training.MfuHigh, precision=0, commas=False)
```
...
class C3CaseStudies:
    ...
    case1_target_mfu_pct_str = fmt_percent(Literature.Training.MfuHigh, precision=0, commas=False)
    ...
    oh_pipeline_str = fmt_percent(Literature.Overheads.PipelineBubble, precision=0, commas=False)
    oh_checkpoint_str = fmt_percent(Literature.Overheads.Checkpoint, precision=0, commas=False)
    oh_failure_str = fmt_percent(Literature.Overheads.FailureRecovery, precision=0, commas=False)
    oh_maintenance_str = fmt_percent(Literature.Overheads.Maintenance, precision=0, commas=False)
```
...
class C3Scorecard:
    ...
    mfu_low_str = fmt_percent(Literature.Training.MfuLow, precision=0, commas=False)
    mfu_high_str = fmt_percent(Literature.Training.MfuHigh, precision=0, commas=False)
    eff_256_str = fmt_percent(Literature.Scaling.Eff256Gpu, precision=0, commas=False)
    eff_8192_str = fmt_percent(Literature.Scaling.Eff8192Gpu, precision=0, commas=False)
    eff_1024_str = fmt_percent(Literature.Scaling.Eff1024Gpu, precision=0, commas=False)
```
...
class C3EffectiveFlops:
    ...
    scaling_pct_str = fmt_percent(Literature.Scaling.Eff8192Gpu, precision=0, commas=False)
...
* **The C$^3$ tax is multiplicative**: Peak FLOP/s$\times$ MFU$\times$ Scaling Efficiency$\times$ Goodput Ratio = Effective FLOP/s. At `{python} C3SummaryRecap.fleet_gpu_count_str` GPUs, expect only ~`{python} C3SummaryRecap.eff_fraction_pct_str` percent of peak.
...
# │
# │ Imports: mlsysim.core.constants (Literature.Training.MfuHigh, Literature.Scaling.Eff1024Gpu), mlsysim.book (fmt, md, fmt_math)
...
class Exercise3:
    ...
    # ┌── 1. LOAD ─────────────────────────────────────────
    _eff_frac_val = Literature.Training.MfuHigh * Literature.Scaling.Eff1024Gpu * 0.85
    ...
    _mfu_high_digits = fmt_int(Literature.Training.MfuHigh * 100, commas=False)
    _eff_1024_digits = fmt_int(Literature.Scaling.Eff1024Gpu * 100, commas=False)
