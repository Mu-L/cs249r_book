...
class C3TrafficLight:
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    mfu_low_str = fmt_percent(defaults.MFU_TRAINING_LOW, precision=0, commas=False)
    mfu_high_str = fmt_percent(defaults.MFU_TRAINING_HIGH, precision=0, commas=False)
```
...
|:------------------|:-------------------------------------------------------|:------------------------------------------------------------------------------------------|:------------------------------------------------------|
| **Computation**   | MFU $>$ `{python} C3TrafficLight.mfu_high_str` percent | MFU `{python} C3TrafficLight.mfu_low_str`--`{python} C3TrafficLight.mfu_high_str` percent | MFU $<$ `{python} C3TrafficLight.mfu_low_str` percent |
...
class C3CaseStudies:
    case1_n_gpus = 4096
    case1_mfu = 0.15
    case1_target_mfu = defaults.MFU_TRAINING_HIGH
    ...
    case1_throughput_ratio = case1_mfu / defaults.MFU_TRAINING_HIGH
    ...
    case3_failure_interval_hours = defaults.GPU_MTTF_HOURS / case3_n_gpus
    ...
    case1_target_mfu_pct_str = fmt_percent(defaults.MFU_TRAINING_HIGH, precision=0, commas=False)
    case1_target_gain_str = fmt(defaults.MFU_TRAINING_HIGH / case1_mfu, precision=1, commas=False)
    ...
    case2_scaling_efficiency_pct_str = fmt_percent(case2_scaling_efficiency, precision=0, commas=False)
    ...
    gpus_per_host_str = fmt(GPUS_PER_HOST, precision=0, commas=False)
    ...
    case3_oh_scheduler_pct_str = fmt_percent(case3_oh_scheduler_pct, precision=0, commas=False)
    oh_pipeline_str = fmt_percent(defaults.OVERHEAD_PIPELINE_BUBBLE, precision=0, commas=False)
    oh_checkpoint_str = fmt_percent(defaults.OVERHEAD_CHECKPOINT, precision=0, commas=False)
    oh_failure_str = fmt_percent(defaults.OVERHEAD_FAILURE_RECOVERY, precision=0, commas=False)
    oh_maintenance_str = fmt_percent(defaults.OVERHEAD_MAINTENANCE, precision=0, commas=False)
```
...
You provision `{python} C3CaseStudies.case1_n_gpus_str` H100 GPUs for a large language model training run. The training loop runs without errors, but the PyTorch Profiler shows MFU of only `{python} C3CaseStudies.case1_mfu_pct_str` percent. The network profiler shows communication accounts for less than 10 percent of step time. The cluster is running but barely working.

#### Diagnosis {.unnumbered}

The **Computation** axis is the bottleneck. With `{python} C3CaseStudies.case1_mfu_pct_str` percent MFU, `{python} C3CaseStudies.case1_wasted_pct_str` percent of the fleet's arithmetic capacity sits idle on every step. This is not a communication or coordination problem---the network is fast enough and the system is stable. The system is not feeding work to the GPUs efficiently.
...
Raising MFU from `{python} C3CaseStudies.case1_mfu_pct_str` percent to `{python} C3CaseStudies.case1_target_mfu_pct_str` percent on the same hardware delivers `{python} C3CaseStudies.case1_target_mfu_pct_str`/`{python} C3CaseStudies.case1_mfu_pct_str` = `{python} C3CaseStudies.case1_target_gain_str`$\times$ more useful work---the equivalent of tripling the fleet without buying a single GPU.

### Case 2: The communication wall (Communication) {#sec-c3-taxonomy-case-2}

#### Symptom {.unnumbered}

A `{python} C3CaseStudies.case2_n_gpus_str`-GPU data-parallel training run achieves `{python} C3CaseStudies.case2_mfu_pct_str` percent MFU on each individual GPU---good per-device efficiency. Yet scaling from `{python} C3CaseStudies.case2_baseline_gpus_str` to `{python} C3CaseStudies.case2_n_gpus_str` GPUs yields only `{python} C3CaseStudies.case2_observed_speedup_str`$\times$ speedup instead of the expected `{python} C3CaseStudies.case2_ideal_speedup_str`$\times$, or `{python} C3CaseStudies.case2_scaling_efficiency_pct_str` percent scaling efficiency. NCCL profiling reveals that AllReduce consumes `{python} C3CaseStudies.case2_comm_pct_str` percent of every training step.
...
If communication were eliminated entirely, throughput would increase by `{python} C3CaseStudies.case2_speedup_str`$\times$ (Amdahl's Law applied to the `{python} C3CaseStudies.case2_compute_pct_str` percent compute fraction). Realistically, reducing communication from `{python} C3CaseStudies.case2_comm_pct_str` percent to 20 percent of step time would recover most of the lost scaling.

### Case 3: The coordination tax (Coordination) {#sec-c3-taxonomy-case-3}

#### Symptom {.unnumbered}

A `{python} C3CaseStudies.case3_n_gpus_str`-GPU training run shows `{python} C3CaseStudies.case3_mfu_pct_str` percent MFU per device and communication accounts for only `{python} C3CaseStudies.case3_comm_pct_str` percent of step time---both healthy. The job's **goodput ratio** (useful training steps/wall-clock time), however, is only `{python} C3CaseStudies.case3_goodput_pct_str` percent. The remaining `{python} C3CaseStudies.case3_coord_pct_str` percent of wall time is consumed by checkpoint writes, failure recovery restarts, pipeline bubble idle time, `{python} C3CaseStudies.case3_oh_scheduler_pct_str` percent scheduler preemptions, and maintenance windows.

#### Diagnosis {.unnumbered}

The **Coordination** axis dominates. Per-GPU computation and inter-node communication are both efficient, but `{python} C3CaseStudies.case3_coord_pct_str` percent of wall time is consumed by nonproductive overhead: `{python} C3CaseStudies.oh_failure_str` percent failure recovery (at `{python} C3CaseStudies.case3_n_gpus_str` GPUs, GPU failures occur about every `{python} C3CaseStudies.case3_failure_interval_hours_str` hours), `{python} C3CaseStudies.oh_pipeline_str` percent pipeline bubbles, `{python} C3CaseStudies.oh_checkpoint_str` percent checkpoint writes, and `{python} C3CaseStudies.oh_maintenance_str` percent maintenance windows. Neither faster GPUs nor faster networks will help---coordination, not computation or communication, consumes the time.

#### The fix {.unnumbered}

Target the Coordination axis:

- **Asynchronous checkpointing**: Overlap checkpoint writes with the next training step, reducing visible checkpoint overhead from `{python} C3CaseStudies.oh_checkpoint_str` percent to near zero.
- **Elastic training**: When a node fails, shrink the job and continue rather than halting all `{python} C3CaseStudies.case3_n_gpus_str` GPUs for recovery. This converts the `{python} C3CaseStudies.oh_failure_str` percent failure recovery cost into a smaller throughput reduction.
...
class C3Scorecard:
    ...
    mfu_low_str = fmt_percent(defaults.MFU_TRAINING_LOW, precision=0, commas=False)
    mfu_high_str = fmt_percent(defaults.MFU_TRAINING_HIGH, precision=0, commas=False)
    eff_256_str = fmt_percent(defaults.SCALING_EFF_256GPU, precision=0, commas=False)
    eff_8192_str = fmt_percent(defaults.SCALING_EFF_8192GPU, precision=0, commas=False)
    eff_1024_str = fmt_percent(defaults.SCALING_EFF_1024GPU, precision=0, commas=False)
```
...
|:------------------|:-------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------|:------------------------------------------------|:------------------------------------------------|
| **Computation**   | **MFU**                                          | $\frac{O_{\text{step}}}{R_{\text{peak}} \times T_{\text{step}}}$                                                     | $<$ `{python} C3Scorecard.mfu_low_str` percent  | $>$ `{python} C3Scorecard.mfu_high_str` percent |
| **Communication** | **Scaling Efficiency** ($\eta_{\text{scaling}}$) | $\frac{T_1}{N \times T_N}$                                                                                           | $<$ `{python} C3Scorecard.eff_8192_str` percent | $>$ `{python} C3Scorecard.eff_256_str` percent  |
...
from mlsysim.fmt import fmt, fmt_percent, MarkdownStr
...
class C3EffectiveFlops:
    ...
    n_gpus_eff = 100_000
    peak_pflops = n_gpus_eff * h100_tflops / 1000
    goodput_all = 1.0 - (
        defaults.OVERHEAD_PIPELINE_BUBBLE + defaults.OVERHEAD_CHECKPOINT
        + defaults.OVERHEAD_FAILURE_RECOVERY + defaults.OVERHEAD_MAINTENANCE + 0.17
    )
    effective_pflops = calc_effective_flops(
        peak_pflops, defaults.MFU_TRAINING_HIGH, defaults.SCALING_EFF_8192GPU, goodput_all
    ).magnitude
    ...
    scaling_pct_str = fmt_percent(defaults.SCALING_EFF_8192GPU, precision=0, commas=False)
    ...
    _scaling_fmt = fmt(defaults.SCALING_EFF_8192GPU, precision=2, commas=False)
...
Consider a 100,000-GPU H100 cluster with `{python} C3EffectiveFlops.peak_pflops_str` PFLOP/s of peak aggregate throughput. After the three C$^3$ losses:

`{python} C3EffectiveFlops.effective_eq_math`

The fleet delivers `{python} C3EffectiveFlops.eff_fraction_pct_str` percent of its peak capacity as useful training work. The **C$^3$ tax**---the ratio of peak to effective---is `{python} C3EffectiveFlops.c3_tax_str`$\times$: achieving a given effective compute budget requires `{python} C3EffectiveFlops.c3_tax_str`$\times$ the raw hardware. Broken down by axis: Computation consumes a `{python} C3EffectiveFlops.mfu_pct_str` percent factor (MFU), Communication consumes a `{python} C3EffectiveFlops.scaling_pct_str` percent factor (using the 8,192-GPU scaling-efficiency reference as an illustrative proxy), and Coordination consumes a `{python} C3EffectiveFlops.goodput_pct_str` percent factor (goodput ratio after pipeline bubbles, checkpoints, failures, scheduler preemptions, and maintenance).
...
class C3SummaryRecap:
    ...
    case3_coord_fraction = 1 - case3_goodput_ratio
    h100_tflops = Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs/second)
    peak_pflops = 100_000 * h100_tflops / 1000
    goodput_all = 1.0 - (
        defaults.OVERHEAD_PIPELINE_BUBBLE + defaults.OVERHEAD_CHECKPOINT
        + defaults.OVERHEAD_FAILURE_RECOVERY + defaults.OVERHEAD_MAINTENANCE + 0.17
    )
...
* **Measure the C$^3$ Scorecard**: Use MFU $>$ `{python} C3SummaryRecap.mfu_high_str` percent, Scaling Efficiency $>$ `{python} C3SummaryRecap.eff_256_str` percent, and Goodput Ratio $>$ 90 percent before investing in optimizations.
* **The C$^3$ tax is multiplicative**: Peak FLOP/s$\times$ MFU$\times$ Scaling Efficiency$\times$ Goodput Ratio = Effective FLOP/s. At 100,000 GPUs, expect only ~`{python} C3SummaryRecap.eff_fraction_pct_str` percent of peak.
* **Coordination is the new axis**: On a single machine, overhead is negligible. At fleet scale, checkpoints, failures, pipeline bubbles, and scheduling consume `{python} C3SummaryRecap.case3_coord_pct_str` percent or more of wall time.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: appendix-c3-exercises-recap
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: ## Exercises — recap case-2 and MFU threshold strings
from mlsysim.fmt import fmt, fmt_percent
from mlsysim.core import defaults

class C3ExercisesRecap:
    case2_n_gpus = 512
    case2_mfu = 0.45
    case2_comm_fraction = 0.55
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
...
A `{python} C3ExercisesRecap.case2_n_gpus_str`-GPU training job shows `{python} C3ExercisesRecap.case2_mfu_pct_str` percent MFU per device, but NCCL logs reveal that AllReduce consumes `{python} C3ExercisesRecap.case2_comm_pct_str` percent of each training step. Which C$^3$ axis is the bottleneck? Name two specific optimizations and explain why each targets the correct axis.

**Answer**: The bottleneck is **Communication** ($C_2$). Per-GPU MFU of `{python} C3ExercisesRecap.case2_mfu_pct_str` percent is healthy (above the `{python} C3ExercisesRecap.mfu_low_str` percent threshold), so Computation is not the problem. The `{python} C3ExercisesRecap.case2_comm_pct_str` percent communication fraction far exceeds the 40 percent red-line threshold. Two optimizations: (1) **Gradient compression** (for example, TopK sparsification) directly reduces the bytes crossing the network, shrinking $T_{\text{comm}}(N)$ while leaving $T_{\text{compute}}/N$ unchanged. (2) **Compute-communication overlap** launches the AllReduce during the backward pass rather than waiting until it completes, converting the sequential sum of compute and communication into the overlapped $\max(T_{\text{compute}}/N, T_{\text{comm}}(N))$ and increasing $T_{\text{overlap}}$---both target the Communication axis.
...
class Exercise2:
    ...
    comm_pct_str = fmt_percent(comm_fraction, precision=0, commas=False)
    coord_pct_str = fmt_percent(coord_fraction, precision=0, commas=False)
...
# │
# │ Imports: mlsysim.core.constants (defaults.MFU_TRAINING_HIGH, defaults.SCALING_EFF_1024GPU), mlsysim.book (fmt, md, fmt_math)
# │ Exports: Exercise3.effective_frac_math, Exercise3.c3_tax_math, Exercise3.provision_math
# └─────────────────────────────────────────────────────────────────────────────

from mlsysim.fmt import fmt, fmt_percent, fmt_int, fmt_math, MarkdownStr
from mlsysim.core import defaults

class Exercise3:
    """Effective FLOP/s calculation for Exercise 3 answer."""
    # ┌── 1. LOAD ─────────────────────────────────────────
    _eff_frac_val = defaults.MFU_TRAINING_HIGH * defaults.SCALING_EFF_1024GPU * 0.85
    ...
    _mfu_high_digits = fmt_int(defaults.MFU_TRAINING_HIGH * 100, commas=False)
    _eff_1024_digits = fmt_int(defaults.SCALING_EFF_1024GPU * 100, commas=False)
    effective_frac_math = MarkdownStr(
        f"$$\\text{{Effective fraction}} = 0.{_mfu_high_digits} \\times 0.{_eff_1024_digits} \\times 0.85 "
        f"= {_eff_frac_pct_str}\\%$$"
    )
...
A team provisions 2,048 H100 GPUs. The cluster achieves `{python} C3ExercisesRecap.mfu_high_str` percent MFU, `{python} C3ExercisesRecap.eff_1024_str` percent scaling efficiency, and 85 percent goodput ratio. Calculate the effective FLOP/s as a fraction of peak FLOP/s. If a scaling law predicts that $10^{24}$ FLOPs of training compute will reach a target loss, how many raw peak-rate FLOPs must be provisioned to account for the C$^3$ tax?
...
1. **"What is the current MFU?"**—*Computation ($C_1$)*. If MFU is below `{python} C3ExercisesRecap.mfu_low_str` percent, the GPUs themselves are underutilized. Faster interconnects cannot help if the GPUs are not doing useful work to begin with. Fix kernel efficiency first.
