...
from mlsysim.fmt import fmt, fmt_percent, check, fmt_math
...
class CommFoundations:
    ...
    f = Hardware.Networks
    ib_ndr = f.InfiniBand_NDR
    ib_hdr = f.InfiniBand_HDR
    roce   = f.Ethernet_100G
    tcp    = f.Ethernet_10G # Simplified for TCP comparison

    ib_ndr_alpha_us = IB_NDR_LATENCY_US
    ib_ndr_beta_gbs = ib_ndr.bandwidth.m_as(GB/second)
    ib_hdr_alpha_us = IB_HDR_LATENCY_US
    ib_hdr_beta_gbs = ib_hdr.bandwidth.m_as(GB/second)
    roce_alpha_us = roce_latency_us = ROCE_LATENCY_US
    ...
    breakeven_times_ms = (10, 40, 100)
    breakeven_ratios = (4, 16, 64, 256)
    ...
    ring_time_s = calc_ring_allreduce_time(ring_msg_bytes, ring_n_gpus, ib_ndr.bandwidth, ib_ndr_alpha_us * ureg.microsecond)
    tree_time_s = calc_tree_allreduce_time(ring_msg_bytes, ring_n_gpus, ib_ndr.bandwidth, ib_ndr_alpha_us * ureg.microsecond)
    ring_time_ms = ring_time_s.m_as("ms")
    tree_time_ms = tree_time_s.m_as("ms")
    ...
    ib_ndr_alpha_us_str = fmt(int(ib_ndr_alpha_us), precision=1, allow_zero=True, commas=False)
    ib_ndr_beta_gbs_str = fmt(int(ib_ndr_beta_gbs), precision=1, allow_zero=True, commas=False)
    ib_hdr_alpha_us_str = fmt(int(ib_hdr_alpha_us), precision=1, allow_zero=True, commas=False)
    ib_hdr_beta_gbs_str = fmt(int(ib_hdr_beta_gbs), precision=1, allow_zero=True, commas=False)
    roce_alpha_us_str = fmt(int(roce_alpha_us), precision=1, allow_zero=True, commas=False)
    roce_beta_gbs_str = fmt(roce_beta_gbs, precision=1, commas=False)
    tcp_alpha_us_str = fmt(int(tcp_alpha_us), precision=1, allow_zero=True, commas=False)
    nvlink_bw_str = fmt(nvlink_h100_bw_gbs, precision=1, allow_zero=True, commas=False)
    pcie_bw_str = fmt(pcie_gen5_bw_gbs, precision=1, allow_zero=True, commas=False)
    nvlink_lat_str = fmt(nvlink_latency_ns, precision=1, allow_zero=True, commas=False)
    pcie_lat_str = fmt(pcie_latency_ns, precision=1, allow_zero=True, commas=False)
    crossover_kb_str = fmt(round(crossover_kb_val), precision=1, allow_zero=True, commas=False)
    ring_n_str = fmt(ring_n_gpus, precision=1, allow_zero=True, commas=False)
    ring_time_ms_str = fmt(ring_time_ms, precision=1, commas=False)
    tree_time_ms_str = fmt(tree_time_ms, precision=1, commas=False)
    ring_msg_gb_str = fmt((ring_msg_bytes * byte).m_as(GB), precision=1, allow_zero=True, commas=False)
    ...
    compress_ratio_str = fmt(int(compress_ratio), precision=1, allow_zero=True, commas=False)
    ...
    nvlink_to_ndr_ratio_str = fmt(nvlink_to_ndr_ratio, precision=1, allow_zero=True, commas=False)

    breakeven_4_10_str = fmt(breakeven_ms(10, 4), precision=1, commas=False)
    breakeven_4_40_str = fmt(breakeven_ms(40, 4), precision=1, allow_zero=True, commas=False)
    breakeven_4_100_str = fmt(breakeven_ms(100, 4), precision=1, allow_zero=True, commas=False)
...
$n^* = \alpha \times \beta =$ `{python} CF.ib_ndr_alpha_us_str` $\times 10^{-6}$ s $\times$ `{python} CF.ib_ndr_beta_gbs_str` $\times 10^{9}$ B/s $=$ `{python} CF.crossover_kb_str` KB
...
**Systems insight**: This is why NCCL and Gloo batch small gradient tensors into larger "buckets" before launching AllReduce. Sending each tensor individually would pay the `{python} CF.ib_ndr_alpha_us_str` $\mu$s startup cost hundreds of times per iteration instead of a handful.
...
1. **Intra-node reduce** (NVLink): Each node reduces its `{python} CF.gpus_per_host_str` local GPUs to a single result using NVLink's `{python} CF.nvlink_bw_str` GB/s bandwidth. This is fast because NVLink is `{python} CF.nvlink_to_ndr_ratio_str`$\times$ faster than InfiniBand NDR.
2. **Inter-node AllReduce** (InfiniBand): The per-node results are all-reduced across nodes. Only one representative per `{python} CF.gpus_per_host_str` local GPUs participates, so the inter-node ring has one-eighth as many participants as a flat GPU-level ring.
3. **Intra-node broadcast** (NVLink): Each node broadcasts the global result to its `{python} CF.gpus_per_host_str` local GPUs.

The hierarchical approach reduces the inter-node ring size from all GPUs to one representative per `{python} CF.gpus_per_host_str` local GPUs, cutting the latency-step term by about `{python} CF.gpus_per_host_str`$\times$ and confining the bandwidth-hungry phases to the fast intra-node links. NCCL selects among topology-aware algorithms such as Ring, Tree, CollNet variants, NVLS/NVLSTree, and PAT based on the collective, message size, topology, architecture, and version; hierarchical communication is common on multi-node systems but is not a universal default.
...
|:--------------------------|:------------------------------------|:-------------------------------------|:-------------------------------------|:-------------------------------------|
| **$p$ = 4**               | `{python} CF.bubble_p4_m8_pct_str`% | `{python} CF.bubble_p4_m16_pct_str`% | `{python} CF.bubble_p4_m32_pct_str`% | `{python} CF.bubble_p4_m64_pct_str`% |
| **$p$ = 8**               | `{python} CF.bubble_p8_m8_pct_str`% | `{python} CF.bubble_p8_m16_pct_str`% | `{python} CF.bubble_p8_m32_pct_str`% | `{python} CF.bubble_p8_m64_pct_str`% |
...
The microbatch term in the denominator is multiplied by $V$ compared to @eq-pipeline-bubble, so the bubble reduction approaches $V$ when $m$ dominates $p - 1$. For $p$ = 8, $m$ = 16, and $V$ = 4, the bubble fraction drops from `{python} CF.bubble_std_pct_str` percent to `{python} CF.bubble_interleaved_pct_str` percent, a `{python} CF.interleaved_bubble_reduction_str`$\times$ reduction that comes at the cost of more frequent, smaller communication between stages.
...
|:----------------------------|:-------------------------------------------|:-------------------------------------------|:--------------------------------------------|
| **4$\times$**               | `{python} CF.breakeven_4_10_str` ms        | `{python} CF.breakeven_4_40_str` ms        | `{python} CF.breakeven_4_100_str` ms        |
| **16$\times$**              | `{python} CF.breakeven_16_10_str` ms       | `{python} CF.breakeven_16_40_str` ms       | `{python} CF.breakeven_16_100_str` ms       |
| **64$\times$**              | `{python} CF.breakeven_64_10_str` ms       | `{python} CF.breakeven_64_40_str` ms       | `{python} CF.breakeven_64_100_str` ms       |
| **256$\times$**             | `{python} CF.breakeven_256_10_str` ms      | `{python} CF.breakeven_256_40_str` ms      | `{python} CF.breakeven_256_100_str` ms      |
