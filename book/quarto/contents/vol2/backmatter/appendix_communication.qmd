...
**Interpretation**: For this `{python} CommCollectives.ring_msg_gb_str` GB message, ring AllReduce takes `{python} CommCollectives.ring_time_ms_str` ms while tree takes `{python} CommCollectives.tree_time_ms_str` ms---the ring is significantly faster because it distributes the bandwidth load across all links. The tree's logarithmic latency advantage is negligible compared to its bandwidth penalty for large messages.
