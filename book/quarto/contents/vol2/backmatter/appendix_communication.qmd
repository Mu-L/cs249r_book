...
class CommCollectives:
    ...
    ring_msg_params_m_str = fmt_int(
        int((ring_msg_bytes * byte).m_as(GB) * BILLION / 2 / MILLION),
        commas=False,
    )
...
**Setup**: `{python} CommCollectives.ring_n_str` H100 GPUs connected via InfiniBand NDR ($\alpha$ = `{python} CommCollectives.ib_ndr_alpha_us_str` $\mu$s, $\beta$ = `{python} CommCollectives.ib_ndr_beta_gbs_str` GB/s). The gradient tensor is `{python} CommCollectives.ring_msg_gb_str` GB (a `{python} CommCollectives.ring_msg_params_m_str`M-parameter model in FP16).
...
class CommCompression:
    ...
    ring_time_ms = calc_ring_allreduce_time(
        ring_msg_bytes, ring_n_gpus, ib_ndr.bandwidth, Systems.Fabrics.InfiniBand_NDR.latency_us * ureg.microsecond
    ).m_as("ms")
