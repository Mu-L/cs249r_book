...
class CommFoundations:
    ...
    ib_ndr_alpha_us_str = fmt(int(ib_ndr_alpha_us), precision=0, commas=False)
    ib_ndr_beta_gbs_str = fmt(int(ib_ndr_beta_gbs), precision=0, commas=False)
    ib_hdr_alpha_us_str = fmt(int(ib_hdr_alpha_us), precision=0, commas=False)
    ib_hdr_beta_gbs_str = fmt(int(ib_hdr_beta_gbs), precision=0, commas=False)
    roce_alpha_us_str = fmt(int(roce_alpha_us), precision=0, commas=False)
    roce_beta_gbs_str = fmt(roce_beta_gbs, precision=1, commas=False)
    tcp_alpha_us_str = fmt(int(tcp_alpha_us), precision=0, commas=False)
    ...
    crossover_kb_str = fmt(round(crossover_kb_val), precision=0, commas=False)
    ...
    compress_ratio_str = fmt(int(compress_ratio), precision=0, commas=False)
...
* **The $\alpha$-$\beta$ model**: The model ($T = \alpha + M/\beta$) decomposes every network transfer into a fixed startup cost and a payload-proportional bandwidth cost. The crossover message size $M_{\text{cross}} = \alpha \times \beta$ determines which cost dominates.
...
* **Algorithm selection**: Message size relative to $M_{\text{cross}}$ determines the algorithm: ring for large messages (gradients), tree for small messages (barriers), and hierarchical two-level schemes for multi-node clusters.
