...
from mlsysim.core.constants import (
    defaults.IB_NDR_LATENCY_US, defaults.IB_HDR_LATENCY_US, defaults.ROCE_LATENCY_US, defaults.TCP_LATENCY_US,
    Hardware.Cloud.H100.nvlink.bandwidth, Hardware.Cloud.H100.interconnect.bandwidth, LATENCY_NVLINK, LATENCY_PCIE_GEN5,
    GB, KB, NS, ureg,
)
...
class CommAlphaBeta:
    ...
    roce = Hardware.Networks.Ethernet_100G

    ib_ndr_alpha_us = defaults.IB_NDR_LATENCY_US
    ib_ndr_beta_gbs = int(ib_ndr.bandwidth.m_as(GB/second))
    ib_hdr_alpha_us = defaults.IB_HDR_LATENCY_US
    ib_hdr_beta_gbs = int(ib_hdr.bandwidth.m_as(GB/second))
    roce_alpha_us = defaults.ROCE_LATENCY_US
    roce_beta_gbs = roce.bandwidth.m_as(GB/second)
    tcp_alpha_us = defaults.TCP_LATENCY_US
    ...
    ib_ndr_beta_gbs_str = fmt_int(ib_ndr_beta_gbs, commas=False)
    ib_hdr_alpha_us_str = fmt_int(ib_hdr_alpha_us, commas=False)
    ib_hdr_beta_gbs_str = fmt_int(ib_hdr_beta_gbs, commas=False)
    ...
    nvlink_bw_str = fmt(nvlink_h100_bw_gbs, precision=0, commas=False)
    pcie_bw_str = fmt(pcie_gen5_bw_gbs, precision=0, commas=False)
    nvlink_lat_str = fmt(nvlink_latency_ns, precision=0, commas=False)
    pcie_lat_str = fmt(pcie_latency_ns, precision=0, commas=False)
    crossover_kb_str = fmt_int(crossover_kb_val, commas=False)
...
The model decomposes every transfer into exactly two costs: a per-message tax ($\alpha$) paid regardless of size, and a per-byte tax ($n/\beta$) that scales linearly with the payload. This decomposition drives all the algorithm selection decisions in @sec-comm-foundations-algorithm-selection.
...
|:------------------------------|:----------------------------------------------------|:--------------------------------------------------|:---------------------------------|
| **NVLink 4.0 (H100)**         | `{python} CommAlphaBeta.nvlink_lat_str` ns          | `{python} CommAlphaBeta.nvlink_bw_str` GB/s       | Intra-node GPU-to-GPU            |
| **PCIe Gen5**                 | `{python} CommAlphaBeta.pcie_lat_str` ns            | `{python} CommAlphaBeta.pcie_bw_str` GB/s         | CPU-GPU, NIC-GPU                 |
| **InfiniBand NDR (400 Gbps)** | `{python} CommAlphaBeta.ib_ndr_alpha_us_str` $\mu$s | `{python} CommAlphaBeta.ib_ndr_beta_gbs_str` GB/s | Inter-node (high-end clusters)   |
| **InfiniBand HDR (200 Gbps)** | `{python} CommAlphaBeta.ib_hdr_alpha_us_str` $\mu$s | `{python} CommAlphaBeta.ib_hdr_beta_gbs_str` GB/s | Inter-node (previous generation) |
| **RoCE v2 (100 GbE)**         | `{python} CommAlphaBeta.roce_alpha_us_str` $\mu$s   | `{python} CommAlphaBeta.roce_beta_gbs_str` GB/s   | Inter-node (Ethernet clusters)   |
...
**Setup**: InfiniBand NDR has $\alpha$ = `{python} CommAlphaBeta.ib_ndr_alpha_us_str` $\mu$s and $\beta$ = `{python} CommAlphaBeta.ib_ndr_beta_gbs_str` GB/s.

**Question**: At what message size does the bandwidth term equal the latency term?

**Math**:

$n^* = \alpha \times \beta =$ `{python} CommAlphaBeta.ib_ndr_alpha_us_str` $\times 10^{-6}$ s $\times$ `{python} CommAlphaBeta.ib_ndr_beta_gbs_str` $\times 10^{9}$ B/s $=$ `{python} CommAlphaBeta.crossover_kb_str` KB

**Interpretation**: Messages smaller than `{python} CommAlphaBeta.crossover_kb_str` KB are latency-dominated on IB NDR. Gradient tensors from a single layer of a large model are typically 10--100 MB---well above this threshold. Control messages, heartbeats, and barrier synchronizations, however, are well below it.
...
from mlsysim.fmt import fmt_int, fmt, check
from mlsysim.core.constants import defaults.IB_NDR_LATENCY_US, GB, byte, ureg
from mlsysim.core.formulas import calc_ring_allreduce_time, calc_tree_allreduce_time
...
class CommCollectives:
    ...
    ring_time_ms = calc_ring_allreduce_time(
        ring_msg_bytes, ring_n_gpus, ib_ndr.bandwidth, ib_ndr_alpha_us * ureg.microsecond
    ).m_as("ms")
    ...
    ib_ndr_alpha_us_str = fmt_int(ib_ndr_alpha_us, commas=False)
    ib_ndr_beta_gbs_str = fmt_int(ib_ndr_beta_gbs, commas=False)
    ring_msg_gb_str = fmt((ring_msg_bytes * byte).m_as(GB), precision=0, commas=False)
...
**Setup**: `{python} CommCollectives.ring_n_str` H100 GPUs connected via InfiniBand NDR ($\alpha$ = `{python} CommCollectives.ib_ndr_alpha_us_str` $\mu$s, $\beta$ = `{python} CommCollectives.ib_ndr_beta_gbs_str` GB/s). The gradient tensor is `{python} CommCollectives.ring_msg_gb_str` GB (a 500M-parameter model in FP16).

**Question**: *How* long does AllReduce take with ring vs. tree?

**Ring AllReduce**:

$T_{\text{ring}} \approx$ **`{python} CommCollectives.ring_time_ms_str` ms**

**Tree AllReduce**:

$T_{\text{tree}} \approx$ **`{python} CommCollectives.tree_time_ms_str` ms**

**Interpretation**: For this 1 GB message, ring AllReduce takes `{python} CommCollectives.ring_time_ms_str` ms while tree takes `{python} CommCollectives.tree_time_ms_str` ms---the ring is significantly faster because it distributes the bandwidth load across all links. The tree's logarithmic latency advantage is negligible compared to its bandwidth penalty for large messages.
...
from mlsysim.core.constants import Hardware.Cloud.H100.nvlink.bandwidth, GB, GPUS_PER_HOST

class CommAlgorithmSelection:
    from mlsysim import Hardware
    ib_ndr_beta_gbs = int(Hardware.Networks.InfiniBand_NDR.bandwidth.m_as(GB/second))
    nvlink_h100_bw_gbs = int(Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/second))
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    nvlink_bw_str = fmt(nvlink_h100_bw_gbs, precision=0, commas=False)
    ib_ndr_beta_gbs_str = fmt(ib_ndr_beta_gbs, precision=0, commas=False)
...
Modern clusters have a two-level topology: GPUs within a node are connected by NVLink (`{python} CommAlgorithmSelection.nvlink_bw_str` GB/s), while nodes are connected by InfiniBand (`{python} CommAlgorithmSelection.ib_ndr_beta_gbs_str` GB/s). Hierarchical AllReduce exploits this asymmetry in three phases:

1. **Intra-node reduce** (NVLink): Each node reduces its `{python} CommAlgorithmSelection.gpus_per_host_str` local GPUs to a single result using NVLink's `{python} CommAlgorithmSelection.nvlink_bw_str` GB/s bandwidth. This is fast because NVLink is `{python} CommAlgorithmSelection.nvlink_to_ndr_ratio_str`$\times$ faster than InfiniBand NDR.
...
class CommPipeline:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    bubble_std_pct_str = fmt_percent(bubble_std, precision=1)
    bubble_interleaved_pct_str = fmt_percent(bubble_interleaved, precision=1)
    interleaved_bubble_reduction_str = fmt(interleaved_bubble_reduction, precision=1, commas=False)
    bubble_p4_m8_pct_str = fmt_percent(bubble_p4_m8, precision=1)
    bubble_p4_m16_pct_str = fmt_percent(bubble_p4_m16, precision=1)
    bubble_p4_m32_pct_str = fmt_percent(bubble_p4_m32, precision=1)
    bubble_p4_m64_pct_str = fmt_percent(bubble_p4_m64, precision=1)
    bubble_p8_m8_pct_str = fmt_percent(bubble_p8_m8, precision=1)
    bubble_p8_m16_pct_str = fmt_percent(bubble_p8_m16, precision=1)
    bubble_p8_m32_pct_str = fmt_percent(bubble_p8_m32, precision=1)
    bubble_p8_m64_pct_str = fmt_percent(bubble_p8_m64, precision=1)
```
...
class CommCompression:
    ...
    ring_time_ms = calc_ring_allreduce_time(
        ring_msg_bytes, ring_n_gpus, ib_ndr.bandwidth, defaults.IB_NDR_LATENCY_US * ureg.microsecond
    ).m_as("ms")
    ...
    compress_overhead_ms_str = fmt(compress_overhead_ms, precision=0, commas=False)
...
**Setup**: Ring AllReduce of a `{python} CommCompression.ring_msg_gb_str` GB gradient across `{python} CommCompression.ring_n_str` H100 GPUs on InfiniBand NDR, taking `{python} CommCompression.uncompressed_ms_str` ms uncompressed. A Top-k sparsification scheme achieves `{python} CommCompression.compress_ratio_str`$\times$ compression with `{python} CommCompression.compress_overhead_ms_str` ms total encode + decode overhead.

**Question**: Does compression reduce the total AllReduce time?

**Math**:

- Uncompressed AllReduce: `{python} CommCompression.uncompressed_ms_str` ms
- Compressed communication: `{python} CommCompression.compressed_comm_ms_str` ms
- Total with compression: `{python} CommCompression.compressed_total_ms_str` ms
- Speedup: `{python} CommCompression.compress_speedup_str`$\times$

**Interpretation**: Compression delivers a `{python} CommCompression.compress_speedup_str`$\times$ speedup in communication time. The `{python} CommCompression.compress_overhead_ms_str` ms codec overhead is a small fraction of the `{python} CommCompression.uncompressed_ms_str` ms saved.
...
|:----------------------------|:---------------------------------------------------|:---------------------------------------------------|:----------------------------------------------------|
| **4$\times$**               | `{python} CommCompression.breakeven_4_10_str` ms   | `{python} CommCompression.breakeven_4_40_str` ms   | `{python} CommCompression.breakeven_4_100_str` ms   |
| **16$\times$**              | `{python} CommCompression.breakeven_16_10_str` ms  | `{python} CommCompression.breakeven_16_40_str` ms  | `{python} CommCompression.breakeven_16_100_str` ms  |
| **64$\times$**              | `{python} CommCompression.breakeven_64_10_str` ms  | `{python} CommCompression.breakeven_64_40_str` ms  | `{python} CommCompression.breakeven_64_100_str` ms  |
| **256$\times$**             | `{python} CommCompression.breakeven_256_10_str` ms | `{python} CommCompression.breakeven_256_40_str` ms | `{python} CommCompression.breakeven_256_100_str` ms |
...
These models are simple enough to use quickly, but they are also easy to apply outside their assumptions. The most common mistakes come from ignoring which term dominates or which communication pattern the formula actually describes.
