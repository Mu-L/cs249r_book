...
from mlsysim.core.constants import (
    H100_FLOPS_FP16_TENSOR, H100_MEM_BW, TFLOPs, TB, second, flop, GB, byte,
    BILLION, MILLION, THOUSAND, MS_PER_SEC, BYTES_FP16,
)
from mlsysim.fmt import MarkdownStr, fmt, fmt_frac, fmt_math, sci_latex, check
...
class DAMTaxonomy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_ridge_str = fmt(h100_ridge_flop_per_byte, precision=0, commas=False)
...
[^fn-intensity-v2]: **Arithmetic Intensity**: The ratio of floating-point operations to bytes transferred (FLOPs/byte). It determines whether a workload is memory-bound or compute-bound by comparison against the hardware's *ridge point* ($R_{\text{peak}}/\text{BW}$).
...
*   **If arithmetic intensity is below the ridge point** (about `{python} DAMTaxonomy.h100_ridge_str` FLOPs/byte on H100): The workload is likely memory bound (Data/Machine boundary).
...
The efficiency ratios in @tbl-v2-dam-scorecard grade a system's performance against its theoretical limit. This "Report Card" is anchored by **MFU**[^fn-mfu-v2]—the ratio of achieved model FLOPs to the hardware's theoretical peak FLOPs.
