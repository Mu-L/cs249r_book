...
from mlsysim.fmt import fmt, fmt_percent, check, md, md_math, sci_latex
...
class FleetFoundations:
    ...
    tpuv6_bw_tbs = fmt(TPUV6_MEM_BW.m_as(TB/second), precision=2, commas=False)
    tpuv6_cap = int(TPUV6_MEM_CAPACITY.m_as(GiB))
    mi300x_flops = int(MI300X_FLOPS_FP16_TENSOR.m_as(TFLOPs/second))
    mi300x_bw_tbs = fmt(MI300X_MEM_BW.m_as(TB/second), precision=2, commas=False)
...
1. **18$\times$ gap**: NVLink bandwidth within a node is ~`{python} FF.nvlink_to_ib`$\times$ faster than InfiniBand between nodes. This ratio determines where parallelism boundaries belong---model parallelism within a node, data parallelism across nodes.
...
| **Category**               | **Number**                                                                                                                                                           | **Use**                                                             |
|:---------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------------------------------|
| **Communication**          | NVLink ~`{python} FleetQuickRef.nvlink_ib`$\times$ IB NDR                                                                                                            | Parallelism boundary (in-node vs. cross-node)                       |
...
| **Coordination**           | 175B checkpoint ~`{python} FleetQuickRef.ckpt_175` GB (14 B/param common Adam state)                                                                                 | Recovery and storage sizing                                         |
...
| **Power & sustainability** | PUE liquid ~`{python} FleetQuickRef.pue_liq`, typical ~`{python} FleetQuickRef.pue_typ`; H100 `{python} FleetQuickRef.h100_tdp` W -> 10K GPUs ≈ 7 MW IT $\times$ PUE | Facility load and carbon (see @sec-fleet-foundations-thermal-power) |
...
class FleetCommNumbers:
    ...
    ib_gxdr_bw   = fmt(FF.ib_gxdr_bw, precision=0)
...
| **IB GXDR (1.6 Tbps)** | `{python} FleetCommNumbers.ib_gxdr_bw` GB/s | Next-gen inter-node (2026+)                 |
| **IB XDR (800 Gbps)**  | `{python} FleetCommNumbers.ib_xdr_bw` GB/s  | Inter-node standard (2025+)                 |
...
Model FLOPS Utilization (MFU) measures what fraction of peak FLOPS a training workload actually achieves. Well-optimized large-model training on current hardware achieves `{python} FF.mfu_low`--`{python} FF.mfu_high` percent MFU. The gap comes from memory stalls, kernel launch overhead, pipeline bubbles, and suboptimal operator fusion. MFU below `{python} FF.mfu_low` percent signals optimization opportunities; MFU above `{python} FF.mfu_high` percent indicates excellent hardware utilization.
...
| **8,192 GPUs**   | ~`{python} FF.eff_8192`%                    | Fleet-scale regime; 65 percent of compute is overhead |
...
| **70B parameters**  | `{python} FleetCheckpointSizes.ckpt_70b` GB  | ~10 seconds               |
| **175B parameters** | `{python} FleetCheckpointSizes.ckpt_175b` GB | ~25 seconds               |
| **1T parameters**   | `{python} FleetCheckpointSizes.ckpt_1t` TB   | ~2.3 minutes              |
...
class FleetOverheadBudget:
    pipeline_pct    = fmt_percent(FF.oh_pipeline,    precision=0, commas=False)
    checkpoint_pct  = fmt_percent(FF.oh_checkpoint,  precision=0, commas=False)
    failure_pct     = fmt_percent(FF.oh_failure,     precision=0, commas=False)
    maintenance_pct = fmt_percent(FF.oh_maintenance, precision=0, commas=False)
...
|:------------------------|:-------------------------------------------------|:-----------------------------------------|
| **Pipeline bubbles**    | ~`{python} FleetOverheadBudget.pipeline_pct`%    | Increase microbatches per pipeline stage |
| **Checkpointing**       | ~`{python} FleetOverheadBudget.checkpoint_pct`%  | Async checkpointing, faster storage      |
| **Failure recovery**    | ~`{python} FleetOverheadBudget.failure_pct`%     | Faster detection, elastic rescheduling   |
| **Maintenance windows** | ~`{python} FleetOverheadBudget.maintenance_pct`% | Rolling upgrades, live migration         |
...
|:---------------------------|:-----------------------------------------------------------|:-------------------------------------------------|
| **Traditional rack**       | `{python} FF.rack_trad` kW                                 | Baseline for non-AI data centers                 |
...
| **H100 TDP**               | `{python} FF.h100_tdp` W per GPU                           | IT load: 10K GPUs $\times$ 700 W = 7 MW          |
...
class FleetHardwareRef:
    ...
    tpuv6_flops = fmt(FF.tpuv6_flops, precision=0)
    tpuv6_cap   = fmt(FF.tpuv6_cap, precision=0)

    mi300x_flops = fmt(FF.mi300x_flops, precision=0)
    mi300x_cap   = fmt(FF.mi300x_cap, precision=0)
    mi300x_tdp   = fmt(FF.mi300x_tdp, precision=0)
```

| **Spec**             | **NVIDIA B200**                               | **AMD MI300X**                                  | **Google TPU v6**                              |
|:---------------------|:----------------------------------------------|:------------------------------------------------|:-----------------------------------------------|
| **BF16/FP16 Peak**   | `{python} FleetHardwareRef.b200_flops` TFLOPS | `{python} FleetHardwareRef.mi300x_flops` TFLOPS | `{python} FleetHardwareRef.tpuv6_flops` TFLOPS |
| **Memory Bandwidth** | `{python} FF.b200_bw_tbs` TB/s                | `{python} FF.mi300x_bw_tbs` TB/s                | `{python} FF.tpuv6_bw_tbs` TB/s                |
| **HBM Capacity**     | `{python} FleetHardwareRef.b200_cap` GB       | `{python} FleetHardwareRef.mi300x_cap` GB       | `{python} FleetHardwareRef.tpuv6_cap` GB       |
| **Intra-Node Link**  | 1,800 GB/s (NVLink 5.0)                       | ~890 GB/s (Infinity Fab)                        | ~2,000 GB/s (estimated)                        |
| **TDP**              | `{python} FleetHardwareRef.b200_tdp` W        | `{python} FleetHardwareRef.mi300x_tdp` W        | ~600 W (estimated)                             |
...
The `{python} FleetAmdahlExample.s_fleet_pct_str` percent serial fraction above is optimistic. In practice, fleet-scale serial overhead accumulates from multiple sources: `{python} FleetOverheadBudget.pipeline_pct` percent pipeline bubbles + `{python} FleetOverheadBudget.checkpoint_pct` percent checkpointing + `{python} FleetOverheadBudget.failure_pct` percent failure recovery + `{python} FleetOverheadBudget.maintenance_pct` percent maintenance. These are not all strictly serial in the Amdahl sense---some overlap with computation---but they illustrate how quickly small per-category overheads compound into significant throughput loss.
:::
...
The cluster delivers `{python} FleetEffectiveFlops.eff_pct` percent of its peak FLOPS as useful training work. The remaining `{python} FleetEffectiveFlops.loss_pct` percent is explained by retained factors of `{python} FleetEffectiveFlops.mfu_pct` percent MFU, `{python} FleetEffectiveFlops.scaling_pct` percent scaling efficiency, and `{python} FleetEffectiveFlops.goodput_pct` percent goodput.
...
|:------------------------------|:------------------------------|:------------------------------------------|
| **Traditional data center**   | `{python} FF.rack_trad` kW    | Standard air cooling sufficient           |
...
A single GPU with a `{python} fmt(GPU_MTTF_HOURS, precision=0)`-hour MTTF seems extremely reliable---that is nearly 6 years. At `{python} fmt(FF.cl_large, precision=0)` GPUs, however, the cluster MTBF drops to `{python} FleetMtbfTable.mtbf_8192` hours (`{python} FleetMtbfTable.mtbf_8192_min` minutes). The probability of at least one failure in a 24-hour training window is `{python} FleetMtbfTable.pfail_8192` percent. Designing a training system without automated failure recovery at this scale guarantees that every long-running job will fail and require manual intervention.
:::
...
- **Compound utilization loss**: After MFU (~`{python} FleetEffectiveFlops.mfu_pct` percent), scaling efficiency (~`{python} FleetEffectiveFlops.scaling_pct` percent), and operational overhead (~`{python} FleetEffectiveFlops.goodput_pct` percent goodput) compound, a 1,024-GPU cluster delivers ~`{python} FleetEffectiveFlops.eff_pct` percent of peak FLOPS as useful work.
