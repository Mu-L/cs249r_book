...
| **Power & sustainability** | PUE liquid ~`{python} FleetQuickRef.pue_liq_str`, typical ~`{python} FleetQuickRef.pue_typ_str`; H100 `{python} FleetQuickRef.h100_tdp_str` W -> 10K GPUs ≈ 7 MW IT $\times$ PUE | Facility load and carbon (see @sec-fleet-foundations-thermal-power) |
...
|:--------------------|:-------------------------------------------------|:--------------------------|
| **7B parameters**   | `{python} FleetCheckpointSizes.ckpt_7b_str` GB   | ~1 second                 |
| **70B parameters**  | `{python} FleetCheckpointSizes.ckpt_70b_str` GB  | ~10 seconds               |
| **175B parameters** | `{python} FleetCheckpointSizes.ckpt_175b_str` GB | ~25 seconds               |
| **1T parameters**   | `{python} FleetCheckpointSizes.ckpt_1t_str` TB   | ~2.3 minutes              |

: **Checkpoint Sizes by Model Scale**: Uses 14 bytes/parameter for common mixed-precision Adam checkpoint state. Write time assumes 100 GB/s aggregate storage bandwidth. Asynchronous checkpointing (@sec-fault-tolerance-reliability) can overlap writes with training, reducing the visible overhead. {#tbl-fleet-checkpoint-sizes}
...
| **H100 TDP**               | `{python} FF.h100_tdp_str` W per GPU     | IT load: 10K GPUs $\times$ 700 W = 7 MW          |
...
Rule of thumb: **IT load (MW) = (number of GPUs $\times$ TDP per GPU)/$10^6$**; **facility load = IT load $\times$ PUE**. A 10,000-GPU H100 cluster at 700 W each is 7 MW IT; at PUE 1.4 that is 9.8 MW facility draw. For carbon and cost, see @sec-fleet-foundations-thermal-power and @sec-sustainable-ai.
...
A training job provisioned with 4,096 GPUs achieves only 2.5$\times$ the throughput of a 1,024-GPU run. Is this expected? Scaling physics provides the diagnostic tools to determine whether the system is performing as physics allows or whether there is an engineering problem to fix.
...
A cluster design calling for 10,000 H100 GPUs at 700 W each---7 MW of IT load---and a PUE of 1.4 puts the facility load at 9.8 MW total. This is the electrical load of a small town. Before optimizing software, the physics of power delivery and heat removal must be feasible.
...
|:---------------------------------|:------------------------------|:-------------------------------|
| **Liquid-cooled AI data center** | `{python} FF.pue_liquid_str`  | 60 kW cooling + infrastructure |
| **Best-in-class air-cooled**     | `{python} FF.pue_air_str`     | 120 kW overhead                |
| **Industry average**             | `{python} FF.pue_typical_str` | 400 kW overhead                |
| **Legacy enterprise**            | `{python} FF.pue_legacy_str`  | 580 kW overhead                |

: **Power Usage Effectiveness (PUE)**: Lower is better. Liquid cooling achieves PUE near 1.0 because it removes heat directly from the chip without the intermediate step of heating air. The gap between legacy (1.58) and liquid-cooled (1.06) represents about a 33 percent reduction in total facility power at fixed IT load; the non-IT overhead drops from 0.58 MW to 0.06 MW per MW of IT load. {#tbl-fleet-pue-fleet}
...
* **The 18$\times$ bandwidth gap**: NVLink provides ~`{python} FleetCommNumbers.nvlink_to_ib_str`$\times$ more bandwidth than InfiniBand. This invariant ratio determines parallelism placement: tensor parallelism within nodes, data parallelism across nodes.
* **Failure is certain at scale**: Cluster MTBF scales as $1/N$. At 8,192 GPUs, expect a failure every `{python} FleetMtbfTable.mtbf_8192_min_str` minutes. Fault tolerance is not optional---it is a prerequisite for completing any fleet-scale training job.
