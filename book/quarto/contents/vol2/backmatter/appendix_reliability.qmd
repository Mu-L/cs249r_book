...
**Implication.** If the cluster were doubled to `{python} WorkedExampleYoungDaly.n_gpus_20k_str`, the MTBF would halve, and the optimal interval would shrink to `{python} WorkedExampleYoungDaly.tau_20k_min_str` minutes---checkpointing more frequently because failures happen more often. This illustrates the fundamental tension at scale: larger clusters are faster but demand more frequent interruption to protect progress.
