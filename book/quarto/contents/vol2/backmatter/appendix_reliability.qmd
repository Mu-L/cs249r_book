...
Individual accelerators fail rarely---but fleets are large, and probability is relentless. Scale a cluster to tens of thousands of GPUs and the expected time between failures drops from years to hours. Scale further and failures arrive faster than any human operator can respond. The math is straightforward, the implications are profound: at fleet scale, failure is not an exceptional event to be debugged but a continuous physical condition to be engineered around, no different from heat dissipation or power delivery.
...
class ReliabilityFoundations:
    ...
    gpu_fit = 1e9 / gpu_mttf
    hbm_fit = 1e9 / hbm_mttf
    nic_fit = 1e9 / nic_mttf
    psu_fit = 1e9 / psu_mttf
    pcie_fit = 1e9 / pcie_mttf
    cable_fit = 1e9 / cable_mttf
    tor_fit = 1e9 / tor_mttf
    ...
    ckpt_175b_bytes = calc_checkpoint_size(175e9, 14)
    ...
    def ckpt_size_gb(cls, n_params):
        return calc_checkpoint_size(n_params, 14).m_as(GB)
...
class ReliabilityStrings:
    ...
    ckpt_write_bw_str     = fmt(R.ckpt_write_bw, precision=0)
    t_recovery            = fmt(R.t_recovery_total_s, precision=1)
    overhead_ckpt_pct_str = fmt(R.overhead_ckpt * 100, precision=0)
    overhead_failure_pct_str = fmt(R.overhead_failure * 100, precision=0)
...
class ComponentFitTable:
    gpu_fit_str        = fmt(R.gpu_fit, precision=0)
    hbm_fit_str        = fmt(R.hbm_fit, precision=0)
    nic_fit_str        = fmt(R.nic_fit, precision=0)
    psu_fit_str        = fmt(R.psu_fit, precision=0)
    pcie_fit_str       = fmt(R.pcie_fit, precision=0)
    cable_fit_str      = fmt(R.cable_fit, precision=0)
    tor_fit_str        = fmt(R.tor_fit, precision=0)
    gpu_mttf_str       = fmt(R.gpu_mttf, precision=0)
    hbm_mttf_str       = fmt(R.hbm_mttf, precision=0)
    nic_mttf_str       = fmt(R.nic_mttf, precision=0)
    psu_mttf_str       = fmt(R.psu_mttf, precision=0)
    pcie_mttf_str      = fmt(R.pcie_mttf, precision=0)
    cable_mttf_str     = fmt(R.cable_mttf, precision=0)
    tor_mttf_str       = fmt(R.tor_mttf, precision=0)
    gpu_mttf_yr_str   = fmt(R.gpu_mttf/HOURS_PER_YEAR, precision=1, commas=False)
    hbm_mttf_yr_str   = fmt(R.hbm_mttf/HOURS_PER_YEAR, precision=1, commas=False)
    nic_mttf_yr_str   = fmt(R.nic_mttf/HOURS_PER_YEAR, precision=1, commas=False)
    psu_mttf_yr_str   = fmt(R.psu_mttf/HOURS_PER_YEAR, precision=1, commas=False)
    pcie_mttf_yr_str  = fmt(R.pcie_mttf/HOURS_PER_YEAR, precision=1, commas=False)
    cable_mttf_yr_str = fmt(R.cable_mttf/HOURS_PER_YEAR, precision=1, commas=False)
    tor_mttf_yr_str   = fmt(R.tor_mttf/HOURS_PER_YEAR, precision=1, commas=False)
```
...
Each component in isolation appears highly reliable---a GPU lasts `{python} ComponentFitTable.gpu_mttf_yr_str` years on average. The trouble begins when we ask how a *node* behaves with many such components operating simultaneously.
...
For a cluster of $N$ identical nodes, the same logic applies one level up, as @eq-mtbf-cluster shows:

$$ \text{MTBF}_\text{cluster} = \frac{\text{MTBF}_\text{node}}{N} $$ {#eq-mtbf-cluster}

This is the **MTBF cascade**: reliability degrades linearly with component count at each level, and the levels compound. A node with `{python} RS.node_mtbf_str`-hour MTBF sounds reliable. A cluster of `{python} RS.nodes_10k_str` such nodes has an MTBF of just `{python} RS.cluster_mtbf_10k_str` hours---a failure every few hours is the expected steady state.
...
for n_gpus in R.cluster_sizes:
    ...
    if mtbf_h_val >= 1.0:
        ...
        mtbf_str = fmt(mtbf_h_val * MINUTES_PER_HOUR, precision=0, commas=False, suffix=" minutes")
...
The optimal checkpoint interval balances two competing costs. Writing a checkpoint takes time $\delta$ (the **checkpoint cost**), during which no useful training occurs. The longer the interval between checkpoints, however, the more work is lost when a failure strikes---on average, half the interval. The **Young-Daly formula** minimizes the expected total overhead:

$$ \tau_{\text{opt}} = \sqrt{2 \times \delta \times \text{MTBF}_{\text{cluster}}} $$ {#eq-young-daly}

where $\delta$ is the checkpoint write time in seconds and $\text{MTBF}_{\text{cluster}}$ is the cluster MTBF in seconds.

The intuition is geometric-mean-like: when checkpoints are cheap relative to the MTBF ($\delta \ll \text{MTBF}_{\text{cluster}}$, the common case), the optimal interval sits between the two time scales. If checkpoints took zero time, the optimal cadence is every step. If the system never failed, no checkpoints would be required. The square root interpolates between these extremes.

The formula assumes that failures follow an exponential distribution (memoryless property) and that checkpoint cost $\delta$ is small compared to $\text{MTBF}_{\text{cluster}}$. Both assumptions hold well for production training clusters: the exponential model fits observed failure data, and modern checkpointing systems write to fast parallel storage in tens of seconds, while MTBF is measured in hours.

### Checkpoint sizing {#sec-reliability-foundations-checkpoint-sizing}

Checkpoint size determines the write time $\delta$ that feeds into the Young-Daly formula. For common mixed-precision training checkpoints with the Adaptive Moment Estimation (Adam) optimizer, each parameter requires approximately 14 bytes of persistent state:
...
$$ \text{Checkpoint Size} = N_\text{params} \times 14 \text{ bytes/param} $$ {#eq-checkpoint-size}
...
for i, n_params in enumerate(R.model_sizes_params):
    ...
    if write_time < 1.0:
        ...
        write_str = fmt(write_time, precision=1, commas=False, suffix=" s")
    ckpt_data.append({
...
| **Model Size**                       | **Checkpoint Size (GB)**           | **Write Time at `{python} RS.ckpt_write_bw_str` GB/s** |
|:-------------------------------------|:-----------------------------------|:-------------------------------------------------------|
...
As @tbl-checkpoint-size shows, at frontier scale (175B+ parameters), checkpoint sizes reach the terabyte range. This makes checkpoint write time a significant cost that directly affects the Young-Daly optimal interval. The checkpoint strategies in @sec-fault-tolerance-reliability discuss techniques for reducing $\delta$---asynchronous checkpointing, incremental deltas, and distributed storage---all of which improve the Young-Daly result by shrinking the numerator under the square root.
...
class WorkedExampleYoungDaly:
    ...
    ckpt_175b_gb_str = _ckpt_gb_str
    ckpt_write_bw_str = _bw_str
```
...
**Setup.** Consider training a 175B-parameter model on a 10,000-GPU cluster. The cluster MTBF is `{python} WorkedExampleYoungDaly.yd_mtbf_h_str` hours (@tbl-mtbf-cluster). The checkpoint size is `{python} WorkedExampleYoungDaly.ckpt_175b_gb_str` GB, and the parallel storage system writes at `{python} WorkedExampleYoungDaly.ckpt_write_bw_str` GB/s.
...
| **$T_\text{replay}$**         | ~`{python} RecoveryAnatomy.t_replay_str` min  | Recompute training steps since last checkpoint  |
| **Total $T_\text{recovery}$** | ~`{python} RecoveryAnatomy.t_total_str` min   | System fully productive again                   |
...
1. **Checkpoint overhead** ($\sim$ `{python} RS.overhead_ckpt_pct_str` percent): Training pauses during each checkpoint write.
2. **Recovery overhead** ($\sim$ `{python} RS.overhead_failure_pct_str` percent): Time lost to detection, rescheduling, reloading, and replay after each failure.
...
for k in R.avail_replicas:
    ...
    if downtime_yr_min > MINUTES_PER_HOUR:
        ...
        dt_str = fmt(downtime_yr_min, precision=0, commas=False, suffix=" minutes")
    elif downtime_yr_min * 60 >= 1:
        dt_str = fmt(downtime_yr_min * 60, precision=1, commas=False, suffix=" seconds")
    else:
        dt_str = fmt(downtime_yr_min * 60000, precision=1, commas=False, suffix=" ms")
    avail_data.append({
        "k": str(k),
...
* **Failure rate scales linearly with component count**: A single GPU fails once per `{python} ComponentFitTable.gpu_mttf_yr_str` years; a 10,000-GPU cluster experiences a failure every `{python} RS.cluster_mtbf_10k_str` hours. At fleet scale, failure is a continuous background condition, not an exceptional event.
...
* **The Young-Daly formula $\tau_\text{opt} = \sqrt{2 \delta \, \text{MTBF}_{\text{cluster}}}$ optimizes checkpoint frequency**: It balances the cost of writing checkpoints against the cost of lost work, requiring only two measurable inputs: checkpoint write time and cluster MTBF.
