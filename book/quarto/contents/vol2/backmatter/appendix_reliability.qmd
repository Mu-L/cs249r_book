...
class ReliabilityStrings:
    ...
    gpu_mttf              = fmt(R.gpu_mttf, precision=1, allow_zero=True)
    node_mtbf             = fmt(R.node_mtbf.m_as(ureg.hour), precision=1, allow_zero=True)
    node_mtbf_str         = fmt(R.node_mtbf.m_as(ureg.hour), precision=1, allow_zero=True)
    nodes_10k_str         = fmt(R.nodes_10k, precision=1, allow_zero=True)
    cluster_mtbf_10k_str  = fmt(R.cluster_mtbf_10k.m_as(ureg.hour), precision=2)
    tau_opt_min           = fmt(R.tau_opt_min, precision=1)
    ckpt_175b_gb          = fmt(R.ckpt_175b_gb, precision=1, allow_zero=True)
    ckpt_write_time       = fmt(R.ckpt_write_time_s, precision=1)
    ckpt_write_bw_str     = fmt(R.ckpt_write_bw, precision=1, allow_zero=True)
    t_recovery            = fmt(R.t_recovery_total_s, precision=1)
    overhead_ckpt_pct_str = fmt(R.overhead_ckpt * 100, precision=1, allow_zero=True)
    overhead_failure_pct_str = fmt(R.overhead_failure * 100, precision=1, allow_zero=True)
...
class ComponentFitTable:
    gpu_fit_str        = fmt(R.gpu_fit, precision=1, allow_zero=True)
    hbm_fit_str        = fmt(R.hbm_fit, precision=1, allow_zero=True)
    nic_fit_str        = fmt(R.nic_fit, precision=1, allow_zero=True)
    psu_fit_str        = fmt(R.psu_fit, precision=1, allow_zero=True)
    pcie_fit_str       = fmt(R.pcie_fit, precision=1, allow_zero=True)
    cable_fit_str      = fmt(R.cable_fit, precision=1, allow_zero=True)
    tor_fit_str        = fmt(R.tor_fit, precision=1, allow_zero=True)
    gpu_mttf_str       = fmt(R.gpu_mttf, precision=1, allow_zero=True)
    hbm_mttf_str       = fmt(R.hbm_mttf, precision=1, allow_zero=True)
    nic_mttf_str       = fmt(R.nic_mttf, precision=1, allow_zero=True)
    psu_mttf_str       = fmt(R.psu_mttf, precision=1, allow_zero=True)
    pcie_mttf_str      = fmt(R.pcie_mttf, precision=1, allow_zero=True)
    cable_mttf_str     = fmt(R.cable_mttf, precision=1, allow_zero=True)
    tor_mttf_str       = fmt(R.tor_mttf, precision=1, allow_zero=True)
...
for n_gpus in R.cluster_sizes:
    ...
    if mtbf_h_val >= 1.0:
        ...
        mtbf_str = fmt(mtbf_h_val * MINUTES_PER_HOUR, precision=1, allow_zero=True, commas=False, suffix=" minutes")
...
for n_gpus in R.cluster_sizes:
    ...
    fp_data[n_gpus] = row
    fp_rows.append({
        "gpus": fmt(n_gpus, precision=1, allow_zero=True),
        "p_1d": row[0],
        "p_1w": row[1],
        "p_30d": row[2],
    })
...
for i, n_params in enumerate(R.model_sizes_params):
    ...
    ckpt_data.append({
        ...
        "ckpt_gb": fmt(ckpt_gb, precision=1, allow_zero=True),
...
class WorkedExampleYoungDaly:
    ...
    yd_t_write_str = fmt(yd_t_write_s, precision=1)
    ...
    _ckpt_gb_str = fmt(R.ckpt_175b_gb, precision=1, allow_zero=True)
    _bw_str = fmt(R.ckpt_write_bw, precision=1, allow_zero=True, commas=False)
    _sec_per_hour_str = fmt(SEC_PER_HOUR, precision=1, allow_zero=True).replace(",", "{,}")
    t_write_eq_math = MarkdownStr(
        ...
        f"= {yd_t_write_str} \\text{{ s}}$$"
    )
    tau_opt_eq_math = MarkdownStr(
        f"$$\\tau_{{\\text{{opt}}}} = \\sqrt{{2 \\times T_{{\\text{{write}}}} "
        f"\\times \\text{{MTBF}}_{{\\text{{system}}}}}} "
        f"= \\sqrt{{2 \\times {yd_t_write_str} \\text{{ s}} "
        f"\\times {yd_mtbf_h_str} \\text{{ h}} \\times {_sec_per_hour_str} \\text{{ s/h}}}} "
...
**Interpretation.** The optimal checkpoint interval is approximately `{python} WorkedExampleYoungDaly.yd_tau_min_str` minutes. The overhead from checkpointing alone is $T_{\text{write}} / \tau_{\text{opt}} \approx$ `{python} WorkedExampleYoungDaly.yd_overhead_str` percent of training time.
...
class RecoveryAnatomy:
    t_detect_str      = fmt(R.t_detect, precision=1, allow_zero=True, commas=False)
    t_reschedule_str  = fmt(R.t_reschedule, precision=1, allow_zero=True, commas=False)
...
for k in R.avail_replicas:
    ...
    if downtime_yr_min > MINUTES_PER_HOUR:
        ...
        dt_str = fmt(downtime_yr_min, precision=1, allow_zero=True, commas=False, suffix=" minutes")
