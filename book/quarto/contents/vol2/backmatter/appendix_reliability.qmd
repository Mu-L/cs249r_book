...
for n_gpus in R.cluster_sizes:
    ...
    if mtbf_h_val >= 1.0:
        ...
        mtbf_str = f"{mtbf_h_val * MINUTES_PER_HOUR:.0f} minutes"
...
**Setup.** Consider training a 175B-parameter model on a 10,000-GPU cluster. The cluster MTBF is `{python} WorkedExampleYoungDaly.yd_mtbf_h_str` hours (@tbl-mtbf-cluster). The checkpoint size is `{python} WorkedExampleYoungDaly.ckpt_175b_gb` GB, and the parallel storage system writes at `{python} WorkedExampleYoungDaly.ckpt_write_bw` GB/s.
...
for k in R.avail_replicas:
    ...
    if downtime_yr_min > MINUTES_PER_HOUR:
        dt_str = f"{downtime_yr_min/MINUTES_PER_HOUR:.1f} hours"
    else:
