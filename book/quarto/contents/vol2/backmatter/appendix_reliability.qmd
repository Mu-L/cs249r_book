...
class ReliabilityFoundations:
    ...
    gpu_mttf = defaults.GPU_MTTF_HOURS
    hbm_mttf = defaults.HBM_MTTF_HOURS
    nic_mttf = defaults.NIC_MTTF_HOURS
    psu_mttf = defaults.PSU_MTTF_HOURS
    pcie_mttf = defaults.PCIE_SWITCH_MTTF_HOURS
    cable_mttf = defaults.CABLE_MTTF_HOURS
    tor_mttf = defaults.TOR_SWITCH_MTTF_HOURS
    ...
    cluster_sizes = [
        defaults.CLUSTER_SMALL_GPUS,    # 256
        1024,                   # 1K
        defaults.CLUSTER_MEDIUM_GPUS,   # 2,048
        defaults.CLUSTER_LARGE_GPUS,    # 8,192
        10_000,                # 10K (canonical worked example)
        defaults.CLUSTER_MEGA_GPUS      # 100K
    ]

    # Recovery Params
    t_detect = defaults.HEARTBEAT_TIMEOUT_S
    t_reschedule = defaults.RESCHEDULE_TIME_S
    ckpt_write_bw = defaults.CHECKPOINT_WRITE_BW_GBS
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    canonical_cluster_gpus_str = fmt(10_000, precision=0, commas=True)
...
class ReliabilityIntroRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpu_mttf_str = fmt(defaults.GPU_MTTF_HOURS, precision=0, commas=False)
```
...
class ComponentFitTable:
    ...
    gpu_mttf_yr_str   = fmt(gpu_mttf/HOURS_PER_YEAR, precision=1, commas=False)
...
| **Model Size**                       | **Checkpoint Size (GB)**           | **Write Time at `{python} ReliabilityStringsCkptRecap.ckpt_write_bw_str` GB/s** |
|:-------------------------------------|:-----------------------------------|:--------------------------------------------------------------------------------|
...
1. **Checkpoint overhead** ($\sim$ `{python} ReliabilityStringsGoodputRecap.overhead_ckpt_pct_str` percent): Training pauses during each checkpoint write.
2. **Recovery overhead** ($\sim$ `{python} ReliabilityStringsGoodputRecap.overhead_failure_pct_str` percent): Time lost to detection, rescheduling, reloading, and replay after each failure.
...
from mlsysim.core.formulas import calc_mtbf_node
from mlsysim.core import defaults

class ReliabilitySummaryRecap:
    n_gpus_per_node = GPUS_PER_HOST
    node_mtbf = calc_mtbf_node(
        defaults.GPU_MTTF_HOURS, n_gpus_per_node,
        defaults.NIC_MTTF_HOURS, 2,
        defaults.PSU_MTTF_HOURS, 2,
    )
    cluster_mtbf_10k = node_mtbf / (10_000 // GPUS_PER_HOST)
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    gpu_mttf_yr_str = fmt(defaults.GPU_MTTF_HOURS / HOURS_PER_YEAR, precision=1, commas=False)
    cluster_mtbf_10k_str = fmt(cluster_mtbf_10k.m_as(ureg.hour), precision=2)
```

::: {.callout-takeaways title="Failure as a physical constraint"}

* **Failure rate scales linearly with component count**: A single GPU fails once per `{python} ReliabilitySummaryRecap.gpu_mttf_yr_str` years; a 10,000-GPU cluster experiences a failure every `{python} ReliabilitySummaryRecap.cluster_mtbf_10k_str` hours. At fleet scale, failure is a continuous background condition, not an exceptional event.
