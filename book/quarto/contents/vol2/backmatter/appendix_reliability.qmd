...
class ReliabilityStrings:
    ...
    gpu_mttf              = fmt(R.gpu_mttf, precision=1)
    node_mtbf             = fmt(R.node_mtbf.m_as(ureg.hour), precision=1)
    node_mtbf_str         = fmt(R.node_mtbf.m_as(ureg.hour), precision=1)
    nodes_10k_str         = fmt(R.nodes_10k, precision=1)
    cluster_mtbf_10k_str  = fmt(R.cluster_mtbf_10k.m_as(ureg.hour), precision=2)
    tau_opt_min           = fmt(R.tau_opt_min, precision=1)
    ckpt_175b_gb          = fmt(R.ckpt_175b_gb, precision=1)
    ckpt_write_time       = fmt(R.ckpt_write_time_s, precision=1)
    ckpt_write_bw_str     = fmt(R.ckpt_write_bw, precision=1)
    t_recovery            = fmt(R.t_recovery_total_s, precision=1)
    overhead_ckpt_pct_str = fmt(R.overhead_ckpt * 100, precision=1)
    overhead_failure_pct_str = fmt(R.overhead_failure * 100, precision=1)
...
class ComponentFitTable:
    gpu_fit_str        = fmt(R.gpu_fit, precision=1)
    hbm_fit_str        = fmt(R.hbm_fit, precision=1)
    nic_fit_str        = fmt(R.nic_fit, precision=1)
    psu_fit_str        = fmt(R.psu_fit, precision=1)
    pcie_fit_str       = fmt(R.pcie_fit, precision=1)
    cable_fit_str      = fmt(R.cable_fit, precision=1)
    tor_fit_str        = fmt(R.tor_fit, precision=1)
    gpu_mttf_str       = fmt(R.gpu_mttf, precision=1)
    hbm_mttf_str       = fmt(R.hbm_mttf, precision=1)
    nic_mttf_str       = fmt(R.nic_mttf, precision=1)
    psu_mttf_str       = fmt(R.psu_mttf, precision=1)
    pcie_mttf_str      = fmt(R.pcie_mttf, precision=1)
    cable_mttf_str     = fmt(R.cable_mttf, precision=1)
    tor_mttf_str       = fmt(R.tor_mttf, precision=1)
...
for i, n_params in enumerate(R.model_sizes_params):
    ...
    ckpt_data.append({
        ...
        "ckpt_gb": fmt(ckpt_gb, precision=1),
...
class WorkedExampleYoungDaly:
    ...
    _ckpt_gb_str = fmt(R.ckpt_175b_gb, precision=1)
    _bw_str = fmt(R.ckpt_write_bw, precision=1, commas=False)
    _sec_per_hour_str = fmt(SEC_PER_HOUR, precision=1).replace(",", "{,}")
...
class RecoveryAnatomy:
    t_detect_str      = fmt(R.t_detect, precision=1, commas=False)
    t_reschedule_str  = fmt(R.t_reschedule, precision=1, commas=False)
