...
class ReliabilityFoundations:
    ...
    nodes_10k = 10_000 // Systems.Nodes.DGX_H100.accelerators_per_node
...
class ReliabilityIntroRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpu_mttf_str = fmt(Systems.Reliability.Gpu.mttf_hours, precision=0, commas=False)
```
...
**Implication.** If the cluster were doubled to `{python} WorkedExampleYoungDaly.n_gpus_20k_str` GPUs, the MTBF would halve, and the optimal interval would shrink to `{python} WorkedExampleYoungDaly.tau_20k_min_str` minutes---checkpointing more frequently because failures happen more often. This illustrates the fundamental tension at scale: larger clusters are faster but demand more frequent interruption to protect progress.
...
class ReliabilitySummaryRecap:
    ...
    cluster_mtbf_10k = node_mtbf / (10_000 // Systems.Nodes.DGX_H100.accelerators_per_node)
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    gpu_mttf_yr_str = fmt(Systems.Reliability.Gpu.mttf_hours / HOURS_PER_YEAR, precision=1, commas=False)
