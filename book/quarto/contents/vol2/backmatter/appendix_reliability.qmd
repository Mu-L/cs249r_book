...
from mlsysim.fmt import fmt, check, md, MarkdownStr
...
for n_gpus in R.cluster_sizes:
    ...
    if mtbf_h_val >= 1.0:
        mtbf_str = MarkdownStr(f"{mtbf_h_val:.1f} hours")
    else:
        mtbf_str = MarkdownStr(f"{mtbf_h_val * MINUTES_PER_HOUR:.0f} minutes")
...
for i, n_params in enumerate(R.model_sizes_params):
    ...
    if write_time < 1.0:
        write_str = MarkdownStr(f"{write_time:.2f} s")
    else:
        write_str = MarkdownStr(f"{write_time:.1f} s")
    ckpt_data.append({
...
for k in R.avail_replicas:
    ...
    if downtime_yr_min > MINUTES_PER_HOUR:
        dt_str = MarkdownStr(f"{downtime_yr_min/MINUTES_PER_HOUR:.1f} hours")
    else:
        dt_str = MarkdownStr(f"{downtime_yr_min:.0f} minutes")
    avail_data.append({
        "k": str(k),
