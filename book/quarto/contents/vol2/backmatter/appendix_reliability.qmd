...
class ReliabilityFoundations:
    ...
    gpu_mttf_str = fmt(gpu_mttf, precision=0, commas=False)
...
class ReliabilitySummaryRecap:
    n_gpus_per_node = GPUS_PER_HOST
    node_mtbf = calc_mtbf_node(
        GPU_MTTF_HOURS, n_gpus_per_node,
        NIC_MTTF_HOURS, 2,
        PSU_MTTF_HOURS, 2,
    )
    cluster_mtbf_10k = node_mtbf / (10_000 // GPUS_PER_HOST)
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    gpu_mttf_yr_str = fmt(GPU_MTTF_HOURS / HOURS_PER_YEAR, precision=1, commas=False)
