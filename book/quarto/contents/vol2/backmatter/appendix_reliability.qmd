...
|:------------------|:------------------------------|:-------------------------------|:-----------------------------------------------|:----------------------------|
| **GPU**           | `{python} f"{R.gpu_fit:,}"`   | `{python} f"{R.gpu_mttf:,}"`   | `{python} ComponentFitTable.gpu_mttf_yr_str`   | Die defect, thermal fatigue |
| **HBM**           | `{python} f"{R.hbm_fit:,}"`   | `{python} f"{R.hbm_mttf:,}"`   | `{python} ComponentFitTable.hbm_mttf_yr_str`   | Bit-flip accumulation, TSV  |
| **NIC**           | `{python} f"{R.nic_fit:,}"`   | `{python} f"{R.nic_mttf:,}"`   | `{python} ComponentFitTable.nic_mttf_yr_str`   | Transceiver degradation     |
| **PSU**           | `{python} f"{R.psu_fit:,}"`   | `{python} f"{R.psu_mttf:,}"`   | `{python} ComponentFitTable.psu_mttf_yr_str`   | Capacitor aging             |
| **PCIe Switch**   | `{python} f"{R.pcie_fit:,}"`  | `{python} f"{R.pcie_mttf:,}"`  | `{python} ComponentFitTable.pcie_mttf_yr_str`  | Solder joint, ESD damage    |
| **Optical Cable** | `{python} f"{R.cable_fit:,}"` | `{python} f"{R.cable_mttf:,}"` | `{python} ComponentFitTable.cable_mttf_yr_str` | Fiber bend, connector wear  |
| **ToR Switch**    | `{python} f"{R.tor_fit:,}"`   | `{python} f"{R.tor_mttf:,}"`   | `{python} ComponentFitTable.tor_mttf_yr_str`   | ASIC failure, fan bearing   |
...
This is the **MTBF cascade**: reliability degrades linearly with component count at each level, and the levels compound. A node with `{python} f"{R.node_mtbf.m_as(ureg.hour):,.0f}"`-hour MTBF sounds reliable. A cluster of `{python} f"{R.nodes_10k:,}"` such nodes has an MTBF of just `{python} RS.cluster_mtbf_10k_str` hours---a failure every few hours is the expected steady state.
...
1. **Checkpoint overhead** ($\sim$ `{python} f"{R.overhead_ckpt * 100:.0f}"` percent): Training pauses during each checkpoint write.
2. **Recovery overhead** ($\sim$ `{python} f"{R.overhead_failure * 100:.0f}"` percent): Time lost to detection, rescheduling, reloading, and replay after each failure.
