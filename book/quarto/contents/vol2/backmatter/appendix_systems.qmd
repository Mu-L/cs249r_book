...
::: {#fig-fleet-vs-single-stack fig-env="figure" fig-pos="htb" fig-cap="**The Evolution of the Stack**: Moving from a single machine to the fleet shifts the systems focus from local hierarchies to global networks. (A) The single-machine stack optimizes for the local memory wall (SRAM/HBM/PCIe). (B) The fleet stack optimizes for the global bisection bandwidth wall (InfiniBand/Ethernet). This volume assumes mastery of (A) and focuses on the engineering of (B)." fig-alt="Side-by-side diagram of two system stacks. The single-machine stack shows application, ML framework, operating system, and hardware. The fleet stack shows governance, serving/ops, distribution, and infrastructure."}
![](images/svg/system-map.svg)
:::

## A.1 The Three Computing Paradigms {#sec-appendix-systems-paradigms}

Large-scale ML infrastructure represents a convergence of three distinct computing lineages. Understanding where ML inherits its assumptions is critical for diagnosing system failures. @Tbl-appendix-systems-hpc-wsc-ml contrasts five system properties across the three lineages:
...
| **Bottleneck** | Compute (FLOP/s)        | I/O (Network/Disk)        | **Network/Bisection Bandwidth** |

: **HPC, WSC, and ML Fleet Compared**: Five system properties (lineage, coupling, state, fault behavior, dominant bottleneck) across the three lineages that converge in modern ML infrastructure. The ML fleet inherits selectively from each—synchronous coupling from HPC, elastic recovery from WSC, and a fleet-scale bisection-bandwidth bottleneck unique to distributed deep learning. {#tbl-appendix-systems-hpc-wsc-ml}
...
### The silicon contract (GEMM) {#sec-appendix-review-machine-learning-systems-foundations-silicon-contract-gemm-4a6d}
The fundamental unit of ML computation is the **general matrix multiply (GEMM)**. Modern accelerators are designed around the "silicon contract": if the software can express its logic as large, dense matrix operations, the hardware guarantees near-peak utilization through specialized systolic arrays or tensor cores. Small, irregular, or sparse operations violate this contract, falling off the "performance cliff" into a regime dominated by instruction overhead rather than math.

### The memory wave of backpropagation {#sec-appendix-review-machine-learning-systems-foundations-memory-wave-backpropagation-c221}
Training introduces a unique memory challenge known as the **Activation Wave**. In standard backpropagation, many intermediate activations are cached during the forward pass so they can be reused during gradient computation. Activation checkpointing reduces this footprint by storing only selected tensors and recomputing others during the backward pass. Even with such techniques, activation memory scales with model depth and batch size, often exceeding the storage available for model weights by 3--5$\times$. This "memory wall" is the primary driver for distributed strategies like **Activation Checkpointing** and **ZeRO**.

## A.3 The Single-Node Memory Hierarchy {#sec-appendix-systems-memory}

The most significant constraint in ML systems is the gap between logic speed and data delivery. We quantify this through the memory hierarchy in @tbl-appendix-memory-hierarchy.
...
| **Primary**   | HBM3/DRAM      | ~80 GB               | ~3 TB/s               | ~300 ns              |
| **Secondary** | NVMe/Flash     | ~4 TB                | ~10 GB/s              | ~100,000 ns          |

: **The Single-Node Memory Hierarchy**: The roughly 7$\times$ bandwidth gap between the table's SRAM and HBM figures, and the roughly 300$\times$ gap between HBM and Non-Volatile Memory Express (NVMe), help determine the "Arithmetic Intensity" required to saturate an accelerator. {#tbl-appendix-memory-hierarchy}
...
$$T = \underbrace{\frac{D_{\text{vol}}}{\text{BW}}}_{\text{Data Movement}} + \underbrace{\frac{O}{R_{\text{peak}} \cdot \eta_{\text{hw}}}}_{\text{Calculation}} + \underbrace{L_{\text{lat}}}_{\text{Fixed Latency}}$$
...
## A.5 The Data-Algorithm-Machine Taxonomy {#sec-appendix-systems-dam}

We organize all system design decisions into the **Data · Algorithm · Machine** taxonomy, often abbreviated **D·A·M** as a mnemonic label rather than mathematical notation.

*   **Data**: The volume, velocity, and quality of information.
*   **Algorithm**: The mathematical structure, such as Transformers and convolutional neural networks (CNNs), and the optimization logic.
*   **Machine**: The physical substrate (Accelerators, Interconnects, Storage).

The central thesis of ML systems engineering is that these three vertices are **inseparable**. A change in the Machine (for example, switching from HBM2 to HBM3) reshapes which Algorithms are efficient, which in turn determines how much Data can be processed within a given power budget.

## A.6 Inference Foundations {#sec-appendix-systems-inference}

### A.6.1 queuing theory for batched inference {#sec-inference-scale-queuing-theory-batched-inference-bbf0}
...
![](../inference/images/svg/queuing-hockey-stick.svg)
:::
...
![](../inference/images/svg/batch-size-tradeoffs.svg)
...
### A.6.2 KV cache fundamentals {#sec-inference-scale-kv-cache-fundamentals-2ccd}
