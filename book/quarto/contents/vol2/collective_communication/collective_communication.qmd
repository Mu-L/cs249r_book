...
**Context**: Uber's Michelangelo platform needed TensorFlow training jobs to scale across many GPUs without forcing every model author to rewrite large portions of training code [@sergeev2018horovod].

**Failure**: Existing TensorFlow multi-GPU approaches imposed communication overhead and heavy code changes, while parameter-server patterns concentrated gradient traffic through a central tier.

**Consequence**: Horovod wrapped efficient ring reduction behind a small API surface, making distributed training easier to adopt while moving the bottleneck from a central server to bandwidth-optimal collective communication.
