...
class AllReduceCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gradient_size_gb_str = fmt(gradient_size_gb, precision=1, commas=False)
    ...
    n_gpus_napkin_str = fmt(n_gpus, precision=1, commas=False)
...
class CriticalMsgSize:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = fmt(alpha_us, precision=1, commas=False)
    beta_str = fmt(beta_gbs, precision=1, commas=False)
    n_star_kb_str = fmt(n_star_kb, precision=1, commas=False)
```
...
class LatencyVsBw:
    ...
    alpha_str = fmt(alpha_us, precision=1, commas=False)
    beta_str = fmt(beta_gbs, precision=1, commas=False)
    bw_a_us_str = fmt(bw_a_us, precision=1, commas=False)
    total_a_us_str = fmt(total_a_us, precision=1, commas=False)
    lat_pct_a_str = fmt(lat_pct_a, precision=1, commas=False)
    bw_b_us_str = fmt(bw_b_us, precision=1)
    bw_b_ms_str = fmt(bw_b_ms, precision=1, commas=False)
    total_b_us_str = fmt(total_b_us, precision=1)
...
class LogPOverlap:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    L_str = fmt(L_us, precision=1, commas=False)
    two_o_str = fmt(two_o_us, precision=1, commas=False)
    compute_str = fmt(compute_us, precision=1, commas=False)
    effective_str = fmt(effective_us, precision=1, commas=False)
```
...
class LlamaBudgetCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gradient_bf16_gb_str       = fmt(gradient_bf16_gb, precision=1, commas=False)
    ...
    flat_total_ms_str          = fmt(flat_total_ms, precision=1)
    ccr_hier_str               = fmt(ccr_hier, precision=1, commas=False)
    ccr_flat_str               = fmt(ccr_flat, precision=1, commas=False)
    n_gpus_llama_str           = fmt(n_gpus_llama, precision=1, commas=False)
    nvlink_bw_str              = fmt(nvlink_bw, precision=1, commas=False)
    ib_bw_str                  = fmt(ib_bw, precision=1, commas=False)
```
...
class MoeAllToAll:
    ...
    alpha_str = fmt(alpha_us, precision=1, commas=False)
    beta_str = fmt(beta_gbs, precision=1, commas=False)
    tokens_per_gpu_str = fmt(tokens_per_gpu, precision=1, commas=False)
    tokens_per_transfer_str = fmt(tokens_per_transfer, precision=1, commas=False)
    kb_per_token_str = fmt(kb_per_token, precision=3, commas=False)
    tokens_per_expert_str = fmt(tokens_per_expert, precision=1, commas=False)
    ...
    bw_time_approx_str = fmt(bw_time_us_approx, precision=1, commas=False)
    per_transfer_us_str = fmt(per_transfer_us, precision=1, commas=False)
    serialized_us_str = fmt(serialized_us, precision=1, commas=False)
    parallel_us_str = fmt(parallel_us, precision=1, commas=False)
```
...
class RingTrace:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    steps_str = fmt(total_steps, precision=1, commas=False)
    elem_str = fmt(elem_per_step, precision=1, commas=False)
    total_str = fmt(total_per_gpu, precision=1, commas=False)
```
...
class RingVsTree:
    ...
    alpha_str = fmt(alpha_us, precision=1, commas=False)
    beta_str = fmt(beta_gbs, precision=1, commas=False)
    ring_lat_str = fmt(ring_lat_us, precision=1)
    tree_lat_str = fmt(tree_lat_us, precision=1, commas=False)
    ring_bw_str = fmt(ring_bw_us, precision=1, commas=False)
    tree_bw_str = fmt(tree_bw_us, precision=1)
    ring_total_str = fmt(ring_total_us, precision=1)
    tree_total_str = fmt(tree_total_us, precision=1)
...
class CommInfraScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_gbs_str = fmt(nvlink_gbs, precision=1, commas=False)
    ib_ndr_bw_gbs_str = fmt(ib_gbs, precision=1, commas=False)
```
...
class HierarchicalAllreduceCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    flat_time_ms_str  = fmt(flat_time_ms, precision=1, commas=False)
    intra_data_mb_str = fmt(intra_data_mb, precision=1, commas=False)
    intra_rs_ms_str   = fmt(intra_rs_ms, precision=1, commas=False)
    inter_data_mb_str = fmt(inter_data_mb, precision=1, commas=False)
    ...
    ib_bw_gbs_str     = fmt(ib_bw_gbs, precision=1, commas=False)
...
class ThreeLevelNapkin:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_gpus_3l_str    = fmt(total_gpus_3l, precision=1, commas=False)
    gradient_3l_gb_str   = fmt(gradient_3l_gb, precision=1, commas=False)
    ...
    flat_3l_ms_str       = fmt(flat_3l_ms, precision=1, commas=False)
    reduction_factor_str = fmt(reduction_factor, precision=1, commas=False)
    nvlink_bw_3l_str     = fmt(nvlink_bw_3l, precision=1, commas=False)
    ib_bw_3l_str         = fmt(ib_bw_3l, precision=1, commas=False)
    cross_rack_bw_str    = fmt(cross_rack_bw, precision=1, commas=False)
```
...
class ErrorFeedbackTrace:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cum_str = fmt(cum_transmitted, precision=1, commas=False)
    error_str = fmt(final_error, precision=1, commas=False)
    true_str = fmt(true_cumulative, precision=1, commas=False)
    naive_str = fmt(naive_transmitted, precision=1, commas=False)
...
class CompressionPayback:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cp_comm_time_ms_str = fmt(t_comm_ms, precision=1, commas=False)
    cp_ratio_str = fmt(ratio, precision=1, commas=False)
    cp_overhead_ms_str = fmt(t_overhead_ms, precision=1, commas=False)
...
class OverlapBudgetCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_backward_ms_str      = fmt(total_backward_ms, precision=1, commas=False)
...
class NapkinOverlap:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gradient_gb_str = fmt(gradient_gb, precision=1, commas=False)
