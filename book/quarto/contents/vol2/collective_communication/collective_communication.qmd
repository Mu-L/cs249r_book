...
- Apply the alpha-beta model to derive bandwidth and latency bounds across cluster scales
- Compare AllReduce algorithms (ring, tree, hierarchical) by their time complexity and identify the crossover points where each becomes optimal
...
When 10,000 GPUs need to agree on a single weight update, they do not simply broadcast data randomly into the void; they execute rigidly choreographed mathematical exchanges. In the Fleet Stack shown in @fig-fleet-stack, communication algorithms sit squarely in the Distribution Layer.
...
1.  **Significance (quantitative)**: A 70B-parameter model in BF16 generates 140 GB of gradient data per worker per step. Synchronizing across 1,000 GPUs via ring AllReduce at 50 GB/s per link requires approximately $2 \times 140/50 \approx 5.6$ seconds of communication per step, dominating the bandwidth (data-movement) term in the iron law and consuming 40--70 percent of total training time before overlap techniques are applied.
...
3.  **Common pitfall**: A frequent misconception is that gradient synchronization overhead scales with the number of GPUs. In ring AllReduce, per-node communication volume is constant as the cluster grows—the bottleneck is the gradient volume per step (proportional to model size), not the number of participants.
...
# │ How: $\alpha$-$\beta$ model: $T = 2(N-1)\alpha + 2(N-1)/N \times n/\beta$; bare arithmetic (no pint)
# │      because this is a step-by-step pedagogical trace.
# │
# │ Imports: mlsysim.book (check), mlsysim.core.constants (MS_PER_SEC)
...
class AllReduceCost:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    llama_params_b = 70
    bytes_per_param = 2 # BF16 gradients (standard for Llama-class training)
    n_gpus = 64
    ib_bw_gbs = 50 # GB/s NDR 400 Gbps per port
    alpha_us = 3 # microseconds

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    gradient_size_gb = llama_params_b * bytes_per_param

    # Step 1: Ring AllReduce bandwidth term: 2 * (N-1)/N * n/beta
    ring_bw_factor = 2 * (n_gpus - 1) / n_gpus
    ring_bw_time_ms = (ring_bw_factor * gradient_size_gb/ib_bw_gbs) * 1000

    # Step 2: Ring AllReduce latency term: 2*(N-1)*\alpha
    ring_lat_time_ms = (2 * (n_gpus - 1) * alpha_us) / MS_PER_SEC

    # Step 3: Total
    ring_total_ms = ring_bw_time_ms + ring_lat_time_ms

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(ring_total_ms > 5000, f"AllReduce ({ring_total_ms:.0f}ms) should be significant (>5s) for 70B BF16 model.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gradient_size_gb_str = fmt(gradient_size_gb, precision=0, commas=False)
    ring_bw_time_ms_str = fmt(ring_bw_time_ms, precision=0)
    ring_lat_time_ms_str = fmt(ring_lat_time_ms, precision=1, commas=False)
    ring_total_ms_str = fmt(ring_total_ms, precision=0)
    n_gpus_napkin_str = fmt(n_gpus, precision=0, commas=False)
    ring_bw_factor_str = fmt(ring_bw_factor, precision=2, commas=False)
```

::: {#nbk-collective-communication-allreduce-cost-70b-model .callout-notebook title="AllReduce cost for a 70B model"}

**Problem**: A 70 billion parameter model trains with data parallelism across `{python} AllReduceCost.n_gpus_napkin_str` GPUs connected by InfiniBand NDR (50 GB/s per port). Each GPU computes gradients in BF16 (2 bytes per parameter, standard for Llama-class training). How long does one AllReduce take?

**Step 1**: Size the gradient payload.
Each GPU produces a full gradient tensor: $70 \times 10^9 \times 2\ \text{bytes} =$ `{python} AllReduceCost.gradient_size_gb_str` GB.

**Step 2**: Apply the Ring AllReduce bandwidth formula.
$$T_{\text{bandwidth}} = 2 \cdot \frac{N-1}{N} \cdot \frac{n}{\beta}$$

Substituting: $T_{\text{bandwidth}}$ = `{python} AllReduceCost.ring_bw_factor_str`$\times$ 140 GB/50 GB/s $\approx$ `{python} AllReduceCost.ring_bw_time_ms_str` ms.

**Step 3**: Add the latency term.
$$T_{\text{latency}} = 2(N-1) \cdot \alpha$$

Substituting: $T_{\text{latency}} = 2 \times 63 \times 3\ \mu\text{s} =$ `{python} AllReduceCost.ring_lat_time_ms_str` ms.
...
The calculation reveals why data movement, not computation, becomes the governing constraint at scale. A single AllReduce on a 70B model's gradients consumes seconds of wall-clock time, during which all GPUs would otherwise sit idle. This asymmetry between local computation (which parallelizes perfectly) and global coordination (which requires physical data movement) motivates every algorithm in the remainder of this chapter. Halving that multi-second AllReduce would save thousands of GPU-hours over a typical training campaign, translating directly to reduced cost and faster time to deployment.
...
::: {#psp-collective-communication-fleet-stack-connection .callout-perspective title="Fleet stack connection"}
...
ax.set_xlim(0, 105)
ax.invert_yaxis()
ax.set_ylim(len(gpu_counts)-0.5, -0.7) # <-- space above top bar
...
*   Archetype B (DLRM at Scale)—the Deep Learning Recommendation Model (DLRM) workload[^fn-dlrm-embedding]: Our gradient (or activation) is sparse and targeted. It does not need to meet everyone; it needs to find one specific "Expert" GPU across the data center. Its vehicle is the AllToAll.

Understanding this mapping is essential: the *what* of parallelism directly determines the *how* of communication. At frontier scale, these strategies are not mutually exclusive. A single training run for a large language model typically employs 3D parallelism (combining data, tensor, and pipeline parallelism simultaneously), which means multiple collective primitives execute concurrently on overlapping subsets of GPUs. Tensor parallelism drives AllReduce operations within each node (over NVLink), pipeline parallelism drives point-to-point sends between pipeline stages (often between nodes), and data parallelism drives AllReduce operations across groups of nodes (over InfiniBand). Each primitive operates on a different **process group**\index{Process Group}, a subset of the total GPU population that participates in that particular collective. The communication library must manage these overlapping process groups without creating contention between them, a challenge that requires careful allocation of network channels and bandwidth across the concurrent collectives [@thakur2005].
...
|:-----------------------------------------------------|:--------------------------------------|:---------------------------|:----------------------------------|
| **Data Parallelism**\index{Data Parallelism}         | Meet everyone, compute global average | AllReduce                  | Bandwidth (Large payloads)        |
| **FSDP/ZeRO**                                        | Find shards, reconstruct the whole    | AllGather + ReduceScatter  | Bandwidth (High frequency)        |
| **Tensor Parallelism**\index{Tensor Parallelism}     | Quick handshake within the node       | AllReduce                  | Latency (Speed is life)           |
| **Pipeline Parallelism**\index{Pipeline Parallelism} | Handoff to the next neighbor          | Point-to-Point (Send/Recv) | Latency (Sequential dependencies) |
...
### The reality: When physics fights back {#sec-communication-collective-operations-collective-operations-alphabeta-model-f9b4}
...
3.  **Common pitfall**: A frequent misconception is that gradient compression always helps. If the compressed gradient size remains well above $n^*$, compression reduces the bandwidth term but leaves the latency term unchanged—gradient sparsification at 99 percent compression on a 70B model still transfers 1.4 GB per AllReduce, remaining firmly bandwidth-bound and not recovering latency-regime performance.
...
The **critical message size**\index{Critical Message Size} $n^* = \alpha \cdot \beta$ marks the crossover point: messages smaller than $n^*$ are latency-bound; messages larger are bandwidth-bound.
...
| **InfiniBand HDR 200 Gbps**  |                 2–5 μs |                 25 GB/s |                    ~75 KB |
| **PCIe Gen5 (GPU↔CPU)**      |                 2–5 μs |                 64 GB/s |                   ~200 KB |
| **Ethernet 100 Gbps (RoCE)** |                5–10 μs |                 12 GB/s |                   ~100 KB |

: **Interconnect Performance Parameters**\index{Interconnect Performance Parameters}: Typical latency and bandwidth values for modern data center interconnects. The critical size shows the crossover point where communication transitions from latency-bound to bandwidth-bound. {#tbl-interconnect-parameters}
...
class CriticalMsgSize:
    ...
    beta_gbs = 50
    # ┌── 2. EXECUTE ───────────────────────────────────────
    n_star_bytes = alpha_us * 1e-6 * beta_gbs * 1e9
    n_star_kb = round((n_star_bytes * byte).m_as(KB))
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(n_star_kb == 100, f"n* = {n_star_kb} KB, expected 100")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = fmt(alpha_us, precision=0, commas=False)
    beta_str = fmt(beta_gbs, precision=0, commas=False)
    n_star_kb_str = fmt(n_star_kb, precision=0, commas=False)
```

::: {#nbk-collective-communication-critical-message-size .callout-notebook title="The critical message size"}

**Problem**: Consider a cluster using InfiniBand NDR 400 Gbps with $\alpha =$ `{python} CriticalMsgSize.alpha_str` $\mu$s and $\beta =$ `{python} CriticalMsgSize.beta_str` GB/s. At what message size does optimizing for bandwidth start to matter more than optimizing for latency?

**Math**:

$n^* = \alpha \cdot \beta = 2 \times 10^{-6}$ s $\times 50 \times 10^9$ B/s $=$ `{python} CriticalMsgSize.n_star_kb_str` KB

**Systems insight**: Messages under `{python} CriticalMsgSize.n_star_kb_str` KB (like MoE tokens, pipeline activations) are **latency-bound**\index{Latency!bound}: buy lower-latency switches and reduce software overhead. Messages over `{python} CriticalMsgSize.n_star_kb_str` KB, such as large language model (LLM) gradients, are **bandwidth-bound**\index{Bandwidth!bound}: buy more bandwidth and compress the data. Applying the wrong optimization wastes money without improving performance.
...
2.  **Bandwidth-Bound** $(n \gg n^*)$: For large messages (for example, LLM gradients, optimizer states), time is dominated by $n/\beta$. Optimization focuses on **compression** \index{Compression} (FP8, Top-k sparsity), **algorithm choice** (Ring vs. Tree), and **link aggregation** (multi-rail NICs).

A direct comparison of two message sizes illustrates how the bottleneck shifts between these regimes:

```{python}
#| label: latency-vs-bw-calc
#| echo: false
from mlsysim.fmt import check, fmt
from mlsysim.core.constants import MS_PER_SEC
...
class LatencyVsBw:
    ...
    alpha_us = 2
    beta_gbs = 50
    msg_a_bytes = 1e6     # 1 MB
    msg_b_bytes = 1e9     # 1 GB
    # ┌── 2. EXECUTE ───────────────────────────────────────
    bw_a_us = msg_a_bytes / (beta_gbs * 1e9) * 1e6
    total_a_us = bw_a_us + alpha_us
    lat_pct_a = alpha_us/total_a_us * 100
    bw_b_us = msg_b_bytes / (beta_gbs * 1e9) * 1e6
    bw_b_ms = bw_b_us / MS_PER_SEC
    total_b_us = bw_b_us + alpha_us
    lat_pct_b = alpha_us/total_b_us * 100
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(bw_a_us == 20, f"Case A bw = {bw_a_us} μs, expected 20")
    check(total_a_us == 22, f"Case A total = {total_a_us} μs, expected 22")
    check(bw_b_us == 20000, f"Case B bw = {bw_b_us} μs, expected 20000")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = fmt(alpha_us, precision=0, commas=False)
    beta_str = fmt(beta_gbs, precision=0, commas=False)
    bw_a_us_str = fmt(bw_a_us, precision=0, commas=False)
    total_a_us_str = fmt(total_a_us, precision=0, commas=False)
    lat_pct_a_str = fmt(lat_pct_a, precision=0, commas=False)
    bw_b_us_str = fmt(bw_b_us, precision=0)
    bw_b_ms_str = fmt(bw_b_ms, precision=0, commas=False)
    total_b_us_str = fmt(total_b_us, precision=0)
    lat_pct_b_str = fmt(lat_pct_b, precision=2, commas=False)
```

::: {#nbk-collective-communication-latency-vs-bandwidth-dominance .callout-notebook title="Latency vs. bandwidth dominance"}

**Problem**: Consider synchronizing a 1 MB buffer vs. a 1 GB buffer on InfiniBand NDR with $\alpha =$ `{python} LatencyVsBw.alpha_str` $\mu$s and $\beta =$ `{python} LatencyVsBw.beta_str` GB/s. How does the bottleneck shift?

**Case A: 1 MB Message**

*   Bandwidth Time: $10^6 / (50 \times 10^{9})$ = `{python} LatencyVsBw.bw_a_us_str` $\mu\text{s}$.
*   Latency Time: $2\ \mu s$.
*   **Total**: `{python} LatencyVsBw.total_a_us_str` μs. Latency is `{python} LatencyVsBw.lat_pct_a_str` percent of total, still meaningful.

**Case B: 1 GB Message**

*   Bandwidth Time: $10^9 / (50 \times 10^{9})$ = `{python} LatencyVsBw.bw_b_us_str` $\mu\text{s}$ = `{python} LatencyVsBw.bw_b_ms_str` ms.
*   Latency Time: $2\ \mu s$.
*   **Total**: `{python} LatencyVsBw.total_b_us_str` μs. Latency is `{python} LatencyVsBw.lat_pct_b_str` percent, completely negligible.

**Systems insight**: For Data Parallelism (large gradients), the target is $\beta$: compress gradients and add NICs. For Pipeline/Expert Parallelism (small activations), every microsecond of $\alpha$ matters: kernel bypass and topology optimization. The $\alpha$-$\beta$ model identifies *which constraint to optimize*.

:::

The same distinction between latency-bound and bandwidth-bound communication is the diagnostic skill to practice.
...
- [ ] A message of 10 KB is being sent over a link where $\alpha=2 \mu s$ and $\beta=10 GB/s$. Is this message **Latency-Bound** or **Bandwidth-Bound**?
- [ ] If you reduce the "Software Overhead" ($\alpha$) by half, which type of parallelism benefits more: Data Parallelism or Tensor Parallelism?
- [ ] What happens to the critical message size ($n^* = \alpha \beta$) as you upgrade from 100 Gbps to 400 Gbps networking while keeping software latency constant?
- [ ] True or False: In the bandwidth-bound regime, doubling the number of NICs per node effectively doubles the transmission speed for large tensors.

:::

### The LogP model {#sec-communication-collective-operations-collective-operations-logp-model-e45d}

The α-β model assumes the processor is idle during communication. For **pipelined systems** where we overlap communication with computation, this assumption fails. The **LogP** model [@culler1993] extends α-β with two additional parameters:

*   **$L_{\text{lat}}$ (Latency)**: The time for a message to traverse the network (similar to $\alpha$).
*   **$o$ (Overhead)**: The CPU/GPU time spent initiating or receiving a transfer. During this time, the processor **cannot compute**\index{Cannot Compute}, making this the nonoverlappable cost.
...
*   **$N$ (Processors)**: The number of processors in the system.

LogP distinguishes **network latency** (L, which can be hidden) from **processor overhead** (o, which *cannot*). A system can overlap communication with computation only if the compute kernel runs longer than the overhead. A small overlap calculation makes this distinction concrete:
...
class LogPOverlap:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    two_o_us = 2 * o_us
    effective_us = two_o_us + max(compute_us, L_us)
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(two_o_us == 100, f"2o = {two_o_us}, expected 100")
    check(effective_us == 600, f"Effective = {effective_us}, expected 600")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    L_str = fmt(L_us, precision=0, commas=False)
    two_o_str = fmt(two_o_us, precision=0, commas=False)
    compute_str = fmt(compute_us, precision=0, commas=False)
    effective_str = fmt(effective_us, precision=0, commas=False)
```
...
**Problem**: A training pipeline attempts to overlap gradient AllReduce with the next layer's backward pass. The backward pass takes `{python} LogPOverlap.compute_str` μs. The AllReduce has network latency $L_{\text{lat}} = 100\ \mu\text{s}$ but processor overhead $o = 50\ \mu s$ to initiate and $o = 50\ \mu s$ to receive. Can the communication be hidden?

**Math**:

1.  **Overlappable portion**\index{Overlappable Portion}: Network latency $L_{\text{lat}}$ = `{python} LogPOverlap.L_str` $\mu\text{s}$ (data in flight while GPU computes).
2.  **Non-overlappable portion**\index{Non-Overlappable Portion}: $2o$ = `{python} LogPOverlap.two_o_str` $\mu\text{s}$ (GPU busy initiating/receiving).
3.  **Compute available**\index{Compute Available}: `{python} LogPOverlap.compute_str` $\mu\text{s}$.
4.  **Hidden**\index{Hidden}: All `{python} LogPOverlap.L_str` μs of $L_{\text{lat}}$ can overlap with compute.
5.  **Exposed**\index{Exposed}: The `{python} LogPOverlap.two_o_str` μs of $o$ cannot overlap.

**Effective time**\index{Effective Time}: `{python} LogPOverlap.two_o_str` $\mu$s + max(`{python} LogPOverlap.compute_str`, `{python} LogPOverlap.L_str`) = `{python} LogPOverlap.effective_str` $\mu$s. The network latency is hidden, but the processor overhead remains exposed. @fig-comm-compute-overlap shows this overlap visually.
...
# │
# │ Imports: mlsysim.core.constants (MS_PER_SEC, NVLINK_H100_BW,
...
# │          nvlink_bw_str, ib_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MS_PER_SEC, NVLINK_H100_BW, INFINIBAND_NDR_BW, GB, second
...
class LlamaBudgetCalc:
    ...
    nvlink_bw   = NVLINK_H100_BW.m_as(GB/second)         # GB/s (H100 NVLink 4.0)
    nvlink_alpha = 1.5                                    # us
    ib_bw        = INFINIBAND_NDR_BW.m_as(GB/second)        # GB/s per port (NDR 400 Gbps)
    ib_alpha     = 3                                      # us
    ...
    inter_ring_lat_time_ms    = 2 * (n_nodes_llama - 1) * ib_alpha / MS_PER_SEC
    ...
    flat_ring_lat_ms = 2 * (n_gpus_llama - 1) * ib_alpha / MS_PER_SEC
    ...
    check(flat_total_ms > hier_total_llama_ms, "Flat AllReduce should be much slower than Hierarchical")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gradient_bf16_gb_str       = fmt(gradient_bf16_gb, precision=0, commas=False)
    intra_rs_time_ms_str       = fmt(intra_rs_time_ms, precision=1, commas=False)
    inter_payload_gb_str       = fmt(inter_payload_gb, precision=1, commas=False)
    inter_ring_bw_time_ms_str  = fmt(inter_ring_bw_time_ms, precision=0, commas=False)
    inter_ring_lat_time_ms_str = fmt(inter_ring_lat_time_ms, precision=2, commas=False)
    intra_ag_time_ms_str       = fmt(intra_ag_time_ms, precision=1, commas=False)
    hier_total_llama_ms_str    = fmt(hier_total_llama_ms, precision=0, commas=False)
    flat_total_ms_str          = fmt(flat_total_ms, precision=0)
    ccr_hier_str               = fmt(ccr_hier, precision=1, commas=False)
    ccr_flat_str               = fmt(ccr_flat, precision=1, commas=False)
    n_gpus_llama_str           = fmt(n_gpus_llama, precision=0, commas=False)
    nvlink_bw_str              = fmt(nvlink_bw, precision=0, commas=False)
    ib_bw_str                  = fmt(ib_bw, precision=0, commas=False)
```

Using BF16 gradients (a standard practice that halves communication volume to `{python} LlamaBudgetCalc.gradient_bf16_gb_str` GB), the hierarchical AllReduce decomposes as follows:

1. **Intra-Node ReduceScatter** (NVLink at `{python} LlamaBudgetCalc.nvlink_bw_str` GB/s): Each GPU exchanges 7/8 of the gradient locally. Time: `{python} LlamaBudgetCalc.intra_rs_time_ms_str` ms.
2. **Inter-Node Ring AllReduce** (InfiniBand NDR at `{python} LlamaBudgetCalc.ib_bw_str` GB/s): Each GPU synchronizes only `{python} LlamaBudgetCalc.inter_payload_gb_str` GB across 16 nodes. Bandwidth time: `{python} LlamaBudgetCalc.inter_ring_bw_time_ms_str` ms. Latency time: `{python} LlamaBudgetCalc.inter_ring_lat_time_ms_str` ms.
3. **Intra-Node AllGather** (NVLink): Distributes the final result locally. Time: `{python} LlamaBudgetCalc.intra_ag_time_ms_str` ms.

The total hierarchical AllReduce takes approximately `{python} LlamaBudgetCalc.hier_total_llama_ms_str` ms, compared to `{python} LlamaBudgetCalc.flat_total_ms_str` ms for a flat Ring AllReduce that ignores the bandwidth hierarchy. This difference is not marginal; it determines whether communication can be hidden behind computation or whether it becomes the critical path.

### Theory vs. practice: The NCCL reality gap {#sec-communication-nccl-reality}

The bandwidth-latency trade-off (Principle \ref{pri-alpha-beta-model}) provides useful first-order predictions, but real communication libraries introduce overheads that the idealized $\alpha$-$\beta$ model does not capture. NCCL, the dominant GPU communication library, adds protocol negotiation, memory registration, and internal pipelining that modify the effective alpha and beta values. @tbl-nccl-vs-theory compares one-message alpha-beta payload predictions against measured NCCL performance for common message sizes on an 8-node DGX H100 cluster (64 GPUs, InfiniBand NDR 400G).
...
1.  **Significance (quantitative)**: The right collective algorithm determines whether communication scales with cluster size or remains constant. Ring AllReduce achieves bandwidth-optimal $2(N-1)/N \times n / \beta$ per node (constant in $N$), while a naive reduce-then-broadcast approach costs $O(N \times n / \beta)$, making it 500$\times$ worse for $N=1024$. Algorithm selection thus directly determines whether the $\text{BW}$ term in the iron law is the training bottleneck.
...
3.  **Common pitfall**: A frequent misconception is that one collective algorithm suits all message sizes. Ring AllReduce achieves near-optimal bandwidth for large gradient tensors but has $O(N)$ latency—for small messages like MoE routing decisions (~4 KB), a tree-based algorithm reduces latency to $O(\log N)$ steps at the cost of suboptimal bandwidth utilization (the bandwidth-optimal recursive halving-doubling alternative is covered in @sec-communication-butterfly), requiring workload-specific algorithm selection rather than a single default.
...
[^fn-fsdp-memory]: **FSDP (Fully Sharded Data Parallel)**: PyTorch's implementation of ZeRO-3, which shards parameters, gradients, and optimizer states across $N$ workers, reducing per-GPU memory from $16P$ bytes under FP32 Adaptive Moment Estimation (Adam) to $16P/N$ bytes. The trade-off is communication frequency: FSDP issues $2L_{\text{layers}}$ collective operations per step (AllGather + ReduceScatter per layer) instead of data parallelism's single AllReduce, making it more sensitive to the $\alpha$ overhead. \index{FSDP!memory trade-off}
...
In an AllToAll, every process has a unique piece of data for every other process. This creates $O(N^2)$ logical connections. At the hardware level, this leads to **network contention**\index{Network Contention}: if 1024 GPUs all try to send data to different targets simultaneously, the "Fat-Tree" or "Spine" switches in the data center become the bottleneck.

This is why **Expert Parallelism (MoE)** and large-scale **Recommendation Systems**\index{Recommendation Systems} often hit a "communication wall" much earlier than standard data-parallel models. The algorithm choice (AllReduce vs. AllToAll) determines the scaling ceiling.
...
class MoeAllToAll:
    ...
    n_experts = 64
    alpha_us = 3                  # IB NDR latency
    beta_gbs = 50                 # IB NDR bandwidth
    # ┌── 2. EXECUTE ───────────────────────────────────────
    tokens_per_gpu = total_tokens // n_gpus
    experts_per_gpu = n_experts // n_gpus
    bytes_per_token = hidden_dim * bytes_per_elem
    kb_per_token = bytes_per_token / 1000
    tokens_per_expert = total_tokens // n_experts
    tokens_per_transfer = tokens_per_gpu // n_gpus
    remote_gpus = n_gpus - 1
    data_per_transfer_kb = tokens_per_transfer * kb_per_token
    total_sent_kb = remote_gpus * data_per_transfer_kb
    total_sent_mb = total_sent_kb / 1000
    data_per_transfer_bytes = tokens_per_transfer * bytes_per_token
    bw_time_us = data_per_transfer_bytes / (beta_gbs * 1e9) * 1e6
    bw_time_us_approx = round(bw_time_us)
    per_transfer_us = alpha_us + bw_time_us_approx
    serialized_us = remote_gpus * per_transfer_us
    parallel_us = per_transfer_us
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(tokens_per_gpu == 512, f"tokens/GPU = {tokens_per_gpu}")
    check(abs(kb_per_token - 4.096) < 1e-9, f"KB/token = {kb_per_token}")
    check(tokens_per_expert == 64, f"tokens/expert = {tokens_per_expert}")
    check(abs(data_per_transfer_kb - 262.144) < 1e-9, f"data/transfer = {data_per_transfer_kb} KB")
    check(per_transfer_us == 8, f"T/transfer = {per_transfer_us} μs")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = fmt(alpha_us, precision=0, commas=False)
    beta_str = fmt(beta_gbs, precision=0, commas=False)
    tokens_per_gpu_str = fmt(tokens_per_gpu, precision=0, commas=False)
    tokens_per_transfer_str = fmt(tokens_per_transfer, precision=0, commas=False)
    kb_per_token_str = fmt(kb_per_token, precision=3, commas=False)
    tokens_per_expert_str = fmt(tokens_per_expert, precision=0, commas=False)
    data_per_transfer_kb_str = fmt(data_per_transfer_kb, precision=3, commas=False)
    total_sent_mb_str = fmt(total_sent_mb, precision=2, commas=False)
    bw_time_approx_str = fmt(bw_time_us_approx, precision=0, commas=False)
    per_transfer_us_str = fmt(per_transfer_us, precision=0, commas=False)
    serialized_us_str = fmt(serialized_us, precision=0, commas=False)
    parallel_us_str = fmt(parallel_us, precision=0, commas=False)
```

::: {#nbk-collective-communication-alltoall-moe-token-routing .callout-notebook title="AllToAll for MoE token routing"}

**Problem**: A MoE model processes a batch of 4096 tokens across 8 GPUs (`{python} MoeAllToAll.tokens_per_gpu_str` tokens per GPU). Each token is a 2048-dimensional hidden state in BF16 (`{python} MoeAllToAll.kb_per_token_str` KB per token). The gating network assigns each token to exactly 1 of 64 experts (8 experts per GPU). Assuming uniform routing (each expert receives $4096/64$ = `{python} MoeAllToAll.tokens_per_expert_str` tokens), how much data does each GPU send and receive?

**Math**:

Each GPU holds `{python} MoeAllToAll.tokens_per_gpu_str` tokens that need to reach 64 different experts across 8 GPUs. With uniform routing, each GPU sends $512/8$ = `{python} MoeAllToAll.tokens_per_transfer_str` tokens to each of the other 7 GPUs (keeping `{python} MoeAllToAll.tokens_per_transfer_str` tokens local).

*   Data per GPU-to-GPU transfer: `{python} MoeAllToAll.tokens_per_transfer_str` tokens $\times$ `{python} MoeAllToAll.kb_per_token_str` KB/token = `{python} MoeAllToAll.data_per_transfer_kb_str` KB
*   Total data sent per GPU: 7 $\times$ `{python} MoeAllToAll.data_per_transfer_kb_str` KB = `{python} MoeAllToAll.total_sent_mb_str` MB
*   Total data received per GPU: `{python} MoeAllToAll.total_sent_mb_str` MB (symmetric)

**Latency Analysis (InfiniBand NDR, $\alpha =$ `{python} MoeAllToAll.alpha_str` $\mu$s, $\beta =$ `{python} MoeAllToAll.beta_str` GB/s)**:

Each of the 7 transfers is a `{python} MoeAllToAll.data_per_transfer_kb_str` KB message. From the alpha-beta model: $T$ = 3 + `{python} MoeAllToAll.bw_time_approx_str` = `{python} MoeAllToAll.per_transfer_us_str` $\mu\text{s}$ per transfer.

If serialized: 7 $\times$ `{python} MoeAllToAll.per_transfer_us_str` $\mu\text{s}$ = `{python} MoeAllToAll.serialized_us_str` $\mu\text{s}$. If all 7 transfers run in parallel (full-duplex, non-blocking): $\approx$ `{python} MoeAllToAll.parallel_us_str` $\mu\text{s}$.
...
The consequence is a trade-off between memory and communication frequency. Standard data parallelism communicates once per training step (a single AllReduce of the full gradient). FSDP communicates twice per layer per step (AllGather in forward, ReduceScatter in backward), but each communication is smaller (only that layer's parameters, sharded across ranks). For a model with $L_{\text{layers}}$ layers, FSDP issues $2L_{\text{layers}}$ collective operations per step instead of 1; across all those operations, the total communication volume can be comparable to the single full-gradient AllReduce, but it is spread across many smaller operations.

The higher operation count makes FSDP more sensitive to latency ($\alpha$) than standard data parallelism. If each of the $2L_{\text{layers}}$ collectives pays the full NCCL startup overhead (25--50 $\mu$s per operation from @tbl-nccl-vs-theory), the aggregate overhead for a 100-layer model is 5--10 ms, which can represent a meaningful fraction of step time. FSDP implementations mitigate this through prefetching\index{Prefetching} (launching the next layer's AllGather while the current layer is computing) and **communication stream pipelining** (using dedicated CUDA streams for communication that overlap with compute streams).
...
*   **Bottleneck**\index{Bottleneck}: Rank 0's bandwidth. It must receive $N \times M$ bytes and send $N \times M$ bytes.
*   **Time**\index{Time}: $T \propto N \times M / \beta$.
*   **Result**\index{Result}: Linear slowdown. With 1000 GPUs, rank 0 melts.
...
**The context**: Uber's Michelangelo platform needed TensorFlow training jobs to scale across many GPUs without forcing every model author to rewrite large portions of training code.

**The failure**: Existing TensorFlow multi-GPU approaches imposed communication overhead and heavy code changes, while parameter-server patterns concentrated gradient traffic through a central tier.

**The consequence**: Horovod wrapped efficient ring reduction behind a small API surface, making distributed training easier to adopt while moving the bottleneck from a central server to bandwidth-optimal collective communication.

**The systems lesson**: A collective algorithm is also a developer interface. Scaling improves when the communication primitive is both bandwidth efficient and easy enough that teams actually use it correctly [@sergeev2018horovod].
...
1.  **Scatter-Reduce Phase**\index{Scatter-Reduce Phase}: In $N-1$ steps, chunks flow around the ring, accumulating sums. At the end, each node holds the complete sum for *one* chunk ($1/N$ of the total result).
2.  **AllGather Phase**\index{AllGather Phase}: In $N-1$ steps, the partial sums circulate the ring until every node has *all* chunks.

The data flow during Step 1 is illustrated in @fig-ring-allreduce.
...
**Setup**\index{Setup}: 4 GPUs in a ring $(0 \to 1 \to 2 \to 3 \to 0)$. Each GPU holds a vector of 4 values. We use concrete numbers for clarity:
...
: **Ring AllReduce Scatter-Reduce trace**: Three sequential steps across 4 GPUs. Each row shows the chunk and value every GPU forwards to its right neighbor and the resulting partial sums after the receive-and-add. By step 3, each GPU owns the complete sum of one chunk. {#tbl-collective-communication-ring-scatter-reduce-trace}
...
: **Ring AllReduce AllGather trace**: Three sequential steps in which each GPU forwards its fully-reduced chunk around the ring. After step 6 every GPU holds the same final vector $[10, 26, 18, 34]$. {#tbl-collective-communication-ring-allgather-trace}

**Total data movement per GPU**\index{Total Data Movement Per GPU}: In each of the $2(N-1)$ = `{python} RingTrace.steps_str` steps, each GPU sends exactly $M/N$ = `{python} RingTrace.elem_str` element. Total per GPU: `{python} RingTrace.steps_str` $\times$ `{python} RingTrace.elem_str` = `{python} RingTrace.total_str` elements sent, `{python} RingTrace.total_str` received.

:::

The trace leaves two mechanics to verify before moving on: the $2(N-1)$ step count and the fact that every rank sends and receives in each step.
...
- [ ] In a ring of $N$ GPUs, how many **sequential steps**\index{Sequential Steps} are required to complete the full AllReduce?
- [ ] Why is Ring AllReduce considered bandwidth-optimal? How many times does each byte of the total message $M$ traverse the network per GPU?
- [ ] If one GPU in the ring is 50 percent slower than the others, what is the impact on the total completion time of the collective?
...
1.  **Reduce Phase**\index{Reduce Phase}: Leaves send to parents, parents sum and send up. Reaches root in $\log_2 N$ steps.
2.  **Broadcast Phase**\index{Broadcast Phase}: Root sends result down to leaves in another $\log_2 N$ steps.
...
The resulting time complexity depends on the specific tree variant. A simple reduce-then-broadcast tree has logarithmic latency but nonuniform traffic and root/interior bottlenecks; recursive-doubling-style variants can incur $O(\log N)$ full-message bandwidth per rank. The simplified model below captures the latency advantage and the potential bandwidth penalty of tree-like collectives:
...
Recursive halving-doubling is used in MPI-style collective implementations and is useful as a theoretical point in the latency-bandwidth trade-off. NCCL's documented algorithm choices instead include Ring, Tree/double-tree-derived variants, CollNet, NVLS/NVLSTree, and PAT depending on version and topology. For clusters with thousands of GPUs, nonlocal communication patterns can create contention on shared network links that erodes theoretical advantages.
...
@tbl-allreduce-comparison summarizes the four algorithms and their performance characteristics. As @fig-allreduce-crossover illustrates, the choice between algorithms depends on message size: Tree dominates for small messages while Ring wins for large ones.
...
: **AllReduce Algorithm Comparison**\index{AllReduce!algorithm comparison}: Each algorithm occupies a different point in the latency-bandwidth trade-off space. Ring and Butterfly achieve bandwidth optimality with different latency characteristics, while Double Binary Tree provides the best practical compromise. {#tbl-allreduce-comparison}
...
t_tree = 2 * np.log2(N) * latency_sec + (2 * np.log2(N)) * (message_sizes/bandwidth_bytes_sec)
...
As @fig-allreduce-crossover illustrates, the choice between Ring and Tree depends on the crossover point derived by setting $T_{\text{ring}} = T_{\text{tree}}$ and solving for $M$.
...
For a cluster with $\alpha=5 \mu s$, $\beta=50$ GB/s, and $N=100$, the direct approximation gives:
...
#| echo: false
from mlsysim.fmt import check, fmt
from mlsysim.core.constants import US, second, GB, MB
import math
...
class RingVsTree:
    ...
    ring_lat_us = 2 * (N - 1) * alpha_us
    tree_lat_us = 2 * log2N * alpha_us
    ring_bw_us = 2 * (N - 1) / N * M_mb/beta_gbs * 1e3
    tree_bw_us = 2 * log2N * M_mb/beta_gbs * 1e3
    ring_total_us = ring_lat_us + ring_bw_us
    tree_total_us = tree_lat_us + tree_bw_us
    crossover_mb = (N * (alpha_us * US) * (beta_gbs * GB / second) / log2N).m_as(MB)
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(ring_lat_us == 1260, f"Ring lat = {ring_lat_us}")
    check(tree_lat_us == 120, f"Tree lat = {tree_lat_us}")
    check(abs(ring_bw_us - 200) < 5, f"Ring bw = {ring_bw_us}")
    check(abs(tree_bw_us - 1200) < 1, f"Tree bw = {tree_bw_us}")
    check(abs(crossover_mb - 1.1) < 0.05, f"Crossover = {crossover_mb}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = fmt(alpha_us, precision=0, commas=False)
    beta_str = fmt(beta_gbs, precision=0, commas=False)
    ring_lat_str = fmt(ring_lat_us, precision=0)
    tree_lat_str = fmt(tree_lat_us, precision=0, commas=False)
    ring_bw_str = fmt(ring_bw_us, precision=0, commas=False)
    tree_bw_str = fmt(tree_bw_us, precision=0)
    ring_total_str = fmt(ring_total_us, precision=0)
    tree_total_str = fmt(tree_total_us, precision=0)
    crossover_str = fmt(crossover_mb, precision=1, commas=False)
```

::: {#nbk-collective-communication-ring-vs-tree-crossover .callout-notebook title="The ring vs. tree crossover"}

**Problem**: A cluster synchronizes a **1 MB** buffer across **64 GPUs**. The network has latency $\alpha =$ `{python} RingVsTree.alpha_str` $\mu$s and bandwidth $\beta =$ `{python} RingVsTree.beta_str` GB/s. Which algorithm performs better: Ring or Tree?

**Math**:

**Latency**:

1.  **Ring Latency**\index{Latency!ring}: $2(N-1)\alpha = 2 \times 63 \times 10\ \mu$s = **`{python} RingVsTree.ring_lat_str` $\mu$s**.
2.  **Tree Latency**\index{Latency!tree}: $2(\log_2 N)\alpha = 2 \times 6 \times 10\ \mu$s = **`{python} RingVsTree.tree_lat_str` $\mu$s**.

**Bandwidth** (note the difference):

3.  **Ring Bandwidth**\index{Bandwidth!ring}: $2\frac{N-1}{N}\frac{M}{\beta} \approx 2 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = **`{python} RingVsTree.ring_bw_str` $\mu$s** (optimal: each byte sent once).
4.  **Tree Bandwidth**\index{Bandwidth!tree}: $2\log_2 N \cdot \frac{M}{\beta} = 12 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = **`{python} RingVsTree.tree_bw_str` $\mu$s** (each level sends full message).

@tbl-collective-communication-ring-vs-tree-1mb-64gpu sums the latency and bandwidth contributions into a head-to-head comparison\index{Total Time}:

| **Algorithm** |                           **Latency** |                        **Bandwidth** |                                   **Total** |
|:--------------|--------------------------------------:|-------------------------------------:|--------------------------------------------:|
| **Ring**      | `{python} RingVsTree.ring_lat_str` μs | `{python} RingVsTree.ring_bw_str` μs | **`{python} RingVsTree.ring_total_str` μs** |
| **Tree**      | `{python} RingVsTree.tree_lat_str` μs | `{python} RingVsTree.tree_bw_str` μs | **`{python} RingVsTree.tree_total_str` μs** |
: **Ring vs. tree AllReduce timing for a 1 MB buffer on 64 GPUs**: Latency, bandwidth, and total-time terms with $\alpha = 10\ \mu$s and $\beta = 10$ GB/s. Tree wins by roughly 10 percent because the message sits just below the log-aware crossover point $M_{\text{crossover}} \approx N \alpha \beta / \log_2 N$. {#tbl-collective-communication-ring-vs-tree-1mb-64gpu}

**Tree wins**\index{Tree Wins}, but only by 10 percent. For this 1 MB message, we are near the crossover point.

**Systems insight**: Ring's latency penalty (10$\times$ worse than Tree) nearly balances Tree's bandwidth penalty (6$\times$ worse than Ring). The log-aware crossover estimate predicts $M_{\text{crossover}} \approx N \alpha \beta / \log_2 N = 64 \times 10\ \mu s \times 10\ \text{GB/s} / 6 \approx$ `{python} RingVsTree.crossover_str` MB. At 1 MB, we are just below crossover, so Tree wins. At 10 MB, Ring would dominate.
...
# │ Exports: nvlink_h100_bw_gbs_str, ib_ndr_bw_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
...
class CommInfraScenario:
    ...
    ib_gbs = ib_ndr_raw.m_as(GB/second)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt
    check(nvlink_gbs > ib_gbs, "NVLink bandwidth should exceed InfiniBand bandwidth")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_gbs_str = fmt(nvlink_gbs, precision=0, commas=False)
    ib_ndr_bw_gbs_str = fmt(ib_gbs, precision=0, commas=False)
```

A GPU node delivers an order of magnitude more intra-node bandwidth (NVLink at `{python} CommInfraScenario.nvlink_h100_bw_gbs_str` GB/s) than inter-node bandwidth (InfiniBand at `{python} CommInfraScenario.ib_ndr_bw_gbs_str` GB/s).

### Hierarchical AllReduce {#sec-communication-collective-operations-collective-operations-hierarchical-allreduce-1338}

The algorithms above (Ring, Tree, Butterfly, Double Binary Tree) all assume a **flat** network where every link has the same bandwidth. This assumption holds within a single node (where all GPUs are connected by NVLink at equal bandwidth) but fails by 18$\times$ in multi-node clusters. Real clusters are **hierarchical**\index{Hierarchical}, with fundamentally different bandwidths at each tier, as @tbl-bandwidth-hierarchy quantifies:
...
The net effect: inter-node traffic is reduced by a factor of $G$ (GPUs per node), effectively **multiplying the apparent inter-node bandwidth by $G$**. A simple 8-node budget makes this bandwidth multiplication concrete:
...
# │ Show: speedup_str ≈ 6.3×—inline in callout insight sentence.
# │ How: Flat: T = 2M/β_IB; Hierarchical: T = T_rs + T_inter + T_ag with
# │      NVLink for intra-node phases and IB for inter-node phase.
# │
# │ Imports: mlsysim.core.constants (MB, GB, second, ms, NVLINK_H100_BW,
# │          INFINIBAND_NDR_BW, MS_PER_SEC)
# │ Exports: flat_time_ms_str, intra_data_mb_str, intra_rs_ms_str,
# │          inter_data_mb_str, inter_time_ms_str, intra_ag_ms_str,
# │          hier_total_ms_str, speedup_str, ib_bw_gbs_str, nvlink_bw_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MB, GB, second, ms, NVLINK_H100_BW, INFINIBAND_NDR_BW, MS_PER_SEC
from mlsysim.fmt import fmt
...
class HierarchicalAllreduceCalc:
    ...
    ib_bw_gbs      = INFINIBAND_NDR_BW.m_as(GB/second)        # InfiniBand NDR 400 Gbps per port
    nvlink_bw_gbs  = NVLINK_H100_BW.m_as(GB/second)            # NVLink 4.0 (H100)
    ib_alpha_us    = 3
    ...
    intra_rs_ms   = ((intra_data_mb * MB) / (nvlink_bw_gbs * GB / second)).m_as(ms)

    inter_data_mb = int(gradient_gb * 1000 / gpus_per_node)  # 125 MB
    inter_ring_factor = 2 * (n_nodes_har - 1) / n_nodes_har
    inter_bw_ms = ((inter_ring_factor * inter_data_mb * MB) / (ib_bw_gbs * GB / second)).m_as(ms)
    inter_lat_ms = 2 * (n_nodes_har - 1) * ib_alpha_us / MS_PER_SEC
    inter_time_ms = inter_bw_ms + inter_lat_ms
    ...
    check(abs(inter_time_ms - 4.4) < 0.1, f"Inter-node time = {inter_time_ms:.1f} ms, expected ~4.4")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    flat_time_ms_str  = fmt(flat_time_ms, precision=0, commas=False)
    intra_data_mb_str = fmt(intra_data_mb, precision=0, commas=False)
    intra_rs_ms_str   = fmt(intra_rs_ms, precision=0, commas=False)
    inter_data_mb_str = fmt(inter_data_mb, precision=0, commas=False)
    inter_time_ms_str = fmt(inter_time_ms, precision=0, commas=False)
    intra_ag_ms_str   = fmt(intra_ag_ms, precision=0, commas=False)
    hier_total_ms_str = fmt(hier_total_ms, precision=0, commas=False)
    ib_bw_gbs_str     = fmt(ib_bw_gbs, precision=0, commas=False)
...
**Problem**: A cluster has 8 nodes, each with 8 GPUs (64 GPUs total). The system must AllReduce a 1 GB gradient buffer. Compare flat Ring AllReduce vs. Hierarchical AllReduce.
...
- **Effective bandwidth**\index{Bandwidth!effective}: Limited by the slowest link = `{python} HierarchicalAllreduceCalc.ib_bw_gbs_str` GB/s (InfiniBand).
- **Time**: $\approx 2 \times 1\ \text{GB} / 50\ \text{GB/s}$ = `{python} HierarchicalAllreduceCalc.flat_time_ms_str` ms (bandwidth term dominates).

**Hierarchical AllReduce (3-step decomposition)**:

1. **Intra-node ReduceScatter**: Each GPU sends `{python} HierarchicalAllreduceCalc.intra_data_mb_str` MB at `{python} HierarchicalAllreduceCalc.nvlink_bw_gbs_str` GB/s → ~`{python} HierarchicalAllreduceCalc.intra_rs_ms_str` ms
2. **Inter-Node AllReduce**\index{AllReduce!inter-node}: Each GPU AllReduces a $1\ \text{GB}/8$ = `{python} HierarchicalAllreduceCalc.inter_data_mb_str` MB shard over InfiniBand; Ring moves roughly $2(N-1)/N$ times that payload, giving ~`{python} HierarchicalAllreduceCalc.inter_time_ms_str` ms at `{python} HierarchicalAllreduceCalc.ib_bw_gbs_str` GB/s

   (Only 1/8 of the data crosses InfiniBand!)

3. **Intra-node AllGather**: Each GPU receives `{python} HierarchicalAllreduceCalc.intra_data_mb_str` MB at `{python} HierarchicalAllreduceCalc.nvlink_bw_gbs_str` GB/s → ~`{python} HierarchicalAllreduceCalc.intra_ag_ms_str` ms
4. **Total time**: $\approx$ `{python} HierarchicalAllreduceCalc.intra_rs_ms_str` + `{python} HierarchicalAllreduceCalc.inter_time_ms_str` + `{python} HierarchicalAllreduceCalc.intra_ag_ms_str` = `{python} HierarchicalAllreduceCalc.hier_total_ms_str` ms

**Systems insight**: Hierarchical AllReduce achieves a **`{python} HierarchicalAllreduceCalc.speedup_str`$\times$ speedup** by reducing the inter-node payload from 1 GB to `{python} HierarchicalAllreduceCalc.inter_data_mb_str` MB per GPU before the Ring traffic multiplier. With 8 GPUs per node, we effectively get 8$\times$ the apparent inter-node bandwidth. This is why NVIDIA's NCCL and similar libraries default to hierarchical algorithms on multi-node clusters.

:::

These three phases confine most traffic within each node before crossing the slower inter-node fabric.

The hierarchical approach generalizes beyond two levels. Large clusters with multiple racks connected through spine switches introduce a third bandwidth tier (rack-to-rack at reduced bisection bandwidth). Extending the same calculation to a rack-scale cluster quantifies the impact of this additional hierarchy.
...
# │ Show: total_3l_ms_str ≈ 15.1 ms vs. flat_3l_ms_str ≈ 160 ms—inline.
...
# │
# │ Imports: mlsysim.core.constants (NVLINK_H100_BW, INFINIBAND_NDR_BW,
# │          GB, second)
...
class ThreeLevelNapkin:
    ...
    from mlsysim.core.constants import NVLINK_H100_BW, INFINIBAND_NDR_BW, GB, second as _sec_3l
    gradient_3l_gb = 2    # 2 GB gradient (BF16 for a ~1B model)
    nvlink_bw_3l   = NVLINK_H100_BW.m_as(GB/_sec_3l)            # H100 NVLink 4.0
    ib_bw_3l       = INFINIBAND_NDR_BW.m_as(GB/_sec_3l)         # InfiniBand NDR 400 Gbps per port
    cross_rack_bw  = ib_bw_3l / 2                                # 2:1 oversubscription
    ...
    # Totals
    total_3l_ms      = 2 * step1_time_ms + step2_ring_bw_ms + step3_ring_bw_ms
    flat_3l_ms       = 2 * gradient_3l_gb/cross_rack_bw * 1000
    reduction_factor = gradient_3l_gb/step3_payload_gb

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt
    check(flat_3l_ms > total_3l_ms, "Flat 3L should be much slower than Total 3L")
    check(abs(total_3l_ms - 15.1) < 0.1, f"Total 3L = {total_3l_ms:.1f} ms, expected ~15.1")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_gpus_3l_str    = fmt(total_gpus_3l, precision=0, commas=False)
    gradient_3l_gb_str   = fmt(gradient_3l_gb, precision=0, commas=False)
    step1_payload_gb_str = fmt(step1_payload_gb, precision=2, commas=False)
    step1_time_ms_str    = fmt(step1_time_ms, precision=1, commas=False)
    step2_payload_gb_str = fmt(step2_payload_gb, precision=2, commas=False)
    step2_ring_bw_ms_str = fmt(step2_ring_bw_ms, precision=1, commas=False)
    step3_payload_gb_str = fmt(step3_payload_gb, precision=3, commas=False)
    step3_ring_bw_ms_str = fmt(step3_ring_bw_ms, precision=2, commas=False)
    total_3l_ms_str      = fmt(total_3l_ms, precision=1, commas=False)
    flat_3l_ms_str       = fmt(flat_3l_ms, precision=0, commas=False)
    reduction_factor_str = fmt(reduction_factor, precision=0, commas=False)
    nvlink_bw_3l_str     = fmt(nvlink_bw_3l, precision=0, commas=False)
    ib_bw_3l_str         = fmt(ib_bw_3l, precision=0, commas=False)
    cross_rack_bw_str    = fmt(cross_rack_bw, precision=0, commas=False)
```

::: {#nbk-collective-communication-three-level-hierarchical-bandwidth-budget .callout-notebook title="Three-level hierarchical bandwidth budget"}

**Problem**: A `{python} ThreeLevelNapkin.total_gpus_3l_str`-GPU cluster is arranged as 4 racks of 4 nodes of 8 GPUs. Cross-rack bandwidth is oversubscribed 2:1 (effective 25 GB/s). How much does 3-level hierarchical AllReduce reduce cross-rack traffic for a `{python} ThreeLevelNapkin.gradient_3l_gb_str` GB gradient?

**Level 1 (Intra-Node, NVLink)**: ReduceScatter reduces each GPU's contribution by 8$\times$. Each GPU sends `{python} ThreeLevelNapkin.step1_payload_gb_str` GB at `{python} ThreeLevelNapkin.nvlink_bw_3l_str` GB/s = `{python} ThreeLevelNapkin.step1_time_ms_str` ms.

**Level 2 (Intra-Rack, InfiniBand)**: The intra-rack ReduceScatter/AllGather pair among 4 nodes moves a `{python} ThreeLevelNapkin.step2_payload_gb_str` GB rack-level payload. Combined ring traffic is roughly $2(N-1)/N$ times that payload at `{python} ThreeLevelNapkin.ib_bw_3l_str` GB/s. Time: `{python} ThreeLevelNapkin.step2_ring_bw_ms_str` ms.

**Level 3 (Cross-Rack, Spine)**: Ring AllReduce among 4 racks. Each GPU AllReduces only a `{python} ThreeLevelNapkin.step3_payload_gb_str` GB cross-rack payload; Ring traffic again applies the $2(N-1)/N$ multiplier at `{python} ThreeLevelNapkin.cross_rack_bw_str` GB/s. Time: `{python} ThreeLevelNapkin.step3_ring_bw_ms_str` ms.

**Result**: Cross-rack payload per GPU is reduced by `{python} ThreeLevelNapkin.reduction_factor_str`$\times$ compared to the original gradient before the Ring traffic multiplier is applied. Total time, including the matching return-path gather phases, is `{python} ThreeLevelNapkin.total_3l_ms_str` ms vs. `{python} ThreeLevelNapkin.flat_3l_ms_str` ms for flat AllReduce. The hierarchical decomposition concentrates traffic where bandwidth is abundant and minimizes traffic where bandwidth is scarce.
...
::: {#exmp-collective-communication-nccl-topology-discovery .callout-example title="NCCL topology discovery"}
...
Even after perfectly tuning topology and selecting the optimal AllReduce algorithm, a system may remain fundamentally bottlenecked by the physical speed of light across the data center. The only remaining option is to send fewer bits. The previous section attacked the communication bottleneck from the algorithm side; now we look at payload compression.
...
The most common method is **Top-k compression**: for a gradient vector $g \in \mathbb{R}^d$, transmit only the $K_{\text{top}}$ elements with the largest absolute magnitude, setting the rest to zero:

$$\text{TopK}(g) = g \odot \mathbf{1}_{|g| \geq |g|_{(K_{\text{top}})}}$$

where $|g|_{(K_{\text{top}})}$ is the $K_{\text{top}}$-th largest element by magnitude. With $K_{\text{top}} = 0.001 \times d$ (keeping only 0.1 percent of elements), this achieves **1000$\times$ compression**.

#### Encoding sparse gradients {#sec-collective-communication-encoding-sparse-gradients-ddee}

Transmitting a sparse gradient requires sending both the nonzero values and their indices. For a gradient vector of dimension $d$ with $K_{\text{top}}$ nonzero elements, the encoded message contains $K_{\text{top}}$ values (each in FP32 or FP16) plus $K_{\text{top}}$ indices (typically INT32). The total message size is $K_{\text{top}} \times (4 + 4) = 8 K_{\text{top}}$ bytes for FP32 values with INT32 indices. The effective compression ratio is therefore $4d / (8 K_{\text{top}}) = d / (2 K_{\text{top}})$. For $K_{\text{top}} = 0.001 d$, this yields a compression ratio of 500$\times$ (not 1000$\times$), because the index overhead is significant. Reducing the index size to INT16 (for $d < 65536$) or using run-length encoding for structured sparsity patterns can improve the effective ratio.

#### Random-k as an alternative {#sec-collective-communication-randomk-alternative-3668}

An alternative to Top-k is **Random-k sparsification**\index{Random-k Sparsification}, which selects $K_{\text{rand}}$ elements uniformly at random and scales them by $d/K_{\text{rand}}$ to maintain an unbiased gradient estimate. Random-k has the advantage of being an unbiased compressor even without error feedback, because $\mathbb{E}[\text{RandomK}(g)] = g$. However, it introduces higher variance than Top-k because it discards large gradients with the same probability as small ones. In practice, Top-k with error feedback converges faster than Random-k for the same compression ratio, because Top-k preserves the most informative gradient components at each step.
...
class ErrorFeedbackTrace:
    ...
    # ┌── 2. EXECUTE (unrolled 5-step trace) ───────────────
    e0 = 0.0
    v1_in = gradients[0] + e0;   v1 = round(v1_in);   e1 = v1_in - v1
    v2_in = gradients[1] + e1;   v2 = round(v2_in);   e2 = v2_in - v2
    v3_in = gradients[2] + e2;   v3 = round(v3_in);   e3 = v3_in - v3
    v4_in = gradients[3] + e3;   v4 = round(v4_in);   e4 = v4_in - v4
    v5_in = gradients[4] + e4;   v5 = round(v5_in);   e5 = v5_in - v5
    cum_transmitted = int(v1 + v2 + v3 + v4 + v5)
    final_error = round(e5, 4)
    ...
    error_str = fmt(final_error, precision=1, commas=False)
    true_str = fmt(true_cumulative, precision=1, commas=False)
    naive_str = fmt(naive_transmitted, precision=0, commas=False)
    conservation_str = MarkdownStr(f"{cum_transmitted} + ({final_error:.1f})")
```
...
: **Naive Compression Trace (No Error Feedback)**: Five-step worked trace showing how a greedy 1-bit quantizer transmits zero whenever individual gradients fall below the threshold. The true cumulative gradient grows to 1.6, but every transmitted value is zero, so the parameter never moves. {#tbl-error-feedback-naive}

**Naive compression outcome**: After 5 steps, the system has transmitted **`{python} ErrorFeedbackTrace.naive_str`** but the true cumulative gradient is **`{python} ErrorFeedbackTrace.true_str`**. The parameter never updates, and **100 percent of gradient information is lost**.
...
: **Error-Feedback Compression Trace**: Same five-step sequence under error-feedback compression. Each step's quantization residual $e_t$ is added back to the next gradient, so the transmitted signal $v$ converges toward the true cumulative gradient. Transmitted + residual error conserves the full signal. {#tbl-error-feedback}
...
# │ Exports: cp_comm_time_ms_str, cp_ratio_str, cp_overhead_ms_str, cp_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt
...
class CompressionPayback:
    ...
    t_total_new = t_compressed_comm + t_overhead_ms
    speedup = t_comm_ms/t_total_new
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(5 < speedup < 6, f"Speedup {speedup:.1f}x unexpected")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cp_comm_time_ms_str = fmt(t_comm_ms, precision=0, commas=False)
    cp_ratio_str = fmt(ratio, precision=0, commas=False)
    cp_overhead_ms_str = fmt(t_overhead_ms, precision=0, commas=False)
    cp_speedup_str = fmt(speedup, precision=1, commas=False)

```

::: {#nbk-collective-communication-payback-compression .callout-notebook title="The payback of compression"}

**Problem**: A training job synchronizes a `{python} CompressionPayback.cp_comm_time_ms_str` ms gradient buffer. The system implements **1-bit Adam**, which reduces communication volume by 32$\times$ but adds `{python} CompressionPayback.cp_overhead_ms_str` ms of CPU/GPU overhead for the compression logic. If the effective network throughput improves by `{python} CompressionPayback.cp_ratio_str`$\times$, what is the actual communication speedup?

**Math**:

1. **Compressed communication time**: `{python} CompressionPayback.cp_comm_time_ms_str` ms divided by `{python} CompressionPayback.cp_ratio_str` = **5 ms**.
2. **Total new time**: 5 ms (comm) + `{python} CompressionPayback.cp_overhead_ms_str` ms (overhead) = **7 ms**.
3. **Effective speedup**: `{python} CompressionPayback.cp_comm_time_ms_str` divided by 7 $\approx$ **`{python} CompressionPayback.cp_speedup_str`$\times$**.

**Systems insight**: Compression is a trade of **Compute for Bandwidth**. It pays off only when $T_{\text{overhead}} < T_{\text{comm}} \times (1 - 1/\text{Ratio})$. In this case, spending 2 ms to save 35 ms of network time is an exceptional trade. However, on a high-speed NVLink network where $T_{\text{comm}}$ is only 1 ms, this same compression logic would slow down training. Always profile the network before adding compression.
...
- [ ] When would you choose Top-k sparsification\index{Top-k Sparsification} over quantization? Consider the trade-off between compression ratio and the overhead of encoding sparse indices.
...
| **INT8 + Error FB**              |             4$\times$ |        Minor slowdown (~5--10 percent) | Bandwidth-constrained clusters |
| **Top-k (1 percent) + Error FB** |           100$\times$ |    Moderate slowdown (~10--20 percent) | Cross-data center training     |
| **1-bit + Error FB**             |            32$\times$ | Significant slowdown (~20--30 percent) | Extreme bandwidth constraints  |
...
When a distributed training job runs slower than expected, the communication library provides the first diagnostic signals. A systematic workflow isolates whether the bottleneck is in computation, communication, or their interaction:
...
class OverlapBudgetCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt
    check(no_overlap_ms > with_overlap_ms, "No overlap should take longer than with overlap")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_backward_ms_str      = fmt(total_backward_ms, precision=0, commas=False)
    allreduce_per_layer_ms_str = fmt(allreduce_per_layer_ms, precision=0, commas=False)
    no_overlap_ms_str          = fmt(no_overlap_ms, precision=0, commas=False)
    with_overlap_ms_str        = fmt(with_overlap_ms, precision=0, commas=False)
    overlap_savings_pct_str    = fmt(overlap_savings_pct, precision=0, commas=False)
    exposed_per_layer_ms_str   = fmt(exposed_per_layer_ms, precision=0, commas=False)
```

::: {#nbk-collective-communication-overlap-budget-7b-transformer .callout-notebook title="Overlap budget for a 7B transformer"}

**Problem**: A 32-layer transformer model (7B parameters) is trained on 64 GPUs. Each layer's backward pass takes 15 ms. The hierarchical AllReduce for each layer's gradients (~880 MB per layer) takes `{python} OverlapBudgetCalc.allreduce_per_layer_ms_str` ms using 100 MB buckets. What is the step time with and without overlap?

**Without overlap (sequential)**:

$T_{\text{sequential}} = T_{\text{backward}} + T_{\text{comm}}$ = `{python} OverlapBudgetCalc.total_backward_ms_str` ms + 32$\times$ `{python} OverlapBudgetCalc.allreduce_per_layer_ms_str` ms = `{python} OverlapBudgetCalc.no_overlap_ms_str` ms.
...
```{python}
#| label: napkin-overlap-calc
#| echo: false
from mlsysim.fmt import check, fmt

class NapkinOverlap:
    # ┌── 1. LOAD ──────────────────────────────────────────
    from mlsysim.core.constants import NVLINK_H100_BW, GB, second as _sec_no
    params_b = 70
    bytes_per_param = 2       # FP16
    nvlink_bw_gbs = NVLINK_H100_BW.m_as(GB/_sec_no)
    compute_s = 2.1
    # ┌── 2. EXECUTE ───────────────────────────────────────
    gradient_gb = params_b * bytes_per_param
    comm_s = 2 * gradient_gb/nvlink_bw_gbs
    overlap_pct = comm_s/compute_s * 100
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(gradient_gb == 140, f"gradient = {gradient_gb} GB")
    check(abs(comm_s - 0.311) < 0.01, f"comm = {comm_s:.3f} s")
    check(abs(overlap_pct - 15) < 1, f"overlap = {overlap_pct:.0f}%")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gradient_gb_str = fmt(gradient_gb, precision=0, commas=False)
    comm_s_str = fmt(comm_s, precision=2, commas=False)
    compute_s_str = fmt(compute_s, precision=1, commas=False)
    overlap_pct_str = fmt(overlap_pct, precision=0, commas=False)
```

::: {#nbk-collective-communication-napkin-math-overlap-budget .callout-notebook title="Napkin math: The overlap budget"}
...
1.  **Gradient size**: 70B params $\times$ 2 bytes (FP16) = `{python} NapkinOverlap.gradient_gb_str` GB.
2.  **Communication time**: Using Ring AllReduce on NVLink (900 GB/s), time is $\approx \frac{2 \cdot M}{900\text{ GB/s}}$ = `{python} NapkinOverlap.comm_s_str` s.
3.  **Compute time**: A typical forward/backward pass for a block of this size takes $\approx$ `{python} NapkinOverlap.compute_s_str` s.
4.  **Overlap ratio**: `{python} NapkinOverlap.comm_s_str` s divided by `{python} NapkinOverlap.compute_s_str` s $\approx$ `{python} NapkinOverlap.overlap_pct_str` percent.

Since communication takes only `{python} NapkinOverlap.overlap_pct_str` percent of the compute time, it fits comfortably within the backward pass "overlap budget," meaning the network cost effectively disappears.
...
Ring achieves bandwidth-optimal $2\frac{N-1}{N}\frac{M}{\beta}$ but pays $O(N)$ latency. For a 1 MB gradient across 64 GPUs with $\alpha = 10\ \mu s$, Tree AllReduce wins because Ring's 1,260 μs latency penalty exceeds Tree's 1,000 μs bandwidth penalty. The log-aware crossover estimate $M_{\text{crossover}} \approx N \alpha \beta / \log_2 N$ determines when to switch algorithms; $N\alpha\beta$ is only a coarse upper-scale heuristic.
...
A flat Ring AllReduce treats all links as equal, routing data across `{python} CommInfraScenario.ib_ndr_bw_gbs_str` GB/s InfiniBand when `{python} CommInfraScenario.nvlink_h100_bw_gbs_str` GB/s NVLink is available within the node. For an 8-node cluster with 8 GPUs per node, hierarchical AllReduce reduces inter-node traffic by 8$\times$ compared to flat Ring, achieving roughly a 6$\times$ speedup. Ignoring the bandwidth hierarchy leaves the majority of intra-node bandwidth unused while overloading the scarce inter-node bandwidth.
...
The Ring and Tree AllReduce\index{Tree AllReduce} algorithms occupy opposite ends of the latency-bandwidth trade-off, with the log-aware crossover estimate $M_{\text{crossover}} \approx N \alpha \beta / \log_2 N$ determining which is optimal for a given configuration. Hierarchical AllReduce and Rail-Optimized Routing exploit the bandwidth hierarchy of modern GPU clusters to multiply effective inter-node bandwidth, while in-network reduction (SHARP) eliminates round-trips entirely by performing aggregation inside network switches.

When even the fastest wires are not enough, gradient compression provides the final squeeze. Quantization, sparsification, and compression-aware optimizers like 1-bit Adam reduce communication volume by 4--1000$\times$. Error Feedback ensures that while the journey may be compressed or delayed, no vital information is permanently lost, preserving the mathematical truth of the model's convergence. Finally, communication-computation overlap through layer-by-layer pipelining and bucket fusion\index{Bucket Fusion} hides the remaining communication time behind useful computation, reducing the effective communication overhead to 5--15 percent of total step time in well-configured systems.
...
* **Algorithm choice is scale-dependent**: Ring AllReduce is bandwidth-optimal but pays $O(N)$ latency; Tree AllReduce is latency-optimal ($O(\log N)$) but bandwidth-inefficient. Use the log-aware crossover estimate $M_{\text{crossover}} \approx N \alpha \beta / \log_2 N$ to reason about the switch point, and treat $N\alpha\beta$ only as a coarse upper-scale heuristic.
...
: **Archetype-to-Collective Mapping**: Each lighthouse archetype's communication pattern is determined by its dominant bottleneck. Dense transformer training is bandwidth-bound and benefits from hierarchical AllReduce; recommender systems are latency-bound and benefit from topology-aware AllToAll; federated learning is connectivity-bound and benefits from aggressive compression with error feedback. {#tbl-archetype-collective-mapping}

The key insight is that Archetype A and Archetype B, despite both operating at data center scale, face fundamentally different bottlenecks. Archetype A (GPT-4/Llama-3) is bandwidth-bound and benefits from hierarchical AllReduce combined with communication-computation overlap. Archetype B (DLRM at Scale) is latency-bound and benefits from low-latency switches and topology-aware AllToAll routing. Applying the wrong optimization to the wrong archetype wastes engineering effort without improving performance.
...
The fleet now has its *logic* (Parallelism) and its *traffic patterns* (Communication). The map is drawn and the vehicles are built. In production, however, hardware fails as a statistical certainty: GPUs overheat, networks drop packets, and nodes fail mid-calculation.
...
[^fn-speed-of-light-latency]: **The Speed of Light Constraint**: Light travels through optical fiber at approximately 200,000 km/s, or 200 meters per microsecond. In a massive data center where cables between racks span 100 meters, the "wire delay" alone contributes 500 ns to every message—a physical limit that no amount of better networking hardware can reduce. \index{Speed of Light!latency bound}
