...
class AllReduceCost:
    ...
    ring_bw_time_ms = (ring_bw_factor * gradient_size_gb / ib_bw.bandwidth.m_as(GB/second)) * THOUSAND
    ...
    ring_bw_time_ms_str = fmt(ring_bw_time_ms, precision=1, suffix=" ms")
    ring_lat_time_ms_str = fmt(ring_lat_time_ms, precision=1, commas=False, suffix=" ms")
    ring_total_ms_str = fmt(ring_total_ms, precision=1, suffix=" ms")
    n_gpus_napkin_str = fmt(n_gpus, precision=0, commas=False, suffix=" GPUs")
    ring_bw_factor_str = fmt(ring_bw_factor, precision=2, commas=False)
    ib_bw_gbs_str = fmt(ib_bw.bandwidth.m_as(GB/second), precision=0, commas=False, suffix=" GB/s")
    alpha_us_str = fmt(alpha_us, precision=0, commas=False)
    lat_steps_str = fmt(2 * (n_gpus - 1), precision=0, commas=False)
...
**Problem**: A `{python} AllReduceCost.params_b_str` billion parameter model trains with data parallelism across `{python} AllReduceCost.n_gpus_napkin_str` connected by InfiniBand NDR (`{python} AllReduceCost.ib_bw_gbs_str` per port). Each GPU computes gradients in BF16 (2 bytes per parameter, standard for Llama-class training). How long does one AllReduce take?

**Step 1**: Size the gradient payload.
Each GPU produces a full gradient tensor: $70 \times 10^9 \times 2\ \text{bytes} =$ `{python} AllReduceCost.gradient_size_gb_str`.

**Step 2**: Apply the Ring AllReduce bandwidth formula.
$$T_{\text{bandwidth}} = 2 \cdot \frac{N-1}{N} \cdot \frac{n}{\beta}$$

Substituting: $T_{\text{bandwidth}}$ = `{python} AllReduceCost.ring_bw_factor_str` $\times$ `{python} AllReduceCost.gradient_size_gb_str` / `{python} AllReduceCost.ib_bw_gbs_str` $\approx$ `{python} AllReduceCost.ring_bw_time_ms_str`.

**Step 3**: Add the latency term.
$$T_{\text{latency}} = 2(N-1) \cdot \alpha$$

Substituting: $T_{\text{latency}} =$ `{python} AllReduceCost.lat_steps_str` $\times$ `{python} AllReduceCost.alpha_us_str` $\mu\text{s} =$ `{python} AllReduceCost.ring_lat_time_ms_str`.
...
for n in gpu_counts_num:
    ...
    bw = h.memory.bandwidth if n <= 8 else Systems.Fabrics.InfiniBand_NDR.bandwidth
...
class CriticalMsgSize:
    ...
    beta_gbs = 50
    # ┌── 2. EXECUTE ───────────────────────────────────────
    n_star_bytes = alpha_us / MILLION * beta_gbs * BILLION
...
**Problem**: Consider a cluster using InfiniBand NDR 400 Gbps with $\alpha =$ `{python} CriticalMsgSize.alpha_str` $\mu\text{s}$ and $\beta =$ `{python} CriticalMsgSize.beta_str`/s. At what message size does optimizing for bandwidth start to matter more than optimizing for latency?

**Math**:

$n^* = \alpha \cdot \beta = 2 \times 10^{-6}$ s $\times 50 \times 10^9$ B/s $=$ `{python} CriticalMsgSize.n_star_kb_str`
...
class LatencyVsBw:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    bw_a_us = msg_a_bytes / (beta_gbs * BILLION) * MILLION
    total_a_us = bw_a_us + alpha_us
    lat_pct_a = alpha_us/total_a_us * 100
    bw_b_us = msg_b_bytes / (beta_gbs * BILLION) * MILLION
    ...
    lat_pct_a_str = fmt(lat_pct_a, precision=1, commas=False)
    ...
    msg_a_mb_str = fmt(msg_a_bytes / MILLION, precision=0, commas=False)
    msg_b_gb_str = fmt(msg_b_bytes / BILLION, precision=0, commas=False)
```

::: {#nbk-collective-communication-latency-vs-bandwidth-dominance .callout-notebook title="Latency vs. bandwidth dominance"}

**Problem**: Consider synchronizing a `{python} LatencyVsBw.msg_a_mb_str` buffer vs. a `{python} LatencyVsBw.msg_b_gb_str` buffer on InfiniBand NDR with $\alpha =$ `{python} LatencyVsBw.alpha_str` $\mu\text{s}$ and $\beta =$ `{python} LatencyVsBw.beta_str`/s. How does the bottleneck shift?

**Case A: `{python} LatencyVsBw.msg_a_mb_str` Message**

*   Bandwidth Time: $10^6 / (50 \times 10^{9})$ = `{python} LatencyVsBw.bw_a_us_str` $\mu\text{s}$.
*   Latency Time: $2\ \mu\text{s}$.
*   **Total**: `{python} LatencyVsBw.total_a_us_str` μs. Latency is `{python} LatencyVsBw.lat_pct_a_str` of total, still meaningful.

**Case B: `{python} LatencyVsBw.msg_b_gb_str` Message**

*   Bandwidth Time: $10^9 / (50 \times 10^{9})$ = `{python} LatencyVsBw.bw_b_us_str` $\mu\text{s}$ = `{python} LatencyVsBw.bw_b_ms_str`.
*   Latency Time: $2\ \mu\text{s}$.
*   **Total**: `{python} LatencyVsBw.total_b_us_str` μs. Latency is `{python} LatencyVsBw.lat_pct_b_str`, completely negligible.
...
# │
# │ Imports: mlsysim.core.constants (MS_PER_SEC, Hardware.Cloud.H100.nvlink.bandwidth,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MS_PER_SEC, GB, second
...
class LlamaBudgetCalc:
    ...
    nvlink_bw   = Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/second)         # GB/s (H100 NVLink 4.0)
    nvlink_alpha = 1.5                                    # us
    ib_bw        = Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(GB/second)        # GB/s per port (NDR 400 Gbps)
    ib_alpha     = 3                                      # us
    ...
    intra_rs_time_ms  = intra_payload_gb/nvlink_bw * THOUSAND

    # Step 2: Inter-node AllReduce (reduced payload)
    inter_payload_gb          = gradient_bf16_gb/gpus_per_node_llama
    inter_ring_bw_time_ms     = 2 * (n_nodes_llama - 1) / n_nodes_llama * inter_payload_gb/ib_bw * THOUSAND
    ...
    flat_ring_bw_ms = 2 * (n_gpus_llama - 1) / n_gpus_llama * gradient_bf16_gb/ib_bw * THOUSAND
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gradient_bf16_gb_str       = fmt(gradient_bf16_gb, precision=0, commas=False, suffix=" GB")
    intra_rs_time_ms_str       = fmt(intra_rs_time_ms, precision=1, commas=False, suffix=" ms")
    inter_payload_gb_str       = fmt(inter_payload_gb, precision=1, commas=False, suffix=" GB")
    inter_ring_bw_time_ms_str  = fmt(inter_ring_bw_time_ms, precision=1, commas=False, suffix=" ms")
    inter_ring_lat_time_ms_str = fmt(inter_ring_lat_time_ms, precision=2, commas=False, suffix=" ms")
    intra_ag_time_ms_str       = fmt(intra_ag_time_ms, precision=1, commas=False, suffix=" ms")
    hier_total_llama_ms_str    = fmt(hier_total_llama_ms, precision=1, commas=False, suffix=" ms")
    flat_total_ms_str          = fmt_int(round(flat_total_ms), suffix=" ms")
    ...
    nvlink_bw_str              = fmt(nvlink_bw, precision=0, commas=False, suffix=" GB/s")
    ib_bw_str                  = fmt(ib_bw, precision=0, commas=False, suffix=" GB/s")
```

Using BF16 gradients (a standard practice that halves communication volume to `{python} LlamaBudgetCalc.gradient_bf16_gb_str`), the hierarchical AllReduce decomposes as follows:

1. **Intra-Node ReduceScatter** (NVLink at `{python} LlamaBudgetCalc.nvlink_bw_str`): Each GPU exchanges 7/8 of the gradient locally. Time: `{python} LlamaBudgetCalc.intra_rs_time_ms_str`.
2. **Inter-Node Ring AllReduce** (InfiniBand NDR at `{python} LlamaBudgetCalc.ib_bw_str`): Each GPU synchronizes only `{python} LlamaBudgetCalc.inter_payload_gb_str` across 16 nodes. Bandwidth time: `{python} LlamaBudgetCalc.inter_ring_bw_time_ms_str`. Latency time: `{python} LlamaBudgetCalc.inter_ring_lat_time_ms_str`.
3. **Intra-Node AllGather** (NVLink): Distributes the final result locally. Time: `{python} LlamaBudgetCalc.intra_ag_time_ms_str`.

The total hierarchical AllReduce takes approximately `{python} LlamaBudgetCalc.hier_total_llama_ms_str`, compared to `{python} LlamaBudgetCalc.flat_total_ms_str` for a flat Ring AllReduce that ignores the bandwidth hierarchy. This difference is not marginal; it determines whether communication can be hidden behind computation or whether it becomes the critical path.
...
[^fn-fsdp-memory]: **FSDP (Fully Sharded Data Parallel)**: PyTorch's implementation of ZeRO-3 (Zero Redundancy Optimizer), which shards parameters, gradients, and optimizer states across $N$ accelerators, reducing per-GPU memory from $16P$ bytes under FP32 Adaptive Moment Estimation (Adam) to $16P/N$ bytes. The trade-off is communication frequency: FSDP issues $2N_L$ collective operations per step (AllGather + ReduceScatter per layer) instead of data parallelism's single AllReduce, making it more sensitive to the $\alpha$ overhead. \index{FSDP!memory trade-off}
...
class MoeAllToAll:
    ...
    kb_per_token = bytes_per_token / THOUSAND
    ...
    total_sent_mb = total_sent_kb / THOUSAND
    data_per_transfer_bytes = tokens_per_transfer * bytes_per_token
    bw_time_us = data_per_transfer_bytes / (beta_gbs * BILLION) * MILLION
    ...
    total_tokens_str = fmt(total_tokens, precision=0, commas=False)
...
**Problem**: A MoE model processes a batch of `{python} MoeAllToAll.total_tokens_str` across `{python} MoeAllToAll.n_gpus_str` (`{python} MoeAllToAll.tokens_per_gpu_str` per GPU). Each token is a `{python} MoeAllToAll.hidden_dim_str`-dimensional hidden state in BF16 (`{python} MoeAllToAll.kb_per_token_str` per token). The gating network assigns each token to exactly 1 of `{python} MoeAllToAll.n_experts_str` experts (`{python} MoeAllToAll.experts_per_gpu_str` experts per GPU). Assuming uniform routing (each expert receives `{python} MoeAllToAll.tokens_per_expert_str`), how much data does each GPU send and receive?

**Math**:

Each GPU holds `{python} MoeAllToAll.tokens_per_gpu_str` that need to reach `{python} MoeAllToAll.n_experts_str` different experts across `{python} MoeAllToAll.n_gpus_str`. With uniform routing, each GPU sends `{python} MoeAllToAll.tokens_per_transfer_str` to each of the other `{python} MoeAllToAll.remote_gpus_str` (keeping `{python} MoeAllToAll.tokens_per_transfer_str` local).
...
*   Total data sent per GPU: 7 $\times$ `{python} MoeAllToAll.data_per_transfer_kb_str` = `{python} MoeAllToAll.total_sent_mb_str`
*   Total data received per GPU: `{python} MoeAllToAll.total_sent_mb_str` (symmetric)

**Latency Analysis (InfiniBand NDR, $\alpha =$ `{python} MoeAllToAll.alpha_str` $\mu\text{s}$, $\beta =$ `{python} MoeAllToAll.beta_str`/s)**:

Each of the 7 transfers is a `{python} MoeAllToAll.data_per_transfer_kb_str` message. From the $\alpha$-$\beta$ model: $T_{\text{transfer}}$ = 3 + `{python} MoeAllToAll.bw_time_approx_str` = `{python} MoeAllToAll.per_transfer_us_str` $\mu\text{s}$ per transfer.
...
class RingVsTree:
    ...
    ring_bw_us = 2 * (N - 1) / N * M_mb/beta_gbs * THOUSAND
    tree_bw_us = 2 * log2N * M_mb/beta_gbs * THOUSAND
    ...
    tree_win_margin_pct = (ring_total_us - tree_total_us) / ring_total_us * 100
    # ┌── 3. GUARD ─────────────────────────────────────────
    ...
    ring_bw_str = fmt(ring_bw_us, precision=1, commas=False)
    tree_bw_str = fmt(tree_bw_us, precision=0)
    ring_total_str = fmt(ring_total_us, precision=1)
    tree_total_str = fmt(tree_total_us, precision=0)
    crossover_str = fmt(crossover_mb, precision=1, commas=False)
    M_mb_str = fmt(M_mb, precision=0, commas=False)
    N_str = fmt(N, precision=0, commas=False)
    tree_win_margin_pct_str = fmt_int(tree_win_margin_pct, commas=False)
```

::: {#nbk-collective-communication-ring-vs-tree-crossover .callout-notebook title="The ring vs. tree crossover"}

**Problem**: A cluster synchronizes a `{python} RingVsTree.M_mb_str` buffer across `{python} RingVsTree.N_str`. The network has latency $\alpha =$ `{python} RingVsTree.alpha_str` $\mu\text{s}$ and bandwidth $\beta =$ `{python} RingVsTree.beta_str`/s. Which algorithm performs better: Ring or Tree?
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.nvlink.bandwidth, Systems.Fabrics.InfiniBand_NDR.bandwidth, GB, second)
...
class CommInfraScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_gbs_str = fmt(nvlink_gbs, precision=0, commas=False, suffix=" GB/s")
    ib_ndr_bw_gbs_str = fmt(ib_gbs, precision=0, commas=False, suffix=" GB/s")
```

A GPU node delivers an order of magnitude more intra-node bandwidth (NVLink at `{python} CommInfraScenario.nvlink_h100_bw_gbs_str`) than inter-node bandwidth (InfiniBand at `{python} CommInfraScenario.ib_ndr_bw_gbs_str`).
...
# │
# │ Imports: mlsysim.core.constants (MB, GB, second, ms, Hardware.Cloud.H100.nvlink.bandwidth,
# │          Systems.Fabrics.InfiniBand_NDR.bandwidth, MS_PER_SEC)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MB, GB, second, ms, MS_PER_SEC, THOUSAND
...
class HierarchicalAllreduceCalc:
    ...
    ib_bw_gbs      = Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(GB/second)        # InfiniBand NDR 400 Gbps per port
    nvlink_bw_gbs  = Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/second)            # NVLink 4.0 (H100)
    ib_alpha_us    = 3

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    flat_data_gb  = 2 * gradient_gb   # bandwidth-optimal Ring sends ~2×
    flat_time_ms  = flat_data_gb/ib_bw_gbs * THOUSAND

    intra_data_mb = int(gradient_gb * THOUSAND * (gpus_per_node - 1) / gpus_per_node)  # ~875 MB
    intra_rs_ms   = ((intra_data_mb * MB) / (nvlink_bw_gbs * GB / second)).m_as(ms)

    inter_data_mb = int(gradient_gb * THOUSAND / gpus_per_node)  # 125 MB
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    flat_time_ms_str  = fmt(flat_time_ms, precision=0, commas=False, suffix=" ms")
    intra_data_mb_str = fmt(intra_data_mb, precision=0, commas=False, suffix=" MB")
    intra_rs_ms_str   = fmt(intra_rs_ms, precision=1, commas=False, suffix=" ms")
    inter_data_mb_str = fmt(inter_data_mb, precision=0, commas=False, suffix=" MB")
    inter_time_ms_str = fmt(inter_time_ms, precision=1, commas=False, suffix=" ms")
    intra_ag_ms_str   = fmt(intra_ag_ms, precision=1, commas=False, suffix=" ms")
    hier_total_ms_str = fmt(hier_total_ms, precision=1, commas=False, suffix=" ms")
    ib_bw_gbs_str     = fmt(ib_bw_gbs, precision=0, commas=False, suffix=" GB/s")
    nvlink_bw_gbs_str = fmt(nvlink_bw_gbs, precision=0, commas=False, suffix=" GB/s")
    speedup_str       = fmt(speedup, precision=1, commas=False)
    n_nodes_str       = fmt(n_nodes_har, precision=0, commas=False, suffix=" nodes")
    ...
    flat_send_gb_str  = fmt(flat_data_gb, precision=0, commas=False, suffix=" GB")
```
...
- **Effective bandwidth**\index{Bandwidth!effective}: Limited by the slowest link = `{python} HierarchicalAllreduceCalc.ib_bw_gbs_str` (InfiniBand).
- **Time**: $\approx 2 \times 1\ \text{GB} / 50\ \text{GB/s}$ = `{python} HierarchicalAllreduceCalc.flat_time_ms_str` (bandwidth term dominates).

**Hierarchical AllReduce (3-step decomposition)**:

1. **Intra-node ReduceScatter**: Each GPU sends `{python} HierarchicalAllreduceCalc.intra_data_mb_str` at `{python} HierarchicalAllreduceCalc.nvlink_bw_gbs_str` → ~`{python} HierarchicalAllreduceCalc.intra_rs_ms_str`
2. **Inter-Node AllReduce**\index{AllReduce!inter-node}: Each GPU AllReduces a $1\ \text{GB}/8$ = `{python} HierarchicalAllreduceCalc.inter_data_mb_str` shard over InfiniBand; Ring moves roughly $2(N-1)/N$ times that payload, giving ~`{python} HierarchicalAllreduceCalc.inter_time_ms_str` at `{python} HierarchicalAllreduceCalc.ib_bw_gbs_str`

   (Only 1/8 of the data crosses InfiniBand!)

3. **Intra-node AllGather**: Each GPU receives `{python} HierarchicalAllreduceCalc.intra_data_mb_str` at `{python} HierarchicalAllreduceCalc.nvlink_bw_gbs_str` → ~`{python} HierarchicalAllreduceCalc.intra_ag_ms_str`
...
**Systems insight**: Hierarchical AllReduce achieves a `{python} HierarchicalAllreduceCalc.speedup_str`$\times$ speedup by reducing the inter-node payload from 1 GB to `{python} HierarchicalAllreduceCalc.inter_data_mb_str` per GPU before the Ring traffic multiplier. With 8 GPUs per node, we effectively get 8$\times$ the apparent inter-node bandwidth. This is why NVIDIA's NCCL and similar libraries default to hierarchical algorithms on multi-node clusters.
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.nvlink.bandwidth, Systems.Fabrics.InfiniBand_NDR.bandwidth,
# │          GB, second)
...
class ThreeLevelNapkin:
    ...
    from mlsysim.core.constants import GB, second as _sec_3l
    gradient_3l_gb = 2    # 2 GB gradient (BF16 for a ~1B model)
    nvlink_bw_3l   = Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/_sec_3l)            # H100 NVLink 4.0
    ib_bw_3l       = Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(GB/_sec_3l)         # InfiniBand NDR 400 Gbps per port
    ...
    step1_time_ms     = step1_payload_gb/nvlink_bw_3l * THOUSAND

    # Level 2: InfiniBand within rack
    step2_payload_gb  = gradient_3l_gb/gpus_per_node_3l               # 0.25 GB per GPU
    step2_ring_bw_ms  = 2 * (nodes_per_rack - 1) / nodes_per_rack * step2_payload_gb/ib_bw_3l * THOUSAND

    # Level 3: Spine switch between racks
    step3_payload_gb  = step2_payload_gb/nodes_per_rack
    step3_ring_bw_ms  = 2 * (n_racks - 1) / n_racks * step3_payload_gb/cross_rack_bw * THOUSAND

    # Totals
    total_3l_ms      = 2 * step1_time_ms + step2_ring_bw_ms + step3_ring_bw_ms
    flat_3l_ms       = 2 * gradient_3l_gb/cross_rack_bw * THOUSAND
    ...
    gradient_3l_gb_str   = fmt(gradient_3l_gb, precision=0, commas=False, suffix=" GB")
    step1_payload_gb_str = fmt(step1_payload_gb, precision=2, commas=False, suffix=" GB")
    step1_time_ms_str    = fmt(step1_time_ms, precision=1, commas=False, suffix=" ms")
    step2_payload_gb_str = fmt(step2_payload_gb, precision=2, commas=False, suffix=" GB")
    step2_ring_bw_ms_str = fmt(step2_ring_bw_ms, precision=1, commas=False, suffix=" ms")
    step3_payload_gb_str = fmt(step3_payload_gb, precision=3, commas=False, suffix=" GB")
    step3_ring_bw_ms_str = fmt(step3_ring_bw_ms, precision=2, commas=False, suffix=" ms")
    total_3l_ms_str      = fmt(total_3l_ms, precision=1, commas=False, suffix=" ms")
    flat_3l_ms_str       = fmt(flat_3l_ms, precision=0, commas=False, suffix=" ms")
    reduction_factor_str = fmt(reduction_factor, precision=0, commas=False)
    nvlink_bw_3l_str     = fmt(nvlink_bw_3l, precision=0, commas=False, suffix=" GB/s")
    ib_bw_3l_str         = fmt(ib_bw_3l, precision=0, commas=False, suffix=" GB/s")
    cross_rack_bw_str    = fmt(cross_rack_bw, precision=0, commas=False, suffix=" GB/s")
    n_racks_str          = fmt(n_racks, precision=0, commas=False)
    nodes_per_rack_str   = fmt(nodes_per_rack, precision=0, commas=False, suffix=" nodes")
    gpus_per_node_3l_str = fmt(gpus_per_node_3l, precision=0, commas=False, suffix=" GPUs")
```
...
**Problem**: A `{python} ThreeLevelNapkin.total_gpus_3l_str`-GPU cluster is arranged as `{python} ThreeLevelNapkin.n_racks_str` racks of `{python} ThreeLevelNapkin.nodes_per_rack_str` of `{python} ThreeLevelNapkin.gpus_per_node_3l_str`. Cross-rack bandwidth is oversubscribed 2:1 (effective `{python} ThreeLevelNapkin.cross_rack_bw_str`). How much does 3-level hierarchical AllReduce reduce cross-rack traffic for a `{python} ThreeLevelNapkin.gradient_3l_gb_str` gradient?

**Level 1 (Intra-Node, NVLink)**: ReduceScatter reduces each GPU's contribution by 8$\times$. Each GPU sends `{python} ThreeLevelNapkin.step1_payload_gb_str` at `{python} ThreeLevelNapkin.nvlink_bw_3l_str` = `{python} ThreeLevelNapkin.step1_time_ms_str`.

**Level 2 (Intra-Rack, InfiniBand)**: The intra-rack ReduceScatter/AllGather pair among 4 nodes moves a `{python} ThreeLevelNapkin.step2_payload_gb_str` rack-level payload. Combined ring traffic is roughly $2(N-1)/N$ times that payload at `{python} ThreeLevelNapkin.ib_bw_3l_str`. Time: `{python} ThreeLevelNapkin.step2_ring_bw_ms_str`.

**Level 3 (Cross-Rack, Spine)**: Ring AllReduce among 4 racks. Each GPU AllReduces only a `{python} ThreeLevelNapkin.step3_payload_gb_str` cross-rack payload; Ring traffic again applies the $2(N-1)/N$ multiplier at `{python} ThreeLevelNapkin.cross_rack_bw_str`. Time: `{python} ThreeLevelNapkin.step3_ring_bw_ms_str`.

**Result**: Cross-rack payload per GPU is reduced by `{python} ThreeLevelNapkin.reduction_factor_str`$\times$ compared to the original gradient before the Ring traffic multiplier is applied. Total time, including the matching return-path gather phases, is `{python} ThreeLevelNapkin.total_3l_ms_str` vs. `{python} ThreeLevelNapkin.flat_3l_ms_str` for flat AllReduce. The hierarchical decomposition concentrates traffic where bandwidth is abundant and minimizes traffic where bandwidth is scarce.
...
where $|g|_{(K_{\text{top}})}$ is the $K_{\text{top}}$-th largest element by magnitude. With $K_{\text{top}} = 0.001 \times d_{\text{grad}}$ (keeping only 0.1 percent of elements), this achieves THOUSAND$\times$ compression.
...
class ErrorFeedbackTrace:
    ...
    naive_loss_pct_str = fmt(100, precision=0, commas=False)
```
...
class CompressionPayback:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cp_comm_time_ms_str = fmt(t_comm_ms, precision=0, commas=False, suffix=" ms")
    cp_ratio_str = fmt(ratio, precision=0, commas=False)
    cp_overhead_ms_str = fmt(t_overhead_ms, precision=0, commas=False, suffix=" ms")
...
**Problem**: A training job synchronizes a `{python} CompressionPayback.cp_comm_time_ms_str` gradient buffer. The system implements 1-bit Adam, which reduces communication volume by 32$\times$ but adds `{python} CompressionPayback.cp_overhead_ms_str` of CPU/GPU overhead for the compression logic. If the effective network throughput improves by `{python} CompressionPayback.cp_ratio_str`$\times$, what is the actual communication speedup?

**Math**:

1. **Compressed communication time**: `{python} CompressionPayback.cp_comm_time_ms_str` divided by `{python} CompressionPayback.cp_ratio_str` = 5 ms.
2. **Total new time**: 5 ms (comm) + `{python} CompressionPayback.cp_overhead_ms_str` (overhead) = 7 ms.
...
class OverlapBudgetCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_backward_ms_str      = fmt(total_backward_ms, precision=0, commas=False, suffix=" ms")
    allreduce_per_layer_ms_str = fmt(allreduce_per_layer_ms, precision=1, commas=False, suffix=" ms")
    no_overlap_ms_str          = fmt(no_overlap_ms, precision=1, commas=False, suffix=" ms")
    with_overlap_ms_str        = fmt(with_overlap_ms, precision=1, commas=False, suffix=" ms")
    overlap_savings_pct_str    = fmt(overlap_savings_pct, precision=1, commas=False, suffix=" percent")
    exposed_per_layer_ms_str   = fmt(exposed_per_layer_ms, precision=1, commas=False, suffix=" ms")
    n_layers_str               = fmt(n_layers_overlap, precision=0, commas=False)
    backward_per_layer_ms_str  = fmt(backward_per_layer_ms, precision=0, commas=False, suffix=" ms")
    n_gpus_overlap_str         = fmt(n_gpus_overlap, precision=0, commas=False, suffix=" GPUs")
    gradient_per_layer_mb_str  = fmt(gradient_per_layer_mb_val, precision=0, commas=False, suffix=" MB")
    bucket_size_mb_str         = fmt(bucket_size_mb, precision=0, commas=False, suffix=" MB")
    params_b_overlap_str       = fmt(params_b_overlap, precision=0, commas=False)
```

::: {#nbk-collective-communication-overlap-budget-7b-transformer .callout-notebook title="Overlap budget for a 7B transformer"}

**Problem**: A `{python} OverlapBudgetCalc.n_layers_str`-layer transformer model (`{python} OverlapBudgetCalc.params_b_overlap_str`B parameters) is trained on `{python} OverlapBudgetCalc.n_gpus_overlap_str`. Each layer's backward pass takes `{python} OverlapBudgetCalc.backward_per_layer_ms_str`. The hierarchical AllReduce for each layer's gradients (~`{python} OverlapBudgetCalc.gradient_per_layer_mb_str` per layer) takes `{python} OverlapBudgetCalc.allreduce_per_layer_ms_str` using `{python} OverlapBudgetCalc.bucket_size_mb_str` buckets. What is the step time with and without overlap?

**Without overlap (sequential)**:

$T_{\text{sequential}} = T_{\text{backward}} + T_{\text{comm}}(N)$ = `{python} OverlapBudgetCalc.total_backward_ms_str` + 32$\times$ `{python} OverlapBudgetCalc.allreduce_per_layer_ms_str` = `{python} OverlapBudgetCalc.no_overlap_ms_str`.

**With overlap (pipelined)**:

Each layer's AllReduce (`{python} OverlapBudgetCalc.allreduce_per_layer_ms_str`) runs in parallel with the next layer's backward pass (15 ms). Since `{python} OverlapBudgetCalc.allreduce_per_layer_ms_str` > 15 ms, there is `{python} OverlapBudgetCalc.exposed_per_layer_ms_str` of exposed communication per layer that cannot be hidden.
...
The preceding per-layer analysis exposed the case where AllReduce time exceeds backward-pass time and overlap is only partial. For larger models on fast intra-node interconnects, the balance reverses: the gradient volume grows with parameter count, but so does the compute per layer, and NVLink bandwidth is high enough that the communication fits entirely within the backward pass. A single napkin calculation confirms whether communication disappears into the overlap budget or remains exposed.
...
class NapkinOverlap:
    # ┌── 1. LOAD ──────────────────────────────────────────
    from mlsysim.core.constants import GB, second as _sec_no
    params_b = Models.Language.Llama2_70B.parameters.m_as(Bparam)
    bytes_per_param = 2       # FP16
    nvlink_bw_gbs = Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/_sec_no)
    compute_s = 2.1
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    overlap_pct_str = fmt(overlap_pct, precision=1, commas=False)
...
::: {#nbk-collective-communication-napkin-math-overlap-budget .callout-notebook title="The overlap budget"}
...
1.  **Gradient size**: `{python} NapkinOverlap.params_b_str` params $\times$ `{python} NapkinOverlap.bytes_per_param_str` bytes (FP16) = `{python} NapkinOverlap.gradient_gb_str`.
2.  **Communication time**: Using Ring AllReduce on NVLink (`{python} NapkinOverlap.nvlink_bw_gbs_str`/s), time is $\approx 2 \cdot M / \beta$ = `{python} NapkinOverlap.comm_s_str` s.
...
4.  **Overlap ratio**: `{python} NapkinOverlap.comm_s_str` s divided by `{python} NapkinOverlap.compute_s_str` s $\approx$ `{python} NapkinOverlap.overlap_pct_str`.

Since communication takes only `{python} NapkinOverlap.overlap_pct_str` of the compute time, it fits comfortably within the backward pass "overlap budget," meaning the network cost effectively disappears.
...
# │ Goal: Recap NVLink vs InfiniBand NDR bandwidth locally.
# │ Exports: CommInfraRecap.nvlink_h100_bw_gbs_str, CommInfraRecap.ib_ndr_bw_gbs_str
from mlsysim.core.constants import GB, second
class CommInfraRecap:
    nvlink_gbs = Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/second)
    ib_gbs = Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(GB/second)
    from mlsysim.fmt import fmt
    nvlink_h100_bw_gbs_str = fmt(nvlink_gbs, precision=0, commas=False, suffix=" GB/s")
    ib_ndr_bw_gbs_str = fmt(ib_gbs, precision=0, commas=False, suffix=" GB/s")
```

A flat Ring AllReduce treats all links as equal, routing data across `{python} CommInfraRecap.ib_ndr_bw_gbs_str` InfiniBand when `{python} CommInfraRecap.nvlink_h100_bw_gbs_str` NVLink is available within the node. For an 8-node cluster with 8 GPUs per node, hierarchical AllReduce reduces inter-node traffic by 8$\times$ compared to flat Ring, achieving roughly a 6$\times$ speedup. Ignoring the bandwidth hierarchy leaves the majority of intra-node bandwidth unused while overloading the scarce inter-node bandwidth.
