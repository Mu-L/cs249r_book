...
2.  **Distinction (durable)**: Unlike parameter-server approaches (where all workers send gradients to a centralized aggregator whose bandwidth scales as $\mathcal{O}(N)$), ring AllReduce distributes the communication across all workers so that each worker's per-step communication cost stays constant at $2 \times (N-1)/N \times M$ regardless of cluster size.
...
class AllReduceCost:
    ...
    ib_ndr = Hardware.Networks.InfiniBand_NDR
    n_gpus = 64
    ib_bw = Hardware.Networks.InfiniBand_NDR
    alpha_us = 3 # microseconds

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Gradient size (BF16 = 2 bytes/param)
    gradient_qty = m.parameters.m_as(param) * 2 * ureg.byte
    gradient_size_gb = gradient_qty.m_as(GB)

    # Use library formula for Ring AllReduce time
    # Note: calc_ring_allreduce_time handles both bandwidth and latency (alpha)
    t_ring = calc_ring_allreduce_time(
        message_bytes=gradient_qty,
        n_gpus=n_gpus,
        bandwidth_bytes_s=ib_ndr.bandwidth,
        latency_s=alpha_us * ureg.microsecond
    )
    ring_total_ms = t_ring.m_as("ms")

    # Break out terms for pedagogical display
    ring_bw_factor = 2 * (n_gpus - 1) / n_gpus
    ring_bw_time_ms = (ring_bw_factor * gradient_size_gb / ib_bw.bandwidth.m_as(GB/second)) * 1000
    ring_lat_time_ms = ring_total_ms - ring_bw_time_ms

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(ring_total_ms > 5000, f"AllReduce ({ring_total_ms:.0f}ms) should be significant (>5s) for 70B BF16 model.")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gradient_size_gb_str = fmt(gradient_size_gb, precision=1, allow_zero=True, commas=False)
    ring_bw_time_ms_str = fmt(ring_bw_time_ms, precision=1, allow_zero=True)
    ring_lat_time_ms_str = fmt(ring_lat_time_ms, precision=1, commas=False)
    ring_total_ms_str = fmt(ring_total_ms, precision=1, allow_zero=True)
    n_gpus_napkin_str = fmt(n_gpus, precision=1, allow_zero=True, commas=False)
...
from mlsysim import viz, Models, Hardware, Engine

fig, ax, COLORS, plt = viz.setup_plot(figsize=(8, 4.5))

# ┌── 2. COMPUTE FRACTIONS ─────────────────────────────────────────────────────
# Use Engine to solve for real compute vs comm ratios across scales
m = Models.Language.Llama2_70B
...
for n in gpu_counts_num:
    # Use Engine.solve for large-batch training step approximation
    p = Engine.solve(m, h, batch_size=32, is_training=True)
    t_comp = p.latency_compute.m_as("ms")

    # Simple Ring AllReduce estimate for the scale
    from mlsysim.core.formulas import calc_ring_allreduce_time
    grad_size = m.parameters.m_as(param) * 2 * ureg.byte
    # Effective BW decreases at scale due to topology
    bw = h.memory.bandwidth if n <= 8 else Hardware.Networks.InfiniBand_NDR.bandwidth
    t_comm = calc_ring_allreduce_time(grad_size, n, bw, 0*ureg.ms).m_as("ms")

    total = t_comp + t_comm
    # Add synthetic sync overhead (scales with log N)
    sync_val = 0.05 * total * np.log2(n/8 + 1)
    total_with_sync = total + sync_val

    compute_pct.append(t_comp / total_with_sync * 100)
    comm_pct.append(t_comm / total_with_sync * 100)
    sync_pct.append(sync_val / total_with_sync * 100)
...
class CriticalMsgSize:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = fmt(alpha_us, precision=1, allow_zero=True, commas=False)
    beta_str = fmt(beta_gbs, precision=1, allow_zero=True, commas=False)
    n_star_kb_str = fmt(n_star_kb, precision=1, allow_zero=True, commas=False)
```
...
class LatencyVsBw:
    ...
    alpha_str = fmt(alpha_us, precision=1, allow_zero=True, commas=False)
    beta_str = fmt(beta_gbs, precision=1, allow_zero=True, commas=False)
    bw_a_us_str = fmt(bw_a_us, precision=1, allow_zero=True, commas=False)
    total_a_us_str = fmt(total_a_us, precision=1, allow_zero=True, commas=False)
    lat_pct_a_str = fmt(lat_pct_a, precision=1, allow_zero=True, commas=False)
    bw_b_us_str = fmt(bw_b_us, precision=1, allow_zero=True)
    bw_b_ms_str = fmt(bw_b_ms, precision=1, allow_zero=True, commas=False)
    total_b_us_str = fmt(total_b_us, precision=1, allow_zero=True)
...
class LogPOverlap:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    L_str = fmt(L_us, precision=1, allow_zero=True, commas=False)
    two_o_str = fmt(two_o_us, precision=1, allow_zero=True, commas=False)
    compute_str = fmt(compute_us, precision=1, allow_zero=True, commas=False)
    effective_str = fmt(effective_us, precision=1, allow_zero=True, commas=False)
```
...
class LlamaBudgetCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gradient_bf16_gb_str       = fmt(gradient_bf16_gb, precision=1, allow_zero=True, commas=False)
    ...
    inter_ring_bw_time_ms_str  = fmt(inter_ring_bw_time_ms, precision=1, allow_zero=True, commas=False)
    ...
    hier_total_llama_ms_str    = fmt(hier_total_llama_ms, precision=1, allow_zero=True, commas=False)
    flat_total_ms_str          = fmt(flat_total_ms, precision=1, allow_zero=True)
    ccr_hier_str               = fmt(ccr_hier, precision=1, commas=False)
    ccr_flat_str               = fmt(ccr_flat, precision=1, commas=False)
    n_gpus_llama_str           = fmt(n_gpus_llama, precision=1, allow_zero=True, commas=False)
    nvlink_bw_str              = fmt(nvlink_bw, precision=1, allow_zero=True, commas=False)
    ib_bw_str                  = fmt(ib_bw, precision=1, allow_zero=True, commas=False)
```
...
3.  **AllReduce**: Data from all workers is aggregated, and the result is distributed to all workers.
    ...
    *   Complexity: Ring achieves bandwidth-optimal $2\frac{N-1}{N}\frac{M}{\beta}$ but $\mathcal{O}(N)$ latency; Tree achieves $\mathcal{O}(\log N)$ latency but suboptimal bandwidth.
...
[^fn-fsdp-memory]: **FSDP (Fully Sharded Data Parallel)**: PyTorch's implementation of ZeRO-3, which shards parameters, gradients, and optimizer states across $N$ accelerators, reducing per-GPU memory from $16P$ bytes under FP32 Adaptive Moment Estimation (Adam) to $16P/N$ bytes. The trade-off is communication frequency: FSDP issues $2N_L$ collective operations per step (AllGather + ReduceScatter per layer) instead of data parallelism's single AllReduce, making it more sensitive to the $\alpha$ overhead. \index{FSDP!memory trade-off}
...
class MoeAllToAll:
    ...
    alpha_str = fmt(alpha_us, precision=1, allow_zero=True, commas=False)
    beta_str = fmt(beta_gbs, precision=1, allow_zero=True, commas=False)
    tokens_per_gpu_str = fmt(tokens_per_gpu, precision=1, allow_zero=True, commas=False)
    tokens_per_transfer_str = fmt(tokens_per_transfer, precision=1, allow_zero=True, commas=False)
    kb_per_token_str = fmt(kb_per_token, precision=3, commas=False)
    tokens_per_expert_str = fmt(tokens_per_expert, precision=1, allow_zero=True, commas=False)
    ...
    bw_time_approx_str = fmt(bw_time_us_approx, precision=1, allow_zero=True, commas=False)
    per_transfer_us_str = fmt(per_transfer_us, precision=1, allow_zero=True, commas=False)
    serialized_us_str = fmt(serialized_us, precision=1, allow_zero=True, commas=False)
    parallel_us_str = fmt(parallel_us, precision=1, allow_zero=True, commas=False)
```
...
Each of the 7 transfers is a `{python} MoeAllToAll.data_per_transfer_kb_str` KB message. From the $\alpha$-$\beta$ model: $T_{\text{transfer}}$ = 3 + `{python} MoeAllToAll.bw_time_approx_str` = `{python} MoeAllToAll.per_transfer_us_str` $\mu\text{s}$ per transfer.
...
The consequence is a trade-off between memory and communication frequency. Standard data parallelism communicates once per training step (a single AllReduce of the full gradient). FSDP communicates twice per layer per step (AllGather in forward, ReduceScatter in backward), but each communication is smaller (only that layer's parameters, sharded across ranks). For a model with $N_L$ layers, FSDP issues $2N_L$ collective operations per step instead of 1; across all those operations, the total communication volume can be comparable to the single full-gradient AllReduce, but it is spread across many smaller operations.
...
class RingTrace:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    steps_str = fmt(total_steps, precision=1, allow_zero=True, commas=False)
    elem_str = fmt(elem_per_step, precision=1, allow_zero=True, commas=False)
    total_str = fmt(total_per_gpu, precision=1, allow_zero=True, commas=False)
```
...
class RingVsTree:
    ...
    alpha_str = fmt(alpha_us, precision=1, allow_zero=True, commas=False)
    beta_str = fmt(beta_gbs, precision=1, allow_zero=True, commas=False)
    ring_lat_str = fmt(ring_lat_us, precision=1, allow_zero=True)
    tree_lat_str = fmt(tree_lat_us, precision=1, allow_zero=True, commas=False)
    ring_bw_str = fmt(ring_bw_us, precision=1, allow_zero=True, commas=False)
    tree_bw_str = fmt(tree_bw_us, precision=1, allow_zero=True)
    ring_total_str = fmt(ring_total_us, precision=1, allow_zero=True)
    tree_total_str = fmt(tree_total_us, precision=1, allow_zero=True)
...
1.  **Ring Latency**\index{Ring AllReduce!latency}: $2(N-1)\alpha = 2 \times 63 \times 10\ \mu\text{s}$ = **`{python} RingVsTree.ring_lat_str` $\mu\text{s}$**.
2.  **Tree Latency**\index{Tree AllReduce!latency}: $2(\log_2 N)\alpha = 2 \times 6 \times 10\ \mu\text{s}$ = **`{python} RingVsTree.tree_lat_str` $\mu\text{s}$**.

**Bandwidth** (note the difference):

3.  **Ring Bandwidth**\index{Ring AllReduce!bandwidth}: $2\frac{N-1}{N}\frac{M}{\beta} \approx 2 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = **`{python} RingVsTree.ring_bw_str` $\mu\text{s}$** (optimal: each byte sent once).
4.  **Tree Bandwidth**\index{Tree AllReduce!bandwidth}: $2\log_2 N \cdot \frac{M}{\beta} = 12 \times \frac{1\text{ MB}}{10\text{ GB/s}}$ = **`{python} RingVsTree.tree_bw_str` $\mu\text{s}$** (each level sends full message).
...
- [ ] Can you explain why Ring AllReduce achieves bandwidth-optimal communication (approaching $2M/\beta$) while Tree AllReduce incurs $\mathcal{O}(\log N)$ bandwidth overhead?
...
- [ ] Can you explain why Ring AllReduce's $\mathcal{O}(N)$ latency makes it impractical for tensor parallelism (where message sizes are small and latency dominates)?

:::

[^fn-moe-sparsity]: **Mixture of Experts (MoE)**: An architecture where each token activates only a subset of specialized subnetworks (experts), reducing per-token FLOPs while maintaining total model capacity. The systems trade-off is stark: MoE replaces the bandwidth-bound AllReduce of dense models with latency-bound AllToAll, shifting the communication bottleneck from $\beta$ to $\alpha$ and creating $\mathcal{O}(N^2)$ contention that limits practical cluster size. \index{Mixture of Experts!communication}
...
class CommInfraScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_bw_gbs_str = fmt(nvlink_gbs, precision=1, allow_zero=True, commas=False)
    ib_ndr_bw_gbs_str = fmt(ib_gbs, precision=1, allow_zero=True, commas=False)
```
...
class HierarchicalAllreduceCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    flat_time_ms_str  = fmt(flat_time_ms, precision=1, allow_zero=True, commas=False)
    intra_data_mb_str = fmt(intra_data_mb, precision=1, allow_zero=True, commas=False)
    intra_rs_ms_str   = fmt(intra_rs_ms, precision=1, allow_zero=True, commas=False)
    inter_data_mb_str = fmt(inter_data_mb, precision=1, allow_zero=True, commas=False)
    inter_time_ms_str = fmt(inter_time_ms, precision=1, allow_zero=True, commas=False)
    intra_ag_ms_str   = fmt(intra_ag_ms, precision=1, allow_zero=True, commas=False)
    hier_total_ms_str = fmt(hier_total_ms, precision=1, allow_zero=True, commas=False)
    ib_bw_gbs_str     = fmt(ib_bw_gbs, precision=1, allow_zero=True, commas=False)
...
class ThreeLevelNapkin:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_gpus_3l_str    = fmt(total_gpus_3l, precision=1, allow_zero=True, commas=False)
    gradient_3l_gb_str   = fmt(gradient_3l_gb, precision=1, allow_zero=True, commas=False)
    ...
    flat_3l_ms_str       = fmt(flat_3l_ms, precision=1, allow_zero=True, commas=False)
    reduction_factor_str = fmt(reduction_factor, precision=1, allow_zero=True, commas=False)
    nvlink_bw_3l_str     = fmt(nvlink_bw_3l, precision=1, allow_zero=True, commas=False)
    ib_bw_3l_str         = fmt(ib_bw_3l, precision=1, allow_zero=True, commas=False)
    cross_rack_bw_str    = fmt(cross_rack_bw, precision=1, allow_zero=True, commas=False)
```
...
class ErrorFeedbackTrace:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cum_str = fmt(cum_transmitted, precision=1, allow_zero=True, commas=False)
    error_str = fmt(final_error, precision=1, commas=False)
    true_str = fmt(true_cumulative, precision=1, commas=False)
    naive_str = fmt(naive_transmitted, precision=1, allow_zero=True, commas=False)
...
class CompressionPayback:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cp_comm_time_ms_str = fmt(t_comm_ms, precision=1, allow_zero=True, commas=False)
    cp_ratio_str = fmt(ratio, precision=1, allow_zero=True, commas=False)
    cp_overhead_ms_str = fmt(t_overhead_ms, precision=1, allow_zero=True, commas=False)
...
class OverlapBudgetCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_backward_ms_str      = fmt(total_backward_ms, precision=1, allow_zero=True, commas=False)
    allreduce_per_layer_ms_str = fmt(allreduce_per_layer_ms, precision=1, allow_zero=True, commas=False)
    no_overlap_ms_str          = fmt(no_overlap_ms, precision=1, allow_zero=True, commas=False)
    with_overlap_ms_str        = fmt(with_overlap_ms, precision=1, allow_zero=True, commas=False)
    overlap_savings_pct_str    = fmt(overlap_savings_pct, precision=1, allow_zero=True, commas=False)
    exposed_per_layer_ms_str   = fmt(exposed_per_layer_ms, precision=1, allow_zero=True, commas=False)
```
...
$T_{\text{sequential}} = T_{\text{backward}} + T_{\text{comm}}(N)$ = `{python} OverlapBudgetCalc.total_backward_ms_str` ms + 32$\times$ `{python} OverlapBudgetCalc.allreduce_per_layer_ms_str` ms = `{python} OverlapBudgetCalc.no_overlap_ms_str` ms.
...
$T_{\text{pipelined}} = T_{\text{backward}} + T_{\text{first layer comm}} + (N_L - 1) \times T_{\text{exposed}}$ = `{python} OverlapBudgetCalc.with_overlap_ms_str` ms.
...
class NapkinOverlap:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gradient_gb_str = fmt(gradient_gb, precision=1, allow_zero=True, commas=False)
    comm_s_str = fmt(comm_s, precision=2, commas=False)
    compute_s_str = fmt(compute_s, precision=1, commas=False)
    overlap_pct_str = fmt(overlap_pct, precision=1, allow_zero=True, commas=False)
```
