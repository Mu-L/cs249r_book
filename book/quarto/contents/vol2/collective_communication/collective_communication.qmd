...
class CriticalMsgSize:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    alpha_str = f"{alpha_us}"
    beta_str = f"{beta_gbs}"
    n_star_kb_str = f"{n_star_kb}"
```
...
class LatencyVsBw:
    ...
    alpha_str = f"{alpha_us}"
    beta_str = f"{beta_gbs}"
    bw_a_us_str = f"{bw_a_us:.0f}"
    total_a_us_str = f"{total_a_us:.0f}"
    lat_pct_a_str = f"{lat_pct_a:.0f}"
    bw_b_us_str = f"{bw_b_us:,.0f}"
    bw_b_ms_str = f"{bw_b_ms:.0f}"
    total_b_us_str = f"{total_b_us:,.0f}"
    lat_pct_b_str = f"{lat_pct_b:.2f}"
```
...
class LogPOverlap:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    L_str = f"{L_us}"
    two_o_str = f"{two_o_us}"
    compute_str = f"{compute_us}"
    effective_str = f"{effective_us}"
```
...
class LlamaBudgetCalc:
    ...
    nvlink_bw_str              = f"{nvlink_bw:.0f}"
    ib_bw_str                  = f"{ib_bw:.0f}"
```
...
class MoeAllToAll:
    ...
    alpha_str = f"{alpha_us}"
    beta_str = f"{beta_gbs}"
    tokens_per_gpu_str = f"{tokens_per_gpu}"
    tokens_per_transfer_str = f"{tokens_per_transfer}"
    kb_per_token_str = f"{kb_per_token:.0f}"
    tokens_per_expert_str = f"{tokens_per_expert}"
    data_per_transfer_kb_str = f"{data_per_transfer_kb:.0f}"
    total_sent_mb_str = f"{total_sent_mb:.2f}"
    bw_time_approx_str = f"{bw_time_us_approx}"
    per_transfer_us_str = f"{per_transfer_us}"
    serialized_us_str = f"{serialized_us}"
    parallel_us_str = f"{parallel_us}"
```
...
class RingTrace:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    steps_str = f"{total_steps}"
    elem_str = f"{elem_per_step}"
    total_str = f"{total_per_gpu}"
```
...
class RingVsTree:
    ...
    alpha_str = f"{alpha_us}"
    beta_str = f"{beta_gbs}"
    ring_lat_str = f"{ring_lat_us:,}"
    tree_lat_str = f"{tree_lat_us}"
    ring_bw_str = f"{ring_bw_us:.0f}"
    tree_bw_str = f"{tree_bw_us:,.0f}"
    ring_total_str = f"{ring_total_us:,.0f}"
    tree_total_str = f"{tree_total_us:,.0f}"
    crossover_str = f"{crossover_mb:.1f}"
```
...
A GPU node delivers an order of magnitude more intra-node bandwidth (NVLink at `{python} CommInfraScenario.nvlink_h100_bw_gbs` GB/s) than inter-node bandwidth (InfiniBand at `{python} CommInfraScenario.ib_ndr_bw_gbs` GB/s).
...
class ThreeLevelNapkin:
    ...
    nvlink_bw_3l_str     = f"{nvlink_bw_3l:.0f}"
    ib_bw_3l_str         = f"{ib_bw_3l:.0f}"
    cross_rack_bw_str    = f"{cross_rack_bw:.0f}"
```
...
class ErrorFeedbackTrace:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cum_str = f"{cum_transmitted}"
    error_str = f"{final_error:.1f}"
    true_str = f"{true_cumulative:.1f}"
    naive_str = f"{naive_transmitted}"
    conservation_str = f"{cum_transmitted} + ({final_error:.1f})"
```
...
class CompressionPayback:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cp_comm_time_ms_str = f"{t_comm_ms}"
    cp_ratio_str = f"{ratio}"
    cp_overhead_ms_str = f"{t_overhead_ms}"
    cp_speedup_str = f"{speedup:.1f}"
...
class NapkinOverlap:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gradient_gb_str = f"{gradient_gb}"
    comm_s_str = f"{comm_s:.2f}"
    compute_s_str = f"{compute_s}"
    overlap_pct_str = f"{overlap_pct:.0f}"
```
...
```{python}
#| label: fallacy-bandwidth-refs
#| echo: false
from mlsysim.core.constants import NVLINK_H100_BW, INFINIBAND_NDR_BW, GB, second
from mlsysim.fmt import check

class FallacyBandwidthRefs:
    """Local bandwidth constants for the fallacies section."""
    # ┌── 1. LOAD ──────────────────────────────────────────
    nvlink_raw = NVLINK_H100_BW
    ib_raw = INFINIBAND_NDR_BW
    # ┌── 2. EXECUTE ───────────────────────────────────────
    nvlink_h100_bw_gbs_val = nvlink_raw.m_as(GB/second)
    ib_ndr_bw_gbs_val = ib_raw.m_as(GB/second)
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(abs(nvlink_h100_bw_gbs_val - 900) < 1, f"NVLink = {nvlink_h100_bw_gbs_val:.0f} GB/s")
    check(abs(ib_ndr_bw_gbs_val - 50) < 1, f"IB NDR = {ib_ndr_bw_gbs_val:.0f} GB/s")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    nvlink_h100_bw_gbs = f"{nvlink_h100_bw_gbs_val:.0f}"
    ib_ndr_bw_gbs = f"{ib_ndr_bw_gbs_val:.0f}"
```
...
A flat Ring AllReduce treats all links as equal, routing data across `{python} FallacyBandwidthRefs.ib_ndr_bw_gbs` GB/s InfiniBand when `{python} FallacyBandwidthRefs.nvlink_h100_bw_gbs` GB/s NVLink is available within the node. For an 8-node cluster with 8 GPUs per node, hierarchical AllReduce reduces inter-node traffic by 8$\times$ compared to flat Ring, achieving roughly a 6$\times$ speedup. Ignoring the bandwidth hierarchy leaves the majority of intra-node bandwidth unused while overloading the scarce inter-node bandwidth.
