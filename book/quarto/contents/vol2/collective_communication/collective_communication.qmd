...
**Problem**: A `{python} AllReduceCost.params_b_str` billion parameter model trains with data parallelism across `{python} AllReduceCost.n_gpus_napkin_str` GPUs connected by InfiniBand NDR (`{python} AllReduceCost.ib_bw_gbs_str` GB/s per port). Each GPU computes gradients in BF16 (2 bytes per parameter, standard for Llama-class training). How long does one AllReduce take?
...
Substituting: $T_{\text{bandwidth}}$ = `{python} AllReduceCost.ring_bw_factor_str` $\times$ `{python} AllReduceCost.gradient_size_gb_str` GB / `{python} AllReduceCost.ib_bw_gbs_str` GB/s $\approx$ `{python} AllReduceCost.ring_bw_time_ms_str` ms.

**Step 3**: Add the latency term.
$$T_{\text{latency}} = 2(N-1) \cdot \alpha$$

Substituting: $T_{\text{latency}} =$ `{python} AllReduceCost.lat_steps_str` $\times$ `{python} AllReduceCost.alpha_us_str` $\mu\text{s} =$ `{python} AllReduceCost.ring_lat_time_ms_str` ms.
...
class LatencyVsBw:
    ...
    msg_a_mb_str = fmt(msg_a_bytes / 1e6, precision=0, commas=False)
    msg_b_gb_str = fmt(msg_b_bytes / 1e9, precision=0, commas=False)
```

::: {#nbk-collective-communication-latency-vs-bandwidth-dominance .callout-notebook title="Latency vs. bandwidth dominance"}

**Problem**: Consider synchronizing a `{python} LatencyVsBw.msg_a_mb_str` MB buffer vs. a `{python} LatencyVsBw.msg_b_gb_str` GB buffer on InfiniBand NDR with $\alpha =$ `{python} LatencyVsBw.alpha_str` $\mu\text{s}$ and $\beta =$ `{python} LatencyVsBw.beta_str` GB/s. How does the bottleneck shift?

**Case A: `{python} LatencyVsBw.msg_a_mb_str` MB Message**
...
**Case B: `{python} LatencyVsBw.msg_b_gb_str` GB Message**
...
class LlamaBudgetCalc:
    ...
    inter_ring_bw_time_ms_str  = fmt(inter_ring_bw_time_ms, precision=1, commas=False)
    ...
    hier_total_llama_ms_str    = fmt(hier_total_llama_ms, precision=1, commas=False)
    flat_total_ms_str          = fmt_int(round(flat_total_ms))
...
**Problem**: A MoE model processes a batch of `{python} MoeAllToAll.total_tokens_str` tokens across `{python} MoeAllToAll.n_gpus_str` GPUs (`{python} MoeAllToAll.tokens_per_gpu_str` tokens per GPU). Each token is a `{python} MoeAllToAll.hidden_dim_str`-dimensional hidden state in BF16 (`{python} MoeAllToAll.kb_per_token_str` KB per token). The gating network assigns each token to exactly 1 of `{python} MoeAllToAll.n_experts_str` experts (`{python} MoeAllToAll.experts_per_gpu_str` experts per GPU). Assuming uniform routing (each expert receives `{python} MoeAllToAll.tokens_per_expert_str` tokens), how much data does each GPU send and receive?

**Math**:

Each GPU holds `{python} MoeAllToAll.tokens_per_gpu_str` tokens that need to reach `{python} MoeAllToAll.n_experts_str` different experts across `{python} MoeAllToAll.n_gpus_str` GPUs. With uniform routing, each GPU sends `{python} MoeAllToAll.tokens_per_transfer_str` tokens to each of the other `{python} MoeAllToAll.remote_gpus_str` GPUs (keeping `{python} MoeAllToAll.tokens_per_transfer_str` tokens local).
...
class RingVsTree:
    ...
    tree_win_margin_pct_str = fmt(tree_win_margin_pct, precision=0, commas=False)
```

::: {#nbk-collective-communication-ring-vs-tree-crossover .callout-notebook title="The ring vs. tree crossover"}

**Problem**: A cluster synchronizes a **`{python} RingVsTree.M_mb_str` MB** buffer across **`{python} RingVsTree.N_str` GPUs**. The network has latency $\alpha =$ `{python} RingVsTree.alpha_str` $\mu\text{s}$ and bandwidth $\beta =$ `{python} RingVsTree.beta_str` GB/s. Which algorithm performs better: Ring or Tree?
...
**Tree wins**, but only by `{python} RingVsTree.tree_win_margin_pct_str` percent. For this `{python} RingVsTree.M_mb_str` MB message, we are near the crossover point.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MB, GB, second, ms, MS_PER_SEC
...
class HierarchicalAllreduceCalc:
    ...
    intra_rs_ms_str   = fmt(intra_rs_ms, precision=1, commas=False)
    inter_data_mb_str = fmt(inter_data_mb, precision=0, commas=False)
    inter_time_ms_str = fmt(inter_time_ms, precision=1, commas=False)
    intra_ag_ms_str   = fmt(intra_ag_ms, precision=1, commas=False)
    hier_total_ms_str = fmt(hier_total_ms, precision=1, commas=False)
    ...
    n_nodes_str       = fmt(n_nodes_har, precision=0, commas=False)
    ...
    flat_send_gb_str  = fmt(flat_data_gb, precision=0, commas=False)
```
...
**Problem**: A cluster has `{python} HierarchicalAllreduceCalc.n_nodes_str` nodes, each with `{python} HierarchicalAllreduceCalc.gpus_per_node_str` GPUs (`{python} HierarchicalAllreduceCalc.n_gpus_total_str` GPUs total). The system must AllReduce a `{python} HierarchicalAllreduceCalc.gradient_gb_str` GB gradient buffer. Compare flat Ring AllReduce vs. Hierarchical AllReduce.

**Flat Ring AllReduce (ignoring hierarchy)**:

- Each GPU sends ~`{python} HierarchicalAllreduceCalc.flat_send_gb_str` GB total (the bandwidth-optimal Ring AllReduce formula).
...
class ThreeLevelNapkin:
    ...
    gpus_per_node_3l_str = fmt(gpus_per_node_3l, precision=0, commas=False)
```
...
**Problem**: A `{python} ThreeLevelNapkin.total_gpus_3l_str`-GPU cluster is arranged as `{python} ThreeLevelNapkin.n_racks_str` racks of `{python} ThreeLevelNapkin.nodes_per_rack_str` nodes of `{python} ThreeLevelNapkin.gpus_per_node_3l_str` GPUs. Cross-rack bandwidth is oversubscribed 2:1 (effective `{python} ThreeLevelNapkin.cross_rack_bw_str` GB/s). How much does 3-level hierarchical AllReduce reduce cross-rack traffic for a `{python} ThreeLevelNapkin.gradient_3l_gb_str` GB gradient?
...
**Naive compression outcome**: After 5 steps, the system has transmitted **`{python} ErrorFeedbackTrace.naive_str`** but the true cumulative gradient is **`{python} ErrorFeedbackTrace.true_str`**. The parameter never updates, and **`{python} ErrorFeedbackTrace.naive_loss_pct_str` percent of gradient information is lost**.
...
class OverlapBudgetCalc:
    ...
    allreduce_per_layer_ms_str = fmt(allreduce_per_layer_ms, precision=1, commas=False)
    no_overlap_ms_str          = fmt(no_overlap_ms, precision=1, commas=False)
    with_overlap_ms_str        = fmt(with_overlap_ms, precision=1, commas=False)
    overlap_savings_pct_str    = fmt(overlap_savings_pct, precision=1, commas=False)
    exposed_per_layer_ms_str   = fmt(exposed_per_layer_ms, precision=1, commas=False)
    n_layers_str               = fmt(n_layers_overlap, precision=0, commas=False)
    backward_per_layer_ms_str  = fmt(backward_per_layer_ms, precision=0, commas=False)
    n_gpus_overlap_str         = fmt(n_gpus_overlap, precision=0, commas=False)
    gradient_per_layer_mb_str  = fmt(gradient_per_layer_mb_val, precision=0, commas=False)
    bucket_size_mb_str         = fmt(bucket_size_mb, precision=0, commas=False)
...
**Problem**: A `{python} OverlapBudgetCalc.n_layers_str`-layer transformer model (`{python} OverlapBudgetCalc.params_b_overlap_str`B parameters) is trained on `{python} OverlapBudgetCalc.n_gpus_overlap_str` GPUs. Each layer's backward pass takes `{python} OverlapBudgetCalc.backward_per_layer_ms_str` ms. The hierarchical AllReduce for each layer's gradients (~`{python} OverlapBudgetCalc.gradient_per_layer_mb_str` MB per layer) takes `{python} OverlapBudgetCalc.allreduce_per_layer_ms_str` ms using `{python} OverlapBudgetCalc.bucket_size_mb_str` MB buckets. What is the step time with and without overlap?
...
1.  **Gradient size**: `{python} NapkinOverlap.params_b_str` params $\times$ `{python} NapkinOverlap.bytes_per_param_str` bytes (FP16) = `{python} NapkinOverlap.gradient_gb_str` GB.
2.  **Communication time**: Using Ring AllReduce on NVLink (`{python} NapkinOverlap.nvlink_bw_gbs_str` GB/s), time is $\approx 2 \cdot M / \beta$ = `{python} NapkinOverlap.comm_s_str` s.
...
A flat Ring AllReduce treats all links as equal, routing data across `{python} CommInfraRecap.ib_ndr_bw_gbs_str` GB/s InfiniBand when `{python} CommInfraRecap.nvlink_h100_bw_gbs_str` GB/s NVLink is available within the node. For an 8-node cluster with 8 GPUs per node, hierarchical AllReduce reduces inter-node traffic by 8$\times$ compared to flat Ring, achieving roughly a 6$\times$ speedup. Ignoring the bandwidth hierarchy leaves the majority of intra-node bandwidth unused while overloading the scarce inter-node bandwidth.
