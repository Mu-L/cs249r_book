...
- Apply the $\alpha$-$\beta$ model to derive bandwidth and latency bounds and identify when communication dominates at different cluster scales
...
**Systems conclusion**: For Data Parallelism (large gradients), the target is $\beta$: compress gradients and add NICs. For Pipeline/Expert Parallelism (small activations), every microsecond of $\alpha$ matters: kernel bypass and topology optimization. The $\alpha$-$\beta$ model identifies *which constraint to optimize*.
...
class LogPOverlap:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    two_o_us = 2 * o_us
    effective_us = max(compute_us, L_us + two_o_us)
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(two_o_us == 100, f"2o = {two_o_us}, expected 100")
    check(effective_us == 500, f"Effective = {effective_us}, expected 500")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
**Effective time**\index{Effective Time}: max(`{python} LogPOverlap.compute_str`, `{python} LogPOverlap.L_str` + `{python} LogPOverlap.two_o_str`) = `{python} LogPOverlap.effective_str` $\mu$s (communication hidden!). @fig-comm-compute-overlap shows this overlap visually.
...
: **$\alpha$-$\beta$ Predictions vs. Measured NCCL Performance**: For small messages, NCCL's protocol overhead (memory registration, channel setup, kernel launch) inflates the effective latency by 7--8$\times$ over the bare-wire alpha. For large messages, NCCL achieves within 8--15 percent of theoretical bandwidth, validating the model's predictions in the bandwidth-bound regime. Values are illustrative reference numbers for Ring AllReduce on 8-node DGX H100 clusters with InfiniBand NDR 400G; actual measurements vary with topology, NCCL version, and tuning. {#tbl-nccl-vs-theory}
...
3.  **Common pitfall**: A frequent misconception is that one collective algorithm suits all message sizes. Ring AllReduce achieves near-optimal bandwidth for large gradient tensors but has $O(N)$ latency—for small messages like MoE routing decisions (~4 KB), a recursive-halving (Rabenseifner) algorithm reduces latency to $O(\log N)$ steps at the cost of suboptimal bandwidth utilization, requiring workload-specific algorithm selection rather than a single default.
...
#| echo: false
from mlsysim.fmt import check, fmt
from mlsysim.core.constants import KB, MB
...
class MoeAllToAll:
    ...
    kb_per_token = bytes_per_token / 1024  # binary KB (4096 B = 4 KiB)
    ...
    total_sent_mb = (total_sent_kb * KB).m_as(MB)
    bw_time_us = data_per_transfer_kb * 1e3 / (beta_gbs * 1e9) * 1e6
    ...
    check(kb_per_token == 4, f"KB/token = {kb_per_token}")
    check(tokens_per_expert == 64, f"tokens/expert = {tokens_per_expert}")
    check(data_per_transfer_kb == 256, f"data/transfer = {data_per_transfer_kb} KB")
    ...
    kb_per_token_str = fmt(kb_per_token, precision=0, commas=False)
    tokens_per_expert_str = fmt(tokens_per_expert, precision=0, commas=False)
    data_per_transfer_kb_str = fmt(data_per_transfer_kb, precision=0, commas=False)
...
The trace gives the mechanics; the checkpoint tests whether the bandwidth and sequencing consequences are clear.
...
::: {#ws-collective-communication-nccl-topology-discovery .callout-war-story title="NCCL topology discovery"}
...
**Result**: After 5 steps, the system has transmitted **`{python} ErrorFeedbackTrace.naive_str`** but the true cumulative gradient is **`{python} ErrorFeedbackTrace.true_str`**. The parameter never updates, and **100 percent of gradient information is lost**.
...
That payback arithmetic turns compression from a feature toggle into a profiling decision.
