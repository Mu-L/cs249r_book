...
The H100's `{python} H100PerformanceScenario.h100_tflops_str` TFLOPS of FP8 Tensor Core throughput at `{python} H100Specs.tdp_str` W represents a 6.8$\times$ efficiency improvement over V100, achieved through a combination of process technology (TSMC 4N), architectural innovation (Transformer Engine), and precision engineering (FP8 support).
...
| **Hopper** (H100)    |   2022   | Transformer Engine, FP8 |  `{python} H100PerformanceScenario.h100_tflops_str` TFLOPS   |         `{python} H100Specs.tdp_str` W         | `{python} H100Specs.nvlink_str` GB/s |
...
1. **Compute Time**\index{Compute!compute time}: Each token requires $2 \times 70\text{B} = 1.4 \times 10^{11}$ floating-point operations. At `{python} InfraSetup.h100_tflops_str` TFLOPS of low-precision Tensor Core peak throughput, the arithmetic takes:
...
The roofline in @fig-roofline-landscape is defined by two ceilings: memory bandwidth (diagonal) and peak compute (horizontal), meeting at the FP16 ridge point (~`{python} InfraSetup.h100_ridge_str` FLOP/byte for `{python} InfraSetup.h100_tflops_str` TFLOPS over `{python} InfraSetup.h100_bw_str` TB/s). ML workloads span the full range: LLM decode at batch size 1 is deeply memory-bound, while LLM training at large batch sizes is compute-bound. The position of each workload determines whether faster memory or faster compute would improve performance.
...
The result is that the 8 GPUs within a DGX H100 can be treated as a single logical device with `{python} NodeCapacityScenario.node_hbm_cap_str` GB of aggregate HBM and a combined compute throughput of 8$\times$ `{python} NodeCapacityScenario.h100_tflops_str` TFLOPS. This abstraction is critical for software: the training framework can partition a model across the 8 GPUs using tensor parallelism as if they were a single larger processor, with the NVSwitch fabric making the partitioning nearly transparent from a performance perspective.
...
2. **Cluster throughput**: 8,192 H100 GPUs at `{python} NodeCapacityScenario.h100_tflops_str` TFLOPS peak, operating at 45 percent MFU:
...
- **Per-GPU compute time** on 1,024 GPUs, each at `{python} NodeCapacityScenario.h100_tflops_str` TFLOPS (50 percent utilization):
...
- H100 GPU: `{python} NodeCapacityScenario.h100_tflops_str` TFLOPS peak, assume 45 percent MFU
