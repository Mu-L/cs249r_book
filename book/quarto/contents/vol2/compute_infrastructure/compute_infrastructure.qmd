...
class TcoScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_cost_str = MarkdownStr(f"${fmt(cloud_node_annual, precision=0)}")
    annual_elec_str = MarkdownStr(f"${fmt(annual_elec, precision=0)}")
    onprem_str = MarkdownStr(f"${fmt(onprem_node_annual, precision=0)}")
