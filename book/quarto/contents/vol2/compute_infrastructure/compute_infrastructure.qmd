...
1. **Compute Time**\index{Latency!compute time}: Each token requires $2 \times 70\text{B} = 1.4 \times 10^{11}$ floating-point operations. At `{python} InfraSetup.h100_tflops_str` TFLOPS of low-precision Tensor Core peak throughput, the arithmetic takes:
...
**Setup**: Training a `{python} InfraSetup.frontier_params_b_str`B model on a DGX H100 cluster with `{python} InfraSetup.ib_bw_str` Gbps InfiniBand per GPU.
