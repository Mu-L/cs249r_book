...
The Blackwell architecture (2024) continued this trajectory with the B200, which pairs two GPU dies in a single package via a high-bandwidth chip-to-chip link, effectively creating a "dual-die GPU" that delivers about `{python} B200PerformanceScenario.fp8_tflops_str` TFLOP/s of dense FP8 Tensor Core throughput at `{python} B200TdpRecap.tdp_w_str` W. FP16/BF16 comparisons should use the lower precision-specific vendor figure rather than the FP8 peak.
...
class V100TableRecap:
    ...
    flops_fp16_str = fmt(h_v100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    tdp_str = fmt(h_v100.tdp.m_as(watt), precision=0, commas=False)
    nvlink_str = fmt(h_v100.nvlink.bandwidth.m_as(GB/second), precision=0, commas=False)
```
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthPitfallRecap.h100_bw_str
...
| **Bandwidth**\index{HBM!bandwidth}      | ~`{python} MemoryBandwidthDdrRecap.ddr_bw_str` GB/s | **~`{python} MemoryBandwidthScenarioSecComputeMemoryWallRecap.h100_bw_str` TB/s** |    **~17$\times$ Faster**    |
...
**Systems insight**: The processor spends **`{python} TokenLatencyDominanceRecap.dominance_str` percent** of its time waiting for data from memory. The arithmetic units are idle for almost the entire token generation. Even a hypothetical processor with *infinite* compute throughput would generate tokens only negligibly faster, because the memory transfer time dominates completely. This is *why* HBM bandwidth improvements deliver nearly linear speedups for inference workloads.
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthRooflineFnRecap.h100_bw_str
...
Consider our `{python} InfraFrontierMemoryMxRecap.frontier_params_b_str`B model on an H100 with `{python} RooflineInferenceMath.h100_tflops_str` TFLOP/s peak compute and `{python} RooflineServingRecap.h100_bw_str` TB/s memory bandwidth. The ridge point is `{python} RooflineCalloutRecap.h100_ridge_str` FLOP/byte.
...
class TensorCoreTdpRecap:
    ...
    h100_fp32_cuda_str = fmt(Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs/second)/64, precision=0, commas=False)
```
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthScenarioSecComputeMemoryWallRecap.h100_bw_str
class MemoryBandwidthScenarioSecComputeMemoryWallRecap:
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: B200TdpRecap.tdp_w_str
class B200TdpRecap:
...
The A100 (2020) increased to `{python} A100TdpTrajectoryRecap.tdp_str` W. The H100 (2022) reached `{python} H100Specs.tdp_str` W.
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: RooflineServingRecap.h100_bw_str
class RooflineServingRecap:
...
the lower TDP (`{python} A100ServingTdpRecap.tdp_str` W vs. `{python} H100Specs.tdp_str` W) allows higher rack density and lower cooling costs.
...
class BandwidthHierarchyTableRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_str = fmt(NVLINK_H100_BW.m_as(GB/second), precision=0, commas=False)
    pcie_bw_str = fmt(PCIE_GEN5_X16_BW.m_as(GB/second), precision=0, commas=False)
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: NodeCapacityDenseRecap.node_hbm_cap_str
...
The result is that the 8 GPUs within a DGX H100 can be treated as a single logical device with `{python} NodeCapacityDenseRecap.node_hbm_cap_str` GB of aggregate HBM and a combined compute throughput of 8$\times$ `{python} NodeCapacityPodRecap.h100_tflops_str` TFLOP/s. This abstraction is critical for software: the training framework can partition a model across the 8 GPUs using tensor parallelism as if they were a single larger processor, with the NVSwitch fabric making the partitioning nearly transparent from a performance perspective.
...
- **CPU Offloading**\index{CPU Offloading}: Optimizer states, which are accessed only during the parameter update step (once per training step), can be stored in host DDR5 DRAM and transferred to GPU HBM only when needed. The PCIe Gen5 link at `{python} BandwidthHierarchyPcieRecap.pcie_bw_str` GB/s introduces a transfer delay, but since the parameter update is a small fraction of total step time, the overhead is manageable.
...
1. **Intra-Node (NVLink)**: Bandwidth = `{python} BandwidthHierarchyNvlinkRecap.nvlink_bw_str` GB/s.
...
2. **Cluster throughput**: 8,192 H100 GPUs at `{python} NodeCapacityPodRecap.h100_tflops_str` TFLOP/s peak, operating at 45 percent MFU:
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: NodeCapacityPodRecap.h100_tflops_str
class NodeCapacityPodRecap:
...
| **H100 (2022)** |       `{python} TensorCoreEfficiencyRecap.h100_fp8_tc_str` (FP8) |                    `{python} H100Specs.tdp_str` | `{python} GpuEfficiencyTableRecap.h100_ef_str` |       6.8$\times$       |
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TcoSummaryRecap.breakeven_util_str
class TcoSummaryRecap:
    cloud_h100_hourly = 4.0
    dgx_h100_price = 350000
    node_power_kw = (Hardware.Cloud.H100.tdp * GPUS_PER_HOST).m_as(kilowatt) * 1.2
    denominator = (cloud_h100_hourly * GPUS_PER_HOST * HOURS_PER_YEAR) - (node_power_kw * HOURS_PER_YEAR * 0.07 * 1.1)
    breakeven_util = (dgx_h100_price / 3) / denominator * 100

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    breakeven_util_str = fmt(breakeven_util, precision=1, commas=False)
```
