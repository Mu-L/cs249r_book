...
class InfraSetup:
    ...
    # OUTPUTS
    h100_tflops_str = fmt(h100_tflops_val, precision=0, commas=False)
    h100_bw_str = fmt(h100_bw_val, precision=2, commas=False)
    h100_ridge_str = fmt(h100_ridge_val, precision=0, commas=False)
    gpt3_params_b_str = fmt(gpt3_params_b_val, precision=0, commas=False)
    frontier_params_b_str = fmt(frontier_params_b_val, precision=0, commas=False)
    ib_bw_str = fmt(ib_bw_gbps_val, precision=0, commas=False)
    ddr_bw_str = MarkdownStr("200")  # Approximate high-end DDR5 server bandwidth
    dominance_str = fmt(dominance, precision=1, commas=False)

    # Token latency math blocks (used in @nbk-token-latency)
    t_comp_math = fmt_math(f"T_{{\\text{{comp}}}} \\approx \\mathbf{{{t_comp_ms:.2f} \\text{{ ms}}}}")
...
| **Blackwell** (B200) |   2024   | Dual-die, NVLink 5      | `{python} B200PerformanceScenario.fp8_tflops_str` TFLOP/s FP8 | `{python} B200PerformanceScenario.tdp_w_str` W |              1,800 GB/s              |
...
class MemoryBandwidthScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ddr_bw_str = fmt(ddr_gbs, precision=0, commas=False)
...
| **Bandwidth**\index{Bandwidth} | ~`{python} MemoryBandwidthScenario.ddr_bw_str` GB/s | **~`{python} MemoryBandwidthScenario.h100_bw_str` TB/s** |    **~17$\times$ Faster**    |
...
1. **Compute Time**\index{Compute!compute time}: Each token requires $2 \times 70 \times 10^9 = 1.4 \times 10^{11}$ floating-point operations. At `{python} InfraSetup.h100_tflops_str` TFLOP/s of low-precision Tensor Core peak throughput, the arithmetic takes:
   `{python} InfraSetup.t_comp_math`
2. **Memory Time**\index{Memory!time}: Loading 140 GB of weights from HBM at `{python} MemoryBandwidthScenario.h100_bw_str` TB/s takes:
...
::: {#nbk-compute-infrastructure-memory-wall-h100-vs-h200 .callout-notebook title="The memory wall: H100 vs. H200"}

\index{NVIDIA!h200, memory wall}\index{LLM!inference, memory bottleneck}To understand *why* the "memory wall" is the primary constraint for modern large language models (LLMs), we compare the NVIDIA H100 against its successor, the H200. While both chips share the same compute cores (same peak TFLOP/s), the H200 provides `{python} H200Comparison.bw_gain_str` higher memory bandwidth and `{python} H200Comparison.cap_gain_str` more HBM capacity.
...
2.  **Distinction (durable)**: Unlike **Peak FLOPs**\index{Peak FLOP/s} (which only describes the horizontal ceiling), the ridge point describes the **Balance** of the architecture. A rising ridge point over hardware generations indicates that compute is growing faster than bandwidth, making utilization harder.
...
# ┌── 2. ARRAYS ────────────────────────────────────────────────────────────────
P_PEAK_TFLOPS = RooflineLandscapeMath.peak_tflops
...
class RooflineScenario:
    ...
    h100_ridge_str = fmt(ridge, precision=0, commas=False)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import BYTES_FP16, TFLOPs, TB, GB, second, byte
...
class TensorCoreH100Specs:
    ...
    mfu_example_tokens_str = fmt(mfu_tokens_per_step, precision=0)
    ...
    # stays in sync if H100_FLOPS_FP16_TENSOR or the example anchors
...
1.  **Significance (quantitative)**: $\text{MFU} = C_{\text{step}} / (R_{\text{peak}} \cdot T_{\text{step}})$, where $C_{\text{step}} \approx 6PN_{\text{tokens}}$ FLOPs for a $P$-parameter transformer processing $N_{\text{tokens}}$ tokens in the step. For a 7B-parameter model on H100 ($R_{\text{peak}} =$ `{python} TensorCoreH100Specs.h100_fp16_tc_str` TFLOP/s FP16) processing `{python} TensorCoreH100Specs.mfu_example_tokens_str` tokens per step (a batch of 4 sequences of 2,048 tokens) with a step time of 1.0 second: `{python} TensorCoreH100Specs.mfu_example_math` $\approx 35\%$. A practical ceiling of 55-65 percent applies even in highly optimized implementations; the gap represents inescapable memory stalls ($\text{BW}$ bottleneck), AllReduce communication time $(T_{\text{comm}})$, and pipeline bubbles.
...
class GpuEfficiencyScenario:
    ...
    b200_fp8_tflops_str = fmt(b200_low_precision.m_as(TFLOPs/second), precision=0, commas=False)
    b200_tdp_w_str = fmt(b200_tdp.m_as(watt), precision=0, commas=False)
```
...
::: {#fig-node-topology-comparison fig-env="figure" fig-pos="htb" fig-cap="**Intra-Node GPU Topology Comparison**: Three approaches to wiring GPUs inside a node. (Left) Ring: sequential pass-through, ~50 GB/s per link, latency scales with distance. (Center) Full Mesh: all-to-all direct links, $N_{\text{GPU}}-1$ links per GPU, impractical beyond ~8 GPUs. (Right) NVSwitch Crossbar: non-blocking fabric with 112 GB/s per NVLink and 900 GB/s aggregate, any-GPU-to-any-GPU in one hop, enabling tensor-parallel AllReduce within a node." fig-alt="Three panels comparing intra-node GPU topologies. Left: Ring with 8 GPUs and arrows around the loop. Center: Full Mesh showing all-to-all links between 8 GPUs. Right: 8 GPUs connected through a central NVSwitch crossbar fabric."}
...
class BandwidthHierarchyScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_str = fmt(nvlink_gbs, precision=0, commas=False)
    pcie_bw_str = fmt(pcie_gbs, precision=0, commas=False)
    ib_bw_str = fmt(ib_gbs, precision=0, commas=False)
```
...
class NodeCapacityScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_str = fmt(total_mem.m_as(GiB), precision=0, commas=False)
    h100_tflops_str = fmt(flops.m_as(TFLOPs/second), precision=0, commas=False)
```
...
class NodeReliabilityScenario:
    ...
    canonical_gpu_mtbf_hours = 50_000
    gpus_per_node = GPUS_PER_HOST
    recovery_low_min = 10
    ...
    expected_failures_str = fmt(expected_failures, precision=0, commas=False)
    ...
    restart_overhead_low_str = fmt(restart_overhead_low_pct, precision=0, commas=False)
    restart_overhead_high_str = fmt(restart_overhead_high_pct, precision=0, commas=False)
```
...
::: {#exmp-compute-infrastructure-memory-budget-175b-training-dgx-h100 .callout-example title="Memory budget for 175B training on DGX H100"}
...
4. **Transfer**\index{Transfer}: The assembled batch is transferred from host DRAM to GPU HBM over PCIe. For tokenized language modeling, this transfer is usually small because token IDs and masks are compact; multimodal batches or pipelines that materialize FP16 embeddings on the host can reach 100--200 MB and take milliseconds at PCIe Gen5 bandwidth.
...
1. **Silicon Path**\index{Silicon Path}: GPU supply chains are volatile, but typical enterprise lead times are **`{python} GridQueue.gpu_lead_time_months_str` months**.
2. **Infrastructure Path**\index{Infrastructure Path}: Permitting, EPC (Engineering, Procurement, Construction), and grid connection for a new 10+ MW substation averages **`{python} GridQueue.substation_lead_time_months_str` months**.
...
::: {#ws-compute-infrastructure-power-ramp-crash .callout-war-story title="The power ramp crash"}

**The context**: In one of the early large-scale training deployments, a 512-GPU cluster experienced intermittent node failures during the first week of operation. The failures occurred at random intervals, with no apparent correlation to the model or software.

**The failure**: Hardware diagnostics showed no component defects. The root cause turned out to be the data center's power infrastructure: the cluster's synchronous training steps created a few-hundred-kilowatt load step -- roughly 300 kW for a 600 W idle-to-peak swing across 512 GPUs before facility overhead -- that exceeded the response time of the facility's UPS battery inverters.

**The consequence**: The resulting voltage sags (drops of 5--8 percent lasting 10--50 ms) triggered the GPUs' undervoltage protection circuits, forcing hard resets. The fix required installing supercapacitor banks capable of delivering 500 kW for 100 ms, smoothing the transient until the UPS could respond.

**The systems lesson**: ML clusters do not behave like traditional data center workloads: the *temporal correlation* of power demand across thousands of chips creates a qualitatively different electrical engineering challenge.
...
class WscThroughput:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    n_gpus = 10_000
    per_gpu_inject_gbs = 50            # InfiniBand NDR 400 Gb/s = 50 GB/s
...
class CheckpointOverheadBudget:
    ...
    ckpt_required_bw_gbs_str           = fmt(required_bw_gbs, precision=0, commas=False)
    ckpt_reference_write_s_str         = fmt(reference_write_s, precision=0, commas=False)
    ckpt_reference_bw_gbs_str          = fmt(reference_bw_gbs, precision=0, commas=False)
    ...
    ckpt_sustained_bw_gbs_str          = fmt(sustained_bw_gbs, precision=0, commas=False)
```
...
- **Total checkpoint data**\index{Total Checkpoint Data}: `{python} CheckpointOverheadBudget.ckpt_run_count_str` $\times$ `{python} CheckpointOverheadBudget.ckpt_total_tb_str` TB = `{python} CheckpointOverheadBudget.ckpt_run_total_tb_str` TB
...
Before examining specific pod architectures, we must understand the metric that determines whether scaling from one node to many is worthwhile: **scaling efficiency**. If a model trains in time $T_1$ on one node, ideal (linear) scaling would train it in time $T_1/N_{\text{node}}$ on $N_{\text{node}}$ nodes. In practice, communication overhead, pipeline bubbles, and load imbalances reduce the speedup. Scaling efficiency is defined as:

$$\eta_{\text{scaling}} = \frac{T_1}{N_{\text{node}} \times T_{N_{\text{node}}}}$$ {#eq-cluster-scaling-efficiency}

where $T_{N_{\text{node}}}$ is the training time on $N_{\text{node}}$ nodes. An efficiency of 1.0 means perfect linear scaling; an efficiency of 0.5 means we achieve only half the expected speedup.

::: {#dfn-compute-infrastructure-scaling-efficiency .callout-definition title="Scaling efficiency"}

***Scaling Efficiency $(\eta_{\text{scaling}})$***\index{Scaling Efficiency!definition} is the ratio of actual throughput to ideal linear throughput when increasing the number of compute nodes ($N_{\text{node}}$).

1.  **Significance (quantitative)**: It is the most important metric for cluster productivity ($\eta_{\text{scaling}} = \frac{T_1}{N_{\text{node}} \times T_{N_{\text{node}}}}$). A scaling efficiency of 0.50 means that a 10,000-GPU cluster is delivering only the same useful work as a 5,000-GPU cluster, wasting 50 percent of the hardware investment.
2.  **Distinction (durable)**: Unlike **single-node efficiency** (which captures local bottlenecks like $\text{BW}$), scaling efficiency captures the **cluster-level overhead** of communication time $(T_{\text{comm}})$ and synchronization.
3.  **Common pitfall**: A frequent misconception is that scaling efficiency is constant. In reality, it is a **function of problem size**\index{Function of Problem Size}: as $N_{\text{node}}$ increases, the communication-to-compute ratio typically worsens (Amdahl's Law), making it harder to maintain high efficiency for small models.
...
class CommScalingMath:
    ...
    comm_time_formula_math = fmt_math(f"2 \\times (N_{{\\text{{node}}}}-1)/N_{{\\text{{node}}}} \\times 350 / {_ib_gbs}")
    comm_approx_math = fmt_math(f"2 \\times 350 / {_ib_gbs}")
    compute_per_step_math = fmt_math(f"C = 6 \\times {InfraSetup.gpt3_params_b_str} \\times 10^9 \\times 2 \\times 10^6 \\approx 2.1 \\times 10^{{18}}")
    t_compute_math = fmt_math(f"T_{{\\text{{compute}}}} \\approx 2.1")
    t_comm_math = fmt_math(f"T_{{\\text{{comm}}}} \\approx {_effective_time_s:.1f}")
```

For data-parallel training of our `{python} InfraSetup.frontier_params_b_str`B model, the communication cost per step is dominated by the AllReduce of 350 GB of gradients. Using ring-AllReduce over InfiniBand at `{python} BandwidthHierarchyScenario.ib_bw_str` GB/s effective bandwidth, the raw communication time is approximately `{python} CommScalingMath.comm_time_formula_math`, which for large $N_{\text{node}}$ approaches `{python} CommScalingMath.comm_approx_math` seconds. With 75 percent overlap between gradient communication and the backward pass, the effective exposed communication time drops to `{python} CommScalingMath.t_comm_math` seconds. Under the same assumptions, compute takes `{python} CommScalingMath.t_compute_math` seconds, so the exposed step time is about 5.6 seconds and naive data-parallel scaling efficiency is $2.1/5.6 \approx 0.375$. Production systems recover much of this loss by combining tensor parallelism, pipeline parallelism, topology-aware placement, and more effective communication overlap, achieving 70--90 percent scaling efficiency for well-optimized training systems.
...
class FabricSizingScenario:
    ...
    endpoint_links_per_gpu_str = fmt(endpoint_links_per_gpu, precision=0, commas=False)
...
class BuildVsBuyFigureScenario:
    ...
    breakeven_month_str = fmt(breakeven_month, precision=0, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (H100_TDP, GPUS_PER_HOST,
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import H100_TDP, GPUS_PER_HOST, HOURS_PER_YEAR, kilowatt
...
class TcoScenario:
    ...
    annual_elec_str = fmt(annual_elec, precision=0, prefix="$")
    onprem_str = fmt(onprem_node_annual, precision=0, prefix="$")
    util_pct_str = fmt(utilization*100, precision=0, commas=False)
    breakeven_util_str = fmt(breakeven, precision=0, commas=False)
```
...
class TenKGpuClusterTco:
    ...
    annual_opex_str = fmt(annual_opex_usd / 1e6, precision=1, prefix="$", suffix="M")
    three_year_total_str = fmt(three_year_total_usd / 1e6, precision=0, prefix="$", suffix="M")
    cloud_annual_str = fmt(cloud_annual_usd / 1e6, precision=0, prefix="$", suffix="M")
    cloud_three_year_str = fmt(cloud_three_year_usd / 1e6, precision=0, prefix="$", suffix="M")
    savings_str = fmt(savings_usd / 1e6, precision=0, prefix="$", suffix="M")
    breakeven_util_str = fmt(breakeven_utilization * 100, precision=0, commas=False)
```
...
- **Hardware CapEx**\index{Hardware CapEx}: `{python} TenKGpuClusterTco.nodes_str` nodes$\times$ \$350,000 = `{python} TenKGpuClusterTco.hardware_capex_str`
- **Network CapEx**\index{Network CapEx}: ~`{python} TenKGpuClusterTco.network_capex_str` (InfiniBand fat-tree fabric)
...
# │ How: total_flops = 6 * params * tokens; sustained = peak * MFU * GPUs.
# │ Exports: PlanningThroughputScenario.*_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import H100_FLOPS_FP16_TENSOR, TFLOPs, PFLOPs, second, SEC_PER_DAY
...
class PlanningThroughputScenario:
    ...
    mfu = 0.45
    gpus = 1_024
    h100_peak_tflops = H100_FLOPS_FP16_TENSOR.m_as(TFLOPs / second)
    ...
    per_gpu_sustained_tflops_str = fmt(per_gpu_sustained_tflops, precision=0, commas=False)
    gpus_str = fmt(gpus, precision=0)
    cluster_sustained_pflops_str = fmt(cluster_sustained_pflops, precision=0, commas=False)
