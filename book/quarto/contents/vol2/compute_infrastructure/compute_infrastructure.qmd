...
# │ Exports: (none)
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import MarkdownStr, fmt, fmt_sci, check, fmt_math
```
...
class V100NvlinkRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_str = fmt(Hardware.Cloud.V100.nvlink.bandwidth.m_as(GB/second), precision=0, commas=False)
```

The Ampere architecture (2020) expanded Tensor Core support to additional data types (TF32, BF16, INT8, INT4), reflecting the growing importance of mixed-precision training and quantized inference. The A100 also introduced Multi-Instance GPU (MIG), which partitions a single GPU into up to seven isolated instances, enabling efficient sharing of expensive hardware across multiple inference workloads. The most consequential change for infrastructure was Ampere's third-generation NVLink, which doubled bidirectional bandwidth to `{python} A100Specs.nvlink_str` GB/s per GPU from Volta's `{python} V100NvlinkRecap.nvlink_str` GB/s, directly addressing the communication wall for tensor parallelism.
...
class H100PerformanceScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(h100_tf, precision=0, commas=False)
```

The H100's `{python} H100PerformanceScenario.h100_tflops_str` TFLOP/s of FP8 Tensor Core throughput at `{python} H100Specs.tdp_str` W represents a 6.8$\times$ efficiency improvement over V100, achieved through a combination of process technology (TSMC 4N), architectural innovation (Transformer Engine), and precision engineering (FP8 support).
...
class B200PerformanceScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fp8_tflops_str = fmt(fp8_tflops, precision=0)
    tdp_w_str = fmt(tdp_w, precision=0, commas=False)
    nvlink_str = fmt(nvlink_gbs, precision=0)
```

The Blackwell architecture (2024) continued this trajectory with the B200, which pairs two GPU dies in a single package via a high-bandwidth chip-to-chip link, effectively creating a "dual-die GPU" that delivers about `{python} B200PerformanceScenario.fp8_tflops_str` TFLOP/s of dense FP8 Tensor Core throughput at `{python} B200PerformanceScenario.tdp_w_str` W. FP16/BF16 comparisons should use the lower precision-specific vendor figure rather than the FP8 peak.
...
class B200NvlinkRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_str = fmt(Hardware.Cloud.B200.nvlink.bandwidth.m_as(GB/second), precision=0, commas=False)
```

Blackwell also introduced fifth-generation NVLink at `{python} B200NvlinkRecap.nvlink_str` GB/s per GPU, doubling the intra-node bandwidth again. The die-to-die link within the B200 package operates at 10 TB/s, fast enough that the two dies appear as a single logical GPU to the software, with no performance penalty for operations that span the die boundary.
...
|:---------------------|:--------:|:------------------------|:-------------------------------------------------------------:|:----------------------------------------------:|:--------------------------------------------------:|
| **Volta** (V100)     |   2017   | First Tensor Core       |        `{python} V100TableRecap.flops_fp16_str` TFLOP/s       |      `{python} V100TableRecap.tdp_str` W       |     `{python} V100TableRecap.nvlink_str` GB/s      |
| **Ampere** (A100)    |   2020   | Multi-precision, MIG    |          `{python} A100Specs.flops_fp16_str` TFLOP/s          |         `{python} A100Specs.tdp_str` W         |        `{python} A100Specs.nvlink_str` GB/s        |
| **Hopper** (H100)    |   2022   | Transformer Engine, FP8 |   `{python} H100PerformanceScenario.h100_tflops_str` TFLOP/s  |         `{python} H100Specs.tdp_str` W         |        `{python} H100Specs.nvlink_str` GB/s        |
| **Blackwell** (B200) |   2024   | Dual-die, NVLink 5      | `{python} B200PerformanceScenario.fp8_tflops_str` TFLOP/s FP8 | `{python} B200PerformanceScenario.tdp_w_str` W | `{python} B200PerformanceScenario.nvlink_str` GB/s |

: **NVIDIA GPU Architecture Evolution**: Each generation introduces architectural features targeted at the dominant ML workload pattern of its era. Efficiency rises substantially but unevenly across generations, while NVLink bandwidth increases from 300 GB/s on V100 to `{python} B200PerformanceScenario.nvlink_str` GB/s on B200, with a smaller 1.5$\times$ A100-to-H100 step between the larger doubling jumps. {#tbl-gpu-evolution}
...
from mlsysim import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: MemoryBandwidthCacheRecap.h100_bw_str
...
Further down is the 50 MB L2 cache (~12 TB/s), and

finally the 80 GB of HBM3 at `{python} MemoryBandwidthCacheRecap.h100_bw_str` TB/s.
...
class V100MemoryWallRecap:
    h_v100 = Hardware.Cloud.V100
    mem_bw_str = fmt(h_v100.memory.bandwidth.m_as(GB/second), precision=0, commas=False)
    flops_fp16_str = fmt(h_v100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
...
class MemoryBandwidthPitfallRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_bw_str = fmt(Hardware.Cloud.H100.memory.bandwidth.m_as(TB/second), precision=2, commas=False)
    h100_flops_fp16_str = fmt(Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
```

3.  **Common pitfall**: A frequent misconception is that HBM solves the memory wall. HBM moves the wall rather than eliminates it: Tensor Core throughput $(R_{\text{peak}})$ has scaled faster than HBM bandwidth across generations (from `{python} V100MemoryWallRecap.mem_bw_str` GB/s with `{python} V100MemoryWallRecap.flops_fp16_str` FP16 TFLOP/s in V100 to `{python} MemoryBandwidthPitfallRecap.h100_bw_str` TB/s with `{python} MemoryBandwidthPitfallRecap.h100_flops_fp16_str` FP16 TFLOP/s in H100), so the ridge point continues rising and more operations remain memory-bound despite HBM improvements.
...
class MemoryBandwidthDdrRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ddr_bw_str = fmt(HOST_DRAM_BW.m_as(GB/second), precision=0, commas=False)
```

Traditional DDR memory connects to the processor through pins on the edge of a printed circuit board (PCB). Each DIMM communicates over a 64-bit bus, and even with multiple channels (8 channels is typical for a high-end server CPU), a modern server tops out at roughly `{python} MemoryBandwidthDdrRecap.ddr_bw_str` GB/s of aggregate memory bandwidth.
...
| **Bandwidth**\index{HBM!bandwidth}      | ~`{python} MemoryBandwidthDdrRecap.ddr_bw_str` GB/s | **~`{python} MemoryBandwidthPitfallRecap.h100_bw_str` TB/s** |    **~17$\times$ Faster**    |
...
class A100HbmQuantRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    model_params_b_str = fmt(params / BILLION, precision=0, commas=False)
    weights_gb_str = fmt(weights_gb, precision=0, commas=False)
    mem_bw_tbs_str = fmt(a100_bw_tbs, precision=2, commas=False)
    a100_token_ms_str = fmt(a100_token_ms, precision=1, commas=False)
    h100_bw_tbs_str = fmt(h100_bw_tbs, precision=2, commas=False)
    h100_token_ms_str = fmt(h100_token_ms, precision=1, commas=False)
    b200_bw_tbs_str = fmt(b200_bw_tbs, precision=1, commas=False)
    b200_token_ms_str = fmt(b200_token_ms, precision=1, commas=False)
```
...
A `{python} A100HbmQuantRecap.model_params_b_str`B FP16 model has `{python} A100HbmQuantRecap.weights_gb_str` GB of weights, so a bandwidth-floor calculation gives roughly `{python} A100HbmQuantRecap.weights_gb_str` GB/`{python} A100HbmQuantRecap.mem_bw_tbs_str` TB/s = `{python} A100HbmQuantRecap.a100_token_ms_str` ms per token at A100-class bandwidth, `{python} A100HbmQuantRecap.weights_gb_str` GB/`{python} A100HbmQuantRecap.h100_bw_tbs_str` TB/s = `{python} A100HbmQuantRecap.h100_token_ms_str` ms at H100-class bandwidth, and `{python} A100HbmQuantRecap.weights_gb_str` GB/`{python} A100HbmQuantRecap.b200_bw_tbs_str` TB/s = `{python} A100HbmQuantRecap.b200_token_ms_str` ms at B200-class bandwidth. The A100 and H100 cases do not imply that the full FP16 model fits on one device; they isolate the latency floor imposed by a single accelerator-equivalent HBM path. For interactive applications (chatbots, code assistants, real-time translation), where users perceive delays above 50 ms as "slow," these bandwidth improvements translate directly into better user experience and into the ability to serve larger models within latency budgets.
...
class TokenLatencyScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(h_h100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    dominance_str = fmt(dominance, precision=1, commas=False)
    weight_gb_str = fmt(weight_vol.m_as(GB), precision=0, commas=False)
...
**Problem**: Generating one token from Llama-3 70B (`{python} TokenLatencyScenario.weight_gb_str` GB of FP16 weights) requires both arithmetic and a full weight load from HBM. Assuming capacity has been solved (by sharding, a higher-capacity device, or quantization), which dominates the per-token time on an H100: compute or memory transfer?

1. **Compute Time**\index{Latency!compute time}: Each token requires $2 \times 70 \times 10^9 = 1.4 \times 10^{11}$ floating-point operations. At `{python} TokenLatencyScenario.h100_tflops_str` TFLOP/s of low-precision Tensor Core peak throughput, the arithmetic takes:
...
Loading `{python} TokenLatencyScenario.weight_gb_str` GB of weights from HBM at `{python} MemoryBandwidthLatencyRecap.h100_bw_str` TB/s takes:
   `{python} TokenLatencyScenario.t_mem_math`

**Systems insight**: The processor spends **`{python} TokenLatencyScenario.dominance_str` percent** of its time waiting for data from memory. The arithmetic units are idle for almost the entire token generation. Even a hypothetical processor with *infinite* compute throughput would generate tokens only negligibly faster, because the memory transfer time dominates completely. This is *why* HBM bandwidth improvements deliver nearly linear speedups for inference workloads.
...
class MemoryBandwidthRooflineFnRecap:
    ...
    h100_bw_str = fmt(Hardware.Cloud.H100.memory.bandwidth.m_as(TB/second), precision=2, commas=False)
    h100_flops_fp16_str = fmt(Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    h100_ridge_str = fmt((Hardware.Cloud.H100.peak_flops/Hardware.Cloud.H100.memory.bandwidth).to(flop/byte).magnitude, precision=1, commas=False)
```
...
class RooflineLandscapeMath:
    ...
    bandwidth_tbs = h_h100.memory_bw.m_as(TB/second)
    ridge = (h_h100.peak_flops / h_h100.memory_bw).to(flop/byte).magnitude
...
class RooflineCaptionRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(h_h100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    h100_bw_str = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False)
    h100_ridge_str = fmt((h_h100.peak_flops/h_h100.memory_bw).to(flop/byte).magnitude, precision=1, commas=False)
```

The roofline in @fig-roofline-landscape is defined by two ceilings: memory bandwidth (diagonal) and peak compute (horizontal), meeting at the FP16 ridge point (~`{python} RooflineCaptionRecap.h100_ridge_str` FLOP/byte for `{python} RooflineCaptionRecap.h100_tflops_str` TFLOP/s over `{python} RooflineCaptionRecap.h100_bw_str` TB/s). ML workloads span the full range: LLM decode at batch size 1 is deeply memory-bound, while LLM training at large batch sizes is compute-bound. The position of each workload determines whether faster memory or faster compute would improve performance.
...
class RooflineInferenceMath:
    ...
    batch_size = 2048
    h_h100 = Hardware.Cloud.H100
    h100_bw_tbs = h_h100.memory_bw.m_as(TB/second)
    h100_peak_tflops = h_h100.peak_flops.m_as(TFLOPs/second)
    ridge = (h_h100.peak_flops / h_h100.memory_bw).to(flop/byte).magnitude
    params_b_str = fmt(params / BILLION, precision=0, commas=False)
    h100_bw_str = fmt(h100_bw_tbs, precision=2, commas=False)
    h100_tflops_str = fmt(h100_peak_tflops, precision=0, commas=False)
...
class TensorCoreH100Specs:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    fp16_tc_tflops = Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs / second)
    fp8_tc_tflops = Hardware.Cloud.H100.compute.precision_flops["fp8"].m_as(TFLOPs / second)
    fp32_cuda_tflops = Hardware.Cloud.H100.compute.precision_flops["fp32_cuda"].m_as(TFLOPs / second)
    ...
    tc_vs_cuda_ratio_str = fmt(ratio, precision=1, commas=False)
```
...
1.  **Significance (quantitative)**: Tensor Cores provide the bulk of the H100's `{python} TensorCoreTdpRecap.h100_fp16_tc_str` TFLOP/s FP16/BF16 peak (and ~`{python} TensorCoreTdpRecap.h100_fp8_tc_str` TFLOP/s at FP8)—roughly `{python} TensorCoreTdpRecap.tc_vs_cuda_ratio_str`$\times$ the ~`{python} TensorCoreTdpRecap.h100_fp32_cuda_str` TFLOP/s delivered by CUDA (vector) cores alone. However, this peak is only achievable for operations that decompose into tiled matrix multiplications. Layer normalization and softmax, which involve reductions and element-wise operations rather than matrix multiplications, run at approximately 3–8 percent of peak throughput on the same hardware.
...
Even with the H100's `{python} MemoryBandwidthTensorRecap.h100_bw_str` TB/s HBM bandwidth, the Tensor Cores can consume data faster than HBM can deliver it for certain matrix dimensions, creating idle cycles while the arithmetic units wait for data.
...
class H100TdpWallRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt(Hardware.Cloud.H100.tdp.m_as(watt), precision=0, commas=False)
    flops_fp16_str = fmt(Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
```
...
class V100DennardRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt(Hardware.Cloud.V100.tdp.m_as(watt), precision=0, commas=False)
```

[^fn-dennard-scaling]: **Dennard Scaling**: Named after @dennard1974 at IBM, who observed that shrinking transistors could maintain constant power density because voltage and current scale with dimensions. The breakdown around 2006 -- when subthreshold leakage and gate-oxide tunneling prevented further voltage reduction below ~28 nm -- is the direct cause of the V100-to-B200 TDP trajectory (`{python} V100DennardRecap.tdp_str` W to 1,000 W). For ML infrastructure, the post-Dennard era means every TFLOP/s gain arrives with a proportional watt increase, making cooling and power delivery co-equal design constraints with the silicon itself. \index{Dennard Scaling!power wall}
...
class A100TdpTrajectoryRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt(Hardware.Cloud.A100.tdp.m_as(watt), precision=0, commas=False)
```
The numbers are stark. The V100 (2017) operated at `{python} V100TdpRecap.tdp_str` W TDP.
...
The A100 (2020) increased to `{python} A100TdpTrajectoryRecap.tdp_str` W. The H100 (2022) reached `{python} H100TdpTrajectoryRecap.tdp_str` W.
...
class B200PerformanceScenarioSecComputeTdpRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_w_str = fmt(Hardware.Cloud.B200.tdp.m_as(watt), precision=0, commas=False)
```

The B200 (2024) draws `{python} B200PerformanceScenarioSecComputeTdpRecap.tdp_w_str` W. In seven years, TDP has more than tripled, growing at approximately 18 percent per year.
...
Dynamic power management means that the actual power draw of a GPU varies continuously during training. A typical training step might see power swing from 400 W (during communication phases when most SMs are idle) to `{python} H100TdpWallRecap.tdp_str` W (during the forward and backward pass when all Tensor Cores are active) and back, with each transition occurring in microseconds. The VRMs on the baseboard must respond to these transitions fast enough to maintain a stable supply voltage despite the rapidly changing current draw.

To appreciate the physical challenge, consider what `{python} H100TdpWallRecap.tdp_str` W means in terms of heat flux. An H100 die measures approximately 814 mm$^2$ (roughly $29{\times}28$ mm). Dissipating `{python} H100TdpWallRecap.tdp_str` W from this area produces a heat flux of approximately 86 W/cm$^2$. For comparison, the surface of an electric stovetop burner at high heat produces approximately 10 W/cm$^2$. An H100 die generates 8.6$\times$ the heat flux of a stove burner, concentrated on an area the size of a large postage stamp.
...
class GpuEfficiencyTrajectoryRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_ef_str = fmt(Hardware.Cloud.V100.compute.peak_flops.m_as(TFLOPs/second)/Hardware.Cloud.V100.tdp.m_as(watt), precision=2, commas=False)
    a100_ef_str = fmt(Hardware.Cloud.A100.compute.peak_flops.m_as(TFLOPs/second)/Hardware.Cloud.A100.tdp.m_as(watt), precision=2, commas=False)
    h100_ef_str = fmt(Hardware.Cloud.H100.compute.precision_flops['fp8'].m_as(TFLOPs/second)/Hardware.Cloud.H100.tdp.m_as(watt), precision=2, commas=False)
    b200_ef_str = fmt(Hardware.Cloud.B200.compute.precision_flops['fp8'].m_as(TFLOPs/second)/Hardware.Cloud.B200.tdp.m_as(watt), precision=2, commas=False)
```

The power efficiency trajectory provides a partial counterweight to rising TDP. While absolute power has increased, the advertised low-precision TFLOP/s delivered per watt has improved dramatically: from `{python} GpuEfficiencyTrajectoryRecap.v100_ef_str` TFLOP/s per watt (V100 FP16) to `{python} GpuEfficiencyTrajectoryRecap.h100_ef_str` TFLOP/s per watt (H100 FP8) to `{python} GpuEfficiencyTrajectoryRecap.b200_ef_str` TFLOP/s per watt (B200 FP8). These are not apples-to-apples FP16 numbers; the precision mode is part of the hardware trend. For a fixed compute budget that can use the newer precision modes, each generation requires fewer chips and therefore less total power. Frontier models, however, grow faster than efficiency improves, so the absolute power demand of training the latest model increases with every generation. This is the treadmill of infrastructure: efficiency gains buy time, but scale growth consumes it.
...
The infrastructure implications are substantial. A fleet designed for BF16 training (the current standard for most large models) delivers `{python} TensorCorePrecisionRecap.h100_fp16_tc_str` TFLOP/s per GPU on an H100. The same fleet running FP8 training delivers `{python} TensorCorePrecisionRecap.h100_fp8_tc_str` TFLOP/s per GPU, effectively doubling the compute density without additional hardware. For a 10,000-GPU cluster priced at roughly \$350,000 per 8-GPU node, the difference between BF16 and FP8 training is equivalent to adding another 10,000 GPUs, or roughly \$440 million in node hardware.
...
At fleet scale, the energy cost of data movement determines the electricity bill. For a 10,000-GPU cluster running at `{python} H100TdpEnergyRecap.tdp_str` W per GPU, approximately 400--500 W per GPU is consumed by the memory subsystem (HBM reads and writes, on-chip network transfers, register file accesses), and only 150--250 W is consumed by the Tensor Cores performing useful arithmetic. The rest goes to clock distribution, I/O, and leakage. Improving data reuse, through techniques like kernel fusion, FlashAttention, and activation checkpointing, reduces the energy wasted on data movement and increases the fraction of power that performs useful computation.
...
class InfraFrontierTdpServingRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(Models.Language.GPT3.parameters.m_as(param) / BILLION, precision=0, commas=False)
    weight_gb_str = fmt((Models.Language.GPT3.parameters * BYTES_FP16).m_as(GB), precision=0, commas=False)
```

The macro impact appears when serving our `{python} InfraFrontierTdpServingRecap.frontier_params_b_str`B model. Generating a single token requires loading the entire `{python} InfraFrontierTdpServingRecap.weight_gb_str` GB weight tensor from HBM:
...
the lower TDP (`{python} A100ServingTdpRecap.tdp_str` W vs. `{python} H100TdpServingRecap.tdp_str` W) allows higher rack density and lower cooling costs.
...
class H100NvlinkNodeRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_str = fmt(Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/second), precision=0, commas=False)
```
...
class InfraFrontierNodeBudgetRecap:
    ...
    weights_gb_str = fmt(weights_gb, precision=0, commas=False)
    optimizer_gb_str = fmt(optimizer_gb, precision=0, commas=False)
    ...
    gpu_hbm_gb_str = fmt(gpu_hbm_gb, precision=0, commas=False)
    node_hbm_gb_str = fmt(node_hbm_gb, precision=0, commas=False)
```

Understanding how the node partitions memory across its components is essential for selecting parallelism strategies. Consider the memory budget for training our `{python} InfraFrontierNodeBudgetRecap.frontier_params_b_str`B model. The model weights in FP16 require `{python} InfraFrontierNodeBudgetRecap.weights_gb_str` GB. The Adam optimizer maintains two additional copies of every parameter (first and second moments) in FP32 precision, adding `{python} InfraFrontierNodeBudgetRecap.frontier_params_b_str` $\times 10^9 \times 2 \times 4$ bytes = `{python} InfraFrontierNodeBudgetRecap.optimizer_gb_str` GB. Gradients require another `{python} InfraFrontierNodeBudgetRecap.gradients_gb_str` GB. The total is approximately `{python} InfraFrontierNodeBudgetRecap.total_static_tb_str` TB before activations, far exceeding both a single accelerator's `{python} InfraFrontierNodeBudgetRecap.gpu_hbm_gb_str` GB HBM and an 8-H100 node's `{python} InfraFrontierNodeBudgetRecap.node_hbm_gb_str` GB aggregate HBM.
...
class BandwidthHierarchyTableRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_str = fmt(Hardware.Cloud.H100.nvlink.bandwidth.m_as(GB/second), precision=0, commas=False)
    pcie_bw_str = fmt(Hardware.Cloud.H100.interconnect.bandwidth.m_as(GB/second), precision=0, commas=False)
    ib_bw_str = fmt(Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(GB/second), precision=0, commas=False)
```
...
| **Intra-Node**      | NVLink/ICI         | **~`{python} BandwidthHierarchyTableRecap.nvlink_bw_str` GB/s** |           ~1 μs            | Node (8--16 Chips) |
| **Intra-Node (IO)** | PCIe Gen5 x16      |  **~`{python} BandwidthHierarchyTableRecap.pcie_bw_str` GB/s**  |           ~2 μs            | CPU-GPU, NIC-GPU   |
| **Inter-Node**      | InfiniBand NDR     |   **~`{python} BandwidthHierarchyTableRecap.ib_bw_str` GB/s**   |         ~5--10 μs          | Pod (Thousands)    |
...
class BandwidthStaircase:
    ...
    transfer_gb = 10
    # ┌── 2. EXECUTE ───────────────────────────────────────
    hbm_ms = transfer_gb / hbm_bw_gbs * 1000
    nvlink_ms = transfer_gb / nvlink_bw_gbs * 1000
    ib_ms = transfer_gb / ib_bw_gbs * 1000
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    transfer_gb_str = fmt(transfer_gb, precision=0, commas=False)
    hbm_ms_str = fmt(hbm_ms, precision=0, commas=False)
    nvlink_ms_str = fmt(nvlink_ms, precision=0, commas=False)
    ib_ms_str = fmt(ib_ms, precision=0, commas=False)
    bs_hbm_bw_str = fmt(hbm_bw_gbs, precision=0)
    bs_nvlink_bw_str = fmt(nvlink_bw_gbs, precision=0, commas=False)
    bs_ib_bw_str = fmt(ib_bw_gbs, precision=0, commas=False)
...
**Problem**: A `{python} BandwidthStaircase.transfer_gb_str` GB buffer is synchronized across the fleet. How much does the physical "Layer" of the fleet affect transfer time?

**Math**:
Transfer time is $T_{\text{transfer}} = D_{\text{vol}} / \text{BW}$.

1. **HBM (Intra-Chip)**: `{python} BandwidthStaircase.transfer_gb_str` GB / `{python} BandwidthStaircase.bs_hbm_bw_str` GB/s $\approx$ **`{python} BandwidthStaircase.hbm_ms_str` ms**.
2. **NVLink (Intra-Node)**: `{python} BandwidthStaircase.transfer_gb_str` GB / `{python} BandwidthStaircase.bs_nvlink_bw_str` GB/s $\approx$ **`{python} BandwidthStaircase.nvlink_ms_str` ms**.
3. **InfiniBand (Inter-Node)**: `{python} BandwidthStaircase.transfer_gb_str` GB / `{python} BandwidthStaircase.bs_ib_bw_str` GB/s $\approx$ **`{python} BandwidthStaircase.ib_ms_str` ms**.
...
class InfraFrontierNodeTpRecap:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    activation_mb = microbatch * seq_len * hidden_dim * BYTES_FP16.m_as(byte) / 1e6
...
For our `{python} InfraFrontierNodeTpRecap.frontier_params_b_str`B model with hidden dimension `{python} InfraFrontierNodeTpRecap.hidden_dim_str` and a microbatch of `{python} InfraFrontierNodeTpRecap.microbatch_str` sequences at `{python} InfraFrontierNodeTpRecap.seq_len_str` tokens, the activation tensor at each stage boundary is approximately $4 \times 2{,}048 \times 12{,}288 \times 2$ bytes (FP16) $\approx$ `{python} InfraFrontierNodeTpRecap.activation_mb_str` MB. This is far smaller than the `{python} InfraFrontierNodeTpRecap.gradient_allreduce_gb_str` GB gradient AllReduce required by data parallelism, which is why pipeline parallelism places less demanding requirements on the inter-node network.
...
While NVLink provides a high-bandwidth freeway for GPU-to-GPU communication, **PCIe Gen5** serves as the universal glue connecting the heterogeneous components of the node. It links GPUs to the host CPU, GPUs to InfiniBand Host Channel Adapters (HCAs), the host CPU to NVMe storage, and the host CPU to system DRAM. Understanding this topology is critical because

PCIe bandwidth -- though substantial at ~`{python} BandwidthHierarchyPcieRecap.pcie_bw_str` GB/s per direction (128 GB/s bidirectional) per x16 link -- is often the bottleneck for operations that cross the accelerator boundary, such as data loading, checkpointing, and inter-node gradient synchronization.
...
any GPU can communicate with any other GPU at the full `{python} BandwidthHierarchyNvlinkRecap.nvlink_bw_str` GB/s bidirectional bandwidth simultaneously, without contention. This is the equivalent of giving every pair of GPUs their own private highway, rather than forcing them to share a single road.
...
class NodeCapacityDenseRecap:
    ...
    h100_tflops_str = fmt(Hardware.Cloud.H100.compute.precision_flops['fp8'].m_as(TFLOPs/second), precision=0, commas=False)
```

The result is that the 8 GPUs within a DGX H100 can be treated as a single logical device with `{python} NodeCapacityDenseRecap.node_hbm_cap_str` GB of aggregate HBM and a combined compute throughput of 8$\times$ `{python} NodeCapacityDenseRecap.h100_tflops_str` TFLOP/s. This abstraction is critical for software: the training framework can partition a model across the 8 GPUs using tensor parallelism as if they were a single larger processor, with the NVSwitch fabric making the partitioning nearly transparent from a performance perspective.
...
For our `{python} InfraFrontierNodeTrainRecap.frontier_params_b_str`B model training across `{python} NodeReliabilityScenario.nodes_str` nodes, the probability of at least one node experiencing a hardware issue during a two-week training run is substantial. Under a composite node-level MTBF assumption of `{python} NodeReliabilityScenario.node_mtbf_str` hours, which includes GPUs, host components, power, cooling, and network links, the expected number of node failures during a `{python} NodeReliabilityScenario.run_hours_str`-hour run is approximately `{python} NodeReliabilityScenario.expected_failures_str`. This means the training run will experience, on average, roughly `{python} NodeReliabilityScenario.failures_per_day_str` node failures per day. A GPU-only lower bound from the canonical `{python} NodeReliabilityScenario.gpu_mtbf_str`-hour per-GPU MTBF would predict only `{python} NodeReliabilityScenario.gpu_only_outages_str` GPU-driven node outages over the same run, before adding host, power, cooling, and network failure terms. Each failure requires detecting the degraded node, draining its workload, substituting a spare node, and resuming from the last checkpoint -- a process that takes `{python} NodeReliabilityScenario.recovery_low_str`--`{python} NodeReliabilityScenario.recovery_high_str` minutes with automated tooling. With automated recovery, the direct restart overhead is roughly `{python} NodeReliabilityScenario.restart_overhead_low_str`--`{python} NodeReliabilityScenario.restart_overhead_high_str` percent of the run; without spare nodes and automated recovery, multi-hour interventions can push overhead into the 20--30 percent range, consuming millions of dollars in wasted GPU-hours.
...
The model weights in FP16 occupy `{python} InfraFrontierNodeWeightsRecap.weights_gb_str` GB (`{python} InfraFrontierNodeWeightsRecap.frontier_params_b_str` $\times 10^9 \times 2$ bytes). The Adam optimizer maintains two FP32 state tensors per parameter (first moment $m$ and second moment $v$), adding `{python} InfraFrontierNodeWeightsRecap.frontier_params_b_str` $\times 10^9 \times 4 \times 2$ = `{python} InfraFrontierNodeWeightsRecap.optimizer_gb_str` GB. The gradients in FP16 add another `{python} InfraFrontierNodeWeightsRecap.gradients_gb_str` GB. Intermediate activations (needed for the backward pass) depend on the batch size and sequence length but typically require `{python} InfraFrontierNodeWeightsRecap.activation_range_gb_str` GB for practical training configurations. The total memory footprint for training is therefore `{python} InfraFrontierNodeWeightsRecap.total_footprint_tb_str` TB, roughly 6--7$\times$ the raw weight size.
...
class NodeCapacityTrainMemRecap:
    ...
    gpu_hbm_gb_str = fmt(gpu_hbm_gb, precision=0, commas=False)
    zero3_per_gpu_gb_str = fmt(zero3_per_gpu_gb, precision=0, commas=False)
    tp_weight_shard_gb_str = fmt(tp_weight_shard_gb, precision=2, commas=False)
    optimizer_per_gpu_gb_str = fmt(optimizer_per_gpu_gb, precision=1, commas=False)
    optimizer_per_tp_group_gb_str = fmt(optimizer_per_tp_group_gb, precision=1, commas=False)
    weight_plus_optimizer_gb_str = fmt(weight_plus_optimizer_gb, precision=0, commas=False)
...
```
A DGX H100 node provides `{python} NodeCapacityTrainMemRecap.node_hbm_cap_str` GB of aggregate HBM across its `{python} NodeCapacityTrainMemRecap.gpus_per_node_str` GPUs, plus `{python} NodeCapacityTrainMemRecap.host_dram_tb_str` TB of host DDR5 DRAM. The HBM capacity alone is insufficient for the full training state. To appreciate why, consider the memory arithmetic for different parallelism strategies. With **pure data parallelism**\index{Data Parallelism!pure}, each GPU must hold the complete model: `{python} NodeCapacityTrainMemRecap.weights_gb_str` GB weights + `{python} NodeCapacityTrainMemRecap.gradients_gb_str` GB gradients + `{python} NodeCapacityTrainMemRecap.optimizer_gb_str` GB optimizer states = `{python} NodeCapacityTrainMemRecap.total_dp_gb_str` GB -- physically impossible on an `{python} NodeCapacityTrainMemRecap.gpu_hbm_gb_str` GB device. Even **ZeRO Stage 3**\index{ZeRO!stage 3}, which shards all three components across only `{python} NodeCapacityTrainMemRecap.gpus_per_node_str` GPUs, yields `{python} NodeCapacityTrainMemRecap.total_dp_gb_str`/`{python} NodeCapacityTrainMemRecap.gpus_per_node_str` ≈ `{python} NodeCapacityTrainMemRecap.zero3_per_gpu_gb_str` GB per GPU, still more than triple the available HBM. The solution is to combine tensor parallelism (which splits the model's layers across GPUs, reducing per-GPU weights to `{python} NodeCapacityTrainMemRecap.weights_gb_str`/`{python} NodeCapacityTrainMemRecap.tp_degree_str` = `{python} NodeCapacityTrainMemRecap.tp_weight_shard_gb_str` GB) with data-parallel sharding across many replicas. In a configuration with TP-`{python} NodeCapacityTrainMemRecap.tp_degree_str` within each node and `{python} NodeCapacityTrainMemRecap.dp_degree_str`-way data parallelism across the cluster, each GPU holds `{python} NodeCapacityTrainMemRecap.tp_weight_shard_gb_str` GB of weight shards. The optimizer state associated with that tensor-parallel shard is then sharded across the `{python} NodeCapacityTrainMemRecap.dp_degree_str` data-parallel replicas, so each GPU holds approximately `{python} NodeCapacityTrainMemRecap.optimizer_gb_str`/(`{python} NodeCapacityTrainMemRecap.tp_optimizer_shard_divisor_str`) ≈ `{python} NodeCapacityTrainMemRecap.optimizer_per_gpu_gb_str` GB of optimizer state, or `{python} NodeCapacityTrainMemRecap.optimizer_per_tp_group_gb_str` GB per `{python} NodeCapacityTrainMemRecap.gpus_per_node_str`-GPU tensor-parallel group, before gradients, activations, buffers, and fragmentation. The weight-plus-optimizer subtotal of roughly `{python} NodeCapacityTrainMemRecap.weight_plus_optimizer_gb_str` GB is only the starting point; the memory-budget exercise below shows why gradients and activations still require checkpointing and offload. This arithmetic is why tensor parallelism, optimizer sharding, activation checkpointing, and memory tiering are used together for frontier model training. Several additional strategies further optimize this memory budget:

- **ZeRO Stage 3 (Full Sharding)**: The optimizer states, gradients, and parameters are sharded across all GPUs participating in data parallelism. With `{python} NodeCapacityTrainMemRecap.tp_degree_str`-way data parallelism within a node, each GPU holds only 1/`{python} NodeCapacityTrainMemRecap.tp_degree_str` of each component, reducing per-GPU memory to roughly `{python} NodeCapacityTrainMemRecap.zero3_per_gpu_gb_str` GB of equivalent storage (`{python} NodeCapacityTrainMemRecap.total_dp_gb_str` GB total state ÷ `{python} NodeCapacityTrainMemRecap.gpus_per_node_str`). This still exceeds `{python} NodeCapacityTrainMemRecap.gpu_hbm_gb_str` GB per GPU, requiring the combination with other techniques.
...
|:----------------|:------------------------:|:-----------------------------------------------------------------:|:-----------:|:----------------------|
| **GPU HBM3**    | 80 GB$\times$ 8 = 640 GB | `{python} MemoryBandwidthNodeTableRecap.h100_bw_str` TB/s per GPU |    <1 μs    | Active computation    |
...
class InfraFrontierNodeModelRecap:
    ...
    total_hbm_gb_str = fmt(h100_mem_gb * node_gpus, precision=0, commas=False)
    host_ddr_tb_str = fmt(host_ddr_tb, precision=0, commas=False)
    weight_per_gpu_gb_str = fmt(weight_per_gpu_gb, precision=2, commas=False)
    ...
    total_per_gpu_gb_str = fmt(total_per_gpu_gb, precision=0, commas=False)
...
- Node: `{python} InfraFrontierNodeModelRecap.node_gpus_str`$\times$ H100 GPUs, `{python} InfraFrontierNodeModelRecap.h100_mem_gb_str` GB HBM each (`{python} InfraFrontierNodeModelRecap.total_hbm_gb_str` GB total HBM)
- Host: `{python} InfraFrontierNodeModelRecap.host_ddr_tb_str` TB DDR5
...
Per-GPU weight memory: `{python} InfraFrontierNodeModelRecap.weight_per_gpu_gb_str` GB (FP16).

**Step 2**: Optimizer State with ZeRO Stage 1
ZeRO Stage 1 shards the optimizer states across data-parallel workers. Within a single node using only TP (no data parallelism), the optimizer is not sharded. Each GPU stores the full optimizer state for its TP shard: `{python} InfraFrontierNodeModelRecap.weight_per_gpu_gb_str` GB$\times$ 4 (FP32 $m$ and $v$) = `{python} InfraFrontierNodeModelRecap.opt_state_gb_str` GB.

The total exceeds the `{python} InfraFrontierNodeModelRecap.h100_mem_gb_str` GB HBM capacity. **Solution**: offload optimizer states to host DDR5. The `{python} InfraFrontierNodeModelRecap.host_ddr_tb_str` TB of DDR5 can hold the full `{python} InfraFrontierNodeModelRecap.optimizer_pool_gb_str` GB of optimizer state with room to spare.
...
Gradients match the weight size: `{python} InfraFrontierNodeModelRecap.grad_gb_str` GB per GPU in FP16.

**Total per-GPU HBM usage**\index{HBM!per-GPU usage}:

- Weights: `{python} InfraFrontierNodeModelRecap.weight_per_gpu_gb_str` GB
- Activations: ~`{python} InfraFrontierNodeModelRecap.act_gb_str` GB (with checkpointing)
- Gradients: `{python} InfraFrontierNodeModelRecap.grad_gb_str` GB
- Buffers and fragmentation: ~`{python} InfraFrontierNodeModelRecap.buf_gb_str` GB
- **Total: ~`{python} InfraFrontierNodeModelRecap.total_per_gpu_gb_str` GB** (exceeds `{python} InfraFrontierNodeModelRecap.h100_mem_gb_str` GB HBM)
...
class InterconnectCommMath:
    ...
    gradient_buffer_gb_str = fmt(gradient_buffer_gb, precision=0, commas=False)
```
...
**Problem**: A training step must synchronize a `{python} InterconnectCommMath.gradient_buffer_gb_str` GB gradient buffer. How much longer does the synchronization take when it crosses the node boundary (InfiniBand) compared to staying within the node (NVLink)?

1. **Intra-Node (NVLink)**: Bandwidth = `{python} BandwidthHierarchyNvlinkPodRecap.nvlink_bw_str` GB/s.
...
2. **Inter-Node (InfiniBand NDR)**: Bandwidth = `{python} BandwidthHierarchyPodRecap.ib_bw_str` GB/s.
...
class H100TdpRackRecap:
    ...
    power_rate_gws = power_delta_w * cluster_gpus / (transition_us / 1e6) / 1e9

    # ┌── 3. GUARD ─────────────────────────────────────────
    check(power_delta_kw > 300, "Power ramp should exceed 300 kW")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    tdp_str = fmt(gpu_tdp_w, precision=0, commas=False)
    ...
    power_delta_kw_str = fmt(power_delta_kw, precision=0, commas=False)
    transition_us_str = fmt(transition_us, precision=0, commas=False)
    power_rate_gws_str = fmt(power_rate_gws, precision=2, commas=False)
...
A standard 42U server rack in a traditional data center draws 5--10 kW and can be cooled by room-temperature air pushed through perforated floor tiles. Now place four DGX H100 nodes in that same rack: 32 GPUs, each drawing `{python} H100TdpRackRecap.tdp_str` W, plus host CPUs, memory, networking, power conversion losses, and cooling overhead. The rack power reaches `{python} RackPowerRackRecap.rack_power_str` kW, an order of magnitude beyond what traditional data center infrastructure was designed to deliver or cool. At this density, the engineering constraints shift from silicon and signal integrity to power delivery and thermodynamics. The rack is where the power wall and the laws of heat transfer become the dominant design forces.
...
For our `{python} InfraFrontierRackPowerRecap.frontier_params_b_str`B model, training across 1,024 GPUs requires approximately 32 racks (4 nodes of 8 GPUs each per rack). Each rack dissipates `{python} RackPowerRackRecap.rack_power_str` kW as heat, the thermal output of a small industrial furnace. The aggregate facility-relevant power draw of the training cluster is approximately 1.1 MW, enough to power several hundred homes. Delivering this power reliably, converting it efficiently, and removing the resulting heat without allowing any component to exceed its thermal limit is a multi-disciplinary engineering challenge that spans electrical, mechanical, and civil engineering. A failure at any point in the power delivery chain, from the utility substation to the individual GPU voltage regulator, can halt the entire training run, wasting hours of computation and potentially corrupting the training state.
...
::: {#nbk-compute-infrastructure-napkin-math-interconnection-queue .callout-notebook title="The interconnection queue"}

**Problem**: Compare the deployment timeline for `{python} GridQueue.cluster_gpus_str` GPUs vs. the electrical substation required to power them (`{python} GridQueue.cluster_power_mw_str` MW).

1. **Silicon Path**: GPU supply chains are volatile, but typical enterprise lead times are **`{python} GridQueue.gpu_lead_time_months_str` months**.
2. **Infrastructure Path**: Permitting, EPC (Engineering, Procurement, Construction), and grid connection for a new 10+ MW substation averages **`{python} GridQueue.substation_lead_time_months_str` months**.
...
::: {#chk-compute-infrastructure-power-delivery-physics .callout-checkpoint title="Power delivery physics"}
...
- [ ] If a node has `{python} H100TdpRackRecap.node_gpus_str` GPUs at `{python} H100TdpRackRecap.tdp_str` W each, why is the total node power budgeted at ~`{python} H100TdpRackRecap.node_power_budget_kw_str` kW rather than `{python} H100TdpRackRecap.node_gpu_only_kw_str` kW?
...
::: {#exmp-compute-infrastructure-power-ramp-sizing .callout-example title="Power-ramp sizing"}
...
To appreciate the magnitude of these transients, consider a `{python} H100TdpRackRecap.cluster_gpus_str`-GPU cluster transitioning from communication phase (`{python} H100TdpRackRecap.comm_phase_w_str` W per GPU average) to matrix multiplication phase (`{python} H100TdpRackRecap.tdp_str` W per GPU). The power delta is `{python} H100TdpRackRecap.power_delta_w_str` W$\times$ `{python} H100TdpRackRecap.cluster_gpus_str` = `{python} H100TdpRackRecap.power_delta_kw_str` kW, and this transition occurs in approximately `{python} H100TdpRackRecap.transition_us_str` microseconds. The rate of power change is therefore `{python} H100TdpRackRecap.power_delta_kw_str` kW/`{python} H100TdpRackRecap.transition_us_str` $\mu$s = `{python} H100TdpRackRecap.power_rate_gws_str` GW/s. No passive electrical component can respond at this rate; only energy storage devices (supercapacitors) positioned physically close to the load can absorb the transient before it propagates into the building's electrical distribution.
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-compute-cooling; PUE footnote and cooling-overhead prose.
...
class PueSavings:
    ...
    savings_15_to_11_usd = diff_15_to_11_mw * 1000 * HOURS_PER_YEAR * electricity_rate
    savings_158_to_110_usd = diff_158_to_110_mw * 1000 * HOURS_PER_YEAR * electricity_rate
    ...
    savings_15_to_11_musd_str = fmt(savings_15_to_11_usd / 1e6, precision=1, prefix="$", suffix=" million")
    savings_158_to_110_musd_str = fmt(savings_158_to_110_usd / 1e6, precision=1, prefix="$", suffix=" million")
...
[^fn-pue-metric]: **PUE (Power Usage Effectiveness)**: Defined by The Green Grid consortium in 2007 as $P_{\text{total}} / P_{\text{IT}}$. Google's fleet-wide PUE averages 1.10; the industry average is ~1.58. For a 10,000-GPU cluster consuming 7 MW of IT power, reducing PUE from 1.58 to 1.10 saves `{python} PueSavings.diff_158_to_110_mw_str` MW of cooling overhead -- roughly `{python} PueSavings.savings_158_to_110_musd_str` per year in electricity and the equivalent of removing `{python} PueSavings.households_158_to_110_str` residential homes from the grid. ML-specific facilities with direct-to-chip liquid cooling have demonstrated PUE values of 1.03--1.08. \index{PUE!efficiency metric}

::: {#dfn-compute-infrastructure-power-usage-effectiveness-pue .callout-definition title="Power usage effectiveness (PUE)"}
...
class H100TdpCoolingRecap:
    ...
    it_power_kw_str = fmt(it_power_kw, precision=0, commas=False)
    air_total_kw_str = fmt(air_total_kw, precision=0, commas=False)
    air_cooling_kw_str = fmt(air_cooling_kw, precision=0, commas=False)
    liquid_total_kw_str = fmt(liquid_total_kw, precision=0, commas=False)
    liquid_cooling_kw_str = fmt(liquid_cooling_kw, precision=0, commas=False)
    savings_kw_str = fmt(savings_kw, precision=0, commas=False)
    annual_savings_str = fmt(annual_savings_usd, precision=0, prefix="\\$")
    electricity_rate_str = fmt(electricity_rate, precision=2, commas=False, prefix="\\$", suffix="/kWh")
    three_year_savings_str = fmt(three_year_savings_usd, precision=0, prefix="\\$")
```

Water has a specific heat capacity of 4.18 kJ/kg/K, over four times that of air, and a thermal conductivity roughly 25$\times$ higher. These physical properties make water an inherently superior heat transfer medium. To appreciate the magnitude of the difference, consider how much fluid must flow to remove `{python} H100TdpCoolingRecap.tdp_str` W from a single GPU. Air at a 15-degree temperature rise requires tens of liters per second of airflow (a small wind tunnel). Water at the same temperature rise requires only about 0.01 liters per second (a thin stream). This thousands-fold difference in volumetric flow rate is why air cooling requires massive fans and carefully designed airflow paths, while liquid cooling requires only thin pipes and small pumps.
...
32-rack training cluster at `{python} RackPowerCoolingRecap.rack_power_str` kW per rack, direct-to-chip liquid cooling is the standard choice, balancing density, serviceability, and cost. Immersion cooling offers marginal PUE improvement (1.03 vs. 1.08) but introduces operational complexity that most organizations find unjustified at current rack densities.

::: {#nbk-compute-infrastructure-cooling-tax .callout-notebook title="The cooling tax"}

Consider a `{python} H100TdpCoolingRecap.n_gpus_str`-GPU cluster of H100s at `{python} H100TdpCoolingRecap.tdp_str` W TDP each.

- **IT Power**\index{IT Power}: `{python} H100TdpCoolingRecap.n_gpus_str` $\times$ `{python} H100TdpCoolingRecap.tdp_str` W = `{python} H100TdpCoolingRecap.it_power_kw_str` kW
- **Air cooling (PUE 1.5)**: Total facility power = `{python} H100TdpCoolingRecap.it_power_kw_str` kW$\times$ 1.5 = `{python} H100TdpCoolingRecap.air_total_kw_str` kW. Cooling overhead = `{python} H100TdpCoolingRecap.air_cooling_kw_str` kW.
- **Liquid cooling (PUE 1.08)**: Total facility power = `{python} H100TdpCoolingRecap.it_power_kw_str` kW$\times$ 1.08 = `{python} H100TdpCoolingRecap.liquid_total_kw_str` kW. Cooling overhead = `{python} H100TdpCoolingRecap.liquid_cooling_kw_str` kW.

**Savings**: Liquid cooling saves `{python} H100TdpCoolingRecap.savings_kw_str` kW of continuous power. At `{python} H100TdpCoolingRecap.electricity_rate_str`, the annual savings are approximately `{python} H100TdpCoolingRecap.annual_savings_str`. Over a 3-year hardware lifecycle, the cooling savings alone total `{python} H100TdpCoolingRecap.three_year_savings_str`, which often exceeds the capital cost of installing the liquid cooling infrastructure.
...
class PodTrainingMath:
    ...
    h100_tflops_str = fmt(h100_fp8_tflops, precision=0, commas=False)
    pod_flops_math = fmt_math(f"6 \\times {gpt3_params_b_str} \\times 300 \\times 10^{{18}}")
    total_flops_math = fmt_math(f"3.15 \\times 10^{{23}}")
    n_params_math = fmt_math(f"P = {gpt3_params_b_str} \\times 10^9")
    cluster_throughput_math = fmt_math(f"8{{,}}192 \\times {h100_tflops_str} \\times 10^{{12}} \\times 0.45")
    cluster_eflops = 8192 * h100_fp8_tflops * 1e12 * 0.45 / 1e18
    cluster_eflops_str = fmt(cluster_eflops, precision=2, commas=False)
    n_gpus_str = fmt(8192, precision=0)
    mfu_pct_str = fmt(45, precision=0, commas=False)
...
1. **Total FLOPs**\index{Training Time!total FLOPs}: Using the approximation $6 \times P \times D$, where `{python} PodTrainingMath.n_params_math` and $D = 300 \times 10^9$ tokens:
   `{python} PodTrainingMath.total_flops_math` FLOPs

2. **Cluster throughput**: `{python} PodTrainingMath.n_gpus_str` H100 GPUs at `{python} PodTrainingMath.h100_tflops_str` TFLOP/s peak, operating at `{python} PodTrainingMath.mfu_pct_str` percent MFU:
...
3. **Idealized Training Time** (the "physics limit"):
   (`{python} PodTrainingMath.total_flops_math` FLOPs)/(`{python} PodTrainingMath.cluster_throughput_math` FLOP/s) $\div 3600 \approx$ **`{python} TrainingTime175B.t_physics_hr_str` hours**
...
class CheckpointOverheadBudget:
    ...
    required_bw_gbs = checkpoint_tb * 1000 / allowed_write_s
    reference_bw_gbs = checkpoint_tb * 1000 / reference_write_s
    ...
    sustained_bw_gbs = checkpoint_tb * 1000 / cadence_s
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    ckpt_total_tb_str                  = fmt(checkpoint_tb, precision=2, commas=False)
    ckpt_cadence_min_str               = fmt(cadence_min, precision=0, commas=False)
    ckpt_target_overhead_pct_str       = fmt(target_overhead_pct, precision=0, commas=False)
    ckpt_allowed_write_s_str           = fmt(allowed_write_s, precision=0, commas=False)
    ckpt_required_bw_gbs_str           = fmt(required_bw_gbs, precision=1, commas=False)
    ckpt_reference_write_s_str         = fmt(reference_write_s, precision=0, commas=False)
    ckpt_reference_bw_gbs_str          = fmt(reference_bw_gbs, precision=1, commas=False)
    ckpt_reference_overhead_pct_str    = fmt(reference_overhead_pct, precision=0, commas=False)
    ckpt_reference_min_cadence_min_str = fmt(reference_min_cadence_min, precision=0, commas=False)
    ckpt_run_count_str                 = fmt(checkpoints_per_run, precision=0)
    ckpt_run_total_tb_str              = fmt(run_total_tb, precision=0)
    ckpt_recent_retention_tb_str       = fmt(recent_retention_tb, precision=1, commas=False)
    ckpt_sustained_bw_gbs_str          = fmt(sustained_bw_gbs, precision=1, commas=False)
...
Before examining specific pod architectures, we must understand the metric that determines whether scaling from one device to many is worthwhile: **scaling efficiency**. If a model trains in time $T_1$ on one device, ideal (linear) scaling would train it in time $T_1/N$ on $N$ devices. In practice, communication overhead, pipeline bubbles, and load imbalances reduce the speedup. Scaling efficiency is defined as:

$$\eta_{\text{scaling}} = \frac{T_1}{N \times T_N}$$ {#eq-cluster-scaling-efficiency}
...
::: {#dfn-compute-infrastructure-scaling-efficiency .callout-definition title="Scaling efficiency"}
...
class CommScalingMath:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gradient_gb_str = fmt(_gradient_size_gb, precision=0, commas=False)
    exposed_step_s_str = fmt(exposed_step_s, precision=1, commas=False)
    scaling_efficiency_str = fmt(scaling_efficiency, precision=3, commas=False)
    t_compute_s_str = fmt(t_compute_s, precision=1, commas=False)
    batch_tokens_m_str = fmt(batch_tokens_m, precision=0, commas=False)
    n_gpus_str = fmt(n_gpus, precision=0)
    mfu_pct_str = fmt(mfu_pct, precision=0, commas=False)
...
::: {#nbk-compute-infrastructure-scaling-efficiency-175b-model .callout-notebook title="Scaling efficiency for a 175B model"}
...
**Setup**\index{Setup}: Training a `{python} InfraFrontierPodSetupRecap.frontier_params_b_str`B model on a DGX H100 cluster with `{python} InfraIbBwPodRecap.ib_bw_str` Gb/s InfiniBand per GPU.

- **Compute per step** (assuming batch size `{python} CommScalingMath.batch_tokens_m_str`M tokens, `{python} CommScalingMath.flops_per_token_str` FLOPs per parameter per token):
  `{python} CommScalingMath.compute_per_step_math` FLOPs

- **Per-GPU compute time** on `{python} CommScalingMath.n_gpus_str` GPUs, each at `{python} PodSetupComputeRecap.h100_tflops_str` TFLOP/s (`{python} CommScalingMath.mfu_pct_str` percent utilization):
  `{python} CommScalingMath.t_compute_math` seconds

- **AllReduce time**\index{AllReduce}\index{AllReduce!ring algorithm} for `{python} CommScalingMath.gradient_gb_str` GB of gradients using ring-AllReduce with overlap:
  `{python} CommScalingMath.t_comm_math` seconds (raw transfer is about `{python} CommScalingMath.raw_comm_s_str` seconds; this example assumes `{python} CommScalingMath.overlap_pct_str` percent overlap with the backward pass)

- **Scaling efficiency**: $\eta_{\text{scaling}} \approx \frac{T_{\text{compute}}/N}{T_{\text{compute}}/N + T_{\text{comm}}(N) + T_{\text{sync}}(N) - T_{\text{overlap}}} = 2.1/5.6 \approx 0.375$ in this simplified example, where synchronization is included in the AllReduce term and `{python} CommScalingMath.overlap_pct_str` percent communication overlap has already been applied.

The low efficiency (`{python} CommScalingMath.scaling_efficiency_pct_str` percent) shows why naive data parallelism at this scale is insufficient. <!-- lego-ok-block: production hierarchy-aware scaling efficiency range -->
...
::: {#dfn-compute-infrastructure-hierarchy-aware-parallelism .callout-definition title="Hierarchy-aware parallelism"}
...
class CheckpointComputeTradeoff:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(abs(t_write_s - 20) < 1, "Checkpoint write time should be about 20 s")
    check(800 < tau_opt_s < 900, "Optimal checkpoint interval should be about 14 min")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(params / BILLION, precision=0, commas=False)
    training_state_tb_str = fmt(training_state_tb, precision=0, commas=False)
    write_bw_gbs_str = fmt(write_bw_gbs, precision=0, commas=False)
    t_write_s_str = fmt(t_write_s, precision=0, commas=False)
    num_gpus_str = fmt(num_gpus, precision=0, commas=True)
    mtbf_system_s_str = fmt(mtbf_system_s, precision=0, commas=True)
    mtbf_system_hr_str = fmt(mtbf_system_hr, precision=0, commas=False)
    tau_opt_s_str = fmt(tau_opt_s, precision=0, commas=False)
    tau_opt_min_str = fmt(tau_opt_s / SECONDS_PER_MINUTE, precision=0, commas=False)
    overhead_pct_str = fmt(overhead_pct, precision=1, commas=False)
```

The reliability challenge is formalized by the **checkpoint-compute trade-off**\index{Checkpoint-Compute Trade-Off}, which balances the time lost to saving state against the time lost to recomputing work after a failure. The **Young/Daly formula** defines the optimal checkpoint interval $\tau_{\text{opt}}$ as $\sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}}$, where $T_{\text{write}}$ is the time to write a checkpoint and $\text{MTBF}_{\text{system}}$ is the system-wide Mean Time Between Failures. For our `{python} CheckpointComputeTradeoff.frontier_params_b_str`B model, the full training state -- weights, optimizer moments, and gradients -- approaches `{python} CheckpointComputeTradeoff.training_state_tb_str` TB. Even with a high-performance parallel filesystem capable of `{python} CheckpointComputeTradeoff.write_bw_gbs_str` GB/s write throughput, committing this state to persistent storage takes approximately $T_{\text{write}} \approx$ `{python} CheckpointComputeTradeoff.t_write_s_str` seconds. In a cluster of `{python} CheckpointComputeTradeoff.num_gpus_str` GPUs where $\text{MTBF}_{\text{system}} \approx$ `{python} CheckpointComputeTradeoff.mtbf_system_s_str` s (`{python} CheckpointComputeTradeoff.mtbf_system_hr_str` hours), the formula yields $\tau_{\text{opt}} \approx$ `{python} CheckpointComputeTradeoff.tau_opt_s_str` s, an optimal checkpoint interval of roughly `{python} CheckpointComputeTradeoff.tau_opt_min_str` minutes. This creates a steady cadence: every `{python} CheckpointComputeTradeoff.tau_opt_min_str` minutes, the entire cluster pauses for `{python} CheckpointComputeTradeoff.t_write_s_str` seconds to serialize its state to disk, introducing a sustained training overhead of approximately `{python} CheckpointComputeTradeoff.overhead_pct_str` percent. If the interval is too long, the cost of recomputing lost work after a failure exceeds the savings from fewer checkpoints; if too short, the I/O overhead dominates the training budget. @sec-fault-tolerance-young-daly examines checkpointing strategies, including asynchronous checkpointing and incremental state saving, that reduce this overhead.
...
class BuildVsBuyFigureScenario:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(nodes == 125, "1,000 GPUs at 8 GPUs/node should require 125 nodes")
    ...
    upfront_capex_str = fmt(upfront_capex_usd / 1e6, precision=2, prefix="$", suffix="M")
    annual_opex_str = fmt(annual_opex_usd / 1e6, precision=1, prefix="$", suffix="M")
    cloud_monthly_str = fmt(cloud_monthly_usd / 1e6, precision=2, prefix="$", suffix="M")
    cloud_horizon_str = fmt(cloud_horizon_usd / 1e6, precision=1, prefix="$", suffix="M")
...
class TenKGpuClusterTco:
    ...
    annual_electricity_usd = gpus * gpu_tdp_w / 1000 * pue * HOURS_PER_YEAR * electricity_rate
    ...
    hardware_capex_str = fmt(hardware_capex_usd / 1e6, precision=1, prefix="$", suffix="M")
    network_capex_str = fmt(network_capex_usd / 1e6, precision=0, prefix="$", suffix="M")
    facility_capex_str = fmt(facility_capex_usd / 1e6, precision=0, prefix="$", suffix="M")
    annual_electricity_str = fmt(annual_electricity_usd / 1e6, precision=1, prefix="$", suffix="M")
    annual_staffing_str = fmt(annual_staffing_usd / 1e6, precision=0, prefix="$", suffix="M")
    upfront_capex_str = fmt(upfront_capex_usd / 1e6, precision=1, prefix="$", suffix="M")
    annual_opex_str = fmt(annual_opex_usd / 1e6, precision=1, prefix="$", suffix="M")
    three_year_total_str = fmt(three_year_total_usd / 1e6, precision=1, prefix="$", suffix="M")
    cloud_annual_str = fmt(cloud_annual_usd / 1e6, precision=1, prefix="$", suffix="M")
    cloud_three_year_str = fmt(cloud_three_year_usd / 1e6, precision=1, prefix="$", suffix="M")
    savings_str = fmt(savings_usd / 1e6, precision=1, prefix="$", suffix="M")
    breakeven_util_str = fmt(breakeven_utilization * 100, precision=1, commas=False)
    gpus_str = fmt(gpus, precision=0, commas=True)
    node_price_str = fmt(node_price_usd, precision=0, commas=True, prefix="$", suffix="")
    gpu_tdp_str = fmt(gpu_tdp_w, precision=0, commas=False)
    pue_str = fmt(pue, precision=1, commas=False)
    hours_per_year_str = fmt(HOURS_PER_YEAR, precision=0, commas=True)
    electricity_rate_str = fmt(electricity_rate, precision=2, commas=False, prefix="$", suffix="/kWh")
    cloud_gpu_hour_str = fmt(cloud_gpu_hour_usd, precision=2, commas=False, prefix="$", suffix="/hr")
    utilization_str = fmt(utilization, precision=2, commas=False)
    cloud_utilization_pct_str = fmt(utilization * 100, precision=0, commas=False)
...
Consider a cluster of `{python} TenKGpuClusterTco.nodes_str` DGX H100 nodes (`{python} TenKGpuClusterTco.gpus_str` GPUs) for training our `{python} InfraFrontierTcoTenKRecap.frontier_params_b_str`B model.

**On-Premises (3-year lifecycle)**:

- **Hardware CapEx**\index{CapEx!hardware}: `{python} TenKGpuClusterTco.nodes_str` nodes$\times$ `{python} TenKGpuClusterTco.node_price_str` = `{python} TenKGpuClusterTco.hardware_capex_str`
...
**Cloud (3-year rental at `{python} TenKGpuClusterTco.cloud_utilization_pct_str` percent utilization)**:

- **Annual Cost**\index{Cost!annual cost}: `{python} TenKGpuClusterTco.gpus_str` GPUs$\times$ `{python} TenKGpuClusterTco.cloud_gpu_hour_str`$\times$ `{python} TenKGpuClusterTco.hours_per_year_str` h$\times$ `{python} TenKGpuClusterTco.utilization_str` = `{python} TenKGpuClusterTco.cloud_annual_str`/year
- **3-Year Total**: **~`{python} TenKGpuClusterTco.cloud_three_year_str`**

**Break-Even**\index{Total Cost of Ownership!break-even}: On-premises saves approximately `{python} TenKGpuClusterTco.savings_str` over three years at `{python} TenKGpuClusterTco.cloud_utilization_pct_str` percent utilization. The savings disappear if utilization drops below roughly `{python} TenKGpuClusterTco.breakeven_util_str` percent, because the on-premises hardware still incurs facility and staffing costs regardless of load.
...
class GpuEfficiencyTableRecap:
    ...
    h100_ef_str = fmt(Hardware.Cloud.H100.compute.precision_flops['fp8'].m_as(TFLOPs/second)/Hardware.Cloud.H100.tdp.m_as(watt), precision=2, commas=False)
    b200_ef_str = fmt(Hardware.Cloud.B200.compute.precision_flops['fp8'].m_as(TFLOPs/second)/Hardware.Cloud.B200.tdp.m_as(watt), precision=2, commas=False)
    b200_fp8_tflops_str = fmt(Hardware.Cloud.B200.compute.precision_flops['fp8'].m_as(TFLOPs/second), precision=0, commas=False)
    b200_tdp_w_str = fmt(Hardware.Cloud.B200.tdp.m_as(watt), precision=0, commas=False)
```
...
class V100EfficiencyRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    flops_fp16_str = fmt(Hardware.Cloud.V100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    tdp_str = fmt(Hardware.Cloud.V100.tdp.m_as(watt), precision=0, commas=False)
```
...
class A100EfficiencyRowRecap:
    flops_fp16_str = fmt(Hardware.Cloud.A100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    tdp_str = fmt(Hardware.Cloud.A100.tdp.m_as(watt), precision=0, commas=False)
...
Consider a concrete refresh scenario. An organization operating 1,000 V100 GPUs (`{python} V100RefreshRecap.tdp_str` W each, `{python} GpuEfficiencyTableRecap.v100_ef_str` TFLOP/s per watt) consumes 300 kW of IT power for 125,000 TFLOP/s of aggregate throughput. Replacing them with 1,000 H100 GPUs (`{python} H100TdpEfficiencyRecap.tdp_str` W each, `{python} GpuEfficiencyTableRecap.h100_ef_str` TFLOP/s per watt) increases power consumption to 700 kW but delivers 1,979,000 TFLOP/s, a 15.8$\times$ throughput increase for a 2.3$\times$ power increase.
...
class TcoDecisionExercise:
    ...
    gpu_hours_per_model_str = fmt(gpu_hours_per_model, precision=0)
    cluster_gpus_str = fmt(cluster_gpus, precision=0, commas=False)
    cloud_gpu_hour_str = fmt(cloud_gpu_hour_usd, precision=2, commas=False, prefix="\\$")
...
- **Network design**: Based on the communication pattern and cluster size, select the network topology (fat-tree, torus, or rail-optimized) and size the switch fabric. Calculate the expected scaling efficiency using @eq-cluster-scaling-efficiency to verify that the cluster size is within the efficient scaling regime.
...
class PlanningThroughputScenario:
    ...
    params = Models.Language.GPT3.parameters.m_as(param)
    tokens = 300e9
    training_flops_per_token_param = 6
    mfu = 0.45
    gpus = 1_024
    ...
    cluster_sustained_pflops = (per_gpu_sustained_tflops * gpus) / 1_000
    ideal_seconds = total_flops / (cluster_sustained_pflops * 1e15)
    ...
    h100_peak_tflops_str = fmt(h100_peak_tflops, precision=0, commas=False)
    per_gpu_sustained_tflops_str = fmt(per_gpu_sustained_tflops, precision=1, commas=False)
    gpus_str = fmt(gpus, precision=0)
    cluster_sustained_pflops_str = fmt(cluster_sustained_pflops, precision=1, commas=False)
...
To illustrate this methodology concretely, consider planning the infrastructure for training our `{python} InfraFrontierTcoPlanningRecap.frontier_params_b_str`B model. The training workload at large batch sizes is compute-bound, so the design optimizes for TFLOP/s per dollar and selects the H100 as the highest available TFLOP/s at reasonable cost. The 2.2 TB training state requires tensor parallelism within each node plus optimizer sharding, activation checkpointing, and offload across the wider data-parallel group, yielding a DGX H100 configuration. Sizing the cluster follows from the compute budget: at 6 FLOPs per parameter-token over 175 billion parameters and 300 billion tokens, the run requires `{python} PlanningThroughputScenario.total_flops_str` FLOPs. Using a conservative BF16/FP16 planning basis rather than the FP8 headline peak, each H100 delivers `{python} PlanningThroughputScenario.h100_peak_tflops_str` TFLOP/s peak and `{python} PlanningThroughputScenario.per_gpu_sustained_tflops_str` TFLOP/s sustained at `{python} PlanningThroughputScenario.mfu_pct_str` percent MFU; with `{python} PlanningThroughputScenario.gpus_str` GPUs, the cluster reaches roughly `{python} PlanningThroughputScenario.cluster_sustained_pflops_str` PFLOP/s sustained and completes the run in about `{python} PlanningThroughputScenario.ideal_days_str` days of idealized compute time, or 2--4 weeks with operational overhead. The TP-8, PP-4, DP-32 configuration generates structured AllReduce traffic suited to a rail-optimized InfiniBand fabric.

The 128 nodes draw `{python} RackPowerPlanningRecap.rack_power_str` kW per 4-node rack across 32 racks at approximately 1.1 MW of facility-relevant load, necessitating liquid cooling. The 3-year TCO comes to approximately \$63M on-premises; renting the six-runs-per-year workload in the cloud is roughly \$24--48M depending on whether each run lasts 2 or 4 weeks, while continuous cloud use is more expensive. The break-even depends on sustained utilization beyond the initial training run. GPU procurement (6--12 months) and facility preparation must begin immediately, with phased deployment targeting initial capacity within 3 months.
...
class InfraPlanningExercise:
    ...
    weeks_str = fmt(weeks, precision=0, commas=False)
    power_mw_str = fmt(power_mw, precision=0, commas=False)
    mfu_pct_str = fmt(mfu_pct, precision=0, commas=False)
    pue_str = fmt(pue, precision=1, commas=False)
    non_gpu_overhead_pct_str = fmt(non_gpu_overhead_pct, precision=0, commas=False)
    cloud_gpu_hour_str = fmt(cloud_gpu_hour_usd, precision=2, commas=False, prefix="\\$")
    node_price_str = fmt(node_price_usd, precision=0, prefix="\\$")
```

Your team needs to train a `{python} InfraPlanningExercise.params_b_str`B-parameter model on `{python} InfraPlanningExercise.tokens_t_str` trillion tokens within `{python} InfraPlanningExercise.weeks_str` weeks. Using the following specifications:
...
- H100 GPU: `{python} NodeCapacityScenarioSecComputeTcoRecap.h100_tflops_str` TFLOP/s peak, assume `{python} InfraPlanningExercise.mfu_pct_str` percent MFU
- Compute budget: $6 \times 70 \times 10^9 \times 10^{12} = 4.2 \times 10^{23}$ FLOPs
- Available power: `{python} InfraPlanningExercise.power_mw_str` MW

Determine: (a) the minimum number of GPUs needed, (b) whether the `{python} InfraPlanningExercise.power_mw_str` MW power budget is sufficient (assume PUE `{python} InfraPlanningExercise.pue_str` and `{python} H100TdpExerciseRecap.tdp_str` W per GPU with `{python} InfraPlanningExercise.non_gpu_overhead_pct_str` percent overhead for non-GPU components), and (c) the approximate cost of the training run at `{python} InfraPlanningExercise.cloud_gpu_hour_str`/GPU-hour for cloud or `{python} InfraPlanningExercise.node_price_str` per 8-GPU node for on-premises.
...
The correct approach is to profile the communication-to-computation ratio at the current scale and use @eq-cluster-scaling-efficiency to predict whether additional GPUs will provide sufficient return on investment before committing to procurement.
...
A single rack of four DGX H100 systems requires `{python} RackPowerFallaciesRecap.rack_power_str` kW of power. Upgrading a data center's electrical infrastructure (new transformers, switchgear, and UPS capacity) requires 12--18 months of lead time. Cooling plant upgrades (chillers, piping, pumps) require 6--12 months. Even seemingly simple changes, such as upgrading the PDU in a single rack from 20 kW to 60 kW, can require new cabling and circuit breaker panels.
...
class A100FallaciesRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mem_bw_tbs_str = fmt(a100_bw_tbs, precision=2, commas=False)
    option_a_hbm_gb_str = fmt(option_a_hbm_gb, precision=0, commas=False)
    weights_fp16_gb_str = fmt(weights_fp16_gb, precision=0, commas=False)
    option_a_lat_ms_str = fmt(option_a_lat_ms, precision=1, commas=False)
    option_b_hbm_gb_str = fmt(option_b_hbm_gb, precision=0, commas=False)
    weights_int8_gb_str = fmt(weights_int8_gb, precision=0, commas=False)
    option_b_lat_ms_str = fmt(option_b_lat_ms, precision=0, commas=False)
...
`{python} A100FallaciesRecap.option_a_hbm_gb_str` GB of HBM2e at `{python} A100FallaciesRecap.mem_bw_tbs_str` TB/s bandwidth, and (b) an accelerator with `{python} A100FallaciesRecap.option_b_hbm_gb_str` GB of HBM3 at `{python} MemoryBandwidthFallaciesRecap.h100_bw_str` TB/s bandwidth. Option (a) has more than enough capacity (the model requires only `{python} A100FallaciesRecap.weights_fp16_gb_str` GB in FP16) but delivers tokens at `{python} A100FallaciesRecap.weights_fp16_gb_str` GB/`{python} A100FallaciesRecap.mem_bw_tbs_str` TB/s = `{python} A100FallaciesRecap.option_a_lat_ms_str` ms per token. Option (b) requires careful quantization to fit (`{python} A100FallaciesRecap.weights_fp16_gb_str` GB in FP16 would not fit in `{python} A100FallaciesRecap.option_b_hbm_gb_str` GB, but `{python} A100FallaciesRecap.weights_int8_gb_str` GB in INT8 does) but delivers tokens at `{python} A100FallaciesRecap.weights_int8_gb_str` GB/`{python} MemoryBandwidthFallaciesRecap.h100_bw_str` TB/s = `{python} A100FallaciesRecap.option_b_lat_ms_str` ms per token, which is `{python} A100FallaciesRecap.speedup_str`$\times$ faster. For latency-sensitive serving, option (b) is the better choice despite its smaller capacity. The right accelerator is the one whose bandwidth and capacity *jointly* satisfy the workload's requirements.
...
class A100UniformFleetRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mem_bw_tbs_str = fmt(Hardware.Cloud.A100.memory.bandwidth.m_as(TB/second), precision=2, commas=False)
```
...
class SummaryScaleScenario:
    ...
    h100_tflops_str = fmt(flops.m_as(TFLOPs/second), precision=0, commas=False)
    h100_bw_str = fmt(bw.m_as(TB/second), precision=2, commas=False)
    h100_tdp_str = fmt(tdp.m_as(watt), precision=0, commas=False)
    rack_power_str = fmt(rack_power, precision=1, commas=False)
```
...
class TcoSummaryBreakevenRecap:
    cloud_h100_hourly = 4.0
    gpus_per_node = Systems.Nodes.DGX_H100.accelerators_per_node
    dgx_h100_price = 350000
    node_power_kw = (Hardware.Cloud.H100.tdp * Systems.Nodes.DGX_H100.accelerators_per_node).m_as(kilowatt) * 1.2
    denominator = (cloud_h100_hourly * gpus_per_node * HOURS_PER_YEAR) - (node_power_kw * HOURS_PER_YEAR * 0.07 * 1.1)
    breakeven_util_str = fmt((dgx_h100_price / 3) / denominator * 100, precision=1, commas=False)
...
* **TCO, not CapEx**: The total cost of ownership includes electricity, facility construction, networking, and staffing. Under the simplified per-node hardware-plus-electricity model, on-premises infrastructure becomes favorable above approximately `{python} TcoSummaryBreakevenRecap.breakeven_util_str` percent sustained utilization; in the full 10,000-GPU TCO example including facility, network, and staffing costs, the threshold is roughly 54 percent.
