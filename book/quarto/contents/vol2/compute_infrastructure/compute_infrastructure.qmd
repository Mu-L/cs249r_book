...
The token latency breakdown confirms that memory bandwidth, not arithmetic throughput, sets the floor for autoregressive generation. Because compute occupies less than 2 percent of each token's wall time, any improvement in HBM bandwidth translates almost directly into lower decode latency. That relationship gives us a clean way to compare adjacent hardware generations: if compute stays fixed while HBM capacity and bandwidth improve, decode latency should move with the memory system rather than the arithmetic units.
...
Those timings are the operational form of a broader concept: bandwidth falls in tiers as data crosses physical boundaries. The 18$\times$ gap between NVLink and InfiniBand is not an anomaly; it is one step in a staircase that spans four orders of magnitude from on-chip SRAM to the wide-area network. Each step reflects a different signal-propagation regime, a different wiring density, and a different energy cost per bit transferred.
...
The magnitude of NVLink's impact on tensor parallelism becomes clear when compared to what happens without it.
...
The narrow margin left after mitigations means that any increase in batch size or sequence length can push the working set past the physical limit. When that limit is exceeded, the system must either spill to DDR5 (at 10$\times$ lower bandwidth) or redistribute shards across the network (at 100$\times$ lower bandwidth), creating a performance cliff rather than a gradual degradation. The interconnect communication overhead puts concrete numbers on that cost.
...
A rack of four DGX H100 nodes draws 33 to 40 kW, roughly 5$\times$ the power of a conventional server rack. Delivering that power reliably requires a chain of transformations between the utility grid and the GPU's voltage regulators, and understanding this chain is essential for capacity planning. The chain has five stages, each with its own conversion technology, efficiency, and failure modes.
...
The training-time calculation reveals that even under optimistic assumptions, a 175B-parameter model requires weeks of continuous execution across thousands of GPUs. Hardware failures, maintenance windows, and operational overhead inflate the ideal compute time by 15 percent or more, and no single node or rack can absorb the workload alone. At this scale, the unit of analysis is no longer a rack or pod of independent servers; the facility has to behave as one coherent machine.
