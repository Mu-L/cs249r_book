...
# │ Exports: InfraSetup.frontier_params_b_str, InfraSetup.gpt3_params_b_str,
# │          InfraSetup.h100_tflops_str, InfraSetup.h100_bw_str, InfraSetup.h100_ridge_str,
...
class InfraSetup:
    ...
    m_frontier = Models.Language.GPT3  # 175B running example used throughout the chapter
    ...
    # OUTPUTS
    h100_tflops_str = fmt(h100_tflops_val, precision=1, allow_zero=True, commas=False)
    h100_bw_str = fmt(h100_bw_val, precision=2, commas=False)
    h100_ridge_str = fmt(h100_ridge_val, precision=1, allow_zero=True, commas=False)
    gpt3_params_b_str = fmt(gpt3_params_b_val, precision=1, allow_zero=True, commas=False)
    frontier_params_b_str = fmt(frontier_params_b_val, precision=1, allow_zero=True, commas=False)
    ib_bw_str = fmt(ib_bw_gbps_val, precision=1, allow_zero=True, commas=False)
    ddr_bw_str = fmt(HOST_DRAM_BW.m_as(GB/second), precision=1, allow_zero=True, commas=False)
    dominance_str = fmt(dominance, precision=1, commas=False)

    # Token latency math blocks (used in @nbk-token-latency)
...
# │
# │ Goal: Export FP16 Tensor TFLOP/s, memory BW, TDP, and NVLink BW per chip
...
class V100Specs:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    flops_fp16_tc = Hardware.Cloud.V100.compute.peak_flops
    mem_bw = Hardware.Cloud.V100.memory.bandwidth
    tdp = Hardware.Cloud.V100.tdp
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(flops_fp16_tflops > 100, "V100 FP16 Tensor should be > 100 TFLOP/s")
    check(900 <= mem_bw_gbs <= 901, "V100 HBM2 should be 900 GB/s")

    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    flops_fp16_str = fmt(flops_fp16_tflops, precision=1, allow_zero=True, commas=False)
    mem_bw_str = fmt(mem_bw_gbs, precision=1, allow_zero=True, commas=False)
    tdp_str = fmt(tdp_w, precision=1, allow_zero=True, commas=False)
    nvlink_str = fmt(nvlink_gbs, precision=1, allow_zero=True, commas=False)
...
class H100PerformanceScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(h100_tf, precision=1, allow_zero=True, commas=False)
```
...
# │                                  NVLINK_B200_BW, TFLOPs, GB, second, watt)
...
class B200PerformanceScenario:
    ...
    nvlink_gbs = NVLINK_B200_BW.m_as(GB / second)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fp8_tflops_str = fmt(fp8_tflops, precision=1, allow_zero=True)
    tdp_w_str = fmt(tdp_w, precision=1, allow_zero=True, commas=False)
    nvlink_str = fmt(nvlink_gbs, precision=1, allow_zero=True)
```
...
Blackwell also introduced fifth-generation NVLink at `{python} B200PerformanceScenario.nvlink_str` GB/s per GPU, doubling the intra-node bandwidth again. The die-to-die link within the B200 package operates at 10 TB/s, fast enough that the two dies appear as a single logical GPU to the software, with no performance penalty for operations that span the die boundary.
...
|:---------------------|:--------:|:------------------------|:-------------------------------------------------------------:|:----------------------------------------------:|:--------------------------------------------------:|
| **Volta** (V100)     |   2017   | First Tensor Core       |          `{python} V100Specs.flops_fp16_str` TFLOP/s          |         `{python} V100Specs.tdp_str` W         |        `{python} V100Specs.nvlink_str` GB/s        |
...
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (HOST_DRAM_BW, Hardware.Cloud.H100.memory.bandwidth, GB, TB, second)
# │ Exports: InfraSetup.ddr_bw_str, InfraSetup.h100_bw_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class MemoryBandwidthScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ddr_bw_str = fmt(ddr_gbs, precision=1, allow_zero=True, commas=False)
    h100_bw_str = fmt(hbm_tbs, precision=2, commas=False)
```
...
Further down is the 50 MB L2 cache (~12 TB/s), and finally the 80 GB of HBM3 at `{python} MemoryBandwidthScenario.h100_bw_str` TB/s.
...
Despite the `{python} InfraSetup.frontier_params_b_str`B model's massive total memory footprint, the active working set at any given microsecond must be meticulously managed to reside in that top 30 MB of register space, or the chip's theoretical performance becomes a mirage.
...
| **Bandwidth**\index{HBM!bandwidth}      | ~`{python} MemoryBandwidthScenario.ddr_bw_str` GB/s | **~`{python} MemoryBandwidthScenario.h100_bw_str` TB/s** |    **~17$\times$ Faster**    |
...
An important architectural consideration is the number of HBM stacks per accelerator and how they connect to the processor. The H100 has 5 HBM3 stacks, each providing approximately 670 GB/s, for a total of `{python} MemoryBandwidthScenario.h100_bw_str` TB/s aggregate bandwidth. A B200-class Blackwell GPU package uses 8 HBM3e stacks for an aggregate of about 8 TB/s. The number of stacks is constrained by the interposer area available for HBM placement: each HBM stack occupies approximately 100 mm$^2$ of interposer area, and the total interposer must accommodate both the processor die(s) and all HBM stacks. Larger interposers allow more HBM stacks (and therefore higher bandwidth and capacity) but are more expensive and harder to manufacture.
...
1. **Compute Time**\index{Latency!compute time}: Each token requires $2 \times 70 \times 10^9 = 1.4 \times 10^{11}$ floating-point operations. At `{python} InfraSetup.h100_tflops_str` TFLOP/s of low-precision Tensor Core peak throughput, the arithmetic takes:
   `{python} InfraSetup.t_comp_math`
2. **Memory Time**\index{Latency!memory time}: Loading 140 GB of weights from HBM at `{python} MemoryBandwidthScenario.h100_bw_str` TB/s takes:
   `{python} InfraSetup.t_mem_math`

**Systems insight**: The processor spends **`{python} InfraSetup.dominance_str` percent** of its time waiting for data from memory. The arithmetic units are idle for almost the entire token generation. Even a hypothetical processor with *infinite* compute throughput would generate tokens only negligibly faster, because the memory transfer time dominates completely. This is *why* HBM bandwidth improvements deliver nearly linear speedups for inference workloads.
...
class RooflineLandscapeMath:
    ...
    # ┌── 1. LOAD ─────────────────────────────────────────────
    peak_tflops = InfraSetup.h100_tflops_val
    bandwidth_tbs = InfraSetup.h100_bw_val
    ridge = InfraSetup.h100_ridge_val
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────────
    decode_intensity_str = fmt(decode_intensity, precision=1, allow_zero=True, commas=False)
    training_batch_size_str = fmt(training_batch_size, precision=1, allow_zero=True, commas=False)
    train_intensity_str = fmt(train_intensity, precision=1, allow_zero=True, commas=True)
    train_gap_str = fmt(train_gap, precision=1, allow_zero=True, commas=True)
```
...
The roofline in @fig-roofline-landscape is defined by two ceilings: memory bandwidth (diagonal) and peak compute (horizontal), meeting at the FP16 ridge point (~`{python} InfraSetup.h100_ridge_str` FLOP/byte for `{python} InfraSetup.h100_tflops_str` TFLOP/s over `{python} InfraSetup.h100_bw_str` TB/s). ML workloads span the full range: LLM decode at batch size 1 is deeply memory-bound, while LLM training at large batch sizes is compute-bound. The position of each workload determines whether faster memory or faster compute would improve performance.
...
# │ Show: ~989 FP16 TFLOP/s; ~3.35 TB/s; ~295 FLOP/byte ridge (FP16).
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.compute.peak_flops, Hardware.Cloud.H100.memory.bandwidth, Models.Language.GPT3.parameters,
# │          TFLOPs, TB, second, param, BILLION, flop, byte)
# │ Exports: RooflineScenario.gpt3_params_b_str, RooflineScenario.h100_tflops_str, RooflineScenario.h100_bw_str, RooflineScenario.h100_ridge_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, fmt
...
class RooflineScenario:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    flops = Hardware.Cloud.H100.compute.peak_flops
    ...
    h100_tflops_str = fmt(flops.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    h100_bw_str = fmt(bw.m_as(TB/second), precision=2, commas=False)
    h100_ridge_str = fmt(ridge, precision=1, allow_zero=True, commas=False)
```
...
class RooflineInferenceMath:
    ...
    # ┌── 1. LOAD ─────────────────────────────────────────────
    params = RooflineScenario.gpt3_params
    weight_bytes_per_param = BYTES_FP16.m_as(byte)
    batch_size = 2048
    h100_bw_tbs = RooflineScenario.bw.m_as(TB/second)
    h100_peak_tflops = RooflineScenario.flops.m_as(TFLOPs/second)
    ...
    check(train_intensity > RooflineScenario.ridge, "Training example should be compute-bound")

    # ┌── 4. OUTPUT ───────────────────────────────────────────
    params_b_str = RooflineScenario.gpt3_params_b_str
    inf_weight_gb_str = fmt(inf_weight_gb, precision=1, allow_zero=True, commas=True)
    inf_intensity_str = fmt(inf_intensity, precision=1, commas=False)
    ...
    inf_throughput_math = fmt_math(f"{RooflineScenario.h100_bw_str} \\times {inf_intensity_str} = {inf_throughput_str}")
...
Consider our `{python} InfraSetup.frontier_params_b_str`B model on an H100 with `{python} RooflineScenario.h100_tflops_str` TFLOP/s peak compute and `{python} RooflineScenario.h100_bw_str` TB/s memory bandwidth. The ridge point is `{python} RooflineScenario.h100_ridge_str` FLOP/byte.

**Inference (Decode, Batch Size 1)**: Each token loads `{python} RooflineInferenceMath.inf_weight_gb_str` GB of FP16 weights and performs `{python} RooflineInferenceMath.inf_flops_math` FLOPs. Arithmetic intensity: $I =$ `{python} RooflineInferenceMath.inf_intensity_math` FLOP/byte. Since `{python} RooflineInferenceMath.inf_intensity_str` $\ll$ `{python} RooflineScenario.h100_ridge_str` (the ridge point), the workload is deeply memory-bound. The achievable throughput is `{python} RooflineInferenceMath.inf_throughput_math` TFLOP/s, which is about `{python} RooflineInferenceMath.inf_peak_pct_str` percent of the H100's peak `{python} RooflineScenario.h100_tflops_str` TFLOP/s. No amount of additional compute will help; only more memory bandwidth improves throughput.

**Training (Forward Pass, Batch Size `{python} RooflineInferenceMath.batch_size_str`)**: The same weight tensor is multiplied by a batch of `{python} RooflineInferenceMath.batch_size_str` activation vectors simultaneously, turning matrix-vector operations into matrix-matrix operations. The FLOPs increase by `{python} RooflineInferenceMath.batch_size_str`$\times$ while the weight loading remains constant: $I =$ `{python} RooflineInferenceMath.train_intensity_math` FLOP/byte. Since `{python} RooflineInferenceMath.train_intensity_str` $\gg$ `{python} RooflineScenario.h100_ridge_str`, the workload is compute-bound. The achievable throughput is now limited by the peak compute ceiling of `{python} RooflineScenario.h100_tflops_str` TFLOP/s, and the memory bandwidth is irrelevant. Adding more TFLOP/s (through a newer GPU generation) directly improves performance.
...
from mlsysim import *
from mlsysim.core.constants import *
#| label: tensor-core-h_h100-specs
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
...
# │          tc_vs_cuda_ratio_str, mfu_example_tokens_str, mfu_example_math
# └─────────────────────────────────────────────────────────────────────────────
...
class TensorCoreH100Specs:
    ...
    # MFU-example anchors: 7B-parameter transformer, single H100, batch
    # of 4 sequences × 2,048 tokens = 8,192 tokens per step, 1.0 second
    # step time. Pedagogical anchor — typical single-GPU pretraining
    # workload that lands at ~35% MFU, below the 40-50% "good" band.
    mfu_params = 7e9
    mfu_tokens_per_step = 8192
    mfu_step_seconds = 1.0

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    ratio = fp16_tc_tflops / fp32_cuda_tflops  # ≈ 15× (989 / 67)
    mfu_example_value = (
        6 * mfu_params * mfu_tokens_per_step
        / (fp16_tc_tflops * 1e12 * mfu_step_seconds)
    )

    # ┌── 3. GUARD ─────────────────────────────────────────────────────────
    check(14 < ratio < 16, "TC-vs-CUDA ratio should be ~15x for H100")
    check(
        0.33 < mfu_example_value < 0.37,
        f"MFU example should land ~35%, got {mfu_example_value:.3f}",
    )

    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    h100_fp16_tc_str = fmt(fp16_tc_tflops, precision=1, allow_zero=True, commas=False)
    h100_fp8_tc_str = fmt(fp8_tc_tflops, precision=1, allow_zero=True, commas=True)
    h100_fp32_cuda_str = fmt(fp32_cuda_tflops, precision=1, allow_zero=True, commas=False)
    tc_vs_cuda_ratio_str = fmt(ratio, precision=1, allow_zero=True, commas=False)
    mfu_example_tokens_str = fmt(mfu_tokens_per_step, precision=1, allow_zero=True)
    # Pre-build the MFU-example math expression with constant-driven
    # H100 FP16 peak and explicit tokens-per-step factor so the formula
    # stays in sync if Hardware.Cloud.H100.compute.peak_flops or the example anchors
    # change. Branded result (~35%) is the pedagogical anchor.
    mfu_example_math = fmt_math(
        f"\\text{{MFU}} = 6 \\times 7 \\times 10^9 \\times {mfu_tokens_per_step} / "
        f"({h100_fp16_tc_str} \\times 10^{{12}} \\times {mfu_step_seconds:.1f})"
    )
```
...
1.  **Significance (quantitative)**: Tensor Cores provide the bulk of the H100's `{python} TensorCoreH100Specs.h100_fp16_tc_str` TFLOP/s FP16/BF16 peak (and ~`{python} TensorCoreH100Specs.h100_fp8_tc_str` TFLOP/s at FP8)—roughly `{python} TensorCoreH100Specs.tc_vs_cuda_ratio_str`$\times$ the ~`{python} TensorCoreH100Specs.h100_fp32_cuda_str` TFLOP/s delivered by CUDA (vector) cores alone. However, this peak is only achievable for operations that decompose into tiled matrix multiplications. Layer normalization and softmax, which involve reductions and element-wise operations rather than matrix multiplications, run at approximately 3–8 percent of peak throughput on the same hardware.
...
The practical implication for fleet design is that peak TFLOP/s specifications are precision-dependent. The H100 delivers `{python} TensorCoreH100Specs.h100_fp16_tc_str` TFLOP/s in FP16 but twice that (`{python} TensorCoreH100Specs.h100_fp8_tc_str` TFLOP/s) in FP8. A fleet designed for FP8 training effectively has twice the compute density of the same fleet running FP16, with no additional hardware. This makes precision engineering a first-class optimization lever for infrastructure planners, not just a model accuracy concern.

To achieve this peak throughput in practice, the entire accelerator must be viewed as a rigid pipeline where data flows from HBM through a deepening hierarchy of caches before reaching the Tensor Cores. The fundamental constraint is **pipeline balance**\index{Pipeline!balance}: the rate at which data is staged into registers must match or exceed the rate at which the arithmetic units consume it. When the arithmetic intensity of an operation falls below the ridge point, the pipeline stalls, leaving teraflops of compute potential idle while waiting for data. This makes **kernel fusion**\index{Kernel Fusion} the single most critical software optimization for large-scale training. By fusing multiple operations (matrix multiplication, bias addition, activation function) into a single kernel, the system eliminates the round-trips to HBM that would occur if each operation were executed sequentially. Consider the attention mechanism: a naive, unfused implementation writes the $S{\times}S$ attention matrix to HBM only to read it back for the softmax operation, a round trip capped by the `{python} MemoryBandwidthScenario.h100_bw_str` TB/s memory bandwidth. A fused implementation like FlashAttention keeps these intermediate matrices entirely in on-chip SRAM, bypassing HBM and allowing the Tensor Cores to run near their theoretical peak. For our `{python} InfraSetup.frontier_params_b_str`B model, where each training step involves thousands of matrix operations across 96 Transformer layers, the difference between fused and unfused kernels can be a 2--3$\times$ throughput improvement, separating a 2-week training run from a 6-week one.
...
Memory stalls occur when the Tensor Cores complete their current tile multiplication before the next tile has been loaded from HBM. Even with the H100's `{python} MemoryBandwidthScenario.h100_bw_str` TB/s HBM bandwidth, the Tensor Cores can consume data faster than HBM can deliver it for certain matrix dimensions, creating idle cycles while the arithmetic units wait for data.
...
::: {#dfn-compute-infrastructure-pipeline-bubble .callout-definition title="Pipeline bubble"}
...
1.  **Significance (quantitative)**: It represents a direct loss in **scaling efficiency** $(\eta_{\text{scaling}})$. For a model with $p$ pipeline stages and $m$ micro-batches, the bubble fraction is approximately **$(p-1)/m$**, dictating the maximum theoretical utilization of the cluster.
...
::: {#dfn-compute-infrastructure-model-flops-utilization-mfu .callout-definition title="Model FLOPs utilization (MFU)"}

***Model FLOPs Utilization (MFU)***\index{Model FLOPs Utilization!definition} is the ratio of a model's theoretical FLOP count per training step (calculated from architecture parameters and tokens processed) to the product of hardware peak throughput and elapsed wall-clock time, measuring what fraction of the hardware's theoretical capacity is doing useful model computation rather than overhead.

1.  **Significance (quantitative)**: $\text{MFU} = O_{\text{step}} / (R_{\text{peak}} \cdot T_{\text{step}})$, where $O_{\text{step}} \approx 6PN_{\text{tokens}}$ FLOPs for a $P$-parameter transformer processing $N_{\text{tokens}}$ tokens in the step. For a 7B-parameter model on H100 ($R_{\text{peak}} =$ `{python} TensorCoreH100Specs.h100_fp16_tc_str` TFLOP/s FP16) processing `{python} TensorCoreH100Specs.mfu_example_tokens_str` tokens per step (a batch of 4 sequences of 2,048 tokens) with a step time of 1.0 second: `{python} TensorCoreH100Specs.mfu_example_math` $\approx 35\%$. A practical ceiling of 55-65 percent applies even in highly optimized implementations; the gap represents inescapable memory stalls ($\text{BW}$ bottleneck), AllReduce communication time $(T_{\text{comm}}(N))$, and pipeline bubbles.
...
1.  **Significance (quantitative)**: The H100 SXM5 operates at `{python} H100Specs.tdp_str` W TDP. When a liquid cooling system can only sustain 500 W of heat removal (inadequate cooling), the GPU firmware reduces clock frequency via thermal throttling, dropping $R_{\text{peak}}$ from `{python} H100Specs.flops_fp16_str` to roughly 712 TFLOP/s FP16, a 28 percent reduction in training throughput that propagates directly into longer wall-clock training time and higher cost per model.
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.tdp, GPUS_PER_HOST, kilowatt)
...
# │
# │ Goal: Provide energy efficiency (TFLOP/s/W) across GPU generations.
# │ Show: advertised low-precision TFLOP/s/W trend across generations.
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.V100.compute.peak_flops, Hardware.Cloud.V100.tdp,
# │          Hardware.Cloud.A100.compute.peak_flops, Hardware.Cloud.A100.tdp, Hardware.Cloud.H100.compute.precision_flops["fp8"],
# │          Hardware.Cloud.H100.tdp, Hardware.Cloud.B200.compute.precision_flops["fp8"], Hardware.Cloud.B200.tdp, TFLOPs, second, watt)
...
class GpuEfficiencyScenario:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    v100_f16 = Hardware.Cloud.V100.compute.peak_flops
    v100_tdp = Hardware.Cloud.V100.tdp
    a100_f16 = Hardware.Cloud.A100.compute.peak_flops
    a100_tdp = Hardware.Cloud.A100.tdp
    h100_low_precision = Hardware.Cloud.H100.compute.precision_flops["fp8"]
    h100_tdp = Hardware.Cloud.H100.tdp
    b200_low_precision = Hardware.Cloud.B200.compute.precision_flops["fp8"]
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_ef_str = fmt(v100_eff, precision=2, commas=False)
    a100_ef_str = fmt(a100_eff, precision=2, commas=False)
    h100_ef_str = fmt(h100_eff, precision=2, commas=False)
    b200_ef_str = fmt(b200_eff, precision=2, commas=False)
    b200_fp8_tflops_str = fmt(b200_low_precision.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    b200_tdp_w_str = fmt(b200_tdp.m_as(watt), precision=1, allow_zero=True, commas=False)
```

The power efficiency trajectory provides a partial counterweight to rising TDP. While absolute power has increased, the advertised low-precision TFLOP/s delivered per watt has improved dramatically: from `{python} GpuEfficiencyScenario.v100_ef_str` TFLOP/s per watt (V100 FP16) to `{python} GpuEfficiencyScenario.h100_ef_str` TFLOP/s per watt (H100 FP8) to `{python} GpuEfficiencyScenario.b200_ef_str` TFLOP/s per watt (B200 FP8). These are not apples-to-apples FP16 numbers; the precision mode is part of the hardware trend. For a fixed compute budget that can use the newer precision modes, each generation requires fewer chips and therefore less total power. Frontier models, however, grow faster than efficiency improves, so the absolute power demand of training the latest model increases with every generation. This is the treadmill of infrastructure: efficiency gains buy time, but scale growth consumes it.
...
The macro impact appears when serving our `{python} InfraSetup.frontier_params_b_str`B model. Generating a single token requires loading the entire 350 GB weight tensor from HBM:
...
Our `{python} InfraSetup.frontier_params_b_str`B model needs 350 GB for its FP16 weights alone. A single H100 provides 80 GB of HBM, so we need at least five accelerators just to place the weights. An 8-H100 dense node can hold those weight shards, but it cannot hold the full Adam training state in HBM: optimizer moments, gradients, activations, and communication buffers push the requirement into the 2 TB range. This arithmetic forces us beyond the single die to the next physical level: the **node**, and it also forces the training system to combine tensor parallelism with optimizer sharding and memory offload.
...
class BandwidthHierarchyScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_str = fmt(nvlink_gbs, precision=1, allow_zero=True, commas=False)
    pcie_bw_str = fmt(pcie_gbs, precision=1, allow_zero=True, commas=False)
    ib_bw_str = fmt(ib_gbs, precision=1, allow_zero=True, commas=False)
```
...
| **Inter-Node**      | InfiniBand NDR     |   **~`{python} BandwidthHierarchyScenario.ib_bw_str` GB/s**   |         ~5--10 μs          | Pod (Thousands)    |
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.memory.bandwidth, NVLINK_H100_BW, INFINIBAND_NDR_BW,
...
class BandwidthStaircase:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    bs_hbm_bw_str = fmt(hbm_bw_gbs, precision=1, allow_zero=True)
    bs_nvlink_bw_str = fmt(nvlink_bw_gbs, precision=1, allow_zero=True, commas=False)
    bs_ib_bw_str = fmt(ib_bw_gbs, precision=1, allow_zero=True, commas=False)
    bs_gap_hbm_nv_str = fmt(gap_hbm_nv, precision=1, commas=False)
    bs_gap_nv_ib_str = fmt(gap_nv_ib, precision=1, allow_zero=True, commas=False)
...
For our `{python} InfraSetup.frontier_params_b_str`B model with hidden dimension 12,288 and a microbatch of 4 sequences at 2,048 tokens, the activation tensor at each stage boundary is approximately $4 \times 2{,}048 \times 12{,}288 \times 2$ bytes (FP16) $\approx$ 200 MB. This is far smaller than the 350 GB gradient AllReduce required by data parallelism, which is why pipeline parallelism places less demanding requirements on the inter-node network.
...
# │ Show: ~640 GB aggregate HBM; ~15.8 PFLOP/s aggregate FP8.
# │ How: Hardware.Cloud.H100.memory.capacity * GPUS_PER_HOST; Hardware.Cloud.H100.compute.precision_flops["fp8"] * GPUS_PER_HOST.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.memory.capacity, Hardware.Cloud.H100.compute.precision_flops["fp8"],
# │          GPUS_PER_HOST, GiB, TFLOPs, second)
# │ Exports: node_hbm_cap, RooflineScenario.h100_tflops_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class NodeCapacityScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_str = fmt(total_mem.m_as(GiB), precision=1, allow_zero=True, commas=False)
    h100_tflops_str = fmt(flops.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
```

The result is that the 8 GPUs within a DGX H100 can be treated as a single logical device with `{python} NodeCapacityScenario.node_hbm_cap_str` GB of aggregate HBM and a combined compute throughput of 8$\times$ `{python} NodeCapacityScenario.h100_tflops_str` TFLOP/s. This abstraction is critical for software: the training framework can partition a model across the 8 GPUs using tensor parallelism as if they were a single larger processor, with the NVSwitch fabric making the partitioning nearly transparent from a performance perspective.
...
The MI300X's 192 GB of HBM per accelerator is particularly notable. While the H100 offers 80 GB, the MI300X's larger capacity means that a single 8-GPU node can hold 1,536 GB of HBM, enough to fit the full weight tensor of our `{python} InfraSetup.frontier_params_b_str`B model (350 GB in FP16) with much more headroom for gradients and activations. It still does not hold the entire 2.2--2.5 TB Adam training footprint in HBM, but the extra capacity reduces offload pressure, enables larger microbatches, and can improve scaling efficiency.
...
class NodeReliabilityScenario:
    ...
    canonical_gpu_mtbf_hours = GPU_MTTF_HOURS
    gpus_per_node = GPUS_PER_HOST
    recovery_low_min = 10
...
As @tbl-memory-breakdown shows, for our `{python} InfraSetup.frontier_params_b_str`B model, the training memory breaks down as follows:

| **Component**                          | **Precision** | **Memory per Parameter** | **Total for `{python} InfraSetup.frontier_params_b_str`B Model** |
|:---------------------------------------|:-------------:|:------------------------:|:----------------------------------------------------------------:|
...
The model weights in FP16 occupy 350 GB ($`{python} InfraSetup.frontier_params_b_str` \times 10^9 \times 2$ bytes). The Adam optimizer maintains two FP32 state tensors per parameter (first moment $m$ and second moment $v$), adding $`{python} InfraSetup.frontier_params_b_str` \times 10^9 \times 4 \times 2 = 1{,}400$ GB. The gradients in FP16 add another 350 GB. Intermediate activations (needed for the backward pass) depend on the batch size and sequence length but typically require 100--400 GB for practical training configurations. The total memory footprint for training is therefore 2.2--2.5 TB, roughly 6--7$\times$ the raw weight size.

A DGX H100 node provides `{python} NodeCapacityScenario.node_hbm_cap_str` GB of aggregate HBM across its 8 GPUs, plus 2 TB of host DDR5 DRAM. The HBM capacity alone is insufficient for the full training state. To appreciate why, consider the memory arithmetic for different parallelism strategies. With **pure data parallelism**\index{Data Parallelism!pure}, each GPU must hold the complete model: 350 GB weights + 350 GB gradients + 1,400 GB optimizer states = 2,100 GB -- physically impossible on an 80 GB device. Even **ZeRO Stage 3**\index{ZeRO!stage 3}, which shards all three components across only 8 GPUs, yields 2,100/8 ≈ 263 GB per GPU, still more than triple the available HBM. The solution is to combine tensor parallelism (which splits the model's layers across GPUs, reducing per-GPU weights to 350/8 = 43.75 GB) with data-parallel sharding across many replicas. In a configuration with TP-8 within each node and 128-way data parallelism across the cluster, each GPU holds 43.75 GB of weight shards. The optimizer state associated with that tensor-parallel shard is then sharded across the 128 data-parallel replicas, so each GPU holds approximately 1,400/(8 × 128) ≈ 1.4 GB of optimizer state, or 10.9 GB per 8-GPU tensor-parallel group, before gradients, activations, buffers, and fragmentation. The weight-plus-optimizer subtotal of roughly 45 GB is only the starting point; the memory-budget exercise below shows why gradients and activations still require checkpointing and offload. This arithmetic is why tensor parallelism, optimizer sharding, activation checkpointing, and memory tiering are used together for frontier model training. Several additional strategies further optimize this memory budget:
...
|:----------------|:------------------------:|:-----------------------------------------------------------:|:-----------:|:----------------------|
| **GPU HBM3**    | 80 GB$\times$ 8 = 640 GB | `{python} MemoryBandwidthScenario.h100_bw_str` TB/s per GPU |    <1 μs    | Active computation    |
...
The orchestration is complex but essential: without it, training our `{python} InfraSetup.frontier_params_b_str`B model on currently available hardware would be impossible. @sec-distributed-training-systems examines these memory optimization strategies and their interaction with parallelism in full detail.

#### Putting it together: Memory budget exercise {#sec-compute-infrastructure-putting-together-memory-budget-exercise-f357}

To solidify the memory planning concepts, consider a concrete sizing exercise for our `{python} InfraSetup.frontier_params_b_str`B model on a DGX H100 node.
...
- Model: `{python} InfraSetup.frontier_params_b_str`B parameters
...
class InterconnectCommMath:
    ...
    t_intra_math = fmt_math(f"T_{{\\text{{intra}}}} = 1\\text{{ GB}} / {_nvlink_bw_val:.0f}\\text{{ GB/s}} \\approx \\mathbf{{1.1 \\text{{ ms}}}}")
    t_inter_math = fmt_math(f"T_{{\\text{{inter}}}} = 1\\text{{ GB}} / {_ib_bw_gbs_val:.0f}\\text{{ GB/s}} \\approx \\mathbf{{20 \\text{{ ms}}}}")
```
...
2. **Inter-Node (InfiniBand NDR)**: Bandwidth = `{python} BandwidthHierarchyScenario.ib_bw_str` GB/s.
...
For our `{python} InfraSetup.frontier_params_b_str`B model, a single DGX H100 node provides `{python} NodeCapacityScenario.node_hbm_cap_str` GB of aggregate HBM, enough to hold the FP16 model weights but not the full Adam training state without sharding and offload. Training also requires processing trillions of tokens, and a single node's compute throughput limits training time to months. To complete training in a reasonable timeframe (weeks rather than months), we need tens or hundreds of nodes. Stacking those nodes into a physical enclosure brings us to the next level of infrastructure, where the constraints shift from bandwidth and capacity to raw power and heat.
...
class GridQueue:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ratio_str = fmt(lead_time_ratio, precision=1, allow_zero=True, commas=False)
    queue_str = fmt(grid_queue_size_gw, precision=1, allow_zero=True, commas=False)
    gpu_lead_time_months_str = fmt(gpu_lead_time_months, precision=1, allow_zero=True, commas=False)
    substation_lead_time_months_str = fmt(substation_lead_time_months, precision=1, allow_zero=True, commas=False)
...
::: {#nbk-compute-infrastructure-napkin-math-interconnection-queue .callout-notebook title="Napkin math: The interconnection queue"}
...
**Systems lesson**: ML clusters do not behave like traditional data center workloads: the *temporal correlation* of power demand across thousands of chips creates a qualitatively different electrical engineering challenge.
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.defaults import PUE_LEGACY
from mlsysim.fmt import fmt, check
...
class PueSavings:
    ...
    households_158_to_110_str = fmt(households_158_to_110_rounded, precision=1, allow_zero=True, commas=True)
```
...
The rack concentrates power and heat into a physical volume where thermodynamics, not software, sets the limits. A single rack of 32 GPUs, however, is far from sufficient for our `{python} InfraSetup.frontier_params_b_str`B model, which may require thousands of accelerators to train in a reasonable timeframe. The next level of infrastructure aggregates hundreds of racks into a unified computing system: the pod, whose network topology (@fig-pod-network-topology) ensures that any GPU can communicate with any other at full line rate.
...
class PodTrainingMath:
    ...
    pod_flops_math = fmt_math(f"6 \\times {InfraSetup.gpt3_params_b_str} \\times 300 \\times 10^{{18}}")
    total_flops_math = fmt_math(f"3.15 \\times 10^{{23}}")
    n_params_math = fmt_math(f"P = {InfraSetup.gpt3_params_b_str} \\times 10^9")
    cluster_throughput_math = fmt_math(f"8{{,}}192 \\times {H100PerformanceScenario.h100_tflops_str} \\times 10^{{12}} \\times 0.45")
    # Numeric sustained throughput in EFLOP/s — keeps the prose claim aligned with
    # H100PerformanceScenario.h100_tf if the FP8 peak ever shifts.
    cluster_eflops = 8192 * H100PerformanceScenario.h100_tf * 1e12 * 0.45 / 1e18
    cluster_eflops_str = fmt(cluster_eflops, precision=2, commas=False)
```

Training a `{python} InfraSetup.frontier_params_b_str`B model on this cluster requires `{python} PodTrainingMath.pod_flops_math` total floating-point operations (assuming 300 billion training tokens). At 45 percent MFU -- a strong but achievable figure for well-tuned large-model training -- the cluster sustains `{python} PodTrainingMath.cluster_throughput_math` FLOP/s, or about `{python} PodTrainingMath.cluster_eflops_str` EFLOP/s, yielding a physics-limit training time of approximately 12 hours. This is the absolute floor: every GPU computing useful FLOPs at 45 percent of FP8 peak, with zero communication overhead, zero pipeline bubbles, and zero downtime.
...
class TrainingTime175B:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    t_physics_hr_str = fmt(t_physics_hr, precision=1, allow_zero=True)
...
We can derive the training time for our `{python} InfraSetup.frontier_params_b_str`B model from first principles.
...
2. **Cluster throughput**: 8,192 H100 GPUs at `{python} NodeCapacityScenario.h100_tflops_str` TFLOP/s peak, operating at 45 percent MFU:
...
class WscThroughput:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    n_gpus = 10_000
    per_gpu_inject_gbs = INFINIBAND_NDR_BW.m_as(GB/second)
    ...
    wsc_inject_tbs_str = fmt(wsc_inject_tbs, precision=1, allow_zero=True, commas=False)
    wsc_oversub_tbs_str = fmt(wsc_oversub_tbs, precision=1, allow_zero=True, commas=False)
...
For our `{python} InfraSetup.frontier_params_b_str`B model with data parallelism across 128 nodes, each training step requires an AllReduce of approximately 350 GB of gradients. If the network *cannot* overlap this communication with the next forward pass, the GPUs sit idle during synchronization.
...
To put this in perspective: if training our `{python} InfraSetup.frontier_params_b_str`B model takes 2 weeks (336 hours) on a 10,000-GPU cluster, we should expect approximately 67 individual GPU failures during the training run. Each failure potentially corrupts the training state and requires either rolling back to a checkpoint or reorganizing the cluster to work around the failed node.
...
class CommScalingMath:
    ...
    _ib_gbs = BandwidthHierarchyScenario.ib_bw_str
...
- **Per-GPU compute time** on 1,024 GPUs, each at `{python} NodeCapacityScenario.h100_tflops_str` TFLOP/s (50 percent utilization):
...
To illustrate this interaction concretely, consider three parallelism configurations for training our `{python} InfraSetup.frontier_params_b_str`B model on a 1,024-GPU cluster organized as 128 nodes of 8 GPUs each.
...
class BuildVsBuyFigureScenario:
    ...
    gpus_str = fmt(gpus, precision=1, allow_zero=True)
    nodes_str = fmt(nodes, precision=1, allow_zero=True, commas=False)
    util_pct_str = fmt(utilization * 100, precision=1, allow_zero=True, commas=False)
    horizon_months_str = fmt(horizon_months, precision=1, allow_zero=True, commas=False)
    ...
    monthly_opex_str = fmt(monthly_opex_usd / 1e6, precision=3, prefix="$", suffix="M")
    ...
    breakeven_month_str = fmt(breakeven_month, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.tdp, GPUS_PER_HOST,
...
class TcoScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_cost_str = fmt(cloud_node_annual, precision=1, allow_zero=True, prefix="$")
    annual_elec_str = fmt(annual_elec, precision=1, allow_zero=True, prefix="$")
    onprem_str = fmt(onprem_node_annual, precision=1, allow_zero=True, prefix="$")
    util_pct_str = fmt(utilization*100, precision=1, allow_zero=True, commas=False)
    breakeven_util_str = fmt(breakeven, precision=1, allow_zero=True, commas=False)
```
...
class TenKGpuClusterTco:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    nodes_str = fmt(nodes, precision=1, allow_zero=True, commas=True)
    hardware_capex_str = fmt(hardware_capex_usd / 1e6, precision=1, prefix="$", suffix="M")
    network_capex_str = fmt(network_capex_usd / 1e6, precision=1, allow_zero=True, prefix="$", suffix="M")
    facility_capex_str = fmt(facility_capex_usd / 1e6, precision=1, allow_zero=True, prefix="$", suffix="M")
    annual_electricity_str = fmt(annual_electricity_usd / 1e6, precision=1, prefix="$", suffix="M")
    annual_staffing_str = fmt(annual_staffing_usd / 1e6, precision=1, allow_zero=True, prefix="$", suffix="M")
    upfront_capex_str = fmt(upfront_capex_usd / 1e6, precision=1, prefix="$", suffix="M")
    annual_opex_str = fmt(annual_opex_usd / 1e6, precision=1, prefix="$", suffix="M")
    three_year_total_str = fmt(three_year_total_usd / 1e6, precision=1, allow_zero=True, prefix="$", suffix="M")
    cloud_annual_str = fmt(cloud_annual_usd / 1e6, precision=1, allow_zero=True, prefix="$", suffix="M")
    cloud_three_year_str = fmt(cloud_three_year_usd / 1e6, precision=1, allow_zero=True, prefix="$", suffix="M")
    savings_str = fmt(savings_usd / 1e6, precision=1, allow_zero=True, prefix="$", suffix="M")
    breakeven_util_str = fmt(breakeven_utilization * 100, precision=1, allow_zero=True, commas=False)
```

::: {#nbk-compute-infrastructure-10-000-gpu-cluster .callout-notebook title="The 10,000-GPU cluster"}

Consider a cluster of `{python} TenKGpuClusterTco.nodes_str` DGX H100 nodes (10,000 GPUs) for training our `{python} InfraSetup.frontier_params_b_str`B model.

**On-Premises (3-year lifecycle)**:

- **Hardware CapEx**\index{CapEx!hardware}: `{python} TenKGpuClusterTco.nodes_str` nodes$\times$ \$350,000 = `{python} TenKGpuClusterTco.hardware_capex_str`
...
- **Annual Electricity**\index{Electricity!annual electricity}: simplified GPU-only estimate: 10,000 GPUs $\times$ `{python} H100Specs.tdp_str` W $\times$ PUE 1.1 $\times$ 8,760 h $\times$ \$0.07/kWh = `{python} TenKGpuClusterTco.annual_electricity_str`/year
- **Annual Staffing**\index{Staffing!annual staffing}: ~`{python} TenKGpuClusterTco.annual_staffing_str`/year
- **3-Year Total**: `{python} TenKGpuClusterTco.upfront_capex_str` + 3$\times$ `{python} TenKGpuClusterTco.annual_opex_str` = **~`{python} TenKGpuClusterTco.three_year_total_str`**

**Cloud (3-year rental at 80 percent utilization)**:

- **Annual Cost**\index{Cost!annual cost}: 10,000 GPUs$\times$ \$4.00/hr$\times$ 8,760 h$\times$ 0.80 = `{python} TenKGpuClusterTco.cloud_annual_str`/year
- **3-Year Total**: **~`{python} TenKGpuClusterTco.cloud_three_year_str`**

**Break-Even**\index{Total Cost of Ownership!break-even}: On-premises saves approximately `{python} TenKGpuClusterTco.savings_str` over three years at 80 percent utilization. The savings disappear if utilization drops below roughly `{python} TenKGpuClusterTco.breakeven_util_str` percent, because the on-premises hardware still incurs facility and staffing costs regardless of load.
...
- **Node sizing**: Determine the number of accelerators per node and the memory tier allocation. For our `{python} InfraSetup.frontier_params_b_str`B model, eight GPUs per node with tensor parallelism, 2 TB host DRAM for optimizer state offloading.
...
- H100 GPU: `{python} NodeCapacityScenario.h100_tflops_str` TFLOP/s peak, assume 45 percent MFU
...
For ML training, memory pooling addresses the optimizer state problem directly. The 1,400 GB of optimizer state for our `{python} InfraSetup.frontier_params_b_str`B model could reside in a shared CXL memory pool rather than being replicated or sharded across individual nodes. Nodes that are executing the compute-intensive forward and backward passes would access only their weight and activation shards from HBM, while the optimizer state would be fetched from the CXL pool only during the parameter update step. This architecture would reduce the per-node memory requirement and potentially allow training of larger models on fewer nodes.

The challenge is bandwidth. CXL 3.0 over a PCIe Gen5 x16 link provides approximately 64 GB/s of read bandwidth, which is roughly 50$\times$ slower than HBM3 (`{python} MemoryBandwidthScenario.h100_bw_str` TB/s). Data that must be accessed at HBM speeds (weights and activations during the forward and backward passes) cannot reside in CXL memory without creating severe bottlenecks. CXL memory is therefore a complement to HBM, not a replacement: it extends the capacity of the memory hierarchy without competing with HBM's bandwidth tier.
...
The limitation is that 44 GB of SRAM is insufficient for frontier models. Our `{python} InfraSetup.frontier_params_b_str`B model requires 350 GB for weights alone, far exceeding the WSE's capacity. Cerebras addresses this through a model-parallel scheme where the WSE processes layers sequentially, with weights streamed from external memory (MemoryX units) to the chip. This weight streaming approach works well for inference (where each layer is used once per token) but introduces pipelining complexity for training (where each layer is used in both forward and backward passes with different activations).
...
Amdahl's Law\index{Amdahl's Law} establishes the theoretical limit: the sequential fraction of the computation (gradient synchronization, pipeline bubble time, data loading stalls) bounds the maximum speedup regardless of parallelism. For a `{python} InfraSetup.frontier_params_b_str`B-parameter model with 350 GB of gradients, the AllReduce at each training step requires every GPU to exchange data with every other GPU.
...
# │          Hardware.Cloud.H100.tdp, GPUS_PER_HOST, param, BILLION, TFLOPs, second, TB, kilowatt)
# │ Exports: RooflineScenario.gpt3_params_b_str, NodeCapacityScenario.h100_tflops_str, RooflineScenario.h100_bw_str, RackPowerScenario.rack_power_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
