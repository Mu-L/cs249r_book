...
# │
# │ Imports: mlsysim.core.constants (V100_*, A100_*, H100_*, TFLOPs,
# │          second, GB, watt)
...
The result is that the 8 GPUs within a DGX H100 can be treated as a single logical device with `{python} NodeCapacityDenseRecap.node_hbm_cap_str` GB of aggregate HBM and a combined compute throughput of 8$\times$ `{python} H100PerformanceScenario.h100_tflops_str` TFLOP/s. This abstraction is critical for software: the training framework can partition a model across the 8 GPUs using tensor parallelism as if they were a single larger processor, with the NVSwitch fabric making the partitioning nearly transparent from a performance perspective.
...
2. **Cluster throughput**: 8,192 H100 GPUs at `{python} H100PerformanceScenario.h100_tflops_str` TFLOP/s peak, operating at 45 percent MFU:
