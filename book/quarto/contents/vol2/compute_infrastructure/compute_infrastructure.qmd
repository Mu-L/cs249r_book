...
**Problem**: Generating one token from Llama-3 70B (140 GB of FP16 weights) requires both arithmetic and a full weight load from HBM. Assuming capacity has been solved (by sharding, a higher-capacity device, or quantization), which dominates the per-token time on an H100: compute or memory transfer?
...
Loading 140 GB of weights from HBM at `{python} MemoryBandwidthLatencyRecap.h100_bw_str` TB/s takes:
...
The macro impact appears when serving our `{python} InfraFrontierTdpServingRecap.frontier_params_b_str`B model. Generating a single token requires loading the entire 350 GB weight tensor from HBM:
...
- [ ] If a node has 8 GPUs at `{python} H100TdpRackRecap.tdp_str` W each, why is the total node power budgeted at ~7 kW rather than 5.6 kW?
...
Consider a 1,000-GPU cluster of H100s at `{python} H100TdpCoolingRecap.tdp_str` W TDP each.
...
2. **Cluster throughput**: 8,192 H100 GPUs at `{python} PodTrainingMath.h100_tflops_str` TFLOP/s peak, operating at 45 percent MFU:
...
- **Per-GPU compute time** on 1,024 GPUs, each at `{python} PodSetupComputeRecap.h100_tflops_str` TFLOP/s (50 percent utilization):
...
Consider a cluster of `{python} TenKGpuClusterTco.nodes_str` DGX H100 nodes (10,000 GPUs) for training our `{python} InfraFrontierTcoTenKRecap.frontier_params_b_str`B model.
