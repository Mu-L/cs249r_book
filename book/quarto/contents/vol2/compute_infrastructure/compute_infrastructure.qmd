...
class RooflineInferenceMath:
    ...
    inf_flops_math = fmt_math(f"2 \\times {InfraSetup.gpt3_params_b_str} \\times 10^9")
    inf_throughput_math = fmt_math(f"{InfraSetup.h100_bw_str}")
    train_intensity_math = fmt_math(f"2048 \\times 1.0")
```
...
Consider a cluster of 1,250 DGX H100 nodes (10,000 GPUs) for training our `{python} InfraSetup.frontier_params_b_str`B model.
...
**Break-Even**\index{Total Cost of Ownership!break-even}: On-premises saves approximately \$273M over three years at 80 percent utilization. The savings disappear if utilization drops below roughly 54 percent, because the on-premises hardware still incurs facility and staffing costs regardless of load.
