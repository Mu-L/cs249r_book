...
finally the 80 GB of HBM3 at `{python} MemoryBandwidthCacheRecap.h100_bw_str`.
...
class TokenLatencyScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(h_h100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
...
class RooflineCaptionRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(h_h100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    h100_bw_str = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False, suffix=" TB/s")
...
class RooflineCalloutRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(Hardware.Cloud.H100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False, suffix=" TFLOP/s")
    h100_ridge_str = fmt((Hardware.Cloud.H100.peak_flops/Hardware.Cloud.H100.memory.bandwidth).to(flop/byte).magnitude, precision=1, commas=False)
```
...
PCIe bandwidth -- though substantial at ~`{python} BandwidthHierarchyPcieRecap.pcie_bw_str` per direction (128 GB/s bidirectional) per x16 link -- is often the bottleneck for operations that cross the accelerator boundary, such as data loading, checkpointing, and inter-node gradient synchronization.
...
class InfraFrontierTcoClusterRecap:
    ...
    hardware_capex_m_str = fmt(hardware_capex_usd / 1e6, precision=2, commas=False, suffix=" M")
```
