...
class InfraSetup:
    ...
    # OUTPUTS
    h100_tflops_str = fmt(h100_tflops_val, precision=1, commas=False)
    h100_bw_str = fmt(h100_bw_val, precision=2, commas=False)
    h100_ridge_str = fmt(h100_ridge_val, precision=1, commas=False)
    gpt3_params_b_str = fmt(gpt3_params_b_val, precision=1, commas=False)
    frontier_params_b_str = fmt(frontier_params_b_val, precision=1, commas=False)
    ib_bw_str = fmt(ib_bw_gbps_val, precision=1, commas=False)
    ddr_bw_str = fmt(HOST_DRAM_BW.m_as(GB/second), precision=1, commas=False)
...
class V100Specs:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    flops_fp16_str = fmt(flops_fp16_tflops, precision=1, commas=False)
    mem_bw_str = fmt(mem_bw_gbs, precision=1, commas=False)
    tdp_str = fmt(tdp_w, precision=1, commas=False)
    nvlink_str = fmt(nvlink_gbs, precision=1, commas=False)
...
class H100PerformanceScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(h100_tf, precision=1, commas=False)
```
...
class B200PerformanceScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    fp8_tflops_str = fmt(fp8_tflops, precision=1)
    tdp_w_str = fmt(tdp_w, precision=1, commas=False)
    nvlink_str = fmt(nvlink_gbs, precision=1)
```
...
class MemoryBandwidthScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ddr_bw_str = fmt(ddr_gbs, precision=1, commas=False)
...
class RooflineLandscapeMath:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────────
    decode_intensity_str = fmt(decode_intensity, precision=1, commas=False)
    training_batch_size_str = fmt(training_batch_size, precision=1, commas=False)
    train_intensity_str = fmt(train_intensity, precision=1, commas=True)
    train_gap_str = fmt(train_gap, precision=1, commas=True)
```
...
class RooflineScenario:
    ...
    h100_tflops_str = fmt(flops.m_as(TFLOPs/second), precision=1, commas=False)
...
class RooflineInferenceMath:
    ...
    inf_weight_gb_str = fmt(inf_weight_gb, precision=1, commas=True)
...
class TensorCoreH100Specs:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    h100_fp16_tc_str = fmt(fp16_tc_tflops, precision=1, commas=False)
    h100_fp8_tc_str = fmt(fp8_tc_tflops, precision=1, commas=True)
    h100_fp32_cuda_str = fmt(fp32_cuda_tflops, precision=1, commas=False)
    tc_vs_cuda_ratio_str = fmt(ratio, precision=1, commas=False)
    mfu_example_tokens_str = fmt(mfu_tokens_per_step, precision=1)
...
class GpuEfficiencyScenario:
    ...
    b200_fp8_tflops_str = fmt(b200_low_precision.m_as(TFLOPs/second), precision=1, commas=False)
    b200_tdp_w_str = fmt(b200_tdp.m_as(watt), precision=1, commas=False)
```
...
class BandwidthHierarchyScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_bw_str = fmt(nvlink_gbs, precision=1, commas=False)
    pcie_bw_str = fmt(pcie_gbs, precision=1, commas=False)
    ib_bw_str = fmt(ib_gbs, precision=1, commas=False)
```
...
class BandwidthStaircase:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    bs_hbm_bw_str = fmt(hbm_bw_gbs, precision=1)
    bs_nvlink_bw_str = fmt(nvlink_bw_gbs, precision=1, commas=False)
    bs_ib_bw_str = fmt(ib_bw_gbs, precision=1, commas=False)
    bs_gap_hbm_nv_str = fmt(gap_hbm_nv, precision=1, commas=False)
    bs_gap_nv_ib_str = fmt(gap_nv_ib, precision=1, commas=False)
...
class NodeCapacityScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_str = fmt(total_mem.m_as(GiB), precision=1, commas=False)
    h100_tflops_str = fmt(flops.m_as(TFLOPs/second), precision=1, commas=False)
```
...
class NodeReliabilityScenario:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    nodes_str = fmt(nodes, precision=1, commas=False)
    run_hours_str = fmt(run_hours, precision=1, commas=False)
    node_mtbf_str = fmt(composite_node_mtbf_hours, precision=1)
    expected_failures_str = fmt(expected_failures, precision=1, commas=False)
    failures_per_day_str = fmt(failures_per_day, precision=1, commas=False)
    gpu_mtbf_str = fmt(canonical_gpu_mtbf_hours, precision=1)
    gpu_only_outages_str = fmt(gpu_only_expected_outages, precision=1, commas=False)
    recovery_low_str = fmt(recovery_low_min, precision=1, commas=False)
    recovery_high_str = fmt(recovery_high_min, precision=1, commas=False)
...
class GridQueue:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ratio_str = fmt(lead_time_ratio, precision=1, commas=False)
    queue_str = fmt(grid_queue_size_gw, precision=1, commas=False)
    gpu_lead_time_months_str = fmt(gpu_lead_time_months, precision=1, commas=False)
    substation_lead_time_months_str = fmt(substation_lead_time_months, precision=1, commas=False)
...
class PueSavings:
    ...
    households_158_to_110_str = fmt(households_158_to_110_rounded, precision=1, commas=True)
```
...
class WscThroughput:
    ...
    wsc_inject_tbs_str = fmt(wsc_inject_tbs, precision=1, commas=False)
    wsc_oversub_tbs_str = fmt(wsc_oversub_tbs, precision=1, commas=False)
    n_gpus_str = fmt(n_gpus, precision=1, commas=False)
```
...
class CheckpointOverheadBudget:
    ...
    ckpt_cadence_min_str               = fmt(cadence_min, precision=1, commas=False)
    ckpt_target_overhead_pct_str       = fmt(target_overhead_pct, precision=1, commas=False)
    ckpt_allowed_write_s_str           = fmt(allowed_write_s, precision=1, commas=False)
    ckpt_required_bw_gbs_str           = fmt(required_bw_gbs, precision=1, commas=False)
    ckpt_reference_write_s_str         = fmt(reference_write_s, precision=1, commas=False)
    ckpt_reference_bw_gbs_str          = fmt(reference_bw_gbs, precision=1, commas=False)
    ckpt_reference_overhead_pct_str    = fmt(reference_overhead_pct, precision=1, commas=False)
    ckpt_reference_min_cadence_min_str = fmt(reference_min_cadence_min, precision=1, commas=False)
    ckpt_run_count_str                 = fmt(checkpoints_per_run, precision=1)
    ckpt_run_total_tb_str              = fmt(run_total_tb, precision=1)
...
class FabricSizingScenario:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    gpus_str = fmt(gpus, precision=1)
    nodes_str = fmt(nodes, precision=1, commas=False)
    switch_ports_str = fmt(switch_ports, precision=1, commas=False)
    endpoint_links_per_gpu_str = fmt(endpoint_links_per_gpu, precision=1, commas=False)
    leaf_switches_str = fmt(leaf_switches, precision=1, commas=False)
    spine_switches_str = fmt(spine_switches, precision=1, commas=False)
    fabric_links_str = fmt(fabric_links, precision=1)
```
...
class BuildVsBuyFigureScenario:
    ...
    gpus_str = fmt(gpus, precision=1)
    nodes_str = fmt(nodes, precision=1, commas=False)
    util_pct_str = fmt(utilization * 100, precision=1, commas=False)
    horizon_months_str = fmt(horizon_months, precision=1, commas=False)
...
class TcoScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cloud_cost_str = fmt(cloud_node_annual, precision=1, prefix="$")
...
class TenKGpuClusterTco:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    nodes_str = fmt(nodes, precision=1, commas=True)
    hardware_capex_str = fmt(hardware_capex_usd / 1e6, precision=1, prefix="$", suffix="M")
    network_capex_str = fmt(network_capex_usd / 1e6, precision=1, prefix="$", suffix="M")
    facility_capex_str = fmt(facility_capex_usd / 1e6, precision=1, prefix="$", suffix="M")
    annual_electricity_str = fmt(annual_electricity_usd / 1e6, precision=1, prefix="$", suffix="M")
    annual_staffing_str = fmt(annual_staffing_usd / 1e6, precision=1, prefix="$", suffix="M")
...
class PlanningThroughputScenario:
    ...
    h100_peak_tflops_str = fmt(h100_peak_tflops, precision=1, commas=False)
