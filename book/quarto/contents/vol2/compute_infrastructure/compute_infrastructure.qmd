...
class A100HbmQuantRecap:
    ...
    b200_bw_tbs_str = fmt(b200_bw_tbs, precision=0, commas=False)
...
class BandwidthStaircase:
    ...
    hbm_ms_str = fmt_int(hbm_ms, commas=False)
    nvlink_ms_str = fmt_int(nvlink_ms, commas=False)
...
class PodTrainingMath:
    ...
    cluster_throughput_math = fmt_math(
        f"8{{,}}192 \\times {h100_tflops_str} \\times 10^{{12}} \\times {mfu}"
    )
    cluster_eflops = 8192 * h100_fp8_tflops * 1e12 * mfu / 1e18
...
class A100FallaciesRecap:
    ...
    weights_fp16_gb_str = fmt_int(weights_fp16_gb, commas=False)
    ...
    weights_int8_gb_str = fmt_int(weights_int8_gb, commas=False)
    option_b_lat_ms_str = fmt_int(option_b_lat_ms, commas=False)
...
class TcoSummaryBreakevenRecap:
    ...
    denominator = (
        ...
        - node_power_kw * HOURS_PER_YEAR * 0.07 * facility_pue
    )
