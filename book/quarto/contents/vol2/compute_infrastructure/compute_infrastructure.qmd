...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: A100HbmQuantRecap.mem_bw_tbs_str
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: TensorCoreMfuRecap.h100_fp16_tc_str, TensorCoreMfuRecap.mfu_example_tokens_str, TensorCoreMfuRecap.mfu_example_math
class TensorCoreMfuRecap:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp16_tc_str = fmt(Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    mfu_example_tokens_str = fmt(8192, precision=0, commas=True)
    mfu_example_math = fmt_math('\\approx 35\\%')
```

1.  **Significance (quantitative)**: $\text{MFU} = O_{\text{step}} / (R_{\text{peak}} \cdot T_{\text{step}})$, where $O_{\text{step}} \approx 6PN_{\text{tokens}}$ FLOPs for a $P$-parameter transformer processing $N_{\text{tokens}}$ tokens in the step. For a 7B-parameter model on H100 ($R_{\text{peak}} =$ `{python} TensorCoreMfuRecap.h100_fp16_tc_str` TFLOP/s FP16) processing `{python} TensorCoreMfuRecap.mfu_example_tokens_str` tokens per step (a batch of 4 sequences of 2,048 tokens) with a step time of 1 second: `{python} TensorCoreMfuRecap.mfu_example_math` $\approx 35\%$. A practical ceiling of 55-65 percent applies even in highly optimized implementations; the gap represents inescapable memory stalls ($\text{BW}$ bottleneck), AllReduce communication time $(T_{\text{comm}}(N))$, and pipeline bubbles.
...
class RackPowerTdpRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    rack_power_str = fmt((Hardware.Cloud.H100.tdp*GPUS_PER_HOST).m_as(kilowatt)*4+10.3, precision=1, commas=False)
```
...
For our `{python} InfraFrontierNodeTpRecap.frontier_params_b_str`B model with hidden dimension 12,288 and a microbatch of 4 sequences at 2,048 tokens, the activation tensor at each stage boundary is approximately $4 \times 2{,}048 \times 12{,}288 \times 2$ bytes (FP16) $\approx$ 200 MB. This is far smaller than the 350 GB gradient AllReduce required by data parallelism, which is why pipeline parallelism places less demanding requirements on the inter-node network.
...
class NodeCapacityDenseRecap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_hbm_cap_str = fmt((Hardware.Cloud.H100.memory.capacity * GPUS_PER_HOST).m_as(GiB), precision=0, commas=False)
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: InfraFrontierStragglerRecap7.frontier_params_b_str
class InfraFrontierStragglerRecap7:

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(Models.Language.GPT3.parameters.m_as(param) / BILLION, precision=0, commas=False)
```

The model weights in FP16 occupy 350 GB ($`{python} InfraFrontierNodeWeightsRecap.frontier_params_b_str` \times 10^9 \times 2$ bytes). The Adam optimizer maintains two FP32 state tensors per parameter (first moment $m$ and second moment $v$), adding $`{python} InfraFrontierStragglerRecap7.frontier_params_b_str` \times 10^9 \times 4 \times 2 = 1{,}400$ GB. The gradients in FP16 add another 350 GB. Intermediate activations (needed for the backward pass) depend on the batch size and sequence length but typically require 100--400 GB for practical training configurations. The total memory footprint for training is therefore 2.2--2.5 TB, roughly 6--7$\times$ the raw weight size.
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: H100TdpRackRecap.tdp_str
...
class GridQueue:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────
    gpu_lead_time_months = defaults.LEAD_TIME_GPU_MONTHS
    substation_lead_time_months = defaults.LEAD_TIME_SUBSTATION_MONTHS
    grid_queue_size_gw = defaults.GRID_INTERCONNECTION_QUEUE_US_GW
    # ┌── 2. EXECUTE ───────────────────────────────────────
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.defaults import defaults.PUE_LEGACY
from mlsysim.fmt import fmt, check
...
class PueSavings:
    ...
    it_power_mw = 7.0
    pue_air_simple = 1.5
    pue_air_industry = defaults.PUE_LEGACY
    pue_liquid = 1.10
...
class CommScalingIbRecap:
    ...
    ib_bw_str = fmt(Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(GB/second), precision=0, commas=False)
```
For data-parallel training of our `{python} InfraFrontierStragglerRecap13.frontier_params_b_str`B model, the communication cost per step is dominated by the AllReduce of 350 GB of gradients.
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: H100TdpTcoRecap.tdp_str
class H100TdpTcoRecap:
...
- **Annual Electricity**\index{Electricity!annual electricity}: simplified GPU-only estimate: 10,000 GPUs $\times$ `{python} H100TdpTcoRecap.tdp_str` W $\times$ PUE 1.1 $\times$ 8,760 h $\times$ \$0.07/kWh = `{python} TenKGpuClusterTco.annual_electricity_str`/year
...
from mlsysim.core.constants import *
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: A100FallaciesRecap.mem_bw_tbs_str
...
192 GB of HBM2e at `{python} A100FallaciesRecap.mem_bw_tbs_str` TB/s bandwidth, and (b) an accelerator with 80 GB of HBM3 at `{python} MemoryBandwidthFallaciesRecap.h100_bw_str` TB/s bandwidth. Option (a) has more than enough capacity (the model requires only 140 GB in FP16) but delivers tokens at 140 GB/`{python} A100FallaciesRecap.mem_bw_tbs_str` TB/s = 68.6 ms per token. Option (b) requires careful quantization to fit (140 GB in FP16 would not fit in 80 GB, but 70 GB in INT8 does) but delivers tokens at 70 GB/`{python} MemoryBandwidthFallaciesRecap.h100_bw_str` TB/s = 21 ms per token, which is 3.3$\times$ faster. For latency-sensitive serving, option (b) is the better choice despite its smaller capacity. The right accelerator is the one whose bandwidth and capacity *jointly* satisfy the workload's requirements.
...
class TcoSummaryBreakevenRecap:
    ...
    node_power_kw = (Hardware.Cloud.H100.tdp * GPUS_PER_HOST).m_as(kilowatt) * 1.2
