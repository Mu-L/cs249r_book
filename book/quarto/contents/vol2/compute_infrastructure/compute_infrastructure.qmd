...
For our `{python} InfraSetup.frontier_params_b_str`B model training across 128 nodes, the probability of at least one node experiencing a hardware issue during a two-week training run is substantial. If each node has an MTBF of 1,000 hours (a reasonable estimate for a DGX H100 under sustained load), the expected number of node failures during a 336-hour training run across 128 nodes is $128 \times 336/1{,}000 \approx 43$ failures. This means the training run will experience, on average, roughly three node failures per day. Each failure requires detecting the degraded node, draining its workload, substituting a spare node, and resuming from the last checkpoint -- a process that takes 10--30 minutes with automated tooling. With automated recovery, the direct restart overhead is roughly 2--6 percent of the run; without spare nodes and automated recovery, multi-hour interventions can push overhead into the 20--30 percent range, consuming millions of dollars in wasted GPU-hours.
...
::: {#fig-tco-build-vs-buy fig-env="figure" fig-pos="htb" fig-cap="**TCO: Build vs. Buy**: Cumulative cost over 30 months at 80 percent sustained utilization. The figure plots a 1,000-GPU H100 reference cluster (125 nodes at 8 GPUs/node) using \\$4/GPU-hr cloud pricing, with the on-prem CapEx step and OpEx slope drawn explicitly. Under the adjacent full-cluster assumptions (\\$58.75M upfront and about \\$1.5M/year OpEx), on-premises breaks even against cloud on-demand around month 27, and cloud grows linearly with no upfront cost." fig-alt="Cumulative cost vs. months plot for a 1,000-GPU reference cluster. The cloud line rises linearly to about 70 million dollars after 30 months. The on-premises line starts with a 58.75 million dollar CapEx jump and has a shallow OpEx slope. The lines cross near month 27."}
...
* **Memory wall diagnosis starts with roofline**: \index{Memory Wall!dominance} For single-request LLM inference, memory bandwidth determines latency because arithmetic completes in microseconds while data transfer takes milliseconds. The ridge point ($I_{\text{ridge}}$ = peak FLOPS/memory BW) separates memory-bound LLM decode ($I \approx 1$) from compute-bound training ($I > 2{,}000$).
