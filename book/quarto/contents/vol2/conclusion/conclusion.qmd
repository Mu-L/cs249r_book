...
class ConclusionScaleFacts:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    tail_servers_str = f"{tail_servers}"
    tail_percentile_str = f"{int(tail_percentile * 100)}"
    tail_hit_pct_str = f"{tail_hit_probability * 100:.0f}"
    llama_failures_str = f"{llama_failures}"
    llama_days_str = f"{llama_days}"
    llama_hours_per_failure_str = f"{llama_hours_per_failure:.0f}"
    straggler_speed_pct_str = f"{straggler_speed * 100:.0f}"
    straggler_drop_pct_str = f"{straggler_drop * 100:.0f}"
...
# │ Exports: machine_ops_sci, machine_power_mw_str, brain_ops_sci,
...
class FermiEstimate:
    ...
    machine_ops_sci = md_math(f"{_machine_coeff:.2f} \\times 10^{{{_machine_exp}}}")
    ...
    n_gpus_str = f"{n_gpus:,}"
    w_per_gpu_str = f"{int(w_per_gpu)}"
    tflops_per_gpu_str = f"{tflops_per_gpu:.0f}"
    power_calc_math = md_math(
...
*   **Ops/sec**\index{Ops/sec}: $25{,}000 \times \text{H100 FP16 tensor peak} \approx$ **`{python} FermiEstimate.machine_ops_sci` FLOPS** (rounded; H100 FP16 tensor peak is `{python} FermiEstimate.tflops_per_gpu_str` TFLOPS).
