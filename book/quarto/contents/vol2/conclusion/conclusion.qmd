...
[^fn-ring-allreduce]: **Ring AllReduce**: A collective that moves $2(N-1)M/N$ bytes per worker for message size $M$, giving a bandwidth term of $\frac{2(N-1)}{N}\frac{M}{\text{BW}}$ plus latency overhead. The coefficient approaches 2$\times$ message volume per worker as $N$ grows, while remaining bandwidth-optimal for large messages. Detailed in @sec-collective-communication.
...
*   **Ops/sec**\index{Ops/sec}: $25{,}000 \times \text{H100 FP16 tensor peak} \approx$ **`{python} FermiEstimate.machine_ops_math` FLOP/s** (rounded; H100 FP16 tensor peak is `{python} FermiEstimate.tflops_per_gpu_str` TFLOP/s).
