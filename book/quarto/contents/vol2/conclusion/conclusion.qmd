...
class ConclusionScaleFacts:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    tail_servers_str = fmt(tail_servers, precision=0, commas=False)
    ...
    llama_failures_str = fmt(llama_failures, precision=0, commas=False)
    llama_days_str = fmt(llama_days, precision=0, commas=False)
    llama_hours_per_failure_str = fmt(llama_hours_per_failure, precision=1, commas=False)
    straggler_speed_pct_str = fmt(straggler_speed * 100, precision=0, commas=False)
    straggler_drop_pct_str = fmt(straggler_drop * 100, precision=0, commas=False)
```
...
class FleetEvolution:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    fe_target_gain_str = fmt(target_gain, precision=0)
    fe_hw_gain_str = fmt(hw_gen_gain, precision=0)
    fe_algo_gain_str = fmt(algo_pruning_gain, precision=1)
    fe_orch_gain_str = fmt(orch_required, precision=0)
```
...
class PostSilicon:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ps_elec_pj_str = fmt(e_elec_pj, precision=0)
    ps_opt_pj_str = fmt(e_opt_pj, precision=0)
    ps_efficiency_gain_str = fmt(efficiency_gain, precision=0)
```
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt_int,  check, fmt, fmt_math
...
class FermiEstimate:
    ...
    throughput_ratio_str = fmt(throughput_ratio, precision=0)
    efficiency_gap_str = fmt(efficiency_gap, precision=1)
    n_gpus_str = fmt(n_gpus, precision=0)
    w_per_gpu_str = fmt_int(w_per_gpu)
    tflops_per_gpu_str = fmt(tflops_per_gpu, precision=0)
