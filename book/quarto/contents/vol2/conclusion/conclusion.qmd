...
class ConclusionScaleFacts:
    ...
    tail_percentile_str = fmt(int(tail_percentile * 100), precision=0, commas=False)
    tail_hit_pct_str = fmt(tail_hit_probability * 100, precision=0, commas=False)
    llama_failures_str = fmt(llama_failures, precision=0, commas=False)
    llama_days_str = fmt(llama_days, precision=0, commas=False)
    llama_hours_per_failure_str = fmt(llama_hours_per_failure, precision=0, commas=False)
...
from mlsysim.core.constants import (
    H100_FLOPS_FP16_TENSOR, TFLOPs, second, H100_TDP, watt, kilowatt, THOUSAND,
)
...
class FermiEstimate:
    ...
    efficiency_gap_str = fmt(efficiency_gap, precision=0)
    n_gpus_str = fmt(n_gpus, precision=0)
    w_per_gpu_str = fmt(int(w_per_gpu), precision=0)
