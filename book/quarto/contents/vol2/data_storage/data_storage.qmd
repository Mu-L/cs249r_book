...
from mlsysim.core.constants import (
    ...
    BYTES_FP16, BYTES_FP32, GB, TB, PB, second, byte, BILLION
)
...
class StorageHierarchyAnalysis:
    ...
    ckpt_fleet_total_pb_val = (ckpt_total_gb_val * 4320 * GB).m_as(PB)
...
For our 3 TB dataset distributed across 256 nodes, each node is responsible for approximately 12 GB of data. If a node fails, its replacement must stage that 12 GB from the PFS. At an uncontended PFS read speed of 4 GB/s, this staging takes 3 seconds; under recovery contention, an effective 400 MB/s per-node rate stretches the same copy to about 30 seconds. If the orchestrator can instead schedule the replacement workload on an idle node that already has the data cached from a previous run, staging time is zero and training resumes instantly. At scale, this optimization compounds: in a 10,000-node cluster experiencing 10 node failures per day, locality-aware scheduling can save roughly 30 seconds of staging time per failure, or 5 minutes per day of aggregate cluster idle time, in the contended case.
...
2. **Provenance Overhead**\index{Provenance Overhead}: `{python} int(SyntheticStorage.provenance_metadata_overhead * 100)` percent extra for lineage hashes, generation logs, and reward-model scores.
...
4. **The amplification**: Total footprint = 1 TB $\times$ `{python} 1 + SyntheticStorage.provenance_metadata_overhead` $\times$ `{python} SyntheticStorage.verification_passes_str` = **`{python} SyntheticStorage.total_tb_str` TB**.
