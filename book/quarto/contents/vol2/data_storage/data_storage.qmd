...
class StorageAccessPatternFacts:
    ...
    tokenized_text_tb_str = fmt(tokenized_text_tb_val, precision=0, commas=False)
    compressed_text_tb_str = fmt(compressed_text_tb_val, precision=0, commas=False)
    ckpt_total_gb_str = fmt_qty(ckpt_total, GB, precision=0)
...
|:-------------------------|:---------------------|---------------------------------------------------:|---------------------------:|-----------------:|
| **GPU HBM**              | 80 GB                | `{python} StorageHierarchyTable.h100_bw_tbs_str`/s |                    ~100 ns |           ~15.00 |
...
class NvmeTierContext:
    ...
    nvme_per_drive_bw = NVME_SEQUENTIAL_BW
    ...
    multimodal_images_tb_str = fmt(multimodal_images_tb, precision=0, commas=False)
    multimodal_text_tb_str = fmt(multimodal_text_tb, precision=0, commas=False)
    multimodal_audio_tb_str = fmt(multimodal_audio_tb, precision=0, commas=False)
...
**Problem**\index{ImageNet!bottleneck analysis}: A ResNet-50 training job on ImageNet (`{python} NVMeTierCalcs.imagenet_images_m_str`, ~`{python} NVMeTierCalcs.imagenet_avg_kb_str` average) targets `{python} NVMeTierCalcs.images_per_sec_str`/second. The question is whether to use individual JPEG files on an HDD or NVMe.

**Math**:

1.  **Raw Bandwidth**\index{ImageNet!raw bandwidth calculation}: `{python} NVMeTierCalcs.images_per_sec_str`/s $\times$ `{python} NVMeTierCalcs.img_size_kb_str`/image = **`{python} NVMeTierCalcs.raw_bw_str`/s**.
2.  **HDD Reality**: A 7200 RPM hard disk drive (HDD) delivers `{python} NVMeTierCalcs.hdd_iops_str` random IOPS\index{HDD!random IOPS bottleneck}. Sustaining `{python} NVMeTierCalcs.images_per_sec_str`/s requires **`{python} NVMeTierCalcs.hdd_slowdown_factor_str`$\times$ more IOPS** than the disk provides.
...
As a separate image-training example, consider 256 GPUs training with ImageNet-scale images, 200 ms iterations, and 80 percent target utilization. This configuration requires **`{python} PipelineCalcs.req_bw_imagenet_str`/s** of aggregate storage throughput. The bandwidth must be sustained continuously for the duration of training, which may last days or weeks. If at any point the storage system delivers less than `{python} PipelineCalcs.req_bw_imagenet_str`/s, accelerators begin to idle. For the chapter's 175B language-model running example, tokenized text has a much lower per-batch bandwidth demand, but the total data volume and checkpoint traffic over the run are much larger, shifting the bottleneck from peak training-data bandwidth to sustained throughput and checkpoint movement over weeks.
...
[^fn-hash-cost]: **Hash Cost (SHA-256)**: Verifying the integrity of a `{python} HashValidationCost.dataset_tb_str` dataset using SHA-256 consumes approximately `{python} HashValidationCost.core_hours_str` CPU-core hours under a pipeline-level throughput assumption of `{python} HashValidationCost.throughput_mb_s_str`/s per core. For a 30-day foundation model training run, this metadata validation step must be pipelined or performed during ingestion to avoid stalling the fleet's startup phase. \index{Cryptographic Hash!validation cost}
...
**Problem**: A training node with `{python} GDSLatency.n_gpus_str` loads `{python} GDSLatency.image_kb_str` at `{python} GDSLatency.images_per_s_per_gpu_str`/second per GPU (`{python} GDSLatency.gds_total_images_per_s_str`/second total). Compare the CPU load under traditional I/O vs. GDS.

**Traditional path**\index{Traditional I/O!data path}: Each image requires a DMA from NVMe to DRAM, a `memcpy` from kernel to user space, and a PCIe transfer to GPU. At `{python} GDSLatency.gds_total_images_per_s_str`/second with `{python} GDSLatency.gds_trad_us_str` μs of CPU time per image, the CPU spends `{python} GDSLatency.gds_trad_cpu_seconds_str` of CPU time per wall-clock second, consuming roughly `{python} GDSLatency.gds_trad_cpu_cores_str` cores worth of processing just for data movement.
...
1. **Per-node write to local NVMe** (`{python} CheckpointStormCalc.local_drive_count_str` drives at `{python} CheckpointStormCalc.local_drive_bw_gbs_str`/s each = `{python} CheckpointStormCalc.local_bw_gbs_str`/s): `{python} CheckpointStormCalc.per_node_shard_gb_str` $\div$ `{python} CheckpointStormCalc.local_bw_gbs_str`/s $\approx$ `{python} CheckpointStormCalc.local_write_s_str`.
2. **Async copy to PFS**\index{Checkpointing!async PFS copy}: `{python} CheckpointStormCalc.nodes_str`$\times$ `{python} CheckpointStormCalc.per_node_shard_gb_str` $\approx$ `{python} CheckpointStormCalc.total_pfs_tb_str` total. If the PFS provides `{python} CheckpointStormCalc.pfs_aggregate_bw_tbs_str`/s aggregate, the storm completes in roughly `{python} CheckpointStormCalc.pfs_copy_s_str`.
3. **Per-node PFS bandwidth**\index{Bandwidth!per-node pfs}: If all `{python} CheckpointStormCalc.nodes_str` write simultaneously, each gets `{python} CheckpointStormCalc.pfs_aggregate_bw_gbs_str`/`{python} CheckpointStormCalc.nodes_str` = `{python} CheckpointStormCalc.per_node_pfs_bw_gbs_str`/s.
...
2. If each node writes to local NVMe at `{python} CheckpointStorageDesignPrompt.local_nvme_bw_gbs_str`/s, what is $T_{\text{write}}$ for the local write?
3. If the parallel file system provides `{python} CheckpointStorageDesignPrompt.pfs_aggregate_tbs_str`/s aggregate and all `{python} CheckpointStorageDesignPrompt.nodes_str` write simultaneously, what is the per-node write bandwidth, and how long does the async copy take?
