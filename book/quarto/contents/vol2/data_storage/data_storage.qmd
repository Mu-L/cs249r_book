...
from mlsysim.fmt import MarkdownStr, fmt, fmt_sci, check
...
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ STORAGE-COMPUTE GAP, DATASET FOOTPRINT, AND CHECKPOINT SCALE
# ├─────────────────────────────────────────────────────────────────────────────
...
# │
# │ Imports: mlsysim.core.constants (H100_MEM_BW, NVME_SEQUENTIAL_BW, GPT3_PARAMS,
# │          BYTES_FP16, BYTES_FP32, GB, TB, second, byte, BILLION, TRILLION)
# │ Exports: StorageHierarchyAnalysis.h100_bw_tbs_str, StorageHierarchyAnalysis.nvme_bw_str,
# │          StorageHierarchyAnalysis.ckpt_total_gb_str, StorageHierarchyAnalysis.gpt3_params_b_str,
# │          StorageHierarchyAnalysis.bw_ratio_str, StorageHierarchyAnalysis.ckpt_fleet_total_pb_str,
# │          StorageHierarchyAnalysis.checkpoint_count_str,
# │          StorageHierarchyAnalysis.retention_recent_tb_str,
# │          StorageHierarchyAnalysis.retained_every_100_count_str,
# │          StorageHierarchyAnalysis.compressed_text_tb_str,
# │          StorageHierarchyAnalysis.tokenized_text_tb_str,
# │          StorageHierarchyAnalysis.baseline_training_tb_str,
# │          StorageHierarchyAnalysis.tokenized_shard_count_str,
# │          StorageHierarchyAnalysis.tokenized_shard_gb_str,
# │          StorageHierarchyAnalysis.node_tokenized_gb_str,
# │          StorageHierarchyAnalysis.node_stage_uncontended_s_str,
# │          StorageHierarchyAnalysis.node_stage_contended_s_str,
# │          StorageHierarchyAnalysis.tokenized_local_stream_min_str,
# │          StorageHierarchyAnalysis.host_dram_bw_gbs_str,
# │          StorageHierarchyAnalysis.hbm_host_dram_ratio_str,
# │          StorageHierarchyAnalysis.local_nvme_capacity_tb_str,
# │          StorageHierarchyAnalysis.multimodal_images_tb_str,
# │          StorageHierarchyAnalysis.multimodal_text_tb_str,
# │          StorageHierarchyAnalysis.multimodal_audio_tb_str,
# │          StorageHierarchyAnalysis.multimodal_total_tb_str,
# │          StorageHierarchyAnalysis.multimodal_excess_tb_str,
# │          StorageHierarchyAnalysis.archive_lineage_tb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    H100_MEM_BW, NVME_SEQUENTIAL_BW, GPT3_PARAMS,
    BYTES_FP16, BYTES_FP32, GB, TB, PB, second, byte, BILLION, TRILLION
)
...
class StorageHierarchyAnalysis:
    ...
    host_dram_bw_gbs_val = 200
    nvme_drive_count_val = 4
    nvme_drive_capacity_tb_val = 7.68
    multimodal_images_tb_val = 20
    multimodal_text_tb_val = 10
    multimodal_audio_tb_val = 5
    ...
    retention_recent_tb_val = 3 * ckpt_total_gb_val / 1000
    retained_every_100_count_val = checkpoint_count_val // 100
    archive_retained_tb_val = retained_every_100_count_val * ckpt_total_gb_val / 1000
    ...
    archive_lineage_tb_val = archive_retained_tb_val + baseline_training_tb_val
    tokenized_shard_count_val = round(tokenized_text_tb_val * 1000 / tokenized_shard_gb_val)
    node_tokenized_gb_val = tokenized_text_tb_val * 1000 / n_training_nodes
    ...
    tokenized_local_stream_min_val = (
        tokenized_text_tb_val * 1000 / (4 * nvme_bw_raw.m_as(GB/second)) / 60
    )
    local_nvme_capacity_tb_val = nvme_drive_count_val * nvme_drive_capacity_tb_val
    multimodal_total_tb_val = (
        multimodal_images_tb_val + multimodal_text_tb_val + multimodal_audio_tb_val
    )
    ...
    check(retained_every_100_count_val == 43, "Every-100th retention count should be 43")
    check(abs(tokenized_text_tb_val - 6) < 1e-9, "1.5T int32 tokens should be 6 TB")
    check(tokenized_shard_count_val == 2000, "6 TB at 3 GB/shard should need 2000 shards")
    check(16 < hbm_host_dram_ratio_val < 17, "HBM/host-DRAM bandwidth ratio should be about 17x")
    check(multimodal_total_tb_val > local_nvme_capacity_tb_val, "Multimodal example should exceed local NVMe")
    check(80 < archive_lineage_tb_val < 90, "Archive retained lineage should be about 84 TB")

    # ┌── 4. OUTPUT ───────────────────────────────────────
    h100_bw_tbs_str                = fmt(h100_bw.m_as(TB/second), precision=2, commas=False)
    nvme_bw_str                    = fmt(nvme_bw_raw.m_as(GB/second), precision=1, commas=False)
    gpt3_params_b_str              = fmt(gpt3_params/BILLION, precision=0, commas=False)
    ckpt_total_gb_str              = fmt(ckpt_total_gb_val, precision=0)
    bw_ratio_str                   = fmt(bw_ratio_val, precision=0)
    ckpt_fleet_total_pb_str        = fmt(ckpt_fleet_total_pb_val, precision=1)
    checkpoint_count_str           = fmt(checkpoint_count_val, precision=0)
    retention_recent_tb_str        = fmt(retention_recent_tb_val, precision=2, commas=False)
    retained_every_100_count_str   = fmt(retained_every_100_count_val, precision=0)
    compressed_text_tb_str         = fmt(compressed_text_tb_val, precision=0, commas=False)
    tokenized_text_tb_str          = fmt(tokenized_text_tb_val, precision=0, commas=False)
    baseline_training_tb_str       = fmt(baseline_training_tb_val, precision=0, commas=False)
    tokenized_shard_count_str      = fmt(tokenized_shard_count_val, precision=0)
    tokenized_shard_gb_str         = fmt(tokenized_shard_gb_val, precision=0, commas=False)
    node_tokenized_gb_str          = fmt(node_tokenized_gb_val, precision=0, commas=False)
    node_stage_uncontended_s_str   = fmt(node_stage_uncontended_s_val, precision=0, commas=False)
    node_stage_contended_s_str     = fmt(node_stage_contended_s_val, precision=0, commas=False)
    tokenized_local_stream_min_str = fmt(tokenized_local_stream_min_val, precision=1, commas=False)
    host_dram_bw_gbs_str           = fmt(host_dram_bw_gbs_val, precision=0, commas=False)
    hbm_host_dram_ratio_str        = fmt(hbm_host_dram_ratio_val, precision=0, commas=False)
    local_nvme_capacity_tb_str     = fmt(local_nvme_capacity_tb_val, precision=1, commas=False)
    multimodal_images_tb_str       = fmt(multimodal_images_tb_val, precision=0, commas=False)
    multimodal_text_tb_str         = fmt(multimodal_text_tb_val, precision=0, commas=False)
    multimodal_audio_tb_str        = fmt(multimodal_audio_tb_val, precision=0, commas=False)
    multimodal_total_tb_str        = fmt(multimodal_total_tb_val, precision=0, commas=False)
    multimodal_excess_tb_str       = fmt(multimodal_excess_tb_val, precision=1, commas=False)
    archive_lineage_tb_str         = fmt(archive_lineage_tb_val, precision=0, commas=False)
```
...
Consider the running example that will thread through this chapter. A 175-billion parameter language model trains on 1.5 trillion tokens of text: roughly `{python} StorageHierarchyAnalysis.compressed_text_tb_str` TB in compressed source form, or `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB once represented as 4-byte token IDs. Each training epoch reads every token once, in a shuffled order determined by the random seed. There is no "hot" subset of data that dominates access; every byte is consumed exactly once per pass. Meanwhile, each accelerator processes its local batch in roughly 200 ms, then waits for the next. If storage cannot deliver data within that 200 ms window, the accelerator sits idle, and the organization pays for silicon that produces heat instead of gradients.

The problem is deceptive because storage technology has improved substantially. NVMe drives achieve `{python} StorageHierarchyAnalysis.nvme_bw_str` GB/s of sequential throughput, a figure that would have seemed out of reach a decade ago. The accelerators, however, improved faster. An H100 GPU consumes data from its HBM at `{python} StorageHierarchyAnalysis.h100_bw_tbs_str` TB/s, roughly `{python} StorageHierarchyAnalysis.bw_ratio_str`$\times$ faster than a single NVMe drive can feed it. The gap between storage delivery and accelerator consumption is the central tension of this chapter, and it cannot be solved by any single technology. Instead, it requires a hierarchy of storage tiers, each carefully matched to a specific phase of the ML lifecycle, connected by pipelines that hide latency through prefetching and pipelining.
...
The canonical training-data footprint for this example is roughly `{python} StorageHierarchyAnalysis.baseline_training_tb_str` TB across the hierarchy: `{python} StorageHierarchyAnalysis.compressed_text_tb_str` TB for the compressed source corpus plus `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB for the tokenized training shards. Additional shuffled or packed variants can raise the staging footprint, but the per-epoch training read over 4-byte token IDs is `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB. The model generates roughly 1.75 TB checkpoints (`{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB total: 350 GB of weights plus 1.4 TB of Adam optimizer state) every 10 minutes. Over a 30-day training run on 256 nodes, the storage system must deliver `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB of tokenized training data per epoch, absorb `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb_str` PB of checkpoint writes, and stage model weights for evaluation runs. These numbers thread through every section of this chapter, grounding abstract principles in concrete engineering constraints.
...
# │ How: P(at least one collision) = 1 - (Shards! / (Shards^Workers * (Shards-Workers)!)).
# │
...
class ShardContention:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    # Classic birthday problem: P(no collision)
    # For small n/N, approx e^(-n^2 / 2N)
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    sc_n_workers_str = fmt(n_workers, precision=0, commas=False)
    sc_n_shards_str = fmt(n_shards, precision=0, commas=False)
    sc_collision_prob_str = fmt(p_collision * 100, precision=1, commas=False)
...
1. **Probability of No Collision**\index{Probability of No Collision}: $\approx e^{-n^2/2N} = e^{-32^2/2000} \approx 0.60$.
2. **Probability of Contention**\index{Probability of Contention}: $1 - 0.60 = \mathbf{40\%}$.

**Systems insight**: Even with a large number of shards, there is a **`{python} ShardContention.sc_collision_prob_str` percent chance** that the storage system will experience a "hot spot" in any given epoch. In a distributed fleet, these collisions create **tail latency**\index{Tail Latency}: the entire cluster waits for the two GPUs sharing a disk to finish. To solve this, production data loaders use **global shuffling** and **deterministic shard assignment** to ensure that workers are perfectly distributed across the storage fabric, eliminating the "thundering herd" effect.
...
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Dual-axis plot: GPU TFLOPS (236×) vs NVMe GB/s (4×) 2016–2024;
import numpy as np
...
| **Locality**\index{Locality} | Strong temporal locality   | No locality (uniform sampling) |

: **ML Workloads Invert Traditional Storage Assumptions**\index{ML Workloads Invert Traditional Storage Assumptions}: Where databases optimize for random IOPS with cacheable working sets, ML training streams sequentially through datasets that exceed all cache levels. {#tbl-storage-assumptions}
...
The practical consequence for our running example is stark. The 1.5 trillion tokens of training data produce roughly `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB of sequential token-ID reads per epoch, even though the compressed source corpus is only `{python} StorageHierarchyAnalysis.compressed_text_tb_str` TB. If each token were stored as an individual file (as naive data collection might produce), the metadata overhead alone would throttle throughput to a fraction of what the storage hardware can deliver. Instead, the data must be preprocessed into large shards, typically 256 MB to 4 GB each, so that each read operation amortizes the fixed overhead of file open, seek, and close across millions of tokens. This preprocessing step transforms the access pattern from random [@zaharia2012resilient] (one file per sample) to sequential (one contiguous read per shard), keeping the workload below the storage ceiling shown in @fig-access-patterns-vol2.
...
|:-------------------------|:---------------------|---------------------------------------------------------:|---------------------------:|-----------------:|
| **GPU HBM**              | 80 GB                | `{python} StorageHierarchyAnalysis.h100_bw_tbs_str` TB/s |                    ~100 ns |           ~15.00 |
...
: **Extended Memory Hierarchy for ML Systems**\index{Extended Memory Hierarchy for ML Systems}: The roughly 30$\times$ aggregate-to-aggregate bandwidth gap between HBM and object storage (and a much larger per-client gap once a single inference instance pulls from a shared object endpoint) drives the need for sophisticated prefetching and caching across multiple levels. {#tbl-storage-hierarchy-merged}
...
class TextImageBandwidth:
    ...
    text_bw_mb_s = text_bw_bytes_s / 1_000_000
    image_bw_gb_s = image_bw_bytes_s / 1_000_000_000
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    text_bw_mb_str = fmt(text_bw_mb_s, precision=0, commas=False)
    image_bw_gb_str = fmt(image_bw_gb_s, precision=0, commas=False)
...
For **text training**\index{Training!text}, the demand is surprisingly low. With a typical batch size of 4,096 tokens per GPU and a 200 ms step time, the aggregate bandwidth is:

`{python} TextImageBandwidth.text_bw_display_math`

This is easily served by a single network-attached storage node.

For **image training**\index{Training!image}, the picture changes dramatically. Using a common batch size of 256 images (ImageNet at $224{\times}224$, roughly 150 KB/image), the aggregate bandwidth explodes:
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import BYTES_FP16, BYTES_FP32, H100_MEM_CAPACITY, GB, byte
from mlsysim.fmt import MarkdownStr, fmt, check
...
class HbmMemoryBudget:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    params = StorageHierarchyAnalysis.gpt3_params
    tp_degree = 8
    dp_nodes = 256
    ...
    comm_min_gb = 2
    comm_max_gb = 4
    hbm_capacity_gb = H100_MEM_CAPACITY.m_as(GB)
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    weight_per_gpu_str = fmt(weight_per_gpu_gb, precision=2, commas=False)
    optimizer_per_gpu_str = fmt(optimizer_per_gpu_gb, precision=1, commas=False)
    gradient_per_gpu_str = fmt(gradient_per_gpu_gb, precision=1, commas=False)
    activation_range_str = MarkdownStr(f"{activation_min_gb}--{activation_max_gb}")
    comm_range_str = MarkdownStr(f"{comm_min_gb}--{comm_max_gb}")
    total_range_str = MarkdownStr(f"{fmt(total_min_gb, precision=0, commas=False)}--{fmt(total_max_gb, precision=0, commas=False)}")
    total_pct_range_str = MarkdownStr(f"{fmt(total_min_pct, precision=0, commas=False)} to {fmt(total_max_pct, precision=0, commas=False)}")
```

| **Component**                               |                                    **Size per GPU** |
|:--------------------------------------------|----------------------------------------------------:|
| Model Weights (FP16, tensor-parallel)       |    `{python} HbmMemoryBudget.weight_per_gpu_str` GB |
| Optimizer State (ZeRO-3 partitioned)        | `{python} HbmMemoryBudget.optimizer_per_gpu_str` GB |
| Activations (variable by sequence length)   |  `{python} HbmMemoryBudget.activation_range_str` GB |
| Gradient Buffers (FP16, ZeRO-3 partitioned) |  `{python} HbmMemoryBudget.gradient_per_gpu_str` GB |
| Communication Buffers (NCCL)                |        `{python} HbmMemoryBudget.comm_range_str` GB |
| **Total Occupied**                          |   **`{python} HbmMemoryBudget.total_range_str` GB** |
...
Even with aggressive partitioning, the total memory footprint consumes `{python} HbmMemoryBudget.total_pct_range_str` percent of the available HBM under these assumptions. This leaves a limited buffer for the incoming data pipeline, especially once fragmentation and framework workspaces are included. Any delay in fetching the next batch from host memory risks starving the accelerator, forcing it to sit idle while the most expensive resource in the system produces heat instead of gradients.
...
# │ How:     Read PCIE_GEN5_BW and NVLINK_H100_BW from constants; extract GB/s.
# │
# │ Imports: mlsysim.core.constants (PCIE_GEN5_BW, NVLINK_H100_BW, GB, second);
# │          mlsysim.fmt (fmt)
...
# │          shard_GB/drive_BW.
# │
# │ Imports: mlsysim.core.constants (NVME_SEQUENTIAL_BW, GB, second, BILLION, THOUSAND,
# │          MILLION)
# │ Exports: NVMeTierCalcs.raw_bw_str, .hdd_slowdown_factor_str,
# │          .hdd_util_ceiling_pct_str, .ckpt_nvme_s_str, .ckpt_pfs_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import NVME_SEQUENTIAL_BW, GB, second, BILLION, THOUSAND, MILLION
from mlsysim.fmt import check, fmt
...
class NVMeTierCalcs:
    ...
    # Checkpoint sizing (from StorageHierarchyAnalysis)
    ckpt_total_gb_val = StorageHierarchyAnalysis.ckpt_total_gb_val
    n_nodes           = 256
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    raw_bw_str             = fmt(raw_bw_val, precision=0, commas=False)
    hdd_slowdown_factor_str = fmt(hdd_slowdown_val, precision=0, commas=False)
    hdd_util_ceiling_pct_str = fmt(hdd_util_ceiling_pct_val, precision=0, commas=False)
    ckpt_nvme_s_str        = fmt(ckpt_nvme_s_val, precision=1, commas=False)
    ckpt_pfs_s_str         = fmt(ckpt_pfs_s_val, precision=1, commas=False)
```

### Tier 2: Local NVMe {#sec-storage-tier2-nvme}

\index{NVMe!ML storage tier}When the working set exceeds host DRAM, the system falls to Tier 2: the local NVMe drives attached directly to the compute node. NVMe[^fn-nvme-protocol] provides a high-performance protocol designed specifically for solid-state drives, achieving `{python} StorageHierarchyAnalysis.nvme_bw_str` GB/s of sequential throughput per drive. With four drives in a RAID-0 configuration, a single node can sustain roughly 28 GB/s of sequential reads before overhead, sufficient to stream the `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB tokenized dataset from local disk in about `{python} StorageHierarchyAnalysis.tokenized_local_stream_min_str` minutes under ideal sequential conditions.
...
The warm cache pattern requires careful capacity planning. A training node with four 7.68 TB NVMe drives provides approximately `{python} StorageHierarchyAnalysis.local_nvme_capacity_tb_str` TB of local storage. For our running example, the `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB tokenized dataset fits comfortably on a single node's local storage, with room for checkpoint staging and temporary augmentation buffers. A multi-modal training job combining `{python} StorageHierarchyAnalysis.multimodal_images_tb_str` TB of images, `{python} StorageHierarchyAnalysis.multimodal_text_tb_str` TB of text, and `{python} StorageHierarchyAnalysis.multimodal_audio_tb_str` TB of audio totals `{python} StorageHierarchyAnalysis.multimodal_total_tb_str` TB, exceeding local capacity by about `{python} StorageHierarchyAnalysis.multimodal_excess_tb_str` TB and forcing the pipeline to stream from the parallel file system for at least part of the dataset. The design trade-off is between provisioning more local NVMe (which increases node cost) and accepting network-dependent reads (which risks latency spikes).
...
For our running example, the 1.5 trillion token dataset is typically stored as a collection of 256 MB to 4 GB binary shards, each containing a contiguous sequence of tokenized text. The data loader opens a shard, reads it sequentially into a buffer, and iterates over tokens within the buffer. When the buffer is exhausted, the loader opens the next shard. The total number of `open()` calls per epoch is the number of shards (roughly `{python} StorageHierarchyAnalysis.tokenized_shard_count_str` if the `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB tokenized corpus is split into `{python} StorageHierarchyAnalysis.tokenized_shard_gb_str` GB shards), not the number of tokens (trillions). This 10,000$\times$ to 100,000$\times$ reduction in metadata operations is what makes streaming from both local NVMe and remote storage feasible at training scale.
...
1.  **Raw Bandwidth**\index{Bandwidth!raw}: 1,000 images/s$\times$ 150 KB/image = **`{python} NVMeTierCalcs.raw_bw_str` MB/s**.
2.  **HDD Reality**\index{HDD Reality}: A 7200 RPM hard disk drive (HDD) delivers 100 random IOPS. Sustaining 1,000 images/s requires **`{python} NVMeTierCalcs.hdd_slowdown_factor_str`$\times$ more IOPS** than the disk provides.
3.  **Result**: Shuffling individual files on an HDD will starve the GPU, capping utilization at about `{python} NVMeTierCalcs.hdd_util_ceiling_pct_str` percent before decode, seek, and file-system overheads.
...
class LocalCacheRoi:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    lc_compute_ms_str = fmt(t_compute_ms, precision=0, commas=False)
    lc_remote_io_ms_str = fmt(t_io_remote_ms, precision=0, commas=False)
    lc_util_remote_str = fmt(util_remote, precision=1, commas=False)
    lc_util_local_str = fmt(util_local, precision=1, commas=False)
...
**Problem**: A vision-model training pipeline runs each step in `{python} LocalCacheRoi.lc_compute_ms_str` ms. Fetching data from a shared Parallel File System adds `{python} LocalCacheRoi.lc_remote_io_ms_str` ms of I/O wait because of network congestion. How much does adding local NVMe SSDs to each node improve GPU utilization?

**Math**:
GPU utilization $(\eta_{\text{hw}})$ is the fraction of step time spent in computation.

1. **Remote Only**\index{Remote Only}: `{python} LocalCacheRoi.lc_compute_ms_str` / (`{python} LocalCacheRoi.lc_compute_ms_str` + `{python} LocalCacheRoi.lc_remote_io_ms_str`) $\approx$ **`{python} LocalCacheRoi.lc_util_remote_str` percent**.
2. **Local Cache**\index{Local Cache}: Using prefetching into local NVMe reduces the exposed I/O wait to near zero.
   - New Util: `{python} LocalCacheRoi.lc_compute_ms_str` / (`{python} LocalCacheRoi.lc_compute_ms_str` + 10) $\approx$ **`{python} LocalCacheRoi.lc_util_local_str` percent**.
...
Parallel file systems provide the **strong consistency** (linearizability) required for reliable checkpoints [@corbett2013]. When a checkpoint save completes, every node that subsequently reads that checkpoint sees the exact same consistent state. This guarantee is essential for fault recovery: if a node fails and restarts, it must read the checkpoint that the surviving nodes wrote, not a partially flushed version. Achieving this consistency at scale requires careful coordination between the MDS lock manager and the distributed OSS write paths, which is one reason that checkpoint writes are more expensive than training reads.
...
Applying this streaming architecture to our running example makes the design concrete. The tokenized training set is stored in S3 as roughly `{python} StorageHierarchyAnalysis.tokenized_shard_count_str` shards of `{python} StorageHierarchyAnalysis.tokenized_shard_gb_str` GB each. The streaming architecture assigns each of the 256 compute nodes a unique subset of roughly 8 shards. Within each node, 8 data loader worker processes issue concurrent HTTP `Range` requests to fetch 256 MB chunks from their assigned shards. Across the cluster, this results in 2,048 concurrent requests, allowing the aggregate throughput from S3 to reach 50–100 GB/s, easily saturating the roughly `{python} TextImageBandwidth.text_bw_mb_str` MB/s required for text model training. In this regime, the bottleneck often shifts from the object store's bandwidth to the network bandwidth between the compute VPC and the S3 endpoint. Organizations that co-locate compute and storage in the same cloud availability zone can reduce this cross-network latency from a typical 20 ms to roughly 5 ms, a 4$\times$ improvement that directly reduces the required prefetch buffer depth and improves overall pipeline efficiency.
...
For our 175B model training run, the archive tier holds the complete training lineage: the raw `{python} StorageHierarchyAnalysis.compressed_text_tb_str` TB compressed source corpus, the `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB tokenized shard set, all preprocessing scripts, the final checkpoint, and a sampled subset of intermediate checkpoints. If the organization budgets a conservative 100 TB for this retained lineage, the two-year archive cost is roughly \$9,600 (100 TB at \$0.004/GB/month $\times$ 24 months), well under one day of GPU time on the training cluster. The asymmetry between archive cost and compute cost makes retention economically trivial; the only reason organizations fail to archive is neglect, not expense.
...
The second family, **columnar formats**\index{Columnar Format} like Apache Parquet and Apache Arrow, was designed for analytics workloads [@melnik2010]. They organize data by column rather than by row, which allows for excellent compression as data within a column is typically of the same type and has lower entropy. This structure also enables **predicate pushdown**\index{Parquet}\index{Predicate Pushdown}, where the storage engine reads only the specific columns (features) required by a query, avoiding unnecessary I/O. These formats are well-suited for tabular ML tasks and feature engineering but are less natural for unstructured data like images or audio, which are treated as atomic blobs.
...
: **Data Format Comparison for ML Training**\index{Data!format comparison for ml training}: Per-sample overhead determines whether the storage hardware's bandwidth is realized or wasted on metadata operations. {#tbl-data-formats}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import BILLION, THOUSAND, ms, second
...
class PipelineCalcs:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    req_bw_imagenet_str = fmt(req_bw_imagenet_gbs, precision=1, commas=False)
...
$$\text{BW}_{\text{required}} = N_{\text{GPUs}} \times U_{\text{target}} \times \frac{D_{\text{vol,batch}}}{T_{\text{iteration}}}$$ {#eq-pipeline-throughput}

where $N_{\text{GPUs}}$ is the number of accelerators, $U_{\text{target}}$ is the target utilization (typically 0.8 to 0.95), $D_{\text{vol,batch}}$ is the size of one local batch in bytes, and $T_{\text{iteration}}$ is the time for one forward-backward pass.

The equation reveals four levers for controlling storage demand. Reducing $N_{\text{GPUs}}$ (fewer accelerators) directly reduces bandwidth requirements but also reduces training throughput. Lowering $U_{\text{target}}$ (accepting lower utilization) reduces the bandwidth needed but wastes expensive hardware. Decreasing $D_{\text{vol,batch}}$ (smaller batches) reduces per-iteration data volume but may harm model convergence. Increasing $T_{\text{iteration}}$ (slower iterations, perhaps from a larger model) gives storage more time to deliver each batch but extends total training time. In practice, none of these levers is free; the pipeline must be engineered to deliver the bandwidth that the training configuration demands.

As a separate image-training example, consider 256 GPUs training with ImageNet-scale images, 200 ms iterations, and 80 percent target utilization. This configuration requires **`{python} PipelineCalcs.req_bw_imagenet_str` GB/s** of aggregate storage throughput. The bandwidth must be sustained continuously for the duration of training, which may last days or weeks. If at any point the storage system delivers less than `{python} PipelineCalcs.req_bw_imagenet_str` GB/s, accelerators begin to idle. For the chapter's 175B language-model running example, tokenized text has a much lower per-batch bandwidth demand, but the total data volume and checkpoint traffic over the run are much larger, shifting the bottleneck from peak training-data bandwidth to sustained throughput and checkpoint movement over weeks.

The consequences of falling short are captured by the **data stall ratio**, defined in @eq-data-stall as the fraction of each training step where the accelerator waits for data:
...
**Systems insight**: Even with pipelining, the accelerator is idle `{python} PipelineCalcs.data_stall_pct_str` percent of the time. To eliminate the stall entirely, the next batch must arrive within the 200 ms compute window, either by increasing effective storage bandwidth, hiding tail latency with deeper prefetch buffers, or using more parallel I/O streams.
...
**The consequence**: Across thousands of workers, per-file metadata checks can generate millions of metadata requests per minute, overwhelming the parallel file system's metadata server. GPUs then stall on file-open operations, a delay invisible to standard I/O bandwidth counters.

**The systems lesson**: Precompute a file manifest at job start and have loaders read it once, eliminating per-file metadata checks on the hot path. Storage bottlenecks are often metadata bottlenecks, not byte-throughput bottlenecks.

:::

### Pipelining and prefetching {#sec-storage-pipelining}

The primary weapon against data stalls is **pipelining**\index{Pipelining!data loading}: the CPU prepares batch $N+1$ while the GPU processes batch $N$. @fig-data-pipeline-vol2 illustrates this overlap. When I/O time is less than compute time, pipelining hides the storage latency entirely, and the accelerator never stalls.
...
A single slow I/O can create a "bubble" in the pipeline that propagates forward, stalling the accelerator. If the CPU was preparing batch $N+1$ and the read took twice as long as expected, batch $N+1$ is not ready when the GPU finishes batch $N$. The GPU idles until the read completes, and the pipeline falls one batch behind. If subsequent reads are also slow, the pipeline never recovers. The defense is a prefetch buffer: rather than preparing just one batch ahead, the CPU maintains a queue of $D_{\text{prefetch}}$ preloaded batches.

The minimum buffer depth $D_{\text{prefetch}}$ must absorb the worst-case I/O latency without the queue draining:

$$D_{\text{prefetch,min}} = \left\lceil \frac{T_{\text{I/O,p99}}}{T_{\text{compute}}} \right\rceil$$ {#eq-prefetch-depth}
...
For our `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB tokenized dataset distributed across 256 nodes, each node is responsible for approximately `{python} StorageHierarchyAnalysis.node_tokenized_gb_str` GB of data. If a node fails, its replacement must stage that shard subset from the PFS. At an uncontended PFS read speed of 4 GB/s, this staging takes about `{python} StorageHierarchyAnalysis.node_stage_uncontended_s_str` seconds; under recovery contention, an effective 400 MB/s per-node rate stretches the same copy to about `{python} StorageHierarchyAnalysis.node_stage_contended_s_str` seconds. If the orchestrator can instead schedule the replacement workload on an idle node that already has the data cached from a previous run, staging time is zero and training resumes instantly. At scale, this optimization compounds: in a 10,000-node cluster experiencing 10 node failures per day, locality-aware scheduling can save roughly a minute of staging time per failure, or about 10 minutes per day of aggregate cluster idle time, in the contended case.
...
class HashValidationCost:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    dataset_mb = dataset_tb * 1_000_000
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    dataset_tb_str = fmt(dataset_tb, precision=0, commas=False)
    throughput_mb_s_str = fmt(throughput_mb_s_per_core, precision=1, commas=False)
    core_hours_str = fmt(core_hours, precision=0)
```
...
[^fn-hash-cost]: **Hash Cost (SHA-256)**: Verifying the integrity of a `{python} HashValidationCost.dataset_tb_str` TB dataset using SHA-256 consumes approximately `{python} HashValidationCost.core_hours_str` CPU-core hours under a pipeline-level throughput assumption of `{python} HashValidationCost.throughput_mb_s_str` MB/s per core. For a 30-day foundation model training run, this metadata validation step must be pipelined or performed during ingestion to avoid stalling the fleet's startup phase. \index{Cryptographic Hash!validation cost}
...
class GDSLatency:
    ...
    gds_trad_us   = 120
    gds_bypass_us = 30
    total_images_per_s = 64_000

    # ┌── 2. EXECUTE ──────────────────────────────────────
    gds_speedup_val = gds_trad_us/gds_bypass_us
    gds_latency_reduction_pct_val = (1 - gds_bypass_us/gds_trad_us) * 100
    gds_trad_cpu_seconds_val = total_images_per_s * gds_trad_us / 1_000_000
    gds_bypass_cpu_seconds_val = total_images_per_s * gds_bypass_us / 1_000_000
    ...
    gds_bypass_us_str = fmt(gds_bypass_us, precision=0, commas=False)
    gds_speedup_str   = fmt(gds_speedup_val, precision=0, commas=False)
    gds_latency_reduction_pct_str = fmt(gds_latency_reduction_pct_val, precision=0, commas=False)
    gds_total_images_per_s_str = fmt(total_images_per_s, precision=0)
    gds_trad_cpu_seconds_str = fmt(gds_trad_cpu_seconds_val, precision=2, commas=False)
    gds_trad_cpu_cores_str = fmt(round(gds_trad_cpu_seconds_val), precision=0, commas=False)
    gds_bypass_cpu_seconds_str = fmt(gds_bypass_cpu_seconds_val, precision=2, commas=False)
    gds_freed_cpu_cores_str = fmt(round(gds_freed_cpu_cores_val), precision=0, commas=False)
```
...
**Problem**\index{GPU Direct Storage!CPU bypass}: A training node with 8 GPUs loads 150 KB images at 8,000 images/second per GPU (`{python} GDSLatency.gds_total_images_per_s_str` images/second total). Compare the CPU load under traditional I/O vs. GDS.

**Traditional path**\index{Traditional Path}: Each image requires a DMA from NVMe to DRAM, a `memcpy` from kernel to user space, and a PCIe transfer to GPU. At `{python} GDSLatency.gds_total_images_per_s_str` images/second with `{python} GDSLatency.gds_trad_us_str` μs of CPU time per image, the CPU spends `{python} GDSLatency.gds_trad_cpu_seconds_str` seconds of CPU time per wall-clock second, consuming roughly `{python} GDSLatency.gds_trad_cpu_cores_str` cores worth of processing just for data movement.

**GDS path**\index{GDS Path}: Each image is DMA'd directly from NVMe to GPU. At `{python} GDSLatency.gds_bypass_us_str` μs of CPU time per image (for initiating the DMA), the CPU spends `{python} GDSLatency.gds_bypass_cpu_seconds_str` seconds of CPU time per wall-clock second, freeing roughly `{python} GDSLatency.gds_freed_cpu_cores_str` cores for data augmentation.
...
The GDS design principle also extends to checkpoint writes. In the traditional path, checkpoint data flows from GPU HBM through host DRAM (via PCIe) to NVMe (via CPU-mediated write). With GDS, the checkpoint can be written directly from GPU HBM to NVMe, eliminating the host DRAM copy. For a `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB checkpoint, this removes one full data copy and reduces the CPU's involvement in the write path, lowering $T_{\text{write}}$ further.

### The complete data path {#sec-storage-complete-path}

Combining GDS with the multi-tier hierarchy, we can now trace the complete data path for our running example. At the beginning of the training job, the `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB tokenized shard set, derived from the `{python} StorageHierarchyAnalysis.compressed_text_tb_str` TB compressed source corpus, is staged from object storage into the cluster's local NVMe caches, with each node receiving its assigned shard subset during setup. The data loader workers read tokenized shards from NVMe, assemble packed sequences in pinned host DRAM, and transfer those batches to the accelerator. With GDS enabled, token IDs that are already in a GPU-consumable layout can be loaded directly into GPU memory without CPU involvement; compressed formats still require CPU or GPU decompression before use.
...
The volume of data moved during a 30-day training run reveals a counterintuitive reality. A single staging copy of the `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB tokenized dataset from object storage to local NVMe accounts for a modest transfer. The checkpointing process, however, generates a vastly larger data stream. With a roughly `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB checkpoint created every 10 minutes across 256 nodes for 30 days, the system produces approximately `{python} StorageHierarchyAnalysis.checkpoint_count_str` checkpoints, totaling over `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb_str` petabytes of state that must traverse the storage hierarchy. Checkpoint data movement dwarfs training data movement for large models: while the `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB training dataset might be read a handful of times (once per epoch), the `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb_str` PB of checkpoint data is generated anew, making checkpoint I/O the dominant storage workload. This insight explains why checkpoint staging strategy (write locally, replicate asynchronously) has a larger impact on overall storage design than training data pipeline optimization for frontier-scale models.
...
# │
# │ Imports: (none—uses literal cost constants)
# │ Exports: EconRatios.tier_cost_ratio_str,
# │          EconRatios.egress_100tb_str
# └─────────────────────────────────────────────────────────────────────────────
class EconRatios:
    # ┌── 1. LOAD ─────────────────────────────────────────
    cost_nvme_gb_mo = 0.10
...
class BuildVsBuyStorageEconomics:
    ...
    cloud_egress_gb = 0.09
    dataset_pb = 1
    reads_per_year = 10

    # ┌── 2. EXECUTE ──────────────────────────────────────
    dataset_gb = dataset_pb * 1_000_000
    ...
    breakeven_low_years_str = fmt(breakeven_low_years, precision=1, commas=False)
    breakeven_high_years_str = fmt(breakeven_high_years, precision=1, commas=False)
...
- **Object storage (S3 Standard)**: \$`{python} ObjectStorageCost.s3_annual_str`/year
- **Local NVMe (provisioned)**: \$`{python} ObjectStorageCost.nvme_annual_str`/year
- **Archive (Glacier)**: \$`{python} GlacierCost.glacier_annual_str`/year
...
Among those hidden costs, egress is often the term that flips the architecture decision because it scales with every epoch, not just with stored capacity.
...
A `{python} StorageHierarchyAnalysis.gpt3_params_b_str`B parameter model with Adam optimizer\index{Adam Optimizer!state size} generates checkpoints of approximately `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB. The checkpoint includes model weights (`350` GB in FP16), optimizer state (momentum and variance, `1,400` GB in FP32), learning rate scheduler state, random number generator state, and the current data loader position. Every GPU in the cluster saves its shard of the checkpoint simultaneously, creating a checkpoint storm that the storage system must absorb without disrupting ongoing training reads.
...
class CheckpointStormDefinition:
    ...
    zero3_total_tb = zero3_total_gb / 1000
    zero3_write_s = zero3_total_gb / fabric_bw_gbs
    naive_total_tb = weights_gb * nodes / 1000
    naive_write_s = naive_total_tb * 1000 / fabric_bw_gbs
    ...
    check(1390 < naive_write_s < 1410, "Naive 140 TB write at 100 GB/s should be about 1,400 s")
    ...
    master_weights_gb_str = fmt(master_weights_gb, precision=0, commas=False)
    adam_moments_gb_str = fmt(adam_moments_gb, precision=0, commas=False)
    optimizer_state_gb_str = fmt(optimizer_state_gb, precision=0, commas=False)
    zero3_total_tb_str = fmt(zero3_total_tb, precision=2, commas=False)
    zero3_write_s_str = fmt(zero3_write_s, precision=1, commas=False)
    naive_total_tb_str = fmt(naive_total_tb, precision=0, commas=False)
    naive_write_s_str = fmt(naive_write_s, precision=0)
    naive_write_min_str = fmt(naive_write_min, precision=0, commas=False)
```
...
1.  **Significance (quantitative)**: The storm magnitude scales as $T_{\text{write}} = N \times \text{per-node-shard} / \text{BW}_{\text{fabric}}$. For a `{python} CheckpointStormDefinition.params_b_str`B-parameter model in FP16 (`{python} CheckpointStormDefinition.weights_gb_str` GB of weights) trained across 1,000 nodes with vanilla data parallelism (every node holding its own complete copy), a naive checkpoint generates `{python} CheckpointStormDefinition.naive_total_tb_str` TB of simultaneous writes; at 100 GB/s fabric bandwidth, $T_{\text{write}} \approx$ `{python} CheckpointStormDefinition.naive_write_s_str` seconds, over `{python} CheckpointStormDefinition.naive_write_min_str` minutes of training stall per checkpoint event. ZeRO-3 sharding collapses this to roughly `{python} CheckpointStormDefinition.zero3_total_tb_str` TB total (`{python} CheckpointStormDefinition.weights_gb_str` GB weights + `{python} CheckpointStormDefinition.gradients_gb_str` GB gradients + `{python} CheckpointStormDefinition.master_weights_gb_str` GB FP32 master weights + `{python} CheckpointStormDefinition.adam_moments_gb_str` GB Adam moments, split across 1,000 nodes), bringing $T_{\text{write}}$ down to roughly `{python} CheckpointStormDefinition.zero3_write_s_str` seconds. This stall adds directly to the $L_{\text{lat}}$ term and can dwarf the compute time between checkpoints when the sharding optimization is omitted.
...
A concrete 175B example shows why the local-write phase, rather than the durable copy, governs the exposed training pause.
...
class CheckpointStormCalc:
    ...
    nodes = 256
    local_drive_count = 4
    local_drive_bw_gbs = 7
    pfs_aggregate_bw_gbs = 1000
    checkpoint_interval_s = 600

    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    total_checkpoint_gb = StorageHierarchyAnalysis.ckpt_total_gb_val
    ...
    total_pfs_tb = total_checkpoint_gb / 1000
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    per_node_shard_gb_str = fmt(per_node_shard_gb, precision=1, commas=False)
    local_bw_gbs_str = fmt(local_bw_gbs, precision=0, commas=False)
    local_write_s_str = fmt(local_write_s, precision=2, commas=False)
    total_pfs_tb_str = fmt(total_pfs_tb, precision=1, commas=False)
    pfs_copy_s_str = fmt(pfs_copy_s, precision=1, commas=False)
    per_node_pfs_bw_gbs_str = fmt(per_node_pfs_bw_gbs, precision=1, commas=False)
    overhead_pct_str = fmt(overhead_pct, precision=2, commas=False)
```

::: {#nbk-data-storage-checkpoint-storm .callout-notebook title="The checkpoint storm"}

**Problem**: A 256-node cluster saves a 175B-parameter checkpoint every 10 minutes. Each checkpoint totals `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB. With ZeRO-3, each node saves roughly `{python} CheckpointStormCalc.per_node_shard_gb_str` GB.

1. **Per-node write to local NVMe** (4 drives at 7 GB/s each = `{python} CheckpointStormCalc.local_bw_gbs_str` GB/s): `{python} CheckpointStormCalc.per_node_shard_gb_str` GB $\div$ `{python} CheckpointStormCalc.local_bw_gbs_str` GB/s $\approx$ `{python} CheckpointStormCalc.local_write_s_str` seconds.
2. **Async copy to PFS**\index{Async Copy to PFS}: 256 nodes$\times$ `{python} CheckpointStormCalc.per_node_shard_gb_str` GB $\approx$ `{python} CheckpointStormCalc.total_pfs_tb_str` TB total. If the PFS provides 1 TB/s aggregate, the storm completes in roughly `{python} CheckpointStormCalc.pfs_copy_s_str` seconds.
3. **Per-node PFS bandwidth**\index{Bandwidth!per-node pfs}: If all 256 nodes write simultaneously, each gets $1000/256 =$ `{python} CheckpointStormCalc.per_node_pfs_bw_gbs_str` GB/s. Per-node async copy: `{python} CheckpointStormCalc.per_node_shard_gb_str` GB / `{python} CheckpointStormCalc.per_node_pfs_bw_gbs_str` GB/s $\approx$ `{python} CheckpointStormCalc.pfs_copy_s_str` seconds.
4. **Training pause**\index{Training!pause}: only about `{python} CheckpointStormCalc.local_write_s_str` seconds (the local NVMe write). The PFS copy overlaps with the next training iteration.
5. **Overhead**\index{Overhead}: `{python} CheckpointStormCalc.local_write_s_str` s pause every 600 s $\approx$ **`{python} CheckpointStormCalc.overhead_pct_str` percent** training time lost to checkpointing.

**Systems insight**: Under these 1 TB/s aggregate-PFS assumptions, tiered staging reduces checkpoint overhead from about a `{python} CheckpointStormCalc.pfs_copy_s_str` second PFS-direct write to a sub-second local write. The key insight is that $T_{\text{write}}$ for the training pipeline is the *local* write time, not the *durable* write time.

:::

The **tiered staging** strategy minimizes $T_{\text{write}}$ by writing in two phases. In the first phase, each node writes its checkpoint shard to local NVMe at full bandwidth. With four NVMe drives providing 28 GB/s aggregate, a per-node shard of roughly 7 GB (a 256-node ZeRO-3 split of the `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB total checkpoint) completes in roughly 0.25 seconds. The training pipeline can resume immediately after the local write completes.
...
The total storage consumed by checkpoints over the life of a training run is substantial. A 175B parameter model checkpointed every 10 minutes over a 30-day training run generates roughly `{python} StorageHierarchyAnalysis.checkpoint_count_str` checkpoints, each `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB, for a total of approximately `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb_str` PB of checkpoint data. Retaining all of them is neither necessary nor economical. A common retention policy keeps the three most recent checkpoints on the parallel file system for fast recovery, copies every 100th checkpoint to object storage for long-term auditability, and deletes the rest. This policy reduces the parallel file system checkpoint footprint from `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb_str` PB to roughly `{python} StorageHierarchyAnalysis.retention_recent_tb_str` TB (three live full checkpoints) while preserving `{python} StorageHierarchyAnalysis.retained_every_100_count_str` historical snapshots in object storage for posttraining analysis.
...
The checkpoint storage challenge interacts directly with the fault tolerance strategy examined in @sec-fault-tolerance-reliability. From the storage perspective, the design goal is to minimize $T_{\text{write}}$ (the time the training pipeline pauses) while ensuring that at least one durable copy of the checkpoint exists before the next failure window opens. The Young-Daly formula (@sec-fault-tolerance-young-daly) formalizes this trade-off by deriving the optimal checkpoint interval from the cluster's mean time between failures and the checkpoint write time. The storage engineer's contribution to this equation is reducing $T_{\text{write}}$ through tiered staging.

::: {#ws-data-storage-gitlab-database-incident .callout-war-story title="The backup that had not been restored"}

**The context**: GitLab.com depended on PostgreSQL replication, backups, and operational runbooks to protect production database state.

**The failure**: During a January 2017 outage, production database data was accidentally removed from the primary server. Several expected recovery paths were unavailable, stale, incomplete, or not fast enough to avoid data loss.

**The consequence**: GitLab reported many hours of service unavailability and unrecoverable loss of database changes made during a roughly six-hour window.

**The systems lesson**: Storage reliability is a restore property, not a backup property. A checkpoint, replica, or dump only counts if the team regularly proves that it can be located, validated, and restored under incident conditions [@gitlab2017postmortem].

:::

::: {#chk-data-storage-checkpoint-storage-design .callout-checkpoint title="Checkpoint storage design"}

A training cluster of 1,024 nodes saves a 175B-parameter model checkpoint every 10 minutes. Each checkpoint is `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB total, distributed across all nodes.

1. What is the per-node checkpoint size?
2. If each node writes to local NVMe at 28 GB/s, what is $T_{\text{write}}$ for the local write?
...
## Retrieval Infrastructure: Vector Indexes and Hybrid Search {#sec-data-storage-retrieval-infrastructure}
...
@tbl-data-storage-175b-footprint assembles the complete storage picture for our running example, a 30-day training run of a 175B-parameter model on 256 nodes:

| **Category**                               |                                                     **Volume** | **Primary Tier**                                           |
|:-------------------------------------------|---------------------------------------------------------------:|:-----------------------------------------------------------|
| Training dataset (compressed source)       |  `{python} StorageHierarchyAnalysis.compressed_text_tb_str` TB | Object Storage                                             |
| Training dataset (tokenized, per epoch)    |   `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB | Object Storage $\to$ NVMe cache $\to$ Host DRAM            |
...
| All checkpoints (30 days, 10-min interval) | `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb_str` PB | NVMe (transient) $\to$ PFS (recent) $\to$ Object (archive) |
| Archive (retained checkpoints + dataset)   | ~`{python} StorageHierarchyAnalysis.archive_lineage_tb_str` TB | Glacier                                                    |

: **175B-Parameter Training Storage Footprint**: Volume and primary storage tier for each data category in a 30-day, 256-node training run. Checkpoint data dominates the footprint and drives the design of the NVMe-to-PFS-to-Object tiered hierarchy. {#tbl-data-storage-175b-footprint}

**Total data moved through the hierarchy**\index{Total Data Moved Through the Hierarchy}: approximately `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb_str` PB of checkpoint data plus `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB$\times$ epochs of training data reads. For a single-epoch language model training run, checkpoint I/O dominates data loading I/O by a factor of over 1,000.
...
::: {#nbk-data-storage-napkin-math-synthetic-tax .callout-notebook title="Napkin math: The synthetic tax"}
...
* **The pipeline equation governs design**: Required bandwidth scales linearly with GPU count and inversely with iteration time. Use $\text{BW}_{\text{required}} = N_{\text{GPUs}} \times U_{\text{target}} \times D_{\text{vol,batch}} / T_{\text{iteration}}$ to size every tier.
...
* **GPU Direct Storage eliminates CPU bottlenecks**: GDS\index{GPU Direct Storage!CPU bypass} bypasses the CPU in the data path, reducing per-transfer latency by `{python} GDSLatency.gds_speedup_str`$\times$ and freeing CPU cores for augmentation.
...
* **Economics drive tiering**: The `{python} EconRatios.tier_cost_ratio_str`$\times$ cost difference between local NVMe and object storage\index{Storage Tiering!cost drivers} (and the >3,000$\times$ span from HBM to archive) mandates a tiering strategy. Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.
