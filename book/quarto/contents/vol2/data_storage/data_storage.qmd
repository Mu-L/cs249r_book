...
# │ Exports: StorageHierarchyAnalysis.h100_bw_tbs, StorageHierarchyAnalysis.nvme_bw_str,
# │          StorageHierarchyAnalysis.ckpt_total_gb_str, StorageHierarchyAnalysis.gpt3_params_b,
# │          StorageHierarchyAnalysis.bw_ratio_str, StorageHierarchyAnalysis.ckpt_fleet_total_pb,
...
class StorageHierarchyAnalysis:
    ...
    bw_ratio_str      = f"{bw_ratio_val:,.0f}"
    ckpt_fleet_total_pb = f"{ckpt_fleet_total_pb_val:,.1f}"
    checkpoint_count_str = f"{checkpoint_count_val:,}"
    retention_recent_tb_str = f"{retention_recent_tb_val:.2f}"
    retained_every_100_count_str = f"{retained_every_100_count_val:,}"
    compressed_text_tb_str = f"{compressed_text_tb_val:.0f}"
    tokenized_text_tb_str = f"{tokenized_text_tb_val:.0f}"
    baseline_training_tb_str = f"{baseline_training_tb_val:.0f}"
    tokenized_shard_count_str = f"{tokenized_shard_count_val:,}"
    tokenized_shard_gb_str = f"{tokenized_shard_gb_val:.0f}"
    node_tokenized_gb_str = f"{node_tokenized_gb_val:.0f}"
    node_stage_uncontended_s_str = f"{node_stage_uncontended_s_val:.0f}"
    node_stage_contended_s_str = f"{node_stage_contended_s_val:.0f}"
    tokenized_local_stream_min_str = f"{tokenized_local_stream_min_val:.1f}"
```
...
The problem is deceptive because storage technology has improved substantially. NVMe drives achieve `{python} StorageHierarchyAnalysis.nvme_bw_str` GB/s of sequential throughput, a figure that would have seemed out of reach a decade ago. The accelerators, however, improved faster. An H100 GPU consumes data from its HBM at `{python} StorageHierarchyAnalysis.h100_bw_tbs` TB/s, roughly `{python} StorageHierarchyAnalysis.bw_ratio_str`$\times$ faster than a single NVMe drive can feed it. The gap between storage delivery and accelerator consumption is the central tension of this chapter, and it cannot be solved by any single technology. Instead, it requires a hierarchy of storage tiers, each carefully matched to a specific phase of the ML lifecycle, connected by pipelines that hide latency through prefetching and pipelining.
...
class ShardContention:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    sc_n_workers_str = f"{n_workers}"
    sc_n_shards_str = f"{n_shards}"
    sc_collision_prob_str = f"{p_collision * 100:.1f}"
...
|:-------------------------|:---------------------|-----------------------------------------------------:|---------------------------:|-----------------:|
| **GPU HBM**              | 80 GB                | `{python} StorageHierarchyAnalysis.h100_bw_tbs` TB/s |                     ~10 ns |           ~15.00 |
...
# │          MILLION)
# │ Exports: NVMeTierCalcs.raw_bw_str, .hdd_slowdown_factor,
...
2.  **HDD Reality**\index{HDD Reality}: A 7200 RPM hard disk drive (HDD) delivers 100 random IOPS. Sustaining 1,000 images/s requires **`{python} NVMeTierCalcs.hdd_slowdown_factor`$\times$ more IOPS** than the disk provides.
...
class LocalCacheRoi:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    lc_compute_ms_str = f"{t_compute_ms}"
    lc_remote_io_ms_str = f"{t_io_remote_ms}"
    lc_util_remote_str = f"{util_remote:.1f}"
    lc_util_local_str = f"{util_local:.1f}"
...
::: {#def-data-storage-parallel-file-system .callout-definition title="Parallel file system"}
...
::: {#def-data-storage-small-file-problem .callout-definition title="Small file problem"}
...
::: {#def-data-storage-erasure-coding .callout-definition title="Erasure coding"}
...
class GDSLatency:
    ...
    gds_speedup_str   = f"{gds_speedup_val:.0f}"
```
...
::: {#def-data-storage-gpu-direct-storage .callout-definition title="GPU Direct Storage"}
...
A `{python} StorageHierarchyAnalysis.gpt3_params_b`B parameter model with Adam optimizer\index{Adam Optimizer!state size} generates checkpoints of approximately `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB. The checkpoint includes model weights (`350` GB in FP16), optimizer state (momentum and variance, `1,400` GB in FP32), learning rate scheduler state, random number generator state, and the current data loader position. Every GPU in the cluster saves its shard of the checkpoint simultaneously, creating a checkpoint storm that the storage system must absorb without disrupting ongoing training reads.

::: {#def-data-storage-checkpoint-storm .callout-definition title="Checkpoint storm"}
...
The total storage consumed by checkpoints over the life of a training run is substantial. A 175B parameter model checkpointed every 10 minutes over a 30-day training run generates roughly `{python} StorageHierarchyAnalysis.checkpoint_count_str` checkpoints, each `{python} StorageHierarchyAnalysis.ckpt_total_gb_str` GB, for a total of approximately `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb` PB of checkpoint data. Retaining all of them is neither necessary nor economical. A common retention policy keeps the three most recent checkpoints on the parallel file system for fast recovery, copies every 100th checkpoint to object storage for long-term auditability, and deletes the rest. This policy reduces the parallel file system checkpoint footprint from `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb` PB to roughly `{python} StorageHierarchyAnalysis.retention_recent_tb_str` TB (three live full checkpoints) while preserving `{python} StorageHierarchyAnalysis.retained_every_100_count_str` historical snapshots in object storage for posttraining analysis.
...
| All checkpoints (30 days, 10-min interval) |    `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb` PB | NVMe (transient) $\to$ PFS (recent) $\to$ Object (archive) |
...
**Total data moved through the hierarchy**\index{Total Data Moved Through the Hierarchy}: approximately `{python} StorageHierarchyAnalysis.ckpt_fleet_total_pb` PB of checkpoint data plus `{python} StorageHierarchyAnalysis.tokenized_text_tb_str` TB$\times$ epochs of training data reads. For a single-epoch language model training run, checkpoint I/O dominates data loading I/O by a factor of over 1,000.
...
class SyntheticStorage:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    provenance_overhead_pct_str = f"{provenance_metadata_overhead * 100:.0f}"
    footprint_multiplier_str = f"{1 + provenance_metadata_overhead:.1f}"
...
3. **Verification Factor**\index{Verification Factor}: To avoid "Self-Poisoning," each sample is verified by **`{python} SyntheticStorage.verification_passes` independent "Judge" models**.
4. **The amplification**: Total footprint = 1 TB $\times$ `{python} SyntheticStorage.footprint_multiplier_str` $\times$ `{python} SyntheticStorage.verification_passes` = **`{python} SyntheticStorage.total_tb_str` TB**.
