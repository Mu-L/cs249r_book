...
**Problem**\index{ImageNet!bottleneck analysis}: A ResNet-50 training job on ImageNet (`{python} NVMeTierCalcs.imagenet_images_m_str`, ~`{python} NVMeTierCalcs.imagenet_avg_kb_str` average) targets `{python} NVMeTierCalcs.images_per_sec_str` images/second. The question is whether to use individual JPEG files on an HDD or NVMe.
...
```
For our `{python} NodeStagingContext.tokenized_text_tb_str` tokenized dataset distributed across 256 nodes, each node is responsible for approximately `{python} NodeStagingContext.node_tokenized_gb_str` of data. If a node fails, its replacement must stage that shard subset from the PFS. At an uncontended PFS read speed of 4 GB/s, this staging takes about `{python} NodeStagingContext.node_stage_uncontended_s_str`; under recovery contention, an effective 400 MB/s per-node rate stretches the same copy to about `{python} NodeStagingContext.node_stage_contended_s_str`. If the orchestrator can instead schedule the replacement workload on an idle node that already has the data cached from a previous run, staging time is zero and training resumes instantly. At scale, this optimization compounds: in a 10,000-node cluster experiencing 10 node failures per day, locality-aware scheduling can save roughly a minute of staging time per failure, or about 10 minutes per day of aggregate cluster idle time, in the contended case.
...
**Problem**: A training node with `{python} GDSLatency.n_gpus_str` loads `{python} GDSLatency.image_kb_str` images at `{python} GDSLatency.images_per_s_per_gpu_str` images/second per GPU (`{python} GDSLatency.gds_total_images_per_s_str` images/second total). Compare the CPU load under traditional I/O vs. GDS.

**Traditional path**\index{Traditional I/O!data path}: Each image requires a DMA from NVMe to DRAM, a `memcpy` from kernel to user space, and a PCIe transfer to GPU. At `{python} GDSLatency.gds_total_images_per_s_str` images/second with `{python} GDSLatency.gds_trad_us_str` μs of CPU time per image, the CPU spends `{python} GDSLatency.gds_trad_cpu_seconds_str` of CPU time per wall-clock second, consuming roughly `{python} GDSLatency.gds_trad_cpu_cores_str` cores worth of processing just for data movement.

**GDS path**\index{GPU Direct Storage!data path}: Each image is DMA'd directly from NVMe to GPU. At `{python} GDSLatency.gds_bypass_us_str` μs of CPU time per image (for initiating the DMA), the CPU spends `{python} GDSLatency.gds_bypass_cpu_seconds_str` of CPU time per wall-clock second, freeing roughly `{python} GDSLatency.gds_freed_cpu_cores_str` cores for data augmentation.
...
1. **Per-node write to local NVMe** (`{python} CheckpointStormCalc.local_drive_count_str` drives at `{python} CheckpointStormCalc.local_drive_bw_gbs_str` GB/s each = `{python} CheckpointStormCalc.local_bw_gbs_str` GB/s): `{python} CheckpointStormCalc.per_node_shard_gb_str` $\div$ `{python} CheckpointStormCalc.local_bw_gbs_str` GB/s $\approx$ `{python} CheckpointStormCalc.local_write_s_str`.
2. **Async copy to PFS**\index{Checkpointing!async PFS copy}: `{python} CheckpointStormCalc.nodes_str` nodes$\times$ `{python} CheckpointStormCalc.per_node_shard_gb_str` $\approx$ `{python} CheckpointStormCalc.total_pfs_tb_str` total. If the PFS provides `{python} CheckpointStormCalc.pfs_aggregate_bw_tbs_str` TB/s aggregate, the storm completes in roughly `{python} CheckpointStormCalc.pfs_copy_s_str`.
3. **Per-node PFS bandwidth**\index{Bandwidth!per-node pfs}: If all `{python} CheckpointStormCalc.nodes_str` write simultaneously, each gets `{python} CheckpointStormCalc.pfs_aggregate_bw_gbs_str`/`{python} CheckpointStormCalc.nodes_str` = `{python} CheckpointStormCalc.per_node_pfs_bw_gbs_str` GB/s.
...
3. If the parallel file system provides `{python} CheckpointStorageDesignPrompt.pfs_aggregate_tbs_str` TB/s aggregate and all `{python} CheckpointStorageDesignPrompt.nodes_str` write simultaneously, what is the per-node write bandwidth, and how long does the async copy take?
