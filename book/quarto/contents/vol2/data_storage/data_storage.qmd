...
The canonical training-data footprint for this example is roughly `{python} StorageFuelLineContext.baseline_training_tb_str` TB across the hierarchy: `{python} StorageFuelLineContext.compressed_text_tb_str` TB for the compressed source corpus plus `{python} StorageFuelLineContext.tokenized_text_tb_str` TB for the tokenized training shards. Additional shuffled or packed variants can raise the staging footprint, but the per-epoch training read over 4-byte token IDs is `{python} StorageFuelLineContext.tokenized_text_tb_str` TB. The model generates roughly 1.75 TB checkpoints (`{python} StorageFuelLineContext.ckpt_total_gb_str` GB total: 350 GB of weights plus 1.4 TB of Adam optimizer state) every 10 minutes. Over a 30-day training run on 256 nodes, the storage system must deliver `{python} StorageFuelLineContext.tokenized_text_tb_str` TB of tokenized training data per epoch, absorb `{python} StorageFuelLineContext.ckpt_fleet_total_pb_str` PB of checkpoint writes, and stage model weights for evaluation runs. These numbers thread through every section of this chapter, grounding abstract principles in concrete engineering constraints.
...
class StorageAccessPatternFacts:
    ...
    token_id_bytes = BYTES_INT32.m_as(byte)
    tokenized_text_tb_val = (training_tokens * token_id_bytes * byte).m_as(TB)
    ckpt_total_gb_val = (gpt3_params.m_as(param) * 10) / 1e9
    checkpoint_interval_min = 10
...
The third inversion is **write pattern**. Transactional systems generate continuous streams of small writes, each immediately durable. ML systems generate occasional massive writes when saving checkpoints. A `{python} StorageAccessPatternFacts.gpt3_params_b_str`B parameter model checkpoint, including optimizer state, occupies roughly `{python} StorageAccessPatternFacts.ckpt_total_gb_str` GB. Saving it every `{python} StorageAccessPatternFacts.checkpoint_interval_min_str` minutes generates concentrated bursts that saturate bandwidth for seconds, followed by long idle periods. These bursts are the **checkpoint storms**\index{Checkpoint Storm!definition} that parallel file systems must absorb without disrupting ongoing training reads. The bursty write pattern is particularly challenging because all nodes in the cluster write their checkpoint shards simultaneously. If `{python} StorageAccessPatternFacts.ckpt_node_count_str` nodes each write `{python} StorageAccessPatternFacts.ckpt_shard_gb_str` GB, the parallel file system receives `{python} StorageAccessPatternFacts.ckpt_burst_tb_str` TB of writes in a single burst, which must complete before the training pipeline can resume.
...
class StorageHierarchyTable:
    # ┌── 4. OUTPUT ───────────────────────────────────────
    h100_bw_tbs_str = fmt(Hardware.Cloud.H100.memory_bw.m_as(TB/second), precision=2, commas=False)
```

| **Storage Tier**         | **Typical Capacity** |                        **Bandwidth**\index{Bandwidth} | **Latency**\index{Latency} | **Cost (\$/GB)** |
|:-------------------------|:---------------------|------------------------------------------------------:|---------------------------:|-----------------:|
| **GPU HBM**              | 80 GB                | `{python} StorageHierarchyTable.h100_bw_tbs_str` TB/s |                    ~100 ns |           ~15.00 |
...
class StoragePyramidContext:
    ...
    h100_bw = Hardware.Cloud.H100.memory_bw
    tokenized_text_tb_val = (training_tokens * token_id_bytes * byte).m_as(TB)
...
class TextImageBandwidth:
    ...
    step_ms_str = fmt(step_s * 1000, precision=0, commas=False)
...
For **text training**\index{Training!text}, the demand is surprisingly low. With a typical batch size of `{python} TextImageBandwidth.text_tokens_per_gpu_str` tokens per GPU and a `{python} TextImageBandwidth.step_ms_str` ms step time, the aggregate bandwidth is:

`{python} TextImageBandwidth.text_bw_display_math`

This is easily served by a single network-attached storage node.

For **image training**\index{Training!image}, the picture changes dramatically. Using a common batch size of `{python} TextImageBandwidth.image_batch_per_gpu_str` images per GPU (ImageNet at $224{\times}224$, roughly 150 KB/image), the aggregate bandwidth explodes:
...
class NVMeTierCalcs:
    ...
    ckpt_total_gb_val = (Models.Language.GPT3.parameters.m_as(param) * 10) / 1e9
    n_nodes           = 256
    ...
    images_per_sec_str     = fmt(images_per_sec, precision=0, commas=False)
    img_size_kb_str        = fmt(img_size / THOUSAND, precision=0, commas=False)
    imagenet_images_m_str  = fmt(imagenet_images / MILLION, precision=2, commas=False) + "M"
    imagenet_avg_kb_str    = fmt(img_size / THOUSAND, precision=0, commas=False)
...
class NvmeTierContext:
    ...
    tokenized_local_stream_min_val = tokenized_text_tb_val * 1000 / (4 * nvme_bw_raw.m_as(GB/second)) / 60
    local_nvme_capacity_tb_val = 8.0
    multimodal_total_tb_val = 35.0
    ...
    multimodal_images_tb_str = fmt(20, precision=0, commas=False)
    multimodal_text_tb_str = fmt(10, precision=0, commas=False)
    multimodal_audio_tb_str = fmt(5, precision=0, commas=False)
    multimodal_total_tb_str = fmt(35, precision=0, commas=False)
...
\index{NVMe!ML storage tier}When the working set exceeds host DRAM, the system falls to Tier 2: the local NVMe drives attached directly to the compute node. NVMe[^fn-nvme-protocol] provides a high-performance protocol designed specifically for solid-state drives, achieving `{python} NvmeTierContext.nvme_bw_str` GB/s of sequential throughput per drive. With four drives in a RAID-0 configuration, a single node can sustain roughly 28 GB/s of sequential reads before overhead, sufficient to stream the `{python} NvmeTierContext.tokenized_text_tb_str` TB tokenized dataset from local disk in about `{python} NvmeTierContext.tokenized_local_stream_min_str` minutes under ideal sequential conditions.
...
The warm cache pattern requires careful capacity planning. A training node with four 7.68 TB NVMe drives provides approximately `{python} NvmeTierContext.local_nvme_capacity_tb_str` TB of local storage. For our running example, the `{python} NvmeTierContext.tokenized_text_tb_str` TB tokenized dataset fits comfortably on a single node's local storage, with room for checkpoint staging and temporary augmentation buffers. A multi-modal training job combining `{python} NvmeTierContext.multimodal_images_tb_str` TB of images, `{python} NvmeTierContext.multimodal_text_tb_str` TB of text, and `{python} NvmeTierContext.multimodal_audio_tb_str` TB of audio totals `{python} NvmeTierContext.multimodal_total_tb_str` TB, exceeding local capacity by about `{python} NvmeTierContext.multimodal_excess_tb_str` TB and forcing the pipeline to stream from the parallel file system for at least part of the dataset. The design trade-off is between provisioning more local NVMe (which increases node cost) and accepting network-dependent reads (which risks latency spikes).
...
**Problem**\index{ImageNet!bottleneck analysis}: A ResNet-50 training job on ImageNet (`{python} NVMeTierCalcs.imagenet_images_m_str` images, ~`{python} NVMeTierCalcs.imagenet_avg_kb_str` KB average) targets `{python} NVMeTierCalcs.images_per_sec_str` images/second. The question is whether to use individual JPEG files on an HDD or NVMe.

**Math**:

1.  **Raw Bandwidth**\index{ImageNet!raw bandwidth calculation}: `{python} NVMeTierCalcs.images_per_sec_str` images/s $\times$ `{python} NVMeTierCalcs.img_size_kb_str` KB/image = **`{python} NVMeTierCalcs.raw_bw_str` MB/s**.
2.  **HDD Reality**: A 7200 RPM hard disk drive (HDD) delivers `{python} NVMeTierCalcs.hdd_iops_str` random IOPS\index{HDD!random IOPS bottleneck}. Sustaining `{python} NVMeTierCalcs.images_per_sec_str` images/s requires **`{python} NVMeTierCalcs.hdd_slowdown_factor_str`$\times$ more IOPS** than the disk provides.
...
class ObjectStreamContext:
    ...
    tokenized_shard_count_val = round(tokenized_text_tb_val * 1000 / tokenized_shard_gb_val)
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    text_bw_mb_str = fmt(2048 * 4096 * token_id_bytes / 0.2 / 1_000_000, precision=1, commas=False)
    tokenized_shard_count_str = fmt(tokenized_shard_count_val, precision=0)
    tokenized_shard_gb_str = fmt(tokenized_shard_gb_val, precision=0, commas=False)
```
Applying this streaming architecture to our running example makes the design concrete. The tokenized training set is stored in S3 as roughly `{python} ObjectStreamContext.tokenized_shard_count_str` shards of `{python} ObjectStreamContext.tokenized_shard_gb_str` GB each. The streaming architecture assigns each of the 256 compute nodes a unique subset of roughly 8 shards. Within each node, 8 data loader worker processes issue concurrent HTTP `Range` requests to fetch 256 MB chunks from their assigned shards. Across the cluster, this results in 2,048 concurrent requests, allowing the aggregate throughput from S3 to reach 50–100 GB/s, easily saturating the roughly `{python} ObjectStreamContext.text_bw_mb_str` MB/s required for text model training. In this regime, the bottleneck often shifts from the object store's bandwidth to the network bandwidth between the compute VPC and the S3 endpoint. Organizations that co-locate compute and storage in the same cloud availability zone can reduce this cross-network latency from a typical 20 ms to roughly 5 ms, a 4$\times$ improvement that directly reduces the required prefetch buffer depth and improves overall pipeline efficiency.
...
```
For our `{python} NodeStagingContext.tokenized_text_tb_str` TB tokenized dataset distributed across 256 nodes, each node is responsible for approximately `{python} NodeStagingContext.node_tokenized_gb_str` GB of data. If a node fails, its replacement must stage that shard subset from the PFS. At an uncontended PFS read speed of 4 GB/s, this staging takes about `{python} NodeStagingContext.node_stage_uncontended_s_str` seconds; under recovery contention, an effective 400 MB/s per-node rate stretches the same copy to about `{python} NodeStagingContext.node_stage_contended_s_str` seconds. If the orchestrator can instead schedule the replacement workload on an idle node that already has the data cached from a previous run, staging time is zero and training resumes instantly. At scale, this optimization compounds: in a 10,000-node cluster experiencing 10 node failures per day, locality-aware scheduling can save roughly a minute of staging time per failure, or about 10 minutes per day of aggregate cluster idle time, in the contended case.
...
class GDSLatency:
    ...
    image_kb_str = fmt(150, precision=0, commas=False)
    n_gpus_str = fmt(Systems.Nodes.DGX_H100.accelerators_per_node, precision=0, commas=False)
...
**Problem**: A training node with `{python} GDSLatency.n_gpus_str` GPUs loads `{python} GDSLatency.image_kb_str` KB images at `{python} GDSLatency.images_per_s_per_gpu_str` images/second per GPU (`{python} GDSLatency.gds_total_images_per_s_str` images/second total). Compare the CPU load under traditional I/O vs. GDS.
...
class ObjectStorageCostRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    s3_annual_str = fmt(100 * 1000 * 0.02 * 12, precision=0)
    nvme_annual_str = fmt(100 * 1000 * Infrastructure.Pricing.Storage.NvmeLowPerTbMonth.rate.m_as("USD / GB / month") * 12, precision=0)
```
...
class GlacierCostRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    glacier_annual_str = fmt(100 * 1000 * 0.004 * 12, precision=0)
```
...
class StorageTieringCorpus:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compressed_text_tb_str = fmt(3, precision=0, commas=False)
    tokenized_text_tb_str = fmt((1.5 * TRILLION * BYTES_INT32.m_as(byte) * byte).m_as(TB), precision=0, commas=False)
```
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: @sec-storage-checkpointing
# │ Exports: CheckpointStorageContext.*
from mlsysim import Models
from mlsysim.core.constants import *
from mlsysim.fmt import fmt

class CheckpointStorageContext:
    gpt3_params = Models.Language.GPT3.parameters
    training_tokens = 1.5 * TRILLION
    compressed_text_tb_val = 3
    token_id_bytes = BYTES_INT32.m_as(byte)
    ...
    archive_lineage_tb_val = retained_every_100_count_val * ckpt_total_gb_val / 1000 + compressed_text_tb_val + tokenized_text_tb_val
...
A `{python} CheckpointStorageContext.gpt3_params_b_str`B parameter model with Adam optimizer\index{Adam Optimizer!state size} generates checkpoints of approximately `{python} CheckpointStorageContext.ckpt_total_gb_str` GB. The checkpoint includes model weights (`350` GB in FP16), optimizer state (momentum and variance, `1,400` GB in FP32), learning rate scheduler state, random number generator state, and the current data loader position. Every GPU in the cluster saves its shard of the checkpoint simultaneously, creating a checkpoint storm that the storage system must absorb without disrupting ongoing training reads.
...
1.  **Significance (quantitative)**: The storm magnitude scales as $T_{\text{write}} = H \times \text{per-node shard} / \text{BW}_{\text{fabric}}$, where $H$ is the host-node count. For a `{python} CheckpointStormDefinition.params_b_str`B-parameter model in FP16 (`{python} CheckpointStormDefinition.weights_gb_str` GB of weights) trained across `{python} CheckpointStormDefinition.nodes_str` replicas with vanilla data parallelism (every replica holding its own complete copy), a naive checkpoint generates `{python} CheckpointStormDefinition.naive_total_tb_str` TB of simultaneous writes; at 100 GB/s fabric bandwidth, $T_{\text{write}} \approx$ `{python} CheckpointStormDefinition.naive_write_s_str` seconds, over `{python} CheckpointStormDefinition.naive_write_min_str` minutes of training stall per checkpoint event. ZeRO-3 sharding collapses this to roughly `{python} CheckpointStormDefinition.zero3_total_tb_str` TB total (`{python} CheckpointStormDefinition.weights_gb_str` GB weights + `{python} CheckpointStormDefinition.gradients_gb_str` GB gradients + `{python} CheckpointStormDefinition.optimizer_state_gb_str` GB optimizer state, split across `{python} CheckpointStormDefinition.nodes_str` replicas), bringing $T_{\text{write}}$ down to roughly `{python} CheckpointStormDefinition.zero3_write_s_str` seconds. This stall is the checkpoint $T_{\text{write}}$ term and can dwarf the compute time between checkpoints when the sharding optimization is omitted.
...
class CheckpointStormCalc:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────────────────────
    total_checkpoint_gb = (Models.Language.GPT3.parameters.m_as(param) * 10) / 1e9
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    ckpt_total_gb_str = fmt(total_checkpoint_gb, precision=0)
    ...
    local_drive_bw_gbs_str = fmt(local_drive_bw_gbs, precision=0, commas=False)
    nodes_str = fmt(nodes, precision=0, commas=False)
    pfs_aggregate_bw_tbs_str = fmt(pfs_aggregate_bw_gbs / 1000, precision=0, commas=False)
    checkpoint_interval_min_str = fmt(checkpoint_interval_min, precision=0, commas=False)
    checkpoint_interval_s_str = fmt(checkpoint_interval_s, precision=0, commas=False)
    pfs_aggregate_bw_gbs_str = fmt(pfs_aggregate_bw_gbs, precision=0, commas=False)
```

::: {#nbk-data-storage-checkpoint-storm .callout-notebook title="The checkpoint storm"}

**Problem**: A `{python} CheckpointStormCalc.nodes_str`-node cluster saves a 175B-parameter checkpoint every `{python} CheckpointStormCalc.checkpoint_interval_min_str` minutes. Each checkpoint totals `{python} CheckpointStorageContext.ckpt_total_gb_str` GB. With ZeRO-3, each node saves roughly `{python} CheckpointStormCalc.per_node_shard_gb_str` GB.

1. **Per-node write to local NVMe** (`{python} CheckpointStormCalc.local_drive_count_str` drives at `{python} CheckpointStormCalc.local_drive_bw_gbs_str` GB/s each = `{python} CheckpointStormCalc.local_bw_gbs_str` GB/s): `{python} CheckpointStormCalc.per_node_shard_gb_str` GB $\div$ `{python} CheckpointStormCalc.local_bw_gbs_str` GB/s $\approx$ `{python} CheckpointStormCalc.local_write_s_str` seconds.
2. **Async copy to PFS**\index{Checkpointing!async PFS copy}: `{python} CheckpointStormCalc.nodes_str` nodes$\times$ `{python} CheckpointStormCalc.per_node_shard_gb_str` GB $\approx$ `{python} CheckpointStormCalc.total_pfs_tb_str` TB total. If the PFS provides `{python} CheckpointStormCalc.pfs_aggregate_bw_tbs_str` TB/s aggregate, the storm completes in roughly `{python} CheckpointStormCalc.pfs_copy_s_str` seconds.
3. **Per-node PFS bandwidth**\index{Bandwidth!per-node pfs}: If all `{python} CheckpointStormCalc.nodes_str` nodes write simultaneously, each gets `{python} CheckpointStormCalc.pfs_aggregate_bw_gbs_str`/`{python} CheckpointStormCalc.nodes_str` = `{python} CheckpointStormCalc.per_node_pfs_bw_gbs_str` GB/s.
...
5. **Overhead**\index{Checkpointing!overhead}: `{python} CheckpointStormCalc.local_write_s_str` s pause every `{python} CheckpointStormCalc.checkpoint_interval_s_str` s $\approx$ **`{python} CheckpointStormCalc.overhead_pct_str` percent** training time lost to checkpointing.
...
The **tiered staging** strategy minimizes $T_{\text{write}}$ by writing in two phases. In the first phase, each node writes its checkpoint shard to local NVMe at full bandwidth. With four NVMe drives providing 28 GB/s aggregate, a per-node shard of roughly 7 GB (a 256-node ZeRO-3 split of the `{python} CheckpointStorageContext.ckpt_total_gb_str` GB total checkpoint) completes in roughly 0.25 seconds. The training pipeline can resume immediately after the local write completes.
...
The total storage consumed by checkpoints over the life of a training run is substantial. A 175B parameter model checkpointed every 10 minutes over a 30-day training run generates roughly `{python} CheckpointStorageContext.checkpoint_count_str` checkpoints, each `{python} CheckpointStorageContext.ckpt_total_gb_str` GB, for a total of approximately `{python} CheckpointStorageContext.ckpt_fleet_total_pb_str` PB of checkpoint data. Retaining all of them is neither necessary nor economical. A common retention policy keeps the three most recent checkpoints on the parallel file system for fast recovery, copies every 100th checkpoint to object storage for long-term auditability, and deletes the rest. This policy reduces the parallel file system checkpoint footprint from `{python} CheckpointStorageContext.ckpt_fleet_total_pb_str` PB to roughly `{python} CheckpointStorageContext.retention_recent_tb_str` TB (three live full checkpoints) while preserving `{python} CheckpointStorageContext.retained_every_100_count_str` historical snapshots in object storage for posttraining analysis.
...
class CheckpointStorageDesignPrompt:
    ...
    nodes_str = fmt(nodes, precision=0, commas=True)
    checkpoint_interval_min_str = fmt(checkpoint_interval_min, precision=0, commas=False)
    local_nvme_bw_gbs_str = fmt(local_nvme_bw_gbs, precision=0, commas=False)
    pfs_aggregate_tbs_str = fmt(pfs_aggregate_tbs, precision=0, commas=False)
```

::: {#chk-data-storage-checkpoint-storage-design .callout-checkpoint title="Checkpoint storage design"}

A training cluster of `{python} CheckpointStorageDesignPrompt.nodes_str` nodes saves a 175B-parameter model checkpoint every `{python} CheckpointStorageDesignPrompt.checkpoint_interval_min_str` minutes. Each checkpoint is `{python} CheckpointStorageContext.ckpt_total_gb_str` GB total, distributed across all nodes.

1. What is the per-node checkpoint size?
2. If each node writes to local NVMe at `{python} CheckpointStorageDesignPrompt.local_nvme_bw_gbs_str` GB/s, what is $T_{\text{write}}$ for the local write?
3. If the parallel file system provides `{python} CheckpointStorageDesignPrompt.pfs_aggregate_tbs_str` TB/s aggregate and all `{python} CheckpointStorageDesignPrompt.nodes_str` nodes write simultaneously, what is the per-node write bandwidth, and how long does the async copy take?
...
class StorageFootprintTable:
    ...
    checkpoint_count_val = training_days * 24 * 60 // checkpoint_interval_min
    ckpt_fleet_total_pb_val = (ckpt_total_gb_val * checkpoint_count_val * GB).m_as(PB)
    archive_lineage_tb_val = (checkpoint_count_val // 100) * ckpt_total_gb_val / 1000 + 3 + tokenized_text_tb_val
    ...
    ckpt_io_dominance_factor_str = fmt(
        int((ckpt_fleet_total_pb_val * 1000) / tokenized_text_tb_val),
        precision=0,
        commas=True,
    )
    training_nodes_str = fmt(training_nodes, precision=0, commas=False)
```

::: {#nbk-data-storage-175b-models-storage-footprint .callout-notebook title="The 175B model's storage footprint"}

@tbl-data-storage-175b-footprint assembles the complete storage picture for our running example, a 30-day training run of a 175B-parameter model on `{python} StorageFootprintTable.training_nodes_str` nodes:

| **Category**                               |                                                  **Volume** | **Primary Tier**                                           |
|:-------------------------------------------|------------------------------------------------------------:|:-----------------------------------------------------------|
| Training dataset (compressed source)       |  `{python} StorageFootprintTable.compressed_text_tb_str` TB | Object Storage                                             |
| Training dataset (tokenized, per epoch)    |   `{python} StorageFootprintTable.tokenized_text_tb_str` TB | Object Storage $\to$ NVMe cache $\to$ Host DRAM            |
...
| Archive (retained checkpoints + dataset)   | ~`{python} StorageFootprintTable.archive_lineage_tb_str` TB | Glacier                                                    |
...
**Total data moved through the hierarchy**\index{Storage!total data movement}: approximately `{python} StorageFootprintTable.ckpt_fleet_total_pb_str` PB of checkpoint data plus `{python} StorageFootprintTable.tokenized_text_tb_str` TB$\times$ `{python} StorageFootprintTable.epochs_str` epochs of training data reads. For a single-epoch language model training run, checkpoint I/O dominates data loading I/O by a factor of over `{python} StorageFootprintTable.ckpt_io_dominance_factor_str`.
...
**Problem**: Calculate the storage amplification of a `{python} SyntheticStorage.data_size_tb_str` TB synthetic dataset that requires cryptographic lineage and multi-model verification.

1. **Raw Payload**\index{Synthetic Data!raw payload}: `{python} SyntheticStorage.data_size_tb_str` TB.
...
4. **The amplification**: Total footprint = `{python} SyntheticStorage.data_size_tb_str` TB $\times$ `{python} SyntheticStorage.footprint_multiplier_str` $\times$ `{python} SyntheticStorage.verification_passes_str` = **`{python} SyntheticStorage.total_tb_str` TB**.
