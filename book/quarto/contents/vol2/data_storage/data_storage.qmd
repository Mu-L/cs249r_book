...
# │ Show:    150 MB/s raw image BW; 10x HDD slowdown; 0.3s NVMe / 1.0s PFS
...
# │          .ckpt_nvme_s_str, .ckpt_pfs_s_str
# └─────────────────────────────────────────────────────────────────────────────
...
3.  **Result**: Shuffling individual files on an HDD will starve the GPU, reducing utilization to <5 percent.
...
# │ Show:    120 μs traditional vs 30 μs GDS; 4x speedup.
...
**GDS path**\index{GDS Path}: Each image is DMA'd directly from NVMe to GPU. At `{python} GDSLatency.gds_bypass_us_str` μs of CPU time per image (for initiating the DMA), the CPU spends 1.92 seconds of CPU time per wall-clock second, freeing 6 cores for data augmentation.
...
1.  **Significance (quantitative)**: The storm magnitude scales as $T_{\text{write}} = N \times \text{per-node-shard} / \text{BW}_{\text{fabric}}$. For a 70B-parameter model in FP16 (140 GB of weights) trained across 1,000 nodes with vanilla data parallelism (every node holding its own complete copy), a naive checkpoint generates 140 TB of simultaneous writes; at 100 GB/s fabric bandwidth, $T_{\text{write}} \approx 1{,}400$ seconds, over 23 minutes of training stall per checkpoint event. ZeRO-3 sharding collapses this to roughly 1.1 TB total (140 GB weights + 140 GB gradients + 840 GB optimizer state, split across 1,000 nodes), bringing $T_{\text{write}}$ down to roughly 11 seconds. This stall adds directly to the $L_{\text{lat}}$ term and can dwarf the compute time between checkpoints when the sharding optimization is omitted.
