...
# │ Exports: StorageHierarchyTable.*, StoragePyramidContext.*
...
# │ Exports: ObjectStorageCostRecap.*, GlacierCostRecap.*
from mlsysim.core.constants import STORAGE_COST_NVME_LOW
from mlsysim.fmt import fmt

class ObjectStorageCostRecap:
    s3_annual_str = fmt(100 * 1000 * 0.02 * 12, precision=0)
    nvme_annual_str = fmt(100 * 1000 * STORAGE_COST_NVME_LOW.m_as("USD / GB / month") * 12, precision=0)
...
# │ Exports: StorageEconomicsCorpus.*, StorageTieringCorpus.*
from mlsysim.core.constants import *
from mlsysim.fmt import fmt
...
**Problem**: A 256-node cluster saves a 175B-parameter checkpoint every 10 minutes. Each checkpoint totals `{python} CheckpointStorageContext.ckpt_total_gb_str` GB. With ZeRO-3, each node saves roughly `{python} CheckpointStormCalc.per_node_shard_gb_str` GB.
...
A training cluster of 1,024 nodes saves a 175B-parameter model checkpoint every 10 minutes. Each checkpoint is `{python} CheckpointStorageContext.ckpt_total_gb_str` GB total, distributed across all nodes.
...
**Total data moved through the hierarchy**\index{Storage!total data movement}: approximately `{python} StorageFootprintTable.ckpt_fleet_total_pb_str` PB of checkpoint data plus `{python} StorageFootprintTable.tokenized_text_tb_str` TB$\times$ epochs of training data reads. For a single-epoch language model training run, checkpoint I/O dominates data loading I/O by a factor of over 1,000.
...
# │ Exports: GDSLatencyRecap.gds_speedup_str, EconRatiosRecap.tier_cost_ratio_str
from mlsysim.core.constants import STORAGE_COST_NVME_LOW
from mlsysim.fmt import fmt
...
class EconRatiosRecap:
    tier_cost_ratio_str = fmt(STORAGE_COST_NVME_LOW.m_as("USD / GB / month") / 0.02, precision=0, commas=False)
```
