...
# │          mlsysim.core.formulas (calc_kv_cache_size, model_memory)
# │ Exports: StorageHierarchyAnalysis.h100_bw_tbs_str, .nvme_bw_str,
...
class StorageHierarchyAnalysis:
    ...
    h100_bw       = h_h100.memory_bw
    nvme_bw_raw   = Hardware.Cloud.H100.storage.bandwidth # Sourced from registry
    ...
    nvme_drive_capacity_tb_val = 2.0
    ...
    # 10 bytes/param = 2 (FP16) + 8 (Adam FP32)
    bytes_per_param_ckpt = 10
    ckpt_total_gb_val   = (gpt3_params.m_as(param) * bytes_per_param_ckpt) / 1e9
    ...
    gpt3_params_b_str              = fmt(gpt3_params.m_as(param) / BILLION, precision=1, allow_zero=True, commas=False)
    ckpt_total_gb_str              = fmt(ckpt_total_gb_val, precision=1, allow_zero=True)
    bw_ratio_str                   = fmt(bw_ratio_val, precision=1, allow_zero=True)
    ckpt_fleet_total_pb_str        = fmt(ckpt_fleet_total_pb_val, precision=1)
    checkpoint_count_str           = fmt(checkpoint_count_val, precision=1, allow_zero=True)
    retention_recent_tb_str        = fmt(retention_recent_tb_val, precision=2, commas=False)
    retained_every_100_count_str   = fmt(retained_every_100_count_val, precision=1, allow_zero=True)
    compressed_text_tb_str         = fmt(compressed_text_tb_val, precision=1, allow_zero=True, commas=False)
    tokenized_text_tb_str          = fmt(tokenized_text_tb_val, precision=1, allow_zero=True, commas=False)
    baseline_training_tb_str       = fmt(baseline_training_tb_val, precision=1, allow_zero=True, commas=False)
    tokenized_shard_count_str      = fmt(tokenized_shard_count_val, precision=1, allow_zero=True)
    tokenized_shard_gb_str         = fmt(tokenized_shard_gb_val, precision=1, allow_zero=True, commas=False)
    node_tokenized_gb_str          = fmt(node_tokenized_gb_val, precision=1, allow_zero=True, commas=False)
    node_stage_uncontended_s_str   = fmt(node_stage_uncontended_s_val, precision=1, allow_zero=True, commas=False)
    node_stage_contended_s_str     = fmt(node_stage_contended_s_val, precision=1, allow_zero=True, commas=False)
    tokenized_local_stream_min_str = fmt(tokenized_local_stream_min_val, precision=1, commas=False)
    host_dram_bw_gbs_str           = fmt(host_dram_bw_gbs_val, precision=1, allow_zero=True, commas=False)
    hbm_host_dram_ratio_str        = fmt(hbm_host_dram_ratio_val, precision=1, allow_zero=True, commas=False)
    local_nvme_capacity_tb_str     = fmt(local_nvme_capacity_tb_val, precision=1, commas=False)
    multimodal_images_tb_str       = fmt(multimodal_images_tb_val, precision=1, allow_zero=True, commas=False)
    multimodal_text_tb_str         = fmt(multimodal_text_tb_val, precision=1, allow_zero=True, commas=False)
    multimodal_audio_tb_str        = fmt(multimodal_audio_tb_val, precision=1, allow_zero=True, commas=False)
    multimodal_total_tb_str        = fmt(multimodal_total_tb_val, precision=1, allow_zero=True, commas=False)
    multimodal_excess_tb_str       = fmt(multimodal_excess_tb_val, precision=1, commas=False)
    archive_lineage_tb_str         = fmt(archive_lineage_tb_val, precision=1, allow_zero=True, commas=False)
```
...
class ShardContention:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    sc_n_workers_str = fmt(n_workers, precision=1, allow_zero=True, commas=False)
    sc_n_shards_str = fmt(n_shards, precision=1, allow_zero=True, commas=False)
...
storage_gbs   = [
    NVME_GEN3_SEQUENTIAL_BW.m_as(GB/second),
    NVME_GEN3_SEQUENTIAL_BW.m_as(GB/second),
    NVME_SEQUENTIAL_BW.m_as(GB/second),
    NVME_SEQUENTIAL_BW.m_as(GB/second),
    NVME_GEN5_SEQUENTIAL_BW.m_as(GB/second),
]
...
class TextImageBandwidth:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    text_bw_mb_str = fmt(text_bw_mb_s, precision=1, allow_zero=True, commas=False)
    image_bw_gb_str = fmt(image_bw_gb_s, precision=1, allow_zero=True, commas=False)
    ratio_rounded_str = fmt(ratio_rounded_val, precision=1, allow_zero=True)
...
class HbmMemoryBudget:
    ...
    total_range_str = MarkdownStr(f"{fmt(total_min_gb, precision=1, allow_zero=True, commas=False)}--{fmt(total_max_gb, precision=1, allow_zero=True, commas=False)}")
    total_pct_range_str = MarkdownStr(f"{fmt(total_min_pct, precision=1, allow_zero=True, commas=False)} to {fmt(total_max_pct, precision=1, allow_zero=True, commas=False)}")
```
...
class DRAMInterconnect:
    ...
    pcie5_bw_gbs_str  = fmt(pcie5_val, precision=1, allow_zero=True, commas=False)
    nvlink_bw_gbs_str = fmt(nvlink_val, precision=1, allow_zero=True, commas=False)
```
...
class NVMeTierCalcs:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    raw_bw_str             = fmt(raw_bw_val, precision=1, allow_zero=True, commas=False)
    hdd_slowdown_factor_str = fmt(hdd_slowdown_val, precision=1, allow_zero=True, commas=False)
    hdd_util_ceiling_pct_str = fmt(hdd_util_ceiling_pct_val, precision=1, allow_zero=True, commas=False)
...
class LocalCacheRoi:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    lc_compute_ms_str = fmt(t_compute_ms, precision=1, allow_zero=True, commas=False)
    lc_remote_io_ms_str = fmt(t_io_remote_ms, precision=1, allow_zero=True, commas=False)
...
# │
# │ Imports: mlsysim.core.constants (STORAGE_COST_NVME_LOW)
...
class ObjectStorageCost:
    ...
    dataset_size_tb  = 100
    cost_s3_gb_mo    = 0.02
    cost_nvme_gb_mo  = STORAGE_COST_NVME_LOW.m_as("USD / GB / month")
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    s3_annual_str   = fmt(s3_annual_val, precision=1, allow_zero=True)
    nvme_annual_str = fmt(nvme_annual_val, precision=1, allow_zero=True)
```
...
class GlacierCost:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    glacier_annual_str = fmt(glacier_annual_val, precision=1, allow_zero=True)
```
...
class PipelineCalcs:
    ...
    data_stall_pct_str  = fmt(data_stall_pct_val, precision=1, allow_zero=True, commas=False)
    t_comp_stall_str    = fmt(t_comp_val, precision=1, allow_zero=True, commas=False)
    t_io_stall_str      = fmt(t_io_val, precision=1, allow_zero=True, commas=False)
...
points = [
    ('HDD', 0.5),
    ('Network FS', 2.0),
    ('NVMe', NVME_SEQUENTIAL_BW.m_as(GB/second)),
    ('NVMe RAID', 25.0),
]
...
class HashValidationCost:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    dataset_tb_str = fmt(dataset_tb, precision=1, allow_zero=True, commas=False)
    throughput_mb_s_str = fmt(throughput_mb_s_per_core, precision=1, commas=False)
    core_hours_str = fmt(core_hours, precision=1, allow_zero=True)
```
...
class GDSLatency:
    ...
    total_images_per_s = GPUS_PER_HOST * images_per_s_per_gpu
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    gds_trad_us_str   = fmt(gds_trad_us, precision=1, allow_zero=True, commas=False)
    gds_bypass_us_str = fmt(gds_bypass_us, precision=1, allow_zero=True, commas=False)
    gds_speedup_str   = fmt(gds_speedup_val, precision=1, allow_zero=True, commas=False)
    gds_latency_reduction_pct_str = fmt(gds_latency_reduction_pct_val, precision=1, allow_zero=True, commas=False)
    gds_total_images_per_s_str = fmt(total_images_per_s, precision=1, allow_zero=True)
    gds_trad_cpu_seconds_str = fmt(gds_trad_cpu_seconds_val, precision=2, commas=False)
    gds_trad_cpu_cores_str = fmt(round(gds_trad_cpu_seconds_val), precision=1, allow_zero=True, commas=False)
    gds_bypass_cpu_seconds_str = fmt(gds_bypass_cpu_seconds_val, precision=2, commas=False)
    gds_freed_cpu_cores_str = fmt(round(gds_freed_cpu_cores_val), precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (CLOUD_EGRESS_PER_GB, MILLION,
# │          STORAGE_COST_NVME_LOW)
...
class EconRatios:
    ...
    cost_nvme_gb_mo = STORAGE_COST_NVME_LOW.m_as("USD / GB / month")
    cost_s3_gb_mo   = 0.02
    cost_egress_gb  = CLOUD_EGRESS_PER_GB.m_as("USD / GB")
    dataset_size_tb = 100
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    tier_cost_ratio_str = fmt(tier_cost_ratio_val, precision=1, allow_zero=True, commas=False)
    egress_100tb_str    = fmt(egress_100tb_val, precision=1, allow_zero=True)
...
class BuildVsBuyStorageEconomics:
    ...
    cloud_egress_gb = CLOUD_EGRESS_PER_GB.m_as("USD / GB")
    dataset_pb = 1
    reads_per_year = 10
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    cloud_storage_annual_str = fmt(cloud_storage_annual, precision=1, allow_zero=True)
    cloud_egress_annual_str = fmt(cloud_egress_annual, precision=1, allow_zero=True)
    cloud_total_annual_str = fmt(cloud_total_annual, precision=1, allow_zero=True)
    annual_ops_str = fmt(annual_ops, precision=1, allow_zero=True)
    net_annual_savings_str = fmt(net_annual_savings, precision=1, allow_zero=True)
...
class CheckpointStormDefinition:
    ...
    fabric_bw_gbs = INFINIBAND_XDR_BW.m_as(GB/second)
    ...
    weights_gb_str = fmt(weights_gb, precision=1, allow_zero=True, commas=False)
    gradients_gb_str = fmt(gradients_gb, precision=1, allow_zero=True, commas=False)
    master_weights_gb_str = fmt(master_weights_gb, precision=1, allow_zero=True, commas=False)
    adam_moments_gb_str = fmt(adam_moments_gb, precision=1, allow_zero=True, commas=False)
    optimizer_state_gb_str = fmt(optimizer_state_gb, precision=1, allow_zero=True, commas=False)
    ...
    naive_total_tb_str = fmt(naive_total_tb, precision=1, allow_zero=True, commas=False)
    naive_write_s_str = fmt(naive_write_s, precision=1, allow_zero=True)
    naive_write_min_str = fmt(naive_write_min, precision=1, allow_zero=True, commas=False)
```
...
class CheckpointStormCalc:
    ...
    local_drive_bw_gbs = NVME_SEQUENTIAL_BW.m_as(GB/second)
    ...
    local_bw_gbs_str = fmt(local_bw_gbs, precision=1, allow_zero=True, commas=False)
...
class SyntheticStorage:
    ...
    verification_passes_str = fmt(verification_passes, precision=1, allow_zero=True, commas=False)
    provenance_overhead_pct_str = fmt(provenance_metadata_overhead * 100, precision=1, allow_zero=True, commas=False)
