...
class StorageFuelLineContext:
    ...
    tokenized_text_tb_str = fmt_int(round(tokenized_text_tb_val), commas=False)
    baseline_training_tb_str = fmt_int(round(baseline_training_tb_val), commas=False)
...
class HbmTierBandwidth:
    ...
    hbm_host_dram_ratio_str = fmt(hbm_host_dram_ratio_val.m_as(ureg.dimensionless), precision=1, commas=False)
```
...
class StorageEconomicsCorpus:
    # ┌── 4. OUTPUT ───────────────────────────────────────
    tokenized_text_tb_str = fmt_int(round((1.5 * TRILLION * BYTES_INT32.m_as(byte) * byte).m_as(TB)), commas=False)
```
