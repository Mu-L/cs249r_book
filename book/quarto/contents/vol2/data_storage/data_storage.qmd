...
**Systems conclusion**: The pipeline must either use NVMe (sub-10 μs random access) or convert the dataset to a sequential format (TFRecord/WebDataset) to achieve sequential throughput.
...
**Systems conclusion**: GDS reduces latency and, more importantly, shifts CPU utilization from data copying (which adds no value) to data augmentation (which improves model quality).
...
**Systems conclusion**: Local NVMe caching saves \$282,000/year at 20 epochs per run, enough to fund 3 additional H100 GPUs. Including the \$60,000/year provisioned NVMe cost and the once-per-run staging egress, the break-even point is just over 4 epochs per run; by the fifth epoch, local caching is cheaper than streaming from S3 every epoch.
...
*   **ML workloads invert storage assumptions**\index{ML Workloads Invert Storage Assumptions}: Traditional caching, IOPS optimization, and small-write durability patterns all produce the wrong answer for ML training. Throughput (GB/s) matters more than IOPS. Small files are the enemy; aggregated shards are the solution.
...
*   **GPU Direct Storage eliminates CPU bottlenecks**\index{GPU Direct Storage!eliminates cpu bottlenecks}: GDS bypasses the CPU in the data path, reducing per-transfer latency by `{python} GDSLatency.gds_speedup_str`$\times$ and freeing CPU cores for augmentation.
...
*   **Checkpoint staging minimizes $T_{\text{write}}$**\index{Checkpointing!staging strategy}: Write to local NVMe first, then replicate asynchronously to shared storage. This decouples checkpoint pause time from parallel file system performance.
*   **Economics drive tiering**\index{Economics Drive Tiering}: The `{python} EconRatios.tier_cost_ratio_str`$\times$ cost difference between local NVMe and object storage (and the >3,000$\times$ span from HBM to archive) mandates a tiering strategy. Egress fees often exceed storage fees; design for total cost of data delivery, not storage cost alone.
