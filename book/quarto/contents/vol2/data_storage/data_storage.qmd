...
1. **Probability of No Collision**: $\approx e^{-n^2/2N} = e^{-32^2/2000} \approx 0.60$.
...
1.  **Raw Bandwidth**\index{ImageNet!raw bandwidth calculation}: 1,000 images/s$\times$ 150 KB/image = **`{python} NVMeTierCalcs.raw_bw_str` MB/s**.
2.  **HDD Reality**: A 7200 RPM hard disk drive (HDD) delivers 100 random IOPS\index{HDD!random IOPS bottleneck}. Sustaining 1,000 images/s requires **`{python} NVMeTierCalcs.hdd_slowdown_factor_str`$\times$ more IOPS** than the disk provides.
...
1. **Remote Only**\index{Warm Cache!remote-read baseline}: `{python} LocalCacheRoi.lc_compute_ms_str` / (`{python} LocalCacheRoi.lc_compute_ms_str` + `{python} LocalCacheRoi.lc_remote_io_ms_str`) $\approx$ **`{python} LocalCacheRoi.lc_util_remote_str` percent**.
...
**Scenario**: A GPU processes a batch in `{python} PipelineCalcs.t_comp_stall_str` ms, but storage takes `{python} PipelineCalcs.t_io_stall_str` ms to deliver the next batch.
...
**Problem**: A training node with 8 GPUs loads 150 KB images at 8,000 images/second per GPU (`{python} GDSLatency.gds_total_images_per_s_str` images/second total). Compare the CPU load under traditional I/O vs. GDS.
...
**GDS path**\index{GPU Direct Storage!data path}: Each image is DMA'd directly from NVMe to GPU. At `{python} GDSLatency.gds_bypass_us_str` μs of CPU time per image (for initiating the DMA), the CPU spends `{python} GDSLatency.gds_bypass_cpu_seconds_str` seconds of CPU time per wall-clock second, freeing roughly `{python} GDSLatency.gds_freed_cpu_cores_str` cores for data augmentation.
...
2. **Async copy to PFS**\index{Checkpointing!async PFS copy}: 256 nodes$\times$ `{python} CheckpointStormCalc.per_node_shard_gb_str` GB $\approx$ `{python} CheckpointStormCalc.total_pfs_tb_str` TB total. If the PFS provides 1 TB/s aggregate, the storm completes in roughly `{python} CheckpointStormCalc.pfs_copy_s_str` seconds.
...
4. **Training pause**\index{Checkpointing!training pause}: only about `{python} CheckpointStormCalc.local_write_s_str` seconds (the local NVMe write). The PFS copy overlaps with the next training iteration.
5. **Overhead**\index{Checkpointing!overhead}: `{python} CheckpointStormCalc.local_write_s_str` s pause every 600 s $\approx$ **`{python} CheckpointStormCalc.overhead_pct_str` percent** training time lost to checkpointing.
...
1. **Raw Payload**\index{Synthetic Data!raw payload}: 1 TB.
2. **Provenance Overhead**\index{Synthetic Data!provenance overhead}: `{python} SyntheticStorage.provenance_overhead_pct_str` percent extra for lineage hashes, generation logs, and reward-model scores.
...
* **GPU Direct Storage eliminates CPU bottlenecks**: GDS bypasses the CPU in the data path, reducing per-transfer latency by `{python} GDSLatency.gds_speedup_str`$\times$ and freeing CPU cores for augmentation.
