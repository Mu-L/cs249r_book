...
**Context**: GitLab.com depended on PostgreSQL replication, backups, and operational runbooks to protect production database state [@gitlab2017postmortem].

**Failure**: During a January 2017 outage, production database data was accidentally removed from the primary server. Several expected recovery paths were unavailable, stale, incomplete, or not fast enough to avoid data loss.

**Consequence**: GitLab reported many hours of service unavailability and unrecoverable loss of database changes made during a roughly six-hour window.
