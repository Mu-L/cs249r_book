...
class StorageHierarchyAnalysis:
    ...
    gpt3_params_b_str              = fmt(gpt3_params.m_as(param) / BILLION, precision=1, commas=False)
    ckpt_total_gb_str              = fmt(ckpt_total_gb_val, precision=1)
    bw_ratio_str                   = fmt(bw_ratio_val, precision=1)
    ckpt_fleet_total_pb_str        = fmt(ckpt_fleet_total_pb_val, precision=1)
    checkpoint_count_str           = fmt(checkpoint_count_val, precision=1)
    retention_recent_tb_str        = fmt(retention_recent_tb_val, precision=2, commas=False)
    retained_every_100_count_str   = fmt(retained_every_100_count_val, precision=1)
    compressed_text_tb_str         = fmt(compressed_text_tb_val, precision=1, commas=False)
    tokenized_text_tb_str          = fmt(tokenized_text_tb_val, precision=1, commas=False)
    baseline_training_tb_str       = fmt(baseline_training_tb_val, precision=1, commas=False)
    tokenized_shard_count_str      = fmt(tokenized_shard_count_val, precision=1)
    tokenized_shard_gb_str         = fmt(tokenized_shard_gb_val, precision=1, commas=False)
    node_tokenized_gb_str          = fmt(node_tokenized_gb_val, precision=1, commas=False)
    node_stage_uncontended_s_str   = fmt(node_stage_uncontended_s_val, precision=1, commas=False)
    node_stage_contended_s_str     = fmt(node_stage_contended_s_val, precision=1, commas=False)
    tokenized_local_stream_min_str = fmt(tokenized_local_stream_min_val, precision=1, commas=False)
    host_dram_bw_gbs_str           = fmt(host_dram_bw_gbs_val, precision=1, commas=False)
    hbm_host_dram_ratio_str        = fmt(hbm_host_dram_ratio_val, precision=1, commas=False)
    local_nvme_capacity_tb_str     = fmt(local_nvme_capacity_tb_val, precision=1, commas=False)
    multimodal_images_tb_str       = fmt(multimodal_images_tb_val, precision=1, commas=False)
    multimodal_text_tb_str         = fmt(multimodal_text_tb_val, precision=1, commas=False)
    multimodal_audio_tb_str        = fmt(multimodal_audio_tb_val, precision=1, commas=False)
    multimodal_total_tb_str        = fmt(multimodal_total_tb_val, precision=1, commas=False)
    multimodal_excess_tb_str       = fmt(multimodal_excess_tb_val, precision=1, commas=False)
    archive_lineage_tb_str         = fmt(archive_lineage_tb_val, precision=1, commas=False)
```
...
class ShardContention:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    sc_n_workers_str = fmt(n_workers, precision=1, commas=False)
    sc_n_shards_str = fmt(n_shards, precision=1, commas=False)
...
class TextImageBandwidth:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    text_bw_mb_str = fmt(text_bw_mb_s, precision=1, commas=False)
    image_bw_gb_str = fmt(image_bw_gb_s, precision=1, commas=False)
    ratio_rounded_str = fmt(ratio_rounded_val, precision=1)
...
class HbmMemoryBudget:
    ...
    total_range_str = MarkdownStr(f"{fmt(total_min_gb, precision=1, commas=False)}--{fmt(total_max_gb, precision=1, commas=False)}")
    total_pct_range_str = MarkdownStr(f"{fmt(total_min_pct, precision=1, commas=False)} to {fmt(total_max_pct, precision=1, commas=False)}")
```
...
class DRAMInterconnect:
    ...
    pcie5_bw_gbs_str  = fmt(pcie5_val, precision=1, commas=False)
    nvlink_bw_gbs_str = fmt(nvlink_val, precision=1, commas=False)
```
...
class NVMeTierCalcs:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    raw_bw_str             = fmt(raw_bw_val, precision=1, commas=False)
    hdd_slowdown_factor_str = fmt(hdd_slowdown_val, precision=1, commas=False)
    hdd_util_ceiling_pct_str = fmt(hdd_util_ceiling_pct_val, precision=1, commas=False)
...
class LocalCacheRoi:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    lc_compute_ms_str = fmt(t_compute_ms, precision=1, commas=False)
    lc_remote_io_ms_str = fmt(t_io_remote_ms, precision=1, commas=False)
...
class ObjectStorageCost:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    s3_annual_str   = fmt(s3_annual_val, precision=1)
    nvme_annual_str = fmt(nvme_annual_val, precision=1)
```
...
class GlacierCost:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    glacier_annual_str = fmt(glacier_annual_val, precision=1)
```
...
class PipelineCalcs:
    ...
    data_stall_pct_str  = fmt(data_stall_pct_val, precision=1, commas=False)
    t_comp_stall_str    = fmt(t_comp_val, precision=1, commas=False)
    t_io_stall_str      = fmt(t_io_val, precision=1, commas=False)
...
class HashValidationCost:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    dataset_tb_str = fmt(dataset_tb, precision=1, commas=False)
...
class GDSLatency:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    gds_trad_us_str   = fmt(gds_trad_us, precision=1, commas=False)
    gds_bypass_us_str = fmt(gds_bypass_us, precision=1, commas=False)
    gds_speedup_str   = fmt(gds_speedup_val, precision=1, commas=False)
    gds_latency_reduction_pct_str = fmt(gds_latency_reduction_pct_val, precision=1, commas=False)
    gds_total_images_per_s_str = fmt(total_images_per_s, precision=1)
    gds_trad_cpu_seconds_str = fmt(gds_trad_cpu_seconds_val, precision=2, commas=False)
    gds_trad_cpu_cores_str = fmt(round(gds_trad_cpu_seconds_val), precision=1, commas=False)
    gds_bypass_cpu_seconds_str = fmt(gds_bypass_cpu_seconds_val, precision=2, commas=False)
    gds_freed_cpu_cores_str = fmt(round(gds_freed_cpu_cores_val), precision=1, commas=False)
```
...
class EconRatios:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    tier_cost_ratio_str = fmt(tier_cost_ratio_val, precision=1, commas=False)
    egress_100tb_str    = fmt(egress_100tb_val, precision=1)
...
class BuildVsBuyStorageEconomics:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    cloud_storage_annual_str = fmt(cloud_storage_annual, precision=1)
    cloud_egress_annual_str = fmt(cloud_egress_annual, precision=1)
    cloud_total_annual_str = fmt(cloud_total_annual, precision=1)
    annual_ops_str = fmt(annual_ops, precision=1)
    net_annual_savings_str = fmt(net_annual_savings, precision=1)
...
class CheckpointStormDefinition:
    ...
    weights_gb_str = fmt(weights_gb, precision=1, commas=False)
    gradients_gb_str = fmt(gradients_gb, precision=1, commas=False)
    master_weights_gb_str = fmt(master_weights_gb, precision=1, commas=False)
    adam_moments_gb_str = fmt(adam_moments_gb, precision=1, commas=False)
    optimizer_state_gb_str = fmt(optimizer_state_gb, precision=1, commas=False)
    ...
    naive_total_tb_str = fmt(naive_total_tb, precision=1, commas=False)
    naive_write_s_str = fmt(naive_write_s, precision=1)
    naive_write_min_str = fmt(naive_write_min, precision=1, commas=False)
```
...
class CheckpointStormCalc:
    ...
    local_bw_gbs_str = fmt(local_bw_gbs, precision=1, commas=False)
...
class SyntheticStorage:
    ...
    verification_passes_str = fmt(verification_passes, precision=1, commas=False)
    provenance_overhead_pct_str = fmt(provenance_metadata_overhead * 100, precision=1, commas=False)
