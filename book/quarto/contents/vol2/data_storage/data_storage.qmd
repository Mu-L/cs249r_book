...
class StorageHierarchyTable:
    # ┌── 4. OUTPUT ───────────────────────────────────────
    h100_bw_tbs_str = fmt(Hardware.Cloud.H100.memory_bw.m_as(TB/second), precision=2, commas=False, suffix=' TB/s')
```
...
class TextImageBandwidth:
    ...
    step_ms_str = fmt(step_s * 1000, precision=0, commas=False, suffix=' ms')
...
class ObjectStreamContext:
    ...
    tokenized_shard_count_val = round(tokenized_text_tb_val * 1000 / tokenized_shard_gb_val)
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    text_bw_mb_str = fmt(2048 * 4096 * token_id_bytes / 0.2 / 1_000_000, precision=1, commas=False, suffix=' MB/s')
    tokenized_shard_count_str = fmt(tokenized_shard_count_val, precision=0)
    tokenized_shard_gb_str = fmt(tokenized_shard_gb_val, precision=0, commas=False, suffix=' GB')
```
...
class CheckpointStormCalc:
    ...
    pfs_aggregate_bw_tbs_str = fmt(pfs_aggregate_bw_gbs / 1000, precision=0, commas=False, suffix=" TB/s")
    checkpoint_interval_min_str = fmt(checkpoint_interval_min, precision=0, commas=False, suffix=" minutes")
...
3. **Per-node PFS bandwidth**\index{Bandwidth!per-node pfs}: If all `{python} CheckpointStormCalc.nodes_str` write simultaneously, each gets `{python} CheckpointStormCalc.pfs_aggregate_bw_gbs_str``{python} CheckpointStormCalc.nodes_str` = `{python} CheckpointStormCalc.per_node_pfs_bw_gbs_str`.
