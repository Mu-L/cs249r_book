...
# │           NVLINK_A100_BW, NVLINK_H100_BW, GB, second)
...
class DistTrainConstants:
    ...
    ib_ndr_gbs = (INFINIBAND_NDR_BW / 8).m_as(GB/second)  # 400 Gbps -> 50 GB/s per port

    # ┌── 2. EXECUTE (The Compute) ─────────────────────────
    # (direct unit conversions only)
...
Before optimizing algorithms, we must understand the physical constraints of the Machine Learning Fleet. The performance of any distributed training job is governed by the iron law of scale introduced in @sec-vol2-introduction-fleet-law:
...
::: {#fig-distributed-training fig-env="figure" fig-pos="htb" fig-cap="**Distributed Training Schedule**: Forward and backward microbatches scheduled across pipeline stages (F1B1 pattern). The schedule preserves the dependency between forward and backward passes while exposing the idle bubbles that pipeline parallelism must amortize. Data parallelism (the simpler partitioning strategy described in the surrounding prose) replicates the model across GPUs and uses AllReduce on gradients; pipeline parallelism, depicted here, partitions the model itself across stages." fig-alt="Pipeline schedule diagram showing forward and backward microbatches across multiple pipeline stages in an F1B1 pattern, with idle bubbles between dependent operations."}
![](images/svg/data-parallel-pipeline.svg)
:::
...
- **Naive single-stream baseline**: A flat 3 GB transfer over one InfiniBand link takes $3 \text{ GB} / 25 \text{ GB/s} = 120\text{ ms}$, and a flat ring across all 128 nodes pays $2(N-1)/N \times 3 \text{ GB} / 25 \text{ GB/s} \approx 240\text{ ms}$. A hierarchical AllReduce (intra-node ring over NVLink + inter-node ring over InfiniBand) brings the inter-node cost down to roughly 80 ms because each node only ships its 1/8 share over the slow link.
...
- **Diagnosis check**: A flat single ring over all 1,024 GPUs would have measured close to 240 ms, so the algorithm is already hierarchical
...
**Root Cause Diagnosis**: NCCL is already using a hierarchical algorithm (100 ms vs. the 240 ms flat-ring prediction confirms this). The remaining 20 ms gap between observed and modeled latency comes from switch congestion on a handful of inter-node uplinks, not from a suboptimal algorithm.

**Solution**: Monitor InfiniBand switch port utilization to identify hot spots. Consider the rail-optimized topology in @sec-network-fabrics-rail-optimized or further tuning of the hierarchical AllReduce hand-off so it explicitly partitions intra-node (NVLink) from inter-node (InfiniBand) communication. The residual 20 ms gap likely represents achievable optimization through better network provisioning rather than algorithm changes.
...
For a model with $N_{\text{layers}}$ layers, FSDP performs $2 N_{\text{layers}}$ collective operations per training step vs. 1 AllReduce for DDP. However, FSDP enables overlapping: while layer $i$ computes, layer $i+1$ can prefetch parameters.
...
Even with ZeRO-3 fully deployed, sharding optimizer states, gradients, and parameters across workers, some architectures remain intractable. As @fig-model-parallel-flow shows, model parallelism addresses this by partitioning the model itself across devices using two complementary strategies. For a `{python} FrontierTrainingContext.frontier_params_b_str`B parameter model, weights alone occupy 350 GB, or about 5.5 GB per GPU across 64 GPUs. Full Adam training state is much larger: 2,450 GB globally, or roughly 38 GB per GPU before activations. This distinction matters because optimizer sharding reduces static state, but it does not eliminate the activation and per-layer capacity constraints that force model parallelism.
...
class MemoryWallAnalysis:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    param_gb = params_b * bytes_fp16
    optim_gb = params_b * bytes_adam
    total_static_gb = param_gb + optim_gb
    per_gpu_static = total_static_gb/n_gpus
    total_per_gpu = int(per_gpu_static) + activation_approx_gb

    # ┌── 3. GUARD (Invariants) ───────────────────────────────────────────
    check(param_gb == 350, f"param storage ({param_gb})")
    check(optim_gb == 2100, f"optim storage ({optim_gb})")
    check(total_static_gb == 2450, f"total static ({total_static_gb})")
    check(int(per_gpu_static) == 38, f"per-GPU static ({per_gpu_static:.1f})")
    check(total_per_gpu == 88, f"total per GPU ({total_per_gpu})")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    per_gpu_str = fmt(int(per_gpu_static), precision=0, commas=False)
    activation_str = fmt(activation_approx_gb, precision=0, commas=False)
    total_per_gpu_str = fmt(total_per_gpu, precision=0, commas=False)
```
...
1.  **Parameter storage**: `{python} FrontierTrainingContext.frontier_params_b_str`B params $\times$ 2 bytes (FP16) = **350 GB**.
2.  **Optimizer state**: `{python} FrontierTrainingContext.frontier_params_b_str`B params $\times$ 12 bytes (Adam FP32) = **2,100 GB**.
3.  **Total static memory**: 2,450 GB.
4.  **ZeRO-3 Sharding**: With 64 GPUs, per-GPU static memory = $2{,}450/64 \approx$ **`{python} MemoryWallAnalysis.per_gpu_str` GB**.
...
**Systems conclusion**: 38 GB (Static) + 50 GB (Dynamic) = **88 GB**. This exceeds the `{python} A100CapacityContext.a100_mem_str` GB capacity of the A100. Even with full ZeRO-3 sharding, **pure Data Parallelism fails**. The only recourse is **Tensor Parallelism** to split the layers themselves.
...
That optimized split turns the review question from "what are the axes?" into "which physical constraint does each axis satisfy?"
...
class RLHFBudget:
    ...
    remaining_str = fmt(int(remaining), precision=0, commas=False)
    kv_total_gb_str = RLHFKVCache.kv_total_gb_str
    kv_per_gpu_str = fmt(kv_per_gpu, precision=1, commas=False)
    remaining_kv_str = fmt(int(remaining_after_kv), precision=0, commas=False)
```
