...
class DistTrainConstants:
    ...
    h100_mem_str = fmt(h100_mem_gb, precision=1, allow_zero=True, commas=False)
    nvlink_a100_str = fmt(nvlink_a100_gbs, precision=1, allow_zero=True, commas=False)
    nvlink_h100_str = fmt(nvlink_h100_gbs, precision=1, allow_zero=True, commas=False)
    ib_hdr_str = fmt(ib_hdr_gbs, precision=1, allow_zero=True, commas=False)
    ib_ndr_str = fmt(ib_ndr_gbs, precision=1, allow_zero=True, commas=False)
    nvlink_hdr_ratio_low_str = fmt(nvlink_hdr_ratio_low, precision=1, allow_zero=True, commas=False)
    nvlink_hdr_ratio_high_str = fmt(nvlink_hdr_ratio_high, precision=1, allow_zero=True, commas=False)
    reliability_node_count_str = fmt(reliability_node_count, precision=1, allow_zero=True, commas=False)
    ...
    gpt4_mem_tb_str = fmt((Models.Language.GPT4.parameters.m_as('param') * 2 / 1e12), precision=1, commas=False)
...
class FrontierTrainingContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    frontier_params_b_str = fmt(params_b, precision=1, allow_zero=True)
...
class HierarchicalAllReduceDebug:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────────────────────
    from mlsysim.core.formulas import calc_hierarchical_allreduce_time, calc_ring_allreduce_time
    tensor_bytes = 3.0 * ureg.GB
    nodes = 128
    gpus_per_node = 8
    ib_hdr_bw = 25.0 * ureg.GB / ureg.second
    nvlink_bw = DistTrainConstants.nvlink_a100_gbs * ureg.GB / ureg.second
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    flat_ring_ms_str = fmt(flat_ring_ms, precision=1, allow_zero=True, commas=False)
    inter_node_ms_str = fmt(inter_node_ms, precision=1, allow_zero=True, commas=False)
    hierarchical_total_ms_str = fmt(hierarchical_total_ms, precision=1, allow_zero=True, commas=False)
    observed_ms_str = fmt(observed_ms, precision=1, allow_zero=True, commas=False)
    residual_ms_str = fmt(residual_ms, precision=1, allow_zero=True, commas=False)
```
...
class Scaling8GPU:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    total_8gpu_str = fmt(total_8gpu_ms_val, precision=1, allow_zero=True, commas=False)
    ...
    efficiency_8gpu_str = fmt(efficiency_8gpu_val, precision=1, allow_zero=True, commas=False)
    ...
    nvlink_h100_str = fmt(nvlink_bw.m_as(GB/second), precision=1, allow_zero=True, commas=False)
    ...
    compute_8gpu_ms_str = fmt(compute_8gpu_ms, precision=1, allow_zero=True, commas=False)
    # Suffix-bearing prose aliases for raw ints/floats referenced in prose
    batch_per_gpu_str = fmt(batch_per_gpu_val, precision=1, allow_zero=True, commas=False)
    training_hours_1gpu_str = fmt(training_hours_1gpu, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (NETWORK_10G_BW, GB, second, ...)
...
class Scaling32GPU:
    ...
    params_b = Models.Language.GPT2.parameters.m_as(Bparam)  # GPT-2
    net_bw = NETWORK_10G_BW.m_as(GB/second) # 10 Gbps = 1.25 GB/s (Commodity network)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compute_pct_str = fmt(compute_pct_val, precision=1, allow_zero=True, commas=False)
    comm_pct_str = fmt(comm_pct_val, precision=1, allow_zero=True, commas=False)
    total_32gpu_str = fmt(total_32gpu_ms_val, precision=1, allow_zero=True, commas=False)
    ...
    efficiency_32gpu_str = fmt(efficiency_32gpu_val, precision=1, allow_zero=True, commas=False)
    training_32gpu_str = fmt(training_32gpu_hours_val, precision=1, commas=False)
    gpt2_sync_size_gb_str = fmt(sync_size_gb, precision=1, commas=False)
    inter_node_ms_str = fmt(inter_node_ms_val, precision=1, allow_zero=True, commas=False)
    ib_hdr_str = fmt(net_bw, precision=2, commas=False)
    # compute_32gpu_ms int "1800" — preserve original (no commas) via MarkdownStr
    compute_32gpu_ms_str = fmt(compute_32gpu_ms, precision=1, allow_zero=True, commas=False)
    comm_32gpu_ms_str = fmt(comm_32gpu_ms_val, precision=1, allow_zero=True, commas=False)
```
...
class GradAccumScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    effective_batch_str = fmt(effective_batch_val, precision=1, allow_zero=True)
    ...
    baseline_cost_str = fmt(baseline_cost_val, precision=1, allow_zero=True)
    ga_cost_str = fmt(ga_cost_val, precision=1, allow_zero=True)
    ga_savings_str = fmt(ga_savings_val, precision=1, allow_zero=True)
    ga_savings_pct_str = fmt(ga_savings_pct_val, precision=1, allow_zero=True, commas=False)
    # Suffix-bearing prose aliases for raw ints referenced in prose
    n_gpus_ga_str = fmt(n_gpus_ga, precision=1, allow_zero=True, commas=False)
...
class ScalingWorkers:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    batch_8_str = fmt(batch_8, precision=1, allow_zero=True)
    batch_64_str = fmt(batch_64, precision=1, allow_zero=True)
    ...
    base_iters_k_str = fmt(int(base_iters // THOUSAND), precision=1, allow_zero=True, commas=False, suffix="K")
    ...
    critical_batch_str = fmt(critical_batch_size, precision=1, allow_zero=True)
```
...
class ModelParallelMemoryFacts:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    weight_gb_str = fmt(weight_gb, precision=1, allow_zero=True, commas=False)
    weight_per_gpu_str = fmt(weight_per_gpu_gb, precision=1, commas=False)
    full_state_gb_str = fmt(full_state_gb, precision=1, allow_zero=True)
...
class A100CapacityContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_str = fmt(mem_gb, precision=1, allow_zero=True, commas=False)
...
class MemoryWallAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    param_gb_str = fmt(param_gb, precision=1, allow_zero=True, commas=False)
    gradient_gb_str = fmt(gradient_gb, precision=1, allow_zero=True, commas=False)
    optim_gb_str = fmt(optim_gb, precision=1, allow_zero=True)
    total_static_gb_str = fmt(total_static_gb, precision=1, allow_zero=True)
    per_gpu_str = fmt(per_gpu_static, precision=1, commas=False)
    activation_str = fmt(activation_approx_gb, precision=1, allow_zero=True, commas=False)
...
class PipelineBubble:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    pb_stages_str = fmt(p_stages, precision=1, allow_zero=True, commas=False)
    pb_microbatches_str = fmt(m_microbatches, precision=1, allow_zero=True, commas=False)
...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.memory.bandwidth, NVLINK_H100_BW, TB, GB, second)
# │ Imports: mlsysim.book (fmt)
...
class FlashAttentionOverlapScenario:
    ...
    nvlink_h100_str = fmt(nvlink_bw, precision=1, allow_zero=True, commas=False)
```
...
class TensorParallel3DTraffic:
    ...
    # ┌── 4. OUTPUT ───────────────────────────────────────
    tp_str = fmt(tp, precision=1, allow_zero=True, commas=False)
    pp_str = fmt(pp, precision=1, allow_zero=True, commas=False)
    dp_str = fmt(dp, precision=1, allow_zero=True, commas=False)
    total_gpus_str = fmt(total_gpus, precision=1, allow_zero=True)
    activation_payload_mb_str = fmt(activation_payload_bytes / 1_000_000, precision=1, allow_zero=True, commas=False)
...
class HybridMemoryAccounting:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    weights_gb_str = fmt(weights_gb, precision=1, allow_zero=True)
    ...
    tp_only_static_gb_str = fmt(tp_only_static_gb, precision=1, allow_zero=True, commas=False)
...
class ThreeDParallelism:
    ...
    pp_h100 = 16
    dp_h100 = 128
    nvlink5_bw_tbs = NVLINK_B200_BW.m_as(TB/second)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    total_gpus_h100_str = fmt(total_gpus_h100, precision=1, allow_zero=True)
...
class AutoParallelism:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    opt_tp_str = fmt(best_tp, precision=1, allow_zero=True, commas=False)
    opt_pp_str = fmt(best_pp, precision=1, allow_zero=True, commas=False)
    opt_dp_str = fmt(best_dp, precision=1, allow_zero=True, commas=False)
...
class RLHFKVCache:
    ...
    hidden_dim_str = fmt(hidden_dim, precision=1, allow_zero=True)
    ...
    seq_len_str = fmt(seq_len, precision=1, allow_zero=True)
    batch_size_str = fmt(batch_size, precision=1, allow_zero=True, commas=False)
    bytes_per_element_str = fmt(bytes_per_element, precision=1, allow_zero=True, commas=False)
    kv_total_gb_str = fmt(kv_total_gb, precision=1, allow_zero=True, commas=False)
    dense_total_gb_str = fmt(dense_total_gb, precision=1, allow_zero=True, commas=False)
```
...
class RLHFBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    policy_mem_str = fmt(policy_mem, precision=1, allow_zero=True)
    ref_mem_str = fmt(ref_mem, precision=1, allow_zero=True, commas=False)
    n_gpus_str = fmt(n_gpus, precision=1, allow_zero=True, commas=False)
    policy_per_gpu_str = fmt(policy_per_gpu, precision=1, allow_zero=True, commas=False)
    ref_per_gpu_str = fmt(ref_per_gpu, precision=1, commas=False)
    ref_per_gpu_sep_str = fmt(ref_per_gpu_sep, precision=1, commas=False)
    reward_mem_str = fmt(reward_mem, precision=1, allow_zero=True, commas=False)
    value_mem_str = fmt(value_mem, precision=1, allow_zero=True, commas=False)
    ppo_total_mem_str = fmt(ppo_total_mem, precision=1, allow_zero=True)
    dpo_total_mem_str = fmt(dpo_total_mem, precision=1, allow_zero=True)
    dpo_reduction_pct_str = fmt_percent(dpo_reduction_pct, precision=1)
...
class YoungDaly:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    t_opt_min_str = fmt(t_opt_min_val, precision=1, allow_zero=True)
    ...
    daily_savings_str = fmt(diff_daily, precision=1, allow_zero=True, commas=True)
    cluster_mtbf_hr_str = fmt(cluster_mtbf_hr, precision=1, allow_zero=True)
    sparse_interval_hr_str = fmt(sparse_interval_hr_val, precision=1, allow_zero=True)
```
