...
|:----------|:-------------------------------|-----------------------------------------------------------------------:|:-----------------------|:--------------------------------------|
| **BSP**   | $\tau_{\text{stale}} = 0$      |                                            $\mathcal{O}(1/\sqrt{NbT})$ | Full ($1/N$)           | Final training, reproducibility       |
| **SSP**   | $\tau_{\text{stale}} \leq s$   |                    $\mathcal{O}(1/\sqrt{NbT}) + O(s^2\eta^2 L_s^2/Nb)$ | Partial                | Heterogeneous clusters                |
| **ASP**   | $\tau_{\text{stale}} = \infty$ | $\mathcal{O}(1/\sqrt{T}) + O(\bar{\tau}_{\text{stale}}^2\eta^2 L_s^2)$ | None                   | Maximum throughput, early exploration |
...
class YoungDaly:
    ...
    num_gpus = 1024
    gpu_cost_hr = 2.0
    # System MTBF derived from canonical per-GPU MTBF: MTBF_system = MTBF_GPU/N.
