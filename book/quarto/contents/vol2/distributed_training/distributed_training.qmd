...
$$ T_{\text{step}}(N) = \underbrace{\frac{T_{\text{compute}}}{N}}_{\text{FLOP/s Term}} + \underbrace{T_{\text{comm}}(N)}_{\text{Bandwidth Term}} + \underbrace{T_{\text{sync}}(N)}_{\text{Coordination Term}} - T_{\text{overlap}} $$

Where:

*   **FLOP/s Term** $(T_{\text{compute}})$: The total computation required for the batch. In an ideal world, this scales perfectly with $N$.
*   **Bandwidth Term** $(T_{\text{comm}})$: The time spent moving data. This is governed by the iron law's data-movement term, $D_{\text{vol}}/\text{BW}$. For Ring AllReduce, this term is $\frac{2(N-1)}{N} \times \frac{M}{\text{BW}_{\text{net}}}$, where $M$ is the communicated gradient or model-state bytes and $\text{BW}_{\text{net}}$ is network bandwidth.
*   **Coordination Term** $(T_{\text{sync}})$: The nonoverlapped cost of barriers, ordering, and straggler waiting.
...
1.  **Significance (quantitative)**: It represents the **Synchronization Error** in distributed optimization. Increasing $\tau_{\text{stale}}$ can improve throughput by reducing barrier waits $(L_{\text{lat}})$, but it typically degrades the **Rate of Convergence**, requiring more operations $(O)$ to reach the same accuracy.
...
$$
\mathbb{E}[\mathcal{L}(\theta_T)] - \mathcal{L}^{\star} \leq O\left(\frac{1}{\sqrt{T}}\right) + O\left(\bar{\tau}_{\text{stale}}^2 \eta^2 L_s^2\right)
$$
...
|:----------|:-------------------------------|-------------------------------------------------------------:|:-----------------------|:--------------------------------------|
| **BSP**   | $\tau_{\text{stale}} = 0$      |                                            $O(1/\sqrt{NbT})$ | Full ($1/N$)           | Final training, reproducibility       |
| **SSP**   | $\tau_{\text{stale}} \leq s$   |                    $O(1/\sqrt{NbT}) + O(s^2\eta^2 L_s^2/Nb)$ | Partial                | Heterogeneous clusters                |
| **ASP**   | $\tau_{\text{stale}} = \infty$ | $O(1/\sqrt{T}) + O(\bar{\tau}_{\text{stale}}^2\eta^2 L_s^2)$ | None                   | Maximum throughput, early exploration |
...
Here $\lambda$ is the optimizer's weight-decay coefficient in the LARS/LAMB update, not the failure-rate parameter used in reliability analysis.
...
class YoungDaly:
    ...
    num_gpus = 1024
    gpu_cost_hr = 2.0
    # Cluster MTBF derived from canonical per-GPU MTBF: MTBF_cluster = MTBF_GPU/N.
