...
class DistTrainConstants:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────
    a100_mem_str = f"{a100_mem_gb:.0f}"
    h100_mem_str = f"{h100_mem_gb:.0f}"
    nvlink_a100_str = f"{nvlink_a100_gbs:.0f}"
    nvlink_h100_str = f"{nvlink_h100_gbs:.0f}"
    ib_ndr_str = f"{ib_ndr_gbs:.0f}"
    gpt4_mem_tb_str = f"{(GPT4_EST_PARAMS.m_as('param') * 2 / 1e12):.1f}"
    dlrm_mem_tb_str = "10"
```
...
::: {#def-distributed-training-distributed-training .callout-definition title="Distributed training"}
...
::: {#def-distributed-training-data-parallelism .callout-definition title="Data parallelism"}
...
# │          Scaling8GPU.gpt2_sync_size_gb_str, Scaling8GPU.nvlink_h100_gbs,
# │          Scaling8GPU.comm_8gpu_ms_str, Scaling8GPU.compute_8gpu_ms_str,
# │          Scaling8GPU.batch_per_gpu_val, Scaling8GPU.single_gpu_step_s,
# │          Scaling8GPU.training_hours_1gpu, Scaling8GPU.total_8gpu_s_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
...
class Scaling8GPU:
    ...
    total_8gpu_s_str = f"{total_8gpu_s_val:.3f}"
...
# │          Scaling32GPU.inter_node_ms_str, Scaling32GPU.ib_hdr_gbs,
...
class Scaling32GPU:
    ...
    total_32gpu_s_str = f"{total_32gpu_s_val:.3f}"
    ...
    ib_hdr_gbs = f"{net_bw:.2f}"
    compute_32gpu_ms_str = f"{compute_32gpu_ms}"
    comm_32gpu_ms_str = f"{comm_32gpu_ms_val:.0f}"
```
...
class GradAccumScenario:
    ...
    comm_8gpu_ms_str = f"{comm_8gpu_ms:.1f}"
    comm_overhead_pct_str = f"{overhead_val:.2f}"
    ga_training_hours_str = f"{ga_training_hours_val:.1f}"
    baseline_cost_str = f"{baseline_cost_val:,.0f}"
...
- Batch size: `{python} Scaling8GPU.batch_per_gpu_val` (with gradient checkpointing, fits in 32 GB)
- Time per step: `{python} Scaling8GPU.single_gpu_step_s` seconds
- Time to 50K steps: **`{python} Scaling8GPU.training_hours_1gpu` hours**
...
- Gradient synchronization: `{python} Scaling8GPU.gpt2_sync_size_gb_str` GB @ `{python} Scaling8GPU.nvlink_h100_gbs` GB/s (NVLink) $\approx$ `{python} Scaling8GPU.comm_8gpu_ms_str` ms
...
- Inter-node communication: `{python} Scaling8GPU.gpt2_sync_size_gb_str` GB @ `{python} Scaling32GPU.ib_hdr_gbs` GB/s (10GbE) $\approx$ `{python} Scaling32GPU.inter_node_ms_str` ms
...
The full configuration is: `{python} GradAccumScenario.n_gpus_ga` GPUs $\times$ batch `{python} GradAccumScenario.batch_per_gpu` $\times$ `{python} GradAccumScenario.accum_steps` accumulation steps = `{python} GradAccumScenario.effective_batch_str` effective batch, with communication overhead `{python} GradAccumScenario.comm_8gpu_ms_str` ms ÷ (`{python} GradAccumScenario.accum_steps` $\times$ `{python} GradAccumScenario.compute_8gpu_ms` ms) = `{python} GradAccumScenario.comm_overhead_pct_str` percent. Training time is `{python} GradAccumScenario.ga_training_hours_str` hours, costing USD `{python} GradAccumScenario.ga_cost_str` vs. USD `{python} GradAccumScenario.baseline_cost_str` for the 32-GPU commodity-network run—a USD `{python} GradAccumScenario.ga_savings_str` savings (`{python} GradAccumScenario.ga_savings_pct_str` percent reduction) at the cost of a modestly longer wall-clock run.
...
::: {#thm-distributed-training-convergence-rate-distributed-sgd .callout-theorem title="Convergence rate for distributed SGD"}
...
::: {#def-distributed-training-gradient-staleness .callout-definition title="Gradient staleness"}
...
::: {#def-distributed-training-critical-batch-size .callout-definition title="Critical batch size"}
...
class ScalingWorkers:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    batch_8_str = f"{batch_8:,}"
    batch_64_str = f"{batch_64:,}"
    exp_iter_8_str = f"{exp_iter_8/THOUSAND:.1f}K"
    exp_iter_64_str = f"{exp_iter_64/THOUSAND:.2f}K"
    speedup_8_str = f"{speedup_8:.1f}"
    speedup_64_str = f"{speedup_64:.1f}"
    speedup_ssp_str = f"{speedup_ssp:.1f}"
    lr_ssp_str = f"{lr_factor_ssp:.2f}"
    base_iters_k_str = f"{int(base_iters // THOUSAND)}K"
    base_batch_str = str(b)
    n_workers_8_str = str(n8)
    n_workers_64_str = str(n64)
    ssp_staleness_str = str(staleness)
    critical_batch_str = f"{critical_batch_size:,}"
```
...
class MemoryWallAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    per_gpu_str = f"{int(per_gpu_static)}"
    activation_str = f"{activation_approx_gb}"
    total_per_gpu_str = f"{total_per_gpu}"
```

::: {#nbk-distributed-training-memory-wall-scale .callout-notebook title="The memory wall of scale"}

**Problem**: Training a **`{python} FrontierTrainingContext.frontier_params_b_str` Billion parameter** model (like GPT-3) on NVIDIA A100s (`{python} A100CapacityContext.a100_mem` GB). Can Data Parallelism with ZeRO-3 handle this?
...
**Systems conclusion**: 38 GB (Static) + 50 GB (Dynamic) = **88 GB**. This exceeds the `{python} A100CapacityContext.a100_mem` GB capacity of the A100. Even with full ZeRO-3 sharding, **pure Data Parallelism fails**. The only recourse is **Tensor Parallelism** to split the layers themselves.
...
Consider our running example: the `{python} FrontierTrainingContext.frontier_params_b_str`B parameter model requires 350 GB of memory in FP16, exceeding the `{python} A100CapacityContext.a100_mem` GB capacity of a single A100 by a factor of four. Model parallelism addresses this **capacity wall** by partitioning model weights across multiple devices, effectively stitching them into a single super-accelerator. Unlike data parallelism, where every GPU holds a full replica of the model and processes a unique fraction of the global batch, model parallelism requires each GPU to hold a unique fraction of the model and process the *same* data stream sequentially. With 8-way partitioning on A100s, BF16 weights alone occupy approximately 44 GB per GPU. That is only the weights budget: full training still needs activations, gradients, and optimizer state, so pure 8-way model parallelism is not sufficient by itself. Training requires additional sharding or offload for optimizer and gradient state, plus pipeline or hybrid parallelism to keep activation memory within capacity.
...
::: {#def-distributed-training-pipeline-parallelism .callout-definition title="Pipeline parallelism"}
...
class PipelineBubble:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    pb_stages_str = f"{p_stages}"
    pb_microbatches_str = f"{m_microbatches}"
    pb_bubble_pct_str = f"{bubble_pct:.1f}"
...
::: {#def-distributed-training-tensor-parallelism .callout-definition title="Tensor parallelism"}
...
class FlashAttentionOverlapScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_hbm_bw_str = f"{hbm_bw:.2f}"
    nvlink_h100 = f"{nvlink_bw:.0f}"
```

If the compute time for one tile exceeds the communication time for transferring one tile over NVLink, the communication is fully hidden. On an H100 with `{python} FlashAttentionOverlapScenario.h100_hbm_bw_str` TB/s HBM bandwidth and `{python} FlashAttentionOverlapScenario.nvlink_h100` GB/s NVLink bandwidth, this overlap is achievable for typical tile sizes.
...
class ThreeDParallelism:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    total_gpus_h100_str = f"{total_gpus_h100:,}"
    nvlink5_bw_str = f"{nvlink5_bw_tbs} TB/s"
```
...
class AutoParallelism:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    opt_tp_str = f"{best_tp}"
    opt_pp_str = f"{best_pp}"
    opt_dp_str = f"{best_dp}"
...
class RLHFKVCache:
    ...
    kv_heads_str = f"{kv_heads}"
    head_dim_str = f"{head_dim}"
    seq_len_str = f"{seq_len:,}"
    batch_size_str = f"{batch_size}"
    bytes_per_element_str = f"{bytes_per_element:.0f}"
    kv_total_gb_str = f"{kv_total_gb:.0f}"
    dense_total_gb_str = f"{dense_total_gb:.0f}"
```
...
class RLHFBudget:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    policy_mem_str = f"{policy_mem:,}"
    ref_mem_str = f"{ref_mem}"
    n_gpus_str = f"{n_gpus}"
    policy_per_gpu_str = f"{policy_per_gpu:.0f}"
    ref_per_gpu_str = f"{ref_per_gpu:.1f}"
    ref_per_gpu_sep_str = f"{ref_per_gpu_sep:.1f}"
    reward_mem_str = f"{reward_mem}"
    value_mem_str = f"{value_mem}"
    ppo_total_mem_str = f"{ppo_total_mem:,.0f}"
    dpo_total_mem_str = f"{dpo_total_mem:,.0f}"
    dpo_reduction_pct_str = fmt_percent(dpo_reduction_pct, precision=0)
    static_per_gpu_str = f"{static_per_gpu:.1f}"
    remaining_str = f"{int(remaining)}"
    kv_total_gb_str = RLHFKVCache.kv_total_gb_str
    kv_per_gpu_str = f"{kv_per_gpu:.1f}"
    remaining_kv_str = f"{remaining_after_kv:.0f}"
```
...
* **Tensor parallelism is node-local**: Because it partitions matrix operations, it requires the `{python} DistTrainConstants.nvlink_a100_str`-`{python} FlashAttentionOverlapScenario.nvlink_h100` GB/s bandwidth of NVLink. Scaling it across racks on standard Ethernet will stall the fleet.
