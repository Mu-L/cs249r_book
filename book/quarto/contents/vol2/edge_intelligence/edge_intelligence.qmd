...
- Quantify edge-device training overhead (memory amplification 4--12$\times$, compute 2--3$\times$, energy) relative to inference-only deployment
...
::: {#ws-edge-intelligence-device-keyboard-learning .callout-war-story title="On-device keyboard learning"}
...
* **The Mobile Memory Wall**\index{Mobile Memory Wall}: While NPUs provide raw compute (TOPS), on-device LLMs are strictly bandwidth-bound. The 30--50$\times$ gap between mobile RAM and data center HBM makes quantization a mandatory "bandwidth survival strategy" for interactive decode speeds.
...
* **Heterogeneity is the default**\index{Heterogeneity Is the Default}: Unlike uniform data center racks, the edge is a fragmented ecosystem of microcontrollers and SoCs. Federated learning must account for "stragglers" and participation bias generated by these physical disparities.
...
* **Edge is not a scaled-down cloud**\index{Edge Is Not a Scaled-Down Cloud}: Data center and edge deployments face qualitatively different constraints. Thermal throttling, intermittent connectivity, battery budgets, and the absence of centralized orchestration make edge deployment a fundamentally distinct engineering discipline.
