...
**The context**: A 2015 Tesla Model S operating with Traffic-Aware Cruise Control and Autosteer performed perception and control decisions locally on the vehicle, with the human driver expected to supervise.

**The failure**: In the May 7, 2016 Williston, Florida crash, the automation operated outside a safety envelope that reliably ensured driver engagement. The NTSB found the driver's overreliance on automation contributed to the fatal collision.

**The consequence**: The incident became a reference case for operational design domains, driver-monitoring requirements, and the limits of treating human takeover as a generic fallback for edge autonomy.

**The systems lesson**: Edge intelligence is a closed-loop system in a physical environment. Latency, perception, user attention, and handoff design must be engineered together; local inference alone does not make a deployment safe [@ntsb2017tesla].
...
The heterogeneity factor $\beta$ quantifies the divergence between local and global optima. Formally, $\beta^2 = \frac{1}{K}\sum_{k=1}^{K} \|\nabla F_k(\theta) - \nabla F(\theta)\|^2$ measures how much local gradients disagree at the global optimum [@li2020; @zhao2018federated]. When data is IID, $\beta = 0$ and the bound reduces to the standard $\mathcal{O}(1/\sqrt{CER})$ rate. When data is highly heterogeneous, $\beta$ can exceed 1, significantly slowing convergence.
