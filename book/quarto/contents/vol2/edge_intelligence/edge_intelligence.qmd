...
class EdgeIntelligenceSetup:
    ...
    h100_mem_bw_gbs = f"{int(h100_mem_bw_raw.m_as(GB/second)):,}"
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    # Outputs are computed above as formatted strings.
```
...
class EdgeNpuSpeedup:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    npu_cpu_lat_ms_str = f"{t_cpu_ms}"
    npu_lat_ms_str = f"{t_npu_ms}"
    npu_cpu_energy_j_str = f"{e_cpu_j:g}"
    npu_energy_j_str = f"{e_npu_j:g}"
    npu_speedup_str = f"{speedup:.0f}"
    npu_energy_gain_str = f"{energy_gain:.0f}"
...
Memory consumption during training is not static. It fluctuates dynamically, reaching a maximum during the backward pass when activations, gradients, and optimizer states must coexist. This peak memory usage determines whether a model can be trained on a device. For a 10M parameter model on a smartphone with `{python} EdgeIntelligenceSetup.smartphone_ram_gb` GB RAM, the 40 MB of FP32 weights might spike to over 200 MB during backpropagation as activations, gradients, and optimizer states accumulate---competing directly with the operating system and foreground applications for the device's limited memory. Techniques like gradient checkpointing mitigate this by discarding intermediate activations during the forward pass and recomputing them on-demand during the backward pass. This approach trades increased computation (20 to 30 percent) for a dramatic reduction in peak memory, often achieving 3--4$\times$ reduction.
...
[^fn-neural-engine-evolution]: **Apple Neural Engine**\index{Apple Neural Engine}: From the A11 (0.6 TOPS) to high-end mobile NPUs in the tens of TOPS, with `{python} EdgeIntelligenceSetup.mobile_npu_tops` TOPS used here as a generic reference point, mobile neural acceleration has improved by roughly two orders of magnitude. The systems consequence is not just lower inference latency: compact personalization jobs can move from CPU-bound foreground work to short accelerator-assisted bursts that fit within strict thermal and battery policies. \index{Neural Engine!on-device training}
...
- **Data Center (H100)**: HBM3 provides `{python} EdgeIntelligenceSetup.h100_mem_bw_gbs` GB/s bandwidth.
- **Flagship Mobile (LPDDR5X-class)**: mobile DRAM provides roughly 64–100 GB/s bandwidth.
...
class EdgeBattery:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    battery_power_w_str = f"{p_train_w:.1f}"
    battery_duration_min_str = f"{t_train_min}"
    battery_duration_h_str = f"{t_train_h:.1f}"
    battery_energy_wh_str = f"{energy_wh:.2f}"
    battery_drain_pct_str = f"{drain_pct:.1f}"
    battery_total_wh_str = f"{battery_wh}"
...
Recent mobile accelerators span a wide range of capabilities and optimization targets. Flagship-class mobile NPUs provide tens of TOPS of peak performance, with `{python} EdgeIntelligenceSetup.mobile_npu_tops` TOPS serving here as a generic high-end reference point rather than a single named chip. Apple's Neural Engine is optimized primarily for CoreML inference patterns with limited public training support, making it better suited for inference-heavy adaptation techniques. Qualcomm Hexagon-class DSPs\index{Hexagon DSP!mixed-precision training} provide programmable vector and mixed-precision paths, while Google Tensor-class accelerators are tightly integrated with TensorFlow Lite and Android federated learning infrastructure. The energy-efficiency comparison reveals why dedicated neural processing units are essential: NPUs can provide order-of-magnitude better neural-network efficiency than general-purpose CPUs, making the difference between feasible and infeasible on-device adaptation.
...
class StorageWall:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ls_contexts_str = f"{n_contexts}"
    ls_full_mb_str = f"{size_full_mb:.0f}"
    ls_adapter_kb_str = f"{size_adapter_kb:.0f}"
    ls_adapter_mb_str = f"{size_adapter_mb:.1f}"
    ls_savings_ratio_str = f"{per_context_savings:.0f}"
    ls_total_full_mb_str = f"{total_full_mb:.0f}"
    ls_total_adapter_mb_str = f"{total_adapter_mb:.0f}"
    ls_total_savings_ratio_str = f"{total_storage_savings:.1f}"
...
class FederatedSavings:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    fs_raw_data_mb_str = f"{raw_data_mb:.0f}"
    fs_update_mb_str = f"{update_mb:.1f}"
    fs_reduction_str = f"{reduction:.0f}"
...
class FederatedConvergence:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    fc_total_clients_str = f"{total_clients}"
    fc_clients_per_round_str = f"{clients_per_round}"
    fc_local_epochs_str = f"{local_epochs}"
    fc_reduced_local_epochs_str = f"{reduced_local_epochs}"
    fc_epsilon_str = f"{epsilon:.2f}"
    fc_epsilon_sq_str = f"{epsilon_sq:.4f}"
    fc_sigma_str = f"{sigma}"
    fc_beta_iid_str = f"{beta_iid}"
    fc_beta_non_iid_str = f"{beta_non_iid:.1f}"
    fc_beta_non_iid_sq_str = f"{beta_non_iid_sq:.2f}"
    fc_local_epochs_sq_str = f"{local_epochs_sq}"
    fc_reduced_local_epochs_sq_str = f"{reduced_local_epochs_sq}"
    fc_iid_rounds_str = f"{iid_rounds:,}"
    fc_iid_client_epochs_str = f"{iid_client_epochs:,}"
    fc_non_iid_rounds_str = f"{non_iid_rounds:,}"
    fc_non_iid_client_epochs_str = f"{non_iid_client_epochs:,}"
    fc_reduced_rounds_str = f"{reduced_rounds:,}"
    fc_reduced_client_epochs_str = f"{reduced_client_epochs:,}"
    fc_round_increase_str = f"{round_increase:.1f}"
    fc_round_reduction_str = f"{round_reduction:.2f}"
    fc_local_compute_ratio_str = f"{local_compute_ratio:.1f}"
    fc_client_epoch_reduction_str = f"{client_epoch_reduction:.1f}"
```
