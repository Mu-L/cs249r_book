...
# │                           TFLOPs, second, GB, joule, watt, hour, ureg),
...
class EdgeIntelligenceSetup:
    ...
    mobile_npu_tops = int(mobile_npu_tops_raw.m_as(TFLOPs/second))
...
1.  **Significance (quantitative)**: It enables **Hyper-Personalization** and autonomous operation under severe resource constraints. Within the iron law\index{Iron Law}, on-device learning must maximize **Energy Efficiency $(\eta_{\text{hw}})$** because every gradient update consumes limited battery power and must compete with other system tasks for **Peak Throughput $(R_{\text{peak}})$**.
...
rects2_a = ax.bar(x + width/2, activations, width, bottom=weights+grads+opt_state, label='Activations (Batch N)', color='#e74c3c', edgecolor='white')
...
The scaling heuristic above assumes IID data across clients, an assumption that rarely holds in practice. Surveys and empirical studies identify non-IID data as a core federated-learning convergence challenge and quantify it with measures such as local dissimilarity, earth mover's distance, or weight divergence [@li2020; @zhao2018federated]. To reason about this systems effect, let $\beta$ denote an abstract heterogeneity penalty that grows as local client objectives drift away from the global objective:

$$
\mathbb{E}[F(\theta^R) - F(\theta)] \leq \mathcal{O}\left(\frac{1 + \beta}{\sqrt{CER}} + \frac{\beta^2 E^2}{R}\right)
$$

Here, $\beta$ is a compact way to represent client drift in the chapter's systems model rather than a universal metric shared by all analyses. When data is close to IID, $\beta$ is near zero and the bound reduces toward the standard $\mathcal{O}(1/\sqrt{CER})$ rate. When data is highly heterogeneous, larger $\beta$ values significantly slow convergence.
...
Non-IID case $(\beta = 1.5)$: The heterogeneity penalty requires satisfying both terms of the bound. With $E=5$, the variance term requires `{python} FederatedConvergence.fc_non_iid_variance_rounds_str` rounds, while the heterogeneity term dominates:
...
$$
R \geq \max\left(\frac{(1+\beta)^2}{C \cdot E \cdot \epsilon^2}, \frac{\beta^2 E^2}{\epsilon}\right)
