...
| **Constraint Dimension**                             | **Inference**\index{Inference}        | **Training Amplification**                                    | **Impact on Design**                                       |
|:-----------------------------------------------------|:--------------------------------------|:--------------------------------------------------------------|:-----------------------------------------------------------|
...
1. **Total Energy**\index{Energy!total}: `{python} EdgeBattery.battery_power_w_str` W $\times$ `{python} EdgeBattery.battery_duration_h_str` hours = **`{python} EdgeBattery.battery_energy_wh_str` Wh**.
2. **Battery Impact**\index{Battery!battery impact}: `{python} EdgeBattery.battery_energy_wh_str` Wh/`{python} EdgeBattery.battery_total_wh_str` Wh = **`{python} EdgeBattery.battery_drain_pct_str` percent**.
...
: **Constraint-Solution Mapping**\index{Mapping!constraint-solution}: The three fundamental constraint categories in on-device learning each drive corresponding solution approaches through direct necessity. {#tbl-constraint-solution-mapping}
...
2. **Adapter Approach**\index{Adapter!adapter approach}: (1 $\times$ `{python} StorageWall.ls_full_mb_str` MB backbone) + (`{python} StorageWall.ls_contexts_str` $\times$ `{python} StorageWall.ls_adapter_kb_str` KB adapters) $\approx$ **`{python} StorageWall.ls_total_adapter_mb_str` MB**.
...
1. **Raw Data Upload**\index{Raw Data Upload}: **`{python} FederatedSavings.fs_raw_data_mb_str` MB/week**.
2. **Federated Update**\index{Federated Update}: A compressed model update (5M params) is only **`{python} FederatedSavings.fs_update_mb_str` MB**.
...
1. **Model Initialization**\index{Initialization!model}: Each device initializes its local model parameters, often by downloading the latest global model from the server.

2. **Local Data Sampling**\index{Local Data Sampling}: The device samples a subset of its local data for training. This data may be non-IID, meaning that it may not be uniformly distributed across devices.
...
4. **Model Update**\index{Model Update}: After local training, the device computes a model update (e.g., the difference between the updated and initial parameters) and prepares to send it to the server.
...
6. **Model Aggregation**\index{Model Aggregation}: The server aggregates the updates from multiple devices to produce a new global model, which is then distributed back to the participating devices.
...
* **The mobile memory wall**: While NPUs provide raw compute (TOPS), on-device LLMs\index{Memory Wall!mobile} are strictly bandwidth-bound. The 30--50$\times$ gap between mobile RAM and data center HBM makes quantization a mandatory "bandwidth survival strategy" for interactive decode speeds.
...
* **Heterogeneity is the default**: Unlike uniform data center racks, the edge\index{Edge!device heterogeneity} is a fragmented ecosystem of microcontrollers and SoCs. Federated learning must account for "stragglers" and participation bias generated by these physical disparities.
