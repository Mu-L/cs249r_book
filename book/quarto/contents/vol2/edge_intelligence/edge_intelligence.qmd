...
class EdgeMobilenetSizing:
    ...
    model_budget_mb = 4
    model_budget_mb_str = fmt(model_budget_mb, precision=0, commas=False)
```
...
class EdgeH100BwRecap:
    ...
    from mlsysim.fmt import fmt
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_bw_gbs_str = fmt(h100_mem_bw_gbs, precision=0)
```
...
- **Data Center (H100)**: HBM3 provides `{python} EdgeH100BwRecap.h100_mem_bw_gbs_str` GB/s bandwidth.
...
# │ Exports: EdgeFedNpuRecap.mobile_npu_tops_str
