...
2. **Energy Efficiency**\index{Energy Efficiency}: 0.8 J (CPU)/0.016 J (NPU) = **`{python} EdgeNpuSpeedup.npu_energy_gain_str`$\times$**.

**Systems insight**: Specialized hardware does not just make the model faster; it makes it **feasible**. A **50$\times$ energy gain** translates into 50 times more inferences on the same battery charge. On the edge, the CPU is for coordination, but the NPU is for survival. Missing the NPU target and falling back to the CPU does not merely cost performance; it typically renders the application unusable through rapid battery drain and thermal throttling.
...
[^fn-gboard-federated-scale]: **Gboard (2017)**: The first commercial federated learning deployment at planetary scale, aggregating model updates across over 1 billion devices. Gboard demonstrated 10--20 percent accuracy improvements over static models while maintaining differential privacy guarantees, proving that the communication overhead of federated coordination (each device uploading only a few KB of compressed updates) was architecturally viable even over constrained mobile networks. \index{Gboard!federated learning}
...
[^fn-neural-engine-evolution]: **Apple Neural Engine**\index{Apple Neural Engine}: From the A11 (0.6 TOPS) to current flagship-class mobile NPUs around `{python} EdgeIntelligenceSetup.mobile_npu_tops_str` TOPS, mobile neural acceleration has improved by roughly two orders of magnitude. The systems consequence: fine-tuning a MobileNet classifier takes approximately 2 seconds on the neural accelerator vs. 45 seconds on CPU, while consuming only approximately 500 mW additional power. This roughly 23$\times$ speedup at minimal power cost is what makes on-device adaptation thermally feasible during normal phone usage. \index{Neural Engine!on-device training}

[^fn-tensor-soc-federated]: **Google Tensor SoC (2021)**: Features a custom Edge Tensor Processing Unit (TPU) derived from Google's data center TPU v1, optimized for 8-bit integer ML operations at 2 W. Unlike general-purpose NPUs, Tensor is co-designed with TensorFlow Lite and federated learning infrastructure, reducing the software overhead of local training. This tight hardware-software coupling demonstrates the trend toward ML-specific SoC design for on-device adaptation. \index{Tensor!google, federated learning}
...
1. **Total Energy**\index{Energy!total}: `{python} EdgeBattery.battery_power_w_str` W $\times$ (30/60) hours = **2.25 Wh**.
2. **Battery Impact**\index{Battery!battery impact}: 2.25 Wh/15 Wh = **`{python} EdgeBattery.battery_drain_pct_str` percent**.
...
**Problem**: A team is deploying a 10M parameter vision model to a smartphone with support for 10 different "User Contexts" (Home, Office, Car, etc.). If a full fine-tuned model requires `{python} StorageWall.ls_full_mb_str` MB, how much storage does using **Residual Adapters**\index{Residual Adapter!parameter efficient finetuning} save instead?

**Math**:

1. **Full Fine-Tuning**\index{Full Fine-Tuning}: 10 contexts $\times$ 40 MB = **400 MB**.
2. **Adapter Approach**\index{Adapter!adapter approach}: (1 $\times$ 40 MB backbone) + (10 $\times$ `{python} StorageWall.ls_adapter_kb_str` KB adapters) $\approx$ **42 MB**.
3. **Storage Savings**\index{Storage!savings}: 400 MB/42 MB $\approx$ **9.5$\times$** total device storage reduction.
4. **Per-Context Efficiency**\index{Efficiency!per-context}: `{python} StorageWall.ls_full_mb_str` MB/0.2 MB = **`{python} StorageWall.ls_savings_ratio_str`$\times$**.
...
::: {.callout-war-story title="Apple's on-device keyboard learning"}

Apple's QuickType keyboard represents one of the largest deployments of federated learning in history. The system updates next-word prediction models across billions of devices without raw keystrokes ever leaving the phone. Training is strictly constrained: it only runs when the device is plugged in, on Wi-Fi, and idle. Updates are cryptographically aggregated using a localized differential privacy budget $(\epsilon < 8)$. By averaging millions of small, noisy weight updates ($\Delta W$), the global model improves on new slang and typing patterns daily, processing petabytes of effective training data while maintaining strict user privacy.
...
As a worked example, consider a production federated deployment with 100 clients: $K = 100$ total clients in the population, $C = 10$ clients selected per round, $E = 5$ local epochs per round, target optimality gap $\epsilon = 0.01$, and two scenarios comparing IID data $(\beta = 0)$ vs. moderately non-IID data $(\beta = 1.5)$.
