...
1. **Latency Speedup**\index{Latency!speedup}: `{python} EdgeNpuSpeedup.npu_cpu_lat_ms_str` ms/`{python} EdgeNpuSpeedup.npu_lat_ms_str` = **`{python} EdgeNpuSpeedup.npu_speedup_str`$\times$**.
...
2. **Battery Impact**\index{Battery!on-device training impact}: `{python} EdgeBattery.battery_energy_wh_str` Wh/`{python} EdgeBattery.battery_total_wh_str` = **`{python} EdgeBattery.battery_drain_pct_str`**.
...
3. **Storage Savings**\index{Storage!savings}: `{python} StorageWall.ls_total_full_mb_str` MB/`{python} StorageWall.ls_total_adapter_mb_str` $\approx$ **`{python} StorageWall.ls_total_savings_ratio_str`$\times$** total device storage reduction.
4. **Per-Context Efficiency**\index{Adapter!per-context efficiency}: `{python} StorageWall.ls_full_mb_str` MB/`{python} StorageWall.ls_adapter_mb_str` = **`{python} StorageWall.ls_savings_ratio_str`$\times$**.
...
3. **Bandwidth Reduction**\index{Bandwidth!reduction}: `{python} FederatedSavings.fs_raw_data_mb_str` MB/`{python} FederatedSavings.fs_update_mb_str` = **`{python} FederatedSavings.fs_reduction_str`$\times$ savings**.
