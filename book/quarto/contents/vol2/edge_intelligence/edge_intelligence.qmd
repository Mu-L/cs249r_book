...
class EdgeNpuSpeedup:
    ...
    check(energy_gain == 60, f"Energy gain {energy_gain:.1f}x unexpected")
...
- **Data Center (H100)**: HBM3 provides `{python} EdgeH100BwRecap.h100_mem_bw_gbs_str`/s bandwidth.
...
1. **Total Energy**: `{python} EdgeBattery.battery_power_w_str` W $\times$ `{python} EdgeBattery.battery_duration_h_str` = **`{python} EdgeBattery.battery_energy_wh_str`**.
...
class EdgeFedNpuRecap:
    ...
    mobile_npu_tops_str = fmt(mobile_npu_tops, precision=0, commas=False)
    compute_ratio_str = fmt_int(round(compute_ratio), commas=True)
```
