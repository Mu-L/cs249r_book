...
# │
# │ Imports: mlsysim.core.constants (MOBILE_NPU_TOPS_INT8, H100_MEM_BW,
...
class EdgeIntelligenceSetup:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    mobile_npu_tops_raw = MOBILE_NPU_TOPS_INT8
...
class EdgeNpuSpeedup:
    # ┌── 1. LOAD ──────────────────────────────────────────
    t_cpu_ms = 400 # Typical CPU inference for MobileNet
    t_npu_ms = 20  # NPU accelerated
    e_cpu_j = 0.8  # Joules per inference on CPU
    e_npu_j = 0.016 # Joules per inference on NPU
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    npu_cpu_lat_ms_str = fmt(t_cpu_ms, precision=0, commas=False)
    npu_lat_ms_str = fmt(t_npu_ms, precision=0, commas=False)
...
Privacy considerations provide a third compelling driver. Many modern applications involve sensitive or regulated data including biometric measurements, typed input, location traces, or health information. Local learning can reduce privacy exposure by keeping raw data on the device and limiting transmission to centralized systems. This can simplify compliance engineering, but it does not by itself establish adherence to regulations such as the General Data Protection Regulation (GDPR),[^fn-gdpr-ondevice] the Health Insurance Portability and Accountability Act (HIPAA)[^fn-hipaa-ondevice] [@hhs2026hipaaSecurity], or region-specific data sovereignty laws.
...
# │
# │ Imports: mlsysim.core.constants (byte, KB, MB), mlsysim.fmt (fmt, check)
# │ Exports: *_str values for prose references.
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import byte, KB, MB
...
class EdgeModelSizing:
    ...
    stm32_hidden_kb_str = fmt(stm32_hidden_kb, precision=0, commas=False)
    stm32_hidden_sram_pct_str = fmt(stm32_hidden_sram_pct, precision=0, commas=False)
...
class EdgeBattery:
    ...
    battery_wh = 15  # Modern flagship smartphone battery
    # ┌── 2. EXECUTE ───────────────────────────────────────
    t_train_h = t_train_min / 60
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import byte, KB, MB, MILLION
from mlsysim.fmt import check, fmt
...
class StorageWall:
    ...
    model_params_m = 10
    n_contexts = 10
    bytes_per_param = 4 # FP32 for training/fine-tuning
...
Compressed representations serve two complementary goals. First, they reduce the footprint of stored data, allowing devices to maintain longer histories or replay buffers under tight memory budgets [@hayes2020remind]. Second, they simplify the learning task by projecting raw inputs into more structured feature spaces, often learned via pretraining or meta-learning, in which efficient adaptation is possible with minimal supervision.
...
class FederatedSavings:
    ...
    update_mb = (model_params_m * MILLION * bits_per_update_param / 8 * byte).m_as(MB)
...
The scaling heuristic above assumes IID data across clients, an assumption that rarely holds in practice. Surveys and empirical studies identify non-IID data as a core federated-learning convergence challenge and quantify it with measures such as local dissimilarity, earth mover's distance, or weight divergence [@li2020; @zhao2018federated]. To reason about this systems effect, let $\beta_{\text{het}}$ denote an abstract heterogeneity penalty that grows as local client objectives drift away from the global objective:
...
Federated learning achieves practical convergence only when several deployment conditions are favorable, a theme emphasized in surveys of federated-learning systems and open problems [@kairouz2021]. First, heterogeneity must remain bounded enough that client drift does not dominate training time; when populations are highly heterogeneous, clustering clients into more homogeneous groups or using personalization may be preferable to one global model. Second, client participation must be sufficient to provide stable aggregate updates; very low participation makes gradient estimates noisy and exacerbates selection bias. Third, each participating client needs enough local data for meaningful update computation; tiny local batches can contribute high-variance updates that destabilize training.
...
From a memory perspective, training incurs higher peak usage than inference, especially because backpropagation must retain or recompute intermediate activations[^fn-activation-caching-memory]; efficient on-device training systems reduce this activation burden and limit the amount of trainable state [@cai2020tinytl; @kwon2024tinytrain].
