...
class EdgeIntelligenceSetup:
    ...
    mobile_npu_tops_str = fmt(mobile_npu_tops, precision=3, commas=False)
    h100_mem_bw_gbs_str = fmt(h100_mem_bw_gbs_int, precision=3)
    smartphone_ram_gb_str = fmt(smartphone_ram_gb, precision=3, commas=False)
```
...
class EdgeNpuSpeedup:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    npu_cpu_lat_ms_str = fmt(t_cpu_ms, precision=3, commas=False)
    npu_lat_ms_str = fmt(t_npu_ms, precision=3, commas=False)
    npu_cpu_energy_j_str = fmt(e_cpu_j, precision=1, commas=False)
    npu_energy_j_str = fmt(e_npu_j, precision=3, commas=False)
    npu_speedup_str = fmt(speedup, precision=3, commas=False)
    npu_energy_gain_str = fmt(energy_gain, precision=3, commas=False)
...
class EdgeModelSizing:
    ...
    stm32_hidden_inputs_str = fmt(stm32_hidden_inputs, precision=3, commas=False)
    stm32_hidden_kb_str = fmt(stm32_hidden_kb, precision=3, commas=False)
    stm32_hidden_sram_pct_str = fmt(stm32_hidden_sram_pct, precision=3, commas=False)
    tinytl_param_reduction_str = fmt(tinytl_param_reduction, precision=3, commas=False)
    brain_tops_per_w_str = fmt(brain_tops_per_w, precision=3, commas=False)
    brain_vs_npu_low_str = fmt(brain_vs_npu_low, precision=3, commas=False)
    brain_vs_npu_high_str = fmt(brain_vs_npu_high, precision=3, commas=False)
```
...
class EdgeBattery:
    ...
    battery_duration_min_str = fmt(t_train_min, precision=3, commas=False)
    ...
    battery_total_wh_str = fmt(battery_wh, precision=3, commas=False)
...
class StorageWall:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ls_contexts_str = fmt(n_contexts, precision=3, commas=False)
    ls_full_mb_str = fmt(size_full_mb, precision=3, commas=False)
    ls_adapter_kb_str = fmt(size_adapter_kb, precision=3, commas=False)
    ls_adapter_mb_str = fmt(size_adapter_mb, precision=1, commas=False)
    ls_savings_ratio_str = fmt(per_context_savings, precision=3, commas=False)
    ls_total_full_mb_str = fmt(total_full_mb, precision=3, commas=False)
    ls_total_adapter_mb_str = fmt(total_adapter_mb, precision=3, commas=False)
...
class FederatedSavings:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    fs_raw_data_mb_str = fmt(raw_data_mb, precision=3, commas=False)
    fs_update_mb_str = fmt(update_mb, precision=1, commas=False)
    fs_reduction_str = fmt(reduction, precision=3, commas=False)
...
class FederatedConvergence:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    fc_total_clients_str = fmt(total_clients, precision=3, commas=False)
    fc_clients_per_round_str = fmt(clients_per_round, precision=3, commas=False)
    fc_local_epochs_str = fmt(local_epochs, precision=3, commas=False)
    fc_reduced_local_epochs_str = fmt(reduced_local_epochs, precision=3, commas=False)
    fc_epsilon_str = fmt(epsilon, precision=2, commas=False)
    fc_epsilon_sq_str = fmt(epsilon_sq, precision=4, commas=False)
    fc_sigma_str = fmt(sigma, precision=3, commas=False)
    fc_beta_iid_str = fmt(beta_iid, precision=3, commas=False)
    ...
    fc_local_epochs_sq_str = fmt(local_epochs_sq, precision=3, commas=False)
    fc_reduced_local_epochs_sq_str = fmt(reduced_local_epochs_sq, precision=3, commas=False)
    fc_iid_rounds_str = fmt(iid_rounds, precision=3)
    fc_iid_client_epochs_str = fmt(iid_client_epochs, precision=3)
    fc_non_iid_rounds_str = fmt(non_iid_rounds, precision=3)
    fc_non_iid_variance_rounds_str = fmt(non_iid_variance_rounds, precision=3)
    fc_non_iid_client_epochs_str = fmt(non_iid_client_epochs, precision=3)
    fc_reduced_rounds_str = fmt(reduced_rounds, precision=3)
    fc_reduced_variance_rounds_str = fmt(reduced_variance_rounds, precision=3)
    fc_reduced_heterogeneity_rounds_str = fmt(reduced_heterogeneity_rounds, precision=3)
    fc_reduced_client_epochs_str = fmt(reduced_client_epochs, precision=3)
