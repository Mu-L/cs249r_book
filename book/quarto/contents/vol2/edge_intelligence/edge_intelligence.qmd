...
# │ Show: speedup ≈ 20x, energy_gain ≈ 60x—inline in notebook result.
...
class EdgeNpuSpeedup:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    npu_cpu_lat_ms_str = fmt(t_cpu_ms, precision=3, commas=False, suffix=' ms')
    npu_lat_ms_str = fmt(t_npu_ms, precision=3, commas=False, suffix=' ms')
    npu_cpu_energy_j_str = fmt(e_cpu_j, precision=3, commas=False)
    npu_energy_j_str = fmt(e_npu_j, precision=5, commas=False)
    npu_speedup_str = fmt_int(speedup, commas=False)
    npu_energy_gain_str = fmt_int(energy_gain, commas=False)
```
...
1. **Latency Speedup**\index{Latency!speedup}: `{python} EdgeNpuSpeedup.npu_cpu_lat_ms_str`/`{python} EdgeNpuSpeedup.npu_lat_ms_str` = `{python} EdgeNpuSpeedup.npu_speedup_str`$\times$.
...
# │ Exports: EdgeSmartphoneRamRecap.smartphone_ram_gb_str
class EdgeSmartphoneRamRecap:
    smartphone_ram_gb = int(Platforms.Mobile.ram.m_as(GB))
    from mlsysim.fmt import fmt_int, fmt
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    smartphone_ram_gb_str = fmt_int(smartphone_ram_gb, commas=False, suffix=' GB')
```
...
# │ Exports: mobilenetv2_*_str, stm32_hidden_*_str
...
class EdgeMobilenetSizing:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobilenetv2_alpha10_params_m_str = fmt(mobilenetv2_alpha10_params/ MILLION, precision=1, commas=False)
    mobilenetv2_alpha10_mb_str = fmt_int(alpha10_mb, commas=False, suffix=' MB')
    mobilenetv2_alpha05_total_params_m_str = fmt(mobilenetv2_alpha05_total_params/ MILLION, precision=1, commas=False)
    mobilenetv2_alpha05_total_mb_str = fmt(alpha05_total_mb, precision=1, commas=False, suffix=' MB')
    mobilenetv2_alpha05_feature_params_m_str = fmt(mobilenetv2_alpha05_feature_params/ MILLION, precision=2, commas=False)
    mobilenetv2_alpha05_feature_mb_str = fmt(alpha05_feature_mb, precision=1, commas=False, suffix=' MB')
    stm32_hidden_inputs_str = fmt_int(stm32_hidden_inputs, commas=False)
    stm32_hidden_kb_str = fmt(stm32_hidden_kb, precision=3, commas=False, suffix=' KB')
    stm32_hidden_sram_pct_str = fmt(stm32_hidden_sram_pct, precision=3, commas=False, suffix=' percent')
    model_budget_mb = 4
    model_budget_mb_str = fmt_int(model_budget_mb, commas=False, suffix=' MB')
```
...
Modularity is a key design property. MobileNets [@howard2017mobilenets] and MobileNetV2 [@sandler2018] can be configured with different width multipliers and resolution settings to balance performance and resource usage. A complete MobileNetV2 with width multiplier $\alpha_{\text{width}}=1.0$ has approximately `{python} EdgeMobilenetSizing.mobilenetv2_alpha10_params_m_str` M parameters (`{python} EdgeMobilenetSizing.mobilenetv2_alpha10_mb_str` in FP32). With $\alpha_{\text{width}}=0.5$, the complete 1000-class model has approximately `{python} EdgeMobilenetSizing.mobilenetv2_alpha05_total_params_m_str` M parameters (`{python} EdgeMobilenetSizing.mobilenetv2_alpha05_total_mb_str`), while a feature-extractor-only deployment with a small task-specific head can be as small as `{python} EdgeMobilenetSizing.mobilenetv2_alpha05_feature_params_m_str` M parameters (`{python} EdgeMobilenetSizing.mobilenetv2_alpha05_feature_mb_str`), fitting within a `{python} EdgeMobilenetSizing.model_budget_mb_str` model budget.
...
# │ Exports: EdgeMobileNpuRecap.mobile_npu_tops_str
class EdgeMobileNpuRecap:
    mobile_npu_tops = int(Hardware.Mobile.iPhone15Pro.compute.peak_flops.m_as(TOPS))
    from mlsysim.fmt import fmt_int, fmt, check
    check(mobile_npu_tops > 0, "Mobile NPU TOPS should be strictly positive")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mobile_npu_tops_str = fmt_int(mobile_npu_tops, commas=False)
```
...
# │ Exports: EdgeH100BwRecap.h100_mem_bw_gbs_str
class EdgeH100BwRecap:
    h100_mem_bw_gbs = int(Hardware.Cloud.H100.memory.bandwidth.m_as(GB/second))
    from mlsysim.fmt import fmt_int, fmt
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_bw_gbs_str = fmt_int(h100_mem_bw_gbs, suffix=' GB/s')
```
...
- **Data Center (H100)**: HBM3 provides `{python} EdgeH100BwRecap.h100_mem_bw_gbs_str` bandwidth.
...
Edge accelerator marketing emphasizes peak compute throughput (TOPS), but for autoregressive workloads the relevant ceiling is set by memory bandwidth, not arithmetic. Each generated token requires streaming the model's weights from memory to the compute units, so the iron law's Data term $(D_{\text{vol}} / \text{BW})$ dominates the Compute term whenever weights exceed on-chip cache, which is the case for any billion-parameter model on current mobile silicon.

The 30 to 50$\times$ bandwidth gap between data center HBM3 and mobile LPDDR5X therefore propagates directly into a 30 to 50$\times$ gap in tokens per second, regardless of how many TOPS the NPU advertises. This is why mobile LLM deployment leans on aggressive quantization (INT4 or INT2) and weight-sharing techniques: their first-order effect is to shrink $D_{\text{vol}}$ so the same LPDDR5X budget feeds more tokens per second, not to reduce arithmetic per token.

A practical corollary: when ranking edge accelerator options for transformer decoding, the figure of merit is the GB/s a chip can sustain to its on-package memory, not the peak TOPS on its spec sheet. The two diverge by orders of magnitude on modern mobile silicon.
...
class EdgeBattery:
    ...
    battery_wh = Scenarios.PhoneBattery.EnergyWh.m_as(watt * hour)  # Modern flagship smartphone battery
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    battery_duration_min_str = fmt_int(t_train_min, commas=False, suffix=' minutes')
    battery_duration_h_str = fmt(t_train_h, precision=1, commas=False)
    battery_energy_wh_str = fmt(energy_wh, precision=2, commas=False, suffix=' Wh')
    battery_drain_pct_str = fmt_int(drain_pct, commas=False, suffix=' percent')
    battery_total_wh_str = fmt_int(battery_wh, commas=False, suffix=' Wh')
```
...
1. **Total Energy**: `{python} EdgeBattery.battery_power_w_str` W $\times$ `{python} EdgeBattery.battery_duration_h_str` h = `{python} EdgeBattery.battery_energy_wh_str`.
2. **Battery Impact**\index{Battery!on-device training impact}: `{python} EdgeBattery.battery_energy_wh_str`/`{python} EdgeBattery.battery_total_wh_str` = `{python} EdgeBattery.battery_drain_pct_str`.
...
# │ Exports: EdgeMobileNpuTierRecap.mobile_npu_tops_str
...
class StorageWall:
    ...
    ls_full_mb_str = fmt_int(size_full_mb, commas=False, suffix=' MB')
    ls_adapter_kb_str = fmt_int(size_adapter_kb, commas=False, suffix=' KB')
    ls_adapter_mb_str = fmt(size_adapter_mb, precision=1, commas=False)
    ls_savings_ratio_str = fmt_int(per_context_savings, commas=False)
    ls_total_full_mb_str = fmt_int(total_full_mb, commas=False, suffix=' MB')
    ls_total_adapter_mb_str = fmt_int(total_adapter_mb, commas=False, suffix=' MB')
...
**Problem**: A team is deploying a 10M parameter vision model to a smartphone with support for `{python} StorageWall.ls_contexts_str` different "User Contexts" (Home, Office, Car, etc.). If a full fine-tuned model requires `{python} StorageWall.ls_full_mb_str`, how much storage does using **Residual Adapters**\index{Residual Adapter!parameter efficient finetuning} save instead?

**Math**:

1. **Full Fine-Tuning**\index{Full Fine-Tuning}: `{python} StorageWall.ls_contexts_str` contexts $\times$ `{python} StorageWall.ls_full_mb_str` = `{python} StorageWall.ls_total_full_mb_str`.
2. **Adapter Approach**\index{Adapter!storage efficiency}: (1 $\times$ `{python} StorageWall.ls_full_mb_str` backbone) + (`{python} StorageWall.ls_contexts_str` $\times$ `{python} StorageWall.ls_adapter_kb_str` adapters) $\approx$ `{python} StorageWall.ls_total_adapter_mb_str`.
3. **Storage Savings**\index{Storage!savings}: `{python} StorageWall.ls_total_full_mb_str`/`{python} StorageWall.ls_total_adapter_mb_str` $\approx$ `{python} StorageWall.ls_total_savings_ratio_str`$\times$ total device storage reduction.
4. **Per-Context Efficiency**\index{Adapter!per-context efficiency}: `{python} StorageWall.ls_full_mb_str`/`{python} StorageWall.ls_adapter_mb_str` = `{python} StorageWall.ls_savings_ratio_str`$\times$.
...
The freeze drastically reduces the memory footprint: by avoiding storage of full activation maps and updating only the lightweight bias parameters, TinyTL allows on-device training under severe resource constraints.
...
# │ Exports: EdgeTinyTlRecap.tinytl_param_reduction_str
...
# │ Exports: EdgeAdapterRamRecap.smartphone_ram_gb_str
...
class FederatedSavings:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    fs_raw_data_mb_str = fmt_int(raw_data_mb, commas=False, suffix=' MB')
    fs_update_mb_str = fmt(update_mb, precision=1, commas=False, suffix=' MB')
    fs_reduction_str = fmt_int(reduction, commas=False)
```
...
**Problem**: A team is designing a federated camera-personalization system that learns from `{python} FederatedSavings.fs_raw_data_mb_str` of compressed user images per week. Should the system upload the raw images to the cloud for training, or use federated learning to send model updates instead?

**Math**:
Bandwidth efficiency is the ratio of raw data volume to model update size.

1. **Raw Data Upload**\index{Data Upload!raw}: `{python} FederatedSavings.fs_raw_data_mb_str`/week.
2. **Federated Update**\index{Federated Learning!model updates}: A compressed model update (5M params) is only `{python} FederatedSavings.fs_update_mb_str`.
3. **Bandwidth Reduction**\index{Bandwidth!reduction}: `{python} FederatedSavings.fs_raw_data_mb_str`/`{python} FederatedSavings.fs_update_mb_str` = `{python} FederatedSavings.fs_reduction_str`$\times$ savings.

**Systems insight**: Federated learning is a *network multiplier*. For a user on a limited cellular plan, uploading `{python} FederatedSavings.fs_raw_data_mb_str` of raw data is expensive and slow. Uploading a `{python} FederatedSavings.fs_update_mb_str` update is nearly invisible. Shifting the "Compute to the Data" reduces network load by `{python} FederatedSavings.fs_reduction_str`$\times$, enabling continuous learning even in bandwidth-constrained environments. In the Machine Learning Fleet, this is how systems scale to large user populations without bankrupting the networking budget or violating user privacy.
...
You are architecting a federated learning system for a fleet of 10 million mobile devices. The data is highly non-IID (users have distinct, clustered typing patterns), and the network environment is constrained (1-10 Mbps). Before choosing optimizations, use @sec-appdx-c3-taxonomy-bottleneck-table to diagnose whether the constrained bandwidth makes this a communication-bound system or whether the non-IID heterogeneity makes it coordination-bound, since the table shows which mitigations pay off for each axis and which are wasted effort.
**Design decision**:
...
# │ Exports: EdgeFedNpuRecap.mobile_npu_tops_str, compute_ratio_str
class EdgeFedNpuRecap:
    from mlsysim.fmt import fmt, check, fmt_int
    mobile_npu_tops = Hardware.Mobile.iPhone15Pro.compute.peak_flops.m_as(TOPS)
    iot_tops = 0.03 * TOPS
    compute_ratio = mobile_npu_tops / iot_tops.m_as(TOPS)
    check(1000 < compute_ratio < 2000, f"Flagship-to-IoT compute ratio unexpected: {compute_ratio:.0f}x")
    mobile_npu_tops_str = fmt_int(mobile_npu_tops, commas=False)
    compute_ratio_str = fmt_int(compute_ratio)
```
...
# │ Exports: brain_tops_per_w_str, brain_vs_npu_low_str, brain_vs_npu_high_str
...
- [ ] **Device-tier capability matching**: A fleet contains Cortex-M0+ wake-word sensors, Cortex-A78 smartphones, and edge servers. Which adaptation technique from @fig-tiered-adaptation does each tier support, and what fails if you try to push a single technique across all three tiers?
- [ ] **Fairness in federated participation**: When 90 percent of a federated round's compute comes from the top 10 percent of devices by capability, what does that imply for the global model's representation of long-tail user behavior, and which defense from the data efficiency pillar addresses it?
- [ ] **Drift detection without labels**: A deployed personalization model has no ground-truth labels at the edge. How can the device detect that its local distribution has shifted enough to warrant a federated round, using only signals available to the on-device inference path?
- [ ] **Secure aggregation's participation floor**: Secure aggregation requires a minimum number of participants per round to provide a useful privacy guarantee. If the floor is 100 devices and the fleet has only 30 active devices in a given hour, what does the system do, and which pillar must compensate?
- [ ] **The auditable-autonomy trade-off**: Federated learning lets devices learn without uploading raw data, but central operators still need to audit that the global model is converging correctly and not being poisoned. What can the operator observe, what must remain hidden, and where does the catastrophic forgetting risk land in this split?
