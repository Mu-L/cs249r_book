...
# │ How: τ = sqrt(2 * T_write * MTBF_system); bare arithmetic for clarity.
# │
...
**Problem**: A 10,000-GPU cluster has an MTBF of `{python} YoungDalyOptimal.mtbf_h_str` hours. A full model checkpoint takes `{python} YoungDalyOptimal.t_write_s_str` seconds to write. What is the optimal checkpoint frequency?

**Math**:
Apply the Young-Daly formula: $\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}}$

1. **Convert to common units**: $\text{MTBF}_{\text{system}} =$ `{python} YoungDalyOptimal.mtbf_h_str` $\times$ 3,600s = `{python} YoungDalyOptimal.mtbf_s_str` seconds.
...
::: {#wrn-fault-tolerance-checkpoint-storm .callout-warning title="The checkpoint storm"}
...
::: {#fig-elastic-flow fig-env="figure" fig-pos="htb" fig-cap="**Elastic Training Recovery**: Unlike static training which aborts on failure, elastic training adapts. When a worker fails, the job pauses, redistributes the dataset and model shards across the remaining $W-1$ workers, and resumes training from the last consistent state. This capability transforms hard failures into temporary throughput degradations." fig-alt="Flowchart with 5 steps and decision diamond. Training on W workers flows to monitor alert diamond. On failure: pause training, rescale batch and learning rate to W-1 workers, resume training. Loop returns to monitoring. Annotation highlights key step."}
...
$$ \eta_{\text{new}} = \eta_{\text{base}} \times \sqrt{\frac{W_{\text{new}}}{W_{\text{base}}}} $$ {#eq-lr-scaling}

where $W$ represents the number of workers (and thus the global batch size). The linear rule [@goyal2017accurate] is often preferred for large batch training with warmup, while square root scaling provides a more conservative alternative when stability is a concern.
