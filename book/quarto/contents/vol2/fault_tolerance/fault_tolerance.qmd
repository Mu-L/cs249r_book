...
1.  **Single GPU Availability Probability** $(R_{\text{GPU}})$: A GPU with `{python} NinesReliability.availability_str` percent availability has an all-up probability $R_{\text{GPU}} =$ `{python} NinesReliability.survival_str` at a randomly chosen instant.
...
# │ How: Hardware.Cloud.V100.memory.bandwidth.m_as(GB/second), format with fmt.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.V100.memory.bandwidth, GB, second), mlsysim.fmt (fmt, check)
...
class V100Spec:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(mem_bw_gbs > 0, "V100 memory bandwidth must be positive")
...
class FaultToleranceCheckpointRecap:
    ...
    gpt3_ckpt_tb_str = fmt(gpt3_ckpt_bytes.m_as(TB), precision=1, commas=False)
    gpt3_adam_tb_str = fmt(gpt3_adam_bytes.m_as(TB), precision=1, commas=False)
    gpt3_shard_gb_str = fmt(gpt3_ckpt_bytes.m_as(GB) / n_workers, precision=1, commas=False)
```

[^fn-adam-state-memory]: **Adam Optimizer State**\index{Adam Optimizer!state}: Adaptive Moment Estimation (Adam) maintains per-parameter first-moment ($m$) and second-moment ($v$) estimates, tripling memory requirements compared to vanilla SGD (3$\times$: parameters + two state vectors). For a `{python} FaultToleranceCheckpointRecap.gpt3_params_b_str`B parameter model, optimizer state alone reaches `{python} FaultToleranceCheckpointRecap.gpt3_adam_tb_str` TB, which typically dominates checkpoint size and recovery time, making optimizer state the primary bottleneck in checkpoint I/O design. \index{Adam Optimizer!checkpoint memory}
...
For our 10,000-GPU cluster with calculated system MTBF of 3.69 hours (from @sec-fault-tolerance-reliability-reliability-worked-example-cluster-mtbf-calculation-9255) and checkpoint time of 21 seconds (from `{python} FaultToleranceCheckpointRecap.gpt3_ckpt_tb_str` TB checkpoint at 100 GB/s), the Young-Daly formula computes the optimal interval.
...
# │   mtbf_s_str, radicand_str, and tau_opt_s_str support the arithmetic.
# │ How: tau_opt = sqrt(2 * T_write * MTBF_system); bare arithmetic for clarity.
# │
# │ Imports: mlsysim.book (check)
# │ Exports: t_write_s_str, mtbf_h_str, mtbf_s_str, radicand_str,
# │   tau_opt_s_str, tau_opt_min_str, overhead_pct_str, hourly_overhead_pct_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
from mlsysim.core.constants import SECONDS_PER_HOUR
import math
...
class YoungDalyOptimal:
    ...
    mtbf_h = 3.69
    # ┌── 2. EXECUTE ───────────────────────────────────────
    mtbf_s = mtbf_h * SECONDS_PER_HOUR
    ...
    hourly_interval_s = SECONDS_PER_HOUR
    ...
    mtbf_h_str = fmt(mtbf_h, precision=2, commas=False)
    mtbf_s_str = fmt(mtbf_s, precision=0)
    radicand_str = fmt(radicand, precision=0)
    tau_opt_s_str = fmt(tau_opt_s, precision=1)
    tau_opt_min_str = fmt(tau_opt_min, precision=1, commas=False)
    overhead_pct_str = fmt(overhead_pct, precision=1, commas=False)
    hourly_overhead_pct_str = fmt(hourly_overhead_pct, precision=1, commas=False)
    seconds_per_hour_str = fmt(SECONDS_PER_HOUR, precision=0, commas=False)
...
**Problem**: A `{python} YoungDalyOptimal.n_gpus_str`-GPU cluster has an MTBF of `{python} YoungDalyOptimal.mtbf_h_str` hours. A full model checkpoint takes `{python} YoungDalyOptimal.t_write_s_str` seconds to write. What is the optimal checkpoint frequency?

**Math**:
Apply the Young-Daly formula: $\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}}$

1. **Convert to common units**: $\text{MTBF}_{\text{system}} =$ `{python} YoungDalyOptimal.mtbf_h_str` hours $\times$ `{python} YoungDalyOptimal.seconds_per_hour_str` s/hr = **`{python} YoungDalyOptimal.mtbf_s_str` seconds**.
2. **Calculate**: With $T_{\text{write}} =$ `{python} YoungDalyOptimal.t_write_s_str` seconds and $\text{MTBF}_{\text{system}} =$ `{python} YoungDalyOptimal.mtbf_s_str` seconds, the Young-Daly radicand is `{python} YoungDalyOptimal.radicand_str`, so $\tau_{\text{opt}} \approx$ `{python} YoungDalyOptimal.tau_opt_s_str` seconds.
...
class CheckpointDebug:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weights_gb_str = fmt(m.parameters.m_as(param) * 2 / BILLION, precision=0, commas=False)
    optimizer_gb_str = fmt(m.parameters.m_as(param) * 8 / BILLION, precision=0, commas=False)
    total_ckpt_gb_str = fmt(total_ckpt_gb_val, precision=0, commas=False)
    nfs_gbs_str = fmt(nfs_bw.m_as(GB/second), precision=2, commas=False)
    min_write_s_str = fmt(min_write_s_val, precision=0, commas=False)
    min_write_min_str = fmt(min_write_min_val, precision=1, commas=False)
    per_node_mbs_str = fmt_int(nfs_bw.m_as(MB/second) / n_nodes, commas=False)
    per_node_shard_gb_str = fmt(per_node_shard_gb_val, precision=1, commas=False)
    per_node_write_min_str = fmt(min_write_min_val, precision=1, commas=False) # Aggregated BW bottleneck
    ...
    extended_weeks_str = fmt(base_weeks * (1 + overhead_pct / 100), precision=1, commas=False)
    ...
    n_gpus_str = fmt(n_gpus_ckpt, precision=0, commas=False)
    n_nodes_str = fmt(n_nodes, precision=0, commas=False)
    observed_write_min_str = fmt(observed_write_min, precision=0, commas=False)
    target_write_min_str = fmt(target_write_min, precision=0, commas=False)
    overhead_pct_str = fmt(overhead_pct, precision=0, commas=False)
    sla_weeks_str = fmt(base_weeks, precision=0, commas=False)
    nvme_bw_gbs_str = fmt(nvme_bw_gbs, precision=1, commas=False)
    pcie_gbs_str = fmt(pcie_bw.m_as(GB/second), precision=0, commas=False)
    residual_overhead_pct_str = fmt(residual_overhead_pct, precision=0, commas=False)
```

::: {#exmp-fault-tolerance-debugging-checkpoint-overhead .callout-example title="Debugging checkpoint overhead"}

A team training a `{python} CheckpointDebug.params_b_str` parameter model observes that checkpointing takes `{python} CheckpointDebug.observed_write_min_str` minutes per checkpoint, far exceeding their expected `{python} CheckpointDebug.target_write_min_str`-minute target. Training throughput has dropped `{python} CheckpointDebug.overhead_pct_str` percent because the cluster sits idle during checkpoints. How can this be diagnosed and resolved?
...
- **Checkpoint mode**: Synchronous, all `{python} CheckpointDebug.n_gpus_str` GPUs pause and wait
- **Write pattern**\index{Write Pattern}: All `{python} CheckpointDebug.n_nodes_str` nodes writing simultaneously to shared NFS
...
The Distribution Layer reveals the shared-storage bottleneck. With `{python} CheckpointDebug.n_nodes_str` nodes competing for 10 Gbps, each node effectively receives only `{python} CheckpointDebug.per_node_mbs_str` MB/s (10 Gbps/`{python} CheckpointDebug.n_nodes_str` nodes). If the checkpoint is sharded evenly, each node writes about `{python} CheckpointDebug.per_node_shard_gb_str` GB, so the write still lands near `{python} CheckpointDebug.per_node_write_min_str` minutes. The observed `{python} CheckpointDebug.observed_write_min_str` minutes is therefore close to the bandwidth floor, not something more simultaneous NFS writers can solve.

**Governance Layer Analysis (Control Plane)**

At the top layer, organizational constraints are considered:

- **SLA requirement**\index{SLA!requirement}: Training must complete within `{python} CheckpointDebug.sla_weeks_str` weeks
- **Current trajectory**: `{python} CheckpointDebug.overhead_pct_str` percent overhead extends training to `{python} CheckpointDebug.extended_weeks_str` weeks
...
1. **infrastructure layer**\index{Infrastructure Layer}: Install local Non-Volatile Memory Express (NVMe) staging drives (`{python} CheckpointDebug.nvme_bw_gbs_str` GB/s per node)
2. **Distribution Layer**\index{Distribution Layer}: Implement asynchronous checkpointing:
   - Phase 1: GPU $\rightarrow$ CPU memory copy (fast, `{python} CheckpointDebug.pcie_gbs_str` GB/s PCIe)
   - Phase 2: CPU $\rightarrow$ local NVMe (`{python} CheckpointDebug.nvme_bw_gbs_str` GB/s, training resumes)
   - Phase 3: Background NVMe $\rightarrow$ NFS copy (overlapped with training)
3. **Validation**\index{Validation}: The critical-path data copy drops to approximately `{python} CheckpointDebug.gpu_cpu_parallel_s_str` seconds if GPU-to-CPU staging proceeds in parallel across `{python} CheckpointDebug.n_nodes_str` nodes (or `{python} CheckpointDebug.gpu_cpu_single_path_s_str` seconds if serialized through one `{python} CheckpointDebug.pcie_gbs_str` GB/s path), reducing training impact from `{python} CheckpointDebug.overhead_pct_str` percent to under `{python} CheckpointDebug.residual_overhead_pct_str` percent once background NVMe and NFS copies are overlapped
...
[^fn-sharded-ckpt-io]: **Sharded Checkpointing**: Each worker saves only its local partition rather than gathering state to a single writer. For ZeRO-3 or FSDP with 1,000 workers training a `{python} FaultToleranceShardRecap.gpt3_params_b_str`B model, each worker writes approximately `{python} FaultToleranceShardRecap.gpt3_shard_gb_str` GB instead of one writer handling `{python} FaultToleranceShardRecap.gpt3_ckpt_tb_str` TB, parallelizing I/O across all nodes. The trade-off: recovery requires all shards to be present and consistent, making the checkpoint protocol more complex and the failure of any single shard's storage fatal to the entire checkpoint. \index{Sharded Checkpointing!I/O parallelism}

This approach enables efficient checkpointing even for massive models. A `{python} FaultToleranceShardRecap.gpt3_params_b_str`B parameter model with `{python} FaultToleranceShardRecap.gpt3_ckpt_tb_str` TB checkpoint distributed across 1,000 workers requires each worker to write only `{python} FaultToleranceShardRecap.gpt3_shard_gb_str` GB, achievable in seconds with local NVMe storage.
...
class RecoveryBudget:
    ...
    check(abs(daily_loss_min_10k - 31.0) < 1.0, f"10k daily loss ({daily_loss_min_10k:.1f} min)")

    # ┌── 4. OUTPUT ────────────────────────────────────────
    t_total_str = fmt(t_total_min, precision=1, commas=False)
    t_detect_s_str = fmt(t_detect_s, precision=0, commas=False)
    t_restart_min_str = fmt(t_restart_min, precision=0, commas=False)
    t_load_s_str = fmt(t_load_s, precision=0, commas=False)
    t_warmup_min_str = fmt(t_warmup_min, precision=0, commas=False)
    aggregate_read_gbs_str = fmt(aggregate_read_gbs, precision=0, commas=False)
    shard_read_gbs_str = fmt(shard_read_gbs, precision=0, commas=False)
...
Consider our `{python} FaultToleranceRecoveryRecap.params_b_str`B parameter model training on `{python} FaultToleranceRecoveryRecap.n_workers_str` GPUs. The checkpoint size is approximately `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` TB (weights + Adam optimizer state). How much does a single failure event cost?

**The budget ($T_{\text{recovery}}$)**:

1. **$T_{\text{detect}}$**: `{python} RecoveryBudget.t_detect_s_str` seconds (conservative heartbeat timeout with verification retries)
2. **$T_{\text{restart}}$**: `{python} RecoveryBudget.t_restart_min_str` minutes (scheduler queue time + container launch + Python import overhead + NCCL initialization)
3. **$T_{\text{load}}$**: `{python} RecoveryBudget.t_load_s_str` seconds (this is the aggregate-shared-storage read of the full `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` TB checkpoint at `{python} RecoveryBudget.aggregate_read_gbs_str` GB/s aggregate; with sharded local-NVMe reads at `{python} RecoveryBudget.shard_read_gbs_str` GB/s, each `{python} FaultToleranceRecoveryRecap.gpt3_shard_gb_str` GB shard loads in well under one second)
4. **$T_{\text{warmup}}$**: `{python} RecoveryBudget.t_warmup_min_str` minutes (JIT kernel compilation, data pipeline buffer fill, TCP connection re-establishment)
...
**Impact at scale**\index{Recovery Time!scale impact}: With the GPU-only baseline from @tbl-cluster-mtbf-scaling, a `{python} RecoveryBudget.n_gpus_1k_str`-GPU cluster has MTBF of `{python} RecoveryBudget.mtbf_1k_hrs_str` hours and experiences about `{python} RecoveryBudget.failures_per_day_1k_str` failures per day, losing about `{python} RecoveryBudget.daily_loss_min_1k_str` minutes daily, a modest `{python} RecoveryBudget.overhead_pct_1k_str` percent overhead. However, a `{python} RecoveryBudget.n_gpus_10k_str`-GPU cluster has GPU-only MTBF of `{python} RecoveryBudget.mtbf_10k_hrs_str` hours and experiences about `{python} RecoveryBudget.failures_per_day_10k_str` failures per day, losing about `{python} RecoveryBudget.daily_loss_min_10k_str` minutes daily—a `{python} RecoveryBudget.overhead_pct_10k_str` percent overhead equivalent to wasting roughly `{python} RecoveryBudget.waste_gpu_hours_10k_str` GPU-hours every day (`{python} RecoveryBudget.daily_loss_min_10k_str` min $\times$ `{python} RecoveryBudget.n_gpus_10k_str` GPUs ÷ 60). This is why recovery time optimization matters more as clusters grow.
...
Warm restarts can reduce $T_{\text{load}}$ and $T_{\text{warmup}}$ to near-zero for 99.9 percent of the cluster, cutting total recovery time from minutes to seconds. For our 1,000-GPU example, a warm restart avoids reloading `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` TB from storage, saving the 21-second $T_{\text{load}}$ and 2-minute $T_{\text{warmup}}$ for all but the replacement node.
...
As @fig-elastic-flow illustrates, elastic training converts what would otherwise be hard failures into graceful capacity adjustments: a lost worker reduces the active count rather than aborting the job, the running step adapts to the smaller resource allocation, and preemption in shared clusters becomes a routine rebalancing event rather than a full restart.
...
DeepSpeed [@rasley2020] provides ZeRO-based distributed training and checkpointing building blocks for large models; in elastic deployments, recovery also depends on the surrounding launcher, resource manager, and checkpoint policy.
...
| **DeepSpeed**       | Partial             | Checkpoint-based       | Deployment-dependent | External schedulers     |
| **Ray Train**       | Yes                 | Yes                    | Automatic            | Ray Cluster             |
| **Horovod Elastic** | Yes                 | Yes                    | Manual               | SLURM, Kubernetes       |

: **Elastic Training Framework Comparison**\index{Elastic Training!framework support}: Modern frameworks provide varying levels of support for elastic training, with different approaches to automatic recovery and state management. {#tbl-elastic-training-comparison}
