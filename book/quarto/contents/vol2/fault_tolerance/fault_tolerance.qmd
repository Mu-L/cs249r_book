...
# │
# │ Imports: numpy (np), matplotlib.pyplot (plt), mlsysim.core.viz (viz),
# │          mlsysim.constants (defaults.GPU_MTTF_HOURS)
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
...
mtbf_theoretical = defaults.GPU_MTTF_HOURS/n_theoretical


# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
ax.plot(n_theoretical, mtbf_theoretical, color=COLORS['grid'], linestyle='--',
        linewidth=1.5, label=f'Theoretical: {defaults.GPU_MTTF_HOURS:,}h/N', zorder=1)
...
class SDCCollective:
    ...
    # ┌── 1. LOAD ──────────────────────────────────────────
    n_gpus = 100000
    p_sdc_per_gpu_hr = defaults.P_SDC_PER_GPU_HR # 1 in a million chance per hour
    training_step_s = 2
    # ┌── 2. EXECUTE ───────────────────────────────────────
...
Consider our 175B parameter model training on 1,000 GPUs. The checkpoint size is approximately `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` TB (weights + Adam optimizer state). How much does a single failure event cost?
...
3. **$T_{\text{load}}$**: 21 seconds (this is the aggregate-shared-storage read of the full `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` TB checkpoint at 100 GB/s aggregate; with sharded local-NVMe reads at 5 GB/s, each `{python} FaultToleranceRecoveryRecap.gpt3_shard_gb_str` GB shard loads in well under one second)
