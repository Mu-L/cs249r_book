...
class FaultToleranceCheckpointRecap:
    ...
    aggregate_read_gbs_str = fmt(aggregate_ckpt_bw.m_as(GB / second), precision=0, commas=False)
```
...
For our 10,000-GPU cluster with calculated system MTBF of 3.69 hours (from @sec-fault-tolerance-reliability-reliability-worked-example-cluster-mtbf-calculation-9255) and checkpoint time of `{python} FaultToleranceCheckpointRecap.ckpt_write_s_str` s (from `{python} FaultToleranceCheckpointRecap.gpt3_ckpt_tb_str` TB checkpoint at `{python} FaultToleranceCheckpointRecap.aggregate_read_gbs_str` GB/s), the Young-Daly formula computes the optimal interval.
...
**Problem**: A `{python} YoungDalyOptimal.n_gpus_str`-GPU cluster has an MTBF of `{python} YoungDalyOptimal.mtbf_h_str` hours. A full model checkpoint takes `{python} YoungDalyOptimal.t_write_s_str` s to write. What is the optimal checkpoint frequency?

**Math**:
Apply the Young-Daly formula: $\tau_{\text{opt}} = \sqrt{2 \cdot T_{\text{write}} \cdot \text{MTBF}_{\text{system}}}$

1. **Convert to common units**: $\text{MTBF}_{\text{system}} =$ `{python} YoungDalyOptimal.mtbf_h_str` hr $\times$ `{python} YoungDalyOptimal.seconds_per_hour_str` s/hr = **`{python} YoungDalyOptimal.mtbf_s_str` s**.
2. **Substitute and solve**: With $T_{\text{write}} =$ `{python} YoungDalyOptimal.t_write_s_str` s and $\text{MTBF} =$ `{python} YoungDalyOptimal.mtbf_s_str` s, the formula gives $\tau_{\text{opt}} \approx$ **`{python} YoungDalyOptimal.tau_opt_s_str` s**, or about **`{python} YoungDalyOptimal.tau_opt_min_str` min**.
...
- **Minimum write time**\index{Minimum Write Time}: `{python} CheckpointDebug.total_ckpt_gb_str` GB / `{python} CheckpointDebug.nfs_gbs_str` GB/s = `{python} CheckpointDebug.min_write_s_str` s (`{python} CheckpointDebug.min_write_min_str` min)
...
The Distribution Layer reveals the shared-storage bottleneck. With `{python} CheckpointDebug.n_nodes_str` nodes competing for 10 Gbps, each node effectively receives only `{python} CheckpointDebug.per_node_mbs_str` MB/s (10 Gbps / `{python} CheckpointDebug.n_nodes_str`). If the checkpoint is sharded evenly, each node writes about `{python} CheckpointDebug.per_node_shard_gb_str` GB, so the write still lands near `{python} CheckpointDebug.per_node_write_min_str` min. The observed `{python} CheckpointDebug.observed_write_min_str` min is therefore close to the bandwidth floor, not something more simultaneous NFS writers can solve.
...
3. **Validation**\index{Validation}: The critical-path data copy drops to approximately `{python} CheckpointDebug.gpu_cpu_parallel_s_str` s if GPU-to-CPU staging proceeds in parallel across `{python} CheckpointDebug.n_nodes_str` nodes (or `{python} CheckpointDebug.gpu_cpu_single_path_s_str` s if serialized through one `{python} CheckpointDebug.pcie_gbs_str` GB/s path), reducing training impact from `{python} CheckpointDebug.overhead_pct_str` percent to under `{python} CheckpointDebug.residual_overhead_pct_str` percent once background NVMe and NFS copies are overlapped
...
[^fn-sharded-ckpt-io]: **Sharded Checkpointing**: Each worker saves only its local partition rather than gathering state to a single writer. For ZeRO-3 or FSDP with `{python} FaultToleranceShardRecap.n_workers_str` workers training a `{python} FaultToleranceShardRecap.gpt3_params_b_str`B model, each worker writes approximately `{python} FaultToleranceShardRecap.gpt3_shard_gb_str` GB instead of one writer handling `{python} FaultToleranceShardRecap.gpt3_ckpt_tb_str` TB, parallelizing I/O across all nodes. The trade-off: recovery requires all shards to be present and consistent, making the checkpoint protocol more complex and the failure of any single shard's storage fatal to the entire checkpoint. \index{Sharded Checkpointing!I/O parallelism}

This approach enables efficient checkpointing even for massive models. A `{python} FaultToleranceShardRecap.gpt3_params_b_str`B parameter model with a `{python} FaultToleranceShardRecap.gpt3_ckpt_tb_str` TB checkpoint distributed across `{python} FaultToleranceShardRecap.n_workers_str` workers requires each worker to write only `{python} FaultToleranceShardRecap.gpt3_shard_gb_str` GB, achievable in seconds with local NVMe storage.
...
2. **$T_{\text{restart}}$**: `{python} RecoveryBudget.t_restart_min_str` min (scheduler queue time + container launch + Python import overhead + NCCL initialization)
3. **$T_{\text{load}}$**: `{python} RecoveryBudget.t_load_s_str` s (this is the aggregate-shared-storage read of the full `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` TB checkpoint at `{python} RecoveryBudget.aggregate_read_gbs_str` GB/s aggregate; with sharded local-NVMe reads at `{python} RecoveryBudget.shard_read_gbs_str` GB/s, each `{python} FaultToleranceRecoveryRecap.gpt3_shard_gb_str` GB shard loads in well under one second)
4. **$T_{\text{warmup}}$**: `{python} RecoveryBudget.t_warmup_min_str` min (JIT kernel compilation, data pipeline buffer fill, TCP connection re-establishment)
...
Warm restarts can reduce $T_{\text{load}}$ and $T_{\text{warmup}}$ to near-zero for 99.9 percent of the cluster, cutting total recovery time from minutes to seconds. For our `{python} FaultToleranceRecoveryRecap.n_workers_str`-GPU example, a warm restart avoids reloading `{python} FaultToleranceRecoveryRecap.gpt3_ckpt_tb_str` TB from storage, saving the `{python} RecoveryBudget.t_load_s_str`-second $T_{\text{load}}$ and 2-minute $T_{\text{warmup}}$ for all but the replacement node.
