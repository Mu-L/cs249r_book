...
::: {#exmp-fault-tolerance-recommendation-fallback .callout-example title="Recommendation fallback"}

**Scenario**: Resilience is not always about restoring the primary system; sometimes it is about graceful degradation. A complex deep learning recommendation engine becomes unavailable during a backend outage.

**Failure mode**: The serving infrastructure automatically fails over to a simple "Top-n" popularity model, which has a tiny fraction of the parameters and no personalization.

**Consequence**: The product remains usable but less personalized. This is intentional degradation, not silent success: the fallback path must be monitored so the team knows when quality is being traded for availability.

**Systems lesson**: Complex models often fight for the "last mile" of accuracy, while simple heuristics provide the bulk of the utility.
...
The case studies that follow draw on published Meta OPT-175B training logbooks, Netflix's published chaos-engineering literature, and Microsoft DeepSpeed documentation. Specific operational numbers (failure counts, MFU achieved, and overhead percentages) reflect what those public sources have reported; treat them as representative magnitudes for the engineering pattern rather than precision benchmarks reproducible from this prose alone. Production deployments illustrate how leading organizations implement fault tolerance in ML systems, threading the principles developed throughout this chapter. Meta's OPT-175B training run on a 992-GPU cluster provides a well-documented study in handling dozens of hardware failures over the run without aborting training.
