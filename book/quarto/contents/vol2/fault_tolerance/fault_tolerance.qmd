...
The linear relationship between component count and failure rate has a direct implication: a rare component event becomes a continuous operating condition at fleet scale.
...
This protocol ensures that either all workers complete their writes or the checkpoint is incomplete. Valid checkpoints have complete metadata. Incomplete checkpoints have missing metadata and can be detected. Partial checkpoints can be garbage collected. In practice, the design space is defined by how strictly this coordination is enforced.
