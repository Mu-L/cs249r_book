...
ax.plot(
    tau * 60,
    over_ckpt,
    label='Checkpoint Overhead ($T_{\\mathrm{write}}/\\tau_{\\mathrm{ckpt}}$)',
    color=COLORS['BlueLine'],
    linestyle='--'
)
...
ax.plot(
    tau * 60,
    over_rework,
    label='Expected Rework ($\\tau_{\\mathrm{ckpt}} / 2\\,\\mathrm{MTBF}$)',
    color=COLORS['RedLine'],
    linestyle='--'
)
...
#—Annotation ---
ax.annotate(
    f'$\\tau_{{\\mathrm{{opt}}}} \\approx {tau_opt*60:.0f}$ min',
...
ax.set_xlabel('Checkpoint Interval $\\tau_{\\mathrm{ckpt}}$ (minutes)')
