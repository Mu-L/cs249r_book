...
# │ How: Multiply Models.Language.GPT3.parameters.m_as(param) by bytes-per-param for each state;
# │   convert result pint Quantity with .m_as(TB) and .m_as(GB).
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT3.parameters, param, byte, TB, GB, BILLION),
...
class FaultToleranceSetup:
    ...
    # Use library model for memory decomposition
    # Note: Scenario is pure FP32 training (12 bytes/param: 4 weights + 8 Adam)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt3_params_b_str = fmt(m.parameters.m_as(param) / BILLION, precision=1, allow_zero=True, commas=False)
...
class NinesReliability:
    ...
    cluster_pct_str = fmt(int(round(cluster_prob * 100)), precision=1, allow_zero=True, commas=False)
```
...
class V100Spec:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_mem_bw_str = fmt(mem_bw_gbs, precision=1, allow_zero=True)
```
...
class YoungDalyOptimal:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    t_write_s_str = fmt(t_write_s, precision=1, allow_zero=True, commas=False)
    mtbf_h_str = fmt(mtbf_h, precision=2, commas=False)
    mtbf_s_str = fmt(mtbf_s, precision=1, allow_zero=True)
    radicand_str = fmt(radicand, precision=1, allow_zero=True)
    tau_opt_s_str = fmt(tau_opt_s, precision=1, allow_zero=True)
...
class CheckpointDebug:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weights_gb_str = fmt(m.parameters.m_as(param) * 2 / BILLION, precision=1, allow_zero=True, commas=False)
    optimizer_gb_str = fmt(m.parameters.m_as(param) * 8 / BILLION, precision=1, allow_zero=True, commas=False)
    total_ckpt_gb_str = fmt(total_ckpt_gb_val, precision=1, allow_zero=True, commas=False)
    nfs_gbs_str = fmt(nfs_bw.m_as(GB/second), precision=2, commas=False)
    min_write_s_str = fmt(min_write_s_val, precision=1, allow_zero=True, commas=False)
    min_write_min_str = fmt(min_write_min_val, precision=1, commas=False)
    per_node_mbs_str = fmt(nfs_bw.m_as(MB/second) / n_nodes, precision=0, commas=False)
    ...
    extra_cost_k_str = fmt(extra_cost_k, precision=1, allow_zero=True, commas=False)
```
...
**Impact at scale**\index{Recovery Time!scale impact}: With the GPU-only baseline from @tbl-cluster-mtbf-scaling, a 1,000-GPU cluster has MTBF of 50 hours and experiences about 0.48 failures per day, losing about 3 minutes daily, a modest 0.2 percent overhead. However, a 10,000-GPU cluster has GPU-only MTBF of 5 hours and experiences about 4.8 failures per day, losing about 31 minutes daily—a 2.1 percent overhead equivalent to wasting roughly 5,000 GPU-hours every day (31 min $\times$ 10,000 GPUs ÷ 60). This is why recovery time optimization matters more as clusters grow.
...
class StragglerTax:
    ...
    throttle_pct_str = fmt(int(throttle_factor * 100), precision=1, allow_zero=True, commas=False)
    straggler_step_str = fmt(straggler_step_s, precision=1, commas=False)
    healthy_gpus_str = fmt(healthy_gpus, precision=1, allow_zero=True)
    ...
    throughput_loss_str = fmt(int(throughput_loss_pct), precision=1, allow_zero=True, commas=False)
    gpu_cost_str = fmt(int(gpu_cost_per_hour), precision=1, allow_zero=True, commas=False)
    wasted_cost_str = fmt(wasted_cost_per_hour, precision=1, allow_zero=True)
```
