...
class FaultToleranceSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gpt3_params_b_str = fmt(m.parameters.m_as(param) / BILLION, precision=1, commas=False)
...
class NinesReliability:
    ...
    cluster_pct_str = fmt(int(round(cluster_prob * 100)), precision=1, commas=False)
```
...
class V100Spec:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    v100_mem_bw_str = fmt(mem_bw_gbs, precision=1)
```
...
class YoungDalyOptimal:
    ...
    mtbf_s_str = fmt(mtbf_s, precision=1)
    radicand_str = fmt(radicand, precision=1)
...
class CheckpointDebug:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    weights_gb_str = fmt(m.parameters.m_as(param) * 2 / BILLION, precision=1, commas=False)
    optimizer_gb_str = fmt(m.parameters.m_as(param) * 8 / BILLION, precision=1, commas=False)
    total_ckpt_gb_str = fmt(total_ckpt_gb_val, precision=1, commas=False)
    nfs_gbs_str = fmt(nfs_bw.m_as(GB/second), precision=2, commas=False)
    min_write_s_str = fmt(min_write_s_val, precision=1, commas=False)
    ...
    extra_cost_k_str = fmt(extra_cost_k, precision=1, commas=False)
```
...
class StragglerTax:
    ...
    throttle_pct_str = fmt(int(throttle_factor * 100), precision=1, commas=False)
    straggler_step_str = fmt(straggler_step_s, precision=1, commas=False)
    healthy_gpus_str = fmt(healthy_gpus, precision=1)
    ...
    throughput_loss_str = fmt(int(throughput_loss_pct), precision=1, commas=False)
    gpu_cost_str = fmt(int(gpu_cost_per_hour), precision=1, commas=False)
    wasted_cost_str = fmt(wasted_cost_per_hour, precision=1)
```
