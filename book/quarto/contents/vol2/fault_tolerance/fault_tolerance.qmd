...
# │ Show: gpt3_ckpt_tb_str="2.1" TB (full checkpoint),
# │   gpt3_shard_gb_str="2.1" GB (per-worker shard at 1000 workers)—inline in prose.
...
# │ Exports: gpt3_params_b_str, gpt3_ckpt_tb_str, gpt3_adam_tb_str, gpt3_shard_gb_str
# │ Note: PERSISTENT—gpt3_ckpt_tb_str used in §Young-Daly (line ~1957),
# │   §Sharded Checkpointing (line ~2289), §Recovery (line ~2365, ~2385);
# │   gpt3_shard_gb_str used in §Sharded Checkpointing (line ~2289), §Recovery (~2371, ~2385).
# └─────────────────────────────────────────────────────────────────────────────
...
class NinesReliability:
    ...
    cluster_pct_str = fmt(int(round(cluster_prob * 100)), precision=0, commas=False)
```
...
# │ How: GPU_MTTF_HOURS/N; Kokolis/Llama data; viz.setup_plot().
# │
# │ Imports: numpy (np), matplotlib.pyplot (plt), mlsysim.core.viz (viz),
...
After surviving infant mortality, components enter the useful life phase, where they exhibit relatively constant failure rates [@klutke2003]. This phase represents the longest portion of component lifetime and is the period where the exponential reliability model in @eq-single-component-reliability applies most accurately. For data center GPUs, the useful life phase is best treated as a fleet-specific multiyear operating assumption rather than a universal constant.
...
# │
# │ Imports: mlsysim.core.constants (V100_MEM_BW, GB, second), mlsysim.fmt (fmt, check)
...
::: {#nbk-fault-tolerance-napkin-math-sdc-certainty .callout-notebook title="Napkin math: The SDC certainty"}
...
[^fn-adam-state-memory]: **Adam Optimizer State**\index{Adam Optimizer!state}: Adaptive Moment Estimation (Adam) maintains per-parameter first-moment ($m$) and second-moment ($v$) estimates, tripling memory requirements compared to vanilla SGD (3$\times$: parameters + two state vectors). For a `{python} FaultToleranceSetup.gpt3_params_b_str`B parameter model, optimizer state alone reaches `{python} FaultToleranceSetup.gpt3_adam_tb_str` TB, which typically dominates checkpoint size and recovery time, making optimizer state the primary bottleneck in checkpoint I/O design. \index{Adam Optimizer!checkpoint memory}
...
For our 10,000-GPU cluster with calculated system MTBF of 3.69 hours (from @sec-fault-tolerance-reliability-reliability-worked-example-cluster-mtbf-calculation-9255) and checkpoint time of 21 seconds (from `{python} FaultToleranceSetup.gpt3_ckpt_tb_str` TB checkpoint at 100 GB/s), the Young-Daly formula computes the optimal interval.
...
# │ Exports: t_save_s_str, mtbf_h_str, mtbf_s_str, radicand_str,
...
1. **Convert to common units**: MTBF = `{python} YoungDalyOptimal.mtbf_h_str` $\times$ 3,600s = `{python} YoungDalyOptimal.mtbf_s_str` seconds.
2. **Calculate**: With $T_{\text{write}} =$ `{python} YoungDalyOptimal.t_save_s_str` seconds and MTBF = `{python} YoungDalyOptimal.mtbf_s_str` seconds, the Young-Daly radicand is `{python} YoungDalyOptimal.radicand_str`, so $\tau_{\text{opt}} \approx$ `{python} YoungDalyOptimal.tau_opt_s_str` seconds.
...
class CheckpointDebug:
    ...
    per_node_shard_gb_val = total_ckpt_gb_val / n_nodes
    per_node_write_min_val = (per_node_shard_gb_val/per_node_gbs_val) / 60
    gpu_cpu_single_path_s_val = total_ckpt_gb_val/pcie_gbs
    gpu_cpu_parallel_s_val = per_node_shard_gb_val/pcie_gbs
    ...
    check(9 < per_node_write_min_val < 10, "Per-node shard write should match aggregate bandwidth floor")
    check(0.3 < gpu_cpu_parallel_s_val < 0.4, "Parallel GPU-to-CPU staging should be sub-second per node")
    ...
    nfs_gbs_str = fmt(nfs_gbs_val, precision=2, commas=False)
    ...
    per_node_mbs_str = fmt(per_node_mbs_val, precision=0, commas=False)
    per_node_shard_gb_str = fmt(per_node_shard_gb_val, precision=1, commas=False)
    per_node_write_min_str = fmt(per_node_write_min_val, precision=1, commas=False)
    ...
    extended_weeks_str = fmt(extended_weeks_val, precision=1, commas=False)
...
The Distribution Layer reveals the shared-storage bottleneck. With 64 nodes competing for 10 Gbps, each node effectively receives only `{python} CheckpointDebug.per_node_mbs_str` MB/s (10 Gbps/64 nodes). If the checkpoint is sharded evenly, each node writes about `{python} CheckpointDebug.per_node_shard_gb_str` GB, so the write still lands near `{python} CheckpointDebug.per_node_write_min_str` minutes. The observed 10 minutes is therefore close to the bandwidth floor, not something more simultaneous NFS writers can solve.
...
3. **Validation**\index{Validation}: The critical-path data copy drops to approximately `{python} CheckpointDebug.gpu_cpu_parallel_s_str` seconds if GPU-to-CPU staging proceeds in parallel across 64 nodes (or `{python} CheckpointDebug.gpu_cpu_single_path_s_str` seconds if serialized through one 32 GB/s path), reducing training impact from 30 percent to under 1 percent once background NVMe and NFS copies are overlapped
...
[^fn-sharded-ckpt-io]: **Sharded Checkpointing**: Each worker saves only its local partition rather than gathering state to a single writer. For ZeRO-3 or FSDP with 1,000 workers training a `{python} FaultToleranceSetup.gpt3_params_b_str`B model, each worker writes approximately `{python} FaultToleranceSetup.gpt3_shard_gb_str` GB instead of one writer handling `{python} FaultToleranceSetup.gpt3_ckpt_tb_str` TB, parallelizing I/O across all nodes. The trade-off: recovery requires all shards to be present and consistent, making the checkpoint protocol more complex and the failure of any single shard's storage fatal to the entire checkpoint. \index{Sharded Checkpointing!I/O parallelism}

This approach enables efficient checkpointing even for massive models. A `{python} FaultToleranceSetup.gpt3_params_b_str`B parameter model with `{python} FaultToleranceSetup.gpt3_ckpt_tb_str` TB checkpoint distributed across 1,000 workers requires each worker to write only `{python} FaultToleranceSetup.gpt3_shard_gb_str` GB, achievable in seconds with local NVMe storage.
...
Consider our 175B parameter model training on 1,000 GPUs. The checkpoint size is approximately `{python} FaultToleranceSetup.gpt3_ckpt_tb_str` TB (weights + Adam optimizer state). How much does a single failure event cost?
...
3. **$T_{\text{load}}$**: 21 seconds (this is the aggregate-shared-storage read of the full `{python} FaultToleranceSetup.gpt3_ckpt_tb_str` TB checkpoint at 100 GB/s aggregate; with sharded local-NVMe reads at 5 GB/s, each `{python} FaultToleranceSetup.gpt3_shard_gb_str` GB shard loads in well under one second)
...
Warm restarts can reduce $T_{\text{load}}$ and $T_{\text{warmup}}$ to near-zero for 99.9 percent of the cluster, cutting total recovery time from minutes to seconds. For our 1,000-GPU example, a warm restart avoids reloading `{python} FaultToleranceSetup.gpt3_ckpt_tb_str` TB from storage, saving the 21-second $T_{\text{load}}$ and 2-minute $T_{\text{warmup}}$ for all but the replacement node.
...
class StragglerTax:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    n_gpus_str = fmt(n_gpus, precision=0)
    normal_step_str = fmt(normal_step_s, precision=1, commas=False)
    throttle_pct_str = fmt(int(throttle_factor * 100), precision=0, commas=False)
    straggler_step_str = fmt(straggler_step_s, precision=1, commas=False)
    ...
    throughput_loss_str = fmt(int(throughput_loss_pct), precision=0, commas=False)
    gpu_cost_str = fmt(int(gpu_cost_per_hour), precision=0, commas=False)
...
::: {#ws-fault-tolerance-recommendation-fallback .callout-war-story title="The recommendation fallback"}

**The context**: Resilience is not always about restoring the primary system; sometimes it is about graceful degradation. During a major data center outage, a leading e-commerce platform's complex deep learning recommendation engine (DLRM) became unavailable.

**The failure**: The serving infrastructure automatically failed over to a simple "Top-n" popularity model, which had 1/100th the parameters and zero personalization.

**The consequence**: This fallback persisted for 3 hours before engineers noticed, because the revenue impact was only **2 percent**.

**The systems lesson**: Complex models often fight for the "last mile" of accuracy, while simple heuristics provide the bulk of the utility.
...
Netflix's published Chaos Engineering work frames resilience as an empirical practice for complex software-based distributed systems: deliberately inject failures, observe system behavior, and use the result to harden recovery paths [@basiri2016]. The same discipline applies to real-time ML serving, where feature stores, embedding services, model servers, and fallback paths create a distributed system whose quality can degrade without a clean crash.

Representative ML-serving chaos experiments include adding latency to feature retrieval, serving stale embeddings in staging, corrupting a model artifact before deployment validation, skewing feature distributions to test drift monitors, and killing inference replicas during peak-like traffic to expose retry storms. The expected recovery behavior is a fallback hierarchy: switch from the primary model to a cheaper model, cached response, or popularity-based result before user-facing SLOs collapse.

The useful metric is not whether the injected fault is dramatic but whether the system detects it quickly, contains blast radius, and preserves an acceptable response. Resilience in ML serving is not a static property but a continuous practice of active verification.
