...
class ClusterEconomicsContext:
    ...
    util_low = 0.50
    util_high = 0.80
    a100_mem = Hardware.Cloud.A100.memory_capacity
    h100_mem = Hardware.Cloud.H100.memory_capacity
    ...
    a100_mem_str = fmt(a100_mem.m_as(GiB), precision=0, commas=False)
    h100_mem_str = fmt(h100_mem.m_as(GiB), precision=0, commas=False)
```
...
**Problem**: A `{python} GangSchedulingDeadlock.gs_n_gpus_str`-GPU cluster receives two frontier training jobs from separate teams, each requiring the full cluster. A naive non-gang scheduler allocates `{python} GangSchedulingDeadlock.gs_partial_gpus_str` GPUs to Team A and `{python} GangSchedulingDeadlock.gs_partial_gpus_str` to Team B, then waits for more GPUs to become available. What is the steady-state outcome?
...
At `{python} QueuingTheory.rho_pct_str` percent utilization $(\rho = 0.8)$: with an exponential workload $(C_s = 1)$, $W_q =$ `{python} QueuingTheory.wq_uniform_str` $\times$ the average job duration. With a typical ML workload $(C_s = 3)$, $W_q =$ `{python} QueuingTheory.wq_ml_str` $\times$ the average job duration. This explains the "utilization wall" in ML infrastructure. A web server cluster feels responsive at `{python} QueuingTheory.rho_pct_str` percent load, but an ML cluster at the same utilization feels broken, with jobs languishing in the queue for days. The heavy tail of the service distribution acts as a latency multiplier, forcing operators to run ML clusters at lower utilization (often 60 to 70 percent) to maintain acceptable responsiveness for interactive users.
...
class FleetFragmentation:
    ...
    nvlink_h100_val = h_h100.memory_bw.m_as(GB/second)
...
Consider a 64-node cluster with 8 GPUs per node, totaling `{python} FleetFragmentation.total_gpus_str` GPUs. If jobs request 6 GPUs each, each job occupies one full node but wastes 2 GPUs per node, reducing effective capacity to `{python} FleetFragmentation.effective_capacity_pct_str` percent. The remaining 2 GPUs per node cannot be combined across nodes because GPU workloads require local memory access. This **fragmentation** grows worse with heterogeneous job sizes: a mix of 1-GPU, 3-GPU, and 7-GPU jobs creates irregular gaps distributed across many nodes, where no single pending job can fit into any individual gap, yet the total free capacity would be sufficient if the gaps were contiguous.
...
class ClusterEconomicsIdleRecap:
    ...
    savings_annual_m_str = fmt(savings_annual_m, precision=0, commas=False)
```
...
- **At `{python} ClusterEconomicsIdleRecap.util_low_pct_str` percent utilization**: `{python} ClusterEconomicsIdleRecap.productive_low_str` GPUs productive, `{python} ClusterEconomicsIdleRecap.idle_low_str` idle = \$`{python} ClusterEconomicsIdleRecap.waste_low_str`/day wasted
- **At `{python} ClusterEconomicsIdleRecap.util_high_pct_str` percent utilization**: `{python} ClusterEconomicsIdleRecap.productive_high_str` GPUs productive, `{python} ClusterEconomicsIdleRecap.idle_high_str` idle = \$`{python} ClusterEconomicsIdleRecap.waste_high_str`/day wasted
- **Improvement value**: Moving from `{python} ClusterEconomicsIdleRecap.util_low_pct_str` percent to `{python} ClusterEconomicsIdleRecap.util_high_pct_str` percent saves \$`{python} ClusterEconomicsIdleRecap.savings_daily_str`/day, or about \$`{python} ClusterEconomicsIdleRecap.savings_annual_m_str`M/year
...
class ClusterEconomicsMigRecap:
    from mlsysim import Hardware
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_str = fmt(Hardware.Cloud.A100.memory_capacity.m_as(GiB), precision=0, commas=False)
    h100_mem_str = fmt(Hardware.Cloud.H100.memory_capacity.m_as(GiB), precision=0, commas=False)
```

**Multi-Instance GPU (MIG)**[^fn-mig-isolation] technology addresses this inefficiency by partitioning A100 and H100 GPUs into hardware-isolated instances. An A100 with `{python} ClusterEconomicsMigRecap.a100_mem_str` GB of memory can be divided into configurations ranging from 7 small instances of approximately 10 GB each to 2 large instances of approximately 40 GB each. Unlike software-based GPU sharing (time-slicing), MIG provides hardware isolation: each instance has dedicated memory, cache, and compute resources, preventing one workload from interfering with another and eliminating the noisy neighbor[^fn-noisy-neighbor-etymology] effect. The device plugin exposes MIG instances as separate schedulable resources, allowing the scheduler to treat each instance as an independent GPU for allocation purposes.
...
class FleetTopologyInterconnect:
    ...
    ib_ndr = Systems.Fabrics.InfiniBand_NDR
    nvlink_h100_val = h_h100.memory_bw.m_as(GB/second)
    nvlink_a100_val = h_a100.memory_bw.m_as(GB/second)
    ib_ndr_gbps_val = ib_ndr.bandwidth.m_as('Gbps')
    ib_ndr_gbs_val = ib_ndr.bandwidth.m_as(GB/second)
    activation_gb = 1
    # ┌── 2. EXECUTE ───────────────────────────────────────
    nvlink_xfer_ms = activation_gb / nvlink_h100_val * 1000
    ib_xfer_ms = activation_gb / ib_ndr_gbs_val * 1000
    xfer_slowdown = ib_xfer_ms / nvlink_xfer_ms
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(abs(nvlink_xfer_ms - 1.1) < 0.2, f"NVLink xfer ms unexpected: {nvlink_xfer_ms}")
    check(abs(ib_xfer_ms - 20) < 2, f"IB xfer ms unexpected: {ib_xfer_ms}")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_str = fmt(nvlink_h100_val, precision=0)
    nvlink_a100_str = fmt(nvlink_a100_val, precision=0)
    ib_ndr_str = fmt(ib_ndr_gbps_val, precision=0)
    ib_ndr_gbs_str = fmt(ib_ndr_gbs_val, precision=0)
    activation_gb_str = fmt(activation_gb, precision=0, commas=False)
    nvlink_xfer_ms_str = fmt(nvlink_xfer_ms, precision=1, commas=False)
    ib_xfer_ms_str = fmt(ib_xfer_ms, precision=0, commas=False)
...
Within a single node, GPUs communicate via **NVLink**\index{NVLink}, providing `{python} FleetTopologyInterconnect.nvlink_a100_str` GB/s per GPU on A100 systems and `{python} FleetTopologyInterconnect.nvlink_h100_str` GB/s on H100 systems. This high-bandwidth, low-latency interconnect enables efficient tensor parallelism, where matrix operations are split across GPUs that must exchange intermediate results (activations and gradients) at every transformer layer. The NVLink bandwidth is sufficient to overlap communication with computation for most model architectures, meaning tensor parallel operations can proceed at near-ideal throughput when confined to a single node.

Between nodes within the same rack or **leaf switch domain**\index{Leaf Switch Domain}, communication traverses InfiniBand or RoCE at `{python} FleetTopologyInterconnect.ib_ndr_str` Gbps (NDR InfiniBand), roughly `{python} FleetTopologyInterconnect.ib_ndr_gbs_str` GB/s per link. This represents an order of magnitude reduction from NVLink bandwidth. Data parallelism, which requires AllReduce of gradients after each training step, operates efficiently at this level because the gradient synchronization pattern allows overlap between communication and the next iteration's forward pass. The latency penalty is modest (microseconds vs. nanoseconds), and the aggregate bandwidth is sufficient for gradient tensors that are typically smaller than the activation tensors exchanged in tensor parallelism.
...
- Tensor parallel groups: 32 groups of 8 GPUs, each confined to one 8-GPU node (NVLink: `{python} FleetTopologyInterconnect.nvlink_h100_str` GB/s)
- Pipeline stages: 4 consecutive nodes within the same rack (1 switch hop, InfiniBand: `{python} FleetTopologyInterconnect.ib_ndr_gbs_str` GB/s)
...
The tensor parallel communication alone illustrates the impact. With NVLink at `{python} FleetTopologyInterconnect.nvlink_h100_str` GB/s, a `{python} FleetTopologyInterconnect.activation_gb_str` GB activation transfer takes approximately `{python} FleetTopologyInterconnect.nvlink_xfer_ms_str` ms. Over InfiniBand at `{python} FleetTopologyInterconnect.ib_ndr_gbs_str` GB/s, the same transfer takes approximately `{python} FleetTopologyInterconnect.ib_xfer_ms_str` ms, an `{python} FleetTopologyInterconnect.xfer_slowdown_str`$\times$ slowdown. Since tensor parallelism communicates at every transformer layer (typically 80+ layers for large models), this difference compounds to 15 to 30 percent total training throughput degradation.
...
In that adaptive view, resource allocation becomes a contract that can change during execution.

::: {#dfn-fleet-orchestration-elastic-training .callout-definition title="Elastic training"}
...
Horovod's original design [@sergeev2018horovod] provides the ring AllReduce foundation for Horovod-based training. Elastic variants build on that foundation: when workers change, the training job must pause briefly while the communication topology is rebuilt and workers synchronize their state.
...
class ElasticScaling:
    ...
    es_rescale_str = fmt(rescale_h, precision=2, commas=False)
    es_rescale_min_str = fmt(rescale_min, precision=0, commas=False)
    es_rescale_break_even_h_str = fmt(2, precision=0, commas=False)
    ...
    s2_gpus = 512
    s3_gpus = 256
    es_s1_gpus_str = fmt(s1_gpus, precision=0, commas=False)
    es_window_h_str = fmt(window_h, precision=0, commas=False)
    es_wait_h_str = fmt(wait_h, precision=0, commas=False)
    es_s2_gpus_str = fmt(s2_gpus, precision=0, commas=False)
    es_s2_run_h_str = fmt(s2_run_h, precision=0, commas=False)
    es_s3_gpus_str = fmt(s3_gpus, precision=0, commas=False)
    es_peak_gpus_str = fmt(s2_gpus, precision=0, commas=False)
    es_gpus_128_str = fmt(s1_gpus, precision=0, commas=False)
    es_gpus_256_str = fmt(s3_gpus, precision=0, commas=False)
    es_tp_128_str = fmt(tp_128, precision=1, commas=False)
    es_tp_256_str = fmt(tp_256, precision=1, commas=False)
    es_tp_512_str = fmt(tp_512, precision=1, commas=False)
    es_eff_256_pct_str = fmt(tp_256 / (2 * tp_128) * 100, precision=0, commas=False)
    es_eff_512_pct_str = fmt(tp_512 / (4 * tp_128) * 100, precision=0, commas=False)
    es_vs_wait_pct_str = fmt(elastic_vs_wait_pct, precision=0, commas=False)
    es_vs_imm_mult_str = fmt(elastic_vs_immediate, precision=0, commas=False)
```
...
**Assumptions**: Throughput at `{python} ElasticScaling.es_gpus_128_str` GPUs is `{python} ElasticScaling.es_tp_128_str` epoch/hour (baseline). Throughput at `{python} ElasticScaling.es_gpus_256_str` GPUs is `{python} ElasticScaling.es_tp_256_str` epochs/hour (`{python} ElasticScaling.es_eff_256_pct_str` percent scaling efficiency). Throughput at `{python} ElasticScaling.es_s2_gpus_str` GPUs is `{python} ElasticScaling.es_tp_512_str` epochs/hour (`{python} ElasticScaling.es_eff_512_pct_str` percent scaling efficiency). Re-scaling cost is `{python} ElasticScaling.es_rescale_min_str` minutes (`{python} ElasticScaling.es_rescale_str` hours) to checkpoint, re-shard, and restart.

**Strategy 1 -- Immediate start (no scaling)**: The job starts immediately with `{python} ElasticScaling.es_s1_gpus_str` GPUs and runs for the full `{python} ElasticScaling.es_window_h_str` hours. Total work: $24 \times 1.0 =$ `{python} ElasticScaling.es_s1_str` epochs.

**Strategy 2 -- Wait for capacity (gang scheduling)**: The job waits in the queue for `{python} ElasticScaling.es_wait_h_str` hours until `{python} ElasticScaling.es_s2_gpus_str` GPUs become available, then runs for `{python} ElasticScaling.es_s2_run_h_str` hours. Total work: $20 \times 3.2 =$ `{python} ElasticScaling.es_s2_str` epochs.

**Strategy 3 -- Elastic scaling (step-up)**: The job starts immediately with `{python} ElasticScaling.es_s3_gpus_str` GPUs. After `{python} ElasticScaling.es_wait_h_str` hours, `{python} ElasticScaling.es_s3_gpus_str` more GPUs become available. The job pauses, scales up, and resumes. Phase 1: $4 \times 1.8 =$ `{python} ElasticScaling.es_s3p1_str` epochs. Phase 2: `{python} ElasticScaling.es_s3p2_math` epochs. Total work: `{python} ElasticScaling.es_s3p1_str` + `{python} ElasticScaling.es_s3p2_str` = `{python} ElasticScaling.es_s3_str` epochs.

The elastic strategy outperforms waiting by approximately `{python} ElasticScaling.es_vs_wait_pct_str` percent and the immediate small-scale run by nearly `{python} ElasticScaling.es_vs_imm_mult_str`$\times$. However, if the re-scaling cost exceeded `{python} ElasticScaling.es_rescale_break_even_h_str` hours (due to slow checkpoint transfer or compilation), Strategy 2 would become superior.
...
class SpotTrainingEconomics:
    ...
    restart_total_hours_str = fmt(restart_total_hours, precision=0, commas=False)
    spot_total_hours_str = fmt(spot_total_hours, precision=1, commas=False)
    ...
    spot_gpus_str = fmt(gpus, precision=0, commas=False)
    spot_run_hours_str = fmt(run_hours, precision=0, commas=False)
    checkpoint_overhead_pct_str = MarkdownStr("5 percent")
    restart_interruptions_str = fmt(interruptions, precision=0, commas=False)
    restart_hours_each_str = fmt(restart_hours, precision=1, commas=False)
    restart_min_str = fmt(restart_hours * 60, precision=0, commas=False)
    ...
    spot_discount_pct_str = fmt((1 - spot_rate / on_demand_rate) * 100, precision=0, commas=False)
```

::: {#psp-fleet-orchestration-spot-vs-demand-training .callout-perspective title="Spot vs. on-demand training"}

Consider training a `{python} SpotTrainingEconomics.model_params_b_str` parameter model requiring `{python} SpotTrainingEconomics.spot_gpus_str` GPUs for `{python} SpotTrainingEconomics.spot_run_days_str` days.

**On-demand**\index{On-Demand Capacity}: `{python} SpotTrainingEconomics.spot_gpus_str` GPUs $\times$ `{python} SpotTrainingEconomics.spot_run_hours_str` hours $\times$ \$2.00/GPU-hour = **\$`{python} SpotTrainingEconomics.on_demand_cost_str`**

**Spot** (`{python} SpotTrainingEconomics.spot_discount_pct_str` percent discount, `{python} SpotTrainingEconomics.checkpoint_overhead_pct_str` checkpoint overhead, `{python} SpotTrainingEconomics.restart_interruptions_str` interruption/day with `{python} SpotTrainingEconomics.restart_min_str` min restart):
...
- Checkpoint overhead: `{python} SpotTrainingEconomics.checkpoint_overhead_pct_str` $\times$ `{python} SpotTrainingEconomics.spot_run_hours_str` hours = `{python} SpotTrainingEconomics.checkpoint_extra_hours_str` extra hours
- Restart overhead: `{python} SpotTrainingEconomics.restart_interruptions_str` interruptions $\times$ `{python} SpotTrainingEconomics.restart_hours_each_str` hours = `{python} SpotTrainingEconomics.restart_total_hours_str` hours
- Total time: `{python} SpotTrainingEconomics.spot_run_hours_str` + `{python} SpotTrainingEconomics.checkpoint_extra_hours_str` + `{python} SpotTrainingEconomics.restart_total_hours_str` = `{python} SpotTrainingEconomics.spot_total_hours_str` hours
...
**Savings**: \$`{python} SpotTrainingEconomics.savings_str` (`{python} SpotTrainingEconomics.savings_pct_str` percent), at the cost of approximately `{python} SpotTrainingEconomics.longer_pct_str` percent longer wall-clock time.
...
This failure mode is not a rejection of ML-aware scheduling; it is a reminder that each scheduler must specify the workload class, signal, and objective it is allowed to optimize.
...
Consider a serving fleet with `{python} AutoscalingLag.as_replicas_str` replicas, each capable of `{python} AutoscalingLag.as_qps_per_replica_str` queries per second (QPS) while meeting the `{python} AutoscalingLag.as_slo_ms_str` ms SLO. Total capacity: `{python} AutoscalingLag.as_capacity_str` QPS.

**Scenario**: Traffic ramps from `{python} AutoscalingLag.as_traffic_start_str` QPS to `{python} AutoscalingLag.as_traffic_end_str` QPS linearly over `{python} AutoscalingLag.as_ramp_sec_str` seconds. Model load time: `{python} AutoscalingLag.as_load_time_min_str` minutes (`{python} AutoscalingLag.as_load_time_str` seconds).

**Reactive scaling**\index{Reactive Scaling}: HPA detects overload at $t =$ `{python} AutoscalingLag.as_detect_str` s (when traffic hits `{python} AutoscalingLag.as_capacity_str` QPS). It requests `{python} AutoscalingLag.as_new_replicas_str` new replicas. These replicas become ready at $t =$ `{python} AutoscalingLag.as_ready_str` s (`{python} AutoscalingLag.as_detect_str` + `{python} AutoscalingLag.as_load_time_str`). From $t =$ `{python} AutoscalingLag.as_detect_str` s to $t =$ `{python} AutoscalingLag.as_ready_str` s, demand exceeds capacity. During the ramp, excess demand grows linearly from 0 to `{python} AutoscalingLag.as_excess_str` QPS; after the ramp reaches `{python} AutoscalingLag.as_traffic_end_str` QPS, the full `{python} AutoscalingLag.as_excess_str` QPS overload persists until the new replicas are ready. The area above capacity is approximately `{python} AutoscalingLag.as_violations_str` requests, all of which are queued or dropped and violate the SLO.

**Predictive scaling**: The scaler forecasts the ramp and triggers scale-up at $t =$ `{python} AutoscalingLag.as_pred_trigger_str` s. The `{python} AutoscalingLag.as_new_replicas_str` new replicas come online at $t = 0$ s, pushing capacity to `{python} AutoscalingLag.as_pred_capacity_str` QPS just as the ramp begins. Zero SLO violations.
...
class GpuSharingRoi:
    ...
    n_large = 10
    large_footprint_gb = (Models.Language.GPT3.parameters.m_as(param) * BYTES_FP16).m_as(GB)  # 175B FP16
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gs_excl_util_str = fmt(excl_util_pct, precision=1)
    gs_excl_total_mem_str = fmt_int(excl_total_mem_gb)
    ...
    gs_saved_pct_str = fmt(saved_pct, precision=1)
    gs_annual_str = fmt(annual_savings, precision=0)
    small_model_params_b = 7
    ...
    gs_a100_mem_str = fmt(a100_mem_gb, precision=0, commas=False)
    gs_small_footprint_str = fmt(small_footprint_gb, precision=0, commas=False)
    ...
    gs_gpus_per_large_str = fmt(gpus_per_large, precision=0, commas=False)
    gs_excl_used_mem_str = fmt(excl_used_mem_gb, precision=0, commas=False)
    gs_large_footprint_str = fmt(large_footprint_gb, precision=0, commas=False)
    gs_gpu_hour_rate_str = fmt(gpu_rate.m_as(USD / hour), precision=0, commas=False)
```

::: {#nbk-fleet-orchestration-gpu-sharing-roi .callout-notebook title="GPU sharing ROI"}

Consider a fleet serving `{python} GpuSharingRoi.gs_n_small_str` distinct `{python} GpuSharingRoi.gs_small_model_b_str` models (`{python} GpuSharingRoi.gs_small_footprint_str` GB footprint each) and `{python} GpuSharingRoi.gs_n_large_str` distinct `{python} GpuSharingRoi.gs_large_model_b_str` models (`{python} GpuSharingRoi.gs_large_footprint_str` GB footprint each).

**Strategy A -- Exclusive access (one model per GPU)**: The `{python} GpuSharingRoi.gs_small_model_b_str` models require `{python} GpuSharingRoi.gs_n_small_str` A100 GPUs (`{python} GpuSharingRoi.gs_a100_mem_str` GB each), achieving only `{python} GpuSharingRoi.gs_excl_util_str` percent memory utilization (`{python} GpuSharingRoi.gs_n_small_str` $\times$ `{python} GpuSharingRoi.gs_small_footprint_str` GB used out of `{python} GpuSharingRoi.gs_excl_total_mem_str` GB total). The `{python} GpuSharingRoi.gs_large_model_b_str` models require `{python} GpuSharingRoi.gs_excl_large_str` GPUs (`{python} GpuSharingRoi.gs_n_large_str` models $\times$ `{python} GpuSharingRoi.gs_gpus_per_large_str` GPUs each for tensor parallelism). Total fleet: `{python} GpuSharingRoi.gs_excl_total_str` GPUs.

**Strategy B -- Mixed sharing (MPS for small, exclusive for large)**: Pack `{python} GpuSharingRoi.gs_models_per_gpu_str` models per A100 for the `{python} GpuSharingRoi.gs_small_model_b_str` models (`{python} GpuSharingRoi.gs_models_per_gpu_str` $\times$ `{python} GpuSharingRoi.gs_small_footprint_str` = `{python} GpuSharingRoi.gs_shared_mem_str` GB, well within `{python} GpuSharingRoi.gs_a100_mem_str` GB), requiring only `{python} GpuSharingRoi.gs_shared_gpus_str` GPUs at `{python} GpuSharingRoi.gs_shared_util_str` percent memory utilization. The `{python} GpuSharingRoi.gs_large_model_b_str` models remain unchanged at `{python} GpuSharingRoi.gs_excl_large_str` GPUs. Total fleet: `{python} GpuSharingRoi.gs_shared_total_str` GPUs.

Sharing reduces the fleet size by `{python} GpuSharingRoi.gs_saved_pct_str` percent (`{python} GpuSharingRoi.gs_saved_gpus_str` fewer GPUs). At \$`{python} GpuSharingRoi.gs_gpu_hour_rate_str`/GPU-hour, this saves approximately \$`{python} GpuSharingRoi.gs_annual_str` annually in compute costs, purely through scheduling policy.
...
class DiurnalGpuShift:
    ...
    ds_serving_peak_str = fmt(serving_peak, precision=0, commas=False)
    ds_serving_avg_str = fmt(serving_avg, precision=0, commas=False)
    ds_training_fixed_str = fmt(training_fixed, precision=0, commas=False)
    ds_fleet_gpus_str = fmt(fleet, precision=0, commas=True)
    ds_gpu_hour_rate_str = fmt(gpu_rate.m_as(USD / hour), precision=0, commas=False)
    ds_serving_peak_util_str = fmt(serving_avg / serving_peak * 100, precision=1, commas=False)
    ds_safety_buffer_str = fmt(safety_buffer, precision=0, commas=False)
    ds_serving_reclaimable_str = fmt(serving_reclaimable, precision=0, commas=False)
...
Consider a fleet of `{python} DiurnalGpuShift.ds_fleet_gpus_str` GPUs managed under two regimes:

**Scenario A -- Static Partitioning**: The serving pool reserves `{python} DiurnalGpuShift.ds_serving_peak_str` GPUs to handle peak demand, but average usage is only `{python} DiurnalGpuShift.ds_serving_avg_str` GPUs (`{python} DiurnalGpuShift.ds_serving_peak_util_str` percent utilization). The training pool reserves `{python} DiurnalGpuShift.ds_training_fixed_str` GPUs, and backlog ensures `{python} DiurnalGpuShift.ds_training_full_util_str` utilization. Result: `{python} DiurnalGpuShift.ds_serving_avg_str` + `{python} DiurnalGpuShift.ds_training_fixed_str` = `{python} DiurnalGpuShift.ds_static_active_str` active GPUs. Fleet utilization: **`{python} DiurnalGpuShift.ds_static_util_str` percent**.

**Scenario B -- Dynamic Sharing**: Serving claims GPUs strictly as needed (average `{python} DiurnalGpuShift.ds_serving_avg_str`). Training reclaims unused serving GPUs (average `{python} DiurnalGpuShift.ds_serving_reclaimable_str`), minus a `{python} DiurnalGpuShift.ds_safety_buffer_str`-GPU safety buffer. The orchestrator shifts approximately `{python} DiurnalGpuShift.ds_reclaimed_str` GPUs between roles twice daily based on diurnal forecasts. Result: serving (`{python} DiurnalGpuShift.ds_serving_avg_str`) + training (`{python} DiurnalGpuShift.ds_training_fixed_str` + `{python} DiurnalGpuShift.ds_reclaimed_str`) = `{python} DiurnalGpuShift.ds_dynamic_active_str` active GPUs (the second training term is reclaimed serving capacity). Fleet utilization: **`{python} DiurnalGpuShift.ds_dynamic_util_str` percent**.

Dynamic sharing recovers `{python} DiurnalGpuShift.ds_reclaimed_str` GPUs of effective capacity. At \$`{python} DiurnalGpuShift.ds_gpu_hour_rate_str`/GPU-hour, this creates approximately \$`{python} DiurnalGpuShift.ds_annual_str` million per year in value without purchasing a single additional accelerator.
...
class ChargebackExample:
    ...
    chargeback_cost_str = MarkdownStr(f"{cost / 1000:.0f},000")
```

This data feeds the utilization dashboard, the cluster operator's primary instrument for fleet health. This view must synthesize per-team utilization against quotas, queue depth by priority class, and the **fragmentation index**\index{Fragmentation Index}, a measure of free GPUs that cannot be allocated due to topology constraints. Crucially, it must track the "cost of chaos": preemption rates, spot instance interruption frequencies, and the recovery overhead they induce. When these metrics are visible, they drive behavior. A 64-GPU 175B scheduling run that lasts 34 days consumes about \$`{python} ChargebackExample.chargeback_cost_str` of capacity (64 GPUs $\times$ `{python} ChargebackExample.chargeback_hours_str` hours $\times$ \$2.50/GPU-hour). Without proper accounting, this cost is invisible to the team that requested it. With chargeback, the team must justify this expenditure against the model's expected business value.
...
That policy works only when quota rules distinguish borrowed idle capacity from guaranteed reservations, which is the design pressure behind the next checkpoint.
...
class FleetDebugNvlinkRecap:
    nvlink_a100_val = Hardware.Cloud.A100.memory_bw.m_as(GB/second)
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_a100_str = fmt(nvlink_a100_val, precision=0)
```
...
|:---------------|----------:|:------------|:---------------------------------------------------------------|:-------------------------|
| **A100-80 GB** |       400 | 80 GB HBM2e | NVLink (`{python} FleetDebugNvlinkRecap.nvlink_a100_str` GB/s) | 50 nodes $\times$ 8 GPUs |
| **A100-40 GB** |       400 | 40 GB HBM2e | NVLink (`{python} FleetDebugNvlinkRecap.nvlink_a100_str` GB/s) | 50 nodes $\times$ 8 GPUs |
...
::: {#nbk-fleet-orchestration-value-policy-debugging .callout-notebook title="The value of policy debugging"}

**Problem**: A platform team manages a `{python} DebugValueCalc.debug_cluster_str`-GPU cluster. Initial monitoring shows `{python} DebugValueCalc.debug_util_before_pct_str` percent average utilization. After a week of policy debugging (capability-based scheduling, topology-aware placement, backfill tuning, and workload-hardware guidance), utilization increases to `{python} DebugValueCalc.debug_util_after_pct_str` percent. What is the financial value of that one week of engineering work?

**Math**:

1. **Productive Gain**: `{python} DebugValueCalc.debug_cluster_str` GPUs $\times$ (`{python} DebugValueCalc.debug_util_after_str` - `{python} DebugValueCalc.debug_util_before_str`) = **`{python} DebugValueCalc.debug_productive_gain_str` fully utilized GPU-equivalents**.
2. **Equivalent Raw Hardware**: `{python} DebugValueCalc.debug_productive_gain_str` GPU-equivalents $\div$ `{python} DebugValueCalc.debug_util_before_str` old utilization = **`{python} DebugValueCalc.debug_recovered_str` raw GPUs**.
