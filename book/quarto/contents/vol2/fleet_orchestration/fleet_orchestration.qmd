...
class FleetSetup:
    ...
    # OUTPUTS
    cluster_size_str = fmt(cluster_size, precision=1)
    daily_cost_str = fmt(daily_cost/THOUSAND, precision=1, commas=False, prefix="$", suffix="K")
    annual_cost_str = fmt(annual_cost/MILLION, precision=1, commas=False, prefix="$", suffix="M")
    total_gpus_str = fmt(total_gpus, precision=1)
    effective_capacity_pct_str = fmt(100 - frag_pct, precision=1, commas=False)
    nvlink_h100_str = fmt(nvlink_h100_val, precision=1)
    nvlink_a100_str = fmt(nvlink_a100_val, precision=1)
    ib_ndr_str = fmt(ib_ndr_gbps_val, precision=1)
    ib_ndr_gbs_str = fmt(ib_ndr_gbs_val, precision=1)
...
class ClusterEconomicsContext:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cluster_size_str = fmt(cluster_size, precision=1)
    gpu_hour_cost_str = fmt(gpu_hour_cost_usd, precision=1, commas=False)
    idle_pct_str = fmt(idle_fraction * 100, precision=1, commas=False)
    util_low_pct_str = fmt(util_low * 100, precision=1, commas=False)
    util_high_pct_str = fmt(util_high * 100, precision=1, commas=False)
    daily_cost_str = fmt(daily_cost, precision=1)
    annual_cost_str = fmt(annual_cost_m, precision=1, commas=False)
    daily_waste_str = fmt(daily_waste, precision=1)
    annual_waste_str = fmt(annual_waste_m, precision=1, commas=False)
    productive_gpu_gain_str = fmt(productive_gpu_gain, precision=1)
    raw_gpu_equivalent_gain_str = fmt(raw_gpu_equivalent_gain, precision=1)
    a100_mem_str = fmt(a100_mem.m_as(GiB), precision=1, commas=False)
    h100_mem_str = fmt(h100_mem.m_as(GiB), precision=1, commas=False)
```
...
class GangSchedulingDeadlock:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gs_n_gpus_str = fmt(n_total_gpus, precision=1, commas=False)
    gs_n_jobs_str = fmt(n_jobs, precision=1, commas=False)
    gs_waste_m_str = fmt(waste_per_hour, precision=1)
...
class QueuingTheory:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    wq_uniform_str = fmt(wq_uniform, precision=1)
    wq_ml_str = fmt(wq_ml, precision=1)
```
...
class SpotTrainingEconomics:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    on_demand_cost_str = fmt(on_demand_cost, precision=1)
    ...
    restart_total_hours_str = fmt(restart_total_hours, precision=1, commas=False)
...
class AutoscalingLag:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    as_capacity_str = fmt(capacity, precision=1)
    as_detect_str = fmt(detect_sec, precision=1)
    as_ready_str = fmt(ready_sec, precision=1)
    as_excess_str = fmt(excess_at_peak, precision=1)
    as_violations_str = fmt(slo_violations, precision=1)
    as_pred_trigger_str = fmt(pred_trigger_sec, precision=1)
    as_pred_capacity_str = fmt(pred_capacity, precision=1)
```
...
class GpuSharingRoi:
    ...
    gs_excl_total_mem_str = fmt(int(excl_total_mem_gb), precision=1)
    gs_excl_large_str = fmt(excl_large_gpus, precision=1)
    gs_excl_total_str = fmt(excl_total, precision=1)
    gs_shared_gpus_str = fmt(shared_small_gpus, precision=1)
    gs_shared_mem_str = fmt(shared_mem_per_gpu, precision=1)
    gs_shared_util_str = fmt(shared_util_pct, precision=1)
    gs_shared_total_str = fmt(shared_total, precision=1)
    gs_saved_gpus_str = fmt(saved_gpus, precision=1)
    gs_saved_pct_str = fmt(saved_pct, precision=1)
    gs_annual_str = fmt(annual_savings, precision=1)
```
...
class DiurnalGpuShift:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ds_static_active_str = fmt(static_active, precision=1)
    ds_static_util_str = fmt(static_util_pct, precision=1)
    ds_dynamic_active_str = fmt(dynamic_active, precision=1)
    ds_dynamic_util_str = fmt(dynamic_util_pct, precision=1)
    ds_reclaimed_str = fmt(reclaimed, precision=1)
...
class ChargebackExample:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    chargeback_hours_str = fmt(hours, precision=1)
...
class PreemptionTax:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    recovery_minutes_str = fmt(recovery_minutes, precision=1, commas=False)
    cost_per_event_str = fmt(cost_per_event, precision=1)
    wasted_hours_per_day_str = fmt(wasted_hours_per_day, precision=1, commas=False)
    wasted_gpu_hours_per_day_str = fmt(wasted_gpu_hours_per_day, precision=1)
```
...
class QuotaHoardingCost:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    hoarding_cost_str = fmt(cost, precision=1)
```
...
class DebugValueCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    debug_cluster_str = fmt(debug_cluster, precision=1, commas=False)
    debug_productive_gain_str = fmt(int(debug_productive_gain), precision=1, commas=False)
    debug_recovered_str = fmt(int(debug_recovered), precision=1, commas=False)
    debug_annual_str    = fmt(debug_annual / MILLION, precision=1, commas=False)
```
