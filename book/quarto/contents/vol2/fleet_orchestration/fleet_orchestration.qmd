...
class FleetTopologyInterconnect:
    ...
    ib_ndr_str = fmt(ib_ndr_gbps_val, precision=0, suffix=" Gbps")
    ib_ndr_gbs_str = fmt(ib_ndr_gbs_val, precision=0)
...
Between nodes within the same rack or **leaf switch domain**\index{Leaf Switch Domain}, communication traverses InfiniBand or RoCE at `{python} FleetTopologyInterconnect.ib_ndr_str` (NDR InfiniBand), roughly `{python} FleetTopologyInterconnect.ib_ndr_gbs_str` GB/s per link. This represents an order of magnitude reduction from NVLink bandwidth. Data parallelism, which requires AllReduce of gradients after each training step, operates efficiently at this level because the gradient synchronization pattern allows overlap between communication and the next iteration's forward pass. The latency penalty is modest (microseconds vs. nanoseconds), and the aggregate bandwidth is sufficient for gradient tensors that are typically smaller than the activation tensors exchanged in tensor parallelism.
...
**Spot** (`{python} SpotTrainingEconomics.spot_discount_pct_str` percent discount, `{python} SpotTrainingEconomics.checkpoint_overhead_pct_str` checkpoint overhead, `{python} SpotTrainingEconomics.restart_interruptions_str` interruptions/day with `{python} SpotTrainingEconomics.restart_min_str` minutes restart):

- Base cost: `{python} SpotTrainingEconomics.spot_gpus_str` GPUs $\times$ `{python} SpotTrainingEconomics.spot_run_hours_str` hours $\times$ \$0.70 = \$`{python} SpotTrainingEconomics.spot_base_cost_str`
...
**Strategy B -- Mixed sharing (MPS for small, exclusive for large)**: Pack `{python} GpuSharingRoi.gs_models_per_gpu_str` models per A100 for the `{python} GpuSharingRoi.gs_small_model_b_str` models (`{python} GpuSharingRoi.gs_models_per_gpu_str` $\times$ `{python} GpuSharingRoi.gs_small_footprint_str` GB = `{python} GpuSharingRoi.gs_shared_mem_str` GB, well within `{python} GpuSharingRoi.gs_a100_mem_str` GB), requiring only `{python} GpuSharingRoi.gs_shared_gpus_str` GPUs at `{python} GpuSharingRoi.gs_shared_util_str` percent memory utilization. The `{python} GpuSharingRoi.gs_large_model_b_str` models remain unchanged at `{python} GpuSharingRoi.gs_excl_large_str` GPUs. Total fleet: `{python} GpuSharingRoi.gs_shared_total_str` GPUs.
...
**Scenario B -- Dynamic Sharing**: Serving claims GPUs strictly as needed (average `{python} DiurnalGpuShift.ds_serving_avg_str` GPUs). Training reclaims unused serving GPUs (average `{python} DiurnalGpuShift.ds_serving_reclaimable_str` GPUs), minus a `{python} DiurnalGpuShift.ds_safety_buffer_str`-GPU safety buffer. The orchestrator shifts approximately `{python} DiurnalGpuShift.ds_reclaimed_str` GPUs between roles twice daily based on diurnal forecasts. Result: serving (`{python} DiurnalGpuShift.ds_serving_avg_str`) + training (`{python} DiurnalGpuShift.ds_training_fixed_str` + `{python} DiurnalGpuShift.ds_reclaimed_str`) = `{python} DiurnalGpuShift.ds_dynamic_active_str` active GPUs (the second training term is reclaimed serving capacity). Fleet utilization: **`{python} DiurnalGpuShift.ds_dynamic_util_str` percent**.
...
**Problem**: A platform team manages a `{python} DebugValueCalc.debug_cluster_str`-GPU cluster. Initial monitoring shows `{python} DebugValueCalc.debug_util_before_pct_str` average utilization. After a week of policy debugging (capability-based scheduling, topology-aware placement, backfill tuning, and workload-hardware guidance), utilization increases to `{python} DebugValueCalc.debug_util_after_pct_str`. What is the financial value of that one week of engineering work?

**Math**:

1. **Productive Gain**: `{python} DebugValueCalc.debug_cluster_str` $\times$ (`{python} DebugValueCalc.debug_util_after_str` - `{python} DebugValueCalc.debug_util_before_str`) = **`{python} DebugValueCalc.debug_productive_gain_str` fully utilized GPU-equivalents**.
2. **Equivalent Raw Hardware**: `{python} DebugValueCalc.debug_productive_gain_str`-equivalents $\div$ `{python} DebugValueCalc.debug_util_before_str` old utilization = **`{python} DebugValueCalc.debug_recovered_str` raw GPUs**.
3. **Annual Value**: `{python} DebugValueCalc.debug_recovered_str` $\times$ \$2/hr $\times$ 8,760 hrs/year $\approx$ **\$`{python} DebugValueCalc.debug_annual_str` Million**.
