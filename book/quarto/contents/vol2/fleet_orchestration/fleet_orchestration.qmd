...
# │ How: Multiply GPU count, GPU-hour cost, and 24 hours; pint .m_as() for hardware specs.
# │
...
class FleetSetup:
    ...
    cluster_size = 10000
    hourly_rate = FLEET_GPU_HOUR_COST_REF.m_as(USD / hour)
    daily_cost = cluster_size * hourly_rate * HOURS_PER_DAY
    ...
    # OUTPUTS
    cluster_size_str = fmt(cluster_size, precision=1, allow_zero=True)
    daily_cost_str = fmt(daily_cost/THOUSAND, precision=1, allow_zero=True, commas=False, prefix="$", suffix="K")
    annual_cost_str = fmt(annual_cost/MILLION, precision=1, allow_zero=True, commas=False, prefix="$", suffix="M")
    total_gpus_str = fmt(total_gpus, precision=1, allow_zero=True)
    effective_capacity_pct_str = fmt(100 - frag_pct, precision=1, allow_zero=True, commas=False)
    nvlink_h100_str = fmt(nvlink_h100_val, precision=1, allow_zero=True)
    nvlink_a100_str = fmt(nvlink_a100_val, precision=1, allow_zero=True)
    ib_ndr_str = fmt(ib_ndr_gbps_val, precision=1, allow_zero=True)
    ib_ndr_gbs_str = fmt(ib_ndr_gbs_val, precision=1, allow_zero=True)
...
# │
# │ Imports: mlsysim.core.constants (GPUS_PER_HOST, Hardware.Cloud.A100.memory.capacity, Hardware.Cloud.H100.memory.capacity,
...
class ClusterEconomicsContext:
    ...
    from mlsysim import Hardware
    cluster_size = 10000
    gpu_hour_cost_usd = FLEET_GPU_HOUR_COST_REF.m_as(USD / hour)
    idle_fraction = 0.30
    util_low = 0.50
    util_high = 0.80
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    cluster_size_str = fmt(cluster_size, precision=1, allow_zero=True)
    gpu_hour_cost_str = fmt(gpu_hour_cost_usd, precision=1, allow_zero=True, commas=False)
    idle_pct_str = fmt(idle_fraction * 100, precision=1, allow_zero=True, commas=False)
    util_low_pct_str = fmt(util_low * 100, precision=1, allow_zero=True, commas=False)
    util_high_pct_str = fmt(util_high * 100, precision=1, allow_zero=True, commas=False)
    daily_cost_str = fmt(daily_cost, precision=1, allow_zero=True)
    annual_cost_str = fmt(annual_cost_m, precision=1, allow_zero=True, commas=False)
    daily_waste_str = fmt(daily_waste, precision=1, allow_zero=True)
    annual_waste_str = fmt(annual_waste_m, precision=1, allow_zero=True, commas=False)
    productive_gpu_gain_str = fmt(productive_gpu_gain, precision=1, allow_zero=True)
    raw_gpu_equivalent_gain_str = fmt(raw_gpu_equivalent_gain, precision=1, allow_zero=True)
    a100_mem_str = fmt(a100_mem.m_as(GiB), precision=1, allow_zero=True, commas=False)
    h100_mem_str = fmt(h100_mem.m_as(GiB), precision=1, allow_zero=True, commas=False)
```
...
class GangSchedulingDeadlock:
    ...
    gpu_hour_cost = FLEET_GPU_HOUR_COST_REF.m_as(USD / hour)
    waste_per_hour = n_total_gpus * gpu_hour_cost
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(waste_per_hour == 2048, f"Waste ${waste_per_hour} unexpected")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gs_n_gpus_str = fmt(n_total_gpus, precision=1, allow_zero=True, commas=False)
    gs_n_jobs_str = fmt(n_jobs, precision=1, allow_zero=True, commas=False)
    gs_waste_m_str = fmt(waste_per_hour, precision=1, allow_zero=True)
...
class QueuingTheory:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    wq_uniform_str = fmt(wq_uniform, precision=1, allow_zero=True)
    wq_ml_str = fmt(wq_ml, precision=1, allow_zero=True)
```
...
Consider a 64-node cluster with 8 GPUs per node, totaling `{python} FleetSetup.total_gpus_str` GPUs. If jobs request 6 GPUs each, each job occupies one full node but wastes 2 GPUs per node, reducing effective capacity to `{python} FleetSetup.effective_capacity_pct_str` percent. The remaining 2 GPUs per node cannot be combined across nodes because GPU workloads require local memory access. This **fragmentation** grows worse with heterogeneous job sizes: a mix of 1-GPU, 3-GPU, and 7-GPU jobs creates irregular gaps distributed across many nodes, where no single pending job can fit into any individual gap, yet the total free capacity would be sufficient if the gaps were contiguous.
...
**Systems lesson**: Binary health checks (up/down) are insufficient. Schedulers need **performance-aware topology monitoring**\index{Performance-Aware Topology Monitoring} that incorporates switch counters, retransmission rates, and collective bandwidth tests. Nodes connected to a "limping" switch should be dynamically **tainted**\index{Taint!topology monitoring} until the hardware is repaired.
...
class ElasticScaling:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    es_s1_str = fmt(s1_work, precision=1, commas=False)
    es_s2_str = fmt(s2_work, precision=1, commas=False)
...
**Strategy 3 -- Elastic scaling (step-up)**: The job starts immediately with 256 GPUs. After 4 hours, 256 more GPUs become available. The job pauses, scales up, and resumes. Phase 1: $4 \times 1.8 =$ `{python} ElasticScaling.es_s3p1_str` epochs. Phase 2: `{python} ElasticScaling.es_s3p2_math` epochs. Total work: `{python} ElasticScaling.es_s3p1_str` + `{python} ElasticScaling.es_s3p2_str` = `{python} ElasticScaling.es_s3_str` epochs.
...
# ┌── 2. ARRAYS ────────────────────────────────────────────────────────────────
gpu_cost_per_hour = FLEET_GPU_HOUR_COST_REF.m_as(USD / hour)
...
**Systems lesson**: A robust scheduler must diversify across regions and instance types, monitor interruption risk, and maintain a "break glass" path to on-demand capacity when the spot market becomes toxic.
...
class SpotTrainingEconomics:
    ...
    on_demand_rate = FLEET_GPU_HOUR_COST_REF.m_as(USD / hour)
    spot_rate = FLEET_SPOT_GPU_HOUR_COST_REF.m_as(USD / hour)
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    on_demand_cost_str = fmt(on_demand_cost, precision=1, allow_zero=True)
    spot_base_cost_str = fmt(spot_base_cost, precision=1, allow_zero=True)
    checkpoint_extra_hours_str = fmt(checkpoint_extra_hours, precision=1, commas=False)
    restart_total_hours_str = fmt(restart_total_hours, precision=1, allow_zero=True, commas=False)
    spot_total_hours_str = fmt(spot_total_hours, precision=1, commas=False)
    spot_total_cost_str = fmt(spot_total_cost, precision=1, allow_zero=True)
    savings_str = fmt(savings, precision=1, allow_zero=True)
    savings_pct_str = fmt(savings_pct, precision=1, allow_zero=True, commas=False)
    longer_pct_str = fmt(longer_pct, precision=1, allow_zero=True, commas=False)
```
...
**On-demand**\index{On-Demand Capacity}: 512 GPUs $\times$ 336 hours $\times$ \$2.00/GPU-hour = **\$`{python} SpotTrainingEconomics.on_demand_cost_str`**

**Spot** (65 percent discount, 5 percent checkpoint overhead, 1 interruption/day with 30 min restart):

- Base cost: 512 $\times$ 336 $\times$ \$0.70 = \$`{python} SpotTrainingEconomics.spot_base_cost_str`
- Checkpoint overhead: 5 percent $\times$ 336 hours = `{python} SpotTrainingEconomics.checkpoint_extra_hours_str` extra hours
- Restart overhead: 14 interruptions $\times$ 0.5 hours = `{python} SpotTrainingEconomics.restart_total_hours_str` hours
...
class AutoscalingLag:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    as_capacity_str = fmt(capacity, precision=1, allow_zero=True)
    as_detect_str = fmt(detect_sec, precision=1, allow_zero=True)
    as_ready_str = fmt(ready_sec, precision=1, allow_zero=True)
    as_excess_str = fmt(excess_at_peak, precision=1, allow_zero=True)
    as_violations_str = fmt(slo_violations, precision=1, allow_zero=True)
    as_pred_trigger_str = fmt(pred_trigger_sec, precision=1, allow_zero=True)
    as_pred_capacity_str = fmt(pred_capacity, precision=1, allow_zero=True)
```
...
class GpuSharingRoi:
    ...
    gpu_rate = FLEET_GPU_HOUR_COST_REF.m_as(USD / hour)
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    gs_excl_util_str = fmt(excl_util_pct, precision=1, allow_zero=True)
    gs_excl_total_mem_str = fmt(int(excl_total_mem_gb), precision=1, allow_zero=True)
    gs_excl_large_str = fmt(excl_large_gpus, precision=1, allow_zero=True)
    gs_excl_total_str = fmt(excl_total, precision=1, allow_zero=True)
    gs_shared_gpus_str = fmt(shared_small_gpus, precision=1, allow_zero=True)
    gs_shared_mem_str = fmt(shared_mem_per_gpu, precision=1, allow_zero=True)
    gs_shared_util_str = fmt(shared_util_pct, precision=1, allow_zero=True)
    gs_shared_total_str = fmt(shared_total, precision=1, allow_zero=True)
    gs_saved_gpus_str = fmt(saved_gpus, precision=1, allow_zero=True)
    gs_saved_pct_str = fmt(saved_pct, precision=1, allow_zero=True)
    gs_annual_str = fmt(annual_savings, precision=1, allow_zero=True)
```
...
class DiurnalGpuShift:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ds_static_active_str = fmt(static_active, precision=1, allow_zero=True)
    ds_static_util_str = fmt(static_util_pct, precision=1, allow_zero=True)
    ds_dynamic_active_str = fmt(dynamic_active, precision=1, allow_zero=True)
    ds_dynamic_util_str = fmt(dynamic_util_pct, precision=1, allow_zero=True)
    ds_reclaimed_str = fmt(reclaimed, precision=1, allow_zero=True)
...
class ChargebackExample:
    ...
    gpus = 64
    days = 34
    gpu_hour_rate = FLEET_INTERNAL_CHARGEBACK_PER_HOUR.m_as(USD / hour)
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    chargeback_hours_str = fmt(hours, precision=1, allow_zero=True)
...
class PreemptionTax:
    ...
    reload_minutes = 20
    warmup_minutes = 10
    gpu_hour_rate = FLEET_GPU_HOUR_COST_REF.m_as(USD / hour)
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    recovery_minutes_str = fmt(recovery_minutes, precision=1, allow_zero=True, commas=False)
    cost_per_event_str = fmt(cost_per_event, precision=1, allow_zero=True)
    wasted_hours_per_day_str = fmt(wasted_hours_per_day, precision=1, allow_zero=True, commas=False)
    wasted_gpu_hours_per_day_str = fmt(wasted_gpu_hours_per_day, precision=1, allow_zero=True)
```
...
class QuotaHoardingCost:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    hoarding_cost_str = fmt(cost, precision=1, allow_zero=True)
```
...
class DebugValueCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    debug_cluster_str = fmt(debug_cluster, precision=1, allow_zero=True, commas=False)
    debug_productive_gain_str = fmt(int(debug_productive_gain), precision=1, allow_zero=True, commas=False)
    debug_recovered_str = fmt(int(debug_recovered), precision=1, allow_zero=True, commas=False)
    debug_annual_str    = fmt(debug_annual / MILLION, precision=1, allow_zero=True, commas=False)
```
...
1. **Productive Gain**: 1,000 GPUs $\times$ (0.84 - 0.60) = **`{python} DebugValueCalc.debug_productive_gain_str` fully utilized GPU-equivalents**.
