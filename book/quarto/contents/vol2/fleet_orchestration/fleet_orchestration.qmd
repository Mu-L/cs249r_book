...
# │ How: N_{\text{GPU}} * C_{\text{GPU-hour}} * 24; pint .m_as() for hardware specs.
# │
...
class FleetSetup:
    ...
    daily_cost_str = fmt(daily_cost/THOUSAND, precision=0, commas=False, prefix="$", suffix="K")
    annual_cost_str = fmt(annual_cost/MILLION, precision=0, commas=False, prefix="$", suffix="M")
...
# │ How: N_{\text{GPU}} * C_{\text{GPU-hour}} * 24.
# │
...
class ClusterEconomicsContext:
    ...
    annual_cost_str = fmt(annual_cost_m, precision=0, commas=False)
    daily_waste_str = fmt(daily_waste, precision=0)
    annual_waste_str = fmt(annual_waste_m, precision=0, commas=False)
...
# │ How: \Pr(\text{allocation}) = (\text{free GPUs}/\text{requested GPUs})^{J_{\text{jobs}}}. (Conceptual)
# │
...
# │ How: W_q/(1/\mu) = \rho/(1-\rho) * (1+C_s^2)/2
# │
...
::: {#ws-fleet-orchestration-silent-switch-failure .callout-war-story title="The silent switch failure"}

**The context**: A major research lab spent a week debugging a 30 percent performance regression in their flagship foundation model training run. The GPUs were healthy, the code was unchanged, and the job was placed on a "prime" pod with full bisection bandwidth.

**The failure**: The culprit was a single top-of-rack switch with a corrupted transceiver. The switch was technically "up" and passing standard health checks (ping, link status), but it was silently dropping 50 percent of packets due to CRC errors, forcing massive TCP retransmissions.

**The consequence**: Because the scheduler relied on a static topology map, it continued to schedule the most communication-intensive jobs on the degraded rack, assuming full 800 Gb/s connectivity.

**The systems lesson**: The fix required shifting from binary health checks (up/down) to **performance-aware topology monitoring**\index{Performance-Aware Topology Monitoring}, where the scheduler integrates real-time bandwidth counters from the switch fabric. Nodes connected to a "limping" switch are now dynamically **tainted**\index{Tainted}, forcing the scheduler to route high-bandwidth jobs elsewhere until the hardware is replaced.
...
# │ How: C_{\text{waste}} = N_{\text{GPU}}(1-u) C_{\text{GPU-hour}} T_{\text{year}}; viz.setup_plot().
# │
...
::: {#ws-fleet-orchestration-spot-market-crash .callout-war-story title="The spot market crash"}

**The context**: A major AI lab configured their entire research fleet to use spot instances in a single cloud region to save costs. Two days before a top-tier conference deadline, a massive wave of last-minute experiments from thousands of researchers worldwide hit the same region.

**The failure**: The spot market "crashed": prices spiked to equal on-demand rates, and the provider reclaimed 90 percent of the lab's spot instances within a 15-minute window.

**The consequence**: Dozens of paper-critical training runs died simultaneously. The lab had no fallback strategy to shift to on-demand capacity or other regions.

**The systems lesson**: Spot capacity is correlated with global research deadlines. A robust scheduler must have a "break glass" capability to flee to on-demand capacity when the spot market becomes toxic.
...
# │ How: C_{\text{spot,total}} = N_{\text{GPU}} T_{\text{wall}} C_{\text{GPU-hour}}.
# │
# │ Imports: mlsysim.fmt (check)
...
class SpotTrainingEconomics:
    ...
    spot_base_cost_str = fmt(spot_base_cost, precision=0)
    ...
    spot_total_cost_str = fmt(spot_total_cost, precision=0)
    savings_str = fmt(savings, precision=0)
    savings_pct_str = fmt(savings_pct, precision=0, commas=False)
    longer_pct_str = fmt(longer_pct, precision=0, commas=False)
```
...
**On-demand**\index{On-Demand}: 512 GPUs $\times$ 336 hours $\times$ \$2.00/GPU-hour = **\$`{python} SpotTrainingEconomics.on_demand_cost_str`**
...
**Savings**\index{Savings}: \$`{python} SpotTrainingEconomics.savings_str` (`{python} SpotTrainingEconomics.savings_pct_str` percent), at the cost of approximately `{python} SpotTrainingEconomics.longer_pct_str` percent longer wall-clock time.
...
The risk is not only implementation cost; a scheduler can also optimize a metric that conflicts with the organization's real objective.

::: {#ws-fleet-orchestration-scheduler-that-optimized-wrong-metric .callout-war-story title="The scheduler that optimized the wrong metric"}

**The context**: A platform team at a major autonomous vehicle company deployed a custom scheduler designed to minimize average job completion time (JCT), a standard academic metric. The logic was sound: by prioritizing shorter jobs, the queue clears faster, and developer velocity increases.

**The failure**: The scheduler correctly identified that their massive Transformer pretraining jobs would take weeks to complete. To minimize average JCT, it continuously preempted these long-running jobs to squeeze in thousands of short, minute-long debugging and visualization tasks submitted by other teams.

**The consequence**: While the *average wait time* dropped by 40 percent, the *P99 wait time* for critical-path training jobs exploded from 4 hours to 9 days. The core product, the autonomous driving model, effectively stopped learning for two weeks.

**The systems lesson**: Optimizing for an aggregate metric without distinct Quality of Service (QoS) tiers for different workload classes (interactive vs. batch vs. production) is a recipe for organizational paralysis.
...
class GpuSharingRoi:
    ...
    gs_excl_total_mem_str = fmt(int(excl_total_mem_gb), precision=0)
...
# │ How: utilization = N_{\text{active}}/N_{\text{GPU}}; annual value uses C_{\text{GPU-hour}}.
# │
# │ Imports: mlsysim.book (fmt, check), mlsysim.constants (HOURS_PER_YEAR)
...
# │ How: C_{\text{chargeback}} = N_{\text{GPU}} T_{\text{wall}} C_{\text{GPU-hour}}.
# │
...
# │ How: C_{\text{preempt}} = T_{\text{recover}} N_{\text{GPU}} C_{\text{GPU-hour}}.
# │
...
# │ How: C_{\text{idle}} = N_{\text{GPU}} T_{\text{idle}} C_{\text{GPU-hour}}.
# │
# │ Imports: mlsysim.fmt (fmt, check)
...
A concrete quota incident shows how quickly idle reservations become expensive.

::: {#ws-fleet-orchestration-quota-hoarding-incident .callout-war-story title="The quota hoarding incident"}

**The context**: A computer vision team at a major logistics company reserved a block of 500 GPUs for a "critical" urgent model refresh.

**The failure**: Due to upstream data delays, the training jobs were postponed, but the team retained the reservation to ensure availability "when the data arrives."

**The consequence**: The GPUs sat idle for three weeks, a capital waste of roughly \$`{python} QuotaHoardingCost.hoarding_cost_str`, while the NLP team's queues overflowed, delaying a chatbot release by a month.

**The systems lesson**: The infrastructure team implemented a "use it or lose it" policy. Any reserved quota group operating below 20 percent utilization for 72 consecutive hours is now automatically reclaimed and converted to spot capacity for the general pool. Reclaiming the quota requires VP-level approval, effectively eliminating "parking" on idle hardware.
...
# │ How: W_q = \rho/(1-\rho) T_{\text{service}}; matplotlib.
# │
...
# │ How: N_{\text{productive}} = N_{\text{GPU}}(u_{\text{after}} - u_{\text{before}});
# │      N_{\text{recovered}} = N_{\text{productive}} / u_{\text{before}};
# │      annual_value = N_{\text{recovered}} C_{\text{GPU-hour}} T_{\text{year}}.
# │
...
class DebugValueCalc:
    ...
    debug_productive_gain_str = fmt(int(debug_productive_gain), precision=0, commas=False)
    debug_recovered_str = fmt(int(debug_recovered), precision=0, commas=False)
    debug_annual_str    = fmt(debug_annual / MILLION, precision=0, commas=False)
```
