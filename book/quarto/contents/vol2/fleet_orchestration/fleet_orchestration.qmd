...
**Context**: OpenAI scaled Kubernetes clusters to 7,500 nodes for workloads including GPT-3, CLIP, and DALL-E. Their large training jobs often used StatefulSets and MPI, where all members of a group had to be scheduled before training could make progress [@sigler2021openai_kubernetes].

**Failure**: Default Kubernetes scheduling did not necessarily prioritize fulfilling all requests from one StatefulSet before another. If two experiments each requested the whole cluster, Kubernetes could schedule half of each job instead of all of one job, leaving both jobs unable to run.
