...
1.  **Significance (quantitative)**: It reduces per-token attention compute from $O(n^2)$ to $O(n)$ in the sequence length, but its footprint grows linearly with both context length and batch size, often consuming more HBM than the model weights themselves and becoming the binding constraint on serving capacity. The fundamental sizing math is derived in @sec-appendix-systems-inference.
