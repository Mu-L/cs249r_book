...
# │ Exports: InferenceSetup.h100_mem_gb, InferenceSetup.h100_mem_bw_tbs,
# │          InferenceSetup.a100_mem_bw_tbs, InferenceSetup.nvlink_h100_gbs
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
...
class InferenceSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_gb = f"{h100_mem_gb_val:.0f}"
    h100_mem_bw_tbs = f"{h100_bw_tbs_val:.2f}"
    a100_mem_bw_tbs = f"{a100_bw_tbs_val:.0f}"
...
class ServingCostSimple:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    annual_reqs_b_str = f"{annual_reqs_b}"
    annual_serving_cost_str = f"\\${annual_serving_cost:,.0f}"
    serving_training_ratio_str = f"{serving_training_ratio:.0f}"
    efficiency_savings_str = f"\\${efficiency_savings/1_000_000:.1f}M"
```
...
# │ Exports: InferenceEconomics.lifetime_queries_str,
# │          InferenceEconomics.lifetime_queries_latex_str,
...
class InferenceEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    lifetime_queries_str = f"{lifetime_queries_val:.2e}"
    # LaTeX-safe form: "6.31e+11" breaks as 6\.31e\+11; use \times 10^{11}.
    # Wrap in md_math() so Quarto re-parses the inline {python} substitution
    ...
    if lifetime_queries_val >= 1e9:
        ...
        lifetime_queries_latex_str = md_math(
            f"{mantissa:.2f} \\times 10^{{{exp}}}"
        )
    else:
        lifetime_queries_latex_str = md_math(f"{lifetime_queries_val:,.0f}")
    serving_cost_total_str = f"USD {fmt(serving_total_cost_val, precision=0)}"
    training_cost_total_str = f"USD {fmt(training_total_cost, precision=0)}"
    leverage_ratio_str = f"{leverage_ratio:.0f}"
    qps_str = f"{qps:,}"
    serve_duration_days_str = f"{serve_duration_days}"
    cost_per_million_str = f"{cost_per_query * MILLION:.0f}"
```
...
$Q_{\text{total}} \approx$ **`{python} InferenceEconomics.lifetime_queries_latex_str` queries**
(`{python} InferenceEconomics.qps_str` QPS $\times$ 86,400 s/day $\times$ `{python} InferenceEconomics.serve_duration_days_str` days)

**Total Serving Cost**

$C_{\text{serve\_total}} =$ `{python} InferenceEconomics.lifetime_queries_latex_str` $\times \$10^{-5} =$ **`{python} InferenceEconomics.serving_cost_total_str`**
...
Multi-GPU tensor parallelism shards each layer's weight matrices across multiple GPUs connected by high-bandwidth interconnect (NVLink at `{python} DeploymentTopologyHardware.nvlink_h100_gbs` GB/s on H100). Each GPU computes a partial result for every layer, and an all-reduce operation synchronizes the partial results before the next layer. This topology is necessary when model weights exceed single-device memory and provides latency reduction proportional to the parallelism degree, at the cost of communication overhead per layer. The model sharding section (@sec-inference-scale-model-sharding-inference-82b3) analyzes the communication overhead quantitatively.
...
class BatchingEfficiency:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    bec_llm_knee_str = f"{b_knee_llm}"
    bec_vision_knee_str = f"{b_knee_vis}"
...
class LittlesLawExample:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    required_concurrency_str = f"{required_concurrency}"
    replica_throughput_str = f"{replica_throughput}"
    replicas_needed_str = f"{replicas_needed}"
    total_concurrency_str = f"{total_concurrency}"
```

::: {#thm-inference-littles-law-inference .callout-theorem title="Little's Law for inference"}
...
# │ Exports: Gpt3Scenario.gpt3_params_b
# └─────────────────────────────────────────────────────────────────────────────
...
Consider serving a GPT-3 class model (`{python} Gpt3Scenario.gpt3_params_b`B parameters) under the following system parameters:
...
# │
# │ Imports: mlsysim.book (check)
# │ Exports: GPT3ServingResults.b8_util_pct, GPT3ServingResults.b8_wait_ms,
# │          GPT3ServingResults.b16_service_ms, GPT3ServingResults.b16_wait_ms,
# │          GPT3ServingResults.b32_service_ms, GPT3ServingResults.b32_wait_ms,
# │          GPT3ServingResults.b8_service_ms, LittlesLawVerify.expected_total_str,
# │          LittlesLawVerify.l_str, LittlesLawVerify.in_service_str,
# │          LittlesLawVerify.in_queue_str, LittlesLawVerify.batch_size
# └─────────────────────────────────────────────────────────────────────────────
...
class LittlesLawVerify:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    expected_total_str = f"{expected_total_ms}"
    expected_total_s_str = f"{expected_total_s:.3f}"
    utilization_str = f"{utilization:.3f}"
    l_str = f"{L:.2f}"
    in_service_str = f"{in_service:.1f}"
    in_queue_str = f"{in_queue:.2f}"
```
...
2. **Batch size 8 achieves stability**\index{Batch Size!8 achieves stability}: At `{python} GPT3ServingResults.b8_util_pct` percent utilization, the system is stable but queuing delays contribute significantly to latency (`{python} GPT3ServingResults.b8_wait_ms` ms), as @fig-queuing-hockey-stick illustrates.
...
3. **Batch size 16 minimizes total latency**\index{Batch Size!16 minimizes total latency}: Despite longer service time (`{python} GPT3ServingResults.b16_service_ms` ms vs. `{python} GPT3ServingResults.b8_service_ms` ms), the reduction in queue wait time (`{python} GPT3ServingResults.b16_wait_ms` ms vs. `{python} GPT3ServingResults.b8_wait_ms` ms) yields the lowest total latency, while B=32 leaves more utilization headroom.
...
With batch size `{python} LittlesLawVerify.batch_size` and utilization `{python} GPT3ServingResults.b16_util_pct` percent:

- Expected requests in service: `{python} LittlesLawVerify.batch_size` $\times$ `{python} LittlesLawVerify.utilization_str` = `{python} LittlesLawVerify.in_service_str`
...
where $E[T_{\text{queue}}]$ is the expected queuing delay, $T_{\text{batch}}$ is the batch formation delay (up to $T_{\text{window}}$), and $T_{\text{inference}}(B)$ is the inference time for batch size $B$. Example \ref{exm-dynamic-batching-resnet50} illustrates how these components interact in practice.

::: {#exm-dynamic-batching-resnet50 .callout-example title="Dynamic batching for ResNet-50"}
...
::: {#def-inference-continuous-batching .callout-definition title="Continuous batching"}
...
::: {#exm-inference-continuous-batching-vllm .callout-example title="Continuous batching in vLLM"}
...
class BatchingWorkedExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mean_output_str = f"{mean_output:.0f}"
    total_batch_time_str = f"{total_batch_time_ms:,}"
    total_seq_iters_str = f"{total_seq_iters:,}"
    useful_seq_iters_str = f"{useful_seq_iters:,}"
    r1_lat_str = f"{r1_latency_ms:,}"
    r2_lat_str = f"{r2_latency_ms:,}"
    r3_lat_str = f"{r3_latency_ms:,}"
    r4_lat_str = f"{r4_latency_ms:,}"
    trad_lat_str = f"{trad_latency_ms:,}"
    cont_avg_lat_str = f"{cont_avg_latency_ms:,}"
    latency_red_str = f"{latency_reduction_pct:.1f}"
    r1_imp_str = f"{r1_improvement:.0f}"
    r3_imp_str = f"{r3_improvement:.0f}"
    r4_imp_str = f"{r4_improvement:.2f}"
```
...
# │ Exports: A100HardwareScenario.a100_mem_gb,
# │          InferenceSetup.a100_mem_bw_tbs,
# │          A100HardwareScenario.nvlink_a100_gbs,
# │          A100HardwareScenario.pcie_gen4_gbs
# └─────────────────────────────────────────────────────────────────────────────
...
class A100HardwareScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_gb = f"{a100_mem_gb_val:.0f}"
    a100_mem_bw_tbs = f"{a100_mem_bw_tbs_val:.0f}"
    nvlink_a100_gbs = f"{nvlink_a100_gbs_val:.0f}"
    pcie_gen4_gbs = f"{pcie_gen4_gbs_val:.0f}"
    activation_transfer_mb_str = f"{activation_transfer_mb}"
    pcie_transfer_ms_str = f"{pcie_transfer_ms_val:.1f}"
    nvlink_transfer_ms_str = f"{nvlink_transfer_ms_val:.2f}"
```
...
The system runs on 4 A100-`{python} A100HardwareScenario.a100_mem_gb` GB GPUs connected via PCIe Gen4 (`{python} A100HardwareScenario.pcie_gen4_gbs` GB/s per GPU) rather than NVLink. The server has a dual-socket CPU with NUMA topology. Memory bandwidth per GPU is `{python} A100HardwareScenario.a100_mem_bw_tbs` TB/s (HBM2e), adequate for decode operations. However, PCIe bandwidth limits tensor parallelism communication to `{python} A100HardwareScenario.pcie_gen4_gbs` GB/s vs. NVLink's `{python} A100HardwareScenario.nvlink_a100_gbs` GB/s. For a batch requiring `{python} A100HardwareScenario.activation_transfer_mb_str` MB activation transfers between GPUs, raw bandwidth math gives approximately `{python} A100HardwareScenario.pcie_transfer_ms_str` ms per synchronization point over PCIe vs. `{python} A100HardwareScenario.nvlink_transfer_ms_str` ms over NVLink.
...
::: {#exm-inference-recsys-batching-at-meta-scale .callout-example title="RecSys batching at Meta scale"}
...
::: {#exm-inference-streaming-speech-recognition-pipeline .callout-example title="Streaming speech recognition pipeline"}
...
::: {#exm-inference-adaptive-batching-triton .callout-example title="Adaptive batching: Triton"}
...
::: {#def-inference-kv-cache .callout-definition title="KV cache"}
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check, md
...
class KVCacheCapacityEstimator:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    kv_bytes_per_token_str = f"{kv_bytes_per_token:,}"
    kv_mb_per_token_str = f"{kv_mb_per_token:.2f}"
    context_tokens_str = f"{context_tokens:,}"
    kv_gb_per_request_str = f"{kv_gb_per_request:.0f}"
    available_kv_gb_str = f"{available_kv_gb:.0f}"
    max_batch_str = f"{max_batch}"
...
::: {#def-inference-pagedattention .callout-definition title="PagedAttention"}
...
::: {#exm-inference-pagedattention-implementation-details .callout-example title="PagedAttention implementation details"}
...
class PrefixCachingAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tokens_per_user_str = f"{tokens_per_user:,}"
    kv_tb_no_cache_str = f"{kv_tb_no_cache_val:.1f}"
    total_tokens_cached_str = f"{total_tokens_cached:,}"
    kv_tb_cached_str = f"{kv_tb_cached_val:.1f}"
    memory_reduction_pct_str = f"{memory_reduction_pct_val:.0f}"
    concurrent_multiplier_str = f"{concurrent_multiplier_val:.0f}"
```

::: {#exm-inference-prefix-caching-at-scale .callout-example title="Prefix caching at scale"}
...
class KVCacheCalc:
    ...
    h100_mem_gb = f"{h100_mem.m_as(GiB):.0f}"
    ...
    batch8_kv_fp16_gb_str = f"{batch8_kv_fp16_gb:.0f}"
    gqa_kv_fp16_gb_str = f"{gqa_kv_fp16_gb:.1f}"
```
...
A single request's KV cache alone consumes a substantial fraction of the H100's `{python} KVCacheCalc.h100_mem_gb` GB of HBM. For a batch of 8 concurrent requests, the KV cache would require `{python} KVCacheCalc.batch8_kv_fp16_gb_str` GB, exceeding single-GPU capacity. Reducing this footprint requires two complementary strategies: quantization and architectural optimization.
...
class PrecisionDividend:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    avail_kv_fp16_str = f"{avail_kv_fp16:.0f}"
    int4_weight_per_gpu_str = f"{int4_weight_per_gpu:.2f}"
    avail_kv_int4_str = f"{avail_kv_int4:.2f}"
    kv_fp16_per_gpu_str = f"{kv_fp16_per_gpu:.1f}"
    kv_int8_total_str = f"{KVCacheCalc.kv_fp16_gb/2:.1f}"
    kv_int8_per_gpu_str = f"{kv_int8_per_gpu:.1f}"
    max_batch_fp16_str = f"{max_batch_fp16}"
    max_batch_int8_str = f"{max_batch_int8}"
```
...
- Weights: 35 GB/GPU
- Available for KV cache: $80 -$ `{python} PrecisionDividend.fp16_weight_per_gpu` $=$ `{python} PrecisionDividend.avail_kv_fp16_str` GB/GPU
...
- Weights: `{python} PrecisionDividend.fp16_weight_per_gpu` GB $\times (4/16) =$ `{python} PrecisionDividend.int4_weight_per_gpu_str` GB/GPU (INT4)
...
::: {#def-inference-speculative-decoding .callout-definition title="Speculative decoding"}
...
::: {#def-inference-prefill-decode-phases .callout-definition title="Prefill and decode phases"}
...
::: {#exm-inference-sarathi-chunked-prefill-implementation .callout-example title="Sarathi: Chunked prefill implementation"}
...
# │ Exports: HardwareSetupSharding.h100_mem_gb
# └─────────────────────────────────────────────────────────────────────────────
...
The result exceeds the `{python} HardwareSetupSharding.h100_mem_gb` GB capacity of an H100 GPU, requiring at minimum 2-way sharding.
...
# │ Exports: TpSpeedupCalc.tp_speedup_str, TpSpeedupCalc.tp_theoretical,
# │   TpSpeedupCalc.n_layers, TpSpeedupCalc.seq_total_ms_str,
# │   TpSpeedupCalc.tp8_total_ms_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TpSpeedupCalc:
    ...
    nvlink_h100_gbs = f"{nvlink_bw.m_as(GB/second):.0f}"
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tp_speedup_str = f"{tp_speedup:.1f}"
    seq_total_ms_str = f"{seq_total_ms:,.0f}"
    tp8_total_ms_str = f"{tp8_total_ms:,.0f}"
```

::: {#exm-inference-tensor-parallelism-llama-70b .callout-example title="Tensor parallelism for Llama-70B"}
...
**Minimum sharding**: 2-way (140 GB/`{python} HardwareSetupSharding.h100_mem_gb` GB per H100)
...
**For `{python} TpSpeedupCalc.n_layers` layers on the same prefill workload**:
...
The `{python} TpSpeedupCalc.tp_speedup_str`$\times$ speedup (vs. theoretical `{python} TpSpeedupCalc.tp_theoretical`$\times$) reflects communication overhead. At `{python} TpSpeedupCalc.nvlink_h100_gbs` GB/s, moving an 8 MB activation payload would take only about 0.01 ms by raw bandwidth math; the 0.3 ms budget shown here includes collective launch, synchronization, ring protocol, and framework overheads.
...
class MoECapacityPlan:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    total_weight_gb_str = f"{total_weight_gb:.0f}"
    node_capacity_gb_str = f"{node_capacity_gb:.0f}"
    nodes_needed_str = f"{nodes_needed}"
    cluster_capacity_gb_str = f"{cluster_capacity_gb:,.0f}"
    active_mem_gb_str = f"{active_mem_gb:.0f}"
    h100_bw_tbs_str = f"{h100_bw_tbs:.2f}"
    read_per_gpu_str = f"{read_per_gpu_gb:.1f}"
    t_read_ms_str = f"{t_read_ms:.2f}"
    total_decode_ms_str = f"{total_decode_ms:.1f}"
    tokens_per_sec_str = f"{tokens_per_sec:,.0f}"
```
...
::: {#exm-inference-expert-parallelism-mixtral-8x7b .callout-example title="Expert parallelism for Mixtral-8x7B"}
...
class AllReduceInterconnect:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvlink_bw_gbs_str = f"{nvlink_bw_gbs:.0f}"
    nvlink_ms_str = f"{nvlink_ms:.2f}"
    ib_ms_str = f"{ib_ms:.2f}"
    eth_ms_str = f"{eth_ms:.2f}"
    nvlink_pct_str = f"{nvlink_pct:.2f}"
    ib_pct_str = f"{ib_pct:.1f}"
    eth_pct_str = f"{eth_pct:.1f}"
```
...
class HeterogeneousGpuCluster:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_cap_str = f"{total_cap:,}"
    h100_weight_str = f"{h100_weight * 100:.1f}"
    a100_weight_str = f"{a100_weight * 100:.1f}"
    h100_load_str = f"{h100_load:.0f}"
    a100_load_str = f"{a100_load:.0f}"
    h100_util_str = f"{h100_util:.0f}"
    a100_util_str = f"{a100_util:.0f}"
    equal_load_str = f"{equal_load:.0f}"
    h100_util_equal_str = f"{h100_util_equal:.0f}"
    a100_util_equal_str = f"{a100_util_equal:.0f}"
```

::: {#exm-inference-heterogeneous-gpu-cluster .callout-example title="Heterogeneous GPU cluster"}
...
::: {#exm-inference-least-connections-llm-serving .callout-example title="Least-connections for LLM serving"}
...
::: {#exm-inference-consistent-hashing-kv-cache-affinity .callout-example title="Consistent hashing for KV cache affinity"}
...
::: {#exm-inference-health-checks-gpu-inference .callout-example title="Health checks for GPU inference"}
...
::: {#exm-inference-cascading-failure-prevention .callout-example title="Cascading failure prevention"}
...
::: {#exm-inference-bulkhead-configuration-api-tiers .callout-example title="Bulkhead configuration for API tiers"}
...
::: {#exm-inference-predictive-scaling-traffic .callout-example title="Predictive scaling for traffic"}
...
# │
# │ Imports: mlsysim.core.constants (A100_MEM_CAPACITY, GiB), mlsysim.fmt (check)
...
class QuantizedServingCapacity:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_gb_str = f"{a100_mem_gb:.0f}"
    weight_int4_gb_str = f"{weight_int4_gb:.0f}"
    kv_headroom_gb_str = f"{kv_headroom_gb:.0f}"
    kv_per_1k_sequence_gb_str = f"{kv_per_1k_sequence_gb:.1f}"
    max_1k_sequences_str = f"{max_1k_sequences}"
```
...
# │ Exports: HardwareSetupSharding.h100_mem_gb, InferenceSetup.h100_mem_bw_tbs
# └─────────────────────────────────────────────────────────────────────────────
...
While larger GPU memory enables larger batches for models that fit in memory, the bottleneck often shifts before memory is exhausted. Memory bandwidth limits throughput for bandwidth-bound operations (LLM decode). Compute limits throughput for compute-bound operations (prefill, vision inference). A bandwidth-bound decode workload on an H100 with `{python} FinalSummaryHardware.h100_mem_gb` GB memory eventually plateaus when HBM3 bandwidth (`{python} FinalSummaryHardware.h100_mem_bw_tbs` TB/s) saturates; adding memory alone does not raise token throughput once that point is reached. Understanding whether the workload is compute-bound, memory-bandwidth-bound, or capacity-bound guides appropriate resource allocation.
