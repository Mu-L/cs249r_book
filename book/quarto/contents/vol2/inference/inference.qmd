...
Taking the derivative with respect to $B$ and setting equal to zero yields the optimal batch size. While the full derivation involves solving a cubic equation, the approximate solution for systems where $\alpha \gg \beta$ (high fixed overhead, typical of LLMs) reveals a square-root scaling law for batch sizing:
...
@eq-optimal-batch-approx reveals a fundamental insight: **optimal batch size scales with the square root of the fixed overhead times arrival rate**. This means:
...
The iteration-level description is abstract, but the operational benefit is concrete: freed capacity can be reused immediately instead of waiting for an entire batch to drain.
...
Feature-parallel batching is natural when embeddings are sharded across servers: each embedding server handles lookups for its shard across all requests in the batch. The example below shows how this pattern operates at Meta-scale recommendation traffic.
...
Applying this formula to production hardware determines the maximum serving batch size for a large model.
...
This draft-and-verify structure is easier to reason about as a proposal workflow than as ordinary sequential generation.
...
Both equations make clear that the achievable bandwidth $\text{BW}$ and latency $\alpha$ of the underlying interconnect determine real-world sharding performance. The comparison below quantifies these differences across production hardware.
...
In an LLM serving cluster, the same mechanism can preserve KV cache affinity for stateful conversations.
...
Without these mechanisms, a single degraded server can trigger a chain reaction that takes down the entire serving fleet. The failure mode is easiest to see in a concrete scenario.
