...
class InferenceSetup:
    ...
    h100_mem_gb_val = mem_h_h100.m_as(GiB)
    h100_bw_tbs_val = bw_h_h100.m_as(TB/second)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_gb_str = fmt(h100_mem_gb_val, precision=1, allow_zero=True, commas=False)
    h100_mem_bw_tbs_str = fmt(h100_bw_tbs_val, precision=2, commas=False)
    a100_mem_bw_tbs_str = fmt(a100_bw_tbs_val, precision=1, allow_zero=True, commas=False)
    nvlink_h100_gbs_str = fmt(nvlink_gbs_val, precision=1, allow_zero=True, commas=False)
```
...
class ServingCostSimple:
    ...
    annual_serving_cost_str = fmt(annual_serving_cost, precision=1, allow_zero=True, prefix="\\$")
    serving_training_ratio_str = fmt(serving_training_ratio, precision=1, allow_zero=True, commas=False)
...
class InferenceEconomics:
    ...
    serving_cost_total_str = fmt(serving_total_cost_val, precision=1, allow_zero=True, prefix="USD ")
    training_cost_total_str = fmt(training_total_cost, precision=1, allow_zero=True, prefix="USD ")
    leverage_ratio_str = fmt(leverage_ratio, precision=1, allow_zero=True, commas=False)
    qps_str = fmt(qps, precision=1, allow_zero=True)
    serve_duration_days_str = fmt(serve_duration_days, precision=1, allow_zero=True, commas=False)
    cost_per_million_str = fmt(cost_per_query * MILLION, precision=1, allow_zero=True, commas=False)
```
...
class BatchingEfficiency:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    bec_llm_knee_str = fmt(b_knee_llm, precision=1, allow_zero=True, commas=False)
    bec_vision_knee_str = fmt(b_knee_vis, precision=1, allow_zero=True, commas=False)
...
class LittlesLawExample:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    required_concurrency_str = fmt(required_concurrency, precision=1, allow_zero=True, commas=False)
    replica_throughput_str = fmt(replica_throughput, precision=1, allow_zero=True, commas=False)
    replicas_needed_str = fmt(replicas_needed, precision=1, allow_zero=True, commas=False)
    total_concurrency_str = fmt(total_concurrency, precision=1, allow_zero=True, commas=False)
```
...
class LogicScaling:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    steps_str = fmt(reasoning_steps, precision=1, allow_zero=True, commas=False)
...
class BatchingWorkedExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mean_output_str = fmt(mean_output, precision=1, allow_zero=True, commas=False)
    total_batch_time_str = fmt(total_batch_time_ms, precision=1, allow_zero=True)
    total_seq_iters_str = fmt(total_seq_iters, precision=1, allow_zero=True)
    useful_seq_iters_str = fmt(useful_seq_iters, precision=1, allow_zero=True)
    r1_lat_str = fmt(r1_latency_ms, precision=1, allow_zero=True)
    r2_lat_str = fmt(r2_latency_ms, precision=1, allow_zero=True)
    r3_lat_str = fmt(r3_latency_ms, precision=1, allow_zero=True)
    r4_lat_str = fmt(r4_latency_ms, precision=1, allow_zero=True)
    trad_lat_str = fmt(trad_latency_ms, precision=1, allow_zero=True)
    ...
    cont_avg_lat_str = fmt(cont_avg_latency_ms, precision=1, allow_zero=True)
    latency_red_str = fmt(latency_reduction_pct, precision=1, commas=False)
    r1_imp_str = fmt(r1_improvement, precision=1, allow_zero=True, commas=False)
    r3_imp_str = fmt(r3_improvement, precision=1, allow_zero=True, commas=False)
...
# │
# │ Imports: mlsysim.core.constants (GB, INFINIBAND_HDR_BW, NETWORK_100G_BW,
# │          NVLINK_H100_BW, second), mlsysim.fmt (check, fmt)
...
class ContinuousBatchingBenefit:
    ...
    code_waste_pct_str = fmt(code_waste_pct, precision=1, allow_zero=True, commas=False)
    ...
    chat_waste_pct_str = fmt(chat_waste_pct, precision=1, allow_zero=True, commas=False)
    ...
    text_waste_pct_str = fmt(text_waste_pct, precision=1, allow_zero=True, commas=False)
    ...
    creative_waste_pct_str = fmt(creative_waste_pct, precision=1, allow_zero=True, commas=False)
    ...
    rag_waste_pct_str = fmt(rag_waste_pct, precision=1, allow_zero=True, commas=False)
...
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.A100.memory.capacity, Hardware.Cloud.A100.memory.bandwidth, NVLINK_A100_BW,
...
class A100HardwareScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_gb_str = fmt(a100_mem_gb_val, precision=1, allow_zero=True, commas=False)
    a100_mem_bw_tbs_str = fmt(a100_mem_bw_tbs_val, precision=1, allow_zero=True, commas=False)
    nvlink_a100_gbs_str = fmt(nvlink_a100_gbs_val, precision=1, allow_zero=True, commas=False)
    pcie_gen4_gbs_str = fmt(pcie_gen4_gbs_val, precision=1, allow_zero=True, commas=False)
    activation_transfer_mb_str = fmt(activation_transfer_mb, precision=1, allow_zero=True, commas=False)
...
class KVCacheCapacityEstimator:
    ...
    kv_bytes_per_token = int(kv_cache_one_token.m_as("byte"))
    kv_mb_per_token = kv_cache_one_token.m_as("MB")

    kv_cache_full = calc_kv_cache_size(n_layers, n_kv_heads, d_head, context_tokens, 1, bytes_per_elem)
    kv_gb_per_request = kv_cache_full.m_as("GB")
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    kv_bytes_per_token_str = fmt(kv_bytes_per_token, precision=1, allow_zero=True)
    kv_mb_per_token_str = fmt(kv_mb_per_token, precision=2, commas=False)
    context_tokens_str = fmt(context_tokens, precision=1, allow_zero=True)
    kv_gb_per_request_str = fmt(kv_gb_per_request, precision=1, allow_zero=True, commas=False)
    available_kv_gb_str = fmt(available_kv_gb, precision=1, allow_zero=True, commas=False)
    max_batch_str = fmt(max_batch, precision=1, allow_zero=True, commas=False)
...
class PrefixCachingAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tokens_per_user_str = fmt(tokens_per_user, precision=1, allow_zero=True)
    kv_tb_no_cache_str = fmt(kv_tb_no_cache_val, precision=1, commas=False)
    total_tokens_cached_str = fmt(total_tokens_cached, precision=1, allow_zero=True)
    kv_tb_cached_str = fmt(kv_tb_cached_val, precision=1, commas=False)
    memory_reduction_pct_str = fmt(memory_reduction_pct_val, precision=1, allow_zero=True, commas=False)
    concurrent_multiplier_str = fmt(concurrent_multiplier_val, precision=1, allow_zero=True, commas=False)
```
...
class KVCacheCalc:
    ...
    kv_compression_str = fmt(kv_compression, precision=1, allow_zero=True, commas=False)
    h100_mem_gb_str = fmt(h100_mem.m_as(GiB), precision=1, allow_zero=True, commas=False)
    batch8_kv_fp16_gb_str = fmt(batch8_kv_fp16_gb, precision=1, allow_zero=True, commas=False)
...
class PrecisionDividend:
    ...
    fp16_weight_per_gpu_str = fmt(fp16_weight_per_gpu, precision=1, allow_zero=True, commas=False)
    avail_kv_fp16_str = fmt(avail_kv_fp16, precision=1, allow_zero=True, commas=False)
    ...
    max_batch_fp16_str = fmt(max_batch_fp16, precision=1, allow_zero=True, commas=False)
    max_batch_int8_str = fmt(max_batch_int8, precision=1, allow_zero=True, commas=False)
```
...
class TpSpeedupCalc:
    ...
    seq_total_ms_str = fmt(seq_total_ms, precision=1, allow_zero=True)
    tp8_total_ms_str = fmt(tp8_total_ms, precision=1, allow_zero=True)
    tp_theoretical_str = fmt(tp_theoretical, precision=1, allow_zero=True, commas=False)
    n_layers_str = fmt(n_layers, precision=1, allow_zero=True, commas=False)
    nvlink_h100_gbs_str = fmt(nvlink_h100_gbs_val, precision=1, allow_zero=True, commas=False)
```
...
class MoEEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dense_mem_str = fmt(dense_mem_fp16, precision=1, allow_zero=True, commas=False)
    moe_mem_str = fmt(moe_mem_fp16, precision=1, allow_zero=True, commas=False)
    compute_ratio_str = fmt(compute_ratio, precision=1, allow_zero=True, commas=False)
    bw_ratio_str = fmt(bw_ratio, precision=1, allow_zero=True, commas=False)
```
...
class MoECapacityPlan:
    ...
    node_gpus = GPUS_PER_HOST
    # H100 HBM3 = 80*GiB in mlsysim (binary stacks); vendor label "80 GB".
    h100_mem_gb = Hardware.Cloud.H100.memory.capacity.m_as(GiB)
    n_gpus = 16
    alltoall_overhead_ms = 0.3
    h100_bw_gbs = Hardware.Cloud.H100.memory.bandwidth.m_as(GB/second)
    ...
    tokens_per_sec_str = fmt(tokens_per_sec, precision=1, allow_zero=True)
    total_weight_gb_str = fmt(total_weight_gb, precision=1, allow_zero=True, commas=False)
    node_capacity_gb_str = fmt(node_capacity_gb, precision=1, allow_zero=True, commas=False)
    nodes_needed_str = fmt(nodes_needed, precision=1, allow_zero=True, commas=False)
    cluster_capacity_gb_str = fmt(cluster_capacity_gb, precision=1, allow_zero=True)
    active_mem_gb_str = fmt(active_mem_gb, precision=1, allow_zero=True, commas=False)
...
class MetaEmbeddingScaleCheck:
    ...
    entity_count_t = 10
    min_dimension = 128
    bytes_per_dimension_int8 = BYTES_INT8.m_as(byte)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    storage_tb_str = fmt(storage_tb, precision=1, allow_zero=True, commas=False)
    entity_count_t_str = fmt(entity_count_t, precision=1, allow_zero=True, commas=False)
    min_dimension_str = fmt(min_dimension, precision=1, allow_zero=True, commas=False)
    dense_int8_tb_str = fmt(dense_int8_tb, precision=1, allow_zero=True)
    effective_bytes_per_entity_str = fmt(effective_bytes_per_entity, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (GPUS_PER_HOST), mlsysim.fmt (check, fmt)
...
class AllReduceInterconnect:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    n_devices = GPUS_PER_HOST
    message_mb = 8
    layer_budget_ms = 30
    nvlink_bw_gbs = NVLINK_H100_BW.m_as(GB/second)
    ib_hdr_bw_gbs = INFINIBAND_HDR_BW.m_as(GB/second)
    eth_100g_bw_gbs = NETWORK_100G_BW.m_as(GB/second)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvlink_bw_gbs_str = fmt(nvlink_bw_gbs, precision=1, allow_zero=True, commas=False)
...
class HeterogeneousGpuCluster:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_cap_str = fmt(total_cap, precision=1, allow_zero=True)
    h100_weight_str = fmt(h100_weight * 100, precision=1, commas=False)
    a100_weight_str = fmt(a100_weight * 100, precision=1, commas=False)
    h100_load_str = fmt(h100_load, precision=1, allow_zero=True, commas=False)
    a100_load_str = fmt(a100_load, precision=1, allow_zero=True, commas=False)
    h100_util_str = fmt(h100_util, precision=1, allow_zero=True, commas=False)
    a100_util_str = fmt(a100_util, precision=1, allow_zero=True, commas=False)
    equal_load_str = fmt(equal_load, precision=1, allow_zero=True, commas=False)
    h100_util_equal_str = fmt(h100_util_equal, precision=1, allow_zero=True, commas=False)
    a100_util_equal_str = fmt(a100_util_equal, precision=1, allow_zero=True, commas=False)
```
...
class NoisyNeighborQuotaScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    slo_ms_str = fmt(slo_ms, precision=1, allow_zero=True, commas=False)
    tenant_base_qps_str = fmt(tenant_base_qps, precision=1, allow_zero=True, commas=False)
    tenant_burst_qps_str = fmt(tenant_burst_qps, precision=1, allow_zero=True, commas=False)
    tenant_throttled_qps_str = fmt(tenant_throttled_qps, precision=1, allow_zero=True, commas=False)
    protected_p99_ms_str = fmt(protected_p99_ms, precision=1, allow_zero=True, commas=False)
    burst_p99_ms_str = fmt(burst_p99_ms, precision=1, allow_zero=True, commas=False)
...
class QuantizedServingCapacity:
    ...
    weight_int4_gb_str = fmt(weight_int4_gb, precision=1, allow_zero=True, commas=False)
    kv_headroom_gb_str = fmt(kv_headroom_gb, precision=1, allow_zero=True, commas=False)
    kv_per_1k_sequence_gb_str = fmt(kv_per_1k_sequence_gb, precision=1, commas=False)
    max_1k_sequences_str = fmt(max_1k_sequences, precision=1, allow_zero=True, commas=False)
```
...
class RankingCascadeCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    final_eval_reduction_str = fmt(final_eval_reduction, precision=1, allow_zero=True)
    l3_per_candidate_ms_str = fmt(l3_per_candidate_ms, precision=1, allow_zero=True, commas=False)
    l3_all_s_str = fmt(l3_all_s, precision=1, allow_zero=True)
    cascade_budget_ms_str = fmt(cascade_budget_ms, precision=1, allow_zero=True, commas=False)
    cost_reduction_rounded_str = fmt(cost_reduction_rounded, precision=1, allow_zero=True)
```
