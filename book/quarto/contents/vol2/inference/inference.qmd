...
4.  **Annual Serving Cost**\index{Serving Cost!annual serving cost}: $18 \times 10^9 \times \$0.001 = \mathbf{\$18{,}000{,}000}$.

**Systems conclusion**: Serving costs **9$\times$** more than training in just the first year. A 10 percent serving-cost or latency-linked efficiency improvement saves about **\$1.8M** in the first year, nearly paying for the original training run.
...
from mlsysim.fmt import fmt, check, md_math, MarkdownStr
...
class InferenceEconomics:
    ...
    if lifetime_queries_val >= 1e9:
        ...
        lifetime_queries_latex_math = md_math(
            f"{mantissa:.2f} \\times 10^{{{exp}}}"
        )
    else:
        lifetime_queries_latex_math = md_math(f"{lifetime_queries_val:,.0f}")
    serving_cost_total_str = fmt(fmt(serving_total_cost_val, precision=0), precision=0, commas=False, prefix="USD ")
    training_cost_total_str = fmt(fmt(training_total_cost, precision=0), precision=0, commas=False, prefix="USD ")
...
# ┌── 4. DECORATE ──────────────────────────────────────────────────────────────
ax.text(
    2e3, typical_pricing * 1.14,   # malo iznad
...
$$ M_{\text{KV}} = 2 \times 80 \times 8 \times 128 \times 2 = 327,680 \text{ bytes} \approx \mathbf{0.33 \text{ MB/token}} $$

**Step 2**: Calculate cache per request.
$$ 131,072 \text{ tokens} \times 0.33 \text{ MB/token} \approx \mathbf{43 \text{ GB/request}} $$

**Step 3**: Determine max batch size.
Available Memory for KV = $640 \text{ GB (Total)} - 140 \text{ GB (Weights)} - 20 \text{ GB (System)} = 480 \text{ GB}$.
$$ \text{Max Batch} = \lfloor \frac{480}{43} \rfloor = \mathbf{11} $$
...
A single request's KV cache alone consumes a substantial fraction of the H100's `{python} KVCacheCalc.h100_mem_gb_str` GB of HBM. For a batch of 8 concurrent requests, the KV cache would require `{python} f"{KVCacheCalc.kv_fp16_gb * 8:.0f}"` GB, exceeding single-GPU capacity. Reducing this footprint requires two complementary strategies: quantization and architectural optimization.
...
- KV cache per request: `{python} KVCacheCalc.kv_fp16_gb_str` GB $\div$ 4 GPUs $\approx$ `{python} f"{KVCacheCalc.kv_fp16_gb/4:.1f}"` GB/GPU
- Maximum batch size: $\lfloor$ `{python} PrecisionDividend.avail_kv_fp16_str` $/$ `{python} f"{KVCacheCalc.kv_fp16_gb/4:.1f}"` $\rfloor$ $\approx$ `{python} f"{int(45 / (KVCacheCalc.kv_fp16_gb/4))}"` requests
...
- KV cache per request (INT8): `{python} f"{KVCacheCalc.kv_fp16_gb/2:.1f}"` GB $\div$ 4 $\approx$ `{python} f"{KVCacheCalc.kv_fp16_gb/2/4:.1f}"` GB/GPU
- Maximum batch size: $\lfloor$ `{python} PrecisionDividend.avail_kv_int4_str` $/$ `{python} f"{KVCacheCalc.kv_fp16_gb/2/4:.1f}"` $\rfloor$ $\approx$ `{python} f"{int(float(PrecisionDividend.avail_kv_int4) / (KVCacheCalc.kv_fp16_gb/2/4))}"` requests
...
1.  **Memory**: Total weight memory = $671 \text{ GB}$. A single node (640 GB) is insufficient. Two nodes provide 1,280 GB, leaving enough room for KV caches.
2.  **Latency**: Each decode step reads 37B active parameters (37 GB). Distributed across 16 GPUs, each reads about `{python} MoECapacityPlan.read_per_gpu_str` GB. At `{python} InferenceSetup.h100_mem_bw_tbs_str` TB/s HBM bandwidth, $t_{\text{read}} \approx$ `{python} MoECapacityPlan.t_read_ms_str` ms. AllToAll routing adds about 0.3 ms.
...
| **InfiniBand**\index{InfiniBand} |             0.7 ms |          2.3 percent |
...
A 70B model with 4-bit weights requires approximately 35 GB, leaving 45 GB on an `{python} A100HardwareScenario.a100_mem_gb_str` GB A100 for KV cache. With FP16 KV cache at about 2.5 GB per 1K-token sequence for the MHA baseline, this supports ~18 concurrent 1K-token sequences. With FP16 weights, the same 70B model would not fit on one A100 at all, so weight-only quantization changes both feasibility and batch capacity.
...
Production-scale serving systems test the batching, sharding, caching, and routing techniques developed above against real workload variance, latency budgets, and operational constraints. OpenAI's ChatGPT handles unpredictable global request volume; TikTok serves recommendations to billions of users at sub-100-millisecond latency. The case studies below show how each system combines the chapter's techniques to meet its specific cost and latency targets.
