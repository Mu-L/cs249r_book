...
::: {.callout-checkpoint title="Distribution strategy selection"}
...
The total serving tax often consumes 10–30 percent of the latency budget in distributed systems (@eq-serving-tax):
...
class ServingCostSimple:
    ...
    annual_reqs_b = round(annual_reqs / BILLION)
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    annual_reqs_b_str = fmt(annual_reqs_b, precision=0, commas=False)
...
::: {.callout-notebook title="The serving cost multiplier"}
...
::: {.callout-checkpoint title="The serving hierarchy"}
...
::: {.callout-checkpoint title="Serving dimensions"}
...
Batching efficiency depends on how computation scales with batch size relative to how memory and communication scale. Different model architectures exhibit different scaling relationships, requiring different batching strategies, as @fig-batching-strategies summarizes.\index{Dynamic Batching}
...
::: {.callout-notebook title="The batching efficiency curve"}
...
::: {.callout-theorem title="Little's Law for inference"}
...
::: {.callout-perspective title="Queuing theory and performance"}
...
$$B \approx \sqrt{\frac{2\alpha\lambda}{1 - \rho_{\text{target}}}}$$ {#eq-optimal-batch-approx}

where $\rho_{\text{target}}$ is the target utilization (typically 0.7--0.8 for production systems to maintain latency headroom).
...
3. **Higher target utilization** $(\rho_{\text{target}})$: Optimal batch size increases (more aggressive batching)
...
: **Batch Size Impact on GPT-3 Serving Performance**: Batch sizes below 8 cannot sustain 100 QPS (utilization exceeds 100 percent). The latency minimum occurs near B=16, while B=32 leaves more headroom because it runs at lower utilization despite having a slightly higher total latency. {#tbl-batch-size-comparison}
...
class LittlesLawVerify:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(7 < L < 8, f"L should be ~7.4, got {L:.2f}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
...
3. **Batch size 16 minimizes total latency**\index{Batch Size!16 minimizes total latency}: Despite longer service time (`{python} GPT3ServingResults.b16_service_ms_str` ms vs. `{python} GPT3ServingResults.b8_service_ms_str` ms), the reduction in queue wait time (`{python} GPT3ServingResults.b16_wait_ms_str` ms vs. `{python} GPT3ServingResults.b8_wait_ms_str` ms) yields the lowest total latency, while B=32 leaves more utilization headroom.
...
::: {.callout-notebook title="Applying Little's Law to verify"}
...
where $E[T_{\text{queue}}]$ is the expected queuing delay, $T_{\text{batch}}$ is the batch formation delay (up to $T_{\text{window}}$), and $T_{\text{inference}}(B)$ is the inference time for batch size $B$. A concrete example of *dynamic batching for ResNet-50* illustrates how these components interact in practice.

::: {.callout-example title="Dynamic batching for ResNet-50"}
...
**Tradeoff**\index{Tradeoff}: Option C achieves 25 percent better throughput (lower utilization) at the cost of higher average latency (22 ms vs. 15 ms). Both meet the 50 ms P99 SLO.
...
::: {.callout-definition title="Continuous batching"}
...
::: {.callout-perspective title="Analogy: The restaurant kitchen"}
...
::: {.callout-lighthouse title="Archetype A (GPT-4/Llama-3): Throughput-latency"}
...
::: {.callout-example title="Continuous batching in vLLM"}
...
|:----------------------------|-------:|------:|------------------:|-------------------------:|
| **Code completion**         |    0.3 |   2.5 |        18 percent |              1.2$\times$ |
| **Chat (short responses)**  |    0.6 |   2.0 |        33 percent |              1.5$\times$ |
| **General text generation** |    1.0 |   2.5 |        50 percent |              2.0$\times$ |
| **Creative writing**        |    1.5 |   3.0 |        64 percent |              2.8$\times$ |
| **RAG with variable docs**  |    2.0 |   2.5 |        71 percent |              3.5$\times$ |
...
Feature-parallel batching is natural when embeddings are sharded across servers: each embedding server handles lookups for its shard across all requests in the batch. The following example illustrates how *RecSys batching at Meta scale* operates under these constraints.

::: {.callout-example title="RecSys batching at Meta scale"}
...
::: {.callout-example title="Streaming speech recognition pipeline"}
...
::: {.callout-example title="Adaptive batching: Triton"}
...
::: {.callout-checkpoint title="Batching strategy trade-offs"}
...
::: {.callout-definition title="KV cache"}
...
::: {.callout-example title="KV-cache capacity estimator"}
...
::: {.callout-definition title="PagedAttention"}
...
::: {.callout-perspective title="Analogy: The inefficient hotel"}
...
::: {.callout-example title="PagedAttention implementation details"}
...
All three requests reference the same physical blocks for the system prompt. Only when generating unique tokens do they allocate new blocks. The savings from *prefix caching at scale* can be substantial when many concurrent requests share the same system prompt.
...
::: {.callout-example title="Prefix caching at scale"}
...
::: {.callout-definition title="Speculative decoding"}
...
::: {.callout-perspective title="Analogy: The executive and the assistant"}
...
::: {.callout-notebook title="KV cache memory hierarchy"}
...
::: {.callout-lighthouse title="Blackwell: The FP4 inference frontier"}
...
- **Decode pool**\index{Disaggregated Serving!decode pool}: Optimized for memory bandwidth (for example, nodes with maximum HBM capacity or specialized High Bandwidth Flash as discussed in @sec-data-storage) to handle thousands of concurrent autoregressive streams.
...
::: {.callout-example title="Sarathi: Chunked prefill implementation"}
...
::: {.callout-perspective title="The context switch of machine learning"}
...
::: {.callout-example title="Tensor parallelism for Llama-70B"}
...
::: {.callout-example title="Expert parallelism for Mixtral-8x7B"}
...
::: {.callout-lighthouse title="Embedding sharding at Meta"}
...
Both equations make clear that the achievable bandwidth $\text{BW}$ and latency $\alpha$ of the underlying interconnect determine real-world sharding performance. The following *interconnect technology comparison* quantifies these differences across production hardware.
...
::: {.callout-lighthouse title="Archetype B (DLRM at Scale): The tail at scale"}
...
The power of two choices (Principle \ref{pri-power-two-choices}) provides the theoretical foundation: querying just two random replicas and selecting the least-loaded one exponentially reduces tail latency from $O(\log n)$ to $O(\log \log n)$.
...
# │   a100_util_equal_str="83"% (overloaded)—inline in the callout.
...
::: {.callout-example title="Heterogeneous GPU cluster"}
...
- H100: $15000 \times 0.045$ = `{python} HeterogeneousGpuCluster.h100_load_str` QPS (`{python} HeterogeneousGpuCluster.h100_util_str` percent utilization)
- A100: $15000 \times 0.027$ = `{python} HeterogeneousGpuCluster.a100_load_str` QPS (`{python} HeterogeneousGpuCluster.a100_util_str` percent utilization)
...
- A100 utilization: `{python} HeterogeneousGpuCluster.a100_util_equal_str` percent (overloaded, latency spikes)
...
::: {.callout-example title="Least-connections for LLM serving"}
...
::: {.callout-example title="Consistent hashing for KV cache affinity"}
...
::: {.callout-example title="Health checks for GPU inference"}
...
Without these mechanisms, a single degraded server can trigger a chain reaction that takes down the entire serving fleet. The following scenario illustrates the importance of *cascading failure prevention*.

::: {.callout-example title="Cascading failure prevention"}
...
::: {.callout-notebook title="Quantifying noisy neighbor impact"}
...
@tbl-inference-noisy-neighbor-no-isolation records the noisy-neighbor scenario when tenant 3 bursts to 500 QPS:
...
: **Noisy Neighbor Without Isolation**: Per-tenant QPS, p99 latency, and SLO status when tenant 3 bursts to 500 QPS on a shared GPU pool with no resource quotas. Latencies for every tenant exceed the 10 ms SLO, demonstrating how a single offending tenant cascades into a multi-tenant outage. {#tbl-inference-noisy-neighbor-no-isolation}
...
::: {.callout-example title="Bulkhead configuration for API tiers"}
...
: **Model Load Time by Storage Tier**: Local SSD loads weights at roughly 6 GB/s; remote object storage (S3) loads at roughly 0.5 GB/s. The 12$\times$ gap matters most for the largest models: a 175B model takes about 1 minute from SSD (350 GB at 6 GB/s) vs. 12 minutes from S3, dominating the cold-start budget for any serving deployment that has to pull weights from object storage on each scale-up. {#tbl-model-load-time}
...
::: {.callout-notebook title="Cold start timeline for Llama-70B"}
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, xscale=0.025, yscale=0.8]
  ...
  % Bar Stack
  \node[bar, fill=ProvColor, minimum width=1.625cm] (prov) at (0, 1.5) {Provision};
  \node[bar, fill=DownColor, minimum width=4.5cm, right=0cm of prov] (down) {Model Download (Big!)};
  \node[bar, fill=LoadColor, minimum width=1.125cm, right=0cm of down] (load) {Load};
  \node[bar, fill=WarmColor, minimum width=0.625cm, right=0cm of load] (warm) {Warm};

  % Connect total
  \draw[decorate, decoration={brace, amplitude=5pt, raise=5pt}] (prov.south west |- 0, 2) -- (warm.south east |- 0, 2) node[midway, above=10pt, font=\bfseries\usefont{T1}{phv}{b}{n}] {Total Cold Start: > 5 Minutes};
...
::: {.callout-notebook title="Reactive scaling response analysis"}
...
::: {.callout-war-story title="The ChatGPT traffic spike"}
...
::: {.callout-example title="Predictive scaling for traffic"}
...
: **Quantization Perplexity on LLaMA-2-70B**: Perplexity degradation across four quantization methods on a 70B model. QuaRot's W4A4 lands within 0.2 perplexity of FP16 baseline while delivering roughly 8$\times$ memory reduction, demonstrating that rotation-based methods make 4-bit activation quantization viable for production inference. {#tbl-quarot-perplexity}
...
::: {.callout-war-story title="The continuous batching revolution"}
...
Queuing theory establishes that systems provisioned for average load violate SLOs during traffic peaks. At 80 percent average utilization, a modest 25 percent traffic spike pushes utilization above 100 percent, causing unbounded queue growth. As discussed in @sec-inference-scale-autoscaling-168d, the cold start problem exacerbates this: by the time new capacity is available (5+ minutes for GPU instances), SLO violations have already occurred. Capacity planning must account for peak load plus headroom, not average load.
