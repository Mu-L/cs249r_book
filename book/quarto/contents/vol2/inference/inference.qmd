...
class InferenceSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_gb_str = fmt_int(h100_mem_gb_val, commas=False)
    ...
    nvlink_h100_gbs_str = fmt_int(nvlink_gbs_val, commas=False)
```
...
class ServingCostSimple:
    ...
    annual_serving_cost_str = fmt_int(annual_serving_cost, prefix="\\$")
    ...
    training_cost_str = fmt_int(training_cost, prefix="\\$")
    dau_str = fmt_int(dau)
    ...
    efficiency_gain_pct_str = fmt_int(efficiency_gain * 100, commas=False)
```
...
**Systems insight**: Serving costs **`{python} ServingCostSimple.serving_training_ratio_str`$\times$** more than training in just the first year. A `{python} ServingCostSimple.efficiency_gain_pct_str` serving-cost or latency-linked efficiency improvement saves about **`{python} ServingCostSimple.efficiency_savings_str`** in the first year, nearly paying for the original training run.
...
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.fmt import fmt_int, fmt, check, fmt_math, MarkdownStr
...
class InferenceEconomics:
    ...
    serving_cost_total_str = fmt_int(serving_total_cost_val, prefix="USD ")
    training_cost_total_str = fmt_int(training_total_cost, prefix="USD ")
    ...
    cost_per_million_str = fmt_int(cost_per_query * MILLION, commas=False)
    training_cost_brief_str = fmt_int(training_total_cost, prefix="USD ")
    n_gpu_hours_str = fmt_int(n_gpu_hours)
    cost_per_gpu_hour_str = fmt_int(cost_per_gpu_hour, commas=False, prefix="\\$")
    engineering_multiplier_str = fmt_int(engineering_multiplier, commas=False)
    training_hardware_cost_str = fmt_int(training_hardware_cost, prefix="\\$")
    engineering_overhead_cost_str = fmt_int(training_hardware_cost * engineering_multiplier, prefix="\\$")
    sec_per_day_str = fmt_int(SEC_PER_DAY, commas=False)
```
...
(`{python} InferenceEconomics.qps_str` $\times$ `{python} InferenceEconomics.sec_per_day_str` s/day $\times$ `{python} InferenceEconomics.serve_duration_days_str` days)
...
class InferenceTopologyRecap:
    ...
    from mlsysim.fmt import fmt_int, fmt
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_gbs_str = fmt_int(nvlink_gbs, commas=False)
```
...
class LittlesLawExample:
    ...
    latency_slo_ms_str = fmt_int(latency_slo_s * 1000, commas=False)
    batch_size_str = fmt_int(batch_size, commas=False)
    replica_latency_ms_str = fmt_int(replica_latency_s * 1000, commas=False)
```
...
class BatchingWorkedExample:
    ...
    r1_imp_str = fmt_int(r1_improvement, commas=False)
    r3_imp_str = fmt_int(r3_improvement, commas=False)
...
- R3: $100 \times 20$ ms = `{python} BatchingWorkedExample.r3_lat_str` (`{python} BatchingWorkedExample.r3_imp_str`$\times$ improvement)
- R4: $150 \times 20$ ms = `{python} BatchingWorkedExample.r4_lat_str` (`{python} BatchingWorkedExample.r4_imp_str`$\times$ improvement)
- R2: $200 \times 20$ ms = `{python} BatchingWorkedExample.r2_lat_str` (no improvement for longest request)

Average latency comparison

- Traditional: `{python} BatchingWorkedExample.trad_lat_str` (all requests)
- Continuous: (`{python} BatchingWorkedExample.r1_lat_str` + `{python} BatchingWorkedExample.r3_lat_str` + `{python} BatchingWorkedExample.r4_lat_str` + `{python} BatchingWorkedExample.r2_lat_str`) / 4 = `{python} BatchingWorkedExample.cont_avg_lat_str`
...
class ContinuousBatchingBenefit:
    ...
    text_waste_pct_str = fmt_int(text_waste_pct, commas=False)
    text_speedup_str = fmt_int(text_speedup, commas=False)
...
|:----------------------------|-----------------------------------------------------:|----------------------------------------------------:|------------------------------------------------------------:|------------------------------------------------------------------:|
| **Code completion**         |     `{python} ContinuousBatchingBenefit.code_cv_str` |     `{python} ContinuousBatchingBenefit.code_k_str` |     `{python} ContinuousBatchingBenefit.code_waste_pct_str` |     `{python} ContinuousBatchingBenefit.code_speedup_str`$\times$ |
| **Chat (short responses)**  |     `{python} ContinuousBatchingBenefit.chat_cv_str` |     `{python} ContinuousBatchingBenefit.chat_k_str` |     `{python} ContinuousBatchingBenefit.chat_waste_pct_str` |     `{python} ContinuousBatchingBenefit.chat_speedup_str`$\times$ |
| **General text generation** |     `{python} ContinuousBatchingBenefit.text_cv_str` |     `{python} ContinuousBatchingBenefit.text_k_str` |     `{python} ContinuousBatchingBenefit.text_waste_pct_str` |     `{python} ContinuousBatchingBenefit.text_speedup_str`$\times$ |
| **Creative writing**        | `{python} ContinuousBatchingBenefit.creative_cv_str` | `{python} ContinuousBatchingBenefit.creative_k_str` | `{python} ContinuousBatchingBenefit.creative_waste_pct_str` | `{python} ContinuousBatchingBenefit.creative_speedup_str`$\times$ |
| **RAG with variable docs**  |      `{python} ContinuousBatchingBenefit.rag_cv_str` |      `{python} ContinuousBatchingBenefit.rag_k_str` |      `{python} ContinuousBatchingBenefit.rag_waste_pct_str` |      `{python} ContinuousBatchingBenefit.rag_speedup_str`$\times$ |
...
class A100HardwareScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_gb_str = fmt_int(a100_mem_gb_val, commas=False)
    a100_mem_bw_tbs_str = fmt(a100_mem_bw_tbs_val, precision=1, commas=False)
    nvlink_a100_gbs_str = fmt_int(nvlink_a100_gbs_val, commas=False)
    pcie_gen4_gbs_str = fmt_int(pcie_gen4_gbs_val, commas=False)
    activation_transfer_mb_str = fmt_int(activation_transfer_mb, commas=False)
...
class KVCacheCapacityEstimator:
    ...
    available_kv_gb_str = fmt_int(available_kv_gb, commas=False)
    max_batch_str = fmt_int(max_batch, commas=False)
    weight_gb_str = fmt_int(weight_gb, commas=False)
    total_hbm_gb_str = fmt_int(total_hbm_gb, commas=False)
    system_gb_str = fmt_int(system_gb, commas=False)
    node_gpus_str = fmt_int(Systems.Nodes.DGX_H100.accelerators_per_node, commas=False)
    h100_mem_gb_str = fmt_int(Hardware.Cloud.H100.memory.capacity.m_as(GiB), commas=False)
...
class PrefixCachingAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tokens_per_user_str = fmt_int(tokens_per_user)
    ...
    concurrent_multiplier_str = fmt_int(concurrent_multiplier_val, commas=False)
```
...
class KVCacheCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt_int, check, MarkdownStr, fmt, fmt_math
    ...
    kv_compression_str = fmt_int(kv_compression, commas=False)
    h100_mem_gb_str = fmt_int(h100_mem.m_as(GiB), commas=False)
...
class PrecisionDividend:
    ...
    fp16_weight_per_gpu_str = fmt_int(fp16_weight_per_gpu, commas=False)
    avail_kv_fp16_str = fmt_int(avail_kv_fp16, commas=False)
    ...
    fp16_weight_total_gb_str = fmt_int(fp16_weight_total_gb, commas=False)
...
- Weights: `{python} PrecisionDividend.fp16_weight_per_gpu_str`/GPU
- Available for KV cache: $80 -$ `{python} PrecisionDividend.fp16_weight_per_gpu_str` = `{python} PrecisionDividend.avail_kv_fp16_str`/GPU
- KV cache per request: `{python} KVCacheCalc.kv_fp16_gb_str` GB $\div$ `{python} PrecisionDividend.tensor_parallel_degree_str` $\approx$ `{python} PrecisionDividend.kv_fp16_per_gpu_str` GB/GPU
- Maximum batch size: $\lfloor$ `{python} PrecisionDividend.avail_kv_fp16_str` divided by `{python} PrecisionDividend.kv_fp16_per_gpu_str` $\rfloor$ $\approx$ `{python} PrecisionDividend.max_batch_fp16_str` requests

**After optimization** (INT4 weights, INT8 KV cache):

- Weights: `{python} PrecisionDividend.fp16_weight_per_gpu_str` $\times (4/16) =$ `{python} PrecisionDividend.int4_weight_per_gpu_str`/GPU (INT4)
- Available for KV cache: $80 -$ `{python} PrecisionDividend.int4_weight_per_gpu_str` = `{python} PrecisionDividend.avail_kv_int4_str`/GPU
- KV cache per request (INT8): `{python} PrecisionDividend.kv_int8_total_str` $\div$ 4 $\approx$ `{python} PrecisionDividend.kv_int8_per_gpu_str`/GPU
- Maximum batch size: $\lfloor$ `{python} PrecisionDividend.avail_kv_int4_str` divided by `{python} PrecisionDividend.kv_int8_per_gpu_str` $\rfloor$ $\approx$ `{python} PrecisionDividend.max_batch_int8_str` requests
...
class TpSpeedupCalc:
    ...
    seq_total_ms_str = fmt_int(seq_total_ms)
    tp8_total_ms_str = fmt_int(tp8_total_ms)
    tp_theoretical_str = fmt_int(tp_theoretical, commas=False)
    n_layers_str = fmt_int(n_layers, commas=False)
    nvlink_h100_gbs_str = fmt_int(nvlink_h100_gbs_val, commas=False)
    hidden_dim_str = fmt_int(hidden_dim)
    weight_fp16_gb_str = fmt_int(weight_fp16_gb, commas=False)
...
class MoEEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dense_mem_str = fmt_int(dense_mem_fp16, commas=False)
    moe_mem_str = fmt_int(moe_mem_fp16, commas=False)
...
class MoECapacityPlan:
    ...
    tokens_per_sec_str = fmt_int(tokens_per_sec)
    total_weight_gb_str = fmt_int(total_weight_gb, commas=False)
    node_capacity_gb_str = fmt_int(node_capacity_gb, commas=False)
    nodes_needed_str = fmt_int(nodes_needed, commas=False)
    cluster_capacity_gb_str = fmt_int(cluster_capacity_gb)
    active_mem_gb_str = fmt_int(active_mem_gb, commas=False)
    moe_active_params_b_str = fmt_int(moe_active_params_b, commas=False)
    ...
    h100_mem_gb_str = fmt_int(h100_mem_gb, commas=False)
    moe_total_params_b_str = fmt_int(moe_total_params_b, commas=False)
...
2.  **Latency**: Each decode step reads `{python} MoECapacityPlan.moe_active_params_b_str`B active parameters (`{python} MoECapacityPlan.active_mem_gb_str`). Distributed across `{python} MoECapacityPlan.n_gpus_str`, each reads about `{python} MoECapacityPlan.read_per_gpu_str`. At `{python} MoECapacityPlan.h100_bw_tbs_str`/s HBM bandwidth, $t_{\text{read}} \approx$ `{python} MoECapacityPlan.t_read_ms_str`. AllToAll routing adds about `{python} MoECapacityPlan.alltoall_overhead_ms_str`.
3.  **Floor**: Estimated decode latency $\approx$ `{python} MoECapacityPlan.total_decode_ms_str`/token, or `{python} MoECapacityPlan.tokens_per_sec_str`/second at batch size 1.
...
class AllReduceInterconnect:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvlink_bw_gbs_str = fmt_int(nvlink_bw_gbs, commas=False)
    ...
    message_mb_str = fmt_int(message_mb, commas=False)
...
class HeterogeneousGpuCluster:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_cap_str = fmt_int(total_cap)
    ...
    equal_load_str = fmt_int(equal_load, commas=False)
    h100_util_equal_str = fmt_int(h100_util_equal, commas=False)
    ...
    h100_cap_qps_str = fmt_int(h100_cap_qps, commas=False)
    a100_cap_qps_str = fmt_int(a100_cap_qps, commas=False)
    target_qps_str = fmt_int(target_qps)
```
...
- Total capacity: $10 \times 1000$ + $20 \times 600$ = `{python} HeterogeneousGpuCluster.total_cap_str`
...
- Per-server load: 15000/30 = `{python} HeterogeneousGpuCluster.equal_load_str`
- H100 utilization: `{python} HeterogeneousGpuCluster.h100_util_equal_str` (underutilized)
- A100 utilization: `{python} HeterogeneousGpuCluster.a100_util_equal_str` (high utilization, reduced latency headroom)
...
class NoisyNeighborQuotaScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    slo_ms_str = fmt_int(slo_ms, commas=False)
    tenant_base_qps_str = fmt_int(tenant_base_qps, commas=False)
    tenant_burst_qps_str = fmt_int(tenant_burst_qps, commas=False)
    tenant_throttled_qps_str = fmt_int(tenant_throttled_qps, commas=False)
    ...
    gpu_util_pct_str = fmt_int(70, commas=False)
```
...
class QuantizedServingCapacity:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_gb_str = fmt_int(a100_mem_gb, commas=False)
    weight_int4_gb_str = fmt_int(weight_int4_gb, commas=False)
    kv_headroom_gb_str = fmt_int(kv_headroom_gb, commas=False)
...
class RankingCascadeCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    final_eval_reduction_str = fmt_int(final_eval_reduction)
    l3_per_candidate_ms_str = fmt_int(l3_per_candidate_ms, commas=False)
    l3_all_s_str = fmt_int(l3_all_s)
    cascade_budget_ms_str = fmt_int(cascade_budget_ms, commas=False)
    cost_reduction_rounded_str = fmt_int(cost_reduction_rounded)
```
