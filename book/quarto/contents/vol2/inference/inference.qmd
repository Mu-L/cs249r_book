...
At the single-node level, inference behaves as an M/D/1 or M/M/1 queue. Little's Law ($L = \lambda W$) dictates that the number of requests in the system equals the arrival rate times the processing time. For a rigorous mathematical review of single-node queuing dynamics and utilization curves, see @sec-appdx-inference-scale-queuing-theory-batched-inference-bbf0. @Sec-appdx-appendix-inference-serving works the M/D/1 queue analysis through from first principles, deriving the wait-time distribution that determines whether queue delay fits inside a serving SLO.
