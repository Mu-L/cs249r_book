...
# │
# │ Imports: mlsysim.core.constants (H100_MEM_CAPACITY, H100_MEM_BW, A100_MEM_BW,
# │          NVLINK_H100_BW, GiB, GB, TB, second)
# │ Exports: InferenceSetup.h100_mem_gb_str, InferenceSetup.h100_mem_bw_tbs_str,
# │          InferenceSetup.a100_mem_bw_tbs_str, InferenceSetup.nvlink_h100_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    H100_MEM_CAPACITY, H100_MEM_BW, A100_MEM_BW, NVLINK_H100_BW,
    GiB, GB, TB, second
)
...
class InferenceSetup:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    mem_h100 = H100_MEM_CAPACITY
    bw_h100 = H100_MEM_BW
    bw_a100 = A100_MEM_BW
    nvlink = NVLINK_H100_BW

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    h100_mem_gb_val = mem_h100.m_as(GiB)
    h100_bw_tbs_val = bw_h100.m_as(TB/second)
    a100_bw_tbs_val = bw_a100.m_as(TB/second)
    nvlink_gbs_val = nvlink.m_as(GB/second)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check
    check(h100_bw_tbs_val > a100_bw_tbs_val, "H100 bandwidth should exceed A100 bandwidth")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_gb_str = fmt(h100_mem_gb_val, precision=0, commas=False)
    h100_mem_bw_tbs_str = fmt(h100_bw_tbs_val, precision=2, commas=False)
    a100_mem_bw_tbs_str = fmt(a100_bw_tbs_val, precision=0, commas=False)
    nvlink_h100_gbs_str = fmt(nvlink_gbs_val, precision=0, commas=False)
```
...
| **Throughput**\index{Throughput} | ~10K QPS (vision)        |      100K QPS RecSys | Horizontal replication      |
| **Latency**\index{Latency}       | Model execution time     |      500 ms LLM TTFT | Model sharding              |
...
The perceived responsiveness of interactive systems depends on both **Time to First Token (TTFT)**[^fn-ttft-tpot] and **Time Per Output Token (TPOT)**.
...
- [ ] Identify the trade-off: what is the "serving tax" mentioned in the next section, and how does it affect the decision to shard across machines vs. staying within a single node?
...
| **Aspect**              | **Distributed Training**\index{Distributed Training} | **Distributed Inference**    |
|:------------------------|:-----------------------------------------------------|:-----------------------------|
...
#| echo: false
from mlsysim.core.constants import BILLION
...
class ServingCostSimple:
    ...
    annual_serving_cost_str = fmt(annual_serving_cost, precision=0, prefix="\\$")
    serving_training_ratio_str = fmt(serving_training_ratio, precision=0, commas=False)
    efficiency_savings_str = fmt(efficiency_savings/1_000_000, precision=1, commas=False, prefix="\\$", suffix="M")
```

::: {#nbk-inference-serving-cost-multiplier .callout-notebook title="The serving cost multiplier"}

**Problem**: A team has spent **\$2 million** training a 70B parameter model and now serves it to 1 million daily active users (DAU), each making 50 requests/day. Is training or serving the dominant cost over 1 year?

**Math**:

1.  **Training Cost**\index{Training Cost}: \$2,000,000 (One-time).
...
3.  **Cost per Request**\index{Serving Cost!cost per request}: Assume a 70B model on H100 costs $\approx \$0.001$ per request (input + output).
4.  **Annual Serving Cost**\index{Serving Cost!annual serving cost}: `{python} ServingCostSimple.annual_reqs_b_str` billion requests $\times \$0.001 =$ **`{python} ServingCostSimple.annual_serving_cost_str`**.

**Systems conclusion**: Serving costs **`{python} ServingCostSimple.serving_training_ratio_str`$\times$** more than training in just the first year. A 10 percent serving-cost or latency-linked efficiency improvement saves about **`{python} ServingCostSimple.efficiency_savings_str`** in the first year, nearly paying for the original training run.
...
# │          InferenceEconomics.leverage_ratio_str,
# │          InferenceEconomics.qps_str,
# │          InferenceEconomics.serve_duration_days_str,
# │          InferenceEconomics.cost_per_million_str
# └─────────────────────────────────────────────────────────────────────────────
import math
from mlsysim.core.constants import BILLION, SEC_PER_DAY, SEC_PER_YEAR, THOUSAND, MILLION
...
class InferenceEconomics:
    ...
    serving_cost_total_str = fmt(serving_total_cost_val, precision=0, prefix="USD ")
    training_cost_total_str = fmt(training_total_cost, precision=0, prefix="USD ")
    leverage_ratio_str = fmt(leverage_ratio, precision=0, commas=False)
    qps_str = fmt(qps, precision=0)
    serve_duration_days_str = fmt(serve_duration_days, precision=0, commas=False)
    cost_per_million_str = fmt(cost_per_query * MILLION, precision=0, commas=False)
```

::: {#nbk-inference-serving-cost-multiplier-2 .callout-notebook title="The serving cost multiplier"}

**Scenario**: Recommend-system model (DLRM).

**Training Cost** $(C_{\text{train}})$

- Hardware: 1,000 GPU-hours @ \$3/hr = \$3,000
- Engineering Overhead (Data/Exp): 3$\times$ Hardware = \$9,000
- **Total Training**\index{Training!total}: `{python} InferenceEconomics.training_cost_total_str`

**Serving Cost** $(C_{\text{serve}})$

- Deployment: `{python} InferenceEconomics.serve_duration_days_str` days (constant updates, amortized)
- Traffic: `{python} InferenceEconomics.qps_str` Queries Per Second (QPS)
- Efficiency: `{python} InferenceEconomics.cost_per_million_str` dollars per million queries

**Lifetime Volume** $(Q_{\text{total}})$
$Q_{\text{total}} \approx$ **`{python} InferenceEconomics.lifetime_queries_latex_math` queries**
(`{python} InferenceEconomics.qps_str` QPS $\times$ 86,400 s/day $\times$ `{python} InferenceEconomics.serve_duration_days_str` days)

**Total Serving Cost**

$C_{\text{serve\_total}} =$ `{python} InferenceEconomics.lifetime_queries_latex_math` $\times \$10^{-5} =$ **`{python} InferenceEconomics.serving_cost_total_str`**

**Conclusion**:

Ratio = `{python} InferenceEconomics.serving_cost_total_str`/`{python} InferenceEconomics.training_cost_total_str` $\approx$ **`{python} InferenceEconomics.leverage_ratio_str`$\times$**
Serving optimization leverage is `{python} InferenceEconomics.leverage_ratio_str`$\times$ higher than training optimization. A 1 percent serving efficiency gain saves significant capital, far exceeding the entire training budget.
...
@fig-serving-cost-curve illustrates why optimization matters: the gap between naive and optimized serving determines whether an inference service is profitable or hemorrhaging money.
...
- **Recommendation and ranking**\index{Recommendation and Ranking}: 80–90 percent of requests
- **Vision and image processing**\index{Vision and Image Processing}: 5–10 percent of requests
- **NLP/LLM**\index{NLP/LLM}: 1–5 percent of requests (but growing rapidly)
...
The optimization techniques in this chapter organize into a **serving hierarchy**\index{Serving!hierarchy}, analogous to the memory hierarchy in computer architecture. Each level identifies distinct optimization targets and techniques.
...
The four conceptual levels form a *serving hierarchy*. A related deployment stack appears in @fig-serving-hierarchy, showing how requests pass through edge, routing, and model-serving infrastructure in production.
...
| **Replica**\index{Replica} | Throughput, utilization | Memory optimization, kernel fusion     |
...
- [ ] Which level of the hierarchy is responsible for managing **KV cache fragmentation** to increase concurrent request capacity?
...
Autoregressive language models break this assumption. Each request generates a different number of output tokens, so static batching forces all requests to wait for the longest generation in the batch. The resulting GPU idle time can waste 60--80 percent of available compute. Continuous batching[^fn-vllm-etymology] solves this by allowing requests to enter and exit the batch at each decoding step. When one request finishes generation, a new request immediately fills its slot. Systems like vLLM [@kwon2023] pioneered this approach, achieving 2--4$\times$ throughput over static batching for LLM workloads by eliminating the "padding waste" inherent in fixed-size batches.
...
```{python}
#| label: deployment-topology-hardware
#| echo: false
# ┌─────────────────────────────────────────────────────────────────────────────
# │ DEPLOYMENT TOPOLOGY HARDWARE (LEGO)
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-inference-scale-tensorrtllm-architecture-2794
# │
# │ Goal: Provide local NVLink bandwidth for the deployment topology discussion.
# │ Show: "900" GB/s.
# │ How: pulling NVLINK_H100_BW from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (NVLINK_H100_BW, GB, second)
# │ Exports: DeploymentTopologyHardware.nvlink_h100_gbs
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import NVLINK_H100_BW, GB, second

class DeploymentTopologyHardware:
    """Local hardware reference for deployment topology prose."""
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check
    check(nvlink_gbs_val > 100, "NVLink bandwidth should exceed commodity NIC bandwidth")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    nvlink_h100_gbs = f"{nvlink_gbs_val:.0f}"
```
...
Multi-GPU tensor parallelism shards each layer's weight matrices across multiple GPUs connected by high-bandwidth interconnect (NVLink at `{python} InferenceSetup.nvlink_h100_gbs_str` GB/s on H100). Each GPU computes a partial result for every layer, and an all-reduce operation synchronizes the partial results before the next layer. This topology is necessary when model weights exceed single-device memory and provides latency reduction proportional to the parallelism degree, at the cost of communication overhead per layer. The model sharding section (@sec-inference-scale-model-sharding-inference-82b3) analyzes the communication overhead quantitatively.
...
Batching is not merely a heuristic; it is a trade-off governed by the physics of hardware utilization. We can model the relationship between batch size $(B)$, latency $(L_{\text{lat}})$, and throughput $(X)$ to identify the optimal operating point for any inference system.
...
$$X(B) = \frac{B}{L_{\text{lat}}(B)} = \frac{B}{T_{\text{fixed}} + B \times T_{\text{variable}}}$$
...
The engineering goal is to find the maximum $B$ such that $L_{\text{lat}}(B) \le \text{SLO}$. This formulation explains why vision models (high $T_{\text{variable}}$) saturate at smaller batches than LLMs (high $T_{\text{fixed}}$ due to weight loading), requiring different tuning strategies.
...
class BatchingEfficiency:
    ...
    t_var_vis = 1.0 # ms
    # ┌── 2. EXECUTE ───────────────────────────────────────
    def get_x(b, tf, tv): return b / (tf + b * tv) * 1000 # req/s
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    bec_llm_knee_str = fmt(b_knee_llm, precision=0, commas=False)
    bec_vision_knee_str = fmt(b_knee_vis, precision=0, commas=False)
...
class LittlesLawExample:
    ...
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(required_concurrency == 100, "L should be 100")
    check(replicas_needed == 10, "Should need 10 replicas")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    required_concurrency_str = fmt(required_concurrency, precision=0, commas=False)
    replica_throughput_str = fmt(replica_throughput, precision=0, commas=False)
    replicas_needed_str = fmt(replicas_needed, precision=0, commas=False)
    total_concurrency_str = fmt(total_concurrency, precision=0, commas=False)
```

::: {#thrm-inference-littles-law-inference .callout-theorem title="Little's Law for inference"}

**Concept**: In any stable queuing system, the average number of requests in the system $(N_{\text{req}})$ equals the arrival rate $(\lambda)$ multiplied by the average time a request spends in the system $(T_{\text{lat}})$.
...
**Required Concurrency** $(N_{\text{req}})$: $N_{\text{req}} = 1000 \times 0.1 =$ **`{python} LittlesLawExample.required_concurrency_str`** concurrent requests
...
2. Replicas Needed $= 1000 /$ `{python} LittlesLawExample.replica_throughput_str` $=$ `{python} LittlesLawExample.replicas_needed_str` replicas

**Verification**\index{Verification}:
Total system concurrency $=$ `{python} LittlesLawExample.replicas_needed_str` replicas $\times$ 8 batch $=$ `{python} LittlesLawExample.total_concurrency_str`.
Wait! We needed `{python} LittlesLawExample.required_concurrency_str`.
**Correction**: We must account for queue depth. With `{python} LittlesLawExample.replicas_needed_str` replicas active processing `{python} LittlesLawExample.total_concurrency_str` reqs, 20 reqs are in queues.
Queue wait time added to latency must not violate SLO.

:::

### Queuing theory for batched inference {#sec-inference-scale-queuing-theory-batched-inference-bbf0}
...
#### The M/G/c/K queue model for GPU serving {#sec-inference-scale-mgck-queue-model-gpu-serving-c4c4}
...
For a batched inference system with batch size $B$, service time becomes a function of batch size. Let $S(B)$ denote the time to process a batch of $B$ requests. From the physics of batching (@sec-inference-scale-batching-strategies-scale-6733), we model this as:
...
$$\mu_{\text{eff}}(B) = \frac{B}{S(B)} = \frac{B}{\alpha + \beta \cdot B}$$

The effective rate increases with batch size, approaching the asymptotic limit $1/\beta$ as $B \to \infty$.

#### Response time analysis {#sec-inference-scale-response-time-analysis-ffc3}

The total response time $T$ for a request consists of three components:

$$E[T] = E[W] + E[S_{\text{batch}}] + E[S_{\text{compute}}]$$

where:

- $E[W]$ is the expected waiting time in queue before joining a batch
- $E[S_{\text{batch}}]$ is the expected time to form a complete batch (batch accumulation delay)
- $E[S_{\text{compute}}]$ is the expected inference time once the batch executes

For dynamic batching with maximum wait time $T_{\text{max}}$ and maximum batch size $B_{\text{max}}$, the batch formation process is bounded. Under Poisson arrivals with rate $\lambda$, the expected number of requests accumulated in time $T_{\text{max}}$ is $\lambda \cdot T_{\text{max}}$. The actual batch size $B$ follows:

$$E[B] = \min(B_{\text{max}}, \lambda \cdot T_{\text{max}})$$

The batch accumulation delay depends on whether the batch fills by reaching $B_{\text{max}}$ or by timeout:

$$E[S_{\text{batch}}] = \begin{cases}
\frac{B_{\text{max}}}{2\lambda} & \text{if } \lambda \cdot T_{\text{max}} \geq B_{\text{max}} \text{ (batch fills)} \\
\frac{T_{\text{max}}}{2} & \text{if } \lambda \cdot T_{\text{max}} < B_{\text{max}} \text{ (timeout triggers)}
\end{cases}$$

The factor of $1/2$ in both cases reflects the average wait for requests arriving uniformly throughout the batch window.

#### Optimal batch size derivation {#sec-inference-scale-optimal-batch-size-derivation-3a3c}

The optimal batch size $B$ minimizes expected response time subject to throughput requirements. We seek:

$$B = \operatorname{arg\,min}_{B} E[T(B)] \quad \text{subject to} \quad \mu_{\text{eff}}(B) \geq \lambda$$
...
For production sizing, teams often first enforce a utilization headroom target rather than solving the full latency minimization problem. With $S(B)=\alpha+\beta B$, the constraint $\rho=\lambda S(B)/B \leq \rho_{\text{target}}$ gives:

$$B_{\min}(\rho_{\text{target}}) = \frac{\lambda\alpha}{\rho_{\text{target}} - \lambda\beta}$$ {#eq-optimal-batch-approx}

where $\rho_{\text{target}}$ is the target utilization (typically 0.7--0.8 for production systems to maintain latency headroom) and $\rho_{\text{target}} > \lambda\beta$.
...
1. **Higher traffic** $(\lambda)$: Required batch size increases
2. **Higher fixed overhead** $(\alpha)$: Required batch size increases to amortize overhead
3. **Higher target utilization** $(\rho_{\text{target}})$: Required batch size decreases, but with less latency headroom

This utilization-constrained sizing explains why LLMs (high $\alpha$ from weight loading) benefit from larger batches than vision models (low $\alpha$), even at the same arrival rate.

:::

#### Worked example: GPT-3 serving at 100 QPS {#sec-inference-scale-worked-example-gpt3-serving-100-qps-edcf}
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-inference-scale-worked-example-gpt3-serving-100-qps-edcf
# │
...
| **Size B** | **S(B) (ms)** | **(req/s)** |             **$\rho$** | **E[W] (ms)** | **B/(2λ) (ms)** | **E[T] (ms)** |
|:-----------|--------------:|------------:|-----------------------:|:--------------|----------------:|:--------------|
...
# │      apply L = lambda * W to verify requests in system.
# │
# │ Imports: mlsysim.book (check)
...
# │          LittlesLawVerify.expected_total_str,
# │          LittlesLawVerify.l_str, LittlesLawVerify.in_service_str,
# │          LittlesLawVerify.in_queue_str, LittlesLawVerify.batch_size_str
# └─────────────────────────────────────────────────────────────────────────────
...
class GPT3ServingResults:
    ...
    b8_service_ms_str = fmt(b8_service_ms, precision=1, commas=False)
    b8_util_pct_str = fmt(b8_util_pct, precision=1, commas=False)
    b8_wait_ms_str = fmt(b8_wait_ms, precision=1, commas=False)
    b16_service_ms_str = fmt(b16_service_ms, precision=1, commas=False)
    b16_util_pct_str = fmt(b16_util_pct, precision=1, commas=False)
    b16_wait_ms_str = fmt(b16_wait_ms, precision=1, commas=False)

class LittlesLawVerify:
    # ┌── 1. LOAD ──────────────────────────────────────────
    arrival_rate = 100
    batch_size = 16
    expected_total_ms = GPT3ServingResults.b16_total_ms
    utilization = GPT3ServingResults.b16_util_pct / 100
    # ┌── 2. EXECUTE ───────────────────────────────────────
    expected_total_s = (expected_total_ms * ms).m_as(second)
    L = arrival_rate * expected_total_s
    in_service = batch_size * utilization
    in_queue = L - in_service
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(15 < L < 16, f"L should be ~15.5, got {L:.2f}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    batch_size_str = fmt(batch_size, precision=0, commas=False)
    expected_total_str = fmt(expected_total_ms, precision=1, commas=False)
    expected_total_s_str = fmt(expected_total_s, precision=3, commas=False)
    utilization_str = fmt(utilization, precision=3, commas=False)
    l_str = fmt(L, precision=2, commas=False)
...
2. **Batch size 8 achieves stability**\index{Batch Size!8 achieves stability}: At `{python} GPT3ServingResults.b8_util_pct_str` percent utilization, the system is stable but queuing delays contribute significantly to latency (`{python} GPT3ServingResults.b8_wait_ms_str` ms), as @fig-queuing-hockey-stick illustrates.

::: {#fig-queuing-hockey-stick fig-env="figure" fig-pos="htb" fig-cap="**The Queuing Hockey Stick**: Relationship between system utilization ρ and queue length (M/M/1: L = ρ/(1−ρ)). Three zones are shaded: a Safe Zone (ρ < 0.7), a Caution band (0.7--0.85), and a Danger Zone (ρ > 0.85) where queue depth grows unboundedly. Production systems typically target ρ ≤ 0.7 to maintain latency headroom." fig-alt="Plot of Queue Length (requests) vs. Utilization rho from 0 to 1. The curve is flat until rho around 0.7, bends upward through a caution band (0.7 to 0.85), then rises sharply in a Danger Zone toward saturation at rho=1."}
![](images/svg/queuing-hockey-stick.svg)
:::

3. **Batch sizes 8--16 form the latency knee**\index{Batch Size!latency knee}: Batch size 8 gives the lowest mean latency in this model. Batch size 16 trades a few milliseconds of additional total latency for lower utilization and queue wait time (`{python} GPT3ServingResults.b16_wait_ms_str` ms vs. `{python} GPT3ServingResults.b8_wait_ms_str` ms), while B=32 leaves still more utilization headroom but pays too much batch-accumulation delay.
...
**Verification using Little's Law**\index{Little's Law!verification using}: $N_{\text{req}} = \lambda \cdot T_{\text{lat}}$

At B=16 with $\lambda = 100$ req/s and $E[T] =$ `{python} LittlesLawVerify.expected_total_str` ms:

$L = 100 \times$ `{python} LittlesLawVerify.expected_total_s_str` $=$ `{python} LittlesLawVerify.l_str` requests in system

With batch size `{python} LittlesLawVerify.batch_size_str` and utilization `{python} GPT3ServingResults.b16_util_pct_str` percent:
...
- Expected requests in queue: `{python} LittlesLawVerify.l_str` - `{python} LittlesLawVerify.in_service_str` = `{python} LittlesLawVerify.in_queue_str`
...
#### Decision framework: Batch size selection given SLA {#sec-inference-scale-decision-framework-batch-size-selection-given-sla-8e4e}
...
Find minimum batch size $B_{\text{min}}$ such that $\mu_{\text{eff}}(B_{\text{min}}) > \lambda$:

$$B_{\text{min}} = \frac{\alpha \lambda}{1 - \beta \lambda}$$
...
#### Trade-off curves: Visualizing the operating region {#sec-inference-scale-tradeoff-curves-visualizing-operating-region-9f95}
...
::: {#fig-batch-tradeoff-curves fig-env="figure" fig-pos="htb" fig-cap="**Batch Size Trade-Off Curves**: As batch size grows on the x-axis, throughput (left y-axis, blue) grows sublinearly while latency P50 (right y-axis, red) grows near-linearly. An optimal point is marked at BS=16; a shaded OOM/memory-capacity region flags batch sizes that exceed device memory. Benchmark: Llama-2 70B in FP16 tensor-parallel across four A100 80 GB GPUs." fig-alt="Chart with Batch Size on the x-axis (1 to 128). Left y-axis Throughput in tokens/s (blue, sublinear); right y-axis Latency P50 in ms (red, near-linear). Green dashed Optimal BS=16 marker; shaded OOM region on the right."}
...
Dynamic batching collects requests for a maximum time window $T_{\text{window}}$ or until reaching maximum batch size $B_{\text{max}}$, whichever occurs first. The expected latency under Poisson arrivals with rate $\lambda$ follows @eq-dynamic-batch-latency:
...
- **Per-image inference time**\index{Per-Image Inference Time}: 5 ms at batch=1, 25 ms at batch=32
- **Number of replicas**\index{Number of Replicas}: 10 (each handling 500 QPS)
...
Expected requests per window: $E[B] = \lambda \times T_{\text{window}} = 500 \times 0.01 = 5$
...
**Tradeoff**\index{Tradeoff}: Option C achieves about 33 percent higher per-replica capacity, or 25 percent lower utilization, at the cost of higher average latency (22 ms vs. 15 ms). Both meet the 50 ms P99 SLO.
...
1.  **Significance (quantitative)**: It maximizes the system throughput $(\eta_{\text{hw}})$ by eliminating the padding waste and head-of-line blocking inherent in static batching. It ensures the GPU remains saturated even when requests have widely varying sequence lengths.
2.  **Distinction (durable)**: Unlike static or dynamic batching (which group requests at the *request* level), continuous batching operates at the **Iteration Level**\index{Iteration Level}, dynamically reshaping the compute tensor at each clock cycle.
...
::: {#lhs-inference-archetype-gpt-4-llama-3-throughput-latency .callout-lighthouse title="Archetype A (GPT-4/Llama-3): Throughput-latency"}
...
: **Continuous Batching Throughput on Llama-2 70B (8x A100)**: Tokens-per-second and GPU utilization for static, dynamic, and continuous batching of a Llama-2 70B serving workload on an 8x A100 node. Continuous batching reaches 1,200 tokens/s at 92 percent GPU utilization, roughly 3$\times$ the static baseline of 400 tokens/s at 45 percent, by inserting new sequences at every iteration boundary instead of waiting for the longest in-flight sequence to finish. {#tbl-inference-continuous-batching-throughput}

As @tbl-inference-continuous-batching-throughput shows, the 3$\times$ throughput improvement from continuous batching comes from eliminating idle GPU cycles during sequence length variation.

:::

## The Logic Wall: Test-Time Compute Scaling {#sec-inference-logic-wall}

As models transition from "Fast Thinking" (instant pattern matching) to "Slow Thinking" (deliberative reasoning), the bottleneck shifts from HBM Bandwidth to **Test-Time Compute**\index{Test-Time Compute} [@wei2022emergent; @schaeffer2023mirage]. This is the **Logic Wall**\index{Logic Wall}: the realization that for complex problems, the fleet must scale compute *per request* proportional to the difficulty of the task, often through search or "Chain-of-Thought" (CoT) unrolling.
...
class LogicScaling:
    ...
    # ┌── 2. EXECUTE ───────────────────────────────────────
    t_reasoning_s = (t_std_ms * reasoning_steps * ms).m_as(second)
    latency_expansion = reasoning_steps
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(latency_expansion == 128, f"Exp {latency_expansion} unexpected")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    steps_str = fmt(reasoning_steps, precision=0, commas=False)
    time_s_str = fmt(t_reasoning_s, precision=1, commas=False)
...
::: {#nbk-inference-napkin-math-scaling-reasoning-depth .callout-notebook title="Napkin math: Scaling reasoning depth"}
...
1. **Standard Response**\index{Standard Response}: 1 token answer = 100 ms.
2. **Reasoning Response**\index{Reasoning Response}: `{python} LogicScaling.steps_str` tokens of internal search/CoT before the answer.
...
$$C_{\text{traditional}} = n \times L_{\text{max}} \times c_{\text{decode}}$$

where $L_{\text{max}} = \max_i(L_i)$ and $c_{\text{decode}}$ is the compute cost per decode iteration per sequence. However, the useful compute is only:

$$C_{\text{useful}} = \sum_{i=1}^{n} L_i \times c_{\text{decode}}$$

The **waste ratio** quantifies the inefficiency:

$$W = 1 - \frac{C_{\text{useful}}}{C_{\text{traditional}}} = 1 - \frac{\sum_{i=1}^{n} L_i}{n \times L_{\text{max}}} = 1 - \frac{\bar{L}}{L_{\text{max}}}$$ {#eq-waste-ratio}

where $\bar{L}$ is the mean output length. This reveals that waste depends entirely on the ratio of mean to maximum output length within the batch. For uniform output lengths $(\bar{L} = L_{\text{max}})$, waste is zero. For highly variable lengths, waste can exceed 50 percent.
...
# │ Show: trad_lat_str="4,000" ms, cont_avg_lat_str="2,500" ms,
...
# │   BatchingWorkedExample.total_batch_time_str,
# │   BatchingWorkedExample.total_seq_iters_str,
# │   BatchingWorkedExample.useful_seq_iters_str,
# │   BatchingWorkedExample.r1_lat_str, BatchingWorkedExample.r2_lat_str,
# │   BatchingWorkedExample.r3_lat_str, BatchingWorkedExample.r4_lat_str,
# │   BatchingWorkedExample.trad_lat_str, BatchingWorkedExample.cont_avg_lat_str,
# │   BatchingWorkedExample.latency_red_str, BatchingWorkedExample.r1_imp_str,
# │   BatchingWorkedExample.r3_imp_str, BatchingWorkedExample.r4_imp_str
# └─────────────────────────────────────────────────────────────────────────────
...
class BatchingWorkedExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mean_output_str = fmt(mean_output, precision=0, commas=False)
    total_batch_time_str = fmt(total_batch_time_ms, precision=0)
    total_seq_iters_str = fmt(total_seq_iters, precision=0)
    useful_seq_iters_str = fmt(useful_seq_iters, precision=0)
    r1_lat_str = fmt(r1_latency_ms, precision=0)
    r2_lat_str = fmt(r2_latency_ms, precision=0)
    r3_lat_str = fmt(r3_latency_ms, precision=0)
    r4_lat_str = fmt(r4_latency_ms, precision=0)
    trad_lat_str = fmt(trad_latency_ms, precision=0)
    ...
    cont_avg_lat_str = fmt(cont_avg_latency_ms, precision=0)
    latency_red_str = fmt(latency_reduction_pct, precision=1, commas=False)
    r1_imp_str = fmt(r1_improvement, precision=0, commas=False)
    r3_imp_str = fmt(r3_improvement, precision=0, commas=False)
    r4_imp_str = fmt(r4_improvement, precision=2, commas=False)
```
...
- Mean output length: (50 + 200 + 100 + 150) / 4 = `{python} BatchingWorkedExample.mean_output_str` tokens
...
- Total batch time: $200 \times 20$ ms = `{python} BatchingWorkedExample.total_batch_time_str` ms
- Request completion times:
  - R1 completes useful work at iteration 50, but waits until iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str` ms
  - R2 completes at iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str` ms
  - R3 completes useful work at iteration 100, but waits until iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str` ms
  - R4 completes useful work at iteration 150, but waits until iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str` ms
...
- R1: $50 \times 20$ ms = `{python} BatchingWorkedExample.r1_lat_str` ms (`{python} BatchingWorkedExample.r1_imp_str`$\times$ improvement over traditional)
- R3: $100 \times 20$ ms = `{python} BatchingWorkedExample.r3_lat_str` ms (`{python} BatchingWorkedExample.r3_imp_str`$\times$ improvement)
- R4: $150 \times 20$ ms = `{python} BatchingWorkedExample.r4_lat_str` ms (`{python} BatchingWorkedExample.r4_imp_str`$\times$ improvement)
- R2: $200 \times 20$ ms = `{python} BatchingWorkedExample.r2_lat_str` ms (no improvement for longest request)

Average latency comparison

- Traditional: `{python} BatchingWorkedExample.trad_lat_str` ms (all requests)
- Continuous: (`{python} BatchingWorkedExample.r1_lat_str` + `{python} BatchingWorkedExample.r3_lat_str` + `{python} BatchingWorkedExample.r4_lat_str` + `{python} BatchingWorkedExample.r2_lat_str`) / 4 = `{python} BatchingWorkedExample.cont_avg_lat_str` ms

The result, depicted in @fig-continuous-batching, is a **`{python} BatchingWorkedExample.latency_red_str` percent reduction in average latency**, exactly matching the waste ratio.
...
The analysis above reveals that continuous batching's benefit scales with output length variance. A useful upper-bound intuition comes from comparing the longest sequence in a traditional batch with the average sequence length:

Let $\text{CV} = \sigma / \mu$ be the coefficient of variation of output lengths. If the longest request in a batch is roughly $k$ standard deviations above the mean, then:

$$\text{Improvement} \approx \frac{L_{\text{max}}}{\bar{L}} = \frac{\mu + k\sigma}{\mu} = 1 + k \cdot \text{CV}$$
...
class ContinuousBatchingBenefit:
    ...
    code_waste_pct_str = fmt(code_waste_pct, precision=0, commas=False)
    code_speedup_str = fmt(code_speedup, precision=1, commas=False)

    chat_cv_str = fmt(chat_cv, precision=1, commas=False)
    chat_k_str = fmt(chat_k, precision=1, commas=False)
    chat_waste_pct_str = fmt(chat_waste_pct, precision=0, commas=False)
    chat_speedup_str = fmt(chat_speedup, precision=1, commas=False)

    text_cv_str = fmt(text_cv, precision=1, commas=False)
    text_k_str = fmt(text_k, precision=1, commas=False)
    text_waste_pct_str = fmt(text_waste_pct, precision=0, commas=False)
    text_speedup_str = fmt(text_speedup, precision=1, commas=False)

    creative_cv_str = fmt(creative_cv, precision=1, commas=False)
    creative_k_str = fmt(creative_k, precision=1, commas=False)
    creative_waste_pct_str = fmt(creative_waste_pct, precision=0, commas=False)
    creative_speedup_str = fmt(creative_speedup, precision=1, commas=False)

    rag_cv_str = fmt(rag_cv, precision=1, commas=False)
    rag_k_str = fmt(rag_k, precision=1, commas=False)
    rag_waste_pct_str = fmt(rag_waste_pct, precision=0, commas=False)
    rag_speedup_str = fmt(rag_speedup, precision=1, commas=False)
```

| **Workload Type**           |                                               **CV** |                                               **k** |                                                   **Waste (Trad.)** |                                          **Speedup (Continuous)** |
|:----------------------------|-----------------------------------------------------:|----------------------------------------------------:|--------------------------------------------------------------------:|------------------------------------------------------------------:|
| **Code completion**         |     `{python} ContinuousBatchingBenefit.code_cv_str` |     `{python} ContinuousBatchingBenefit.code_k_str` |     `{python} ContinuousBatchingBenefit.code_waste_pct_str` percent |     `{python} ContinuousBatchingBenefit.code_speedup_str`$\times$ |
| **Chat (short responses)**  |     `{python} ContinuousBatchingBenefit.chat_cv_str` |     `{python} ContinuousBatchingBenefit.chat_k_str` |     `{python} ContinuousBatchingBenefit.chat_waste_pct_str` percent |     `{python} ContinuousBatchingBenefit.chat_speedup_str`$\times$ |
| **General text generation** |     `{python} ContinuousBatchingBenefit.text_cv_str` |     `{python} ContinuousBatchingBenefit.text_k_str` |     `{python} ContinuousBatchingBenefit.text_waste_pct_str` percent |     `{python} ContinuousBatchingBenefit.text_speedup_str`$\times$ |
| **Creative writing**        | `{python} ContinuousBatchingBenefit.creative_cv_str` | `{python} ContinuousBatchingBenefit.creative_k_str` | `{python} ContinuousBatchingBenefit.creative_waste_pct_str` percent | `{python} ContinuousBatchingBenefit.creative_speedup_str`$\times$ |
| **RAG with variable docs**  |      `{python} ContinuousBatchingBenefit.rag_cv_str` |      `{python} ContinuousBatchingBenefit.rag_k_str` |      `{python} ContinuousBatchingBenefit.rag_waste_pct_str` percent |      `{python} ContinuousBatchingBenefit.rag_speedup_str`$\times$ |
...
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (A100_MEM_CAPACITY, A100_MEM_BW, NVLINK_A100_BW,
# │          PCIE_GEN4_BW, GB, GiB, TB, second)
# │ Exports: A100HardwareScenario.a100_mem_gb_str,
# │          A100HardwareScenario.a100_mem_bw_tbs_str,
# │          A100HardwareScenario.nvlink_a100_gbs_str,
# │          A100HardwareScenario.pcie_gen4_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import A100_MEM_CAPACITY, A100_MEM_BW, NVLINK_A100_BW, PCIE_GEN4_BW, GB, GiB, TB, second
from mlsysim.fmt import fmt
...
class A100HardwareScenario:
    ...
    pcie = PCIE_GEN4_BW
    activation_transfer_mb = 100

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    a100_mem_gb_val = mem.m_as(GiB)
    a100_mem_bw_tbs_val = bw.m_as(TB/second)
    nvlink_a100_gbs_val = nvlink.m_as(GB/second)
    pcie_gen4_gbs_val = pcie.m_as(GB/second)
    activation_transfer_gb = activation_transfer_mb / 1000
    pcie_transfer_ms_val = (activation_transfer_gb / pcie_gen4_gbs_val) * 1000
    nvlink_transfer_ms_val = (activation_transfer_gb / nvlink_a100_gbs_val) * 1000

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check
    check(nvlink_a100_gbs_val > pcie_gen4_gbs_val, "NVLink should be faster than PCIe")
    check(pcie_transfer_ms_val > nvlink_transfer_ms_val, "PCIe transfer should be slower than NVLink")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_gb_str = fmt(a100_mem_gb_val, precision=0, commas=False)
    a100_mem_bw_tbs_str = fmt(a100_mem_bw_tbs_val, precision=0, commas=False)
    nvlink_a100_gbs_str = fmt(nvlink_a100_gbs_val, precision=0, commas=False)
    pcie_gen4_gbs_str = fmt(pcie_gen4_gbs_val, precision=0, commas=False)
    activation_transfer_mb_str = fmt(activation_transfer_mb, precision=0, commas=False)
    pcie_transfer_ms_str = fmt(pcie_transfer_ms_val, precision=1, commas=False)
    nvlink_transfer_ms_str = fmt(nvlink_transfer_ms_val, precision=2, commas=False)
```

::: {.callout-tip title="Debugging high P99 latency"}
...
The system runs on 4 A100-`{python} A100HardwareScenario.a100_mem_gb_str` GB GPUs connected via PCIe Gen4 (`{python} A100HardwareScenario.pcie_gen4_gbs_str` GB/s per GPU) rather than NVLink. The server has a dual-socket CPU with NUMA topology. Memory bandwidth per GPU is `{python} A100HardwareScenario.a100_mem_bw_tbs_str` TB/s (HBM2e), adequate for decode operations. However, PCIe bandwidth limits tensor parallelism communication to `{python} A100HardwareScenario.pcie_gen4_gbs_str` GB/s vs. NVLink's `{python} A100HardwareScenario.nvlink_a100_gbs_str` GB/s. For a batch requiring `{python} A100HardwareScenario.activation_transfer_mb_str` MB activation transfers between GPUs, raw bandwidth math gives approximately `{python} A100HardwareScenario.pcie_transfer_ms_str` ms per synchronization point over PCIe vs. `{python} A100HardwareScenario.nvlink_transfer_ms_str` ms over NVLink.

**Distribution Layer Analysis** (Algorithmic Behavior):

Dynamic request-level batching is configured with max batch size 32 and timeout 50 ms. Examining the batch size distribution reveals the problem: 90 percent of batches contain 4 to 8 requests (explaining good P50/P95), but 5 percent of batches reach the full 32 requests. These large batches occur during traffic bursts and experience head-of-line blocking\index{Head-of-Line Blocking}: short requests that would complete quickly must wait for long-sequence requests in the same batch to finish all their decode iterations.
...
**Lesson**\index{Lesson}: When tail latency exceeds expectations despite healthy utilization, examine the batching and scheduling policies. High average GPU utilization can mask head-of-line blocking that only affects a small percentage of requests but drives P99. The fleet stack methodology systematically isolates whether bottlenecks arise from hardware constraints or algorithmic decisions; @fig-fleet-stack gives the complete framework.
...
Recommendation systems have distinct batching requirements from vision or language models. As @fig-feature-parallel-pipeline shows, the computation pattern involves:
...
**Request characteristics**\index{Request Characteristics}:
...
**Requests per batch**\index{Requests Per Batch}: 10,000 (at 10M QPS)
**Lookups per shard per batch**\index{Lookups Per Shard Per Batch}: 50K
...
: **Latency Breakdown for a 10M-QPS Recommender Request**: Per-phase contribution to end-to-end latency for a single recommender request at 10M QPS with 1,000 embedding shards. Request routing, batch accumulation, embedding lookup, feature processing, and ranking-model scoring sum to 5.2 ms, leaving roughly half of the 10 ms SLO as headroom. Embedding lookup is the largest single phase at 2 ms, reflecting the cost of fanning 50K lookups per shard out to SSD-backed memory. {#tbl-inference-recsys-latency-breakdown}
...
For streaming applications, the relevant metric is not throughput but **time to process each input**\index{Time to Process Each Input}:
...
: **Streaming Speech Recognition Pipeline Latency**: Per-stage latency for a streaming speech-to-text pipeline operating on 20 ms audio frames against a 100 ms end-to-end budget. Network transfer, feature extraction, encoder inference, decoder step, text formatting, and response transmission sum to 90 ms, leaving 10 ms of jitter headroom. Encoder inference at 30 ms is the largest single stage, which is why streaming systems trade GPU utilization (30--50 percent) for the latency guarantee. {#tbl-inference-streaming-speech-pipeline}

**Key constraints**\index{Key Constraints}:
...
::: {#exmp-inference-adaptive-batching-triton .callout-example title="Adaptive batching: Triton"}
...
: **Triton Adaptive Batching: ResNet-50 on V100**: Observed batch size, average latency, and sustained throughput for the Triton inference server's adaptive batcher under increasing traffic on ResNet-50 (V100). Average batch size scales from 2.1 at 100 QPS to 24.1 at 2,000 QPS, allowing throughput to track offered load while latency only grows from 8 ms to 28 ms. The system reaches saturation near 2,000 QPS, where measured throughput (1,980 QPS) begins to fall below offered load. {#tbl-inference-triton-adaptive-batching}

As @tbl-inference-triton-adaptive-batching shows, the system automatically increases batch size to maintain throughput as traffic grows, with measured throughput tracking offered load until saturation near 2,000 QPS.
...
The choice of batching strategy depends on model characteristics, traffic patterns, and latency requirements. @fig-inference-lifecycle visualizes the end-to-end request path, highlighting latency sources at each stage.
...
- [ ] For an LLM serving chat requests, why does the "Waste Ratio" ($1 - \bar{L}/L_{\text{max}}$) collapse to zero when moving from static to continuous batching?
...
### KV cache fundamentals {#sec-inference-scale-kv-cache-fundamentals-2ccd}
...
1.  **Significance (quantitative)**: It reduces per-token computation from **$O(t^2)$** to **$O(t)$**, making generation feasible for long sequences. However, it grows linearly with sequence length and batch size, often exceeding the memory footprint of the model weights and becoming the primary constraint on **Concurrent Capacity**.
...
3.  **Common pitfall**: A frequent misconception is that the KV Cache is "fixed-size." In reality, it is **Dynamic and Fragmented**\index{Dynamic and Fragmented}: because different requests have different lengths, the cache can cause massive memory waste (up to 60--80 percent) due to internal fragmentation if not managed by a virtual memory system.
...
# │ How: KV = 2*80*8*128*2 bytes/token; gb_per_1k_tokens; viz.set_book_style().
# │
# │ Imports: numpy (np), matplotlib.pyplot (plt), mlsysim.core.viz (viz)
# │ Exports: (figure only, no prose variables)
# └─────────────────────────────────────────────────────────────────────────────
...
# │          KVCacheCapacityEstimator.step2_cache_per_request_math,
# │          KVCacheCapacityEstimator.step3_max_batch_math
# └─────────────────────────────────────────────────────────────────────────────
...
class KVCacheCapacityEstimator:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    kv_bytes_per_token = 2 * n_layers * n_kv_heads * d_head * bytes_per_elem
    kv_mb_per_token = kv_bytes_per_token / 1e6
    kv_gb_per_request = context_tokens * kv_bytes_per_token / 1e9
    available_kv_gb = total_hbm_gb - weight_gb - system_gb
    max_batch = int(available_kv_gb // kv_gb_per_request)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    kv_bytes_per_token_str = fmt(kv_bytes_per_token, precision=0)
    kv_mb_per_token_str = fmt(kv_mb_per_token, precision=2, commas=False)
    context_tokens_str = fmt(context_tokens, precision=0)
    kv_gb_per_request_str = fmt(kv_gb_per_request, precision=0, commas=False)
    available_kv_gb_str = fmt(available_kv_gb, precision=0, commas=False)
    ...
    step1_kv_per_token_math = MarkdownStr(
        ...
        f"\\mathbf{{{kv_mb_per_token:.2f} \\text{{ MB/token}}}} $$"
    )
    step2_cache_per_request_math = MarkdownStr(
        f"$$ {context_tokens:,} \\text{{ tokens}} \\times {kv_mb_per_token:.2f} \\text{{ MB/token}} \\approx "
        f"\\mathbf{{{kv_gb_per_request:.0f} \\text{{ GB/request}}}} $$"
    )
    step3_max_batch_math = MarkdownStr(
        f"$$ \\text{{Max Batch}} = \\left\\lfloor \\frac{{{available_kv_gb:.0f}}}{{{kv_gb_per_request:.0f}}} \\right\\rfloor "
        f"= \\mathbf{{{max_batch}}} $$"
    )
```
...
**Problem**: A deployment serves Llama-3-70B (FP16 weights $\approx 140$ GB) on an 8$\times$ H100 node (640 GB total HBM). The goal is to determine the maximum batch size for a context length of 128K tokens.

**Formula**\index{Formula}:
$$ M_{\text{KV}} = 2 \times n_{\text{layers}} \times n_{\text{heads}} \times d_{\text{head}} \times s_{\text{elem}} $$
...
***PagedAttention***\index{PagedAttention!block table}\index{PagedAttention!definition} is a memory management technique that applies virtual memory principles to **KV Cache**\index{KV Cache!quantization} allocation, storing attention states in noncontiguous, fixed-size physical blocks.

1.  **Significance (quantitative)**: It eliminates **Internal and External Fragmentation**\index{Internal and External Fragmentation}, which can waste up to 80 percent of GPU memory. By allowing sequences to grow dynamically, it enables 2--4$\times$ higher **Concurrent Throughput** $(\eta_{\text{hw}})$ on the same hardware.
...
**Performance impact**\index{Performance Impact}:
...
: **PagedAttention vs. Contiguous KV Cache**: KV cache pool utilization and relative serving throughput for contiguous allocation against PagedAttention. PagedAttention raises pool utilization from 30--40 percent to over 95 percent by allocating fixed-size physical blocks and indirecting through a per-sequence page table, which translates into a 2.5--4$\times$ throughput improvement because the same memory now fits proportionally more concurrent sequences. {#tbl-inference-pagedattention-throughput}

As @tbl-inference-pagedattention-throughput shows, the 2.5--4$\times$ throughput improvement comes from fitting more concurrent sequences in the same memory.
...
All three requests reference the same physical blocks for the system prompt. Only when generating unique tokens do they allocate new blocks. The savings can be substantial when many concurrent requests share the same system prompt.
...
# │   (with), memory_reduction_pct_str="80"%, concurrent_multiplier_str="5"—
# │   inline in the "Prefix Caching at Scale" callout.
# │ How: Multiply token count × KV pairs × layers × hidden_dim × bytes/element,
...
# │   PrefixCachingAnalysis.concurrent_multiplier_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class PrefixCachingAnalysis:
    ...
    n_layers = 80       # large LLM (e.g., 70B-class)
    head_dim = 8192     # hidden dimension
    ...
    kv_bytes_no_cache = total_tokens_no_cache * n_kv_pairs * n_layers * head_dim * bytes_per_element
    kv_tb_no_cache_val = kv_bytes_no_cache / 1e12
    ...
    kv_bytes_cached = total_tokens_cached * n_kv_pairs * n_layers * head_dim * bytes_per_element
    kv_tb_cached_val = kv_bytes_cached / 1e12

    # Step 3: Savings
    memory_reduction_pct_val = (1 - kv_tb_cached_val/kv_tb_no_cache_val) * 100
    concurrent_multiplier_val = kv_tb_no_cache_val/kv_tb_cached_val

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(kv_tb_cached_val < kv_tb_no_cache_val, "Cached must be smaller than uncached")
    check(memory_reduction_pct_val > 70, "Prefix caching should give substantial savings (>70%)")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tokens_per_user_str = fmt(tokens_per_user, precision=0)
    kv_tb_no_cache_str = fmt(kv_tb_no_cache_val, precision=1, commas=False)
    total_tokens_cached_str = fmt(total_tokens_cached, precision=0)
    kv_tb_cached_str = fmt(kv_tb_cached_val, precision=1, commas=False)
    memory_reduction_pct_str = fmt(memory_reduction_pct_val, precision=0, commas=False)
    concurrent_multiplier_str = fmt(concurrent_multiplier_val, precision=0, commas=False)
```
...
- KV cache per user: 2000 + 500 (avg response) = `{python} PrefixCachingAnalysis.tokens_per_user_str` tokens
- Total KV cache: `{python} PrefixCachingAnalysis.tokens_per_user_str`$\times$ 1000$\times$ 2$\times$ 80$\times$ $8192 \times 2$ = `{python} PrefixCachingAnalysis.kv_tb_no_cache_str` TB

**With prefix caching**\index{With Prefix Caching}:

- Shared prefix: 2000 tokens (once)
- Unique per user: 500 tokens
- Total: $(2000 \times 1) + (500 \times 1000)$ = `{python} PrefixCachingAnalysis.total_tokens_cached_str` tokens
- Memory: `{python} PrefixCachingAnalysis.total_tokens_cached_str`$\times$ 2$\times$ 80$\times$ $8192 \times 2$ = `{python} PrefixCachingAnalysis.kv_tb_cached_str` TB

**Savings**\index{Savings}: `{python} PrefixCachingAnalysis.memory_reduction_pct_str` percent reduction in KV cache memory, enabling `{python} PrefixCachingAnalysis.concurrent_multiplier_str`$\times$ more concurrent users.

**Prefix hit rate** determines effectiveness. @tbl-inference-prefix-caching-workloads contrasts hit rates and memory savings across workloads:
...
# │          KVCacheCalc.kv_savings_gb_str, KVCacheCalc.kv_compression_str,
# │          KVCacheCalc.batch8_kv_fp16_gb_str, KVCacheCalc.gqa_kv_fp16_gb_str
# └─────────────────────────────────────────────────────────────────────────────

# ┌── LEGO ───────────────────────────────────────────────
from mlsysim.core.constants import H100_MEM_CAPACITY, GiB, GB, BYTES_FP16, BYTES_INT4, byte
class KVCacheCalc:
    """KV cache memory footprint: FP16 vs INT4 for a 70B MHA model."""

    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    kv_layers = 80
    kv_heads = 64  # MHA baseline; GQA (8 heads) is introduced as the reduction below
    kv_head_dim = 128
    kv_seq_len = 4096
    kv_bytes_fp16 = BYTES_FP16.m_as(byte)
    kv_bytes_int4 = BYTES_INT4.m_as(byte)
    ...
    kv_fp16_gb = (kv_fp16_bytes * byte).m_as(GB)
    kv_int4_gb = (kv_int4_bytes * byte).m_as(GB)
    kv_savings_gb = kv_fp16_gb - kv_int4_gb
    kv_compression = kv_fp16_gb/kv_int4_gb
    batch8_kv_fp16_gb = kv_fp16_gb * 8
    gqa_kv_fp16_gb = kv_fp16_gb * (8 / kv_heads)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, MarkdownStr, fmt
    check(kv_fp16_gb > 5, f"KV cache should be substantial, got {kv_fp16_gb:.1f} GB")
    check(abs(gqa_kv_fp16_gb - (kv_fp16_gb / 8)) < 1e-6, "GQA cache should be 8x smaller than MHA baseline")
    ...
    kv_fp16_gb_str = fmt(kv_fp16_gb, precision=1, commas=False)
    kv_int4_gb_str = fmt(kv_int4_gb, precision=1, commas=False)
    kv_savings_gb_str = fmt(kv_savings_gb, precision=1, commas=False)
    kv_compression_str = fmt(kv_compression, precision=0, commas=False)
    h100_mem_gb_str = fmt(h100_mem.m_as(GiB), precision=0, commas=False)
    batch8_kv_fp16_gb_str = fmt(batch8_kv_fp16_gb, precision=0, commas=False)
    gqa_kv_fp16_gb_str = fmt(gqa_kv_fp16_gb, precision=1, commas=False)
```

$$
\text{KV cache} = 2 \times 80 \times 64 \times 128 \times 4096 \times 2 \text{ bytes} \approx \text{`{python} KVCacheCalc.kv_fp16_gb_str`} \text{ GB (FP16)}
$$

A single request's KV cache alone consumes a substantial fraction of the H100's `{python} KVCacheCalc.h100_mem_gb_str` GB of HBM. For a batch of 8 concurrent requests, the KV cache would require `{python} KVCacheCalc.batch8_kv_fp16_gb_str` GB, exceeding single-GPU capacity. Reducing this footprint requires two complementary strategies: quantization and architectural optimization.
...
Compressing the KV cache to INT4 reduces it to approximately `{python} KVCacheCalc.kv_int4_gb_str` GB per request, a `{python} KVCacheCalc.kv_compression_str`$\times$ reduction. This freed memory directly translates into larger batch sizes and higher serving throughput. KV cache values exhibit different distributions than model weights: KIVI observes channel-wise outliers in keys and uses per-channel key quantization, while values lack the same channel-wise pattern and are quantized per-token.
...
$$
\text{KV cache (GQA)} = 2 \times 80 \times 8 \times 128 \times 4096 \times 2 \approx \text{`{python} KVCacheCalc.gqa_kv_fp16_gb_str`} \text{ GB (FP16)}
$$

At `{python} KVCacheCalc.gqa_kv_fp16_gb_str` GB, the GQA cache is dramatically more manageable than the `{python} KVCacheCalc.kv_fp16_gb_str` GB required by MHA. Combining GQA with INT8 KV cache quantization yields sub-gigabyte per-request cache sizes, often enabling substantially larger batches on a single GPU. GQA has become a common architectural choice for inference-optimized LLMs because it reduces KV-cache bandwidth and capacity pressure with comparatively small quality impact in many deployments.

```{python}
#| label: precision-dividend
#| echo: false
from mlsysim.fmt import check, fmt
from mlsysim.core.constants import H100_MEM_CAPACITY, GiB

class PrecisionDividend:
    # ┌── 1. LOAD ──────────────────────────────────────────
    # H100 HBM3 is stored as 80 * GiB in mlsysim (binary stacks); NVIDIA
    # markets it as "80 GB" — book convention uses the vendor's GB label,
    # so .m_as(GiB) gives the 80 that prose anchors to.
    h100_mem_gb = H100_MEM_CAPACITY.m_as(GiB)
    fp16_weight_per_gpu = 35
    int4_bits = 4
    fp16_bits = 16
    # ┌── 2. EXECUTE ───────────────────────────────────────
    avail_kv_fp16 = h100_mem_gb - fp16_weight_per_gpu
    int4_weight_per_gpu = fp16_weight_per_gpu * (int4_bits/fp16_bits)
    avail_kv_int4 = h100_mem_gb - int4_weight_per_gpu
    kv_fp16_per_gpu = KVCacheCalc.kv_fp16_gb / 4
    kv_int8_per_gpu = (KVCacheCalc.kv_fp16_gb / 2) / 4
    max_batch_fp16 = int(avail_kv_fp16 / kv_fp16_per_gpu)
    max_batch_int8 = int(avail_kv_int4 / kv_int8_per_gpu)
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(avail_kv_fp16 == 45, f"Expected 45, got {avail_kv_fp16}")
    check(abs(int4_weight_per_gpu - 8.75) < 0.01, f"Expected 8.75, got {int4_weight_per_gpu}")
    check(max_batch_int8 > max_batch_fp16, "INT4 weights plus INT8 KV should increase max batch")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    # Keep raw `fp16_weight_per_gpu` int for arithmetic; add _str alias for prose.
    fp16_weight_per_gpu_str = fmt(fp16_weight_per_gpu, precision=0, commas=False)
    avail_kv_fp16_str = fmt(avail_kv_fp16, precision=0, commas=False)
    int4_weight_per_gpu_str = fmt(int4_weight_per_gpu, precision=2, commas=False)
    avail_kv_int4_str = fmt(avail_kv_int4, precision=2, commas=False)
    kv_fp16_per_gpu_str = fmt(kv_fp16_per_gpu, precision=1, commas=False)
    kv_int8_total_str = fmt(KVCacheCalc.kv_fp16_gb/2, precision=1, commas=False)
    kv_int8_per_gpu_str = fmt(kv_int8_per_gpu, precision=1, commas=False)
    max_batch_fp16_str = fmt(max_batch_fp16, precision=0, commas=False)
    max_batch_int8_str = fmt(max_batch_int8, precision=0, commas=False)
```

::: {#nbk-inference-precision-dividend .callout-notebook title="The precision dividend"}

**Problem**: A team serves a 70B model on 4$\times$ H100 GPUs. Weights in FP16 consume 140 GB (35 GB/GPU). KV cache at FP16 consumes `{python} KVCacheCalc.kv_fp16_gb_str` GB per request. How does quantizing weights to INT4 and KV cache to INT8 change the maximum batch size?

**Before optimization** (all FP16):

- Weights: 35 GB/GPU
- Available for KV cache: $80 -$ `{python} PrecisionDividend.fp16_weight_per_gpu_str` $=$ `{python} PrecisionDividend.avail_kv_fp16_str` GB/GPU
- KV cache per request: `{python} KVCacheCalc.kv_fp16_gb_str` GB $\div$ 4 GPUs $\approx$ `{python} PrecisionDividend.kv_fp16_per_gpu_str` GB/GPU
- Maximum batch size: $\lfloor$ `{python} PrecisionDividend.avail_kv_fp16_str` $/$ `{python} PrecisionDividend.kv_fp16_per_gpu_str` $\rfloor$ $\approx$ `{python} PrecisionDividend.max_batch_fp16_str` requests

**After optimization** (INT4 weights, INT8 KV cache):

- Weights: `{python} PrecisionDividend.fp16_weight_per_gpu_str` GB $\times (4/16) =$ `{python} PrecisionDividend.int4_weight_per_gpu_str` GB/GPU (INT4)
- Available for KV cache: $80 -$ `{python} PrecisionDividend.int4_weight_per_gpu_str` $=$ `{python} PrecisionDividend.avail_kv_int4_str` GB/GPU
- KV cache per request (INT8): `{python} PrecisionDividend.kv_int8_total_str` GB $\div$ 4 $\approx$ `{python} PrecisionDividend.kv_int8_per_gpu_str` GB/GPU
- Maximum batch size: $\lfloor$ `{python} PrecisionDividend.avail_kv_int4_str` $/$ `{python} PrecisionDividend.kv_int8_per_gpu_str` $\rfloor$ $\approx$ `{python} PrecisionDividend.max_batch_int8_str` requests

**Takeaway**\index{Takeaway}: Precision engineering fundamentally changes serving economics by enabling larger batch sizes. Larger batches amortize the fixed cost of weight loading, shifting operations from memory-bound toward compute-bound.
...
***Speculative Decoding***\index{Speculative Decoding!target model}\index{Speculative Decoding!definition} is a latency optimization that uses a smaller **Draft Model** to predict multiple future tokens, which are then verified in parallel by the full **Target Model** in a single forward pass.
...
1.  **Draft phase**\index{Draft Phase}: A small, fast model (the "draft model") generates $K$ candidate tokens autoregressively. Because the draft model is much smaller (for example, 1B parameters vs. 70B for the target), each draft step is fast. Generating $K$ draft tokens typically takes roughly the time of one or two target model decode steps.
2.  **Verification phase**\index{Verification Phase}: The target model processes all $K$ draft tokens *in parallel* in a single forward pass. Because this is a forward pass over $K$ known tokens (not autoregressive generation), the operation is a standard general matrix multiply (GEMM) with much higher arithmetic intensity than batch-1 decode.
3.  **Acceptance phase**\index{Speculative Decoding!acceptance phase}: Starting from the first draft token, the system compares the draft model's probability distribution with the target model's distribution. Continue accepting tokens until a draft token is rejected or all $K$ are accepted. On rejection, sample a corrected token from a modified distribution that accounts for the draft model's error. @fig-speculative-decoding-inf illustrates this three-phase process.
...
The speedup depends on the **acceptance rate** $\alpha$ (probability that the draft token matches the target model's distribution) and the **draft length** $K$. The expected number of emitted tokens per round, including the target model's correction token, is approximately $\sum_{i=0}^{K} \alpha^i = \frac{1 - \alpha^{K+1}}{1 - \alpha}$.
...
# │
# │ Imports: mlsysim.book (check)
# │ Exports: SpeculativeSpeedupAnalysis.sp_high_str,
# │          SpeculativeSpeedupAnalysis.sp_med_str,
# │          SpeculativeSpeedupAnalysis.sp_low_str,
...
class SpeculativeSpeedup:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    sp_high_str = fmt(sp_high, precision=1, commas=False)
    sp_med_str = fmt(sp_med, precision=1, commas=False)
    sp_low_str = fmt(sp_low, precision=1, commas=False)
    et_high_str = fmt(et_high, precision=1, commas=False)
    et_med_str = fmt(et_med, precision=1, commas=False)
    et_low_str = fmt(et_low, precision=1, commas=False)
```

For $K = 5$ draft tokens and a draft model that is 20$\times$ faster than the target:

- **High acceptance** $(\alpha = 0.9)$: Expected `{python} SpeculativeSpeedup.et_high_str` tokens per round, speedup of `{python} SpeculativeSpeedup.sp_high_str`$\times$.
- **Medium acceptance** $(\alpha = 0.7)$: Expected `{python} SpeculativeSpeedup.et_med_str` tokens per round, speedup of `{python} SpeculativeSpeedup.sp_med_str`$\times$.
- **Low acceptance** $(\alpha = 0.5)$: Expected `{python} SpeculativeSpeedup.et_low_str` tokens per round, speedup of `{python} SpeculativeSpeedup.sp_low_str`$\times$.

The acceptance rate varies dramatically by context. Predictable text achieves $\alpha > 0.9$, while creative reasoning or code generation may see $\alpha < 0.5$. In practice, well-aligned model pairs (sharing training data and architecture) commonly achieve average acceptance rates of 0.6--0.8, yielding 1.5--3$\times$ latency improvements.

#### System design and hardware cost {#sec-inference-scale-speculative-hardware}

Deploying speculative decoding requires solving several system-level challenges. **Draft model selection** balances speed against acceptance rate. **Self-speculative decoding**\index{Self-Speculative Decoding} uses early exit from the target model itself as the draft, avoiding the need for a separate model at the cost of lower alignment. Advanced variants like **Medusa** [@cai2024medusa] add lightweight prediction heads to the target model backbone, while **Lookahead decoding** uses Jacobi iteration to avoid draft models entirely.
...
Max concurrent sequences = KV pool size / (avg_seq_length$\times$ per_token_cache)
```
...
@tbl-inference-kv-cache-memory-hierarchy lays out the three-tier hierarchy production systems use to manage KV-cache memory pressure:
...
: **KV Cache Memory Hierarchy**: Capacity, swap-in latency, and intended use case for the three tiers production inference servers use to manage KV-cache memory pressure. GPU HBM (80 GB) holds active sequences at zero swap cost; CPU DRAM (1 TB) absorbs preempted sequences at 1--5 ms; NVMe SSD (10 TB) backs long-term prefix and session caches at 10--50 ms. The roughly 125$\times$ capacity range across the listed tiers is what makes swap-based preemption viable when GPU memory fills. {#tbl-inference-kv-cache-memory-hierarchy}
...
1.  **Significance (quantitative)**: The **Prefill Phase** (processing the prompt) is **Compute-Bound** $(R_{\text{peak}})$ with high arithmetic intensity, while the **Decode Phase** (generating tokens) is **Bandwidth-Bound** $(\text{BW})$\index{Bandwidth-Bound} with extremely low arithmetic intensity. This mismatch means a single request's efficiency $(\eta_{\text{hw}})$ varies wildly during its lifecycle.
...
|:------------|--------------------------------------------------------------------------------:|:---------------------------------------|:---------------|:------------------------------------------|
| **Prefill** |                                                               O(prompt_length²) | Activation and attention tiles         | Compute        | Moderate request batches or large chunks  |
| **Decode**  | O(1) dense weight work plus O(context length) attention over KV cache per token | Weight and KV-cache reads              | Bandwidth      | Large continuous batches of active tokens |
...
- **Prefill pool**\index{Prefill Pool}: Optimized for compute intensity (for example, H100 nodes with high TFLOPS) using moderate request batches or large token chunks to maximize throughput without blocking decode.
...
**Interleaving schedule**\index{Interleaving Schedule}: Each GPU iteration processes either:
...
# │ Show: "80" GB.
# │ How: pulling H100_MEM_CAPACITY from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (H100_MEM_CAPACITY, GiB)
...
The result exceeds the `{python} HardwareSetupSharding.h100_mem_gb_str` GB capacity of an H100 GPU, requiring at minimum 2-way sharding.
...
$$T_{\text{parallel}} = \frac{T_{\text{sequential}}}{N} + T_{\text{communication}}$$ {#eq-parallel-time}
...
# │ Show: tp_speedup_str="6.9"×, seq_total_ms_str="2,400" ms,
...
# │ Exports: TpSpeedupCalc.tp_speedup_str, TpSpeedupCalc.tp_theoretical_str,
# │   TpSpeedupCalc.n_layers_str, TpSpeedupCalc.seq_total_ms_str,
# │   TpSpeedupCalc.tp8_total_ms_str, TpSpeedupCalc.nvlink_h100_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
...
class TpSpeedupCalc:
    ...
    n_layers = 80              # Llama-70B transformer layer count
    ...
    nvlink_h100_gbs_val = nvlink_bw.m_as(GB/second)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tp_speedup_str = fmt(tp_speedup, precision=1, commas=False)
    seq_total_ms_str = fmt(seq_total_ms, precision=0)
    tp8_total_ms_str = fmt(tp8_total_ms, precision=0)
    tp_theoretical_str = fmt(tp_theoretical, precision=0, commas=False)
    n_layers_str = fmt(n_layers, precision=0, commas=False)
    nvlink_h100_gbs_str = fmt(nvlink_h100_gbs_val, precision=0, commas=False)
```
...
**Model specifications**\index{Model Specifications}:
...
**Minimum sharding**: 2-way (140 GB/`{python} HardwareSetupSharding.h100_mem_gb_str` GB per H100)

**Recommended sharding**\index{Recommended Sharding}: 8-way for optimal latency

@tbl-inference-tp8-layer-breakdown decomposes the per-layer cost of 8-way tensor parallelism on 8xH100 (NVLink interconnect) into compute and AllReduce phases:
...
**For `{python} TpSpeedupCalc.n_layers_str` layers on the same prefill workload**:

- Sequential: `{python} TpSpeedupCalc.seq_total_ms_str` ms for the full prefill pass
- 8-way TP: `{python} TpSpeedupCalc.tp8_total_ms_str` ms for the full prefill pass

The `{python} TpSpeedupCalc.tp_speedup_str`$\times$ speedup (vs. theoretical `{python} TpSpeedupCalc.tp_theoretical_str`$\times$) reflects communication overhead. At `{python} TpSpeedupCalc.nvlink_h100_gbs_str` GB/s, moving an 8 MB activation payload would take only about 0.01 ms by raw bandwidth math; the 0.3 ms budget shown here includes collective launch, synchronization, ring protocol, and framework overheads.

**Time-to-first-token** (1024-token prompt):

- Prefill compute: ~`{python} TpSpeedupCalc.tp8_total_ms_str` ms on 8-way TP (compute-bound, near-linear scaling minus communication overhead)
...
# │
# │ Imports: mlsysim.book (check)
# │ Exports: MoEEconomics.dense_mem_str, MoEEconomics.moe_mem_str,
# │          MoEEconomics.compute_ratio_str, MoEEconomics.bw_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
...
class MoEEconomics:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    dense_params_b = 400
    moe_total_params_b = 671
    moe_active_params_b = 37
    ...
    compute_ratio = dense_params_b/moe_active_params_b  # ~10.8x
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dense_mem_str = fmt(dense_mem_fp16, precision=0, commas=False)
    moe_mem_str = fmt(moe_mem_fp16, precision=0, commas=False)
    compute_ratio_str = fmt(compute_ratio, precision=0, commas=False)
    bw_ratio_str = fmt(bw_ratio, precision=0, commas=False)
```

The performance advantages are striking. During autoregressive decode at batch size 1, the dominant cost is reading weights from HBM. A dense 400B model in FP16 reads `{python} MoEEconomics.dense_mem_str` GB per step. DeepSeek-V3, despite having more total parameters, reads only the 37B active parameters per step (approximately 74 GB in FP16), a `{python} MoEEconomics.bw_ratio_str`$\times$ reduction in per-token bandwidth. The compute savings are proportional: `{python} MoEEconomics.compute_ratio_str`$\times$ fewer FLOPs per token.

The trade-off is memory capacity: all experts must reside in memory even though only a fraction are active at any time. DeepSeek-V3's full model in FP16 requires approximately `{python} MoEEconomics.moe_mem_str` GB, necessitating distribution across many GPUs.

```{python}
#| label: moe-capacity-planning
#| echo: false
from mlsysim.core.constants import H100_MEM_BW, H100_MEM_CAPACITY, GB, GiB, TB, second
from mlsysim.fmt import check, fmt
...
class MoECapacityPlan:
    ...
    moe_total_params_b = 671
    moe_active_params_b = 37
    bytes_fp8 = BYTES_INT8.m_as(byte)  # FP8 = 1 byte
    node_gpus = 8
    # H100 HBM3 = 80*GiB in mlsysim (binary stacks); vendor label "80 GB".
    h100_mem_gb = H100_MEM_CAPACITY.m_as(GiB)
    n_gpus = 16
    alltoall_overhead_ms = 0.3
    h100_bw_gbs = H100_MEM_BW.m_as(GB/second)
    h100_bw_tbs = H100_MEM_BW.m_as(TB/second)
    # ┌── 2. EXECUTE ───────────────────────────────────────
    total_weight_gb = moe_total_params_b * bytes_fp8
    node_capacity_gb = node_gpus * h100_mem_gb
    nodes_needed = int((total_weight_gb + node_capacity_gb - 1) // node_capacity_gb)
    cluster_capacity_gb = nodes_needed * node_capacity_gb
    active_mem_gb = moe_active_params_b * bytes_fp8
    read_per_gpu_gb = active_mem_gb/n_gpus
    t_read_ms = (read_per_gpu_gb/h100_bw_gbs) * 1000
    total_decode_ms = t_read_ms + alltoall_overhead_ms
    tokens_per_sec = round(1000 / total_decode_ms, -2)
    # ┌── 3. GUARD ─────────────────────────────────────────
    check(nodes_needed == 2, f"Expected two nodes, got {nodes_needed}")
    check(0.5 < total_decode_ms < 2.0, f"Expected ~1ms, got {total_decode_ms:.2f}")
    # ┌── 4. OUTPUT ────────────────────────────────────────
    read_per_gpu_str = fmt(read_per_gpu_gb, precision=1, commas=False)
    t_read_ms_str = fmt(t_read_ms, precision=2, commas=False)
    total_decode_ms_str = fmt(total_decode_ms, precision=1, commas=False)
    tokens_per_sec_str = fmt(tokens_per_sec, precision=0)
    total_weight_gb_str = fmt(total_weight_gb, precision=0, commas=False)
    node_capacity_gb_str = fmt(node_capacity_gb, precision=0, commas=False)
    nodes_needed_str = fmt(nodes_needed, precision=0, commas=False)
    cluster_capacity_gb_str = fmt(cluster_capacity_gb, precision=0)
    active_mem_gb_str = fmt(active_mem_gb, precision=0, commas=False)
...
**Problem**: A team deploys DeepSeek-V3\index{DeepSeek-V3} (671B total parameters, 37B active per token) for a chatbot application. The model uses FP8 weights (1 byte per parameter). The cluster has 8-GPU nodes, each with 8$\times$ H100 GPUs (80 GB HBM per GPU, 640 GB per node). How many nodes are needed, and what is the per-token decode latency?

**Math**:

1.  **Memory**: Total weight memory = `{python} MoECapacityPlan.total_weight_gb_str` GB. A single node (`{python} MoECapacityPlan.node_capacity_gb_str` GB) is insufficient. `{python} MoECapacityPlan.nodes_needed_str` nodes provide `{python} MoECapacityPlan.cluster_capacity_gb_str` GB, leaving enough room for KV caches.
2.  **Latency**: Each decode step reads 37B active parameters (`{python} MoECapacityPlan.active_mem_gb_str` GB). Distributed across 16 GPUs, each reads about `{python} MoECapacityPlan.read_per_gpu_str` GB. At `{python} MoECapacityPlan.h100_bw_tbs_str` TB/s HBM bandwidth, $t_{\text{read}} \approx$ `{python} MoECapacityPlan.t_read_ms_str` ms. AllToAll routing adds about 0.3 ms.
3.  **Floor**\index{Floor}: Estimated decode latency $\approx$ `{python} MoECapacityPlan.total_decode_ms_str` ms/token, or `{python} MoECapacityPlan.tokens_per_sec_str` tokens/second at batch size 1.

**Takeaway**: MoE allows a 671B model to approach the per-token bandwidth cost of a much smaller dense model, but capacity planning is still driven by the full parameter set that must remain resident.
...
MoE[^fn-moe-serving] models present unique sharding challenges because computation is dynamically routed to different experts based on input. Popular models like Mixtral [@jiang2024mixtral] use MoE to achieve high capacity with lower inference cost.
...
**Model characteristics**\index{Model Characteristics}:
...
**Total MoE layer time**\index{Total MoE Layer Time}: ~1.5 ms (vs. ~4 ms for equivalent dense layer)

@tbl-inference-moe-load-balancing reports how routing imbalance erodes GPU utilization and throughput across the eight experts\index{Load Balancing!metrics}:
...
: **MoE Routing Load Balance vs. Throughput**: GPU utilization and throughput impact as routing imbalance grows across the eight experts of a Mixtral-8x7B MoE layer with 4-way expert parallelism. A 20 percent imbalance erodes utilization to 83 percent (a 17 percent throughput loss); a 50 percent imbalance drops utilization to 67 percent (a 33 percent loss), which is why production systems monitor routing statistics and may retrain or fine-tune the gating network to restore balance. {#tbl-inference-moe-load-balancing}
...
Recommendation systems typically contain embedding tables that dwarf model weights in size. Meta's DLRM-scale models [@naumov2019deep] have embedding tables exceeding 100 TB, requiring sharding strategies distinct from tensor or pipeline parallelism.
...
\begin{tikzpicture}[font=\small\usefont{T1}{phv}{m}{n}, scale=0.85, transform shape]
  ...
  \node[right, font=\scriptsize\usefont{T1}{phv}{m}{n}] at (3.5, 2.75) {Svr 2: Users K..N};
...
class MetaEmbeddingScaleCheck:
    ...
    entity_count_t = 10
    min_dimension = 128
    bytes_per_dimension_int8 = 1

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    entity_count = entity_count_t * 1_000_000_000_000
    storage_bytes = storage_tb * 1_000_000_000_000
    dense_int8_tb = entity_count * min_dimension * bytes_per_dimension_int8 / 1_000_000_000_000
    effective_bytes_per_entity = storage_bytes / entity_count

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(dense_int8_tb > storage_tb, "Dense INT8 lower bound should exceed the stated storage total")
    check(effective_bytes_per_entity < min_dimension, "100 TB over 10T entities cannot hold a dense 128d INT8 vector per entity")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    storage_tb_str = fmt(storage_tb, precision=0, commas=False)
    entity_count_t_str = fmt(entity_count_t, precision=0, commas=False)
    min_dimension_str = fmt(min_dimension, precision=0, commas=False)
    dense_int8_tb_str = fmt(dense_int8_tb, precision=0)
    effective_bytes_per_entity_str = fmt(effective_bytes_per_entity, precision=0, commas=False)
```

These scale bullets describe infrastructure-level totals, not one dense resident table in which every unique entity has a 128--256-dimensional vector. A dense INT8 table with `{python} MetaEmbeddingScaleCheck.entity_count_t_str` trillion entities and `{python} MetaEmbeddingScaleCheck.min_dimension_str` dimensions would require at least `{python} MetaEmbeddingScaleCheck.dense_int8_tb_str` TB, while `{python} MetaEmbeddingScaleCheck.storage_tb_str` TB across the same entity count averages only `{python} MetaEmbeddingScaleCheck.effective_bytes_per_entity_str` bytes per entity. The production system therefore relies on compression, tiering, cold storage, and uneven entity coverage rather than a single fully materialized dense matrix.
...
@tbl-inference-meta-embedding-perf compares network round-trip, lookup latency, and bandwidth before and after batching plus shard-aware dispatch:
...
$$T_{\text{p2p}} = \alpha + \frac{M}{\text{BW}}$$ {#eq-p2p-time}

where $\alpha$ is the network latency and $M/\text{BW}$ is the transfer time.

@eq-alltoall-time captures the more complex AllToAll communication\index{AllToAll!alltoall communication} for expert parallelism, where each device exchanges distinct data with every other device.

$$T_{\text{alltoall}} = (N-1) \times \left(\alpha + \frac{M/N}{\text{BW}}\right)$$ {#eq-alltoall-time}

Both equations make clear that the achievable bandwidth $\text{BW}$ and latency $\alpha$ of the underlying interconnect determine real-world sharding performance. Production hardware differs enough on both axes that the interconnect choice can determine whether a sharding plan is viable.
...
class AllReduceInterconnect:
    ...
    nvlink_bw_gbs = 900
    ib_hdr_bw_gbs = 25
    eth_100g_bw_gbs = 12.5

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    message_gb = message_mb / 1000
    allreduce_factor = 2 * (n_devices - 1) / n_devices

    nvlink_ms = allreduce_factor * message_gb / nvlink_bw_gbs * 1000
    ib_ms = allreduce_factor * message_gb / ib_hdr_bw_gbs * 1000
    eth_ms = allreduce_factor * message_gb / eth_100g_bw_gbs * 1000

    nvlink_pct = nvlink_ms / layer_budget_ms * 100
    ib_pct = ib_ms / layer_budget_ms * 100
    eth_pct = eth_ms / layer_budget_ms * 100

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(nvlink_ms < ib_ms < eth_ms, "Interconnect timing should follow bandwidth order")
    check(eth_pct < 5, f"Raw 100G Ethernet share should remain under 5%, got {eth_pct:.1f}%")

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    nvlink_bw_gbs_str = fmt(nvlink_bw_gbs, precision=0, commas=False)
    nvlink_ms_str = fmt(nvlink_ms, precision=2, commas=False)
    ib_ms_str = fmt(ib_ms, precision=2, commas=False)
    eth_ms_str = fmt(eth_ms, precision=2, commas=False)
    nvlink_pct_str = fmt(nvlink_pct, precision=2, commas=False)
    ib_pct_str = fmt(ib_pct, precision=1, commas=False)
    eth_pct_str = fmt(eth_pct, precision=1, commas=False)
```

::: {#nbk-inference-interconnect-technology-comparison .callout-notebook title="Interconnect technology comparison"}

Communication overhead depends heavily on the interconnect technology. @tbl-inference-interconnect-specs sets the physical limits that bound achievable sharding speedups:

| **Interconnect**   | **Bandwidth**\index{Bandwidth} | **Latency**\index{Latency!tail} | **Use Case**           |
|:-------------------|-------------------------------:|--------------------------------:|:-----------------------|
...
**Example: 8-Way tensor parallelism communication.** Activation size: 8 MB per all-reduce (batch=1, hidden=8192). @tbl-inference-allreduce-interconnect translates each interconnect's bandwidth into AllReduce time as a fraction of a 30 ms transformer-layer budget:

| **Interconnect**                 |                                **AllReduce Time** |                                    **% of 30 ms Layer** |
|:---------------------------------|--------------------------------------------------:|--------------------------------------------------------:|
| **NVLink**                       | `{python} AllReduceInterconnect.nvlink_ms_str` ms | `{python} AllReduceInterconnect.nvlink_pct_str` percent |
| **InfiniBand**\index{InfiniBand} |     `{python} AllReduceInterconnect.ib_ms_str` ms |     `{python} AllReduceInterconnect.ib_pct_str` percent |
| **100G Ethernet**                |    `{python} AllReduceInterconnect.eth_ms_str` ms |    `{python} AllReduceInterconnect.eth_pct_str` percent |
...
[^fn-nvlink-bandwidth-inf]: **NVLink Bandwidth Evolution**\index{NVLink!bandwidth evolution}: From 160 GB/s bidirectional on early NVLink implementations to `{python} TpSpeedupCalc.nvlink_h100_gbs_str` GB/s on Hopper-class GPUs to 1.8 TB/s on Blackwell-class GPUs---roughly a 10$\times$ improvement across three hardware generations. This bandwidth growth is what made intra-node tensor parallelism across 8 GPUs practical with less than 5 percent communication overhead, directly determining the maximum model size servable without crossing the slower cross-node interconnect. \index{NVLink!bandwidth evolution}
...
Load balancing evaluation uses several key metrics. Maximum queue length measures the longest queue across all replicas and determines worst-case latency. Load variance captures the standard deviation of queue lengths and measures balance. Utilization spread represents the difference between most- and least-loaded replicas. Decision overhead quantifies the time required to make routing decisions.
...
Under these realistic conditions, uninformed strategies perform poorly. The maximum queue length under random assignment follows @eq-random-max-queue:
...
The power of two choices (Principle \ref{pri-power-two-choices}) provides the theoretical foundation: querying just two random replicas and selecting the least-loaded one exponentially reduces tail latency from $O(\log n / \log \log n)$ to $O(\log \log n)$.
...
[^fn-balls-bins-routing]: **Balls-into-Bins (Power of Two Choices)**: With random placement, maximum bin load is $\Theta(\log n / \log \log n)$; with two random choices and greedy selection, it drops to $\Theta(\log \log n)$. For a 1,000-server inference fleet, this reduces worst-case queue depth from roughly 5 to roughly 2, cutting P99 tail latency by nearly half while adding only the overhead of a single extra queue-length probe per request. \index{Power of Two Choices!routing}
...
::: {.callout-important title="Exponential gain from one change"}
...
The mechanism is intuitive: random assignment occasionally makes poor choices (routing to an already-busy server), and these mistakes compound. With two choices, the algorithm almost never makes the worst choice, avoiding the tail behavior that creates long queues.
...
A weighted-routing example shows how the policy handles mixed GPU hardware in practice.
...
# │   across H100 (1000 QPS) and A100 (600 QPS) servers at 15,000 QPS target,
# │   versus equal-share routing that overloads A100s.
# │ Show: total_cap_str="22,000" QPS, h100_util_str="68"%, a100_util_str="68"%;
# │   a100_util_equal_str="83"% (reduced headroom)—inline in the callout.
...
# │
# │ Imports: (none—pure Python arithmetic, no pint quantities)
# │ Exports: HeterogeneousGpuCluster.total_cap_str,
# │   HeterogeneousGpuCluster.h100_weight_str,
# │   HeterogeneousGpuCluster.a100_weight_str,
# │   HeterogeneousGpuCluster.h100_load_str,
# │   HeterogeneousGpuCluster.a100_load_str,
# │   HeterogeneousGpuCluster.h100_util_str,
# │   HeterogeneousGpuCluster.a100_util_str,
# │   HeterogeneousGpuCluster.equal_load_str,
# │   HeterogeneousGpuCluster.h100_util_equal_str,
# │   HeterogeneousGpuCluster.a100_util_equal_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class HeterogeneousGpuCluster:
    ...
    n_h100 = 10
    n_a100 = 20
    h100_cap_qps = 1000   # QPS capacity per H100 server
    a100_cap_qps = 600    # QPS capacity per A100 server
    target_qps = 15000    # total incoming traffic
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    assert target_qps <= total_cap, "Target QPS must not exceed total cluster capacity"
    assert abs(h100_util - a100_util) < 1.0, "Weighted routing must equalize utilization"

    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    total_cap_str = fmt(total_cap, precision=0)
    h100_weight_str = fmt(h100_weight * 100, precision=1, commas=False)
    a100_weight_str = fmt(a100_weight * 100, precision=1, commas=False)
    h100_load_str = fmt(h100_load, precision=0, commas=False)
    a100_load_str = fmt(a100_load, precision=0, commas=False)
    h100_util_str = fmt(h100_util, precision=0, commas=False)
    a100_util_str = fmt(a100_util, precision=0, commas=False)
    equal_load_str = fmt(equal_load, precision=0, commas=False)
    h100_util_equal_str = fmt(h100_util_equal, precision=0, commas=False)
    a100_util_equal_str = fmt(a100_util_equal, precision=0, commas=False)
```
...
- Total capacity: $10 \times 1000$ + $20 \times 600$ = `{python} HeterogeneousGpuCluster.total_cap_str` QPS

**Target traffic**\index{Target Traffic}: 15,000 QPS

**Weighted assignment**\index{Weighted Assignment}:

- H100 weight: 1000/22000 = `{python} HeterogeneousGpuCluster.h100_weight_str` percent
- A100 weight: 600/22000 = `{python} HeterogeneousGpuCluster.a100_weight_str` percent

**Expected load per server**\index{Expected Load Per Server}:

- H100: $15000 \times (1000/22000)$ = `{python} HeterogeneousGpuCluster.h100_load_str` QPS (`{python} HeterogeneousGpuCluster.h100_util_str` percent utilization)
- A100: $15000 \times (600/22000)$ = `{python} HeterogeneousGpuCluster.a100_load_str` QPS (`{python} HeterogeneousGpuCluster.a100_util_str` percent utilization)
...
- Per-server load: 15000/30 = `{python} HeterogeneousGpuCluster.equal_load_str` QPS
- H100 utilization: `{python} HeterogeneousGpuCluster.h100_util_equal_str` percent (underutilized)
- A100 utilization: `{python} HeterogeneousGpuCluster.a100_util_equal_str` percent (high utilization, reduced latency headroom)
...
- **Distributed counters with gossip**\index{Distributed Counters with Gossip}: Eventually consistent, may route to stale information
- **Sampled least-connections**\index{Sampled Least-Connections}: Query a subset of servers, choose minimum (combines with two-choices)
...
: **LLM Routing Algorithm Comparison**: Observed p99 latency and load variance under round-robin, least-connections, and least-connections-plus-two-choices routing for highly variable LLM workloads, quantifying how routing choices reshape tail latency. {#tbl-inference-llm-routing-comparison}
...
- **LLM conversations**\index{LLM!conversations}: KV cache from previous turns
- **Recommendation sessions**\index{Recommendation Sessions}: User context and recent interactions
- **Streaming inference**\index{Inference!streaming}: Model state from previous frames
...
[^fn-consistent-hashing-kv]: **Consistent Hashing**: A ring-based load distribution algorithm originally deployed at Akamai for CDN routing. Servers are mapped onto a virtual ring so that adding or removing a node remaps only $K/N$ keys on average. For LLM serving, this minimal-disruption property preserves KV cache locality during autoscaling events: without it, every scale-up would invalidate cached conversation state across the fleet, forcing expensive recomputation. \index{Consistent Hashing!KV cache}

$$\text{server}(request) = \operatorname{arg\,min}_{s \in S} \text{distance}(\text{hash}(key), \text{hash}(s))$$

where servers and keys are mapped onto a ring, and each request routes to the nearest server clockwise.

Key properties:

- **Deterministic**\index{Deterministic}: Same key always routes to same server
- **Minimal disruption**\index{Minimal Disruption}: Adding/removing servers only remaps $K/N$ keys on average
- **Load balancing**: With virtual nodes, load distributes evenly

For LLM serving, the most direct payoff is avoiding unnecessary KV-cache rebuilds across conversation turns.
...
**Handling server failures**\index{Handling Server Failures}:
...
The sharding strategies examined in @sec-inference-scale-model-sharding-inference-82b3 introduce routing complexity: a single inference request may require computation on multiple devices, necessitating coordination.
...
@tbl-inference-health-check-cadences sets timeout-configuration intervals, timeouts, and failure thresholds for the three probe tiers\index{Timeout Configuration}:
...
: **GPU Inference Health-Check Cadences**: Liveness, readiness, and deep-health probe intervals, timeouts, and failure thresholds for GPU inference servers, balancing fast outage detection against the cost of GPU-resident probes. {#tbl-inference-health-check-cadences}
...
**Circuit breaker pattern**\index{Circuit Breaker!circuit breaker pattern}[^fn-circuit-breaker-serving] [@nygard2007releaseit]:
...
Backpressure propagation complements circuit breakers. When servers are overloaded, they signal upstream to reduce the request rate:
...
Without these mechanisms, a single degraded server can trigger a chain reaction that takes down the entire serving fleet. Circuit breakers and backpressure convert that chain reaction into a contained local failure.
...
@tbl-inference-circuit-breaker-gpu fixes the error and latency thresholds, open duration, and half-open probe count for GPU inference\index{Inference!configuration for gpu}:
...
- Cost efficiency\index{Efficiency!cost}: Sharing infrastructure across tenants improves utilization
- **Operational simplicity**\index{Operational Simplicity}: Fewer clusters to manage, monitor, and upgrade
- **Statistical multiplexing**\index{Statistical Multiplexing}: Aggregate traffic is more predictable than per-tenant traffic

However, sharing introduces risks. @tbl-tenancy-comparison weighs utilization gains against isolation challenges:

- **Noisy neighbors**\index{Noisy Neighbor}: One tenant's burst traffic impacts others
- **Resource contention**\index{Resource Contention}: GPU memory, network bandwidth, CPU cycles
- **Security boundaries**\index{Security!boundaries}: Tenant data must remain isolated
- **SLO complexity**\index{SLO!complexity}: Different tenants have different requirements
...
: **Single vs. Multi-Tenant Tradeoffs**\index{Single vs. Multi-Tenant Tradeoffs}: Multi-tenancy reduces cost but requires careful isolation engineering. {#tbl-tenancy-comparison}
...
class NoisyNeighborQuotaScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    slo_ms_str = fmt(slo_ms, precision=0, commas=False)
    tenant_base_qps_str = fmt(tenant_base_qps, precision=0, commas=False)
    tenant_burst_qps_str = fmt(tenant_burst_qps, precision=0, commas=False)
    tenant_throttled_qps_str = fmt(tenant_throttled_qps, precision=0, commas=False)
    protected_p99_ms_str = fmt(protected_p99_ms, precision=0, commas=False)
    burst_p99_ms_str = fmt(burst_p99_ms, precision=0, commas=False)
...
- Each tenant: `{python} NoisyNeighborQuotaScenario.tenant_base_qps_str` QPS, `{python} NoisyNeighborQuotaScenario.slo_ms_str` ms P99 latency
- GPU utilization: 70 percent
- All SLOs met

@tbl-inference-noisy-neighbor-no-isolation records the noisy-neighbor scenario when tenant 3 bursts to `{python} NoisyNeighborQuotaScenario.tenant_burst_qps_str` QPS:
...
: **Noisy Neighbor Without Isolation**: Per-tenant QPS, p99 latency, and SLO status when tenant 3 bursts to `{python} NoisyNeighborQuotaScenario.tenant_burst_qps_str` QPS on a shared GPU pool with no resource quotas. Latencies for every tenant exceed the `{python} NoisyNeighborQuotaScenario.slo_ms_str` ms SLO, demonstrating how a single offending tenant cascades into a multi-tenant outage. {#tbl-inference-noisy-neighbor-no-isolation}

Without isolation, one tenant's burst causes cascade failures for all tenants.

@tbl-inference-noisy-neighbor-quotas reruns the same workload with per-tenant resource quotas:

| **Tenant**       |                                                           **QPS (actual)** |                                               **P99 Latency** | **SLO Status**                                                         |
|:-----------------|---------------------------------------------------------------------------:|--------------------------------------------------------------:|:-----------------------------------------------------------------------|
| **Tenant 1**     |                                                                        100 | `{python} NoisyNeighborQuotaScenario.protected_p99_ms_str` ms | `{python} NoisyNeighborQuotaScenario.protected_status_str`             |
| **Tenant 2**     |                                                                        100 | `{python} NoisyNeighborQuotaScenario.protected_p99_ms_str` ms | `{python} NoisyNeighborQuotaScenario.protected_status_str`             |
| **Tenant 3**     | `{python} NoisyNeighborQuotaScenario.tenant_throttled_qps_str` (throttled) |     `{python} NoisyNeighborQuotaScenario.burst_p99_ms_str` ms | `{python} NoisyNeighborQuotaScenario.burst_status_str` (only for them) |
| **Tenants 4-10** |                                                                   100 each | `{python} NoisyNeighborQuotaScenario.protected_p99_ms_str` ms | `{python} NoisyNeighborQuotaScenario.protected_status_str`             |

: **Noisy Neighbor With Per-Tenant Quotas**: Per-tenant QPS, p99 latency, and SLO status after enabling per-tenant resource quotas in the same workload. Tenant 3 is throttled to `{python} NoisyNeighborQuotaScenario.tenant_throttled_qps_str` QPS and absorbs the latency penalty alone, while the other nine tenants stay within their `{python} NoisyNeighborQuotaScenario.slo_ms_str` ms SLO. {#tbl-inference-noisy-neighbor-quotas}
...
| **Batch size**\index{Batch Size}           | Increases   | Unchanged | Instant                  |
...
: **Cold-Start Timeline for a Llama-70B Replica (H100)**: Per-phase and cumulative duration for bringing up a fresh Llama-70B serving replica on an H100 instance. Cloud API request, GPU provisioning, container startup, S3 model download, GPU load, CUDA warmup, and the readiness probe sum to about 7 minutes, with the 300-second S3 model download dominating the budget. The multi-minute lag is why reactive autoscaling alone cannot absorb sudden traffic spikes for GPU serving fleets with latency SLOs. {#tbl-inference-cold-start-timeline-llama70b}

**Implication**\index{Implication}: Scaling decisions must anticipate demand several minutes in advance. Reactive scaling alone cannot handle sudden traffic spikes.
...
@lst-metric-based-scaling shows a typical metric-based scaling configuration.\index{Metric!based scaling}
...
**Without prewarming**\index{Without Pre-Warming}:
...
::: {#ws-inference-chatgpt-traffic-spike .callout-war-story title="The ChatGPT traffic spike"}
...
@tbl-inference-chatbot-daily-traffic shows the predictable daily patterns of a chatbot service:
...
: **Daily Traffic Pattern for a Chatbot Service**: Hourly QPS bands and the number of replicas required to absorb each band for a chatbot with predictable diurnal traffic. The 6$\times$ swing from off-peak (500 QPS, 5 replicas) to peak (3,000 QPS, 30 replicas) is the input signal a predictive autoscaler uses to schedule capacity ahead of demand. {#tbl-inference-chatbot-daily-traffic}

@tbl-inference-chatbot-predictive-schedule converts that demand curve into a predictive scaling schedule that warms replicas before each ramp:
...
: **Predictive Scaling Schedule**: Per-time-bucket scaling actions, active replicas, and replicas in cold-start warming for the same chatbot service. Warming replicas 30 minutes before each ramp absorbs the cold-start delay so peak capacity is online when traffic arrives, avoiding the over-provisioning that purely reactive autoscaling demands. {#tbl-inference-chatbot-predictive-schedule}
...
| **Warmup**\index{Warmup} |       5--30s | Precompilation          |
...
#### Multi-region architecture patterns {#sec-inference-scale-multiregion-architecture-patterns-5c67}
...
#### Pattern 3: Federated inference with model sharding {#sec-inference-scale-pattern-3-federated-inference-model-sharding-6c44}
...
@lst-active-failover shows the active-active failover routing logic.\index{Failover!active-active failover}
...
- **Gradual recovery**\index{Gradual Recovery}: When the failed region recovers, gradually shift traffic back to avoid oscillation.
...
Consider a Llama-70B layer where most activations fall within [-10, 10] [@touvron2023llama] but specific channels reach magnitudes of 1000+. Symmetric INT8 quantization with range [-127, 127] must choose:
...
The compensation step propagates quantization error to unquantized columns, where the Hessian-weighted update minimizes output deviation. This achieves $O(d_{\text{row}} \cdot d_{\text{col}}^2)$ complexity vs. $O(d_{\text{row}} \cdot d_{\text{col}}^3)$ for naive OBS.
...
| **Quantization**\index{Quantization}\index{Quantization!quantization-aware training}\index{Quantization!rotation-based} | **Best For**                       | **Framework Support**         |
|:------------------------------------------------------------------------------------------------------------------------|:-----------------------------------|:------------------------------|
...
### Framework integration {#sec-inference-scale-framework-integration-945c}

Production serving frameworks integrate quantization with their batching and memory management systems.

@lst-vllm-quantization shows how vLLM loads a prequantized AWQ model.\index{Quantization!vllm}

::: {#lst-vllm-quantization lst-cap="**vLLM Quantization**: Loading an AWQ-quantized model with automatic kernel selection and KV cache management."}
...
@lst-tensorrt-quantization demonstrates the equivalent quantization workflow in TensorRT-LLM.\index{TensorRT!llm quantization}

::: {#lst-tensorrt-quantization lst-cap="**TensorRT-LLM Quantization**: Offline AWQ quantization with calibration data for optimized INT4 inference."}
...
# │
# │ Imports: mlsysim.core.constants (A100_MEM_CAPACITY, GiB), mlsysim.fmt (check, fmt)
...
class QuantizedServingCapacity:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    weight_int4_gb = params_b * bytes_int4
    kv_headroom_gb = a100_mem_gb - weight_int4_gb
    kv_per_1k_sequence_gb = KVCacheCalc.kv_fp16_gb / (KVCacheCalc.kv_seq_len / 1000)
    max_1k_sequences = int(kv_headroom_gb // kv_per_1k_sequence_gb)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(weight_int4_gb == 35, "70B INT4 weights should be 35 GB")
    ...
    weight_int4_gb_str = fmt(weight_int4_gb, precision=0, commas=False)
    kv_headroom_gb_str = fmt(kv_headroom_gb, precision=0, commas=False)
    kv_per_1k_sequence_gb_str = fmt(kv_per_1k_sequence_gb, precision=1, commas=False)
    max_1k_sequences_str = fmt(max_1k_sequences, precision=0, commas=False)
```

A 70B model with 4-bit weights requires approximately `{python} QuantizedServingCapacity.weight_int4_gb_str` GB, leaving `{python} QuantizedServingCapacity.kv_headroom_gb_str` GB on an `{python} QuantizedServingCapacity.a100_mem_gb_str` GB A100 for KV cache. With FP16 KV cache at about `{python} QuantizedServingCapacity.kv_per_1k_sequence_gb_str` GB per 1K-token sequence for the MHA baseline, this supports about `{python} QuantizedServingCapacity.max_1k_sequences_str` concurrent 1K-token sequences before other runtime overheads. With FP16 weights, the same 70B model would not fit on one A100 at all, so weight-only quantization changes both feasibility and batch capacity.
...
#### Decision framework {#sec-inference-quantization-decision-framework-d3c1}
...
Production-scale serving systems test the batching, sharding, caching, and routing techniques developed above against real workload variance, latency budgets, and operational constraints. ChatGPT-style services handle unpredictable global request volume; TikTok-style recommenders serve ranked content at sub-100-millisecond latency. The case studies below show how each system combines the chapter's techniques to meet its specific cost and latency targets.

::: {#ws-inference-continuous-batching-revolution .callout-war-story title="The continuous batching revolution"}
...
OpenAI-style API infrastructure serves GPT-class models to large developer and application workloads. The infrastructure must handle highly variable request sizes (from short prompts to long-context requests) while maintaining quality of service across diverse workloads.
...
- 8xH100 per large-model shard group
...
class RankingCascadeCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    final_eval_reduction_str = fmt(final_eval_reduction, precision=0)
    l3_per_candidate_ms_str = fmt(l3_per_candidate_ms, precision=0, commas=False)
    l3_all_s_str = fmt(l3_all_s, precision=0)
    cascade_budget_ms_str = fmt(cascade_budget_ms, precision=0, commas=False)
    cost_reduction_rounded_str = fmt(cost_reduction_rounded, precision=0)
```
...
The cascade reduces final L3 evaluations by `{python} RankingCascadeCost.final_eval_reduction_str`$\times$ compared to running L3 on all candidates. Under this simplified request-budget model, the L3 stage costs `{python} RankingCascadeCost.l3_per_candidate_ms_str` ms per candidate. Applying L3 to the full million-candidate set would therefore consume about `{python} RankingCascadeCost.l3_all_s_str` seconds of model work, while the cascade budget sums to `{python} RankingCascadeCost.cascade_budget_ms_str` ms. The resulting model-work reduction is roughly `{python} RankingCascadeCost.cost_reduction_rounded_str`$\times$.
...
[^fn-tpu-inference]: **TPU for Inference**\index{TPU!for inference}: Google's search infrastructure runs ranking models on TPUs rather than GPUs because the systolic array architecture provides deterministic, low-variance latency. GPUs show 2--5$\times$ latency variance depending on batch composition, while TPUs deliver consistent per-request timing critical for meeting strict search SLOs where P99 latency directly impacts revenue. \index{TPU!inference}
...
TikTok separates user understanding (online) from content understanding (offline) through a two-tower architecture with caching[^fn-two-tower-serving]. @tbl-tiktok-two-tower-updates contrasts the online and offline tower update cadences:

[^fn-two-tower-serving]: **Two-Tower Architecture**: Encodes user features and item features through separate neural networks ("towers") into embeddings, then scores via dot product. The serving advantage: item embeddings can be precomputed offline and cached, reducing online inference to the user tower forward pass plus a sub-millisecond vector similarity lookup. The trade-off is reduced model expressiveness from late interaction, since the towers cannot attend to each other's features during encoding. \index{Two-Tower Architecture!architecture, two-tower, serving}
...
| **Priority**   | **SLA**\index{SLA} | **Use Case**                 |
|:---------------|-------------------:|:-----------------------------|
| **Critical**   |              5 min | Creator with large following |
| **Standard**   |             30 min | Normal uploads               |
| **Background** |            2 hours | Bulk/imported content        |

: **TikTok Video Ingestion Priority Tiers**: Three-tier SLA for video ingestion. High-follower-count creators get a five-minute SLA to maximize content freshness for their audience; standard uploads get 30 minutes; bulk imports get two hours of slack so the system can absorb them during quiet periods. {#tbl-tiktok-video-priorities}
...
- **Separation of concerns**: All systems separate embedding and retrieval from ranking and generation, enabling specialized optimization for each component.
...
- **Deadline awareness**: All systems propagate deadlines and make explicit trade-offs between quality and latency when under pressure.
...
Queuing theory establishes that systems provisioned for average load violate SLOs during traffic peaks. At 80 percent average utilization, a modest 25 percent traffic spike pushes utilization above 100 percent, causing unbounded queue growth. As discussed in @sec-inference-scale-autoscaling-168d, the cold start problem exacerbates this: by the time new capacity is available (multiple minutes for GPU instances), SLO violations have already occurred. Capacity planning must account for peak load plus headroom, not average load.

**Fallacy**: *Load balancing does not matter much for inference.*

Simple load balancing strategies like round-robin seem adequate until examined quantitatively. As shown in @sec-inference-scale-load-balancing-request-routing-4997, random assignment produces maximum queue lengths of $O(\log n / \log \log n)$ across $n$ servers. Power-of-two-choices reduces this to $O(\log \log n)$, an exponential improvement. For a 1,000-server cluster, this translates from ~4-5 requests maximum queue to ~2 requests. At the tail latencies that determine SLO compliance, this difference is substantial. The choice of load balancing algorithm has first-order impact on system performance.
...
Larger GPU memory enables larger batches only for models whose throughput is capacity-bound. The binding constraint in LLM decode is memory bandwidth, not capacity: once HBM bandwidth saturates, adding memory does not increase tokens per second. The summary section below makes the bottleneck hierarchy explicit.

### Summary: Resource bottleneck hierarchy {#sec-inference-scale-summary}
...
# ├─────────────────────────────────────────────────────────────────────────────
# │ Context: @sec-inference-scale-summary
# │
# │ Goal: Provide H100 memory and bandwidth for the final summary.
# │ Show: "80" GB, "3.35" TB/s.
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (H100_MEM_CAPACITY, H100_MEM_BW, GiB, TB, second)
# │ Exports: FinalSummaryHardware.h100_mem_gb_str, FinalSummaryHardware.h100_mem_bw_tbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import H100_MEM_CAPACITY, H100_MEM_BW, GiB, TB, second
...
class FinalSummaryHardware:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    h100_mem_gb_val = h100_mem.m_as(GiB)
    h100_bw_tbs_val = h100_bw.m_as(TB/second)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, fmt
    check(h100_mem_gb_val == 80, "H100 memory capacity should be 80 GiB")
...
While larger GPU memory enables larger batches for models that fit in memory, the bottleneck often shifts before memory is exhausted. Memory bandwidth limits throughput for bandwidth-bound operations (LLM decode). Compute limits throughput for compute-bound operations (prefill, vision inference). A bandwidth-bound decode workload on an H100 with `{python} FinalSummaryHardware.h100_mem_gb_str` GB memory eventually plateaus when HBM3 bandwidth (`{python} FinalSummaryHardware.h100_mem_bw_tbs_str` TB/s) saturates; adding memory alone does not raise token throughput once that point is reached. Understanding whether the workload is compute-bound, memory-bandwidth-bound, or capacity-bound guides appropriate resource allocation.
...
To distill the most important lessons from this chapter, focus on the patterns that keep reappearing across serving systems.

::: {.callout-important title="Three things to remember"}

1. **Serving cost dominates training cost over a model's lifetime.** For high-volume applications, serving cost exceeds training cost by 100$\times$ or more. Every percentage point of serving efficiency improvement yields ongoing cost reduction. Optimize serving ruthlessly.
...
* **Prefill vs. decode**\index{Prefill vs. Decode}: LLM serving is split into two regimes: compute-bound prefill (input processing) and memory-bandwidth-bound decode (token generation). Optimizing for the "Decode" phase requires high-bandwidth memory (HBM3) or specialized storage-as-memory (HBF).
...
* **Statefulness is the challenge**\index{Statefulness Is the Challenge}: Unlike stateless vision models, LLM serving is inherently stateful due to the KV cache. This requires sticky routing, prefix caching, and sophisticated memory paging (PagedAttention) to prevent fragmentation.
* **Distribution is a tax**\index{Distribution Is a Tax}: Every step of distribution (network round-trips, serialization, and coordination) adds a "serving tax" that must be explicitly budgeted within the milliseconds allowed for a user response.
