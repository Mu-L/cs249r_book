...
$$S(B) = T_{\text{fixed}} + t_{\text{req}} \cdot B$$ {#eq-service-time-batch}
...
$$\mu_{\text{eff}}(B) = \frac{B}{S(B)} = \frac{B}{T_{\text{fixed}} + t_{\text{req}} \cdot B}$$
...
For production sizing, teams often first enforce a utilization headroom target rather than solving the full latency minimization problem. With $S(B)=T_{\text{fixed}}+t_{\text{req}} B$, the constraint $\rho=\lambda S(B)/B \leq \rho_{\text{target}}$ gives:

$$B_{\min}(\rho_{\text{target}}) = \frac{\lambda T_{\text{fixed}}}{\rho_{\text{target}} - \lambda t_{\text{req}}}$$ {#eq-optimal-batch-approx}

where $\rho_{\text{target}}$ is the target utilization (typically 0.7--0.8 for production systems to maintain latency headroom) and $\rho_{\text{target}} > \lambda t_{\text{req}}$.
...
Measure $T_{\text{fixed}}$ and $t_{\text{req}}$ empirically by profiling inference at batch sizes 1, 8, and 32. Fit the linear model $S(B) = T_{\text{fixed}} + t_{\text{req}} B$.

#### Step 2: Compute stability threshold {.unnumbered}

Find minimum batch size $B_{\text{min}}$ such that $\mu_{\text{eff}}(B_{\text{min}}) > \lambda$:

$$B_{\text{min}} = \frac{T_{\text{fixed}} \lambda}{1 - t_{\text{req}} \lambda}$$
