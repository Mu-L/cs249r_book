...
- **Recommendation and ranking**\index{Recommendation Systems!request volume}: 80–90 percent of requests
- **Vision and image processing**\index{Vision Models!request volume}: 5–10 percent of requests
- **NLP/LLM**\index{LLM!request volume}: 1–5 percent of requests (but growing rapidly)
...
**Tradeoff**: Option C achieves about 33 percent higher per-replica capacity, or 25 percent lower utilization, at the cost of higher average latency (22 ms vs. 15 ms). Both meet the 50 ms P99 SLO.
...
***PagedAttention***\index{PagedAttention!block table}\index{PagedAttention!definition} is a memory management technique that applies virtual memory principles to **KV Cache** allocation, storing attention states in noncontiguous, fixed-size physical blocks.
...
3.  **Floor**: Estimated decode latency $\approx$ `{python} MoECapacityPlan.total_decode_ms_str` ms/token, or `{python} MoECapacityPlan.tokens_per_sec_str` tokens/second at batch size 1.
...
@eq-alltoall-time captures the more complex AllToAll communication\index{AllToAll!communication} for expert parallelism, where each device exchanges distinct data with every other device.
...
@tbl-inference-health-check-cadences sets timeout-configuration intervals, timeouts, and failure thresholds for the three probe tiers\index{Health Check!timeout configuration}:
...
- Cost efficiency\index{Multi-Tenancy!cost efficiency}: Sharing infrastructure across tenants improves utilization
...
Traditional quantization methods (GPTQ, AWQ, SmoothQuant) address outliers through compensation or migration. **Rotation-based quantization**\index{Quantization!rotation-based} [@ashkboos2024quarot] takes a fundamentally different approach: mathematically transforming the weight and activation space to eliminate outliers entirely.
...
@lst-vllm-quantization shows how vLLM loads a prequantized AWQ model.\index{Quantization!vLLM integration}
