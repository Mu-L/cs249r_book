...
$$ M_{\text{KV}} = 2 \times 80 \times 8 \times 128 \times 2 = \text{`{python} KVCacheCapacityEstimator.kv_bytes_per_token_str`} \text{ bytes} \approx \mathbf{\text{`{python} KVCacheCapacityEstimator.kv_mb_per_token_str`} \text{ MB/token}} $$

**Step 2**: Calculate cache per request.
$$ \text{`{python} KVCacheCapacityEstimator.context_tokens_str`} \text{ tokens} \times \text{`{python} KVCacheCapacityEstimator.kv_mb_per_token_str`} \text{ MB/token} \approx \mathbf{\text{`{python} KVCacheCapacityEstimator.kv_gb_per_request_str`} \text{ GB/request}} $$
...
$$ \text{Max Batch} = \left\lfloor \frac{\text{`{python} KVCacheCapacityEstimator.available_kv_gb_str`}}{\text{`{python} KVCacheCapacityEstimator.kv_gb_per_request_str`}} \right\rfloor = \mathbf{\text{`{python} KVCacheCapacityEstimator.max_batch_str`}} $$
