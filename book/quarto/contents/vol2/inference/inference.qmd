...
**Problem**: A recommendation model (DLRM) costs \$12,000 to train and then serves `{python} InferenceEconomics.qps_str` queries per second over `{python} InferenceEconomics.serve_duration_days_str` days. By what factor does the lifetime serving cost exceed the one-time training cost?
...
: **Continuous Batching Throughput on Llama-2 70B (8x A100)**: Tokens-per-second and GPU utilization for static, dynamic, and continuous batching of a Llama-2 70B serving workload on an 8x A100 node. {#tbl-inference-continuous-batching-throughput}
...
: **KV Cache Memory Hierarchy**: Capacity, swap-in latency, and intended use case for the three tiers production inference servers use to manage KV-cache memory pressure. {#tbl-inference-kv-cache-memory-hierarchy}
...
: **Noisy Neighbor Without Isolation**: Per-tenant QPS, p99 latency, and SLO status when tenant 3 bursts to `{python} NoisyNeighborQuotaScenario.tenant_burst_qps_str` QPS on a shared GPU pool with no resource quotas. {#tbl-inference-noisy-neighbor-no-isolation}
...
: **Cold-Start Timeline for a Llama-70B Replica (H100)**: Per-phase and cumulative duration for bringing up a fresh Llama-70B serving replica on an H100 instance. {#tbl-inference-cold-start-timeline-llama70b}
...
: **Daily Traffic Pattern for a Chatbot Service**: Hourly QPS bands and the number of replicas required to absorb each band for a chatbot with predictable diurnal traffic. {#tbl-inference-chatbot-daily-traffic}
