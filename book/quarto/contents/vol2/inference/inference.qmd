...
class PrecisionDividend:
    ...
    kv_head_dim = 128
    kv_seq_len = 4096
    kv_fp16_gb = (2 * kv_layers * kv_heads * kv_head_dim * kv_seq_len * BYTES_FP16.m_as(byte) * byte).m_as(GB)
    kv_fp16_per_gpu = kv_fp16_gb / 4
    kv_int8_per_gpu = (kv_fp16_gb / 2) / 4
    ...
    kv_int8_total_str = fmt(kv_fp16_gb/2, precision=1, commas=False)
...
# │ Exports: TpNvlinkRecap.nvlink_h100_gbs_str
class TpNvlinkRecap:
    nvlink_gbs = NVLINK_H100_BW.m_as(GB/second)
    from mlsysim.fmt import fmt
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
...
class QuantizedServingCapacity:
    ...
    kv_seq_len = 4096
    kv_fp16_gb = (2 * kv_layers * kv_heads * kv_head_dim * kv_seq_len * BYTES_FP16.m_as(byte) * byte).m_as(GB)
    kv_per_1k_sequence_gb = kv_fp16_gb / (kv_seq_len / 1000)
