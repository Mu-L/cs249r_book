...
- Total batch time: $200 \times 20$ ms = `{python} BatchingWorkedExample.total_batch_time_str`
- Request completion times:
  - R1 completes useful work at iteration 50, but waits until iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str`
  - R2 completes at iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str`
  - R3 completes useful work at iteration 100, but waits until iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str`
  - R4 completes useful work at iteration 150, but waits until iteration 200 → latency = `{python} BatchingWorkedExample.trad_lat_str`
...
- R1: $50 \times 20$ ms = `{python} BatchingWorkedExample.r1_lat_str` (`{python} BatchingWorkedExample.r1_imp_str`$\times$ improvement over traditional)
...
**Savings**\index{Savings}: `{python} PrefixCachingAnalysis.memory_reduction_pct_str` reduction in KV cache memory, enabling `{python} PrefixCachingAnalysis.concurrent_multiplier_str`$\times$ more concurrent users.
...
Compressing the KV cache to INT4 reduces it to approximately `{python} KVCacheCalc.kv_int4_gb_str` per request, a `{python} KVCacheCalc.kv_compression_str`$\times$ reduction. This freed memory directly translates into larger batch sizes and higher serving throughput. KV cache values exhibit different distributions than model weights: KIVI observes channel-wise outliers in keys and uses per-channel key quantization, while values lack the same channel-wise pattern and are quantized per-token.
...
**Problem**: A team serves a 70B model on `{python} PrecisionDividend.tensor_parallel_degree_str`$\times$ H100 GPUs. Weights in FP16 consume `{python} PrecisionDividend.fp16_weight_total_gb_str` (`{python} PrecisionDividend.fp16_weight_per_gpu_str` GB/GPU). KV cache at FP16 consumes `{python} KVCacheCalc.kv_fp16_gb_str` per request. How does quantizing weights to INT4 and KV cache to INT8 change the maximum batch size?
...
- KV cache per request: `{python} KVCacheCalc.kv_fp16_gb_str` $\div$ `{python} PrecisionDividend.tensor_parallel_degree_str` $\approx$ `{python} PrecisionDividend.kv_fp16_per_gpu_str` GB/GPU
...
- Weights: `{python} PrecisionDividend.fp16_weight_per_gpu_str` $\times (4/16) =$ `{python} PrecisionDividend.int4_weight_per_gpu_str` GB/GPU (INT4)
...
- KV cache per request (INT8): `{python} PrecisionDividend.kv_int8_total_str` $\div$ 4 $\approx$ `{python} PrecisionDividend.kv_int8_per_gpu_str` GB/GPU
...
- **High acceptance** $(\alpha_{\text{acc}} = 0.9)$: Expected `{python} SpeculativeSpeedup.et_high_str` per round, speedup of `{python} SpeculativeSpeedup.sp_high_str`$\times$.
- **Medium acceptance** $(\alpha_{\text{acc}} = 0.7)$: Expected `{python} SpeculativeSpeedup.et_med_str` per round, speedup of `{python} SpeculativeSpeedup.sp_med_str`$\times$.
- **Low acceptance** $(\alpha_{\text{acc}} = 0.5)$: Expected `{python} SpeculativeSpeedup.et_low_str` per round, speedup of `{python} SpeculativeSpeedup.sp_low_str`$\times$.
...
**Minimum sharding**: 2-way (`{python} TpSpeedupCalc.weight_fp16_gb_str` GB/`{python} HardwareSetupSharding.h100_mem_gb_str` per H100)
...
The `{python} TpSpeedupCalc.tp_speedup_str`$\times$ speedup (vs. theoretical `{python} TpSpeedupCalc.tp_theoretical_str`$\times$) reflects communication overhead. At `{python} TpSpeedupCalc.nvlink_h100_gbs_str` GB/s, moving a `{python} TpSpeedupCalc.activation_mb_str` activation payload would take only about 0.01 ms by raw bandwidth math; the 0.3 ms budget shown here includes collective launch, synchronization, ring protocol, and framework overheads.
...
The performance advantages are striking. During autoregressive decode at batch size 1, the dominant cost is reading weights from HBM. A dense 400B model in FP16 reads `{python} MoEEconomics.dense_mem_str` per step. DeepSeek-V3, despite having more total parameters, reads only the 37B active parameters per step (approximately 74 GB in FP16), a `{python} MoEEconomics.bw_ratio_str`$\times$ reduction in per-token bandwidth. The compute savings are proportional: `{python} MoEEconomics.compute_ratio_str`$\times$ fewer FLOPs per token.

The trade-off is memory capacity: all experts must reside in memory even though only a fraction are active at any time. DeepSeek-V3's full model in FP16 requires approximately `{python} MoEEconomics.moe_mem_str`, necessitating distribution across many GPUs.
...
**Example: 8-Way tensor parallelism communication.** Activation size: `{python} AllReduceInterconnect.message_mb_str` per all-reduce (batch=1, hidden=`{python} AllReduceInterconnect.hidden_dim_str`). @tbl-inference-allreduce-interconnect translates each interconnect's bandwidth into AllReduce time as a fraction of a 30 ms transformer-layer budget:
...
- `{python} HeterogeneousGpuCluster.n_h100_str` H100 GPUs (capacity: `{python} HeterogeneousGpuCluster.h100_cap_qps_str` each)
- `{python} HeterogeneousGpuCluster.n_a100_str` A100 GPUs (capacity: `{python} HeterogeneousGpuCluster.a100_cap_qps_str` each)
- Total capacity: $10 \times 1000$ + $20 \times 600$ = `{python} HeterogeneousGpuCluster.total_cap_str`

**Target traffic**\index{Target Traffic}: `{python} HeterogeneousGpuCluster.target_qps_str`

**Weighted assignment**\index{Weighted Assignment}:

- H100 weight: 1000/22000 = `{python} HeterogeneousGpuCluster.h100_weight_str`
- A100 weight: 600/22000 = `{python} HeterogeneousGpuCluster.a100_weight_str`

**Expected load per server**\index{Expected Load Per Server}:

- H100: $15000 \times (1000/22000)$ = `{python} HeterogeneousGpuCluster.h100_load_str` (`{python} HeterogeneousGpuCluster.h100_util_str` utilization)
- A100: $15000 \times (600/22000)$ = `{python} HeterogeneousGpuCluster.a100_load_str` (`{python} HeterogeneousGpuCluster.a100_util_str` utilization)
...
@tbl-inference-noisy-neighbor-no-isolation records the noisy-neighbor scenario when tenant 3 bursts to `{python} NoisyNeighborQuotaScenario.tenant_burst_qps_str`:
...
: **Noisy Neighbor Without Isolation**: Per-tenant QPS, p99 latency, and SLO status when tenant 3 bursts to `{python} NoisyNeighborQuotaScenario.tenant_burst_qps_str` on a shared GPU pool with no resource quotas. {#tbl-inference-noisy-neighbor-no-isolation}
