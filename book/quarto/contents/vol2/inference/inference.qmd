...
$$S(B) = \alpha_{\text{svc}} + \beta_{\text{svc}} \cdot B$$ {#eq-service-time-batch}

where $\alpha_{\text{svc}}$ represents fixed service overhead (kernel launch, weight loading from HBM) and $\beta_{\text{svc}}$ represents marginal per-request computation time. This linear model captures the first-order behavior observed in production systems, though actual service times may exhibit slight sublinearity due to memory bandwidth saturation at large batch sizes.

The **effective service rate** for batched processing is:

$$\mu_{\text{eff}}(B) = \frac{B}{S(B)} = \frac{B}{\alpha_{\text{svc}} + \beta_{\text{svc}} \cdot B}$$

The effective rate increases with batch size, approaching the asymptotic limit $1/\beta_{\text{svc}}$ as $B \to \infty$.
...
For production sizing, teams often first enforce a utilization headroom target rather than solving the full latency minimization problem. With $S(B)=\alpha_{\text{svc}}+\beta_{\text{svc}} B$, the constraint $\rho=\lambda S(B)/B \leq \rho_{\text{target}}$ gives:

$$B_{\min}(\rho_{\text{target}}) = \frac{\lambda\alpha_{\text{svc}}}{\rho_{\text{target}} - \lambda\beta_{\text{svc}}}$$ {#eq-optimal-batch-approx}

where $\rho_{\text{target}}$ is the target utilization (typically 0.7--0.8 for production systems to maintain latency headroom) and $\rho_{\text{target}} > \lambda\beta_{\text{svc}}$.
...
2. **Higher fixed overhead** $(\alpha_{\text{svc}})$: Required batch size increases to amortize overhead
3. **Higher target utilization** $(\rho_{\text{target}})$: Required batch size decreases, but with less latency headroom

This utilization-constrained sizing explains why LLMs (high $\alpha_{\text{svc}}$ from weight loading) benefit from larger batches than vision models (low $\alpha_{\text{svc}}$), even at the same arrival rate.
...
- Weight loading overhead: $\alpha_{\text{svc}} = 50$ ms (time to load attention matrices per forward pass)
- Per-token compute: $\beta_{\text{svc}} = 0.5$ ms per request (amortized across batch)
...
# │      apply N_req = lambda * W to verify requests in system.
# │
# │ Imports: mlsysim.book (check)
...
# │          LittlesLawVerify.n_req_str, LittlesLawVerify.in_service_str,
...
$N_{\text{req}} = 100 \times$ `{python} LittlesLawVerify.expected_total_s_str` $=$ `{python} LittlesLawVerify.n_req_str` requests in system
...
Measure $\alpha_{\text{svc}}$ and $\beta_{\text{svc}}$ empirically by profiling inference at batch sizes 1, 8, and 32. Fit the linear model $S(B) = \alpha_{\text{svc}} + \beta_{\text{svc}} B$.

#### Step 2: Compute stability threshold {.unnumbered}

Find minimum batch size $B_{\text{min}}$ such that $\mu_{\text{eff}}(B_{\text{min}}) > \lambda$:

$$B_{\text{min}} = \frac{\alpha_{\text{svc}} \lambda}{1 - \beta_{\text{svc}} \lambda}$$
...
Traditional batching (also called static batching) processes all requests in a batch through all decode iterations until the longest sequence completes. For a batch of $n$ sequences with output lengths $\{S_1, S_2, ..., S_n\}$, the total compute performed is:

$$C_{\text{traditional}} = n \times S_{\text{max}} \times c_{\text{decode}}$$
...
$$C_{\text{useful}} = \sum_{i=1}^{n} S_i \times c_{\text{decode}}$$

The **waste ratio** quantifies the inefficiency:

$$W = 1 - \frac{C_{\text{useful}}}{C_{\text{traditional}}} = 1 - \frac{\sum_{i=1}^{n} S_i}{n \times S_{\text{max}}} = 1 - \frac{\bar{S}}{S_{\text{max}}}$$ {#eq-waste-ratio}
...
- **High acceptance** $(\alpha_{\text{acc}} = 0.9)$: Expected `{python} SpeculativeSpeedup.et_high_str` tokens per round, speedup of `{python} SpeculativeSpeedup.sp_high_str`$\times$.
- **Medium acceptance** $(\alpha_{\text{acc}} = 0.7)$: Expected `{python} SpeculativeSpeedup.et_med_str` tokens per round, speedup of `{python} SpeculativeSpeedup.sp_med_str`$\times$.
- **Low acceptance** $(\alpha_{\text{acc}} = 0.5)$: Expected `{python} SpeculativeSpeedup.et_low_str` tokens per round, speedup of `{python} SpeculativeSpeedup.sp_low_str`$\times$.
...
$$T_{\text{allreduce}} = 2 \times \frac{(N-1)}{N} \times \frac{M}{\beta}$$ {#eq-allreduce-time}

where $N$ is the number of devices, $M$ is the message size, and $\beta$ is the interconnect bandwidth. The factor of 2 accounts for the reduce-scatter and all-gather phases.
...
$$T_{\text{p2p}} = \alpha + \frac{M}{\beta}$$ {#eq-p2p-time}

where $\alpha$ is the network latency and $M/\beta$ is the transfer time.
...
Mathematically, the key insight is that with random assignment, when $m$ servers have queue length $k$, the probability of queue length $k+1$ growing is proportional to $m/n$. With two choices, this probability drops to $(m/n)^2$, creating a super-exponential decay in queue length distribution.
...
- **Minimal disruption**\index{Minimal Disruption}: Adding/removing servers only remaps $K/N_{\text{servers}}$ keys on average
...
When Server A fails, its 100 virtual nodes are removed from the ring. Requests that would have routed to Server A now route to the next server clockwise. Only ~$1/N_{\text{servers}}$ of requests are affected, where $N_{\text{servers}}$ is the number of servers.
