...
Consider Meta's recommendation infrastructure serving 10 million QPS across the platform. Each request queries ~100 candidate items, each item requires 50 embedding lookups (user, item, cross features), totaling 5,000 embedding lookups per request against a 100 TB embedding table sharded across 1,000 servers.

**Batching strategy**: With 10M QPS and 1,000 embedding shards, each shard receives

$$\text{Lookups per shard} = \frac{10 \times 10^6 \times 5000}{1000} = 50 \text{ million lookups/sec.}$$

Single-threaded processing cannot sustain that rate. A 1 ms batch-accumulation window\index{Batching!batch, accumulation window} absorbs 10,000 concurrent requests per shard, yielding 50K lookups per batch—high memory bandwidth utilization through sequential access patterns. @Tbl-inference-recsys-latency-breakdown shows how the five-phase latency\index{Latency!breakdown} composition fits within the 10 ms SLO, with embedding lookup dominating at 2 ms.
...
Consider a streaming speech-to-text system with 20 ms audio frames and a 100 ms end-to-end latency budget\index{Latency Budget} (5 frames of delay). @Tbl-inference-streaming-speech-pipeline traces the per-stage latency\index{Pipeline!stages} the pipeline must hit to stay within budget; the seven stages sum to 90 ms, leaving 10 ms of slack.
...
At `{python} KVCacheCapacityEstimator.context_tokens_str` tokens $\times$ `{python} KVCacheCapacityEstimator.kv_mb_per_token_str` MB/token, each long-context request consumes approximately `{python} KVCacheCapacityEstimator.kv_gb_per_request_str` GB of KV cache. Subtracting weights (`{python} KVCacheCapacityEstimator.weight_gb_str`) and system overhead (`{python} KVCacheCapacityEstimator.system_gb_str`) from the `{python} KVCacheCapacityEstimator.total_hbm_gb_str` total HBM leaves `{python} KVCacheCapacityEstimator.available_kv_gb_str` available for KV cache, which supports at most `{python} KVCacheCapacityEstimator.max_batch_str` concurrent 128K-context requests. Even with GQA halving KV-head count, long-context inference becomes memory-bound at modest batch sizes, motivating the fragmentation reductions and KV-cache sharding techniques addressed next.
...
**Performance impact**\index{Performance Impact}: The gather operations add overhead, but it is minimal compared to the memory savings; @tbl-inference-pagedattention-throughput shows the 2.5--4$\times$ throughput improvement coming from fitting more concurrent sequences in the same memory.
...
**Prefix hit rate** determines effectiveness; @tbl-inference-prefix-caching-workloads contrasts hit rates and memory savings across workloads, showing where prefix caching pays off and where it does not.
...
@Tbl-inference-tp8-layer-breakdown decomposes the per-layer cost of 8-way tensor parallelism on 8xH100 (NVLink interconnect) into compute and AllReduce phases, exposing the realized 6.9$\times$ speedup as compute scaling minus a small communication tax.
...
Routing imbalance directly erodes GPU utilization and throughput across the eight experts; @tbl-inference-moe-load-balancing quantifies the degradation\index{Load Balancing!metrics}. Production systems monitor routing statistics and may retrain or fine-tune gating to improve balance.
...
Communication overhead depends heavily on the interconnect technology. The physical bandwidth, latency, and intended scope of each candidate interconnect appear in @tbl-inference-interconnect-specs, and @tbl-inference-allreduce-interconnect translates those bandwidths into AllReduce time for an 8-way tensor-parallel layer (activation `{python} AllReduceInterconnect.message_mb_str` per all-reduce; batch=1, hidden=`{python} AllReduceInterconnect.hidden_dim_str`) as a fraction of a 30 ms transformer-layer budget.
...
For GPU inference\index{Inference!configuration for gpu}, the canonical thresholds and recovery settings appear in @tbl-inference-circuit-breaker-gpu.
...
As @tbl-inference-noisy-neighbor-no-isolation records, when tenant 3 bursts to `{python} NoisyNeighborQuotaScenario.tenant_burst_qps_str` without isolation, that burst cascades into SLO violations for all ten tenants on the shared pool. Per-tenant resource quotas contain the impact to the offending tenant alone (@tbl-inference-noisy-neighbor-quotas).
...
Daily patterns make the proactive side of this rule concrete. @Tbl-inference-chatbot-daily-traffic shows a chatbot service's predictable diurnal traffic, and @tbl-inference-chatbot-predictive-schedule converts that demand curve into a predictive schedule that warms replicas before each ramp.
...
For the chatbot service in @tbl-inference-chatbot-daily-traffic and @tbl-inference-chatbot-predictive-schedule, a reactive-only autoscaler must over-provision during each ramp (about 45 replicas at peak) because it cannot anticipate the curve. Predictive scaling sizes the fleet to the actual peak (30 replicas), recovering roughly 33 percent of GPU spend without sacrificing latency headroom.
