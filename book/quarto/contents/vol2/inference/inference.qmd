...
where $N$ is the number of devices, $M$ is the collective payload size, $\alpha$ is startup latency, and $\beta$ is the interconnect bandwidth. The factor of 2 accounts for the reduce-scatter and all-gather phases.
...
where $n$ is the point-to-point payload size, $\alpha$ is the network latency, and $n/\beta$ is the transfer time.
...
The power of two choices (Principle \ref{pri-power-two-choices}) provides the theoretical foundation: querying just two random replicas and selecting the least-loaded one exponentially reduces tail latency from $\mathcal{O}(\log R / \log \log R)$ to $\mathcal{O}(\log \log R)$.
...
@eq-two-choices-max-queue formalizes this exponential improvement, reducing maximum queue length from $\mathcal{O}(\log R / \log \log R)$ to $\mathcal{O}(\log \log R)$:
...
The power-of-two-choices result is one of the most impactful findings in distributed systems theory. By examining just one additional server, maximum queue length improves from $\mathcal{O}(\log R / \log \log R)$ to $\mathcal{O}(\log \log R)$, an exponential improvement.

The practical implications are substantial:

- Near-optimal load balancing with minimal overhead (2 probes vs. $R$ probes)
...
Mathematically, the key insight is that with random assignment, when $m$ servers have queue length $k$, the probability of queue length $k+1$ growing is proportional to $m/R$. With two choices, this probability drops to $(m/R)^2$, creating a super-exponential decay in queue length distribution.
...
3. **Power-of-two-choices provides exponential load balancing improvement.** Maximum queue length improves from $\mathcal{O}(\log R / \log \log R)$ to $\mathcal{O}(\log \log R)$ with minimal overhead (two probes per request). This simple technique should be standard for any distributed inference deployment.
