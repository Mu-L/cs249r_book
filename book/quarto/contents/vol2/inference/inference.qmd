...
class ServingCostSimple:
    ...
    training_cost_str = fmt(training_cost, precision=0, prefix="\\$")
    dau_str = fmt(dau, precision=0)
    reqs_per_day_str = fmt(reqs_per_day, precision=0, commas=False)
    cost_per_request_str = fmt(cost_per_request, precision=3, commas=False, prefix="\\$")
    efficiency_gain_pct_str = fmt(efficiency_gain * 100, precision=0, commas=False)
```
...
3.  **Cost per Request**\index{Serving Cost!cost per request}: Assume a 70B model on H100 costs $\approx$ `{python} ServingCostSimple.cost_per_request_str` per request (input + output).
4.  **Annual Serving Cost**\index{Serving Cost!annual serving cost}: `{python} ServingCostSimple.annual_reqs_b_str` billion requests $\times$ `{python} ServingCostSimple.cost_per_request_str` $=$ **`{python} ServingCostSimple.annual_serving_cost_str`**.

**Systems insight**: Serving costs **`{python} ServingCostSimple.serving_training_ratio_str`$\times$** more than training in just the first year. A `{python} ServingCostSimple.efficiency_gain_pct_str` percent serving-cost or latency-linked efficiency improvement saves about **`{python} ServingCostSimple.efficiency_savings_str`** in the first year, nearly paying for the original training run.
...
class InferenceEconomics:
    ...
    training_cost_brief_str = fmt(training_total_cost, precision=0, prefix="USD ")
    n_gpu_hours_str = fmt(n_gpu_hours, precision=0)
    cost_per_gpu_hour_str = fmt(cost_per_gpu_hour, precision=0, commas=False, prefix="\\$")
    engineering_multiplier_str = fmt(engineering_multiplier, precision=0, commas=False)
    training_hardware_cost_str = fmt(training_hardware_cost, precision=0, prefix="\\$")
    engineering_overhead_cost_str = fmt(training_hardware_cost * engineering_multiplier, precision=0, prefix="\\$")
    sec_per_day_str = fmt(SEC_PER_DAY, precision=0, commas=False)
```

::: {#nbk-inference-serving-cost-multiplier-2 .callout-notebook title="The serving cost multiplier"}

**Problem**: A recommendation model (DLRM) costs `{python} InferenceEconomics.training_cost_brief_str` to train and then serves `{python} InferenceEconomics.qps_str` queries per second over `{python} InferenceEconomics.serve_duration_days_str` days. By what factor does the lifetime serving cost exceed the one-time training cost?

**Training Cost** $(C_{\text{train}})$

- Hardware: `{python} InferenceEconomics.n_gpu_hours_str` GPU-hours @ `{python} InferenceEconomics.cost_per_gpu_hour_str`/hr = `{python} InferenceEconomics.training_hardware_cost_str`
- Engineering Overhead (Data/Exp): `{python} InferenceEconomics.engineering_multiplier_str`$\times$ Hardware = `{python} InferenceEconomics.engineering_overhead_cost_str`
...
(`{python} InferenceEconomics.qps_str` QPS $\times$ `{python} InferenceEconomics.sec_per_day_str` s/day $\times$ `{python} InferenceEconomics.serve_duration_days_str` days)
...
class LittlesLawExample:
    ...
    target_throughput_str = fmt(target_throughput, precision=0)
    latency_slo_ms_str = fmt(latency_slo_s * 1000, precision=0, commas=False)
    batch_size_str = fmt(batch_size, precision=0, commas=False)
    replica_latency_ms_str = fmt(replica_latency_s * 1000, precision=0, commas=False)
```
...
- **Latency SLO** $(T_{\text{lat}})$: `{python} LittlesLawExample.latency_slo_ms_str` ms (0.1 s)
...
class KVCacheCapacityEstimator:
    ...
    total_hbm_gb_str = fmt(total_hbm_gb, precision=0, commas=False)
    system_gb_str = fmt(system_gb, precision=0, commas=False)
    node_gpus_str = fmt(Systems.Nodes.DGX_H100.accelerators_per_node, precision=0, commas=False)
    h100_mem_gb_str = fmt(Hardware.Cloud.H100.memory.capacity.m_as(GiB), precision=0, commas=False)
...
**Problem**: A deployment serves Llama-3-70B (FP16 weights $\approx$ `{python} KVCacheCapacityEstimator.weight_gb_str` GB) on an `{python} KVCacheCapacityEstimator.node_gpus_str`$\times$ H100 node (`{python} KVCacheCapacityEstimator.total_hbm_gb_str` GB total HBM). The goal is to determine the maximum batch size for a context length of 128K tokens.
...
Available Memory for KV = `{python} KVCacheCapacityEstimator.total_hbm_gb_str` GB (Total) - `{python} KVCacheCapacityEstimator.weight_gb_str` GB (Weights) - `{python} KVCacheCapacityEstimator.system_gb_str` GB (System) = `{python} KVCacheCapacityEstimator.available_kv_gb_str` GB.
...
class KVCacheCalc:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check, MarkdownStr, fmt, fmt_math
...
class PrecisionDividend:
    ...
    fp16_weight_total_gb = fp16_weight_per_gpu * tensor_parallel_degree
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    kv_fp16_per_gpu = kv_fp16_gb / tensor_parallel_degree
    kv_int8_per_gpu = (kv_fp16_gb / 2) / tensor_parallel_degree
    ...
    fp16_weight_total_gb_str = fmt(fp16_weight_total_gb, precision=0, commas=False)
    tensor_parallel_degree_str = fmt(tensor_parallel_degree, precision=0, commas=False)
```

::: {#nbk-inference-precision-dividend .callout-notebook title="The precision dividend"}

**Problem**: A team serves a 70B model on `{python} PrecisionDividend.tensor_parallel_degree_str`$\times$ H100 GPUs. Weights in FP16 consume `{python} PrecisionDividend.fp16_weight_total_gb_str` GB (`{python} PrecisionDividend.fp16_weight_per_gpu_str` GB/GPU). KV cache at FP16 consumes `{python} KVCacheCalc.kv_fp16_gb_str` GB per request. How does quantizing weights to INT4 and KV cache to INT8 change the maximum batch size?

**Before optimization** (all FP16):

- Weights: `{python} PrecisionDividend.fp16_weight_per_gpu_str` GB/GPU
...
- KV cache per request: `{python} KVCacheCalc.kv_fp16_gb_str` GB $\div$ `{python} PrecisionDividend.tensor_parallel_degree_str` GPUs $\approx$ `{python} PrecisionDividend.kv_fp16_per_gpu_str` GB/GPU
...
class TpSpeedupCalc:
    ...
    weight_fp16_gb_str = fmt(weight_fp16_gb, precision=0, commas=False)
    activation_mb_str = fmt(activation_mb, precision=0, commas=False)
```
...
**Minimum sharding**: 2-way (`{python} TpSpeedupCalc.weight_fp16_gb_str` GB/`{python} HardwareSetupSharding.h100_mem_gb_str` GB per H100)
...
The `{python} TpSpeedupCalc.tp_speedup_str`$\times$ speedup (vs. theoretical `{python} TpSpeedupCalc.tp_theoretical_str`$\times$) reflects communication overhead. At `{python} TpSpeedupCalc.nvlink_h100_gbs_str` GB/s, moving a `{python} TpSpeedupCalc.activation_mb_str` MB activation payload would take only about 0.01 ms by raw bandwidth math; the 0.3 ms budget shown here includes collective launch, synchronization, ring protocol, and framework overheads.
...
class MoEEconomics:
    ...
    dense_params_b = 400
    moe_total_params_b = _moe.parameters.m_as(Bparam)
    moe_active_params_b = _moe.active_parameters.m_as(Bparam)
...
class MoECapacityPlan:
    ...
    moe_active_params_b_str = fmt(moe_active_params_b, precision=0, commas=False)
    n_gpus_str = fmt(n_gpus, precision=0, commas=False)
    alltoall_overhead_ms_str = fmt(alltoall_overhead_ms, precision=1, commas=False)
    h100_bw_tbs_str = fmt(h100_bw_tbs, precision=2, commas=False)
    h100_mem_gb_str = fmt(h100_mem_gb, precision=0, commas=False)
    moe_total_params_b_str = fmt(moe_total_params_b, precision=0, commas=False)
...
**Problem**: A team deploys DeepSeek-V3\index{DeepSeek-V3} (`{python} MoECapacityPlan.moe_total_params_b_str`B total parameters, `{python} MoECapacityPlan.moe_active_params_b_str`B active per token) for a chatbot application. The model uses FP8 weights (1 byte per parameter). The cluster has `{python} MoECapacityPlan.node_gpus_str`-GPU nodes, each with `{python} MoECapacityPlan.node_gpus_str`$\times$ H100 GPUs (`{python} MoECapacityPlan.h100_mem_gb_str` GB HBM per GPU, `{python} MoECapacityPlan.node_capacity_gb_str` GB per node). How many nodes are needed, and what is the per-token decode latency?
...
2.  **Latency**: Each decode step reads `{python} MoECapacityPlan.moe_active_params_b_str`B active parameters (`{python} MoECapacityPlan.active_mem_gb_str` GB). Distributed across `{python} MoECapacityPlan.n_gpus_str` GPUs, each reads about `{python} MoECapacityPlan.read_per_gpu_str` GB. At `{python} MoECapacityPlan.h100_bw_tbs_str` TB/s HBM bandwidth, $t_{\text{read}} \approx$ `{python} MoECapacityPlan.t_read_ms_str` ms. AllToAll routing adds about `{python} MoECapacityPlan.alltoall_overhead_ms_str` ms.
...
class MetaEmbeddingScaleCheck:
    ...
    n_shards_str = fmt(1_000, precision=0)
    warm_pct_str = fmt(10, precision=0, commas=False)
    cold_pct_str = fmt(89, precision=0, commas=False)
    lookups_per_request_str = fmt(5_000, precision=0)
    tp_parallelism_str = fmt(8, precision=0, commas=False)
```
...
- **Warm embeddings** (next `{python} MetaEmbeddingScaleCheck.warm_pct_str` percent): Column-sharded with `{python} MetaEmbeddingScaleCheck.tp_parallelism_str`-way parallelism
- **Cold embeddings** (remaining `{python} MetaEmbeddingScaleCheck.cold_pct_str` percent): Row-sharded with consistent hashing
...
# │
# │ Imports: mlsysim.core.constants (Systems.Nodes.DGX_H100.accelerators_per_node), mlsysim.fmt (check, fmt)
...
class AllReduceInterconnect:
    ...
    message_mb_str = fmt(message_mb, precision=0, commas=False)
    hidden_dim_str = fmt(8192, precision=0)
```
...
**Example: 8-Way tensor parallelism communication.** Activation size: `{python} AllReduceInterconnect.message_mb_str` MB per all-reduce (batch=1, hidden=`{python} AllReduceInterconnect.hidden_dim_str`). @tbl-inference-allreduce-interconnect translates each interconnect's bandwidth into AllReduce time as a fraction of a 30 ms transformer-layer budget:
...
class HeterogeneousGpuCluster:
    ...
    n_h100_str = fmt(n_h100, precision=0, commas=False)
    n_a100_str = fmt(n_a100, precision=0, commas=False)
    h100_cap_qps_str = fmt(h100_cap_qps, precision=0, commas=False)
    a100_cap_qps_str = fmt(a100_cap_qps, precision=0, commas=False)
    target_qps_str = fmt(target_qps, precision=0)
```
...
- `{python} HeterogeneousGpuCluster.n_h100_str` H100 GPUs (capacity: `{python} HeterogeneousGpuCluster.h100_cap_qps_str` QPS each)
- `{python} HeterogeneousGpuCluster.n_a100_str` A100 GPUs (capacity: `{python} HeterogeneousGpuCluster.a100_cap_qps_str` QPS each)
- Total capacity: $10 \times 1000$ + $20 \times 600$ = `{python} HeterogeneousGpuCluster.total_cap_str` QPS

**Target traffic**\index{Target Traffic}: `{python} HeterogeneousGpuCluster.target_qps_str` QPS
...
class NoisyNeighborQuotaScenario:
    ...
    gpu_util_pct_str = fmt(70, precision=0, commas=False)
```
...
- GPU utilization: `{python} NoisyNeighborQuotaScenario.gpu_util_pct_str` percent
- All SLOs met
