...
class InferenceSetup:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_mem_gb_str = fmt_int(h100_mem_gb_val, commas=False, suffix=' GB')
    h100_mem_bw_tbs_str = fmt(h100_bw_tbs_val, precision=2, commas=False, suffix=' TB/s')
    a100_mem_bw_tbs_str = fmt(a100_bw_tbs_val, precision=1, commas=False, suffix=' TB/s')
...
class ServingCostSimple:
    ...
    efficiency_savings_str = fmt(efficiency_savings/ MILLION, precision=1, commas=False, prefix="$", suffix="M")
...
class InferenceEconomics:
    ...
    serving_cost_total_str = fmt_int(serving_total_cost_val, prefix="$")
    training_cost_total_str = fmt_int(training_total_cost, prefix="$")
    ...
    training_cost_brief_str = fmt_int(training_total_cost, prefix="$")
    n_gpu_hours_str = fmt_int(n_gpu_hours, suffix=' GPU-hours')
...
- Hardware: `{python} InferenceEconomics.n_gpu_hours_str` @ `{python} InferenceEconomics.cost_per_gpu_hour_str`/hr = `{python} InferenceEconomics.training_hardware_cost_str`
...
class LittlesLawExample:
    ...
    latency_slo_ms_str = fmt_int(latency_slo_s * THOUSAND, commas=False)
...
The dynamic batching strategy above works well for vision models where every input has the same shape and every forward pass takes roughly the same time. Autoregressive language models break both assumptions: each request generates a variable number of tokens, and a short prompt finishing in 50 ms shares the GPU with a long response that runs for 2 seconds. Padding every batch to the longest sequence wastes the compute that dynamic batching was designed to recover, and holding finished requests in place until the slowest one completes reintroduces the idle-GPU problem at a different level.
...
class BatchingWorkedExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    mean_output_str = fmt_int(mean_output, commas=False, suffix=' tokens')
    total_batch_time_str = fmt_int(total_batch_time_ms, suffix=' ms')
    total_seq_iters_str = fmt_int(total_seq_iters)
    useful_seq_iters_str = fmt_int(useful_seq_iters)
    r1_lat_str = fmt_int(r1_latency_ms, suffix=' ms')
    r2_lat_str = fmt_int(r2_latency_ms)
    r3_lat_str = fmt_int(r3_latency_ms)
    r4_lat_str = fmt_int(r4_latency_ms)
    trad_lat_str = fmt_int(trad_latency_ms, suffix=' ms')
...
class A100HardwareScenario:
    ...
    activation_transfer_gb = activation_transfer_mb / THOUSAND
    pcie_transfer_ms_val = (activation_transfer_gb / pcie_gen4_gbs_val) * THOUSAND
    nvlink_transfer_ms_val = (activation_transfer_gb / nvlink_a100_gbs_val) * THOUSAND
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_mem_gb_str = fmt_int(a100_mem_gb_val, commas=False, suffix=' GB')
    a100_mem_bw_tbs_str = fmt(a100_mem_bw_tbs_val, precision=1, commas=False, suffix=' TB/s')
    nvlink_a100_gbs_str = fmt_int(nvlink_a100_gbs_val, commas=False, suffix=' GB/s')
    pcie_gen4_gbs_str = fmt_int(pcie_gen4_gbs_val, commas=False, suffix=' GB/s')
    activation_transfer_mb_str = fmt_int(activation_transfer_mb, commas=False, suffix=' MB')
    pcie_transfer_ms_str = fmt(pcie_transfer_ms_val, precision=1, commas=False, suffix=' ms')
    nvlink_transfer_ms_str = fmt(nvlink_transfer_ms_val, precision=2, commas=False, suffix=' ms')
```
...
The system runs on 4 A100-`{python} A100HardwareScenario.a100_mem_gb_str` connected via PCIe Gen4 (`{python} A100HardwareScenario.pcie_gen4_gbs_str` per GPU) rather than NVLink. The server has a dual-socket CPU with NUMA topology. Memory bandwidth per GPU is `{python} A100HardwareScenario.a100_mem_bw_tbs_str` (HBM2e), adequate for decode operations. However, PCIe bandwidth limits tensor parallelism communication to `{python} A100HardwareScenario.pcie_gen4_gbs_str` vs. NVLink's `{python} A100HardwareScenario.nvlink_a100_gbs_str`. For a batch requiring `{python} A100HardwareScenario.activation_transfer_mb_str` activation transfers between GPUs, raw bandwidth math gives approximately `{python} A100HardwareScenario.pcie_transfer_ms_str` per synchronization point over PCIe vs. `{python} A100HardwareScenario.nvlink_transfer_ms_str` over NVLink.
...
# │ Exports: RecsysLatencyBreakdown.total_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class RecsysLatencyBreakdown:
    ...
    # ┌── 3. GUARD (Invariants) ────────────────────────────────────────────
    check(total_ms == 5.2, f"Total latency mismatch: {total_ms}")

    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    total_str = fmt(total_ms, precision=1, commas=False)
```
...
| **Total**                                    | **`{python} RecsysLatencyBreakdown.total_str` ms** | Within 10 ms SLO            |
...
Feature-parallel batching amortizes embedding lookup costs across thousands of queries per second, but it still operates in a request-response pattern: the system collects a batch, processes it, and returns results. Some applications cannot tolerate even that collection delay. Real-time speech recognition, video analysis, and robotics require processing inputs as they arrive, often within a single inter-frame interval of 10 to 33 ms.
...
| **THOUSAND QPS**  |               12.4 |           18 ms |       1000 QPS |
...
class KVCacheCapacityEstimator:
    ...
    available_kv_gb_str = fmt_int(available_kv_gb, commas=False, suffix=' GB')
    max_batch_str = fmt_int(max_batch, commas=False)
    weight_gb_str = fmt_int(weight_gb, commas=False, suffix=' GB')
    total_hbm_gb_str = fmt_int(total_hbm_gb, commas=False, suffix=' GB')
    system_gb_str = fmt_int(system_gb, commas=False, suffix=' GB')
...
The fragmentation overhead measured above is not inherent to autoregressive generation; it is an artifact of treating GPU memory as a contiguous slab. Applying the same virtual-memory abstraction that solved RAM fragmentation decades ago eliminates the problem entirely.
...
class PrefixCachingAnalysis:
    ...
    kv_tb_no_cache_val = kv_bytes_no_cache / TRILLION
    ...
    kv_tb_cached_val = kv_bytes_cached / TRILLION
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    tokens_per_user_str = fmt_int(tokens_per_user, suffix=' tokens')
    kv_tb_no_cache_str = fmt(kv_tb_no_cache_val, precision=1, commas=False, suffix=' TB')
    total_tokens_cached_str = fmt_int(total_tokens_cached)
    kv_tb_cached_str = fmt(kv_tb_cached_val, precision=1, commas=False, suffix=' TB')
    memory_reduction_pct_str = fmt(memory_reduction_pct_val, precision=1, commas=False, suffix=' percent')
...
class KVCacheCalc:
    ...
    kv_fp16_gb_str = fmt(kv_fp16_gb, precision=1, commas=False, suffix=' GB')
    kv_int4_gb_str = fmt(kv_int4_gb, precision=1, commas=False, suffix=' GB')
    kv_savings_gb_str = fmt(kv_savings_gb, precision=1, commas=False)
    kv_compression_str = fmt_int(kv_compression, commas=False)
    h100_mem_gb_str = fmt_int(h100_mem.m_as(GiB), commas=False, suffix=' GB')
    batch8_kv_fp16_gb_str = fmt(batch8_kv_fp16_gb, precision=1, commas=False, suffix=' GB')
    gqa_kv_fp16_gb_str = fmt(gqa_kv_fp16_gb, precision=1, commas=False, suffix=' GB')
    gqa_kv_heads = 8
...
class PrecisionDividend:
    ...
    fp16_weight_per_gpu_str = fmt_int(fp16_weight_per_gpu, commas=False, suffix=' GB')
    ...
    kv_fp16_per_gpu_str = fmt(kv_fp16_per_gpu, precision=1, commas=False, suffix=' GB')
    ...
    fp16_weight_total_gb_str = fmt_int(fp16_weight_total_gb, commas=False, suffix=' GB')
...
class SpeculativeSpeedup:
    ...
    et_high_str = fmt(et_high, precision=1, commas=False, suffix=' tokens')
    et_med_str = fmt(et_med, precision=1, commas=False, suffix=' tokens')
    et_low_str = fmt(et_low, precision=1, commas=False, suffix=' tokens')
```
...
The architectural split shown above rests on a fundamental asymmetry between the two phases of transformer inference: one is compute-bound, the other memory-bandwidth-bound.
...
Chunked prefill and disaggregated serving solve the interference between long prompts and active decode streams, but they assume every request runs the same model weights. Production platforms increasingly serve thousands of fine-tuned variants from a single cluster, where each user's personalized adapter competes for the same on-chip memory that the KV cache already pressures. The memory management problem shifts from paging attention state to swapping model weights on every context switch.
...
class TpSpeedupCalc:
    ...
    seq_total_ms_str = fmt_int(seq_total_ms, suffix=' ms')
    tp8_total_ms_str = fmt_int(tp8_total_ms, suffix=' ms')
    tp_theoretical_str = fmt_int(tp_theoretical, commas=False)
    n_layers_str = fmt_int(n_layers, commas=False)
    nvlink_h100_gbs_str = fmt_int(nvlink_h100_gbs_val, commas=False, suffix=' GB/s')
    hidden_dim_str = fmt_int(hidden_dim)
    weight_fp16_gb_str = fmt_int(weight_fp16_gb, commas=False, suffix=' GB')
...
The `{python} TpSpeedupCalc.tp_speedup_str`$\times$ speedup (vs. theoretical `{python} TpSpeedupCalc.tp_theoretical_str`$\times$) reflects communication overhead. At `{python} TpSpeedupCalc.nvlink_h100_gbs_str`, moving a `{python} TpSpeedupCalc.activation_mb_str` activation payload would take only about 0.01 ms by raw bandwidth math; the 0.3 ms budget shown here includes collective launch, synchronization, ring protocol, and framework overheads.
...
class MoEEconomics:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    dense_mem_str = fmt_int(dense_mem_fp16, commas=False, suffix=' GB')
    moe_mem_str = fmt_int(moe_mem_fp16, commas=False, suffix=' GB')
...
class MoECapacityPlan:
    ...
    t_read_ms = (read_per_gpu_gb/h100_bw_gbs) * THOUSAND
...
class MetaEmbeddingScaleCheck:
    ...
    dense_int8_tb = entity_count * min_dimension * bytes_per_dimension_int8 / TRILLION
...
class AllReduceInterconnect:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    message_gb = message_mb / THOUSAND
    allreduce_factor = 2 * (n_devices - 1) / n_devices

    nvlink_ms = allreduce_factor * message_gb / nvlink_bw_gbs * THOUSAND
    ib_ms = allreduce_factor * message_gb / ib_hdr_bw_gbs * THOUSAND
    eth_ms = allreduce_factor * message_gb / eth_100g_bw_gbs * THOUSAND
    ...
    message_mb_str = fmt_int(message_mb, commas=False, suffix=' MB')
...
class HeterogeneousGpuCluster:
    ...
    h100_weight_str = fmt(h100_weight * 100, precision=1, commas=False, suffix=' percent')
    a100_weight_str = fmt(a100_weight * 100, precision=1, commas=False, suffix=' percent')
    h100_load_str = fmt(h100_load, precision=1, commas=False, suffix=' QPS')
    a100_load_str = fmt(a100_load, precision=1, commas=False, suffix=' QPS')
    h100_util_str = fmt(h100_util, precision=1, commas=False, suffix=' percent')
    a100_util_str = fmt(a100_util, precision=1, commas=False, suffix=' percent')
    ...
    h100_cap_qps_str = fmt_int(h100_cap_qps, commas=False, suffix=' QPS')
    a100_cap_qps_str = fmt_int(a100_cap_qps, commas=False, suffix=' QPS')
    target_qps_str = fmt_int(target_qps, suffix=' QPS')
```
...
class NoisyNeighborQuotaScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    slo_ms_str = fmt_int(slo_ms, commas=False, suffix=' ms')
    tenant_base_qps_str = fmt_int(tenant_base_qps, commas=False, suffix=' QPS')
    tenant_burst_qps_str = fmt_int(tenant_burst_qps, commas=False, suffix=' QPS')
    ...
    gpu_util_pct_str = fmt_int(70, commas=False, suffix=' percent')
```
...
The Hessian matrix $H = X^T X$ captures which weights most affect outputs. GPTQ builds on the Optimal Brain Surgeon (OBS) framework, using the inverse Hessian to guide error compensation.
...
class QuantizedServingCapacity:
    ...
    kv_seq_len = 4096
    kv_fp16_gb = (2 * kv_layers * kv_heads * kv_head_dim * kv_seq_len * BYTES_FP16.m_as(byte) * byte).m_as(GB)
    kv_per_1k_sequence_gb = kv_fp16_gb / (kv_seq_len / THOUSAND)
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    a100_mem_gb_str = fmt_int(a100_mem_gb, commas=False, suffix=' GB')
    weight_int4_gb_str = fmt_int(weight_int4_gb, commas=False, suffix=' GB')
    kv_headroom_gb_str = fmt_int(kv_headroom_gb, commas=False, suffix=' GB')
    kv_per_1k_sequence_gb_str = fmt(kv_per_1k_sequence_gb, precision=1, commas=False, suffix=' GB')
...
Continuous batching transformed LLM serving economics, but it is one component of a larger stack. The three case studies that follow show how recommendation, language model, and edge inference systems each assemble a different subset of the chapter's techniques to meet workload-specific cost and latency constraints.
...
class RankingCascadeCost:
    ...
    l3_per_candidate_ms_str = fmt_int(l3_per_candidate_ms, commas=False, suffix=' ms')
    l3_all_s_str = fmt_int(l3_all_s)
    cascade_budget_ms_str = fmt_int(cascade_budget_ms, commas=False, suffix=' ms')
