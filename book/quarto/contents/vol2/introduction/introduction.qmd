...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.A100.compute.peak_flops, Hardware.Cloud.A100.memory.bandwidth,
...
1.  **From processor to network**\index{Scale Moment!processor-to-network shift}: The **network wall**\index{Network Wall} (bisection bandwidth and speed of light) replaces the local memory wall as the primary performance bottleneck.
...
class Gpt4TrainingScenario:
    ...
    num_gpus = Models.Language.GPT4.training_accelerators_ref.m_as(count)
    gpu_days = Models.Language.GPT4.training_days_ref.m_as(day)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt4_gpus_str = fmt(num_gpus, precision=0, suffix=" GPUs")
    gpt4_days_str = fmt(gpu_days, precision=0)
    cluster_fail_day_str = fmt_int(cluster_failures_per_day)
    mtbf_hours_str = fmt(cluster_mtbf_hours, precision=0, suffix=" hours")
    gpu_mtbf_str = fmt(gpu_mtbf_hours, precision=0, suffix=" hours")
    annual_fails_str = fmt_int(annual_failures, commas=True)
    hours_per_year_str = fmt_int(hours_per_year, commas=True)
...
**Problem**: A training run for a GPT-4-class model uses `{python} Gpt4TrainingScenario.gpt4_gpus_str`. If each individual GPU has a mean time between failures (MTBF) of `{python} Gpt4TrainingScenario.gpu_mtbf_str` (the canonical datacenter-grade figure used throughout this volume), how often will the training job be interrupted by hardware failure?

**Math**:

1. **System MTBF**\index{MTBF!system}: For $N$ independent GPUs, $\text{MTBF}_{\text{system}} = \text{MTBF}_{\text{component}} / N =$ `{python} Gpt4TrainingScenario.gpu_mtbf_str` divided by `{python} Gpt4TrainingScenario.gpt4_gpus_str` $\approx$ `{python} Gpt4TrainingScenario.mtbf_hours_str`.
...
3. **Total annual failures**\index{Failure Rate!annualized count}: `{python} Gpt4TrainingScenario.gpt4_gpus_str` $\times$ (`{python} Gpt4TrainingScenario.hours_per_year_str` divided by `{python} Gpt4TrainingScenario.gpu_mtbf_str`) $\approx$ `{python} Gpt4TrainingScenario.annual_fails_str` failures per year.
...
Meta's published Llama 3 training run shows the same reliability arithmetic in a real frontier-scale system.
...
**Failure**: At that scale, failures were not rare disruptions. The run experienced 419 unexpected interruptions, averaging one interruption every few hours.

**Consequence**: Training at frontier scale became an exercise in continuous recovery: checkpointing, automated diagnosis, and restart workflows determined whether the cluster produced useful model progress or burned time waiting for humans.
...
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT3.parameters, Models.Language.GPT3.training_ops, GB,
# │          param, BILLION, BYTES_FP32), mlsysim.book (sci, check)
# │ Exports: gpt3_params_b_str, gpt3_training_ops_str, gpt3_gradient_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, fmt_sci, MarkdownStr, check
...
class Gpt3TrainingScenario:
    ...
    ops = Models.Language.GPT3.training_ops.m_as(Models.Language.GPT3.training_ops.units)
...
: **Training Compute Evolution**\index{Training!compute evolution}: Growth in hardware scale and training duration across published landmarks and an illustrative GPT-4-class scenario, showing why distributed systems become central as training budgets move from $10^{18}$-scale to frontier-scale runs. {#tbl-training-compute-evolution}
...
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (Systems.Fabrics.InfiniBand_HDR.bandwidth, Systems.Fabrics.InfiniBand_NDR.bandwidth,
# │          Gbps, BITS_PER_BYTE)
# │ Exports: ib_hdr_gbps_str, ib_hdr_gbs_str, ib_ndr_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
...
# │ Exports: InfiniBandStackRecap.nvlink_h100_gbs_str, InfiniBandStackRecap.ib_ndr_gbps_str
class InfiniBandStackRecap:
    ...
    nvlink_h100_gbs_str = fmt(nvlink_h100_gbs_val, precision=0, commas=False, suffix=" GB")
    ib_ndr_gbps_str = fmt(ndr_gbps, precision=0, commas=False, suffix=" Gb")
```

The stack architecture in @fig-vol2-system-scaling-regimes does not change when we scale: every ML system still has hardware, a system envelope, a workload, and a mission. *What* changes is the physics at each layer. Read the figure from bottom to top. At the bottom row, Hardware (NVLink at `{python} InfiniBandStackRecap.nvlink_h100_gbs_str`/s within one node) becomes Infrastructure (InfiniBand RDMA fabric spanning racks at `{python} InfiniBandStackRecap.ib_ndr_gbps_str`/s per link), and the bottleneck shifts from the memory wall to the bisection bandwidth wall. One row up, System Software (a single CUDA runtime managing PCIe DMA) becomes Distribution (NCCL and RDMA libraries coordinating thousands of processes across the fabric).
...
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT3.parameters, GB, param, BILLION, BYTES_FP32)
...
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT3.parameters, Models.Language.GPT3.training_ops, Hardware.Cloud.A100.compute.peak_flops,
...
class Gpt3StatsScenario:
    ...
    params = Models.Language.GPT3.parameters.m_as(param)
    ops = Models.Language.GPT3.training_ops.m_as(Models.Language.GPT3.training_ops.units)
    a100_throughput = Hardware.Cloud.A100.compute.peak_flops.m_as(TFLOPs/second)
    ...
    gpt3_params_b_str = fmt(params_b, precision=0, commas=False)
    gpt3_training_ops_str = MarkdownStr(f"{fmt_sci(ops)} FLOPs")
    a100_fp16_tflops_str = fmt(a100_throughput, precision=0, commas=False, suffix=" TFLOP/s")
```
...
@Fig-compute-optimal illustrates this principle through three related views. The left panel shows IsoFLOP curves\index{IsoFLOP Curve} where each curve corresponds to a constant number of floating-point operations (FLOPs[^fn-flops-work-vs.-rate]) during transformer training. The valleys in these curves identify the most efficient model size for each computational budget. The center and right panels reveal how the optimal number of parameters and tokens scales predictably as computational budgets increase, confirming that coordinated scaling maximizes resource utilization.\index{Compute-Optimal Training!resource allocation}

[^fn-flops-work-vs.-rate]: **FLOPs vs. FLOP/s**\index{FLOP/s vs. FLOPs}: A critical distinction: FLOPs denotes total computational *work* (operations performed), while FLOP/s denotes hardware *throughput* (operations per second). Confusing the two leads to incorrect cost estimates. GPT-3 required `{python} Gpt3StatsScenario.gpt3_training_ops_str` of work; an A100 delivers `{python} Gpt3StatsScenario.a100_fp16_tflops_str` of throughput. Dividing work by peak throughput gives ideal wall-clock time; actual time is approximately $\text{work}/(\eta_{\text{hw}} R_{\text{peak}})$, so utilization below one multiplies ideal time by $1/\eta_{\text{hw}}$. \index{FLOP/s!work vs. throughput}
...
Machine learning systems already face the **Verification Invariant**\index{Verification Invariant}: the impossibility of testing a high-dimensional model against every possible input. At fleet scale, a more physical challenge emerges: the **reliability gap**\index{Reliability Gap!definition}.
...
class ReliabilityGap:
    ...
    prob_1k_str = fmt(p_fleet_small, precision=1, suffix=" percent")
    prob_10k_str = fmt(p_fleet_large, precision=4, suffix=" percent")
```
...
The **CAP Theorem**[^fn-cap-theorem-ml]\index{CAP Theorem} establishes that distributed systems can provide at most two of three properties: **Consistency**\index{CAP Theorem!consistency}, **Availability**, and **Partition Tolerance**. The ML fleet encounters all three constraints, forcing explicit trade-offs.
...
The single-node D·A·M lens reviewed in @sec-appdx-dam-taxonomy optimizes the workload on one node. This volume projects that D·A·M workload onto a distributed fleet. *When* we stretch the nouns across thousands of machines, we are forced to pay the verbs:
...
The C$^3$ taxonomy yields a diagnostic equation for every distributed training step. On a single machine, execution time decomposes into data movement, computation, and overhead. At fleet scale, a parallel decomposition emerges. @Eq-fleet-law states the **Fleet Law**\index{Fleet Law}, which decomposes distributed execution into its canonical distributed-step components:
...
To reason about performance at scale, we must distinguish between the efficiency of a single component and the efficiency of the entire system. On a single machine, execution time is often governed by the iron law of performance: $T \approx D_{\text{vol}}/\text{BW} + O/(R_{\text{peak}}\eta_{\text{hw}}) + L_{\text{lat}}$. At production scale, this law undergoes a transformation. We no longer care merely about the performance of a single machine; we care about the **Efficiency of the Fleet**\index{Efficiency!distributed} across two primary dimensions: **Time** and **Energy**\index{Energy}\index{Efficiency!energy dimension}.

### The law of distributed efficiency {#sec-introduction-law-distributed-efficiency-e440}

By direct analogy with the single-machine iron law, the Fleet Law (@eq-fleet-law) is also called the **Iron Law of Scale**\index{Iron Law!of scale, equation}, or equivalently the law of distributed efficiency. It is the same decomposition introduced above; restated here with each term named explicitly, it quantifies the time required for a single training step across $N$ devices. @Sec-appdx-c3-taxonomy-fleet-law develops the full derivation of the Fleet Law and its decomposition across Computation, Communication, and Coordination, so a reader can attribute each term to a physical cause:

$$ T_{\text{step}}(N) = \frac{T_{\text{compute}}}{N} + T_{\text{comm}}(N) + T_{\text{sync}}(N) - T_{\text{overlap}} $$ {#eq-iron-law-scale}

where:

1.  **Parallel compute ($\frac{T_{\text{compute}}}{N}$)**: The ideal scaling of computation, where $T_{\text{compute}}$ denotes the single-device compute time for the same work before partitioning. As we add $N$ devices, the work per device should ideally shrink linearly.
...
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT3.parameters, Systems.Fabrics.InfiniBand_HDR.bandwidth, Gbps,
...
class GPT3SyncTax:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    params = Models.Language.GPT3.parameters.m_as('param')
    bw_ib = Systems.Fabrics.InfiniBand_HDR.bandwidth.m_as('byte/second') # HDR InfiniBand
    ...
    sync_energy_mj = sync_energy_joules * million_steps / MILLION
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    sync_size_gb_str = fmt((sync_size_bytes * byte).m_as(GB), precision=0, suffix=" GB")
    ib_efficiency_pct_str = fmt(eff_ib, precision=1, suffix=" percent")
    eth_efficiency_pct_str = fmt(eff_eth, precision=1, suffix=" percent")
    # Formatting energy in Joules (J) for scale impact
    sync_energy_j_str = fmt(sync_energy_joules, precision=0)
...
**Physics**:
To synchronize 175B parameters (FP16), a Ring All-Reduce\index{Ring All-Reduce} must move approximately `{python} GPT3SyncTax.sync_size_gb_str` of data through each participating accelerator endpoint per iteration.

**The comparison (iron law of scale)**:

*   **InfiniBand (200 Gbps)**: High bandwidth and low latency yield a scaling efficiency of only `{python} GPT3SyncTax.ib_efficiency_pct_str`, meaning the accelerators compute for just 4 out of every 100 seconds while the rest is consumed by synchronization.
*   **Ethernet (100 Gbps)**: Lower bandwidth and higher overhead collapse the scaling efficiency further to `{python} GPT3SyncTax.eth_efficiency_pct_str`, leaving the accelerators productive for roughly 2 out of every 100 seconds.
...
This textbook organizes around the Fleet Stack\index{Fleet Stack!textbook structure}, progressing from the physical substrate through the logic of distribution to societal governance. The chapters form a continuous, causal chain: the physical constraints of silicon and cooling dictate how we wire the network; the network limits dictate how we partition the training algorithms; the partitioned algorithms dictate how we handle fault tolerance and orchestration; and orchestrating the fleet dictates how we operate, secure, and govern it in the real world. Each Part addresses a fundamental "Scale Impediment" that prevents a single-machine solution from working at production scale.
...
*   **Inference at Scale (@sec-inference-scale)**: The interface. Serving models to millions of users simultaneously while managing the "KV Cache Wall" (the memory bottleneck of autoregressive generation).
...
class EdgeLatencyDistance:
    ...
    delay_ms_str = fmt(delay_ms, precision=0, suffix=" ms")
