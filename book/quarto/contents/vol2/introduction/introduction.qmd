...
The scaling efficiency ($\eta_{\text{scaling}}$) of the fleet is the ratio of ideal parallel compute to the actual step time:
$$ \eta_{\text{scaling}} = \frac{T_{\text{compute}}}{N \times T_{\text{step}}} $$
...
Each gradient synchronization consumes approximately **`{python} GPT3SyncTax.sync_energy_j_str` Joules** of energy across the network fabric (at 15 pJ/bit). In a training run of 1 million steps, the network movement alone consumes about 84 MJ of energy.
