...
# │ Show: "~700" GB sync size, "~4%" IB efficiency, "~2%" Ethernet efficiency,
# │       "~84" J per synchronization, and "~84" MJ for 1M synchronizations.
...
**Physics**:
To synchronize 175B parameters (FP16), a Ring All-Reduce\index{Ring All-Reduce} must move approximately **`{python} GPT3SyncTax.sync_size_gb_str` GB** of data across the network per iteration.
...
Each gradient synchronization consumes approximately **`{python} GPT3SyncTax.sync_energy_j_str` Joules** of energy across the network fabric (at 15 pJ/bit). In a training run of `{python} GPT3SyncTax.million_steps_str` steps, the network movement alone consumes about `{python} GPT3SyncTax.sync_energy_mj_str` MJ of energy.
