...
class Gpt4TrainingScenario:
    ...
    num_gpus = GPT4_CLASS_PUBLIC_ESTIMATE_GPU_COUNT_REF.m_as(count)
    gpu_days = GPT4_CLASS_PUBLIC_ESTIMATE_TRAINING_DAYS_REF.m_as(day)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpt4_gpus_str = fmt(num_gpus, precision=1, allow_zero=True)
    gpt4_days_str = fmt(gpu_days, precision=1, allow_zero=True)
    cluster_fail_day_str = fmt(cluster_failures_per_day, precision=1, allow_zero=True)
    mtbf_hours_str = fmt(cluster_mtbf_hours, precision=1, allow_zero=True)
    gpu_mtbf_str = fmt(gpu_mtbf_hours, precision=1, allow_zero=True)
    annual_fails_str = fmt(annual_failures, precision=1, allow_zero=True)
    hw_name_str = MarkdownStr(GPT4_CLASS_PUBLIC_ESTIMATE_HARDWARE_LABEL)
```
...
1. **System MTBF**\index{MTBF!system}: For $N$ independent GPUs, $\text{MTBF}_{\text{system}} = \text{MTBF}_{\text{component}} / N =$ `{python} Gpt4TrainingScenario.gpu_mtbf_str` $/$ `{python} Gpt4TrainingScenario.gpt4_gpus_str` $\approx$ **`{python} Gpt4TrainingScenario.mtbf_hours_str` hours**.
...
3. **Total annual failures**\index{Failure Rate!annualized count}: `{python} Gpt4TrainingScenario.gpt4_gpus_str` GPUs $\times$ (8{,}760 hours $/$ `{python} Gpt4TrainingScenario.gpu_mtbf_str` hours) $\approx$ **`{python} Gpt4TrainingScenario.annual_fails_str` failures per year**.
...
class Gpt3TrainingScenario:
    ...
    gpt3_gradient_gb_str = fmt(gradient_gb, precision=1, allow_zero=True, commas=False)
```
...
class InfiniBandScenario:
    ...
    ib_ndr_gbps_str = fmt(ndr_gbps, precision=1, allow_zero=True, commas=False)
    ib_hdr_gbs_str = fmt(hdr_gbs, precision=1, allow_zero=True, commas=False)
    ib_ndr_gbs_str = fmt(ndr_gbs, precision=1, allow_zero=True, commas=False)
...
class Gpt3StatsScenario:
    ...
    gpt3_params_b_str = fmt(params_b, precision=1, allow_zero=True, commas=False)
    gpt3_training_ops_str = fmt_sci(ops)
    a100_fp16_tflops_str = fmt(a100_throughput, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (Models.Language.GPT3.parameters, INFINIBAND_HDR_BW, Gbps,
...
class GPT3SyncTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    sync_size_gb_str = fmt((sync_size_bytes * byte).m_as(GB), precision=1, allow_zero=True)
    ib_efficiency_pct_str = fmt(eff_ib, precision=1, allow_zero=True)
    eth_efficiency_pct_str = fmt(eff_eth, precision=1, allow_zero=True)
    # Formatting energy in Joules (J) for scale impact
    sync_energy_j_str = fmt(sync_energy_joules, precision=1)
    million_steps_str = fmt(million_steps, precision=1, allow_zero=True)
    sync_energy_mj_str = fmt(sync_energy_mj, precision=1, allow_zero=True)
```
...
class EdgeLatencyDistance:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    speed_kmh_str = fmt(speed_kmh, precision=1, allow_zero=True)
    delay_ms_str = fmt(delay_ms, precision=1, allow_zero=True)
...
class LinearScalingFallacy:
    ...
    # ┌── 4. OUTPUT (Formatting) ────────────────────────────────────────────
    ideal_speedup_str = fmt(ideal_speedup, precision=1, allow_zero=True)
    coord_overhead_pct_str = fmt(coord_overhead_fraction * 100, precision=1, allow_zero=True)
