...
# │       50,000-hour per-GPU MTBF (GPU_MTTF_HOURS),
...
# │          Gpt4TrainingScenario.gpu_mtbf_str,
...
class Gpt4TrainingScenario:
    ...
    # GPU_MTTF_HOURS is the legacy constant name; use it as per-GPU MTBF here.
    gpu_mtbf_hours = GPU_MTTF_HOURS
    ...
    gpu_mtbf_str = fmt(gpu_mtbf_hours, precision=0)
    annual_fails_str = fmt(annual_failures, precision=0)
    hw_name_str = MarkdownStr("A100-class")
```

Consider a GPT-4-class training scenario using a hypothetical **`{python} Gpt4TrainingScenario.gpt4_gpus_str` `{python} Gpt4TrainingScenario.hw_name_str` GPU** cluster running for **`{python} Gpt4TrainingScenario.gpt4_days_str` days**. These values are illustrative rather than disclosed by OpenAI [@openai2023gpt4]. In a cluster of this size, the probability of at least one failure over an interval $t$, $p_{\text{fail}}(t) = 1 - e^{-t/\text{MTBF}_{\text{cluster}}}$, becomes the dominant constraint.

::: {#nbk-introduction-scale-reliability .callout-notebook title="Scale and reliability"}

**Problem**: A training run for a GPT-4-class model uses `{python} Gpt4TrainingScenario.gpt4_gpus_str` GPUs. If each individual GPU has a mean time between failures (MTBF) of `{python} Gpt4TrainingScenario.gpu_mtbf_str` hours (the canonical datacenter-grade figure used throughout this volume), how often will the training job be interrupted by hardware failure?

**Math**:

1. **Cluster MTBF**\index{MTBF!cluster}: For $N_{\text{GPU}}$ independent GPUs, $\text{MTBF}_{\text{cluster}} = \text{MTBF}_{\text{GPU}} / N_{\text{GPU}} =$ `{python} Gpt4TrainingScenario.gpu_mtbf_str` $/$ `{python} Gpt4TrainingScenario.gpt4_gpus_str` $\approx$ **`{python} Gpt4TrainingScenario.mtbf_hours_str` hours**.
...
3. **Total annual failures**\index{Total Annual Failures}: `{python} Gpt4TrainingScenario.gpt4_gpus_str` GPUs $\times$ (8{,}760 hours $/$ `{python} Gpt4TrainingScenario.gpu_mtbf_str` hours) $\approx$ **`{python} Gpt4TrainingScenario.annual_fails_str` failures per year**.
...
- [ ] Can you explain why **scaling efficiency**\index{Scaling Efficiency} decreases as you add more nodes ($N_{\text{nodes}}$)?
...
3.  **Coordination ($C_3$ - The Logic)**\index{C³ Taxonomy!coordination}: The synchronous management of state across thousands of nodes. This is the domain of collective algorithms (All-Reduce), fault tolerance, and distributed consensus. Coordination is the "Software Tax" that determines how efficiently $N_{\text{nodes}}$ independent nodes can act as a single computer.
...
The **Iron Law of Scale**\index{Iron Law!of scale, equation} (also known as the Law of Distributed Efficiency) quantifies the time required for a single training step across $N_{\text{dev}}$ devices:

$$ T_{\text{step}}(N_{\text{dev}}) = \frac{T_{\text{compute},1}}{N_{\text{dev}}} + T_{\text{comm}}(N_{\text{dev}}) + T_{\text{sync}}(N_{\text{dev}}) - T_{\text{overlap}} $$ {#eq-iron-law-scale}

where:

1.  **Parallel compute ($\frac{T_{\text{compute},1}}{N_{\text{dev}}}$)**: The ideal scaling of computation, where $T_{\text{compute},1}$ denotes the single-device compute time for the same work before partitioning. As we add $N_{\text{dev}}$ devices, the work per device should ideally shrink linearly.
2.  **Communication overhead ($T_{\text{comm}}(N_{\text{dev}})$)**: The network data-movement cost. As $N_{\text{dev}}$ grows, the time spent moving gradients or activations across links grows or stays constant, threatening to dominate the step time.
3.  **Synchronization overhead ($T_{\text{sync}}(N_{\text{dev}})$)**: The coordination cost of barriers, straggler waiting, ordering, and consistency logic that makes the distributed step behave like one coherent update.
...
where $O_{\text{useful}}$ is useful work in FLOPs and $E_{\text{network}}$ often becomes a nonnegligible fraction of the total budget as we move terabytes across optical fabrics. Mastery of scale requires optimizing for the Pareto frontier\index{Pareto Frontier} of both laws: minimizing $T_{\text{step}}$ while maximizing $\rho_{\text{energy}}$.
...
where $T_1$ is the single-device time for the same work and $T_N = T_{\text{step}}(N)$ is the step time on $N$ devices.
...
class GPT3SyncTax:
    ...
    bw_eth = (100 * Gbps).m_as('byte/second') # 100 Gb/s Ethernet
...
**Problem**: Calculate the scaling efficiency and energy cost of training GPT-3 (175B params) on a cluster connected by 100 Gb/s Ethernet vs. 200 Gb/s InfiniBand.
