...
*   **InfiniBand (200 Gbps)**: High bandwidth and low latency yield a scaling efficiency of **`{python} GPT3SyncTax.ib_efficiency_pct_str`**. Even so, most of the step is still spent waiting on synchronization.
*   **Ethernet (100 Gbps)**: Lower bandwidth and higher overhead collapse the scaling efficiency to **`{python} GPT3SyncTax.eth_efficiency_pct_str`**. The fleet spends even more of its time waiting for the network.
