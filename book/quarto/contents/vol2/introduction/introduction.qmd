...
Large-scale systems also have a societal property that small models do not: their impact is amplified by the billions of users they serve. *When* a local model exhibits bias, the harm is contained; *when* a foundation model exhibits bias, it propagates that harm through the fabric of digital life. The discipline this volume builds is the **physics of distribution**\index{Physics of Distribution}, where decisions at the algorithmic level must account for network topology, fault tolerance, and the security of the global "control plane."
...
# │       (GPT3_PARAMS × 4 bytes), cluster MTBF for 25 k A100s at the canonical
# │       50,000-hour per-GPU MTTF (GPU_MTTF_HOURS),
# │       and accelerator peak throughput (A100_FLOPS_FP16_TENSOR,
...
# │ Exports: a100_fp16_tflops_str, a100_mem_bw_gbs, a100_mem_bw_tbs,
# │          h100_fp8_tflops, v100_mem_bw_gbs, tpuv4_bf16_tflops,
# │          nvlink_h100_gbs_str, ib_hdr_gbps_str, ib_ndr_gbps_str,
# │          ib_hdr_gbs_str, ib_ndr_gbs_str,
...
1.  **From processor to network**\index{From Processor to Network}: The **network wall**\index{Network Wall} (bisection bandwidth and speed of light) replaces the local memory wall as the primary performance bottleneck.
...
# │ How: cluster_MTBF = GPU_MTTF / N; fails_per_day = 24 / cluster_MTBF.
# │
...
# │          Gpt4TrainingScenario.gpu_mttf_str,
...
class Gpt4TrainingScenario:
    ...
    gpu_days = 90    # Illustrative frontier-class training window
    hours_per_year = 8760

    # Per-GPU MTTF is the canonical reliability constant for the book
    # (datacenter-grade GPU, steady-state). MTBF_cluster = MTTF_gpu / N.
    gpu_mttf_hours = GPU_MTTF_HOURS

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    cluster_mtbf_hours = gpu_mttf_hours / num_gpus
    cluster_failures_per_day = 24 / cluster_mtbf_hours
    annual_failures = num_gpus * (hours_per_year / gpu_mttf_hours)
    ...
    cluster_fail_day_str = fmt(cluster_failures_per_day, precision=0)
    mtbf_hours_str = fmt(cluster_mtbf_hours, precision=0)
    gpu_mttf_str = fmt(gpu_mttf_hours, precision=0)
    annual_fails_str = fmt(annual_failures, precision=0)
    hw_name_str = MarkdownStr("A100-class")
```

Consider a GPT-4-class training scenario using a hypothetical **`{python} Gpt4TrainingScenario.gpt4_gpus_str` `{python} Gpt4TrainingScenario.hw_name_str` GPU** cluster running for **`{python} Gpt4TrainingScenario.gpt4_days_str` days**. These values are illustrative rather than disclosed by OpenAI [@openai2023gpt4]. In a cluster of this size, the probability of failure ($p_{\text{fail}}$) becomes the dominant constraint.

::: {#nbk-introduction-scale-reliability .callout-notebook title="Scale and reliability"}

**Problem**: A training run for a GPT-4-class model uses `{python} Gpt4TrainingScenario.gpt4_gpus_str` GPUs. If each individual GPU has a Mean Time To Failure (MTTF) of `{python} Gpt4TrainingScenario.gpu_mttf_str` hours (the canonical datacenter-grade figure used throughout this volume), how often will the training job be interrupted by hardware failure?

**Math**:

1. **Cluster MTBF**\index{MTBF!cluster}: For $N$ independent GPUs, $\text{MTBF}_{\text{cluster}} = \text{MTTF}_{\text{GPU}} / N =$ `{python} Gpt4TrainingScenario.gpu_mttf_str` $/$ `{python} Gpt4TrainingScenario.gpt4_gpus_str` $\approx$ **`{python} Gpt4TrainingScenario.mtbf_hours_str` hours**.
2. **Daily failure rate**\index{Daily Failure Rate}: $24 \text{ hours} /$ `{python} Gpt4TrainingScenario.mtbf_hours_str` $\text{ hours} \approx$ **`{python} Gpt4TrainingScenario.cluster_fail_day_str` failures per day**.
3. **Total annual failures**\index{Total Annual Failures}: `{python} Gpt4TrainingScenario.gpt4_gpus_str` GPUs $\times$ (8{,}760 hours $/$ `{python} Gpt4TrainingScenario.gpu_mttf_str` hours) $\approx$ **`{python} Gpt4TrainingScenario.annual_fails_str` failures per year**.
...
::: {#ws-vol2-introduction-llama3-failure-cadence .callout-war-story title="The training run that failed every few hours"}

**The context**: Meta trained Llama 3 405B on 16,384 H100 GPUs for a 54-day final training run.

**The failure**: At that scale, failures were not rare disruptions. The run experienced 419 unexpected interruptions, averaging one interruption every few hours.

**The consequence**: Training at frontier scale became an exercise in continuous recovery: checkpointing, automated diagnosis, and restart workflows determined whether the cluster produced useful model progress or burned time waiting for humans.

**The systems lesson**: The scale moment turns reliability from a hardware specification into a distributed-systems design requirement. A fleet-scale model is trained by the recovery machinery as much as by the optimizer [@dubey2024llama].
...
# │
# │ Imports: mlsysim.core.constants (GPT3_PARAMS, GPT3_TRAINING_OPS, GB,
# │          param, BILLION, BYTES_FP32), mlsysim.book (sci, check)
# │ Exports: gpt3_params_b_str, gpt3_training_ops_str, gpt3_gradient_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GPT3_PARAMS, GPT3_TRAINING_OPS, GB, param, BILLION, BYTES_FP32
from mlsysim.fmt import fmt, fmt_sci, check
...
class Gpt3TrainingScenario:
    ...
    gradient_gb = (params * BYTES_FP32).m_as(GB) # FP32 gradients
    ...
    gpt3_training_ops_str = fmt_sci(ops)
    gpt3_gradient_gb_str = fmt(gradient_gb, precision=0, commas=False)
```

GPT-3 consumed an estimated `{python} Gpt3TrainingScenario.gpt3_training_ops_str` FLOPs during training, requiring a roughly thousand-GPU-scale V100 training run for about a month [@brown2020language]. The surrounding Microsoft infrastructure for this era reached roughly 10,000 GPUs, which is the cluster-size anchor used in @fig-cluster-size-explosion [@patterson2021carbon]. PaLM trained on 6,144 TPU v4 chips for roughly 60 days, consuming approximately $10^{24}$ FLOPs [@chowdhery2022palm]. Public GPT-4-class training estimates often use approximately 25,000 A100 GPUs over 90--100 days [@openai2023gpt4]. This progression spans roughly seven orders of magnitude in training compute over a single decade, from roughly $10^{18}$ FLOPs for AlexNet to $10^{25}$ FLOPs for GPT-4 [@sevilla2022; @amodei2018ai].

::: {#exmp-introduction-training-compute-evolution .callout-example title="Training compute evolution"}
...
| **GPT-3**\index{GPT-3}     |     2020 | ~1,024 V100 GPUs (run est.) |      ~30--34 days |          ~$10^{23}$ |
| **PaLM**                   |     2022 |                  6,144 TPUs |          ~60 days |          ~$10^{24}$ |
| **GPT-4**                  |     2023 |                ~25,000 GPUs |         ~100 days |          ~$10^{25}$ |
...
Three recurring workloads ground these abstract principles in concrete, quantifiable cases throughout the volume.

::: {#lhs-introduction-lighthouse-archetypes-at-scale .callout-lighthouse title="Lighthouse archetypes at scale"}

**Lighthouse Archetypes** are canonical workloads that we track throughout the volume, examining their behavior when distributed across thousands of devices. The full roster with the C$^3$ taxonomy mapping and the binding-constraint analysis appears at @sec-vol2-introduction-archetypes.
...
*   **Archetype B (DLRM at Scale)**—the Deep Learning Recommendation Model (DLRM) workload: We move from fitting embedding tables in memory to **Embedding Sharding**\index{Embedding Sharding} across hundreds of nodes, creating massive all-to-all communication bottlenecks.
...
# │          Gbps, BITS_PER_BYTE)
# │ Exports: ib_hdr_gbps_str, ib_ndr_gbps_str, ib_hdr_gbs_str, ib_ndr_gbs_str,
# │          nvlink_h100_gbs_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import INFINIBAND_HDR_BW, INFINIBAND_NDR_BW, NVLINK_H100_BW, Gbps, GB, second, BITS_PER_BYTE

class InfiniBandScenario:
    """InfiniBand and NVLink bandwidth reference."""
    ...
    nvlink_h100_gbs_val = nvlink_h100.m_as(GB/second)
...
The stack architecture in @fig-vol2-system-scaling-regimes does not change when we scale: every ML system still has hardware, a system envelope, a workload, and a mission. *What* changes is the physics at each layer. Read the figure from bottom to top. At the bottom row, Hardware (NVLink at `{python} InfiniBandScenario.nvlink_h100_gbs_str` GB/s within one node) becomes Infrastructure (InfiniBand RDMA fabric spanning racks at `{python} InfiniBandScenario.ib_ndr_gbps_str` Gb/s per link), and the bottleneck shifts from the memory wall to the bisection bandwidth wall. One row up, System Software (a single CUDA runtime managing PCIe DMA) becomes Distribution (NCCL and RDMA libraries coordinating thousands of processes across the fabric).
...
3.  **Serving/Ops** (Workloads, the Route): The orchestration layer. This layer manages the mathematical workload sharded across the cluster $(O, D_{\text{vol}}, \text{CI})$ through CI/CD pipelines and scheduling. We use Lighthouse Workloads like GPT-4 and DLRM.
...
The Machine Learning Fleet, by contrast, operates under **Synchronous Tight Coupling**. While the parameter server architecture [@li2014] introduced ways to manage distributed state, modern frontier models often require even tighter synchronization to maintain performance.
...
@fig-traditional-vs-ml-fleet illustrates this contrast directly: in MapReduce, workers write independently to shared storage and a straggler delays only its own partition, whereas in the ML Fleet, every worker must arrive at an AllReduce barrier before the training step can proceed.\index{Barrier Synchronization vs. Independent Tasks}
...
# │ How: pulling constants from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (GPT3_PARAMS, GB, param, BILLION, BYTES_FP32)
# │ Exports: Gpt3CommunicationScenario.gpt3_params_b_str,
# │          Gpt3CommunicationScenario.gpt3_gradient_gb_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import GPT3_PARAMS, GB, param, BILLION, BYTES_FP32
from mlsysim.fmt import fmt, check
...
At large scale, *communication dominates*. Distributed training requires synchronizing gradients across workers after each batch. For a model with `{python} Gpt3CommunicationScenario.gpt3_params_b_str` billion parameters, FP32 gradients occupy about `{python} Gpt3CommunicationScenario.gpt3_gradient_gb_str` GB before any collective algorithm is applied. In Ring All-Reduce, each worker sends and receives roughly $2(N-1)/N$ times the gradient tensor size, so network traffic depends on precision, worker count, and collective implementation. *When* using Ring All-Reduce across 1,000 workers on InfiniBand, communication can consume up to 40 percent of the total iteration time.

This ratio explains *why* distributed training systems optimize communication so aggressively. Frameworks like **Horovod**\index{Horovod} [@sergeev2018horovod], **Megatron-LM**\index{Megatron-LM} [@shoeybi2019megatron], and **ZeRO**\index{ZeRO} [@rajbhandari2020] introduce gradient compression, model parallelism, and memory optimization to overcome these bottlenecks. At fleet scale, these techniques are requirements for viability, not optional performance improvements.
...
Training GPT-3 consumed roughly $3 \times 10^{23}$ floating-point operations. Public GPT-4-class training estimates are often on the order of $10^{25}$ FLOPs. Each order-of-magnitude increase in compute delivered measurable improvements in model capability, and each demanded a corresponding expansion of the Machine Learning Fleet. This pattern is not coincidental: it follows from the universal scaling law\index{Universal Scaling Law} (Principle \ref{pri-universal-scaling}), which relates model performance to resource investment through predictable power-law relationships.

Rich Sutton's "bitter lesson" articulated the underlying principle: performance in machine learning is primarily driven by applying general methods at massive scale rather than encoding human knowledge into algorithms. Scaling laws formalize this observation quantitatively: model loss improves sublinearly as a power-law function of compute, dataset size, and parameters, $\mathcal{L}(X) \propto X^{-\alpha}$. Each equal-sized loss improvement therefore requires disproportionately more resources. This exponential hunger drives the transition from single-server training to warehouse-scale clusters.
...
# │          Gpt3StatsScenario.a100_fp16_tflops_str
# └─────────────────────────────────────────────────────────────────────────────
...
class Gpt3StatsScenario:
    ...
    gpt3_params_b_str = fmt(params_b, precision=0, commas=False)
    gpt3_training_ops_str = fmt_sci(ops)
    a100_fp16_tflops_str = fmt(a100_throughput, precision=0, commas=False)
```

GPT-3 (2020) expanded to `{python} Gpt3StatsScenario.gpt3_params_b_str` billion parameters and achieved sophisticated text generation across diverse domains. Each increase in model size brought substantially improved capabilities at exponentially increasing costs.
...
These resource demands reveal *why* scaling laws are necessary for efficient resource allocation. @fig-compute-trends traces *how* computational demands of training frontier models have escalated at an unsustainable rate, growing faster than Moore's Law improvements in hardware.\index{Model Training Compute Trends}
...
@fig-compute-optimal illustrates this principle through three related views. The left panel shows IsoFLOP curves\index{IsoFLOP Curve} where each curve corresponds to a constant number of floating-point operations (FLOPs[^fn-flops-work-vs.-rate]) during transformer training. The valleys in these curves identify the most efficient model size for each computational budget. The center and right panels reveal how the optimal number of parameters and tokens scales predictably as computational budgets increase, confirming that coordinated scaling maximizes resource utilization.\index{Optimal Compute Allocation}

[^fn-flops-work-vs.-rate]: **FLOPs vs. FLOPS**\index{FLOPS vs. FLOPs}: A critical distinction: FLOPs (lowercase 's') measures total computational *work* (operations performed), while FLOPS (uppercase 'S') measures hardware *throughput* (operations per second). Confusing the two leads to incorrect cost estimates. GPT-3 required `{python} Gpt3StatsScenario.gpt3_training_ops_str` FLOPs of work; an A100 delivers `{python} Gpt3StatsScenario.a100_fp16_tflops_str` TFLOPS of throughput. Dividing work by peak throughput gives ideal wall-clock time; actual time is approximately $\text{work}/(\eta_{\text{hw}} R_{\text{peak}})$, so utilization below one multiplies ideal time by $1/\eta_{\text{hw}}$. \index{FLOPS!work vs. throughput}
...
::: {#thrm-introduction-formal-mathematical-formulation .callout-theorem title="Formal mathematical formulation"}
...
These predictions find strong empirical support across multiple model configurations. @fig-loss-vs-n-d shows *how* early-stopped test loss varies predictably with both dataset size and model size, confirming that learning curves across configurations align through appropriate parameterization.\index{Loss vs. Dataset Size Across Model Scales}
...
[^fn-scaling-saturation-qualifier]: **Scaling Law Saturation**\index{Scaling!law saturation}: While power-law relationships suggest unbounded gain, empirical evidence identifies **Semantic Saturation**\index{Power-Law Relationship}\index{Semantic Saturation} points where adding data or parameters yields negligible improvement in downstream utility. This "diminishing returns" regime forces systems engineers to pivot from brute-force scale to data efficiency\index{Data Efficiency} and model compression to extract further value within fixed power budgets. \index{Scaling Laws!saturation}
...
In traditional software, we treat hardware as a reliable abstraction. A single server typically has an availability of "four nines" (99.99 percent), meaning it fails for roughly 53 minutes a year. The Machine Learning Fleet, however, operates at a scale where this abstraction collapses. *When* a fleet coordinates 25,000 GPUs, @eq-reliability-gap multiplies the individual node probabilities to give the probability that the entire system is healthy ($R_{\text{fleet}}$):

$$ R_{\text{fleet}} = (R_{\text{node}})^N $$ {#eq-reliability-gap}
...
class ReliabilityGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_reliability_str = fmt(p_node * 100, precision=1, commas=False, suffix=" percent")
...
#### Security and the fleet threat {#sec-introduction-security-fleet-threat-e320}
...
#### The regulatory wall {#sec-introduction-regulatory-wall-6d74}
...
#### Responsibility as an invariant {#sec-introduction-responsibility-invariant-c7a3}
...
1.  **Computation ($C_1$ - The Math)**\index{C³ Taxonomy!computation}: The local execution of matrix operations on individual accelerators. This is the domain of TFLOPS, memory bandwidth, and arithmetic intensity. At scale, the goal is to keep the math engine running at peak utilization.
...
The single-node D·A·M lens reviewed in @sec-dam-taxonomy optimizes the workload on one node. This volume projects that D·A·M workload onto a distributed fleet. *When* we stretch the nouns across thousands of machines, we are forced to pay the verbs:
...
### The law of distributed efficiency {#sec-introduction-law-distributed-efficiency-e440}
...
$$ T_{\text{step}}(N) = \frac{T_{\text{compute},1}}{N} + T_{\text{comm}}(N) + T_{\text{sync}}(N) - T_{\text{overlap}} $$ {#eq-iron-law-scale}

where:

1.  **Parallel compute ($\frac{T_{\text{compute},1}}{N}$)**: The ideal scaling of computation, where $T_{\text{compute},1}$ denotes the single-device compute time for the same work before partitioning. As we add $N$ devices, the work per device should ideally shrink linearly.
2.  **Communication overhead ($T_{\text{comm}}(N)$)**: The network data-movement cost. As $N$ grows, the time spent moving gradients or activations across links grows or stays constant, threatening to dominate the step time.
3.  **Synchronization overhead ($T_{\text{sync}}(N)$)**: The coordination cost of barriers, straggler waiting, ordering, and consistency logic that makes the distributed step behave like one coherent update.
4.  **Communication hiding ($T_{\text{overlap}}$)**: The efficiency gain. Advanced systems overlap communication with the computation of the next layer to hide part of the communication and synchronization tax.

### The energy-scale invariant {#sec-introduction-energyscale-invariant-a0d6}

While the iron law of scale governs time, the **Energy-Scale Invariant**\index{Energy-Scale Invariant} governs the sustainability and economic viability of the fleet. At scale, every training step is a thermodynamic event. @eq-energy-scale-invariant defines the **Fleet Energy Efficiency** ($\eta_{p}$) as the ratio of useful work to total energy consumed:

$$ \eta_{p} = \frac{\text{Work (FLOPs)}}{E_{\text{Compute}} + E_{\text{Cooling}} + E_{\text{Network}}} $$ {#eq-energy-scale-invariant}

where $E_{\text{Network}}$ often becomes a nonnegligible fraction of the total budget as we move terabytes across optical fabrics. Mastery of scale requires optimizing for the Pareto frontier\index{Pareto Frontier} of both laws: minimizing $T_{\text{step}}$ while maximizing $\eta_{p}$.

The scaling efficiency ($\eta_{\text{scaling}}$) of the fleet is the ratio of ideal parallel step time to actual step time:
$$ \eta_{\text{scaling}} = \frac{T_{\text{compute},1}/N}{T_{\text{step}}} = \frac{T_{\text{compute},1}}{N \times T_{\text{step}}} $$
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import (
    GPT3_PARAMS, INFINIBAND_HDR_BW, Gbps, GB, byte, BITS_PER_BYTE
)
from mlsysim.fmt import fmt, check
...
class GPT3SyncTax:
    ...
    # ┌── 1. LOAD (Constants) ───────────────────────────────────────────────
    params = GPT3_PARAMS.m_as('param')
    bw_ib = INFINIBAND_HDR_BW.m_as('byte/second') # HDR InfiniBand
    ...
    sync_energy_mj = sync_energy_joules * million_steps / 1e6
    ...
    check(abs(sync_energy_mj - 84.0) < 0.5, f"Expected about 84 MJ for 1M syncs, got {sync_energy_mj}")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    sync_size_gb_str = fmt((sync_size_bytes * byte).m_as(GB), precision=0)
    ...
    sync_energy_mj_str = fmt(sync_energy_mj, precision=0)
```
...
**Physics**:
To synchronize 175B parameters (FP16), a Ring All-Reduce\index{Ring All-Reduce} must move approximately **`{python} GPT3SyncTax.sync_size_gb_str` GB** of data through each participating accelerator endpoint per iteration.

**The comparison (iron law of scale)**:

*   **InfiniBand (200 Gbps)**: High bandwidth and low latency yield a scaling efficiency of **`{python} GPT3SyncTax.ib_efficiency_pct_str` percent**. Even so, most of the step is still spent waiting on synchronization.
*   **Ethernet (100 Gbps)**: Lower bandwidth and higher overhead collapse the scaling efficiency to **`{python} GPT3SyncTax.eth_efficiency_pct_str` percent**. The fleet spends even more of its time waiting for the network.

**The thermodynamic cost (energy wall)**:
Each endpoint's gradient synchronization consumes approximately **`{python} GPT3SyncTax.sync_energy_j_str` Joules** across the network fabric (at 15 pJ/bit). In a training run of `{python} GPT3SyncTax.million_steps_str` steps, the endpoint network movement alone accounts for about `{python} GPT3SyncTax.sync_energy_mj_str` MJ of energy before multiplying by the number of participating accelerators.
...
The preceding sections established a concrete engineering problem: training a 175-billion-parameter model across thousands of accelerators requires balancing computation, communication, and coordination while tolerating routine hardware failure and satisfying regulatory obligations. No single framework captures all of these concerns simultaneously, but the C$^3$ Taxonomy introduced earlier (@fig-c3-taxonomy) addresses the most immediate one: *where* wall-clock time is actually spent.
...
\begin{tikzpicture}[line join=round, font=\small\usefont{T1}{phv}{m}{n}, scale=0.9, transform shape]
  \tikzset{
    part/.style={draw, rounded corners=3pt, minimum width=6cm, minimum height=1cm, align=center, font=\bfseries\usefont{T1}{phv}{b}{n}, line width=0.75pt},
    ...
    bluePart3/.style={draw=BlueLine, fill=BlueL!75, text=BlueLine},
...
### Part I: The fleet (core infrastructure) {#sec-introduction-part-i-fleet-core-infrastructure-230f}
...
### Part II: Distributed ML (the logic of scale) {#sec-introduction-part-ii-distributed-ml-logic-scale-8b22}
...
*   **Distributed Training (@sec-distributed-training-systems)**: Splitting the math. Strategies for partitioning models from hundreds of billions to trillions of parameters across thousands of GPUs.
...
### Part III: Deployment at scale (the serving pipeline) {#sec-introduction-part-iii-deployment-scale-serving-pipeline-b3c7}
...
*   **Edge Intelligence (@sec-edge-intelligence)**: The frontier. Moving intelligence from the data center to the user's device, constrained by milliwatt power budgets.
...
### Part IV: The responsible fleet (the governance layer) {#sec-introduction-part-iv-responsible-fleet-governance-layer-b6f2}
...
# │ Exports: LinearScalingFallacy.ideal_speedup_str,
# │          LinearScalingFallacy.coord_overhead_pct_str,
# │          LinearScalingFallacy.realized_speedup_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class LinearScalingFallacy:
    ...
    # ┌── 4. OUTPUT (Formatting) ────────────────────────────────────────────
    ideal_speedup_str = fmt(ideal_speedup, precision=0)
    coord_overhead_pct_str = fmt(coord_overhead_fraction * 100, precision=0)
    realized_speedup_str = fmt(realized_speedup, precision=1)
```

The **Linear Scaling Fallacy**\index{Linear Scaling Fallacy} occurs when teams extrapolate resource requirements using power-law relationships without accounting for coordination overhead. $\mathcal{L}(N) = A N^{-\alpha} + B$ works within validated ranges but fails at the boundaries. A team training 100B-parameter models by extrapolating from 10B-parameter experiments might predict a `{python} LinearScalingFallacy.ideal_speedup_str`$\times$ improvement but achieve only `{python} LinearScalingFallacy.realized_speedup_str`$\times$ if coordination and communication overhead consumes `{python} LinearScalingFallacy.coord_overhead_pct_str` percent of the scaled step time. Production systems designed assuming linear scaling have experienced 2--3$\times$ cost overruns when empirical performance deviated from power-law predictions beyond validated thresholds.
...
* **Scale creates qualitative change**: Techniques that work for 8 GPUs\index{Scale!qualitative effects} fail at 8,000 GPUs due to the **bisection bandwidth wall** and the **reliability gap**.
* **Communication is the new bottleneck**: As models scale, network bandwidth\index{Communication!scaling bottleneck} replaces FLOPs as the primary system constraint (High CI Ratio).
* **Failure is a design constraint**: In a 10,000-GPU cluster, hardware\index{Failure!design constraint} fails every few hours. Systems must be designed to absorb failures continuously.
...
* **Governance is the control plane**: Security, privacy, and fairness\index{Governance!control plane} are not appendices; they are invariants that govern the operation of the fleet.
