...
3. **Total annual failures**\index{Failure Rate!annualized count}: `{python} Gpt4TrainingScenario.gpt4_gpus_str` GPUs $\times$ (`{python} Gpt4TrainingScenario.hours_per_year_str` hours $/$ `{python} Gpt4TrainingScenario.gpu_mtbf_str` hours) $\approx$ **`{python} Gpt4TrainingScenario.annual_fails_str` failures per year**.
...
class ReliabilityGap:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    node_reliability_str = fmt_int(p_node * 100, commas=False, suffix=" percent")
...
class GPT3SyncTax:
    ...
    ib_efficiency_pct_str = fmt(eff_ib, precision=1)
    eth_efficiency_pct_str = fmt(eff_eth, precision=1)
