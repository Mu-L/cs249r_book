...
**When does it become the bottleneck?** If we scale to `{python} AllReduceBottleneck.params_70b_billion_str` billion parameters (`{python} AllReduceBottleneck.grad_70b_gb_str` of gradients) and the per-GPU computation stays similar (through batch size scaling), the bandwidth term alone becomes:
...
1. **Non-blocking (1:1)**: $\text{BW}_{\text{bisect}} =$ `{python} BisectionScaling.bbs_gpus_per_side_str` $\times 50 \text{ GB/s} =$ `{python} BisectionScaling.bbs_bw_1_1_gbs_str` $\text{ GB/s}$ per direction.
   - Time: `{python} BisectionScaling.bbs_total_data_gb_str` $\text{ GB} /$ `{python} BisectionScaling.bbs_bw_1_1_gbs_str` $\text{ GB/s} \approx$ **`{python} BisectionScaling.bbs_time_1_1_ms_str`**.
2. **Oversubscribed (4:1)**: $\text{BW}_{\text{bisect}} =$ `{python} BisectionScaling.bbs_bw_1_1_gbs_str`$/4 =$ `{python} BisectionScaling.bbs_bw_4_1_gbs_str` $\text{ GB/s}$ per direction.
   - Time: `{python} BisectionScaling.bbs_total_data_gb_str` $\text{ GB} /$ `{python} BisectionScaling.bbs_bw_4_1_gbs_str` $\text{ GB/s} \approx$ **`{python} BisectionScaling.bbs_time_4_1_ms_str`**.
...
**Problem**: A cluster has `{python} BisectionBottleneck.num_gpus_str` accelerators across `{python} BisectionBottleneck.num_nodes_str`. Each accelerator has `{python} IbNdrTopologyRecap.ib_ndr_gbps_str` Gb/s (`{python} IbNdrTopologyRecap.ib_ndr_gbs_str` GB/s) network injection bandwidth. An AllReduce job requires full $\text{BW}_{\text{bisect}}$.
...
1. **Total exposure**: `{python} PfcStormRisk.pfc_n_gpus_str` $\times$ `{python} PfcStormRisk.pfc_links_per_gpu_str` tiers = `{python} PfcStormRisk.pfc_total_links_str` links, or `{python} PfcStormRisk.pfc_total_transceivers_str` transceivers.
...
1. **Pluggable Architecture**\index{Pluggable Optics!power cost}: 128 ports $\times$ `{python} OpticalDividend.p_pluggable_w_str` W = **`{python} OpticalDividend.total_pluggable_kw_str`** for optics alone.
2. **CPO Architecture**\index{Co-Packaged Optics!power savings}: 128 engines $\times$ `{python} OpticalDividend.p_cpo_w_str` W = **`{python} OpticalDividend.total_cpo_kw_str`**.
