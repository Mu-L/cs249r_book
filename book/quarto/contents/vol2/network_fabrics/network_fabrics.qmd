...
class FECLatency:
    ...
    ns_low_str  = fmt(ns_low, precision=1, commas=False)
    ns_high_str = fmt(ns_high, precision=1, commas=False)
    three_hop_low_str = fmt(three_hop_low, precision=1, commas=False)
    three_hop_high_str = fmt(three_hop_high, precision=1, commas=False)
    round_trip_low_str = fmt(round_trip_low, precision=1, commas=False)
    round_trip_high_str = fmt(round_trip_high, precision=1, commas=False)
```
...
class NetworkFabricsContextScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_ndr_gbps_str = fmt(ib_ndr_gbps_val, precision=1, commas=False)
    ib_ndr_gbs_str = fmt(ib_ndr_gbs_val, precision=1, commas=False)
    nvlink_h100_gbs_str = fmt(nvlink_h100_gbs_val, precision=1, commas=False)
    nvlink_to_ib_ratio_str = fmt(nvlink_to_ib_ratio_val, precision=1, commas=False)
    h100_tflops_str = fmt(h100_flops_raw.m_as(TFLOPs/second), precision=1, commas=False)
...
class NetworkAlphaBeta:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_cross_ib_kb_str = fmt((n_cross_ib * byte).m_as(KB), precision=1, commas=False)
    t_small_ib_us_str = fmt(t_small_ib * 1e6, precision=2, commas=False)
    small_bw_pct_str = fmt(small_bw_term_pct, precision=1, commas=False)
    t_large_ib_ms_str = fmt(t_large_ib * 1e3, precision=1, commas=False)
    ib_alpha_us_str = fmt(alpha_ib * 1e6, precision=1, commas=False)
    ib_beta_gbs_str = fmt((beta_ib * byte / second).m_as(GB / second), precision=1, commas=False)
```
...
class NonBlockingOversubscription:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    comm_pct_str = fmt(comm_fraction * 100, precision=1, commas=False)
...
class FatTreeScaling:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    fat_tree_hosts_str = fmt(fat_tree_hosts_val, precision=1)
```
...
class BisectionScaling:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    bbs_n_gpus_str = fmt(n_gpus, precision=1, commas=False)
    bbs_gpus_per_side_str = fmt(gpus_per_side, precision=1, commas=False)
    bbs_gradient_gb_str = fmt(gradient_gb, precision=1, commas=False)
    bbs_total_data_gb_str = fmt(total_data_gb, precision=1)
    bbs_bw_1_1_gbs_str = fmt(bw_1_1, precision=1)
    bbs_bw_4_1_gbs_str = fmt(bw_4_1, precision=1)
    bbs_slowdown_str = fmt(int(slowdown), precision=1, commas=False)
    bbs_time_1_1_ms_str = fmt(time_1_1 * 1000, precision=1)
    bbs_time_4_1_ms_str = fmt(time_4_1 * 1000, precision=1)
...
class RailLatency:
    ...
    rol_speedup_str = fmt(speedup, precision=1, commas=False)
...
class BisectionBottleneck:
    ...
    gpus_per_side_str = fmt(gpus_per_side, precision=1, commas=False)
    bisec_bw_a_gbs_str = fmt(total_bisec_bw_a, precision=1)
    bisec_bw_b_gbs_str = fmt(total_bisec_bw_b, precision=1)
    ...
    data_crossing_gb_str = fmt(total_data_crossing_bisection, precision=1)
    time_a_ms_str = fmt(time_a_s * 1000, precision=1, commas=False)
    time_b_ms_str = fmt(time_b_s * 1000, precision=1, commas=False)
...
class PfcStormRisk:
    ...
    # ┌── 4. OUTPUT (Formatting) ────────────────────────────────────────
    pfc_n_gpus_str = fmt(n_gpus, precision=1, commas=False)
    pfc_p_degrade_str = MarkdownStr("0.001%")
    pfc_total_links_str = fmt(n_total_links, precision=1)
    pfc_total_transceivers_str = fmt(n_total_transceivers, precision=1)
    ...
    pfc_storm_prob_rounded_str = fmt(p_storm * 100, precision=1, commas=False)
...
class OpticalDividend:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    p_pluggable_w_str = fmt(p_pluggable_w, precision=1, commas=False)
    p_cpo_w_str = fmt(p_cpo_w, precision=1, commas=False)
    ...
    n_switches_str = fmt(n_switches, precision=1)
    ...
    equivalent_h100_gpus_str = fmt(round(equivalent_h100_gpus / 100) * 100, precision=1)
...
class NextGenLinkDensity:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    xdr_port_gbps_str = fmt(xdr_port_gbps_val, precision=1, commas=False)
    eth_800g_port_gbps_str = fmt(eth_800g_port_gbps_val, precision=1, commas=False)
    ...
    port_ratio_str = fmt(port_ratio_val, precision=1, commas=False)
```
...
class IbHdrReference:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_hdr_gbps_str = fmt(hdr_gbps, precision=1, commas=False)
```
...
class BandwidthFallacy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    hdr_bw_str = fmt(ib_hdr.bandwidth.m_as(Gbps), precision=1, commas=False)
    ndr_bw_str = fmt(ib_ndr.bandwidth.m_as(Gbps), precision=1, commas=False)
