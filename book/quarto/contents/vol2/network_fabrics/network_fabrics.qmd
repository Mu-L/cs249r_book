...
@fig-network-topologies (below) illustrates these definitions: fat-trees achieve non-blocking behavior through redundant switch tiers, rail-optimized networks exploit workload structure to approximate it at lower cost, and dragonflies trade bisection bandwidth for reduced cabling [@kim2008].

The comparison quantifies how fat-tree topologies achieve full bisection bandwidth at the cost of additional switch tiers, while rail-optimized and dragonfly designs sacrifice worst-case bisection bandwidth to reduce cabling complexity and cost. This trade-off between guaranteed bandwidth and economic scalability drives every topology decision in large-scale ML clusters.
...
The calculation turns "non-blocking" from a topology label into a budget decision: which switch hierarchy preserves bisection capacity, and where do additional tiers trade bandwidth for latency?
...
# │ Show: storm_prob_pct ≈ 12%—inline in notebook result.
# │ How: P(storm) = 1 - (1 - p_link_degrade)^N_links.
# │
# │ Imports: mlsysim.book (check)
# │ Exports: pfc_n_gpus_str, pfc_p_degrade_str, pfc_storm_prob_str
# └─────────────────────────────────────────────────────────────────────────────
...
class PfcStormRisk:
    ...
    p_link_degrade = 1e-5 # 0.001% per day
    # ┌── 2. EXECUTE ───────────────────────────────────────
    n_total_links = n_gpus * links_per_gpu
    p_safe = (1 - p_link_degrade) ** n_total_links
    p_storm = 1 - p_safe
    # ┌── 3. GUARD (Invariants) ─────────────────────────────────────────
    check(0.1 < p_storm < 0.2, f"Storm probability {p_storm:.2f} outside expected range")
    # ┌── 4. OUTPUT (Formatting) ────────────────────────────────────────
...
1. **Total links**\index{Total Links}: 4,096 GPUs $\times$ 3 tiers = ~12,288 links.
2. **Probability of zero failures**\index{Probability of Zero Failures}: $P(\text{safe}) = (1 - 0.00001)^{12,288} \approx 0.88$.
3. **Probability of at least one storm**\index{Probability of at Least One Storm}: $P(\text{storm}) = 1 - 0.88 = \mathbf{0.12}$ (12 percent).
