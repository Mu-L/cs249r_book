...
class NetworkFabricsContextScenario:
    ...
    nvlink_h100_gbs = f"{nvlink_h100_gbs_val:.0f}"
...
The concrete bandwidth cliff that separates intra-node from inter-node communication makes this Gradient Bus analogy precise. @sec-compute-infrastructure established that a single H100 delivers `{python} NetworkFabricsContextScenario.h100_tflops` TFLOPS of FP16 throughput with `{python} NetworkFabricsContextScenario.h100_mem_bw` TB/s of memory bandwidth. Within a node, eight such accelerators communicate through NVLink at `{python} NetworkFabricsContextScenario.nvlink_h100_gbs` GB/s. The moment computation crosses a node boundary, however, the available bandwidth drops by a factor of `{python} NetworkFabricsContextScenario.nvlink_to_ib_ratio`$\times$, from `{python} NetworkFabricsContextScenario.nvlink_h100_gbs` GB/s (NVLink) to `{python} NetworkFabricsContextScenario.ib_ndr_gbs` GB/s (NDR InfiniBand per port). This cliff, the transition from intra-node to inter-node communication, is the central challenge of network fabric design. For our 175B model, moving 350 GB of gradients through `{python} NetworkFabricsContextScenario.ib_ndr_gbs` GB/s links means that the AllReduce alone can take seconds, during which every GPU in the cluster sits idle unless the fabric and collective algorithms can overlap that transfer with computation.
...
::: {#def-network-fabrics-remote-direct-memory-access-rdma .callout-definition title="Remote direct memory access (RDMA)"}
...
Standard TCP/IP is architecturally unfit for the `{python} NetworkFabricsContextScenario.ib_ndr_gbps` Gbps era. The protocol stack was designed when network speeds were orders of magnitude slower than CPU memory bandwidth, but today that relationship has inverted. Processing a `{python} NetworkFabricsContextScenario.ib_ndr_gbps` Gbps stream through the Linux kernel imposes a crushing interrupt tax: copying payload data between user space and kernel buffers can consume the entire memory bandwidth of a dual-socket server, requiring tens of CPU cores merely to keep the pipe full. The result is a CPU wall where the host processor becomes the bottleneck for network traffic, starving the application logic it is meant to serve.
...
::: {#def-network-fabrics-model-hockney-model .callout-definition title="α-β model (Hockney Model)"}
...
class NetworkAlphaBeta:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_cross_ib_kb_str = f"{(n_cross_ib * byte).m_as(KB):.0f}"
    ...
    ib_beta_gbs_str = f"{(beta_ib * byte / second).m_as(GB / second):.0f}"
```
...
class AllReduceBottleneck:
    ...
    alpha_math           = md_math(f"\\alpha = {alpha_s * 1e6:.1f} \\;\\mu\\text{{s}}")
    beta_math            = md_math(f"\\beta = {beta_gbs:.0f} \\;\\text{{GB/s}}")
    t_comp_display_math  = md(f"$$ T_{{\\text{{compute}}}} = {t_comp_ms} \\text{{ ms}} $$")
    t_ring_1b_math       = md_math(f"T_{{\\text{{ring}}}} \\approx {t_comm_1b_ms} \\text{{ ms}}")
    comm_frac_display_math = md(f"$$ \\text{{Comm. fraction}} = {comm_frac_1b_pct} \\% $$")
...
Assume each GPU processes a synthetic microbatch requiring $5 \times 10^{13}$ FLOPs, chosen to produce an approximately 100 ms compute phase for this bottleneck example. At `{python} NetworkFabricsContextScenario.h100_tflops` TFLOPS with 50 percent utilization:
...
::: {#def-network-fabrics-bisection-bandwidth .callout-definition title="Bisection bandwidth"}
...
::: {#def-network-fabrics-non-blocking-fabric .callout-definition title="Non-blocking fabric"}
...
::: {#def-network-fabrics-fat-tree .callout-definition title="Fat-tree"}
...
class BisectionScaling:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    bbs_n_gpus_str = f"{n_gpus}"
    bbs_gpus_per_side_str = f"{gpus_per_side}"
    bbs_gradient_gb_str = f"{gradient_gb}"
    bbs_total_data_gb_str = f"{total_data_gb:,}"
    bbs_bw_1_1_gbs_str = f"{bw_1_1:,.0f}"
    bbs_bw_4_1_gbs_str = f"{bw_4_1:,.0f}"
    bbs_slowdown_str = f"{int(slowdown)}"
    bbs_time_1_1_ms_str = f"{time_1_1 * 1000:,.0f}"
    bbs_time_4_1_ms_str = f"{time_4_1 * 1000:,.0f}"
...
class RailLatency:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    rol_hop_lat_us_str = f"{t_hop_us}"
    rol_speedup_str = f"{speedup:.0f}"
...
# │          BisectionBottleneck.bisec_bw_a_gbs,
# │          BisectionBottleneck.bisec_bw_b_gbs,
# │          BisectionBottleneck.bisec_bw_a_tbs,
# │          BisectionBottleneck.bisec_bw_b_tbs,
# │          BisectionBottleneck.data_crossing_gb,
# │          BisectionBottleneck.time_a_ms,
# │          BisectionBottleneck.time_b_ms,
# │          BisectionBottleneck.waste_m
# └─────────────────────────────────────────────────────────────────────────────
...
class BisectionBottleneck:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    gpus_per_side_str = f"{gpus_per_side}"
    bisec_bw_a_gbs = f"{total_bisec_bw_a:,.0f}"
    bisec_bw_b_gbs = f"{total_bisec_bw_b:,.0f}"
    bisec_bw_a_tbs = f"{total_bisec_bw_a / 1000:.1f}"
    bisec_bw_b_tbs = f"{total_bisec_bw_b / 1000:.1f}"
    data_crossing_gb = f"{total_data_crossing_bisection:,.0f}"
    time_a_ms = f"{time_a_s * 1000:.0f}"
    time_b_ms = f"{time_b_s * 1000:.0f}"
    waste_m = f"{waste_val:.0f}"
```

**Problem**: A cluster has 1,024 accelerators across 128 nodes. Each accelerator has `{python} NetworkFabricsContextScenario.ib_ndr_gbps` Gb/s (`{python} NetworkFabricsContextScenario.ib_ndr_gbs` GB/s) network injection bandwidth. An AllReduce job requires full bisection bandwidth.

**Scenario A (non-blocking fat-tree)**: 1:1 oversubscription ratio. Per-direction bisection bandwidth = `{python} BisectionBottleneck.gpus_per_side_str` $\times 50 =$ `{python} BisectionBottleneck.bisec_bw_a_gbs` GB/s = `{python} BisectionBottleneck.bisec_bw_a_tbs` TB/s.

**Scenario B (cost-optimized)**: 4:1 oversubscription at the spine layer. Per-direction bisection bandwidth = `{python} BisectionBottleneck.bisec_bw_a_gbs`$/4 =$ `{python} BisectionBottleneck.bisec_bw_b_gbs` GB/s = `{python} BisectionBottleneck.bisec_bw_b_tbs` TB/s.

**Calculation**: Suppose the AllReduce must exchange 100 GB of gradient data per accelerator. In this simplified per-direction bisection model, `{python} BisectionBottleneck.data_crossing_gb` GB crosses the cut.

- Time on Scenario A: `{python} BisectionBottleneck.time_a_ms` ms
- Time on Scenario B: `{python} BisectionBottleneck.time_b_ms` ms

**Systems conclusion**: Saving money on spine switches (Scenario B) slows every synchronization step of the entire cluster by approximately 4$\times$. If AllReduce accounts for 30 percent of each training iteration, the cluster's effective throughput drops significantly. For a \$300 million supercomputer, this amounts to wasting approximately **\$`{python} BisectionBottleneck.waste_m` million** worth of accelerator time waiting for the network. Network oversubscription is false economy for training workloads.
...
::: {#def-network-fabrics-bulk-synchronous-parallel-bsp .callout-definition title="Bulk synchronous parallel (BSP)"}
...
::: {#def-network-fabrics-priority-flow-control .callout-definition title="Priority flow control"}
...
class PfcStormRisk:
    ...
    # ┌── 4. OUTPUT (Formatting) ────────────────────────────────────────
    pfc_n_gpus_str = f"{n_gpus}"
    pfc_p_degrade_str = "0.001%"
    pfc_storm_prob_str = f"{p_storm * 100:.1f}"
...
::: {#def-network-fabrics-incast .callout-definition title="Incast"}
...
The NVIDIA DGX SuperPOD architecture connects DGX H100 nodes using a two-tier InfiniBand network, serving as the reference implementation for the Gradient Bus concept. Each node acts as a dense compute island, with eight H100 GPUs connected via NVSwitch to provide `{python} NetworkFabricsContextScenario.nvlink_h100_gbs` GB/s of internal bandwidth. Externally, each GPU pairs with a ConnectX-7 NIC delivering `{python} NetworkFabricsContextScenario.ib_ndr_gbps` Gb/s of injection bandwidth. Across a standard scalable unit of 32 nodes (256 GPUs), this yields an aggregate injection bandwidth of 12.8 TB/s ($256 \times 50$ GB/s), ensuring the fabric can ingest gradients as fast as the accelerators produce them.
...
class OpticalDividend:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    p_pluggable_w_str = f"{p_pluggable_w:.0f}"
    p_cpo_w_str = f"{p_cpo_w:.0f}"
    savings_kw_str = f"{savings_kw:.2f}"
    total_pluggable_kw_str = f"{total_pluggable_kw:.2f}"
    total_cpo_kw_str = f"{total_cpo_kw:.2f}"
    n_switches_str = f"{n_switches:,}"
    cluster_pluggable_mw_str = f"{cluster_pluggable_mw:.2f}"
    cluster_savings_mw_str = f"{cluster_savings_mw:.2f}"
    equivalent_h100_gpus_str = f"{equivalent_h100_gpus/100:.0f}00"
...
# │ Exports: NextGenLinkDensity.xdr_port_gbps,
# │          NextGenLinkDensity.eth_800g_port_gbps,
# │          NextGenLinkDensity.eth_1_6t_port_tbps,
# │          NextGenLinkDensity.switch_capacity_tbps,
# │          NextGenLinkDensity.port_ratio
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import check
...
class NextGenLinkDensity:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    xdr_port_gbps = f"{xdr_port_gbps_val}"
    eth_800g_port_gbps = f"{eth_800g_port_gbps_val}"
    eth_1_6t_port_tbps = f"{eth_1_6t_port_gbps_val / 1000:.1f}"
    switch_capacity_tbps = f"{switch_capacity_tbps_val:.1f}"
    port_ratio = f"{port_ratio_val:.0f}"
```

The bandwidth march continues to **800G and 1.6T links**: XDR InfiniBand pushes `{python} NextGenLinkDensity.xdr_port_gbps` Gb/s per port and 800GbE pushes `{python} NextGenLinkDensity.eth_800g_port_gbps` Gb/s per port, while `{python} NextGenLinkDensity.eth_1_6t_port_tbps`T Ethernet switch silicon pushes aggregate capacity to `{python} NextGenLinkDensity.switch_capacity_tbps` Tbps. A single `{python} NextGenLinkDensity.eth_1_6t_port_tbps` Tbps port delivers the bandwidth of `{python} NextGenLinkDensity.port_ratio` 400G ports. The increased density allows architects to flatten the topology: a cluster that previously required three tiers of switches can now be served by two, halving the transceiver count and reducing tail latency by removing an entire hop of switching and FEC overhead.
...
Engineers assume upgrading from HDR (`{python} IbHdrReference.ib_hdr_gbps` Gbps) to NDR (`{python} NetworkFabricsContextScenario.ib_ndr_gbps` Gbps) will yield proportional gains, but the $\alpha$-$\beta$ model (@sec-network-fabrics-performance-model) reveals this is only true in the bandwidth-dominated regime. For small models or the small-message phases of pipeline parallelism, the latency term $\alpha$, dominated by switch hops and FEC (~1 $\mu$s), dictates performance. If a workload is latency-bound, a 2$\times$ bandwidth increase may deliver only a small fraction of the line-rate gain while adding substantial power and transceiver cost.
