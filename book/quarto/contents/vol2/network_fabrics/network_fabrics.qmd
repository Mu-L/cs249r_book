...
**The context**: Microsoft deployed RoCEv2 over commodity Ethernet to support latency-sensitive services at data-center scale. RoCEv2 requires a lossless network, so the fabric used Priority Flow Control (PFC).

**The failure**: The Microsoft team reported that PFC could interact with Ethernet flooding behavior to create a cyclic buffer dependency. In their simplified example, one dead server and an incomplete ARP/MAC mapping caused lossless packets to be flooded; PFC pause frames then formed a loop among switches.

**The consequence**: Once the PFC deadlock occurred, restarting servers did not clear it. The fix was architectural: prevent broadcast and multicast behavior from entering lossless classes, and drop lossless packets when the corresponding ARP entry is incomplete.

**The systems lesson**: PFC congestion spreading is a structural vulnerability of lossless Ethernet. RoCE fabrics require packet-class discipline, PFC pause telemetry, and hardware-level safeguards because a mechanism designed to avoid packet loss can itself become the failure mode [@guo2016rdma].
