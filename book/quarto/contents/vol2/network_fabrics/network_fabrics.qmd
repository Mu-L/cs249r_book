...
# ┌── LEGO ───────────────────────────────────────────────
# │ Exports: NetworkRunningExampleCluster.n_gpus_str, .n_nodes_str, .peer_gpus_str,
# │          .gpus_per_rail_str
class NetworkRunningExampleCluster:
    ...
    n_nodes = _cluster.count
    gpus_per_rail = n_gpus // 8
    production_2k_gpus = Systems.Clusters.Production_2K.total_accelerators
    ...
    gpus_per_rail_str = fmt(gpus_per_rail, precision=0, commas=False)
    production_2k_gpus_str = fmt(production_2k_gpus, precision=0, commas=True)
```

## The Synchronization Backbone {#sec-network-fabrics-introduction}

Consider the running example that threads through this volume: a 175-billion-parameter language model partitioned across `{python} NetworkRunningExampleCluster.n_gpus_str` GPUs. Each training step requires an AllReduce of 350 GB of gradient data, meaning every GPU must send and receive its share before the next step can begin. If even one link in the fabric is slow, all `{python} NetworkRunningExampleCluster.peer_gpus_str` other GPUs wait. The network is not auxiliary infrastructure; it is the synchronization backbone that determines whether this cluster trains efficiently or wastes millions of dollars in idle compute.
...
class IbNdrWireRecap:
    ib_ndr_gbps_str = fmt(Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(Gbps), precision=0, commas=False)
```

Before analyzing protocols and topologies, we must understand the physical medium. Every network design decision is ultimately constrained by *what* the wire can carry. At `{python} IbNdrWireRecap.ib_ndr_gbps_str` Gb/s and beyond, the physics of signal transmission imposes hard limits on cable length, power consumption, and error rates. These are not engineering inconveniences; they are fundamental constraints that shape cluster geometry.
...
1.  **Significance (quantitative)**: RDMA reduces end-to-end message latency from the 50–100 μs typical of kernel TCP to approximately 1–2 μs, cutting the $L_{\text{lat}}$ term in the iron law by 25--50$\times$. For a 175B-parameter model exchanging 350 GB of gradient data across `{python} NetworkRunningExampleCluster.n_gpus_str` GPUs, RDMA also eliminates 700 GB of redundant memory copies per step by allowing the NIC to read GPU memory directly without staging through host RAM (GPUDirect RDMA).
...
Standard TCP/IP is architecturally unfit for the `{python} IbNdrTransportRecap.ib_ndr_gbps_str` Gb/s era. The protocol stack was designed when network speeds were orders of magnitude slower than CPU memory bandwidth, but today that relationship has inverted. Processing a `{python} IbNdrTransportRecap.ib_ndr_gbps_str` Gb/s stream through the Linux kernel imposes a crushing interrupt tax: copying payload data between user space and kernel buffers can consume the entire memory bandwidth of a dual-socket server, requiring tens of CPU cores merely to keep the pipe full. The result is a CPU wall where the host processor becomes the bottleneck for network traffic, starving the application logic it is meant to serve.
...
The trade-off is a severe penalty upon failure. If a network switch drops a single packet 900 MB into a 1 GB gradient transfer, the receiver discards all subsequent packets, forcing the sender to retransmit the entire tail of the message, potentially 100 MB of data for a single missed frame. At `{python} IbNdrTransportRecap.ib_ndr_gbps_str` Gb/s, this retransmission triggers a latency spike orders of magnitude larger than the wire delay. In a synchronous training loop where thousands of GPUs wait for the slowest member, a single dropped packet idles the entire cluster. The network fabric must therefore behave as a lossless medium, pushing the complexity of flow control into the switches via Priority Flow Control (PFC) to ensure buffers never overflow.
...
class NetworkAlphaBeta:
    ...
    n_small_kb_str = fmt((n_small * byte).m_as(KB), precision=0, commas=False)
    n_large_mb_str = fmt((n_large * byte).m_as(MB), precision=0, commas=False)
    n_demo_large_mb_str = fmt((n_demo_large * byte).m_as(MB), precision=0, commas=False)
    n_large_grad_mb_str = fmt((350 * MILLION * byte).m_as(MB), precision=0, commas=False)
    eth_alpha_us = 5.0
    eth_beta_gbs = 12.5
    eth_alpha_us_str = fmt(eth_alpha_us, precision=0, commas=False)
    eth_beta_gbs_str = fmt(eth_beta_gbs, precision=1, commas=False)
    doubled_beta_gbs_str = fmt((beta_ib * byte / second).m_as(GB / second) * 2, precision=0, commas=False)
```
...
2. **Large message (`{python} NetworkAlphaBeta.n_demo_large_mb_str` MB)**:
...
class AllReduceBottleneck:
    ...
    params_1b_billion_str = fmt(m_1b.parameters.m_as(param) / BILLION, precision=1, commas=False)
    grad_1b_gb_str = fmt(grad_1b.m_as(GB), precision=1, commas=False)
    gpu_util_pct_str = fmt(gpu_util * 100, precision=0, commas=False)
    h100_tflops_str = fmt(peak_tflops, precision=0, commas=False)
    params_70b_billion_str = fmt(m_70b.parameters.m_as(param) / BILLION, precision=0, commas=False)
    grad_70b_gb_str = fmt(grad_70b.m_as(GB), precision=0, commas=False)
```

**Setup**\index{Setup}: A cluster of `{python} AllReduceBottleneck.num_gpus_str` H100 GPUs training a model with `{python} AllReduceBottleneck.params_1b_billion_str` billion parameters (`{python} AllReduceBottleneck.grad_1b_gb_str` GB of FP32 gradients). Each GPU computes at `{python} AllReduceBottleneck.h100_tflops_str` TFLOP/s. The network uses NDR InfiniBand (`{python} AllReduceBottleneck.alpha_math` and `{python} AllReduceBottleneck.beta_math` per link).

**Step 1**: Compute time per iteration.
Assume each GPU processes a synthetic microbatch requiring $5 \times 10^{13}$ FLOPs, chosen to produce an approximately 100 ms compute phase for this bottleneck example. At `{python} AllReduceBottleneck.h100_tflops_str` TFLOP/s with `{python} AllReduceBottleneck.gpu_util_pct_str` percent utilization:
...
**When does it become the bottleneck?** If we scale to `{python} AllReduceBottleneck.params_70b_billion_str` billion parameters (`{python} AllReduceBottleneck.grad_70b_gb_str` GB of gradients) and the per-GPU computation stays similar (through batch size scaling), the bandwidth term alone becomes:
...
The $\alpha$-$\beta$ model quantifies the speed of a single link, but our 175B-parameter model requires `{python} NetworkRunningExampleCluster.n_gpus_str` GPUs to work in concert. Scaling these transport primitives from a pair of nodes to a warehouse-scale supercomputer demands a **Topology**\index{Topology}\index{Network Topology!definition}: a specific pattern of connections that maximizes $\text{BW}_{\text{bisect}}$ while minimizing the hop count and cabling cost for global collective operations.
...
class FatTreeScaling:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    fat_tree_hosts_str = fmt(fat_tree_hosts_val, precision=0, commas=False)
    fat_tree_two_tier_hosts_str = fmt(fat_tree_two_tier_hosts_val, precision=0, commas=False)
    fat_tree_switches_str = fmt(fat_tree_switches_val, precision=0, commas=False)
    switch_cost_low_str = fmt(10000, precision=0, commas=False)
    switch_cost_high_str = fmt(50000, precision=0, commas=False)
    infra_cost_low_m_str = fmt(50, precision=0, commas=False)
    infra_cost_high_m_str = fmt(250, precision=0, commas=False)
```

A fat-tree built from switches with radix[^fn-switch-radix-density] $k$ supports $N_{\text{hosts}} = k^3/4$ hosts at three tiers, or $k^2/2$ at two tiers. In a two-tier leaf-spine fabric using radix-64 switches (with $k$ leaves each providing $k/2$ host ports and $k/2$ spine uplinks), the topology supports up to `{python} NetworkRunningExampleCluster.production_2k_gpus_str` GPUs, comfortably accommodating our `{python} NetworkRunningExampleCluster.n_gpus_str`-GPU reference cluster with only leaf and spine layers. (A standard fat-tree pod with the leaf-aggregation pair supports $(k/2)^2 =$ `{python} NetworkRunningExampleCluster.n_gpus_str` hosts per pod; the difference is whether the second tier uplinks to a core layer or terminates as spines.)
...
class BisectionBottleneck:
    ...
    num_gpus_str = fmt(num_gpus, precision=0, commas=False)
    num_nodes_str = fmt(num_nodes, precision=0, commas=False)
...
class IbNdrTopologyRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_ndr_gbps_str = fmt(Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(Gbps), precision=0, commas=False)
    ib_ndr_gbs_str = fmt(Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as(GB/second), precision=0, commas=False)
```

**Problem**: A cluster has `{python} BisectionBottleneck.num_gpus_str` accelerators across `{python} BisectionBottleneck.num_nodes_str` nodes. Each accelerator has `{python} IbNdrTopologyRecap.ib_ndr_gbps_str` Gb/s (`{python} IbNdrTopologyRecap.ib_ndr_gbs_str` GB/s) network injection bandwidth. An AllReduce job requires full $\text{BW}_{\text{bisect}}$.
...
**Math**: Suppose the AllReduce must exchange `{python} BisectionBottleneck.gradient_size_gb_str` GB of gradient data per accelerator. In this simplified per-direction bisection model, `{python} BisectionBottleneck.data_crossing_gb_str` GB crosses the cut.
...
class PfcStormRisk:
    ...
    pfc_links_per_gpu_str = fmt(links_per_gpu, precision=0, commas=False)
```
...
1. **Total exposure**: `{python} PfcStormRisk.pfc_n_gpus_str` GPUs $\times$ `{python} PfcStormRisk.pfc_links_per_gpu_str` tiers = `{python} PfcStormRisk.pfc_total_links_str` links, or `{python} PfcStormRisk.pfc_total_transceivers_str` transceivers.
...
The NVIDIA DGX SuperPOD architecture connects DGX H100 nodes using a two-tier InfiniBand network, serving as the reference implementation for the Gradient Bus concept. Each node acts as a dense compute island, with eight H100 GPUs connected via NVSwitch to provide `{python} NetworkFabricsClusterRecap.nvlink_h100_gbs_str` GB/s of internal bandwidth. Externally, each GPU pairs with a ConnectX-7 NIC delivering `{python} NetworkFabricsClusterRecap.ib_ndr_gbps_str` Gb/s of injection bandwidth. Across a standard scalable unit of 32 nodes (256 GPUs), this yields an aggregate injection bandwidth of 12.8 TB/s ($256 \times 50$ GB/s), ensuring the fabric can ingest gradients as fast as the accelerators produce them.
...
Network performance problems in ML clusters are insidious because they manifest as **silent waste** rather than explicit failures. A degraded transceiver causing a 10 percent reduction in effective bandwidth might slow each training iteration by only 2–3 percent, a drift easily masked by the natural variance of checkpointing or data loading. Over a 30-day training run on `{python} NetworkRunningExampleCluster.n_gpus_str` GPUs, this invisible drag accumulates to roughly 14,000--22,000 wasted GPU-hours, burning about \$40,000--\$65,000 at \$3 per GPU-hour without triggering a single alarm. Traditional IT monitoring tools like SNMP or `ping` measure connectivity, not the sustained throughput required by RDMA. Effective observability\index{Monitoring!network} requires a three-layer approach: **physical monitoring** (FEC errors, signal attenuation), **transport monitoring** (PFC pause frames, retransmission rates), and **application monitoring** (NCCL algorithmic bandwidth) [@jeaugey2017nccl]. Only by correlating signals across these layers can operators detect that a "slow training run" is actually caused by a single degraded cable in one rack.
