...
class FECLatency:
    ...
    check(round(three_hop_low) == 300, "Three-hop one-way FEC lower bound should be 300 ns")
    check(round(round_trip_high) == 1200, "Six-hop round-trip FEC upper bound should be 1200 ns")
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
...
# ┌── 2. ARRAYS ────────────────────────────────────────────────────────────────
alpha_ib = Systems.SwitchFabric.AlphaNdr.m_as(second)
beta_ib = Systems.Fabrics.InfiniBand_NDR.bandwidth.m_as('byte/second')
alpha_eth = Systems.SwitchFabric.AlphaRoce.m_as(second)
beta_eth = Systems.Fabrics.Ethernet_100G.bandwidth.m_as('byte/second')
...
class IbHdrReference:
    ...
    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import fmt, check
    check(round(hdr_gbps) == 200, "HDR bandwidth should be 200 Gb/s")
