...
1.  **Significance (quantitative)**: Topology choice directly shifts $\alpha$ and $\beta$. An InfiniBand HDR link has $\alpha \approx 1\,\mu\text{s}$ and $\beta \approx 25\,\text{GB/s}$, yielding $n^* \approx 25\,\text{KB}$: messages smaller than 25 KB are latency-bound and benefit from topology designs that minimize hop count; messages larger than 25 KB are bandwidth-bound and benefit from fat-tree $\text{BW}_{\text{bisect}}$. In a physical ring topology with $H$ topology nodes, the worst-case path traverses $\lfloor H/2 \rfloor$ hops, so effective startup latency scales as $\alpha_{\text{ring}} \approx \lfloor H/2 \rfloor \cdot \alpha_{\text{hop}}$: for a 64-node ring, $\alpha_{\text{ring}} \approx 32\,\mu\text{s}$ vs. $\alpha \approx 1\,\mu\text{s}$ in a fat-tree, a 32$\times$ startup penalty before the bandwidth term is paid.
