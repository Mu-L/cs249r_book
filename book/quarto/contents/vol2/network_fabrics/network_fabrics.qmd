...
1. **Standard Latency**: 2 hops $\times$ `{python} RailLatency.rol_hop_lat_us_str` $\mu s$ = **1.2 $\mu s$**.
2. **Rail-Optimized**: 1 hop $\times$ `{python} RailLatency.rol_hop_lat_us_str` $\mu s$ = **0.6 $\mu s$**.
3. **Verdict**: **`{python} RailLatency.rol_speedup_str`$\times$ lower latency**.
...
1. **Total exposure**: 4,096 GPUs $\times$ 3 tiers = `{python} PfcStormRisk.pfc_total_links_str` links, or `{python} PfcStormRisk.pfc_total_transceivers_str` transceivers.
2. **Probability of zero transceiver failures**: `{python} PfcStormRisk.pfc_safe_prob_math`.
3. **Probability of at least one storm**: `{python} PfcStormRisk.pfc_storm_prob_math` (about `{python} PfcStormRisk.pfc_storm_prob_rounded_str` percent).
...
1. **Pluggable Architecture**\index{Pluggable Optics!power cost}: 128 ports $\times$ `{python} OpticalDividend.p_pluggable_w_str` W = **`{python} OpticalDividend.total_pluggable_kw_str` kW** for optics alone.
2. **CPO Architecture**\index{Co-Packaged Optics!power savings}: 128 engines $\times$ `{python} OpticalDividend.p_cpo_w_str` W = **`{python} OpticalDividend.total_cpo_kw_str` kW**.
