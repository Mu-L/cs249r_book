...
class IbNdrWireRecap:
    ib_ndr_gbps_str = fmt(INFINIBAND_NDR_BW.m_as(Gbps), precision=0, commas=False)
```
...
#| echo: false
# ┌── LEGO ───────────────────────────────────────────────
# │ Context: AllReduce bottleneck setup — H100 throughput.
# │ Exports: IbNdrAllReduceRecap.h100_tflops_str
class IbNdrAllReduceRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tflops_str = fmt(Hardware.Cloud.H100.compute.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
```

**Setup**\index{Setup}: A cluster of 1,024 H100 GPUs training a model with 1 billion parameters (4 GB of FP32 gradients). Each GPU computes at `{python} IbNdrAllReduceRecap.h100_tflops_str` TFLOP/s. The network uses NDR InfiniBand (`{python} AllReduceBottleneck.alpha_math` and `{python} AllReduceBottleneck.beta_math` per link).

**Step 1**: Compute time per iteration.
Assume each GPU processes a synthetic microbatch requiring $5 \times 10^{13}$ FLOPs, chosen to produce an approximately 100 ms compute phase for this bottleneck example. At `{python} IbNdrAllReduceRecap.h100_tflops_str` TFLOP/s with 50 percent utilization:
...
class IbNdrTopologyRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    ib_ndr_gbps_str = fmt(INFINIBAND_NDR_BW.m_as(Gbps), precision=0, commas=False)
    ib_ndr_gbs_str = fmt(INFINIBAND_NDR_BW.m_as(GB/second), precision=0, commas=False)
```

**Problem**: A cluster has 1,024 accelerators across 128 nodes. Each accelerator has `{python} IbNdrTopologyRecap.ib_ndr_gbps_str` Gb/s (`{python} IbNdrTopologyRecap.ib_ndr_gbs_str` GB/s) network injection bandwidth. An AllReduce job requires full $\text{BW}_{\text{bisect}}$.
...
class NetworkFabricsClusterRecap:
    ib_ndr_gbs = INFINIBAND_NDR_BW.m_as(GB/second)
