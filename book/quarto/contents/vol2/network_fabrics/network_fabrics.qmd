...
# │ Show: "400" Gb/s NDR vs "200" Gb/s HDR vs "900" GB/s NVLink—inline in
...
Standard TCP/IP is architecturally unfit for the `{python} NetworkFabricsContextScenario.ib_ndr_gbps_str` Gb/s era. The protocol stack was designed when network speeds were orders of magnitude slower than CPU memory bandwidth, but today that relationship has inverted. Processing a `{python} NetworkFabricsContextScenario.ib_ndr_gbps_str` Gb/s stream through the Linux kernel imposes a crushing interrupt tax: copying payload data between user space and kernel buffers can consume the entire memory bandwidth of a dual-socket server, requiring tens of CPU cores merely to keep the pipe full. The result is a CPU wall where the host processor becomes the bottleneck for network traffic, starving the application logic it is meant to serve.
...
The trade-off is a severe penalty upon failure. If a network switch drops a single packet 900 MB into a 1 GB gradient transfer, the receiver discards all subsequent packets, forcing the sender to retransmit the entire tail of the message, potentially 100 MB of data for a single missed frame. At `{python} NetworkFabricsContextScenario.ib_ndr_gbps_str` Gb/s, this retransmission triggers a latency spike orders of magnitude larger than the wire delay. In a synchronous training loop where thousands of GPUs wait for the slowest member, a single dropped packet idles the entire cluster. The network fabric must therefore behave as a lossless medium, pushing the complexity of flow control into the switches via Priority Flow Control (PFC) to ensure buffers never overflow.
...
1.  **Significance (quantitative)**: Topology choice directly shifts $\alpha$ and $\beta$. An InfiniBand HDR link has $\alpha \approx 1\,\mu\text{s}$ and $\beta \approx 25\,\text{GB/s}$, yielding $n^* \approx 25\,\text{KB}$: messages smaller than 25 KB are latency-bound and benefit from topology designs that minimize hop count; messages larger than 25 KB are bandwidth-bound and benefit from fat-tree $\text{BW}_{\text{bisect}}$. In a physical ring topology, the worst-case path traverses $\lfloor N/2 \rfloor$ hops, so effective startup latency scales as $\alpha_{\text{ring}} \approx \lfloor N/2 \rfloor \cdot \alpha_{\text{hop}}$: for a 64-node ring, $\alpha_{\text{ring}} \approx 32\,\mu\text{s}$ vs. $\alpha \approx 1\,\mu\text{s}$ in a fat-tree, a 32$\times$ startup penalty before the bandwidth term is paid.
...
# │ How: T = M / BW_bisect; BW_bisect = N * Beta/Oversub_Ratio.
# │
...
1. **Standard Latency**\index{Latency!standard}: 2 hops $\times$ `{python} RailLatency.rol_hop_lat_us_str` $\mu\text{s}$ = **1.2 $\mu\text{s}$**.
2. **Rail-Optimized**: 1 hop $\times$ `{python} RailLatency.rol_hop_lat_us_str` $\mu\text{s}$ = **0.6 $\mu\text{s}$**.
...
**Problem**: A cluster has 1,024 accelerators across 128 nodes. Each accelerator has `{python} NetworkFabricsContextScenario.ib_ndr_gbps_str` Gb/s (`{python} NetworkFabricsContextScenario.ib_ndr_gbs_str` GB/s) network injection bandwidth. An AllReduce job requires full $\text{BW}_{\text{bisect}}$.

**Scenario A (non-blocking fat-tree)**: 1:1 oversubscription ratio. Per-direction $\text{BW}_{\text{bisect}} =$ `{python} BisectionBottleneck.gpus_per_side_str` $\times 50 =$ `{python} BisectionBottleneck.bisec_bw_a_gbs_str` GB/s = `{python} BisectionBottleneck.bisec_bw_a_tbs_str` TB/s.

**Scenario B (cost-optimized)**: 4:1 oversubscription at the spine layer. Per-direction $\text{BW}_{\text{bisect}} =$ `{python} BisectionBottleneck.bisec_bw_a_gbs_str`$/4 =$ `{python} BisectionBottleneck.bisec_bw_b_gbs_str` GB/s = `{python} BisectionBottleneck.bisec_bw_b_tbs_str` TB/s.
...
Engineers assume upgrading from HDR (`{python} IbHdrReference.ib_hdr_gbps_str` Gb/s) to NDR (`{python} NetworkFabricsContextScenario.ib_ndr_gbps_str` Gb/s) will yield proportional gains, but the $\alpha$-$\beta$ model (@sec-network-fabrics-performance-model) reveals this is only true in the bandwidth-dominated regime. For small models or the small-message phases of pipeline parallelism, the latency term $\alpha$, dominated by switch hops and FEC (~1 $\mu\text{s}$), dictates performance. If a workload is latency-bound, a 2$\times$ bandwidth increase may deliver only a small fraction of the line-rate gain while adding substantial power and transceiver cost.
...
class BandwidthFallacy:
    ...
    # ┌── 2. EXECUTE (The Compute) ─────────────────────────────────────────
    # Link rates in GB/s (decimal). Gb/s / 8 = GB/s when using decimal SI.
    hdr_gbs = hdr_gbps / 8
    ndr_gbs = ndr_gbps / 8
...
Consider a 10 KB message (typical for control synchronization). On `{python} BandwidthFallacy.hdr_bw_str` Gb/s InfiniBand, it takes `{python} BandwidthFallacy.t_hdr_str` $\mu\text{s}$. Upgrading to `{python} BandwidthFallacy.ndr_bw_str` Gb/s reduces this to `{python} BandwidthFallacy.t_ndr_str` $\mu\text{s}$, a `{python} BandwidthFallacy.speed_pct_str` percent improvement despite doubling the link rate.
