...
1.  **Significance (quantitative)**: Topology choice directly shifts $\alpha$ and $\beta$. An InfiniBand HDR link has $\alpha \approx 1\,\mu\text{s}$ and $\beta \approx 25\,\text{GB/s}$, yielding $n^* \approx 25\,\text{KB}$: messages smaller than 25 KB are latency-bound and benefit from topology designs that minimize hop count; messages larger than 25 KB are bandwidth-bound and benefit from fat-tree bisection bandwidth. In a ring topology, the worst-case path traverses $\lfloor N/2 \rfloor$ hops, so effective startup latency scales as $\alpha_{\text{ring}} \approx \lfloor N/2 \rfloor \cdot \alpha_{\text{hop}}$: for a 64-node ring, $\alpha_{\text{ring}} \approx 32\,\mu\text{s}$ vs. $\alpha \approx 1\,\mu\text{s}$ in a fat-tree—a 32$\times$ penalty that dominates All-Reduce time even for large gradient tensors.
...
# │ Exports: bbs_n_gpus_str, bbs_gradient_gb_str, bbs_slowdown_str, bbs_time_1_1_ms_str, bbs_time_4_1_ms_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt, check
...
class BisectionScaling:
    ...
    gradient_gb = 100
    # ┌── 2. EXECUTE ───────────────────────────────────────
    bw_1_1 = n_gpus * beta_gbs
    bw_4_1 = bw_1_1 / 4
    total_data_gb = n_gpus * gradient_gb
...
1. **Non-blocking (1:1)**: $\text{BW}_{\text{bisect}} =$ `{python} BisectionScaling.bbs_n_gpus_str` $\times 50 \text{ GB/s} = 51,200 \text{ GB/s}$.
   - Time: `{python} BisectionScaling.bbs_total_data_gb_str` $\text{ GB} / 51,200 \approx$ **`{python} BisectionScaling.bbs_time_1_1_ms_str` ms**.
2. **Oversubscribed (4:1)**: $\text{BW}_{\text{bisect}} = 51,200/4 = 12,800 \text{ GB/s}$.
   - Time: `{python} BisectionScaling.bbs_total_data_gb_str` $\text{ GB} / 12,800 \approx$ **`{python} BisectionScaling.bbs_time_4_1_ms_str` ms**.
...
# │
# │ Imports: mlsysim.core.constants
# │ Exports: BisectionBottleneckAnalysis.bisec_time_a_ms,
# │          BisectionBottleneckAnalysis.bisec_time_b_ms,
# │          BisectionBottleneckAnalysis.waste_millions
# └─────────────────────────────────────────────────────────────────────────────
...
class BisectionBottleneck:
    ...
    total_bisec_bw_a = num_gpus * injection_bw_gbs
    total_bisec_bw_b = total_bisec_bw_a/oversub_ratio_b

    # Step 2: Time = Data/Bandwidth
    # In this simplified model, each GPU contributes one gradient shard to the global exchange.
    total_data_crossing_bisection = num_gpus * gradient_size_gb
...
**Scenario A (non-blocking fat-tree)**: 1:1 oversubscription ratio. Bisection bandwidth = $1{,}024 \times 50 = 51{,}200$ GB/s = 51.2 TB/s.

**Scenario B (cost-optimized)**: 4:1 oversubscription at the spine layer. Bisection bandwidth = $51{,}200/4 = 12{,}800$ GB/s = 12.8 TB/s.

**Calculation**: Suppose the AllReduce must exchange 100 GB of gradient data per accelerator, or 102,400 GB across the cluster in this simplified bisection model.
...
::: {#fig-congestion-cascade fig-env="figure" fig-pos="htb" fig-cap="**The Incast Problem in Distributed Training**: During AllReduce synchronization, every GPU node sends line-rate traffic to a common aggregation switch port, producing a many-to-one traffic burst. The figure shows 256 GPU nodes each at 50 GB/s converging on a switch port with 400 Gb/s (50 GB/s) capacity and a 32 MB buffer: 12.8 TB/s of offered load against 50 GB/s of capacity overflows the buffer by 256×. This deterministic overload is the root cause of the PFC backpressure cascades described later in this section." fig-alt="Eight labeled GPU nodes (with 248 more implied) each at 50 GB/s converge on a central Switch Port with 400 Gb/s capacity and a 32 MB buffer that overflows. Callout notes 256-times overload: 256 times 50 GB/s = 12.8 TB/s vs. 50 GB/s port capacity."}
...
1. **Pluggable Architecture**\index{Architecture!pluggable}: 128 ports $\times$ `{python} int(OpticalDividend.p_pluggable_w)` W = **`{python} OpticalDividend.total_pluggable_kw_str` kW** for optics alone.
2. **CPO Architecture**\index{Architecture!cpo}: 128 engines $\times$ `{python} int(OpticalDividend.p_cpo_w)` W = **1.28 kW**.
3. **The dividend**: The savings reach **`{python} OpticalDividend.savings_kw_str` kW** of power *per switch*.

**Systems insight**: In a cluster with 1,000 switches, pluggable optics consume **2.56 megawatts** just to move light. CPO halves this "network tax," saving **1.28 megawatts** and redirecting enough power to fuel an additional **1,800 GPUs**. At the bisection bandwidth wall, sustainability is not a choice; it is an architectural requirement driven by the thermal limits of the faceplate.
