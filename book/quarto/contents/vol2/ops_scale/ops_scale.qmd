...
class TrainingCapacityCost:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    n_gpus_str = f"{n_gpus:,}"
    gpu_hour_rate_str = f"{gpu_hour_rate:.2f}"
    training_cost_m_str = f"{training_cost / MILLION:.2f}"
```
...
class DebtPriority:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────────────────────
    config_score_str = f"{config_score:.1f}"
    monitoring_score_str = f"{monitoring_score:.1f}"
    pipeline_score_str = f"{pipeline_score:.1f}"
    weeks_saved_str = f"{weeks_saved:.0f}"
    config_savings_str = f"{config_savings / THOUSAND:.0f}K"
```
...
**Problem**: A team spends `{python} DeploymentRoi.roi_manual_hours_str` hours of manual toil per model deployment. Investing `{python} DeploymentRoi.automation_setup_hours` hours in a CI/CD pipeline is projected to reduce deployment toil to `{python} DeploymentRoi.roi_auto_hours_str` hours. At `{python} DeploymentRoi.deploys_per_week` deploys per week, how long until the automation pays for itself?

**Math**:

1. **Automation Cost**\index{Automation!automation cost}: `{python} DeploymentRoi.automation_setup_hours` hours $\times$ \$`{python} DeploymentRoi.roi_engineer_rate_str`/hr = **\$18,000**.
...
::: {#exm-ops-scale-feature-freshness-latency .callout-example title="Feature freshness latency"}
...
class SilentFailure:
    ...
    sf_ctr_drop_str = "0.5 percentage points (10 percent relative)"
...
class FalseAlarmTax:
    ...
    specificity_val_str = f"{specificity:.3f}"
    fpr_str = f"{fpr:.3f}"
    fpr_pct_str = f"{fpr * 100:.1f}"
    daily_alerts_expr_str = f"{int(n_monitors):,} x {int(checks_per_day)} x {fpr:.3f}"
    sigma_label_str = "3-sigma"
```

::: {#nbk-ops-scale-false-alarm-tax .callout-notebook title="The false alarm tax"}

The problem: Consider a system monitoring **`{python} FalseAlarmTax.n_models` models**. Each model has **`{python} FalseAlarmTax.n_metrics` metrics** (latency, accuracy, drift, and others). Alert thresholds are set at **`{python} FalseAlarmTax.sigma_label_str`** (99.7 percent specificity), and a control script re-evaluates every metric on a fixed interval. This configuration generates a massive volume of false alarms that the on-call engineer must address each day.

**Math**:

1.  **Total Monitors**\index{Total Monitors}: `{python} FalseAlarmTax.n_models` models $\times$ `{python} FalseAlarmTax.n_metrics` metrics = **`{python} FalseAlarmTax.n_monitors_str` monitors**.
...
::: {#def-ops-scale-finops-ml .callout-definition title="FinOps for ML"}
...
::: {#def-ops-scale-ml-systems-tco .callout-definition title="ML systems TCO"}
