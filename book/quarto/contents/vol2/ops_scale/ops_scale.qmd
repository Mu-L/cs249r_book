...
::: {#fig-n-models-complexity fig-env="figure" fig-pos="htb" fig-cap="**The N-Models Complexity Explosion**: Monitoring alerts grow linearly with model count, deployment coordination grows as $O(N_{\text{models}} \log N_{\text{models}})$, and dependency conflicts grow quadratically as models share features and data sources. The total operational load crosses team capacity around 50 models, marking the transition from artisanal model management to platform-required operations." fig-alt="Log-scale plot showing O(N_models), O(N_models log N_models), O(N_models²) curves with total operational load crossing team capacity at 50 models, with Artisanal, Growing Pains, and Platform Required zones."}
...
# │
# │ Goal: Plot O(N_models), O(N_models log N_models), O(N_models^2) curves; show total crossing capacity
...
# └─────────────────────────────────────────────────────────────────────────────
# ┌── 1. CANVAS ────────────────────────────────────────────────────────────────
# │ Plot O(N_models), O(N_models log N_models), O(N_models^2) curves; show total crossing capacity
...
# ┌── 3. RENDER ────────────────────────────────────────────────────────────────
ax.plot(n_models_axis, monitoring_alerts, label='Monitoring Alerts—O(N_models)', linestyle=':')
ax.plot(n_models_axis, deployment_coordination, label='Deployment Coordination—O(N_models log N_models)', linestyle='--')
ax.plot(n_models_axis, dependency_conflicts, label='Dependency Conflicts—O(N_models²)', linestyle='-.')
...
ax.set_xlabel("Number of Models in Production (N_models)", fontsize=12)
...
$$n = \frac{(Z_{\alpha_{\text{sig}}} + Z_{\beta_{\text{stat}}})^2 \times 2p(1-p)}{\delta^2}$$ {#eq-ops-scale-ab-sample-size}
...
$$P(\text{at least one false positive}) = 1 - (1-\alpha_{\text{sig}})^k = 1 - 0.95^{20} = 0.642$$
...
Stop and reject $H_0$ if $\Lambda_n \geq \frac{1-\beta_{\text{stat}}}{\alpha_{\text{sig}}}$, stop and accept $H_0$ if $\Lambda_n \leq \frac{\beta_{\text{stat}}}{1-\alpha_{\text{sig}}}$, otherwise continue collecting data.
...
$$P(\text{at least one false alert}) = 1 - (1 - \alpha_{\text{fp}})^{N_{\text{tests}}}$$ {#eq-false-alert-rate}
...
$$C_{\text{storage}} = D_{\text{vol}}^{\text{data}} \times R_{\text{storage/GB}} \times T_{\text{retention}}$$

where $D_{\text{vol}}^{\text{data}}$ is data volume in GB, $R_{\text{storage/GB}}$ is the monthly storage rate, and $T_{\text{retention}}$ is retention period in months.

Egress costs scale with data transfer volume:
$$C_{\text{egress}} = D_{\text{vol}}^{\text{transfer}} \times R_{\text{egress/GB}}$$
...
$$C_{\text{process}} = D_{\text{vol}}^{\text{processed}} \times R_{\text{process/GB}}$$
...
Comprehensive per-model alerting guarantees alert fatigue. @eq-false-alert-rate establishes that for $N_{\text{tests}}=1000$ independent tests (100 models $\times$ 10 metrics) at $\alpha_{\text{fp}}=0.05$, $P(\text{at least one false alert}) = 1 - 0.95^{1000} \approx 1.0$. Operators learn to ignore the noise, and genuine incidents disappear. The solution is hierarchical monitoring: business metrics trigger executive attention, portfolio metrics aggregate across related models, and model-specific metrics serve investigation rather than primary alerting.
