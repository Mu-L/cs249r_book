...
class MultiTenantEfficiency:
    ...
    mt_savings_pct_round_str = fmt_int(round(savings_pct), commas=False)
...
3. **Annual Savings**\index{Savings!annual}: `{python} MultiTenantEfficiency.mt_savings_pct_round_str` percent of \$`{python} MultiTenantEfficiency.mt_annual_budget_m_str`M budget $\approx$ **\$`{python} MultiTenantEfficiency.mt_annual_savings_str`/year**.
...
class SilentFailure:
    ...
    sf_lost_clicks_str = fmt(lost_clicks, precision=0)
```
...
class DriftLatency:
    ...
    dl_confidence_pct_str = fmt(95, precision=0, commas=False)
```
...
1. **Samples Required**\index{Samples Required}: Using a two-sample proportion test, detecting a `{python} DriftLatency.dl_drop_str` drop with `{python} DriftLatency.dl_confidence_pct_str` percent confidence requires **`{python} DriftLatency.dl_samples_needed_str` samples**.
