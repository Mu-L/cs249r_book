...
class PlatformRoi:
    ...
    pr_platform_cost_str = fmt(platform_cost_per_month, precision=0, commas=False)
    pr_gross_savings_str = fmt(total_savings, precision=0, commas=False)
    pr_net_benefit_str = fmt(total_savings - platform_cost_per_month, precision=0, commas=False)
```

::: {#nbk-ops-scale-platform-dividend .callout-notebook title="The platform dividend"}

**Problem**: An organization manages `{python} PlatformRoi.pr_n_models_str` models. A centralized ML Platform team costs \$`{python} PlatformRoi.pr_platform_cost_str`/month. If the platform saves each model team `{python} PlatformRoi.pr_savings_h_str` hours of manual toil per month, is the platform investment profitable?

**Math**:

1. **Gross Monthly Savings**\index{Gross Monthly Savings}: `{python} PlatformRoi.pr_n_models_str` models $\times$ `{python} PlatformRoi.pr_savings_h_str` hrs/model $\times$ \$150/hr = **\$`{python} PlatformRoi.pr_gross_savings_str`**.
...
class MaintenanceDividend:
    ...
    md_maintenance_investment_str = fmt(maintenance_investment_h, precision=0, commas=False)
    md_years_str = fmt(years, precision=0, commas=False)
    md_months_str = fmt(months, precision=0, commas=False)
...
**Problem**: A team spends `{python} MaintenanceDividend.md_toil_hours_str` hours/month manually fixing "broken plumbing" (stale data, failed scripts, manual monitoring). A one-month intensive cleanup (`{python} MaintenanceDividend.md_maintenance_investment_str` hours) is projected to reduce this to `{python} MaintenanceDividend.md_low_toil_str` hours/month. Is the cleanup worth it over a `{python} MaintenanceDividend.md_years_str`-year model lifecycle?

**Math**:

1. **Status quo (`{python} MaintenanceDividend.md_years_str` years)**: `{python} MaintenanceDividend.md_months_str` months $\times$ `{python} MaintenanceDividend.md_toil_hours_str` hrs/month $\times$ \$150/hr = **\$`{python} MaintenanceDividend.md_cost_toil_str`**.
2. **Proactive Path**\index{Proactive Path}: (`{python} MaintenanceDividend.md_maintenance_investment_str` hrs investment + `{python} MaintenanceDividend.md_months_str` months $\times$ `{python} MaintenanceDividend.md_low_toil_str` hrs/month) $\times$ \$150/hr = **\$`{python} MaintenanceDividend.md_cost_proactive_str`**.
...
class DebtPriority:
    ...
    deployment_delay_weeks_str = fmt(deployment_delay_weeks, precision=0, commas=False)
    eliminated_delay_pct_str = fmt(eliminated_delay_fraction * 100, precision=0, commas=False)
    delay_per_model_weeks_str = fmt(deployment_delay_weeks * eliminated_delay_fraction, precision=1, commas=False)
    models_per_year_str = fmt(models_per_year, precision=0, commas=False)
    payback_months_str = fmt(2, precision=0, commas=False)
```
...
**Expected ROI**: `{python} DebtPriority.deployment_delay_weeks_str` weeks engineering investment to build config system. Saves `{python} DebtPriority.eliminated_delay_pct_str` percent of deployment delays = `{python} DebtPriority.delay_per_model_weeks_str` weeks per model $\times$ `{python} DebtPriority.models_per_year_str` models/year = `{python} DebtPriority.weeks_saved_str` weeks of deployment time saved. At \$150/hour engineer cost, `{python} DebtPriority.weeks_saved_str` weeks = \$`{python} DebtPriority.config_savings_str` annual savings. Investment pays back within `{python} DebtPriority.payback_months_str` months.
...
class DeploymentRoi:
    ...
    hours_saved_per_deploy_str = fmt(manual_hours_per_deploy - auto_hours_per_deploy, precision=1, commas=False)
```
...
1. **Automation Cost**\index{Automation!automation cost}: `{python} DeploymentRoi.automation_setup_hours_str` hours $\times$ USD `{python} DeploymentRoi.roi_engineer_rate_str`/hr = **USD `{python} DeploymentRoi.automation_cost_str`**.
2. **Weekly Savings**\index{Savings!weekly}: `{python} DeploymentRoi.hours_saved_per_deploy_str` hrs/deploy $\times$ `{python} DeploymentRoi.deploys_per_week_str` deploys/week $\times$ USD `{python} DeploymentRoi.roi_engineer_rate_str`/hr = **USD `{python} DeploymentRoi.weekly_savings_str`/week**.
3. **Payback Period**\index{Payback Period}: USD `{python} DeploymentRoi.automation_cost_str` / USD `{python} DeploymentRoi.weekly_savings_str` $\approx$ **`{python} DeploymentRoi.roi_payback_weeks_str` weeks**.
...
1. **Total Requests**\index{Total Requests}: `{python} SilentFailure.sf_qps_str` requests/s $\times$ `{python} SilentFailure.sf_sec_per_day_str` seconds = **`{python} SilentFailure.sf_total_requests_str` requests**.
...
2.  **False Positive Rate**\index{False Positive Rate}: `{python} FalseAlarmTax.fpr_eqn_str` (`{python} FalseAlarmTax.fpr_pct_str` percent).
...
class DriftLatency:
    ...
    dl_detection_hours_str = fmt(dl_detection_hours, precision=1, commas=False)
```
...
2. **Detection Latency**\index{Latency!detection}: `{python} DriftLatency.dl_samples_needed_str` samples/`{python} DriftLatency.dl_qps_str` samples/hr $\approx$ **`{python} DriftLatency.dl_detection_hours_str` hours**.
