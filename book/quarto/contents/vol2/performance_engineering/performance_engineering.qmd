...
# │       a GELU element-wise op, and LLM decode at B=1 on a d_{\text{model}}=4096 model
# │       to show why inference is predominantly memory-bound.
# │ Show: ~1365 FLOP/byte (GEMM, compute-bound), ~1.25 (GELU, memory-bound),
# │       ~1 (LLM decode B=1, deeply memory-bound)—inline prose.
...
class WorkloadIntensityCalc:
    ...
    # LLM decode: B=1, d_{\text{model}}=4096
    decode_batch = 1
    decode_hidden = 4096
    ...
    # LLM decode step: B=1, weight matrix is d_{\text{model}} * d_{\text{model}}
...
# │       intermediates for d_{\text{model}}=4096 and B=2048.
# │ Show: 16 MB per tensor, 32 MB materialized, 64 MB write/read traffic.
# │ How: tensor_bytes = d_{\text{model}} * B * bytes_fp16; each intermediate is written
...
# │       S=8192, d_{\text{head}}=128, FP16 configuration.
...
class FlashAttentionSavings:
    ...
    # Read Q, K, V once (3 * S * d_{\text{head}} * bytes), write Y once (S * d_{\text{head}} * bytes)
...
# │ How: 2 * N_L * H_{\text{KV}} * d_{\text{head}} * S * bytes.
# │
...
class OverlapCalc:
    ...
    # Simplified: backward ≈ 4 * P * tokens_per_gpu
    tokens_per_gpu = 2048        # S * B_{\text{per GPU}}
