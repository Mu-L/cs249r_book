...
class MemoryWallScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_str = fmt(flops_fp8.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    h100_hbm_bw_str = fmt(bw.m_as(TB/second), precision=2, commas=False)
    h100_ridge_fp8_str = fmt(ridge_fp8, precision=1, allow_zero=True, commas=False)
    energy_ratio_str = fmt(energy_ratio.m_as(''), precision=1, allow_zero=True, commas=False)
    h100_mem_str = fmt(mem.m_as(GiB), precision=1, allow_zero=True, commas=False)
    h100_tdp_str = fmt(tdp.m_as(watt), precision=1, allow_zero=True, commas=False)
```
...
class RooflineRidgeCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    from mlsysim.fmt import fmt
    a100_ridge_str = fmt(a100_ridge_val, precision=1, allow_zero=True, commas=False)
    h100_fp16_ridge_str = fmt(h100_ridge_fp16_val, precision=1, allow_zero=True, commas=False)
    h100_fp8_ridge_str = fmt(h100_ridge_fp8_val, precision=1, allow_zero=True, commas=False)

    a100_fp16_str = fmt(h_a100.peak_flops.m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
    a100_hbm_bw_str = fmt(h_a100.memory_bw.m_as(TB/second), precision=1, allow_zero=True, commas=False)
    h100_fp8_str = fmt(h_h100.compute.precision_flops["fp8"].m_as(TFLOPs/second), precision=1, allow_zero=True, commas=False)
...
class WorkloadIntensityCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gemm_intensity_str = fmt(gemm_intensity, precision=1, allow_zero=True, commas=False)
...
class RooflineDiagnostic:
    ...
    tps_str = fmt(tokens_per_sec, precision=1, allow_zero=True, commas=False)
```
...
class FusionTrafficCalc:
    ...
    materialized_mb_str = fmt((materialized_bytes * byte).m_as(MB), precision=1, allow_zero=True, commas=False)
    traffic_mb_str = fmt(traffic_mb, precision=1, allow_zero=True, commas=False)
    traffic_gb_str = fmt((traffic_bytes_all_layers * byte).m_as(GB), precision=1, commas=False)
    num_layers_str = fmt(num_layers, precision=1, allow_zero=True, commas=False)
```
...
class FlashAttentionSavings:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    from mlsysim.fmt import fmt
    naive_mb_str = fmt((naive_attn_bytes_total * byte).m_as(MB), precision=1, allow_zero=True, commas=False)
    flash_mb_str = fmt((flash_attn_bytes_total * byte).m_as(MB), precision=1, allow_zero=True, commas=False)
    savings_str = fmt(savings_ratio, precision=1, allow_zero=True, commas=False)

    head_n_str = fmt(seq_len, precision=1, allow_zero=True, commas=False)
    head_d_str = fmt(head_dim, precision=1, allow_zero=True, commas=False)
    head_bytes_str = fmt((seq_len * head_dim * bytes_per_elem * byte).m_as(MB), precision=1, allow_zero=True, commas=False)
    naive_head_traffic_mb_str = fmt((naive_attn_bytes_per_head * byte).m_as(MB), precision=1, allow_zero=True, commas=False)
    flash_head_traffic_mb_str = fmt((flash_attn_bytes_per_head * byte).m_as(MB), precision=1, allow_zero=True, commas=False)
    flash_savings_factor_str = fmt(savings_ratio, precision=1, allow_zero=True, commas=False)
```
...
# │          batch_ratio_str
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.formulas import calc_kv_cache_size, model_memory
...
class KVCacheAnalysis:
    ...
    max_batch_fp16_str = fmt(max_batch_fp16_val, precision=1, allow_zero=True, commas=False)
    max_batch_int8_str = fmt(max_batch_int8_val, precision=1, allow_zero=True, commas=False)
    batch_ratio_str = fmt(batch_ratio, precision=1, commas=False)
    avail_fp16_gb_str = fmt(int(avail_fp16_gb), precision=1, allow_zero=True, commas=False)
...
class CompilationDividend:
    ...
    new_tps_str = fmt(round(new_tps), precision=1, allow_zero=True, commas=False)
    speedup_str = fmt(speedup, precision=2, commas=False)
    gemm_share_str = fmt(round(gemm_share_post), precision=1, allow_zero=True, commas=False)
```
...
class MoEEconomics:
    ...
    # ┌── 1. LOAD (Constants) ──────────────────────────────────────────────
    from mlsysim import Models
    m_dense = Models.Language.Llama2_70B # Using 70B as dense baseline for this comparison
    m_moe = Models.Language.Llama3_8B # Placeholder for MoE comparison logic
...
class NvlinkH100:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    bw_gbps_str = fmt(bw_gbps, precision=1, allow_zero=True, commas=False)
```
...
# │
# │ Imports: mlsysim.core.constants (NVLINK_H100_BW, Hardware.Cloud.H100.compute.peak_flops,
# │          BYTES_FP16, GB, second, flop), mlsysim.core.formulas
...
class OverlapCalc:
    ...
    gradient_gb_str = fmt(gradient_qty.m_as(GB), precision=1, allow_zero=True, commas=False)
...
# │          mlsysim.core.formulas (calc_transformer_training_flops)
...
class FleetEfficiencyCalc:
    ...
    fleet_nodes_str = fmt(n_gpus, precision=1, allow_zero=True, commas=False)
    fleet_t_step_ms_str = fmt(t_fleet_step_ms, precision=1, allow_zero=True, commas=False)
    ...
    fleet_scaling_tax_str = fmt(scaling_tax, precision=1, allow_zero=True, commas=False)
```
