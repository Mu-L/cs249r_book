...
class RooflineRidgeCalc:
    ...
    a100_fp16_str = fmt_int(h_a100.peak_flops.m_as(TFLOPs/second), commas=False, suffix=' TFLOP/s')
    a100_hbm_bw_str = fmt(h_a100.memory_bw.m_as(TB/second), precision=1, commas=False, suffix=' TB/s')
    h100_fp8_str = fmt_int(h_h100.compute.precision_flops["fp8"].m_as(TFLOPs/second), commas=False, suffix=' TFLOP/s')
    h100_hbm_bw_str = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False, suffix=' TB/s')
```
...
class OverlapCalc:
    ...
    allreduce_ms_str = fmt(allreduce_time_s * 1000, precision=1, commas=False, suffix=' ms')
    backward_ms_str = fmt(backward_time_s * 1000, precision=1, commas=False, suffix=' ms')
    step_no_overlap_ms_str = fmt(step_no_overlap * 1000, precision=1, commas=False, suffix=' ms')
    step_overlap_ms_str = fmt(step_with_overlap * 1000, precision=1, commas=False, suffix=' ms')
...
**Problem**: A 7B parameter LLM runs on a single H100 and shows `{python} ProfilerDetective.observed_tps_str` econd during autoregressive generation at batch size 1. The pure FP16 weight-read roofline is higher, but after budgeting roughly `{python} ProfilerDetective.decode_traffic_gb_str` of total per-token traffic for weights, KV-cache reads, sampling, synchronization, and launch overhead, the bandwidth-limited full-decode ceiling is approximately `{python} ProfilerDetective.ceiling_tps_str` econd. Where is the remaining `{python} ProfilerDetective.hidden_performance_pct_str` of realizable decode-step performance hiding?
