...
**The context**: Cloudflare's Web Application Firewall runs on edge machines that process HTTP and HTTPS traffic across the global network.

**The failure**: On July 2, 2019, a newly deployed managed WAF rule caused excessive CPU consumption on every HTTP/HTTPS-handling core across Cloudflare's network.

**The consequence**: Cloudflare reported losing most of its traffic until the team identified the WAF as the cause and used a global disable mechanism to restore service.

**The systems lesson**: Performance engineering includes algorithmic complexity and rollout design. A small rule change can become a global outage when it is deployed everywhere before bounded-runtime checks, canaries, and kill switches catch the cost [@cloudflare2019outage].
