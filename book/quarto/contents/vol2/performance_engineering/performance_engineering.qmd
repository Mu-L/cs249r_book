...
The $\max$ operator simplifies this equation: one term usually dominates at a given operating point. Optimizing a clearly nondominant term yields little or no improvement. Increasing compute throughput for a memory-bound workload, for example, does not materially improve performance until the memory term is reduced. This equation makes diagnosis the first step rather than an afterthought.
...
# │       intermediates for hidden=4096 and batch=2048.
# │ Show: 16 MB per tensor, 32 MB materialized, 64 MB write/read traffic.
# │ How: tensor_bytes = hidden * batch * bytes_fp16; each intermediate is written
# │      once and read once before the next kernel consumes it.
# │
# │ Imports: mlsysim.core.constants (MB, GB, byte), mlsysim.fmt (check)
...
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.core.constants import MB, GB, byte
...
class FusionTrafficCalc:
    ...
    materialized_mb_str = fmt((materialized_bytes * byte).m_as(MB), precision=0, commas=False)
    traffic_mb_str = fmt(traffic_mb, precision=0, commas=False)
    traffic_gb_str = fmt((traffic_bytes_all_layers * byte).m_as(GB), precision=1, commas=False)
    num_layers_str = fmt(num_layers, precision=0, commas=False)
```

Intermediate tensors $Z_1$ and $Z_2$ each occupy the same memory as the output $Y$. For a hidden dimension of 4096 and batch size of 2048 in FP16, each intermediate tensor is `{python} FusionTrafficCalc.tensor_mb_str` MB. The unfused execution materializes `{python} FusionTrafficCalc.materialized_mb_str` MB of intermediate tensors and performs `{python} FusionTrafficCalc.traffic_mb_str` MB of intermediate HBM traffic, writing and then rereading each tensor. A fused kernel avoids that traffic entirely by holding $Z_1$ and $Z_2$ in registers or shared memory (SRAM) within the SM.
...
To appreciate the quantitative impact, consider each category applied to a single transformer layer with hidden dimension 4096 and batch size 2048 in FP16. Element-wise fusion of a bias-GELU-dropout chain in this shape eliminates two intermediate tensors of `{python} FusionTrafficCalc.tensor_mb_str` MB each, saving `{python} FusionTrafficCalc.traffic_mb_str` MB of HBM traffic per layer. Across `{python} FusionTrafficCalc.num_layers_str` layers, this reclaims about `{python} FusionTrafficCalc.traffic_gb_str` GB of memory bandwidth per forward pass. Reduction fusion of LayerNorm avoids materializing large prenormalization tensors and intermediate statistics. Operator-specific attention fusion (FlashAttention) provides the largest single gain by removing the quadratic score and probability matrices that dominate long-context attention. The cumulative effect of all three fusion categories can remove a large fraction of HBM traffic for a memory-bound transformer forward pass, but the end-to-end speedup must still be verified with profiling because the bottleneck may shift.
...
# │ Show: ~65x reduction in materialized quadratic state by avoiding NxN tensors.
# │ How: Naive materializes S and P plus Q/K/V/O; Flash keeps only linear Q/K/V/O
...
FlashAttention [@dao2022] reformulates attention as a **tiled** computation. Instead of materializing the full $N{\times}N$ attention matrix, it processes $Q$, $K$, and $V$ in small blocks that fit in on-chip SRAM. The algorithm loads tiles of $Q$, $K$, and $V$, computes partial attention scores, and maintains running statistics (online softmax) to produce the exact result without ever storing the full attention matrix in HBM.

The reduction in materialized attention state is dramatic. For a sequence length of `{python} FlashAttentionSavings.head_n_str`, 32 heads, and head dimension `{python} FlashAttentionSavings.head_d_str` in FP16, the naive implementation materializes and revisits approximately `{python} FlashAttentionSavings.naive_mb_str` MB of HBM-resident tensors for the full layer, or `{python} FlashAttentionSavings.naive_head_traffic_mb_str` MB per head. FlashAttention avoids the quadratic score and probability tensors; its persistent HBM-visible tensors are dominated by $Q$, $K$, $V$, and $O$, totaling approximately `{python} FlashAttentionSavings.flash_mb_str` MB for the full layer, or `{python} FlashAttentionSavings.flash_head_traffic_mb_str` MB per head. This simplified accounting excludes schedule-dependent tile reloads inside a particular kernel, but it captures the important scaling result: a `{python} FlashAttentionSavings.savings_str`$\times$ reduction in materialized attention state for this configuration.
...
In 2022, Tri Dao challenged the prevailing wisdom that the attention mechanism's $O(N^2)$ complexity required better matrix multiplication kernels. His insight was that the bottleneck was not compute, but memory hierarchy. Standard attention materialized the massive $N{\times}N$ attention score matrix in high-latency HBM. FlashAttention restructured the algorithm using tiling to keep running statistics in on-chip SRAM, computing the softmax without ever writing the full matrix to global memory. This reduced memory complexity to linear $O(N)$ and wall-clock time by 2--4$\times$ on relevant attention workloads. It quickly influenced the attention kernels used across major ML frameworks and serving stacks.

:::

@fig-flashattention-memory-savings quantifies the workspace advantage across sequence lengths. Standard attention allocates the full $N{\times}N$ score matrix in HBM, while FlashAttention maintains only $O(N)$ running statistics. Standard-attention workspace grows quadratically with sequence length while FlashAttention's running state grows linearly, so the savings ratio grows linearly: at a typical 8K context, the quadratic score workspace alone is 4,096$\times$ larger than FlashAttention's running statistics; at 64K long-context, the ratio reaches 32,768$\times$.
...
class KVCacheAnalysis:
    ...
    kv_fp16_per_gpu_gb_str = fmt(kv_fp16_gb_val/4, precision=4, commas=False)
    kv_int8_total_gb_str = fmt(kv_fp16_gb_val/2, precision=3, commas=False)
    kv_int8_per_gpu_gb_str = fmt(kv_fp16_gb_val/2/4, precision=4, commas=False)
...
- KV cache per request: `{python} KVCacheAnalysis.kv_fp16_gb_str` GB total, or approximately `{python} KVCacheAnalysis.kv_fp16_per_gpu_gb_str` GB/GPU
...
- KV cache per request (INT8): approximately `{python} KVCacheAnalysis.kv_int8_total_gb_str` GB total, or `{python} KVCacheAnalysis.kv_int8_per_gpu_gb_str` GB/GPU
...
Hardware FLOPs Utilization (HFU) is the simpler metric: total arithmetic operations executed (including overhead like activation recomputation) divided by peak FLOPS. HFU is usually at least as high as MFU because it counts recomputed operations that do not directly advance the model's computation. The gap between HFU and MFU quantifies the "wasted" compute from techniques like activation checkpointing.
...
* **Operator Fusion** and **Tiling** (FlashAttention) eliminate redundant HBM traffic by keeping intermediate results in on-chip SRAM and avoiding the quadratic attention intermediates that dominate long-context memory use.
