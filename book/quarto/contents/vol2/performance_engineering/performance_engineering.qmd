...
class MemoryWallScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_str = fmt_int(flops_fp8.m_as(TFLOPs/second), commas=False)
    ...
    h100_mem_str = fmt_int(mem.m_as(GiB), commas=False)
    h100_tdp_str = fmt_int(tdp.m_as(watt), commas=False)
    register_total_mb_str = fmt_int(register_total_mb, commas=False)
    l2_cache_mb_str = fmt_int(l2_cache_mb, commas=False)
```
...
To appreciate the quantitative impact, consider each category applied to a single transformer layer with hidden dimension 4096 and batch size 2048 in FP16. Element-wise fusion of a bias-GELU-dropout chain in this shape eliminates two intermediate tensors of `{python} FusionTrafficCalc.tensor_mb_str` MB each, saving `{python} FusionTrafficCalc.traffic_mb_str` MB of HBM traffic per layer. Across `{python} FusionTrafficCalc.num_layers_str` layers, this reclaims about `{python} FusionTrafficCalc.traffic_gb_str` GB of HBM traffic per forward pass. Reduction fusion of LayerNorm avoids materializing large prenormalization tensors and intermediate statistics. Operator-specific attention fusion (FlashAttention) provides the largest single gain by removing the quadratic score and probability matrices that dominate long-context attention. The cumulative effect of all three fusion categories can remove a large fraction of HBM traffic for a memory-bound transformer forward pass, but the end-to-end speedup must still be verified with profiling because the bottleneck may shift.
...
class KVCacheAnalysis:
    ...
    weight_fp16_shard_gb_str = fmt_int(weight_fp16_shard_gb, commas=False)
    weight_fp16_total_gb_str = fmt_int(weight_fp16_shard_gb * tensor_parallel_degree, commas=False)
    tensor_parallel_degree_str = fmt_int(tensor_parallel_degree, commas=False)
    gpu_total_gb_str = fmt_int(gpu_total_gb, commas=False)
...
- Available for KV cache: `{python} KVCacheAnalysis.gpu_total_gb_str` GB - `{python} KVCacheAnalysis.weight_fp16_shard_gb_str` GB = `{python} KVCacheAnalysis.avail_fp16_gb_str` GB/GPU
...
- Available for KV cache: `{python} KVCacheAnalysis.gpu_total_gb_str` GB - `{python} KVCacheAnalysis.weight_int4_gb_str` GB = `{python} KVCacheAnalysis.avail_int8_gb_str` GB/GPU
...
class NvlinkH100:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    bw_gbps_str = fmt_int(bw_gbps, commas=False)
```
...
class OverlapCalc:
    ...
    nvlink_bw_gbs_str = fmt_int(nvlink_bw.m_as(GB/second), commas=False)
```
...
class LayerNormRoofline:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    achieved_tflops_str = fmt_int(achieved_tflops, commas=False)
...
class ProfilerDetective:
    ...
    decode_traffic_gb_str = fmt_int(decode_traffic_gb, commas=False)
    hidden_performance_pct_str = fmt_int(hidden_performance_pct, commas=False)
    gemm_bandwidth_tbs_str = fmt(gemm_bandwidth_tbs, precision=1, commas=False)
    gemm_efficiency_pct_str = fmt_int(gemm_efficiency_pct, commas=False)
    gemm_time_share_pct_str = fmt_int(gemm_time_share_pct, commas=False)
    attn_share_pct_str = fmt_int(attn_share_pct, commas=False)
    layernorm_share_pct_str = fmt_int(layernorm_share_pct, commas=False)
    softmax_share_pct_str = fmt_int(softmax_share_pct, commas=False)
    launch_gap_pct_str = fmt_int(launch_gap_pct, commas=False)
    ...
    non_gemm_share_pct_str = fmt_int(non_gemm_share_pct, commas=False)
```
...
class FleetEfficiencyCalc:
    ...
    fleet_nodes_str = fmt_int(n_gpus, commas=False)
    fleet_t_step_ms_str = fmt_int(t_fleet_step_ms, commas=False)
