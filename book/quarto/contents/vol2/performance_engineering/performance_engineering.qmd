...
Performance engineering operates within a constrained optimization space defined by the iron law of ML system performance\index{Iron Law}:
...
- [ ] A workload is **Memory-Bound**. Will upgrading the GPU's clock frequency (increasing FLOPS) improve performance?
...
FlashAttention\index{FlashAttention} [@dao2022] reformulates attention as a **tiled** computation. Instead of materializing the full $N{\times}N$ attention matrix, it processes $Q$, $K$, and $V$ in small blocks that fit in on-chip SRAM. The algorithm loads tiles of $Q$, $K$, and $V$, computes partial attention scores, and maintains running statistics (online softmax) to produce the exact result without ever storing the full attention matrix in HBM.
...
*   **Local node baseline**\index{Model FLOPs Utilization!local node baseline}: A single 8-GPU node achieves **`{python} FleetEfficiencyCalc.fleet_local_mfu_str` percent MFU**.
*   **Fleet performance**\index{Model FLOPs Utilization!fleet performance}: At 128 GPUs, the step time increases to **`{python} FleetEfficiencyCalc.fleet_t_step_ms_str` ms**, dropping MFU to **`{python} FleetEfficiencyCalc.fleet_global_mfu_str` percent**.
