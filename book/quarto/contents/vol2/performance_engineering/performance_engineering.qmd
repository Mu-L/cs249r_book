...
class MemoryWallScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_str = fmt(flops_fp8.m_as(TFLOPs/second), precision=1, commas=False)
    h100_hbm_bw_str = fmt(bw.m_as(TB/second), precision=2, commas=False)
    h100_ridge_fp8_str = fmt(ridge_fp8, precision=1, commas=False)
    energy_ratio_str = fmt(energy_ratio.m_as(''), precision=1, commas=False)
    h100_mem_str = fmt(mem.m_as(GiB), precision=1, commas=False)
    h100_tdp_str = fmt(tdp.m_as(watt), precision=1, commas=False)
```
...
class RooflineRidgeCalc:
    ...
    a100_fp16_str = fmt(h_a100.peak_flops.m_as(TFLOPs/second), precision=1, commas=False)
    a100_hbm_bw_str = fmt(h_a100.memory_bw.m_as(TB/second), precision=1, commas=False)
    h100_fp8_str = fmt(h_h100.compute.precision_flops["fp8"].m_as(TFLOPs/second), precision=1, commas=False)
...
class FusionTrafficCalc:
    ...
    num_layers_str = fmt(num_layers, precision=1, commas=False)
```
...
class FlashAttentionSavings:
    ...
    savings_str = fmt(savings_ratio, precision=1, commas=False)

    head_n_str = fmt(seq_len, precision=1, commas=False)
    head_d_str = fmt(head_dim, precision=1, commas=False)
    ...
    flash_savings_factor_str = fmt(savings_ratio, precision=1, commas=False)
```
...
class KVCacheAnalysis:
    ...
    max_batch_fp16_str = fmt(max_batch_fp16_val, precision=1, commas=False)
    max_batch_int8_str = fmt(max_batch_int8_val, precision=1, commas=False)
    batch_ratio_str = fmt(batch_ratio, precision=1, commas=False)
    avail_fp16_gb_str = fmt(int(avail_fp16_gb), precision=1, commas=False)
...
class CompilationDividend:
    ...
    new_tps_str = fmt(round(new_tps), precision=1, commas=False)
    speedup_str = fmt(speedup, precision=2, commas=False)
    gemm_share_str = fmt(round(gemm_share_post), precision=1, commas=False)
```
...
class NvlinkH100:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    bw_gbps_str = fmt(bw_gbps, precision=1, commas=False)
```
...
class FleetEfficiencyCalc:
    ...
    fleet_nodes_str = fmt(n_gpus, precision=1, commas=False)
    fleet_t_step_ms_str = fmt(t_fleet_step_ms, precision=1, commas=False)
