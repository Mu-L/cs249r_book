...
class MemoryWallScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp8_str = f"{flops_fp8.m_as(TFLOPs/second):.0f}"
    h100_hbm_bw_str = f"{bw.m_as(TB/second):.2f}"
    h100_ridge_fp8_str = f"{ridge_fp8:.0f}"
    energy_ratio_str = f"{energy_ratio.m_as(''):.0f}"
    h100_mem_str = f"{mem.m_as(GiB):.0f}"
    h100_tdp_str = f"{tdp.m_as(watt):.0f}"
```
...
::: {#def-performance-engineering-arithmetic-intensity .callout-definition title="Arithmetic intensity"}
...
class RooflineDiagnostic:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    flops_B_str = f"{flops_B:.1f}"
    weight_gb_str = f"{weight_gb:.1f}"
    ai_str = f"{arith_intensity:.1f}"
    decode_ms_str = f"{decode_time_ms:.1f}"
...
class FusionTrafficCalc:
    ...
    materialized_mb_str = f"{(materialized_bytes * byte).m_as(MB):.0f}"
    traffic_mb_str = f"{traffic_mb:.0f}"
    traffic_gb_str = f"{(traffic_bytes_all_layers * byte).m_as(GB):.1f}"
...
class FlashAttentionSavings:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    naive_mb_str = f"{(naive_attn_bytes_total * byte).m_as(MB):.0f}"
    flash_mb_str = f"{(flash_attn_bytes_total * byte).m_as(MB):.0f}"
    savings_str = f"{savings_ratio:.0f}"

    head_n_str = f"{seq_len}"
    head_d_str = f"{head_dim}"
    head_bytes_str = f"{(seq_len * head_dim * bytes_per_elem * byte).m_as(MB):.0f}"
    naive_head_traffic_mb_str = f"{(naive_attn_bytes_per_head * byte).m_as(MB):.0f}"
    flash_head_traffic_mb_str = f"{(flash_attn_bytes_per_head * byte).m_as(MB):.0f}"
...
::: {#def-performance-engineering-block-wise-quantization .callout-definition title="Block-wise quantization"}
...
class KVCacheAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    kv_fp16_gb_str = f"{kv_fp16_gb_val:.2f}"
    kv_fp16_per_gpu_gb_str = f"{kv_fp16_gb_val/4:.4f}"
    kv_int8_total_gb_str = f"{kv_fp16_gb_val/2:.3f}"
    kv_int8_per_gpu_gb_str = f"{kv_fp16_gb_val/2/4:.4f}"
    max_batch_fp16_str = f"{max_batch_fp16_val}"
    max_batch_int8_str = f"{max_batch_int8_val}"
    batch_ratio_str = f"{batch_ratio:.1f}"
    avail_fp16_gb_str = f"{int(avail_fp16_gb)}"
    weight_int4_gb_str = f"{weight_int4_shard_gb}"
    avail_int8_gb_str = f"{avail_int8_gb:.2f}"
```
...
class CompilationDividend:
    ...
    elementwise_frac = 0.35    # fraction of time in element-wise kernels
    launch_frac = 0.15         # fraction of time in launch overhead
    gemm_frac = 0.50           # fraction of time in GEMM
    ...
    gemm_ms = _tpt * gemm_frac
    elem_ms = _tpt * elementwise_frac * (1 - fusion_reduction)
    launch_ms = _tpt * launch_frac * (1 - launch_reduction)
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    tpt_str = f"{_tpt:.2f}"
    gemm_ms_str = f"{gemm_ms:.2f}"
    elem_ms_str = f"{elem_ms:.2f}"
    launch_ms_str = f"{launch_ms:.2f}"
    new_time_str = f"{new_time_ms:.2f}"
    new_tps_str = f"{round(new_tps)}"
    speedup_str = f"{speedup:.2f}"
    gemm_share_str = f"{round(gemm_share_post)}"
```
...
class OverlapCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    gradient_gb_str = f"{(gradient_bytes * byte).m_as(GB):.0f}"
...
::: {#def-performance-engineering-model-flops-utilization-mfu .callout-definition title="Model FLOPs utilization (MFU)"}
...
class FleetEfficiencyCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    fleet_params_b_str = "70B"
    fleet_nodes_str = f"{n_gpus}"
    fleet_t_step_ms_str = f"{t_fleet_step_ms:.0f}"
    fleet_local_mfu_str = f"{local_mfu*100:.1f}"
    fleet_global_mfu_str = f"{global_mfu*100:.1f}"
    fleet_scaling_tax_str = f"{scaling_tax:.0f}"
```
