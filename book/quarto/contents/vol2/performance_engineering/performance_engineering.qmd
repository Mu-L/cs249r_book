...
$$
\text{Time} = \max\left( \frac{\text{Compute}}{\text{FLOP/s}}, \; \frac{\text{Memory Access}}{\text{Bandwidth}} \right) + \text{Overhead}
$$ {#eq-iron-law-perf}
...
Any operation performing fewer than `{python} RooflineRidgeCalc.h100_fp16_ridge_str` floating-point operations per byte loaded is memory-bound on the H100 at FP16. At FP8 precision, where compute doubles to `{python} RooflineRidgeCalc.h100_fp8_str` TFLOP/s while bandwidth remains `{python} RooflineRidgeCalc.h100_hbm_bw_str` TB/s, the ridge point rises to approximately `{python} RooflineRidgeCalc.h100_fp8_ridge_str` FLOP/byte. The A100, with `{python} RooflineRidgeCalc.a100_fp16_str` TFLOP/s and `{python} RooflineRidgeCalc.a100_hbm_bw_str` TB/s, has a lower ridge point of approximately `{python} RooflineRidgeCalc.a100_ridge_str` FLOP/byte at FP16. Each hardware generation increases compute faster than bandwidth, pushing the ridge point higher and making more workloads memory-bound.
...
class OverlapCalc:
    ...
    h100_fp16_tflops = H100_FLOPS_FP16_TENSOR.m_as(flop/second)  # Tensor core TFLOP/s
