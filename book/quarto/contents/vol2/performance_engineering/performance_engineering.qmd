...
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.compute.precision_flops["fp8"], Hardware.Cloud.H100.memory.bandwidth,
# │          Hardware.Cloud.H100.memory.capacity, Hardware.Cloud.H100.tdp, ENERGY_DRAM_ACCESS_PJ, ENERGY_SRAM_L1_PJ,
...
class MemoryWallScenario:
    ...
    register_total_mb_str = fmt(register_total_mb, precision=0, commas=False)
    l2_cache_mb_str = fmt(l2_cache_mb, precision=0, commas=False)
```
...
*   **Registers (`{python} MemoryWallScenario.register_total_mb_str` MB)** are *working memory*: instant access, but capacity is small enough to hold only a few values at once.
*   **Shared Memory (SRAM)**\index{SRAM} is a *desk*: very fast to reach, but capacity fits only a few open references.
*   **L2 Cache (`{python} MemoryWallScenario.l2_cache_mb_str` MB)** is a *book cart* beside the desk: a small access cost, holding a moderate working set.
...
class RooflineRidgeCalc:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    a100_ridge_val = h_a100.ridge_point("fp16").m_as("flop/byte")
    h100_ridge_fp16_val = h_h100.ridge_point("fp16").m_as("flop/byte")
    ...
    a100_fp16_str = fmt(h_a100.peak_flops.m_as(TFLOPs/second), precision=0, commas=False)
    a100_hbm_bw_str = fmt(h_a100.memory_bw.m_as(TB/second), precision=1, commas=False)
    h100_fp8_str = fmt(h_h100.compute.precision_flops["fp8"].m_as(TFLOPs/second), precision=0, commas=False)
    h100_hbm_bw_str = fmt(h_h100.memory_bw.m_as(TB/second), precision=2, commas=False)
```
...
class RooflineDiagnostic:
    ...
    ai_str = fmt(arith_intensity, precision=0, commas=False)
...
**Problem**: A 70B parameter LLM is deployed on 8$\times$ H100 GPUs with tensor parallelism. At batch size 1, each GPU holds approximately 8.75B parameters in FP16 (`{python} RooflineDiagnostic.weight_gb_str` GB of weights). Each decode step reads its weight shard to produce one token. What is the achieved arithmetic intensity, and what is the theoretical maximum token generation rate?
...
class FusionTrafficCalc:
    ...
    materialized_mb_str = fmt((materialized_bytes * byte).m_as(MB), precision=1, commas=False)
    traffic_mb_str = fmt(traffic_mb, precision=1, commas=False)
...
class FlashAttentionSavings:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    from mlsysim.fmt import fmt
    naive_mb_str = fmt((naive_attn_bytes_total * byte).m_as(MB), precision=1, commas=False)
    flash_mb_str = fmt((flash_attn_bytes_total * byte).m_as(MB), precision=1, commas=False)
    ...
    naive_head_traffic_mb_str = fmt((naive_attn_bytes_per_head * byte).m_as(MB), precision=1, commas=False)
    flash_head_traffic_mb_str = fmt((flash_attn_bytes_per_head * byte).m_as(MB), precision=1, commas=False)
...
class KVCacheAnalysis:
    ...
    gpu_total_gb = Hardware.Cloud.H100.memory_capacity.m_as(GiB)
    int4_bits = 4
    fp16_bits = 16
    ...
    kv_fp16_per_gpu_gb_str = fmt(kv_fp16_gb_val/tensor_parallel_degree, precision=4, commas=False)
    kv_int8_total_gb_str = fmt(kv_int8_gb_val, precision=3, commas=False)
    kv_int8_per_gpu_gb_str = fmt(kv_int8_per_gpu, precision=4, commas=False)
    ...
    avail_fp16_gb_str = fmt_int(avail_fp16_gb, commas=False)
    ...
    weight_fp16_shard_gb_str = fmt(weight_fp16_shard_gb, precision=0, commas=False)
    weight_fp16_total_gb_str = fmt(weight_fp16_shard_gb * tensor_parallel_degree, precision=0, commas=False)
    tensor_parallel_degree_str = fmt(tensor_parallel_degree, precision=0, commas=False)
    gpu_total_gb_str = fmt(gpu_total_gb, precision=0, commas=False)
    int4_scale_num_str = fmt(int4_bits, precision=0, commas=False)
    int4_scale_den_str = fmt(fp16_bits, precision=0, commas=False)
```
...
**Problem**: A 70B parameter model is served on `{python} KVCacheAnalysis.tensor_parallel_degree_str`$\times$ H100 GPUs. The model weights in FP16 consume `{python} KVCacheAnalysis.weight_fp16_total_gb_str` GB (`{python} KVCacheAnalysis.weight_fp16_shard_gb_str` GB per GPU). KV cache at FP16 consumes `{python} KVCacheAnalysis.kv_fp16_gb_str` GB per request. How does quantizing weights to INT4 and KV cache to INT8 change the maximum batch size?

**Before optimization** (all FP16):

- Weights: `{python} KVCacheAnalysis.weight_fp16_shard_gb_str` GB/GPU
- Available for KV cache: `{python} KVCacheAnalysis.gpu_total_gb_str` - `{python} KVCacheAnalysis.weight_fp16_shard_gb_str` = `{python} KVCacheAnalysis.avail_fp16_gb_str` GB/GPU
...
- Weights: `{python} KVCacheAnalysis.weight_fp16_shard_gb_str` GB $\times$ (`{python} KVCacheAnalysis.int4_scale_num_str`/`{python} KVCacheAnalysis.int4_scale_den_str`) = `{python} KVCacheAnalysis.weight_int4_gb_str` GB/GPU (INT4)
- Available for KV cache: `{python} KVCacheAnalysis.gpu_total_gb_str` - `{python} KVCacheAnalysis.weight_int4_gb_str` $=$ `{python} KVCacheAnalysis.avail_int8_gb_str` GB/GPU
...
**Problem**: A 13B parameter model is deployed for inference. Without compilation, the PyTorch eager mode processes `{python} CompilationDividend.base_tps_str` tokens/second on a single H100. The Nsight Systems trace reveals that `{python} CompilationDividend.elementwise_share_pct_str` percent of step time is spent in element-wise kernels (LayerNorm, GELU, residual additions) and `{python} CompilationDividend.launch_share_pct_str` percent is kernel launch overhead. Applying `torch.compile` with the `max-autotune` backend, estimate the new throughput.

**Math**:

Step 1: Identify the addressable overhead.
Element-wise kernels (`{python} CompilationDividend.elementwise_share_pct_str` percent) and launch overhead (`{python} CompilationDividend.launch_share_pct_str` percent) total `{python} CompilationDividend.addressable_share_pct_str` percent of execution time. torch.compile fuses element-wise operations (reducing their time by approximately `{python} CompilationDividend.fusion_reduction_pct_str` percent due to eliminated HBM round-trips) and reduces kernel launches (eliminating most launch overhead).
...
- Element-wise time (`{python} CompilationDividend.fusion_reduction_pct_str` percent reduction): `{python} CompilationDividend.tpt_str` $\times 0.35 \times 0.30 =$ `{python} CompilationDividend.elem_ms_str` ms
- Launch overhead (`{python} CompilationDividend.launch_reduction_pct_str` percent reduction): `{python} CompilationDividend.tpt_str` $\times 0.15 \times 0.20 =$ `{python} CompilationDividend.launch_ms_str` ms
...
class OverlapCalc:
    ...
    gradient_gb_str = fmt(gradient_qty.m_as(GB), precision=1, commas=False)
...
class RooflineProfilingRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_fp16_ridge_str = fmt(Hardware.Cloud.H100.ridge_point("fp16").m_as("flop/byte"), precision=1, commas=False)
```
...
class MemoryWallProfilingRecap:
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_hbm_bw_str = fmt(Hardware.Cloud.H100.memory.bandwidth.m_as(TB/second), precision=2, commas=False)
```
...
class LayerNormRoofline:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    achieved_tflops_str = fmt(achieved_tflops, precision=0, commas=False)
    achieved_tbs_str = fmt(achieved_tbs, precision=1, commas=False)
    arithmetic_intensity_str = fmt(arithmetic_intensity, precision=1, commas=False)
```
Consider a concrete example: a LayerNorm kernel profiled on an H100 reports `{python} LayerNormRoofline.achieved_tflops_str` TFLOP/s of achieved compute and `{python} LayerNormRoofline.achieved_tbs_str` TB/s of achieved memory bandwidth. Its arithmetic intensity is `{python} LayerNormRoofline.achieved_tflops_str` TFLOP/s / `{python} LayerNormRoofline.achieved_tbs_str` TB/s $\approx$ `{python} LayerNormRoofline.arithmetic_intensity_str` FLOP/byte. The H100's ridge point is approximately `{python} RooflineProfilingRecap.h100_fp16_ridge_str` FLOP/byte at FP16. Since `{python} LayerNormRoofline.arithmetic_intensity_str` is far below `{python} RooflineProfilingRecap.h100_fp16_ridge_str`, the kernel is strictly memory-bound. Its achieved `{python} LayerNormRoofline.achieved_tbs_str` TB/s (84 percent of the H100's `{python} MemoryWallProfilingRecap.h100_hbm_bw_str` TB/s peak bandwidth) confirms it is operating near the physical limit of the memory subsystem. The diagnosis is clear: further FLOP-level optimizations will yield little gain; performance can only be improved materially by reducing data movement, either through fusion (eliminating the HBM round-trip) or precision reduction (halving the bytes per element).
...
class ProfilerDetective:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────
    observed_tps_str = fmt_int(observed_tps, commas=False)
    ceiling_tps_str = fmt_int(ceiling_tps, commas=False)
    decode_traffic_gb_str = fmt(decode_traffic_gb, precision=0, commas=False)
    hidden_performance_pct_str = fmt(hidden_performance_pct, precision=0, commas=False)
    gemm_bandwidth_tbs_str = fmt(gemm_bandwidth_tbs, precision=1, commas=False)
    gemm_efficiency_pct_str = fmt(gemm_efficiency_pct, precision=0, commas=False)
    gemm_time_share_pct_str = fmt(gemm_time_share_pct, precision=0, commas=False)
    attn_share_pct_str = fmt(attn_share_pct, precision=0, commas=False)
    layernorm_share_pct_str = fmt(layernorm_share_pct, precision=0, commas=False)
    softmax_share_pct_str = fmt(softmax_share_pct, precision=0, commas=False)
    launch_gap_pct_str = fmt(launch_gap_pct, precision=0, commas=False)
    kv_cache_bandwidth_tbs_str = fmt(kv_cache_bandwidth_tbs, precision=1, commas=False)
    ...
    non_gemm_share_pct_str = fmt(non_gemm_share_pct, precision=0, commas=False)
```

::: {#nbk-performance-engineering-profiler-detective .callout-notebook title="The profiler detective"}

**Problem**: A 7B parameter LLM runs on a single H100 and shows `{python} ProfilerDetective.observed_tps_str` tokens/second during autoregressive generation at batch size 1. The pure FP16 weight-read roofline is higher, but after budgeting roughly `{python} ProfilerDetective.decode_traffic_gb_str` GB of total per-token traffic for weights, KV-cache reads, sampling, synchronization, and launch overhead, the bandwidth-limited full-decode ceiling is approximately `{python} ProfilerDetective.ceiling_tps_str` tokens/second. Where is the remaining `{python} ProfilerDetective.hidden_performance_pct_str` percent of realizable decode-step performance hiding?

**Investigation**:

Step 1: Kernel-level analysis. Run Nsight Compute on the dominant GEMM kernel. Result: achieves `{python} ProfilerDetective.gemm_bandwidth_tbs_str` TB/s effective bandwidth out of the H100's `{python} MemoryWallProfilingRecap.h100_hbm_bw_str` TB/s peak. Efficiency: `{python} ProfilerDetective.gemm_efficiency_pct_str` percent. This kernel is performing well.

Step 2: Trace-level analysis. Run Nsight Systems on a full decode step. Result: `{python} ProfilerDetective.gemm_time_share_pct_str` percent of step time is spent in GEMM kernels. The remaining `{python} ProfilerDetective.non_gemm_share_pct_str` percent is split between:

- Attention kernels (including KV cache reads): `{python} ProfilerDetective.attn_share_pct_str` percent
- Layer normalization and activation kernels: `{python} ProfilerDetective.layernorm_share_pct_str` percent
- Softmax and top-$k$ sampling: `{python} ProfilerDetective.softmax_share_pct_str` percent
- Kernel launch gaps: `{python} ProfilerDetective.launch_gap_pct_str` percent

Step 3: Identify optimization targets.

- The KV cache attention kernel achieves only `{python} ProfilerDetective.kv_cache_bandwidth_tbs_str` TB/s bandwidth because of irregular memory access patterns. Fix: Implement KV cache quantization (INT8) with better memory layout.
- Kernel launch gaps (`{python} ProfilerDetective.launch_gap_pct_str` percent of time) come from `{python} ProfilerDetective.kernels_per_layer_str`+ individual kernel launches per layer. Fix: Apply `torch.compile` to fuse element-wise operations, reducing to ~`{python} ProfilerDetective.fused_kernels_per_layer_str` kernels per layer.
...
class FleetEfficiencyCalc:
    ...
    fleet_local_mfu_str = fmt_int(round(local_mfu * 100), commas=False)
    fleet_global_mfu_str = fmt_int(round(global_mfu * 100), commas=False)
    fleet_scaling_tax_str = fmt(scaling_tax, precision=1, commas=False)
```
...
*   **Fleet performance**\index{Model FLOPs Utilization!fleet performance}: At `{python} FleetEfficiencyCalc.fleet_nodes_str` GPUs, the step time increases to **`{python} FleetEfficiencyCalc.fleet_t_step_ms_str` ms**, dropping MFU to **`{python} FleetEfficiencyCalc.fleet_global_mfu_str` percent**.
