...
**Systems conclusion**: Fairness is not free. Enforcing parity cost **`{python} FairnessTaxAnalysis.fairness_tax_str` percent accuracy** (a huge drop in credit scoring). This is the fairness tax, the explicit cost of correcting for historical bias.
...
# │ Show: accuracy_loss ≈ 2.5%—inline in notebook result.
...
class BiasTax:
    ...
    ab_gap_str = fmt(gap*100, precision=1, commas=False, suffix="%")
...
3. **The bias tax**: `{python} BiasTax.ab_base_acc_str` - `{python} BiasTax.ab_fair_acc_str` = **`{python} BiasTax.ab_gap_str`**.
