...
class FairnessTaxAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    acc_unconstrained_str = fmt(acc_baseline, precision=1, commas=False)
    acc_parity_str = fmt(acc_constrained, precision=1, commas=False)
    fairness_tax_str = fmt(fairness_tax, precision=1, commas=False)
...
class BiasTax:
    ...
    ab_base_acc_str = fmt(std_acc*100, precision=1, commas=False, suffix="%")
...
class PrivacyPriceAnalysis:
    ...
    acc_drop_strong_str = fmt(acc_drop_strong, precision=1, commas=False)
    overhead_mod_str = fmt(overhead_mod, precision=1, commas=False)
    acc_drop_mod_str = fmt(acc_drop_mod, precision=1, commas=False)
```
...
class UnlearningCostAnalysis:
    ...
    n_shards_str = fmt(n_shards, precision=1, commas=False)
    sisa_cost_str = fmt(sisa_cost_m * 1_000_000, precision=1)
...
class RepresentationTax:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    subgroup_count_str = fmt(subgroup_count, precision=1, commas=False)
    images_per_subgroup_str = fmt(images_per_subgroup, precision=1)
    midpoint_cost_usd_str = fmt(midpoint_cost_per_image, precision=1, commas=False, prefix="$")
    acquisition_total_usd_m_str = fmt(acquisition_total_m, precision=1, commas=False, prefix="$", suffix="M")
    ...
    overhead_low_str = fmt(overhead_low * 100, precision=1, commas=False)
    overhead_high_str = fmt(overhead_high * 100, precision=1, commas=False)
```
...
class ExplainabilityRetrofitCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    shap_base_latency_ms_str = fmt(base_latency_ms, precision=1, commas=False)
    shap_overhead_low_ms_str = fmt(overhead_low_ms, precision=1, commas=False)
    shap_overhead_high_ms_str = fmt(overhead_high_ms, precision=1, commas=False)
    shap_total_low_ms_str = fmt(total_low_ms, precision=1, commas=False)
    shap_total_high_ms_str = fmt(total_high_ms, precision=1, commas=False)
