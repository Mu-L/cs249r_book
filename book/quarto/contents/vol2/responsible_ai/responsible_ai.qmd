...
$$
\text{DI}(a,b) = \frac{\Pr(h(x) = 1 \mid A = a)}{\Pr(h(x) = 1 \mid A = b)}
$$
...
For the loan approval example from earlier, we calculated $\Pr(h(x)=1 \mid A=a) = 0.70$ and $\Pr(h(x)=1 \mid A=b) = 0.40$. The disparate impact ratio is:
$$
...
$$
\text{SPD}(a,b) = \Pr(h(x) = 1 \mid A = a) - \Pr(h(x) = 1 \mid A = b)
$$
...
$$
\text{EOD}(a,b) = \Pr(h(x) = 1 \mid A = a, Y = 1) - \Pr(h(x) = 1 \mid A = b, Y = 1)
$$
...
\begin{align*}
\text{AOD}(a,b) = \frac{1}{2}\Big[&\big|\Pr(h(x) = 1 \mid A = a, Y = 1) - \Pr(h(x) = 1 \mid A = b, Y = 1)\big| \\
&+ \big|\Pr(h(x) = 1 \mid A = a, Y = 0) - \Pr(h(x) = 1 \mid A = b, Y = 0)\big|\Big]
\end{align*}
...
For demographic parity, test the null hypothesis $H_0: \Pr(h(x)=1 \mid A=a) = \Pr(h(x)=1 \mid A=b)$ using a two-proportion z-test. The test statistic is:
$$
