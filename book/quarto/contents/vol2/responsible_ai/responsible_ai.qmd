...
class BiasTax:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ab_base_acc_str = f"{std_acc*100:.0f}%"
    ab_fair_acc_str = f"{fair_acc*100:.1f}%"
    ab_gap_str = f"{gap*100:.1f}%"
...
**Moderate privacy ($\epsilon =$ `{python} PrivacyPriceAnalysis.eps_mod`, Apple's standard for keyboard predictions)**: Less noise ($\sigma = 0.5$). Training overhead is **`{python} PrivacyPriceAnalysis.overhead_mod_str` percent**. Accuracy drops **`{python} PrivacyPriceAnalysis.acc_drop_mod_str` percent**.
...
class UnlearningCostAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    sisa_cost_str = f"{sisa_cost_m * 1_000_000:,.0f}"
...
**Engineering fix (SISA)**: Sharded, Isolated, Sliced, and Aggregated training partitions data into $K =$ `{python} UnlearningCostAnalysis.n_shards` independent shards, training 100 sub-models. To delete one datum, retrain only the specific shard containing it (1 percent of data). New cost: **\$`{python} UnlearningCostAnalysis.sisa_cost_str`**. Time: approximately **`{python} UnlearningCostAnalysis.sisa_time_hr_str` hours**.
