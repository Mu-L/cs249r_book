...
class FairnessTaxAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    acc_unconstrained_str = fmt(acc_baseline, precision=0, commas=False, suffix=' percent')
    acc_parity_str = fmt(acc_constrained, precision=0, commas=False, suffix=' percent')
    ...
    group_a_default_pct_str = fmt(group_a_default_pct, precision=0, commas=False, suffix=' percent')
    group_b_default_pct_str = fmt(group_b_default_pct, precision=0, commas=False, suffix=' percent')
    default_prob_threshold_str = fmt(default_prob_threshold, precision=0, commas=False, suffix=' percent')
    group_a_approval_pct_str = fmt(group_a_approval_pct, precision=0, commas=False, suffix=' percent')
    group_b_approval_unconstrained_pct_str = fmt(group_b_approval_unconstrained_pct, precision=0, commas=False, suffix=' percent')
    default_prob_threshold_parity_str = fmt(default_prob_threshold_parity, precision=0, commas=False, suffix=' percent')
```
...
class BiasTax:
    ...
    ab_gap_str = fmt(gap*100, precision=1, commas=False, suffix=' percent')
    disparity_pct_str = fmt(disparity_pct, precision=0, commas=False, suffix=' percent')
```

::: {#nbk-responsible-ai-fairness-efficiency-frontier .callout-notebook title="The fairness-efficiency frontier"}

**Problem**: Consider optimizing a hiring model. The "unconstrained" model reaches `{python} BiasTax.ab_base_acc_str` accuracy but exhibits a `{python} BiasTax.disparity_pct_str` disparity between demographic groups. Applying a fairness constraint (demographic parity) eliminates the disparity. What is the "bias tax" on model performance?
...
class PrivacyPriceAnalysis:
    ...
    acc_drop_strong_str = fmt(acc_drop_strong, precision=0, commas=False, suffix=' percent')
    overhead_mod_str = fmt(overhead_mod, precision=0, commas=False, suffix=' percent')
    acc_drop_mod_str = fmt(acc_drop_mod, precision=0, commas=False, suffix=' percent')
    clip_fraction_pct_str = fmt(clip_fraction_pct, precision=0, commas=False, suffix=' percent')
```
...
class UnlearningCostAnalysis:
    ...
    sisa_cost_str = fmt(sisa_cost_m * MILLION, precision=0)
    sisa_time_hr_str = fmt(sisa_time_hr, precision=1, commas=False, suffix=' hours')
    ...
    acc_drop_high_pct_str = fmt(acc_drop_high_pct, precision=0, commas=False, suffix=' percent')
...
The preceding material focused on technical methods for fairness, privacy, and explainability. Real-world ML deployment, however, forces a confrontation with value tensions that no algorithm can resolve. Technical excellence is necessary but insufficient for trustworthy AI: stakeholders hold legitimately different conceptions of fairness, privacy, and accountability that cannot be reconciled through better algorithms.
...
class RepresentationTax:
    ...
    overhead_high_str = fmt(overhead_high * 100, precision=0, commas=False, suffix=' percent')
    overall_acc_str = fmt(overall_acc, precision=0, commas=False, suffix=' percent')
    subgroup_acc_str = fmt(subgroup_acc, precision=0, commas=False, suffix=' percent')
...
A medical imaging model trained on data from `{python} RepresentationTax.hospitals_baseline_str` major urban hospitals achieves `{python} RepresentationTax.overall_acc_str` accuracy overall but only `{python} RepresentationTax.subgroup_acc_str` on underrepresented populations (rural patients, elderly patients, patients with darker skin tones). Closing this gap requires representative data from `{python} RepresentationTax.hospitals_target_str`+ hospitals across diverse geographies, demographics, and equipment types.
...
class ExplainabilityRetrofitCost:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    shap_base_latency_ms_str = fmt(base_latency_ms, precision=0, commas=False, suffix=' ms')
    shap_overhead_low_ms_str = fmt(overhead_low_ms, precision=0, commas=False)
    shap_overhead_high_ms_str = fmt(overhead_high_ms, precision=0, commas=False, suffix=' ms')
    shap_total_low_ms_str = fmt(total_low_ms, precision=0, commas=False)
    shap_total_high_ms_str = fmt(total_high_ms, precision=0, commas=False, suffix=' ms')
...
# │ Exports: FairnessGapCalc.approval_gap_str, .tpr_gap_str,
# │   .fpr_gap_str, .ppv_gap_str
# └─────────────────────────────────────────────────────────────────────────────
...
class FairnessGapCalc:
    ...
    # ┌── 4. OUTPUT (Formatting) ───────────────────────────────────────────
    approval_gap_str = fmt_int(approval_gap)
    tpr_gap_str = fmt_int(tpr_gap)
    fpr_gap_str = fmt_int(fpr_gap)
    ppv_gap_str = fmt_int(ppv_gap)
```

| **Metric**                                         | **Definition**                    |      **A** |      **B** |                                        **Gap** |
|:---------------------------------------------------|:----------------------------------|-----------:|-----------:|-----------------------------------------------:|
| **Approval Rate**                                  | (TP + FP) divided by total        | 55 percent | 40 percent | `{python} FairnessGapCalc.approval_gap_str` pp |
| **True Positive Rate**                             | TP divided by positives           | 90 percent | 60 percent |      `{python} FairnessGapCalc.tpr_gap_str` pp |
| **False Positive Rate**\index{False Positive Rate} | FP divided by negatives           | 20 percent | 20 percent |      `{python} FairnessGapCalc.fpr_gap_str` pp |
| **Positive Predictive Value**                      | TP divided by predicted positives | 82 percent | 75 percent |      `{python} FairnessGapCalc.ppv_gap_str` pp |

: **Disaggregated Fairness Metrics**\index{Disaggregated Fairness Metrics}: A hypothetical loan approval system satisfies equalized false positive rates (`{python} FairnessGapCalc.fpr_gap_str` pp gap) but violates demographic parity (`{python} FairnessGapCalc.approval_gap_str` pp approval gap) and equal opportunity (`{python} FairnessGapCalc.tpr_gap_str` pp TPR gap). The metrics expose incompatible operational priorities, so production systems must make explicit choices about which fairness criterion to prioritize. {#tbl-responsible-ai-fairness-summary}
