...
**Systems insight**: Fairness is not free. Enforcing parity cost **`{python} FairnessTaxAnalysis.fairness_tax_str` accuracy points** (about a `{python} FairnessTaxAnalysis.fairness_tax_relative_str` relative accuracy drop in this credit-scoring scenario). This is the fairness tax, the explicit cost of correcting for historical bias.
...
3. **The bias tax**: `{python} BiasTax.ab_base_acc_str` - `{python} BiasTax.ab_fair_acc_str` = **`{python} BiasTax.ab_gap_str` accuracy points**.
...
**Data acquisition cost**: \$`{python} RepresentationTax.cost_low_usd_str`--\$`{python} RepresentationTax.cost_high_usd_str` per labeled medical image, with `{python} RepresentationTax.images_per_subgroup_str` needed per underrepresented subgroup.

**For `{python} RepresentationTax.subgroup_count_str` underrepresented subgroups**:

At the midpoint of the cost range, `{python} RepresentationTax.subgroup_count_str` subgroups times `{python} RepresentationTax.images_per_subgroup_str` per subgroup times \$`{python} RepresentationTax.midpoint_cost_usd_str` per image yields **\$`{python} RepresentationTax.acquisition_total_usd_m_str`M in data acquisition alone**.

**Data harmonization** (normalizing across different scanners, protocols, and labeling conventions) adds `{python} RepresentationTax.overhead_low_str`--`{python} RepresentationTax.overhead_high_str` percent overhead, bringing the total to **\$`{python} RepresentationTax.harmonized_low_usd_m_str`M--\$`{python} RepresentationTax.harmonized_high_usd_m_str`M**.
