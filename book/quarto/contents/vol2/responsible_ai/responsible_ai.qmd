...
In the Fleet Stack shown in @fig-fleet-stack, Responsible AI is the **Governance Layer**\index{Governance Layer}—the top of the stack where the system meets the real world. Earlier parts built the fleet (Part I), the distribution logic (Part II), the serving infrastructure (Part III), and the security and privacy controls (earlier in Part IV). The Governance Layer adds the constraints that determine *why* the machine runs and *whom* it serves, ensuring that an otherwise correct system does not impose societal harms. If the iron law\index{Iron Law} defines efficiency, Responsible AI defines **Stability**\index{Stability}: ensuring that the system's output does not destabilize the society it operates in (for example, through bias loops or privacy erosion).
...
class FairnessTaxAnalysis:
    ...
    group_a_default_pct_str = fmt(group_a_default_pct, precision=0, commas=False)
    group_b_default_pct_str = fmt(group_b_default_pct, precision=0, commas=False)
    default_prob_threshold_str = fmt(default_prob_threshold, precision=0, commas=False)
    group_a_approval_pct_str = fmt(group_a_approval_pct, precision=0, commas=False)
    group_b_approval_unconstrained_pct_str = fmt(group_b_approval_unconstrained_pct, precision=0, commas=False)
    default_prob_threshold_parity_str = fmt(default_prob_threshold_parity, precision=0, commas=False)
```

::: {#nbk-responsible-ai-fairness-tax .callout-notebook title="The fairness tax"}

**Problem**: Consider a credit model with **`{python} FairnessTaxAnalysis.acc_unconstrained_str` percent accuracy**. Group A (majority) has a `{python} FairnessTaxAnalysis.group_a_default_pct_str` percent default rate. Group B (minority) has a `{python} FairnessTaxAnalysis.group_b_default_pct_str` percent default rate due to systemic factors. If **Demographic Parity** (equal approval rates) is enforced, what happens to accuracy?
...
1.  **Unconstrained**: Model approves everyone with predicted default prob < `{python} FairnessTaxAnalysis.default_prob_threshold_str` percent.
    *   Group A approval: `{python} FairnessTaxAnalysis.group_a_approval_pct_str` percent.
    *   Group B approval: `{python} FairnessTaxAnalysis.group_b_approval_unconstrained_pct_str` percent.
    *   Total Accuracy: **`{python} FairnessTaxAnalysis.acc_unconstrained_str` percent**.
2.  **Constrained (Parity)**: Must approve Group B at `{python} FairnessTaxAnalysis.group_a_approval_pct_str` percent rate.
    *   New threshold for Group B: Approve default prob < `{python} FairnessTaxAnalysis.default_prob_threshold_parity_str` percent.
...
2.  **Distinction (durable)**: Unlike Equalized Odds (which focuses on error rates like False Positives), Demographic Parity focuses only on the Final Prediction\index{Demographic Parity!final prediction}, ignoring the relationship between the prediction and the ground truth.
...
class BiasTax:
    ...
    disparity_pct_str = fmt(disparity_pct, precision=0, commas=False)
```

::: {#nbk-responsible-ai-fairness-efficiency-frontier .callout-notebook title="The fairness-efficiency frontier"}

**Problem**: Consider optimizing a hiring model. The "unconstrained" model reaches `{python} BiasTax.ab_base_acc_str` accuracy but exhibits a `{python} BiasTax.disparity_pct_str` percent disparity between demographic groups. Applying a fairness constraint (demographic parity) eliminates the disparity. What is the "bias tax" on model performance?
...
class PrivacyPriceAnalysis:
    ...
    clip_fraction_pct_str = fmt(clip_fraction_pct, precision=0, commas=False)
```
...
class UnlearningCostAnalysis:
    ...
    sisa_time_hr_str = fmt(sisa_time_hr, precision=1, commas=False)
    full_retrain_gpus_str = fmt(full_retrain_gpus, precision=0, commas=True)
    full_time_days_str = fmt(full_time_days, precision=0, commas=False)
    acc_drop_low_pct_str = fmt(acc_drop_low_pct, precision=0, commas=False)
    acc_drop_high_pct_str = fmt(acc_drop_high_pct, precision=0, commas=False)
    deletion_requests_per_day_str = fmt(deletion_requests_per_day, precision=0, commas=True)
```
...
**Baseline (full retraining)**: For a 175B-parameter model at GPT-3 scale, retraining requires `{python} UnlearningCostAnalysis.full_retrain_gpus_str` A100 GPUs for approximately `{python} UnlearningCostAnalysis.full_time_days_str` days at a cost of roughly **\$`{python} UnlearningCostAnalysis.full_cost_m_str` million**.
...
**Failure**: The controversy centered on the engineering failure that compounded the disparity: when customers called to ask *why* they were denied, support staff could not answer. The algorithm offered no recourse, no explanation, and no mechanism for appeal.
...
class RepresentationTax:
    ...
    overall_acc_str = fmt(overall_acc, precision=0, commas=False)
    subgroup_acc_str = fmt(subgroup_acc, precision=0, commas=False)
    hospitals_baseline_str = fmt(hospitals_baseline, precision=0, commas=False)
    hospitals_target_str = fmt(hospitals_target, precision=0, commas=False)
    cost_low_usd_str = fmt(low_cost_per_image, precision=0, commas=False, prefix="$")
    cost_high_usd_str = fmt(high_cost_per_image, precision=0, commas=False, prefix="$")
```

::: {#nbk-responsible-ai-representation-tax .callout-notebook title="The representation tax"}

A medical imaging model trained on data from `{python} RepresentationTax.hospitals_baseline_str` major urban hospitals achieves `{python} RepresentationTax.overall_acc_str` percent accuracy overall but only `{python} RepresentationTax.subgroup_acc_str` percent on underrepresented populations (rural patients, elderly patients, patients with darker skin tones). Closing this gap requires representative data from `{python} RepresentationTax.hospitals_target_str`+ hospitals across diverse geographies, demographics, and equipment types.

**Data acquisition cost**: `{python} RepresentationTax.cost_low_usd_str`--`{python} RepresentationTax.cost_high_usd_str` per labeled medical image, with `{python} RepresentationTax.images_per_subgroup_str` images needed per underrepresented subgroup.
...
class ExplainabilityRetrofitCost:
    ...
    shap_capacity_high_str = fmt(capacity_high, precision=0, commas=False)
```
