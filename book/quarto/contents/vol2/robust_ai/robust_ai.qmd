...
* **Robustness vs. Accuracy**\index{Robustness vs. Accuracy}: Building resilient systems requires an explicit trade-off. Strong ImageNet-scale adversarial training can cost roughly `{python} RobustnessTaxAnalysis.acc_drop_str` percentage points of clean accuracy, while lighter threat models or randomized smoothing may cost single-digit to mid-teen points; compute costs still rise by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
