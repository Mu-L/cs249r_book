...
class RobustAISetup:
    ...
    v100_mem_bw_str = fmt(v100_bw_gbs_val, precision=1, commas=False)
...
class SilentErrorProbability:
    ...
    n_gpus_large_str = fmt(n_gpus_large, precision=1)
    p_total_large_str = fmt(p_at_least_one_val * 100, precision=1)
    n_gpus_certain_str = fmt(n_gpus_certain_val, precision=1)
```
...
class AdversarialPayback:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ap_std_passes_str = fmt(std_passes, precision=1, commas=False)
    ap_n_pgd_steps_str = fmt(n_pgd_steps, precision=1, commas=False)
    ap_slowdown_str = fmt(slowdown, precision=1, commas=False)
    ap_cert_acc_str = fmt(robust_acc*100, precision=1, commas=False, suffix="%")
    ap_gap_pct_str = fmt(accuracy_gap_pct, precision=1, commas=False)
...
class ShiftConfidence:
    ...
    ds_n_samples_str = fmt(n_samples, precision=1)
...
class DistributionShiftMetrics:
    ...
    mmd_n_samples_str = fmt(n_samples_mmd, precision=1)
    mmd_latency_ms_str = fmt(mmd_latency_ms, precision=1, commas=False)
    ks_n_samples_str = fmt(n_samples_ks, precision=1)
...
class RobustnessTaxAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    clean_acc_str = fmt(clean_acc, precision=1, commas=False)
    robust_acc_str = fmt(robust_acc, precision=1, commas=False)
...
class HuberOutlierScaling:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    normal_error_multiple_str = fmt(normal_error_multiple, precision=1, commas=False)
    squared_loss_multiple_str = fmt(squared_loss_multiple, precision=1)
    residual_gradient_multiple_str = fmt(residual_gradient_multiple, precision=1, commas=False)
```
