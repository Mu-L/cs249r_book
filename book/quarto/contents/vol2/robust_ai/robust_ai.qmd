...
**The context**: Google Photos launched automatic image labeling to organize user photos at consumer scale.

**The failure**: In 2015, the system labeled photos of Black users as "gorillas," a severe misclassification that reflected a model and evaluation pipeline that had not prevented harmful slice-level failure.

**The consequence**: Google apologized and disabled the offending label path while working on longer-term fixes, turning a quiet classifier error into a public robustness and trust failure.

**The systems lesson**: Robustness is not aggregate accuracy. Production vision systems need slice-level evaluation, harmful-label tests, and safe fallback behavior for labels whose errors carry high social cost [@kasperkevic2015googlephotos].
...
# │ Show: 10K devices at illustrative p=1e-4/hr gives P>0.6; ~30K gives P>0.95.
...
class SilentErrorProbability:
    ...
    check(n_gpus_certain_val > n_gpus_large, "95% threshold should be above 10K devices")
...
ax.annotate(f'At $p=10^{{-4}}$:\n$P={annot_prob:.2f}$ by 10K devices',
...
Robust AI systems must address two primary categories of challenges (environmental shifts and input-level attacks) along with a cross-cutting concern, software faults, that can amplify or masquerade as either. The **Three Pillars Framework**\index{Three Pillars Framework} in @fig-three-pillars-framework organizes these threats, each representing distinct but interconnected vulnerabilities that require complementary defense strategies:
...
1. **Forward/Backward Passes**\index{Forward/Backward Passes}: `{python} AdversarialPayback.ap_std_passes_str` (Standard) + `{python} AdversarialPayback.ap_n_pgd_steps_str` (Attack Generation) = **`{python} AdversarialPayback.ap_slowdown_str` total passes**.
...
**Problem**: A model monitors a critical input feature. The baseline mean was `{python} ShiftConfidence.baseline_mean_str`. Over the last `{python} ShiftConfidence.ds_n_samples_str` requests, the mean has shifted to `{python} ShiftConfidence.current_mean_str`. The engineering question is whether this is a random fluctuation or a real distribution shift\index{Distribution Shift}.

**Math**:
Detection requires proving that the observed change is statistically unlikely under the baseline distribution.

1. **Difference in Means**\index{Difference in Means}: `{python} ShiftConfidence.ds_mean_diff_str`.
2. **Standard Error**\index{Standard Error}: $0.3/\sqrt{1000} \approx$ `{python} ShiftConfidence.ds_std_error_str`.
3. **Statistical Significance**\index{Statistical Significance}: The shift is **approximately `{python} ShiftConfidence.ds_z_score_str` standard errors** away from the mean.
...
Label shift affects the distribution of output classes without changing the input-output relationship [@lipton2018detecting]. COVID-19 caused dramatic label shift in medical imaging: pneumonia prevalence rose from roughly 12 percent to as high as 35 percent in some hospital systems, requiring recalibration of diagnostic thresholds. Seasonal label shift in agriculture monitoring shows crop disease prevalence varying by 40--60 percent between growing seasons, necessitating adaptive decision boundaries for accurate yield prediction [@hendrycks2017].
...
class DistributionShiftMetrics:
    ...
    mmd_latency_ms_str = fmt(mmd_latency_ms, precision=0, commas=False)
...
The Population Stability Index quantifies how much a distribution has shifted between two time periods, originally developed for credit scoring validation and now widely applied in ML model monitoring [@yurdakul2020]. PSI measures the divergence between an expected (baseline) distribution $P$ and an actual (current) distribution $Q$ by computing a symmetric log-ratio difference across discretized bins.
...
class RobustnessTaxAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    clean_acc_str = fmt(clean_acc, precision=0, commas=False)
    robust_acc_str = fmt(robust_acc, precision=0, commas=False)
...
**Accuracy outcome**: Gaining robustness against rare adversarial attacks sacrifices **`{python} RobustnessTaxAnalysis.acc_drop_str` percentage points of accuracy** on normal inputs.
...
The clean-accuracy cost depends on the threat model and defense. For strong ImageNet-scale adversarial training at $\epsilon = 8/255$, clean accuracy can drop by roughly `{python} RobustnessTaxAnalysis.acc_drop_str` percentage points, as in the robustness-tax example above. Lighter defenses, smaller perturbation budgets, or randomized smoothing can impose smaller costs, often in the single digits to mid-teens, but the trade-off remains fundamental to the robust optimization objective. Model size often increases with robustness-enhancing architectural modifications such as wider networks or additional normalization layers that improve gradient stability.
...
* **Robustness vs. accuracy**: Building resilient systems\index{Robustness!accuracy trade-off} requires an explicit trade-off. Strong ImageNet-scale adversarial training can cost roughly `{python} RobustnessTaxAnalysis.acc_drop_str` percentage points of clean accuracy, while lighter threat models or randomized smoothing may cost single-digit to mid-teen points; compute costs still rise by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
