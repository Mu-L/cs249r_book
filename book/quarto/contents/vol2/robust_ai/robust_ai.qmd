...
class RobustAISetup:
    ...
    base_error_rate = defaults.MEMORY_BIT_ERROR_RATE_PER_BIT
...
class DistributionShiftMetrics:
    ...
    psi_warn = defaults.PSI_WARN_THRESHOLD
    psi_critical = defaults.PSI_CRITICAL_THRESHOLD
...
thresh_warning = defaults.PSI_WARN_THRESHOLD
thresh_critical = defaults.PSI_CRITICAL_THRESHOLD
...
* **Robustness vs. accuracy**: Building resilient systems\index{Robustness!accuracy trade-off} requires an explicit trade-off. Strong ImageNet-scale adversarial training can cost roughly `{python} RobustnessTaxRecap.acc_drop_str` percentage points of clean accuracy, while lighter threat models or randomized smoothing may cost single-digit to mid-teen points; compute costs still rise by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
