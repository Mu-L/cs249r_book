...
class RobustAISetup:
    ...
    base_error_rate = MEMORY_BIT_ERROR_RATE_PER_BIT

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    gpt3_params_b_val = m.parameters.m_as(param) / BILLION
    v100_bw_gbs_val = h.memory_bw.m_as(GB/second)
    bits_per_second_val = h.memory_bw.m_as(byte/second) * BITS_PER_BYTE
    ...
    v100_mem_bw_str = fmt(v100_bw_gbs_val, precision=1, allow_zero=True, commas=False)
...
class SilentErrorProbability:
    ...
    n_gpus_large_str = fmt(n_gpus_large, precision=1, allow_zero=True)
    p_total_large_str = fmt(p_at_least_one_val * 100, precision=1, allow_zero=True)
    n_gpus_certain_str = fmt(n_gpus_certain_val, precision=1, allow_zero=True)
```
...
class AdversarialPayback:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    ap_std_passes_str = fmt(std_passes, precision=1, allow_zero=True, commas=False)
    ap_n_pgd_steps_str = fmt(n_pgd_steps, precision=1, allow_zero=True, commas=False)
    ap_slowdown_str = fmt(slowdown, precision=1, allow_zero=True, commas=False)
    ap_cert_acc_str = fmt(robust_acc*100, precision=1, allow_zero=True, commas=False, suffix="%")
    ap_gap_pct_str = fmt(accuracy_gap_pct, precision=1, allow_zero=True, commas=False)
...
3. **Utility Cost**\index{Adversarial Training!accuracy cost}: Accuracy against the worst-case attack is **`{python} AdversarialPayback.ap_cert_acc_str`**, compared with 95 percent clean-data accuracy for the standard model.
...
class ShiftConfidence:
    ...
    ds_n_samples_str = fmt(n_samples, precision=1, allow_zero=True)
...
2. **Standard Error**: $0.3/\sqrt{1000} \approx$ `{python} ShiftConfidence.ds_std_error_str`.
...
class DistributionShiftMetrics:
    ...
    mmd_n_samples_str = fmt(n_samples_mmd, precision=1, allow_zero=True)
    mmd_latency_ms_str = fmt(mmd_latency_ms, precision=1, allow_zero=True, commas=False)
    ks_n_samples_str = fmt(n_samples_ks, precision=1, allow_zero=True)
...
thresh_warning = PSI_WARN_THRESHOLD
thresh_critical = PSI_CRITICAL_THRESHOLD
...
class RobustnessTaxAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    clean_acc_str = fmt(clean_acc, precision=1, allow_zero=True, commas=False)
    robust_acc_str = fmt(robust_acc, precision=1, allow_zero=True, commas=False)
    acc_drop_str = fmt(acc_drop, precision=1, allow_zero=True, commas=False)
```
...
class HuberOutlierScaling:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    normal_error_multiple_str = fmt(normal_error_multiple, precision=1, allow_zero=True, commas=False)
    squared_loss_multiple_str = fmt(squared_loss_multiple, precision=1, allow_zero=True)
    residual_gradient_multiple_str = fmt(residual_gradient_multiple, precision=1, allow_zero=True, commas=False)
```
