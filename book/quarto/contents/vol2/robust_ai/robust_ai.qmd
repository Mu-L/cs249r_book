...
$$
\mathcal{D}_{\text{KL}}(P \lVert Q) = \int_{-\infty}^{\infty} p(x) \ln\left(\frac{p(x)}{q(x)}\right) dx
$$

where $p(x)$ and $q(x)$ are the probability density functions of the baseline and current distributions, respectively. Unlike PSI, KL divergence is asymmetric: $\mathcal{D}_{\text{KL}}(P \lVert Q) \neq \mathcal{D}_{\text{KL}}(Q \lVert P)$. For drift detection, we typically compute $\mathcal{D}_{\text{KL}}(\text{baseline} \lVert \text{current})$, measuring how much information is lost when using the current distribution to approximate the baseline.
...
$$
\mathcal{D}_{\text{JS}}(P \lVert Q) = \frac{1}{2} \mathcal{D}_{\text{KL}}(P \lVert M) + \frac{1}{2} \mathcal{D}_{\text{KL}}(Q \lVert M)
$$
...
$$
\mathcal{D}_{\text{KL}} = \ln\frac{\sigma_Q}{\sigma_P} + \frac{\sigma_P^2 + (\mu_P - \mu_Q)^2}{2\sigma_Q^2} - \frac{1}{2}
$$
