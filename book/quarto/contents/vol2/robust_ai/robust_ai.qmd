...
# │
# │ Imports: mlsysim.core.constants (*), mlsysim.fmt (check, fmt)
# │ Exports: gpt3_params_b_str, v100_mem_bw_str
# └─────────────────────────────────────────────────────────────────────────────
...
* **Robustness vs. accuracy**: Building resilient systems\index{Robustness!accuracy trade-off} requires an explicit trade-off. Strong ImageNet-scale adversarial training can cost roughly 26 percentage points of clean accuracy, while lighter threat models or randomized smoothing may cost single-digit to mid-teen points; compute costs still rise by 2--10$\times$ due to iterative attack generation or Monte Carlo sampling.
