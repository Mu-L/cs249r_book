...
**Context**: Google Photos launched automatic image labeling to organize user photos at consumer scale [@kasperkevic2015googlephotos].

**Failure**: In 2015, the system labeled photos of Black users as "gorillas," a severe misclassification that reflected a model and evaluation pipeline that had not prevented harmful slice-level failure.

**Consequence**: Google apologized and disabled the offending label path while working on longer-term fixes, turning a quiet classifier error into a public robustness and trust failure.

**Systems lesson**: Robustness is not aggregate accuracy. Production vision systems need slice-level evaluation, harmful-label tests, and safe fallback behavior for labels whose errors carry high social cost.
