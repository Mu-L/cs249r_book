...
1.  **Sensitivity** $(\Delta f)$: The maximum one person can change the sum is **\$`{python} DPCostAnalysis.sensitivity_str`**.
2.  **Privacy Budget** $(\epsilon)$: `{python} DPCostAnalysis.epsilon_str`.
3.  **Laplace Noise Scale** $(b)$: $\Delta f / \epsilon =$ `{python} DPCostAnalysis.sensitivity_str` / `{python} DPCostAnalysis.epsilon_str` = **`{python} DPCostAnalysis.noise_scale_str`**.
...
where $\gamma_{\text{rate}}$ controls sensitivity. A user with anomaly score 0.8 might face 55 percent rate reduction $(e^{-0.8} \approx 0.45)$, while score 0.1 faces negligible impact.
