...
2.  **Query monitoring**:
    ...
    - Implementation: Lightweight online ML model, `{python} DefenseOverheadAnalysis.monitoring_ms_str` overhead per query
...
-   **Extraction fidelity**: Without defenses, `{python} DefenseOverheadAnalysis.output_accuracy_no_defense_pct_str` accuracy surrogate. With defenses, surrogate accuracy drops to `{python} DefenseOverheadAnalysis.output_accuracy_with_defense_pct_str`, an `{python} DefenseOverheadAnalysis.fidelity_drop_pct_str` percentage-point reduction in this scenario.

-   **Legitimate user impact**: Top-5 accuracy preserved at `{python} DefenseOverheadAnalysis.top5_accuracy_pct_str` (vs. `{python} DefenseOverheadAnalysis.baseline_top5_accuracy_pct_str` baseline). Latency increase: +`{python} DefenseOverheadAnalysis.monitoring_ms_str` for monitoring, +`{python} DefenseOverheadAnalysis.perturbation_ms_str` for perturbation = +`{python} DefenseOverheadAnalysis.total_defense_ms_str` total (`{python} DefenseOverheadAnalysis.overhead_pct_str` overhead on `{python} DefenseOverheadAnalysis.base_inference_ms_str` inference)
-   **False positive rate**: `{python} DefenseOverheadAnalysis.false_positive_pct_str` of users flagged for manual review (`{python} DefenseOverheadAnalysis.daily_users_str` users → `{python} DefenseOverheadAnalysis.false_positive_reviews_per_day_str` reviews per day, manageable)
...
class TEEMemoryFootprint:
    ...
    sgx_latency_overflow_ms = sgx_latency_base_ms * epc_overflow_penalty
