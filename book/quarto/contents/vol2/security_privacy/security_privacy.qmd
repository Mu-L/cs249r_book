...
where $\mathcal{D}_{\text{KL}}$ is Kullback-Leibler divergence and $\tau_{\text{alert}}$ is an alert threshold. For a language model API, this might detect unusual token distributions, repetitive patterns, or adversarially crafted prompts designed to probe model boundaries.
...
Pricing strategies can make extraction economically infeasible. If training a model from scratch costs $C_{\text{train}}$ and extraction requires $n_{\text{queries}}$ queries at price $p_{\text{query}}$ per query, extraction is economically rational only if:

$$
n_{\text{queries}} \cdot p_{\text{query}} < C_{\text{train}}
$$

Setting $p_{\text{query}}$ such that $n_{\text{queries}} \cdot p_{\text{query}} > C_{\text{train}}$ eliminates the economic incentive. For example, if training a competitive language model costs \$5M and extraction requires 100M queries, pricing at $p_{\text{query}} > \$0.05$ per query (the \$5M training cost divided by 100M queries) makes extraction more expensive than training.
...
* **Security vs. privacy**: Security defends the system\index{Security!vs. privacy} from *adversarial manipulation* (poisoning, evasion); Privacy protects sensitive info from *unauthorized inference* (membership inference, inversion). Effective fleets require both.
* **LLM linguistic vulnerabilities**: Generative AI introduces nontraditional threats\index{LLM!linguistic vulnerabilities} like Prompt Injection (Direct/Indirect) and PII leakage in RAG systems. These require semantic filtering and output monitoring in the serving pipeline (@sec-security-privacy-output-monitoring-cf37), not traditional network firewalls alone.
