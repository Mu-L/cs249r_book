...
The information leakage invariant (@pri-information-leakage) governs this design space: every model output potentially leaks information about its training data, and perfect privacy is mathematically impossible if the model remains useful. Privacy is a *budget*, not a *switch*—systems requiring strict privacy must implement Differential Privacy (DP) to quantify and cap the leakage $(\epsilon)$ per query.
