...
- Extraction requirements: ~`{python} DefenseOverheadAnalysis.extraction_queries_m_str` queries for `{python} DefenseOverheadAnalysis.target_fidelity_pct_str` fidelity, or `{python} DefenseOverheadAnalysis.queries_per_class_str` queries per class for a `{python} DefenseOverheadAnalysis.class_count_str`-class classifier
...
2.  **Query monitoring**:
    ...
    - Alert on: >`{python} DefenseOverheadAnalysis.alert_queries_per_day_str` queries per day from single user, <`{python} DefenseOverheadAnalysis.alert_interquery_ms_str` median inter-query time, input entropy <threshold
...
3.  **Output perturbation**:
    ...
    - Return top-`{python} OutputRoundingLeakage.top_k_str` classes only (eliminates `{python} OutputRoundingLeakage.topk_eliminated_pct_str` of distribution entries)
