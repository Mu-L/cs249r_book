...
class DPCostAnalysis:
    ...
    sensitivity_str = fmt(sensitivity, precision=1, allow_zero=True)
    noise_scale_str = fmt(noise_scale, precision=1, allow_zero=True)
    error_per_person_str = fmt(error_per_person, precision=1, allow_zero=True)
    error_small_str = fmt(error_small, precision=1, allow_zero=True)
    # Suffix aliases for prose refs (originals kept for arithmetic above)
    n_employees_str = fmt(n_employees, precision=1, allow_zero=True, commas=False)
    epsilon_str = fmt(epsilon, precision=1, commas=False)
    n_small_str = fmt(n_small, precision=1, allow_zero=True, commas=False)
```
...
4.  **Impact on Mean**\index{Differential Privacy!mean-estimation noise}: The noise added to the *sum* has magnitude approximately `{python} DPCostAnalysis.noise_scale_str`.
...
class MultiTenantIsolation:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    mt_peak_tokens_str = fmt(peak_throughput, precision=1, allow_zero=True)
    mt_secure_tokens_str = fmt(secure_throughput, precision=1, allow_zero=True)
    mt_throughput_loss_str = fmt(throughput_loss, precision=1, allow_zero=True)
    mt_overhead_pct_str = fmt(overhead_pct, precision=1, allow_zero=True, commas=False)
...
class OutputRoundingLeakage:
    # ┌── 1. LOAD ──────────────────────────────────────────
    class_count = IMAGENET_NUM_CLASSES
    from_decimals = 6
    to_decimals = 2
    fp32_bits = FP32_BITS
    top_k = 5
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    class_count_str = fmt(class_count, precision=1, allow_zero=True, commas=False)
    from_decimals_str = fmt(from_decimals, precision=1, allow_zero=True, commas=False)
    to_decimals_str = fmt(to_decimals, precision=1, allow_zero=True, commas=False)
    decimal_digits_removed_str = fmt(decimal_digits_removed, precision=1, allow_zero=True, commas=False)
    bits_removed_str = fmt(bits_removed, precision=1, commas=False)
    fp32_bits_str = fmt(fp32_bits, precision=1, allow_zero=True, commas=False)
    ...
    top_k_str = fmt(top_k, precision=1, allow_zero=True, commas=False)
...
class TEEMemoryFootprint:
    # ┌── 1. LOAD ──────────────────────────────────────────
    resnet18_params_m = RESNET18_PARAMS.m_as(Mparam)
    resnet50_params_m = Models.Vision.ResNet50.parameters.m_as(Mparam)
    fp32_bytes = BYTES_FP32.m_as(byte)
    int8_bytes = BYTES_INT8.m_as(byte)
    sgx_epc_mb = SGX_EPC_CAPACITY.m_as(MB)
    sgx_latency_base_ms = SGX_BASE_LATENCY.m_as(ms)
    sgx_latency_overflow_ms = SGX_OVERFLOW_LATENCY.m_as(ms)
    ...
    fp32_bytes_str = fmt(fp32_bytes, precision=1, allow_zero=True, commas=False)
    sgx_epc_mb_str = fmt(sgx_epc_mb, precision=1, allow_zero=True, commas=False)
    resnet50_fp32_epc_pct_str = fmt(resnet50_fp32_epc_pct, precision=1, allow_zero=True, commas=False)
    sgx_latency_base_ms_str = fmt(sgx_latency_base_ms, precision=1, allow_zero=True, commas=False)
    sgx_latency_overflow_ms_str = fmt(sgx_latency_overflow_ms, precision=1, allow_zero=True, commas=False)
    small_model_limit_mb_str = fmt(small_model_limit_mb, precision=1, allow_zero=True, commas=False)
```
...
class DefenseOverheadAnalysis:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    from mlsysim.fmt import fmt
    class_count_str = fmt(class_count, precision=1, allow_zero=True)
    daily_queries_str = fmt(daily_queries/MILLION, precision=1, allow_zero=True, commas=False, suffix="M")
    daily_users_str = fmt(daily_users, precision=1, allow_zero=True)
    training_cost_k_str = fmt(training_cost_usd/1000, precision=1, allow_zero=True, commas=False, suffix="K")
    extraction_queries_m_str = fmt(extraction_queries/MILLION, precision=1, allow_zero=True, commas=False, suffix="M")
    target_fidelity_pct_str = fmt(target_fidelity_pct, precision=1, allow_zero=True, commas=False)
    queries_per_class_str = fmt(queries_per_class, precision=1, allow_zero=True)
    attacker_daily_queries_str = fmt(attacker_daily_queries/1000, precision=1, allow_zero=True, commas=False, suffix="K")
    extraction_days_str = fmt(extraction_days, precision=1, allow_zero=True, commas=False)
    free_tier_queries_str = fmt(free_tier_queries, precision=1, allow_zero=True)
    paid_tier_price_usd_month_str = fmt(paid_tier_price_usd_month, precision=1, allow_zero=True, commas=False)
    paid_tier_queries_str = fmt(paid_tier_queries/1000, precision=1, allow_zero=True, commas=False, suffix="K")
    alert_queries_per_day_str = fmt(alert_queries_per_day/1000, precision=1, allow_zero=True, commas=False, suffix="K")
    alert_interquery_ms_str = fmt(alert_interquery_ms, precision=1, allow_zero=True, commas=False)
    price_per_query_usd_str = fmt(price_per_query_usd, precision=3, commas=False)
    extraction_cost_k_str = fmt(extraction_cost_usd/1000, precision=1, allow_zero=True, commas=False, suffix="K")
    legit_queries_per_day_str = fmt(legit_queries_per_day, precision=1, allow_zero=True, commas=False)
    billing_days_str = fmt(billing_days, precision=1, allow_zero=True, commas=False)
    legit_monthly_cost_usd_str = fmt(legit_monthly_cost_usd, precision=1, allow_zero=True, commas=False)
    output_accuracy_no_defense_pct_str = fmt(output_accuracy_no_defense_pct, precision=1, allow_zero=True, commas=False)
    output_accuracy_with_defense_pct_str = fmt(output_accuracy_with_defense_pct, precision=1, allow_zero=True, commas=False)
    fidelity_drop_pct_str = fmt(fidelity_drop_pct, precision=1, allow_zero=True, commas=False)
    ...
    false_positive_reviews_per_day_str = fmt(false_positive_reviews_per_day, precision=1, allow_zero=True, commas=False)
    monitoring_ms_str = fmt(monitoring_ms, precision=1, allow_zero=True, commas=False)
    perturbation_ms_str = fmt(perturbation_ms, precision=1, commas=False)
    base_inference_ms_str = fmt(base_inference_ms, precision=1, allow_zero=True, commas=False)
...
class EncryptionOverhead:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    eo_base_ms_str = fmt(t_inference_ms, precision=1, allow_zero=True, commas=False)
    eo_aes_overhead_ms_str = fmt(overhead_aes_ms, precision=1, commas=False)
    eo_aes_ms_str = fmt(t_aes, precision=1, commas=False)
    eo_fhe_sec_str = fmt(t_fhe_sec, precision=1, allow_zero=True, commas=False)
