...
- **High Likelihood/Low Impact**\index{High Likelihood/Low Impact}: Model extraction attacks against public APIs. These are common and technically simple but may only affect competitive advantage rather than safety or privacy.
...
- **Low Likelihood/Low Impact**\index{Low Likelihood/Low Impact}: Membership inference attacks[^fn-membership-inference-sec] against models trained on sensitive data. These require some technical skill but mainly threaten individual privacy rather than system integrity.
...
* **LLM linguistic vulnerabilities**: Generative AI introduces nontraditional threats\index{LLM!linguistic vulnerabilities} like Prompt Injection (Direct/Indirect) and PII leakage in retrieval-augmented generation (RAG) systems. These require semantic filtering and output monitoring in the serving pipeline (@sec-security-privacy-output-monitoring-cf37), not traditional network firewalls alone.
