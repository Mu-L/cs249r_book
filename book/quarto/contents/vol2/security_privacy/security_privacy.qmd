...
**Problem**: Consider computing the average salary of **`{python} DPCostAnalysis.n_employees` employees** while guaranteeing privacy budget $\epsilon =$ `{python} DPCostAnalysis.epsilon`. The salaries range from \$0 to \$`{python} DPCostAnalysis.sensitivity_str`. How much noise must the mechanism add?

**Math**:

1.  **Sensitivity** $(S)$: The maximum one person can change the sum is **\$`{python} DPCostAnalysis.sensitivity_str`**.
2.  **Privacy Budget** $(\epsilon)$: `{python} DPCostAnalysis.epsilon`.
3.  **Laplace Noise Scale** $(b)$: $S / \epsilon =$ `{python} DPCostAnalysis.sensitivity_str` / `{python} DPCostAnalysis.epsilon` = **`{python} DPCostAnalysis.noise_scale_str`**.
...
class MultiTenantIsolation:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    mt_peak_tokens_str = f"{peak_throughput:,}"
    mt_secure_tokens_str = f"{secure_throughput:,}"
    mt_overhead_pct_str = f"{overhead_pct:.0f}"
...
class EncryptionOverhead:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    eo_base_ms_str = f"{t_inference_ms}"
    eo_aes_ms_str = f"{t_aes:.1f}"
    eo_fhe_sec_str = f"{t_fhe_sec:.0f}"
    eo_fhe_ratio_str = "10,000"
