...
class CarbonCostGPT3:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_mwh_str = fmt(energy_mwh, precision=1)
    total_emissions_kg_str = fmt(total_emissions_kg, precision=1)
...
class ArchetypeATdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_w_str = fmt(tdp_w, precision=1, commas=False)
```
...
class CarbonFrontier:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cf_energy_mwh_str = fmt(energy_mwh, precision=1)
    cf_quebec_tonnes_str = fmt(quebec_tonnes, precision=1)
    cf_poland_tonnes_str = fmt(poland_tonnes, precision=1)
    cf_ratio_str = fmt(ratio, precision=1, commas=False)
    intensity_quebec_str = fmt(intensity_quebec, precision=1, commas=False)
    intensity_poland_str = fmt(intensity_poland, precision=1, commas=False)
...
class AutoPlacement:
    ...
    carbon_tax_str = fmt(carbon_tax, precision=1, commas=False)
    energy_mwh_str = fmt(energy_mwh, precision=1)
...
class ComputeGrowthScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compute_growth_factor_str = fmt(compute_growth_factor, precision=1)
...
class EnergyWallScenario:
    ...
    battery_gain_pct_str = fmt((battery_gain - 1) * 100, precision=1, commas=False)
    grid_gain_pct_str = fmt((grid_gain - 1) * 100, precision=1, commas=False)
```
...
class LifecycleCarbonEstimate:
    ...
    training_days_str = fmt(training_days, precision=1)
    h_h100_tdp_w_str = fmt(h_h100.tdp.m_as("W"), precision=1, commas=False)
    pue_str = fmt(DummyFleet.effective_pue, precision=1, commas=False)
    grid_ci_str = fmt(DummyRegion.carbon_intensity_g_kwh, precision=1, commas=False)
    it_power_kw_str = fmt(n_gpus * h_h100.tdp.m_as("kW"), precision=1)
    facility_power_kw_str = fmt(n_gpus * h_h100.tdp.m_as("kW") * DummyFleet.effective_pue, precision=1)
    ...
    embodied_kg_per_gpu_str = fmt(embodied_kg_per_gpu, precision=1, commas=False)
...
class GpuEmissionsScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_w_str = fmt(h_h100_tdp_w_val, precision=1, commas=False)
```
...
class PueEfficiency:
    ...
    pue_savings_mwh_str = fmt(annual_savings_mwh, precision=1)
    pue_savings_usd_str = fmt(annual_savings_usd, precision=1)
...
class TrainingEmissions:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_hours_str = fmt(training_hours, precision=1)
    training_days_str = fmt(training_days, precision=1, commas=False)
    n_gpus_str = fmt(n_gpus, precision=1, commas=False)
    gpu_energy_wh_str = fmt(gpu_energy_wh, precision=1)
    gpu_energy_kwh_str = fmt(gpu_energy_kwh, precision=1)
    facility_energy_kwh_str = fmt(facility_energy_kwh, precision=1)
    ...
    emissions_ratio_str = fmt(emissions_ratio, precision=1, commas=False)
    a100_tdp_w_str = fmt(gpu_power_w, precision=1, commas=False)
    embodied_kg_per_accelerator_str = fmt(embodied_kg_per_accelerator, precision=1, commas=False)
    embodied_lifetime_years_str = fmt(embodied_lifetime_years, precision=1, commas=False)
...
class A100PowerScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_tdp_w_str = fmt(a100_tdp_w_val, precision=1, commas=False)
```
...
class EmbodiedCarbonAmort:
    ...
    share_clean_pct_str = fmt(share_clean_pct, precision=1, commas=False)
    t_job_hr_str = fmt(t_job_hr, precision=1, commas=False)
    n_gpus_str = fmt(n_gpus, precision=1, commas=False)
```
...
class InferenceLifecycleExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    queries_per_day_str = fmt(queries_per_day / MILLION, precision=1, commas=False, suffix=" million")
    kwh_per_query_str = fmt(kwh_per_query, precision=3, commas=False)
    daily_energy_kwh_str = fmt(daily_energy_kwh, precision=1)
    annual_energy_kwh_str = fmt(annual_energy_kwh, precision=1)
    grid_ci_kg_per_kwh_str = fmt(grid_ci_kg_per_kwh, precision=3, commas=False)
    annual_emissions_kg_str = fmt(annual_emissions_kg, precision=1)
...
class OnDeviceLearningEnergy:
    ...
    phone_battery_j_str = fmt(phone_battery_j, precision=1)
    battery_budget_pct_str = fmt(battery_budget_pct, precision=1, commas=False)
    budget_j_str = fmt(budget_j, precision=1)
    model_params_b_str = fmt(model_params / BILLION, precision=1, commas=False, suffix="B")
    forward_nj_per_param_str = fmt(forward_nj_per_param, precision=1, commas=False)
    backward_nj_per_param_str = fmt(backward_nj_per_param, precision=1, commas=False)
    train_nj_per_param_str = fmt(train_nj_per_param, precision=1, commas=False)
    joules_per_token_str = fmt(joules_per_token, precision=1, commas=False)
    token_capacity_str = fmt(token_capacity, precision=1)
```
...
class EnergyEfficiencyRange:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    efficiency_range_str = fmt(efficiency_range, precision=1, commas=False)
    cpu_pj_per_flop_str = fmt(cpu_pj_per_flop, precision=1, commas=False)
    gpu_pj_per_flop_str = fmt(gpu_pj_per_flop, precision=1, commas=False)
    tpu_pj_per_flop_str = fmt(tpu_pj_per_flop, precision=1, commas=False)
