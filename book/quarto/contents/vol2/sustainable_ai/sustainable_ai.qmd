...
class CarbonCostGPT3:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_mwh_str = fmt(energy_mwh, precision=0, suffix=' MW')
    energy_kwh_str = fmt(energy_kwh, precision=0, suffix=' kW')
...
**Problem**: A team trains a large model (GPT-3 size) consuming `{python} CarbonCostGPT3.training_mwh_str` h. How much CO~2~ is emitted, and how does that compare to a trans-Atlantic flight?

**Math**:

1.  **Energy**\index{Training!energy consumption}: `{python} CarbonCostGPT3.training_mwh_str` h = `{python} CarbonCostGPT3.energy_kwh_str` h.
...
class CarbonFrontier:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cf_energy_mwh_str = fmt(energy_mwh, precision=0, suffix=' MW')
...
class AutoPlacement:
    ...
    energy_mwh_str = fmt(energy_mwh, precision=0, suffix=' MW')
```
...
**Problem**: A team is planning a large training run requiring `{python} AutoPlacement.energy_mwh_str` h of energy, choosing between three regions with different electricity prices and carbon intensities. With an internal carbon tax of USD `{python} AutoPlacement.carbon_tax_str`/tonne, which region minimizes the true Total Cost of Ownership (TCO)?
...
**Problem**: A team is choosing a data center for a `{python} CarbonFrontier.cf_energy_mwh_str` h training run.
...
class LifecycleCarbonEstimate:
    ...
    facility_energy_kwh_str = fmt_int(round(total_facility_energy_kwh), commas=True, suffix=' kW')
...
class PueEfficiency:
    ...
    pue_savings_mwh_str = fmt(annual_savings_mwh, precision=1, suffix=' MW')
...
2. **Annual energy savings**: `{python} PueEfficiency.pue_it_mw_str` $\times$ `{python} PueEfficiency.pue_diff_str` $\times$ `{python} PueEfficiency.hours_per_year_str` h $\approx$ `{python} PueEfficiency.pue_savings_mwh_str` h.
3. **Financial Savings**\index{PUE!financial savings}: `{python} PueEfficiency.pue_savings_mwh_str` h $\times$ USD `{python} PueEfficiency.elec_price_per_mwh_str`/MWh $\approx$ USD `{python} PueEfficiency.pue_savings_usd_str`.
...
class TrainingEmissions:
    ...
    gpu_energy_kwh_str = fmt(gpu_energy_kwh, precision=1, suffix=' kW')
    facility_energy_kwh_str = fmt(facility_energy_kwh, precision=1, suffix=' kW')
...
- GPU energy: `{python} TrainingEmissions.n_gpus_str` $\times$ `{python} A100PowerScenario.a100_tdp_w_str` W $\times$ `{python} TrainingEmissions.training_hours_str` h = `{python} TrainingEmissions.gpu_energy_wh_str` = `{python} TrainingEmissions.gpu_energy_kwh_str` h

**Step 2**: Apply PUE.
- Facility PUE: `{python} TrainingEmissions.pue_str` (efficient hyperscale data center)
- Total facility energy: `{python} TrainingEmissions.gpu_energy_kwh_str` h $\times$ `{python} TrainingEmissions.pue_str` = `{python} TrainingEmissions.facility_energy_kwh_str` h

**Step 3**: Calculate emissions.
- Grid carbon intensity: `{python} TrainingEmissions.grid_ci_g_per_kwh_str` g CO~2~/kWh (US average)
- Operational emissions: `{python} TrainingEmissions.facility_energy_kwh_str` h $\times$ `{python} TrainingEmissions.grid_ci_g_per_kwh_str` g/kWh = `{python} TrainingEmissions.emissions_kg_str` kg CO~2~ = `{python} TrainingEmissions.emissions_tons_str` metric tons

**Comparison: Same training in low-carbon region**

- Quebec grid intensity: `{python} TrainingEmissions.quebec_ci_str` g CO~2~/kWh
- Emissions: `{python} TrainingEmissions.facility_energy_kwh_str` h $\times$ `{python} TrainingEmissions.quebec_ci_str` g/kWh = `{python} TrainingEmissions.quebec_emissions_kg_str` kg CO~2~
...
class EmbodiedCarbonAmort:
    ...
    op_energy_kwh_str = fmt(op_energy_kwh, precision=0, commas=False, suffix=' kW')
...
*   **Operational**: `{python} EmbodiedCarbonAmort.n_gpus_str` $\times$ `{python} EmbodiedCarbonAmort.h_h100_tdp_kw_str` $\times$ `{python} EmbodiedCarbonAmort.t_job_hr_str` h = `{python} EmbodiedCarbonAmort.op_energy_kwh_str` h. At `{python} EmbodiedCarbonAmort.ci_dirty_str` kg/kWh (US-average grid) = `{python} EmbodiedCarbonAmort.op_carbon_dirty_str` kg CO~2~.
...
class InferenceLifecycleExample:
    ...
    kwh_per_query_str = fmt(kwh_per_query, precision=3, commas=False, suffix=' kW')
    daily_energy_kwh_str = fmt(daily_energy_kwh, precision=0, suffix=' kW')
    annual_energy_kwh_str = fmt(annual_energy_kwh, precision=0, suffix=' kW')
...
Consider a model serving `{python} InferenceLifecycleExample.queries_per_day_str` queries per day at `{python} InferenceLifecycleExample.kwh_per_query_str` h per query. The annual inference energy and emissions break down as follows:

- Daily energy: `{python} InferenceLifecycleExample.queries_per_day_str` queries $\times$ `{python} InferenceLifecycleExample.kwh_per_query_str` h = `{python} InferenceLifecycleExample.daily_energy_kwh_str` h
- Annual energy: `{python} InferenceLifecycleExample.daily_energy_kwh_str` h $\times$ `{python} InferenceLifecycleExample.days_per_year_str` days = `{python} InferenceLifecycleExample.annual_energy_kwh_str` h
- Annual emissions (US grid): `{python} InferenceLifecycleExample.annual_energy_kwh_str` h $\times$ `{python} InferenceLifecycleExample.grid_ci_kg_per_kwh_str` kg/kWh = `{python} InferenceLifecycleExample.annual_emissions_kg_str` kg = `{python} InferenceLifecycleExample.annual_emissions_tonnes_str` metric tons CO~2~
...
- [ ] If a node has `{python} GridQueue.node_gpus_str` at `{python} GridQueue.tdp_str` each, why is the total node power budgeted at ~`{python} GridQueue.node_power_budget_kw_str` rather than `{python} GridQueue.node_gpu_only_kw_str`?
...
To appreciate the magnitude of these transients, consider a `{python} H100TdpRackRecap.cluster_gpus_str`-GPU cluster transitioning from communication phase (`{python} H100TdpRackRecap.comm_phase_w_str` W per GPU average) to matrix multiplication phase (`{python} GridQueue.tdp_str` per GPU). The power delta is `{python} H100TdpRackRecap.power_delta_w_str` W$\times$ `{python} H100TdpRackRecap.cluster_gpus_str` = `{python} H100TdpRackRecap.power_delta_kw_str`, and this transition occurs in approximately `{python} H100TdpRackRecap.transition_us_str` microseconds. The rate of power change is therefore `{python} H100TdpRackRecap.power_delta_kw_str`/`{python} H100TdpRackRecap.transition_us_str` $\mu$s = `{python} H100TdpRackRecap.power_rate_gws_str`. No passive electrical component can respond at this rate; only energy storage devices (supercapacitors) positioned physically close to the load can absorb the transient before it propagates into the building's electrical distribution.
