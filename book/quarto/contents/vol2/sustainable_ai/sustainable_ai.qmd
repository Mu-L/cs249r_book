...
**Embodied**: Assume manufacturing footprint is $\approx$ `{python} LifecycleCarbonEstimate.embodied_kg_per_gpu_str` kg $\text{CO}_2$ per H100 GPU, allocating NVIDIA's HGX H100 baseboard product carbon footprint evenly across its eight GPUs [@nvidia2025hgxh100pcf]. Amortized for 1 month of a 3-year cycle: (`{python} LifecycleCarbonEstimate.n_gpus_str` $\times$ `{python} LifecycleCarbonEstimate.embodied_kg_per_gpu_str` kg) / 36 months $\approx$ **`{python} LifecycleCarbonEstimate.embodied_tonnes_str` metric tons**.
...
For an accelerator with `{python} TrainingEmissions.embodied_kg_per_accelerator_str` kg embodied carbon, based on an even per-GPU allocation of NVIDIA's HGX H100 product carbon footprint [@nvidia2025hgxh100pcf], and a `{python} TrainingEmissions.embodied_lifetime_years_str`-year data center lifetime:
