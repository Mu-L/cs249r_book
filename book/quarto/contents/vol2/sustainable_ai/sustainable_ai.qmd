...
class CarbonCostGPT3:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_mwh_str = fmt(energy_mwh, precision=1, allow_zero=True)
    total_emissions_kg_str = fmt(total_emissions_kg, precision=1, allow_zero=True)
    flight_ratio_str = fmt(flight_ratio, precision=1, allow_zero=True)
    emissions_ratio_str = fmt(emissions_ratio, precision=1, allow_zero=True, commas=False)
...
1.  **Energy**\index{Training!energy consumption}: `{python} CarbonCostGPT3.training_mwh_str` MWh = 1,287,000 kWh.
2.  **Carbon intensity (US average)**: $\approx$ `{python} CarbonCostGPT3.grid_ci_us_kg_str` kg CO$_2$/kWh.
3.  **Total Emissions**\index{Carbon Footprint!training emissions}: $1,287,000 \times 0.429 \approx$ **`{python} CarbonCostGPT3.total_emissions_kg_str` kg CO$_2$**.
4.  **Comparison**:
...
**Systems insight**: A single training run emits as much carbon as hundreds of trans-Atlantic passenger round trips. **Optimization matters.** Moving this job to a hydro-powered region (0.02 kg/kWh) would reduce emissions by **`{python} CarbonCostGPT3.emissions_ratio_str`$\times$** to about 25 passenger round trips.
...
class ArchetypeATdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_w_str = fmt(tdp_w, precision=1, allow_zero=True, commas=False)
```
...
class CarbonFrontier:
    ...
    intensity_quebec = 20 # gCO2/kWh (Hydro) (kept raw for arithmetic)
    intensity_poland = 800 # gCO2/kWh (Coal) (kept raw for arithmetic)
    # ┌── 2. EXECUTE ───────────────────────────────────────
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cf_energy_mwh_str = fmt(energy_mwh, precision=1, allow_zero=True)
    cf_quebec_tonnes_str = fmt(quebec_tonnes, precision=1, allow_zero=True)
    cf_poland_tonnes_str = fmt(poland_tonnes, precision=1, allow_zero=True)
    cf_ratio_str = fmt(ratio, precision=1, allow_zero=True, commas=False)
    intensity_quebec_str = fmt(intensity_quebec, precision=1, allow_zero=True, commas=False)
    intensity_poland_str = fmt(intensity_poland, precision=1, allow_zero=True, commas=False)
...
class AutoPlacement:
    ...
    carbon_tax_str = fmt(carbon_tax, precision=1, allow_zero=True, commas=False)
    energy_mwh_str = fmt(energy_mwh, precision=1, allow_zero=True)
...
1. **Site A Emissions**: 10,000,000 kWh $\times$ 20 g = 200,000,000 g = **`{python} CarbonFrontier.cf_quebec_tonnes_str` tonnes $\text{CO}_2$**.
2. **Site B Emissions**: 10,000,000 kWh $\times$ 800 g = 8,000,000,000 g = **`{python} CarbonFrontier.cf_poland_tonnes_str` tonnes $\text{CO}_2$**.
...
class ComputeGrowthScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    compute_growth_factor_str = fmt(compute_growth_factor, precision=1, allow_zero=True)
...
class EnergyWallScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    energy_wall_gap_str = fmt(energy_wall_gap, precision=1, allow_zero=True)
    battery_gain_pct_str = fmt((battery_gain - 1) * 100, precision=1, allow_zero=True, commas=False)
    grid_gain_pct_str = fmt((grid_gain - 1) * 100, precision=1, allow_zero=True, commas=False)
```
...
While AI logic follows the "iron law" of software optimization, energy follows the laws of chemistry and thermodynamics. Over the same seven-year interval, battery energy density would improve by only ~`{python} EnergyWallScenario.battery_gain_pct_str` percent at a 5 percent annual rate, and grid efficiency by ~`{python} EnergyWallScenario.grid_gain_pct_str` percent at a 2 percent annual rate. The `{python} EnergyWallScenario.energy_wall_gap_str`$\times$ gap between these curves is the **Sustainability Wall**\index{Sustainability!wall}—the point where we can no longer "buy our way out" of the efficiency problem with more power.
...
By treating heat as a byproduct rather than a pollutant, the fleet moves toward a circular energy economy[^fn-pue-evolution] [@un2019circular].
...
class LifecycleCarbonEstimate:
    ...
    operational_tonnes = res.total_energy_kwh.magnitude * DummyRegion.carbon_intensity_g_kwh / 1e6
    embodied_tonnes = n_gpus * embodied_kg_per_gpu / amortization_months / 1000
    total_tonnes = operational_tonnes + embodied_tonnes

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(1.0 < operational_tonnes < 600, f"Unexpected operational carbon: {operational_tonnes:.1f} t")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_gpus_str = fmt(n_gpus, precision=1, allow_zero=True)
    training_days_str = fmt(training_days, precision=1, allow_zero=True)
    h_h100_tdp_w_str = fmt(h_h100.tdp.m_as("W"), precision=1, allow_zero=True, commas=False)
    pue_str = fmt(DummyFleet.effective_pue, precision=1, commas=False)
    grid_ci_str = fmt(DummyRegion.carbon_intensity_g_kwh, precision=1, allow_zero=True, commas=False)
    it_power_kw_str = fmt(n_gpus * h_h100.tdp.m_as("kW"), precision=1, allow_zero=True)
    facility_power_kw_str = fmt(n_gpus * h_h100.tdp.m_as("kW") * DummyFleet.effective_pue, precision=1, allow_zero=True)
    facility_energy_kwh_str = fmt(res.total_energy_kwh.magnitude / 1e6, precision=2, commas=False, suffix=" million")
    operational_tonnes_str = fmt(operational_tonnes, precision=1, allow_zero=True)
    embodied_kg_per_gpu_str = fmt(embodied_kg_per_gpu, precision=1, allow_zero=True, commas=False)
    embodied_tonnes_str = fmt(embodied_tonnes, precision=1, commas=False)
    total_tonnes_str = fmt(total_tonnes, precision=1, allow_zero=True)
```
...
**Operational**: Power = `{python} LifecycleCarbonEstimate.n_gpus_str` $\times$ 0.7 kW $\times$ `{python} LifecycleCarbonEstimate.pue_str` $\approx$ `{python} LifecycleCarbonEstimate.facility_power_kw_str` kW. Energy = `{python} LifecycleCarbonEstimate.facility_power_kw_str` kW $\times$ 24h $\times$ 30 $\approx$ `{python} LifecycleCarbonEstimate.facility_energy_kwh_str` kWh. Emissions $\approx$ **`{python} LifecycleCarbonEstimate.operational_tonnes_str` metric tons $\text{CO}_2$**.

**Embodied**: Assume manufacturing footprint is $\approx$ `{python} LifecycleCarbonEstimate.embodied_kg_per_gpu_str` kg $\text{CO}_2$ per H100 GPU (NVIDIA's product carbon footprint). Amortized for 1 month of a 3-year cycle: (`{python} LifecycleCarbonEstimate.n_gpus_str` $\times$ `{python} LifecycleCarbonEstimate.embodied_kg_per_gpu_str` kg) / 36 months $\approx$ **`{python} LifecycleCarbonEstimate.embodied_tonnes_str` metric tons**.
...
class GpuEmissionsScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_w_str = fmt(h_h100_tdp_w_val, precision=1, allow_zero=True, commas=False)
```

[^fn-embodied-carbon-hardware]: **Embodied Carbon**: The CO~2~ emitted during manufacturing, transport, and disposal before a device computes its first FLOP. A single H100 embodies 150–200 kg CO~2~ from fabrication alone; at `{python} GpuEmissionsScenario.h_h100_tdp_w_str` W on the average U.S. grid, continuous operation matches embodied carbon in roughly three to four weeks. As data centers shift to renewables, embodied carbon's share of total lifetime emissions grows, potentially exceeding 30 percent, making hardware refresh cycles a first-order sustainability decision. \index{Embodied Carbon!hardware lifecycle}
...
class PueEfficiency:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    pue_it_mw_str = fmt(p_it_mw, precision=1, commas=False)
    pue_old_str = fmt(pue_old, precision=2, commas=False)
    pue_new_str = fmt(pue_new, precision=2, commas=False)
    pue_savings_mwh_str = fmt(annual_savings_mwh, precision=1, allow_zero=True)
    pue_savings_usd_str = fmt(annual_savings_usd, precision=1, allow_zero=True)
...
1. **Overhead Reduction**\index{PUE!overhead reduction}: `{python} PueEfficiency.pue_old_str` - `{python} PueEfficiency.pue_new_str` = **0.48**.
2. **Annual energy savings**: $2.0 \text{ MW} \times 0.48 \times 8,760 \text{ hours} \approx$ **`{python} PueEfficiency.pue_savings_mwh_str` MWh**.
3. **Financial Savings**\index{PUE!financial savings}: `{python} PueEfficiency.pue_savings_mwh_str` MWh $\times$ \$70/MWh $\approx$ **\$`{python} PueEfficiency.pue_savings_usd_str`**.
...
For a GPU with $E_{\text{compute}} = 10$ pJ/FLOP and $E_{\text{move}} = 100$ pJ/byte (DRAM access):

$$\text{AI}_{\text{crossover}} = \frac{100 \text{ pJ/byte}}{10 \text{ pJ/FLOP}} = 10 \text{ FLOP/byte}$$
...
For small matrices ($N_{\text{mat}} = 60$):

- $\text{AI} = 60/6 = 10$ FLOP/byte (at crossover)
...
- $\text{AI} = 1000/6 = 167$ FLOP/byte (compute-bound)
...
- Bytes: $3N_{\text{vec}} \times 4 = 12,000$ bytes (read two vectors, write one)
- $\text{AI} = 1000/12000 = 0.083$ FLOP/byte (memory-bound)
...
class TrainingEmissions:
    ...
    embodied_daily_kg = embodied_kg_per_accelerator / (embodied_lifetime_years * 365)
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_hours_str = fmt(training_hours, precision=1, allow_zero=True)
    ...
    gpu_energy_wh_str = fmt(gpu_energy_wh, precision=1, allow_zero=True)
    gpu_energy_kwh_str = fmt(gpu_energy_kwh, precision=1, allow_zero=True)
    facility_energy_kwh_str = fmt(facility_energy_kwh, precision=1, allow_zero=True)
    emissions_kg_str = fmt(emissions_kg, precision=1, allow_zero=True)
    emissions_tons_str = fmt(emissions_tons, precision=1, commas=False)
    quebec_emissions_kg_str = fmt(quebec_emissions_kg, precision=1, allow_zero=True)
    emissions_ratio_str = fmt(emissions_ratio, precision=1, allow_zero=True, commas=False)
    a100_tdp_w_str = fmt(gpu_power_w, precision=1, allow_zero=True, commas=False)
    embodied_kg_per_accelerator_str = fmt(embodied_kg_per_accelerator, precision=1, allow_zero=True, commas=False)
    embodied_lifetime_years_str = fmt(embodied_lifetime_years, precision=1, allow_zero=True, commas=False)
    embodied_daily_kg_str = fmt(embodied_daily_kg, precision=3, commas=False)
    embodied_training_kg_str = fmt(embodied_training_kg, precision=1, allow_zero=True)
    embodied_us_ratio_pct_str = fmt(embodied_us_ratio_pct, precision=1, commas=False)
    embodied_quebec_total_share_pct_str = fmt(embodied_quebec_total_share_pct, precision=1, allow_zero=True, commas=False)
```
...
# │ How: pulling Hardware.Cloud.A100.tdp from mlsysim.core.constants.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.A100.tdp, watt)
...
class A100PowerScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    a100_tdp_w_str = fmt(a100_tdp_w_val, precision=1, allow_zero=True, commas=False)
```
...
- GPU energy: 64 GPUs times `{python} A100PowerScenario.a100_tdp_w_str` W times `{python} TrainingEmissions.training_hours_str`h = `{python} TrainingEmissions.gpu_energy_wh_str` Wh = `{python} TrainingEmissions.gpu_energy_kwh_str` kWh
...
class EmbodiedCarbonAmort:
    ...
    share_clean_pct_str = fmt(share_clean_pct, precision=1, allow_zero=True, commas=False)
    t_job_hr_str = fmt(t_job_hr, precision=1, allow_zero=True, commas=False)
    n_gpus_str = fmt(n_gpus, precision=1, allow_zero=True, commas=False)
```
...
# │ How: pulling Hardware.Cloud.H100.tdp from mlsysim.core.constants and converting to kW.
# │
# │ Imports: mlsysim.core.constants (Hardware.Cloud.H100.tdp, kilowatt)
# │ Exports: h_h100_tdp_kw
# └─────────────────────────────────────────────────────────────────────────────
from mlsysim.fmt import fmt
...
class H100KwScenario:
    ...
    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    h_h100_tdp_kw_val = h_h100_tdp.m_as(kilowatt)

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    from mlsysim.fmt import check
    check(abs(h_h100_tdp_kw_val - 0.7) < 1e-6, "H100 TDP should be 0.7 kW")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h_h100_tdp_kw_str = fmt(h_h100_tdp_kw_val, precision=1, commas=False)
```
...
**Scenario**: Training a model for **`{python} EmbodiedCarbonAmort.t_job_hr_str` hours** on **`{python} EmbodiedCarbonAmort.n_gpus_str` NVIDIA H100s**.

*   **Operational**: 8 GPUs$\times$ `{python} H100KwScenario.h_h100_tdp_kw_str` kW$\times$ 10h = 56 kWh. At 0.4 kg/kWh (gas grid) = **`{python} EmbodiedCarbonAmort.op_carbon_dirty_str` kg CO~2~**.
...
For an accelerator with `{python} TrainingEmissions.embodied_kg_per_accelerator_str` kg embodied carbon (per NVIDIA's product carbon footprint) and a `{python} TrainingEmissions.embodied_lifetime_years_str`-year data center lifetime:

The daily amortized embodied carbon is `{python} TrainingEmissions.embodied_kg_per_accelerator_str` kg / (`{python} TrainingEmissions.embodied_lifetime_years_str` $\times$ 365) $\approx$ `{python} TrainingEmissions.embodied_daily_kg_str` kg/day.

Over a `{python} TrainingEmissions.training_days_str`-day training run using `{python} TrainingEmissions.n_gpus_str` accelerators:

The job's amortized embodied carbon is `{python} TrainingEmissions.n_gpus_str` $\times$ `{python} TrainingEmissions.training_days_str` $\times$ `{python} TrainingEmissions.embodied_daily_kg_str` $\approx$ `{python} TrainingEmissions.embodied_training_kg_str` kg CO~2~.

The embodied contribution of `{python} TrainingEmissions.embodied_training_kg_str` kg represents approximately `{python} TrainingEmissions.embodied_us_ratio_pct_str` percent of the operational emissions (`{python} TrainingEmissions.emissions_kg_str` kg) calculated above for the US average grid. If training occurred in Quebec's low-carbon grid, where the same run produced `{python} TrainingEmissions.quebec_emissions_kg_str` kg of operational emissions, the embodied contribution would be about `{python} TrainingEmissions.embodied_quebec_total_share_pct_str` percent of total emissions.
...
class InferenceLifecycleExample:
    ...
    kwh_per_query = 0.001
    days_per_year = 365
    grid_ci_kg_per_kwh = 0.429
    training_emissions_kg = TrainingEmissions.emissions_kg
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    queries_per_day_str = fmt(queries_per_day / MILLION, precision=1, allow_zero=True, commas=False, suffix=" million")
    kwh_per_query_str = fmt(kwh_per_query, precision=3, commas=False)
    daily_energy_kwh_str = fmt(daily_energy_kwh, precision=1, allow_zero=True)
    annual_energy_kwh_str = fmt(annual_energy_kwh, precision=1, allow_zero=True)
    grid_ci_kg_per_kwh_str = fmt(grid_ci_kg_per_kwh, precision=3, commas=False)
    annual_emissions_kg_str = fmt(annual_emissions_kg, precision=1, allow_zero=True)
    annual_emissions_tonnes_str = fmt(annual_emissions_tonnes, precision=1, allow_zero=True)
    crossover_days_str = fmt(crossover_days, precision=1, commas=False)
```

For models deployed at scale, inference emissions often dominate the lifecycle. Consider a model serving `{python} InferenceLifecycleExample.queries_per_day_str` queries per day at `{python} InferenceLifecycleExample.kwh_per_query_str` kWh per query. The annual inference energy and emissions break down as follows:

- Daily energy: `{python} InferenceLifecycleExample.queries_per_day_str` times `{python} InferenceLifecycleExample.kwh_per_query_str` kWh = `{python} InferenceLifecycleExample.daily_energy_kwh_str` kWh
- Annual energy: `{python} InferenceLifecycleExample.daily_energy_kwh_str` times 365 = `{python} InferenceLifecycleExample.annual_energy_kwh_str` kWh
- Annual emissions (US grid): `{python} InferenceLifecycleExample.annual_energy_kwh_str` times `{python} InferenceLifecycleExample.grid_ci_kg_per_kwh_str` = `{python} InferenceLifecycleExample.annual_emissions_kg_str` kg = `{python} InferenceLifecycleExample.annual_emissions_tonnes_str` metric tons CO~2~

Compared with the 7B/64-A100 training example above (`{python} TrainingEmissions.emissions_tons_str` metric tons), cumulative inference emissions exceed training emissions after about `{python} InferenceLifecycleExample.crossover_days_str` days of deployment at this scale. For frontier-scale training runs, the crossover can shift to weeks or months depending on query volume, per-query energy, and grid intensity.
...
class OnDeviceLearningEnergy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    phone_battery_wh_str = fmt(phone_battery_wh, precision=1, commas=False)
    phone_battery_j_str = fmt(phone_battery_j, precision=1, allow_zero=True)
    battery_budget_pct_str = fmt(battery_budget_pct, precision=1, allow_zero=True, commas=False)
    budget_j_str = fmt(budget_j, precision=1, allow_zero=True)
    model_params_b_str = fmt(model_params / BILLION, precision=1, allow_zero=True, commas=False, suffix="B")
    forward_nj_per_param_str = fmt(forward_nj_per_param, precision=1, allow_zero=True, commas=False)
    backward_nj_per_param_str = fmt(backward_nj_per_param, precision=1, allow_zero=True, commas=False)
    train_nj_per_param_str = fmt(train_nj_per_param, precision=1, allow_zero=True, commas=False)
    joules_per_token_str = fmt(joules_per_token, precision=1, allow_zero=True, commas=False)
    token_capacity_str = fmt(token_capacity, precision=1, allow_zero=True)
```

::: {#nbk-sustainable-ai-energy-learning .callout-notebook title="The energy of learning"}

**Problem**: Consider fine-tuning a small language model (`{python} OnDeviceLearningEnergy.model_params_b_str` parameters) on a user's smartphone overnight. Is this feasible within a **`{python} OnDeviceLearningEnergy.battery_budget_pct_str` percent battery budget**?

**Math**:

1.  **Phone Battery**\index{On-Device Learning!battery budget}: Typical capacity is approximately `{python} OnDeviceLearningEnergy.phone_battery_wh_str` Wh (Watt-hours), or about `{python} OnDeviceLearningEnergy.phone_battery_j_str` Joules.
2.  **Budget**: `{python} OnDeviceLearningEnergy.battery_budget_pct_str` percent of `{python} OnDeviceLearningEnergy.phone_battery_j_str` J = **`{python} OnDeviceLearningEnergy.budget_j_str` Joules**.
...
4.  **Capacity**: `{python} OnDeviceLearningEnergy.budget_j_str` J / `{python} OnDeviceLearningEnergy.joules_per_token_str` J/token = **`{python} OnDeviceLearningEnergy.token_capacity_str` tokens**.
...
class WakeWordPower:
    ...
    reduction_str = fmt(reduction, precision=1, allow_zero=True, commas=False)
...
class EnergyEfficiencyRange:
    ...
    # ┌── 4. OUTPUT (Formatting) ─────────────────────────────────────────────
    efficiency_range_str = fmt(efficiency_range, precision=1, allow_zero=True, commas=False)
    cpu_pj_per_flop_str = fmt(cpu_pj_per_flop, precision=1, allow_zero=True, commas=False)
    gpu_pj_per_flop_str = fmt(gpu_pj_per_flop, precision=1, allow_zero=True, commas=False)
    tpu_pj_per_flop_str = fmt(tpu_pj_per_flop, precision=1, allow_zero=True, commas=False)
