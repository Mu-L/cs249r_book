...
class CarbonCostGPT3:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_mwh_str = fmt(energy_mwh, precision=0)
    total_emissions_kg_str = fmt(total_emissions_kg, precision=0)
    flight_ratio_str = fmt(flight_ratio, precision=0)
    emissions_ratio_str = fmt(emissions_ratio, precision=0, commas=False)
...
class ArchetypeATdp:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tdp_w_str = fmt(tdp_w, precision=0, commas=False)
```
...
class CarbonFrontier:
    ...
    # ┌── 4. OUTPUT ────────────────────────────────────────
    cf_energy_mwh_str = fmt(energy_mwh, precision=0)
...
class EnergyWallScenario:
    ...
    battery_gain_pct_str = fmt((battery_gain - 1) * 100, precision=0, commas=False)
    grid_gain_pct_str = fmt((grid_gain - 1) * 100, precision=0, commas=False)
```
...
class LifecycleCarbonEstimate:
    ...
    pue = 1.3
    grid_ci_g_per_kwh = 400
    embodied_kg_per_gpu = 164
    amortization_months = 36

    # ┌── 2. EXECUTE (The Compute) ────────────────────────────────────────
    training_hours = training_days * hours_per_day
    it_power_kw = n_gpus * h100_tdp_kw
    facility_power_kw = it_power_kw * pue
    facility_energy_kwh = facility_power_kw * training_hours
    operational_tonnes = facility_energy_kwh * grid_ci_g_per_kwh / MILLION
    embodied_tonnes = n_gpus * embodied_kg_per_gpu / amortization_months / THOUSAND
    total_tonnes = operational_tonnes + embodied_tonnes

    # ┌── 3. GUARD (Invariants) ──────────────────────────────────────────
    check(530 < operational_tonnes < 540, f"Unexpected operational carbon: {operational_tonnes:.1f} t")
    check(540 < total_tonnes < 550, f"Unexpected total carbon: {total_tonnes:.1f} t")

    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_gpus_str = fmt(n_gpus, precision=0)
    training_days_str = fmt(training_days, precision=0)
    h100_tdp_w_str = fmt(h100_tdp_kw * THOUSAND, precision=0, commas=False)
    pue_str = fmt(pue, precision=1, commas=False)
    grid_ci_str = fmt(grid_ci_g_per_kwh, precision=0, commas=False)
    it_power_kw_str = fmt(it_power_kw, precision=0)
    facility_power_kw_str = fmt(facility_power_kw, precision=0)
    facility_energy_kwh_str = fmt(facility_energy_kwh / MILLION, precision=2, commas=False, suffix=" million")
    operational_tonnes_str = fmt(operational_tonnes, precision=0)
    ...
    total_tonnes_str = fmt(total_tonnes, precision=0)
```
...
**Parameters**: `{python} LifecycleCarbonEstimate.n_gpus_str` H100 GPUs, `{python} LifecycleCarbonEstimate.training_days_str` days, `{python} LifecycleCarbonEstimate.h100_tdp_w_str` W TDP, PUE `{python} LifecycleCarbonEstimate.pue_str`, grid intensity `{python} LifecycleCarbonEstimate.grid_ci_str` g $\text{CO}_2$/kWh.
...
class GpuEmissionsScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tdp_w_str = fmt(h100_tdp_w_val, precision=0, commas=False)
```

[^fn-embodied-carbon-hardware]: **Embodied Carbon**: The CO~2~ emitted during manufacturing, transport, and disposal before a device computes its first FLOP. A single H100 embodies 150–200 kg CO~2~ from fabrication alone; at `{python} GpuEmissionsScenario.h100_tdp_w_str` W on the average U.S. grid, continuous operation matches embodied carbon in roughly three to four weeks. As data centers shift to renewables, embodied carbon's share of total lifetime emissions grows, potentially exceeding 30 percent, making hardware refresh cycles a first-order sustainability decision. \index{Embodied Carbon!hardware lifecycle}
...
class PueEfficiency:
    ...
    pue_savings_mwh_str = fmt(annual_savings_mwh, precision=0)
...
class TrainingEmissions:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    training_hours_str = fmt(training_hours, precision=0)
    training_days_str = fmt(training_days, precision=0, commas=False)
    n_gpus_str = fmt(n_gpus, precision=0, commas=False)
    gpu_energy_wh_str = fmt(gpu_energy_wh, precision=0)
    gpu_energy_kwh_str = fmt(gpu_energy_kwh, precision=0)
    facility_energy_kwh_str = fmt(facility_energy_kwh, precision=0)
    emissions_kg_str = fmt(emissions_kg, precision=0)
    emissions_tons_str = fmt(emissions_tons, precision=1, commas=False)
    quebec_emissions_kg_str = fmt(quebec_emissions_kg, precision=0)
    emissions_ratio_str = fmt(emissions_ratio, precision=0, commas=False)
    ...
    embodied_training_kg_str = fmt(embodied_training_kg, precision=0)
    embodied_us_ratio_pct_str = fmt(embodied_us_ratio_pct, precision=1, commas=False)
    embodied_quebec_total_share_pct_str = fmt(embodied_quebec_total_share_pct, precision=0, commas=False)
```
...
# │
# │ Imports: mlsysim.fmt (fmt, check — module-wide)
...
class EmbodiedCarbonAmort:
    ...
    share_clean_pct_str = fmt(share_clean_pct, precision=0, commas=False)
    t_job_hr_str = fmt(t_job_hr, precision=0, commas=False)
    n_gpus_str = fmt(n_gpus, precision=0, commas=False)
```
...
class H100KwScenario:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    h100_tdp_kw_str = fmt(h100_tdp_kw_val, precision=1, commas=False)
```
...
*   **Operational**: 8 GPUs$\times$ `{python} H100KwScenario.h100_tdp_kw_str` kW$\times$ 10h = 56 kWh. At 0.4 kg/kWh (gas grid) = **`{python} EmbodiedCarbonAmort.op_carbon_dirty_str` kg CO~2~**.
...
class InferenceLifecycleExample:
    ...
    daily_energy_kwh_str = fmt(daily_energy_kwh, precision=0)
    annual_energy_kwh_str = fmt(annual_energy_kwh, precision=0)
    ...
    annual_emissions_tonnes_str = fmt(annual_emissions_tonnes, precision=0)
...
class OnDeviceLearningEnergy:
    ...
    battery_budget_pct_str = fmt(battery_budget_pct, precision=0, commas=False)
    ...
    token_capacity_str = fmt(token_capacity, precision=0)
```
...
class WakeWordPower:
    ...
    reduction_str = fmt(reduction, precision=0, commas=False)
