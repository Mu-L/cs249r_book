...
class LifecycleCarbonEstimate:
    ...
    facility_power_kw_str = fmt(facility_power_kw, precision=1)
    facility_energy_kwh_str = fmt_int(round(total_facility_energy_kwh), commas=True)
...
2. **Annual energy savings**: `{python} PueEfficiency.pue_it_mw_str` MW $\times$ `{python} PueEfficiency.pue_diff_str` $\times$ `{python} PueEfficiency.hours_per_year_str` h $\approx$ **`{python} PueEfficiency.pue_savings_mwh_str` MWh**.
...
class TrainingEmissions:
    ...
    gpu_energy_wh_str = fmt(gpu_energy.to(ureg.watt_hour).magnitude, precision=0)
...
- Training time: `{python} TrainingEmissions.training_days_str` days $\times$ `{python} TrainingEmissions.hours_per_day_str` h/day = **`{python} TrainingEmissions.training_hours_str` hours**
- GPU energy: `{python} TrainingEmissions.n_gpus_str` $\times$ `{python} A100PowerScenario.a100_tdp_w_str` W $\times$ `{python} TrainingEmissions.training_hours_str` h = **`{python} TrainingEmissions.gpu_energy_wh_str` Wh** = **`{python} TrainingEmissions.gpu_energy_kwh_str` kWh**
...
*   **Operational**: `{python} EmbodiedCarbonAmort.n_gpus_str` $\times$ `{python} EmbodiedCarbonAmort.h_h100_tdp_kw_str` kW $\times$ `{python} EmbodiedCarbonAmort.t_job_hr_str` h = **`{python} EmbodiedCarbonAmort.op_energy_kwh_str` kWh**. At `{python} EmbodiedCarbonAmort.ci_dirty_str` kg/kWh (US-average grid) = **`{python} EmbodiedCarbonAmort.op_carbon_dirty_str` kg CO~2~**.
...
- Daily energy: `{python} InferenceLifecycleExample.queries_per_day_str` queries $\times$ `{python} InferenceLifecycleExample.kwh_per_query_str` kWh = `{python} InferenceLifecycleExample.daily_energy_kwh_str` kWh
- Annual energy: `{python} InferenceLifecycleExample.daily_energy_kwh_str` kWh $\times$ `{python} InferenceLifecycleExample.days_per_year_str` days = **`{python} InferenceLifecycleExample.annual_energy_kwh_str` kWh**
