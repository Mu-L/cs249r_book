...
::: {#def-sustainable-ai-sustainable-ai .callout-definition title="Sustainable AI"}
...
class LifecycleCarbonEstimate:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    n_gpus_str = f"{n_gpus:,}"
    training_days_str = f"{training_days:,}"
    h100_tdp_w_str = f"{h100_tdp_kw * THOUSAND:.0f}"
    pue_str = f"{pue:.1f}"
    grid_ci_str = f"{grid_ci_g_per_kwh:.0f}"
    it_power_kw_str = f"{it_power_kw:,.0f}"
    facility_power_kw_str = f"{facility_power_kw:,.0f}"
    facility_energy_kwh_str = f"{facility_energy_kwh / MILLION:.2f} million"
    operational_tonnes_str = f"{operational_tonnes:,.0f}"
    embodied_kg_per_gpu_str = f"{embodied_kg_per_gpu:.0f}"
    embodied_tonnes_str = f"{embodied_tonnes:.1f}"
    total_tonnes_str = f"{total_tonnes:,.0f}"
```
...
::: {#exm-sustainable-ai-matmul-energy-analysis .callout-example title="MatMul energy analysis"}
...
class TrainingEmissions:
    ...
    embodied_kg_per_accelerator_str = f"{embodied_kg_per_accelerator:.0f}"
    embodied_lifetime_years_str = f"{embodied_lifetime_years:.0f}"
    embodied_daily_kg_str = f"{embodied_daily_kg:.3f}"
    embodied_training_kg_str = f"{embodied_training_kg:.0f}"
    embodied_us_ratio_pct_str = f"{embodied_us_ratio_pct:.1f}"
    embodied_quebec_total_share_pct_str = f"{embodied_quebec_total_share_pct:.0f}"
```
...
::: {#exm-sustainable-ai-training-emissions-calculation .callout-example title="Training emissions calculation"}
...
# │
# │ Imports: (none)
# │ Exports: n_gpus, t_job_hr, op_energy_kwh_str, op_carbon_dirty_str,
# │          op_carbon_clean_str, total_mfg_carbon_str,
# │          total_lifetime_hr_str, hourly_rent_kg_str,
# │          embodied_carbon_job_str, share_clean_pct_str
# └─────────────────────────────────────────────────────────────────────────────
...
class EmbodiedCarbonAmort:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    op_energy_kwh_str = f"{op_energy_kwh:.0f}"
    op_carbon_dirty_str = f"{op_carbon_dirty:.1f}"
    op_carbon_clean_str = f"{op_carbon_clean:.1f}"
    total_mfg_carbon_str = f"{total_mfg_carbon:,.0f}"
    total_lifetime_hr_str = f"{total_lifetime_hr:,.0f}"
    hourly_rent_kg_str = f"{hourly_rent_kg:.3f}"
...
*   **Operational**: `{python} EmbodiedCarbonAmort.n_gpus` GPUs $\times$ `{python} H100KwScenario.h100_tdp_kw` kW $\times$ `{python} EmbodiedCarbonAmort.t_job_hr` h = `{python} EmbodiedCarbonAmort.op_energy_kwh_str` kWh. At 0.4 kg/kWh (gas grid) = **`{python} EmbodiedCarbonAmort.op_carbon_dirty_str` kg CO~2~**.
*   **Embodied**: `{python} EmbodiedCarbonAmort.n_gpus` GPUs $\times$ 164 kg/GPU = `{python} EmbodiedCarbonAmort.total_mfg_carbon_str` kg total.
*   **Amortization**: Lifetime = 3 years (`{python} EmbodiedCarbonAmort.total_lifetime_hr_str` hours).
    ...
    *   Job Cost = `{python} EmbodiedCarbonAmort.hourly_rent_kg_str` $\times$ `{python} EmbodiedCarbonAmort.t_job_hr` = **`{python} EmbodiedCarbonAmort.embodied_carbon_job_str` kg CO~2~**.
...
class InferenceLifecycleExample:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    queries_per_day_str = f"{queries_per_day / MILLION:.0f} million"
    kwh_per_query_str = f"{kwh_per_query:.3f}"
    daily_energy_kwh_str = f"{daily_energy_kwh:,.0f}"
    annual_energy_kwh_str = f"{annual_energy_kwh:,.0f}"
    grid_ci_kg_per_kwh_str = f"{grid_ci_kg_per_kwh:.3f}"
    annual_emissions_kg_str = f"{annual_emissions_kg:,.0f}"
    annual_emissions_tonnes_str = f"{annual_emissions_tonnes:,.0f}"
    crossover_days_str = f"{crossover_days:.1f}"
```
...
::: {#exm-sustainable-ai-battery-life-tinyml .callout-example title="Battery life for TinyML"}
...
class OnDeviceLearningEnergy:
    ...
    # ┌── 4. OUTPUT (Formatting) ──────────────────────────────────────────────
    phone_battery_wh_str = f"{phone_battery_wh:.1f}"
    phone_battery_j_str = f"{phone_battery_j:,.0f}"
    battery_budget_pct_str = f"{battery_budget_pct:.0f}"
    budget_j_str = f"{budget_j:,.0f}"
    model_params_b_str = f"{model_params / 1_000_000_000:.0f}B"
    forward_nj_per_param_str = f"{forward_nj_per_param:.0f}"
    backward_nj_per_param_str = f"{backward_nj_per_param:.0f}"
    train_nj_per_param_str = f"{train_nj_per_param:.0f}"
    joules_per_token_str = f"{joules_per_token:.0f}"
    token_capacity_str = f"{token_capacity:,.0f}"
```
