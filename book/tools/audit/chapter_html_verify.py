...
def _prose_exec(qmd: Path) -> tuple[bool, str]:
    env = {**os.environ, "PYTHONPATH": str(REPO_ROOT / "mlsysim")}
    proc = subprocess.run(
        [sys.executable, "-W", "ignore::UserWarning", str(REPO_ROOT / "book/tools/audit/fmt/audit_prose.py"), str(qmd)],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
