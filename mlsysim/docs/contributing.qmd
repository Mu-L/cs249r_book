...
| Add a grid or rack profile | Intermediate | Medium -- new infra | `mlsysim/infra/` |
...
### Step 1: Register the node in `hardware/registry.py`

Chip and board specs live in the **Silicon Zoo** — not in `core/constants.py`
(that file is physics/units only). Define pint quantities inline in the registry entry
(or as local `_A100_*` helpers at the top of `registry.py` if shared across variants).

```python
# In mlsysim/hardware/registry.py
_A100_PEAK_FP16 = Q_(312, "TFLOP/s")   # Tensor Core, sparsity OFF — SXM4 datasheet

A100 = HardwareNode(
    name="NVIDIA A100",
    release_year=2020,
    compute=ComputeCore(
        peak_flops=_A100_PEAK_FP16,
        precision_flops={
            "fp32": Q_(19.5, "TFLOP/s"),
            "tf32": Q_(156, "TFLOP/s"),
            "int8": Q_(624, "TOPS"),
        },
    ),
    ...
    metadata={
        "provenance": Provenance(
            ...
            url="https://...",           # REQUIRED for datasheet kind
            verified="2025-03-06",
        ),
    },
)
...
### Step 2: Verify with registry parity tests

If you replace a symbol that used to live in `constants.py`, add a row to
`mlsysim/tests/test_registry_parity.py` before deleting the flat constant.

### Step 3: Use the canonical path in book LEGO cells

Chapters import `Hardware.Cloud.A100`, not `A100_FLOPS_FP16_TENSOR`. Run
`python3 book/tools/audit/book_check_registry_sources.py` on touched QMD files.
...
| `check(q, label=...)` | Assert dimensional sanity in tutorials |
...
Llama3_8B = TransformerWorkload(
    ...
    parameters=8e9 * param,
    layers=32,
    hidden_dim=4096,
    heads=32,
    kv_heads=8,
    inference_flops=2 * 8e9 * flop,
)
```
...
Iceland = GridProfile(
    ...
    metadata=Metadata(
        provenance=Provenance(
            ...
            ref="National Energy Authority grid mix report",
            url="https://...",
            verified="2025-06-01",
        ),
    ),
...
mlsysim/
  core/
    constants.py    # Physics, units, and cross-cutting laws only (CI allowlist)
    calibration.py  # Solver/engine default parameters
    literature/     # Cited appendix scalars (Literature.*)
    ops/              # MLOps thresholds (Ops.Monitoring.*)
    infra/pricing.py  # Cloud, storage, labeling, fleet economics
    solver.py       # Analytical solvers (BaseSolver subclasses)
  ...
  hardware/registry.py   # Hardware.* zoo
  models/registry.py     # Models.* zoo
  datasets/registry.py   # Datasets.* zoo
  systems/registry.py    # Systems.Nodes / Fabrics / Clusters / Pods
  infra/registry.py      # Infrastructure.* zoo
...
Registry migration build check (book repo):
