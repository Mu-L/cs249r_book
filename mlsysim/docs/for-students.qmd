...
Whether you are taking your first ML systems course or preparing for industry interviews, MLSYSIM lets you experiment with real hardware specifications and see why systems behave the way they do. Hardware registry specs come from datasheets where available; outputs are derived from documented equations.
...
See the [Getting Started](getting-started.qmd) guide for development installs. Local install is the supported path; Colab works by running `pip install mlsysim` in the first cell, but one-click Colab/Binder launch buttons are not available yet.
...
| TrainingMemoryModel | Training memory breakdown | Training | [Vol I, Ch 8](https://github.com/harvard-edge/cs249r_book/releases/download/slides-latest/vol1_08_training.pdf){target="_blank"} |
| ServingCapacityModel | Replica count from QPS and P99 target | Inference at Scale | [Vol II, Ch 10](https://github.com/harvard-edge/cs249r_book/releases/download/slides-latest/vol2_10_inference.pdf){target="_blank"} |
...
| MoERoutingModel | Expert routing imbalance and all-to-all traffic | Distributed Training | [Vol II, Ch 5](https://github.com/harvard-edge/cs249r_book/releases/download/slides-latest/vol2_05_distributed_training.pdf){target="_blank"} |
...
Not using the textbook? No problem -- MLSYSIM is self-contained. The [Math Foundations](math.qmd) page documents the core equations, and each slide deck stands on its own with full speaker notes.
...
3. **Model** the setup using MLSYSIM to test your prediction against real hardware specs
