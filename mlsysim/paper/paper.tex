...
The \textbf{Hardware} registry records \texttt{metadata.provenance} for release-facing hardware entries where primary vendor sources are available, with optional \texttt{metadata.last\_verified} dates. This keeps datasheet-backed constants auditable while allowing clearly marked estimates for emerging or difficult-to-source hardware.
...
\item \textbf{Specification separation.} The physics-and-units module (\texttt{constants.py}) is budgeted at 220 source lines. Any symbol matching a hardware or model name pattern is rejected, enforcing the principle that device-specific values belong in typed registries, not in a monolithic constants file.
