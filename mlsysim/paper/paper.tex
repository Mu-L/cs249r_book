...
\mlsysim is designed for three audiences: students developing quantitative reasoning skills, instructors preparing demonstrations, and researchers evaluating design trade-offs. We present two representative use cases per persona, each illustrating how solvers compose to answer questions that span multiple walls. For concrete code examples of how users define hardware and invoke these solvers programmatically in Python, see the Appendix (Listings~\ref{lst:speculative},~\ref{lst:appendix-sweep},~\ref{lst:appendix-compose},~\ref{lst:appendix-data}, and~\ref{lst:appendix-synthesis}).
...
To demonstrate how the theoretical 3-tier architecture from \Cref{sec:solver-formalism} is exposed in Python, we provide four complete, runnable examples. These listings illustrate how users can leverage \mlsysim for design-space sweeps, cross-domain composition, data pipeline analysis, and SLA-driven hardware synthesis.
