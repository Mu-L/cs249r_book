...
\begin{frame}[shrink]{Beyond FLOPs: The Data Wall}
...
\begin{frame}[fragile,shrink=10]{Live Demo: Uncovering the Data Wall (Scenario C)}
...
\begin{frame}[shrink]{Wall 4: Prefill vs Decode --- Two Different Physics}
...
\begin{frame}[shrink]{Wall 5: The KV Cache --- Hidden Memory Consumer}
...
\begin{frame}[fragile,shrink=10]{Live Demo: Two-Phase Serving Analysis}
...
\begin{frame}[shrink]{Continuous Batching: Don't Wait, Serve}
...
\begin{frame}[fragile,shrink=10]{PagedAttention: Virtual Memory for KV Cache}
...
\begin{frame}[shrink]{Predict: How Many Concurrent Requests?}
...
\begin{frame}[fragile,shrink=10]{Speculative Decoding: Betting on the Draft}
...
\begin{frame}[fragile,shrink=10]{Disaggregated Serving: Right Hardware for Each Phase}
...
\begin{frame}[fragile,shrink=10]{Exercise 2: Serving Capacity Planning}
...
\begin{frame}[shrink]{Fallacies: Serving Edition}
...
\begin{frame}[shrink]{Part 2: Key Takeaway}
...
\begin{frame}[shrink]{Wall 13: The Fidelity Wall}
...
\begin{frame}[shrink]{Quantization: Trading Bits for Speed}
...
\begin{tikzpicture}[scale=0.8, >=Stealth]
  \foreach \i/\label/\bits/\color in {
    ...
    \node[right, font=\scriptsize] at ({0.15*\bits + 0.2}, 3.3-\i) {\label};
  }
...
\begin{frame}[fragile,shrink=10]{Live Demo: Quantization Impact}
...
\begin{frame}[fragile,shrink=10]{Pruning: The Structure Matters}
...
\begin{frame}[shrink]{Predict: What Does INT4 Change?}
...
\begin{frame}[shrink]{Predict: INT4 Llama-3 8B on H100}
...
\begin{frame}[fragile,shrink=10]{Exercise 3: Compression Tradeoffs}
...
\begin{frame}[shrink]{Compression Changes Fleet Architecture}
...
\begin{frame}[shrink]{Part 3: Key Takeaway}
...
% --- Roadmap: After Lunch ---
\begin{frame}[shrink]{Roadmap: Afternoon Session}
